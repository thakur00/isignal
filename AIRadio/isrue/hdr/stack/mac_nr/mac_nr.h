...
#ifndef ISRUE_MAC_NR_H
#define ISRUE_MAC_NR_H
...
#include "isrue/hdr/stack/mac_common/mac_common.h"
#include "isrue/hdr/stack/mac_nr/demux_nr.h"
#include "isrue/hdr/stack/mac_nr/mux_nr.h"
#include "isrue/hdr/stack/ue_stack_base.h"
...
{
public:
  mac_nr(isrran::ext_task_sched_handle task_sched_);
  ~mac_nr();
  ...
  void bch_decoded_ok(uint32_t tti, isrran::unique_byte_buffer_t payload);
  ...
  void reset();
  int  setup_lcid(const isrran::logical_channel_config_t& config);
  int  set_config(const isrran::bsr_cfg_nr_t& bsr_cfg);
  int  set_config(const isrran::sr_cfg_nr_t& sr_cfg);
  void set_config(const isrran::rach_cfg_nr_t& rach_cfg_nr);
  int  set_config(const isrran::dl_harq_cfg_nr_t& dl_hrq_cfg);
  void set_contention_id(const uint64_t ue_identity);
  bool set_crnti(const uint16_t crnti);
  int  add_tag_config(const isrran::tag_cfg_nr_t& tag_cfg);
  int  set_config(const isrran::phr_cfg_nr_t& phr_cfg);
  ...
  uint16_t get_csrnti() { return ISRRAN_INVALID_RNTI; }; // SPS not supported
  ...
  /// Interface for MUX
  isrran::mac_sch_subpdu_nr::lcg_bsr_t generate_sbsr();
...
private:
  ...
  void handle_pdu(isrran::unique_byte_buffer_t pdu);
  void get_ul_data(const mac_nr_grant_ul_t& grant, isrran::byte_buffer_t* tx_pdu);
  ...
  std::vector<isrran::logical_channel_config_t> logical_channels; // stores the raw configs provide by upper layers
  ...
  std::array<mac_metrics_t, ISRRAN_MAX_CARRIERS> metrics = {};

  isrran::task_multiqueue::queue_handle stack_task_dispatch_queue;
