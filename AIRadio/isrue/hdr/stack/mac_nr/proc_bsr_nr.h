...
#ifndef ISRUE_PROC_BSR_NR_H
#define ISRUE_PROC_BSR_NR_H
...
public:
  ...
  virtual isrran::mac_sch_subpdu_nr::lcg_bsr_t generate_sbsr() = 0;
};
...
public:
  ...
  virtual void generate_bsr_mac_ce(const isrran::bsr_format_nr_t& format) = 0;
};
 ...
 */
class proc_bsr_nr : public isrran::timer_callback, public bsr_interface_mux_nr
{
public:
  explicit proc_bsr_nr(isrlog::basic_logger& logger_) : logger(logger_) {}
...
private:
  ...
  isrran::ext_task_sched_handle* task_sched = nullptr;
  ...
  uint8_t buff_size_bytes_to_field(uint32_t buffer_size, isrran::bsr_format_nr_t format);

  uint32_t find_max_priority_lcg_with_data();

  isrran::timer_handler::unique_timer timer_periodic;
  isrran::timer_handler::unique_timer timer_retx;
  isrran::timer_handler::unique_timer timer_queue_status_print;
};

} // namespace isrue

#endif // ISRUE_PROC_BSR_NR_H
