...
#ifndef ISRUE_MUX_NR_H
#define ISRUE_MUX_NR_H
...
{
public:
  explicit mux_nr(mac_interface_mux_nr& mac_, isrlog::basic_logger& logger);
  ~mux_nr(){};
  void    reset();
  ...
  isrran::unique_byte_buffer_t get_pdu(uint32_t max_pdu_len);

  // Interface for BSR procedure
  void generate_bsr_mac_ce(const isrran::bsr_format_nr_t& format);
...
private:
  ...
  // Msg3 related
  isrran::unique_byte_buffer_t msg3_buff = nullptr;
  ...
  isrran::unique_byte_buffer_t rlc_buff = nullptr;

  isrran::mac_sch_pdu_nr tx_pdu; /// single MAC PDU for packing
