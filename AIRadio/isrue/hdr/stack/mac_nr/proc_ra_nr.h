...
#ifndef ISRUE_PROC_RA_NR_H
#define ISRUE_PROC_RA_NR_H
...
{
public:
  proc_ra_nr(mac_interface_proc_ra_nr& mac_, isrlog::basic_logger& logger_);
  ~proc_ra_nr(){};

  void init(phy_interface_mac_nr* phy_h_, isrran::ext_task_sched_handle* task_sched_);
...
private:
  ...
  uint16_t   rar_rnti   = ISRRAN_INVALID_RNTI;
  std::mutex mutex;

  isrran::rach_cfg_nr_t rach_cfg   = {};
  ...
  isrran::timer_handler::unique_timer prach_send_timer;
  isrran::timer_handler::unique_timer rar_timeout_timer;
  isrran::timer_handler::unique_timer contention_resolution_timer;
  isrran::timer_handler::unique_timer backoff_timer;
