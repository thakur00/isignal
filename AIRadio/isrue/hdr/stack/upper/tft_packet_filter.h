...
#ifndef ISRUE_PACKET_FILTER_H
#define ISRUE_PACKET_FILTER_H
...
public:
  ...
  bool match(const isrran::unique_byte_buffer_t& pdu);
  ...
  bool match_ip(const isrran::unique_byte_buffer_t& pdu);
  bool match_protocol(const isrran::unique_byte_buffer_t& pdu);
  bool match_type_of_service(const isrran::unique_byte_buffer_t& pdu);
  bool match_flow_label(const isrran::unique_byte_buffer_t& pdu);
  bool match_port(const isrran::unique_byte_buffer_t& pdu);
};
...
{
public:
  explicit tft_pdu_matcher(isrlog::basic_logger& logger) : logger(logger) {}
  ~tft_pdu_matcher(){};

  void reset();

  int     check_tft_filter_match(const isrran::unique_byte_buffer_t& pdu, uint8_t& eps_bearer_id);
...
#endif // ISRUE_TFT_PACKET_FILTER_H
