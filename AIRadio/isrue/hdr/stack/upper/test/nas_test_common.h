...
#ifndef ISRUE_NAS_TEST_COMMON
#define ISRUE_NAS_TEST_COMMON
...
public:
  ...
  void        write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t sdu) override {}
...
public:
  ...
  void write_sdu(unique_byte_buffer_t sdu)
  {
    ...
    // isrran_vec_fprint_byte(stdout, sdu->msg, sdu->N_bytes);
  }
  ...
  bool connection_request(isrran::establishment_cause_t cause, isrran::unique_byte_buffer_t sdu)
  {
    printf("NAS generated SDU (len=%d):\n", sdu->N_bytes);
    last_sdu_len = sdu->N_bytes;
    isrran_vec_fprint_byte(stdout, sdu->msg, sdu->N_bytes);
...
public:
  ...
  void init(isrue::nas_5g* nas_5g_) { nas_5g_ptr = nas_5g_; }
  ...
  virtual int  connection_request(isrran::nr_establishment_cause_t cause, isrran::unique_byte_buffer_t sdu)
  {
    return ISRRAN_SUCCESS;
  }
...
private:
  isrue::nas_5g*                  nas_5g_ptr;
  uint32_t                        last_sdu_len;
  nas_interface_rrc::found_plmn_t plmns[nas_interface_rrc::MAX_FOUND_PLMNS];
};
template <typename T>
class test_stack_dummy : public isrue::stack_test_dummy, public stack_interface_gw, public thread
{
public:
  ...
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu) { pdcp->write_sdu(lcid, std::move(sdu)); }
...
class gw_dummy : public gw_interface_nas, public gw_interface_pdcp
{
  ...
  int deactivate_eps_bearer(const uint32_t eps_bearer_id) { return ISRRAN_SUCCESS; }
