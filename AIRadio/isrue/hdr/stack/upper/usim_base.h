...
#ifndef ISRUE_USIM_BASE_H
#define ISRUE_USIM_BASE_H
...
{
public:
  explicit usim_base(isrlog::basic_logger& logger);
  virtual ~usim_base();
  static std::unique_ptr<usim_base> get_instance(usim_args_t* args, isrlog::basic_logger& logger);
  ...
  bool get_home_plmn_id(isrran::plmn_id_t* home_plmn_id) final;
  ...
  void generate_nas_keys(uint8_t*                            k_asme,
                         ...
                         isrran::INTEGRITY_ALGORITHM_ID_ENUM integ_algo) final;

  // RRC interface
  void generate_as_keys(uint8_t* k_asme, uint32_t count_ul, isrran::as_security_config_t* sec_cfg) final;
  void generate_as_keys_ho(uint32_t pci, uint32_t earfcn, int ncc, isrran::as_security_config_t* sec_cfg) final;
  void store_keys_before_ho(const isrran::as_security_config_t& as_ctx) final;
  void restore_keys_from_failed_ho(isrran::as_security_config_t* as_ctx) final;

  // NR RRC interface
  void generate_nr_as_keys(isrran::as_key_t& k_amf, uint32_t count_ul, isrran::as_security_config_t* sec_cfg) final;
  bool generate_nr_context(uint16_t sk_counter, isrran::as_security_config_t* sec_cfg) final;
  bool update_nr_context(isrran::as_security_config_t* sec_cfg) final;
...
protected:
  ...
  isrran::as_key_t k_gnb_initial = {};

  // Current K_eNB context (K_eNB, NH and NCC)
  isrran::k_enb_context_t k_enb_ctx = {};
  isrran::k_gnb_context_t k_gnb_ctx = {};

  // Helpers to restore security context if HO fails
  isrran::k_enb_context_t      old_k_enb_ctx = {};
  isrran::as_security_config_t old_as_ctx    = {};
};

} // namespace isrue

#endif // ISRUE_USIM_BASE_H
