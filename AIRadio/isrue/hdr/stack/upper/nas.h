...
#include "isrue/hdr/stack/upper/nas_emm_state.h"
#include "isrue/hdr/stack/upper/nas_metrics.h"
...
class nas : public nas_interface_rrc, public isrran::timer_callback, public nas_base
{
public:
  explicit nas(isrlog::basic_logger& logger_, isrran::task_sched_handle task_sched_);
  virtual ~nas();
  ...
  bool     paging(isrran::s_tmsi_t* ue_identity) override;
  void     set_barring(isrran::barring_t barring) override;
  void     write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu) override;
  ...
  bool enable_data();
  bool disable_data();
  void start_service_request(isrran::establishment_cause_t cause_);
...
private:
  ...
  isrran::barring_t current_barring = isrran::barring_t::none;

  isrran::plmn_id_t current_plmn;
  isrran::plmn_id_t home_plmn;

  std::vector<isrran::plmn_id_t> known_plmns;
  ...
  isrran::timer_handler::unique_timer t3402;          // started when attach attempt counter reached 5
  isrran::timer_handler::unique_timer t3410;          // started when attach request is sent, on expiry, start t3411
  isrran::timer_handler::unique_timer t3411;          // started when attach failed
  isrran::timer_handler::unique_timer t3421;          // started when detach request is sent
  isrran::timer_handler::unique_timer reattach_timer; // started to trigger delayed re-attach
  ...
  isrran::timer_handler::unique_timer airplane_mode_sim_timer;
  ...
  void start_attach_request(isrran::establishment_cause_t cause_);
  ...
  // Parsers
  void parse_attach_accept(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_attach_reject(uint32_t lcid, isrran::unique_byte_buffer_t pdu, const uint8_t sec_hdr_type);
  void parse_authentication_request(uint32_t lcid, isrran::unique_byte_buffer_t pdu, const uint8_t sec_hdr_type);
  void parse_authentication_reject(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_identity_request(isrran::unique_byte_buffer_t pdu, const uint8_t sec_hdr_type);
  void parse_security_mode_command(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_service_reject(uint32_t lcid, isrran::unique_byte_buffer_t pdu, const uint8_t sec_hdr_type);
  void parse_esm_information_request(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_emm_information(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_detach_request(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_emm_status(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_activate_dedicated_eps_bearer_context_request(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_deactivate_eps_bearer_context_request(isrran::unique_byte_buffer_t pdu);
  void parse_activate_test_mode(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_close_ue_test_loop(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_modify_eps_bearer_context_request(isrran::unique_byte_buffer_t pdu);

  // Packet generators
  void gen_attach_request(isrran::unique_byte_buffer_t& msg);
  void gen_service_request(isrran::unique_byte_buffer_t& msg);
  ...
  isrran::proc_t<plmn_search_proc> plmn_searcher;
};
