...
#ifndef ISRUE_PROC_RA_H
#define ISRUE_PROC_RA_H
...
class ra_proc : public isrran::timer_callback
{
public:
  explicit ra_proc(isrlog::basic_logger& logger) : rar_pdu_msg(20), logger(logger) {}
  ...
  void set_config(isrran::rach_cfg_t& rach_cfg);
  ...
  void start_pcap(isrran::mac_pcap* pcap);
...
private:
  ...
  uint32_t              ra_rnti                     = ISRRAN_INVALID_RNTI;
  uint32_t              ra_tti                      = 0;
  ...
  isrran_softbuffer_rx_t softbuffer_rar = {};
  ...
  isrran::task_multiqueue::queue_handle task_queue;

  isrran::timer_handler::unique_timer* time_alignment_timer = nullptr;
  isrran::timer_handler::unique_timer  contention_resolution_timer;
