...
#ifndef ISRRAN_PHY_CONTROLLER_H
#define ISRRAN_PHY_CONTROLLER_H

#include "isrran/adt/fsm.h"
#include "isrran/adt/observer.h"
...
class phy_controller : public isrran::fsm_t<phy_controller>
{
...
public:
  ...
  explicit phy_controller(phy_interface_rrc_lte*                        phy_,
                          isrran::task_sched_handle                     task_sched_,
                          std::function<void(uint32_t, uint32_t, bool)> on_cell_selection = {});

  // PHY procedures interfaces
  bool start_cell_select(const phy_cell_t& phy_cell, isrran::event_observer<bool> observer = {});
  bool start_cell_search(isrran::event_observer<cell_srch_res> observer, int earfcn);
  ...
  bool set_cell_config(const isrran::phy_cfg_t& config, uint32_t cc_idx = 0);
  void set_phy_to_default();
  void set_phy_to_default_dedicated();
  void set_phy_to_default_pucch_isr();
  ...
  isrran::span<const isrran::phy_cfg_t>   current_cell_config() const { return current_cells_cfg; }
  isrran::span<isrran::phy_cfg_t>         current_cell_config() { return current_cells_cfg; }
  const std::bitset<ISRRAN_MAX_CARRIERS>& current_config_scells() const { return configured_scell_mask; }
  ...
  struct selecting_cell : public subfsm_t<selecting_cell> {
    ...
    isrran::timer_handler::unique_timer wait_in_sync_timer;
  ...
  protected:
    ...
    to_state<             unknown_st,     isrran::failure_ev                                     >
    // +----------------+---------------+--------------+------------------+----------------------+
    >;
    // clang-format on
  };
...
private:
  ...
  isrran::event_observer<bool>                       cell_selection_notifier;
  std::function<void(uint32_t, uint32_t, bool)>      cell_selection_always_observer;
  isrran::event_dispatcher<cell_srch_res>            cell_search_observers;
  uint32_t                                           nof_pending_configs   = 0;
  std::array<isrran::phy_cfg_t, ISRRAN_MAX_CARRIERS> current_cells_cfg     = {};
  std::bitset<ISRRAN_MAX_CARRIERS>                   configured_scell_mask = {};

  bool set_cell_config(const isrran::phy_cfg_t& cfg, uint32_t cc_idx, bool is_set);
