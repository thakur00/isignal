...
#ifndef ISRRAN_RRC_CELL_H
#define ISRRAN_RRC_CELL_H
...
public:
  ...
  explicit meas_cell(isrran::unique_timer timer);
  meas_cell(const phy_cell_t& phy_cell_, isrran::unique_timer timer);

  // comparison based on pci and earfcn
  bool is_valid() const { return phy_cell.earfcn != 0 && isrran_cellid_isvalid(phy_cell.pci); }
  ...
  bool has_sibs(isrran::span<const uint32_t> indexes) const;
...
{
public:
  explicit meas_cell_nr(isrran::unique_timer timer) : meas_cell(std::move(timer)){};
  meas_cell_nr(const phy_cell_t& phy_cell_, isrran::unique_timer timer) : meas_cell(phy_cell_, std::move(timer)){};
  ...
  isrran::plmn_id_t get_plmn(uint32_t idx) const;
...
{
public:
  explicit meas_cell_eutra(isrran::unique_timer timer) : meas_cell(std::move(timer)){};
  meas_cell_eutra(const phy_cell_t& phy_cell_, isrran::unique_timer timer) : meas_cell(phy_cell_, std::move(timer)){};
...
public:
  ...
  explicit meas_cell_list(isrran::task_sched_handle task_sched_);
...
private:
  ...
  // args
  isrlog::basic_logger&     logger = isrlog::fetch_basic_logger("RRC");
  ...
  std::array<std::pair<uint32_t, uint32_t>, ISRRAN_MAX_CARRIERS> current_cell_pci_earfcn = {};
};

} // namespace isrue

#endif // ISRRAN_RRC_CELL_H
