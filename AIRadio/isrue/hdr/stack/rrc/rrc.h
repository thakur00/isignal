...
class rrc : public rrc_interface_nas,
            ...
            public isrran::timer_callback
{
public:
  rrc(stack_interface_rrc* stack_, isrran::task_sched_handle task_sched_);
  ~rrc();
  ...
  void isrran_rrc_log(const char* str);
  ...
  // NAS interface
  void     write_sdu(isrran::unique_byte_buffer_t sdu);
  void     enable_capabilities();
  uint16_t get_mcc();
  uint16_t get_mnc();
  bool     plmn_search() final;
  void     plmn_select(isrran::plmn_id_t plmn_id);
  bool     connection_request(isrran::establishment_cause_t cause, isrran::unique_byte_buffer_t dedicated_info_nas);
  void     set_ue_identity(isrran::s_tmsi_t s_tmsi);
...
private:
  ...
  bool                           running = false;
  isrran::block_queue<cmd_msg_t> cmd_q;

  void process_pcch(isrran::unique_byte_buffer_t pdu);
  ...
  isrran::phy_cfg_t previous_phy_cfg = {};
  isrran::mac_cfg_t current_mac_cfg, previous_mac_cfg = {};
  ...
  isrran::timer_handler::unique_timer t300, t301, t302, t310, t311, t304;

  static const std::string rb_id_str[];

  const char* get_rb_name(uint32_t lcid) { return isrran::is_lte_rb(lcid) ? rb_id_str[lcid].c_str() : "invalid RB"; }
  ...
  isrran::block_queue<std::vector<phy_meas_t> > cell_meas_q;

  void                                             process_cell_meas_nr();
  void                                             process_new_cell_meas_nr(const std::vector<phy_meas_nr_t>& meas);
  isrran::block_queue<std::vector<phy_meas_nr_t> > cell_meas_nr_q;
  ...
  class ho_proc;
  isrran::proc_t<cell_search_proc, rrc_interface_phy_lte::cell_search_ret_t> cell_searcher;
  isrran::proc_t<si_acquire_proc>                                            si_acquirer;
  isrran::proc_t<serving_cell_config_proc>                                   serv_cell_cfg;
  isrran::proc_t<cell_selection_proc, cs_result_t>                           cell_selector;
  isrran::proc_t<go_idle_proc>                                               idle_setter;
  isrran::proc_t<process_pcch_proc>                                          pcch_processor;
  isrran::proc_t<connection_request_proc>                                    conn_req_proc;
  isrran::proc_t<connection_setup_proc>                                      conn_setup_proc;
  isrran::proc_t<plmn_search_proc>                                           plmn_searcher;
  isrran::proc_t<cell_reselection_proc>                                      cell_reselector;
  isrran::proc_t<connection_reest_proc>                                      connection_reest;
  isrran::proc_t<ho_proc>                                                    ho_handler;
  isrran::proc_t<connection_reconf_no_ho_proc>                               conn_recfg_proc;

  isrran::proc_manager_list_t callback_list;
  ...
  isrran::plmn_id_t     selected_plmn_id = {};
  ...
  // Senders
  void send_con_request(isrran::establishment_cause_t cause);
  ...
  void send_con_setup_complete(isrran::unique_byte_buffer_t nas_msg);
  void send_ul_info_transfer(isrran::unique_byte_buffer_t nas_msg);
  void send_security_mode_complete();
  void send_rrc_con_reconfig_complete(bool contains_nr_complete = false);

  // Parsers
  void process_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_dl_ccch(isrran::unique_byte_buffer_t pdu);
  void parse_dl_dcch(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_dl_info_transfer(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu);
  void parse_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  ...
  void set_phy_default_pucch_isr();
