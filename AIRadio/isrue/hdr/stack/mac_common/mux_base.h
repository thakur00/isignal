...
#ifndef ISRUE_MUX_BASE_H
#define ISRUE_MUX_BASE_H
...
public:
    ...
    if (has_logical_channel(config.lcid)) {
      ...
      for (auto& channel : logical_channels) {
        if (channel.priority == config.priority && channel.lcid != config.lcid) {
          isrlog::fetch_basic_logger("MAC").error("LCID %d and %d have same priority.", channel.lcid, config.lcid);
          return ISRRAN_ERROR;
        }
      }
    } else {
  ...
  void print_logical_channel_state(const std::string& info)
  {
    ...
    isrlog::fetch_basic_logger("MAC").debug("%s", logline.c_str());
  }

protected:
  static bool priority_compare(const isrran::logical_channel_config_t& u1, const isrran::logical_channel_config_t& u2)
  {
  ...
  std::vector<isrran::logical_channel_config_t> logical_channels;
};

} // namespace isrue

#endif // ISRUE_MUX_BASE_H