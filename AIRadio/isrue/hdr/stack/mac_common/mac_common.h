...
#ifndef ISRUE_MAC_COMMON_H
#define ISRUE_MAC_COMMON_H
...
public:
  ...
  bool is_in_window(int tti)
  {
    ...
    if ((int)isrran_tti_interval(tti, start) < length + 5) {
      if (tti > start) {
        if (tti <= start + length) {
          return true;
        } else {
