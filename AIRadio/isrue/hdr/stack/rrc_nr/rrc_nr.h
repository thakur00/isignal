...
#ifndef ISRUE_RRC_NR_H
#define ISRUE_RRC_NR_H
...
{
public:
  rrc_nr(isrran::task_sched_handle task_sched_);
  ~rrc_nr();
  ...
  void release_pucch_isr() final;

  // PDCP interface
  void write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu) final;
  void write_pdu_bcch_bch(isrran::unique_byte_buffer_t pdu) final;
  void write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu) final;
  void write_pdu_pcch(isrran::unique_byte_buffer_t pdu) final;
  void write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu) final;
  void notify_pdcp_integrity_error(uint32_t lcid) final;

  // NAS interface
  int      write_sdu(isrran::unique_byte_buffer_t sdu);
  bool     is_connected();
  int      connection_request(isrran::nr_establishment_cause_t cause, isrran::unique_byte_buffer_t sdu);
  uint16_t get_mcc();
  uint16_t get_mnc();

  // RRC (LTE) interface
  int  get_eutra_nr_capabilities(isrran::byte_buffer_t* eutra_nr_caps);
  int  get_nr_capabilities(isrran::byte_buffer_t* eutra_nr_caps);
  void phy_meas_stop();
...
private:
  // parsers
  void decode_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu);
  void decode_dl_ccch(isrran::unique_byte_buffer_t pdu);
  void decode_dl_dcch(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  // senders
  void send_setup_request(isrran::nr_establishment_cause_t cause);
  ...
  isrran::timer_handler::unique_timer sim_measurement_timer;
  ...
  std::map<uint32_t, isrran::nr_drb> lcid_drb; // Map of lcid to drb
  ...
  isrran::static_circular_map<uint32_t, isrran_pucch_nr_resource_t, 128> pucch_res_list;
  std::map<uint32_t, isrran_csi_rs_zp_resource_t>                        csi_rs_zp_res;
  std::map<uint32_t, isrran_csi_rs_nzp_resource_t>                       csi_rs_nzp_res;
  ...
  isrran::proc_t<cell_selection_proc, rrc_cell_search_result_t> cell_selector;
  isrran::proc_t<connection_setup_proc>                         conn_setup_proc;
  isrran::proc_t<connection_reconf_no_ho_proc>                  conn_recfg_proc;
  isrran::proc_t<setup_request_proc>                            setup_req_proc;
