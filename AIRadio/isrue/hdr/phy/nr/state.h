...
#ifndef ISRRAN_STATE_H
#define ISRRAN_STATE_H
...
private:
  struct pending_ul_grant_t {
    bool                enable;
    uint32_t            pid;
    isrran_sch_cfg_nr_t sch_cfg;
  };
  isrran::circular_array<pending_ul_grant_t, TTIMOD_SZ> pending_ul_grant = {};
  ...
  isrran::circular_array<pending_dl_grant_t, TTIMOD_SZ> pending_dl_grant = {};
  mutable std::mutex                                    pending_dl_grant_mutex;

  isrran::circular_array<isrran_pdsch_ack_nr_t, TTIMOD_SZ> pending_ack = {};
  ...
  std::array<isrran_csi_channel_measurements_t, ISRRAN_CSI_MAX_NOF_RESOURCES> csi_measurements = {};
...
public:
  ...
  isrran::tti_semaphore<void*> dl_ul_semaphore;
   ...
   */
  void set_ul_pending_grant(const isrran::phy_cfg_nr_t& cfg,
                            const isrran_slot_cfg_t&    slot_rx,
                            const isrran_dci_ul_nr_t&   dci_ul)
  {
    ...
    if (not cfg.get_pusch_cfg(slot_rx, dci_ul, pusch_cfg)) {
      ...
      isrran_dci_ul_nr_to_str(NULL, &dci_ul, str.data(), str.size());
   ...
   */
  bool get_ul_pending_grant(uint32_t tti_tx, isrran_sch_cfg_nr_t& pusch_cfg, uint32_t& pid)
  {
    // Scope mutex to protect read/write the list
    std::lock_guard<std::mutex> lock(pending_ul_grant_mutex);
   ...
   */
  void
  set_dl_pending_grant(const isrran::phy_cfg_nr_t& cfg, const isrran_slot_cfg_t& slot, const isrran_dci_dl_nr_t& dci_dl)
  {
    ...
    if (not cfg.get_pdsch_cfg(slot, dci_dl, pdsch_cfg)) {
      std::array<char, 512> str;
      isrran_dci_dl_nr_to_str(NULL, &dci_dl, str.data(), str.size());
   ...
   * @param dci_dl The DL DCI message to store
   */
  void set_pending_ack(const uint32_t& tti_rx, const isrran_harq_ack_resource_t& ack_resource, const bool& crc_ok)
  {
    // Calculate Receive TTI
    ...
    isrran_pdsch_ack_nr_t& ack = pending_ack[tti_tx];
    ack.nof_cc                 = 1;

    // Insert PDSCH transmission information
    if (isrran_harq_ack_insert_m(&ack, &ack_m) < ISRRAN_SUCCESS) {
      ERROR("Error inserting ACK m value for Tx slot %d", tti_tx);
    }
  }

  bool get_pending_ack(const uint32_t& tti_tx, isrran_pdsch_ack_nr_t& pdsch_ack)
  {
    // Scope mutex to protect read/write the list
    std::lock_guard<std::mutex> lock(pending_ack_mutex);
  ...
  bool has_valid_sr_resource(const isrran::phy_cfg_nr_t& cfg, uint32_t sr_id)
  {
    for (const isrran_pucch_nr_sr_resource_t& r : cfg.pucch.sr_resources) {
  ...
  void get_pending_sr(const isrran::phy_cfg_nr_t& cfg, const uint32_t& tti, isrran_uci_data_nr_t& uci_data)
  {
    // Calculate all SR opportunities in the given TTI
    uint32_t sr_resource_id[ISRRAN_PUCCH_MAX_NOF_SR_RESOURCES] = {};
    int      n = isrran_ue_ul_nr_sr_send_slot(cfg.pucch.sr_resources, tti, sr_resource_id);
    if (n < ISRRAN_SUCCESS) {
    ...
    uci_data.cfg.o_sr                 = isrran_ra_ul_nr_nof_sr_bits(sr_count_all);
    uci_data.cfg.sr_positive_present  = sr_count_positive > 0;
    uci_data.value.sr                 = sr_count_positive;
  }

  void
  get_periodic_csi(const isrran::phy_cfg_nr_t& cfg, const isrran_slot_cfg_t& slot_cfg, isrran_uci_data_nr_t& uci_data)
  {
    // Generate report configurations
    int n = isrran_csi_reports_generate(&cfg.csi, &slot_cfg, uci_data.cfg.csi);
    if (n > ISRRAN_SUCCESS) {
      uci_data.cfg.nof_csi = n;
    }

    // Quantify reports from measurements
    n = isrran_csi_reports_quantify(uci_data.cfg.csi, csi_measurements.data(), uci_data.value.csi);
    if (n > ISRRAN_SUCCESS) {
      uci_data.cfg.nof_csi = n;
    }
    ...
    if (args.fix_wideband_cqi != 0 && args.fix_wideband_cqi < 15) {
      for (uint32_t i = 0; i < uci_data.cfg.nof_csi; i++) {
        if (uci_data.cfg.csi[i].cfg.quantity == ISRRAN_CSI_REPORT_QUANTITY_CRI_RI_PMI_CQI &&
            uci_data.cfg.csi[i].cfg.freq_cfg == ISRRAN_CSI_REPORT_FREQ_WIDEBAND) {
   ...
   */
  void new_nzp_csi_rs_channel_measurement(const isrran::phy_cfg_nr_t&              cfg,
                                          const isrran_csi_channel_measurements_t& new_measure,
                                          uint32_t                                 resource_set_id)
  {
    std::lock_guard<std::mutex> lock(csi_measurements_mutex);

    if (isrran_csi_new_nzp_csi_rs_measurement(
            cfg.csi.csi_resources, csi_measurements.data(), &new_measure, resource_set_id) < ISRRAN_SUCCESS) {
   ...
   * measured (i.e. SSB)
   */
  void new_csi_trs_measurement(const isrran_csi_trs_measurements_t& new_meas,
                               ...
                               uint32_t                             K_csi_rs        = 0)
  {
    ...
    trs_measurements.rsrp_dB = ISRRAN_VEC_SAFE_EMA(new_meas.rsrp_dB, trs_measurements.rsrp_dB, args.trs_epre_ema_alpha);
    trs_measurements.epre_dB = ISRRAN_VEC_SAFE_EMA(new_meas.epre_dB, trs_measurements.epre_dB, args.trs_rsrp_ema_alpha);
    trs_measurements.snr_dB  = ISRRAN_VEC_SAFE_EMA(new_meas.snr_dB, trs_measurements.snr_dB, args.trs_sinr_ema_alpha);
    // Consider CFO measurement invalid if the SNR is negative. In this case, set CFO to 0.
    if (new_meas.snr_dB > 0.0f) {
      trs_measurements.cfo_hz = ISRRAN_VEC_SAFE_EMA(new_meas.cfo_hz, trs_measurements.cfo_hz, args.trs_cfo_ema_alpha);
    }
