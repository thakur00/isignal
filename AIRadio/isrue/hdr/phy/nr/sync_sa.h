...
#ifndef ISRUE_SYNC_NR_SA_H
#define ISRUE_SYNC_NR_SA_H
...
#include "isrue/hdr/phy/sync_state.h"
 ...
 */
class sync_sa : public isrran::thread, public isrran::phy_common_interface
{
public:
  struct args_t {
  ...
  sync_sa(isrlog::basic_logger& logger, worker_pool& workers_);
  ~sync_sa();

  bool                init(const args_t& args_, stack_interface_phy_nr* stack_, isrran::radio_interface_phy* radio_);
  bool                reset();
  void                stop();
...
private:
  ...
  isrran::radio_interface_phy* radio = nullptr; ///< Radio object
  isrlog::basic_logger&        logger;          ///< General PHY logger
  worker_pool&                 workers;
  ...
  uint32_t                     tti       = 0;
  isrran::tti_semaphore<void*> tti_semaphore;
  ...
  int  radio_recv_fnc(isrran::rf_buffer_t& data, isrran_timestamp_t* rx_time);
  void run_stack_tti();
