...
#ifndef ISRUE_TA_CONTROL_H
#define ISRUE_TA_CONTROL_H
...
public:
  ta_control(isrlog::basic_logger& logger) : logger(logger) {}
  ...
  void set_base_sec(float ta_base_sec)
  {
    ...
    // Update base in nta
    next_base_nta = static_cast<uint32_t>(roundf(next_base_sec / ISRRAN_LTE_TS));
  ...
  void add_ta_offset(uint32_t ta_offset)
  {
    ...
    next_base_sec = static_cast<float>(next_base_nta) * ISRRAN_LTE_TS;
  ...
  void add_ta_cmd_rar(uint32_t tti, uint32_t ta_cmd)
  {
    ...
    // Update base nta
    next_base_nta += isrran_N_ta_new_rar(ta_cmd);
  ...
  void add_ta_cmd_new(uint32_t tti, uint32_t ta_cmd)
  {
    ...
    // Update base nta
    next_base_nta = isrran_N_ta_new(next_base_nta, ta_cmd);
