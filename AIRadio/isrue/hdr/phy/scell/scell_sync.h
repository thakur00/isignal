...
#ifndef ISRRAN_SCELL_SYNC_H
#define ISRRAN_SCELL_SYNC_H
...
private:
  ...
  uint32_t                                        channel  = 0;
  isrran_sync_t                                   find_pss = {};
  int32_t                                         sf_len   = 0;
  int32_t                                         cell_id  = -1;
  std::array<cf_t, BUFFER_LEN* ISRRAN_SF_LEN_MAX> temp     = {};
  ...
  void run_state_search_pss(uint32_t tti, cf_t* buffer)
  {
    ...
    if (buffer == nullptr) {
      isrran_vec_cf_zero(&temp[sf_len], sf_len);
    } else {
      isrran_vec_cf_copy(&temp[sf_len], buffer, sf_len);
    }

    // Run PSS search
    switch (isrran_sync_find(&find_pss, temp.data(), 0, &peak_pos)) {
      case ISRRAN_SYNC_FOUND:
        if (callback != nullptr) {
          // Calculate Sample Offset from TTI difference
          int tti_mod    = (int)tti % (ISRRAN_NOF_SF_X_FRAME / 2);
          int tti_offset = (tti_mod < 3) ? tti_mod : (tti_mod - ISRRAN_NOF_SF_X_FRAME / 2);
    ...
    if (state == STATE_SEARCH_PSS) {
      isrran_vec_cf_copy(&temp[0], buffer, sf_len);
    }
  }
  ...
  {
    uint32_t symbol_sz  = isrran_symbol_sz(new_nof_prb);
    int32_t  new_sf_len = ISRRAN_SF_LEN_PRB(new_nof_prb);

    // Reset Temporal buffer
    isrran_vec_cf_zero(temp.data(), BUFFER_LEN * new_sf_len);
    ...
    if (isrran_sync_resize(&find_pss, BUFFER_LEN * new_sf_len, BUFFER_LEN * new_sf_len, symbol_sz) != ISRRAN_SUCCESS) {
...
public:
  ...
  sync(sync_callback* _callback, uint32_t _channel) : callback(_callback), channel(_channel)
  {
    ...
    if (isrran_sync_init(&find_pss, 2 * ISRRAN_SF_LEN_MAX, 2 * ISRRAN_SF_LEN_MAX, ISRRAN_SYMBOL_SZ_MAX) !=
        ISRRAN_SUCCESS) {
      ERROR("Initiating Synchronizer");
    }
  }

  ~sync() { isrran_sync_free(&find_pss); };
   ...
   */
  void set_cell(const isrran_cell_t& cell)
  {
    // Protect DSP objects and buffers; As it is called by asynchronous thread, it can wait to finish current processing
    std::unique_lock<std::mutex> lock(mutex);
    ...
    if (cell_id != new_cell_id) {
      isrran_sync_set_frame_type(&find_pss, cell.frame_type);
      isrran_sync_set_N_id_2(&find_pss, new_cell_id % ISRRAN_NOF_NID_2);
      isrran_sync_set_N_id_1(&find_pss, new_cell_id / ISRRAN_NOF_NID_2);
      isrran_sync_set_cfo_ema_alpha(&find_pss, 0.1);
      isrran_sync_set_em_alpha(&find_pss, 1);
      isrran_sync_set_threshold(&find_pss, 3.0);
      cell_id = new_cell_id;
    }
