...
#ifndef ISRRAN_SCELL_STATE_H
#define ISRRAN_SCELL_STATE_H
 ...
 *  - ISR transmissions on the SCell;
...
private:
  ...
  std::array<cfg, ISRRAN_MAX_CARRIERS> scell_cfg;
...
public:
    ...
    switch (activation_state) {
      ...
      case transition:
        ...
        if (TTI_SUB(tti, activation_tti) >= activation_margin_tti) {
          // Reload cell states
          for (uint32_t i = 1; i < ISRRAN_MAX_CARRIERS; i++) {
  ...
  void configure(uint32_t cc_idx, uint32_t earfcn, uint32_t pci)
  {
    ...
    if (cc_idx == 0 or cc_idx >= ISRRAN_MAX_CARRIERS) {
