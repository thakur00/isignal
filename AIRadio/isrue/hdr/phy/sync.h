...
#ifndef ISRUE_PHCH_RECV_H
#define ISRUE_PHCH_RECV_H
...
#include "isrran/common/tti_sync_cv.h"
...
class sync : public isrran::thread,
...
{
public:
  sync(isrlog::basic_logger& phy_logger, isrlog::basic_logger& phy_lib_logger) :
    thread("SYNC"),
    search_p(phy_logger),
    sfn_p(phy_logger),
  ...
  void init(isrran::radio_interface_phy* radio_,
  ...
  void set_inter_frequency_measurement(uint32_t cc_idx, uint32_t earfcn_, isrran_cell_t cell_);
  void meas_stop();
  ...
  void     get_current_cell(isrran_cell_t* cell, uint32_t* earfcn = nullptr);
  ...
  int  radio_recv_fnc(isrran::rf_buffer_t&, isrran_timestamp_t* rx_time) override;

  isrran::radio_interface_phy* get_radio() override { return radio_h; }
   ...
   */
  void scell_sync_set(uint32_t cc_idx, const isrran_cell_t& _cell);
...
private:
  void reset();
  void radio_error();
  void set_ue_sync_opts(isrran_ue_sync_t* q, float cfo) override;
   ...
   */
  void
  run_camping_in_sync_state(lte::sf_worker* lte_worker, nr::sf_worker* nr_worker, isrran::rf_buffer_t& sync_buffer);
  ...
  isrlog::basic_logger&        phy_logger;
  isrlog::basic_logger&        phy_lib_logger;
  ...
  isrran::channel_ptr          channel_emulator = nullptr;
  ...
  isrran::rf_buffer_t   sf_buffer             = {};
  isrran::rf_buffer_t   dummy_buffer;
  ...
  public:
    ...
    {
      std::lock_guard<std::mutex> lock(mutex);
      return isrran_cell_isvalid(&cell);
    }
    void set(isrran_cell_t& x)
    {
      std::lock_guard<std::mutex> lock(mutex);
      cell = x;
    }
    ...
    bool equals(isrran_cell_t& x)
    {
      std::lock_guard<std::mutex> lock(mutex);
      return memcmp(&cell, &x, sizeof(isrran_cell_t)) == 0;
    }
  ...
  std::array<uint8_t, ISRRAN_BCH_PAYLOAD_LEN> mib                    = {};
