...
#ifndef ISRUE_LTE_WORKER_POOL_H
#define ISRUE_LTE_WORKER_POOL_H
...
private:
  ...
  private:
    ...
    isrran::phy_cfg_t cfg;     ///< Actual CC configuration

  public:
    phy_cfg_stash_t(uint32_t max_workers) : pending(max_workers) {}
    void                     set_cfg(const isrran::phy_cfg_t& c);
    bool                     is_pending(uint32_t sf_idx);
    const isrran::phy_cfg_t& get_cfg(uint32_t sf_idx);
  };
  std::mutex                                       phy_cfg_mutex; ///< Protects configuration stash
  std::array<phy_cfg_stash_t, ISRRAN_MAX_CARRIERS> phy_cfg_stash; ///< Stores the latest worker configuration
...
public:
   ...
   */
  void set_config(uint32_t cc_idx, const isrran::phy_cfg_t& phy_cfg);
};

} // namespace lte
} // namespace isrue

#endif // ISRUE_LTE_WORKER_POOL_H
