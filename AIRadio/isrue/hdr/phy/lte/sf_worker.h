...
#ifndef ISRUE_LTE_SF_WORKER_H
#define ISRUE_LTE_SF_WORKER_H
...
{
public:
  sf_worker(uint32_t max_prb, phy_common* phy_, isrlog::basic_logger& logger);
  virtual ~sf_worker();

  void reset_cell_nolock(uint32_t cc_idx);
  bool set_cell_nolock(uint32_t cc_idx, isrran_cell_t cell_);
  ...
  void set_config_nolock(uint32_t cc_idx, const isrran::phy_cfg_t& phy_cfg);
...
private:
  ...
  void reset_uci(isrran_uci_data_t* uci_data);
