...
#include "isrue/hdr/stack/mac/demux.h"
...
demux::demux(isrlog::basic_logger& logger) :
...
void demux::init(phy_interface_mac_common*            phy_,
                 ...
                 isrran::timer_handler::unique_timer* time_alignment_timer_)
{
  phy_h                = phy_;
  rlc                  = rlc_;
  mac                  = mac_;
...
void demux::push_pdu_temp_crnti(uint8_t* buff, uint32_t nof_bytes)
{
  if (nof_bytes > 0) {
    ...
    if (is_uecrid_successful) {
      ...
      pdus.push(buff, nof_bytes, isrran::pdu_queue::DCH);
    } else {
...
void demux::push_pdu(uint8_t* buff, uint32_t nof_bytes, uint32_t tti)
{
  ...
  return pdus.push(buff, nof_bytes, isrran::pdu_queue::DCH);
}
...
{
  pdus.push(buff, nof_bytes, isrran::pdu_queue::BCH);
}
...
void demux::push_pdu_mch(uint8_t* buff, uint32_t nof_bytes)
{
  ...
  pdus.push(mch_buffer_ptr, nof_bytes, isrran::pdu_queue::MCH);
  mch_buffer_ptr = NULL;
}
...
void demux::process_pdu(uint8_t* mac_pdu, uint32_t nof_bytes, isrran::pdu_queue::channel_t channel, int ul_nof_prbs)
{
  Debug("Processing MAC PDU channel %d", channel);
  switch (channel) {
    case isrran::pdu_queue::DCH:
      // Unpack DLSCH MAC PDU
...
{
  isrran::sch_pdu mac_msg_rt(20, logger);
...
void demux::process_sch_pdu(isrran::sch_pdu* pdu_msg)
{
  while (pdu_msg->next()) {
    if (pdu_msg->get()->is_sdu()) {
      ...
      if (route_pdu) {
        ...
        if (pdu_msg->get()->get_payload_size() < MAX_PDU_LEN) {
          ...
          char tmp[1024];
          isrran_vec_sprint_hex(tmp, sizeof(tmp), pdu_msg->get()->get_sdu_ptr(), 32);
...
void demux::process_mch_pdu(isrran::mch_pdu* mch_msg)
{
  // disgarding headers that have already been processed
  while (mch_msg->next()) {
    if (isrran::mch_lcid::MCH_SCHED_INFO == mch_msg->get()->mch_ce_type()) {
      uint16_t stop;
      uint8_t  lcid;
    ...
    if (mch_msg->get()->is_sdu()) {
      ...
      if (lcid >= ISRRAN_N_MCH_LCIDS) {
        Error("Radio bearer id must be in [0:%d] - %d", ISRRAN_N_MCH_LCIDS, lcid);
        return;
      }
...
bool demux::process_ce(isrran::sch_subh* subh, uint32_t tti)
{
...
void demux::parse_ta_cmd(isrran::sch_subh* subh, uint32_t tti)
{
