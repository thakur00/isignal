...
mux::mux(isrlog::basic_logger& logger) : logger(logger), pdu_msg(MAX_NOF_SUBHEADERS, logger) {}
...
void mux::step()
{
  ...
  for (auto& channel : logical_channels) {
    ...
    channel.Bj = ISRRAN_MIN((uint32_t)channel.Bj, channel.bucket_size);
...
isrran::ul_sch_lcid bsr_format_convert(bsr_proc::bsr_format_t format)
{
  switch (format) {
    case bsr_proc::LONG_BSR:
      return isrran::ul_sch_lcid::LONG_BSR;
    case bsr_proc::TRUNC_BSR:
      return isrran::ul_sch_lcid::TRUNC_BSR;
    case bsr_proc::SHORT_BSR:
    default:
      return isrran::ul_sch_lcid::SHORT_BSR;
  }
}

uint8_t* mux::pdu_get(isrran::byte_buffer_t* payload, uint32_t pdu_sz)
{
  std::lock_guard<std::mutex> lock(mutex);
...
uint8_t* mux::pdu_get_nolock(isrran::byte_buffer_t* payload, uint32_t pdu_sz)
{
  ...
  for (auto& channel : logical_channels) {
    ...
    if (max_sdu_sz != 0) {
      if (sched_sdu(&channel, &sdu_space, max_sdu_sz)) {
        ...
        last_sdu_subheader_len = ISRRAN_MIN((uint32_t)sdu_space, sch_pdu::size_header_sdu(channel.sched_len));
...
uint32_t mux::allocate_sdu(uint32_t lcid, isrran::sch_pdu* pdu_msg, int max_sdu_sz)
{
  ...
  while (buffer_state > 0 && sdu_space > 0) { // there is pending SDU to allocate
    int requested_sdu_len = ISRRAN_MIN(buffer_state, sdu_space);
...
uint8_t* mux::msg3_get(isrran::byte_buffer_t* payload, uint32_t pdu_sz)
{
  std::lock_guard<std::mutex> lock(mutex);
