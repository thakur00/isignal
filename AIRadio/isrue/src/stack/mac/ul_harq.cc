...
#include "isrue/hdr/stack/mac/ul_harq.h"
...
void ul_harq_entity::set_config(isrran::ul_harq_cfg_t& harq_cfg_)
{
  std::lock_guard<std::mutex> lock(config_mutex);
  harq_cfg = harq_cfg_;
}

void ul_harq_entity::start_pcap(isrran::mac_pcap* pcap_)
{
  pcap = pcap_;
}
                                  ...
                                  mac_interface_phy_lte::tb_action_ul_t* action)
{
  ...
  if (grant.rnti == rntis->get_crnti() || grant.rnti == rntis->get_temp_rnti() || ISRRAN_RNTI_ISRAR(grant.rnti)) {
...
ul_harq_entity::ul_harq_process::ul_harq_process() : logger(isrlog::fetch_basic_logger("MAC"))
{
  pdu_ptr        = NULL;
  payload_buffer = NULL;
...
{
  if (isrran_softbuffer_tx_init(&softbuffer, 110)) {
                                                      ...
                                                      mac_interface_phy_lte::tb_action_ul_t* action)
{
  ...
  harq_entity->average_retx.store(ISRRAN_VEC_CMA((float)current_tx_nb,
