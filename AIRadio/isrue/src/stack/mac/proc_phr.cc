...
#include "isrue/hdr/stack/mac/proc_phr.h"
#include "isrran/interfaces/ue_phy_interfaces.h"

namespace isrue {

phr_proc::phr_proc(isrlog::basic_logger& logger) : logger(logger)
{
  initiated        = false;
  last_pathloss_db = 0;
  phr_cfg          = {};
...
void phr_proc::init(phy_interface_mac_lte* phy_h_, isrran::ext_task_sched_handle* task_sched_)
{
  phy_h      = phy_h_;
...
void phr_proc::set_config(isrran::phr_cfg_t& cfg)
{
  std::lock_guard<std::mutex> lock(mutex);
