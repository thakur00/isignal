...
#include "isrue/hdr/stack/mac/proc_ra.h"
...
void ra_proc::init(phy_interface_mac_lte*               phy_h_,
                   ...
                   mux*                                 mux_unit_,
                   isrran::ext_task_sched_handle*       task_sched_)
{
  phy_h      = phy_h_;
  rntis      = rntis_;
  mux_unit   = mux_unit_;
  ...
  isrran_softbuffer_rx_init(&softbuffer_rar, 10);
...
ra_proc::~ra_proc()
{
  isrran_softbuffer_rx_free(&softbuffer_rar);
}
...
void ra_proc::start_pcap(isrran::mac_pcap* pcap_)
{
  pcap = pcap_;
}
...
void ra_proc::set_config(isrran::rach_cfg_t& rach_cfg_)
{
  rach_cfg = rach_cfg_;
}
...
void ra_proc::state_pdcch_setup()
{
  ...
  if (info.is_transmitted) {
    ...
    isrran::console("Random Access Transmission%s: seq=%d, tti=%d, ra-rnti=0x%x\n",
...
void ra_proc::state_response_reception(uint32_t tti)
{
  ...
  if (!rar_received) {
    uint32_t interval = isrran_tti_interval(tti, ra_tti + 3 + rach_cfg.responseWindowSize - 1);
...
void ra_proc::state_backoff_wait(uint32_t tti)
{
  if (backoff_interval > 0) {
    ...
    if (backoff_interval == 1) {
      ...
      if (isrran_tti_interval(tti, backoff_interval_start) >= backoff_interval) {
        backoff_interval = 0;
        resource_selection();
      }
    }
...
void ra_proc::new_grant_dl(mac_interface_phy_lte::mac_grant_dl_t grant, mac_interface_phy_lte::tb_action_dl_t* action)
{
  ...
  if (grant.tb[0].tbs < MAX_RAR_PDU_LEN) {
    ...
    if (action->tb[0].rv == 0) {
      isrran_softbuffer_rx_reset(&softbuffer_rar);
    }
  } else {
...
void ra_proc::tb_decoded_ok(const uint8_t cc_idx, const uint32_t tti)
{
  ...
  if (rar_pdu_msg.parse_packet(rar_pdu_buffer) != ISRRAN_SUCCESS) {
  ...
  while (rar_pdu_msg.next()) {
    if (rar_pdu_msg.get()->has_rapid() && rar_pdu_msg.get()->get_rapid() == sel_preamble) {
      ...
      uint8_t grant[isrran::rar_subh::RAR_GRANT_LEN] = {};
