...
#include "isrue/hdr/stack/mac/proc_bsr.h"
#include "isrran/interfaces/ue_rlc_interfaces.h"
#include "isrue/hdr/stack/mac/mux.h"

namespace isrue {

void bsr_proc::init(sr_proc* sr_, rlc_interface_mac* rlc_, isrran::ext_task_sched_handle* task_sched_)
{
  rlc        = rlc_;
  sr         = sr_;
...
void bsr_proc::print_state()
{
  ...
  for (auto& lcg : lcgs) {
    for (auto& iter : lcg) {
      n = isrran_print_check(str, 128, n, "%d: %d ", iter.first, iter.second.old_buffer);
    }
  }
  logger.info(
...
void bsr_proc::set_config(isrran::bsr_cfg_t& bsr_cfg_)
{
  std::lock_guard<std::mutex> lock(mutex);
...
bool bsr_proc::generate_bsr(bsr_t* bsr, uint32_t pdu_space)
{
  ...
  if (pdu_space >= CE_SUBHEADER_LEN + ce_size(isrran::ul_sch_lcid::LONG_BSR)) {
    ...
    send_bsr = true;
  } else if (pdu_space >= CE_SUBHEADER_LEN + ce_size(isrran::ul_sch_lcid::SHORT_BSR)) {
...
bool bsr_proc::generate_padding_bsr(uint32_t nof_padding_bytes, bsr_t* bsr)
{
  ...
  if (nof_padding_bytes >= CE_SUBHEADER_LEN + ce_size(isrran::ul_sch_lcid::SHORT_BSR)) {
