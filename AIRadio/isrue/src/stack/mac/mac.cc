...
mac::mac(const char* logname, ext_task_sched_handle task_sched_) :
  logger(isrlog::fetch_basic_logger(logname)),
  mch_msg(10, logger),
  mux_unit(logger),
  demux_unit(logger),
  ra_procedure(logger),
  ...
  task_sched(task_sched_)
{
  ...
  isrran_softbuffer_rx_init(&pch_softbuffer, 100);
...
mac::~mac()
{
  ...
  isrran_softbuffer_rx_free(&pch_softbuffer);
}
...
void mac::bch_decoded_ok(uint32_t cc_idx, uint8_t* payload, uint32_t len)
{
  ...
  if (buf != nullptr) {
    ...
    auto p = stack_task_dispatch_queue.try_push(std::bind(
        [this](isrran::unique_byte_buffer_t& buf) { rlc_h->write_pdu_bcch_bch(std::move(buf)); }, std::move(buf)));
    if (p.is_error()) {
...
void mac::tb_decoded(uint32_t cc_idx, mac_grant_dl_t grant, bool ack[ISRRAN_MAX_CODEWORDS])
{
  if (grant.is_pdcch_order) {
    ra_procedure.set_config_ded(grant.preamble_idx, grant.prach_mask_idx);
    ra_procedure.start_pdcch_order();
  } else if (ISRRAN_RNTI_ISRAR(grant.rnti)) {
    if (ack[0]) {
    ...
    if (pdu != nullptr) {
      ...
      auto ret = stack_task_dispatch_queue.try_push(std::bind(
          [this](isrran::unique_byte_buffer_t& pdu) { rlc_h->write_pdu_pcch(std::move(pdu)); }, std::move(pdu)));
      if (ret.is_error()) {
...
{
  if (ISRRAN_RNTI_ISRAR(grant.rnti)) {
...
void mac::get_metrics(mac_metrics_t m[ISRRAN_MAX_CARRIERS])
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ...
  memcpy(m, metrics, sizeof(mac_metrics_t) * ISRRAN_MAX_CARRIERS);
  m = metrics;
  bzero(&metrics, sizeof(mac_metrics_t) * ISRRAN_MAX_CARRIERS);
}
