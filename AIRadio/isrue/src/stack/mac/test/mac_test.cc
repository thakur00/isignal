...
#include "isrue/hdr/stack/mac/mac.h"
#include "isrue/hdr/stack/mac/mux.h"
#include <iostream>
#include <string.h>
...
// fake classes
class rlc_dummy : public isrue::rlc_dummy_interface
{
public:
  ...
  uint32_t read_pdu(uint32_t lcid, uint8_t* payload, uint32_t nof_bytes) final
  {
    ...
    uint32_t len = ISRRAN_MIN(ul_queues[lcid], nof_bytes);
...
private:
  ...
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RLC");
...
public:
  ...
  void set_rar_grant(uint8_t grant_payload[ISRRAN_RAR_GRANT_LEN], uint16_t rnti) override
  {
    memcpy(rar_payload, grant_payload, ISRRAN_RAR_GRANT_LEN);
    rar_temp_rnti = rnti;
    nof_rar_grants++;
  }
  ...
  {
    bool ack_v[ISRRAN_MAX_CODEWORDS] = {ack, 0};
  ...
  {
    // Generate RAR to MAC
    uint8_t grant[ISRRAN_RAR_GRANT_LEN] = {1};
    ...
    if (preamble_matches) {
      TESTASSERT(!memcmp(rar_payload, grant, ISRRAN_RAR_GRANT_LEN));
...
private:
  ...
  uint8_t  rar_payload[ISRRAN_RAR_GRANT_LEN];
};
...
public:
  ...
  void     release_pucch_isr() { printf("%s\n", __FUNCTION__); }
...
int mac_unpack_test()
{
  ...
  bool dl_ack[ISRRAN_MAX_CODEWORDS] = {true, false};
...
int mac_ul_sch_pdu_test1()
{
  ...
  // create UL action and grant and push MAC PDU
  {
    ...
    // print generated PDU
    isrlog::fetch_basic_logger("MAC").info(
...
int run_mac_ra_test(struct ra_test test, mac* mac, phy_dummy* phy, uint32_t* tti_state, isrran::stack_dummy* stack)
{
  ...
  for (uint32_t j = 0; j < test.nof_prachs; j++) {
    ...
    for (uint32_t i = 0; i < phy->prach_delay + 3 - 1 + test.rar_offset; i++) {
      stack->run_tti(tti);
      TESTASSERT(!ISRRAN_RNTI_ISRAR(mac->get_dl_sched_rnti(tti)));
      tti++;
    }
...
int main(int argc, char** argv)
{
  ...
  auto& phy_logger = isrlog::fetch_basic_logger("PHY", false);
  phy_logger.set_level(isrlog::basic_levels::none);
  phy_logger.set_hex_dump_max_size(100000);
  isrlog::init();

  TESTASSERT(mac_unpack_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_logical_channel_prioritization_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_logical_channel_prioritization_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_logical_channel_prioritization_test3() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_logical_channel_prioritization_test4() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_with_short_bsr_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_with_padding_long_bsr_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_with_padding_long_bsr_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_with_padding_trunc_bsr_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_regular_bsr_retx_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_periodic_bsr_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_trunc_bsr_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_one_byte_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_two_byte_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_ul_sch_pdu_three_byte_test() == ISRRAN_SUCCESS);
  phy_logger.set_level(isrlog::basic_levels::debug);
  TESTASSERT(mac_random_access_test() == ISRRAN_SUCCESS);
  phy_logger.set_level(isrlog::basic_levels::none);
