...
#include "isrue/hdr/stack/mac/proc_sr.h"
...
sr_proc::sr_proc(isrlog::basic_logger& logger) : logger(logger)
{
  initiated = false;
}
...
void sr_proc::set_config(isrran::sr_cfg_t& cfg)
{
      ...
      if (is_pending_sr) {
        if (sr_cfg.enabled) {
          if (sr_counter < sr_cfg.dsr_transmax) {
            ...
            if (need_tx(tti)) {
              Info("SR:    Releasing PUCCH/ISR resources, sr_counter=%d, dsr_transmax=%d",
                   sr_counter,
                   sr_cfg.dsr_transmax);
