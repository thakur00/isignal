...
#include "isrue/hdr/stack/mac/dl_harq.h"
...
dl_harq_entity::dl_harq_entity(uint8_t cc_idx_) :
  proc(ISRRAN_MAX_HARQ_PROC), logger(isrlog::fetch_basic_logger("MAC")), cc_idx(cc_idx_)
{}
...
bool dl_harq_entity::init(ue_rnti* rntis_, demux* demux_unit_)
{
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_HARQ_PROC; i++) {
                                  ...
                                  mac_interface_phy_lte::tb_action_dl_t* action)
{
  ...
  if (grant.rnti != rntis->get_sps_rnti()) {
    ...
    if (grant.rnti == ISRRAN_SIRNTI) {
      ...
      if (grant.pid >= ISRRAN_MAX_HARQ_PROC) {
...
void dl_harq_entity::tb_decoded(mac_interface_phy_lte::mac_grant_dl_t grant, bool ack[ISRRAN_MAX_CODEWORDS])
{
  if (grant.rnti == ISRRAN_SIRNTI) {
...
void dl_harq_entity::start_pcap(isrran::mac_pcap* pcap_)
{
  pcap = pcap_;
}
...
{
  std::unique_lock<std::mutex> lock(retx_cnt_mutex);
  average_retx = ISRRAN_VEC_CMA((float)n_retx, average_retx, nof_pkts++);
}

dl_harq_entity::dl_harq_process::dl_harq_process() : subproc(ISRRAN_MAX_TB) {}
...
dl_harq_entity::dl_harq_process::dl_tb_process::dl_tb_process() : logger(isrlog::fetch_basic_logger("MAC"))
{
  is_initiated = false;
  ack          = false;
  n_retx       = 0;
...
{
  if (isrran_softbuffer_rx_init(&softbuffer, 110)) {
                                                                  ...
                                                                  mac_interface_phy_lte::tb_action_dl_t* action)
{
  ...
  if (is_new_transmission || (cur_grant.tb[tid].tbs != grant.tb[tid].tbs)) {
    ...
    ack    = false;
    n_retx = 0;
    isrran_softbuffer_rx_reset_tbs(&softbuffer, grant.tb[tid].tbs * 8);
  }
