...
#include "isrue/hdr/stack/rrc/rrc_rlf_report.h"
#include "isrran/rrc/rrc_common.h"

namespace isrue {

void rrc_rlf_report::init(isrran::task_sched_handle task_sched)
{
...
void rrc_rlf_report::set_failure(meas_cell_list<meas_cell_eutra>& meas_cells, failure_type_t type)
{
  ...
  rlf_report.meas_result_last_serv_cell_r9.rsrp_result_r9 =
      rrc_value_to_range(isrran::quant_rsrp, meas_cells.serving_cell().get_rsrp());
  rlf_report.meas_result_last_serv_cell_r9.rsrq_result_r9 =
      rrc_value_to_range(isrran::quant_rsrq, meas_cells.serving_cell().get_rsrq());
  ...
  if (meas_cells.nof_neighbours() > 0) {
    ...
    for (const auto& f : meas_cells) {
      ...
      meas.meas_result.rsrp_result         = rrc_value_to_range(isrran::quant_rsrp, f->get_rsrp());
      meas.meas_result.rsrq_result         = rrc_value_to_range(isrran::quant_rsrq, f->get_rsrq());
