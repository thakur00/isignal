...
#include "isrue/hdr/stack/rrc/rrc_cell.h"

namespace isrue {

meas_cell::meas_cell(isrran::unique_timer timer_) : timer(std::move(timer_))
{
  timer.set(neighbour_timeout_ms);
  timer.run();
}
meas_cell::meas_cell(const phy_cell_t& phy_cell_, isrran::unique_timer timer) : meas_cell(std::move(timer))
{
  phy_cell = phy_cell_;
}

isrran::plmn_id_t meas_cell_eutra::get_plmn(uint32_t idx) const
{
  if (idx < sib1.cell_access_related_info.plmn_id_list.size() && has_valid_sib1) {
    return isrran::make_plmn_id_t(sib1.cell_access_related_info.plmn_id_list[idx].plmn_id);
  } else {
    return {};
  }
}
...
bool meas_cell::has_sibs(isrran::span<const uint32_t> indexes) const
{
...
uint16_t meas_cell_eutra::get_mcc() const
{
  ...
  if (has_valid_sib1) {
    if (sib1.cell_access_related_info.plmn_id_list.size() > 0) {
      if (isrran::bytes_to_mcc(&sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mcc[0], &mcc)) {
        return mcc;
      }
    }
  }
...
uint16_t meas_cell_eutra::get_mnc() const
{
  ...
  if (has_valid_sib1) {
    if (sib1.cell_access_related_info.plmn_id_list.size() > 0) {
      if (isrran::bytes_to_mnc(&sib1.cell_access_related_info.plmn_id_list[0].plmn_id.mnc[0],
                               &mnc,
    ...
    if (sib1.cell_access_related_info.plmn_id_list.size() > 0) {
      ...
      if (sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list.size() > 0) {
        if (sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list[0].mcc_present) {
          if (isrran::bytes_to_mcc(&sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list[0].mcc[0], &mcc)) {
...
uint16_t meas_cell_nr::get_mnc() const
{
  ...
  if (has_valid_sib1) {
    if (sib1.cell_access_related_info.plmn_id_list.size() > 0) {
      if (sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list.size() > 0) {
        if (isrran::bytes_to_mnc(&sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list[0].mnc[0],
                                 &mnc,
 ...
 ********************************************/
template <class T>
meas_cell_list<T>::meas_cell_list(isrran::task_sched_handle task_sched_) :
