...
void rrc::si_acquire_proc::then(const isrran::proc_state_t& result)
{
...
rrc::serving_cell_config_proc::serving_cell_config_proc(rrc* parent_) :
  rrc_ptr(parent_), logger(isrlog::fetch_basic_logger("RRC"))
{}
...
isrran::proc_outcome_t rrc::serving_cell_config_proc::launch_sib_acquire()
{
...
isrran::proc_outcome_t rrc::cell_selection_proc::start_phy_cell_selection(const meas_cell_eutra& cell)
{
...
isrran::proc_outcome_t rrc::cell_selection_proc::start_sib_acquisition()
{
...
void rrc::cell_selection_proc::then(const isrran::proc_result_t<cs_result_t>& proc_result) const
{
...
rrc::plmn_search_proc::plmn_search_proc(rrc* parent_) : rrc_ptr(parent_), logger(isrlog::fetch_basic_logger("RRC")) {}
...
void rrc::plmn_search_proc::then(const isrran::proc_state_t& result) const
{
...
proc_outcome_t rrc::connection_request_proc::init(isrran::establishment_cause_t cause_,
                                                  isrran::unique_byte_buffer_t  dedicated_info_nas_)
{
  ...
  if (rrc_ptr->t302.is_running()) {
    ...
    rrc_ptr->nas->set_barring(isrran::barring_t::mo_data);
    return proc_outcome_t::error;
  }
...
void rrc::connection_request_proc::then(const isrran::proc_state_t& result)
{
  if (result.is_error()) {
...
isrran::proc_outcome_t rrc::connection_request_proc::react(const cell_selection_proc::cell_selection_complete_ev& e)
{
  if (state != state_t::cell_selection) {
...
rrc::connection_setup_proc::connection_setup_proc(isrue::rrc* parent_) :
  rrc_ptr(parent_), logger(isrlog::fetch_basic_logger("RRC"))
{}

isrran::proc_outcome_t rrc::connection_setup_proc::init(const asn1::rrc::rr_cfg_ded_s* cnfg_,
                                                        isrran::unique_byte_buffer_t   dedicated_info_nas_)
{
  Info("Starting...");
  ...
  rrc_ptr->nas->set_barring(isrran::barring_t::none);
...
isrran::proc_outcome_t rrc::connection_setup_proc::react(const bool& config_complete)
{
...
void rrc::connection_setup_proc::then(const isrran::proc_state_t& result)
{
  if (result.is_success()) {
...
rrc::connection_reconf_no_ho_proc::connection_reconf_no_ho_proc(isrue::rrc* parent_) : rrc_ptr(parent_) {}

isrran::proc_outcome_t rrc::connection_reconf_no_ho_proc::init(const asn1::rrc::rrc_conn_recfg_s& recfg_)
{
  Info("Starting...");
  ...
  if (rrc_ptr->reestablishment_successful) {
    for (int i = 2; i < ISRRAN_N_RADIO_BEARERS; i++) {
...
isrran::proc_outcome_t rrc::connection_reconf_no_ho_proc::react(const bool& config_complete)
{
  if (not config_complete) {
  ...
  isrran::unique_byte_buffer_t nas_pdu;
  for (auto& pdu : rx_recfg.ded_info_nas_list) {
    nas_pdu = isrran::make_byte_buffer();
    if (nas_pdu != nullptr) {
      memcpy(nas_pdu->msg, pdu.data(), pdu.size());
      nas_pdu->N_bytes = pdu.size();
      rrc_ptr->nas->write_pdu(isrran::srb_to_lcid(isrran::lte_srb::srb1), std::move(nas_pdu));
    } else {
...
void rrc::connection_reconf_no_ho_proc::then(const isrran::proc_state_t& result)
{
...
rrc::process_pcch_proc::process_pcch_proc(isrue::rrc* parent_) :
  rrc_ptr(parent_), logger(isrlog::fetch_basic_logger("RRC"))
{}
...
proc_outcome_t rrc::process_pcch_proc::step()
{
  if (state == state_t::next_record) {
    for (; paging_idx < paging.paging_record_list.size(); ++paging_idx) {
      isrran::s_tmsi_t s_tmsi_paged = isrran::make_s_tmsi_t(paging.paging_record_list[paging_idx].ue_id.s_tmsi());
      ...
      if (rrc_ptr->ue_identity == s_tmsi_paged) {
        if (RRC_STATE_IDLE == rrc_ptr->state) {
          ...
          isrran::console("S-TMSI match in paging message\n");
...
rrc::go_idle_proc::go_idle_proc(isrue::rrc* rrc_) : rrc_ptr(rrc_)
{
...
isrran::proc_outcome_t rrc::go_idle_proc::react(bool timeout)
{
...
void rrc::go_idle_proc::then(const isrran::proc_state_t& result)
{
...
rrc::cell_reselection_proc::cell_reselection_proc(isrue::rrc* rrc_) : rrc_ptr(rrc_)
{
...
void rrc::cell_reselection_proc::then(const isrran::proc_state_t& result)
{
...
rrc::connection_reest_proc::connection_reest_proc(isrue::rrc* rrc_) : rrc_ptr(rrc_), state(state_t::wait_cell_selection)
{}
...
proc_outcome_t rrc::connection_reest_proc::init(asn1::rrc::reest_cause_e cause)
{
  ...
  if (not rrc_ptr->security_is_activated or rrc_ptr->state != RRC_STATE_CONNECTED or crnti == ISRRAN_INVALID_RNTI) {
  ...
  for (int i = 1; i < ISRRAN_N_RADIO_BEARERS; i++) {
    if (rrc_ptr->rlc->has_bearer(i)) {
...
isrran::proc_outcome_t rrc::connection_reest_proc::cell_criteria()
{
...
isrran::proc_outcome_t rrc::connection_reest_proc::start_cell_selection()
{
...
isrran::proc_outcome_t rrc::connection_reest_proc::react(const asn1::rrc::rrc_conn_reest_s& reest_msg)
{
  // 1> stop timer T301;
  rrc_ptr->t301.stop();
...
isrran::proc_outcome_t rrc::connection_reest_proc::react(const t301_expiry& ev)
{
  Info("Timer T301 expired: Going to RRC IDLE");
  isrran::console("Timer T301 expired: Going to RRC IDLE\n");
  rrc_ptr->start_go_idle();

  return proc_outcome_t::error;
}
isrran::proc_outcome_t rrc::connection_reest_proc::step()
{
  if (rrc_ptr->t301.is_running() and not passes_cell_criteria()) {
    Info("Selected cell no longer suitable: Going to RRC IDLE");
    isrran::console("Selected cell no longer suitable: Going to RRC IDLE\n");
    rrc_ptr->start_go_idle();
    return proc_outcome_t::error;
  }
  return proc_outcome_t::yield;
...
isrran::proc_outcome_t rrc::connection_reest_proc::react(const asn1::rrc::rrc_conn_reest_reject_s& reject_msg)
{
  isrran::console("Reestablishment Reject. Going to RRC IDLE\n");
...
// 5.3.7.6 - T311 expiry
isrran::proc_outcome_t rrc::connection_reest_proc::react(const t311_expiry& ev)
{
...
rrc::ho_proc::ho_proc(isrue::rrc* rrc_) : rrc_ptr(rrc_) {}
 ...
 *
 */
isrran::proc_outcome_t rrc::ho_proc::init(const asn1::rrc::rrc_conn_recfg_s& rrc_reconf)
{
  Info("Starting...");
  ...
  isrran::console("Received HO command to target PCell=%d, NCC=%d\n",
  ...
  if (cell_to_ho != nullptr) {
    ...
    isrran::console("Received HO command to unknown PCI=%d\n", mob_ctrl_info->target_pci);
  ...
  if (recfg_r8.security_cfg_ho_present) {
    ...
    if (sec_intralte.security_algorithm_cfg_present) {
      rrc_ptr->sec_cfg.cipher_algo =
          (isrran::CIPHERING_ALGORITHM_ID_ENUM)sec_intralte.security_algorithm_cfg.ciphering_algorithm.to_number();
      rrc_ptr->sec_cfg.integ_algo =
          (isrran::INTEGRITY_ALGORITHM_ID_ENUM)sec_intralte.security_algorithm_cfg.integrity_prot_algorithm.to_number();
      Info("Changed Ciphering to %s and Integrity to %s",
           isrran::ciphering_algorithm_id_text[rrc_ptr->sec_cfg.cipher_algo],
           isrran::integrity_algorithm_id_text[rrc_ptr->sec_cfg.integ_algo]);
    }
  }
...
isrran::proc_outcome_t rrc::ho_proc::react(t304_expiry ev)
{
...
isrran::proc_outcome_t rrc::ho_proc::react(ra_completed_ev ev)
{
  if (ev.success) {
