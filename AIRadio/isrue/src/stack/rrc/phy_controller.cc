...
phy_controller::phy_controller(isrue::phy_interface_rrc_lte*                 phy_,
                               isrran::task_sched_handle                     task_sched_,
                               std::function<void(uint32_t, uint32_t, bool)> on_cell_selection) :
  base_t(isrlog::fetch_basic_logger("RRC")),
  phy(phy_),
...
bool phy_controller::set_cell_config(const isrran::phy_cfg_t& config, uint32_t cc_idx)
{
...
void phy_controller::set_phy_to_default()
{
  ...
  isrran::phy_cfg_t& default_cfg = current_cells_cfg[0];
...
void phy_controller::set_phy_to_default_pucch_isr()
{
  logger.info("Setting default PHY config dedicated");

  isrran::phy_cfg_t& default_cfg_ded = current_cells_cfg[0];
...
bool phy_controller::set_cell_config(const isrran::phy_cfg_t& cfg, uint32_t cc_idx, bool is_set)
{
...
bool phy_controller::start_cell_select(const phy_cell_t& phy_cell, isrran::event_observer<bool> observer)
{
...
bool phy_controller::start_cell_search(isrran::event_observer<cell_srch_res> observer, int earfcn)
{
