...
class mac_test : public isrue::mac_interface_rrc
{
public:
  isrran::task_sched_handle task_sched;
  isrue::rrc*               rrc_ptr;

  mac_test(isrue::rrc* rrc_, isrran::task_sched_handle task_sched_) : rrc_ptr(rrc_), task_sched(task_sched_) {}
  ...
  void set_config(isrran::mac_cfg_t& mac_cfg) override {}
  void set_config(isrran::sr_cfg_t& sr_cfg) override {}
...
class rrc_nr_test final : public isrue::rrc_nr_interface_rrc
{
public:
  ~rrc_nr_test() = default;
  int  get_eutra_nr_capabilities(isrran::byte_buffer_t* eutra_nr_caps) override { return ISRRAN_SUCCESS; };
  int  get_nr_capabilities(isrran::byte_buffer_t* nr_cap) override { return ISRRAN_SUCCESS; };
...
class nas_test : public isrue::nas
{
public:
  nas_test(isrran::task_sched_handle t) : isrue::nas(isrlog::fetch_basic_logger("NAS"), t) {}
  bool is_registered() override { return false; }
};

class pdcp_test : public isrran::pdcp
{
public:
  pdcp_test(const char* logname, isrran::task_sched_handle t) : isrran::pdcp(t, logname) {}
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu, int sn = -1) override
  {
...
class rrc_test : public isrue::rrc
{
...
int cell_select_test()
{
...

  {
    ...
    rrc_test         rrctest(isrlog::fetch_basic_logger("RRC_MEAS").id(), &stack);
    rrctest.init();
    rrctest.connect();
    ...
    TESTASSERT(rrctest.start_cell_select() == ISRRAN_SUCCESS);
...

  {
    ...
    cell_search_ret.found = isrue::rrc_interface_phy_lte::cell_search_ret_t::CELL_FOUND;
    TESTASSERT(rrctest.start_cell_select() == ISRRAN_SUCCESS);
...
int meas_obj_test()
{
  auto& rrc_meas_logger = isrlog::fetch_basic_logger("RRC_MEAS");
  ...
  std::vector<phy_meas_t> phy_meas = {};
  phy_meas.push_back({isrran::isrran_rat_t::lte, 0, 0, 0.0f, 1, 31});
  phy_meas.push_back({isrran::isrran_rat_t::lte, -1, 0, 0.0f, 1, 32});
  phy_meas.push_back({isrran::isrran_rat_t::lte, -2, 0, 0.0f, 1, 33});
  phy_meas.push_back({isrran::isrran_rat_t::lte, -3, 0, 0.0f, 1, 34});
  rrctest.new_cell_meas(phy_meas);
  rrctest.run_tti(1);
  phy_meas = {};
  phy_meas.push_back({isrran::isrran_rat_t::lte, -4, 0, 0.0f, 1, 35});
  phy_meas.push_back({isrran::isrran_rat_t::lte, -5, 0, 0.0f, 1, 36});
  phy_meas.push_back({isrran::isrran_rat_t::lte, -6, 0, 0.0f, 1, 37});
  phy_meas.push_back({isrran::isrran_rat_t::lte, 1, 0, 0.0f, 1, 30});
  phy_meas.push_back({isrran::isrran_rat_t::lte, 0, 0, 0.0f, 2, 31});
  rrctest.new_cell_meas(phy_meas);
  rrctest.run_tti(1);
                 ...
                 const std::vector<uint32_t> pci)
{
  ...
  for (uint32_t i = 0; i < pci.size(); i++) {
    ...
    phy_meas.push_back({isrran::isrran_rat_t::lte, r, -5, 0.0f, e, pci[i]});
  }
  rrctest.new_cell_meas(phy_meas);
  rrctest.run_tti(1);
}
...
int meas_obj_inter_rat_nr_test()
{
  ...
  for (int i = 0; i < ttt_iters; i++) {
    isrlog::fetch_basic_logger("RRC_MEAS").info("Report %d/%d enter condition is true", i, ttt_iters);
    rrctest.add_neighbour_cell_nr(500, 631680, -60.0);
  }
...
{
  auto& rrc_meas_logger = isrlog::fetch_basic_logger("RRC_MEAS", false);
  rrc_meas_logger.set_level(isrlog::basic_levels::debug);
  ...
  TESTASSERT(cell_select_test() == ISRRAN_SUCCESS);
  TESTASSERT(meas_obj_test() == ISRRAN_SUCCESS);
  TESTASSERT(meas_obj_inter_rat_nr_test() == ISRRAN_SUCCESS);
  TESTASSERT(
      a1event_report_test(
  ...
  TESTASSERT(
      ...
      ISRRAN_SUCCESS);
  TESTASSERT(a3event_report_test(6, 3, true) == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
