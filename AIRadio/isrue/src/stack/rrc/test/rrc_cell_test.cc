...
int test_add_neighbours()
{
  ...
  phy_meas_t pmeas;
  pmeas.rat    = isrran::isrran_rat_t::lte;
  pmeas.cfo_hz = 4;
  pmeas.rsrp   = -20;
  pmeas.pci    = 1;
  pmeas.earfcn = 3400;
  ...
  TESTASSERT(list.set_serving_cell(phy_cell_t{2, 3400}, false) == ISRRAN_SUCCESS);
  TESTASSERT(list.nof_neighbours() == 1);
...
int main()
{
  TESTASSERT(test_meas_cell() == ISRRAN_SUCCESS);
  TESTASSERT(test_add_neighbours() == ISRRAN_SUCCESS);
  printf("Success\n");
