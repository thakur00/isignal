...
int main()
{
  ...
  RRC_logger.set_level(isrlog::basic_levels::info);
  RRC_logger.set_hex_dump_max_size(-1);
  isrlog::init();

  TESTASSERT(isrue::test_phy_ctrl_fsm() == ISRRAN_SUCCESS);
  TESTASSERT(isrue::test_phy_cell_select_init_error_handling() == ISRRAN_SUCCESS);
