...
#include "isrran/interfaces/ue_usim_interfaces.h"
#include "isrue/hdr/stack/rrc/phy_controller.h"
#include "isrue/hdr/stack/rrc/rrc_meas.h"
#include "isrue/hdr/stack/rrc/rrc_procedures.h"
...
rrc::rrc(stack_interface_rrc* stack_, isrran::task_sched_handle task_sched_) :
  stack(stack_),
  task_sched(task_sched_),
  state(RRC_STATE_IDLE),
  last_state(RRC_STATE_CONNECTED),
  logger(isrlog::fetch_basic_logger("RRC")),
 ...
 */
void rrc::plmn_select(isrran::plmn_id_t plmn_id)
{
 ...
 */
bool rrc::connection_request(isrran::establishment_cause_t cause, isrran::unique_byte_buffer_t dedicated_info_nas_)
{
...
void rrc::set_ue_identity(isrran::s_tmsi_t s_tmsi)
{
  ...
  for (uint32_t i = 0; i < msg.c1().mbsfn_area_cfg_r9().pmch_info_list_r9.size(); i++) {
    ...
    for (uint32_t j = 0; j < pmch->mbms_session_info_list_r9.size(); j++) {
      ...
      if (serv == sess->tmgi_r9.service_id_r9.to_number()) {
        isrran::console("MBMS service started. Service id=%d, port=%d, lcid=%d\n", serv, port, sess->lc_ch_id_r9);
        ret = true;
...
void rrc::radio_link_failure_process()
{
  ...
  isrran::console("Warning: Detected Radio-Link Failure\n");
...
/* Reception of PUCCH/ISR release procedure (Section 5.3.13) */
void rrc::release_pucch_isr()
{
  // Apply default configuration for PUCCH (CQI and SR) and ISR (release)
  if (initiated) {
    set_phy_default_pucch_isr();
  }
}
...
void rrc::timer_expired(uint32_t timeout_id)
{
  if (timeout_id == t310.id()) {
    ...
    isrran::console("Timer T311 expired: Going to RRC IDLE\n");
    ...
    isrran::console("Timer t304 expired: Handover failed\n");
...
bool rrc::nr_reconfiguration_proc(const rrc_conn_recfg_r8_ies_s& rx_recfg, bool* has_5g_nr_reconfig)
{
  ...
  switch (rrc_conn_recfg_v1510_ies->nr_cfg_r15.type()) {
    ...
    case setup_opts::options::setup:
      ...
      if (rrc_conn_recfg_v1510_ies->nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15_present) {
        ...
        if (secondary_cell_group_r15.unpack(bref0) != ISRASN_SUCCESS) {
        ...
        if (secondary_cell_group_r15.crit_exts.rrc_recfg().secondary_cell_group.size() > 0) {
          ...
          if (cell_group_cfg.unpack(bref1) != ISRASN_SUCCESS) {
  ...
  if (rrc_conn_recfg_v1510_ies->nr_radio_bearer_cfg1_r15_present) {
    ...
    if (radio_bearer_conf.unpack(bref) != ISRASN_SUCCESS) {
...
void rrc::send_con_request(isrran::establishment_cause_t cause)
{
  ...
  if (ue_identity_configured) {
    ...
    isrran::to_asn1(&rrc_conn_req->ue_id.s_tmsi(), ue_identity);
  } else {
...
void rrc::send_con_restablish_request(reest_cause_e cause, uint16_t crnti, uint16_t pci, uint32_t cellid)
{
  ...
  isrran::console("RRC Connection Reestablishment to PCI=%d, EARFCN=%d (Cause: \"%s\")\n",
...
void rrc::send_con_setup_complete(isrran::unique_byte_buffer_t nas_msg)
{
...
void rrc::handle_rrc_connection_release(const asn1::rrc::rrc_conn_release_s& release)
{
  ...
  isrran::console("Received RRC Connection Release (releaseCause: %s)\n", cause.c_str());
  ...
  if (release.crit_exts.c1().rrc_conn_release_r8().redirected_carrier_info_present) {
    switch (release.crit_exts.c1().rrc_conn_release_r8().redirected_carrier_info.type()) {
      case asn1::rrc::redirected_carrier_info_c::types_opts::options::eutra:
        ...
        break;
      default:
        isrran::console("Ignoring RedirectedCarrierInfo with unsupported type (%s)\n",
                        release.crit_exts.c1().rrc_conn_release_r8().redirected_carrier_info.type().to_string());
        break;
    }
    if (earfcn != 0) {
      isrran::console("RedirectedCarrierInfo present (type %s, earfcn: %d) - Redirecting\n",
                      release.crit_exts.c1().rrc_conn_release_r8().redirected_carrier_info.type().to_string(),
                      earfcn);
...
void rrc::write_pdu_bcch_bch(unique_byte_buffer_t pdu)
{
  ...
  asn1::ISRASN_CODE err = bch_msg.unpack(bch_bref);

  if (err != asn1::ISRASN_SUCCESS) {
...
void rrc::handle_sib2()
{
  ...
  if (sib2->mbsfn_sf_cfg_list_present) {
    isrran::mbsfn_sf_cfg_t list[ASN1_RRC_MAX_MBSFN_ALLOCS];
    for (uint32_t i = 0; i < sib2->mbsfn_sf_cfg_list.size(); ++i) {
      list[i] = isrran::make_mbsfn_sf_cfg(sib2->mbsfn_sf_cfg_list[i]);
    }
    phy->set_config_mbsfn_sib2(&list[0], sib2->mbsfn_sf_cfg_list.size());
  }

  // Apply PHY RR Config Common
  isrran::phy_cfg_t& current_pcell = phy_ctrl->current_cell_config()[0];
  ...
  set_phy_cfg_t_common_isr(&current_pcell, sib2->rr_cfg_common.isr_ul_cfg_common);
...
void rrc::handle_sib13()
{
  ...
  phy->set_config_mbsfn_sib13(isrran::make_sib13(*sib13));
...
{
  if (pdu->N_bytes <= 0 or pdu->N_bytes >= ISRRAN_MAX_BUFFER_SIZE_BITS) {
  ...
  if (pcch_msg.unpack(bref) != asn1::ISRASN_SUCCESS or pcch_msg.msg.type().value != pcch_msg_type_c::types_opts::c1) {
...
void rrc::write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  if (pdu->N_bytes <= 0 or pdu->N_bytes >= ISRRAN_MAX_BUFFER_SIZE_BITS) {
    return;
  }
...
void rrc::parse_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  asn1::cbit_ref bref(pdu->msg, pdu->N_bytes);
  if (meas_cells.serving_cell().mcch.unpack(bref) != asn1::ISRASN_SUCCESS or
  ...
  phy->set_config_mbsfn_mcch(isrran::make_mcch_msg(meas_cells.serving_cell().mcch));
  log_rrc_message(
...
void rrc::send_ul_ccch_msg(const ul_ccch_msg_s& msg)
{
  ...
  unique_byte_buffer_t pdcp_buf = isrran::make_byte_buffer();
  if (pdcp_buf == nullptr) {
...
void rrc::write_sdu(isrran::unique_byte_buffer_t sdu)
{
  if (state == RRC_STATE_IDLE) {
...
void rrc::process_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
void rrc::parse_dl_ccch(unique_byte_buffer_t pdu)
{
  ...
  switch (dl_ccch_msg.msg.c1().type().value) {
    case dl_ccch_msg_type_c::c1_c_::types::rrc_conn_reject: {
      ...
      isrran::console("Received ConnectionReject. Wait time: %d\n", reject_r8->wait_time);
    ...
    case dl_ccch_msg_type_c::c1_c_::types::rrc_conn_reest: {
      isrran::console("Reestablishment OK\n");
...
void rrc::handle_ue_capability_enquiry(const ue_cap_enquiry_s& enquiry)
{
  ...
  for (uint32_t i = 0; i < enquiry.crit_exts.c1().ue_cap_enquiry_r8().ue_cap_request.size(); i++) {
    if (enquiry.crit_exts.c1().ue_cap_enquiry_r8().ue_cap_request[i] == rat_type_e::eutra) {
      ...
      if (args.release < ISRRAN_RELEASE_MIN || args.release > ISRRAN_RELEASE_MAX) {
        uint32_t new_release = ISRRAN_MIN(ISRRAN_RELEASE_MAX, ISRRAN_MAX(ISRRAN_RELEASE_MIN, args.release));
      ...
      if (args.ue_category < ISRRAN_UE_CATEGORY_MIN || args.ue_category > ISRRAN_UE_CATEGORY_MAX) {
        uint32_t new_category =
            ISRRAN_MIN(ISRRAN_UE_CATEGORY_MAX, ISRRAN_MAX(ISRRAN_UE_CATEGORY_MIN, args.ue_category));
        logger.error("UE Category is %d. It is out of bounds (%d ... %d), setting it to %d",
                     args.ue_category,
                     ISRRAN_UE_CATEGORY_MIN,
                     ISRRAN_UE_CATEGORY_MAX,
                     new_category);
        args.ue_category = new_category;
      }

      ue_eutra_cap_s cap;
      cap.access_stratum_release = (access_stratum_release_e::options)(args.release - ISRRAN_RELEASE_MIN);
      ...
      if (cap.pack(bref) != asn1::ISRASN_SUCCESS) {
...
void rrc::log_rr_config_common()
{
  ...
  const isrran::phy_cfg_t& current_pcell = phy_ctrl->current_cell_config()[0];
  ...
  if (current_pcell.ul_cfg.isr.configured) {
    logger.info("Set ISR ConfigCommon: BW-Configuration=%d, SF-Configuration=%d, Simult-ACKNACK=%s",
                current_pcell.ul_cfg.isr.bw_cfg,
                current_pcell.ul_cfg.isr.subframe_config,
                current_pcell.ul_cfg.isr.simul_ack ? "yes" : "no");
  }
}
...
void rrc::apply_rr_config_common(rr_cfg_common_s* config, bool send_lower_layers)
{
  ...
  if (config->isr_ul_cfg_common_present) {
    set_phy_cfg_t_common_isr(&current_pcell, config->isr_ul_cfg_common);
  }
...
void rrc::log_phy_config_dedicated()
{
  ...
  if (current_pcell.ul_cfg.isr.configured) {
    logger.info("Set PHY config ded: ISR-ConfigIndex=%d, ISR-bw=%d, ISR-Nrcc=%d, ISR-hop=%d, ISR-Ncs=%d",
                current_pcell.ul_cfg.isr.I_isr,
                current_pcell.ul_cfg.isr.B,
                current_pcell.ul_cfg.isr.n_rrc,
                current_pcell.ul_cfg.isr.b_hop,
                current_pcell.ul_cfg.isr.n_isr);
  }
}
...
void rrc::apply_phy_scell_config(const scell_to_add_mod_r10_s& scell_config, bool enable_cqi)
{
  ...
  if (scell_config.rr_cfg_common_scell_r10_present) {
    ...
    scell.frame_type                            = (rr_cfg->tdd_cfg_v1130.is_present()) ? ISRRAN_TDD : ISRRAN_FDD;
    ...
    switch (non_ul_cfg->phich_cfg_r10.phich_res.value) {
      case phich_cfg_s::phich_res_opts::one_sixth:
        scell.phich_resources = ISRRAN_PHICH_R_1_6;
        break;
      case phich_cfg_s::phich_res_opts::half:
        scell.phich_resources = ISRRAN_PHICH_R_1_2;
        break;
      case phich_cfg_s::phich_res_opts::one:
        scell.phich_resources = ISRRAN_PHICH_R_1;
        break;
      case phich_cfg_s::phich_res_opts::two:
      case phich_cfg_s::phich_res_opts::nulltype:
        scell.phich_resources = ISRRAN_PHICH_R_2;
        break;
    }
  }

  // Initialize scell config with pcell cfg
  isrran::phy_cfg_t scell_cfg = phy_ctrl->current_cell_config()[0];
 ...
 * @param cnfg
 * @param is_handover - whether the SR, CQI, ISR, measurement configs take effect immediately (see TS
 * 36.331 5.3.5.4)
 * @return
 */
...
void rrc::add_drb(const drb_to_add_mod_s& drb_cnfg)
{
  ...
  if (drb_cnfg.lc_ch_id_present) {
    ...
    lcid = isrran::MAX_LTE_SRB_ID + drb_cnfg.drb_id;
  ...
  stack->add_eps_bearer(eps_bearer_id, isrran::isrran_rat_t::lte, lcid);
...
void rrc::set_phy_default_pucch_isr()
{
  if (phy_ctrl != nullptr) {
    phy_ctrl->set_phy_to_default_pucch_isr();
  } else {
    logger.info("RRC not initialized. Skipping default PUCCH/ISR config.");
  }
