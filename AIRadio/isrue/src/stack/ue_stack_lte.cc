...
ue_stack_lte::ue_stack_lte() :
  args(),
  stack_logger(isrlog::fetch_basic_logger("STCK", false)),
  mac_logger(isrlog::fetch_basic_logger("MAC")),
  rlc_logger(isrlog::fetch_basic_logger("RLC", false)),
  pdcp_logger(isrlog::fetch_basic_logger("PDCP", false)),
  rrc_logger(isrlog::fetch_basic_logger("RRC", false)),
  usim_logger(isrlog::fetch_basic_logger("USIM", false)),
  nas_logger(isrlog::fetch_basic_logger("NAS", false)),
  nas5g_logger(isrlog::fetch_basic_logger("NAS5G", false)),
  mac_nr_logger(isrlog::fetch_basic_logger("MAC-NR")),
  rrc_nr_logger(isrlog::fetch_basic_logger("RRC-NR", false)),
  rlc_nr_logger(isrlog::fetch_basic_logger("RLC-NR", false)),
  pdcp_nr_logger(isrlog::fetch_basic_logger("PDCP-NR", false)),
  mac_pcap(),
  mac_nr_pcap(),
  rlc("RLC"),
  mac("MAC", &task_sched),
  ...
  nas(isrlog::fetch_basic_logger("NAS", false), &task_sched),
  nas_5g(isrlog::fetch_basic_logger("NAS5G", false), &task_sched),
  thread("STACK"),
  task_sched(512, 64),
...
int ue_stack_lte::init(const stack_args_t& args_)
{
  ...
  // init own log
  stack_logger.set_level(isrlog::str_to_basic_level(args.log.stack_level));
  stack_logger.set_hex_dump_max_size(args.log.stack_hex_limit);
  byte_buffer_pool::get_instance()->enable_logger(true);

  // init layer logs
  mac_logger.set_level(isrlog::str_to_basic_level(args.log.mac_level));
  mac_logger.set_hex_dump_max_size(args.log.mac_hex_limit);
  rlc_logger.set_level(isrlog::str_to_basic_level(args.log.rlc_level));
  rlc_logger.set_hex_dump_max_size(args.log.rlc_hex_limit);
  pdcp_logger.set_level(isrlog::str_to_basic_level(args.log.pdcp_level));
  pdcp_logger.set_hex_dump_max_size(args.log.pdcp_hex_limit);
  rrc_logger.set_level(isrlog::str_to_basic_level(args.log.rrc_level));
  rrc_logger.set_hex_dump_max_size(args.log.rrc_hex_limit);
  usim_logger.set_level(isrlog::str_to_basic_level(args.log.usim_level));
  usim_logger.set_hex_dump_max_size(args.log.usim_hex_limit);
  nas_logger.set_level(isrlog::str_to_basic_level(args.log.nas_level));
  nas_logger.set_hex_dump_max_size(args.log.nas_hex_limit);

  nas5g_logger.set_level(isrlog::str_to_basic_level(args.log.nas_level));
  nas5g_logger.set_hex_dump_max_size(args.log.nas_hex_limit);
  mac_nr_logger.set_level(isrlog::str_to_basic_level(args.log.mac_level));
  mac_nr_logger.set_hex_dump_max_size(args.log.mac_hex_limit);
  rrc_nr_logger.set_level(isrlog::str_to_basic_level(args.log.rrc_level));
  rrc_nr_logger.set_hex_dump_max_size(args.log.rrc_hex_limit);
  pdcp_nr_logger.set_level(isrlog::str_to_basic_level(args.log.pdcp_level));
  pdcp_nr_logger.set_hex_dump_max_size(args.log.pdcp_hex_limit);
  rlc_nr_logger.set_level(isrlog::str_to_basic_level(args.log.rlc_level));
  rlc_nr_logger.set_hex_dump_max_size(args.log.rlc_hex_limit);

  // Set up pcap
  // parse pcap trace list
  std::vector<std::string> pcap_list;
  isrran::string_parse_list(args.pkt_trace.enable, ',', pcap_list);
  if (pcap_list.empty()) {
  ...
  if (args.pkt_trace.mac_pcap.enable && args.pkt_trace.mac_nr_pcap.enable &&
    ...
    if (mac_pcap.open(args.pkt_trace.mac_pcap.filename.c_str()) == ISRRAN_SUCCESS) {
    ...
    if (args.pkt_trace.mac_nr_pcap.enable) {
      if (mac_nr_pcap.open(args.pkt_trace.mac_nr_pcap.filename.c_str()) == ISRRAN_SUCCESS) {
  ...
  if (args.pkt_trace.nas_pcap.enable) {
    if (nas_pcap.open(args.pkt_trace.nas_pcap.filename.c_str()) == ISRRAN_SUCCESS) {
  ...
  if (usim->init(&args.usim)) {
    isrran::console("Failed to initialize USIM.\n");
    return ISRRAN_ERROR;
  }
...
bool ue_stack_lte::enable_data()
{
  if (running) {
    ue_task_queue.try_push([this]() {
      ...
      isrran::console("Turning off airplane mode.\n");
      nas.enable_data();
    });
  }
  return true;
...
bool ue_stack_lte::disable_data()
{
  if (running) {
    ue_task_queue.try_push([this]() {
      ...
      isrran::console("Turning on airplane mode.\n");
      nas.disable_data();
    });
  }
  return true;
  ...
  if (running) {
    ue_task_queue.try_push([this]() {
      if (args.sa_mode) {
        ...
        nas.start_service_request(isrran::establishment_cause_t::mo_data);
      }
    });
  }
  return true;
...
void ue_stack_lte::add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid)
{
 ...
 * @param sdu
 */
void ue_stack_lte::write_sdu(uint32_t eps_bearer_id, isrran::unique_byte_buffer_t sdu)
{
  auto bearer = bearers.get_radio_bearer(eps_bearer_id);

  auto task   = [this, eps_bearer_id, bearer](isrran::unique_byte_buffer_t& sdu) {
    // route SDU to PDCP entity
    if (bearer.rat == isrran_rat_t::lte) {
      pdcp.write_sdu(bearer.lcid, std::move(sdu));
    } else if (bearer.rat == isrran_rat_t::nr) {
      if (args.sa_mode) {
