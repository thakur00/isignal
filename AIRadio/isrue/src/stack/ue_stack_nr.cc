...
ue_stack_nr::ue_stack_nr() :
  ...
  rlc_logger(isrlog::fetch_basic_logger("RLC-NR", false)),
  pdcp_logger(isrlog::fetch_basic_logger("PDCP-NR", false))
{
  get_background_workers().set_nof_workers(2);
  mac.reset(new mac_nr(&task_sched));
  pdcp.reset(new isrran::pdcp(&task_sched, "PDCP-NR"));
  rlc.reset(new isrran::rlc("RLC-NR"));
 ...
 * @param blocking
 */
void ue_stack_nr::write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
  if (pdcp != nullptr) {
    auto ret = gw_task_queue.try_push(std::bind(
        [this, lcid](isrran::unique_byte_buffer_t& sdu) { pdcp->write_sdu(lcid, std::move(sdu)); }, std::move(sdu)));
    if (ret.is_error()) {
