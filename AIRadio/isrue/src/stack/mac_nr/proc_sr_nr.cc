...
#include "isrue/hdr/stack/mac_nr/proc_sr_nr.h"
...
proc_sr_nr::proc_sr_nr(isrlog::basic_logger& logger) : logger(logger) {}
...
int32_t proc_sr_nr::set_config(const isrran::sr_cfg_nr_t& cfg_)
{
  {
    std::lock_guard<std::mutex> lock(mutex);
    // disable by default
...
void proc_sr_nr::step(uint32_t tti)
{
  ...
  if (sr_counter >= cfg.item[0].trans_max) {
    logger.info(
        "SR:    Releasing PUCCH/ISR resources, sr_counter=%d, sr-TransMax=%d", sr_counter, cfg.item[0].trans_max);
    isrran::console("Scheduling request failed: releasing RRC connection...\n");
    rrc->release_pucch_isr();
