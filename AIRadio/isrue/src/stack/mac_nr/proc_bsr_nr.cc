...
int proc_bsr_nr::set_config(const isrran::bsr_cfg_nr_t& bsr_cfg_)
{
  std::lock_guard<std::mutex> lock(mutex);
 ...
 */
isrran::mac_sch_subpdu_nr::lcg_bsr_t proc_bsr_nr::generate_sbsr()
{
  std::lock_guard<std::mutex>          lock(mutex);
  isrran::mac_sch_subpdu_nr::lcg_bsr_t sbsr = {};
...
void proc_bsr_nr::set_padding_bytes(uint32_t nof_bytes)
{
  ...
  if (nof_bytes >= (SBSR_CE_SUBHEADER_LEN +
                    isrran::mac_sch_subpdu_nr::sizeof_ce(isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::SHORT_BSR, true)) &&
      nof_bytes < (LBSR_CE_SUBHEADER_LEN +
                   isrran::mac_sch_subpdu_nr::sizeof_ce(isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::LONG_BSR, true))) {
    // generate short padding BSR
    mux->generate_bsr_mac_ce(SHORT_BSR);
  } else if (nof_bytes >= (LBSR_CE_SUBHEADER_LEN + isrran::mac_sch_subpdu_nr::sizeof_ce(
                                                       isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::LONG_BSR, true))) {
    // report Long BSR if more than one LCG has data to send
    mux->generate_bsr_mac_ce(LONG_BSR);
  }
}
...
int proc_bsr_nr::setup_lcid(uint32_t lcid, uint32_t new_lcg, uint32_t priority)
{
  ...
  if (new_lcg > isrran::mac_sch_subpdu_nr::max_num_lcg_lbsr) {
