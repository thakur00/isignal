...
#include "isrue/hdr/stack/mac_nr/ul_harq_nr.h"
...
ul_harq_entity_nr::ul_harq_entity_nr(const uint8_t          cc_idx_,
                                     ...
                                     proc_ra_nr*            ra_proc_,
                                     mux_nr*                mux_) :
  mac(mac_), mux(mux_), logger(isrlog::fetch_basic_logger("MAC"))
{}
...
void ul_harq_entity_nr::set_config(isrran::ul_harq_cfg_t& harq_cfg_)
{
  harq_cfg = harq_cfg_;
}
                                     ...
                                     mac_interface_phy_nr::tb_action_ul_t*          action)
{
  ...
  isrran_expect(action->tb.enabled ? action->tb.payload != nullptr : true,
...
ul_harq_entity_nr::ul_harq_process_nr::ul_harq_process_nr() :
  logger(isrlog::fetch_basic_logger("MAC")), softbuffer({0, 0, nullptr})
{}
