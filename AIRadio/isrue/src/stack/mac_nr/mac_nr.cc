...
#include "isrue/hdr/stack/mac_nr/mac_nr.h"
...
#include "isrue/hdr/stack/mac_nr/proc_ra_nr.h"

namespace isrue {

mac_nr::mac_nr(isrran::ext_task_sched_handle task_sched_) :
                 ...
                 rrc_interface_mac*    rrc_)
{
  ...
  if (proc_bsr.init(&proc_sr, &mux, rlc, &task_sched) != ISRRAN_SUCCESS) {
    logger.error("Couldn't initialize BSR procedure.");
    return ISRRAN_ERROR;
  }

  if (mux.init(rlc) != ISRRAN_SUCCESS) {
    logger.error("Couldn't initialize mux unit.");
    return ISRRAN_ERROR;
  }

  if (demux.init(rlc, phy) != ISRRAN_SUCCESS) {
    logger.error("Couldn't initialize demux unit.");
    return ISRRAN_ERROR;
  }

  // Configure PCell HARQ entities
  if (ul_harq.at(PCELL_CC_IDX)->init() != ISRRAN_SUCCESS) {
...
void mac_nr::start_pcap(isrran::mac_pcap* pcap_)
{
  pcap = pcap_;
}
...
mac_interface_phy_nr::sched_rnti_t mac_nr::get_ul_sched_rnti_nr(const uint32_t tti)
{
  if (has_temp_crnti() && has_crnti() == false) {
    ...
    return {rntis.get_temp_rnti(), isrran_rnti_type_c};
  }
  return {rntis.get_crnti(), isrran_rnti_type_c};
}
...
mac_interface_phy_nr::sched_rnti_t mac_nr::get_dl_sched_rnti_nr(const uint32_t tti)
{
  ...
  if (is_si_opportunity()) {
    return {ISRRAN_SIRNTI, isrran_rnti_type_si};
  }

  if (is_paging_opportunity()) {
    return {ISRRAN_PRNTI, isrran_rnti_type_si};
  }

  if (proc_ra.is_rar_opportunity(tti)) {
    return {proc_ra.get_rar_rnti(), isrran_rnti_type_ra};
  }
  ...
  if (has_crnti()) {
    ...
    return {get_crnti(), isrran_rnti_type_c};
  }

  // turn off DCI search for this TTI
  return {ISRRAN_INVALID_RNTI, isrran_rnti_type_c};
}

bool mac_nr::has_temp_crnti()
{
  return rntis.get_temp_rnti() != ISRRAN_INVALID_RNTI;
}
...
{
  return rntis.get_crnti() != ISRRAN_INVALID_RNTI;
}
...
isrran::mac_sch_subpdu_nr::lcg_bsr_t mac_nr::generate_sbsr()
{
...
void mac_nr::bch_decoded_ok(uint32_t tti, isrran::unique_byte_buffer_t payload)
{
  // Send MIB to RLC
...
void mac_nr::write_pcap(const uint32_t cc_idx, const mac_nr_grant_dl_t& grant, tb_action_dl_result_t& tb)
{
  if (pcap) {
    if (tb.ack && tb.payload != nullptr) {
      ...
      } else if (grant.rnti == ISRRAN_PRNTI) {
...
int mac_nr::setup_lcid(const isrran::logical_channel_config_t& config)
{
  if (mux.setup_lcid(config) != ISRRAN_SUCCESS) {
    logger.error("Couldn't register logical channel at MUX unit.");
    return ISRRAN_ERROR;
  }

  if (proc_bsr.setup_lcid(config.lcid, config.lcg, config.priority) != ISRRAN_SUCCESS) {
...
int mac_nr::add_tag_config(const isrran::tag_cfg_nr_t& tag_cfg)
{
...
int mac_nr::set_config(const isrran::phr_cfg_nr_t& phr_cfg)
{
  logger.info("Add phr config not supported yet");
  return ISRRAN_SUCCESS;
}

int mac_nr::set_config(const isrran::dl_harq_cfg_nr_t& dl_hrq_cfg)
{
  for (const auto& cc : dl_harq) {
    if (cc != nullptr) {
      if (cc->set_config(dl_hrq_cfg) != ISRRAN_SUCCESS) {
...
int mac_nr::set_config(const isrran::bsr_cfg_nr_t& bsr_cfg)
{
  return proc_bsr.set_config(bsr_cfg);
}

int mac_nr::set_config(const isrran::sr_cfg_nr_t& sr_cfg)
{
  return proc_sr.set_config(sr_cfg);
}

void mac_nr::set_config(const isrran::rach_cfg_nr_t& rach_cfg)
{
...
void mac_nr::get_metrics(mac_metrics_t m[ISRRAN_MAX_CARRIERS])
{
  int   tx_pkts          = 0;
  int   tx_errors        = 0;
  int   tx_brate         = 0;
  ...
  memcpy(m, metrics.data(), sizeof(mac_metrics_t) * ISRRAN_MAX_CARRIERS);
  metrics = {};
}
...
bool mac_nr::is_in_window(uint32_t tti, int* start, int* len)
{
  ...
  if (isrran_tti_interval(tti, st) < l + 5) {
    if (tti > st) {
      if (tti <= st + l) {
        return true;
      } else {
