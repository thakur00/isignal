...
{
public:
  dummy_mac() : logger(isrlog::fetch_basic_logger("MAC")) {}
...
private:
  uint16_t              crnti = ISRRAN_INVALID_RNTI;
...
int proc_ra_normal_test()
{
  ...
  proc_ra_nr proc_ra_nr(dummy_mac, isrlog::fetch_basic_logger("MAC"));
  ...
  result.payload                                     = isrran::make_byte_buffer();
...
int main()
{
  ...
  // TESTASSERT(proc_ra_normal_test() == ISRRAN_SUCCESS);
  TESTASSERT(proc_ra_timeout_test() == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
