...
public:
  ...
  int set_rar_grant(uint32_t rar_slot_idx,
                    std::array<uint8_t, ISRRAN_RAR_UL_GRANT_NBITS>,
                    uint16_t           rnti,
...
public:
  ...
  virtual void release_pucch_isr() {}
};
...
int mac_nr_ul_logical_channel_prioritization_test1()
{
  ...
  std::vector<isrran::logical_channel_config_t> lcids;
  isrran::logical_channel_config_t              config = {};
  config.lcid                                          = 4;
  config.lcg                                           = 6;
  config.PBR                                           = 0;
  ...
  // Check UL metrics
  mac_metrics_t metrics[ISRRAN_MAX_CARRIERS] = {};
...
int mac_nr_ul_periodic_bsr_test()
{
  ...
  TESTASSERT(mac.set_config(bsr_cfg) == ISRRAN_SUCCESS);
...
int mac_nr_dl_retx_test()
{
  ...
  TESTASSERT(mac.set_config(harq_cfg) == ISRRAN_SUCCESS);
  ...
  // create DL grant for MAC
  {
    ...
    dl_result.ack                                         = true;
    dl_result.payload                                     = isrran::make_byte_buffer();
...
int main()
{
  ...
  TESTASSERT(msg3_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_nr_ul_logical_channel_prioritization_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_nr_ul_logical_channel_prioritization_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_nr_ul_logical_channel_prioritization_test3() == ISRRAN_SUCCESS);
  TESTASSERT(mac_nr_ul_logical_channel_prioritization_test4() == ISRRAN_SUCCESS);

  TESTASSERT(mac_nr_ul_periodic_bsr_test() == ISRRAN_SUCCESS);
  TESTASSERT(mac_nr_dl_retx_test() == ISRRAN_SUCCESS);
