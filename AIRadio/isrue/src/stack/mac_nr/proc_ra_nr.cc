...
proc_ra_nr::proc_ra_nr(mac_interface_proc_ra_nr& mac_, isrlog::basic_logger& logger_) : mac(mac_), logger(logger_) {}

void proc_ra_nr::init(phy_interface_mac_nr* phy_, isrran::ext_task_sched_handle* task_sched_)
{
  phy                         = phy_;
...
void proc_ra_nr::set_config(const isrran::rach_cfg_nr_t& rach_cfg_)
{
  if (state != IDLE) {
    logger.warning("Wrong state for ra reponse reception %s (expected state %s)",
                   isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, state),
                   isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, IDLE));
    return;
  }
  rach_cfg   = rach_cfg_;
  configured = true;
...
{
  std::lock_guard<std::mutex> lock(mutex);
  if (rar_rnti == ISRRAN_INVALID_RNTI || state != WAITING_FOR_RESPONSE_RECEPTION) {
...
{
  std::lock_guard<std::mutex> lock(mutex);
  if (rar_rnti != ISRRAN_INVALID_RNTI) {
    return true;
  }
  return false;
}
...
void proc_ra_nr::ra_response_reception(const mac_interface_phy_nr::tb_action_dl_result_t& tb)
{
  ...
  if (state != WAITING_FOR_RESPONSE_RECEPTION) {
    logger.warning(
        ...
        isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, state),
        isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, WAITING_FOR_RESPONSE_RECEPTION));
    return;
  }
  ...
  if (tb.ack && tb.payload != nullptr) {
    ...
    for (auto& subpdu : pdu.get_subpdus()) {
      if (subpdu.has_rapid() && subpdu.get_rapid() == preamble_index) {
        ...
        phy->set_rar_grant(tb.rx_slot_idx, subpdu.get_ul_grant(), subpdu.get_temp_crnti(), isrran_rnti_type_ra);
...
void proc_ra_nr::ra_contention_resolution(bool received_con_res_matches_ue_id)
{
  if (state != WAITING_FOR_CONTENTION_RESOLUTION) {
    logger.warning(
        ...
        isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, state),
        isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, WAITING_FOR_CONTENTION_RESOLUTION));
    return;
  }
...
void proc_ra_nr::ra_completion()
{
  if (state != WAITING_FOR_COMPLETION) {
    logger.warning("Wrong state for ra completion by phy %s (expected state %s)",
                   isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, state),
                   isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, WAITING_FOR_COMPLETION));
    return;
  }
  ...
  if (!mac.get_crnti()) {
    ...
    mac.set_temp_crnti(ISRRAN_INVALID_RNTI);
  }

  isrran::console("Random Access Complete.     c-rnti=0x%x, ta=%d\n", mac.get_crnti(), current_ta);
  ...
  task_queue.push([this, tti, s_id, t_id, f_id, ul_carrier_id]() {
    if (state != WAITING_FOR_PRACH_SENT) {
      logger.warning("Wrong state for prach sent notification by phy %s (expected state %s)",
                     isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, state),
                     isrran::enum_to_text(state_str_nr, (uint32_t)ra_state_t::MAX_RA_STATES, WAITING_FOR_PRACH_SENT));
      return;
    }
    ...
    logger.info(
        ...
        f_id,
        ul_carrier_id);
    isrran::console("Random Access Transmission: prach_occasion=%d, preamble_index=%d, ra-rnti=0x%x, tti=%d\n",
                    prach_occasion,
                    preamble_index,
                    rar_rnti,
                    tti);
