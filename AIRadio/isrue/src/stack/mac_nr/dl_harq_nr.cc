...
#include "isrue/hdr/stack/mac_nr/dl_harq_nr.h"
...
dl_harq_entity_nr::dl_harq_entity_nr(uint8_t                  cc_idx_,
  ...
  logger(isrlog::fetch_basic_logger("MAC-NR")), cc_idx(cc_idx_), mac(mac_), demux_unit(demux_unit_), bcch_proc(this)
{
...
int32_t dl_harq_entity_nr::set_config(const isrran::dl_harq_cfg_nr_t& cfg_)
{
  isrran::rwlock_write_guard lock(rwlock);
  if (cfg_.nof_procs < 1 || cfg_.nof_procs > ISRRAN_MAX_HARQ_PROC_DL_NR) {
...
dl_harq_entity_nr::dl_harq_process_nr::dl_harq_process_nr(dl_harq_entity_nr* parent_) :
  harq_entity(parent_),
  softbuffer_rx(std::unique_ptr<isrran_softbuffer_rx_t>(new isrran_softbuffer_rx_t())),
...
dl_harq_entity_nr::dl_harq_process_nr::~dl_harq_process_nr()
{
  if (softbuffer_rx != nullptr) {
    isrran_softbuffer_rx_free(softbuffer_rx.get());
  }
}

bool dl_harq_entity_nr::dl_harq_process_nr::init(int pid_)
{
  if (softbuffer_rx == nullptr || isrran_softbuffer_rx_init_guru(softbuffer_rx.get(),
                                                                 ISRRAN_SCH_NR_MAX_NOF_CB_LDPC,
                                                                 ISRRAN_LDPC_MAX_LEN_ENCODED_CB) != ISRRAN_SUCCESS) {
                                                         ...
                                                         mac_interface_phy_nr::tb_action_dl_t* action)
{
      ...
      is_first_tb)                  // 3rd condition (is first tx for this tb)
  {
    // New transmission
    n_retx = 0;
    isrran_softbuffer_rx_reset_tbs(softbuffer_rx.get(), grant.tbs * 8);
