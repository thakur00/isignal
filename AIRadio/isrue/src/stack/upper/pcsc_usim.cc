...
pcsc_usim::pcsc_usim(isrlog::basic_logger& logger) : usim_base(logger), sc(logger) {}
...
int pcsc_usim::init(usim_args_t* args)
{
  ...
  if (sc.init(args) != ISRRAN_SUCCESS) {
    return ret;
  }
