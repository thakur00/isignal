...
gw::gw(isrlog::basic_logger& logger_) : thread("GW"), logger(logger_), tft_matcher(logger) {}
...
int gw::init(const gw_args_t& args_, stack_interface_gw* stack_)
{
  ...
  logger.set_level(isrlog::str_to_basic_level(args.log.gw_level));
...
/*******************************************************************************
  PDCP interface
*******************************************************************************/
void gw::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
void gw::write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  if (pdu->N_bytes > 2) {
    logger.info(pdu->msg,
                pdu->N_bytes,
...
{
  if (lcid > 0 && lcid < ISRRAN_N_MCH_LCIDS) {
...
void gw::run_thread()
{
  ...
  while (run_enable) {
    ...
    if (ISRRAN_MAX_BUFFER_SIZE_BYTES - ISRRAN_BUFFER_HEADER_OFFSET > idx) {
      N_bytes = read(tun_fd, &pdu->msg[idx], ISRRAN_MAX_BUFFER_SIZE_BYTES - ISRRAN_BUFFER_HEADER_OFFSET - idx);
    } else {
      logger.error("GW pdu buffer full - gw receive thread exiting.");
      isrran::console("GW pdu buffer full - gw receive thread exiting.\n");
      break;
    }
    ...
    if (N_bytes <= 0) {
      ...
      isrran::console("Failed to read from TUN interface - gw receive thread exiting.\n");
      break;
    }

    {
      ...
      if (pkt_len == pdu->N_bytes) {
        ...
        isrran_assert(default_eps_bearer_id != NOT_ASSIGNED, "Default EPS bearer not activated");
...
int gw::setup_if_addr4(uint32_t ip_addr, char* err_str)
{
  if (ip_addr != current_ip_addr) {
    ...
    if (inet_pton(ifr.ifr_netmask.sa_family,
      ...
      isrran::console("Invalid tun_dev_netmask: %s\n", args.tun_dev_netmask.c_str());
      perror("inet_pton");
      return ISRRAN_ERROR_CANT_START;
    }
...
void gw::del_ipv6_addr(struct in6_addr* in6p)
{
  ...
  req.ifa.ifa_index     = if_index; // set the tun_isrue index
  req.ifa.ifa_scope     = 0;
