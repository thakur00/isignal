...
#include "isrue/hdr/stack/upper/sdap.h"

namespace isrue {

sdap::sdap(const char* logname) : logger(isrlog::fetch_basic_logger(logname)) {}

bool sdap::init(pdcp_interface_sdap_nr* pdcp_, isrue::gw_interface_pdcp* gw_)
{
  m_pdcp = pdcp_;
  m_gw   = gw_;
...
void sdap::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  if (!running) {
    return;
  }
...
void sdap::write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  if (!running) {
    return;
  }
