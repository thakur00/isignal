...
#include "isrue/hdr/stack/upper/nas_base.h"

using namespace isrran;
namespace isrue {
nas_base::nas_base(isrlog::basic_logger& logger_, uint32_t mac_offset_, uint32_t seq_offset_, uint32_t bearer_id_) :
...
int nas_base::parse_security_algorithm_list(std::string algorithm_string, bool* algorithm_caps)
{
  ...
  isrran::string_parse_list(algorithm_string, ',', cap_list);
  if (cap_list.empty()) {
