...
#include "isrue/hdr/stack/upper/nas_idle_procedures.h"
...
nas::nas(isrlog::basic_logger& logger_, isrran::task_sched_handle task_sched_) :
...
int nas::init(usim_interface_nas* usim_, rrc_interface_nas* rrc_, gw_interface_nas* gw_, const nas_args_t& cfg_)
{
  ...
  if (parse_security_algorithm_list(cfg_.eia, eia_caps) != ISRRAN_SUCCESS) {
  ...
  if (parse_security_algorithm_list(cfg_.eea, eea_caps) != ISRRAN_SUCCESS) {
    ...
    case emm_state_t::state_t::deregistered:
      ...
      switch (state.get_deregistered_substate()) {
        ...
        case emm_state_t::deregistered_substate_t::attach_needed:
          start_attach_request(isrran::establishment_cause_t::mo_data);
          break;
...
void nas::timer_expired(uint32_t timeout_id)
{
  if (timeout_id == t3402.id()) {
    ...
    start_attach_request(isrran::establishment_cause_t::mo_sig);
 ...
 *
 */
void nas::start_attach_request(isrran::establishment_cause_t cause_)
{
  ...
  unique_byte_buffer_t msg = isrran::make_byte_buffer();
  if (msg == nullptr) {
 ...
 *
 */
void nas::start_service_request(isrran::establishment_cause_t cause_)
{
  logger.info("Service Request with cause %s.", to_string(cause_).c_str());
  isrran::console("Service Request with cause %s.\n", to_string(cause_).c_str());
...
bool nas::connection_request_completed(bool outcome)
{
  if (outcome == true) {
    ...
    if (state.get_state() == emm_state_t::state_t::service_request_initiated) {
      isrran::console("Service Request successful.\n");
    ...
    if (state.get_state() == emm_state_t::state_t::service_request_initiated) {
      isrran::console("RRC connection for Service Request failed.\n");
...
bool nas::paging(s_tmsi_t* ue_identity)
{
  if (state.get_state() == emm_state_t::state_t::registered) {
    ...
    start_service_request(isrran::establishment_cause_t::mt_access);
  } else {
...
void nas::select_plmn()
{
  ...
  for (const isrran::plmn_id_t& known_plmn : known_plmns) {
 ...
 */
int nas::apply_security_config(isrran::unique_byte_buffer_t& pdu, uint8_t sec_hdr_type)
{
  if (have_ctxt) {
    if (pdu->N_bytes > 5) {
      if (sec_hdr_type == LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED ||
...
void nas::parse_attach_accept(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  if (attach_accept.eps_attach_result == LIBLTE_MME_EPS_ATTACH_RESULT_EPS_ONLY) {
    ...
    if (LIBLTE_MME_PDN_TYPE_IPV4 == act_def_eps_bearer_context_req.pdn_addr.pdn_type) {
      ...
      isrran::console("Network attach successful. IP: %u.%u.%u.%u\n",
                      act_def_eps_bearer_context_req.pdn_addr.addr[0],
                      act_def_eps_bearer_context_req.pdn_addr.addr[1],
                      act_def_eps_bearer_context_req.pdn_addr.addr[2],
                      act_def_eps_bearer_context_req.pdn_addr.addr[3]);
      ...
      isrran::console("Network attach successful. IPv6 interface Id: %02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
                      act_def_eps_bearer_context_req.pdn_addr.addr[0],
                      act_def_eps_bearer_context_req.pdn_addr.addr[1],
                      act_def_eps_bearer_context_req.pdn_addr.addr[2],
                      act_def_eps_bearer_context_req.pdn_addr.addr[3],
...
void nas::parse_attach_reject(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  isrran::console("Received Attach Reject. Cause= %02X\n", attach_rej.emm_cause);
...
void nas::parse_authentication_request(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  if (LIBLTE_MME_TYPE_OF_SECURITY_CONTEXT_FLAG_NATIVE == auth_req.nas_ksi.tsc_flag) {
    ...
    isrran::console("Warning: NAS mapped security context not currently supported\n");
  }
  ...
  if (auth_result == AUTH_OK) {
    ...
    isrran::console("Warning: Network authentication failure\n");
...
void nas::parse_service_reject(uint32_t lcid, unique_byte_buffer_t pdu, const uint8_t sec_hdr_type)
{
  ...
  isrran::console("Received service reject with EMM cause=0x%x.\n", service_reject.emm_cause);
...
void nas::parse_modify_eps_bearer_context_request(isrran::unique_byte_buffer_t pdu)
{
  ...
  if (gw->apply_traffic_flow_template(request.eps_bearer_id, tft) != ISRRAN_SUCCESS) {
...
void nas::gen_attach_request(isrran::unique_byte_buffer_t& msg)
{
  if (msg == nullptr) {
...
void nas::gen_service_request(isrran::unique_byte_buffer_t& msg)
{
  if (msg == nullptr) {
...
void nas::gen_pdn_connectivity_request(LIBLTE_BYTE_MSG_STRUCT* msg)
{
  ...
  if (cfg.apn_protocol == "ipv4" || cfg.apn_protocol.empty()) {
    ...
    isrran::console("Unsupported PDN prtocol: %s. Defaulting to IPv4\n", cfg.apn_protocol.c_str());
    pdn_con_req.pdn_type = LIBLTE_MME_PDN_TYPE_IPV4;
  }
...
void nas::send_detach_request(bool switch_off)
{
  ...
  if (rrc->is_connected()) {
    ...
    if (not rrc->connection_request(isrran::establishment_cause_t::mo_sig, std::move(pdu))) {
      logger.error("Error starting RRC connection");
    }
  }
