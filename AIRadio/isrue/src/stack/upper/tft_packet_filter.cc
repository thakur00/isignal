...
#include "isrue/hdr/stack/upper/tft_packet_filter.h"
 ...
 */
bool tft_packet_filter_t::match(const isrran::unique_byte_buffer_t& pdu)
{
...
bool tft_packet_filter_t::match_ip(const isrran::unique_byte_buffer_t& pdu)
{
...
bool tft_packet_filter_t::match_protocol(const isrran::unique_byte_buffer_t& pdu)
{
  struct iphdr*   ip_pkt  = (struct iphdr*)pdu->msg;
  struct ipv6hdr* ip6_pkt = (struct ipv6hdr*)pdu->msg;
...
bool tft_packet_filter_t::match_type_of_service(const isrran::unique_byte_buffer_t& pdu)
{
  struct iphdr* ip_pkt = (struct iphdr*)pdu->msg;
...
bool tft_packet_filter_t::match_flow_label(const isrran::unique_byte_buffer_t& pdu)
{
  struct ipv6hdr* ip6_pkt = (struct ipv6hdr*)pdu->msg;
...
bool tft_packet_filter_t::match_port(const isrran::unique_byte_buffer_t& pdu)
{
  struct iphdr*   ip_pkt  = (struct iphdr*)pdu->msg;
  struct ipv6hdr* ip6_pkt = (struct ipv6hdr*)pdu->msg;
 ...
 * @return ISRRAN_SUCCESS if a reference could be found, ISRRAN_ERROR otherwise.
 */
int tft_pdu_matcher::check_tft_filter_match(const isrran::unique_byte_buffer_t& pdu, uint8_t& eps_bearer_id)
{
