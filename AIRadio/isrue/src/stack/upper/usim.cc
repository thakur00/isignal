...
usim::usim(isrlog::basic_logger& logger) : usim_base(logger) {}
...
int usim::init(usim_args_t* args)
{
  ...
  if (32 == args->k.length()) {
    ...
    isrran::console("Invalid length for K: %zu should be %d\n", args->k.length(), 32);
  }
  ...
  if (auth_algo == auth_algo_milenage) {
    if (args->using_op) {
      if (32 == args->op.length()) {
        ...
        isrran::console("Invalid length for OP: %zu should be %d\n", args->op.length(), 32);
      }
    } else {
      if (32 == args->opc.length()) {
        ...
        isrran::console("Invalid length for OPc: %zu should be %d\n", args->opc.length(), 32);
      }
    }
  }
  ...
  if (15 == args->imsi.length()) {
    ...
    isrran::console("Invalid length for IMSI: %zu should be %d\n", args->imsi.length(), 15);
  }
  ...
  if (15 == args->imei.length()) {
    ...
    isrran::console("Invalid length for IMEI: %zu should be %d\n", args->imei.length(), 15);
  }
