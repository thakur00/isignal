...
nas_5g::nas_5g(isrlog::basic_logger& logger_, isrran::task_sched_handle task_sched_) :
                 ...
                 const nas_5g_args_t&     cfg_)
{
  ...
  if (parse_security_algorithm_list(cfg_.ia5g, ia5g_caps) != ISRRAN_SUCCESS) {
  ...
  if (parse_security_algorithm_list(cfg_.ea5g, ea5g_caps) != ISRRAN_SUCCESS) {
    logger.warning(
  ...
  if (init_pdu_sessions(cfg.pdu_session_cfgs) != ISRRAN_SUCCESS) {
...
int nas_5g::write_pdu(isrran::unique_byte_buffer_t pdu)
{
  logger.info(pdu->msg, pdu->N_bytes, "DL PDU (length %d)", pdu->N_bytes);

  nas_5gs_msg nas_msg;

  if (nas_msg.unpack_outer_hdr(pdu) != ISRRAN_SUCCESS) {
  ...
  if (nas_msg.unpack(pdu) != ISRRAN_SUCCESS) {
...
int nas_5g::send_registration_request()
{
  ...
  if (initial_registration_request_stored.pack(pdu) != ISRASN_SUCCESS) {
  ...
  if (rrc_nr->is_connected() == true) {
    ...
    if (rrc_nr->connection_request(nr_establishment_cause_t::mo_Signalling, std::move(pdu)) != ISRRAN_SUCCESS) {
...
int nas_5g::send_registration_complete()
{
  ...
  if (nas_msg.pack(pdu) != ISRASN_SUCCESS) {
...
int nas_5g::send_security_mode_complete(const isrran::nas_5g::security_mode_command_t& security_mode_command)
{
                                                   ...
                                                   const pdu_session_cfg_t& pdu_session_cfg)
{
  ...
  pdu_ses_est_req.pdu_session_type.pdu_session_type_value =
      static_cast<isrran::nas_5g::pdu_session_type_t::PDU_session_type_value_type_::options>(pdu_session_cfg.apn_type);
  ...
  if (nas_msg.pack(ul_nas_msg.payload_container.payload_container_contents) != ISRASN_SUCCESS) {
  ...
  if (env_nas_msg.pack(pdu) != ISRASN_SUCCESS) {
...
int nas_5g::send_identity_response(isrran::nas_5g::identity_type_5gs_t::identity_types_::options identity_type)
{
  unique_byte_buffer_t pdu = isrran::make_byte_buffer();
  if (!pdu) {
    logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
  ...
  switch (identity_type) {
    case (identity_type_5gs_t::identity_types_::suci): {
      isrran::nas_5g::mobile_identity_5gs_t::suci_s& suci = identity_response.mobile_identity.set_suci();
      suci.supi_format = mobile_identity_5gs_t::suci_s::supi_format_type_::options::imsi;
      usim->get_home_mcc_bytes(suci.mcc.data(), suci.mcc.size());
      usim->get_home_mnc_bytes(suci.mnc.data(), suci.mnc.size());
      suci.scheme_output.resize(5);
    ...
    case (identity_type_5gs_t::identity_types_::guti_5g): {
      isrran::nas_5g::mobile_identity_5gs_t::guti_5g_s& guti = identity_response.mobile_identity.set_guti_5g();
      guti                                                   = guti_5g;
    } break;
    case (identity_type_5gs_t::identity_types_::imei): {
      isrran::nas_5g::mobile_identity_5gs_t::imei_s& imei = identity_response.mobile_identity.set_imei();
      usim->get_imei_vec(imei.imei.data(), 15);
    } break;
    case (identity_type_5gs_t::identity_types_::imeisv): {
      isrran::nas_5g::mobile_identity_5gs_t::imeisv_s& imeisv = identity_response.mobile_identity.set_imeisv();
      usim->get_imei_vec(imeisv.imeisv.data(), 15);
      imeisv.imeisv[14] = ue_svn_oct1;
      imeisv.imeisv[15] = ue_svn_oct2;
    } break;
...
int nas_5g::handle_authentication_reject(isrran::nas_5g::authentication_reject_t& authentication_reject)
{
...
int nas_5g::handle_service_accept(isrran::nas_5g::service_accept_t& service_accept)
{
...
int nas_5g::handle_service_reject(isrran::nas_5g::service_reject_t& service_reject)
{
...
int nas_5g::handle_dl_nas_transport(isrran::nas_5g::dl_nas_transport_t& dl_nas_transport)
{
...
int nas_5g::handle_configuration_update_command(
    isrran::nas_5g::configuration_update_command_t& configuration_update_command)
{
...
void nas_5g::fill_security_caps(isrran::nas_5g::ue_security_capability_t& sec_caps)
{
  if (ia5g_caps[0] == true) {
...
bool nas_5g::check_replayed_ue_security_capabilities(isrran::nas_5g::ue_security_capability_t& caps)
{
...
int nas_5g::add_pdu_session(uint16_t                      pdu_session_id,
                            ...
                            isrran::nas_5g::pdu_address_t pdu_address)
{
  ...
  if (pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV4V6 || pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV4) {
    ...
    isrran::console("PDU Session Establishment successful. IP: %u.%u.%u.%u\n",
                    pdu_address.ipv4.data()[0],
                    pdu_address.ipv4.data()[1],
                    pdu_address.ipv4.data()[2],
                    pdu_address.ipv4.data()[3]);
  ...
  if (pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV4V6 || pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV6) {
    ...
    isrran::console("PDU Session Establishment successful. IPv6 interface id: %02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
                    pdu_address.ipv6.data()[0],
                    pdu_address.ipv6.data()[1],
                    pdu_address.ipv6.data()[2],
                    pdu_address.ipv6.data()[3],
