...
#ifdef HAVE_PCSC
#include "isrue/hdr/stack/upper/pcsc_usim.h"
#endif

namespace isrue {

std::unique_ptr<usim_base> usim_base::get_instance(usim_args_t* args, isrlog::basic_logger& logger)
{
#if HAVE_PCSC
  if (args->mode == "pcsc") {
...
usim_base::usim_base(isrlog::basic_logger& logger) : logger(logger) {}
...
bool usim_base::get_home_plmn_id(isrran::plmn_id_t* home_plmn_id)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return false;
...
bool usim_base::get_home_mcc_bytes(uint8_t* mcc_, uint32_t n)
{
  ...
  isrran::string_to_mcc(mcc_str, &mcc_bcd);
  isrran::mcc_to_bytes(mcc_bcd, mcc_);
...
bool usim_base::get_home_mnc_bytes(uint8_t* mnc_, uint32_t n)
{
  ...
  isrran::string_to_mnc(mnc_str, &mnc_bcd);
  isrran::mnc_to_bytes(mnc_bcd, mnc_, &len);
...
void usim_base::generate_as_keys(uint8_t* k_asme_, uint32_t count_ul, isrran::as_security_config_t* sec_cfg)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return;
  ...
  // Generate K_enb
  isrran::security_generate_k_enb(k_asme_, count_ul, k_enb_ctx.k_enb.data());
...
void usim_base::generate_as_keys_ho(uint32_t pci, uint32_t earfcn, int ncc, isrran::as_security_config_t* sec_cfg)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return;
  ...
  while (k_enb_ctx.ncc != (uint32_t)ncc) {
    ...
    // Generate NH
    isrran::security_generate_nh(k_asme, sync, k_enb_ctx.nh.data());
  ...
  // Generate K_enb
  isrran::security_generate_k_enb_star(enb_star_key, pci, earfcn, k_enb_star);
...
void usim_base::store_keys_before_ho(const isrran::as_security_config_t& as_ctx)
{
...
void usim_base::restore_keys_from_failed_ho(isrran::as_security_config_t* as_ctx)
{
...
void usim_base::generate_nr_as_keys(isrran::as_key_t& k_amf, uint32_t count_ul, isrran::as_security_config_t* sec_cfg)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return;
  ...
  // Generate K_gnb
  isrran::security_generate_k_gnb(k_amf, count_ul, k_gnb_ctx.k_gnb);
...
bool usim_base::generate_nr_context(uint16_t sk_counter, isrran::as_security_config_t* sec_cfg)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return false;
  ...
  isrran::security_generate_sk_gnb(k_enb_ctx.k_enb.data(), sk_counter, k_gnb_ctx.sk_gnb.data());
...
bool usim_base::update_nr_context(isrran::as_security_config_t* sec_cfg)
{
  if (!initiated) {
    logger.error("USIM not initiated!");
    return false;
