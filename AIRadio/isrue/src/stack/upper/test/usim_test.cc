...
int gen_auth_response_test()
{
  ...
  auto&       logger = isrlog::fetch_basic_logger("USIM", false);
  isrue::usim usim(logger);
  usim.init(&args);
...
int main(int argc, char** argv)
{
  ...
  isrlog::init();
  TESTASSERT(gen_auth_response_test() == ISRRAN_SUCCESS);
  TESTASSERT(mcc_mnc_msin_test() == ISRRAN_SUCCESS);
}
