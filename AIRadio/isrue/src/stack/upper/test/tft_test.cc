...
{
  isrlog::basic_logger&        logger = isrlog::fetch_basic_logger("TFT");
  isrran::unique_byte_buffer_t ip_msg1, ip_msg2, ip_msg3, ip_msg4, ip_msg5;
  ...
  isrue::tft_packet_filter_t filter(EPS_BEARER_ID, packet_filter, logger);
...
int tft_filter_test_single_local_port()
{
  ...
  isrran::unique_byte_buffer_t ip_msg1, ip_msg2;
  ip_msg1 = make_byte_buffer();
  TESTASSERT(ip_msg1 != nullptr);
  ip_msg2 = make_byte_buffer();
  TESTASSERT(ip_msg2 != nullptr);
  ...
  isrran::uint16_to_uint8(2222, &filter_message[1]);
...
int tft_filter_test_single_remote_port()
{
  ...
  isrran::uint16_to_uint8(2001, &filter_message[1]);
...
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("TFT", false);
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(128);
  isrlog::init();

  isrran::byte_buffer_pool::get_instance();
