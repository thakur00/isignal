...
class test_stack_dummy : public isrue::stack_interface_gw
{
public:
  bool is_registered() { return true; }
  bool start_service_request() { return true; };
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu) { return; }
  bool has_active_radio_bearer(uint32_t eps_bearer_id) { return true; }
};

int gw_test()
{
  isrue::gw_args_t gw_args;
  ...
  if (rtn != ISRRAN_SUCCESS) {
    isrlog::fetch_basic_logger("TEST", false)
        .error("Failed to setup GW interface. Not possible to test function. Try to execute with sudo rights.");
    gw.stop();
    return ISRRAN_SUCCESS;
  }

  TESTASSERT(gw.deactivate_eps_bearer(eps_bearer_id) == ISRRAN_SUCCESS);
  TESTASSERT(gw.deactivate_eps_bearer(non_existing_eps_bearer_id) == ISRRAN_ERROR);
  gw.stop();
  return ISRRAN_SUCCESS;
}
...
int main(int argc, char** argv)
{
  ...
  TESTASSERT(gw_test() == ISRRAN_SUCCESS);
