...
#include "isrue/hdr/stack/rrc_nr/rrc_nr_procedures.h"
...
rrc_nr::rrc_nr(isrran::task_sched_handle task_sched_) :
  logger(isrlog::fetch_basic_logger("RRC-NR")),
                 ...
                 const rrc_nr_args_t&        args_)
{
  ...
  if (rrc_eutra == nullptr) {
    ...
    isrran::dl_harq_cfg_nr_t harq_cfg = {};
...
void rrc_nr::isrran_rrc_log(const char* str) {}

template <class T>
void rrc_nr::log_rrc_message(const std::string&           source,
                             direction_t                  dir,
                             const isrran::byte_buffer_t* pdu,
                             const T&                     msg,
...
// PDCP interface
void rrc_nr::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
void rrc_nr::decode_dl_ccch(unique_byte_buffer_t pdu)
{
  ...
  if (dl_ccch_msg.unpack(bref) != asn1::ISRASN_SUCCESS or
  ...
  switch (dl_ccch_msg.msg.c1().type().value) {
    case dl_ccch_msg_type_c::c1_c_::types::rrc_reject: {
      ...
      isrran::console("Received RRC Reject\n");

      t300.stop();

      if (reject.crit_exts.rrc_reject().wait_time_present) {
        // nas->set_barring(isrran::barring_t::all);
        ...
        // nas->set_barring(isrran::barring_t::none);
        // start_go_idle();
      }
    } break;
...
void rrc_nr::decode_dl_dcch(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  if (dl_dcch_msg.unpack(bref) != asn1::ISRASN_SUCCESS or
...
void rrc_nr::write_pdu_bcch_bch(isrran::unique_byte_buffer_t pdu) {}
void rrc_nr::write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu)
{
  decode_pdu_bcch_dlsch(std::move(pdu));
}

void rrc_nr::decode_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu)
{
  ...
  asn1::ISRASN_CODE err = dlsch_msg.unpack(dlsch_bref);

  if (err != asn1::ISRASN_SUCCESS or dlsch_msg.msg.type().value != bcch_dl_sch_msg_type_c::types_opts::c1) {
...
void rrc_nr::handle_sib1(const sib1_s& sib1)
{
  ...
  if (not make_phy_rach_cfg(sib1.serving_cell_cfg_common.ul_cfg_common.init_ul_bwp.rach_cfg_common.setup(),
                            sib1.serving_cell_cfg_common.tdd_ul_dl_cfg_common_present ? ISRRAN_DUPLEX_MODE_TDD
                                                                                      : ISRRAN_DUPLEX_MODE_FDD,
                            &phy_cfg.prach)) {
...
void rrc_nr::write_pdu_pcch(isrran::unique_byte_buffer_t pdu) {}
void rrc_nr::write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu) {}
void rrc_nr::notify_pdcp_integrity_error(uint32_t lcid) {}

// NAS interface
int rrc_nr::write_sdu(isrran::unique_byte_buffer_t sdu)
{
...
int rrc_nr::connection_request(isrran::nr_establishment_cause_t cause, isrran::unique_byte_buffer_t dedicated_info_nas_)
{
...
void rrc_nr::send_setup_request(isrran::nr_establishment_cause_t cause)
{
...
{
  unique_byte_buffer_t pdu = isrran::make_byte_buffer();
  if (pdu == nullptr) {
  ...
  if (msg.pack(bref) != ISRASN_SUCCESS) {
...
void rrc_nr::send_con_setup_complete(isrran::unique_byte_buffer_t nas_msg)
{
...
int rrc_nr::send_ue_capability_info(const asn1::rrc_nr::ue_cap_enquiry_s& msg)
{
  ...
  for (auto ue_cap_rat_request : ue_cap_enquiry_ies.ue_cap_rat_request_list) {
    if (ue_cap_rat_request.rat_type.value == rat_type_opts::nr) {
      ...
      if (ue_cap.pack(bref_pack) != asn1::ISRASN_SUCCESS) {
...
// EUTRA-RRC interface
int rrc_nr::get_eutra_nr_capabilities(isrran::byte_buffer_t* eutra_nr_caps_pdu)
{
  ...
  band_params_a.isr_tx_switch_present      = true;
  band_params_a.isr_carrier_switch_present = false;
  band_params_a.isr_tx_switch.supported_isr_tx_port_switch =
      band_params_v1540_s::isr_tx_switch_s_::supported_isr_tx_port_switch_opts::not_supported;
  band_combination_v1540.band_list_v1540.push_back(band_params_a);

  band_params_v1540_s band_params_b;
  band_params_b.isr_tx_switch_present = true;
  band_params_b.isr_tx_switch.supported_isr_tx_port_switch =
      band_params_v1540_s::isr_tx_switch_s_::supported_isr_tx_port_switch_opts::t1r2;
  band_params_b.isr_carrier_switch_present = false;
...
int rrc_nr::get_nr_capabilities(isrran::byte_buffer_t* nr_caps_pdu)
{
...
bool rrc_nr::apply_rlc_add_mod(const rlc_bearer_cfg_s& rlc_bearer_cfg)
{
  ...
  if (rlc_bearer_cfg.rlc_cfg_present == true) {
    ...
    if (isrran::make_rlc_config_t(rlc_bearer_cfg.rlc_cfg, bearer_id, &rlc_cfg) != ISRRAN_SUCCESS) {
  ...
  if (rlc_bearer_cfg.mac_lc_ch_cfg_present == true && rlc_bearer_cfg.mac_lc_ch_cfg.ul_specific_params_present) {
    ...
    logical_channel_cfg = isrran::make_mac_logical_channel_cfg_t(lc_ch_id, rlc_bearer_cfg.mac_lc_ch_cfg);
...
bool rrc_nr::apply_mac_cell_group(const mac_cell_group_cfg_s& mac_cell_group_cfg)
{
  if (mac_cell_group_cfg.sched_request_cfg_present) {
    if (mac_cell_group_cfg.sched_request_cfg.sched_request_to_add_mod_list.size() > 0) {
      if (mac_cell_group_cfg.sched_request_cfg.sched_request_to_add_mod_list.size() == 1) {
        ...
        if (mac->set_config(sr_cfg) != ISRRAN_SUCCESS) {
  ...
  if (mac_cell_group_cfg.sched_request_cfg_present)
    ...
    if (mac_cell_group_cfg.bsr_cfg_present) {
      ...
      if (mac->set_config(bsr_cfg) != ISRRAN_SUCCESS) {
        return false;
      }
    }
  ...
  if (mac_cell_group_cfg.tag_cfg_present) {
    if (mac_cell_group_cfg.tag_cfg.tag_to_add_mod_list.size() > 0) {
      for (uint32_t i = 0; i < mac_cell_group_cfg.tag_cfg.tag_to_add_mod_list.size(); i++) {
        ...
        if (mac->add_tag_config(tag_cfg_nr) != ISRRAN_SUCCESS) {
    ...
    if (mac_cell_group_cfg.tag_cfg.tag_to_release_list.size() > 0) {
      for (uint32_t i = 0; i < mac_cell_group_cfg.tag_cfg.tag_to_release_list.size(); i++) {
        ...
        if (mac->remove_tag_config(tag_id) != ISRRAN_SUCCESS) {
  ...
  if (mac_cell_group_cfg.phr_cfg_present) {
    if (mac_cell_group_cfg.phr_cfg.is_setup()) {
      ...
      if (mac->set_config(phr_cfg_nr) != ISRRAN_SUCCESS) {
...
bool rrc_nr::apply_sp_cell_init_dl_pdsch(const asn1::rrc_nr::pdsch_cfg_s& pdsch_cfg)
{
  if (pdsch_cfg.mcs_table_present) {
    switch (pdsch_cfg.mcs_table) {
      case pdsch_cfg_s::mcs_table_opts::qam256:
        phy_cfg.pdsch.mcs_table = isrran_mcs_table_256qam;
        break;
      case pdsch_cfg_s::mcs_table_opts::qam64_low_se:
        phy_cfg.pdsch.mcs_table = isrran_mcs_table_qam64LowSE;
        break;
    ...
    phy_cfg.pdsch.mcs_table = isrran_mcs_table_64qam;
  }
  ...
  if (pdsch_cfg.dmrs_dl_for_pdsch_map_type_a_present) {
    if (pdsch_cfg.dmrs_dl_for_pdsch_map_type_a.type() == setup_release_c<dmrs_dl_cfg_s>::types_opts::setup) {
      ...
      if (make_phy_dmrs_dl_additional_pos(pdsch_cfg.dmrs_dl_for_pdsch_map_type_a.setup(), &isrran_dmrs_sch_add_pos) ==
          true) {
        phy_cfg.pdsch.dmrs_typeA.additional_pos = isrran_dmrs_sch_add_pos;
        phy_cfg.pdsch.dmrs_typeA.present        = true;
      } else {
        logger.warning("Warning while build isrran_dmrs_sch_add_pos structure");
        return false;
      }
    } else {
  ...
  isrran_resource_alloc_t resource_alloc;
...
bool rrc_nr::apply_res_csi_report_cfg(const asn1::rrc_nr::csi_report_cfg_s& csi_report_cfg)
{
  ...
  if (make_phy_csi_report(csi_report_cfg, &isrran_csi_hl_report_cfg) == true) {
    phy_cfg.csi.reports[report_cfg_id] = isrran_csi_hl_report_cfg;
  } else {
      ...
      if (dl_cfg_common.init_dl_bwp.pdsch_cfg_common.is_setup()) {
        ...
        if (pdcch_cfg_common.ctrl_res_set_zero_present) {
          ...
          double pointA_abs_freq_Hz =
              ...
              phy_cfg.carrier.nof_prb * ISRRAN_NRE * ISRRAN_SUBC_SPACING_NR(phy_cfg.carrier.scs) / 2;
          ...
          isrran_subcarrier_spacing_t ssb_scs = phy_cfg.ssb.scs;

          // Select PDCCH subcarrrier spacing from PDCCH BWP
          isrran_subcarrier_spacing_t pdcch_scs = phy_cfg.carrier.scs;

          // Make CORESET Zero from provided field and given subcarrier spacing
          isrran_coreset_t coreset0 = {};
          if (isrran_coreset_zero(phy_cfg.carrier.pci,
                                  ssb_pointA_freq_offset_Hz,
                                  ssb_scs,
                                  pdcch_scs,
                                  pdcch_cfg_common.ctrl_res_set_zero,
                                  &coreset0) < ISRASN_SUCCESS) {
        ...
        if (pdcch_cfg_common.ra_search_space_present) {
          if (phy_cfg.pdcch.search_space_present[pdcch_cfg_common.ra_search_space] == true) {
            ...
            phy_cfg.pdcch.ra_search_space.type    = isrran_search_space_type_common_1;
          } else {
    ...
    if (ul_cfg_common.init_ul_bwp.rach_cfg_common_present) {
      if (ul_cfg_common.init_ul_bwp.rach_cfg_common.type() == setup_release_c<rach_cfg_common_s>::types_opts::setup) {
        ...
        if (not make_phy_rach_cfg(
                ul_cfg_common.init_ul_bwp.rach_cfg_common.setup(), ISRRAN_DUPLEX_MODE_FDD, &phy_cfg.prach)) {
...
bool rrc_nr::apply_sp_cell_ded_ul_pucch(const asn1::rrc_nr::pucch_cfg_s& pucch_cfg)
{
  ...
  if (pucch_cfg.sched_request_res_to_add_mod_list.size() > 0) {
    for (uint32_t i = 0; i < pucch_cfg.sched_request_res_to_add_mod_list.size(); i++) {
      ...
      if (make_phy_sr_resource(pucch_cfg.sched_request_res_to_add_mod_list[i], &isrran_pucch_nr_sr_resource) ==
          true) { // TODO: fix that if indexing is solved
        phy_cfg.pucch.sr_resources[sr_res_id] = isrran_pucch_nr_sr_resource;
        ...
        logger.warning("Warning while building isrran_pucch_nr_sr_resource structure");
        return false;
      }
    }
  } else {
...
bool rrc_nr::apply_sp_cell_ded_ul_pusch(const asn1::rrc_nr::pusch_cfg_s& pusch_cfg)
{
  if (pusch_cfg.mcs_table_present) {
    switch (pusch_cfg.mcs_table) {
      case pusch_cfg_s::mcs_table_opts::qam256:
        phy_cfg.pusch.mcs_table = isrran_mcs_table_256qam;
        break;
      case pusch_cfg_s::mcs_table_opts::qam64_low_se:
        phy_cfg.pusch.mcs_table = isrran_mcs_table_qam64LowSE;
        break;
    ...
    // If the field is absent the UE applies the value 64QAM.
    phy_cfg.pusch.mcs_table = isrran_mcs_table_64qam;
  }
  ...
  if (pusch_cfg.dmrs_ul_for_pusch_map_type_a_present) {
    if (pusch_cfg.dmrs_ul_for_pusch_map_type_a.type() == setup_release_c<dmrs_ul_cfg_s>::types_opts::setup) {
      isrran_dmrs_sch_add_pos_t isrran_dmrs_sch_add_pos;
      if (make_phy_dmrs_ul_additional_pos(pusch_cfg.dmrs_ul_for_pusch_map_type_a.setup(), &isrran_dmrs_sch_add_pos) ==
          true) {
        phy_cfg.pusch.dmrs_typeA.additional_pos = isrran_dmrs_sch_add_pos;
    ...
    if (pusch_cfg.uci_on_pusch.type() == setup_release_c<uci_on_pusch_s>::types_opts::setup) {
      if (pusch_cfg.uci_on_pusch.setup().beta_offsets_present) {
        if (pusch_cfg.uci_on_pusch.setup().beta_offsets.type() ==
          ...
          isrran_beta_offsets_t beta_offsets;
...
{
  // NSA specific handling to defer CSI, SR, ISR config until after RA (see TS 38.331, Section 5.3.5.3)
  isrran_csi_hl_cfg_t prev_csi = phy_cfg.csi;
  ...
  if (sp_cell_cfg.recfg_with_sync_present) {
    ...
    isrran::phy_cfg_nr_t current_phycfg = phy_cfg;
...
bool rrc_nr::apply_srb_add_mod(const srb_to_add_mod_s& srb_cfg)
{
  ...
  isrran::pdcp_config_t pdcp_cfg = isrran::make_nr_srb_pdcp_config_t(srb_cfg.srb_id, true);
...
bool rrc_nr::apply_drb_add_mod(const drb_to_add_mod_s& drb_cfg)
{
  ...
  if (drb_cfg.cn_assoc.type() == drb_to_add_mod_s::cn_assoc_c_::types_opts::eps_bearer_id) {
    ...
    stack->add_eps_bearer(eps_bearer_id, isrran::isrran_rat_t::nr, lcid);
    ...
    stack->add_eps_bearer(pdu_session_id, isrran::isrran_rat_t::nr, lcid);
  } else {
  ...
  isrran::pdcp_config_t pdcp_cfg = make_drb_pdcp_config_t(drb_cfg.drb_id, true, drb_cfg.pdcp_cfg);
  ...
  if (security_cfg.security_algorithm_cfg_present) {
    switch (security_cfg.security_algorithm_cfg.ciphering_algorithm) {
      ...
      default:
        ...
        isrran::console("Ciphering not supported by PDCP-NR at the moment. Requested algorithm=%s\n",
                        security_cfg.security_algorithm_cfg.ciphering_algorithm.to_string());
        return false;
    }
      ...
      switch (security_cfg.security_algorithm_cfg.integrity_prot_algorithm) {
        ...
        default:
          logger.error("Integrity protection not supported by PDCP-NR at the moment. Requested algorithm=%s.",
                       security_cfg.security_algorithm_cfg.ciphering_algorithm.to_string());
          isrran::console("Integrity protection not supported by PDCP-NR at the moment. Requested algorithm %s.\n",
                          security_cfg.security_algorithm_cfg.ciphering_algorithm.to_string());
          return false;
      }
    }
...
bool rrc_nr::handle_rrc_setup(const rrc_setup_s& setup)
{
  ...
  if (cell_group.unpack(bref_cg) != asn1::ISRASN_SUCCESS) {
...
void rrc_nr::handle_rrc_release(const asn1::rrc_nr::rrc_release_s& msg)
{
  ...
  isrran::console("Received RRC Release\n");
