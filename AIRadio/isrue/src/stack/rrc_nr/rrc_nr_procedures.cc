                                                          ...
                                                          const asn1::rrc_nr::rrc_recfg_s& rrc_nr_reconf)
{
  ...
  if (rrc_nr_reconf.crit_exts.rrc_recfg().secondary_cell_group.size() > 0) {
    ...
    if (secondary_cell_group_cfg.unpack(bref0) != asn1::ISRASN_SUCCESS) {
  ...
  if (rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.master_cell_group.size() > 0) {
    ...
    if (master_cell_group_cfg.unpack(bref1) != asn1::ISRASN_SUCCESS) {
...
void rrc_nr::connection_reconf_no_ho_proc::then(const isrran::proc_state_t& result)
{
  if (result.is_success()) {
    Info("Finished %s successfully", name());
    isrran::console("RRC NR reconfiguration successful.\n");
    ...
    isrran::console("RRC NR reconfiguration failed.\n");
...
rrc_nr::setup_request_proc::setup_request_proc(rrc_nr& parent_) :
  rrc_handle(parent_), logger(isrlog::fetch_basic_logger("RRC-NR"))
{}

proc_outcome_t rrc_nr::setup_request_proc::init(isrran::nr_establishment_cause_t cause_,
...
void rrc_nr::setup_request_proc::then(const isrran::proc_state_t& result)
{
  if (result.is_error()) {
...
isrran::proc_outcome_t rrc_nr::setup_request_proc::react(const cell_selection_proc::cell_selection_complete_ev& e)
{
  if (state != state_t::cell_selection) {
...
rrc_nr::connection_setup_proc::connection_setup_proc(isrue::rrc_nr& parent_) :
  rrc_handle(parent_), logger(isrlog::fetch_basic_logger("RRC-NR"))
{}

isrran::proc_outcome_t rrc_nr::connection_setup_proc::init(const asn1::rrc_nr::radio_bearer_cfg_s& radio_bearer_cfg_,
...
isrran::proc_outcome_t rrc_nr::connection_setup_proc::react(const bool& config_complete)
{
  if (not config_complete) {
...
void rrc_nr::connection_setup_proc::then(const isrran::proc_state_t& result)
{
  if (result.is_success()) {
...
rrc_nr::cell_selection_proc::handle_cell_search_result(const rrc_interface_phy_nr::cell_search_result_t& result)
{
  ...
  isrran_csi_meas_info_short(&result.measurements, csi_info_str.data(), (uint32_t)csi_info_str.size());

  // Unpack MIB and convert to text
  isrran_mib_nr_t       mib          = {};
  std::array<char, 512> mib_info_str = {};
  if (isrran_pbch_msg_nr_mib_unpack(&result.pbch_msg, &mib) == ISRASN_SUCCESS) {
    // Convert to text
    isrran_pbch_msg_nr_mib_info(&mib, mib_info_str.data(), (uint32_t)mib_info_str.size());
  } else {
  ...
  if (mib.scs_common != isrran_subcarrier_spacing_15kHz) {
    Error("Unsupported SCS %s", isrran_subcarrier_spacing_to_str(mib.scs_common));
    return proc_outcome_t::error;
  }
  ...
  // Apply MIB settings
  isrran::phy_cfg_nr_t& phy_cfg = rrc_handle.phy_cfg;
