/**
 * Copyright 2013-2022 iSignal Research Labs Pvt Ltd.
 *
 * This file is part of isrRAN.
 *
 * isrRAN is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of
 * the License, or (at your option) any later version.
 *
 * isrRAN is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * A copy of the GNU Affero General Public License can be found in
 * the LICENSE file in the top-level directory of this distribution
 * and at http://www.gnu.org/licenses/.
 *
 */

#include "isrran/common/test_common.h"
#include "isrran/interfaces/ue_gw_interfaces.h"
#include "isrran/interfaces/ue_interfaces.h"
#include "isrran/interfaces/ue_pdcp_interfaces.h"
#include "isrran/interfaces/ue_rlc_interfaces.h"
#include "isrran/interfaces/ue_usim_interfaces.h"
#include "isrue/hdr/stack/rrc/rrc.h"
#include "isrue/hdr/stack/rrc_nr/rrc_nr.h"

using namespace isrue;

class dummy_phy : public phy_interface_rrc_nr
{
  bool           set_config(const isrran::phy_cfg_nr_t& cfg) override { return true; }
  phy_nr_state_t get_state() override { return PHY_NR_STATE_IDLE; };
  void           reset_nr() override{};
  bool           start_cell_search(const cell_search_args_t& req) override { return false; };
  bool           start_cell_select(const cell_select_args_t& req) override { return false; };
};

class dummy_mac : public mac_interface_rrc_nr
{
  void reset() {}
  int  setup_lcid(const isrran::logical_channel_config_t& config) { return ISRRAN_SUCCESS; }
  int  set_config(const isrran::bsr_cfg_nr_t& bsr_cfg) { return ISRRAN_SUCCESS; }
  int  set_config(const isrran::sr_cfg_nr_t& sr_cfg) { return ISRRAN_SUCCESS; }
  int  set_config(const isrran::dl_harq_cfg_nr_t& dl_hrq_cfg) { return ISRRAN_SUCCESS; }
  void set_config(const isrran::rach_cfg_nr_t& rach_cfg) {}
  int  add_tag_config(const isrran::tag_cfg_nr_t& tag_cfg) { return ISRRAN_SUCCESS; }
  int  set_config(const isrran::phr_cfg_nr_t& phr_cfg) { return ISRRAN_SUCCESS; }
  int  remove_tag_config(const uint32_t tag_id) { return ISRRAN_SUCCESS; }
  void bcch_search(bool) {}

  void start_ra_procedure() {}

  void set_contention_id(const uint64_t ue_identity){};

  bool set_crnti(const uint16_t crnti) { return true; };
};

class dummy_rlc : public rlc_interface_rrc
{
  void reset() {}
  void reestablish() {}
  void reestablish(uint32_t lcid) {}
  int  add_bearer(uint32_t lcid, const isrran::rlc_config_t& cnfg) { return ISRRAN_SUCCESS; }
  int  add_bearer_mrb(uint32_t lcid) { return ISRRAN_SUCCESS; }
  void del_bearer(uint32_t lcid) {}
  void suspend_bearer(uint32_t lcid) {}
  void resume_bearer(uint32_t lcid) {}
  void change_lcid(uint32_t old_lcid, uint32_t new_lcid) {}
  bool has_bearer(uint32_t lcid) { return true; }
  bool has_data(const uint32_t lcid) { return true; }
  bool is_suspended(const uint32_t lcid) { return true; }
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)
  {
    last_lcid = lcid;
    last_sdu  = std::move(sdu);
  }

public:
  uint32_t                     last_lcid = 99;
  isrran::unique_byte_buffer_t last_sdu;
};

class dummy_pdcp : public pdcp_interface_rrc
{
  void set_enabled(uint32_t lcid, bool enabled){};
  void reestablish(){};
  void reestablish(uint32_t lcid){};
  void reset(){};
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu, int sn = -1){};
  int  add_bearer(uint32_t lcid, const isrran::pdcp_config_t& cnfg) { return ISRRAN_SUCCESS; };
  void del_bearer(uint32_t lcid){};
  void change_lcid(uint32_t old_lcid, uint32_t new_lcid){};
  void config_security(uint32_t lcid, const isrran::as_security_config_t& sec_cfg){};
  void config_security_all(const isrran::as_security_config_t& sec_cfg){};
  void enable_integrity(uint32_t lcid, isrran::isrran_direction_t direction){};
  void enable_encryption(uint32_t                   lcid,
                         isrran::isrran_direction_t direction = isrran::isrran_direction_t::DIRECTION_TXRX){};
  void send_status_report(){};
  void send_status_report(uint32_t lcid){};
};

class dummy_sdap : public sdap_interface_pdcp_nr, public sdap_interface_gw_nr, public sdap_interface_rrc
{
  void write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu) final{};
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu) final{};
  bool set_bearer_cfg(uint32_t lcid, const sdap_interface_rrc::bearer_cfg_t& cfg) final { return true; };
};

class dummy_gw : public gw_interface_rrc
{
  void add_mch_port(uint32_t lcid, uint32_t port){};
  bool is_running() { return true; };
};

class dummy_eutra : public rrc_eutra_interface_rrc_nr
{
  void new_cell_meas_nr(const std::vector<phy_meas_nr_t>& meas){};
  void nr_rrc_con_reconfig_complete(bool status){};
  void nr_notify_reconfiguration_failure(){};
  void nr_scg_failure_information(const isrran::scg_failure_cause_t cause){};
};

class dummy_nas : public nas_5g_interface_rrc_nr
{
  int      write_pdu(isrran::unique_byte_buffer_t pdu) { return ISRRAN_SUCCESS; };
  int      get_k_amf(isrran::as_key_t& k_amf) { return ISRRAN_SUCCESS; };
  uint32_t get_ul_nas_count() { return ISRRAN_SUCCESS; };
};

class dummy_sim : public usim_interface_rrc_nr
{
  void generate_nr_as_keys(isrran::as_key_t& k_amf, uint32_t count_ul, isrran::as_security_config_t* sec_cfg){};
  bool generate_nr_context(uint16_t sk_counter, isrran::as_security_config_t* sec_cfg) { return true; }
  bool update_nr_context(isrran::as_security_config_t* sec_cfg) { return true; }
};

class dummy_stack : public stack_interface_rrc
{
  isrran::tti_point get_current_tti() final { return isrran::tti_point(); };
  void              add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid) final{};
  void              remove_eps_bearer(uint8_t eps_bearer_id) final{};
  void              reset_eps_bearers() final{};
};

int rrc_nr_cap_request_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);
  isrran::byte_buffer_t     caps;

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};

  rrc_nr_args.supported_bands_eutra.push_back(7);
  rrc_nr_args.supported_bands_nr.push_back(78);

  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  TESTASSERT(rrc_nr.get_eutra_nr_capabilities(&caps) == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nr.get_nr_capabilities(&caps) == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}

int rrc_nsa_reconfig_tdd_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};
  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  uint8_t msg[] = {
      };

  asn1::cbit_ref           bref(msg, sizeof(msg));
  asn1::rrc::dl_dcch_msg_s dl_dcch_msg;

  TESTASSERT(dl_dcch_msg.unpack(bref) == asn1::ISRASN_SUCCESS);
  TESTASSERT(dl_dcch_msg.msg.type().value == dl_dcch_msg_type_c::types_opts::c1);

  dl_dcch_msg_type_c::c1_c_* c1       = &dl_dcch_msg.msg.c1();
  rrc_conn_recfg_r8_ies_s    rx_recfg = c1->rrc_conn_recfg().crit_exts.c1().rrc_conn_recfg_r8();

  const asn1::rrc::rrc_conn_recfg_v1510_ies_s rrc_conn_recfg_v1510_ies =
      rx_recfg.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext;

  bool endc_release_and_add_r15 = false;

  asn1::rrc_nr::rrc_recfg_s rrc_nr_reconf = {};
  rrc_nr_reconf.crit_exts.set_rrc_recfg();

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_cfg_r15.type() == setup_opts::options::setup);

  endc_release_and_add_r15 = rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().endc_release_and_add_r15;

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15_present);

  asn1::cbit_ref bref0(rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15.data(),
                       rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15.size());

  asn1::rrc_nr::rrc_recfg_s secondary_cell_group_r15;
  TESTASSERT(secondary_cell_group_r15.unpack(bref0) == asn1::ISRASN_SUCCESS);
  TESTASSERT(secondary_cell_group_r15.crit_exts.rrc_recfg().secondary_cell_group.size() > 0);

  rrc_nr_reconf.crit_exts.rrc_recfg().secondary_cell_group =
      secondary_cell_group_r15.crit_exts.rrc_recfg().secondary_cell_group;

  TESTASSERT(rrc_conn_recfg_v1510_ies.sk_counter_r15_present);
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext_present                                      = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext_present                         = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext_present            = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext.sk_counter_present = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext.sk_counter =
      rrc_conn_recfg_v1510_ies.sk_counter_r15;

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15_present);
  rrc_nr_reconf.crit_exts.rrc_recfg().radio_bearer_cfg_present = true;
  asn1::rrc_nr::radio_bearer_cfg_s radio_bearer_conf           = {};
  asn1::cbit_ref                   bref2(rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15.data(),
                       rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15.size());
  TESTASSERT(radio_bearer_conf.unpack(bref2) == asn1::ISRASN_SUCCESS);

  rrc_nr_reconf.crit_exts.rrc_recfg().radio_bearer_cfg = radio_bearer_conf;

  rrc_nr.rrc_reconfiguration(endc_release_and_add_r15, rrc_nr_reconf);

  task_sched.run_pending_tasks();
  return ISRRAN_SUCCESS;
}

int rrc_nsa_reconfig_fdd_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};
  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  uint8_t msg[] = {
      };

  asn1::cbit_ref           bref(msg, sizeof(msg));
  asn1::rrc::dl_dcch_msg_s dl_dcch_msg;

  TESTASSERT(dl_dcch_msg.unpack(bref) == asn1::ISRASN_SUCCESS);
  TESTASSERT(dl_dcch_msg.msg.type().value == dl_dcch_msg_type_c::types_opts::c1);

  dl_dcch_msg_type_c::c1_c_* c1       = &dl_dcch_msg.msg.c1();
  rrc_conn_recfg_r8_ies_s    rx_recfg = c1->rrc_conn_recfg().crit_exts.c1().rrc_conn_recfg_r8();

  const asn1::rrc::rrc_conn_recfg_v1510_ies_s rrc_conn_recfg_v1510_ies =
      rx_recfg.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext.non_crit_ext;

  bool endc_release_and_add_r15 = false;

  asn1::rrc_nr::rrc_recfg_s rrc_nr_reconf = {};
  rrc_nr_reconf.crit_exts.set_rrc_recfg();

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_cfg_r15.type() == setup_opts::options::setup);

  endc_release_and_add_r15 = rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().endc_release_and_add_r15;

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15_present);

  asn1::cbit_ref bref0(rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15.data(),
                       rrc_conn_recfg_v1510_ies.nr_cfg_r15.setup().nr_secondary_cell_group_cfg_r15.size());

  asn1::rrc_nr::rrc_recfg_s secondary_cell_group_r15;
  TESTASSERT(secondary_cell_group_r15.unpack(bref0) == asn1::ISRASN_SUCCESS);
  TESTASSERT(secondary_cell_group_r15.crit_exts.rrc_recfg().secondary_cell_group.size() > 0);

  rrc_nr_reconf.crit_exts.rrc_recfg().secondary_cell_group =
      secondary_cell_group_r15.crit_exts.rrc_recfg().secondary_cell_group;

  TESTASSERT(rrc_conn_recfg_v1510_ies.sk_counter_r15_present);
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext_present                                      = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext_present                         = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext_present            = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext.sk_counter_present = true;
  rrc_nr_reconf.crit_exts.rrc_recfg().non_crit_ext.non_crit_ext.non_crit_ext.sk_counter =
      rrc_conn_recfg_v1510_ies.sk_counter_r15;

  TESTASSERT(rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15_present);
  rrc_nr_reconf.crit_exts.rrc_recfg().radio_bearer_cfg_present = true;
  asn1::rrc_nr::radio_bearer_cfg_s radio_bearer_conf           = {};
  asn1::cbit_ref                   bref2(rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15.data(),
                       rrc_conn_recfg_v1510_ies.nr_radio_bearer_cfg1_r15.size());
  TESTASSERT(radio_bearer_conf.unpack(bref2) == asn1::ISRASN_SUCCESS);

  rrc_nr_reconf.crit_exts.rrc_recfg().radio_bearer_cfg = radio_bearer_conf;

  rrc_nr.rrc_reconfiguration(endc_release_and_add_r15, rrc_nr_reconf);

  task_sched.run_pending_tasks();
  return ISRRAN_SUCCESS;
}

int rrc_nr_setup_request_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);
  isrran::byte_buffer_t     caps;

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};

  rrc_nr_args.supported_bands_nr.push_back(78);

  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);
  rrc_nr.connection_request(isrran::nr_establishment_cause_t::mt_Access, nullptr);
  task_sched.run_pending_tasks();

  TESTASSERT(dummy_rlc.last_lcid == 0);         // SRB0 transmission
  TESTASSERT(dummy_rlc.last_sdu->N_bytes == 6); // RRC Setup Request is 6 Bytes long

  return ISRRAN_SUCCESS;
}

int rrc_nr_sib1_decoding_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};
  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  uint8_t msg[] = {0x74, 0x81, 0x01, 0x70, 0x10, 0x40, 0x04, 0x02, 0x00, 0x00, 0x0e, 0x00, 0x00, 0x33, 0x60, 0x38,
                   0x05, 0x01, 0x00, 0x40, 0x1a, 0x00, 0x00, 0x06, 0x6c, 0x6d, 0x92, 0x21, 0xf3, 0x70, 0x40, 0x20,
                   0x00, 0x00, 0x80, 0x80, 0x00, 0x41, 0x06, 0x80, 0xa0, 0x90, 0x9c, 0x20, 0x08, 0x55, 0x19, 0x40,
                   0x00, 0x00, 0x33, 0xa1, 0xc6, 0xd9, 0x22, 0x40, 0x00, 0x00, 0x20, 0xb8, 0x94, 0x63, 0xc0, 0x09,
                   0x28, 0x44, 0x1b, 0x7e, 0xad, 0x8e, 0x1d, 0x00, 0x9e, 0x2d, 0xa3, 0x0a};

  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer();
  memcpy(pdu->msg, msg, sizeof(msg));
  pdu->N_bytes = sizeof(msg);

  rrc_nr.write_pdu_bcch_dlsch(std::move(pdu));
  task_sched.run_pending_tasks();

  return ISRRAN_SUCCESS;
}

int rrc_nr_setup_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};
  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  uint8_t msg[] = {};

  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer();
  memcpy(pdu->msg, msg, sizeof(msg));
  pdu->N_bytes = sizeof(msg);

  rrc_nr.write_pdu(0, std::move(pdu));
  task_sched.run_pending_tasks();

  return ISRRAN_SUCCESS;
}

int rrc_nr_reconfig_test()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC-NR");
  logger.set_level(isrlog::basic_levels::debug);
  logger.set_hex_dump_max_size(-1);
  isrran::task_scheduler    task_sched{512, 100};
  isrran::task_sched_handle task_sched_handle(&task_sched);
  isrue::rrc_nr             rrc_nr(task_sched_handle);

  dummy_phy     dummy_phy;
  dummy_mac     dummy_mac;
  dummy_rlc     dummy_rlc;
  dummy_pdcp    dummy_pdcp;
  dummy_sdap    dummy_sdap;
  dummy_gw      dummy_gw;
  dummy_nas     dummy_nas;
  dummy_eutra   dummy_eutra;
  dummy_sim     dummy_sim;
  dummy_stack   dummy_stack;
  rrc_nr_args_t rrc_nr_args = {};
  TESTASSERT(rrc_nr.init(&dummy_phy,
                         &dummy_mac,
                         &dummy_rlc,
                         &dummy_pdcp,
                         &dummy_sdap,
                         &dummy_gw,
                         &dummy_nas,
                         &dummy_eutra,
                         &dummy_sim,
                         task_sched.get_timer_handler(),
                         &dummy_stack,
                         rrc_nr_args) == ISRRAN_SUCCESS);

  uint8_t msg[] = {};

  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer();
  memcpy(pdu->msg, msg, sizeof(msg));
  pdu->N_bytes = sizeof(msg);

  rrc_nr.write_pdu(1, std::move(pdu));
  task_sched.run_pending_tasks();

  return ISRRAN_SUCCESS;
}

int main(int argc, char** argv)
{
  isrlog::init();

  TESTASSERT(rrc_nr_cap_request_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nsa_reconfig_tdd_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nsa_reconfig_fdd_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nr_setup_request_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nr_sib1_decoding_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nr_setup_test() == ISRRAN_SUCCESS);
  TESTASSERT(rrc_nr_reconfig_test() == ISRRAN_SUCCESS);

  return ISRRAN_SUCCESS;
}
