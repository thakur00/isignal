...
#include "isrue/hdr/phy/search.h"
...
search::~search()
{
  isrran_ue_mib_sync_free(&ue_mib_sync);
  isrran_ue_cellsearch_free(&cs);
}

void search::init(isrran::rf_buffer_t& buffer_, uint32_t nof_rx_channels, search_callback* parent, int force_N_id_2_, int force_N_id_1_)
{
  p = parent;

  buffer = buffer_;

  if (isrran_ue_cellsearch_init_multi(&cs, 8, radio_recv_callback, nof_rx_channels, parent)) {
    Error("SYNC:  Initiating UE cell search");
  }
  isrran_ue_cellsearch_set_nof_valid_frames(&cs, 4);

  if (isrran_ue_mib_sync_init_multi(&ue_mib_sync, radio_recv_callback, nof_rx_channels, parent)) {
...
{
  isrran_set_detect_cp(&cs, enable);
}

void search::reset()
{
  isrran_ue_sync_reset(&ue_mib_sync.ue_sync);
}

float search::get_last_cfo()
{
  return isrran_ue_sync_get_cfo(&ue_mib_sync.ue_sync);
}

void search::set_agc_enable(bool enable)
{
  if (enable) {
    isrran_rf_info_t* rf_info = p->get_radio()->get_info();
    isrran_ue_sync_start_agc(&ue_mib_sync.ue_sync,
...
search::ret_code search::run(isrran_cell_t* cell_, std::array<uint8_t, ISRRAN_BCH_PAYLOAD_LEN>& bch_payload)
{
  isrran_cell_t new_cell = {};
  ...
  if (force_N_id_2 >= 0 && force_N_id_2 < ISRRAN_NOF_NID_2) {
  ...
  if (force_N_id_1 >= 0 && force_N_id_1 < ISRRAN_NOF_NID_1) {
    /* Note that isrran_ue_cellsearch_scan_N_id_2 only finds the strongest cell for a given N_id_2/PSS within the search
    ...
    if (force_N_id_2 >= 0 && force_N_id_2 < ISRRAN_NOF_NID_2) {
      ...
      if (found_cells[max_peak_cell].cell_id / ISRRAN_NOF_NID_2 == (uint32_t)force_N_id_1) {
        N_id_1_found = true;
      }
    } else {
      // Go through the results for all N_id_2 (PSS); pick strongest with matching N_id_1 (SSS)
      float max_peak_value = -1.0;
      for (uint32_t N_id_2 = 0; N_id_2 < ISRRAN_NOF_NID_2; N_id_2++) {
        if (found_cells[N_id_2].cell_id / ISRRAN_NOF_NID_2 == (uint32_t)force_N_id_1) {
  ...
  Info("SYNC:  PSS/SSS detected: Mode=%s, PCI=%d, CFO=%.1f KHz, CP=%s",
       ...
       new_cell.id,
       cfo / 1000,
       isrran_cp_string(new_cell.cp));

  if (isrran_ue_mib_sync_set_cell(&ue_mib_sync, new_cell)) {
  ...
  /* Find and decode MIB */
  int sfn_offset;
  ret = isrran_ue_mib_sync_decode(&ue_mib_sync, 40, bch_payload.data(), &new_cell.nof_ports, &sfn_offset);
  if (ret == 1) {
    isrran_pbch_mib_unpack(bch_payload.data(), &new_cell, NULL);
    // pack MIB and store inplace for PCAP dump
    std::array<uint8_t, ISRRAN_BCH_PAYLOAD_LEN / 8> mib_packed;
    isrran_bit_pack_vector(bch_payload.data(), mib_packed.data(), ISRRAN_BCH_PAYLOAD_LEN);
    ...
    if (!isrran_cell_isvalid(&new_cell)) {
