...
namespace isrue {
namespace nr {
sync_sa::sync_sa(isrlog::basic_logger& logger_, worker_pool& workers_) :
  logger(logger_), workers(workers_), slot_synchronizer(logger_), searcher(logger_), ta(logger_), isrran::thread("SYNC")
{}
...
bool sync_sa::init(const args_t& args, stack_interface_phy_nr* stack_, isrran::radio_interface_phy* radio_)
{
  stack    = stack_;
  radio    = radio_;
  ...
  rx_buffer = isrran_vec_cf_malloc(2 * slot_sz);
  if (rx_buffer == nullptr) {
...
void sync_sa::run_state_cell_search()
{
  ...
  if (cell_search_nof_trials == 0) {
    isrran_vec_cf_zero(rx_buffer, slot_sz);
  }
  ...
  isrran_vec_cf_copy(rx_buffer, rx_buffer + slot_sz, slot_sz);
...
void sync_sa::run_state_sfn_sync()
{
  ...
  if (slot_synchronizer.run_sfn_sync()) {
    ...
    logger.info("SYNC: SFN synchronised successfully (SFN=%d). Transitioning to IDLE...",
                tti / ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz));
...
void sync_sa::worker_end(const isrran::phy_common_interface::worker_context_t& w_ctx,
    ...
    if (radio->is_continuous_tx()) {
      if (is_pending_tx_end) {
        ...
        if (!radio->get_is_start_of_burst()) {
          isrran::rf_buffer_t zeros_multi;
