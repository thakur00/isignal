...
#include "isrue/hdr/phy/nr/slot_sync.h"

namespace isrue {
namespace nr {

slot_sync::slot_sync(isrlog::basic_logger& logger_) : logger(logger_), sfn_sync_buff(1) {}

slot_sync::~slot_sync()
{
  isrran_ue_sync_nr_free(&ue_sync_nr);
}

static int slot_sync_recv_callback(void* ptr, cf_t** buffer, uint32_t nsamples, isrran_timestamp_t* ts)
{
  if (ptr == nullptr) {
  ...
  cf_t* buffer_ptr[ISRRAN_MAX_CHANNELS] = {};
  buffer_ptr[0]                         = buffer[0];

  isrran::rf_buffer_t rf_buffer(buffer_ptr, nsamples);

  return sync->recv_callback(rf_buffer, ts);
}

bool slot_sync::init(const args_t& args, stack_interface_phy_nr* stack_, isrran::radio_interface_phy* radio_)
{
  stack = stack_;
  radio = radio_;

  isrran_ue_sync_nr_args_t ue_sync_nr_args = {};
  ...
  if (isrran_ue_sync_nr_init(&ue_sync_nr, &ue_sync_nr_args) < ISRRAN_SUCCESS) {
...
int slot_sync::set_sync_cfg(const isrran_ue_sync_nr_cfg_t& cfg)
{
  ...
  if (logger.info.enabled()) {
    ...
    isrran_ssb_cfg_to_str(&cfg.ssb, ssb_cfg_str.data(), (uint32_t)ssb_cfg_str.size());
  ...
  if (isrran_ue_sync_nr_set_cfg(&ue_sync_nr, &cfg) < ISRRAN_SUCCESS) {
...
int slot_sync::recv_callback(isrran::rf_buffer_t& data, isrran_timestamp_t* rx_time)
{
...
bool slot_sync::run_sfn_sync()
{
  ...
  if (isrran_ue_sync_nr_zerocopy(&ue_sync_nr, sfn_sync_buff.to_cf_t(), &outcome) < ISRRAN_SUCCESS) {
  ...
  if (outcome.in_sync) {
    slot_cfg.idx = outcome.sfn * ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz) + outcome.sf_idx;
  }
...
bool slot_sync::run_camping(isrran::rf_buffer_t& buffer, isrran::rf_timestamp_t& timestamp)
{
  // Run UE SYNC process using an external baseband buffer
  isrran_ue_sync_nr_outcome_t outcome = {};
  if (isrran_ue_sync_nr_zerocopy(&ue_sync_nr, buffer.to_cf_t(), &outcome) < ISRRAN_SUCCESS) {
    logger.error("SYNC: error in zerocopy");
    return false;
  }
...
{ // check timestamp reset
  if (forced_rx_time_init || isrran_timestamp_iszero(&stack_tti_ts) ||
    ...
    logger.debug("SYNC:  Setting initial TTI time to %f", isrran_timestamp_real(&stack_tti_ts_new));
...
isrran_slot_cfg_t slot_sync::get_slot_cfg()
{
  return slot_cfg;
}
