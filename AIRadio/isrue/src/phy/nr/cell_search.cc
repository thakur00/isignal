...
#include "isrue/hdr/phy/nr/cell_search.h"
...
namespace isrue {
namespace nr {
cell_search::cell_search(isrlog::basic_logger& logger_) : logger(logger_) {}
...
cell_search::ret_t cell_search::run_slot(const cf_t* buffer, uint32_t slot_sz)
{
  ...
  // Search for SSB
  if (isrran_ssb_search(&ssb, buffer, slot_sz + ssb.ssb_sz, &ret.ssb_res) < ISRRAN_SUCCESS) {
