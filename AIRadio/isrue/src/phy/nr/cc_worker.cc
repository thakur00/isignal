...
#include "isrue/hdr/phy/nr/cc_worker.h"
...
cc_worker::cc_worker(uint32_t cc_idx_, isrlog::basic_logger& log, state& phy_state_, const isrran::phy_cfg_nr_t& cfg) :
  cc_idx(cc_idx_), phy(phy_state_), cfg(cfg), logger(log)
{
  cf_t* rx_buffer_c[ISRRAN_MAX_PORTS] = {};

  // Allocate buffers
  buffer_sz = ISRRAN_SF_LEN_PRB(phy.args.dl.nof_max_prb) * 5;
  for (uint32_t i = 0; i < phy.args.dl.nof_rx_antennas; i++) {
    rx_buffer[i]   = isrran_vec_cf_malloc(buffer_sz);
    rx_buffer_c[i] = rx_buffer[i];
    tx_buffer[i]   = isrran_vec_cf_malloc(buffer_sz);
  }

  if (isrran_ue_dl_nr_init(&ue_dl, rx_buffer.data(), &phy.args.dl) < ISRRAN_SUCCESS) {
    ERROR("Error initiating UE DL NR");
    return;
  }

  if (isrran_ue_ul_nr_init(&ue_ul, tx_buffer[0], &phy.args.ul) < ISRRAN_SUCCESS) {
    ERROR("Error initiating UE DL NR");
    return;
  }
...
void cc_worker::update_cfg(const isrran::phy_cfg_nr_t& new_config)
{
  cfg        = new_config;
  configured = false;

  if (isrran_ue_dl_nr_set_carrier(&ue_dl, &cfg.carrier) < ISRRAN_SUCCESS) {
    ERROR("Error setting carrier");
    return;
  }

  if (isrran_ue_ul_nr_set_carrier(&ue_ul, &cfg.carrier) < ISRRAN_SUCCESS) {
    ERROR("Error setting carrier");
    return;
  }

  isrran_dci_cfg_nr_t dci_cfg = cfg.get_dci_cfg();
  if (isrran_ue_dl_nr_set_pdcch_config(&ue_dl, &cfg.pdcch, &dci_cfg) < ISRRAN_SUCCESS) {
    logger.error("Error setting NR PDCCH configuration");
    return;
  }

  isrran_ssb_cfg_t ssb_cfg = cfg.get_ssb_cfg();
  ssb_cfg.srate_hz         = isrran_min_symbol_sz_rb(cfg.carrier.nof_prb) * ISRRAN_SUBC_SPACING_NR(cfg.carrier.scs);
...
{
  std::array<isrran_dci_dl_nr_t, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR> dci_rx = {};
  isrue::mac_interface_phy_nr::sched_rnti_t rnti = phy.stack->get_dl_sched_rnti_nr(dl_slot_cfg.idx);

  // Skip search if no valid RNTI is given
  if (rnti.id == ISRRAN_INVALID_RNTI) {
    return;
  }

  // Search for grants
  int n_dl =
      isrran_ue_dl_nr_find_dl_dci(&ue_dl, &dl_slot_cfg, rnti.id, rnti.type, dci_rx.data(), (uint32_t)dci_rx.size());
  if (n_dl < ISRRAN_SUCCESS) {
    logger.error("Error decoding DL NR-PDCCH for %s=0x%x", isrran_rnti_type_str(rnti.type), rnti.id);
    return;
  }
  ...
  for (int i = 0; i < n_dl; i++) {
    ...
    if (logger.info.enabled()) {
      ...
      isrran_dci_dl_nr_to_str(&ue_dl.dci, &dci_rx[i], str.data(), str.size());
      logger.info("PDCCH: cc=%d, %s", cc_idx, str.data());
    }

    if (logger.debug.enabled()) {
      // log coreset info
      isrran_coreset_t*     coreset = &ue_dl.cfg.coreset[dci_rx[i].ctx.coreset_id];
      std::array<char, 512> coreset_str;
      isrran_coreset_to_str(coreset, coreset_str.data(), coreset_str.size());
  ...
  if (logger.debug.enabled()) {
    for (uint32_t i = 0; i < ue_dl.pdcch_info_count; i++) {
      const isrran_ue_dl_nr_pdcch_info_t* info    = &ue_dl.pdcch_info[i];
      std::array<char, 512>               dci_ctx = {};
      isrran_dci_ctx_to_str(&info->dci_ctx, dci_ctx.data(), (uint32_t)dci_ctx.size());
...
{
  std::array<isrran_dci_ul_nr_t, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR> dci_rx = {};
  isrue::mac_interface_phy_nr::sched_rnti_t rnti = phy.stack->get_ul_sched_rnti_nr(ul_slot_cfg.idx);
  ...
  // Search for grants
  int n_ul =
      isrran_ue_dl_nr_find_ul_dci(&ue_dl, &dl_slot_cfg, rnti.id, rnti.type, dci_rx.data(), (uint32_t)dci_rx.size());
  if (n_ul < ISRRAN_SUCCESS) {
  ...
  for (int i = 0; i < n_ul; i++) {
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str;
      isrran_dci_ul_nr_to_str(&ue_dl.dci, &dci_rx[i], str.data(), str.size());
      logger.info("PDCCH: cc=%d, %s", cc_idx, str.data());
    }
...
bool cc_worker::decode_pdsch_dl()
{
  ...
  if (not dl_action.tb.enabled) {
    // Force positive ACK
    if (pdsch_cfg.grant.rnti_type == isrran_rnti_type_c) {
  ...
  // Get data buffer
  isrran::unique_byte_buffer_t data = isrran::make_byte_buffer();
  if (data == nullptr) {
  ...
  if (isrran_ue_dl_nr_decode_pdsch(&ue_dl, &dl_slot_cfg, &pdsch_cfg, &pdsch_res) < ISRRAN_SUCCESS) {
  ...
  if (logger.info.enabled()) {
    str_info_t str;
    isrran_ue_dl_nr_pdsch_info(&ue_dl, &pdsch_cfg, &pdsch_res, str.data(), (uint32_t)str.size());

    if (logger.debug.enabled()) {
      str_extra_t str_extra;
      isrran_sch_cfg_nr_info(&pdsch_cfg, str_extra.data(), (uint32_t)str_extra.size());
  ...
  if (not pdsch_res.tb[0].crc and phy.args.store_pdsch_ko) {
    ...
    isrran_filesink_t filesink = {};
    if (isrran_filesink_init(&filesink, (char*)isrran::to_c_str(filename), ISRRAN_COMPLEX_FLOAT_BIN) == 0) {
      isrran_filesink_write(&filesink, (void*)rx_buffer[0], ue_dl.fft[0].sf_sz);
      isrran_filesink_free(&filesink);
...
{
  // Measure SSB CSI
  if (isrran_ssb_send(&ssb, dl_slot_cfg.idx)) {
    isrran_csi_trs_measurements_t meas = {};

    // Iterate all possible candidates
    const std::array<bool, ISRRAN_SSB_NOF_CANDIDATES>& position_in_burst = cfg.ssb.position_in_burst;
    for (uint32_t ssb_idx = 0; ssb_idx < ISRRAN_SSB_NOF_CANDIDATES; ssb_idx++) {
      ...
      if (isrran_ssb_csi_measure(&ssb, cfg.carrier.pci, ssb_idx, rx_buffer[0], &meas) < ISRRAN_SUCCESS) {
      ...
      if (logger.debug.enabled()) {
        std::array<char, 512> str = {};
        isrran_csi_meas_info(&meas, str.data(), (uint32_t)str.size());
        logger.debug("SSB-CSI: %s", str.data());
      }

      bool                 hrf      = (dl_slot_cfg.idx % ISRRAN_NSLOTS_PER_FRAME_NR(cfg.carrier.scs) >
                  ISRRAN_NSLOTS_PER_FRAME_NR(cfg.carrier.scs) / 2);
      isrran_pbch_msg_nr_t pbch_msg = {};
      if (isrran_ssb_decode_pbch(&ssb, cfg.carrier.pci, hrf, ssb_idx, rx_buffer[0], &pbch_msg) < ISRRAN_SUCCESS) {
      ...
      if (pbch_msg.crc) {
        ...
        if (mib.sfn != dl_slot_cfg.idx / ISRRAN_NSLOTS_PER_FRAME_NR(cfg.carrier.scs)) {
          logger.error("PBCH-MIB: NR SFN (%d) does not match current SFN (%d)",
                       mib.sfn,
                       dl_slot_cfg.idx / ISRRAN_NSLOTS_PER_FRAME_NR(cfg.carrier.scs));
          dl_slot_cfg.idx = mib.sfn * ISRRAN_NSLOTS_PER_FRAME_NR(cfg.carrier.scs);
        }

        // Log MIB information
        if (logger.debug.enabled()) {
          std::array<char, 512> str = {};
          isrran_pbch_msg_nr_mib_info(&mib, str.data(), (uint32_t)str.size());
  ...
  for (uint32_t resource_set_id = 0; resource_set_id < ISRRAN_PHCH_CFG_MAX_NOF_CSI_RS_SETS; resource_set_id++) {
    // Select NZP-CSI-RS set
    const isrran_csi_rs_nzp_set_t& nzp_set = cfg.pdsch.nzp_csi_rs_sets[resource_set_id];
    ...
    if (not estimate_fft) {
      isrran_ue_dl_nr_estimate_fft(&ue_dl, &dl_slot_cfg);
      estimate_fft = true;
    }

    // Perform measurement, n > 0 is any measurement is performed, n = 0 otherwise
    isrran_csi_trs_measurements_t trs_measurements = {};
    int n = isrran_ue_dl_nr_csi_measure_trs(&ue_dl, &dl_slot_cfg, &nzp_set, &trs_measurements);
    if (n < ISRRAN_SUCCESS) {
    ...
    if (logger.debug.enabled()) {
      std::array<char, 512> str = {};
      isrran_csi_meas_info(&trs_measurements, str.data(), (uint32_t)str.size());
  ...
  for (uint32_t resource_set_id = 0; resource_set_id < ISRRAN_PHCH_CFG_MAX_NOF_CSI_RS_SETS; resource_set_id++) {
    ...
    isrran_csi_channel_measurements_t measurements = {};
    int n = isrran_ue_dl_nr_csi_measure_channel(&ue_dl, &dl_slot_cfg, &nzp_set, &measurements);
    if (n < ISRRAN_SUCCESS) {
      logger.error("Error measuring CSI-RS");
      return false;
    }
...
bool cc_worker::work_dl()
{
  ...
  if (!isrran_duplex_nr_is_dl(&cfg.duplex, 0, dl_slot_cfg.idx)) {
    return true;
  }
  ...
  if (std::isnormal(phy.args.enable_worker_cfo)) {
    ...
    for (cf_t* b : rx_buffer) {
      if (b != nullptr and ue_ul.ifft.sf_sz != 0) {
        isrran_vec_apply_cfo(b, dl_cfo_norm, b, ue_ul.ifft.sf_sz);
      }
    }
  }
...
bool cc_worker::work_ul()
{
  ...
  isrran_pdsch_ack_nr_t pdsch_ack  = {};
  bool                  has_ul_ack = phy.get_pending_ack(ul_slot_cfg.idx, pdsch_ack);

  // Check if it is a UL slot, if not skip
  if (!isrran_duplex_nr_is_ul(&cfg.duplex, 0, ul_slot_cfg.idx)) {
    // No NR signal shall be transmitted
    isrran_vec_cf_zero(tx_buffer[0], ue_ul.ifft.sf_sz);
  ...
  if (has_ul_ack) {
    ...
    if (logger.debug.enabled()) {
      std::array<char, 512> str = {};
      if (isrran_harq_ack_info(&pdsch_ack, str.data(), (uint32_t)str.size()) > 0) {
        logger.debug("%s", str.data());
      }
    }

    if (isrran_harq_ack_pack(&cfg.harq_ack, &pdsch_ack, &uci_data) < ISRRAN_SUCCESS) {
  ...
  isrran_ue_ul_nr_set_freq_offset(&ue_ul, phy.get_ul_cfo());
  ...
  if (has_pusch_grant) {
    ...
    mac_ul_grant.is_rar_grant                            = (pusch_cfg.grant.rnti_type == isrran_rnti_type_ra);
    ...
    isrran_ra_ul_set_grant_uci_nr(&cfg.carrier, &cfg.pusch, &uci_data.cfg, &pusch_cfg);
    ...
    isrran_pusch_data_nr_t data = {};
    data.payload[0]             = ul_action.tb.payload->msg;
    data.uci                    = uci_data.value;

    // Encode PUSCH transmission
    if (isrran_ue_ul_nr_encode_pusch(&ue_ul, &ul_slot_cfg, &pusch_cfg, &data) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      str_info_t str;
      isrran_ue_ul_nr_pusch_info(&ue_ul, &pusch_cfg, &data.uci, str.data(), str.size());

      if (logger.debug.enabled()) {
        str_extra_t str_extra;
        isrran_sch_cfg_nr_info(&pusch_cfg, str_extra.data(), (uint32_t)str_extra.size());
    ...
    ul_m.power        = isrran_convert_power_to_dB(isrran_vec_avg_power_cf(tx_buffer[0], ue_ul.ifft.sf_sz));
    phy.set_ul_metrics(ul_m);

  } else if (isrran_uci_nr_total_bits(&uci_data.cfg) > 0) {
    // Get PUCCH resource
    isrran_pucch_nr_resource_t resource = {};
    if (isrran_ra_ul_nr_pucch_resource(&cfg.pucch, &uci_data.cfg, cfg.carrier.nof_prb, &resource) < ISRRAN_SUCCESS) {
      ERROR("Selecting PUCCH resource");
      return false;
    }

    // Encode PUCCH message
    if (isrran_ue_ul_nr_encode_pucch(&ue_ul, &ul_slot_cfg, &cfg.pucch.common, &resource, &uci_data) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str;
      isrran_ue_ul_nr_pucch_info(&resource, &uci_data, str.data(), str.size());
...
{
  uint32_t nof_re = ue_dl.carrier.nof_prb * ISRRAN_NRE * 12;
  isrran_vec_cf_copy(pdsch_d, ue_dl.pdsch.d[0], nof_re);
  return nof_re;
}
