...
#include "isrue/hdr/phy/nr/sf_worker.h"
...
static void      init_plots(isrue::nr::sf_worker* worker);
...
sf_worker::sf_worker(isrran::phy_common_interface& common_,
                     state&                        phy_state_,
                     const isrran::phy_cfg_nr_t&   cfg,
                     isrlog::basic_logger&         log) :
  phy_state(phy_state_), common(common_), logger(log), sf_len(ISRRAN_SF_LEN_PRB_NR(cfg.carrier.nof_prb))
{
...
void sf_worker::update_cfg(uint32_t cc_idx, const isrran::phy_cfg_nr_t& new_cfg)
{
...
void sf_worker::start_plot()
{
  ...
  if (plot_worker_id == -1) {
    ...
    isrran::console("Starting NR plot for worker_id=%d\n", plot_worker_id);
    init_plots(this);
  } else {
...
static cf_t tmp_pconst_nr[ISRRAN_NSYMB_PER_SLOT_NR * ISRRAN_NRE * ISRRAN_MAX_PRB_NR] = {};
extern bool plot_quit;

static void* plot_thread_run(void* arg)
{
  auto worker         = (isrue::nr::sf_worker*)arg;
