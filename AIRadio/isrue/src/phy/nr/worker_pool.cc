...
worker_pool::worker_pool(isrlog::basic_logger& logger_, uint32_t max_workers) : pool(max_workers), logger(logger_) {}

bool worker_pool::init(const phy_args_nr_t& args, isrran::phy_common_interface& common, stack_interface_phy_nr* stack_)
{
  ...
  for (uint32_t i = 0; i < args.nof_phy_threads; i++) {
    auto& log = isrlog::fetch_basic_logger(fmt::format("{}PHY{}-NR", args.log.id_preamble, i));
    log.set_level(isrlog::str_to_basic_level(args.log.phy_level));
  ...
  // Set PHY loglevel
  logger.set_level(isrlog::str_to_basic_level(args.log.phy_level));
...
sf_worker* worker_pool::wait_worker(uint32_t tti)
{
  ...
  if (prach_buffer->is_ready_to_send(tti, pci)) {
    ...
    // Scale signal to maximum
    {
      ...
      int    max_i = isrran_vec_max_abs_fi(ptr, 2 * sf_sz);
      float  max   = ptr[max_i];
      if (std::isnormal(max)) {
        isrran_vec_sc_prod_cfc(prach_ptr, 0.99f / max, prach_ptr, sf_sz * prach_nof_sf);
      }
    }

    uint32_t                    config_idx = 0;
    isrran_duplex_mode_t        mode       = ISRRAN_DUPLEX_MODE_FDD;
    isrran_subcarrier_spacing_t scs        = isrran_subcarrier_spacing_15kHz;
    {
      std::lock_guard<std::mutex> lock(cfg_mutex);
    ...
    phy_state.stack->prach_sent(
        TTI_TX(tti), isrran_prach_nr_start_symbol(config_idx, mode), ISRRAN_SLOT_NR_MOD(scs, TTI_TX(tti)), 0, 0);
  }
                               ...
                               isrran_rnti_type_t                             rnti_type)
{
  ...
  dci_msg.ctx.format          = isrran_dci_format_nr_rar; // MAC RAR grant shall be unpacked as DCI 0_0 format
  dci_msg.ctx.rnti_type       = rnti_type;
  dci_msg.ctx.ss_type         = isrran_search_space_type_rar; // This indicates it is a MAC RAR
  dci_msg.ctx.rnti            = rnti;
  dci_msg.nof_bits            = ISRRAN_RAR_UL_GRANT_NBITS;
  isrran_vec_u8_copy(dci_msg.payload, packed_ul_grant.data(), ISRRAN_RAR_UL_GRANT_NBITS);

  isrran_dci_ul_nr_t dci_ul = {};
  if (isrran_dci_nr_ul_unpack(NULL, &dci_msg, &dci_ul) < ISRRAN_SUCCESS) {
    logger.error("Couldn't unpack UL grant");
    return ISRRAN_ERROR;
  }

  // initialize with Rx TTI of RAR
  isrran_slot_cfg_t msg3_slot_cfg = {};
  msg3_slot_cfg.idx               = rar_slot_idx;

  if (logger.info.enabled()) {
    std::array<char, 512> str;
    isrran_dci_nr_t       dci = {};
    isrran_dci_ul_nr_to_str(&dci, &dci_ul, str.data(), str.size());
...
bool worker_pool::set_config(const isrran::phy_cfg_nr_t& new_cfg)
{
  uint32_t dl_arfcn = isrran::isrran_band_helper().freq_to_nr_arfcn(new_cfg.carrier.dl_center_frequency_hz);
  sf_sz             = ISRRAN_SF_LEN_PRB_NR(new_cfg.carrier.nof_prb);
  ...
  isrran_cell_t cell = {};
  int           ret  = isrran_carrier_to_cell(&new_cfg.carrier, &cell);
  ...
  isrran_prach_cfg_t prach_cfg           = new_cfg.prach;
  ...
  prach_cfg.tdd_config.configured = (new_cfg.duplex.mode == ISRRAN_DUPLEX_MODE_TDD);
