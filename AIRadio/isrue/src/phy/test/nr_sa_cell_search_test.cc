...
struct args_t {
  ...
  isrran_carrier_nr_t         base_carrier    = ISRRAN_DEFAULT_CARRIER_NR;
  isrran_ssb_pattern_t        ssb_pattern     = ISRRAN_SSB_PATTERN_A;
  isrran_subcarrier_spacing_t ssb_scs         = isrran_subcarrier_spacing_15kHz;
  isrran_duplex_mode_t        duplex_mode     = ISRRAN_DUPLEX_MODE_FDD;
  ...
  void set_ssb_from_band()
  {
    ...
    isrran_assert(band != UINT16_MAX, "Invalid band");
    ...
    isrran_assert(pointA_arfcn != 0, "Invalid frequency");
    ...
    isrran_assert(ssb_arfcn, "Invalid SSB center frequency");
...
public:
  struct args_t {
    isrue::phy_args_nr_t   phy;
    ue_dummy_stack::args_t stack;
  };
  dummy_ue(const args_t& args, isrran::radio_interface_phy* radio) : stack(args.stack, phy), phy("PHY")
  {
    isrran_assert(phy.init(args.phy, &stack, radio) == ISRRAN_SUCCESS, "Failed to initialise PHY");
    phy.wait_initialize();
  }

  bool cell_search_read_and_clear() { return stack.get_cell_search_finished(); }

  bool start_cell_search(const isrue::phy_interface_stack_nr::cell_search_args_t& args)
  {
    return phy.start_cell_search(args);
  }

  bool start_cell_select(const isrue::phy_interface_stack_nr::cell_select_args_t& args)
  {
...
static cell_search_result_t cell_search(const args_t& args, dummy_ue& ue)
{
  ...
  isrue::phy_nr_sa::cell_search_args_t cs_args = {};
  ...
  double   ssb_bw_hz              = ISRRAN_SSB_BW_SUBC * bands.get_ssb_scs(band);
  ...
  uint32_t ssb_scs_hz             = ISRRAN_SUBC_SPACING_NR(args.ssb_scs);

  // Get sync raster
  isrran::isrran_band_helper::sync_raster_t ss = bands.get_sync_raster(band, args.ssb_scs);
  isrran_assert(ss.valid(), "Invalid synchronization raster");
  ...
  while (not ss.end()) {
    ...
    isrran_assert(ue.start_cell_search(cs_args), "Failed cell search start");
    ...
    for (auto& pci : metrics.cell_search) {
      ...
      for (auto& ssb : pci.second) {
        ...
        if (not ret.found) {
          ...
          ret.pci             = pci.first;
          isrran_assert(isrran_pbch_msg_nr_mib_unpack(&ssb.second.last_result.pbch_msg, &ret.mib) == ISRRAN_SUCCESS,
                        "Error unpacking MIB");
        }
      }
    }
...
int main(int argc, char** argv)
{
  ...
  args_t args;
  isrran_assert(parse_args(argc, argv, args) == ISRRAN_SUCCESS, "Failed to parse arguments");

  // Initialise logging infrastructure
  isrlog::init();

  // Radio can be constructed from different options
  std::shared_ptr<isrran::radio_interface_phy> radio = nullptr;
  ...
  if (not args.sim_cell_pci.empty()) {
    ...
    // Instantiate
    std::shared_ptr<isrran::radio> r = std::make_shared<isrran::radio>();
    isrran_assert(r->init(rf_args, nullptr) == ISRRAN_SUCCESS, "Failed Radio initialisation");
  ...
  // Perform cell select
  if (found_cell.found) {
    isrue::phy_interface_stack_nr::cell_select_args_t cs_args = {};
    ...
    isrran_assert(ue.start_cell_select(cs_args), "Failed to start cell selection\n");
