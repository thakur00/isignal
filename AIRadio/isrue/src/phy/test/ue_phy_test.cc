...
class phy_test_bench : public isrran::thread
{
private:
  // Dummy classes
  class dummy_stack final : public isrue::stack_interface_phy_lte
  {
  private:
  ...
  public:
    ...
    dummy_stack() : logger(isrlog::fetch_basic_logger("stack", false)) {}
    ...
    void new_cell_meas(const std::vector<isrue::phy_meas_t>& meas) override
    {
      for (auto& m : meas) {
        notify_new_phy_meas();
    ...
    void cell_search_complete(cell_search_ret_t ret, isrue::phy_cell_t found_cell) override
    {
      cell_search_ret = ret;
    ...
    isrue::phy_cell_t last_found_cell  = {};
    bool              last_cell_select = false, last_config = false, last_scell = false;
  };

  class dummy_radio : public isrran::radio_interface_phy
  {
  private:
    isrlog::basic_logger&            logger;
    std::vector<isrran_ringbuffer_t> ring_buffers;
    ...
    isrran::rf_timestamp_t           tx_last_tx = {};
  ...
  public:
    dummy_radio(uint32_t nof_channels, float base_srate_) :
      logger(isrlog::fetch_basic_logger("radio", false)), ring_buffers(nof_channels), base_srate(base_srate_)
    {
      // Create Ring buffers
      for (auto& rb : ring_buffers) {
        if (isrran_ringbuffer_init(&rb, (uint32_t)sizeof(cf_t) * ISRRAN_SF_LEN_MAX * ISRRAN_NOF_SF_X_FRAME)) {
          perror("init softbuffer");
        }
      }

      // Create temporal buffer
      temp_buffer = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX * ISRRAN_NOF_SF_X_FRAME);
      if (!temp_buffer) {
        perror("malloc");
      }
    ...
    void write_ring_buffers(cf_t** buffer, uint32_t nsamples)
    {
      for (uint32_t i = 0; i < ring_buffers.size(); i++) {
        ...
        do {
          ...
          ret = isrran_ringbuffer_write_timed(&ring_buffers[i], buffer[i], (uint32_t)sizeof(cf_t) * nsamples, 1000);
        } while (ret == ISRRAN_ERROR_TIMEOUT);
      }
    }
    ...
    bool tx(isrran::rf_buffer_interface& buffer, const isrran::rf_timestamp_interface& tx_time) override
    {
      ...
      if (isrran_timestamp_compare(&tx_time.get(0), tx_last_tx.get_ptr(0)) < 0) {
        ret = false;
      }
    ...
    bool rx_now(isrran::rf_buffer_interface& buffer, isrran::rf_timestamp_interface& rxd_time) override
    {
      ...
      for (uint32_t i = 0; i < ring_buffers.size(); i++) {
        ...
        int ret = isrran_ringbuffer_read(&ring_buffers[i], buf_ptr, (uint32_t)sizeof(cf_t) * base_nsamples);
        if (ret < 0) {
      ...
      // Set Rx timestamp
      isrran_timestamp_init_uint64(rxd_time.get_ptr(0), rx_timestamp, (double)base_srate);
    ...
    {
      std::unique_lock<std::mutex> lock(mutex);
      return isrran_convert_amplitude_to_dB(rx_gain);
    }
  ...
  // Phy Instances
  std::unique_ptr<isrue::phy> phy;

  // eNb
  isrran_enb_dl_t    enb_dl                          = {};
  cf_t*              enb_dl_buffer[ISRRAN_MAX_PORTS] = {};
...
public:
  phy_test_bench(const isrue::phy_args_t& phy_args, const isrran_cell_t& cell) :
    radio(cell.nof_ports, isrran_sampling_freq_hz(cell.nof_prb)),
    thread("phy_test_bench"),
    logger(isrlog::fetch_basic_logger("test bench", false))
  {
    // Deduce physical attributes
    sf_len = ISRRAN_SF_LEN_PRB(cell.nof_prb);

    // Initialise UE
    phy = std::unique_ptr<isrue::phy>(new isrue::phy);
    ...
    for (uint32_t i = 0; i < cell.nof_ports; i++) {
      enb_dl_buffer[i] = isrran_vec_cf_malloc(sf_len);
      if (!enb_dl_buffer[i]) {
        perror("malloc");
      }
    }
    ...
    // Initialise eNb DL
    isrran_enb_dl_init(&enb_dl, enb_dl_buffer, ISRRAN_MAX_PRB);
    isrran_enb_dl_set_cell(&enb_dl, cell);
  }
  ...
  isrue::phy_interface_rrc_lte* get_phy_interface_rrc() { return phy.get(); }
  isrue::phy_interface_mac_lte* get_phy_interface_mac() { return phy.get(); }

  void configure_dedicated(uint16_t rnti, isrran::phy_cfg_t& phy_cfg)
  {
  ...
  void run_thread() override
  {
    ...
    do {
      ...
      isrran_enb_dl_put_base(&enb_dl, &dl_sf_cfg);
  ...
  {
    logger.set_level(isrlog::str_to_basic_level(str));
...
int main(int argc, char** argv)
{
  ...
  TESTASSERT(phy_test->get_stack()->cell_search_ret.found ==
             isrue::rrc_interface_phy_lte::cell_search_ret_t::CELL_FOUND);

  // 2. Cell select
  isrue::phy_cell_t phy_cell = phy_test->get_stack()->last_found_cell;
  ...
  phy_cfg       = {};
  phy_cfg.set_defaults();
  phy_cfg.dl_cfg.cqi_report.periodic_mode       = ISRRAN_CQI_MODE_12;
