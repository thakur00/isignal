...
#include "isrran/isrlog/bundled/fmt/chrono.h"
#include "isrran/isrlog/isrlog.h"
#include "isrue/hdr/phy/scell/intra_measure_lte.h"
...
static isrran_cell_t     cell_base = {.nof_prb         = 6,
                                  .nof_ports       = 1,
                                  .id              = 0,
                                  .cp              = ISRRAN_CP_NORM,
                                  .phich_length    = ISRRAN_PHICH_NORM,
                                  .phich_resources = ISRRAN_PHICH_R_1_6,
...
static isrue::phy_args_t phy_args;
...
static isrran_tm_t serving_cell_pdsch_tm;
...
{
  uint32_t mask = 0;
  auto     nb   = (uint32_t)ceilf((float)cell_base.nof_prb / isrran_ra_type0_P(cell_base.nof_prb));
...
private:
  ...
  isrran::channel_ptr   channel;
  cf_t*                 signal_buffer[ISRRAN_MAX_PORTS] = {};
  isrlog::basic_logger& logger;

public:
  test_enb(const isrran_cell_t& cell, const isrran::channel::args_t& channel_args) :
    enb_dl(), logger(isrlog::fetch_basic_logger("Channel pci=" + std::to_string(cell.id)))
  {
    logger.set_level(isrlog::str_to_basic_level(channel_log_level));

    channel = isrran::channel_ptr(new isrran::channel(channel_args, cell_base.nof_ports, logger));
    channel->set_srate(isrran_sampling_freq_hz(cell.nof_prb));

    // Allocate buffer for eNb
    for (uint32_t i = 0; i < cell_base.nof_ports; i++) {
      signal_buffer[i] = isrran_vec_cf_malloc(ISRRAN_SF_LEN_PRB(cell_base.nof_prb));
      if (!signal_buffer[i]) {
           ...
           const isrran::rf_timestamp_t& ts)
  {
    ...
    if (dci && dci_cfg && softbuffer_tx && data_tx) {
      ...
      if (isrran_ra_dl_dci_to_grant(&enb_dl.cell, dl_sf, serving_cell_pdsch_tm, false, dci, &pdsch_cfg.grant)) {
      ...
      pdsch_cfg.p_b          = (serving_cell_pdsch_tm > ISRRAN_TM1) ? 1 : 0; // 0 dB
    ...
    for (uint32_t i = 1; i < enb_dl.cell.nof_ports; i++) {
      isrran_vec_sum_ccc(signal_buffer[0], signal_buffer[i], signal_buffer[0], sf_len);
    }

    // Undo isrran_enb_dl_gen_signal scaling
    ...
    if (enb_dl.cell.id != *pcis_to_simulate.begin()) {
      scale *= isrran_convert_dB_to_amplitude(-ncell_attenuation_dB);
    }

    // Scale signal
    isrran_vec_sc_prod_cfc(signal_buffer[0], scale, signal_buffer[0], sf_len);

    // Add signal to baseband buffer
    isrran_vec_sum_ccc(signal_buffer[0], baseband_buffer, baseband_buffer, sf_len);
...
/// Report context.
using report_context_t = isrlog::build_context_type<metric_timestamp_tag, mlist_cell>;

class meas_itf_listener : public isrue::scell::intra_measure_base::meas_itf
{
  isrlog::log_channel& json_channel;
...
public:
  ...
  explicit meas_itf_listener(isrlog::log_channel& json_channel) : json_channel(json_channel) {}
  ...
  bool print_stats()
  {
    ...
    ctx.write<metric_timestamp_tag>(isrran::to_c_str(ts_buffer));
...
int parse_args(int argc, char** argv)
{
  ...
  common.add_options()
      ...
      ("phy_lib_log_level",         bpo::value<int>(&phy_lib_log_level)->default_value(ISRRAN_VERBOSE_NONE),       "Phy lib log level (0: none, 1: info, 2: debug)")
  ...
  simulation.add_options()
      ...
      ("serving_cell_pdsch_tm",     bpo::value<int>((int*) &serving_cell_pdsch_tm)->default_value(ISRRAN_TM1),     "Simulated Transmission mode 0: TM1, 1: TM2, 2: TM3, 3: TM4")
...
int main(int argc, char** argv)
{
  ...
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("intra_measure");
  isrlog::sink& json_sink = isrlog::fetch_file_sink(json_report_filename, 0, false, isrlog::create_json_formatter());
  isrlog::log_channel& json_channel = isrlog::fetch_log_channel("JSON_channel", json_sink, {});
  json_channel.set_enabled(enable_json_report);
  isrlog::init();

  cf_t*                           baseband_buffer = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX);
  isrran::rf_timestamp_t          ts              = {};
  meas_itf_listener               rrc(json_channel);
  isrue::scell::intra_measure_lte intra_measure(logger, rrc);
  ...
  set_isrran_verbose_level(phy_lib_log_level);

  // Allocate PDSCH data and tx-soft-buffers only if pdsch is enabled and radio is not available
  for (int i = 0; i < ISRRAN_MAX_TB && serving_cell_pdsch_enable && radio == nullptr; i++) {
    isrran_random_t random_gen = isrran_random_init(serving_cell_pdsch_rnti);
    ...
    if (isrran_softbuffer_tx_init(softbuffer_tx[i], cell_base.nof_prb)) {
      ERROR("Error initiating softbuffer_tx");
    }

    data_tx[i] = isrran_vec_u8_malloc(nof_bytes);
    if (!data_tx[i]) {
      ERROR("Error allocating data tx");
    } else {
      for (uint32_t j = 0; j < nof_bytes; j++) {
        data_tx[i][j] = (uint8_t)isrran_random_uniform_int_dist(random_gen, 0, 255);
      }
    }
  ...
  logger.set_level(isrlog::str_to_basic_level(intra_meas_log_level));

  isrue::scell::intra_measure_base::args_t args = {};
  ...
  intra_measure.set_primary_cell(ISRRAN_MAX(earfcn_dl, 0), cell_base);

  if (earfcn_dl >= 0) {
    // Create radio log
    auto& radio_logger = isrlog::fetch_basic_logger("RF", false);
    radio_logger.set_level(isrlog::str_to_basic_level(radio_log_level));
    ...
    // Set sampling rate
    radio->set_rx_srate(isrran_sampling_freq_hz(cell_base.nof_prb));

    // Set frequency
    radio->set_rx_freq(0, isrran_band_fd(earfcn_dl) * 1e6);
    ...
    for (const auto& pci : pcis_to_simulate) {
      // Initialise cell
      isrran_cell_t cell = cell_base;
      cell.id            = pci;

      // Initialise channel and push back
      isrran::channel::args_t channel_args;
      ...
      channel_args.awgn_signal_power_dBfs = isrran_enb_dl_get_maximum_signal_power_dBfs(cell.nof_prb);
  ...
  for (uint32_t sf_idx = 0; sf_idx < duration_execution_s * 1000; sf_idx++) {
    ...
    // Clean buffer
    isrran_vec_cf_zero(baseband_buffer, ISRRAN_SF_LEN_MAX);

    if (radio) {
      // Receive radio
      isrran::rf_buffer_t radio_buffer(baseband_buffer, ISRRAN_SF_LEN_PRB(cell_base.nof_prb));
      ...
      for (auto& enb : test_enb_v) {
        if (put_pdsch) {
          ...
          prbset_num                  = (int)ceilf((float)cell_base.nof_prb / isrran_ra_type0_P(cell_base.nof_prb));
          ...
          // Set TB
          if (serving_cell_pdsch_tm < ISRRAN_TM3) {
            ...
            dci.tb[1].mcs_idx = 0;
            dci.tb[1].rv      = 1;
          } else if (serving_cell_pdsch_tm == ISRRAN_TM3) {
          ...
          } else if (serving_cell_pdsch_tm == ISRRAN_TM4) {
    ...
    intra_measure.run_tti(sf_idx % 10240, baseband_buffer, ISRRAN_SF_LEN_PRB(cell_base.nof_prb));
    if (sf_idx % 1000 == 0) {
  ...
  ret = rrc.print_stats() ? ISRRAN_SUCCESS : ISRRAN_ERROR;
