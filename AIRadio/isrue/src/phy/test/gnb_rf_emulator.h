...
#ifndef ISRRAN_GNB_RF_EMULATOR_H
#define ISRRAN_GNB_RF_EMULATOR_H
...
class gnb_rf_emulator final : public isrran::radio_interface_phy
{
private:
  ...
  isrran::rf_timestamp_t                      ts_write       = {};
  ...
  void run_async_slot()
  {
    ...
    for (std::shared_ptr<gnb_emulator>& gnb : gnb_vector) {
      isrran_assert(gnb->work(slot_idx, buffer.data(), ts_write) == ISRRAN_SUCCESS, "Failed to run gNb emulator");
    }

    // Write slot samples in ringbuffer
    isrran_assert(isrran_ringbuffer_write(&ringbuffer, buffer.data(), (int)sizeof(cf_t) * sf_len) > ISRRAN_SUCCESS,
...
public:
  struct args_t {
    double                      srate_hz;
    isrran_carrier_nr_t         base_carrier;
  ...
  gnb_rf_emulator(const args_t& args)
  {
    ...
    isrran_assert(isrran_ringbuffer_init(&ringbuffer, sizeof(cf_t) * BUFFER_SIZE_SF * sf_len) >= ISRRAN_SUCCESS,
  ...
  bool tx(isrran::rf_buffer_interface& tx_buffer, const isrran::rf_timestamp_interface& tx_time) override
  {
    return false;
  }
  bool rx_now(isrran::rf_buffer_interface& rx_buffer, isrran::rf_timestamp_interface& rxd_time) override
  {
    ...
    while (isrran_ringbuffer_status(&ringbuffer) < nbytes and running) {
      run_async_slot();
    }
    ...
    isrran_assert(isrran_ringbuffer_read(&ringbuffer, temp_buffer, nbytes) >= ISRRAN_SUCCESS,
...
#endif // ISRRAN_GNB_RF_EMULATOR_H
