...
void worker_pool::phy_cfg_stash_t::set_cfg(const isrran::phy_cfg_t& c)
{
...
const isrran::phy_cfg_t& worker_pool::phy_cfg_stash_t::get_cfg(uint32_t sf_idx)
{
  if (sf_idx < (uint32_t)pending.size()) {
...
bool worker_pool::init(phy_common* common, int prio)
{
  ...
  for (uint32_t i = 0; i < common->args->nof_phy_threads; i++) {
    isrlog::basic_logger& log = isrlog::fetch_basic_logger(fmt::format("PHY{}", i));
    log.set_level(isrlog::str_to_basic_level(common->args->log.phy_level));
    log.set_hex_dump_max_size(common->args->log.phy_hex_limit);

    auto w = std::unique_ptr<lte::sf_worker>(new lte::sf_worker(ISRRAN_MAX_PRB, common, log));
...
sf_worker* worker_pool::wait_worker(uint32_t tti)
{
  ...
  for (uint32_t cc_idx = 0; cc_idx < ISRRAN_MAX_CARRIERS; cc_idx++) {
...
void worker_pool::set_config(uint32_t cc_idx, const isrran::phy_cfg_t& phy_cfg)
{
