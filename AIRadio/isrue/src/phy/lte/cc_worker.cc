...
#include "isrue/hdr/phy/lte/cc_worker.h"
...
cc_worker::cc_worker(uint32_t cc_idx_, uint32_t max_prb, isrue::phy_common* phy_, isrlog::basic_logger& logger) :
  logger(logger)
{
  cc_idx = cc_idx_;
  phy    = phy_;

  isrran_cfr_cfg_t cfr_config = phy->get_cfr_config();

  signal_buffer_max_samples = 3 * ISRRAN_SF_LEN_PRB(max_prb);

  for (uint32_t i = 0; i < phy->args->nof_rx_ant; i++) {
    signal_buffer_rx[i] = isrran_vec_cf_malloc(signal_buffer_max_samples);
    if (!signal_buffer_rx[i]) {
      Error("Allocating memory");
      return;
    }
    signal_buffer_tx[i] = isrran_vec_cf_malloc(signal_buffer_max_samples);
  ...
  if (isrran_ue_dl_init(&ue_dl, signal_buffer_rx, max_prb, phy->args->nof_rx_ant)) {
    Error("Initiating UE DL");
    return;
  }

  if (isrran_ue_ul_init(&ue_ul, signal_buffer_tx[0], max_prb)) {
    Error("Initiating UE UL");
    return;
  }

  if (isrran_ue_ul_set_cfr(&ue_ul, &cfr_config) < ISRRAN_SUCCESS) {
  ...
  isrran_softbuffer_rx_init(&mch_softbuffer, 100);
...
cc_worker::~cc_worker()
{
  ...
  isrran_softbuffer_rx_free(&mch_softbuffer);
...
bool cc_worker::set_cell_nolock(isrran_cell_t cell_)
{
  if (cell.id != cell_.id || !cell_initiated) {
    ...
    if (isrran_ue_dl_set_mbsfn_area_id(&ue_dl, 1)) {
    ...
    if (cell.frame_type == ISRRAN_TDD && ue_dl_cfg.chest_cfg.estimator_alg != ISRRAN_ESTIMATOR_ALG_INTERPOLATE) {
      chest_default_cfg.estimator_alg = ISRRAN_ESTIMATOR_ALG_INTERPOLATE;
      isrran::console("Enabling subframe interpolation for TDD cells (recommended setting)\n");
    }
...
void cc_worker::set_tdd_config_nolock(isrran_tdd_config_t config)
{
...
{
  bool dl_ack[ISRRAN_MAX_CODEWORDS] = {};
   ...
   */
  uint32_t mi_set_len;
  if (cell.frame_type == ISRRAN_TDD && !sf_cfg_dl.tdd_config.configured) {
    mi_set_len = 3;
  } else {
    mi_set_len = 1;
  }
  ...
  for (uint32_t i = 0; i < mi_set_len && !found_dl_grant; i++) {
    if (mi_set_len == 1) {
      isrran_ue_dl_set_mi_auto(&ue_dl);
    } else {
      isrran_ue_dl_set_mi_manual(&ue_dl, i);
    }
  ...
  if (has_dl_grant) {
    ...
    if (not dci_dl.is_pdcch_order) {
      ...
      // Generate PHY grant
      if (isrran_ue_dl_dci_to_pdsch_grant(&ue_dl, &sf_cfg_dl, &ue_dl_cfg, &dci_dl, &ue_dl_cfg.cfg.pdsch.grant)) {
    ...
    isrran_pdsch_ack_resource_t ack_resource = {dci_dl.dai, dci_dl.location.ncce, grant_cc_idx, dci_dl.tpc_pucch};
...
bool cc_worker::work_dl_mbsfn(isrran_mbsfn_cfg_t mbsfn_cfg)
{
  mac_interface_phy_lte::tb_action_dl_t dl_action = {};
  ...
  isrran_ue_dl_set_mbsfn_area_id(&ue_dl, mbsfn_cfg.mbsfn_area_id);
  isrran_ue_dl_set_non_mbsfn_region(&ue_dl, mbsfn_cfg.non_mbsfn_region_length);

  sf_cfg_dl.sf_type = ISRRAN_SF_MBSFN;
  ...
  if (mbsfn_cfg.enable) {
    isrran_configure_pmch(&pmch_cfg, &cell, &mbsfn_cfg);
    isrran_ra_dl_compute_nof_re(&cell, &sf_cfg_dl, &pmch_cfg.pdsch_cfg.grant);
...
void cc_worker::dl_phy_to_mac_grant(isrran_pdsch_grant_t*                         phy_grant,
                                    isrran_dci_dl_t*                              dl_dci,
                                    isrue::mac_interface_phy_lte::mac_grant_dl_t* mac_grant)
{
  ...
  if (mac_grant->rnti == ISRRAN_SIRNTI) {
...
int cc_worker::decode_pdcch_dl()
{
  ...
  if (dl_rnti != ISRRAN_INVALID_RNTI) {
    isrran_dci_dl_t dci[ISRRAN_MAX_CARRIERS] = {};
    ...
    for (int i = 0; i < (ue_dl_cfg.cfg.dci.cif_present ? 2 : 1) && !nof_grants; i++) {
      ...
      nof_grants                    = isrran_ue_dl_find_dl_dci(&ue_dl, &sf_cfg_dl, &ue_dl_cfg, dl_rnti, dci);
      if (nof_grants < 0) {
    ...
    // If RAR dci, save TTI
    if (nof_grants > 0 && ISRRAN_RNTI_ISRAR(dl_rnti)) {
    ...
    for (int k = 0; k < nof_grants; k++) {
      ...
      if (logger.info.enabled()) {
        char str[512];
        isrran_dci_dl_info(&dci[k], str, 512);
...
int cc_worker::decode_pdsch(isrran_pdsch_ack_resource_t            ack_resource,
                            mac_interface_phy_lte::tb_action_dl_t* action,
                            bool                                   mac_acks[ISRRAN_MAX_CODEWORDS])
{
  isrran_pdsch_res_t pdsch_dec[ISRRAN_MAX_CODEWORDS] = {};

  // See if at least 1 codeword needs to be decoded. If not need to be decode, resend ACK
  bool decode_enable                   = false;
  bool tb_enable[ISRRAN_MAX_CODEWORDS] = {};
  ...
  // Run PDSCH decoder
  if (decode_enable) {
    if (isrran_ue_dl_decode_pdsch(&ue_dl, &sf_cfg_dl, &ue_dl_cfg.cfg.pdsch, pdsch_dec)) {
  ...
  uint32_t nof_tb                             = 0;
  uint8_t  pending_acks[ISRRAN_MAX_CODEWORDS] = {};
  for (uint32_t tb = 0; tb < ISRRAN_MAX_CODEWORDS; tb++) {
  ...
  if (decode_enable) {
    ...
    if (logger.info.enabled()) {
      char str[512];
      isrran_pdsch_rx_info(&ue_dl_cfg.cfg.pdsch, pdsch_dec, str, 512);
...
int cc_worker::decode_pmch(mac_interface_phy_lte::tb_action_dl_t* action, isrran_mbsfn_cfg_t* mbsfn_cfg)
{
  isrran_pdsch_res_t pmch_dec = {};
  ...
  if (action->tb[0].enabled) {
    isrran_softbuffer_rx_reset_tbs(pmch_cfg.pdsch_cfg.softbuffers.rx[0], pmch_cfg.pdsch_cfg.grant.tb[0].tbs);

    if (isrran_ue_dl_decode_pmch(&ue_dl, &sf_cfg_dl, &pmch_cfg, &pmch_dec)) {
      Error("Decoding PMCH");
      return -1;
    }
...
void cc_worker::decode_phich()
{
  ...
  for (uint32_t I_phich = 0; I_phich < 2; I_phich++) {
    ...
    if (phy->get_ul_pending_ack(&sf_cfg_dl, cc_idx, &phich_grant, &dci_ul)) {
      if (isrran_ue_dl_decode_phich(&ue_dl, &sf_cfg_dl, &ue_dl_cfg, &phich_grant, &phich_res)) {
...
bool cc_worker::work_ul(isrran_uci_data_t* uci_data)
{
  bool signal_ready;
  ...
  if (ul_grant_available || ul_mac_grant.phich_available) {
    ...
    // Generate PHY grant
    if (isrran_ue_ul_dci_to_pusch_grant(&ue_ul, &sf_cfg_ul, &ue_ul_cfg, &dci_ul, &ue_ul_cfg.ul_cfg.pusch.grant)) {
      if (logger.info.enabled()) {
        char str[128];
        isrran_dci_ul_info(&dci_ul, str, sizeof(str));
        Info("Converting DCI message to UL grant %s", str);
      }
      ul_grant_available = false;
    } else if (ue_ul_cfg.ul_cfg.pusch.grant.tb.mod == ISRRAN_MOD_BPSK) {
      ...
      isrran_ue_ul_pusch_hopping(&ue_ul, &sf_cfg_ul, &ue_ul_cfg, &ue_ul_cfg.ul_cfg.pusch.grant);
    }
  }
  ...
  if (ul_action.expect_ack) {
    ...
    if (cell.frame_type == ISRRAN_TDD && sf_cfg_ul.tdd_config.sf_config == 0) {
...
void cc_worker::ul_phy_to_mac_grant(isrran_pusch_grant_t*                         phy_grant,
                                    isrran_dci_ul_t*                              dci_ul,
                                    uint32_t                                      pid,
                                    bool                                          ul_grant_available,
                                    isrue::mac_interface_phy_lte::mac_grant_ul_t* mac_grant)
{
  ...
  mac_grant->pid            = pid;
  mac_grant->is_rar         = dci_ul->format == ISRRAN_DCI_FORMAT_RAR;
...
int cc_worker::decode_pdcch_ul()
{
  ...
  isrran_dci_ul_t dci[ISRRAN_MAX_CARRIERS];
  ZERO_OBJECT(dci);
  ...
  if (ul_rnti) {
    ...
    for (int i = 0; i < (ue_dl_cfg.cfg.dci.cif_present ? 2 : 1) && !nof_grants; i++) {
      ue_dl_cfg.cfg.dci.cif_enabled = i > 0;
      ue_dl_cfg.cfg.dci_common_ss   = (cc_idx == 0);
      nof_grants                    = isrran_ue_dl_find_ul_dci(&ue_dl, &sf_cfg_dl, &ue_dl_cfg, ul_rnti, dci);
      if (nof_grants < 0) {
    ...
    for (int k = 0; k < nof_grants; k++) {
      ...
      if (logger.info.enabled()) {
        char str[512];
        isrran_dci_ul_info(&dci[k], str, 512);
...
bool cc_worker::encode_uplink(mac_interface_phy_lte::tb_action_ul_t* action, isrran_uci_data_t* uci_data)
{
  ...
  if (ue_ul_cfg.ul_cfg.pucch.rnti == ISRRAN_INVALID_RNTI) {
    return false;
  }

  // Encode signal
  int ret = isrran_ue_ul_encode(&ue_ul, &sf_cfg_ul, &ue_ul_cfg, &data);
  if (ret < 0) {
  ...
  if (logger.info.enabled()) {
    char str[512];
    if (isrran_ue_ul_info(&ue_ul_cfg, &sf_cfg_ul, &data.uci, str, 512)) {
      logger.info("%s", str);
    }
  }
...
void cc_worker::set_uci_sr(isrran_uci_data_t* uci_data)
{
  Debug("set_uci_sr() query: sr_enabled=%d, last_tx_tti=%d", phy->sr.is_triggered(), phy->sr.get_last_tx_tti());
  if (isrran_ue_ul_gen_sr(&ue_ul_cfg, &sf_cfg_ul, uci_data, phy->sr.is_triggered())) {
...
uint32_t cc_worker::get_wideband_cqi()
{
  ...
  uint32_t wb_cqi_value = isrran_cqi_from_snr(phy->get_sinr_db(cc_idx) + ue_dl_cfg.snr_to_cqi_offset);
...
void cc_worker::set_uci_periodic_cqi(isrran_uci_data_t* uci_data)
{
  // Load last reported RI
  ue_dl_cfg.last_ri = phy->last_ri;

  isrran_ue_dl_gen_cqi_periodic(&ue_dl, &ue_dl_cfg, get_wideband_cqi(), CURRENT_TTI_TX, uci_data);
...
void cc_worker::set_uci_aperiodic_cqi(isrran_uci_data_t* uci_data)
{
  if (ue_dl_cfg.cfg.cqi_report.aperiodic_configured) {
    isrran_ue_dl_gen_cqi_aperiodic(&ue_dl, &ue_dl_cfg, get_wideband_cqi(), uci_data);
  } else {
    Warning("Received CQI request but aperiodic mode is not configured");
  }
}

void cc_worker::set_uci_ack(isrran_uci_data_t* uci_data,
                            bool               is_grant_available,
                            uint32_t           V_dai_ul,
                            bool               is_pusch_available)
{
  isrran_pdsch_ack_t ack_info                = {};
  ...
  isrran_ue_dl_gen_ack(&ue_dl.cell, &sf_cfg_dl, &ack_info, uci_data);
}
 ...
 */
void cc_worker::set_config_nolock(const isrran::phy_cfg_t& phy_cfg)
{
  // Save configuration
...
void cc_worker::upd_config_dci_nolock(const isrran_dci_cfg_t& dci_cfg)
{
...
{
  uint32_t sz = (uint32_t)isrran_symbol_sz(cell.nof_prb);
  isrran_vec_f_zero(ce_abs, sz);
  int g = (sz - 12 * cell.nof_prb) / 2;
  isrran_vec_abs_dB_cf(ue_dl.chest_res.ce[tx_antenna][rx_antenna], -80, &ce_abs[g], ISRRAN_NRE * cell.nof_prb);
  return sz;
}
...
void cc_worker::new_mch_dl(mac_interface_phy_lte::tb_action_dl_t* action)
{
  ...
  isrran_softbuffer_rx_reset_cb(&mch_softbuffer, 1);
}
