...
static void      init_plots(isrue::lte::sf_worker* worker);
...
sf_worker::sf_worker(uint32_t max_prb, phy_common* phy_, isrlog::basic_logger& logger) : logger(logger)
{
  phy = phy_;
...
bool sf_worker::set_cell_nolock(uint32_t cc_idx, isrran_cell_t cell_)
{
...
void sf_worker::set_tdd_config_nolock(isrran_tdd_config_t config)
{
  for (auto& cc_worker : cc_workers) {
...
void sf_worker::set_config_nolock(uint32_t cc_idx, const isrran::phy_cfg_t& phy_cfg)
{
  if (cc_idx < cc_workers.size()) {
...
void sf_worker::work_imp()
{
  ...
  isrran::rf_buffer_t tx_signal_ptr = {};
  if (!cell_initiated) {
  ...
  uint32_t nof_samples     = ISRRAN_SF_LEN_PRB(cell.nof_prb);
  ...
  for (uint32_t carrier_idx = 0; carrier_idx < cc_workers.size(); carrier_idx++) {
    ...
    if (isrran_sfidx_tdd_type(tdd_config, tti % 10) != ISRRAN_TDD_SF_U || cell.frame_type == ISRRAN_FDD) {
  ...
  /* If TTI+4 is an uplink subframe (TODO: Support short PRACH and ISR in UpPts special subframes) */
  if ((isrran_sfidx_tdd_type(tdd_config, TTI_TX(tti) % 10) == ISRRAN_TDD_SF_U) || cell.frame_type == ISRRAN_FDD) {
...
void sf_worker::reset_uci(isrran_uci_data_t* uci_data)
{
  isrran_uci_data_reset(uci_data);
}
...
#ifdef ENABLE_GUI
plot_real_t    pce[ISRRAN_MAX_PORTS][ISRRAN_MAX_PORTS];
plot_scatter_t pconst;
plot_scatter_t pconst_nr;
bool           pconst_nr_ready = false;
#define SCATTER_PDSCH_BUFFER_LEN (20 * 6 * ISRRAN_SF_LEN_RE(ISRRAN_MAX_PRB, ISRRAN_CP_NORM))
#define SCATTER_PDSCH_PLOT_LEN 4000
float tmp_plot[SCATTER_PDSCH_BUFFER_LEN];
cf_t  tmp_plot2[ISRRAN_SF_LEN_RE(ISRRAN_MAX_PRB, ISRRAN_CP_NORM)];
...
{
  auto     worker    = (isrue::lte::sf_worker*)arg;
  ...
  for (uint32_t tx = 0; tx < worker->get_cell_nof_ports(); tx++) {
    for (uint32_t rx = 0; rx < worker->get_rx_nof_antennas(); rx++) {
      ...
      plot_real_addToWindowGrid(&pce[tx][rx], (char*)"isrue", tx, rx);
    }
  }
  ...
  plot_scatter_addToWindowGrid(&pconst, (char*)"isrue", 0, row_count++);
  ...
  if (plot_nr_enable) {
    ...
    plot_scatter_addToWindowGrid(&pconst_nr, (char*)"isrue", 0, row_count++);
  ...
  plot_scatter_addToWindowGrid(&pcfo, (char*)"isrue", 1, row_count++);
#endif /* CFO_PLOT_LEN > 0 */
  ...
  plot_scatter_addToWindowGrid(&psync, (char*)"isrue", 1, row_count++);
#endif /* SYNC_PLOT_LEN > 0 */
...
void init_plots(isrue::lte::sf_worker* worker)
{
