...
static void isrran_phy_handler(phy_logger_level_t log_level, void* ctx, char* str)
{
  phy* r = (phy*)ctx;
  r->isrran_phy_logger(log_level, str);
}

void phy::isrran_phy_logger(phy_logger_level_t log_level, char* str)
{
  switch (log_level) {
...
bool phy::check_args(const phy_args_t& args_)
{
  if (args_.nof_phy_threads > MAX_WORKERS) {
    isrran::console("Error in PHY args: nof_phy_threads must be 1, 2 or 3\n");
    return false;
  }
  if (args_.snr_ema_coeff > 1.0) {
    isrran::console("Error in PHY args: snr_ema_coeff must be 0<=w<=1\n");
    return false;
  }
  return true;
}

int phy::init(const phy_args_t& args_, stack_interface_phy_lte* stack_, isrran::radio_interface_phy* radio_)
{
  ...
  // Add PHY lib log
  auto lib_log_level = isrlog::str_to_basic_level(args.log.phy_lib_level);
  logger_phy_lib.set_level(lib_log_level);
  logger_phy_lib.set_hex_dump_max_size(args.log.phy_hex_limit);
  if (lib_log_level != isrlog::basic_levels::none) {
    isrran_phy_log_register_handler(this, isrran_phy_handler);
  }

  // set default logger
  logger_phy.set_level(isrlog::str_to_basic_level(args.log.phy_level));
...
{
  std::unique_lock<std::mutex> lock(config_mutex);
  prach_buffer.init(ISRRAN_MAX_PRB);
...
void phy::get_metrics(const isrran::isrran_rat_t& rat, phy_metrics_t* m)
{
  // Zero structure by default
  *m = {};

  // Get LTE metrics
  if (rat == isrran::isrran_rat_t::lte && args.nof_lte_carriers > 0) {
  ...
  // Get NR metrics
  if (rat == isrran::isrran_rat_t::nr && args.nof_nr_carriers > 0) {
...
void phy::set_cells_to_meas(uint32_t earfcn, const std::set<uint32_t>& pci)
{
  ...
  cmd_worker.add_cmd([this, earfcn, pci, pcell_earfcn]() {
    ...
    if (not available and cc_empty != 0) {
      ...
      isrran_cell_t cell = selected_cell;
...
void phy::set_rar_grant(uint8_t grant_payload[ISRRAN_RAR_GRANT_LEN], uint16_t rnti)
{
...
bool phy::set_config(const isrran::phy_cfg_t& config_, uint32_t cc_idx)
{
  if (!is_initialized()) {
  ...
  if (cc_idx >= args.nof_lte_carriers) {
    isrran::console("Received SCell configuration for index %d but there are not enough CC workers available\n",
                    cc_idx);
    return true;
  }
...
bool phy::set_scell(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn)
{
  return set_scell(cell_info, cc_idx, earfcn, true);
}

bool phy::set_scell(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn, bool run_in_background)
{
  if (!is_initialized()) {
    fprintf(stderr, "Error calling set_config(): PHY not initialized\n");
    return false;
  ...
  if (!isrran_cell_isvalid(&cell_info)) {
...
void phy::set_scell_cmd(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn, bool earfcn_is_different)
{
  ...
  if (earfcn_is_different) {
    double dl_freq = isrran_band_fd(earfcn) * 1e6;
    double ul_freq = isrran_band_fu(common.get_ul_earfcn(earfcn)) * 1e6;
...
void phy::set_config_tdd(isrran_tdd_config_t& tdd_config_)
{
  tdd_config = tdd_config_;

  if (!tdd_config.configured) {
    isrran::console("Setting TDD-config: %d, SS config: %d\n", tdd_config.sf_config, tdd_config.ss_config);
  }
...
void phy::set_config_mbsfn_sib2(isrran::mbsfn_sf_cfg_t* cfg_list, uint32_t nof_cfgs)
{
  if (nof_cfgs > 1) {
...
void phy::set_config_mbsfn_sib13(const isrran::sib13_t& sib13)
{
...
void phy::set_config_mbsfn_mcch(const isrran::mcch_msg_t& mcch)
{
...
int phy::init(const phy_args_nr_t& args_, stack_interface_phy_nr* stack_, isrran::radio_interface_phy* radio_)
{
  stack_nr = stack_;
...
bool phy::set_config(const isrran::phy_cfg_nr_t& cfg)
{
