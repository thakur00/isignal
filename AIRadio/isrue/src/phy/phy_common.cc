...
static isrran::rf_buffer_t zeros_multi(1);

phy_common::phy_common(isrlog::basic_logger& logger) : logger(logger), ta(logger)
{
  reset();
}
                      ...
                      rsrp_insync_itf*             _chest_loop)
{
  ...
  if (args->ul_channel_args.enable) {
    ul_channel = isrran::channel_ptr(
        new isrran::channel(args->ul_channel_args, args->nof_lte_carriers * args->nof_rx_ant, logger));
  }
...
void phy_common::set_ue_dl_cfg(isrran_ue_dl_cfg_t* ue_dl_cfg)
{
  ue_dl_cfg->snr_to_cqi_offset = args->snr_to_cqi_offset;

  isrran_chest_dl_cfg_t* chest_cfg = &ue_dl_cfg->chest_cfg;

  // Setup estimator filter
  bzero(chest_cfg, sizeof(isrran_chest_dl_cfg_t));
  ...
  chest_cfg->filter_type = ISRRAN_CHEST_FILTER_GAUSS;

  if (args->snr_estim_alg == "refs") {
    chest_cfg->noise_alg = ISRRAN_NOISE_ALG_REFS;
  } else if (args->snr_estim_alg == "empty") {
    chest_cfg->noise_alg = ISRRAN_NOISE_ALG_EMPTY;
  } else {
    chest_cfg->noise_alg = ISRRAN_NOISE_ALG_PSS;
  }

  chest_cfg->rsrp_neighbour    = false;
  chest_cfg->sync_error_enable = args->correct_sync_error;
  chest_cfg->estimator_alg =
      args->interpolate_subframe_enabled ? ISRRAN_ESTIMATOR_ALG_INTERPOLATE : ISRRAN_ESTIMATOR_ALG_AVERAGE;
  chest_cfg->cfo_estimate_enable  = args->cfo_ref_mask != 0;
  chest_cfg->cfo_estimate_sf_mask = args->cfo_ref_mask;
}

void phy_common::set_pdsch_cfg(isrran_pdsch_cfg_t* pdsch_cfg)
{
  ...
  pdsch_cfg->decoder_type       = (args->equalizer_mode == "zf") ? ISRRAN_MIMO_DECODER_ZF : ISRRAN_MIMO_DECODER_MMSE;
}

void phy_common::set_ue_ul_cfg(isrran_ue_ul_cfg_t* ue_ul_cfg)
{
  // Setup uplink configuration
  bzero(ue_ul_cfg, sizeof(isrran_ue_ul_cfg_t));
  ue_ul_cfg->cfo_en = true;
  if (args->force_ul_amplitude > 0.0f) {
    ue_ul_cfg->force_peak_amplitude = args->force_ul_amplitude;
    ue_ul_cfg->normalize_mode       = ISRRAN_UE_UL_NORMALIZE_MODE_FORCE_AMPLITUDE;
  } else {
    ue_ul_cfg->normalize_mode = ISRRAN_UE_UL_NORMALIZE_MODE_AUTO;
  }
  ue_ul_cfg->ul_cfg.pucch.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_NORMAL;
}

isrran::radio_interface_phy* phy_common::get_radio()
{
  return radio_h;
}

// Unpack RAR dci as defined in Section 6.2 of 36.213
void phy_common::set_rar_grant(uint8_t             grant_payload[ISRRAN_RAR_GRANT_LEN],
                               uint16_t            rnti,
...
#error "Error MSG3_DELAY_MS can't be negative"
  ...
  isrran_dci_rar_grant_t rar_grant;
  isrran_dci_rar_unpack(grant_payload, &rar_grant);

  if (isrran_dci_rar_to_ul_dci(&cell, &rar_grant, &dci_ul)) {
    Error("Converting RAR message to UL dci");
    return;
  }

  dci_ul.format = ISRRAN_DCI_FORMAT_RAR; // Use this format to identify a RAR grant
  dci_ul.rnti   = rnti;
  ...
  if (cell.frame_type == ISRRAN_TDD) {
    while (isrran_sfidx_tdd_type(tdd_config, msg3_tx_tti % 10) != ISRRAN_TDD_SF_U) {
      msg3_tx_tti++;
    }
  }
...
uint32_t phy_common::ul_pidof(uint32_t tti, isrran_tdd_config_t* tdd_config)
{
  if (tdd_config->configured) {
    ...
    return tti % ISRRAN_FDD_NOF_HARQ;
  }
  return 0;
}
...
#define tti_phich(sf)                                                                                                  \
  (sf->tti + (cell.frame_type == ISRRAN_FDD ? FDD_HARQ_DELAY_UL_MS : k_phich[sf->tdd_config.sf_config][sf->tti % 10]))

// Here SF->TTI is when PUSCH is transmitted
void phy_common::set_ul_pending_ack(isrran_ul_sf_cfg_t*  sf,
                                    uint32_t             cc_idx,
...
bool phy_common::get_ul_pending_ack(isrran_dl_sf_cfg_t*   sf,
                                    uint32_t              cc_idx,
                                    isrran_phich_grant_t* phich_grant,
                                    isrran_dci_ul_t*      dci_ul)
{
...
#define tti_pusch_hi(sf)                                                                                               \
  (sf->tti +                                                                                                           \
   (cell.frame_type == ISRRAN_FDD ? FDD_HARQ_DELAY_UL_MS                                                               \
    : I_phich                     ? 7                                                                                  \
...
void phy_common::set_ul_pending_grant(isrran_dl_sf_cfg_t* sf, uint32_t cc_idx, isrran_dci_ul_t* dci)
{
  std::lock_guard<std::mutex> lock(pending_ul_grant_mutex);
...
bool phy_common::get_ul_pending_grant(isrran_ul_sf_cfg_t* sf, uint32_t cc_idx, uint32_t* pid, isrran_dci_ul_t* dci)
{
  std::lock_guard<std::mutex> lock(pending_ul_grant_mutex);
  bool                        ret           = false;
...
void phy_common::set_ul_received_ack(isrran_dl_sf_cfg_t* sf,
                                     uint32_t            cc_idx,
                                     bool                ack_value,
                                     uint32_t            I_phich,
                                     isrran_dci_ul_t*    dci_ul)
...
bool phy_common::get_ul_received_ack(isrran_ul_sf_cfg_t* sf, uint32_t cc_idx, bool* ack_value, isrran_dci_ul_t* dci_ul)
{
  std::lock_guard<std::mutex> lock(received_ul_ack_mutex);
  bool                        ret          = false;
...
void phy_common::set_dl_pending_ack(isrran_dl_sf_cfg_t*         sf,
                                    uint32_t                    cc_idx,
                                    ...
                                    isrran_pdsch_ack_resource_t resource)
{
  ...
  if (!pending_ack.enable) {
    ...
    memcpy(pending_ack.value, value, ISRRAN_MAX_CODEWORDS * sizeof(uint8_t));
...
bool phy_common::get_dl_pending_grant(uint32_t tti, uint32_t cc_idx, uint32_t* grant_cc_idx, isrran_dci_dl_t* dl_dci)
{
  std::lock_guard<std::mutex> lock(pending_dl_grant_mutex);
  if (pending_dl_grant[tti % FDD_HARQ_DELAY_UL_MS][cc_idx].enable) {
    // Read grant
...
bool phy_common::get_dl_pending_ack(isrran_ul_sf_cfg_t* sf, uint32_t cc_idx, isrran_pdsch_ack_cc_t* ack)
{
  std::lock_guard<std::mutex> lock(pending_dl_ack_mutex);
  bool                        ret = false;
  uint32_t                    M;
  ...
  for (uint32_t i = 0; i < M; i++) {
    uint32_t k =
        (cell.frame_type == ISRRAN_FDD) ? FDD_HARQ_DELAY_UL_MS : das_table[sf->tdd_config.sf_config][sf->tti % 10].K[i];
    ...
    if (pending_ack.enable) {
      ...
      memcpy(ack->m[i].value, pending_ack.value, ISRRAN_MAX_CODEWORDS * sizeof(uint8_t));
 ...
 */
void phy_common::worker_end(const worker_context_t& w_ctx, const bool& tx_enable, isrran::rf_buffer_t& buffer)
{
  ...
  isrran::rf_timestamp_t tx_time = w_ctx.tx_time; // get transmit time from the last worker
    ...
    if (radio_h->is_continuous_tx()) {
      if (is_pending_tx_end) {
        ...
        if (!radio_h->get_is_start_of_burst()) {
          if (ul_channel) {
            isrran_vec_cf_zero(zeros_multi.get(0), buffer.get_nof_samples());
  ...
  tx_enabled = false;
  for (uint32_t ch = 0; ch < ISRRAN_MAX_CHANNELS; ch++) {
...
void phy_common::set_cell(const isrran_cell_t& c)
{
  cell = c;

  if (ul_channel) {
    ul_channel->set_srate((uint32_t)isrran_sampling_freq_hz(cell.nof_prb));
  }
}
...
void phy_common::update_cfo_measurement(uint32_t cc_idx, float cfo_hz)
{
  ...
  avg_cfo_hz[cc_idx] = ISRRAN_VEC_SAFE_EMA(cfo_hz, avg_cfo_hz[cc_idx], args->snr_ema_coeff);
}

void phy_common::reset_measurements(uint32_t cc_idx)
{
  // If the CC index exceeds the maximum number of carriers, reset them all
  if (cc_idx >= ISRRAN_MAX_CARRIERS) {
    for (uint32_t cc = 0; cc < ISRRAN_MAX_CARRIERS; cc++) {
                                     ...
                                     cf_t*                        rssi_power_buffer)
{
  bool insync = true;
  {
    ...
    if (isrran_sfidx_tdd_type(sf_cfg_dl.tdd_config, sf_cfg_dl.tti % 10) == ISRRAN_TDD_SF_S &&
        isrran_sfidx_tdd_nof_dw(sf_cfg_dl.tdd_config) < 4) {
      return;
    }
    ...
    if (rssi_power_buffer) {
      if (!update_rxgain_cnt) {
        ...
        float rssi_dbm = ISRRAN_SF_LEN_PRB(cell.nof_prb) > 0
                             ? (isrran_convert_power_to_dB(
                                    isrran_vec_avg_power_cf(rssi_power_buffer, ISRRAN_SF_LEN_PRB(cell.nof_prb))) +
                                30)
                             : 0;
        if (std::isnormal(rssi_dbm)) {
          avg_rssi_dbm[0] = ISRRAN_VEC_SAFE_EMA(rssi_dbm, avg_rssi_dbm[0], args->snr_ema_coeff);
        }
    ...
    if (std::isnormal(rsrq_db)) {
      ...
      avg_rsrq_db[cc_idx] = ISRRAN_VEC_SAFE_CMA(rsrq_db, avg_rsrq_db[cc_idx], sf_cfg_dl.tti % pcell_report_period);
    }

    // Average RSRP taken from CRS
    float rsrp_lin = chest_res.rsrp;
    if (std::isnormal(rsrp_lin)) {
      avg_rsrp[cc_idx] = ISRRAN_VEC_SAFE_EMA(rsrp_lin, avg_rsrp[cc_idx], snr_ema_coeff);
    }
    ...
    if (std::isnormal(rsrp_dbm)) {
      ...
      avg_rsrp_dbm[cc_idx] = ISRRAN_VEC_SAFE_CMA(rsrp_dbm, avg_rsrp_dbm[cc_idx], sf_cfg_dl.tti % pcell_report_period);
    }
    ...
    if (std::isnormal(cur_noise)) {
      avg_noise[cc_idx] = ISRRAN_VEC_SAFE_EMA(cur_noise, avg_noise[cc_idx], snr_ema_coeff);
    }
    ...
    if (std::isnormal(avg_rsrp_neigh[cc_idx])) {
      cur_noise /= isrran_convert_dB_to_power(rx_gain_offset - 30);
      ...
      uint32_t nof_re_x_prb = ISRRAN_NRE * (ISRRAN_CP_NSYMB(cell.cp));
      float    factor       = nof_re_x_prb / (isrran_refsignal_cs_nof_pilots_x_slot(cell.nof_ports));
      sinr_db = avg_rsrp_dbm[cc_idx] - isrran_convert_power_to_dB(avg_rsrp_neigh[cc_idx] / factor + cur_noise);
    }

    // Average sinr in the log domain
    if (std::isnormal(sinr_db)) {
      avg_sinr_db[cc_idx] = ISRRAN_VEC_SAFE_EMA(sinr_db, avg_sinr_db[cc_idx], snr_ema_coeff);
    }

    // Average snr in the log domain
    if (std::isnormal(chest_res.snr_db)) {
      avg_snr_db[cc_idx] = ISRRAN_VEC_SAFE_EMA(chest_res.snr_db, avg_snr_db[cc_idx], snr_ema_coeff);
    }
...
void phy_common::reset()
{
  ...
  reset_measurements(ISRRAN_MAX_CARRIERS);
...
void phy_common::build_mch_table()
{
  ...
  if (mbsfn_config.mbsfn_subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::one_frame) {
    generate_mch_table(&mch_table[0], (uint32_t)mbsfn_config.mbsfn_subfr_cnfg.sf_alloc, 1u);
  } else if (mbsfn_config.mbsfn_subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::four_frames) {
...
uint32_t phy_common::get_ul_earfcn(uint32_t dl_earfcn)
{
  ...
  uint32_t ul_earfcn = isrran_band_ul_earfcn(dl_earfcn);
...
bool phy_common::is_mch_subframe(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti)
{
  ...
  if (sib13_configured) {
    isrran::mbsfn_sf_cfg_t&    subfr_cnfg = mbsfn_config.mbsfn_subfr_cnfg;
    isrran::mbsfn_area_info_t& area_info  = mbsfn_config.mbsfn_area_info;
    offset                                = subfr_cnfg.radioframe_alloc_offset;
    period                                = isrran::enum_to_number(subfr_cnfg.radioframe_alloc_period);
    if (period == (uint8_t)-1) {
      return false;
    }

    if (subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::one_frame) {
      if ((sfn % period == offset) && (mch_table[sf] > 0)) {
        ...
        if (mcch_configured) {
          ...
          isrran::mcch_msg_t& mcch = mbsfn_config.mcch;
    ...
    } else if (subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::four_frames) {
...
bool phy_common::is_mcch_subframe(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti)
{
  uint32_t sfn; // System Frame Number
  uint8_t  sf;  // Subframe
  uint8_t  offset;
...
bool phy_common::is_mbsfn_sf(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti)
{
