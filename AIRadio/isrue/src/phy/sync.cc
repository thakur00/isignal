...
static int
radio_recv_callback(void* obj, cf_t* data[ISRRAN_MAX_CHANNELS], uint32_t nsamples, isrran_timestamp_t* rx_time)
{
  isrran::rf_buffer_t x(data, nsamples);
  return ((sync*)obj)->radio_recv_fnc(x, rx_time);
}

static ISRRAN_AGC_CALLBACK(callback_set_rx_gain)
{
  ((sync*)h)->set_rx_gain(gain_db);
}

void sync::init(isrran::radio_interface_phy* _radio,
                ...
                int                          sync_cpu_affinity)
{
  ...
  if (nof_rf_channels == 0 || nof_rf_channels > ISRRAN_MAX_CHANNELS) {
    Error("SYNC:  Invalid number of RF channels (%d)", nof_rf_channels);
    return;
  }

  if (isrran_ue_sync_init_multi(&ue_sync, ISRRAN_MAX_PRB, false, radio_recv_callback, nof_rf_channels, this) !=
      ISRRAN_SUCCESS) {
    Error("SYNC:  Initiating ue_sync");
    return;
  }
  isrran_ue_sync_cp_en(&ue_sync, worker_com->args->detect_cp);

  if (worker_com->args->dl_channel_args.enable) {
    channel_emulator =
        isrran::channel_ptr(new isrran::channel(worker_com->args->dl_channel_args, nof_rf_channels, phy_logger));
  }
...
bool sync::cell_select_init(phy_cell_t new_cell)
{
  ...
  // Move state to IDLE
  if (!isrran_cellid_isvalid(new_cell.pci)) {
...
{
  isrran_cell_t tmp_cell = cell.get();
  ...
  switch (sfn_p.run_subframe(&old_cell, &tti, mib)) {
    case sfn_sync::SFN_FOUND:
      if (!cell.equals(old_cell)) {
        ...
        isrran::console("Detected cell during SFN synchronization differs from configured cell. Cell reselection "
...
void sync::run_camping_in_sync_state(lte::sf_worker*      lte_worker,
                                     ...
                                     isrran::rf_buffer_t& sync_buffer)
{
  // Update logging TTI
  ...
  if (isrran_ue_sync_get_sfidx(&ue_sync) != tti % 10) {
    uint32_t sfn = tti / 10;
    tti          = (sfn * 10 + isrran_ue_sync_get_sfidx(&ue_sync)) % 10240;
  ...
  if (force_camping_sfn_sync) {
    uint32_t           _tti      = 0;
    isrran_cell_t      temp_cell = cell.get();
    ...
    if (ret == sfn_sync::SFN_FOUND) {
      ...
      if (!cell.equals(temp_cell)) {
        ...
        isrran::console("Detected cell during SFN synchronization differs from configured cell. Cell "
                        "reselection to cells with different MIB is not supported\n");
      } else {
  ...
  lte_worker->set_prach(prach_ptr ? &prach_ptr[prach_sf_cnt * ISRRAN_SF_LEN_PRB(cell.get().nof_prb)] : nullptr,
                        prach_power);
  ...
  if (nr_worker != nullptr) {
    ...
    nr_worker_pool->set_ul_ext_cfo(isrran_ue_sync_get_cfo(&ue_sync));
...
void sync::run_camping_state()
{
  ...
  if (ref_cfo != 0.0) {
    isrran_ue_sync_set_cfo_ref(&ue_sync, ref_cfo);
    ref_cfo = 0.0; // reset until value changes again
  }

  // Primary Cell (PCell) Synchronization
  int sync_result = isrran_ue_sync_zerocopy(&ue_sync, sync_buffer.to_cf_t(), lte_worker->get_buffer_len());
  cfo             = isrran_ue_sync_get_cfo(&ue_sync);
  sfo             = isrran_ue_sync_get_sfo(&ue_sync);
...
void sync::run_idle_state()
{
  if (radio_h->is_init()) {
    ...
    isrran_timestamp_t rx_time = {};
    dummy_buffer.set_nof_samples(nsamples);
    if (radio_recv_fnc(dummy_buffer, &rx_time) == ISRRAN_SUCCESS) {
      isrran::console("SYNC:  Receiving from radio while in IDLE_RX\n");
    }
...
void sync::set_agc_enable(bool enable)
{
  ...
  isrran_rf_info_t* rf_info = radio_h->get_info();
  if (rf_info == nullptr) {
...
void sync::set_ue_sync_opts(isrran_ue_sync_t* q, float cfo_)
{
  if (worker_com->args->cfo_integer_enabled) {
    isrran_ue_sync_set_cfo_i_enable(q, true);
  }

  isrran_ue_sync_set_cfo_ema(q, worker_com->args->cfo_pss_ema);
  isrran_ue_sync_set_cfo_tol(q, worker_com->args->cfo_correct_tol_hz);
  isrran_ue_sync_set_cfo_loop_bw(q,
  ...
  if (std::isnormal(cfo_)) {
    isrran_ue_sync_cfo_reset(q, cfo_);
    q->cfo_current_value       = cfo_ / 15000;
    q->cfo_is_copied           = true;
    q->cfo_correct_enable_find = true;
    isrran_sync_set_cfo_cp_enable(&q->sfind, false, 0);
  }

  // Set SFO ema and correct period
  isrran_ue_sync_set_sfo_correct_period(q, worker_com->args->sfo_correct_period);
  isrran_ue_sync_set_sfo_ema(q, worker_com->args->sfo_ema);
  ...
  isrran_sync_set_sss_algorithm(&q->strack, (sss_alg_t)sss_alg);
  isrran_sync_set_sss_algorithm(&q->sfind, (sss_alg_t)sss_alg);
}
...
bool sync::set_cell(float cfo_in)
{
  ...
  if (isrran_ue_sync_set_cell(&ue_sync, cell.get())) {
...
bool sync::set_frequency()
{
  ...
  if (dl_freq > 0 && ul_freq > 0) {
    ...
    set_dl_freq = 1e6 * isrran_band_fd(current_earfcn);
    if (isrran_band_is_tdd(isrran_band_get_band(current_earfcn))) {
      set_ul_freq = set_dl_freq;
    } else {
      set_ul_freq = 1e6 * isrran_band_fu(worker_com->get_ul_earfcn(current_earfcn));
    }
  }
...
{
  float new_srate = (float)isrran_sampling_freq_hz(cell.get().nof_prb);
  if (new_srate < 0.0) {
...
void sync::get_current_cell(isrran_cell_t* cell_, uint32_t* earfcn_)
{
  if (cell_) {
    *cell_ = cell.get();
  }
...
int sync::radio_recv_fnc(isrran::rf_buffer_t& data, isrran_timestamp_t* rx_time)
{
  ...
  isrran::rf_timestamp_t  dummy_ts     = {};
  isrran::rf_timestamp_t& rf_timestamp = (rx_time == nullptr) ? dummy_ts : last_rx_time;
  ...
  isrran_timestamp_t dummy_flat_ts = {};
...
void sync::run_stack_tti()
{
  ...
  if (forced_rx_time_init || isrran_timestamp_iszero(&stack_tti_ts) ||
      isrran_timestamp_compare(&stack_tti_ts_new, &stack_tti_ts) < 0) {
    if (isrran_timestamp_compare(&stack_tti_ts_new, &stack_tti_ts) < 0) {
      phy_logger.warning("SYNC:  radio time seems to be going backwards (rx_time=%f, tti_ts=%f)",
                         isrran_timestamp_real(&stack_tti_ts_new),
                         isrran_timestamp_real(&stack_tti_ts));
      // time-stamp will be set to rx time below and run_tti() will be called with MIN_TTI_JUMP
    }

    // init tti_ts with last rx time
    phy_logger.debug("SYNC:  Setting initial TTI time to %f", isrran_timestamp_real(&stack_tti_ts_new));
    isrran_timestamp_copy(&stack_tti_ts, &stack_tti_ts_new);
    forced_rx_time_init = false;
  }

  // Advance stack in time
  if (isrran_timestamp_compare(&stack_tti_ts_new, &stack_tti_ts) >= 0) {
    isrran_timestamp_t temp = {};
    isrran_timestamp_copy(&temp, &stack_tti_ts_new);
    isrran_timestamp_sub(&temp, stack_tti_ts.full_secs, stack_tti_ts.frac_secs);
    int32_t tti_jump = static_cast<int32_t>(isrran_timestamp_uint64(&temp, 1e3));
    tti_jump         = ISRRAN_MAX(tti_jump, MIN_TTI_JUMP);
    if (tti_jump > MAX_TTI_JUMP) {
      phy_logger.warning("SYNC:  TTI jump of %d limited to %d", tti_jump, int(MAX_TTI_JUMP));
      tti_jump = ISRRAN_MIN(tti_jump, MAX_TTI_JUMP);
    }
...
void sync::set_inter_frequency_measurement(uint32_t cc_idx, uint32_t earfcn_, isrran_cell_t cell_)
{
  std::lock_guard<std::mutex> lock(intra_freq_cfg_mutex);
...
void sync::scell_sync_set(uint32_t cc_idx, const isrran_cell_t& _cell)
{
