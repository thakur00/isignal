...
intra_measure_nr::intra_measure_nr(isrlog::basic_logger& logger_, meas_itf& new_meas_itf_) :
...
bool intra_measure_nr::measure_rat(const measure_context_t& context, std::vector<cf_t>& buffer, float rx_gain_offset)
{
  ...
  uint32_t                      N_id = 0;
  if (isrran_ssb_csi_search(&ssb, buffer.data(), context.sf_len * context.meas_len_ms, &N_id, &meas) < ISRRAN_SUCCESS) {
  ...
  if ((logger.info.enabled() and valid) or logger.debug.enabled()) {
    ...
    isrran_csi_rs_measure_info(&meas, str_info.data(), (uint32_t)str_info.size());
