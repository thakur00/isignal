...
intra_measure_lte::intra_measure_lte(isrlog::basic_logger& logger_, meas_itf& new_cell_itf_) :
...
{
  scell_rx.deinit();
  isrran_refsignal_dl_sync_free(&refsignal_dl_sync);
}
...
void intra_measure_lte::init(uint32_t cc_idx, const args_t& args)
{
  ...
  isrran_refsignal_dl_sync_init(&refsignal_dl_sync, ISRRAN_CP_NORM);

  // Start scell
  scell_rx.init(args.len_ms);
}

void intra_measure_lte::set_primary_cell(uint32_t earfcn, isrran_cell_t cell)
{
  {
  ...
  set_current_sf_len((uint32_t)ISRRAN_SF_LEN_PRB(cell.nof_prb));
}
...
bool intra_measure_lte::measure_rat(const measure_context_t& context, std::vector<cf_t>& buffer, float rx_gain_offset)
{
  ...
  isrran_cell_t serving_cell_copy{};
  {
    std::lock_guard<std::mutex> lock(mutex);
  ...
  for (const uint32_t& id : cells_to_measure) {
    ...
    isrran_cell_t cell = serving_cell_copy;
    cell.id            = id;

    if (isrran_refsignal_dl_sync_set_cell(&refsignal_dl_sync, cell) < ISRRAN_SUCCESS) {
      Log(error, "Error setting refsignal DL cell");
      return false;
    }

    if (isrran_refsignal_dl_sync_run(&refsignal_dl_sync, buffer.data(), context.meas_len_ms * context.sf_len) <
        ISRRAN_SUCCESS) {
