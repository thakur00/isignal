 ...
 *
 */
#include "isrue/hdr/phy/scell/intra_measure_base.h"
...
intra_measure_base::intra_measure_base(isrlog::basic_logger& logger, meas_itf& new_cell_itf_) :
  logger(logger), context(new_cell_itf_), thread("SYNC_INTRA_MEASURE")
{}

intra_measure_base::~intra_measure_base()
{
  isrran_ringbuffer_free(&ring_buffer);
}
...
void intra_measure_base::init_generic(uint32_t cc_idx_, const args_t& args)
{
  ...
  } else if (get_rat() == isrran::isrran_rat_t::lte) {
    // Select maximum subframe size for LTE
    context.sf_len = ISRRAN_SF_LEN_PRB(ISRRAN_MAX_PRB);
  } else {
    // No maximum subframe length is defined for other RATs
    ERROR("A sampling rate was expected for %s. Undefined behaviour.", isrran::to_string(get_rat()).c_str());
    return;
  }
  ...
  if (ring_buffer.capacity < max_required_bytes) {
    ...
    if (isrran_ringbuffer_init(&ring_buffer, max_required_bytes) < ISRRAN_SUCCESS) {
...
void intra_measure_base::stop()
{
  ...
  isrran_ringbuffer_stop(&ring_buffer);
}
...
void intra_measure_base::write(cf_t* data, uint32_t nsamples)
{
  ...
  nbytes = ISRRAN_MIN(isrran_ringbuffer_space(&ring_buffer), nbytes);

  // Try writing in the buffer
  if (isrran_ringbuffer_write(&ring_buffer, data, nbytes) < nbytes) {
    ...
    if (isrran_ringbuffer_status(&ring_buffer) >= required_nbytes) {
  ...
  switch (state.get_state()) {
    ...
    case internal_state::wait_first:
      ...
      if (receive_tti_trigger(tti)) {
        ...
        isrran_ringbuffer_reset(&ring_buffer);
...
void intra_measure_base::measure_proc()
{
  ...
  int ret = isrran_ringbuffer_read_timed(&ring_buffer,
                                         search_buffer.data(),
