...
#include "isrue/hdr/metrics_csv.h"
#include "isrue/hdr/metrics_json.h"
#include "isrue/hdr/metrics_stdout.h"
...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("rrc.ue_category",         bpo::value<string>(&args->stack.rrc.ue_category_str)->default_value(ISRRAN_UE_CATEGORY_DEFAULT),  "UE Category (1 to 10)")
    ...
    ("rrc.release",             bpo::value<uint32_t>(&args->stack.rrc.release)->default_value(ISRRAN_RELEASE_DEFAULT),            "UE Release (8 to 15)")
    ...
    ("gw.ip_devname", bpo::value<string>(&args->gw.tun_dev_name)->default_value("tun_isrue"), "Name of the tun_isrue device")
    ("gw.ip_netmask", bpo::value<string>(&args->gw.tun_dev_netmask)->default_value("255.255.255.0"), "Netmask of the tun_isrue device")
    ...
    ("phy.force_ul_amplitude",
       ...
       "Forces the peak amplitude in the PUCCH, PUSCH and ISR (set 0.0 to 1.0, set to 0 or negative for disabling)")
  ...
  args->stack.rrc_nr.scs     = isrran_subcarrier_spacing_from_str(scs_khz.c_str());
  args->stack.rrc_nr.ssb_scs = isrran_subcarrier_spacing_from_str(ssb_scs_khz.c_str());
  if (args->stack.rrc_nr.scs == isrran_subcarrier_spacing_invalid ||
      args->stack.rrc_nr.ssb_scs == isrran_subcarrier_spacing_invalid) {
...
int main(int argc, char* argv[])
{
  ...
  log_sink = (args.log.filename == "stdout")
                 ? isrlog::create_stdout_sink()
                 : isrlog::create_file_sink(args.log.filename, fixup_log_file_maxsize(args.log.file_max_size));
  if (!log_sink) {
    return ISRRAN_ERROR;
  }
  ...
  isrran::metrics_hub<ue_metrics_t> metricshub;
  ...
  isrlog::sink& json_sink =
      isrlog::fetch_file_sink(args.general.metrics_json_filename, 0, false, isrlog::create_json_formatter());
  ...
  isrue::metrics_json json_metrics(json_channel);
