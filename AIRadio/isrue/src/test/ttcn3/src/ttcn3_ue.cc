...
extern "C" {
ISRRAN_API char* isrran_get_build_info();
ISRRAN_API char* isrran_get_build_mode();
}

ttcn3_ue::ttcn3_ue() : logger(isrlog::fetch_basic_logger("UE", false)), tft_matcher(logger) {}
...
int ttcn3_ue::init(all_args_t args, syssim_interface_phy* syssim_, const std::string tc_name_)
{
  ...
  logger.info("Built in %s mode using %s.", isrran_get_build_mode(), isrran_get_build_info());
  ...
  args.stack.rrc.ue_category_str     = ISRRAN_UE_CATEGORY_DEFAULT;
  ...
  phy = std::unique_ptr<isrue::lte_ttcn3_phy>(new isrue::lte_ttcn3_phy);
  if (!phy) {
...
void ttcn3_ue::new_grant_ul(const isrue::mac_interface_phy_lte::mac_grant_ul_t grant)
{
  phy->new_grant_ul(grant);
}

void ttcn3_ue::new_tb(const isrue::mac_interface_phy_lte::mac_grant_dl_t mac_grant, const uint8_t* data)
{
...
void ttcn3_ue::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
void ttcn3_ue::write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu) {}
...
void ttcn3_ue::loop_back_pdu_with_tft(isrran::unique_byte_buffer_t pdu)
{
  ...
  if (tft_matcher.check_tft_filter_match(pdu, target_eps_bearer_id) == ISRRAN_SUCCESS) {
