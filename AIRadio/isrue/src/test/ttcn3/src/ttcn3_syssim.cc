...
ttcn3_syssim::ttcn3_syssim(ttcn3_ue* ue_) :
  logger(isrlog::fetch_basic_logger("SS")),
  ut_logger(isrlog::fetch_basic_logger("UT", false)),
  sys_logger(isrlog::fetch_basic_logger("SYS", false)),
  ip_sock_logger(isrlog::fetch_basic_logger("IP_S", false)),
  ip_ctrl_logger(isrlog::fetch_basic_logger("IP_C", false)),
  srb_logger(isrlog::fetch_basic_logger("SRB", false)),
  drb_logger(isrlog::fetch_basic_logger("DRB", false)),
  ss_mac_logger(isrlog::fetch_basic_logger("SS-MAC", false)),
  ss_rlc_logger(isrlog::fetch_basic_logger("SS-RLC", false)),
  ss_pdcp_logger(isrlog::fetch_basic_logger("SS-PDCP", false)),
  ut(ut_logger),
  sys(sys_logger),
  ...
  timer_handler(create_tti_timer(), [&](uint64_t res) { new_tti_indication(res); })
{
  ...
  if (ue->init(args, this, "INIT_TEST") != ISRRAN_SUCCESS) {
    ue->stop();
...
int ttcn3_syssim::init(const all_args_t& args_)
{
  ...
  if (args.log.filename == "stdout") {
    auto* swp_sink = isrlog::find_sink(swappable_sink::name());
    if (!swp_sink) {
      logger.error("Unable to find the swappable sink");
      isrran::console("Unable to find the swappable sink\n");
      return ISRRAN_ERROR;
    }
  ...
  auto logger_lvl = isrlog::str_to_basic_level(args.log.all_level);
  ...
  if (add_epoll(signal_fd, epoll_fd) != ISRRAN_SUCCESS) {
  ...
  if (add_port_handler() != ISRRAN_SUCCESS) {
...
void ttcn3_syssim::new_tti_indication(uint64_t res)
{
  ...
  while (not event_queue.empty()) {
    ...
    switch (ev) {
      case UE_SWITCH_ON:
        isrran::console("Switching on UE ID=%d\n", run_id);
        ue->switch_on();
        break;
      case UE_SWITCH_OFF:
        isrran::console("Switching off UE ID=%d\n", run_id);
        ue->switch_off();
        break;
      case ENABLE_DATA:
        isrran::console("Enabling data for UE ID=%d\n", run_id);
        ue->enable_data();
        break;
      case DISABLE_DATA:
        isrran::console("Disabling data for UE ID=%d\n", run_id);
        ue->disable_data();
        break;
    }
  }
  ...
  if (ISRRAN_RNTI_ISSI(dl_rnti) && (tti % 2 == 0)) {
  ...
  } else if (ISRRAN_RNTI_ISRAR(dl_rnti)) {
    if (prach_tti != -1) {
    ...
    if (dl_rnti != ISRRAN_INVALID_RNTI) {
      ...
      for (int lcid = 0; lcid < ISRRAN_N_RADIO_BEARERS; lcid++) {
...
void ttcn3_syssim::tc_start(const char* name)
{
  ...
  isrran::console("Initializing UE ID=%d for TC=%s\n", run_id, tc_name.c_str());
  ...
  if (add_epoll(timer_handler.get_timer_fd(), epoll_fd) != ISRRAN_SUCCESS) {
...
void ttcn3_syssim::tc_end()
{
  ...
  isrran::console("Deinitializing UE ID=%d\n", run_id);
  ue->stop();
...
void ttcn3_syssim::send_rar(uint32_t preamble_index)
{
  ...
  isrran_dci_rar_pack(&rar_grant, grant_buffer);

  // Create MAC PDU and add RAR subheader
  isrran::rar_pdu rar_pdu;
  rar_buffer.clear();
...
bool ttcn3_syssim::process_ce(isrran::sch_subh* subh)
{
  ...
  switch (subh->ul_sch_ce_type()) {
    ...
    case isrran::ul_sch_lcid::SHORT_BSR:
      ...
      ss_mac_logger.info("CE:    Received %s BSR rnti=0x%x, lcg=%d, value=%d",
                         subh->ul_sch_ce_type() == isrran::ul_sch_lcid::SHORT_BSR ? "Short" : "Trunc",
                         rnti,
                         idx,
                         buff_size_idx[idx]);
      is_bsr = true;
...
{
  return tti_ % ISRRAN_FDD_NOF_HARQ;
}
                                ...
                                unique_byte_buffer_t               pdu)
{
  if (timing.now) {
    ...
    auto task = [this, cell_name](isrran::unique_byte_buffer_t& pdu) { add_ccch_pdu_impl(cell_name, std::move(pdu)); };
                                ...
                                bool                               follow_on_flag)
{
  if (timing.now) {
    ...
    auto task = [this, cell_name](uint32_t lcid, isrran::unique_byte_buffer_t& pdu, bool follow_on_flag) {
      add_dcch_pdu_impl(cell_name, lcid, std::move(pdu), follow_on_flag);
    };
...
void ttcn3_syssim::add_srb_impl(const std::string cell_name, const uint32_t lcid, const pdcp_config_t pdcp_config)
{
  ...
  cell->rlc.add_bearer(lcid, isrran::rlc_config_t::srb_config(lcid));
}
...
void ttcn3_syssim::add_drb(const ttcn3_helpers::timing_info_t timing,
                           const std::string                  cell_name,
                           const uint32_t                     lcid,
                           const isrran::pdcp_config_t        pdcp_config)
{
  if (timing.now) {
...
void ttcn3_syssim::add_drb_impl(const std::string cell_name, const uint32_t lcid, const pdcp_config_t pdcp_config)
{
  ...
  if (lcid > 2) {
    ...
    cell->pdcp.add_bearer(lcid, pdcp_config);
    cell->rlc.add_bearer(lcid, isrran::rlc_config_t::default_rlc_am_config());
  }
}
...
void ttcn3_syssim::set_as_security(const ttcn3_helpers::timing_info_t        timing,
                                   ...
                                   const isrran::CIPHERING_ALGORITHM_ID_ENUM cipher_algo_,
                                   const isrran::INTEGRITY_ALGORITHM_ID_ENUM integ_algo_,
...
void ttcn3_syssim::select_cell(isrran_cell_t phy_cell)
{
  std::lock_guard<std::mutex> lock(syssim_mutex);
...
ttcn3_helpers::pdcp_count_map_t ttcn3_syssim::get_pdcp_count(const std::string cell_name)
{
  ...
  for (uint32_t i = 0; i < rb_id_vec.size(); i++) {
    if (cell->pdcp.is_lcid_enabled(i)) {
      ...
      isrran::pdcp_lte_state_t    pdcp_state;
