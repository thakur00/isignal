...
{
  std::cout << "Built in " << isrran_get_build_mode() << " mode using " << isrran_get_build_info() << "." << std::endl;
  ...
  if (!isrlog::install_custom_sink(swappable_sink::name(),
                                   std::unique_ptr<swappable_sink>(new swappable_sink(
                                       dut_args.log_filename, isrlog::get_default_log_formatter())))) {
    return ISRRAN_ERROR;
  }
  auto* default_sink = isrlog::find_sink(swappable_sink::name());
  if (!default_sink) {
    return ISRRAN_ERROR;
  }
  isrlog::set_default_sink(*default_sink);
  ...
  if (syssim.init(ue_args) != ISRRAN_SUCCESS) {
