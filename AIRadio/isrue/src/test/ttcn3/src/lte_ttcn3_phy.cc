...
lte_ttcn3_phy::lte_ttcn3_phy() : logger(isrlog::fetch_basic_logger("PHY")) {}
...
int lte_ttcn3_phy::init(const phy_args_t& args_, stack_interface_phy_lte* stack_, syssim_interface_phy* syssim_)
{
  ...
  logger.set_level(isrlog::str_to_basic_level(args_.log.phy_level));
...
void lte_ttcn3_phy::get_metrics(const isrran::isrran_rat_t& rat, phy_metrics_t* m) {}
...
void lte_ttcn3_phy::set_config_tdd(isrran_tdd_config_t& tdd_config) {}
...
bool lte_ttcn3_phy::set_config(const isrran::phy_cfg_t& config, uint32_t cc_idx_)
{
  logger.debug("%s not implemented.", __FUNCTION__);
...
bool lte_ttcn3_phy::set_scell(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn)
{
  logger.debug("%s not implemented.", __FUNCTION__);
...
void lte_ttcn3_phy::set_rar_grant(uint8_t grant_payload[ISRRAN_RAR_GRANT_LEN], uint16_t rnti)
{
...
void lte_ttcn3_phy::new_tb(const isrue::mac_interface_phy_lte::mac_grant_dl_t dl_grant, const uint8_t* data)
{
  std::lock_guard<std::mutex> lock(phy_mutex);
