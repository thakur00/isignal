...
#ifndef ISRUE_TTCN3_PORT_HANDLER_H
#define ISRUE_TTCN3_PORT_HANDLER_H

#include "isrran/common/epoll_helper.h"
...
{
public:
  explicit ttcn3_port_handler(isrlog::basic_logger& logger) :
  ...
  int port_listen()
  {
    ...
    if (not isrran::net_utils::set_sockaddr(&bind_addr, net_ip.c_str(), net_port)) {
      isrran::console("Invalid net_ip: %s\n", net_ip.c_str());
      return ISRRAN_ERROR;
    }
    int one = 1;
...
#endif // ISRUE_TTCN3_PORT_HANDLER_H
