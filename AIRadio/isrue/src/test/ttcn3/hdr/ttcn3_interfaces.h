...
#ifndef ISRUE_TTCN3_INTERFACES_H
#define ISRUE_TTCN3_INTERFACES_H
...
{
public:
  virtual void add_bcch_dlsch_pdu(const std::string cell_name, isrran::unique_byte_buffer_t pdu) = 0;
  virtual void add_pch_pdu(isrran::unique_byte_buffer_t pdu)                                     = 0;
  ...
  virtual void add_srb(const ttcn3_helpers::timing_info_t timing,
                       const std::string                  cell_name,
                       const uint32_t                     lcid,
                       const isrran::pdcp_config_t        pdcp_config)                                                     = 0;
  ...
  virtual void set_as_security(const ttcn3_helpers::timing_info_t        timing,
                               ...
                               const isrran::CIPHERING_ALGORITHM_ID_ENUM cipher_algo,
                               const isrran::INTEGRITY_ALGORITHM_ID_ENUM integ_algo,
...
public:
  virtual void add_ccch_pdu(const ttcn3_helpers::timing_info_t timing,
                            const std::string                  cell_name,
                            isrran::unique_byte_buffer_t       pdu) = 0;
...
public:
  ...
  virtual void select_cell(isrran_cell_t cell)                                      = 0;
};
...
public:
  ...
  virtual void     new_grant_ul(const isrue::mac_interface_phy_lte::mac_grant_ul_t grant)                    = 0;
  virtual void     new_tb(const isrue::mac_interface_phy_lte::mac_grant_dl_t mac_grant, const uint8_t* data) = 0;
};

#endif // ISRUE_TTCN3_INTERFACES_H
