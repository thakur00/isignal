...
#ifndef ISRUE_TTCN3_LTE_PHY_H
#define ISRUE_TTCN3_LTE_PHY_H
...
public:
  ...
  void        get_metrics(const isrran::isrran_rat_t& rat, phy_metrics_t* m) override;
  ...
  bool set_config(const isrran::phy_cfg_t& config, uint32_t cc_idx = 0) override;
  bool set_scell(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn) override;
  ...
  void new_tb(const isrue::mac_interface_phy_lte::mac_grant_dl_t, const uint8_t* data);
