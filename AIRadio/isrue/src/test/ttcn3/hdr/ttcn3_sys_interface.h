...
#ifndef ISRUE_TTCN3_SYS_INTERFACE_H
#define ISRUE_TTCN3_SYS_INTERFACE_H
...
{
public:
  explicit ttcn3_sys_interface(isrlog::basic_logger& logger) : ttcn3_port_handler(logger) {}
  ~ttcn3_sys_interface(){};
...
private:
  ...
  void handle_request_cell_basic(Document& document, const uint8_t* payload, const uint16_t len)
  {
    ...
    if (document["Request"]["Cell"]["AddOrReconfigure"]["Basic"].HasMember("StaticCellInfo")) {
      ...
      cell.phy_cell.cp =
          (strcmp(dl_config["CyclicPrefix"].GetString(), "normal") == 0) ? ISRRAN_CP_NORM : ISRRAN_CP_EXT;
    ...
    while (consumed_bytes < len) {
      ...
      unique_byte_buffer_t sib = isrran::make_byte_buffer();
      if (sib == nullptr) {
  ...
  void handle_request_as_security(Document& document)
  {
    ...
    if (as_sec.HasMember("StartRestart")) {
      ...
      if (int_algo_string == "eia0") {
        integ_algo = isrran::INTEGRITY_ALGORITHM_ID_EIA0;
      } else if (int_algo_string == "eia1") {
        integ_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA1;
      } else if (int_algo_string == "eia2") {
        integ_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA2;
      } else {
      ...
      if (cipher_algo_string == "eea0") {
        cipher_algo = isrran::CIPHERING_ALGORITHM_ID_EEA0;
      } else if (cipher_algo_string == "eea1") {
        cipher_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA1;
      } else if (cipher_algo_string == "eea2") {
        cipher_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA2;
      } else {
  ...
  void handle_request_paging(Document& document, const uint8_t* payload, const uint16_t len)
  {
    ...
    unique_byte_buffer_t pch = isrran::make_byte_buffer();
    if (pch == nullptr) {
      logger.error("Couldn't allocate buffer in %s().", __FUNCTION__);
      return;
    }
...
#endif // ISRUE_TTCN3_SYS_INTERFACE_H
