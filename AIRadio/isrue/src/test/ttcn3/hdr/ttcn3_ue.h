...
#ifndef ISRUE_TTCN3_UE_H
#define ISRUE_TTCN3_UE_H
...
public:
  ...
  void new_grant_ul(const isrue::mac_interface_phy_lte::mac_grant_ul_t grant);

  void new_tb(const isrue::mac_interface_phy_lte::mac_grant_dl_t mac_grant, const uint8_t* data);
  ...
  void loop_back_pdu_with_tft(isrran::unique_byte_buffer_t pdu);
...
private:
  ...
  isrran::timer_handler::unique_timer                            pdu_delay_timer;
  block_queue<isrran::unique_byte_buffer_t>                      pdu_queue; // PDU for UL data
