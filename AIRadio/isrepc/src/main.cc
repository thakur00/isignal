...
#include "isrepc/hdr/mme/mme.h"
#include "isrepc/hdr/spgw/spgw.h"
#include "isrran/build_info.h"
#include "isrran/common/bcd_helpers.h"
#include "isrran/common/common_helper.h"
#include "isrran/common/config_file.h"
...
static isrlog::sink*     log_sink = nullptr;
...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("mme.name",            bpo::value<string>(&mme_name)->default_value("isrmme01"),        "MME Name")
    ...
    ("mme.short_net_name",  bpo::value<string>(&short_net_name)->default_value("isrRAN"),    "Short name of the network")
    ...
    ("spgw.sgi_if_name",    bpo::value<string>(&sgi_if_name)->default_value("isr_spgw_sgi"), "Name of TUN interface for the SGi connection")
  ...
  if (!isrran::string_to_mcc(mcc, &args->mme_args.s1ap_args.mcc)) {
    cout << "Error parsing mme.mcc:" << mcc << " - must be a 3-digit string." << endl;
  }
  if (!isrran::string_to_mnc(mnc, &args->mme_args.s1ap_args.mnc)) {
    cout << "Error parsing mme.mnc:" << mnc << " - must be a 2 or 3-digit string." << endl;
  }

  // Convert MCC/MNC strings
  if (!isrran::string_to_mcc(mcc, &args->hss_args.mcc)) {
    cout << "Error parsing mme.mcc:" << mcc << " - must be a 3-digit string." << endl;
  }
  if (!isrran::string_to_mnc(mnc, &args->hss_args.mnc)) {
    cout << "Error parsing mme.mnc:" << mnc << " - must be a 2 or 3-digit string." << endl;
  }

  std::transform(encryption_algo.begin(), encryption_algo.end(), encryption_algo.begin(), ::tolower);
  if (encryption_algo == "eea0") {
    args->mme_args.s1ap_args.encryption_algo = isrran::CIPHERING_ALGORITHM_ID_EEA0;
  } else if (encryption_algo == "eea1") {
    args->mme_args.s1ap_args.encryption_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA1;
  } else if (encryption_algo == "eea2") {
    args->mme_args.s1ap_args.encryption_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA2;
  } else if (encryption_algo == "eea3") {
    args->mme_args.s1ap_args.encryption_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA3;
  } else {
    args->mme_args.s1ap_args.encryption_algo = isrran::CIPHERING_ALGORITHM_ID_EEA0;
  ...
  if (integrity_algo == "eia0") {
    args->mme_args.s1ap_args.integrity_algo = isrran::INTEGRITY_ALGORITHM_ID_EIA0;
    ...
    args->mme_args.s1ap_args.integrity_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA1;
  } else if (integrity_algo == "eia2") {
    args->mme_args.s1ap_args.integrity_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA2;
  } else if (integrity_algo == "eia3") {
    args->mme_args.s1ap_args.integrity_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA3;
  } else {
    args->mme_args.s1ap_args.integrity_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA1;
...
{
  isrran_register_signal_handler(signal_handler);
  ...
  // Setup logging.
  log_sink = (args.log_args.filename == "stdout") ? isrlog::create_stdout_sink()
                                                  : isrlog::create_file_sink(args.log_args.filename);
  if (!log_sink) {
    return ISRRAN_ERROR;
  }
  isrlog::log_channel* chan = isrlog::create_log_channel("main_channel", *log_sink);
  if (!chan) {
    return ISRRAN_ERROR;
  }
  isrlog::set_default_sink(*log_sink);

  // Start the log backend.
  isrlog::init();

  if (args.log_args.filename != "stdout") {
    auto& epc_logger = isrlog::fetch_basic_logger("EPC", false);
    epc_logger.info("\n\n%s\n---  Software Radio Systems EPC log ---\n\n", get_build_string().c_str());
  }

  isrran::log_args(argc, argv, "EPC");

  isrlog::basic_logger& nas_logger = isrlog::fetch_basic_logger("NAS", false);
  nas_logger.set_level(isrlog::str_to_basic_level(args.log_args.nas_level));
  nas_logger.set_hex_dump_max_size(args.log_args.nas_hex_limit);

  auto& s1ap_logger = isrlog::fetch_basic_logger("S1AP", false);
  s1ap_logger.set_level(isrlog::str_to_basic_level(args.log_args.s1ap_level));
  s1ap_logger.set_hex_dump_max_size(args.log_args.s1ap_hex_limit);

  auto& mme_gtpc_logger = isrlog::fetch_basic_logger("MME GTPC", false);
  mme_gtpc_logger.set_level(isrlog::str_to_basic_level(args.log_args.mme_gtpc_level));
  mme_gtpc_logger.set_hex_dump_max_size(args.log_args.mme_gtpc_hex_limit);

  auto& hss_logger = isrlog::fetch_basic_logger("HSS", false);
  hss_logger.set_level(isrlog::str_to_basic_level(args.log_args.hss_level));
  hss_logger.set_hex_dump_max_size(args.log_args.hss_hex_limit);

  auto& spgw_gtpc_logger = isrlog::fetch_basic_logger("SPGW GTPC", false);
  spgw_gtpc_logger.set_level(isrlog::str_to_basic_level(args.log_args.spgw_gtpc_level));
  spgw_gtpc_logger.set_hex_dump_max_size(args.log_args.spgw_gtpc_hex_limit);

  auto& gtpu_logger = isrlog::fetch_basic_logger("GTPU", false);
  gtpu_logger.set_level(isrlog::str_to_basic_level(args.log_args.gtpu_level));
  gtpu_logger.set_hex_dump_max_size(args.log_args.gtpu_hex_limit);

  auto& spgw_logger = isrlog::fetch_basic_logger("SPGW", false);
  spgw_logger.set_level(isrlog::str_to_basic_level(args.log_args.spgw_level));
