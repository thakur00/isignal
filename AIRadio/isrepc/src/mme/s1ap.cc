...
int s1ap::init(const s1ap_args_t& s1ap_args)
{
  ...
  isrran::s1ap_mccmnc_to_plmn(s1ap_args.mcc, s1ap_args.mnc, &m_plmn);
...
void s1ap::stop()
{
  ...
  while (enb_it != m_active_enbs.end()) {
    ...
    isrran::console("Deleting eNB context. eNB Id: 0x%x\n", enb_it->second->enb_id);
  ...
  while (ue_it != m_imsi_to_nas_ctx.end()) {
    ...
    isrran::console("Deleting UE EMM context. IMSI: %015" PRIu64 "\n", ue_it->first);
    delete ue_it->second;
...
int s1ap::enb_listen()
{
  ...
  if (sock_fd == -1) {
    isrran::console("Could not create SCTP socket\n");
    return ISRRAN_ERROR;
  }
  ...
  if (setsockopt(sock_fd, IPPROTO_SCTP, SCTP_EVENTS, &evnts, sizeof(evnts))) {
    close(sock_fd);
    isrran::console("Subscribing to sctp_data_io_events failed\n");
    return ISRRAN_ERROR;
  }

  // S1-MME bind
  bzero(&s1mme_addr, sizeof(s1mme_addr));
  if (not isrran::net_utils::set_sockaddr(&s1mme_addr, m_s1ap_args.mme_bind_addr.c_str(), S1MME_PORT)) {
    close(sock_fd);
    m_logger.error("Invalid mme_bind_addr: %s", m_s1ap_args.mme_bind_addr.c_str());
    isrran::console("Invalid mme_bind_addr: %s\n", m_s1ap_args.mme_bind_addr.c_str());
    return ISRRAN_ERROR;
  }

  if (not isrran::net_utils::bind_addr(sock_fd, s1mme_addr)) {
    close(sock_fd);
    m_logger.error("Error binding SCTP socket");
    isrran::console("Error binding SCTP socket\n");
    return ISRRAN_ERROR;
  }
  ...
  if (err != 0) {
    ...
    isrran::console("Error in SCTP socket listen\n");
    return ISRRAN_ERROR;
  }
...
bool s1ap::s1ap_tx_pdu(const asn1::s1ap::s1ap_pdu_c& pdu, struct sctp_sndrcvinfo* enb_sri)
{
  ...
  if (pdu.pack(bref) != asn1::ISRASN_SUCCESS) {
  ...
  if (n_sent == -1) {
    isrran::console("Failed to send S1AP PDU. Error: %s\n", strerror(errno));
...
void s1ap::handle_s1ap_rx_pdu(isrran::byte_buffer_t* pdu, struct sctp_sndrcvinfo* enb_sri)
{
  // Save PCAP
  if (m_pcap_enable) {
...
void s1ap::handle_initiating_message(const asn1::s1ap::init_msg_s& msg, struct sctp_sndrcvinfo* enb_sri)
{
  ...
  switch (msg.value.type().value) {
    ...
    default:
      ...
      isrran::console("Unhandled S1APinitiating message: %s\n", msg.value.type().to_string());
  }
}
...
{
  isrran::console("Releasing UEs context\n");
  std::map<int32_t, std::set<uint32_t> >::iterator ues_in_enb = m_enb_assoc_to_ue_ids.find(enb_assoc);
  std::set<uint32_t>::iterator                     ue_id      = ues_in_enb->second.begin();
  if (ue_id == ues_in_enb->second.end()) {
    isrran::console("No UEs to be released\n");
  } else {
    while (ue_id != ues_in_enb->second.end()) {
      ...
      isrran::console("Releasing UE ECM context. UE-MME S1AP Id: %d\n", ecm_ctx->mme_ue_s1ap_id);
