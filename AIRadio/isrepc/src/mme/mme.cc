...
void mme::run_thread()
{
  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer("mme::run_thread");
  if (pdu == nullptr) {
    m_s1ap_logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
    return;
  }
  uint32_t                     sz  = ISRRAN_MAX_BUFFER_SIZE_BYTES - ISRRAN_BUFFER_HEADER_OFFSET;
