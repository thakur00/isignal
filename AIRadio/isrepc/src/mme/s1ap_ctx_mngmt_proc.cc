...
#include "isrepc/hdr/mme/s1ap_ctx_mngmt_proc.h"
...
bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request(nas* nas_ctx, uint16_t erab_to_setup)
{
  ...
  isrran::unique_byte_buffer_t nas_buffer = isrran::make_byte_buffer();
  if (emm_ctx->state == EMM_STATE_DEREGISTERED) {
    // Attach procedure initiated from an attach request
    isrran::console("Adding attach accept to Initial Context Setup Request\n");
  ...
  isrran::console("Sent Initial Context Setup Request. E-RAB id %d \n", erab_ctx_req.erab_id);
  m_logger.info(
    ...
    const asn1::s1ap::init_context_setup_resp_s& in_ctxt_resp)
{
  ...
  isrran::console("Received Initial Context Setup Response\n");
  ...
  for (const asn1::protocol_ie_single_container_s<asn1::s1ap::erab_setup_item_ctxt_su_res_ies_o>& ie_container :
    ...
    isrran::console("E-RAB Context Setup. E-RAB id %d\n", esm_ctx->erab_id);
    isrran::console("E-RAB Context -- eNB TEID 0x%x; eNB GTP-U Address %s\n", esm_ctx->enb_fteid.teid, enb_addr_str);
  }

  if (emm_ctx->state == EMM_STATE_REGISTERED) {
    isrran::console("Initial Context Setup Response triggered from Service Request.\n");
    isrran::console("Sending Modify Bearer Request.\n");
                                                            ...
                                                            struct sctp_sndrcvinfo*                         enb_sri)
{
  ...
  isrran::console("Received UE Context Release Request. MME-UE S1AP Id %d\n", mme_ue_s1ap_id);

  nas* nas_ctx = m_s1ap->find_nas_ctx_from_mme_ue_s1ap_id(mme_ue_s1ap_id);
  if (nas_ctx == nullptr) {
    m_logger.info("No UE context to release found. MME-UE S1AP Id: %d", mme_ue_s1ap_id);
    isrran::console("No UE context to release found. MME-UE S1AP Id: %d\n", mme_ue_s1ap_id);
    return false;
  }
...
bool s1ap_ctx_mngmt_proc::send_ue_context_release_command(nas* nas_ctx)
{
  ...
  if (active_erabs) {
    ...
    isrran::console("There are active E-RABs, send release access bearers request\n");
...
bool s1ap_ctx_mngmt_proc::handle_ue_context_release_complete(const asn1::s1ap::ue_context_release_complete_s& rel_comp)
{
  ...
  isrran::console("Received UE Context Release Complete. MME-UE S1AP Id %d\n", mme_ue_s1ap_id);
  ...
  isrran::console("UE Context Release Completed.\n");
  return true;
}
} // namespace isrepc
