...
#include "isrepc/hdr/mme/s1ap.h"
#include "isrepc/hdr/mme/s1ap_nas_transport.h"
#include "isrran/common/liblte_security.h"
...
void nas::reset()
{
  ...
  isrran::INTEGRITY_ALGORITHM_ID_ENUM integ_algo  = m_sec_ctx.integ_algo;
  isrran::CIPHERING_ALGORITHM_ID_ENUM cipher_algo = m_sec_ctx.cipher_algo;
  m_sec_ctx                                       = {};
                                ...
                                const nas_if_t&         itf)
{
  ...
  auto&                                          nas_logger  = isrlog::fetch_basic_logger("NAS");
  ...
  if (attach_req.eps_mobile_id.type_of_id == LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI) {
    ...
    isrran::console("Attach request -- IMSI: %015" PRIu64 "\n", imsi);
    ...
    isrran::console("Attach request -- M-TMSI: 0x%x\n", m_tmsi);
  ...
  isrran::console("Attach request -- eNB-UE S1AP Id: %d\n", enb_ue_s1ap_id);
  nas_logger.info("Attach request -- eNB-UE S1AP Id: %d", enb_ue_s1ap_id);
  isrran::console("Attach request -- Attach type: %d\n", attach_req.eps_attach_type);
  nas_logger.info("Attach request -- Attach type: %d", attach_req.eps_attach_type);
  isrran::console("Attach Request -- UE Network Capabilities EEA: %d%d%d%d%d%d%d%d\n",
  ...
  nas_logger.info("Attach Request -- UE Network Capabilities EEA: %d%d%d%d%d%d%d%d",
                  ...
                  attach_req.ue_network_cap.eea[6],
                  attach_req.ue_network_cap.eea[7]);
  isrran::console("Attach Request -- UE Network Capabilities EIA: %d%d%d%d%d%d%d%d\n",
  ...
  if (nas_ctx == NULL) {
    ...
    isrran::console("Attach Request -- Found previously attach UE.\n");
    if (attach_req.eps_mobile_id.type_of_id == LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI) {
...
{
  nas*                         nas_ctx;
  isrran::unique_byte_buffer_t nas_tx;
  auto&                        nas_logger = isrlog::fetch_basic_logger("NAS");
  ...
  if (!hss->gen_auth_info_answer(nas_ctx->m_emm_ctx.imsi,
    ...
    isrran::console("User not found. IMSI %015" PRIu64 "\n", nas_ctx->m_emm_ctx.imsi);
  ...
  nas_tx = isrran::make_byte_buffer();
  if (nas_tx == nullptr) {
  ...
  isrran::console("Downlink NAS: Sending Authentication Request\n");
  return true;
}
...

{
  ...
  nas_tx = isrran::make_byte_buffer();
  if (nas_tx == nullptr) {
    isrlog::fetch_basic_logger("NAS").error("Couldn't allocate PDU in %s().", __FUNCTION__);
    return false;
  }
                                              ...
                                              const nas_if_t&                                       itf)
{
  ...
  isrran::console("Found UE context. IMSI: %015" PRIu64 ", old eNB UE S1ap Id %d, old MME UE S1AP Id %d\n",
                  emm_ctx->imsi,
  ...
  if (msg_valid == true && emm_ctx->state == EMM_STATE_DEREGISTERED) {
    ...
    // Re-generate K_eNB
    isrran::security_generate_k_enb(sec_ctx->k_asme, sec_ctx->ul_nas_count, sec_ctx->k_enb);
    nas_logger.info("Generating KeNB with UL NAS COUNT: %d", sec_ctx->ul_nas_count);
    isrran::console("Generating KeNB with UL NAS COUNT: %d\n", sec_ctx->ul_nas_count);
    ...
    if (ecm_ctx->eit) {
      isrran::console("Secure ESM information transfer requested.\n");
      ...
      isrran::console("Getting subscription information -- QCI %d\n", nas_ctx->m_esm_ctx[default_bearer].qci);
    ...
    if (emm_ctx->state != EMM_STATE_DEREGISTERED) {
      ...
      isrran::console("Received GUTI-Attach Request from attached user.\n");
    ...
    isrran::console("GUTI Attach request NAS integrity failed.\n");
    isrran::console("RE-starting authentication procedure.\n");
    ...
    isrran::console("Downlink NAS: Sent Authentication Request\n");
    return true;
  }
}
                                 ...
                                 const nas_if_t&         itf)
{
  ...
  isrran::console("Service request -- S-TMSI 0x%x\n", m_tmsi);
  nas_logger.info("Service request -- eNB UE S1AP Id %d", enb_ue_s1ap_id);
  isrran::console("Service request -- eNB UE S1AP Id %d\n", enb_ue_s1ap_id);
  ...
  if (imsi == 0) {
    isrran::console("Could not find IMSI from M-TMSI. M-TMSI 0x%x\n", m_tmsi);
    ...
    isrran::unique_byte_buffer_t nas_tx = isrran::make_byte_buffer();
    if (nas_tx == nullptr) {
      nas_logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
      return false;
    }
  ...
  if (nas_ctx == NULL || nas_ctx->m_emm_ctx.state != EMM_STATE_REGISTERED) {
    isrran::console("UE is not EMM-Registered.\n");
  ...
  if (mac_valid) {
    isrran::console("Service Request -- Short MAC valid\n");
    ...
    isrran::console("Service Request -- User is ECM DISCONNECTED\n");
    ...
    isrran::console("UE previously assigned IP: %s\n", inet_ntoa(emm_ctx->ue_ip));
    ...
    isrran::console("Service Request -- Short MAC invalid\n");
    ...
    isrran::console("Service Request -- Short MAC invalid. Sending service reject.\n");
                                ...
                                const nas_if_t&         itf)
{
  ...
  isrran::console("Detach Request -- S-TMSI 0x%x\n", m_tmsi);
  nas_logger.info("Detach Request -- eNB UE S1AP Id %d", enb_ue_s1ap_id);
  isrran::console("Detach Request -- eNB UE S1AP Id %d\n", enb_ue_s1ap_id);
  ...
  nas* nas_ctx = s1ap->find_nas_ctx_from_imsi(imsi);
  if (nas_ctx == NULL) {
    isrran::console("Could not find UE context from IMSI\n");
  ...
  isrran::console("Received. M-TMSI 0x%x\n", m_tmsi);
                                              ...
                                              const nas_if_t&         itf)
{
  ...
  isrran::console("Tracking Area Update Request -- S-TMSI 0x%x\n", m_tmsi);
  nas_logger.info("Tracking Area Update Request -- eNB UE S1AP Id %d", enb_ue_s1ap_id);
  isrran::console("Tracking Area Update Request -- eNB UE S1AP Id %d\n", enb_ue_s1ap_id);

  isrran::console("Warning: Tracking area update requests are not handled yet.\n");
 ...
 *
 ***************************************/
bool nas::handle_attach_request(isrran::byte_buffer_t* nas_rx)
{
  uint32_t                                       m_tmsi      = 0;
  uint64_t                                       imsi        = 0;
  LIBLTE_MME_ATTACH_REQUEST_MSG_STRUCT           attach_req  = {};
  ...
  if (m_emm_ctx.imsi == 0) {
    ...
    isrran::console("DL NAS: Sending Authentication Request\n");
    return true;
  } else {
...
bool nas::handle_pdn_connectivity_request(isrran::byte_buffer_t* nas_rx)
{
  LIBLTE_MME_PDN_CONNECTIVITY_REQUEST_MSG_STRUCT pdn_con_req = {};
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing PDN connectivity reject\n");
    return false;
  }
  ...
  isrran::console("DL NAS: Sending PDN Connectivity Reject\n");

  return true;
}

bool nas::handle_authentication_response(isrran::byte_buffer_t* nas_rx)
{
  ...
  isrran::console("Authentication Response -- IMSI %015" PRIu64 "\n", m_emm_ctx.imsi);
  ...
  if (!ue_valid) {
    ...
    isrran::console("UE Authentication Rejected.\n");
    ...
    isrran::console("UE Authentication Accepted.\n");
    ...
    isrran::console("Downlink NAS: Sending NAS Security Mode Command.\n");
  }
...
bool nas::handle_security_mode_complete(isrran::byte_buffer_t* nas_rx)
{
  ...
  isrran::console("Security Mode Command Complete -- IMSI: %015" PRIu64 "\n", m_emm_ctx.imsi);
  ...
  if (m_ecm_ctx.eit == true) {
    ...
    isrran::console("Sending ESM information request\n");
...
bool nas::handle_attach_complete(isrran::byte_buffer_t* nas_rx)
{
  ...
  isrran::console("Unpacked Attached Complete Message. IMSI %" PRIu64 "\n", m_emm_ctx.imsi);
  isrran::console("Unpacked Activate Default EPS Bearer message. EPS Bearer id %d\n", act_bearer.eps_bearer_id);
  ...
  if (m_emm_ctx.state == EMM_STATE_DEREGISTERED) {
    ...
    isrran::console("Sending EMM Information\n");
...
bool nas::handle_esm_information_response(isrran::byte_buffer_t* nas_rx)
{
  LIBLTE_MME_ESM_INFORMATION_RESPONSE_MSG_STRUCT esm_info_resp;

  // Get NAS authentication response
  LIBLTE_ERROR_ENUM err =
      isrran_mme_unpack_esm_information_response_msg((LIBLTE_BYTE_MSG_STRUCT*)nas_rx, &esm_info_resp);
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error unpacking NAS authentication response. Error: %s", liblte_error_text[err]);
    return false;
  }
  ...
  if (esm_info_resp.protocol_cnfg_opts_present) {
    ...
    isrran::console("ESM Info: %d Protocol Configuration Options\n", esm_info_resp.protocol_cnfg_opts.N_opts);
  }
...
bool nas::handle_identity_response(isrran::byte_buffer_t* nas_rx)
{
  isrran::unique_byte_buffer_t      nas_tx;
  ...
  isrran::console("ID Response -- IMSI: %015" PRIu64 "\n", imsi);
...
bool nas::handle_tracking_area_update_request(isrran::byte_buffer_t* nas_rx)
{
  isrran::console("Warning: Tracking Area Update Request messages not handled yet.\n");
...
bool nas::handle_authentication_failure(isrran::byte_buffer_t* nas_rx)
{
  ...
  switch (auth_fail.emm_cause) {
    case 20:
      isrran::console("MAC code failure\n");
      m_logger.info("MAC code failure");
      break;
    case 26:
      isrran::console("Non-EPS authentication unacceptable\n");
      m_logger.info("Non-EPS authentication unacceptable");
      break;
    case 21:
      isrran::console("Authentication Failure -- Synchronization Failure\n");
      ...
      if (!m_hss->resync_sqn(m_emm_ctx.imsi, auth_fail.auth_fail_param)) {
        isrran::console("Resynchronization failed. IMSI %015" PRIu64 "\n", m_emm_ctx.imsi);
...
bool nas::handle_detach_request(isrran::byte_buffer_t* nas_msg)
{
  isrran::console("Detach request -- IMSI %015" PRIu64 "\n", m_emm_ctx.imsi);
...
bool nas::pack_authentication_request(isrran::byte_buffer_t* nas_buffer)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing Authentication Request\n");
    return false;
  }
  return true;
}

bool nas::pack_authentication_reject(isrran::byte_buffer_t* nas_buffer)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing Authentication Reject\n");
    return false;
  }
  return true;
}

bool nas::pack_security_mode_command(isrran::byte_buffer_t* nas_buffer)
{
  ...
  isrran::security_generate_k_nas(
  ...
  uint8_t key_enb[32];
  isrran::security_generate_k_enb(m_sec_ctx.k_asme, m_sec_ctx.ul_nas_count, m_sec_ctx.k_enb);
...
bool nas::pack_esm_information_request(isrran::byte_buffer_t* nas_buffer)
{
  ...
  LIBLTE_ERROR_ENUM err = isrran_mme_pack_esm_information_request_msg(
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing ESM information request\n");
    return false;
  }
...
bool nas::pack_attach_accept(isrran::byte_buffer_t* nas_buffer)
{
  ...
  if (inet_pton(AF_INET, m_dns.c_str(), &(dns_addr.sin_addr)) != 1) {
    ...
    isrran::console("Invalid m_dns: %s\n", m_dns.c_str());
    perror("inet_pton");
    return false;
  }
...
bool nas::pack_identity_request(isrran::byte_buffer_t* nas_buffer)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing Identity Request\n");
    return false;
  }
  return true;
}

bool nas::pack_emm_information(isrran::byte_buffer_t* nas_buffer)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing EMM Information\n");
    return false;
  }
...
bool nas::pack_service_reject(isrran::byte_buffer_t* nas_buffer, uint8_t emm_cause)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing Service Reject\n");
    return false;
  }
  return true;
}

bool nas::pack_tracking_area_update_reject(isrran::byte_buffer_t* nas_buffer, uint8_t emm_cause)
{
  ...
  if (err != LIBLTE_SUCCESS) {
    ...
    isrran::console("Error packing Tracking Area Update Reject\n");
    return false;
  }
  return true;
}
 ...
 *
 ************************/
bool nas::short_integrity_check(isrran::byte_buffer_t* pdu)
{
  ...
  switch (m_sec_ctx.integ_algo) {
    case isrran::INTEGRITY_ALGORITHM_ID_EIA0:
      break;
    case isrran::INTEGRITY_ALGORITHM_ID_128_EIA1:
      isrran::security_128_eia1(&m_sec_ctx.k_nas_int[16],
                                estimated_count,
                                0,
                                ...
                                &exp_mac[0]);
      break;
    case isrran::INTEGRITY_ALGORITHM_ID_128_EIA2:
      isrran::security_128_eia2(&m_sec_ctx.k_nas_int[16],
                                estimated_count,
                                0,
                                isrran::SECURITY_DIRECTION_UPLINK,
                                &pdu->msg[0],
                                ...
                                &exp_mac[0]);
      break;
    case isrran::INTEGRITY_ALGORITHM_ID_128_EIA3:
      isrran::security_128_eia3(&m_sec_ctx.k_nas_int[16],
                                estimated_count,
                                0,
                                isrran::SECURITY_DIRECTION_UPLINK,
                                &pdu->msg[0],
...
bool nas::integrity_check(isrran::byte_buffer_t* pdu, bool warn_failure)
{
  uint8_t        exp_mac[4] = {};
  ...
  for (int i = 0; i < 4; i++) {
    if (exp_mac[i] != mac[i]) {
      isrlog::log_channel& channel = warn_failure ? m_logger.warning : m_logger.info;
...
void nas::integrity_generate(isrran::byte_buffer_t* pdu, uint8_t* mac)
{
  switch (m_sec_ctx.integ_algo) {
    case isrran::INTEGRITY_ALGORITHM_ID_EIA0:
      break;
  ...
  m_logger.debug("Generating MAC with inputs: Algorithm %s, DL COUNT %d",
                 isrran::integrity_algorithm_id_text[m_sec_ctx.integ_algo],
                 m_sec_ctx.dl_nas_count);
}

void nas::cipher_decrypt(isrran::byte_buffer_t* pdu)
{
  isrran::byte_buffer_t tmp_pdu;
  switch (m_sec_ctx.cipher_algo) {
    case isrran::CIPHERING_ALGORITHM_ID_EEA0:
      break;
    case isrran::CIPHERING_ALGORITHM_ID_128_EEA1:
      isrran::security_128_eea1(&m_sec_ctx.k_nas_enc[16],
                                pdu->msg[5],
                                0, // Bearer always 0 for NAS
                                isrran::SECURITY_DIRECTION_UPLINK,
                                &pdu->msg[6],
      ...
      break;
    case isrran::CIPHERING_ALGORITHM_ID_128_EEA2:
      isrran::security_128_eea2(&m_sec_ctx.k_nas_enc[16],
                                pdu->msg[5],
                                0, // Bearer always 0 for NAS
                                isrran::SECURITY_DIRECTION_UPLINK,
                                &pdu->msg[6],
      ...
      memcpy(&pdu->msg[6], &tmp_pdu.msg[6], pdu->N_bytes - 6);
      break;
    case isrran::CIPHERING_ALGORITHM_ID_128_EEA3:
      isrran::security_128_eea3(&m_sec_ctx.k_nas_enc[16],
                                pdu->msg[5],
                                0, // Bearer always 0 for NAS
                                isrran::SECURITY_DIRECTION_UPLINK,
                                &pdu->msg[6],
...
void nas::cipher_encrypt(isrran::byte_buffer_t* pdu)
{
  isrran::byte_buffer_t pdu_tmp;
  switch (m_sec_ctx.cipher_algo) {
    case isrran::CIPHERING_ALGORITHM_ID_EEA0:
      break;
    case isrran::CIPHERING_ALGORITHM_ID_128_EEA1:
...
bool nas::expire_t3413()
{
  ...
  isrran::console("T3413 expired -- Could not page the ue.\n");
  if (m_emm_ctx.state != EMM_STATE_REGISTERED) {
  ...
  m_gtpc->send_downlink_data_notification_failure_indication(m_emm_ctx.imsi,
                                                             isrran::GTPC_CAUSE_VALUE_UE_NOT_RESPONDING);
  return true;
}
