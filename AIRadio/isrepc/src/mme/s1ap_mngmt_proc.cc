...
#include "isrepc/hdr/mme/s1ap_mngmt_proc.h"
...
{
  isrran::console("Received S1 Setup Request.\n");
  ...
  if (enb_ctx.plmn != m_s1ap->get_plmn()) {
    isrran::console("Sending S1 Setup Failure - Unknown PLMN\n");
    ...
    isrran::console("Sending S1 Setup Failure - No matching TAC\n");
    ...
    isrran::console("Sending S1 Setup Response\n");
...
bool s1ap_mngmt_proc::unpack_s1_setup_request(const asn1::s1ap::s1_setup_request_s& msg, enb_ctx_t* enb_ctx)
{
  ...
  isrran::s1ap_plmn_to_mccmnc(enb_ctx->plmn, &enb_ctx->mcc, &enb_ctx->mnc);
...
bool s1ap_mngmt_proc::send_s1_setup_response(const s1ap_args_t& s1ap_args, struct sctp_sndrcvinfo* enb_sri)
{
  ...
  uint32_t plmn = 0;
  isrran::s1ap_mccmnc_to_plmn(s1ap_args.mcc, s1ap_args.mnc, &plmn);
  plmn = htonl(plmn);
