...
#include "isrepc/hdr/mme/mme_gtpc.h"
...
bool mme_gtpc::init()
{
  ...
  isrran::console("MME GTP-C Initialized\n");
  return true;
}
...
bool mme_gtpc::init_s11()
{
  ...
  isrran::console("MME S11 Initialized\n");
  return true;
}

bool mme_gtpc::send_s11_pdu(const isrran::gtpc_pdu& pdu)
{
  int n;
  ...
  if (n < 0) {
    ...
    isrran::console("Error sending to socket. Error %s\n", strerror(errno));
    return false;
  } else {
...
void mme_gtpc::handle_s11_pdu(isrran::byte_buffer_t* msg)
{
  m_logger.debug("Received S11 message");

  isrran::gtpc_pdu* pdu;
  pdu = (isrran::gtpc_pdu*)msg->msg;
  m_logger.debug("MME Received GTP-C PDU. Message type %s", isrran::gtpc_msg_type_to_str(pdu->header.type));
  switch (pdu->header.type) {
    case isrran::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE:
      handle_create_session_response(pdu);
      break;
    case isrran::GTPC_MSG_TYPE_MODIFY_BEARER_RESPONSE:
      handle_modify_bearer_response(pdu);
      break;
    case isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION:
...
bool mme_gtpc::send_create_session_request(uint64_t imsi)
{
  ...
  isrran::console("Sending Create Session Request.\n");
  struct isrran::gtpc_pdu cs_req_pdu;
  // Initialize GTP-C message to zero
  std::memset(&cs_req_pdu, 0, sizeof(cs_req_pdu));

  struct isrran::gtpc_create_session_request* cs_req = &cs_req_pdu.choice.create_session_request;
  ...
  cs_req_pdu.header.type         = isrran::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST;
  ...
  isrran::console("Creating Session Response -- IMSI: %" PRIu64 "\n", imsi);
  isrran::console("Creating Session Response -- MME control TEID: %d\n", cs_req->sender_f_teid.teid);
  ...
  // RAT Type
  // cs_req->rat_type = isrran::GTPC_RAT_TYPE::EUTRAN;
...
bool mme_gtpc::handle_create_session_response(isrran::gtpc_pdu* cs_resp_pdu)
{
  struct isrran::gtpc_create_session_response* cs_resp = &cs_resp_pdu->choice.create_session_response;
  m_logger.info("Received Create Session Response");
  isrran::console("Received Create Session Response\n");
  if (cs_resp_pdu->header.type != isrran::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE) {
    m_logger.warning("Could not create GTPC session. Not a create session response");
    // TODO Handle error
    return false;
  }
  if (cs_resp->cause.cause_value != isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED) {
  ...
  isrran::gtp_fteid_t sgw_ctr_fteid = {};
  ...
  isrran::console("Create Session Response -- SPGW control TEID %d\n", sgw_ctr_fteid.teid);
  ...
  isrran::console("Create Session Response -- SPGW S1-U Address: %s\n", inet_ntoa(s1u_addr));
  ...
  if (cs_resp->paa.pdn_type != isrran::GTPC_PDN_TYPE_IPV4) {
  ...
  isrran::console("SPGW Allocated IP %s to IMSI %015" PRIu64 "\n", inet_ntoa(emm_ctx->ue_ip), emm_ctx->imsi);
...
bool mme_gtpc::send_modify_bearer_request(uint64_t imsi, uint16_t erab_to_modify, isrran::gtp_fteid_t* enb_fteid)
{
  m_logger.info("Sending GTP-C Modify bearer request");
  isrran::gtpc_pdu mb_req_pdu;
  ...
  isrran::gtp_fteid_t sgw_ctr_fteid = it->second.sgw_ctr_fteid;

  isrran::gtpc_header* header = &mb_req_pdu.header;
  header->teid_present        = true;
  header->teid                = sgw_ctr_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_MODIFY_BEARER_REQUEST;

  isrran::gtpc_modify_bearer_request* mb_req                = &mb_req_pdu.choice.modify_bearer_request;
...
void mme_gtpc::handle_modify_bearer_response(isrran::gtpc_pdu* mb_resp_pdu)
{
...
bool mme_gtpc::send_delete_session_request(uint64_t imsi)
{
  ...
  isrran::gtpc_pdu del_req_pdu;
  std::memset(&del_req_pdu, 0, sizeof(del_req_pdu));
  isrran::gtp_fteid_t sgw_ctr_fteid;
  isrran::gtp_fteid_t mme_ctr_fteid;
  ...
  isrran::gtpc_header* header = &del_req_pdu.header;
  header->teid_present        = true;
  header->teid                = sgw_ctr_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_DELETE_SESSION_REQUEST;

  isrran::gtpc_delete_session_request* del_req = &del_req_pdu.choice.delete_session_request;
  del_req->cause.cause_value                   = isrran::GTPC_CAUSE_VALUE_ISR_DEACTIVATION;
...
void mme_gtpc::send_release_access_bearers_request(uint64_t imsi)
{
  ...
  // Set GTP-C header
  isrran::gtpc_header* header = &rel_req_pdu.header;
  header->teid_present        = true;
  header->teid                = sgw_ctr_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_RELEASE_ACCESS_BEARERS_REQUEST;

  isrran::gtpc_release_access_bearers_request* rel_req = &rel_req_pdu.choice.release_access_bearers_request;
...
bool mme_gtpc::handle_downlink_data_notification(isrran::gtpc_pdu* dl_not_pdu)
{
  uint32_t                                 mme_ctrl_teid = dl_not_pdu->header.teid;
  isrran::gtpc_downlink_data_notification* dl_not        = &dl_not_pdu->choice.downlink_data_notification;
  std::map<uint32_t, uint64_t>::iterator   imsi_it       = m_mme_ctr_teid_to_imsi.find(mme_ctrl_teid);
  if (imsi_it == m_mme_ctr_teid_to_imsi.end()) {
    m_logger.error("Could not find IMSI from control TEID");
    return false;
...
void mme_gtpc::send_downlink_data_notification_acknowledge(uint64_t imsi, enum isrran::gtpc_cause_value cause)
{
  m_logger.debug("Sending GTP-C Data Notification Acknowledge. Cause %d", cause);
  isrran::gtpc_pdu    not_ack_pdu;
  isrran::gtp_fteid_t sgw_ctr_fteid;
  ...
  // set gtp-c header
  isrran::gtpc_header* header = &not_ack_pdu.header;
  header->teid_present        = true;
  header->teid                = sgw_ctr_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION_ACKNOWLEDGE;

  isrran::gtpc_downlink_data_notification_acknowledge* not_ack =
...
bool mme_gtpc::send_downlink_data_notification_failure_indication(uint64_t imsi, enum isrran::gtpc_cause_value cause)
{
  m_logger.debug("Sending GTP-C Data Notification Failure Indication. Cause %d", cause);
  isrran::gtpc_pdu    not_fail_pdu;
  isrran::gtp_fteid_t sgw_ctr_fteid;
  ...
  // set gtp-c header
  isrran::gtpc_header* header = &not_fail_pdu.header;
  header->teid_present        = true;
  header->teid                = sgw_ctr_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION_FAILURE_INDICATION;

  isrran::gtpc_downlink_data_notification_failure_indication* not_fail =
