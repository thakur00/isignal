...
int hss::init(hss_args_t* hss_args)
{
  ...
  if (read_db_file(hss_args->db_file) == false) {
    isrran::console("Error reading user database file %s\n", hss_args->db_file.c_str());
    return -1;
  }
  ...
  isrran::console("HSS Initialized.\n");
  return 0;
}
...
bool hss::read_db_file(std::string db_filename)
{
  ...
  while (std::getline(m_db_file, line)) {
    if (line[0] != '#' && line.length() > 0) {
      uint                     column_size = 10;
      std::vector<std::string> split       = isrran::split_string(line, ',');
      if (split.size() != column_size) {
        ...
        isrran::console("Perhaps you are using an old user_db.csv?\n");
        isrran::console("See 'isrepc/user_db.csv.example' for an example.\n\n");
        return false;
      }
      ...
      isrran::get_uint_vec_from_hex_str(split[3], ue_ctx->key, 16);
      if (split[4] == std::string("op")) {
        ue_ctx->op_configured = true;
        isrran::get_uint_vec_from_hex_str(split[5], ue_ctx->op, 16);
        isrran::compute_opc(ue_ctx->key, ue_ctx->op, ue_ctx->opc);
      } else if (split[4] == std::string("opc")) {
        ue_ctx->op_configured = false;
        isrran::get_uint_vec_from_hex_str(split[5], ue_ctx->opc, 16);
      } else {
        m_logger.error("Neither OP nor OPc configured.");
        return false;
      }
      isrran::get_uint_vec_from_hex_str(split[6], ue_ctx->amf, 2);
      isrran::get_uint_vec_from_hex_str(split[7], ue_ctx->sqn, 6);
...
bool hss::write_db_file(std::string db_filename)
{
  ...
  while (it != m_imsi_to_ue_ctx.end()) {
    ...
    m_db_file << ",";
    m_db_file << isrran::hex_string(it->second->key, 16);
    m_db_file << ",";
    if (it->second->op_configured) {
      m_db_file << "op,";
      m_db_file << isrran::hex_string(it->second->op, 16);
    } else {
      m_db_file << "opc,";
      m_db_file << isrran::hex_string(it->second->opc, 16);
    }
    m_db_file << ",";
    m_db_file << isrran::hex_string(it->second->amf, 2);
    m_db_file << ",";
    m_db_file << isrran::hex_string(it->second->sqn, 6);
    m_db_file << ",";
...
bool hss::gen_auth_info_answer(uint64_t imsi, uint8_t* k_asme, uint8_t* autn, uint8_t* rand, uint8_t* xres)
{
  ...
  if (ue_ctx == nullptr) {
    isrran::console("User not found at HSS. IMSI: %015" PRIu64 "\n", imsi);
                                        ...
                                        uint8_t*      xres)
{
  ...
  isrran::security_milenage_f2345(k, opc, rand, xres, ck, ik, ak);
  ...
  isrran::security_milenage_f1(k, opc, rand, sqn, amf, mac);
  ...
  // Generate K_asme
  isrran::security_generate_k_asme(ck, ik, ak_xor_sqn, mcc, mnc, k_asme);
...
void hss::resync_sqn_xor(hss_ue_ctx_t* ue_ctx, uint8_t* auts)
{
  ...
  isrran::console("XOR SQNs synchronization not supported yet\n");
  return;
}
...
void hss::resync_sqn_milenage(hss_ue_ctx_t* ue_ctx, uint8_t* auts)
{
  ...
  isrran::security_milenage_f5_star(k, opc, last_rand, ak);
  ...
  isrran::security_milenage_f1_star(k, opc, last_rand, sqn_ms, dummy_amf, mac_s_tmp);
