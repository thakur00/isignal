...
#include "isrepc/hdr/mbms-gw/mbms-gw.h"
...
int mbms_gw::init(mbms_gw_args_t* args)
{
  ...
  if (err != ISRRAN_SUCCESS) {
    isrran::console("Error initializing SGi-MB.\n");
  ...
  isrran::console("MBMS GW Initiated\n");
...
int mbms_gw::init_sgi_mb_if(mbms_gw_args_t* args)
{
  ...
  if (not isrran::net_utils::set_sockaddr(addr, args->sgi_mb_if_addr.c_str(), 0)) {
    m_logger.error("Invalid sgi_mb_if_addr: %s", args->sgi_mb_if_addr.c_str());
    isrran::console("Invalid sgi_mb_if_addr: %s\n", args->sgi_mb_if_addr.c_str());
    close(m_sgi_mb_if);
    close(sgi_mb_sock);
    return ISRRAN_ERROR_CANT_START;
  }
  ...
  if (inet_pton(ifr.ifr_netmask.sa_family, args->sgi_mb_if_mask.c_str(), &((struct sockaddr_in*)&ifr.ifr_netmask)->sin_addr.s_addr ) != 1) {
    ...
    isrran::console("Invalid sgi_mb_if_mask: %s\n", args->sgi_mb_if_mask.c_str());
    perror("inet_pton");
    return ISRRAN_ERROR_CANT_START;
  }
...
int mbms_gw::init_m1_u(mbms_gw_args_t* args)
{
  ...
  if (inet_pton(AF_INET, args->m1u_multi_if.c_str(), &local_if.s_addr) != 1) {
    ...
    isrran::console("Invalid m1u_multi_if: %s\n", args->m1u_multi_if.c_str());
    perror("inet_pton");
    return ISRRAN_ERROR_CANT_START;
  }
  ...
  if (inet_pton(m_m1u_multi_addr.sin_family, args->m1u_multi_addr.c_str(), &m_m1u_multi_addr.sin_addr.s_addr) != 1) {
    ...
    isrran::console("Invalid m1u_multi_addr: %s\n", args->m1u_multi_addr.c_str());
    perror("inet_pton");
    return ISRRAN_ERROR_CANT_START;
  }
...
void mbms_gw::run_thread()
{
  ...
  m_running                        = true;
  isrran::unique_byte_buffer_t msg = isrran::make_byte_buffer();
  if (msg == nullptr) {
  ...
  while (m_running) {
    ...
    int n;
    do {
      n = read(m_sgi_mb_if, msg->msg, ISRRAN_MAX_BUFFER_SIZE_BYTES);
...
void mbms_gw::handle_sgi_md_pdu(isrran::byte_buffer_t* msg)
{
  uint8_t               version;
  ...
  if (!isrran::gtpu_write_header(&header, msg, m_logger)) {
    isrran::console("Error writing GTP-U header on PDU\n");
  }

  int n = sendto(m_m1u, msg->msg, msg->N_bytes, 0, (sockaddr*)&m_m1u_multi_addr, sizeof(struct sockaddr));
  if (n < 0) {
    isrran::console("Error writing to M1-U socket.\n");
  } else {
