...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("mbms_gw.name",      bpo::value<string>(&mbms_gw_name)->default_value("isrmbmsgw01"), "MBMS-GW Name")
...
int main(int argc, char* argv[])
{
  ...
  isrlog::sink* log_sink = (args.log_args.filename == "stdout") ? isrlog::create_stdout_sink()
  ...
  if (args.log_args.filename != "stdout") {
    auto& mbms_gw_logger = isrlog::fetch_basic_logger("MBMS GW", false);
    mbms_gw_logger.info("\n---  Software Radio Systems MBMS log ---\n\n");
  }

  auto& mbms_gw_logger = isrlog::fetch_basic_logger("MBMS", false);
  mbms_gw_logger.set_level(isrlog::str_to_basic_level(args.log_args.mbms_gw_level));
