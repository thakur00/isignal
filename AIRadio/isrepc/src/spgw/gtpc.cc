...
#include "isrepc/hdr/spgw/gtpc.h"
#include <algorithm>
#include <arpa/inet.h>
#include <cstring>
#include <fcntl.h>
                     ...
                     const std::map<std::string, uint64_t>& ip_to_imsi)
{
  ...
  if (err != ISRRAN_SUCCESS) {
    isrran::console("Could not initialize the S11 interface.\n");
    return err;
  }
  ...
  if (err != ISRRAN_SUCCESS) {
    isrran::console("Could not initialize the IP pool.\n");
    return err;
  }
  ...
  isrran::console("SPGW S11 Initialized.\n");
  return 0;
}
...
void spgw::gtpc::stop()
{
  ...
  while (it != m_teid_to_tunnel_ctx.end()) {
    ...
    isrran::console("Deleting SP-GW GTP-C Tunnel. IMSI: %015" PRIu64 "\n", it->second->imsi);
    delete it->second;
...
int spgw::gtpc::init_s11(spgw_args_t* args)
{
  ...
  if (m_s11 < 0) {
    ...
    return ISRRAN_ERROR_CANT_START;
  }
...
bool spgw::gtpc::send_s11_pdu(const isrran::gtpc_pdu& pdu)
{
...
void spgw::gtpc::handle_s11_pdu(isrran::byte_buffer_t* msg)
{
  // TODO add deserialization code here
  isrran::gtpc_pdu* pdu = (isrran::gtpc_pdu*)msg->msg;
  isrran::console("Received GTP-C PDU. Message type: %s\n", isrran::gtpc_msg_type_to_str(pdu->header.type));
  m_logger.debug("Received GTP-C PDU. Message type: %s", isrran::gtpc_msg_type_to_str(pdu->header.type));
  switch (pdu->header.type) {
    case isrran::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST:
      handle_create_session_request(pdu->choice.create_session_request);
      break;
    case isrran::GTPC_MSG_TYPE_MODIFY_BEARER_REQUEST:
      handle_modify_bearer_request(pdu->header, pdu->choice.modify_bearer_request);
      break;
    case isrran::GTPC_MSG_TYPE_DELETE_SESSION_REQUEST:
      handle_delete_session_request(pdu->header, pdu->choice.delete_session_request);
      break;
    case isrran::GTPC_MSG_TYPE_RELEASE_ACCESS_BEARERS_REQUEST:
      handle_release_access_bearers_request(pdu->header, pdu->choice.release_access_bearers_request);
      break;
    case isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION_ACKNOWLEDGE:
      handle_downlink_data_notification_acknowledge(pdu->header, pdu->choice.downlink_data_notification_acknowledge);
      break;
    case isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION_FAILURE_INDICATION:
...
void spgw::gtpc::handle_create_session_request(const struct isrran::gtpc_create_session_request& cs_req)
{
  ...
  if (gtpc_present) {
    isrran::console("SPGW: GTP-C context for IMSI %015" PRIu64 " already exists.\n", cs_req.imsi);
    delete_gtpc_ctx(m_imsi_to_ctr_teid[cs_req.imsi]);
    isrran::console("SPGW: Deleted previous context.\n");
  }
  ...
  isrran::gtpc_pdu cs_resp_pdu;
  std::memset(&cs_resp_pdu, 0, sizeof(cs_resp_pdu));
  isrran::gtpc_header*                  header  = &cs_resp_pdu.header;
  isrran::gtpc_create_session_response* cs_resp = &cs_resp_pdu.choice.create_session_response;
  ...
  header->type         = isrran::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE;

  // Initialize to zero
  std::memset(cs_resp, 0, sizeof(struct isrran::gtpc_create_session_response));
  // Setup Cause
  cs_resp->cause.cause_value = isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED;
  ...
  cs_resp->eps_bearer_context_created.cause.cause_value       = isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED;
  ...
  cs_resp->paa.pdn_type     = isrran::GTPC_PDN_TYPE_IPV4;
...
void spgw::gtpc::handle_modify_bearer_request(const struct isrran::gtpc_header&                mb_req_hdr,
                                              const struct isrran::gtpc_modify_bearer_request& mb_req)
{
  ...
  if (tunnel_ctx->paging_pending == true) {
    ...
    isrran::console("Modify Bearer Request received after Downling Data Notification was sent\n");
    m_gtpu->send_all_queued_packets(tunnel_ctx->dw_user_fteid, tunnel_ctx->paging_queue);
  }

  // Setting up Modify bearer response PDU
  // Header
  isrran::gtpc_pdu mb_resp_pdu;
  std::memset(&mb_resp_pdu, 0, sizeof(mb_resp_pdu));
  isrran::gtpc_header* header = &mb_resp_pdu.header;
  header->piggyback           = false;
  header->teid_present        = true;
  header->teid                = tunnel_ctx->dw_ctrl_fteid.teid;
  header->type                = isrran::GTPC_MSG_TYPE_MODIFY_BEARER_RESPONSE;

  // PDU
  isrran::gtpc_modify_bearer_response* mb_resp           = &mb_resp_pdu.choice.modify_bearer_response;
  mb_resp->cause.cause_value                             = isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED;
  mb_resp->eps_bearer_context_modified.ebi               = tunnel_ctx->ebi;
  mb_resp->eps_bearer_context_modified.cause.cause_value = isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED;
...
void spgw::gtpc::handle_delete_session_request(const isrran::gtpc_header&                 header,
                                               const isrran::gtpc_delete_session_request& del_req_pdu)
{
...
void spgw::gtpc::handle_release_access_bearers_request(const isrran::gtpc_header&                         header,
...
bool spgw::gtpc::send_downlink_data_notification(uint32_t spgw_ctr_teid)
{
  ...
  struct isrran::gtpc_pdu dl_not_pdu;
  std::memset(&dl_not_pdu, 0, sizeof(dl_not_pdu));
  struct isrran::gtpc_header*                     header = &dl_not_pdu.header;
  struct isrran::gtpc_downlink_data_notification* dl_not = &dl_not_pdu.choice.downlink_data_notification;
  ...
  isrran::console("Found UE for Downlink Notification \n");
  isrran::console("MME Ctr TEID 0x%x, IMSI: %015" PRIu64 "\n", tunnel_ctx->dw_ctrl_fteid.teid, tunnel_ctx->imsi);

  // Setup GTP-C header
  header->piggyback    = false;
  header->teid_present = true;
  header->teid         = tunnel_ctx->dw_ctrl_fteid.teid; // Send downlink data notification to the UE's MME Ctrl TEID
  header->type         = isrran::GTPC_MSG_TYPE_DOWNLINK_DATA_NOTIFICATION;
    ...
    const isrran::gtpc_downlink_data_notification_acknowledge& not_ack)
{
  ...
  spgw_tunnel_ctx_t* tunnel_ctx = tunnel_it->second;
  if (not_ack.cause.cause_value == isrran::GTPC_CAUSE_VALUE_CONTEXT_NOT_FOUND ||
      not_ack.cause.cause_value == isrran::GTPC_CAUSE_VALUE_UE_ALREADY_RE_ATTACHED ||
      not_ack.cause.cause_value == isrran::GTPC_CAUSE_VALUE_UNABLE_TO_PAGE_UE ||
      not_ack.cause.cause_value == isrran::GTPC_CAUSE_VALUE_UNABLE_TO_PAGE_UE_DUE_TO_SUSPENSION) {
    m_logger.warning("Downlink Data Notification Acknowledge indicates failure.");
    free_all_queued_packets(tunnel_ctx);
    tunnel_ctx->paging_pending = false;
  } else if (not_ack.cause.cause_value != isrran::GTPC_CAUSE_VALUE_REQUEST_ACCEPTED) {
    ...
    const isrran::gtpc_downlink_data_notification_failure_indication& not_fail)
{
  ...
  spgw_tunnel_ctx_t* tunnel_ctx = tunnel_it->second;
  if (not_fail.cause.cause_value == isrran::GTPC_CAUSE_VALUE_UE_NOT_RESPONDING ||
      not_fail.cause.cause_value == isrran::GTPC_CAUSE_VALUE_SERVICE_DENIED ||
      not_fail.cause.cause_value == isrran::GTPC_CAUSE_VALUE_UE_ALREADY_RE_ATTACHED) {
 ...
 */
spgw_tunnel_ctx_t* spgw::gtpc::create_gtpc_ctx(const struct isrran::gtpc_create_session_request& cs_req)
{
  ...
  isrran::console("SPGW: Allocated Ctrl TEID %" PRIu64 "\n", spgw_uplink_ctrl_teid);
  isrran::console("SPGW: Allocated User TEID %" PRIu64 "\n", spgw_uplink_user_teid);
  struct in_addr ue_ip_;
  ue_ip_.s_addr = ue_ip;
  isrran::console("SPGW: Allocate UE IP %s\n", inet_ntoa(ue_ip_));
  ...
  std::memset(&tunnel_ctx->dw_user_fteid, 0, sizeof(isrran::gtp_fteid_t));
 ...
 * Queueing functions
 */
bool spgw::gtpc::queue_downlink_packet(uint32_t ctrl_teid, isrran::unique_byte_buffer_t msg)
{
  spgw_tunnel_ctx_t* tunnel_ctx;
  if (!m_teid_to_tunnel_ctx.count(ctrl_teid)) {
...
bool spgw::gtpc::free_all_queued_packets(spgw_tunnel_ctx_t* tunnel_ctx)
{
  ...
  while (!tunnel_ctx->paging_queue.empty()) {
    isrran::unique_byte_buffer_t pkt = std::move(tunnel_ctx->paging_queue.front());
...
int spgw::gtpc::init_ue_ip(spgw_args_t* args, const std::map<std::string, uint64_t>& ip_to_imsi)
{
  ...
  for (uint32_t n = 1; n < 254; ++n) {
    ...
    if (inet_pton(AF_INET, args->sgi_if_addr.c_str(), &ue_addr.s_addr) != 1) {
      ...
      isrran::console("Invalid sgi_if_addr: %s\n", args->sgi_if_addr.c_str());
      perror("inet_pton");
      return ISRRAN_ERROR;
    }
