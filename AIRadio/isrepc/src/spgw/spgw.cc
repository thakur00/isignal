...
int spgw::init(spgw_args_t* args, const std::map<std::string, uint64_t>& ip_to_imsi)
{
  ...
  // Init GTP-U
  if (m_gtpu->init(args, this, m_gtpc) != ISRRAN_SUCCESS) {
    isrran::console("Could not initialize the SPGW's GTP-U.\n");
    return ISRRAN_ERROR_CANT_START;
  }

  // Init GTP-C
  if (m_gtpc->init(args, this, m_gtpu, ip_to_imsi) != ISRRAN_SUCCESS) {
    isrran::console("Could not initialize the S1-U interface.\n");
    return ISRRAN_ERROR_CANT_START;
  }

  m_logger.info("SP-GW Initialized.");
  isrran::console("SP-GW Initialized.\n");
...
void spgw::run_thread()
{
  ...
  m_running = true;
  isrran::unique_byte_buffer_t sgi_msg, s1u_msg, s11_msg;
  s1u_msg = isrran::make_byte_buffer("spgw::run_thread::s1u");
  s11_msg = isrran::make_byte_buffer("spgw::run_thread::s11");
  ...
  size_t buf_len = ISRRAN_MAX_BUFFER_SIZE_BYTES - ISRRAN_BUFFER_HEADER_OFFSET;
  ...
  while (m_running) {
    ...
    if (n == -1) {
      ...
      if (FD_ISSET(sgi, &set)) {
        ...
        sgi_msg          = isrran::make_byte_buffer("spgw::run_thread::sgi_msg");
