 ...
 *
 */
#ifndef ISREPC_MME_GTPC_H
#define ISREPC_MME_GTPC_H
...
public:
  ...
  bool         handle_create_session_response(isrran::gtpc_pdu* cs_resp_pdu);
  virtual bool send_modify_bearer_request(uint64_t imsi, uint16_t erab_to_modify, isrran::gtp_fteid_t* enb_fteid);
  void         handle_modify_bearer_response(isrran::gtpc_pdu* mb_resp_pdu);
  void         send_release_access_bearers_request(uint64_t imsi);
  virtual bool send_delete_session_request(uint64_t imsi);
  bool         handle_downlink_data_notification(isrran::gtpc_pdu* dl_not_pdu);
  void         send_downlink_data_notification_acknowledge(uint64_t imsi, enum isrran::gtpc_cause_value cause);
  virtual bool send_downlink_data_notification_failure_indication(uint64_t imsi, enum isrran::gtpc_cause_value cause);
...
private:
  ...
  isrlog::basic_logger& m_logger = isrlog::fetch_basic_logger("MME GTPC");
  s1ap*                 m_s1ap;
