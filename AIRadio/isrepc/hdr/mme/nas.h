...
#include "isrran/asn1/gtpc_ies.h"
...
typedef struct {
  ...
  isrran::gtpc_f_teid_ie sgw_ctrl_fteid;
} emm_ctx_t;
...
typedef struct {
  ...
  esm_state_t                            state;
  uint8_t                                qci;
  isrran::gtpc_f_teid_ie                 enb_fteid;
  isrran::gtpc_f_teid_ie                 sgw_s1u_fteid;
  isrran::gtpc_pdn_address_allocation_ie pdn_addr_alloc;
} esm_ctx_t;
...
public:
  ...
  bool handle_attach_request(isrran::byte_buffer_t* nas_rx);
  bool handle_pdn_connectivity_request(isrran::byte_buffer_t* nas_rx);
  bool handle_authentication_response(isrran::byte_buffer_t* nas_rx);
  bool handle_security_mode_complete(isrran::byte_buffer_t* nas_rx);
  bool handle_attach_complete(isrran::byte_buffer_t* nas_rx);
  bool handle_esm_information_response(isrran::byte_buffer_t* nas_rx);
  bool handle_identity_response(isrran::byte_buffer_t* nas_rx);
  bool handle_tracking_area_update_request(isrran::byte_buffer_t* nas_rx);
  bool handle_authentication_failure(isrran::byte_buffer_t* nas_rx);
  bool handle_detach_request(isrran::byte_buffer_t* nas_rx);

  /* Downlink NAS messages packing */
  bool pack_authentication_request(isrran::byte_buffer_t* nas_buffer);
  bool pack_authentication_reject(isrran::byte_buffer_t* nas_buffer);
  bool pack_security_mode_command(isrran::byte_buffer_t* nas_buffer);
  bool pack_esm_information_request(isrran::byte_buffer_t* nas_buffer);
  bool pack_identity_request(isrran::byte_buffer_t* nas_buffer);
  bool pack_emm_information(isrran::byte_buffer_t* nas_buffer);
  bool pack_service_reject(isrran::byte_buffer_t* nas_buffer, uint8_t emm_cause);
  bool pack_tracking_area_update_reject(isrran::byte_buffer_t* nas_buffer, uint8_t emm_cause);
  bool pack_attach_accept(isrran::byte_buffer_t* nas_buffer);

  /* Security functions */
  bool integrity_check(isrran::byte_buffer_t* pdu, bool warn_failure = true);
  bool short_integrity_check(isrran::byte_buffer_t* pdu);
  void integrity_generate(isrran::byte_buffer_t* pdu, uint8_t* mac);
...
private:
  isrlog::basic_logger& m_logger = isrlog::fetch_basic_logger("NAS");
