...
#ifndef ISRENB_X2_ADAPTER_H
#define ISRENB_X2_ADAPTER_H
...
public:
  x2_adapter(enb_stack_lte* eutra_stack_, gnb_stack_nr* nr_stack_) :
    nr_stack(nr_stack_), eutra_stack(eutra_stack_), logger(isrlog::fetch_basic_logger("X2"))
  {}
  ...
  // gtpu_interface_pdcp
  void write_pdu(uint16_t rnti, uint32_t bearer_id, isrran::unique_byte_buffer_t pdu) override
  {
    if (eutra_stack == nullptr) {
      return;
    }
