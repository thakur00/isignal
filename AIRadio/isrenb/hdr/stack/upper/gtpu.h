...
#ifndef ISRENB_GTPU_H
#define ISRENB_GTPU_H
...
class gtpu_tunnel_manager
{
  ...
  using buffered_sdu_list = isrran::bounded_vector<std::pair<uint32_t, isrran::unique_byte_buffer_t>, BUFFER_SIZE>;
...
public:
  ...
  struct tunnel {
    ...
    uint32_t eps_bearer_id = isrran::INVALID_EPS_BEARER_ID;
    uint32_t teid_in       = 0;
    uint32_t teid_out      = 0;
    uint32_t spgw_addr     = 0;

    tunnel_state                                    state = tunnel_state::pdcp_active;
    isrran::unique_timer                            rx_timer;
    isrran::byte_buffer_pool_ptr<buffered_sdu_list> buffer;
    tunnel*                                         fwd_tunnel = nullptr; ///< forward Rx SDUs to this TEID
    isrran::move_callback<void()>                   on_removal;
  ...
  using ue_bearer_tunnel_list = isrran::bounded_vector<bearer_teid_pair, MAX_TUNNELS_PER_UE>;

  explicit gtpu_tunnel_manager(isrran::task_sched_handle task_sched_, isrlog::basic_logger& logger);
  ...
  isrran::span<bearer_teid_pair> find_rnti_bearer_tunnels(uint16_t rnti, uint32_t eps_bearer_id);
  ...
  void buffer_pdcp_sdu(uint32_t teid, uint32_t pdcp_sn, isrran::unique_byte_buffer_t sdu);
...
private:
  using tunnel_list_t  = isrran::static_id_obj_pool<uint32_t, tunnel, ISRENB_MAX_UES * MAX_TUNNELS_PER_UE>;
...
{
public:
  explicit gtpu(isrran::task_sched_handle   task_sched_,
                isrlog::basic_logger&       logger,
  ...
  // gtpu_interface_rrc
  isrran::expected<uint32_t> add_bearer(uint16_t            rnti,
                                        uint32_t            eps_bearer_id,
                                        uint32_t            addr,
                                        uint32_t            teid_out,
                                        uint32_t&           addr_in,
  ...
  // gtpu_interface_pdcp
  void write_pdu(uint16_t rnti, uint32_t eps_bearer_id, isrran::unique_byte_buffer_t pdu) override;

  // stack interface
  void handle_gtpu_s1u_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr);
  void handle_gtpu_m1u_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr);
...
private:
  ...
  isrran::socket_manager_itf* rx_socket_handler = nullptr;
  isrran::task_queue_handle   gtpu_queue;

  gtpu_args_t                  args;
  std::string                  gtp_bind_addr;
  std::string                  mme_addr;
  isrenb::pdcp_interface_gtpu* pdcp = nullptr;
  isrlog::basic_logger&        logger;
  isrran::task_sched_handle    task_sched;
  ...
  public:
    ...
    void         handle_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr);
  ...
  void send_pdu_to_tunnel(const gtpu_tunnel& tx_tun, isrran::unique_byte_buffer_t pdu, int pdcp_sn = -1);
  ...
  void handle_msg_data_pdu(const isrran::gtpu_header_t& header,
   ...
   ***************************************************************************/
  void log_message(const gtpu_tunnel& tun, bool is_rx, isrran::span<uint8_t> pdu, int pdcp_sn = -1);
};
