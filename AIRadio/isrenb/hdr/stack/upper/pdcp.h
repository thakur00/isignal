...
#ifndef ISRENB_PDCP_H
#define ISRENB_PDCP_H
...
{
public:
  pdcp(isrran::task_sched_handle task_sched_, isrlog::basic_logger& logger);
  virtual ~pdcp() {}
  void init(rlc_interface_pdcp* rlc_, rrc_interface_pdcp* rrc_, gtpu_interface_pdcp* gtpu_);
  void stop();

  // pdcp_interface_rlc
  void write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu) override;
  void notify_delivery(uint16_t rnti, uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sn) override;
  void notify_failure(uint16_t rnti, uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sn) override;
  ...
  void config_security(uint16_t rnti, uint32_t lcid, const isrran::as_security_config_t& cfg_sec) override;
  void enable_integrity(uint16_t rnti, uint32_t lcid) override;
  void enable_encryption(uint16_t rnti, uint32_t lcid) override;
  bool get_bearer_state(uint16_t rnti, uint32_t lcid, isrran::pdcp_lte_state_t* state) override;
...
private:
  class user_interface_rlc : public isrue::rlc_interface_pdcp
  {
  public:
    uint16_t                    rnti;
    isrenb::rlc_interface_pdcp* rlc;
    // rlc_interface_pdcp
  ...
  class user_interface_gtpu : public isrue::gw_interface_pdcp
  {
  public:
    uint16_t                     rnti;
    isrenb::gtpu_interface_pdcp* gtpu;
    // gw_interface_pdcp
    void write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
    void write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t sdu) {}
  };

  class user_interface_rrc : public isrue::rrc_interface_pdcp
  {
  public:
    uint16_t                    rnti;
    isrenb::rrc_interface_pdcp* rrc;
    // rrc_interface_pdcp
    void        write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  ...
  public:
    ...
    unique_rnti_ptr<isrran::pdcp> pdcp;
  };
