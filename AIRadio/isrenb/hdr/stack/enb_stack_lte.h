...
#ifndef ISRRAN_ENB_STACK_LTE_H
#define ISRRAN_ENB_STACK_LTE_H
...
{
public:
  enb_stack_lte(isrlog::sink& log_sink);
  ~enb_stack_lte() final;
  ...
  // gtpu_interface_pdcp
  void write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t pdu);
...
private:
  ...
  isrlog::basic_logger& s1ap_logger;
  isrlog::basic_logger& gtpu_logger;
  isrlog::basic_logger& stack_logger;

  // PCAP and trace option
  isrran::mac_pcap     mac_pcap;
  isrran::mac_pcap_net mac_pcap_net;
  isrran::s1ap_pcap    s1ap_pcap;

  // task handling
  isrran::task_scheduler    task_sched;
  isrran::task_queue_handle enb_task_queue, sync_task_queue, metrics_task_queue, x2_task_queue;
  ...
  isrran::dyn_blocking_queue<stack_metrics_t> pending_stack_metrics;
};

} // namespace isrenb

#endif // ISRRAN_ENB_STACK_LTE_H
