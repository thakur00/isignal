...
#ifndef ISRENB_S1AP_H
#define ISRENB_S1AP_H
...
#include "isrran/common/s1ap_pcap.h"
...
public:
  using erab_id_list   = isrran::bounded_vector<uint16_t, ASN1_S1AP_MAXNOOF_ERABS>;
  using erab_item_list = isrran::bounded_vector<asn1::s1ap::erab_item_s, ASN1_S1AP_MAXNOOF_ERABS>;

  s1ap(isrran::task_sched_handle   task_sched_,
  ...
  void initial_ue(uint16_t                              rnti,
                  ...
                  isrran::unique_byte_buffer_t          pdu) override;
  void initial_ue(uint16_t                              rnti,
                  uint32_t                              enb_cc_idx,
                  asn1::s1ap::rrc_establishment_cause_e cause,
  ...
  void write_pdu(uint16_t rnti, isrran::unique_byte_buffer_t pdu) override;
  ...
  bool send_ho_req_ack(const asn1::s1ap::ho_request_s&                msg,
                       ...
                       isrran::const_span<asn1::s1ap::erab_item_s>    not_admitted_bearers) override;
  ...
  bool send_error_indication(const asn1::s1ap::cause_c& cause,
                             ...
                             isrran::optional<uint32_t> mme_ue_s1ap_id = {});
  bool send_ue_cap_info_indication(uint16_t rnti, isrran::unique_byte_buffer_t ue_radio_cap) override;
  ...
  // Stack interface
  bool
  handle_mme_rx_msg(isrran::unique_byte_buffer_t pdu, const sockaddr_in& from, const sctp_sndrcvinfo& sri, int flags);
  void start_pcap(isrran::s1ap_pcap* pcap_);
...
private:
  ...
  s1ap_args_t                 args;
  isrlog::basic_logger&       logger;
  isrlog::log_channel&        alarms_channel;
  isrran::task_sched_handle   task_sched;
  isrran::task_queue_handle   mme_task_queue;
  isrran::socket_manager_itf* rx_socket_handler;

  isrran::unique_socket mme_socket;
  ...
  isrran::unique_timer  mme_connect_timer, s1setup_timeout;
  ...
  bool handle_s1ap_rx_pdu(isrran::byte_buffer_t* pdu);
  ...
  struct ue {
    ...
    public:
      ...
      isrran::proc_outcome_t init(uint32_t                     target_eci_,
      ...
      isrran::proc_outcome_t react(ts1_reloc_prep_expired e);
      isrran::proc_outcome_t react(const asn1::s1ap::ho_prep_fail_s& msg);
      isrran::proc_outcome_t react(const asn1::s1ap::ho_cmd_s& msg);
    ...
    bool send_ulnastransport(isrran::unique_byte_buffer_t pdu);
    ...
    bool send_ue_cap_info_indication(isrran::unique_byte_buffer_t ue_radio_cap);
  ...
  private:
    ...
    isrran::unique_timer ts1_reloc_prep;    ///< TS1_{RELOCprep} - max time for HO preparation
    isrran::unique_timer ts1_reloc_overall; ///< TS1_{RELOCOverall}
    isrran::unique_timer overall_procedure_timeout;

    // Procedure state
    s1ap_proc_id_t                                                           current_state = s1ap_proc_id_t::nulltype;
    erab_id_list                                                             updated_erabs;
    isrran::bounded_vector<asn1::s1ap::erab_item_s, ASN1_S1AP_MAXNOOF_ERABS> failed_cfg_erabs;

  public:
    // user procedures
    isrran::proc_t<ho_prep_proc_t> ho_prep_proc;
  };
  ...
  public:
    ...
    isrran::proc_outcome_t step() { return isrran::proc_outcome_t::yield; }
    isrran::proc_outcome_t react(const s1connectresult& event);
    isrran::proc_outcome_t react(const s1setupresult& event);
    void                   then(const isrran::proc_state_t& result);
    const char*            name() const { return "MME Connection"; }
    uint16_t               connect_count = 0;

  private:
    isrran::proc_outcome_t start_mme_connection();
  ...
  void        log_s1ap_msg(const asn1::s1ap::s1ap_pdu_c& msg, isrran::const_span<uint8_t> sdu, bool is_rx);

  isrran::proc_t<s1_setup_proc_t> s1setup_proc;
};
