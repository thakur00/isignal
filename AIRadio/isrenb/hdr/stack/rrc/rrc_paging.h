...
#ifndef ISRRAN_RRC_PAGING_H
#define ISRRAN_RRC_PAGING_H
...
public:
  ...
  bool add_imsi_paging(uint32_t ueid, isrran::const_byte_span imsi);

  /// add new TMSI paging record
  bool add_tmsi_paging(uint32_t ueid, uint8_t mmec, isrran::const_byte_span m_tmsi);
...
private:
  ...
  struct subframe_info {
    ...
    isrran::deque<pcch_info*> transmitted_pcch;
...
bool paging_manager::add_imsi_paging(uint32_t ueid, isrran::const_byte_span imsi)
{
...
bool paging_manager::add_tmsi_paging(uint32_t ueid, uint8_t mmec, isrran::const_byte_span m_tmsi)
{
  asn1::rrc::paging_record_s paging_elem;
...
bool paging_manager::add_paging_record(uint32_t ueid, const asn1::rrc::paging_record_s& paging_record)
{
  ...
  if (pending_pcch.pdu == nullptr) {
    pending_pcch.pdu = isrran::make_byte_buffer();
    if (pending_pcch.pdu == nullptr) {
  ...
  if (pending_pcch.pcch_msg.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
