...
#ifndef ISRENB_RRC_MOBILITY_H
#define ISRENB_RRC_MOBILITY_H
...
class rrc::ue::rrc_mobility : public isrran::fsm_t<rrc::ue::rrc_mobility>
{
public:
  // public events
  ...
  rrc_mobility(isrenb::rrc::ue* outer_ue);

  bool fill_conn_recfg_no_ho_cmd(asn1::rrc::rrc_conn_recfg_r8_ies_s* conn_recfg);
  void handle_ue_meas_report(const asn1::rrc::meas_report_s& msg, isrran::unique_byte_buffer_t pdu);
...
private:
  ...
  struct intraenb_ho_st {
    ...
    uint16_t               last_temp_crnti = ISRRAN_INVALID_RNTI;
  ...
  protected:
    ...
    //      +-------------------+------------------+---------------------+-----------------------+---------------------+
    to_state<                     idle_st,            isrran::failure_ev                                               >,
