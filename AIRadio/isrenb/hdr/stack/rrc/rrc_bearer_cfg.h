...
#ifndef ISRRAN_RRC_BEARER_CFG_H
#define ISRRAN_RRC_BEARER_CFG_H
...
{
public:
  explicit security_cfg_handler(const rrc_cfg_t& cfg_) : cfg(&cfg_), logger(isrlog::fetch_basic_logger("RRC")) {}
  explicit security_cfg_handler(const security_cfg_handler& other) : logger(isrlog::fetch_basic_logger("RRC"))
  {
    cfg                   = other.cfg;
  ...
  const isrran::as_security_config_t& get_as_sec_cfg() const { return sec_cfg; }
...
public:
  ...
  isrran::expected<uint32_t> add_gtpu_bearer(uint32_t                                erab_id,
                                             uint32_t                                teid_out,
                                             uint32_t                                addr,
