...
#include "isrran/common/timeout.h"
...
{
public:
  explicit rrc(isrran::task_sched_handle task_sched_, enb_bearer_manager& manager_);
  ~rrc();
  ...
  void     ho_preparation_complete(uint16_t                     rnti,
                                   ...
                                   isrran::unique_byte_buffer_t rrc_container) override;
  ...
  int notify_ue_erab_updates(uint16_t rnti, isrran::const_byte_span nas_pdu) override;
  ...
  template <class T>
  void log_broadcast_rrc_message(uint16_t rnti, isrran::const_byte_span pdu, const T& msg, const char* msg_type)
  {
...
private:
  ...
  void config_mac();
  void parse_ul_dcch(ue& ue, uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  void parse_ul_ccch(ue& ue, isrran::unique_byte_buffer_t pdu);
  ...
  void log_rx_pdu_fail(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu, const char* cause);
  void
  log_rxtx_pdu_impl(direction_t dir, uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu, const char* msg_type);
  ...
  bool                                running = false;
  isrran::dyn_blocking_queue<rrc_pdu> rx_pdu_queue;
