...
#ifndef ISRRAN_RRC_UE_H
#define ISRRAN_RRC_UE_H

#include "mac_controller.h"
#include "rrc.h"
#include "isrran/adt/pool/batch_mem_pool.h"
...
public:
  ...
  void send_connection_reconf(isrran::unique_byte_buffer_t sdu             = {},
  ...
  void parse_ul_dcch(uint32_t lcid, isrran::unique_byte_buffer_t pdu);
  ...
  void handle_rrc_con_setup_complete(asn1::rrc::rrc_conn_setup_complete_s* msg, isrran::unique_byte_buffer_t pdu);
  void handle_rrc_con_reest_req(asn1::rrc::rrc_conn_reest_request_s* msg);
  void handle_rrc_con_reest_complete(asn1::rrc::rrc_conn_reest_complete_s* msg, isrran::unique_byte_buffer_t pdu);
  void handle_rrc_reconf_complete(asn1::rrc::rrc_conn_recfg_complete_s* msg, isrran::unique_byte_buffer_t pdu);
  ...
  void handle_ue_info_resp(const asn1::rrc::ue_info_resp_r9_s& msg, isrran::unique_byte_buffer_t pdu);
  ...
  bool send_dl_dcch(const asn1::rrc::dl_dcch_msg_s* dl_dcch_msg,
                    isrran::unique_byte_buffer_t    pdu       = isrran::unique_byte_buffer_t(),
                    std::string*                    octet_str = nullptr);

  void save_ul_message(isrran::unique_byte_buffer_t pdu) { last_ul_msg = std::move(pdu); }
...
private:
  isrran::unique_timer activity_timer; // for basic DL/UL activity timeout

  /// Radio link failure handling uses distinct timers for PHY (DL and UL) and RLC signaled RLF
  isrran::unique_timer phy_dl_rlf_timer; // can be stopped through recovered DL activity
  isrran::unique_timer phy_ul_rlf_timer; // can be stopped through recovered UL activity
  isrran::unique_timer rlc_rlf_timer;    // can only be stoped through UE reestablishment
  ...
  std::map<uint16_t, isrran::pdcp_lte_state_t> old_reest_pdcp_state = {};
  ...
  isrran::rrc_ue_capabilities_t              ue_capabilities;
  ...
  isrran::unique_byte_buffer_t last_ul_msg;
