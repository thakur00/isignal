...
#ifndef ISRENB_SCHEDULER_HARQ_H
#define ISRENB_SCHEDULER_HARQ_H
...
public:
  ...
  isrran::tti_point get_tti() const;
  bool              get_ndi(uint32_t tb_idx) const;
  uint32_t          max_nof_retx() const;
  int               get_mcs(uint32_t tb_idx) const { return last_mcs[tb_idx]; }

protected:
  void new_tx_common(uint32_t tb_idx, isrran::tti_point tti, int mcs, int tbs, uint32_t max_retx_);
  void new_retx_common(uint32_t tb_idx, isrran::tti_point tti, int* mcs, int* tbs);
  ...
  isrlog::basic_logger*               logger    = nullptr;
  std::array<bool, ISRRAN_MAX_TB>     ack_state = {};
  std::array<bool, ISRRAN_MAX_TB>     active    = {};
  std::array<bool, ISRRAN_MAX_TB>     ndi       = {};
  uint32_t                            id        = 0;
  uint32_t                            max_retx  = 5;
  std::array<uint32_t, ISRRAN_MAX_TB> n_rtx     = {};
  std::array<uint32_t, ISRRAN_MAX_TB> tx_cnt    = {};
  std::array<int, ISRRAN_MAX_TB>      last_mcs  = {};
  std::array<int, ISRRAN_MAX_TB>      last_tbs  = {};
  isrran::tti_point                   tti;
};
...
public:
  ...
  void new_tx(isrran::tti_point tti, int mcs, int tbs, prb_interval alloc, uint32_t max_retx_, bool is_msg3);
  void new_retx(isrran::tti_point tti_, int* mcs, int* tbs, prb_interval alloc);
  bool set_ack(uint32_t tb_idx, bool ack);
  bool retx_requires_pdcch(isrran::tti_point tti_, prb_interval alloc) const;
...
public:
   ...
   */
  int set_ul_crc(isrran::tti_point tti_tx_ul, uint32_t tb_idx, bool ack_);

  //! Resets pending harq ACKs and cleans UL Harqs with maxretx == 0
  void finish_tti(isrran::tti_point tti_rx);

private:
  dl_harq_proc* get_oldest_dl_harq(tti_point tti_tx_dl);

  std::array<tti_point, ISRRAN_FDD_NOF_HARQ> last_ttis;
