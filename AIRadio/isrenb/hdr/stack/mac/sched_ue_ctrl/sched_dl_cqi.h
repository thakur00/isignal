...
#ifndef ISRRAN_SCHED_DL_CQI_H
#define ISRRAN_SCHED_DL_CQI_H
...
public:
  sched_dl_cqi(uint32_t cell_nof_prb_, uint32_t K_, uint32_t init_dl_cqi) :
    ...
    subband_cqi(std::max(1, isrran_cqi_hl_get_no_subbands(cell_nof_prb)), 0)
  {
    isrran_assert(K <= 4, "K=%d outside of {0, 4}", K);
    isrran_assert(K == 0 or cell_nof_prb_ > 6, "K > 0 not allowed for nof_prbs=6");
  }
  ...
  {
    isrran_assert(K <= 4, "K=%d outside of {0, 4}", K);
    isrran_assert(K == 0 or cell_nof_prb > 6, "K > 0 not allowed for nof_prbs=6");
    K = K_;
  }
  ...
  void cqi_sb_info(tti_point tti, uint32_t sb_index, uint32_t cqi_value)
  {
    ...
    isrran::interval<uint32_t> interv = get_bp_sb_indexes(bp_idx);
...
private:
  ...
  static uint32_t nof_bandwidth_parts(uint32_t nof_prb)
  {
    ...
    uint32_t              index    = isrran::lte_cell_nof_prb_to_index(nof_prb);

    isrran_assert(index < nrb_size, "nrb index out of bounds");
  ...
  isrran::interval<uint32_t> get_bp_sb_indexes(uint32_t bp_idx) const
  {
    return isrran::interval<uint32_t>{bp_idx * N() / J(), (bp_idx + 1) * N() / J()};
  }
  ...
  isrran::bounded_vector<bandwidth_part_context, max_bandwidth_parts> bp_list;
  isrran::bounded_vector<float, max_nof_subbands>                     subband_cqi;
};
