...
#ifndef ISRENB_SCHEDULER_H
#define ISRENB_SCHEDULER_H
...
public:
  ...
  std::array<int, ISRRAN_MAX_CARRIERS> get_enb_ue_cc_map(uint16_t rnti) final;
  std::array<int, ISRRAN_MAX_CARRIERS> get_enb_ue_activ_cc_map(uint16_t rnti) final;
...
protected:
  ...
  bool is_generated(isrran::tti_point, uint32_t enb_cc_idx) const;
  // Helper methods
