...
#ifndef ISRRAN_SCHED_HELPERS_H
#define ISRRAN_SCHED_HELPERS_H
 ...
 */
void generate_cce_location(isrran_regs_t*          regs,
...
{
  int tbs_idx = isrran_ra_tbs_idx_from_mcs(mcs, use_tbs_index_alt, is_ul);
  assert(tbs_idx != ISRRAN_ERROR);

  int tbs = isrran_ra_tbs_from_idx((uint32_t)tbs_idx, nof_alloc_prb);
  assert(tbs != ISRRAN_ERROR);
...
void log_dl_cc_results(isrlog::basic_logger&                  logger,
                       uint32_t                               enb_cc_idx,
