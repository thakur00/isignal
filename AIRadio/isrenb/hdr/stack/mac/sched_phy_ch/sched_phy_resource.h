...
#ifndef ISRRAN_SCHED_PHY_RESOURCE_H
#define ISRRAN_SCHED_PHY_RESOURCE_H
...
inline uint32_t cell_nof_prb_to_rbg(uint32_t nof_prbs)
{
  switch (nof_prbs) {
    ...
    case 100:
      return 25;
    default:
      isrran_assertion_failure("Provided nof PRBs not valid");
  }
  return 0;
}
...
using pdcch_mask_t = isrran::bounded_bitset<MAX_NOF_CCES, true>;

/// Bitmask that stores the allocared DL RBGs
using rbgmask_t = isrran::bounded_bitset<MAX_NOF_RBGS, true>;

/// Bitmask that stores the allocated UL PRBs
using prbmask_t = isrran::bounded_bitset<MAX_NOF_PRBS, true>;

/// Struct to express a {min,...,max} range of RBGs
struct prb_interval;
struct rbg_interval : public isrran::interval<uint32_t> {
...
struct prb_interval : public isrran::interval<uint32_t> {
  ...
  {
    uint32_t P = isrran_ra_type0_P(cell_nof_prb);
...
{
  uint32_t P = isrran_ra_type0_P(cell_nof_prb);
  return rbg_interval{prbs.start() / P, isrran::ceil_div(prbs.stop(), P)};
}
...
inline uint32_t count_prb_per_tb(const rbgmask_t& bitmask)
{
  ...
  uint32_t P       = isrran_ra_type0_P(Nprb);
...
template <>
struct formatter<isrenb::rbg_interval> : public formatter<isrran::interval<uint32_t> > {};
template <>
struct formatter<isrenb::prb_interval> : public formatter<isrran::interval<uint32_t> > {};

} // namespace fmt

#endif // ISRRAN_SCHED_PHY_RESOURCE_H
