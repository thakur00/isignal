...
#ifndef ISRENB_SCHEDULER_UE_H
#define ISRENB_SCHEDULER_UE_H
...
public:
  ...
  isrran::interval<uint32_t> get_requested_dl_bytes(uint32_t enb_cc_idx);
  ...
  int generate_format0(sched_interface::ul_sched_data_t* data,
                       ...
                       prb_interval                      alloc,
                       bool                              needs_pdcch,
                       isrran_dci_location_t             cce_range,
                       int                               explicit_mcs = -1,
                       uci_pusch_t                       uci_type     = UCI_PUSCH_NONE);

  isrran_dci_format_t           get_dci_format();
  ...
  std::bitset<ISRRAN_MAX_CARRIERS> scell_activation_mask() const;
