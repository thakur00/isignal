...
#ifndef ISRRAN_SCHED_INTERFACE_H
#define ISRRAN_SCHED_INTERFACE_H
...
public:
  ...
  struct cell_cfg_t {
    ...
    /* pusch configuration */
    isrran_pusch_hopping_cfg_t pusch_hopping_cfg;
    ...
    uint32_t isr_subframe_config;
    uint32_t isr_subframe_offset;
    uint32_t isr_bw_config;
  ...
  struct ue_cfg_t {
    ...
    isrran_uci_offset_cfg_t             uci_offset       = {15, 12, 10};
  ...
  struct dl_sched_data_t {
    isrran_dci_dl_t dci;
    uint32_t        tbs[ISRRAN_MAX_TB];
    bool            mac_ce_ta;
    bool            mac_ce_rnti;
    uint32_t        nof_pdu_elems[ISRRAN_MAX_TB];
    dl_sched_pdu_t  pdu[ISRRAN_MAX_TB][MAX_RLC_PDU_LIST];
  };
  ...
  typedef struct {
    ...
    isrran_dci_rar_grant_t grant;
  } dl_sched_rar_grant_t;

  struct dl_sched_rar_t {
    uint32_t                                                   tbs;
    isrran_dci_dl_t                                            dci;
    isrran::bounded_vector<dl_sched_rar_grant_t, MAX_RAR_LIST> msg3_grant;
  };
  ...
  struct dl_sched_res_t {
    uint32_t                                               cfi;
    isrran::bounded_vector<dl_sched_data_t, MAX_DATA_LIST> data;
    isrran::bounded_vector<dl_sched_rar_t, MAX_RAR_LIST>   rar;
    isrran::bounded_vector<dl_sched_bc_t, MAX_BC_LIST>     bc;
    isrran::bounded_vector<dl_sched_po_t, MAX_PO_LIST>     po;
  };
  ...
  struct ul_sched_res_t {
    isrran::bounded_vector<ul_sched_data_t, MAX_DATA_LIST>   pusch;
    isrran::bounded_vector<ul_sched_phich_t, MAX_PHICH_LIST> phich;
  };
  ...
  virtual std::array<int, ISRRAN_MAX_CARRIERS> get_enb_ue_cc_map(uint16_t rnti)                            = 0;
  virtual std::array<int, ISRRAN_MAX_CARRIERS> get_enb_ue_activ_cc_map(uint16_t rnti)                      = 0;
  virtual int                                  ul_buffer_add(uint16_t rnti, uint32_t lcid, uint32_t bytes) = 0;
};

} // namespace isrenb

#endif // ISRRAN_SCHED_INTERFACE_H
