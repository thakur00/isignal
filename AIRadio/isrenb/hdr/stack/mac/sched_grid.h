...
#ifndef ISRRAN_SCHED_GRID_H
#define ISRRAN_SCHED_GRID_H
...
public:
  ...
  void             new_tti(isrran::tti_point tti_rx);
  bool             has_sf(isrran::tti_point tti_rx) const { return results[tti_rx.to_uint()].tti_rx == tti_rx; }
  sf_sched_result* get_sf(isrran::tti_point tti_rx)
  {
    assert(has_sf(tti_rx));
    return &results[tti_rx.to_uint()];
  }
  const sf_sched_result* get_sf(isrran::tti_point tti_rx) const
  {
    assert(has_sf(tti_rx));
    return &results[tti_rx.to_uint()];
  }
  const cc_sched_result* get_cc(isrran::tti_point tti_rx, uint32_t enb_cc_idx) const
  {
    return get_sf(tti_rx)->get_cc(enb_cc_idx);
  }
  cc_sched_result* get_cc(isrran::tti_point tti_rx, uint32_t enb_cc_idx) { return get_sf(tti_rx)->get_cc(enb_cc_idx); }

private:
  uint32_t                                           nof_carriers = 1;
  isrran::circular_array<sf_sched_result, TTIMOD_SZ> results;
};
...
{
public:
  sf_grid_t() : logger(isrlog::fetch_basic_logger("MAC")) {}
...
public:
  ...
  sf_sched();
  void init(const sched_cell_params_t& cell_params_);
  void new_tti(isrran::tti_point tti_rx_, sf_sched_result* cc_results);
  ...
  isrran::const_span<rar_alloc_t> get_allocated_rars() const { return rar_allocs; }
...
private:
  ...
  isrran::bounded_vector<bc_alloc_t, sched_interface::MAX_BC_LIST>   bc_allocs;
  isrran::bounded_vector<rar_alloc_t, sched_interface::MAX_RAR_LIST> rar_allocs;
  isrran::bounded_vector<po_alloc_t, sched_interface::MAX_PO_LIST>   po_allocs;
  isrran::bounded_vector<dl_alloc_t, sched_interface::MAX_DATA_LIST> data_allocs;
  isrran::bounded_vector<ul_alloc_t, sched_interface::MAX_DATA_LIST> ul_data_allocs;
