...
#ifndef ISRENB_PHCH_COMMON_H
#define ISRENB_PHCH_COMMON_H
...
public:
  ...
  isrran_cell_t get_cell(uint32_t cc_idx)
  {
    isrran_cell_t c = {};
    if (cc_idx < cell_list_lte.size()) {
  ...
  void set_cell_gain(uint32_t cell_id, float gain_db)
  {
    ...
    isrran::console("cell ID %d not found\n", cell_id);
  }
  ...
  void             set_cfr_config(isrran_cfr_cfg_t cfr_cfg) { cfr_config = cfr_cfg; }
  isrran_cfr_cfg_t get_cfr_config() { return cfr_config; }

  // Common Physical Uplink DMRS configuration
  isrran_refsignal_dmrs_pusch_cfg_t dmrs_pusch_cfg = {};

  isrran::radio_interface_phy* radio      = nullptr;
  stack_interface_phy_lte*     stack      = nullptr;
  isrran::channel_ptr          dl_channel = nullptr;
  ...
  void configure_mbsfn(isrran::phy_cfg_mbsfn_t* cfg);
  void build_mch_table();
  void build_mcch_table();
  bool is_mbsfn_sf(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti);
...
private:
  ...
  isrran::circular_array<stack_interface_phy_lte::ul_sched_list_t, TTIMOD_SZ> ul_grants   = {};
