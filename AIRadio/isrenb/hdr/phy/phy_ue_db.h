...
#ifndef ISRENB_PHY_UE_DB_H_
#define ISRENB_PHY_UE_DB_H_
...
private:
  ...
  struct cell_info_t {
    ...
    isrran::circular_array<isrran_ra_tb_t, ISRRAN_MAX_HARQ_PROC> last_tb =
        {}; ///< Stores last PUSCH Resource allocation
    bool                                    stash_use_tbs_index_alt = false;
    isrran::phy_cfg_t                       phy_cfg;            ///< Configuration, it has a default constructor
    isrran::circular_array<bool, TTIMOD_SZ> is_grant_available; ///< Indicates whether there is an available grant
  };
  ...
  struct common_ue {
    ...
    isrran::circular_array<isrran_pdsch_ack_t, TTIMOD_SZ> pdsch_ack = {}; ///< Pending acknowledgements for this Cell
    std::array<cell_info_t, ISRRAN_MAX_CARRIERS>          cell_info = {}; ///< Cell information, indexed by ue_cell_idx
  };
   ...
   * @return ISRRAN_SUCCESS if the RNTI is not duplicated and is added successfully, ISRRAN_ERROR code if it exists
   */
   ...
   */
  inline void _set_common_config_rnti(uint16_t rnti, isrran::phy_cfg_t& phy_cfg) const;

  /**
   * Gets the SCell index for a given RNTI and a eNb cell/carrier. It returns the SCell index (0 if PCell) if the cc_idx
   * is found among the configured cells/carriers. Otherwise, it returns ISRRAN_MAX_CARRIERS.
   *
   * @param rnti identifier of the UE (requires assertion prior to call)
   ...
   * @return ISRRAN_SUCCESS if the indicated RNTI exists, otherwise it returns ISRRAN_ERROR
   */
   ...
   * @return ISRRAN_SUCCESS if the indicated eNb cell/carrier of the RNTI is a PCell, otherwise it returns ISRRAN_ERROR
   */
   ...
   * @return ISRRAN_SUCCESS if the indicated cell/carrier index is valid, otherwise it returns ISRRAN_ERROR
   */
   ...
   * @return ISRRAN_SUCCESS if the indicated eNb cell/carrier is active, otherwise it returns ISRRAN_ERROR
   */
  inline int _assert_active_enb_cc(uint16_t rnti, uint32_t enb_cc_idx) const;

  /**
   * Internal eNb stack assertion
   * @return ISRRAN_SUCCESS if available, otherwise it returns ISRRAN_ERROR
   */
   ...
   * @return ISRRAN_SUCCESS if provided context is correct, ISRRAN_ERROR code otherwise
   */
  inline int _get_rnti_config(uint16_t rnti, uint32_t enb_cc_idx, isrran::phy_cfg_t& phy_cfg) const;
...
public:
   ...
   *
   * @param rnti identifier of the UE
   * @return ISRRAN_SUCCESS if provided RNTI exists, ISRRAN_ERROR code otherwise
   */
   ...
   * @param scell_idx
   * @param activate
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
   ...
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
  int get_dl_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dl_cfg_t& dl_cfg) const;
   ...
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
  int get_dci_dl_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dci_cfg_t& dci_cfg) const;
   ...
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
  int get_ul_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_ul_cfg_t& ul_cfg) const;
   ...
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
  int get_dci_ul_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dci_cfg_t& dci_cfg) const;
   ...
   *
   */
  bool set_ack_pending(uint32_t tti, uint32_t enb_cc_idx, const isrran_dci_dl_t& dci);
   ...
   * @return ISRRAN_SUCCESS if provided RNTI exists in the given cell, ISRRAN_ERROR code otherwise
   */
  int set_last_ul_tb(uint16_t rnti, uint32_t enb_cc_idx, uint32_t pid, isrran_ra_tb_t tb);
   ...
   * @return ISRRAN_SUCCESS if the provided context is valid
   */
  int get_last_ul_tb(uint16_t rnti, uint32_t enb_cc_idx, uint32_t pid, isrran_ra_tb_t& ra_tb) const;
