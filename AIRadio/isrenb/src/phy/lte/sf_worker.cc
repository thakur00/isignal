...
#include "isrenb/hdr/phy/lte/sf_worker.h"
...
void       init_plots(isrenb::lte::sf_worker* worker);
pthread_t  plot_thread;
sem_t      plot_sem;
static int plot_worker_id = -1;
#else
#pragma message "Compiling without isrGUI support"
#endif
/*********************************************/
...
void sf_worker::init(phy_common* phy_)
{
  ...
  if (isrran_softbuffer_tx_init(&temp_mbsfn_softbuffer, phy->get_nof_prb(0))) {
...
void sf_worker::set_context(const isrran::phy_common_interface::worker_context_t& w_ctx)
{
  tti_rx    = w_ctx.sf_idx;
...
void sf_worker::work_imp()
{
  ...
  tx_buffer.set_nof_samples(ISRRAN_SF_LEN_PRB(phy->get_nof_prb(0)));
  ...
  isrran_sf_t        sf_type = phy->is_mbsfn_sf(&mbsfn_cfg, tti_tx_dl) ? ISRRAN_SF_MBSFN : ISRRAN_SF_NORM;
  ...
  if (phy->ue_db.set_ul_grant_available(tti_rx, ul_grants) < ISRRAN_SUCCESS) {
  ...
  for (uint32_t cc = 0; cc < cc_workers.size(); cc++) {
    ...
    dl_sf.cfi = ISRRAN_MAX(dl_sf.cfi, 1);
    dl_sf.cfi = ISRRAN_MIN(dl_sf.cfi, 3);
...
#ifdef DEBUG_WRITE_FILE
  fwrite(signal_buffer_tx, ISRRAN_SF_LEN_PRB(phy->cell.nof_prb) * sizeof(cf_t), 1, f);
#endif
...
uint32_t sf_worker::get_metrics(std::vector<phy_metrics_t>& metrics)
{
  ...
  for (uint32_t cc = 0; cc < phy->get_nof_carriers_lte(); cc++) {
    ...
    for (uint32_t r = 0; r < cnt; r++) {
      ...
      m->dl.mcs         = ISRRAN_VEC_SAFE_PMA(m->dl.mcs, m->dl.n_samples, m_->dl.mcs, m_->dl.n_samples);
      m->dl.n_samples += m_->dl.n_samples;
      m->ul.n          = ISRRAN_VEC_SAFE_PMA(m->ul.n, m->ul.n_samples, m_->ul.n, m_->ul.n_samples);
      m->ul.pusch_sinr = ISRRAN_VEC_SAFE_PMA(m->ul.pusch_sinr, m->ul.n_samples, m_->ul.pusch_sinr, m_->ul.n_samples);
      m->ul.pucch_sinr =
          ISRRAN_VEC_SAFE_PMA(m->ul.pucch_sinr, m->ul.n_samples_pucch, m_->ul.pucch_sinr, m_->ul.n_samples_pucch);
      m->ul.mcs        = ISRRAN_VEC_SAFE_PMA(m->ul.mcs, m->ul.n_samples, m_->ul.mcs, m_->ul.n_samples);
      m->ul.pusch_rssi = ISRRAN_VEC_SAFE_PMA(m->ul.pusch_rssi, m->ul.n_samples, m_->ul.pusch_rssi, m_->ul.n_samples);
      m->ul.pucch_rssi =
          ISRRAN_VEC_SAFE_PMA(m->ul.pucch_rssi, m->ul.n_samples_pucch, m_->ul.pucch_rssi, m_->ul.n_samples_pucch);
      m->ul.pucch_ni =
          ISRRAN_VEC_SAFE_PMA(m->ul.pucch_ni, m->ul.n_samples_pucch, m_->ul.pucch_ni, m_->ul.n_samples_pucch);
      m->ul.turbo_iters = ISRRAN_VEC_SAFE_PMA(m->ul.turbo_iters, m->ul.n_samples, m_->ul.turbo_iters, m_->ul.n_samples);
...
void sf_worker::start_plot()
{
  ...
  if (plot_worker_id == -1) {
    ...
    isrran::console("Starting plot for worker_id=%d\n", plot_worker_id);
    init_plots(this);
  } else {
    isrran::console("Trying to start a plot but already started by worker_id=%d\n", plot_worker_id);
  }
#else
  isrran::console("Trying to start a plot but plots are disabled (ENABLE_GUI constant in sf_worker.cc)\n");
#endif
}
...
#define SCATTER_PUSCH_BUFFER_LEN (20 * 6 * ISRRAN_SF_LEN_RE(ISRRAN_MAX_PRB, ISRRAN_CP_NORM))
static float tmp_plot[SCATTER_PUSCH_BUFFER_LEN];
static float tmp_plot_arg[SCATTER_PUSCH_BUFFER_LEN];
static cf_t  tmp_plot2[ISRRAN_SF_LEN_RE(ISRRAN_MAX_PRB, ISRRAN_CP_NORM)];
static cf_t  tmp_pucch_plot[ISRRAN_PUCCH_MAX_BITS / 2];

void* plot_thread_run(void* arg)
{
  auto worker = (isrenb::lte::sf_worker*)arg;

  for (uint32_t cc_idx = 0; cc_idx < worker->get_nof_carriers(); cc_idx++) {
    plot_cc_s& p = plots[cc_idx];

    char title[32] = {};
    snprintf(title, sizeof(title), "isrENB PCI %d", worker->get_carrier_pci(cc_idx));
