...
#include "isrenb/hdr/phy/nr/slot_worker.h"
...
namespace isrenb {
namespace nr {
slot_worker::slot_worker(isrran::phy_common_interface& common_,
...
bool slot_worker::init(const args_t& args)
{
  ...
  for (uint32_t i = 0; i < args.nof_tx_ports; i++) {
    tx_buffer[i] = isrran_vec_cf_malloc(sf_len);
  ...
  // Initialise DL
  if (isrran_gnb_dl_init(&gnb_dl, tx_buffer.data(), &dl_args) < ISRRAN_SUCCESS) {
    logger.error("Error gNb DL init");
    return false;
  }

  // Prepare UL arguments
  isrran_gnb_ul_args_t ul_args   = {};
  ...
  // Initialise UL
  if (isrran_gnb_ul_init(&gnb_ul, rx_buffer[0], &ul_args) < ISRRAN_SUCCESS) {
    logger.error("Error gNb DL init");
    return false;
  }
...
void slot_worker::set_context(const isrran::phy_common_interface::worker_context_t& w_ctx)
{
...
bool slot_worker::work_ul()
{
  ...
  // Demodulate
  if (isrran_gnb_ul_fft(&gnb_ul) < ISRRAN_SUCCESS) {
  ...
  for (stack_interface_phy_nr::pucch_t& pucch : ul_sched->pucch) {
    isrran::bounded_vector<stack_interface_phy_nr::pucch_info_t, stack_interface_phy_nr::MAX_PUCCH_CANDIDATES>
    ...
    // Inform stack
    if (stack.pucch_info(ul_slot_cfg, pucch_info[best_candidate]) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      ...
      isrran_gnb_ul_pucch_info(&gnb_ul,
  ...
  for (stack_interface_phy_nr::pusch_t& pusch : ul_sched->pusch) {
    ...
    pusch_info.pdu                                  = isrran::make_byte_buffer();
    ...
    // Decode PUSCH
    if (isrran_gnb_ul_get_pusch(&gnb_ul, &ul_slot_cfg, &pusch.sch, &pusch.sch.grant, &pusch_info.pusch_data) <
        ISRRAN_SUCCESS) {
    ...
    // Inform stack
    if (stack.pusch_info(ul_slot_cfg, pusch_info) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str;
      isrran_gnb_ul_pusch_info(&gnb_ul, &pusch.sch, &pusch_info.pusch_data, str.data(), (uint32_t)str.size());

      if (logger.debug.enabled()) {
        std::array<char, 1024> str_extra = {};
        isrran_sch_cfg_nr_info(&pusch.sch, str_extra.data(), (uint32_t)str_extra.size());
...
bool slot_worker::work_dl()
{
  ...
  for (const stack_interface_phy_nr::pdcch_dl_t& pdcch : dl_sched_ptr->pdcch_dl) {
    ...
    if (isrran_gnb_dl_set_pdcch_config(&gnb_dl, &pdcch_cfg, &pdcch.dci_cfg) < ISRRAN_SUCCESS) {
      logger.error("PDCCH: Error setting DL configuration");
      return false;
    }

    // Put PDCCH message
    if (isrran_gnb_dl_pdcch_put_dl(&gnb_dl, &dl_slot_cfg, &pdcch.dci) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str = {};
      isrran_gnb_dl_pdcch_dl_info(&gnb_dl, &pdcch.dci, str.data(), (uint32_t)str.size());
  ...
  for (const stack_interface_phy_nr::pdcch_ul_t& pdcch : dl_sched_ptr->pdcch_ul) {
    ...
    // Put PDCCH message
    if (isrran_gnb_dl_pdcch_put_ul(&gnb_dl, &dl_slot_cfg, &pdcch.dci) < ISRRAN_SUCCESS) {
      logger.error("PDCCH: Error putting DL message");
      return false;
    }
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str = {};
      isrran_gnb_dl_pdcch_ul_info(&gnb_dl, &pdcch.dci, str.data(), (uint32_t)str.size());
      logger.info("PDCCH: cc=%d %s tti_tx=%d", cell_index, str.data(), dl_slot_cfg.idx);
    }
  }
  ...
  for (const stack_interface_phy_nr::pdsch_t& pdsch : dl_sched_ptr->pdsch) {
    ...
    // Put PDSCH message
    if (isrran_gnb_dl_pdsch_put(&gnb_dl, &dl_slot_cfg, &pdsch.sch, data) < ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      std::array<char, 512> str = {};
      isrran_gnb_dl_pdsch_info(&gnb_dl, &pdsch.sch, str.data(), (uint32_t)str.size());

      if (logger.debug.enabled()) {
        std::array<char, 1024> str_extra = {};
        isrran_sch_cfg_nr_info(&pdsch.sch, str_extra.data(), (uint32_t)str_extra.size());
  ...
  // Put NZP-CSI-RS
  for (const isrran_csi_rs_nzp_resource_t& nzp_csi_rs : dl_sched_ptr->nzp_csi_rs) {
    if (isrran_gnb_dl_nzp_csi_rs_put(&gnb_dl, &dl_slot_cfg, &nzp_csi_rs) < ISRRAN_SUCCESS) {
  ...
  for (const stack_interface_phy_nr::ssb_t& ssb : dl_sched_ptr->ssb) {
    if (isrran_gnb_dl_add_ssb(&gnb_dl, &ssb.pbch_msg, dl_slot_cfg.idx) < ISRRAN_SUCCESS) {
...
void slot_worker::work_imp()
{
  ...
  isrran::rf_buffer_t tx_rf_buffer = {};
...
bool slot_worker::set_common_cfg(const isrran_carrier_nr_t&   carrier,
                                 const isrran_pdcch_cfg_nr_t& pdcch_cfg_,
                                 const isrran_ssb_cfg_t&      ssb_cfg_)
{
  std::lock_guard<std::mutex> lock(mutex);
  // Set gNb DL carrier
  if (isrran_gnb_dl_set_carrier(&gnb_dl, &carrier) < ISRRAN_SUCCESS) {
    logger.error("Error setting DL carrier");
    return false;
  }

  // Configure SSB
  if (isrran_gnb_dl_set_ssb_config(&gnb_dl, &ssb_cfg_) < ISRRAN_SUCCESS) {
    logger.error("Error setting SSB");
    return false;
  }

  // Set gNb UL carrier
  if (isrran_gnb_ul_set_carrier(&gnb_ul, &carrier) < ISRRAN_SUCCESS) {
  ...
  sf_len = ISRRAN_SF_LEN_PRB_NR(carrier.nof_prb);
