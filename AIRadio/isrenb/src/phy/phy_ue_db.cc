...
#include "isrenb/hdr/phy/phy_ue_db.h"
...
inline void phy_ue_db::_clear_tti_pending_rnti(uint32_t tti, uint16_t rnti)
{
  ...
  isrran_pdsch_ack_t& pdsch_ack = ue.pdsch_ack[tti];
...
inline void phy_ue_db::_set_common_config_rnti(uint16_t rnti, isrran::phy_cfg_t& phy_cfg) const
{
  ...
  phy_cfg.ul_cfg.pucch.threshold_format1             = ISRRAN_PUCCH_DEFAULT_THRESHOLD_FORMAT1;
  phy_cfg.ul_cfg.pucch.threshold_data_valid_format1a = ISRRAN_PUCCH_DEFAULT_THRESHOLD_FORMAT1A;
  phy_cfg.ul_cfg.pucch.threshold_data_valid_format2  = ISRRAN_PUCCH_DEFAULT_THRESHOLD_FORMAT2;
  phy_cfg.ul_cfg.pucch.threshold_data_valid_format3  = ISRRAN_PUCCH_DEFAULT_THRESHOLD_FORMAT3;
  phy_cfg.ul_cfg.pucch.threshold_dmrs_detection      = ISRRAN_PUCCH_DEFAULT_THRESHOLD_DMRS;
...
inline uint32_t phy_ue_db::_get_ue_cc_idx(uint16_t rnti, uint32_t enb_cc_idx) const
{
  ...
  for (; ue_cc_idx < ISRRAN_MAX_CARRIERS; ue_cc_idx++) {
...
{
  // Assert RNTI exist
  if (_assert_rnti(rnti) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Check Component Carrier is part of UE SCell map
  if (_get_ue_cc_idx(rnti, enb_cc_idx) == ISRRAN_MAX_CARRIERS) {
    return ISRRAN_ERROR;
  }
...
{
  return _assert_enb_cc(rnti, enb_cc_idx) == ISRRAN_SUCCESS;
}

inline int phy_ue_db::_assert_enb_pcell(uint16_t rnti, uint32_t enb_cc_idx) const
{
  if (_assert_enb_cc(rnti, enb_cc_idx) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
...
inline int phy_ue_db::_assert_ue_cc(uint16_t rnti, uint32_t ue_cc_idx) const
{
  ...
  if (ue_cc_idx >= ISRRAN_MAX_CARRIERS) {
    return ISRRAN_ERROR;
  }
...
inline int phy_ue_db::_get_rnti_config(uint16_t rnti, uint32_t enb_cc_idx, isrran::phy_cfg_t& phy_cfg) const
{
  isrran::phy_cfg_t default_cfg = {};
  ...
  if (not ISRRAN_RNTI_ISUSER(rnti)) {
    phy_cfg = default_cfg;
    return ISRRAN_SUCCESS;
  }
...
void phy_ue_db::addmod_rnti(uint16_t rnti, const phy_interface_rrc_lte::phy_rrc_cfg_list_t& phy_cfg_list)
{
  ...
  uint32_t nof_cc = ISRRAN_MIN(phy_cfg_list.size(), ISRRAN_MAX_CARRIERS);
  ...
  for (uint32_t i = nof_cc; i < ISRRAN_MAX_CARRIERS; i++) {
...
uint32_t phy_ue_db::_count_nof_configured_scell(uint16_t rnti)
{
  ...
  for (uint32_t ue_cc_idx = 0; ue_cc_idx < ISRRAN_MAX_CARRIERS; ue_cc_idx++) {
...
int phy_ue_db::activate_deactivate_scell(uint16_t rnti, uint32_t ue_cc_idx, bool activate)
{
  ...
  if (_assert_ue_cc(rnti, ue_cc_idx) != ISRRAN_SUCCESS) {
    return ISRRAN_SUCCESS;
  }
...
{
  std::lock_guard<std::mutex> lock(mutex);
  return _assert_enb_pcell(rnti, enb_cc_idx) == ISRRAN_SUCCESS;
}

int phy_ue_db::get_dl_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dl_cfg_t& dl_cfg) const
{
  std::lock_guard<std::mutex> lock(mutex);
  isrran::phy_cfg_t           phy_cfg = {};

  if (_get_rnti_config(rnti, enb_cc_idx, phy_cfg) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  if (ue_db.count(rnti) && ISRRAN_RNTI_ISUSER(rnti)) {
    uint32_t ue_cc_idx = _get_ue_cc_idx(rnti, enb_cc_idx);
    if (ue_cc_idx == 0) {
...
int phy_ue_db::get_dci_dl_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dci_cfg_t& dci_cfg) const
{
  std::lock_guard<std::mutex> lock(mutex);
  isrran::phy_cfg_t           phy_cfg = {};
...
int phy_ue_db::get_ul_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_ul_cfg_t& ul_cfg) const
{
  std::lock_guard<std::mutex> lock(mutex);
  isrran::phy_cfg_t           phy_cfg = {};
...
int phy_ue_db::get_dci_ul_config(uint16_t rnti, uint32_t enb_cc_idx, isrran_dci_cfg_t& dci_cfg) const
{
  std::lock_guard<std::mutex> lock(mutex);
  isrran::phy_cfg_t           phy_cfg = {};
...
bool phy_ue_db::set_ack_pending(uint32_t tti, uint32_t enb_cc_idx, const isrran_dci_dl_t& dci)
{
  std::lock_guard<std::mutex> lock(mutex);

  // Assert rnti and cell exits and it is active
  if (_assert_active_enb_cc(dci.rnti, enb_cc_idx) != ISRRAN_SUCCESS) {
    return false;
  }
  ...
  isrran_pdsch_ack_cc_t& pdsch_ack_cc = ue.pdsch_ack[tti].cc[ue_cc_idx];
  pdsch_ack_cc.M                      = 1; ///< Hardcoded for FDD

  // Fill PDSCH ACK information
  isrran_pdsch_ack_m_t& pdsch_ack_m  = pdsch_ack_cc.m[0]; ///< Assume FDD only
  ...
  // Set TB info
  for (uint32_t tb_idx = 0; tb_idx < ISRRAN_MAX_CODEWORDS; tb_idx++) {
    // Count only if the TB is enabled and the TB index is valid for the DCI format
    if (ISRRAN_DCI_IS_TB_EN(dci.tb[tb_idx]) and tb_idx < isrran_dci_format_max_tb(dci.format)) {
                            ...
                            isrran_uci_cfg_t& uci_cfg)
{
  ...
  if (_assert_cell_list_cfg() != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Assert eNb Cell/Carrier for the given RNTI
  if (_assert_active_enb_cc(rnti, enb_cc_idx) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  const isrran::phy_cfg_t& pcell_cfg    = ue.cell_info[0].phy_cfg;
  bool                     uci_required = false;

  const cell_info_t&   pcell_info = ue.cell_info[0];
  const isrran_cell_t& pcell      = cell_cfg_list->at(pcell_info.enb_cc_idx).cell;

  // Check if SR opportunity (will only be used in PUCCH)
  uci_cfg.is_scheduling_request_tti = (isrran_ue_ul_sr_send_tti(&pcell_cfg.ul_cfg.pucch, tti) == 1);
  ...
  for (uint32_t cell_idx = 0; cell_idx < ISRRAN_MAX_CARRIERS and not periodic_cqi_required; cell_idx++) {
    const cell_info_t&     cell_info = ue.cell_info[cell_idx];
    const isrran_dl_cfg_t& dl_cfg    = cell_info.phy_cfg.dl_cfg;
    ...
    if (cell_info.state == cell_state_primary or cell_info.state == cell_state_secondary_active) {
      const isrran_cell_t& cell = cell_cfg_list->at(cell_info.enb_cc_idx).cell;

      // Check if CQI report is required
      periodic_cqi_required = isrran_enb_dl_gen_cqi_periodic(&cell, &dl_cfg, tti, cell_info.last_ri, &uci_cfg.cqi);
  ...
  if ((not periodic_cqi_required) and aperiodic_cqi_request) {
    ...
    const isrran_dl_cfg_t& dl_cfg = pcell_info.phy_cfg.dl_cfg;

    uci_required = isrran_enb_dl_gen_cqi_aperiodic(&pcell, &dl_cfg, pcell_info.last_ri, &uci_cfg.cqi);
  }
  ...
  isrran_enb_dl_gen_ack(&pcell, &dl_sf_cfg, &pdsch_ack, &uci_cfg);
  uci_required |= (isrran_uci_cfg_total_ack(&uci_cfg) > 0);

  // Return whether UCI needs to be decoded
  return uci_required ? 1 : ISRRAN_SUCCESS;
}

void phy_ue_db::send_cqi_data(uint32_t                      tti,
                             uint16_t                       rnti,
                             uint32_t                       cqi_cc_idx,
                             const isrran_cqi_cfg_t&        cqi_cfg,
                             const isrran_cqi_value_t&      cqi_value,
  ...
  switch (cqi_cfg.type) {
    ...
    case ISRRAN_CQI_TYPE_SUBBAND_UE:
      ...
      stack->sb_cqi_info(tti,
                         rnti,
                         cqi_cc_idx,
                         isrran_cqi_get_sb_idx(tti, cqi_value.subband_ue.subband_label, &cqi_report_cfg, &cell),
                         stack_value);
      break;
                             ...
                             const isrran_uci_value_t& uci_value)
{
  ...
  // Assert Stack
  if (_assert_stack() != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  // Get ACK info
  isrran_pdsch_ack_t& pdsch_ack = ue.pdsch_ack[tti];
  const isrran_cell_t& cell      = cell_cfg_list->at(ue.cell_info[0].enb_cc_idx).cell;
  isrran_enb_dl_get_ack(&cell, &uci_cfg, &uci_value, &pdsch_ack);

  // Iterate over the ACK information
  for (uint32_t ue_cc_idx = 0; ue_cc_idx < ISRRAN_MAX_CARRIERS; ue_cc_idx++) {
    const isrran_pdsch_ack_cc_t& pdsch_ack_cc = pdsch_ack.cc[ue_cc_idx];
  ...
  if (_assert_ue_cc(rnti, uci_cfg.cqi.scell_index) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
...
int phy_ue_db::set_last_ul_tb(uint16_t rnti, uint32_t enb_cc_idx, uint32_t pid, isrran_ra_tb_t tb)
{
  std::lock_guard<std::mutex> lock(mutex);
...
int phy_ue_db::get_last_ul_tb(uint16_t rnti, uint32_t enb_cc_idx, uint32_t pid, isrran_ra_tb_t& ra_tb) const
{
  std::lock_guard<std::mutex> lock(mutex);
...
int phy_ue_db::set_ul_grant_available(uint32_t tti, const stack_interface_phy_lte::ul_sched_list_t& ul_sched_list)
{
  ...
  for (uint32_t enb_cc_idx = 0; enb_cc_idx < (uint32_t)ul_sched_list.size(); enb_cc_idx++) {
    ...
    for (uint32_t i = 0; i < ul_sched.nof_grants; i++) {
      ...
      if (_assert_active_enb_cc(rnti, enb_cc_idx) != ISRRAN_SUCCESS) {
        ret = ISRRAN_ERROR;
        isrlog::fetch_basic_logger("PHY").info("Error setting grant for rnti=0x%x, cc=%d", rnti, enb_cc_idx);
        continue;
      }
