...
phy::phy(isrlog::sink& log_sink) :
  log_sink(log_sink),
  phy_log(isrlog::fetch_basic_logger("PHY", log_sink)),
  phy_lib_log(isrlog::fetch_basic_logger("PHY_LIB", log_sink)),
...
{
  if (init_lte(args, cfg, radio_, stack_lte_, enb_) != ISRRAN_SUCCESS) {
    phy_log.error("Couldn't initialize LTE PHY");
    return ISRRAN_ERROR;
  }

  if (init_nr(args, cfg, stack_nr_) != ISRRAN_SUCCESS) {
...
int phy::init_lte(const phy_args_t&            args,
                  ...
                  enb_time_interface*          enb_)
{
  if (cfg.phy_cell_cfg.size() > ISRRAN_MAX_CARRIERS) {
    phy_log.error(
        "Number of carriers=%zd provided above its maximum=%zd", cfg.phy_cell_cfg.size(), ISRRAN_MAX_CARRIERS);
    return ISRRAN_ERROR;
  }

  // Add PHY lib log.
  isrlog::basic_levels lib_log_lvl = isrlog::str_to_basic_level(args.log.phy_lib_level);
  isrlog::basic_levels log_lvl     = isrlog::str_to_basic_level(args.log.phy_level);

  phy_lib_log.set_level(lib_log_lvl);
  phy_lib_log.set_hex_dump_max_size(args.log.phy_hex_limit);
  if (log_lvl != isrlog::basic_levels::none) {
...
void phy::set_activation_deactivation_scell(uint16_t rnti, const std::array<bool, ISRRAN_MAX_CARRIERS>& activation)
{
  // Iterate all elements except 0 that is reserved for primary cell
  for (uint32_t scell_idx = 1; scell_idx < ISRRAN_MAX_CARRIERS; scell_idx++) {
...
void phy::complete_config(uint16_t rnti)
{
  ...
  if (workers_common.ue_db.complete_config(rnti) < ISRRAN_SUCCESS) {
    Error("Error completing configuration for RNTI %x. It does not exist.", rnti);
  }
}

void phy::configure_mbsfn(isrran::sib2_mbms_t* sib2, isrran::sib13_t* sib13, const isrran::mcch_msg_t& mcch)
{
