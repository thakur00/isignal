...
txrx::txrx(isrlog::basic_logger& logger) : thread("TXRX"), logger(logger), running(false)
{
  /* Do nothing */
}
                ...
                uint32_t                     prio_)
{
  ...
  if (worker_com->params.ul_channel_args.enable) {
    ul_channel = isrran::channel_ptr(
        new isrran::channel(worker_com->params.ul_channel_args, worker_com->get_nof_rf_channels(), logger));
  }
...
void txrx::run_thread()
{
  ...
  isrran::rf_timestamp_t timestamp = {};
  uint32_t               sf_len    = ISRRAN_SF_LEN_PRB(worker_com->get_nof_prb(0));

  float samp_rate = isrran_sampling_freq_hz(worker_com->get_nof_prb(0));
  ...
  for (uint32_t cc_idx = 0; cc_idx < worker_com->get_nof_carriers(); cc_idx++) {
    ...
    if (cc_idx < worker_com->get_nof_carriers_lte()) {
      isrran::console("Setting frequency: DL=%.1f Mhz, UL=%.1f MHz for cc_idx=%d nof_prb=%d\n",
                      tx_freq_hz / 1e6f,
                      rx_freq_hz / 1e6f,
                      cc_idx,
