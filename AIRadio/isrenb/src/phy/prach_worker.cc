...
#include "isrenb/hdr/phy/prach_worker.h"
#include "isrran/interfaces/enb_mac_interfaces.h"
#include "isrran/isrran.h"

namespace isrenb {

int prach_worker::init(const isrran_cell_t&      cell_,
                       ...
                       uint32_t                  nof_workers_)
{
  ...
  if (isrran_prach_init(&prach, isrran_symbol_sz(cell.nof_prb))) {
    return -1;
  }

  if (isrran_prach_set_cfg(&prach, &prach_cfg, cell.nof_prb)) {
    ERROR("Error initiating PRACH");
    return -1;
  }

  isrran_prach_set_detect_factor(&prach, 60);
...
int prach_worker::new_tti(uint32_t tti_rx, cf_t* buffer_rx)
{
  ...
  if (isrran_prach_tti_opportunity(&prach, tti_rx, -1) || sf_cnt) {
    if (sf_cnt == 0) {
    ...
    if (current_buffer->nof_samples + ISRRAN_SF_LEN_PRB(cell.nof_prb) < sf_buffer_sz) {
      memcpy(&current_buffer->samples[sf_cnt * ISRRAN_SF_LEN_PRB(cell.nof_prb)],
             buffer_rx,
             sizeof(cf_t) * ISRRAN_SF_LEN_PRB(cell.nof_prb));
      current_buffer->nof_samples += ISRRAN_SF_LEN_PRB(cell.nof_prb);
      if (sf_cnt == 0) {
    ...
    if (prach_nof_det) {
      for (uint32_t i = 0; i < prach_nof_det; i++) {
        ...
        if (prach_offsets[i] * 1e6 < max_prach_offset_us) {
          ...
          uint32_t n_ta = (uint32_t)(prach_offsets[i] / (16 * ISRRAN_LTE_TS));

          stack->rach_detected(b->tti, cc_idx, prach_indices[i], n_ta);

#if defined(ENABLE_GUI) and ENABLE_PRACH_GUI
          uint32_t nof_samples = ISRRAN_MIN(nof_sf * ISRRAN_SF_LEN_PRB(cell.nof_prb), 3 * ISRRAN_SF_LEN_MAX);
          isrran_vec_abs_cf(b->samples, plot_buffer.data(), nof_samples);
