...
bool phy_common::init(const phy_cell_cfg_list_t&    cell_list_,
                      ...
                      isrran::radio_interface_phy*  radio_h_,
                      stack_interface_phy_lte*      stack_)
{
  ...
  if (params.dl_channel_args.enable) {
    ...
    dl_channel = isrran::channel_ptr(
        new isrran::channel(params.dl_channel_args, get_nof_rf_channels(), isrlog::fetch_basic_logger("PHY")));
    dl_channel->set_srate((uint32_t)isrran_sampling_freq_hz(channel_prbs));
    dl_channel->set_signal_power_dBfs(isrran_enb_dl_get_maximum_signal_power_dBfs(channel_prbs));
  }
...
void phy_common::configure_mbsfn(isrran::phy_cfg_mbsfn_t* cfg)
{
  mbsfn            = *cfg;
...
void phy_common::build_mch_table()
{
  ...
  uint32_t nof_sfs = 0;
  if (mbsfn.mbsfn_subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::one_frame) {
    generate_mch_table(&mch_table[0], (uint32_t)mbsfn.mbsfn_subfr_cnfg.sf_alloc, 1);
    nof_sfs = 10;
  } else if (mbsfn.mbsfn_subfr_cnfg.nof_alloc_subfrs == isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::four_frames) {
...
bool phy_common::is_mcch_subframe(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti)
{
  ...
  if (sib13_configured) {
    ...
    isrran::mbsfn_area_info_t* area_info = &mbsfn.mbsfn_area_info;
...
bool phy_common::is_mch_subframe(isrran_mbsfn_cfg_t* cfg, uint32_t phy_tti)
{
  ...
  isrran::mbsfn_sf_cfg_t*    subfr_cnfg = &mbsfn.mbsfn_subfr_cnfg;
  isrran::mbsfn_area_info_t* area_info  = &mbsfn.mbsfn_area_info;
