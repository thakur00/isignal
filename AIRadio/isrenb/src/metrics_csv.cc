...
void metrics_csv::set_metrics(const enb_metrics_t& metrics, const uint32_t period_usec)
{
  if (file.is_open() && enb != NULL) {
    ...
    // DL rate
    if (dl_rate_sum > 0) {
      file << float_to_string(ISRRAN_MAX(0.1, (float)dl_rate_sum), 2);
    } else {
    ...
    // UL rate
    if (ul_rate_sum > 0) {
      file << float_to_string(ISRRAN_MAX(0.1, (float)ul_rate_sum), 2);
    } else {
      file << float_to_string(0, 2);
    }

    // Write system metrics.
    const isrran::sys_metrics_t& m = metrics.sys;
