...
{
  int freq1 = isrran::get_carrier_freq(lhs);
  int freq2 = isrran::get_carrier_freq(rhs);
  return freq1 != -1 and freq1 == freq2;
}

meas_obj_t* find_meas_obj(meas_obj_to_add_mod_list_l& l, uint32_t earfcn)
{
  auto same_earfcn = [earfcn](const meas_obj_t& obj) { return (int)earfcn == isrran::get_carrier_freq(obj); };
...
std::pair<bool, meas_obj_t*> add_meas_obj(meas_obj_list& list, uint32_t dl_earfcn, uint32_t allowed_meas_bw)
{
  ...
  meas_obj_t new_obj;
  new_obj.meas_obj_id                = isrran::find_rrc_obj_id_gap(list);
  ...
  obj                       = isrran::add_rrc_obj(list, new_obj);
  return {true, obj};
}
                                                                    ...
                                                                    const meas_cell_cfg_t& cellcfg)
{
  ...
  if (ret.first != nullptr) {
    ...
    if (ret.second != nullptr) {
      ...
      new_cell.cell_idx                       = isrran::find_rrc_obj_id_gap(eutra_obj.cells_to_add_mod_list);
      ret.second                              = isrran::add_rrc_obj(eutra_obj.cells_to_add_mod_list, new_cell);
...
meas_obj_to_add_mod_s* meascfg_add_meas_obj(meas_cfg_s* meas_cfg, const meas_obj_to_add_mod_s& meas_obj)
{
  ...
  auto meas_obj_it = isrran::add_rrc_obj_id(meas_cfg->meas_obj_to_add_mod_list, meas_obj.meas_obj_id);
                        ...
                        meas_obj_to_add_mod_s*  added_obj)
{
  ...
  isrran::compute_cfg_diff(src_it.cells_to_add_mod_list,
...
report_cfg_to_add_mod_s* add_report_cfg(report_cfg_list& list, const report_cfg_eutra_s& reportcfg)
{
  ...
  new_rep.report_cfg_id                     = isrran::find_rrc_obj_id_gap(list);
  new_rep.report_cfg.set_report_cfg_eutra() = reportcfg;

  return isrran::add_rrc_obj(list, new_rep);
}
...
{
  isrran::compute_cfg_diff(src_cfg.report_cfg_to_add_mod_list,
...
meas_id_to_add_mod_s* add_measid_cfg(meas_id_to_add_mod_list_l& meas_id_list, uint8_t measobjid, uint8_t measrepid)
{
  ...
  new_measid.meas_id       = isrran::find_rrc_obj_id_gap(meas_id_list);
  return isrran::add_rrc_obj(meas_id_list, new_measid);
}

void compute_diff_meas_ids(const meas_cfg_s& src_cfg, const meas_cfg_s& target_cfg, meas_cfg_s& meas_cfg)
{
  isrran::compute_cfg_diff(src_cfg.meas_id_to_add_mod_list,
...
meas_gap_cfg_c make_measgap(const meas_obj_list& measobjs, const ue_cell_ded& pcell)
{
  ...
  switch (pcell.cell_common->cell_cfg.meas_cfg.meas_gap_period) {
    ...
    case 0: // no meas gaps configured
      break;
    default:
      isrlog::fetch_basic_logger("RRC").error("Error setting measurement gap.");
  }
  return meas_gap;
}
    ...
    if (target_offset.type().value != src_offset.type().value) {
      ...
      switch (target_offset.type().value) {
        ...
        case meas_gap_cfg_c::setup_s_::gap_offset_c_::types_opts::gp1:
          ...
          break;
        default:
          isrlog::fetch_basic_logger("RRC").warning("MeasGap of type %s not supported",
