...
#include "isrran/interfaces/enb_mac_interfaces.h"
...
rrc::rrc(isrran::task_sched_handle task_sched_, enb_bearer_manager& manager_) :
  logger(isrlog::fetch_basic_logger("RRC")), bearer_manager(manager_), task_sched(task_sched_), rx_pdu_queue(128)
{
}
                  ...
                  rrc_nr_interface_rrc*  rrc_nr_)
{
  ...
  if (generate_sibs() != ISRRAN_SUCCESS) {
...
int rrc::add_user(uint16_t rnti, const sched_interface::ue_cfg_t& sched_ue_cfg)
{
  ...
  if (rnti == ISRRAN_MRNTI) {
    for (auto& mbms_item : mcch.msg.c1().mbsfn_area_cfg_r9().pmch_info_list_r9[0].mbms_session_info_list_r9) {
      ...
      rlc->add_bearer_mrb(ISRRAN_MRNTI, lcid);
      bearer_manager.add_eps_bearer(ISRRAN_MRNTI, 1, isrran::isrran_rat_t::lte, lcid);
      pdcp->add_bearer(ISRRAN_MRNTI, lcid, isrran::make_drb_pdcp_config_t(1, false));
      gtpu->add_bearer(ISRRAN_MRNTI, lcid, 1, 1, addr_in);
    }
  }
  return ISRRAN_SUCCESS;
}
...
void rrc::send_rrc_connection_reject(uint16_t rnti)
{
  ...
  if (dl_ccch_msg.pack(bref) != asn1::ISRASN_SUCCESS) {
...
/*******************************************************************************
  PDCP interface
*******************************************************************************/
void rrc::write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
/*******************************************************************************
  S1AP interface
*******************************************************************************/
void rrc::write_dl_info(uint16_t rnti, isrran::unique_byte_buffer_t sdu)
{
...
int rrc::notify_ue_erab_updates(uint16_t rnti, isrran::const_byte_span nas_pdu)
{
  auto user_it = users.find(rnti);
  if (user_it == users.end()) {
    logger.warning("Unrecognised rnti: 0x%x", rnti);
...
int rrc::setup_erab(uint16_t                                           rnti,
                    ...
                    isrran::const_span<uint8_t>                        nas_pdu,
...
{
  auto read_func = [this, payload, buffer_size](isrran::const_byte_span pdu, const pcch_msg_s& msg, bool first_tx) {
    ...
    if (first_tx) {
      logger.info("Assembling PCCH payload with %d UE identities, payload_len=%d bytes",
                  ...
                  pdu.size());
      log_broadcast_rrc_message(ISRRAN_PRNTI, pdu, msg, msg.msg.c1().type().to_string());
    }
    return true;
  };
...
void rrc::parse_ul_ccch(ue& ue, isrran::unique_byte_buffer_t pdu)
{
  isrran_assert(pdu != nullptr, "handle_ul_ccch called for empty message");

  ul_ccch_msg_s  ul_ccch_msg;
  asn1::cbit_ref bref(pdu->msg, pdu->N_bytes);
  if (ul_ccch_msg.unpack(bref) != asn1::ISRASN_SUCCESS or
  ...
  // Log Rx message
  log_rrc_message(
      Rx, ue.rnti, isrran::srb_to_lcid(lte_srb::srb0), *pdu, ul_ccch_msg, ul_ccch_msg.msg.c1().type().to_string());
...
void rrc::parse_ul_dcch(ue& ue, uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  isrran_assert(pdu != nullptr, "handle_ul_dcch called for empty message");
...
void rrc::rem_user(uint16_t rnti)
{
  ...
  if (user_it != users.end()) {
    ...
    isrran::console("Disconnecting rnti=0x%x.\n", rnti);
...
void rrc::config_mac()
{
  ...
  for (uint32_t ccidx = 0; ccidx < cfg.cell_list.size(); ++ccidx) {
    ...
    item.nrb_pucch = ISRRAN_MAX(cfg.sr_cfg.nof_prb, item.nrb_cqi);
 ...
 *
 * @return ISRRAN_SUCCESS on success, ISRRAN_ERROR on failure
 */
uint32_t rrc::generate_sibs()
{
  ...
  for (uint32_t cc_idx = 0; cc_idx < cfg.cell_list.size(); cc_idx++) {
    ...
    for (uint32_t msg_index = 0; msg_index < nof_messages; msg_index++) {
      isrran::unique_byte_buffer_t sib_buffer = isrran::make_byte_buffer();
      if (sib_buffer == nullptr) {
        logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
        return ISRRAN_ERROR;
      }
      asn1::bit_ref bref(sib_buffer->msg, sib_buffer->get_tailroom());
      if (msg[msg_index].pack(bref) != asn1::ISRASN_SUCCESS) {
      ...
      log_broadcast_rrc_message(ISRRAN_SIRNTI, *cell_ctxt->sib_buffer.back(), msg[msg_index], isrran::to_c_str(membuf));
    }
...
void rrc::configure_mbsfn_sibs()
{
  ...
  for (int i = 0; i < sibs2.nof_mbsfn_sf_cfg; i++) {
    sibs2.mbsfn_sf_cfg_list[i].nof_alloc_subfrs = isrran::mbsfn_sf_cfg_t::sf_alloc_type_t::one_frame;
    ...
    sibs2.mbsfn_sf_cfg_list[i].radioframe_alloc_period =
        (isrran::mbsfn_sf_cfg_t::alloc_period_t)cfg.sibs[1].sib2().mbsfn_sf_cfg_list[i].radioframe_alloc_period.value;
  ...
  isrran::sib13_t sibs13;
  sibs13.notif_cfg.notif_offset = cfg.sibs[12].sib13_v920().notif_cfg_r9.notif_offset_r9;
  sibs13.notif_cfg.notif_repeat_coeff =
      (isrran::mbms_notif_cfg_t::coeff_t)cfg.sibs[12].sib13_v920().notif_cfg_r9.notif_repeat_coeff_r9.value;
  ...
  for (uint32_t i = 0; i < sibs13.nof_mbsfn_area_info; i++) {
    ...
    sibs13.mbsfn_area_info_list[i].mcch_cfg.sig_mcs = (isrran::mbsfn_area_info_t::mcch_cfg_t::sig_mcs_t)cfg.sibs[12]
                                                          .sib13_v920()
    ...
    sibs13.mbsfn_area_info_list[i].mcch_cfg.mcch_repeat_period =
        (isrran::mbsfn_area_info_t::mcch_cfg_t::repeat_period_t)cfg.sibs[12]
            .sib13_v920()
            .mbsfn_area_info_list_r9[i]
    ...
    sibs13.mbsfn_area_info_list[i].mcch_cfg.mcch_mod_period =
        (isrran::mbsfn_area_info_t::mcch_cfg_t::mod_period_t)cfg.sibs[12]
            .sib13_v920()
            .mbsfn_area_info_list_r9[i]
            .mcch_cfg_r9.mcch_mod_period_r9.value;
    sibs13.mbsfn_area_info_list[i].non_mbsfn_region_len = (isrran::mbsfn_area_info_t::region_len_t)cfg.sibs[12]
                                                              .sib13_v920()
                                                              .mbsfn_area_info_list_r9[i]
  ...
  pack_mcch();
  isrran::mcch_msg_t mcch_t;
  mcch_t.common_sf_alloc_period         = isrran::mcch_msg_t::common_sf_alloc_period_t::rf64;
  mcch_t.nof_common_sf_alloc            = 1;
  isrran::mbsfn_sf_cfg_t sf_alloc_item  = mcch_t.common_sf_alloc[0];
  sf_alloc_item.radioframe_alloc_offset = 0;
  sf_alloc_item.radioframe_alloc_period = isrran::mbsfn_sf_cfg_t::alloc_period_t::n1;
  sf_alloc_item.sf_alloc                = 63;
  mcch_t.nof_pmch_info                  = 1;
  isrran::pmch_info_t* pmch_item        = &mcch_t.pmch_info_list[0];
  ...
  pmch_item->mch_sched_period = isrran::pmch_info_t::mch_sched_period_t::rf64;
...
int rrc::pack_mcch()
{
  ...
  isrran::plmn_id_t plmn_obj;
  plmn_obj.from_string("00003");
  isrran::to_asn1(&pmch_item->mbms_session_info_list_r9[0].tmgi_r9.plmn_id_r9.explicit_value_r9(), plmn_obj);
  ...
  if (mcch.pack(bref) != asn1::ISRASN_SUCCESS) {
...
void rrc::log_rx_pdu_fail(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu, const char* cause_str)
{
  logger.error(
                            ...
                            const char*             msg_type)
{
  ...
  if (rnti != ISRRAN_PRNTI and rnti != ISRRAN_SIRNTI) {
    if (dir == Tx or dir == Rx) {
      fmt::format_to(membuf, "{} ", isrran::get_srb_name(isrran::lte_lcid_to_srb(lcid)));
    }
  ...
  logger.info(pdu.data(), pdu.size(), "%s", isrran::to_c_str(membuf));
}
