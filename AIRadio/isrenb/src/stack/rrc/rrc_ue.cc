...
#include "isrenb/hdr/stack/rrc/mac_controller.h"
#include "isrenb/hdr/stack/rrc/rrc_endc.h"
#include "isrenb/hdr/stack/rrc/rrc_mobility.h"
#include "isrenb/hdr/stack/rrc/ue_rr_cfg.h"
...
#include "isrran/interfaces/enb_s1ap_interfaces.h"
...
void rrc::ue::parse_ul_dcch(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  ul_dcch_msg_s  ul_dcch_msg;
  asn1::cbit_ref bref(pdu->msg, pdu->N_bytes);
  if (ul_dcch_msg.unpack(bref) != asn1::ISRASN_SUCCESS or
  ...
  isrran::unique_byte_buffer_t original_pdu = std::move(pdu);
  ...
  switch (ul_dcch_msg.msg.c1().type()) {
    ...
    case ul_dcch_msg_type_c::c1_c_::types::rrc_conn_recfg_complete:
      ...
      isrran::console("User 0x%x connected\n", rnti);
    ...
    case ul_dcch_msg_type_c::c1_c_::types::ue_cap_info:
      if (handle_ue_cap_info(&ul_dcch_msg.msg.c1().ue_cap_info()) == ISRRAN_SUCCESS) {
...
void rrc::ue::handle_rrc_con_setup_complete(rrc_conn_setup_complete_s* msg, isrran::unique_byte_buffer_t pdu)
{
  // Log event.
  asn1::json_writer json_writer;
  msg->to_json(json_writer);
...
void rrc::ue::handle_rrc_con_reest_req(rrc_conn_reest_request_s* msg)
{
  ...
  if (not parent->s1ap->is_mme_connected()) {
    ...
    isrran::console("RRCReestablishmentReject for rnti=0x%x. Cause: MME not connected.\n", rnti);
    return;
  }
  ...
  if (not is_idle()) {
    ...
    isrran::console("ERROR: RRCReestablishmentReject for rnti=0x%x not in RRC_IDLE\n", rnti);
    return;
  }
  ...
  if (old_ue_it == parent->users.end() or old_cell == nullptr or
    ...
    isrran::console("RRCReestablishmentReject for rnti=0x%x. Cause: no context available\n", rnti);
    return;
  }
  ...
  if (not old_ue_supported_endc and req_r8.reest_cause.value == reest_cause_opts::recfg_fail) {
    ...
    isrran::console("RRCReestablishmentReject for rnti=0x%x. Cause: Unhandled Reestablishment due to ReconfigFailure\n",
                    rnti);
    return;
  }
...
void rrc::ue::handle_rrc_con_reest_complete(rrc_conn_reest_complete_s* msg, isrran::unique_byte_buffer_t pdu)
{
  // Log event.
  asn1::json_writer json_writer;
  msg->to_json(json_writer);
 ...
 */
void rrc::ue::send_connection_reconf(isrran::unique_byte_buffer_t pdu,
...
void rrc::ue::handle_rrc_reconf_complete(rrc_conn_recfg_complete_s* msg, isrran::unique_byte_buffer_t pdu)
{
  // Inform PHY about the configuration completion
  parent->phy->complete_config(rnti);
...
void rrc::ue::handle_ue_info_resp(const asn1::rrc::ue_info_resp_r9_s& msg, isrran::unique_byte_buffer_t pdu)
{
 ...
 *
 * @return int ISRRAN_SUCCESS if unpacking was ok. ISRRAN_ERROR otherwise
 */
int rrc::ue::handle_ue_cap_info(ue_cap_info_s* msg)
{
  ...
  for (uint32_t i = 0; i < msg_r8->ue_cap_rat_container_list.size(); i++) {
    ...
    if (eutra_capabilities.unpack(bref) != asn1::ISRASN_SUCCESS) {
    ...
    ue_capabilities             = isrran::make_rrc_ue_capabilities(eutra_capabilities);
  ...
  if (eutra_capabilities_unpacked) {
    ...
    if (ue_rat_caps.pack(bref2) != asn1::ISRASN_SUCCESS) {
                        ...
                        asn1::s1ap::cause_c&                               cause)
{
  ...
  if (bearer_list.addmod_erab(erab_id, qos_params, addr, gtpu_teid_out, nas_pdu, cause) != ISRRAN_SUCCESS) {
    parent->logger.error("Couldn't add E-RAB id=%d for rnti=0x%x", erab_id, rnti);
    return ISRRAN_ERROR;
  }
  if (bearer_list.add_gtpu_bearer(erab_id) != ISRRAN_SUCCESS) {
...
bool rrc::ue::send_dl_dcch(const dl_dcch_msg_s* dl_dcch_msg, isrran::unique_byte_buffer_t pdu, std::string* octet_str)
{
  if (pdu == nullptr) {
    pdu = isrran::make_byte_buffer();
    if (pdu == nullptr) {
  ...
  asn1::bit_ref bref(pdu->msg, pdu->get_tailroom());
  if (dl_dcch_msg->pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
...
void rrc::ue::apply_setup_phy_common(const asn1::rrc::rr_cfg_common_sib_s& config, bool update_phy)
{
  ...
  set_phy_cfg_t_common_isr(&current_phy_cfg, config.isr_ul_cfg_common);
...
void rrc::ue::apply_setup_phy_config_dedicated(const asn1::rrc::phys_cfg_ded_s& phys_cfg_ded)
{
  ...
  isrran::set_phy_cfg_t_dedicated_cfg(&phy_rrc_dedicated_list[0].phy_cfg, phys_cfg_ded);
...
void rrc::ue::apply_reconf_phy_config(const rrc_conn_recfg_r8_ies_s& reconfig_r8, bool update_phy)
{
  ...
  if (reconfig_r8.rr_cfg_ded_present) {
    ...
    if (rr_cfg_ded.phys_cfg_ded_present) {
      ...
      isrran::set_phy_cfg_t_dedicated_cfg(&phy_rrc_dedicated_list[0].phy_cfg, phys_cfg_ded);
      isrran::set_phy_cfg_t_enable_64qam(
      ...
      if (reconfig_r920.non_crit_ext_present) {
        ...
        if (reconfig_r1020.scell_to_add_mod_list_r10_present) {
          ...
          for (const scell_to_add_mod_r10_s& scell : list) {
            ...
            isrran::set_phy_cfg_t_scell_config(&scell_phy_rrc_ded.phy_cfg, scell);
...
void rrc::ue::apply_pdcp_srb_updates(const rr_cfg_ded_s& pending_rr_cfg)
{
  for (const srb_to_add_mod_s& srb : pending_rr_cfg.srb_to_add_mod_list) {
    parent->pdcp->add_bearer(rnti, srb.srb_id, isrran::make_srb_pdcp_config_t(srb.srb_id, false));
...
void rrc::ue::apply_pdcp_drb_updates(const rr_cfg_ded_s& pending_rr_cfg)
{
  ...
  for (const drb_to_add_mod_s& drb : pending_rr_cfg.drb_to_add_mod_list) {
    ...
    if (drb.pdcp_cfg_present) {
      isrran::pdcp_config_t pdcp_cnfg_drb = isrran::make_drb_pdcp_config_t(drb.drb_id, false, drb.pdcp_cfg);
      parent->pdcp->add_bearer(rnti, drb.lc_ch_id, pdcp_cnfg_drb);
    } else {
      isrran::pdcp_config_t pdcp_cnfg_drb = isrran::make_drb_pdcp_config_t(drb.drb_id, false);
      parent->pdcp->add_bearer(rnti, drb.lc_ch_id, pdcp_cnfg_drb);
    }
...
void rrc::ue::apply_rlc_rb_updates(const rr_cfg_ded_s& pending_rr_cfg)
{
  for (const srb_to_add_mod_s& srb : pending_rr_cfg.srb_to_add_mod_list) {
    ...
    if (srb.srb_id == 1) {
      ...
      isrran_assertion_failure("Invalid LTE SRB id=%d", srb.srb_id);
    }

    if (srb_cfg->rlc_cfg.type() == srb_to_add_mod_s::rlc_cfg_c_::types_opts::explicit_value) {
      isrran::rlc_config_t rlc_cfg = isrran::make_rlc_config_t(srb_cfg->rlc_cfg.explicit_value());
      if (rlc_cfg.rlc_mode == isrran::rlc_mode_t::am and srb_cfg->enb_dl_max_retx_thres > 0) {
        rlc_cfg.am.max_retx_thresh = srb_cfg->enb_dl_max_retx_thres;
      }
      parent->rlc->add_bearer(rnti, srb.srb_id, rlc_cfg);
    } else {
      isrran::rlc_config_t rlc_cfg = isrran::rlc_config_t::srb_config(srb.srb_id);
      if (rlc_cfg.rlc_mode == isrran::rlc_mode_t::am and srb_cfg->enb_dl_max_retx_thres > 0) {
        rlc_cfg.am.max_retx_thresh = srb_cfg->enb_dl_max_retx_thres;
      }
      parent->rlc->add_bearer(rnti, srb.srb_id, rlc_cfg);
  ...
  for (const drb_to_add_mod_s& drb : pending_rr_cfg.drb_to_add_mod_list) {
    ...
    isrran::rlc_config_t              rlc_cfg = isrran::make_rlc_config_t(drb.rlc_cfg);
    const bearer_cfg_handler::erab_t& erab    = bearer_list.get_erabs().at(drb.eps_bearer_id);
    if (rlc_cfg.rlc_mode == isrran::rlc_mode_t::am and
    ...
    parent->bearer_manager.add_eps_bearer(rnti, drb.eps_bearer_id, isrran::isrran_rat_t::lte, drb.lc_ch_id);
  }
}
