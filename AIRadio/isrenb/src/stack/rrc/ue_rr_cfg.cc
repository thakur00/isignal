...
#include "isrenb/hdr/stack/rrc/rrc_bearer_cfg.h"
#include "isrenb/hdr/stack/rrc/rrc_cell_cfg.h"
#include "isrenb/hdr/stack/rrc/rrc_config.h"
...
add_srb(srb_to_add_mod_list_l& srbs, uint8_t srb_id, const asn1::rrc::srb_to_add_mod_s::rlc_cfg_c_& srb_cfg)
{
  if (srb_id > 2 or srb_id == 0) {
    isrlog::fetch_basic_logger("RRC").error("Invalid SRB id=%d", srb_id);
    return nullptr;
  }

  // Set SRBtoAddMod
  auto srb_it               = isrran::add_rrc_obj_id(srbs, srb_id);
...
void fill_srbs_reconf(srb_to_add_mod_list_l& srbs, const srb_to_add_mod_list_l& current_srbs, const rrc_cfg_t& enb_cfg)
{
  ...
  if (isrran::find_rrc_obj_id(current_srbs, 1) == current_srbs.end()) {
    add_srb(srbs, 1, enb_cfg.srb1_cfg.rlc_cfg);
  }

  if (isrran::find_rrc_obj_id(current_srbs, 2) == current_srbs.end()) {
...
int fill_sr_cfg_setup(sched_request_cfg_c& sr_cfg, const ue_cell_ded_list& ue_cell_list)
{
  if (ue_cell_list.get_sr_res()) {
    ...
    isrlog::fetch_basic_logger("RRC").error("SR resource is not configured.");
    return ISRRAN_ERROR;
  }
}
...
int16_t get_ri(uint32_t m_ri)
{
  ...
  switch (m_ri) {
    ...
    case 32:
      ...
      break;
    default:
      isrlog::fetch_basic_logger("RRC").error("Allocating RI: invalid m_ri=%d", m_ri);
      return -1;
  }
...
int fill_cqi_report_setup(cqi_report_cfg_s& cqi_rep, const rrc_cfg_t& enb_cfg, const ue_cell_ded_list& ue_cell_list)
{
  ...
  if (cqi_rep.cqi_report_periodic_present) {
    ...
    if (pcell == nullptr or not pcell->cqi_res_present) {
      isrlog::fetch_basic_logger("RRC").warning("PCell CQI resources haven\'t been allocated yet");
      return ISRRAN_ERROR;
    }
...
int fill_cqi_report_reconf(cqi_report_cfg_s& cqi_rep, const rrc_cfg_t& enb_cfg, const ue_cell_ded_list& ue_cell_list)
{
  ...
  if (fill_cqi_report_setup(cqi_rep, enb_cfg, ue_cell_list) == ISRRAN_ERROR) {
    return ISRRAN_ERROR;
  }
  ...
  if (cqi_rep.cqi_report_periodic_present) {
    ...
    if ((enb_cfg.antenna_info.tx_mode == ant_info_ded_s::tx_mode_e_::tm3) ||
      ...
      if (ri_idx >= 0) {
        ...
        isrlog::fetch_basic_logger("RRC").warning("Warning: Configured wrong M_ri parameter.");
        return ISRRAN_ERROR;
      }
    }
  }
...
void fill_phy_cfg_ded_enb_cfg(phys_cfg_ded_s& phy_cfg, const rrc_cfg_t& enb_cfg)
{
  ...
  phy_cfg.ul_pwr_ctrl_ded.pisr_offset          = 3;
...
int fill_phy_cfg_ded_reconf(phys_cfg_ded_s&                      phy_cfg,
                            ...
                            const isrran::rrc_ue_capabilities_t& ue_caps)
{
                           ...
                           bool                                 phy_cfg_updated)
{
  ...
  isrran::compute_cfg_diff(current_rr_cfg.drb_to_add_mod_list,
...
void apply_rr_cfg_ded_diff(rr_cfg_ded_s& current_rr_cfg_ded, const rr_cfg_ded_s& pending_rr_cfg_ded)
{
  ...
  if (pending_rr_cfg_ded.srb_to_add_mod_list_present) {
    ...
    isrran::apply_addmodlist_diff(current_rr_cfg_ded.srb_to_add_mod_list,
  ...
  if (pending_rr_cfg_ded.drb_to_add_mod_list_present or pending_rr_cfg_ded.drb_to_release_list_present) {
    isrran::apply_addmodremlist_diff(current_rr_cfg_ded.drb_to_add_mod_list,
                        ...
                        const isrran::rrc_ue_capabilities_t& ue_caps)
{
  ...
  for (size_t ue_cc_idx = 1; ue_cc_idx < ue_cell_list.nof_cells(); ++ue_cc_idx) {
    ...
    ul_cfg.isr_ul_cfg_common_r10                             = cc_cfg_sib.isr_ul_cfg_common;
    ...
    ul_cfg_ded.ul_pwr_ctrl_ded_scell_r10.pisr_offset_ap_r10_present = true;
    ul_cfg_ded.ul_pwr_ctrl_ded_scell_r10.pisr_offset_ap_r10         = 3;
...
#if ISR_ENABLED
    ul_cfg_ded.isr_ul_cfg_ded_r10_present   = true;
    auto& isr_setup                         = ul_cfg_ded.isr_ul_cfg_ded_r10.set_setup();
    isr_setup.isr_bw.value                  = isr_ul_cfg_ded_c::setup_s_::isr_bw_opts::bw0;
    isr_setup.isr_hop_bw.value              = isr_ul_cfg_ded_c::setup_s_::isr_hop_bw_opts::hbw0;
    isr_setup.freq_domain_position          = 0;
    isr_setup.dur                           = true;
    isr_setup.isr_cfg_idx                   = 167;
    isr_setup.tx_comb                       = 0;
    isr_setup.cyclic_shift.value            = isr_ul_cfg_ded_c::setup_s_::cyclic_shift_opts::cs0;
    ul_cfg_ded.isr_ul_cfg_ded_v1020_present = true;
    asn1::number_to_enum(ul_cfg_ded.isr_ul_cfg_ded_v1020.isr_ant_port_r10, enb_cfg.cell.nof_ports);
    ul_cfg_ded.isr_ul_cfg_ded_aperiodic_r10_present = true;
    ul_cfg_ded.isr_ul_cfg_ded_aperiodic_r10.set(setup_opts::release);
#endif // ISR_ENABLED
  }
                              ...
                              const rrc_conn_recfg_r8_ies_s&          recfg_r8)
{
  if (recfg_r8.non_crit_ext_present or recfg_r8.non_crit_ext.non_crit_ext_present or
    ...
    isrran::apply_addmodremlist_diff(
