  ...
  for (auto& cipher_item : cfg->eea_preference_list) {
    ...
    switch (cipher_item) {
      case isrran::CIPHERING_ALGORITHM_ID_EEA0:
        ...
        sec_cfg.cipher_algo = isrran::CIPHERING_ALGORITHM_ID_EEA0;
        enc_algo_found      = true;
        logger.info("Selected EEA0 as RRC encryption algorithm");
        break;
      case isrran::CIPHERING_ALGORITHM_ID_128_EEA1:
        // “first bit” – 128-EEA1,
        if (v.get(v.length() - isrran::CIPHERING_ALGORITHM_ID_128_EEA1)) {
          sec_cfg.cipher_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA1;
          enc_algo_found      = true;
      ...
      case isrran::CIPHERING_ALGORITHM_ID_128_EEA2:
        // “second bit” – 128-EEA2,
        if (v.get(v.length() - isrran::CIPHERING_ALGORITHM_ID_128_EEA2)) {
          sec_cfg.cipher_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA2;
          enc_algo_found      = true;
      ...
      case isrran::CIPHERING_ALGORITHM_ID_128_EEA3:
        // “third bit” – 128-EEA3,
        if (v.get(v.length() - isrran::CIPHERING_ALGORITHM_ID_128_EEA3)) {
          sec_cfg.cipher_algo = isrran::CIPHERING_ALGORITHM_ID_128_EEA3;
          enc_algo_found      = true;
  ...
  for (auto& eia_enum : cfg->eia_preference_list) {
    ...
    switch (eia_enum) {
      case isrran::INTEGRITY_ALGORITHM_ID_EIA0:
        ...
        sec_cfg.integ_algo = isrran::INTEGRITY_ALGORITHM_ID_EIA0;
        integ_algo_found   = true;
        break;
      case isrran::INTEGRITY_ALGORITHM_ID_128_EIA1:
        // “first bit” – 128-EIA1,
        if (v.get(v.length() - isrran::INTEGRITY_ALGORITHM_ID_128_EIA1)) {
          sec_cfg.integ_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA1;
          integ_algo_found   = true;
      ...
      case isrran::INTEGRITY_ALGORITHM_ID_128_EIA2:
        // “second bit” – 128-EIA2,
        if (v.get(v.length() - isrran::INTEGRITY_ALGORITHM_ID_128_EIA2)) {
          sec_cfg.integ_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA2;
          integ_algo_found   = true;
      ...
      case isrran::INTEGRITY_ALGORITHM_ID_128_EIA3:
        // “third bit” – 128-EIA3,
        if (v.get(v.length() - isrran::INTEGRITY_ALGORITHM_ID_128_EIA3)) {
          sec_cfg.integ_algo = isrran::INTEGRITY_ALGORITHM_ID_128_EIA3;
          integ_algo_found   = true;
...
void security_cfg_handler::generate_as_keys()
{
  ...
  isrran::security_generate_k_rrc(
...
void security_cfg_handler::regenerate_keys_handover(uint32_t new_pci, uint32_t new_dl_earfcn)
{
  ...
  isrran::security_generate_k_enb_star(k_enb, new_pci, new_dl_earfcn, k_enb_star);
...
bearer_cfg_handler::bearer_cfg_handler(uint16_t rnti_, const rrc_cfg_t& cfg_, gtpu_interface_rrc* gtpu_) :
  rnti(rnti_), cfg(&cfg_), gtpu(gtpu_), logger(&isrlog::fetch_basic_logger("RRC"))
{}
                                    ...
                                    asn1::s1ap::cause_c&                               cause)
{
  ...
  if (drbid > isrran::MAX_LTE_DRB_ID) {
  ...
  if (erabs.count(erab_id) > 0) {
    for (auto& drb : current_drbs) {
      if (drb.eps_bearer_id_present and drb.eps_bearer_id == erab_id) {
        isrran::rem_rrc_obj_id(current_drbs, drb.drb_id);
        break;
      }
    }
  }
  ...
  // Set DRBtoAddMod
  auto drb_it                                              = isrran::add_rrc_obj_id(current_drbs, drbid);
...
int bearer_cfg_handler::release_erab(uint8_t erab_id)
{
  ...
  isrran::rem_rrc_obj_id(current_drbs, (uint8_t)drb_id);
...
int bearer_cfg_handler::add_gtpu_bearer(uint32_t erab_id)
{
  auto it = erabs.find(erab_id);
  if (it != erabs.end()) {
    isrran::expected<uint32_t> teidin =
...
isrran::expected<uint32_t> bearer_cfg_handler::add_gtpu_bearer(uint32_t                                erab_id,
                                                               uint32_t                                teid_out,
                                                               uint32_t                                addr,
                                                               const gtpu_interface_rrc::bearer_props* props)
{
  ...
  bearer.addr                       = addr;
  isrran::expected<uint32_t> teidin = gtpu->add_bearer(rnti, erab.id, addr, teid_out, addr_in, props);
  if (teidin.is_error()) {
    logger->error("Adding erab_id=%d to GTPU", erab_id);
    return isrran::default_error_t();
  }
