...
#include "isrenb/hdr/stack/rrc/ue_meas_cfg.h"
...
{
  static isrlog::basic_logger& logger = isrlog::fetch_basic_logger("RRC");
  // Compute shortMAC-I
  ...
  if (var_short_mac.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) { // already zeroed, so no need to align
  ...
  switch (integ_algo) {
    ...
    case isrran::INTEGRITY_ALGORITHM_ID_128_EIA1:
      isrran::security_128_eia1(&k_rrc_int[16],
    ...
    case isrran::INTEGRITY_ALGORITHM_ID_128_EIA2:
      isrran::security_128_eia2(&k_rrc_int[16],
                                0xffffffff, // 32-bit all to ones
                                0x1f,       // 5-bit all to ones
                                1,          // 1-bit to one
                                varShortMAC_packed,
      ...
      break;
    default:
      isrran::console_stderr("ERROR: Unsupported integrity algorithm %d.\n", integ_algo);
  }
                                         ...
                                         asn1::s1ap::cause_c&                                              cause)
{
  ...
  ue_cfg.supported_cc_list[0].dl_cfg.tm  = ISRRAN_TM1;
  ...
  if (add_user(rnti, ue_cfg) != ISRRAN_SUCCESS) {
...
void rrc::ue::rrc_mobility::handle_ue_meas_report(const meas_report_s& msg, isrran::unique_byte_buffer_t pdu)
{
  ...
  auto                 measid_it    = isrran::find_rrc_obj_id(measid_list, meas_res.meas_id);
  ...
  auto              obj_it = isrran::find_rrc_obj_id(measobj_list, measid_it->meas_obj_id);
                                                 ...
                                                 bool     fwd_direct_path_available)
{
  isrran::plmn_id_t target_plmn =
      isrran::make_plmn_id_t(rrc_enb->cfg.sib1.cell_access_related_info.plmn_id_list[0].plmn_id);
  ...
  if (not rrc_ue->eutra_capabilities_unpacked) {
    ...
    capitem.feature_group_inds.from_number(0xe6041000); // 0x5d0ffc80); // 0xe6041c00;
    {
      ...
      if (capitem.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
    ...
    if (rrc_ue->eutra_capabilities.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
      logger.error("Failed to pack UE EUTRA Capability");
      return false;
    }
  ...
  asn1::bit_ref bref(buffer->msg, buffer->get_tailroom());
  if (hoprep.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
                                                           ...
                                                           isrran::unique_byte_buffer_t container)
{
  ...
  asn1::rrc::ho_cmd_s rrchocmd;
  {
    ...
    if (rrchocmd.unpack(bref) != asn1::ISRASN_SUCCESS) {
                                                        ...
                                                        uint32_t                  src_pci)
{
  ...
  mob_info.rr_cfg_common.isr_ul_cfg_common_present  = true;
  mob_info.rr_cfg_common.isr_ul_cfg_common          = target_cell.sib2.rr_cfg_common.isr_ul_cfg_common;
...
{
  isrran::console("Starting S1 Handover of rnti=0x%x to cellid=0x%x.\n", rrc_ue->rnti, ev.target_eci);
...
void rrc::ue::rrc_mobility::s1_source_ho_st::handle_ho_cmd(wait_ho_cmd& s, const ho_cmd_msg& ho_cmd)
{
  ...
  asn1::rrc::dl_dcch_msg_s dl_dcch_msg;
  {
    ...
    if (dl_dcch_msg.unpack(bref) != asn1::ISRASN_SUCCESS) {
...
void rrc::ue::rrc_mobility::handle_ho_requested(idle_st& s, const ho_req_rx_ev& ho_req)
{
  ...
  asn1::rrc::ho_prep_info_s hoprep;
  if (hoprep.unpack(bref) != asn1::ISRASN_SUCCESS) {
  ...
  isrran::unique_byte_buffer_t ho_cmd_pdu = isrran::make_byte_buffer();
  ...
  if (dl_dcch_msg.pack(bref2) != asn1::ISRASN_SUCCESS) {
  ...
  if (ho_cmd.pack(bref2) != asn1::ISRASN_SUCCESS) {
    logger.error("Failed to pack HandoverCommand");
    cause.set_protocol().value = asn1::s1ap::cause_protocol_opts::transfer_syntax_error;
    trigger(ho_failure_ev{cause});
    return;
  ...
  for (const auto& erab : rrc_ue->bearer_list.get_erabs()) {
    ...
    isrran::uint32_to_uint8(erab.second.teid_in, admitted_erab.gtp_teid.data());
    ...
    if (ho_req.transparent_container->erab_info_list_present) {
      ...
      if (fwd_erab.dl_forwarding_present and
        ...
        isrran::expected<uint32_t> dl_teid_in = rrc_ue->bearer_list.add_gtpu_bearer(
        ...
        isrran::uint32_to_uint8(dl_teid_in.value(), admitted_erabs.back().dl_g_tp_teid.data());
      }
    }
  }
                                              ...
                                              asn1::s1ap::cause_c&                  cause)
{
  ...
  for (const auto& erab_item : ho_req_msg->erab_to_be_setup_list_ho_req.value) {
    ...
    uint32_t teid_out = 0;
    isrran::uint8_to_uint32(erab.gtp_teid.data(), &teid_out);
    ...
    if (rrc_ue->bearer_list.add_gtpu_bearer(erab.erab_id) != ISRRAN_SUCCESS) {
      erabs_failed_to_setup.emplace_back();
      erabs_failed_to_setup.back().erab_id = erab.erab_id;
  ...
  for (const auto& cap : ho_prep.ue_radio_access_cap_info) {
    if (cap.rat_type.value == rat_type_opts::eutra) {
      ...
      if (rrc_ue->eutra_capabilities.unpack(bref) != asn1::ISRASN_SUCCESS) {
      ...
      rrc_ue->ue_capabilities             = isrran::make_rrc_ue_capabilities(rrc_ue->eutra_capabilities);
...
void rrc::ue::rrc_mobility::handle_status_transfer(s1_target_ho_st& s, const status_transfer_ev& erabs)
{
  ...
  for (const auto& erab : erabs) {
    ...
    isrran::pdcp_lte_state_t drb_state{};
    ...
    uint8_t  sn_len                     = isrran::get_pdcp_drb_sn_len(drb_it->pdcp_cfg);
...
void rrc::ue::rrc_mobility::intraenb_ho_st::enter(rrc_mobility* f, const ho_meas_report_ev& meas_report)
{
  ...
  last_temp_crnti = ISRRAN_INVALID_RNTI;
...
void rrc::ue::rrc_mobility::handle_crnti_ce(intraenb_ho_st& s, const user_crnti_upd_ev& ev)
{
  ...
  bool is_first_crnti_ce = s.last_temp_crnti == ISRRAN_INVALID_RNTI;
