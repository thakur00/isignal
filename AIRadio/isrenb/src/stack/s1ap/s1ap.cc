...
using isrran::s1ap_mccmnc_to_plmn;
using isrran::uint32_to_uint8;
...
asn1::bounded_bitstring<1, 160, true, true> addr_to_asn1(const char* addr_str)
{
  ...
  if (inet_pton(AF_INET, addr_str, addr) != 1) {
    isrran::console("Invalid addr_str: %s\n", addr_str);
    perror("inet_pton");
  }
...
void add_repeated_erab_ids(const List&                                                   list,
                           isrran::bounded_vector<erab_item_s, ASN1_S1AP_MAXNOOF_ERABS>& failed_cfg_erabs)
{
...
bool contains_erab_id(isrran::bounded_vector<erab_item_s, ASN1_S1AP_MAXNOOF_ERABS>& failed_cfg_erabs, uint16_t erab_id)
{
  erab_item_s dummy;
...
isrran::proc_outcome_t s1ap::ue::ho_prep_proc_t::init(uint32_t                     target_eci_,
                                                      uint16_t                     target_tac_,
                                                      isrran::plmn_id_t            target_plmn_,
                                                      isrran::span<uint32_t>       fwd_erabs,
                                                      isrran::unique_byte_buffer_t rrc_container_,
...
isrran::proc_outcome_t s1ap::ue::ho_prep_proc_t::react(ts1_reloc_prep_expired e)
{
  // do nothing for now
  procError("timer TS1Relocprep has expired.");
  return isrran::proc_outcome_t::error;
}
isrran::proc_outcome_t s1ap::ue::ho_prep_proc_t::react(const ho_prep_fail_s& msg)
{
  ...
  isrran::console("HO preparation Failure. Cause: %s\n", cause.c_str());
 ...
 */
isrran::proc_outcome_t s1ap::ue::ho_prep_proc_t::react(const asn1::s1ap::ho_cmd_s& msg)
{
  // update timers
  ue_ptr->ts1_reloc_prep.stop();
  ...
  if (container.unpack(bref) != asn1::ISRASN_SUCCESS) {
  ...
  rrc_container = isrran::make_byte_buffer();
...
void s1ap::ue::ho_prep_proc_t::then(const isrran::proc_state_t& result)
{
  if (result.is_error()) {
...
isrran::proc_outcome_t s1ap::s1_setup_proc_t::init()
{
...
isrran::proc_outcome_t s1ap::s1_setup_proc_t::start_mme_connection()
{
  ...
  isrran::get_background_workers().push_task(connect_callback);
  procDebug("Connection to MME requested.");

  return isrran::proc_outcome_t::yield;
}

isrran::proc_outcome_t s1ap::s1_setup_proc_t::react(const isrenb::s1ap::s1_setup_proc_t::s1connectresult& event)
{
  if (event.success) {
    ...
    if (not s1ap_ptr->setup_s1()) {
      ...
      isrran::console("S1 setup failed\n");
...
isrran::proc_outcome_t s1ap::s1_setup_proc_t::react(const isrenb::s1ap::s1_setup_proc_t::s1setupresult& event)
{
  ...
  isrran::console("S1setup failed\n");
  return isrran::proc_outcome_t::error;
}

void s1ap::s1_setup_proc_t::then(const isrran::proc_state_t& result)
{
  if (result.is_error()) {
    procInfo("Failed to initiate S1 connection. Attempting reconnection in %d seconds",
             s1ap_ptr->mme_connect_timer.duration() / 1000);
    isrran::console("Failed to initiate S1 connection. Attempting reconnection in %d seconds\n",
                    s1ap_ptr->mme_connect_timer.duration() / 1000);
    ...
    if (s1ap_ptr->args.max_s1_setup_retries > 0 && connect_count > s1ap_ptr->args.max_s1_setup_retries) {
      ...
      isrran_terminate("Error connecting to MME");
    }
...
s1ap::s1ap(isrran::task_sched_handle   task_sched_,
           isrlog::basic_logger&       logger,
           isrran::socket_manager_itf* rx_socket_handler_) :
  s1setup_proc(this),
  logger(logger),
  task_sched(task_sched_),
  rx_socket_handler(rx_socket_handler_),
  alarms_channel(isrlog::fetch_log_channel("alarms"))
{
...
void s1ap::initial_ue(uint16_t                              rnti,
                      ...
                      isrran::unique_byte_buffer_t          pdu)
{
...
void s1ap::write_pdu(uint16_t rnti, isrran::unique_byte_buffer_t pdu)
{
...
bool s1ap::connect_mme()
{
  ...
  if (not mme_socket.open_socket(
          isrran::net_utils::addr_family::ipv4, socket_type::seqpacket, isrran::net_utils::protocol_type::SCTP)) {
    return false;
  }
  ...
  auto rx_callback =
      [this](isrran::unique_byte_buffer_t pdu, const sockaddr_in& from, const sctp_sndrcvinfo& sri, int flags) {
  ...
  rx_socket_handler->add_socket_handler(mme_socket.fd(),
                                        isrran::make_sctp_sdu_handler(logger, mme_task_queue, rx_callback));
...
bool s1ap::handle_mme_rx_msg(isrran::unique_byte_buffer_t pdu,
                             ...
                             int                          flags)
{
  ...
  if (flags & MSG_NOTIFICATION) {
    ...
    if (notification->sn_header.sn_type == SCTP_SHUTDOWN_EVENT) {
      ...
      isrran::console("SCTP Association Shutdown. Association: %d\n", sri.sinfo_assoc_id);
      restart_s1 = true;
      ...
      isrran::console("SCTP association changed. Association: %d\n", sri.sinfo_assoc_id);
    }
    if (restart_s1) {
      logger.info("Restarting S1 connection");
      isrran::console("Restarting S1 connection\n");
...
bool s1ap::handle_s1ap_rx_pdu(isrran::byte_buffer_t* pdu)
{
  ...
  if (rx_pdu.unpack(bref) != asn1::ISRASN_SUCCESS) {
  ...
  log_s1ap_msg(rx_pdu, isrran::make_span(*pdu), true);
...
bool s1ap::handle_erabreleasecommand(const erab_release_cmd_s& msg)
{
  ...
  for (const auto& item : msg->erab_to_be_released_list.value) {
    ...
    if (rrc->release_erab(u->ctxt.rnti, erab.erab_id) == ISRRAN_SUCCESS) {
      updated_erabs.push_back(erab.erab_id);
    } else {
      failed_cfg_erabs.push_back(erab_item_s());
      failed_cfg_erabs.back().erab_id                         = erab.erab_id;
...
bool s1ap::handle_s1setupfailure(const asn1::s1ap::s1_setup_fail_s& msg)
{
  ...
  isrran::console("S1 Setup Failure. Cause: %s\n", cause.c_str());
  return true;
}
...
bool s1ap::handle_handover_request(const asn1::s1ap::ho_request_s& msg)
{
  ...
  isrran_assert(users.add_user(std::move(ue_ptr)) != nullptr, "Unexpected failure to create S1AP UE");
...
void s1ap::send_ho_failure(uint32_t mme_ue_s1ap_id, const asn1::s1ap::cause_c& cause)
{
  ...
  sctp_send_s1ap_pdu(tx_pdu, ISRRAN_INVALID_RNTI, "HandoverFailure");
}

bool s1ap::send_ho_req_ack(const asn1::s1ap::ho_request_s&                msg,
                           uint16_t                                       rnti,
                           uint32_t                                       enb_cc_idx,
                           isrran::unique_byte_buffer_t                   ho_cmd,
                           isrran::span<asn1::s1ap::erab_admitted_item_s> admitted_bearers,
                           isrran::const_span<asn1::s1ap::erab_item_s>    not_admitted_bearers)
{
  s1ap_pdu_c tx_pdu;
  ...
  if (transparent_container.pack(bref) != asn1::ISRASN_SUCCESS) {
...
bool s1ap::send_ue_cap_info_indication(uint16_t rnti, isrran::unique_byte_buffer_t ue_radio_cap)
{
  ue* user_ptr = users.find_ue_rnti(rnti);
  if (user_ptr == nullptr) {
    return false;
...
bool s1ap::send_error_indication(const asn1::s1ap::cause_c& cause,
                                 isrran::optional<uint32_t> enb_ue_s1ap_id,
                                 isrran::optional<uint32_t> mme_ue_s1ap_id)
{
  ...
  if (enb_ue_s1ap_id.has_value()) {
    ...
    rnti                            = user_ptr != nullptr ? user_ptr->ctxt.rnti : ISRRAN_INVALID_RNTI;
  }
...
bool s1ap::ue::send_ulnastransport(isrran::unique_byte_buffer_t pdu)
{
...
bool s1ap::ue::send_ue_cap_info_indication(isrran::unique_byte_buffer_t ue_radio_cap)
{
  asn1::s1ap::s1ap_pdu_c tx_pdu;
...
void s1ap::ue::get_erab_addr(uint16_t erab_id, transp_addr_t& transp_addr, asn1::fixed_octstring<4, true>& gtpu_teid_id)
{
  ...
  isrran_expect(ret == ISRRAN_SUCCESS, "Invalid E-RAB setup");
  ...
  if (!s1ap_ptr->args.gtp_advertise_addr.empty()) {
    if (inet_pton(AF_INET, s1ap_ptr->args.gtp_advertise_addr.c_str(), addr) != 1) {
      ...
      isrran::console("Invalid gtp_advertise_addr: %s\n", s1ap_ptr->args.gtp_advertise_addr.c_str());
      perror("inet_pton");
    }
  } else {
    if (inet_pton(AF_INET, s1ap_ptr->args.gtp_bind_addr.c_str(), addr) != 1) {
      ...
      isrran::console("Invalid gtp_bind_addr: %s\n", s1ap_ptr->args.gtp_bind_addr.c_str());
      perror("inet_pton");
    }
  }
  for (uint32_t j = 0; j < 4; ++j) {
...
bool s1ap::send_ho_required(uint16_t                     rnti,
                            uint32_t                     target_eci,
                            uint16_t                     target_tac,
                            isrran::plmn_id_t            target_plmn,
                            isrran::span<uint32_t>       fwd_erabs,
                            isrran::unique_byte_buffer_t rrc_container,
                            bool                         has_direct_fwd_path)
{
  if (!mme_connected) {
    return false;
...
{
  static isrlog::basic_logger& logger = isrlog::fetch_basic_logger("S1AP");
...
{
  if (not mme_connected and rnti != ISRRAN_INVALID_RNTI) {
  ...
  if (tx_pdu.pack(bref) != asn1::ISRASN_SUCCESS) {
  ...
  uint16_t streamid = rnti == ISRRAN_INVALID_RNTI ? NONUE_STREAM_ID : users.find_ue_rnti(rnti)->stream_id;
...
void s1ap::start_pcap(isrran::s1ap_pcap* pcap_)
{
  pcap = pcap_;
}
/*******************************************************************************
...
void s1ap::log_s1ap_msg(const asn1::s1ap::s1ap_pdu_c& msg, isrran::const_span<uint8_t> sdu, bool is_rx)
{
  const char* msg_type;
