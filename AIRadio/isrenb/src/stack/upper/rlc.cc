...
void rlc::add_user(uint16_t rnti)
{
  ...
  if (users.count(rnti) == 0) {
    auto obj = make_rnti_obj<isrran::rlc>(rnti, logger.id().c_str());
    obj->init(&users[rnti],
              &users[rnti],
              timers,
...
void rlc::clear_buffer(uint16_t rnti)
{
  ...
  if (users.count(rnti)) {
    ...
    for (int i = 0; i < ISRRAN_N_RADIO_BEARERS; i++) {
...
void rlc::add_bearer(uint16_t rnti, uint32_t lcid, const isrran::rlc_config_t& cnfg)
{
  pthread_rwlock_rdlock(&rwlock);
  if (users.count(rnti)) {
...
void rlc::write_sdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
  pthread_rwlock_rdlock(&rwlock);
  if (users.count(rnti)) {
    if (rnti != ISRRAN_MRNTI) {
...
void rlc::user_interface::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
...
void rlc::user_interface::notify_delivery(uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sns)
{
  pdcp->notify_delivery(rnti, lcid, pdcp_sns);
}

void rlc::user_interface::notify_failure(uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sns)
{
  pdcp->notify_failure(rnti, lcid, pdcp_sns);
}

void rlc::user_interface::write_pdu_bcch_bch(isrran::unique_byte_buffer_t sdu)
{
  ERROR("Error: Received BCCH from ue=%d", rnti);
}

void rlc::user_interface::write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t sdu)
{
  ERROR("Error: Received BCCH from ue=%d", rnti);
}

void rlc::user_interface::write_pdu_pcch(isrran::unique_byte_buffer_t sdu)
{
