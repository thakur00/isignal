...
gtpu_tunnel_manager::gtpu_tunnel_manager(isrran::task_sched_handle task_sched_, isrlog::basic_logger& logger) :
...
isrran::span<gtpu_tunnel_manager::bearer_teid_pair>
gtpu_tunnel_manager::find_rnti_bearer_tunnels(uint16_t rnti, uint32_t eps_bearer_id)
{
  ...
  return isrran::span<bearer_teid_pair>(&(*bearer_it_begin), &(*bearer_it_end));
}
...
bool gtpu_tunnel_manager::update_rnti(uint16_t old_rnti, uint16_t new_rnti)
{
  ...
  isrran::bounded_vector<uint32_t, MAX_TUNNELS_PER_UE> to_remove;
...
bool gtpu_tunnel_manager::remove_tunnel(uint32_t teidin)
{
  ...
  isrran_assert(bearer_it->teid == tun.teid_in and bearer_it->eps_bearer_id == tun.eps_bearer_id,
...
void gtpu_tunnel_manager::activate_tunnel(uint32_t teid)
{
  ...
  auto lower_sn = [](const std::pair<uint32_t, isrran::unique_byte_buffer_t>& lhs,
                     const std::pair<uint32_t, isrran::unique_byte_buffer_t>& rhs) { return lhs.first < rhs.first; };
...
void gtpu_tunnel_manager::buffer_pdcp_sdu(uint32_t teid, uint32_t pdcp_sn, isrran::unique_byte_buffer_t sdu)
{
  tunnel& rx_tun = tunnels[teid];

  isrran_assert(rx_tun.state == tunnel_state::buffering, "Buffering of PDCP SDUs only enabled when PDCP is not active");
...
void gtpu_tunnel_manager::setup_forwarding(uint32_t rx_teid, uint32_t tx_teid)
{
  ...
  isrran::gtpu_ntoa(addrbuf, htonl(rx_tun.spgw_addr));
  fmt::format_to(addrbuf, ":0x{:x} > ", rx_tun.teid_out);
  isrran::gtpu_ntoa(addrbuf, htonl(tx_tun.spgw_addr));
...
gtpu::gtpu(isrran::task_sched_handle   task_sched_,
...
int gtpu::init(const gtpu_args_t& gtpu_args, pdcp_interface_gtpu* pdcp_)
{
  ...
  if (not net_utils::bind_addr(fd, gtp_bind_addr.c_str(), GTPU_PORT, &bindaddr)) {
    ...
    isrran::console("Failed to bind on address %s, port %d: %s\n", gtp_bind_addr.c_str(), int(GTPU_PORT), errbuf);
    return ISRRAN_ERROR;
  }

  // Assign a handler to rx S1U packets
  auto rx_callback = [this](isrran::unique_byte_buffer_t pdu, const sockaddr_in& from) {
    handle_gtpu_s1u_rx_packet(std::move(pdu), from);
  };
  rx_socket_handler->add_socket_handler(fd, isrran::make_sdu_handler(logger, gtpu_queue, rx_callback));
...
// gtpu_interface_pdcp
void gtpu::write_pdu(uint16_t rnti, uint32_t eps_bearer_id, isrran::unique_byte_buffer_t pdu)
{
  isrran::span<gtpu_tunnel_manager::bearer_teid_pair> teids = tunnels.find_rnti_bearer_tunnels(rnti, eps_bearer_id);
  if (teids.empty()) {
  ...
  log_message(tx_tun, false, isrran::make_span(pdu));
  send_pdu_to_tunnel(tx_tun, std::move(pdu));
}

void gtpu::send_pdu_to_tunnel(const gtpu_tunnel& tx_tun, isrran::unique_byte_buffer_t pdu, int pdcp_sn)
{
...
isrran::expected<uint32_t> gtpu::add_bearer(uint16_t            rnti,
                                            ...
                                            const bearer_props* props)
{
  ...
  if (props != nullptr) {
    ...
    if (props->forward_from_teidin_present) {
      if (create_dl_fwd_tunnel(props->forward_from_teidin, teid_in) != ISRRAN_SUCCESS) {
        rem_tunnel(teid_in);
        return default_error_t();
      }
    }
...
{
  isrran::span<gtpu_tunnel_manager::bearer_teid_pair> bearer_tuns =
...
void gtpu::handle_gtpu_s1u_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr)
{
  isrran_assert(pdu != nullptr, "Called with null PDU");
                               ...
                               isrran::unique_byte_buffer_t pdu)
{
  ...
  log_message(rx_tunnel, true, isrran::make_span(pdu));
...
void gtpu::handle_gtpu_m1u_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr)
{
...
int gtpu::create_dl_fwd_tunnel(uint32_t rx_teid_in, uint32_t tx_teid_in)
{
  ...
  std::map<uint32_t, isrran::unique_byte_buffer_t> pdus = pdcp->get_buffered_pdus(rx_tun->rnti, rx_tun->eps_bearer_id);
  for (auto& pdu_pair : pdus) {
    uint32_t pdcp_sn = pdu_pair.first;
    log_message(*tx_tun, false, isrran::make_span(pdu_pair.second), pdcp_sn);
...
void gtpu::log_message(const gtpu_tunnel& tun, bool is_rx, isrran::span<uint8_t> pdu, int pdcp_sn)
{
  ...
  isrran::gtpu_ntoa(addrbuf, htonl(tun.spgw_addr));
  if (is_rx) {
    dir = "Rx";
    fmt::format_to(strbuf2, "{}:0x{:0x} > ", isrran::to_c_str(addrbuf), tun.teid_in);
    switch (tun.state) {
      case gtpu_tunnel_manager::tunnel_state::buffering:
      ...
      case gtpu_tunnel_manager::tunnel_state::forward_to: {
        addrbuf.clear();
        isrran::gtpu_ntoa(addrbuf, htonl(tun.fwd_tunnel->spgw_addr));
        fmt::format_to(strbuf2, "{}:0x{:0x} (forwarded), ", isrran::to_c_str(addrbuf), tun.fwd_tunnel->teid_in);
        break;
      }
      case gtpu_tunnel_manager::tunnel_state::pdcp_active:
  ...
  if (ip_pkt->version == 4) {
    addrbuf.clear();
    strbuf2.clear();
    isrran::gtpu_ntoa(addrbuf, ip_pkt->saddr);
    isrran::gtpu_ntoa(strbuf2, ip_pkt->daddr);
    fmt::format_to(strbuf, " {} > {}", isrran::to_c_str(addrbuf), isrran::to_c_str(strbuf2));
    if (ntohs(ip_pkt->tot_len) != pdu.size()) {
      logger.error("IP Len and PDU N_bytes mismatch");
    }
  }
  logger.info(pdu.data(), pdu.size(), "%s", isrran::to_c_str(strbuf));
}
...
bool gtpu::m1u_handler::init(std::string m1u_multiaddr_, std::string m1u_if_addr_)
{
  ...
  if (inet_pton(AF_INET, m1u_multiaddr.c_str(), &mreq.imr_multiaddr) != 1) {
    ...
    isrran::console("Invalid m1u_multiaddr: %s\n", m1u_multiaddr.c_str());
    perror("inet_pton");
    return false;
  }
  ...
  if (inet_pton(AF_INET, m1u_if_addr.c_str(), &mreq.imr_interface) != 1) {
    ...
    isrran::console("Invalid m1u_if_addr: %s\n", m1u_if_addr.c_str());
    perror("inet_pton");
    return false;
  }
  ...
  parent->rx_socket_handler->add_socket_handler(m1u_sd,
                                                isrran::make_sdu_handler(logger, parent->gtpu_queue, rx_callback));

  return true;
}

void gtpu::m1u_handler::handle_rx_packet(isrran::unique_byte_buffer_t pdu, const sockaddr_in& addr)
{
  logger.debug("Received %d bytes from M1-U interface", pdu->N_bytes);

  gtpu_header_t header;
  gtpu_read_header(pdu.get(), &header, logger);
  pdcp->write_sdu(ISRRAN_MRNTI, bearer_counter, std::move(pdu));
}
