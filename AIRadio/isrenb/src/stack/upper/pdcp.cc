...
#include "isrenb/hdr/stack/upper/pdcp.h"
...
pdcp::pdcp(isrran::task_sched_handle task_sched_, isrlog::basic_logger& logger_) :
...
void pdcp::add_user(uint16_t rnti)
{
  if (users.count(rnti) == 0) {
    unique_rnti_ptr<isrran::pdcp> obj = make_rnti_obj<isrran::pdcp>(rnti, task_sched, logger.id().c_str());
...
void pdcp::add_bearer(uint16_t rnti, uint32_t lcid, const isrran::pdcp_config_t& cfg)
{
  if (users.count(rnti)) {
...
void pdcp::config_security(uint16_t rnti, uint32_t lcid, const isrran::as_security_config_t& sec_cfg)
{
  if (users.count(rnti)) {
...
{
  users[rnti].pdcp->enable_integrity(lcid, isrran::DIRECTION_TXRX);
}

void pdcp::enable_encryption(uint16_t rnti, uint32_t lcid)
{
  users[rnti].pdcp->enable_encryption(lcid, isrran::DIRECTION_TXRX);
}

bool pdcp::get_bearer_state(uint16_t rnti, uint32_t lcid, isrran::pdcp_lte_state_t* state)
{
  if (users.count(rnti) == 0) {
    return false;
  }
...
bool pdcp::set_bearer_state(uint16_t rnti, uint32_t lcid, const isrran::pdcp_lte_state_t& state)
{
  if (users.count(rnti) == 0) {
    return false;
  }
...
void pdcp::notify_delivery(uint16_t rnti, uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sns)
{
  if (users.count(rnti)) {
...
void pdcp::notify_failure(uint16_t rnti, uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sns)
{
  if (users.count(rnti)) {
...
void pdcp::write_sdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu, int pdcp_sn)
{
  if (users.count(rnti)) {
    if (rnti != ISRRAN_MRNTI) {
...
std::map<uint32_t, isrran::unique_byte_buffer_t> pdcp::get_buffered_pdus(uint16_t rnti, uint32_t lcid)
{
  if (users.count(rnti)) {
...
void pdcp::write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
  if (users.count(rnti)) {
...
void pdcp::user_interface_gtpu::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  gtpu->write_pdu(rnti, lcid, std::move(pdu));
}

void pdcp::user_interface_rlc::write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
...
void pdcp::user_interface_rrc::write_pdu(uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
...
void pdcp::user_interface_rrc::write_pdu_bcch_bch(isrran::unique_byte_buffer_t pdu)
{
  ERROR("Error: Received BCCH from ue=%d", rnti);
}

void pdcp::user_interface_rrc::write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu)
{
  ERROR("Error: Received BCCH from ue=%d", rnti);
}

void pdcp::user_interface_rrc::write_pdu_pcch(isrran::unique_byte_buffer_t pdu)
{
...
{
  return isrenb::get_rb_name(lcid);
}
