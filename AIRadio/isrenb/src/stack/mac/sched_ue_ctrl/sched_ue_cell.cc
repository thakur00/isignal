...
#include "isrenb/hdr/stack/mac/sched_ue_ctrl/sched_ue_cell.h"
...
int sched_ue_cell::get_ul_cqi() const
{
  ...
  return isrran_cqi_from_snr(snr + ul_snr_coeff);
}
...
uint32_t sched_ue_cell::get_aggr_level(uint32_t nof_bits) const
{
  ...
  return isrenb::get_aggr_level(nof_bits,
                                dl_cqi,
...
tbs_info cqi_to_tbs_dl(const sched_ue_cell& cell,
                       ...
                       uint32_t             nof_re,
                       isrran_dci_format_t  dci_format,
                       uint32_t             req_bytes)
{
  bool     use_tbs_index_alt = cell.get_ue_cfg()->use_tbs_index_alt and dci_format != ISRRAN_DCI_FORMAT1A;
  ...
  if (cell.fixed_mcs_dl < 0 or not cell.dl_cqi().is_cqi_info_received()) {
    ...
    // If coderate > ISRRAN_MIN(max_coderate, 0.932 * Qm) we should set TBS=0. We don't because it's not correctly
                        ...
                        uint32_t             req_bytes)
{
  ...
  std::tuple<uint32_t, int, uint32_t, int> ret = false_position_method(
      1U, cell.cell_cfg->nof_prb(), (int)req_bytes, compute_tbs_approx, [](int y) { return y == ISRRAN_ERROR; });
...
uint32_t get_required_prb_ul(const sched_ue_cell& cell, uint32_t req_bytes)
{
  ...
  auto compute_tbs_approx = [&cell](uint32_t nof_prb) {
    const uint32_t N_isr  = 0;
    uint32_t       nof_re = (2 * (ISRRAN_CP_NSYMB(cell.cell_cfg->cfg.cell.cp) - 1) - N_isr) * nof_prb * ISRRAN_NRE;
  ...
  std::tuple<uint32_t, int, uint32_t, int> ret =
      false_position_method(1U, max_prbs, target_tbs, compute_tbs_approx, [](int y) { return y == ISRRAN_ERROR; });
  ...
  while (!isrran_dft_precoding_valid_prb(req_prbs) && req_prbs < cell.cell_cfg->nof_prb()) {
    req_prbs++;
  }
  return req_prbs;
}
...
bool find_optimal_rbgmask(const sched_ue_cell&       ue_cell,
                          ...
                          isrran_dci_format_t        dci_format,
                          isrran::interval<uint32_t> req_bytes,
                          tbs_info&                  tb,
                          rbgmask_t&                 newtxmask)
{
  // Find the largest set of available RBGs possible
  newtxmask = find_available_rbgmask(dl_mask.size(), dci_format == ISRRAN_DCI_FORMAT1A, dl_mask);
  ...
  if (not ue_cell.dl_cqi().subband_cqi_enabled()) {
    ...
    isrran::bounded_vector<tbs_info, MAX_NOF_RBGS> tb_table(newtxmask.count());
    ...
    std::tuple<uint32_t, int, uint32_t, int> ret = false_position_method(
        1U, tb_table.size(), (int)req_bytes.stop(), compute_tbs_approx, [](int y) { return y == ISRRAN_ERROR; });
