...
#include "isrenb/hdr/stack/mac/sched_ue_ctrl/sched_lch.h"
...
{
  if (base_type::dl_buffer_state(lcid, tx_queue, prio_tx_queue) == ISRRAN_SUCCESS) {
...
{
  if (base_type::ul_bsr(lcg_id, val) == ISRRAN_SUCCESS) {
    if (logger.debug.enabled()) {
      fmt::memory_buffer str_buffer;
      fmt::format_to(str_buffer, "{}", lcg_bsr);
      logger.debug(
          "SCHED: rnti=0x%x, lcg_id=%d, bsr=%d. Current state=%s", rnti, lcg_id, val, isrran::to_c_str(str_buffer));
    }
  }
}
...
uint32_t allocate_mac_ces(sched_interface::dl_sched_data_t* data, lch_ue_manager& lch_handler, uint32_t total_tbs)
{
  ...
  while (not lch_handler.pending_ces.empty() and data->nof_pdu_elems[0] < sched_interface::MAX_RLC_PDU_LIST) {
    int toalloc = isrran::ce_total_size(lch_handler.pending_ces.front());
    if (rem_tbs < toalloc) {
      break;
    }
