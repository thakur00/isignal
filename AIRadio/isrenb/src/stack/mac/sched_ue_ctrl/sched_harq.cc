...
#include "isrran/common/log_helper.h"
...
harq_proc::harq_proc() : logger(&isrlog::fetch_basic_logger("MAC")) {}
...
{
  isrran_assert(pending_phich, "pop_pending_phich called for HARQ with no pending PHICH");
...
std::tuple<uint32_t, int, int> harq_entity::set_ack_info(tti_point tti_rx, uint32_t tb_idx, bool ack)
{
  for (auto& h : dl_harqs) {
    if (h.get_tti() + FDD_HARQ_DELAY_DL_MS == tti_rx) {
      if (h.set_ack(tb_idx, ack) == ISRRAN_SUCCESS) {
