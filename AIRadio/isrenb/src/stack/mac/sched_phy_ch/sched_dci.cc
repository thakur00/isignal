...
{
  static isrlog::basic_logger& logger = isrlog::fetch_basic_logger("MAC");
  return logger;
}
                             ...
                             int&     tbs_idx)
{
  ...
  tbs_idx              = isrran_ra_tbs_to_table_idx(max_tbs, nof_prb, max_tbs_idx);
  if (tbs_idx <= 0) {
    return ISRRAN_ERROR;
  }
  ...
  mcs = isrran_ra_mcs_from_tbs_idx(tbs_idx, use_tbs_index_alt, is_ul);
  if (mcs < 0) {
    return ISRRAN_ERROR;
  }
  if (mcs > (int)max_mcs) {
    // bound mcs
    mcs     = max_mcs;
    tbs_idx = isrran_ra_tbs_idx_from_mcs(mcs, use_tbs_index_alt, is_ul);
  }
...
tbs_info compute_mcs_and_tbs(uint32_t nof_prb,
                             ...
                             bool     use_tbs_index_alt)
{
  float max_coderate = isrran_cqi_to_coderate(std::min(cqi + 1U, 15U), use_tbs_index_alt);
                             ...
                             bool     use_tbs_index_alt)
{
  ...
  do {
    ...
    int tbs_idx = 0;
    if (compute_mcs_from_max_tbs(nof_prb, max_tbs, max_mcs, is_ul, use_tbs_index_alt, mcs, tbs_idx) != ISRRAN_SUCCESS) {
      return tbs_info{};
    }
    ...
    int   tbs      = isrran_ra_tbs_from_idx(tbs_idx, nof_prb);
    float coderate = isrran_coderate(tbs, nof_re);

    // update max coderate based on mcs
    isrran_mod_t mod = (is_ul) ? isrran_ra_ul_mod_from_mcs(mcs) : isrran_ra_dl_mod_from_mcs(mcs, use_tbs_index_alt);
    uint32_t     Qm  = isrran_mod_bits_x_symbol(mod);
...
int generate_ra_bc_dci_format1a_common(isrran_dci_dl_t&           dci,
                                       uint16_t                   rnti,
                                       tti_point                  tti_tx_dl,
                                       uint32_t                   req_bytes,
                                       ...
                                       uint32_t                   current_cfi)
{
  ...
  for (uint32_t i = 0; i < 27; i++) {
    if (isrran_ra_tbs_from_idx(i, 2) >= tbs) {
      dci.type2_alloc.n_prb1a = isrran_ra_type2_t::ISRRAN_RA_TYPE2_NPRB1A_2;
      mcs                     = i;
      tbs                     = isrran_ra_tbs_from_idx(i, 2);
      break;
    }
    if (isrran_ra_tbs_from_idx(i, 3) >= tbs) {
      dci.type2_alloc.n_prb1a = isrran_ra_type2_t::ISRRAN_RA_TYPE2_NPRB1A_3;
      mcs                     = i;
      tbs                     = isrran_ra_tbs_from_idx(i, 3);
      break;
    }
  }
  if (mcs < 0) {
  ...
  dci.type2_alloc.riv    = isrran_ra_type2_to_riv(prb_range.length(), prb_range.start(), cell_params.nof_prb());
  dci.pid                = 0;
  ...
  if (isrran_coderate(tbs, nof_re) >= max_ctrl_coderate) {
    return -1;
  }

  get_mac_logger().debug("ra_tbs=%d/%d, tbs_bytes=%d, tbs=%d, mcs=%d",
                         isrran_ra_tbs_from_idx(mcs, 2),
                         isrran_ra_tbs_from_idx(mcs, 3),
                         req_bytes,
                         tbs,
                         mcs);
                      ...
                      uint32_t                        current_cfi)
{
  ...
  int tbs_bits = generate_ra_bc_dci_format1a_common(
      bc.dci, ISRRAN_SIRNTI, tti_tx_dl, cell_params.cfg.sibs[sib_idx].len, rbg_range, cell_params, current_cfi);
  if (tbs_bits < 0) {
    return false;
  }
                         ...
                         uint32_t                        current_cfi)
{
  bc           = {};
  int tbs_bits = generate_ra_bc_dci_format1a_common(
      bc.dci, ISRRAN_PRNTI, tti_tx_dl, req_bytes, rbg_range, cell_params, current_cfi);
  if (tbs_bits < 0) {
    return false;
  }
                      ...
                      uint32_t                         current_cfi)
{
  ...
  for (uint32_t i = 0; i < nof_grants; ++i) {
    ...
    rar.msg3_grant[i].grant.rba       = isrran_ra_type2_to_riv(msg3_Lcrb, start_msg3_prb, cell_params.nof_prb());
                              ...
                              uint32_t                        current_cfi)
{
  ...
  pdcch_order.dci.format         = ISRRAN_DCI_FORMAT1A;
  pdcch_order.dci.alloc_type     = ISRRAN_RA_ALLOC_TYPE2; // TODO: is this correct?
