...
bool is_pucch_sr_collision(const isrran_pucch_cfg_t& ue_pucch_cfg, tti_point tti_tx_dl_ack, uint32_t n1_pucch)
{
  if (ue_pucch_cfg.sr_configured && isrran_ue_ul_sr_send_tti(&ue_pucch_cfg, tti_tx_dl_ack.to_uint())) {
...
bool sf_cch_allocator::alloc_dfs_node(const alloc_record& record, uint32_t start_dci_idx)
{
  ...
  node.rnti        = record.user != nullptr ? record.user->get_rnti() : ISRRAN_INVALID_RNTI;
  ...
  for (; node.dci_pos_idx < dci_pos_list.size(); ++node.dci_pos_idx) {
    ...
    if (record.alloc_type == alloc_type_t::DL_DATA and not record.pusch_uci) {
      ...
      node.pucch_n_prb = isrran_pucch_n_prb(&cc_cfg->cfg.cell, &pucch_cfg_common, 0);
