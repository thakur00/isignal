...
static isrlog::basic_logger& get_mac_logger()
{
  static isrlog::basic_logger& mac_logger = isrlog::fetch_basic_logger("MAC");
  return mac_logger;
}

const char* to_string_short(isrran_dci_format_t dcifmt)
{
  switch (dcifmt) {
...
void fill_dl_cc_result_info(custom_mem_buffer& strbuf, const dl_sched_data_t& data)
{
  ...
  for (uint32_t i = 0; i < data.nof_pdu_elems[0]; ++i) {
    if (isrran::is_mac_ce(static_cast<isrran::dl_sch_lcid>(data.pdu[i]->lcid))) {
      first_ce = i;
      break;
    }
  }
  ...
  for (uint32_t i = 0; i < data.nof_pdu_elems[0]; ++i) {
    ...
    prefix                   = (ces_found) ? " | " : "";
    isrran::dl_sch_lcid lcid = static_cast<isrran::dl_sch_lcid>(pdu.lcid);
    if (isrran::is_mac_ce(lcid)) {
      fmt::format_to(strbuf, "{}CE \"{}\"", prefix, isrran::to_string_short(lcid));
      ces_found = true;
    }
  }
...
void log_dl_cc_results(isrlog::basic_logger& logger, uint32_t enb_cc_idx, const sched_interface::dl_sched_res_t& result)
{
  ...
  if (strbuf.size() != 0) {
    if (logger.debug.enabled()) {
      logger.debug("SCHED: DL MAC PDU payload cc=%d:\n%s", enb_cc_idx, isrran::to_c_str(strbuf));
    } else {
      logger.info("SCHED: DL MAC CEs cc=%d: %s", enb_cc_idx, isrran::to_c_str(strbuf));
    }
  }
}

void log_phich_cc_results(isrlog::basic_logger&                  logger,
                          uint32_t                               enb_cc_idx,
                          const sched_interface::ul_sched_res_t& result)
{
  ...
  if (strbuf.size() != 0) {
    logger.debug("SCHED: Allocated PHICHs, cc=%d: [%s]", enb_cc_idx, isrran::to_c_str(strbuf));
  }
}
...
sched_cell_params_t::dl_nof_re_table generate_nof_re_table(const isrran_cell_t& cell)
{
  ...
  for (uint32_t cfi = 0; cfi < ISRRAN_NOF_CFI; ++cfi) {
    dl_sf.cfi = cfi + 1;
...
sched_cell_params_t::dl_lb_nof_re_table get_lb_nof_re_x_prb(const sched_cell_params_t::dl_nof_re_table& table)
{
  ...
  for (uint32_t sf_idx = 0; sf_idx < ISRRAN_NOF_SF_X_FRAME; ++sf_idx) {
    ...
    isrran::bounded_vector<uint32_t, ISRRAN_MAX_PRB> re_prb_vec(table.size());
    for (uint32_t p = 0; p < table.size(); ++p) {
      for (uint32_t s = 0; s < ISRRAN_NOF_SLOTS_PER_SF; ++s) {
        // assume max CFI to compute lower bound
        re_prb_vec[p] += table[p][sf_idx][s][ISRRAN_NOF_CFI - 1];
      }
    }

    isrran::bounded_vector<uint32_t, ISRRAN_MAX_PRB> re_prb_vec2(re_prb_vec.size());
...
void sched_cell_params_t::regs_deleter::operator()(isrran_regs_t* p)
{
  if (p != nullptr) {
    isrran_regs_free(p);
    delete p;
                                  ...
                                  const sched_interface::sched_args_t& sched_args)
{
  ...
  if (cfg.cell.nof_prb == 6) {
    ...
    if (cfg.nrb_pucch > 1) {
      isrran::console("Invalid PUCCH configuration: nrb_pucch=%d does not allow space for Msg3 transmission..\n",
                      cfg.nrb_pucch);
      return false;
    }
  ...
  if (invalid_prach) {
    ...
    isrran::console("Invalid PRACH configuration: frequency offset=%d outside bandwidth limits\n",
  ...
  // init regs
  regs.reset(new isrran_regs_t{});
  if (isrran_regs_init(regs.get(), cfg.cell) != ISRRAN_SUCCESS) {
    Error("Getting DCI locations");
    return false;
  }

  // Compute Common locations for DCI for each CFI
  for (uint32_t cfix = 0; cfix < ISRRAN_NOF_CFI; cfix++) {
  ...
  for (uint32_t cfix = 0; cfix < nof_cce_table.size(); ++cfix) {
    int ret = isrran_regs_pdcch_ncce(regs.get(), cfix + 1);
    if (ret < 0) {
  ...
  pucch_cfg_common.format            = ISRRAN_PUCCH_FORMAT_1;
  ...
  P        = isrran_ra_type0_P(cfg.cell.nof_prb);
  nof_rbgs = isrran::ceil_div(cfg.cell.nof_prb, P);
...
uint32_t sched_cell_params_t::get_dl_lb_nof_re(tti_point tti_tx_dl, uint32_t nof_prbs_alloc) const
{
  ...
  // sanity check
  assert(nof_re <= isrran_ra_dl_approx_nof_re(&cfg.cell, nof_prbs_alloc, ISRRAN_NOF_CFI));
  return nof_re;
}

uint32_t
sched_cell_params_t::get_dl_nof_res(isrran::tti_point tti_tx_dl, const isrran_dci_dl_t& dci, uint32_t cfi) const
{
  ...
  isrran_ra_dl_grant_to_grant_prb_allocation(&dci, &grant, nof_prb());
...
cce_frame_position_table generate_cce_location_table(uint16_t rnti, const sched_cell_params_t& cell_cfg)
{
  ...
  for (int cfi = 0; cfi < ISRRAN_NOF_CFI; cfi++) {
    for (int sf_idx = 0; sf_idx < ISRRAN_NOF_SF_X_FRAME; sf_idx++) {
...
void generate_cce_location(isrran_regs_t*          regs_,
                           ...
                           uint16_t                rnti)
{
  ...
  isrran_dci_location_t loc[64];
  uint32_t              nloc = 0;
  if (rnti == 0) {
    nloc = isrran_pdcch_common_locations_ncce(isrran_regs_pdcch_ncce(regs_, cfi), loc, 64);
  } else {
    nloc = isrran_pdcch_ue_locations_ncce(isrran_regs_pdcch_ncce(regs_, cfi), loc, 64, sf_idx, rnti);
  }
...
{
  float    max_coderate = isrran_cqi_to_coderate(dl_cqi, use_tbs_index_alt);
  float    factor       = 1.5;
  uint32_t l_max        = 3;
  if (cell_nof_prb == 6) {
    factor = 1.0;
  ...
  float    coderate = isrran_pdcch_coderate(nof_bits, l);
  while (factor * coderate > max_coderate and l < l_max) {
    l++;
    coderate = isrran_pdcch_coderate(nof_bits, l);
  }
