...
#include "isrenb/hdr/stack/mac/common/base_ue_buffer_manager.h"
...
template <bool isNR>
base_ue_buffer_manager<isNR>::base_ue_buffer_manager(uint16_t rnti_, isrlog::basic_logger& logger_) :
...
template <bool isNR>
void base_ue_buffer_manager<isNR>::config_lcids(isrran::const_span<mac_lc_ch_cfg_t> bearer_cfg_list)
{
  bool                                            log_enabled = logger.info.enabled();
  isrran::bounded_vector<uint32_t, MAX_NOF_LCIDS> changed_list;
  ...
  if (not changed_list.empty()) {
    ...
    logger.info("SCHED: rnti=0x%x, new lcid configuration: [%s]", rnti, isrran::to_c_str(fmtbuf));
  }
}
