...
#include "isrran/interfaces/enb_rrc_interface_mac.h"
  ...
  nof_tx_harq_proc(nof_tx_harq_proc_), nof_rx_harq_proc(nof_rx_harq_proc_)
{
  ...
  for (isrran_softbuffer_rx_t& buffer : softbuffer_rx_list) {
    isrran_softbuffer_rx_init(&buffer, nof_prb);
  }

  // Create and init Tx buffers
  softbuffer_tx_list.resize(nof_tx_harq_proc * ISRRAN_MAX_TB);
  for (auto& buffer : softbuffer_tx_list) {
    isrran_softbuffer_tx_init(&buffer, nof_prb);
  }
}
...
ue_cc_softbuffers::~ue_cc_softbuffers()
{
  for (auto& buffer : softbuffer_rx_list) {
    isrran_softbuffer_rx_free(&buffer);
  }
  softbuffer_rx_list.clear();

  for (auto& buffer : softbuffer_tx_list) {
    isrran_softbuffer_tx_free(&buffer);
  }
...
{
  for (auto& buffer : softbuffer_rx_list) {
    isrran_softbuffer_rx_reset(&buffer);
  }
  for (auto& buffer : softbuffer_tx_list) {
    isrran_softbuffer_tx_reset(&buffer);
  }
}

cc_used_buffers_map::cc_used_buffers_map() : logger(&isrlog::fetch_basic_logger("MAC")) {}
...
isrran::unique_byte_buffer_t cc_used_buffers_map::release_pdu(tti_point tti)
{
  ...
  isrran::unique_byte_buffer_t pdu = std::move(pdu_map[tti.to_uint()]);
...
uint8_t* cc_used_buffers_map::request_pdu(tti_point tti, uint32_t len)
{
  ...
  isrran_assert(len < pdu->get_tailroom(), "Requested UL pdu doesn't fit in byte_buffer");
  pdu->N_bytes = len;

  auto inserted_elem = pdu_map.insert(tti.to_uint(), std::move(pdu));
  isrran_assert(inserted_elem.has_value(), "Failure to allocate new buffer in mac::ue");
...
{
  std::unique_lock<std::mutex> lock(mutex);
  static const uint32_t        old_tti_threshold = ISRRAN_FDD_NOF_HARQ + 4;
...
cc_buffer_handler::cc_buffer_handler()
{
  for (auto& harq_buffers : tx_payload_buffer) {
    for (isrran::unique_byte_buffer_t& tb_buffer : harq_buffers) {
      tb_buffer = isrran::make_byte_buffer();
      if (tb_buffer == nullptr) {
        isrlog::fetch_basic_logger("MAC").error("Failed to allocate HARQ buffers for UE");
        return;
      }
    }
  }
 ...
 */
void cc_buffer_handler::allocate_cc(isrran::unique_pool_ptr<ue_cc_softbuffers> cc_softbuffers_)
{
  isrran_assert(empty(), "Cannot allocate softbuffers in CC that is already initialized");
...
ue::ue(uint16_t                                 rnti_,
       ...
       uint32_t                                 nof_cells_,
       isrran::obj_pool_itf<ue_cc_softbuffers>* softbuffer_pool_) :
  rnti(rnti_),
  sched(sched_),
  rrc(rrc_),
  rlc(rlc_),
...
void ue::start_pcap_net(isrran::mac_pcap_net* pcap_net_)
{
  pcap_net = pcap_net_;
}

void ue::start_pcap(isrran::mac_pcap* pcap_)
{
  pcap = pcap_;
}
...
isrran_softbuffer_rx_t* ue::get_rx_softbuffer(uint32_t enb_cc_idx, uint32_t tti)
{
...
isrran_softbuffer_tx_t* ue::get_tx_softbuffer(uint32_t enb_cc_idx, uint32_t harq_process, uint32_t tb_idx)
{
  if ((size_t)enb_cc_idx >= cc_buffers.size() or cc_buffers[enb_cc_idx].empty()) {
    ERROR("eNB CC Index (%d/%zd) out-of-range", enb_cc_idx, cc_buffers.size());
    return nullptr;
...
{
  isrran_assert(len > 0, "UE buffers: Requesting buffer for zero bytes");
...
uint32_t ue::set_ta(int ta_)
{
  ...
  int      ta_value = 0;
  do {
    ta_value = ISRRAN_MAX(-31, ISRRAN_MIN(32, ta));
    ta -= ta_value;
...
void ue::process_pdu(isrran::unique_byte_buffer_t pdu, uint32_t ue_cc_idx, uint32_t grant_nof_prbs)
{
  ...
  if (logger.info.enabled()) {
    ...
    logger.info("0x%x %s", rnti, isrran::to_c_str(str_buffer));
  }
  ...
  while (mac_msg_ul.next()) {
    ...
    if (mac_msg_ul.get()->is_sdu()) {
      ...
      if (mac_msg_ul.get()->get_sdu_lcid() == 0 && route_pdu) {
        int nbytes = isrran::sch_subh::MAC_CE_CONTRES_LEN;
...
isrran::unique_byte_buffer_t ue::release_pdu(uint32_t tti, uint32_t enb_cc_idx)
{
  return cc_buffers[enb_cc_idx].get_rx_used_buffers().release_pdu(tti_point(tti));
}

bool ue::process_ce(isrran::sch_subh* subh, uint32_t grant_nof_prbs)
{
  ...
  switch (subh->ul_sch_ce_type()) {
    case isrran::ul_sch_lcid::PHR_REPORT:
      phr = subh->get_phr();
      isrran_assert(grant_nof_prbs > 0, "Invalid nof prbs=%d provided for PHR handling", grant_nof_prbs);
      sched->ul_phr(rnti, (int)phr, grant_nof_prbs);
      metrics_phr(phr);
      break;
    case isrran::ul_sch_lcid::CRNTI:
      ...
      break;
    case isrran::ul_sch_lcid::TRUNC_BSR:
    case isrran::ul_sch_lcid::SHORT_BSR:
      ...
      is_bsr = true;
      break;
    case isrran::ul_sch_lcid::LONG_BSR:
      ...
      is_bsr = true;
      break;
    case isrran::ul_sch_lcid::PADDING:
      break;
    default:
...
void ue::allocate_sdu(isrran::sch_pdu* pdu, uint32_t lcid, uint32_t total_sdu_len)
{
  const int min_sdu_len = lcid == 0 ? 1 : 2;
  int       sdu_space   = pdu->get_sdu_space();
  if (sdu_space > 0) {
    int sdu_len = ISRRAN_MIN(total_sdu_len, (uint32_t)sdu_space);
    int n       = 1;
...
void ue::allocate_ce(isrran::sch_pdu* pdu, uint32_t lcid)
{
  switch ((isrran::dl_sch_lcid)lcid) {
    case isrran::dl_sch_lcid::TA_CMD:
      if (pdu->new_subh()) {
        uint32_t ta_cmd = 31;
      ...
      break;
    case isrran::dl_sch_lcid::CON_RES_ID:
      if (pdu->new_subh()) {
      ...
      break;
    case isrran::dl_sch_lcid::SCELL_ACTIVATION:
      if (pdu->new_subh()) {
        std::array<int, ISRRAN_MAX_CARRIERS>  active_ccs = sched->get_enb_ue_activ_cc_map(rnti);
        std::array<bool, ISRRAN_MAX_CARRIERS> active_scell_list{};
                          ...
                          uint32_t                              grant_size)
{
  ...
  uint8_t*                    ret = nullptr;
  if (enb_cc_idx < ISRRAN_MAX_CARRIERS && harq_pid < ISRRAN_FDD_NOF_HARQ && tb_idx < ISRRAN_MAX_TB) {
    isrran::byte_buffer_t* buffer = cc_buffers[enb_cc_idx].get_tx_payload_buffer(harq_pid, tb_idx);
    buffer->clear();
    mac_msg_dl.init_tx(buffer, grant_size, false);
    for (uint32_t i = 0; i < nof_pdu_elems; i++) {
      if (pdu[i].lcid <= (uint32_t)isrran::ul_sch_lcid::PHR_REPORT) {
                              ...
                              uint32_t                             grant_size)
{
  std::lock_guard<std::mutex> lock(mutex);
  uint8_t*                    ret    = nullptr;
  isrran::byte_buffer_t*      buffer = cc_buffers[0].get_tx_payload_buffer(harq_pid, 0);
  buffer->clear();
  mch_mac_msg_dl.init_tx(buffer, grant_size);

  for (uint32_t i = 0; i < nof_pdu_elems; i++) {
    if (sched_.pdu[i].lcid == (uint32_t)isrran::mch_lcid::MCH_SCHED_INFO) {
    ...
    } else if (sched_.pdu[i].lcid <= (uint32_t)isrran::mch_lcid::MTCH_MAX_LCID) {
      mch_mac_msg_dl.new_subh();
...
void ue::metrics_read(mac_ue_metrics_t* metrics_)
{
  ...
  // set PCell sector id
  std::array<int, ISRRAN_MAX_CARRIERS> cc_list = sched->get_enb_ue_cc_map(rnti);
...
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ue_metrics.phr = ISRRAN_VEC_CMA(phr, ue_metrics.phr, phr_counter);
  phr_counter++;
}
...
void ue::metrics_dl_ri(uint32_t dl_ri)
{
  ...
  if (ue_metrics.dl_ri == 0.0f) {
    ...
    ue_metrics.dl_ri = ISRRAN_VEC_EMA((float)dl_ri + 1.0f, ue_metrics.dl_ri, 0.5f);
  }
  dl_ri_counter++;
}
...
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ue_metrics.dl_pmi = ISRRAN_VEC_CMA((float)dl_ri, ue_metrics.dl_pmi, dl_pmi_counter);
  dl_pmi_counter++;
}
...
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ue_metrics.dl_cqi = ISRRAN_VEC_CMA((float)dl_cqi, ue_metrics.dl_cqi, dl_cqi_counter);
  dl_cqi_counter++;
}
...
void ue::tic()
{
  ...
  if (nof_ta_count) {
    sched->dl_mac_buffer_state(rnti, (uint32_t)isrran::dl_sch_lcid::TA_CMD, nof_ta_count);
  }
}
