...
#include "isrenb/hdr/stack/mac/sched_helpers.h"
#include "isrenb/hdr/stack/mac/sched_ue.h"
#include "isrran/common/string_helpers.h"
#include "isrran/isrlog/bundled/fmt/ranges.h"
...
sched_ue::sched_ue(uint16_t rnti_, const std::vector<sched_cell_params_t>& cell_list_params_, const ue_cfg_t& cfg_) :
  logger(isrlog::fetch_basic_logger("MAC")), rnti(rnti_), lch_handler(rnti_)
{
...
void sched_ue::set_cfg(const ue_cfg_t& cfg_)
{
  ...
  if (scell_activation_state_changed and not is_handover) {
    lch_handler.pending_ces.emplace_back(isrran::dl_sch_lcid::SCELL_ACTIVATION);
...
bool sched_ue::pdsch_enabled(isrran::tti_point tti_rx, uint32_t enb_cc_idx) const
{
...
int sched_ue::generate_dl_dci_format(uint32_t                          pid,
                                     ...
                                     const rbgmask_t&                  user_mask)
{
  isrran_dci_format_t dci_format = get_dci_format();
  int                 tbs_bytes  = 0;
                                ...
                                const rbgmask_t&                  user_mask)
{
  ...
  dci->type2_alloc.mode = isrran_ra_type2_t::ISRRAN_RA_TYPE2_LOC;
  ...
  dci->type2_alloc.riv  = isrran_ra_type2_to_riv(L_crb, RB_start, cell.nof_prb);
...
{
  isrran_assert(cells[enb_cc_idx].configured(), "computation of MCS/TBS called for non-configured CC");
  isrran::interval<uint32_t> req_bytes = get_requested_dl_bytes(enb_cc_idx);
...
{
  dl_harq_proc* h                    = &cells[enb_cc_idx].harq_ent.dl_harq_procs()[pid];
  bool          tb_en[ISRRAN_MAX_TB] = {false};
  ...
  for (uint32_t tb = 0; tb < ISRRAN_MAX_TB; tb++) {
    ...
    if (tbinfo.tbs_bytes > 0 && tb_en[tb]) {
      ...
      if (!ISRRAN_DCI_IS_TB_EN(dci->tb[tb])) {
        dci->tb[tb].rv = 2;
      }
      ...
      ISRRAN_DCI_TB_DISABLE(dci->tb[tb]);
      data->tbs[tb] = 0;
    }
  }
                               ...
                               const rbgmask_t&                  user_mask)
{
  ...
  if ((ISRRAN_DCI_IS_TB_EN(data->dci.tb[0]) + ISRRAN_DCI_IS_TB_EN(data->dci.tb[1])) == 1) {
                               ...
                               uci_pusch_t                       uci_type)
{
  ...
  if (is_newtx) {
    if (tbinfo.mcs >= 0) {
      ...
      uint32_t N_isr     = 0;
      uint32_t nof_symb  = 2 * (ISRRAN_CP_NSYMB(cell.cp) - 1) - N_isr;
      uint32_t nof_re    = nof_symb * alloc.length() * ISRRAN_NRE;
      ...
      if (uci_type != UCI_PUSCH_NONE) {
        ...
        if (uci_type == UCI_PUSCH_ACK || uci_type == UCI_PUSCH_ACK_CQI) {
          float beta = isrran_sch_beta_ack(cfg.uci_offset.I_offset_ack);
          nof_uci_re +=
              isrran_qprime_ack_ext(alloc.length(), nof_symb, 8 * tbinfo.tbs_bytes, cfg.supported_cc_list.size(), beta);
        }
        ...
        if (uci_type == UCI_PUSCH_CQI || uci_type == UCI_PUSCH_ACK_CQI || cqi_request) {
          float beta = isrran_sch_beta_cqi(cfg.uci_offset.I_offset_cqi);
          nof_uci_re += isrran_qprime_cqi_ext(alloc.length(), nof_symb, 8 * tbinfo.tbs_bytes, beta);
        }
  ...
  if (tbinfo.tbs_bytes >= 0) {
    ...
    dci->rnti           = rnti;
    dci->format         = ISRRAN_DCI_FORMAT0;
    dci->ue_cc_idx      = cells[enb_cc_idx].get_ue_cc_idx();
    dci->tb.ndi         = h->get_ndi(0);
    dci->cqi_request    = cqi_request;
    dci->freq_hop_fl    = isrran_dci_ul_t::ISRRAN_RA_PUSCH_HOP_DISABLED;
    dci->tpc_pusch      = cells[enb_cc_idx].tpc_fsm.encode_pusch_tpc();

    dci->type2_alloc.riv = isrran_ra_type2_to_riv(alloc.length(), alloc.start(), cell.nof_prb);
...
bool sched_ue::needs_cqi(uint32_t tti, uint32_t enb_cc_idx, bool will_send)
{
  ...
  if (phy_config_dedicated_enabled && cfg.supported_cc_list[0].aperiodic_cqi_period &&
    ...
    if (needscqi) {
      uint32_t interval_sent = isrran_tti_interval(tti, cqi_request_tti);
      if (interval_sent >= 16) {
        if (will_send) {
          cqi_request_tti = tti;
        }
...
rbg_interval sched_ue::get_required_dl_rbgs(uint32_t enb_cc_idx)
{
  ...
  isrran::interval<uint32_t> req_bytes  = get_requested_dl_bytes(enb_cc_idx);
  if (req_bytes == isrran::interval<uint32_t>{0, 0}) {
    return {0, 0};
  }
 ...
 * @return
 */
isrran::interval<uint32_t> sched_ue::get_requested_dl_bytes(uint32_t enb_cc_idx)
{
  ...
  if (cells[enb_cc_idx].is_pcell()) {
    if (srb0_data == 0 and not lch_handler.pending_ces.empty() and
        lch_handler.pending_ces.front() == isrran::dl_sch_lcid::CON_RES_ID) {
    ...
    for (const lch_ue_manager::ce_cmd& ce : lch_handler.pending_ces) {
      sum_ce_data += isrran::ce_total_size(ce);
    }
  }
  ...
  if (not lch_handler.pending_ces.empty() and lch_handler.pending_ces.front() == lch_ue_manager::ce_cmd::CON_RES_ID) {
    min_data += isrran::ce_total_size(lch_handler.pending_ces.front());
  }
  if (min_data == 0) {
    if (sum_ce_data > 0) {
      min_data = isrran::ce_total_size(lch_handler.pending_ces.front());
...
uint32_t sched_ue::get_expected_dl_bitrate(uint32_t enb_cc_idx, int nof_rbgs) const
{
  ...
  float max_coderate = isrran_cqi_to_coderate(std::min(cc.get_dl_cqi() + 1u, 15u), cfg.use_tbs_index_alt);

  // Inverse of isrran_coderate(tbs, nof_re)
...
uint32_t sched_ue::get_expected_ul_bitrate(uint32_t enb_cc_idx, int nof_prbs) const
{
  ...
  uint32_t N_isr        = 0;
  uint32_t nof_symb     = 2 * (ISRRAN_CP_NSYMB(cell.cp) - 1) - N_isr;
  uint32_t nof_re       = nof_symb * nof_prbs_alloc * ISRRAN_NRE;
  float    max_coderate = isrran_cqi_to_coderate(std::min(cells[enb_cc_idx].get_ul_cqi() + 1u, 15u), false);
...
{
  return isrenb::get_required_prb_ul(cells[enb_cc_idx], req_bytes);
}
...
isrran_dci_format_t sched_ue::get_dci_format()
{
  isrran_dci_format_t ret = ISRRAN_DCI_FORMAT1A;
...
std::bitset<ISRRAN_MAX_CARRIERS> sched_ue::scell_activation_mask() const
{
  std::bitset<ISRRAN_MAX_CARRIERS> ret{0};
