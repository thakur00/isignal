...
#define Error(fmt, ...) isrlog::fetch_basic_logger("MAC").error(fmt, ##__VA_ARGS__)
...
std::array<int, ISRRAN_MAX_CARRIERS> sched::get_enb_ue_cc_map(uint16_t rnti)
{
  std::array<int, ISRRAN_MAX_CARRIERS> ret{};
...
std::array<int, ISRRAN_MAX_CARRIERS> sched::get_enb_ue_activ_cc_map(uint16_t rnti)
{
  std::array<int, ISRRAN_MAX_CARRIERS> ret{};
  ret.fill(-1); // -1 for inactive & non-existent carriers
  ue_db_access_locked(
...
int sched::ul_sched(uint32_t tti, uint32_t enb_cc_idx, isrenb::sched_interface::ul_sched_res_t& sched_result)
{
  std::lock_guard<std::mutex> lock(sched_mutex);
  if (not configured) {
    return 0;
...
bool sched::is_generated(isrran::tti_point tti_rx, uint32_t enb_cc_idx) const
{
