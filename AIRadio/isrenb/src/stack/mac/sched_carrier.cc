...
#include "isrenb/hdr/stack/mac/schedulers/sched_time_pf.h"
...
bc_sched::bc_sched(const sched_cell_params_t& cfg_, isrenb::rrc_interface_mac* rrc_) :
  cc_cfg(&cfg_), rrc(rrc_), logger(isrlog::fetch_basic_logger("MAC"))
{}
...
void bc_sched::alloc_sibs(sf_sched* tti_sched)
{
  ...
  for (uint32_t sib_idx = 0; sib_idx < pending_sibs.size(); sib_idx++) {
    ...
    uint32_t nof_tx          = (sib_idx > 0) ? ISRRAN_MIN(isrran::ceil_div(cc_cfg->cfg.si_window_ms, 10), 4) : 4;
...
ra_sched::ra_sched(const sched_cell_params_t& cfg_, sched_ue_list& ue_db_) :
  cc_cfg(&cfg_), logger(isrlog::fetch_basic_logger("MAC")), ue_db(&ue_db_)
{}
...
void ra_sched::dl_sched(sf_sched* tti_sched)
{
  ...
  for (auto it = pending_rars.begin(); it != pending_rars.end();) {
    ...
    isrran::tti_interval rar_window{rar.prach_tti + PRACH_RAR_OFFSET,
...
{
  isrran::const_span<sf_sched::rar_alloc_t> alloc_rars = sf_dl_sched->get_allocated_rars();
...
void sched::carrier_sched::alloc_dl_users(sf_sched* tti_result)
{
  ...
  if (cc_cfg->nof_prb() == 6) {
    ...
    if (isrran_prach_in_window_config_fdd(cc_cfg->cfg.prach_config, tti_rx_ack.to_uint(), -1)) {
