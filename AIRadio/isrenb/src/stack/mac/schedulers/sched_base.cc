...
#include "isrenb/hdr/stack/mac/schedulers/sched_base.h"
...
alloc_result try_dl_retx_alloc(sf_sched& tti_sched, sched_ue& ue, const dl_harq_proc& h)
{
  ...
  bool   is_contiguous_alloc = ue.get_dci_format() == ISRRAN_DCI_FORMAT1A;
...
alloc_result try_dl_newtx_alloc_greedy(sf_sched& tti_sched, sched_ue& ue, const dl_harq_proc& h, rbgmask_t* result_mask)
{
  ...
  isrran::interval<uint32_t> req_bytes = ue.get_requested_dl_bytes(tti_sched.get_enb_cc_idx());
  if (req_bytes.stop() == 0) {
    return alloc_result::no_rnti_opportunity;
  }

  sched_ue_cell* ue_cell = ue.find_ue_carrier(tti_sched.get_enb_cc_idx());
  isrran_assert(ue_cell != nullptr, "dl newtx alloc called for invalid cell");
  isrran_dci_format_t dci_format = ue.get_dci_format();
  tbs_info            tb;
  rbgmask_t           opt_mask;
