...
#include "isrenb/hdr/stack/mac/sched_grid.h"
...
void sched_result_ringbuffer::new_tti(isrran::tti_point tti_rx)
{
...
void sf_grid_t::init(const sched_cell_params_t& cell_params_)
{
  ...
  isrran_pucch_cfg_t pucch_cfg  = cell_params_.pucch_cfg_common;
  ...
  if (cc_cfg->sched_cfg->pucch_harq_max_rb > 0) {
    ...
    harq_pucch = isrran_pucch_m(&pucch_cfg, cc_cfg->cfg.cell.cp) / 2 + 1;
  }
...
void sf_grid_t::new_tti(tti_point tti_rx_)
{
  ...
  if (isrran_prach_in_window_config_fdd(cc_cfg->cfg.prach_config, to_tx_ul(tti_rx).to_uint(), -1)) {
    ...
    if (logger.debug.enabled()) {
      ...
      logger.debug("%s", isrran::to_c_str(buffer));
    }
  }
...
{
  isrran_dci_format_t dci_format = user->get_dci_format();
  uint32_t            nof_bits   = isrran_dci_format_sizeof(&cc_cfg->cfg.cell, nullptr, nullptr, dci_format);
...
alloc_result sf_grid_t::alloc_ul_data(sched_ue* user, prb_interval alloc, bool needs_pdcch, bool strict)
{
  ...
  if (needs_pdcch) {
    uint32_t nof_bits = isrran_dci_format_sizeof(&cc_cfg->cfg.cell, nullptr, nullptr, ISRRAN_DCI_FORMAT0);
    uint32_t aggr_idx = user->get_aggr_level(cc_cfg->enb_cc_idx, nof_bits);
...
alloc_result sf_grid_t::reserve_ul_prbs(const prbmask_t& prbmask, bool strict)
{
  ...
  if (strict and (ul_mask & prbmask).any()) {
    if (logger.info.enabled()) {
      ...
      logger.info("%s", isrran::to_c_str(tmp_buffer));
...
bool sf_grid_t::find_ul_alloc(uint32_t L, prb_interval* alloc) const
{
  *alloc = {};
  ...
  while (!isrran_dft_precoding_valid_prb(alloc->length())) {
    alloc->resize_by(-1);
  }
...
sf_sched::sf_sched() : logger(isrlog::fetch_basic_logger("MAC")) {}
...
void sf_sched::new_tti(tti_point tti_rx_, sf_sched_result* cc_results_)
{
  ...
  if (isrran_prach_in_window_config_fdd(cc_cfg->cfg.prach_config, tti_msg3_alloc.to_uint(), -1)) {
...
bool is_periodic_cqi_expected(const sched_interface::ue_cfg_t& ue_cfg, tti_point tti_tx_ul)
{
  for (const sched_interface::ue_cfg_t::cc_cfg_t& cc : ue_cfg.supported_cc_list) {
    if (cc.dl_cfg.cqi_report.periodic_configured) {
      if (isrran_cqi_periodic_send(&cc.dl_cfg.cqi_report, tti_tx_ul.to_uint(), ISRRAN_FDD)) {
        return true;
      }
    }
  }
...
alloc_result sf_sched::alloc_dl_user(sched_ue* user, const rbgmask_t& user_mask, uint32_t pid)
{
  ...
  if (not user->pdsch_enabled(isrran::tti_point{get_tti_rx()}, cc_cfg->enb_cc_idx)) {
    return alloc_result::no_rnti_opportunity;
  }

  isrran_dci_format_t dci_format = user->get_dci_format();
  if (dci_format == ISRRAN_DCI_FORMAT1A and not is_contiguous(user_mask)) {
  ...
  if (h.is_empty()) {
    // It is newTx
    isrran::interval<uint32_t> req_bytes = user->get_requested_dl_bytes(get_enb_cc_idx());
  ...
  std::bitset<ISRRAN_MAX_CARRIERS> scells    = user->scell_activation_mask();
  ...
  if (user->nof_carriers_configured() > 1 and (ue_cc_idx == 0 or scells[ue_cc_idx]) and
    ...
    prb_interval alloc = {};
    uint32_t L = user->get_required_prb_ul(cc_cfg->enb_cc_idx, isrran::ceil_div(ISRRAN_UCI_CQI_CODED_PUCCH_B + 2, 8));
                                        ...
                                        sched_ue_list&                          ue_list)
{
  for (const auto& data_alloc : data_allocs) {
    ...
    if (tbs <= 0) {
      ...
      logger.warning("%s", isrran::to_c_str(str_buffer));
      continue;
    }
                            ...
                            uint32_t               current_enb_cc_idx)
{
  ...
  for (uint32_t enbccidx = 0; enbccidx < other_cc_results.enb_cc_list.size() and uci_alloc != UCI_PUSCH_ACK_CQI;
    ...
    const isrran_cqi_report_cfg_t& cqi_report = ue_cfg.supported_cc_list[ueccidx].dl_cfg.cqi_report;
    if (isrran_cqi_periodic_send(&cqi_report, sf_sched->get_tti_tx_ul().to_uint(), ISRRAN_FDD)) {
                                   ...
                                   sched_ue_list&                          ue_list)
{
  ...
  for (const auto& ul_alloc : ul_data_allocs) {
    ...
    isrran_dci_location_t cce_range = {0, 0};
...
alloc_result sf_sched::alloc_msg3(sched_ue* user, const sched_interface::dl_sched_rar_grant_t& rargrant)
{
  ...
  if (ret != alloc_result::success) {
    ...
    logger.warning("SCHED: Could not allocate msg3 within %s.", isrran::to_c_str(str_buffer));
  }
  return ret;
}
  ...
  for (auto& ue_pair : ue_db) {
    ...
    if (h != nullptr and not h->is_empty() and not is_ul_alloc(rnti)) {
      ...
      if (phich != phich_list.end()) {
        isrran_assert(phich->phich == phich_t::NACK, "Expected hi=0 in case of active UL HARQ that was not retx");
