...
#include "isrenb/hdr/stack/mac/mac.h"
#include "isrran/adt/pool/obj_pool.h"
...
#include "isrran/isrlog/event_trace.h"
...
mac::mac(isrran::ext_task_sched_handle task_sched_, isrlog::basic_logger& logger) :
  logger(logger), rar_payload(), common_buffers(ISRRAN_MAX_CARRIERS), task_sched(task_sched_)
{
               ...
               rrc_interface_mac*       rrc)
{
  ...
  for (auto& cc : common_buffers) {
    for (int i = 0; i < NOF_BCCH_DLSCH_MSG; i++) {
      isrran_softbuffer_tx_init(&cc.bcch_softbuffer_tx[i], args.nof_prb);
    }
    // Init softbuffer for PCCH
    isrran_softbuffer_tx_init(&cc.pcch_softbuffer_tx, args.nof_prb);

    // Init softbuffer for RAR
    isrran_softbuffer_tx_init(&cc.rar_softbuffer_tx, args.nof_prb);
  }

  // Initiate common pool of softbuffers
  uint32_t nof_prb          = args.nof_prb;
  auto     init_softbuffers = [nof_prb](void* ptr) {
    new (ptr) ue_cc_softbuffers(nof_prb, ISRRAN_FDD_NOF_HARQ, ISRRAN_FDD_NOF_HARQ);
  };
  auto recycle_softbuffers = [](ue_cc_softbuffers& softbuffers) { softbuffers.clear(); };
  softbuffer_pool.reset(new isrran::background_obj_pool<ue_cc_softbuffers>(
...
void mac::stop()
{
  isrran::rwlock_write_guard lock(rwlock);
  if (started) {
    started = false;
    ...
    for (auto& cc : common_buffers) {
      for (int i = 0; i < NOF_BCCH_DLSCH_MSG; i++) {
        isrran_softbuffer_tx_free(&cc.bcch_softbuffer_tx[i]);
      }
      isrran_softbuffer_tx_free(&cc.pcch_softbuffer_tx);
      isrran_softbuffer_tx_free(&cc.rar_softbuffer_tx);
    }
  }
}

void mac::start_pcap(isrran::mac_pcap* pcap_)
{
  isrran::rwlock_read_guard lock(rwlock);
  pcap = pcap_;
...
void mac::start_pcap_net(isrran::mac_pcap_net* pcap_net_)
{
  isrran::rwlock_read_guard lock(rwlock);
  pcap_net = pcap_net_;
  // Set pcap in all UEs for UL messages
...
int mac::ue_cfg(uint16_t rnti, const sched_interface::ue_cfg_t* cfg)
{
  ...
  if (cfg) {
    if (scheduler.ue_cfg(rnti, *cfg) == ISRRAN_ERROR) {
...
int mac::ue_set_crnti(uint16_t temp_crnti, uint16_t crnti, const sched_interface::ue_cfg_t& cfg)
{
  ...
  if (temp_crnti == crnti) {
    ...
    scheduler.dl_mac_buffer_state(crnti, (uint32_t)isrran::dl_sch_lcid::CON_RES_ID);
  }
                  ...
                  uint32_t ul_nof_prbs)
{
  ...
  isrran::unique_byte_buffer_t pdu = ue_db[rnti]->release_pdu(tti_rx, enb_cc_idx);
  if (pdu == nullptr) {
  ...
  if (crc) {
    ...
    isrran_expect(nof_bytes == pdu->size(),
                  "Inconsistent PDU length for rnti=0x%x, tti_rx=%d (%d!=%d)",
                  rnti,
                  tti_rx,
                  nof_bytes,
                  (int)pdu->size());
    auto process_pdu_task = [this, rnti, enb_cc_idx, ul_nof_prbs](isrran::unique_byte_buffer_t& pdu) {
      isrran::rwlock_read_guard lock(rwlock);
      if (check_ue_active(rnti)) {
...
int mac::ta_info(uint32_t tti, uint16_t rnti, float ta_us)
{
  ...
  if (nof_ta_count > 0) {
    return scheduler.dl_mac_buffer_state(rnti, (uint32_t)isrran::dl_sch_lcid::TA_CMD, nof_ta_count);
  }
  return ISRRAN_SUCCESS;
}
...
uint16_t mac::allocate_ue(uint32_t enb_cc_idx)
{
  ...
  do {
    ...
    // Add UE to rnti map
    isrran::rwlock_write_guard rw_lock(rwlock);
    if (not is_valid_rnti_unprotected(rnti)) {
      continue;
    }
...
uint16_t mac::reserve_new_crnti(const sched_interface::ue_cfg_t& uecfg)
{
  ...
  if (ue_cfg(rnti, &uecfg) != ISRRAN_SUCCESS) {
    return ISRRAN_INVALID_RNTI;
  }
  return rnti;
}
...
{
  static isrran::mutexed_tprof<isrran::avg_time_stats> rach_tprof("rach_tprof", "MAC", 1);
  logger.set_context(tti);
  ...
  stack_task_queue.push([this, tti, enb_cc_idx, preamble_idx, time_adv, rach_tprof_meas]() mutable {
    ...
    if (not is_po_prach) {
      ...
      uecfg.supported_cc_list[0].dl_cfg.tm      = ISRRAN_TM1;
      if (ue_cfg(rnti, &uecfg) != ISRRAN_SUCCESS) {
        return;
      }

      // Register new user in RRC
      if (rrc_h->add_user(rnti, uecfg) == ISRRAN_ERROR) {
        ue_rem(rnti);
        return;
      }
    }
    ...
    logger.info("%sRACH:  tti=%d, cc=%d, pci=%d, preamble=%d, offset=%d, temp_crnti=0x%x",
                ...
                time_adv,
                rnti);
    isrran::console("%sRACH:  tti=%d, cc=%d, pci=%d, preamble=%d, offset=%d, temp_crnti=0x%x\n",
                    (is_po_prach) ? "PDCCH order " : "",
                    tti,
                    enb_cc_idx,
                    pci,
...
int mac::get_mch_sched(uint32_t tti, bool is_mcch, dl_sched_list_t& dl_sched_res_list)
{
  ...
  isrran_dl_fill_ra_mcs(&mcs, 0, cell_config[0].cell.nof_prb, false);
  isrran_dl_fill_ra_mcs(&mcs_data, 0, cell_config[0].cell.nof_prb, false);
  if (is_mcch) {
    ...
    if (mch.current_sf_allocation_num <= mtch_stop) {
      int requested_bytes = (mcs_data.tbs / 8 > (int)mch.mtch_sched[mtch_index].lcid_buffer_size)
                                ...
                                : ((mcs_data.tbs / 8) - 2);
      int bytes_received = ue_db[ISRRAN_MRNTI]->read_pdu(current_lcid, mtch_payload_buffer, requested_bytes);
      ...
      if (bytes_received) {
        ue_db[ISRRAN_MRNTI]->metrics_tx(true, mcs.tbs);
        dl_sched_res->pdsch[0].data[0] =
            ue_db[ISRRAN_MRNTI]->generate_mch_pdu(tti % ISRRAN_FDD_NOF_HARQ, mch, 1, mcs_data.tbs / 8);
      }
    } else {
                           ...
                           uint32_t                               tti)
{
  ...
  if (pdu_len < rar_payload_len && rar_idx < rar_pdu_msg.size()) {
    isrran::rar_pdu* pdu = &rar_pdu_msg[rar_idx];
    ...
    for (uint32_t i = 0; i < nof_grants; i++) {
      isrran_dci_rar_pack(&grants[i].grant, grant_buffer);
      if (pdu->new_subh()) {
    ...
    for (uint32_t i = 0; i < sched_result.pusch.size(); i++) {
      if (sched_result.pusch[i].tbs > 0) {
        ...
        if (ue_db.contains(rnti)) {
          ...
          phy_ul_sched_res->pusch[n].pid           = TTI_RX(tti_tx_ul) % ISRRAN_FDD_NOF_HARQ;
          ...
          if (sched_result.pusch[n].current_tx_nb == 0) {
            isrran_softbuffer_rx_reset_tbs(phy_ul_sched_res->pusch[n].softbuffer_rx, sched_result.pusch[i].tbs * 8);
          }
...
void mac::write_mcch(const isrran::sib2_mbms_t* sib2_,
                     const isrran::sib13_t*     sib13_,
                     const isrran::mcch_msg_t*  mcch_,
                     ...
                     const uint8_t              mcch_payload_length)
{
  ...
  unique_rnti_ptr<ue> ue_ptr = make_rnti_obj<ue>(
      ISRRAN_MRNTI, ISRRAN_MRNTI, 0, &scheduler, rrc_h, rlc_h, phy_h, logger, cells.size(), softbuffer_pool.get());

  auto ret = ue_db.insert(ISRRAN_MRNTI, std::move(ue_ptr));
  if (!ret) {
    logger.info("Failed to allocate rnti=0x%x.for eMBMS", ISRRAN_MRNTI);
  }
  rrc_h->add_user(ISRRAN_MRNTI, {});
}
