...
#include "isrran/interfaces/enb_x2_interfaces.h"
#include "isrran/rlc/bearer_mem_pool.h"
...
enb_stack_lte::enb_stack_lte(isrlog::sink& log_sink) :
  thread("STACK"),
  mac_logger(isrlog::fetch_basic_logger("MAC", log_sink)),
  rlc_logger(isrlog::fetch_basic_logger("RLC", log_sink, false)),
  pdcp_logger(isrlog::fetch_basic_logger("PDCP", log_sink, false)),
  rrc_logger(isrlog::fetch_basic_logger("RRC", log_sink, false)),
  s1ap_logger(isrlog::fetch_basic_logger("S1AP", log_sink, false)),
  gtpu_logger(isrlog::fetch_basic_logger("GTPU", log_sink, false)),
  stack_logger(isrlog::fetch_basic_logger("STCK", log_sink, false)),
  task_sched(512, 128),
                        ...
                        x2_interface*            x2_)
{
  ...
  s1ap_logger.set_level(isrlog::str_to_basic_level(args.log.s1ap_level));
  ...
  if (rrc.init(rrc_cfg, phy, &mac, &rlc, &pdcp, &s1ap, &gtpu, x2_) != ISRRAN_SUCCESS) {
    stack_logger.error("Couldn't initialize RRC");
    return ISRRAN_ERROR;
  }
  if (s1ap.init(args.s1ap, &rrc) != ISRRAN_SUCCESS) {
  ...
  if (gtpu.init(gtpu_args, gtpu_adapter.get()) != ISRRAN_SUCCESS) {
...
void enb_stack_lte::write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  // call GTPU adapter to map to EPS bearer
  auto task = [this, rnti, lcid](isrran::unique_byte_buffer_t& pdu) {
