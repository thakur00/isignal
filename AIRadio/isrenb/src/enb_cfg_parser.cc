...
#include "isrran/rrc/rrc_common.h"
...
int field_additional_plmns::parse(libconfig::Setting& root)
{
  ...
  for (uint32_t i = 0; i < data->plmn_id_list.size() - 1; i++) {
    ...
    isrran::plmn_id_t plmn;
    if (plmn.from_string(mcc_str + mnc_str) == ISRRAN_ERROR) {
      ERROR("Could not convert %s to a plmn_id in additional_plmn=%d", (mcc_str + mnc_str).c_str(), i);
      return ISRRAN_ERROR;
    }
    isrran::to_asn1(&data->plmn_id_list[i + 1].plmn_id, plmn);
...
int field_5g_srb::parse(libconfig::Setting& root)
{
  ...
  if (root.exists("ul_am")) {
    ...
    if (rlc_sn_size_ul.parse(root["ul_am"]) == ISRRAN_ERROR) {
  ...
  if (root.exists("dl_am")) {
    ...
    if (rlc_sn_size_ul.parse(root["dl_am"]) == ISRRAN_ERROR) {
...
int field_five_qi::parse(libconfig::Setting& root)
{
  ...
  for (uint32_t i = 0; i < nof_five_qi; i++) {
    ...
    if (pdcp_sn_size_ul.parse(drb) == ISRRAN_ERROR) {
    ...
    if (pdcp_sn_size_dl.parse(drb) == ISRRAN_ERROR) {
    ...
    if (t_reordering.parse(pdcp_nr) == ISRRAN_ERROR) {
    ...
    if (rlc_cfg->type() == asn1::rrc_nr::rlc_cfg_c::types::am) {
      ...
      if (rlc_sn_size_ul.parse(rlc_am_ul) == ISRRAN_ERROR) {
      ...
      if (rlc_sn_size_dl.parse(rlc_am_dl) == ISRRAN_ERROR) {
      ...
      if (rlc_sn_size_ul.parse(rlc_um_ul) == ISRRAN_ERROR) {
      ...
      if (rlc_sn_size_dl.parse(rlc_um_dl) == ISRRAN_ERROR) {
...
static int parse_meas_cell_list(rrc_meas_cfg_t* meas_cfg, Setting& root)
{
  ...
  for (uint32_t i = 0; i < meas_cfg->meas_cells.size(); ++i) {
    ...
    cell.pci    = (unsigned int)root[i]["pci"] % ISRRAN_NUM_PCI;
    ...
    isrran_assert(isrran::is_lte_cell_nof_prb(cell.allowed_meas_bw), "Invalid measurement Bandwidth");
  }
  return 0;
}
      ...
      case 1:
        ...
        if (meas_item.trigger_quant == report_cfg_eutra_s::trigger_quant_opts::rsrp) {
          event.event_id.set_event_a1().a1_thres.set_thres_rsrp() =
              rrc_value_to_range(isrran::quant_rsrp, (int)root[i]["a1_thresh"]);
        } else {
          event.event_id.set_event_a1().a1_thres.set_thres_rsrq() =
              rrc_value_to_range(isrran::quant_rsrq, (int)root[i]["a1_thresh"]);
        }
        break;
      case 2:
        ...
        if (meas_item.trigger_quant == report_cfg_eutra_s::trigger_quant_opts::rsrp) {
          event.event_id.set_event_a2().a2_thres.set_thres_rsrp() =
              rrc_value_to_range(isrran::quant_rsrp, (int)root[i]["a2_thresh"]);
        } else {
          event.event_id.set_event_a2().a2_thres.set_thres_rsrq() =
              rrc_value_to_range(isrran::quant_rsrq, (int)root[i]["a2_thresh"]);
        }
        break;
      case 3:
      ...
      case 4:
        ...
        if (meas_item.trigger_quant == report_cfg_eutra_s::trigger_quant_opts::rsrp) {
          event.event_id.set_event_a4().a4_thres.set_thres_rsrp() =
              rrc_value_to_range(isrran::quant_rsrp, (int)root[i]["a4_thresh"]);
        } else {
          event.event_id.set_event_a4().a4_thres.set_thres_rsrq() =
              rrc_value_to_range(isrran::quant_rsrq, (int)root[i]["a4_thresh"]);
        }
        break;
      case 5:
        // a5-threshold1
        ...
        if (meas_item.trigger_quant == report_cfg_eutra_s::trigger_quant_opts::rsrp) {
          event.event_id.set_event_a5().a5_thres1.set_thres_rsrp() =
              rrc_value_to_range(isrran::quant_rsrp, (int)root[i]["a5_thresh1"]);
        } else {
          event.event_id.set_event_a5().a5_thres1.set_thres_rsrq() =
              rrc_value_to_range(isrran::quant_rsrq, (int)root[i]["a5_thresh1"]);
        }
        ...
        if (meas_item.trigger_quant == report_cfg_eutra_s::trigger_quant_opts::rsrp) {
          event.event_id.set_event_a5().a5_thres2.set_thres_rsrp() =
              rrc_value_to_range(isrran::quant_rsrp, (int)root[i]["a5_thresh2"]);
        } else {
          event.event_id.set_event_a5().a5_thres2.set_thres_rsrq() =
              rrc_value_to_range(isrran::quant_rsrq, (int)root[i]["a5_thresh2"]);
        }
        break;
      default:
...
static int parse_cell_list(all_args_t* args, rrc_cfg_t* rrc_cfg, Setting& root)
{
  for (uint32_t n = 0; n < (uint32_t)root.getLength(); ++n) {
    ...
    cell_cfg.pci = cell_cfg.pci % ISRRAN_NUM_PCI;
    ...
    if (cellroot.exists("meas_gap_offset_subframe")) {
      ...
      for (uint32_t j = 0; j < (uint32_t)cellroot["meas_gap_offset_subframe"].getLength(); ++j) {
        ...
        isrran_assert(cell_cfg.meas_cfg.meas_gap_offset_subframe[j] < cell_cfg.meas_cfg.meas_gap_period,
    ...
    isrran_assert(isrran::is_lte_cell_nof_prb(cell_cfg.meas_cfg.allowed_meas_bw), "Invalid measurement Bandwidth");
...
static int parse_nr_cell_list(all_args_t* args, rrc_nr_cfg_t* rrc_cfg_nr, rrc_cfg_t* rrc_cfg_eutra, Setting& root)
{
  for (uint32_t n = 0; n < (uint32_t)root.getLength(); ++n) {
    ...
    cell_cfg.phy_cell.carrier.pci = cell_cfg.phy_cell.carrier.pci % ISRRAN_NOF_NID_NR;
  ...
  isrran::isrran_band_helper band_helper;
  // Configuration check
  // counter for every RF port used by the eNB to avoid misconfiguration/mapping of cells
...
int parse_cell_cfg(all_args_t* args_, isrran_cell_t* cell)
{
  cell->frame_type = ISRRAN_FDD;
  cell->cp         = args_->phy.extended_cp ? ISRRAN_CP_EXT : ISRRAN_CP_NORM;
  ...
  cell->phich_length    = (isrran_phich_length_t)(int)phichcfg.phich_dur;
  cell->phich_resources = (isrran_phich_r_t)(int)phichcfg.phich_res;
...
int parse_cfr_args(all_args_t* args, isrran_cfr_cfg_t* cfr_config)
{
...
{
  // Parse config files
  isrran_cell_t cell_common_cfg = {};

  try {
    if (enb_conf_sections::parse_cell_cfg(args_, &cell_common_cfg) != ISRRAN_SUCCESS) {
  ...
  try {
    if (sib_sections::parse_sibs(args_, rrc_cfg_, phy_cfg_) != ISRRAN_SUCCESS) {
  ...
  try {
    if (rr_sections::parse_rr(args_, rrc_cfg_, rrc_nr_cfg_) != ISRRAN_SUCCESS) {
  ...
  try {
    if (rb_sections::parse_rb(args_, rrc_cfg_, rrc_nr_cfg_) != ISRRAN_SUCCESS) {
  ...
  if (enb_conf_sections::set_derived_args(args_, rrc_cfg_, phy_cfg_, cell_common_cfg) != ISRRAN_SUCCESS) {
    fprintf(stderr, "Error deriving EUTRA cell parameters\n");
    return ISRRAN_ERROR;
  }

  // do the same for NR
  if (enb_conf_sections::set_derived_args_nr(args_, rrc_nr_cfg_, phy_cfg_) != ISRRAN_SUCCESS) {
  ...
  if (rrc_nr_cfg_->cell_list.size() > 0) {
    ...
    if (rrc_nr_cfg_->is_standalone) {
      ...
      for (uint32_t i = 0; i < rrc_cfg_->eea_preference_list.size(); i++) {
        rrc_nr_cfg_->nea_preference_list[i] = (isrran::CIPHERING_ALGORITHM_ID_NR_ENUM)rrc_cfg_->eea_preference_list[i];
        rrc_nr_cfg_->nia_preference_list[i] = (isrran::INTEGRITY_ALGORITHM_ID_NR_ENUM)rrc_cfg_->eia_preference_list[i];
      }
  ...
  // Parse CFR args
  if (parse_cfr_args(args_, &phy_cfg_->cfr_config) < ISRRAN_SUCCESS) {
...
int set_derived_args(all_args_t* args_, rrc_cfg_t* rrc_cfg_, phy_cfg_t* phy_cfg_, const isrran_cell_t& cell_cfg_)
{
  // Sanity checks
  ASSERT_VALID_CFG(args_->stack.mac.nof_prealloc_ues <= ISRENB_MAX_UES,
  ...
  for (auto it = rrc_cfg_->cell_list.begin(); it != rrc_cfg_->cell_list.end(); ++it) {
    ...
    if (cfg.dl_freq_hz > 0) {
      ...
      phy_cell_cfg.dl_freq_hz = 1e6 * isrran_band_fd(cfg.dl_earfcn);
    ...
    if (cfg.ul_freq_hz > 0) {
      ...
      if (cfg.ul_earfcn == 0) {
        cfg.ul_earfcn = isrran_band_ul_earfcn(cfg.dl_earfcn);
      }
      phy_cell_cfg.ul_freq_hz = 1e6 * isrran_band_fu(cfg.ul_earfcn);
  ...
  int i = 0;
  for (auto it = eea_pref_list.begin(); it != eea_pref_list.end() && i < isrran::CIPHERING_ALGORITHM_ID_N_ITEMS; it++) {
    boost::trim_left(*it);
    if ((*it) == "EEA0") {
      rrc_cfg_->eea_preference_list[i] = isrran::CIPHERING_ALGORITHM_ID_EEA0;
      i++;
    } else if ((*it) == "EEA1") {
      rrc_cfg_->eea_preference_list[i] = isrran::CIPHERING_ALGORITHM_ID_128_EEA1;
      i++;
    } else if ((*it) == "EEA2") {
      rrc_cfg_->eea_preference_list[i] = isrran::CIPHERING_ALGORITHM_ID_128_EEA2;
      i++;
    } else if ((*it) == "EEA3") {
      rrc_cfg_->eea_preference_list[i] = isrran::CIPHERING_ALGORITHM_ID_128_EEA3;
      i++;
    } else {
  ...
  i = 0;
  for (auto it = eia_pref_list.begin(); it != eia_pref_list.end() && i < isrran::INTEGRITY_ALGORITHM_ID_N_ITEMS; it++) {
    boost::trim_left(*it);
    if ((*it) == "EIA0") {
      rrc_cfg_->eia_preference_list[i] = isrran::INTEGRITY_ALGORITHM_ID_EIA0;
      i++;
    } else if ((*it) == "EIA1") {
      rrc_cfg_->eia_preference_list[i] = isrran::INTEGRITY_ALGORITHM_ID_128_EIA1;
      i++;
    } else if ((*it) == "EIA2") {
      rrc_cfg_->eia_preference_list[i] = isrran::INTEGRITY_ALGORITHM_ID_128_EIA2;
      i++;
    } else if ((*it) == "EIA3") {
      rrc_cfg_->eia_preference_list[i] = isrran::INTEGRITY_ALGORITHM_ID_128_EIA3;
      i++;
    } else {
  ...
  if (args_->rf.device_name == "zmq") {
    isrlog::fetch_basic_logger("ENB").info("Using sync queue size of one for ZMQ based radio.");
...
int set_derived_args_nr(all_args_t* args_, rrc_nr_cfg_t* rrc_nr_cfg_, phy_cfg_t* phy_cfg_)
{
  ...
  if (set_derived_nr_rrc_params(*rrc_nr_cfg_) != ISRRAN_SUCCESS) {
...
int parse_sib9(std::string filename, sib_type9_s* data)
{
  ...
  if (!parser::parse_section(std::move(filename), &sib9)) {
    ...
    if (name_enabled) {
      data->hnb_name.resize(ISRRAN_MIN((uint32_t)hnb_name.size(), 48));
...
int parse_sibs(all_args_t* args_, rrc_cfg_t* rrc_cfg_, isrenb::phy_cfg_t* phy_config_common)
{
  ...
  if (sib_sections::parse_sib1(args_->enb_files.sib_config, sib1) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Fill rest of data from enb config
  std::string mnc_str;
  if (not isrran::mnc_to_string(args_->stack.s1ap.mnc, &mnc_str)) {
    ERROR("The provided mnc=%d is not valid", args_->stack.s1ap.mnc);
    return ISRRAN_ERROR;
  }
  std::string mcc_str;
  if (not isrran::mcc_to_string(args_->stack.s1ap.mcc, &mcc_str)) {
  ...
  isrran::to_asn1(&cell_access->plmn_id_list[0].plmn_id, plmn);
  ...
  // Generate SIB2
  if (sib_sections::parse_sib2(args_->enb_files.sib_config, sib2) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // ISR not yet supported
  sib2->rr_cfg_common.isr_ul_cfg_common.set(isr_ul_cfg_common_c::types::release);
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type3)) {
    if (sib_sections::parse_sib3(args_->enb_files.sib_config, sib3) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type4)) {
    if (sib_sections::parse_sib4(args_->enb_files.sib_config, sib4) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type5)) {
    if (sib_sections::parse_sib5(args_->enb_files.sib_config, sib5) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type6)) {
    if (sib_sections::parse_sib6(args_->enb_files.sib_config, sib6) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type7)) {
    if (sib_sections::parse_sib7(args_->enb_files.sib_config, sib7) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type9)) {
    if (sib_sections::parse_sib9(args_->enb_files.sib_config, sib9) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }

  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type13_v920)) {
    if (sib_sections::parse_sib13(args_->enb_files.sib_config, sib13) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  phy_config_common->isr_ul_cnfg = sib2->rr_cfg_common.isr_ul_cfg_common;
