...
#include "isrenb/hdr/metrics_csv.h"
#include "isrenb/hdr/metrics_json.h"
#include "isrenb/hdr/metrics_stdout.h"
...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("enb.name",               bpo::value<string>(&args->stack.s1ap.enb_name)->default_value("isrenb01"), "eNodeB Name")
  ...
  if (vm.count("version")) {
    cout << "Version " << isrran_get_version_major() << "." << isrran_get_version_minor() << "."
         << isrran_get_version_patch() << endl;
    exit(0);
  }
  ...
  if (!isrran::string_to_mcc(mcc, &args->stack.s1ap.mcc)) {
    cout << "Error parsing enb.mcc:" << mcc << " - must be a 3-digit string." << endl;
  }
  if (!isrran::string_to_mnc(mnc, &args->stack.s1ap.mnc)) {
    cout << "Error parsing enb.mnc:" << mnc << " - must be a 2 or 3-digit string." << endl;
  }
  if (!isrran::string_to_mcc(mcc, &args->nr_stack.ngap.mcc)) {
    cout << "Error parsing enb.mcc:" << mcc << " - must be a 3-digit string." << endl;
  }
  if (!isrran::string_to_mnc(mnc, &args->nr_stack.ngap.mnc)) {
    cout << "Error parsing enb.mnc:" << mnc << " - must be a 2 or 3-digit string." << endl;
  }
  ...
  args->phy.cfr_args.mode = isrran_cfr_str2mode(cfr_mode.c_str());
  if (args->phy.cfr_args.mode == ISRRAN_CFR_THR_INVALID) {
...
static void execute_cmd(metrics_stdout* metrics, isrenb::enb_command_interface* control, const string& cmd_line)
{
  vector<string> cmd;
  isrran::string_parse_list(cmd_line, ' ', cmd);
  if (cmd[0] == "t") {
    ...
    int nseconds = isrran::string_cast<int>(cmd[1]);
    if (nseconds <= 0) {
      return;
    }
    ...
    uint32_t cell_id = isrran::string_cast<uint32_t>(cmd[1]);
    float    gain_db = isrran::string_cast<float>(cmd[2]);
...
static void* input_loop(metrics_stdout* metrics, isrenb::enb_command_interface* control)
{
  ...
  while (running) {
    ...
    if (ret == 1) {
      ...
      if (cin.eof() || cin.bad()) {
        ...
        isrran::string_parse_list(input_line, ';', cmd_list);
...
int main(int argc, char* argv[])
{
  ...
  all_args_t                         args = {};
  isrran::metrics_hub<enb_metrics_t> metricshub;
  ...
  isrlog::set_default_sink(
      (args.log.filename == "stdout")
          ? isrlog::fetch_stdout_sink()
          : isrlog::fetch_file_sink(args.log.filename, fixup_log_file_maxsize(args.log.file_max_size)));

  // Alarms log channel creation.
  isrlog::sink&        alarm_sink     = isrlog::fetch_file_sink(args.general.alarms_filename, 0, true);
  isrlog::log_channel& alarms_channel = isrlog::fetch_log_channel("alarms", alarm_sink, {"ALRM", '\0', false});
  alarms_channel.set_enabled(args.general.alarms_log_enable);

#ifdef ENABLE_ISRLOG_EVENT_TRACE
  if (args.general.tracing_enable) {
    if (!isrlog::event_trace_init(args.general.tracing_filename, args.general.tracing_buffcapacity)) {
      return ISRRAN_ERROR;
    }
  }
#endif
  ...
  isrlog::fetch_basic_logger("ALL").set_level(isrlog::basic_levels::warning);
  isrlog::fetch_basic_logger("POOL").set_level(isrlog::basic_levels::warning);
  isrran::log_args(argc, argv, "ENB");

  isrran::check_scaling_governor(args.rf.device_name);

  // Set up the JSON log channel used by metrics and events.
  isrlog::sink& json_sink =
      isrlog::fetch_file_sink(args.general.report_json_filename, 0, false, isrlog::create_json_formatter());
  ...
  if (mlockall((uint32_t)MCL_CURRENT | (uint32_t)MCL_FUTURE) == -1) {
    isrran::console("Failed to `mlockall`: {}", errno);
  }

  // Create eNB
  unique_ptr<isrenb::enb> enb{new isrenb::enb(isrlog::get_default_sink())};
  if (enb->init(args) != ISRRAN_SUCCESS) {
    enb->stop();
    return ISRRAN_ERROR;
  }
  ...
  isrenb::metrics_csv metrics_file(args.general.metrics_csv_filename, enb.get());
  if (args.general.metrics_csv_enable) {
    metricshub.add_listener(&metrics_file);
  }

  isrenb::metrics_json json_metrics(json_channel, enb.get());
  if (args.general.report_json_enable) {
