...
{
  return (tti % ISRRAN_MAX_CARRIERS) != ue_cc_idx;
}
...
class dummy_radio final : public isrran::radio_interface_phy
{
private:
  std::mutex                        mutex;
  std::condition_variable           cvar;
  isrlog::basic_logger&             logger;
  std::vector<isrran_ringbuffer_t*> ringbuffers_tx;
  std::vector<isrran_ringbuffer_t*> ringbuffers_rx;
  isrran::rf_timestamp_t            ts_rx    = {};
  double                            rx_srate = 0.0;
...
public:
  explicit dummy_radio(uint32_t nof_channels, uint32_t nof_prb, const std::string& log_level) :
    logger(isrlog::fetch_basic_logger("RADIO", false))
  {
    logger.set_level(isrlog::str_to_basic_level(log_level));

    // Allocate receive ring buffer
    for (uint32_t i = 0; i < nof_channels; i++) {
      auto* rb = (isrran_ringbuffer_t*)isrran_vec_malloc(sizeof(isrran_ringbuffer_t));
      if (not rb) {
        ERROR("Allocating ring buffer");
      }

      if (isrran_ringbuffer_init(rb, ISRRAN_SF_LEN_PRB(nof_prb) * ISRRAN_NOF_SF_X_FRAME * (uint32_t)sizeof(cf_t))) {
  ...
  int read_tx(std::vector<cf_t*>& buffers, uint32_t nof_samples)
  {
    ...
    for (uint32_t i = 0; i < ringbuffers_tx.size() and i < buffers.size(); i++) {
      do {
        err = isrran_ringbuffer_read_timed(ringbuffers_tx[i], buffers[i], nbytes, 1000);
      } while (err < ISRRAN_SUCCESS and running);
    }
  ...
  void write_rx(std::vector<cf_t*>& buffers, uint32_t nof_samples)
  {
    ...
    for (uint32_t i = 0; i < ringbuffers_rx.size() and i < buffers.size(); i++) {
      isrran_ringbuffer_write(ringbuffers_rx[i], buffers[i], nbytes);
    }
  }
  ...
  bool tx(isrran::rf_buffer_interface& buffer, const isrran::rf_timestamp_interface& tx_time) override
  {
    ...
    // Write ring buffer
    for (uint32_t i = 0; i < ringbuffers_tx.size() and err >= ISRRAN_SUCCESS; i++) {
      err = isrran_ringbuffer_write(ringbuffers_tx[i], buffer.get(i), nbytes);
    }

    // Notify call
    notify_tx();

    // Return True if err >= ISRRAN_SUCCESS
    return err >= ISRRAN_SUCCESS;
  }
  ...
  bool rx_now(isrran::rf_buffer_interface& buffer, isrran::rf_timestamp_interface& rxd_time) override
  {
    ...
    if (not running) {
      for (uint32_t i = 0; i < buffer.size(); i++) {
        isrran_vec_cf_zero(buffer.get(i), buffer.get_nof_samples());
      }
      return true;
    }
    ...
    // Write ring buffer
    for (uint32_t i = 0; i < ringbuffers_rx.size() and err >= ISRRAN_SUCCESS; i++) {
      do {
        err = isrran_ringbuffer_read_timed(ringbuffers_rx[i], buffer.get(i), nbytes, 1000);
      } while (err < ISRRAN_SUCCESS and running);
    }
  ...
  void              reset() override {}
  isrran_rf_info_t* get_info() override { return nullptr; }
};

typedef std::unique_ptr<dummy_radio> unique_dummy_radio_t;

class dummy_stack final : public isrenb::stack_interface_phy_lte
{
private:
  ...
  isrenb::phy_cell_cfg_list_t                       phy_cell_cfg;
  isrenb::phy_interface_rrc_lte::phy_rrc_cfg_list_t phy_rrc;
  std::mutex                                        mutex;
  std::condition_variable                           cvar;
  isrlog::basic_logger&                             logger;
  isrran_softbuffer_tx_t                            softbuffer_tx                                           = {};
  isrran_softbuffer_rx_t                            softbuffer_rx[ISRRAN_MAX_CARRIERS][ISRRAN_FDD_NOF_HARQ] = {};
  uint8_t*                                          data                                                    = nullptr;
  uint16_t                                          ue_rnti                                                 = 0;
  isrran_random_t                                   random_gen                                              = nullptr;
...
public:
  explicit dummy_stack(const isrenb::phy_cfg_t&                                 phy_cfg_,
                       const isrenb::phy_interface_rrc_lte::phy_rrc_cfg_list_t& phy_rrc_,
                       const std::string&                                       log_level,
                       uint16_t                                                 rnti_) :
    logger(isrlog::fetch_basic_logger("STACK", false)),
    ue_rnti(rnti_),
    random_gen(isrran_random_init(rnti_)),
    phy_cell_cfg(phy_cfg_.phy_cell_cfg),
    phy_rrc(phy_rrc_)
  {
    logger.set_level(isrlog::str_to_basic_level(log_level));
    isrran_softbuffer_tx_init(&softbuffer_tx, ISRRAN_MAX_PRB);
    for (uint32_t i = 0; i < phy_rrc.size(); i++) {
      for (auto& sb : softbuffer_rx[i]) {
        isrran_softbuffer_rx_init(&sb, ISRRAN_MAX_PRB);
      }
    }

    isrran_pdcch_t pdcch = {};
    isrran_regs_t  regs  = {};
    isrran_regs_init(&regs, phy_cell_cfg[0].cell);
    isrran_pdcch_init_enb(&pdcch, phy_cell_cfg[0].cell.nof_prb);
    isrran_pdcch_set_cell(&pdcch, &regs, phy_cell_cfg[0].cell);
    for (uint32_t i = 0; i < ISRRAN_NOF_SF_X_FRAME; i++) {
      ...
      isrran_dci_location_t _dci_locations[ISRRAN_MAX_CANDIDATES_UE] = {};
      _nof_locations = isrran_pdcch_ue_locations(&pdcch, &sf_cfg_dl, _dci_locations, ISRRAN_MAX_CANDIDATES_UE, ue_rnti);

      // Take L == 0 aggregation levels
      for (uint32_t j = 0; j < _nof_locations && nof_locations[i] < ISRRAN_MAX_CANDIDATES_UE; j++) {
    ...
    do {
      if (isrran_dft_precoding_valid_prb(L_prb)) {
        ul_riv = isrran_ra_type2_to_riv(L_prb, 1, phy_cell_cfg[0].cell.nof_prb);
      } else {
        L_prb--;
      }
    } while (ul_riv == 0);
  ...
  int get_dl_sched(uint32_t tti, dl_sched_list_t& dl_sched_res) override
  {
    ...
    for (uint32_t& cc_idx : active_cell_list) {
      ...
      bool sched_tb[ISRRAN_MAX_TB] = {};

      sched_tb[0] = isrran_random_bool(random_gen, prob_dl_grant);

      // Schedule second TB for TM3 or TM4
      if (phy_rrc[ue_cc_idx].phy_cfg.dl_cfg.tm == ISRRAN_TM3 or phy_rrc[ue_cc_idx].phy_cfg.dl_cfg.tm == ISRRAN_TM4) {
        sched_tb[1] = isrran_random_bool(random_gen, prob_dl_grant);
      }
      ...
      sched &= (nof_locations[tti % ISRRAN_NOF_SF_X_FRAME] > 1);

      // Schedule grant
      if (sched) {
        uint32_t              location_idx = tti % nof_locations[tti % ISRRAN_NOF_SF_X_FRAME];
        isrran_dci_location_t location     = dci_locations[tti % ISRRAN_NOF_SF_X_FRAME][location_idx];
        ...
        dl_sched.pdsch[0].dci.alloc_type              = ISRRAN_RA_ALLOC_TYPE0;
        dl_sched.pdsch[0].data[0]                     = data;
        dl_sched.pdsch[0].data[1]                     = data;
        dl_sched.pdsch[0].dci.tpc_pucch               = (location.ncce) % ISRRAN_PUCCH_SIZE_AN_CS;
        ...
        switch (phy_rrc[0].phy_cfg.dl_cfg.tm) {
          ...
          case ISRRAN_TM1:
          case ISRRAN_TM2:
            dl_sched.pdsch[0].dci.format = ISRRAN_DCI_FORMAT1;
            break;
          case ISRRAN_TM3:
            dl_sched.pdsch[0].dci.format = ISRRAN_DCI_FORMAT2A;
            break;
          case ISRRAN_TM4:
            dl_sched.pdsch[0].dci.format = ISRRAN_DCI_FORMAT2;
            break;
        }
  ...
  int get_ul_sched(uint32_t tti, ul_sched_list_t& ul_sched_res) override
  {
    ...
    for (uint32_t cc_idx = 0; cc_idx < ul_sched_res.size(); cc_idx++) {
      ...
      // Random decision on whether transmit or not
      bool sched = isrran_random_bool(random_gen, prob_ul_grant);
      ...
      if (sched) {
        ...
        uint32_t              location_idx = (tti_pdcch + 1) % nof_locations[tti_pdcch % ISRRAN_NOF_SF_X_FRAME];
        isrran_dci_location_t location     = dci_locations[tti_pdcch % ISRRAN_NOF_SF_X_FRAME][location_idx];

        ul_sched.nof_grants                       = 1;
        ul_sched.pusch[0]                         = {};
        ul_sched.pusch[0].dci.rnti                = ue_rnti;
        ul_sched.pusch[0].dci.format              = ISRRAN_DCI_FORMAT0;
        ul_sched.pusch[0].dci.location            = location;
        ul_sched.pusch[0].dci.type2_alloc.riv     = ul_riv;
        ul_sched.pusch[0].dci.type2_alloc.n_prb1a = isrran_ra_type2_t::ISRRAN_RA_TYPE2_NPRB1A_2;
        ul_sched.pusch[0].dci.type2_alloc.n_gap   = isrran_ra_type2_t::ISRRAN_RA_TYPE2_NG1;
        ul_sched.pusch[0].dci.type2_alloc.mode    = isrran_ra_type2_t::ISRRAN_RA_TYPE2_LOC;
        ul_sched.pusch[0].dci.freq_hop_fl         = isrran_dci_ul_t::ISRRAN_RA_PUSCH_HOP_DISABLED;
        ...
        ul_sched.pusch[0].softbuffer_rx = &softbuffer_rx[scell_idx][tti % ISRRAN_FDD_NOF_HARQ];

        // Reset Rx softbuffer
        isrran_softbuffer_rx_reset(ul_sched.pusch[0].softbuffer_rx);
...
{
private:
  std::vector<isrran_ue_dl_t*>                      ue_dl_v       = {};
  std::vector<isrran_ue_ul_t*>                      ue_ul_v       = {};
  std::vector<cf_t*>                                buffers       = {};
  dummy_radio*                                      radio         = nullptr;
  uint32_t                                          sf_len        = 0;
  uint32_t                                          nof_ports     = 0;
  uint16_t                                          rnti          = 0;
  isrran_dl_sf_cfg_t                                sf_dl_cfg     = {};
  isrran_ul_sf_cfg_t                                sf_ul_cfg     = {};
  isrran_softbuffer_tx_t                            softbuffer_tx = {};
  uint8_t*                                          tx_data       = nullptr;
  isrenb::phy_interface_rrc_lte::phy_rrc_cfg_list_t phy_rrc_cfg   = {};
  isrlog::basic_logger&                             logger;
  std::map<uint32_t, uint32_t>                      last_ri = {};

public:
  dummy_ue(dummy_radio* _radio, const isrenb::phy_cell_cfg_list_t& cell_list, std::string log_level, uint16_t rnti_) :
    radio(_radio), logger(isrlog::fetch_basic_logger("UPHY"))
  {
    // Calculate subframe length
    nof_ports = cell_list[0].cell.nof_ports;
    sf_len    = static_cast<uint32_t>(ISRRAN_SF_LEN_PRB(cell_list[0].cell.nof_prb));
    rnti      = rnti_;
    ...
    for (uint32_t i = 0; i < cell_list.size() * nof_ports; i++) {
      // Allocate buffers
      cf_t* buffer = isrran_vec_cf_malloc(sf_len);
      if (not buffer) {
    ...
    for (uint32_t cc_idx = 0; cc_idx < (uint32_t)cell_list.size(); cc_idx++) {
      const isrran_cell_t& cell = cell_list[cc_idx].cell;

      // Allocate UE DL
      auto* ue_dl = (isrran_ue_dl_t*)isrran_vec_malloc(sizeof(isrran_ue_dl_t));
      if (not ue_dl) {
      ...
      // Initialise UE DL
      if (isrran_ue_dl_init(ue_dl, &buffers[cc_idx * nof_ports], cell.nof_prb, cell.nof_ports)) {
      ...
      // Allocate UE UL
      auto* ue_ul = (isrran_ue_ul_t*)isrran_vec_malloc(sizeof(isrran_ue_ul_t));
      if (not ue_ul) {
      ...
      // Initialise UE UL
      if (isrran_ue_ul_init(ue_ul, buffers[cc_idx * nof_ports], cell.nof_prb)) {
        ERROR("Setting UE UL cell");
      }

      // Set cell
      if (isrran_ue_ul_set_cell(ue_ul, cell)) {
        ERROR("Setting UE DL cell");
      }
    }

    // Initialise softbuffer
    if (isrran_softbuffer_tx_init(&softbuffer_tx, cell_list[0].cell.nof_prb)) {
      ERROR("Initialising Tx softbuffer");
    }

    // Initialise dummy tx data
    tx_data = isrran_vec_u8_malloc(ISRENB_MAX_BUFFER_SIZE_BYTES);
    if (not tx_data) {
      ERROR("Allocating Tx data");
    }

    for (uint32_t i = 0; i < ISRENB_MAX_BUFFER_SIZE_BYTES; i++) {
  ...
  void reconfigure(const isrenb::phy_interface_rrc_lte::phy_rrc_cfg_list_t& phy_rrc_cfg_)
  {
  ...
  int work_dl(isrran_pdsch_ack_t& pdsch_ack, isrran_uci_data_t& uci_data)
  {
    // Read DL
    TESTASSERT(radio->read_tx(buffers, sf_len) >= ISRRAN_SUCCESS);
    ...
    for (uint32_t ue_cc_idx = 0; ue_cc_idx < phy_rrc_cfg.size(); ue_cc_idx++) {
      ...
      isrran::phy_cfg_t& dedicated = phy_rrc_cfg[ue_cc_idx].phy_cfg;
      ...
      ue_dl_cfg.cfg.cqi_report.periodic_mode        = ISRRAN_CQI_MODE_12;
      ...
      isrran_ue_dl_decode_fft_estimate(ue_dl_v[cc_idx], &sf_dl_cfg, &ue_dl_cfg);

      // Get DL Grants
      int nof_dl_grants = isrran_ue_dl_find_dl_dci(ue_dl_v[cc_idx], &sf_dl_cfg, &ue_dl_cfg, rnti, dci_dl);
      TESTASSERT(nof_dl_grants >= ISRRAN_SUCCESS);

      // Generate ACKs
      if (nof_dl_grants) {
        char str[256] = {};
        isrran_dci_dl_info(dci_dl, str, sizeof(str));
        logger.info("[DL DCI] %s", str);

        if (isrran_ue_dl_dci_to_pdsch_grant(
                ue_dl_v[cc_idx], &sf_dl_cfg, &ue_dl_cfg, dci_dl, &ue_dl_cfg.cfg.pdsch.grant)) {
          logger.error("Converting DCI message to DL dci");
          return ISRRAN_ERROR;
        }

        isrran_pdsch_tx_info(&ue_dl_cfg.cfg.pdsch, str, 512);
        ...
        for (uint32_t tb_idx = 0; tb_idx < ISRRAN_MAX_TB; tb_idx++) {
      ...
      isrran_ue_dl_gen_cqi_periodic(ue_dl_v[cc_idx], &ue_dl_cfg, 0x0f, sf_ul_cfg.tti, &uci_data);

      if (isrran_cqi_periodic_ri_send(&ue_dl_cfg.cfg.cqi_report, sf_ul_cfg.tti, ue_dl_v[cc_idx]->cell.frame_type) &&
  ...
  void fill_uci(uint32_t             cc_idx,
                ...
                isrran_pusch_data_t& pusch_data)
  {
    // Generate scheduling request
    isrran_ue_ul_gen_sr(&ue_ul_cfg, &sf_ul_cfg, &uci_data, (bool)(sf_ul_cfg.tti % 20 == 0));

    // Generate Acknowledgements
    isrran_ue_dl_gen_ack(&ue_dl_v[cc_idx]->cell, &sf_dl_cfg, &pdsch_ack, &uci_data);
  ...
  int work_ul(isrran_pdsch_ack_t& pdsch_ack, isrran_uci_data_t& uci_data)
  {
    bool first_pusch = true;

    // Zero all IQ UL buffers
    for (auto& buffer : buffers) {
      isrran_vec_cf_zero(buffer, ISRRAN_SF_LEN_PRB(ue_ul_v[0]->cell.nof_prb));
    }

    for (uint32_t i = 0; i < phy_rrc_cfg.size(); i++) {
      isrran_dci_ul_t    dci_ul[ISRRAN_MAX_DCI_MSG] = {};
      isrran::phy_cfg_t& dedicated                  = phy_rrc_cfg[i].phy_cfg;
      ...
      // Get UL grants
      int nof_ul_grants = isrran_ue_dl_find_ul_dci(ue_dl_v[cc_idx], &sf_dl_cfg, &ue_dl_cfg, rnti, dci_ul);
      TESTASSERT(nof_ul_grants >= ISRRAN_SUCCESS);

      isrran_pusch_data_t pusch_data = {};
      pusch_data.ptr                 = tx_data;

      if (nof_ul_grants > ISRRAN_SUCCESS) {
        TESTASSERT(isrran_ue_ul_dci_to_pusch_grant(
      ...
      // Work UL
      TESTASSERT(isrran_ue_ul_encode(ue_ul_v[cc_idx], &sf_ul_cfg, &ue_ul_cfg, &pusch_data) >= ISRRAN_SUCCESS);

      char str[256] = {};
      isrran_ue_ul_info(&ue_ul_cfg, &sf_ul_cfg, &pusch_data.uci, str, sizeof(str));
      if (str[0]) {
    ...
    if (first_pusch) {
      ...
      isrran::phy_cfg_t& dedicated = phy_rrc_cfg[0].phy_cfg;
  ...
  int run_tti()
  {
    ...
    // Work DL
    TESTASSERT(work_dl(pdsch_ack, uci_data) == ISRRAN_SUCCESS);

    // Work UL
    TESTASSERT(work_ul(pdsch_ack, uci_data) == ISRRAN_SUCCESS);
...
typedef std::unique_ptr<isrenb::phy> unique_isrenb_phy_t;

class phy_test_bench : public isrenb::enb_time_interface
{
public:
  struct args_t {
...
private:
  ...
  unique_isrenb_phy_t   enb_phy;
  unique_dummy_ue_phy_t ue_phy;
  isrlog::basic_logger& logger;

  args_t                                            args = {};   ///< Test arguments
  isrenb::phy_args_t                                phy_args = {};   ///< PHY arguments
  isrenb::phy_cfg_t                                 phy_cfg  = {};   ///< eNb Cell/Carrier configuration
  isrenb::phy_interface_rrc_lte::phy_rrc_cfg_list_t phy_rrc_cfg; ///< UE PHY configuration
...
public:
  phy_test_bench(args_t& args_, isrlog::sink& log_sink) :
    logger(isrlog::fetch_basic_logger("TEST BENCH", log_sink, false))
  {
    // Copy test arguments
    args = args_;
    ...
    for (uint32_t i = 0; i < args.nof_enb_cells; i++) {
      ...
      q.cell.id      = i;
      q.cell_id      = i;
      q.cell.cp      = args.extended_cp ? ISRRAN_CP_EXT : ISRRAN_CP_NORM;
    ...
    enb_phy = unique_isrenb_phy_t(new isrenb::phy(log_sink));
  }

  int init()
  {
    // Create base UE dedicated configuration
    isrran::phy_cfg_t dedicated = {};
    ...
    dedicated.dl_cfg.cqi_report.periodic_mode       = ISRRAN_CQI_MODE_20;

    if (args.tm == ISRRAN_TM3 or args.tm == ISRRAN_TM4) {
    ...
    dedicated.ul_cfg.pucch.ack_nack_feedback_mode  = isrran_string_ack_nack_feedback_mode(args.ack_mode.c_str());
    ...
    // Configure UE PHY
    std::array<bool, ISRRAN_MAX_CARRIERS> activation = {}; ///< Activation/Deactivation vector
  ...
  int run_tti()
  {
    ...
    TESTASSERT(ue_phy->run_tti() >= ISRRAN_SUCCESS);
    TESTASSERT(stack->run_tti(change_state == change_state_assert) >= ISRRAN_SUCCESS);
...
int parse_args(int argc, char** argv, phy_test_bench::args_t& args)
{
  ...
  if (not args.ue_cell_list_str.empty()) {
    isrran::string_parse_list(args.ue_cell_list_str, ',', args.ue_cell_list);
  } else {
    return ISRRAN_ERROR;
  }
...
int main(int argc, char** argv)
{
  ...
  // isrenb/test/phy/isrRAN.backtrace.crash
  isrran_debug_handle_crash(argc, argv);

  phy_test_bench::args_t test_args;

  // Parse arguments
  TESTASSERT(parse_args(argc, argv, test_args) == ISRRAN_SUCCESS);
  ...
  // Create Test Bench
  unique_phy_test_bench test_bench = unique_phy_test_bench(new phy_test_bench(test_args, isrlog::get_default_sink()));
  int                   err_code   = test_bench->init();
  bool                  valid_cfg  = test_args.nof_enb_cells <= ISRRAN_MAX_CARRIERS;
  if (not valid_cfg) {
    // Verify that phy returns with an error if provided an invalid configuration
    TESTASSERT(err_code != ISRRAN_SUCCESS);
    return ISRRAN_SUCCESS;
  }

  // Run Simulation
  for (uint32_t i = 0; i < test_args.duration and err_code >= ISRRAN_SUCCESS; i++) {
