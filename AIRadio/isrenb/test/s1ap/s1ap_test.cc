...
struct mme_dummy {
  ...
  {
    isrran::net_utils::set_sockaddr(&mme_sockaddr, addr_str, port);
    {
...
struct rrc_tester : public rrc_dummy {
  ...
  uint16_t              last_released_rnti = ISRRAN_INVALID_RNTI;
...
void run_s1_setup(s1ap& s1ap_obj, mme_dummy& mme)
{
  ...
  isrran::unique_byte_buffer_t sdu = mme.read_msg();
  TESTASSERT(sdu->N_bytes > 0);
  asn1::cbit_ref cbref(sdu->msg, sdu->N_bytes);
  TESTASSERT(s1ap_pdu.unpack(cbref) == asn1::ISRASN_SUCCESS);
...
void add_rnti(s1ap& s1ap_obj, mme_dummy& mme)
{
  ...
  TESTASSERT(sdu->N_bytes > 0);
  asn1::cbit_ref cbref{sdu->msg, sdu->N_bytes};
  TESTASSERT(s1ap_pdu.unpack(cbref) == ISRRAN_SUCCESS);
  TESTASSERT(s1ap_pdu.type().value == asn1::s1ap::s1ap_pdu_c::types_opts::init_msg);
...
void test_s1ap_erab_setup(test_event event)
{
  ...
  isrlog::basic_logger&        logger = isrlog::fetch_basic_logger("S1AP");
  ...
  args.enb_name      = "isrenb01";

  TESTASSERT(s1ap_obj.init(args, &rrc) == ISRRAN_SUCCESS);
  ...
  TESTASSERT(mod_req_pdu.pack(bref) == ISRRAN_SUCCESS);
  sdu->N_bytes = bref.distance_bytes();

  TESTASSERT(rrc.last_released_rnti == ISRRAN_INVALID_RNTI);
  TESTASSERT(s1ap_obj.handle_mme_rx_msg(std::move(sdu), mme_addr, rcvinfo, flags));
  sdu = mme.read_msg();
  TESTASSERT(sdu->N_bytes > 0);
  asn1::cbit_ref cbref{sdu->msg, sdu->N_bytes};
...
{
  // Setup logging.
  auto& logger = isrlog::fetch_basic_logger("S1AP");
