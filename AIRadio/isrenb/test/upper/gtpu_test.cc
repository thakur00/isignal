...
#include "isrenb/test/common/dummy_classes.h"
...
{
public:
  void write_sdu(uint16_t rnti, uint32_t eps_bearer_id, isrran::unique_byte_buffer_t sdu, int pdcp_sn) override
  {
  ...
  void push_buffered_pdu(uint32_t sn, isrran::unique_byte_buffer_t pdu) { buffered_pdus[sn] = std::move(pdu); }
  ...
  void clear()
  {
    ...
    last_rnti          = ISRRAN_INVALID_RNTI;
  }

  std::map<uint32_t, isrran::unique_byte_buffer_t> buffered_pdus;
  isrran::unique_byte_buffer_t                     last_sdu;
  int                                              last_pdcp_sn       = -1;
  uint16_t                                         last_rnti          = ISRRAN_INVALID_RNTI;
...
isrran::unique_byte_buffer_t encode_ipv4_packet(isrran::span<uint8_t>     data,
                                                uint32_t                  teid,
...
isrran::unique_byte_buffer_t encode_gtpu_packet(isrran::span<uint8_t>     data,
                                                uint32_t                  teid,
                                                const struct sockaddr_in& src_sockaddr_in,
                                                const struct sockaddr_in& dest_sockaddr_in)
{
  isrran::unique_byte_buffer_t pdu = encode_ipv4_packet(data, teid, src_sockaddr_in, dest_sockaddr_in);

  // header
  isrran::gtpu_header_t header = {};
  header.flags                 = GTPU_FLAGS_VERSION_V1 | GTPU_FLAGS_GTP_PROTOCOL;
  header.message_type          = GTPU_MSG_DATA_PDU;
  header.length                = pdu->N_bytes;
  header.teid                  = teid;

  gtpu_write_header(&header, pdu.get(), isrlog::fetch_basic_logger("GTPU"));
  return pdu;
}

isrran::unique_byte_buffer_t encode_end_marker(uint32_t teid)
{
  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer();
...
isrran::unique_byte_buffer_t read_socket(int fd)
{
  isrran::unique_byte_buffer_t pdu = isrran::make_byte_buffer();
...
void test_gtpu_tunnel_manager()
{
  ...
  isrran::net_utils::set_sockaddr(&sgw_sockaddr, sgw_addr_str, GTPU_PORT);
  ...
  gtpu_tunnel_manager tunnels(&task_sched, isrlog::fetch_basic_logger("GTPU"));
...
int test_gtpu_direct_tunneling(tunnel_test_event event)
{
  ...
  std::mt19937          g(rd());
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("TEST");
  ...
  isrran::net_utils::set_sockaddr(&senb_sockaddr, senb_addr_str, GTPU_PORT);
  isrran::net_utils::set_sockaddr(&sgw_sockaddr, sgw_addr_str, GTPU_PORT);
  isrran::net_utils::set_sockaddr(&tenb_sockaddr, tenb_addr_str, GTPU_PORT);
  ...
  // Initiate layers
  isrlog::basic_logger& logger1 = isrlog::fetch_basic_logger("GTPU1");
  logger1.set_hex_dump_max_size(2048);
  isrlog::basic_logger& logger2 = isrlog::fetch_basic_logger("GTPU2");
  logger2.set_hex_dump_max_size(2048);
  isrran::task_scheduler task_sched;
  dummy_socket_manager   senb_rx_sockets, tenb_rx_sockets;
  isrenb::gtpu senb_gtpu(&task_sched, logger1, &senb_rx_sockets), tenb_gtpu(&task_sched, logger2, &tenb_rx_sockets);
  ...
  pdu_view = isrran::make_span(tenb_pdcp.last_sdu);
  ...
  isrran::span<uint8_t> encoded_data2{tenb_pdcp.last_sdu->msg + 20u, tenb_pdcp.last_sdu->msg + 30u};
...
{
  // Setup logging.
  auto& logger = isrlog::fetch_basic_logger("GTPU", false);
  ...
  isrenb::test_gtpu_tunnel_manager();
  TESTASSERT(isrenb::test_gtpu_direct_tunneling(isrenb::tunnel_test_event::success) == ISRRAN_SUCCESS);
  TESTASSERT(isrenb::test_gtpu_direct_tunneling(isrenb::tunnel_test_event::wait_end_marker_timeout) == ISRRAN_SUCCESS);
  TESTASSERT(isrenb::test_gtpu_direct_tunneling(isrenb::tunnel_test_event::ue_removal_no_marker) == ISRRAN_SUCCESS);
  TESTASSERT(isrenb::test_gtpu_direct_tunneling(isrenb::tunnel_test_event::reest_senb) == ISRRAN_SUCCESS);

  isrlog::flush();

  isrran::console("Success");
}
