...
#include "isrenb/hdr/stack/mac/sched_phy_ch/sched_dci.h"
...
struct tbs_test_args {
  ...
  {
    if (is_ul) {
      return isrran_cqi_to_coderate(std::min(cqi + 1u, 15u), false);
    }
    return isrran_cqi_to_coderate(std::min(cqi + 1u, 15u), use_tbs_index_alt);
  }
};

bool lower_coderate(tbs_info tb, uint32_t nof_re, const tbs_test_args& args)
{
  float max_coderate = isrran_cqi_to_coderate(std::min(args.cqi + 1u, 15u), args.use_tbs_index_alt);
  float coderate     = isrran_coderate(tb.tbs_bytes * 8, nof_re);
  if (coderate > max_coderate) {
    return false;
  }
  isrran_mod_t mod =
      (args.is_ul) ? isrran_ra_ul_mod_from_mcs(tb.mcs) : isrran_ra_dl_mod_from_mcs(tb.mcs, args.use_tbs_index_alt);
  float Qm = std::min(args.get_max_Qm(), isrran_mod_bits_x_symbol(mod));
...
int test_mcs_tbs_dl_helper(const sched_cell_params_t& cell_params, const tbs_test_args& args, tbs_info* result)
{
  ...
  if (isrran_coderate(16, nof_re) > max_coderate) {
  ...
  uint32_t tbs_idx      = isrran_ra_tbs_idx_from_mcs(ret.mcs, args.use_tbs_index_alt, args.is_ul);
  int      expected_tbs = isrran_ra_tbs_from_idx(tbs_idx, args.prb_grant_size);
  ...
  for (tb2.mcs = ret.mcs + 1; tb2.mcs <= (int)args.max_mcs; ++tb2.mcs) {
    int tbs_idx2  = isrran_ra_tbs_idx_from_mcs(tb2.mcs, args.use_tbs_index_alt, args.is_ul);
    tb2.tbs_bytes = isrran_ra_tbs_from_idx(tbs_idx2, args.prb_grant_size) / 8U;
                          ...
                          bool     alt_cqi_table = false)
{
  ...
  TESTASSERT(test_mcs_tbs_dl_helper(cell_params, args, &expected_result) == ISRRAN_SUCCESS);
...
int test_mcs_lookup_specific()
{
  ...
  TESTASSERT(assert_mcs_tbs_result(6, 5, 1, 40, 3) == ISRRAN_SUCCESS);
  TESTASSERT(assert_mcs_tbs_result(6, 5, 4, 256, 4) == ISRRAN_SUCCESS);

  TESTASSERT(assert_mcs_tbs_result(100, 9, 1, 712, 28) == ISRRAN_SUCCESS);
  TESTASSERT(assert_mcs_tbs_result(100, 10, 10, 5736, 25) == ISRRAN_SUCCESS);

  // cqi=15
  TESTASSERT(assert_mcs_tbs_result(6, 15, 1, 336, 19) == ISRRAN_SUCCESS);     // I_tbs=17
  TESTASSERT(assert_mcs_tbs_result(6, 15, 6, 2152, 19) == ISRRAN_SUCCESS);    // I_tbs=17
  TESTASSERT(assert_mcs_tbs_result(100, 15, 1, 712, 28) == ISRRAN_SUCCESS);   // I_tbs=26
  TESTASSERT(assert_mcs_tbs_result(100, 15, 2, 1480, 28) == ISRRAN_SUCCESS);  // I_tbs=26
  TESTASSERT(assert_mcs_tbs_result(100, 15, 10, 7480, 28) == ISRRAN_SUCCESS); // I_tbs=26
  TESTASSERT(assert_mcs_tbs_result(100, 15, 1, 968, 27, true) == ISRRAN_SUCCESS);
...
int test_mcs_tbs_consistency_all()
{
  ...
  for (auto& nof_prb_cell : isrran::lte_cell_nof_prbs) {
    ...
    for (uint32_t prb_grant = 1; prb_grant < nof_prb_cell; ++prb_grant) {
      for (uint32_t cqi = 1; cqi < 15; ++cqi) {
        for (uint32_t max_mcs = 1; max_mcs <= 28; ++max_mcs) {
          ...
          TESTASSERT(test_mcs_tbs_dl_helper(cell_params, args, nullptr) == ISRRAN_SUCCESS);
        }
      }
    }
  }
...
int test_min_mcs_tbs_dl_helper(const sched_cell_params_t& cell_params, const tbs_test_args& args, tbs_info* result)
{
  ...
  *result         = compute_min_mcs_and_tbs_from_required_bytes(args.prb_grant_size,
  ...
  tbs_info tb_max;
  TESTASSERT(test_mcs_tbs_dl_helper(cell_params, args, &tb_max) == ISRRAN_SUCCESS);
...
int test_min_mcs_tbs_specific()
{
  ...
  args.req_bytes      = 10;
  TESTASSERT(test_min_mcs_tbs_dl_helper(cell_params, args, &result) == ISRRAN_SUCCESS);
...
void test_ul_mcs_tbs_derivation()
{
  ...
  auto compute_tbs_mcs = [&prbs, &cell_params, &max_mcs, &cqi](uint32_t Nprb, uint32_t prb_grant_size) {
    ...
    uint32_t N_isr     = 0;
    uint32_t nof_symb  = 2 * (ISRRAN_CP_NSYMB(cell_params.cfg.cell.cp) - 1) - N_isr;
    uint32_t nof_re    = nof_symb * prbs.count() * ISRRAN_NRE;
...
int main()
{
  ...
  mac_log.set_level(isrlog::basic_levels::info);
  auto& test_log = isrlog::fetch_basic_logger("TEST");
  test_log.set_level(isrlog::basic_levels::info);

  // Start the log backend.
  isrlog::init();

  TESTASSERT(isrenb::test_mcs_lookup_specific() == ISRRAN_SUCCESS);
  TESTASSERT(isrenb::test_mcs_tbs_consistency_all() == ISRRAN_SUCCESS);
  TESTASSERT(isrenb::test_min_mcs_tbs_specific() == ISRRAN_SUCCESS);
  isrenb::test_ul_mcs_tbs_derivation();
