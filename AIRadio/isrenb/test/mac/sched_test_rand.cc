...
#include "isrenb/hdr/stack/mac/sched.h"
#include "isrenb/hdr/stack/mac/sched_carrier.h"
#include "isrenb/hdr/stack/mac/sched_ue.h"
#include <algorithm>
#include <chrono>
#include <random>
#include <set>
...
{
public:
  explicit sched_diagnostic_printer(isrran::log_sink_spy& s) : s(s) {}

  ~sched_diagnostic_printer()
  {
    auto& logger = isrlog::fetch_basic_logger("TEST");
...
struct sched_tester : public isrenb::common_sched_tester {
  struct tester_user_results {
    isrenb::ul_harq_proc ul_harq;
  };
...
void sched_tester::before_sched()
{
  ...
  for (auto& it : ue_db) {
    ...
    isrenb::sched_ue*   user = it.second.get();
    ...
    tti_data.ue_data[rnti].ul_harq = *user->get_ul_harq(isrenb::to_tx_ul(tti_rx), CARRIER_IDX);
  }
}

int sched_tester::process_results()
{
  const isrenb::cc_sched_result* cc_result = sched_results.get_cc(tti_rx, CARRIER_IDX);
  isrenb::sf_output_res_t        sf_out{sched_cell_params, tti_rx, tti_info.ul_sched_result, tti_info.dl_sched_result};

  // Common tests
  TESTASSERT(test_pdcch_collisions(sf_out, CARRIER_IDX, &cc_result->pdcch_mask) == ISRRAN_SUCCESS);
  TESTASSERT(test_dci_content_common(sf_out, CARRIER_IDX) == ISRRAN_SUCCESS);
  TESTASSERT(test_sib_scheduling(sf_out, CARRIER_IDX) == ISRRAN_SUCCESS);
  TESTASSERT(test_pusch_collisions(sf_out, CARRIER_IDX, &cc_result->ul_mask) == ISRRAN_SUCCESS);
  TESTASSERT(test_pdsch_collisions(sf_out, CARRIER_IDX, &cc_result->dl_mask) == ISRRAN_SUCCESS);

  // UE dedicated tests
  TESTASSERT(run_ue_ded_tests_and_update_ctxt(sf_out) == ISRRAN_SUCCESS);
  TESTASSERT(test_harqs() == ISRRAN_SUCCESS);
  TESTASSERT(update_ue_stats() == ISRRAN_SUCCESS);
...
int sched_tester::test_harqs()
{
  ...
  for (uint32_t i = 0; i < tti_info.dl_sched_result[CARRIER_IDX].data.size(); ++i) {
    ...
    const isrenb::dl_harq_proc& h    = ue_db[rnti]->get_dl_harq(h_id, CARRIER_IDX);
    CONDERROR(h.get_tti() != isrenb::to_tx_dl(tti_rx),
              "The scheduled DL harq pid=%d does not a valid tti=%u",
              h_id,
              isrenb::to_tx_dl(tti_rx).to_uint());
  ...
  for (uint32_t i = 0; i < tti_info.ul_sched_result[CARRIER_IDX].phich.size(); ++i) {
    ...
    const auto* h     = ue_db[phich.rnti]->get_ul_harq(isrenb::to_tx_ul(tti_rx), CARRIER_IDX);
...
int sched_tester::update_ue_stats()
{
  ...
  for (uint32_t i = 0; i < tti_info.ul_sched_result[CARRIER_IDX].pusch.size(); ++i) {
    ...
    uint32_t    L, RBstart;
    isrran_ra_type2_from_riv(pusch.dci.type2_alloc.riv,
                             &L,
                             &RBstart,
  ...
  isrran::bounded_bitset<100, true> alloc_mask(sched_cell_params[CARRIER_IDX].cfg.cell.nof_prb);
  for (uint32_t i = 0; i < tti_info.dl_sched_result[CARRIER_IDX].data.size(); ++i) {
    auto& data = tti_info.dl_sched_result[CARRIER_IDX].data[i];
    TESTASSERT(isrenb::extract_dl_prbmask(sched_cell_params[CARRIER_IDX].cfg.cell,
                                          tti_info.dl_sched_result[CARRIER_IDX].data[i].dci,
                                          alloc_mask) == ISRRAN_SUCCESS);
...
int test_scheduler_rand(isrenb::sched_sim_events sim)
{
  // Create classes
  sched_tester  tester;
  isrenb::sched my_sched;

  tester.sim_cfg(std::move(sim.sim_args));

  TESTASSERT(tester.test_next_ttis(sim.tti_events) == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
...
{
  return *(v.begin() + std::uniform_int_distribution<size_t>{0, v.size() - 1}(isrenb::get_rand_gen()));
}

sched_sim_events rand_sim_params(uint32_t nof_ttis)
{
  auto             boolean_dist = []() { return std::uniform_int_distribution<>{0, 1}(isrenb::get_rand_gen()); };
  sched_sim_events sim_gen;
  uint32_t         max_conn_dur = 10000, min_conn_dur = 500;
  float            P_ul_sr = isrenb::randf() * 0.5, P_dl = isrenb::randf() * 0.5;
  ...
  uint32_t                                prb_idx = dist_prb_idx(isrenb::get_rand_gen());
  uint32_t                                nof_prb = isrran::lte_cell_nof_prbs[prb_idx];
  ...
  sim_gen.sim_args.default_ue_sim_cfg.ue_cfg.maxharq_tx = std::uniform_int_distribution<>{1, 5}(isrenb::get_rand_gen());
  ...
  sim_gen.sim_args.default_ue_sim_cfg.ue_cfg.measgap_offset   = std::uniform_int_distribution<uint32_t>{
      0, std::max(sim_gen.sim_args.default_ue_sim_cfg.ue_cfg.measgap_period, 1u) - 1}(isrenb::get_rand_gen());
  sim_gen.sim_args.default_ue_sim_cfg.ue_cfg.pucch_cfg.n_pucch_sr =
      std::uniform_int_distribution<uint32_t>{0, 2047}(isrenb::get_rand_gen());

  sim_gen.sim_args.start_tti = 0;
  sim_gen.sim_args.sched_args.pdsch_mcs =
      boolean_dist() ? -1 : std::uniform_int_distribution<>{0, 24}(isrenb::get_rand_gen());
  sim_gen.sim_args.sched_args.pusch_mcs =
      boolean_dist() ? -1 : std::uniform_int_distribution<>{0, 24}(isrenb::get_rand_gen());
  sim_gen.sim_args.sched_args.min_aggr_level = std::uniform_int_distribution<>{0, 3}(isrenb::get_rand_gen());
  ...
  for (uint32_t tti = 0; tti < nof_ttis; ++tti) {
    for (auto& u : generator.current_users) {
      ...
      if (isrenb::randf() < P_ul_sr) {
        float exp = ul_sr_exps[0] + isrenb::randf() * (ul_sr_exps[1] - ul_sr_exps[0]);
        generator.add_ul_data(rnti, (uint32_t)pow(10, exp));
      }
      if (isrenb::randf() < P_dl) {
        float exp = dl_data_exps[0] + isrenb::randf() * (dl_data_exps[1] - dl_data_exps[0]);
    ...
    bool is_prach_tti =
        isrran_prach_tti_opportunity_config_fdd(sim_gen.sim_args.cell_cfg[CARRIER_IDX].prach_config, tti, -1);
    if (is_prach_tti and generator.current_users.size() < max_nof_users and isrenb::randf() < P_prach) {
      generator.add_new_default_user(connection_dur_dist(isrenb::get_rand_gen()), sim_gen.sim_args.default_ue_sim_cfg);
    }
    generator.step_tti();
  }
...
{
  // Setup seed
  isrenb::set_randseed(isrenb::seed);
  ...
  auto& mac_log = isrlog::fetch_basic_logger("MAC");
  mac_log.set_level(isrlog::basic_levels::debug);
  auto& test_log = isrlog::fetch_basic_logger("TEST", *spy, false);
  test_log.set_level(isrlog::basic_levels::info);
  ...
  isrenb::sched_diagnostic_printer printer(*spy);
  for (uint32_t n = 0; n < N_runs; ++n) {
    printf("Sim run number: %u\n", n + 1);
    isrenb::sched_sim_events sim = isrenb::rand_sim_params(nof_ttis);
    TESTASSERT(isrenb::test_scheduler_rand(std::move(sim)) == ISRRAN_SUCCESS);
  }
