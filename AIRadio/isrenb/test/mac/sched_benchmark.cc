...
#include "isrran/adt/accumulators.h"
...
struct run_params_range {
  std::vector<uint32_t>    nof_prbs{isrran::lte_cell_nof_prbs.begin(), isrran::lte_cell_nof_prbs.end()};
...
{
  static std::vector<sched_interface::cell_cfg_t> get_cell_cfg(isrran::span<const sched_cell_params_t> cell_params)
  {
...
public:
  ...
  isrlog::basic_logger& mac_logger = isrlog::fetch_basic_logger("MAC");
  sched*                sched_ptr;
  ...
  struct throughput_stats {
    isrran::rolling_average<float>  mean_dl_tbs, mean_ul_tbs, avg_dl_mcs, avg_ul_mcs;
    isrran::rolling_average<double> avg_latency;
  ...
  int advance_tti()
  {
    ...
    for (uint32_t cc = 0; cc < get_cell_params().size(); ++cc) {
      ...
      TESTASSERT(sched_ptr->dl_sched(to_tx_dl(tti_rx).to_uint(), cc, dl_result[cc]) == ISRRAN_SUCCESS);
      TESTASSERT(sched_ptr->ul_sched(to_tx_ul(tti_rx).to_uint(), cc, ul_result[cc]) == ISRRAN_SUCCESS);
...
int run_benchmark_scenario(run_params params, std::vector<run_data>& run_results)
{
  ...
  for (uint32_t ue_idx = 0; ue_idx < params.nof_ues; ++ue_idx) {
    ...
    while (not isrran_prach_tti_opportunity_config_fdd(
        tester.get_cell_params()[ue_cfg_default.supported_cc_list[0].enb_cc_idx].cfg.prach_config,
        tester.get_tti_rx().to_uint(),
        -1)) {
      TESTASSERT(tester.advance_tti() == ISRRAN_SUCCESS);
    }
    TESTASSERT(tester.add_user(rnti, ue_cfg_default, 16) == ISRRAN_SUCCESS);
    TESTASSERT(tester.advance_tti() == ISRRAN_SUCCESS);
  }
...
run_data expected_run_result(run_params params)
{
  ...
  run_data ret{};
  int      tbs_idx      = isrran_ra_tbs_idx_from_mcs(28, false, false);
  int      tbs          = isrran_ra_tbs_from_idx(tbs_idx, params.nof_prbs);
  ret.avg_dl_throughput = static_cast<float>(tbs) * 1e3F; // bps

  tbs_idx                 = isrran_ra_tbs_idx_from_mcs(24, false, true);
  uint32_t nof_pusch_prbs = params.nof_prbs - (params.nof_prbs == 6 ? 2 : 4);
  tbs                     = isrran_ra_tbs_from_idx(tbs_idx, nof_pusch_prbs);
...
void print_benchmark_results(const std::vector<run_data>& run_results)
{
  ...
  for (uint32_t i = 0; i < run_results.size(); ++i) {
    ...
    int   tbs_idx           = isrran_ra_tbs_idx_from_mcs(28, false, false);
    int   tbs               = isrran_ra_tbs_from_idx(tbs_idx, r.params.nof_prbs);
...
int run_rate_test()
{
  ...
  for (size_t r = 0; r < nof_runs; ++r) {
    ...
    mac_logger.info("\n=== New run {} ===\n", r);
    TESTASSERT(run_benchmark_scenario(runparams, run_results) == ISRRAN_SUCCESS);
  }
  ...
  return success ? ISRRAN_SUCCESS : ISRRAN_ERROR;
}
...
int main(int argc, char* argv[])
{
  ...
  mac_log.set_level(isrlog::basic_levels::warning);
  auto& test_log = isrlog::fetch_basic_logger("TEST", *spy, false);
  test_log.set_level(isrlog::basic_levels::warning);
  ...
  if (argc == 1 or strcmp(argv[1], "test") == 0) {
    TESTASSERT(isrenb::run_rate_test() == ISRRAN_SUCCESS);
  } else if (strcmp(argv[1], "benchmark") == 0) {
    TESTASSERT(isrenb::run_benchmark() == ISRRAN_SUCCESS);
  } else {
    TESTASSERT(isrenb::run_all() == ISRRAN_SUCCESS);
  }
