...
{
  return cell_nof_prb_to_rbg(isrran::lte_cell_nof_prbs[std::uniform_int_distribution<uint32_t>{
      0, isrran::lte_cell_nof_prbs.size() - 1}(rand_gen)]);
}
