...
int test_pdu_alloc_successful(isrenb::lch_ue_manager&          lch_handler,
...
int test_retx_until_empty(isrenb::lch_ue_manager& lch_handler, int lcid, uint32_t rlc_payload_size)
{
  ...
  for (int i = 0; i < nof_pdus; ++i) {
    ...
    TESTASSERT(test_pdu_alloc_successful(lch_handler, pdu, lcid, expected_payload_size) == ISRRAN_SUCCESS);
...
int test_newtx_until_empty(isrenb::lch_ue_manager& lch_handler, int lcid, uint32_t rlc_payload_size)
{
...
{
  isrenb::lch_ue_manager lch_handler{0x46};
...
int test_lc_ch_pbr_finite()
{
  ...
  TESTASSERT(test_pdu_alloc_successful(lch_handler, pdu, drb_to_lcid(lte_drb::drb2), 200) == ISRRAN_SUCCESS);
  // Bj={0, infinity, 0, 12800, 200}
  TESTASSERT(test_pdu_alloc_successful(lch_handler, pdu, drb_to_lcid(lte_drb::drb2), 600) == ISRRAN_SUCCESS);
  // Bj={0, infinity, 0, 256000, -400}

  // TEST5 - DRB1 has lower prio, but DRB2 Bj <= 0.
  for (uint32_t i = 0; i < 50; ++i) {
    lch_handler.new_tti();
    TESTASSERT(test_pdu_alloc_successful(lch_handler, pdu, drb_to_lcid(lte_drb::drb1), 50) == ISRRAN_SUCCESS);
  }

  // TEST6 - new tti restores DRB2 Bj>=0, and DRB2 gets allocated
  lch_handler.new_tti();
  // Bj={0, infinity, 0, 256000, 8}
  TESTASSERT(test_pdu_alloc_successful(lch_handler, pdu, drb_to_lcid(lte_drb::drb2), 50) == ISRRAN_SUCCESS);
...
int main()
{
  isrenb::set_randseed(seed);
  isrran::console("This is the chosen seed: %u\n", seed);

  auto& test_log = isrlog::fetch_basic_logger("TEST", false);
  test_log.set_level(isrlog::basic_levels::info);

  // Start the log backend.
  isrlog::init();

  TESTASSERT(test_lc_ch_pbr_infinity() == ISRRAN_SUCCESS);
  TESTASSERT(test_lc_ch_pbr_finite() == ISRRAN_SUCCESS);
