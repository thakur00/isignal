...
sim_sched_args generate_default_sim_args(uint32_t nof_prb, uint32_t nof_ccs)
{
  ...
  // setup two cells
  std::vector<isrenb::sched_interface::cell_cfg_t> cell_cfg(nof_ccs, generate_default_cell_cfg(nof_prb));
...
int test_scell_activation(uint32_t sim_number, test_scell_activation_params params)
{
  ...
  uint32_t nof_prb   = isrran::lte_cell_nof_prbs[std::uniform_int_distribution<uint32_t>{0, 5}(get_rand_gen())];
  uint32_t nof_ccs   = 2;
  ...
  while (not tester.sched_sim->find_rnti(rnti1)->get_ctxt().msg3_tti_rx.is_valid() or
         isrenb::to_tx_ul(tester.sched_sim->find_rnti(rnti1)->get_ctxt().msg3_tti_rx).to_uint() >
  ...
  TESTASSERT(tester.test_next_ttis(generator.tti_events) == ISRRAN_SUCCESS);
    ...
    if (not tester.tti_info.dl_sched_result[params.pcell_idx].data.empty()) {
      ...
      if (tester.tti_info.dl_sched_result[params.pcell_idx].data[0].nof_pdu_elems[0] > 0) {
        ...
        TESTASSERT(tester.tti_info.dl_sched_result[params.pcell_idx].data[0].pdu[0][0].lcid ==
                   (uint32_t)isrran::dl_sch_lcid::SCELL_ACTIVATION);
        break;
      }
    }
    generator.step_tti();
...
int main()
{
  ...
  for (uint32_t n = 0; n < N_runs; ++n) {
    ...
    p.pcell_idx                    = 0;
    TESTASSERT(test_scell_activation(n * 2, p) == ISRRAN_SUCCESS);

    p           = {};
    p.pcell_idx = 1;
    TESTASSERT(test_scell_activation(n * 2 + 1, p) == ISRRAN_SUCCESS);
  }
