...
#ifndef ISRRAN_SCHED_SIM_UE_H
#define ISRRAN_SCHED_SIM_UE_H
...
struct ue_harq_ctxt_t {
  ...
  uint32_t              tbs       = 0;
  isrran::tti_point     last_tti_rx, first_tti_rx;
};
struct ue_cc_ctxt_t {
  std::array<ue_harq_ctxt_t, ISRRAN_FDD_NOF_HARQ> dl_harqs;
  std::array<ue_harq_ctxt_t, ISRRAN_FDD_NOF_HARQ> ul_harqs;
};
struct sim_ue_ctxt_t {
  bool                      conres_rx = false;
  uint16_t                  rnti;
  uint32_t                  preamble_idx, msg3_riv;
  isrran::tti_point         prach_tti_rx, rar_tti_rx, msg3_tti_rx, msg4_tti_rx;
...
struct sim_enb_ctxt_t {
  isrran::span<const sched_cell_params_t>  cell_params;
...
public:
  ...
  isrran::const_span<sched_cell_params_t> get_cell_params() { return cell_params; }
...
private:
  ...
  isrran::tti_point                             current_tti_rx;
