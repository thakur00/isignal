...
{
  isrran_dci_format_t dci_format = sched_ue.get_dci_format();
  uint32_t nof_dci_bits = isrran_dci_format_sizeof(&cell_params[enb_cc_idx].cfg.cell, nullptr, nullptr, dci_format);
...
int test_pdcch_one_ue()
{
  ...
  // Params
  uint32_t          nof_prb = isrran::lte_cell_nof_prbs[rand_uint{0, 5}(get_rand_gen())];
  uint16_t          rnti    = rand_uint{70, 120}(get_rand_gen());
  isrran::tti_point start_tti{rand_uint{0, 10240}(get_rand_gen())};
  ...
  for (; tti_counter < nof_ttis; ++tti_counter) {
    ...
    uint32_t dl_cqi = std::uniform_int_distribution<uint32_t>{1, 25}(isrenb::get_rand_gen());
    ...
    isrlog::fetch_basic_logger("TEST").info("PDCCH alloc result: %s", pdcch.result_to_string(true).c_str());
  }
...
int main()
{
  ...
  TESTASSERT(test_pdcch_one_ue() == ISRRAN_SUCCESS);
  TESTASSERT(test_pdcch_ue_and_sibs() == ISRRAN_SUCCESS);
  TESTASSERT(test_6prbs() == ISRRAN_SUCCESS);
