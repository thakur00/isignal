...
ue_sim::ue_sim(uint16_t                         rnti_,
               ...
               isrran::tti_point                prach_tti_rx_,
               uint32_t                         preamble_idx) :
  logger(isrlog::fetch_basic_logger("MAC"))
{
  ctxt.rnti         = rnti_;
...
void ue_sim::update_conn_state(const sf_output_res_t& sf_out)
{
  ...
  if (not ctxt.rar_tti_rx.is_valid()) {
    ...
    isrran::tti_interval rar_window{ctxt.prach_tti_rx + 3, ctxt.prach_tti_rx + 3 + rar_win_size};
  ...
  if (ctxt.rar_tti_rx.is_valid() and not ctxt.msg3_tti_rx.is_valid()) {
    ...
    isrran::tti_point expected_msg3_tti_rx = ctxt.rar_tti_rx + MSG3_DELAY_MS;
    ...
    for (uint32_t i = 0; i < dl_cc_result.data.size(); ++i) {
      if (dl_cc_result.data[i].dci.rnti == ctxt.rnti) {
        for (uint32_t j = 0; j < dl_cc_result.data[i].nof_pdu_elems[0]; ++j) {
          if (dl_cc_result.data[i].pdu[0][j].lcid == (uint32_t)isrran::dl_sch_lcid::CON_RES_ID) {
            // ConRes found
...
sched_sim_base::sched_sim_base(sched_interface*                                sched_ptr_,
  ...
  logger(isrlog::fetch_basic_logger("TEST")), sched_ptr(sched_ptr_), cell_params(cell_cfg_list.size())
{
...
{
  CONDERROR(!isrran_prach_tti_opportunity_config_fdd(
  ...
  CONDERROR(sched_ptr->ue_cfg(rnti, rach_cfg) != ISRRAN_SUCCESS, "Configuring new user rnti=0x%x to sched", rnti);
  ...
  TESTASSERT(sched_ptr->dl_rach_info(pcell_idx, rar_info) == ISRRAN_SUCCESS);
...
int sched_sim_base::ue_recfg(uint16_t rnti, const sched_interface::ue_cfg_t& ue_cfg_)
{
  ...
  CONDERROR(sched_ptr->ue_cfg(rnti, ue_cfg_) != ISRRAN_SUCCESS, "Configuring new user rnti=0x%x to sched", rnti);
...
int sched_sim_base::set_default_tti_events(const sim_ue_ctxt_t& ue_ctxt, ue_tti_events& pending_events)
{
  ...
  for (uint32_t enb_cc_idx = 0; enb_cc_idx < pending_events.cc_list.size(); ++enb_cc_idx) {
    ...
    for (uint32_t pid = 0; pid < ISRRAN_FDD_NOF_HARQ; ++pid) {
      ...
      // Set default DL CQI
      if (isrran_cqi_periodic_send(&ue_ctxt.ue_cfg.supported_cc_list[cc_feedback.ue_cc_idx].dl_cfg.cqi_report,
...
int sched_sim_base::apply_tti_events(sim_ue_ctxt_t& ue_ctxt, const ue_tti_events& events)
{
  ...
  if (not ue_ctxt.conres_rx and ue_ctxt.msg3_tti_rx.is_valid() and to_tx_ul(ue_ctxt.msg3_tti_rx) <= events.tti_rx) {
    ...
    if (cc_feedback.ul_pid >= 0 and cc_feedback.ul_ack) {
      ...
      TESTASSERT(ue_recfg(ue_ctxt.rnti, ue_cfg) == ISRRAN_SUCCESS);

      uint32_t lcid = srb_to_lcid(lte_srb::srb0); // Use SRB0 to schedule Msg4
      TESTASSERT(sched_ptr->dl_rlc_buffer_state(ue_ctxt.rnti, lcid, 50, 0) == ISRRAN_SUCCESS);
      TESTASSERT(sched_ptr->dl_mac_buffer_state(ue_ctxt.rnti, (uint32_t)isrran::dl_sch_lcid::CON_RES_ID, 1) ==
                 ISRRAN_SUCCESS);
    }
    ...
    if (cc_feedback.dl_pid >= 0 and cc_feedback.dl_ack) {
      ...
      TESTASSERT(ue_recfg(ue_ctxt.rnti, final_ue_cfg[ue_ctxt.rnti]) == ISRRAN_SUCCESS);
    }
  }
...
void sched_sim_base::new_tti(isrran::tti_point tti_rx)
{
