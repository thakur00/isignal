...
void ::isrenb::set_randseed(uint64_t seed)
{
  rand_gen = std::default_random_engine(seed);
}

std::default_random_engine& ::isrenb::get_rand_gen()
{
  return rand_gen;
}
...
common_sched_tester::common_sched_tester() : logger(isrlog::fetch_basic_logger("TEST")) {}
...
void common_sched_tester::new_test_tti()
{
  if (not tti_rx.is_valid()) {
    tti_rx = isrran::tti_point{sim_args0.start_tti};
  } else {
    tti_rx++;
  }
...
int common_sched_tester::run_ue_ded_tests_and_update_ctxt(const sf_output_res_t& sf_out)
{
  ...
  TESTASSERT(test_all_ues(enb_ctxt, sf_out) == ISRRAN_SUCCESS);
...
int common_sched_tester::process_results()
{
  ...
  TESTASSERT(test_all_common(sf_out) == ISRRAN_SUCCESS);
  ...
  for (const tti_ev::user_cfg_ev& ue_ev : tti_ev.user_updates) {
    ...
    if (ue_ev.ue_sim_cfg != nullptr) {
      if (not sched_sim->user_exists(ue_ev.rnti)) {
        // new user
        TESTASSERT(add_user(ue_ev.rnti, *ue_ev.ue_sim_cfg) == ISRRAN_SUCCESS);
      } else {
        // reconfiguration
        TESTASSERT(reconf_user(ue_ev.rnti, ue_ev.ue_sim_cfg->ue_cfg) == ISRRAN_SUCCESS);
      }
    }

    // There is a user to remove
    if (ue_ev.rem_user) {
      TESTASSERT(rem_user(ue_ev.rnti) == ISRRAN_SUCCESS);
    }
    ...
    if (ue_ev.bearer_cfg != nullptr) {
      ...
      TESTASSERT(sched_sim->bearer_cfg(ue_ev.rnti, 0, *ue_ev.bearer_cfg) == ISRRAN_SUCCESS);
    }
...
int common_sched_tester::run_tti(const tti_ev& tti_events)
{
  ...
  TESTASSERT(process_results() == ISRRAN_SUCCESS);
  tti_count++;
  return ISRRAN_SUCCESS;
}
...
int common_sched_tester::test_next_ttis(const std::vector<tti_ev>& tti_events)
{
  while (tti_count < tti_events.size()) {
    TESTASSERT(run_tti(tti_events[tti_count]) == ISRRAN_SUCCESS);
  }
  return ISRRAN_SUCCESS;
}
