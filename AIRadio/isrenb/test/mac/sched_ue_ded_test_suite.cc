...
#include "lib/include/isrran/mac/pdu.h"
#include "isrenb/hdr/stack/mac/sched_helpers.h"
#include "isrenb/hdr/stack/mac/sched_phy_ch/sf_cch_allocator.h"
                     ...
                     const sched_interface::dl_sched_data_t& pdsch)
{
  ...
  if (h.nof_txs == 0 or h.ndi != pdsch.dci.tb[0].ndi) {
    ...
    isrran_ra_dl_grant_to_grant_prb_allocation(&pdsch.dci, &grant, cell_params.nof_prb());
    uint32_t     nof_re   = isrran_ra_dl_grant_nof_re(&cell_params.cfg.cell, &dl_sf, &grant);
    float        coderate = isrran_coderate(pdsch.tbs[0] * 8, nof_re);
    isrran_mod_t mod      = isrran_ra_dl_mod_from_mcs(pdsch.dci.tb[0].mcs_idx, ue_ctxt.ue_cfg.use_tbs_index_alt);
    uint32_t     max_Qm   = ue_ctxt.ue_cfg.use_tbs_index_alt ? 8 : 6;
    uint32_t     Qm       = std::min(max_Qm, isrran_mod_bits_x_symbol(mod));
...
int test_dl_sched_result(const sim_enb_ctxt_t& enb_ctxt, const sf_output_res_t& sf_out)
{
  for (uint32_t cc = 0; cc < enb_ctxt.cell_params.size(); ++cc) {
    for (uint32_t i = 0; i < sf_out.dl_cc_result[cc].data.size(); ++i) {
      ...
      TESTASSERT(test_pdsch_grant(enb_ctxt, sf_out, cc, data) == ISRRAN_SUCCESS);
    }
  }
  return ISRRAN_SUCCESS;
}

bool is_in_measgap(isrran::tti_point tti, uint32_t period, uint32_t offset)
{
  if (period == 0) {
    return false;
  }
...
int test_ra(const sim_enb_ctxt_t& enb_ctxt, const sf_output_res_t& sf_out)
{
  for (uint32_t cc = 0; cc < enb_ctxt.cell_params.size(); ++cc) {
    ...
    for (const auto& ue_pair : enb_ctxt.ue_db) {
      ...
      isrran::tti_interval rar_window{ue.prach_tti_rx + 3, ue.prach_tti_rx + 3 + rar_win_size};
      isrran::tti_point    tti_tx_dl = to_tx_dl(sf_out.tti_rx);
      ...
      if (ue.rar_tti_rx.is_valid() and not ue.msg3_tti_rx.is_valid()) {
        ...
        isrran::tti_point expected_msg3_tti_rx = ue.rar_tti_rx + MSG3_DELAY_MS;
        ...
        for (uint32_t i = 0; i < dl_cc_res.data.size(); ++i) {
          if (dl_cc_res.data[i].dci.rnti == rnti) {
            ...
            for (uint32_t j = 0; j < dl_cc_res.data[i].nof_pdu_elems[0]; ++j) {
              if (dl_cc_res.data[i].pdu[0][j].lcid == (uint32_t)isrran::dl_sch_lcid::CON_RES_ID) {
                // ConRes found
                CONDERROR(dl_cc_res.data[i].dci.format != ISRRAN_DCI_FORMAT1 and
                              dl_cc_res.data[i].dci.format != ISRRAN_DCI_FORMAT1A,
      ...
      if (not ue.msg4_tti_rx.is_valid()) {
        ...
        for (uint32_t i = 0; i < ul_cc_res.pusch.size(); ++i) {
          if (ul_cc_res.pusch[i].dci.rnti == rnti) {
            ...
            isrran::tti_point expected_msg3_tti = rar_tti_rx + MSG3_DELAY_MS;
...
int test_meas_gaps(const sim_enb_ctxt_t& enb_ctxt, const sf_output_res_t& sf_out)
{
  for (uint32_t cc = 0; cc < enb_ctxt.cell_params.size(); ++cc) {
    ...
    for (const auto& ue_pair : enb_ctxt.ue_db) {
      ...
      uint16_t          rnti      = ue.rnti;
      uint32_t          ue_cc_idx = ue.enb_to_ue_cc_idx(cc);
      isrran::tti_point tti_tx_ul = to_tx_ul(sf_out.tti_rx), tti_tx_dl = to_tx_dl(sf_out.tti_rx),
...
{
  TESTASSERT(test_dl_sched_result(enb_ctxt, sf_out) == ISRRAN_SUCCESS);

  TESTASSERT(test_ul_sched_result(enb_ctxt, sf_out) == ISRRAN_SUCCESS);

  TESTASSERT(test_ra(enb_ctxt, sf_out) == ISRRAN_SUCCESS);

  TESTASSERT(test_meas_gaps(enb_ctxt, sf_out) == ISRRAN_SUCCESS);
