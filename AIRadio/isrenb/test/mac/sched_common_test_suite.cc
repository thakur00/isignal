...
int test_pusch_collisions(const sf_output_res_t& sf_out, uint32_t enb_cc_idx, const prbmask_t* expected_ul_mask)
{
  ...
  bool is_prach_tti_tx_ul =
      isrran_prach_in_window_config_fdd(cell_params.cfg.prach_config, to_tx_ul(sf_out.tti_rx).to_uint(), -1);
  ...
  isrran_pucch_cfg_t pucch_cfg = cell_params.pucch_cfg_common;
  pucch_cfg.n_pucch            = cell_params.nof_cce_table[ISRRAN_NOF_CFI - 1] - 1 + cell_params.cfg.n1pucch_an;
  pucch_nrb                    = std::max(pucch_nrb, isrran_pucch_m(&pucch_cfg, cell_params.cfg.cell.cp) / 2 + 1);
  ...
  for (uint32_t i = 0; i < ul_result.pusch.size(); ++i) {
    uint32_t L, RBstart;
    isrran_ra_type2_from_riv(ul_result.pusch[i].dci.type2_alloc.riv, &L, &RBstart, nof_prb, nof_prb);
                       ...
                       isrran::bounded_bitset<100, true>& alloc_mask)
{
  ...
  CONDERROR(isrran_ra_dl_dci_to_grant(&cell, &dl_sf, ISRRAN_TM1, false, &dci, &grant) == ISRRAN_ERROR,
...
int test_pdsch_collisions(const sf_output_res_t& sf_out, uint32_t enb_cc_idx, const rbgmask_t* expected_rbgmask)
{
  ...
  isrran::bounded_bitset<100, true> dl_allocs(cell_params.nof_prb()), alloc_mask(cell_params.nof_prb());
  rbgmask_t                         rbgmask{cell_params.nof_rbgs};

  auto try_dl_mask_fill = [&](const isrran_dci_dl_t& dci, const char* channel) {
    if (extract_dl_prbmask(cell_params.cfg.cell, dci, alloc_mask) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  ...
  for (uint32_t i = 0; i < dl_result.bc.size(); ++i) {
    TESTASSERT(try_dl_mask_fill(dl_result.bc[i].dci, "BC") == ISRRAN_SUCCESS);
  }

  // Decode RAR allocations, check collisions, and fill cumulative mask
  for (uint32_t i = 0; i < dl_result.rar.size(); ++i) {
    TESTASSERT(try_dl_mask_fill(dl_result.rar[i].dci, "RAR") == ISRRAN_SUCCESS);
  }

  // forbid Data in DL if its ACKs conflict with PRACH for PRB==6
  if (cell_params.nof_prb() == 6) {
    if (isrran_prach_in_window_config_fdd(cell_params.cfg.prach_config, to_tx_dl_ack(sf_out.tti_rx).to_uint(), -1)) {
  ...
  for (uint32_t i = 0; i < dl_result.data.size(); ++i) {
    TESTASSERT(try_dl_mask_fill(dl_result.data[i].dci, "data") == ISRRAN_SUCCESS);
  }

  // TEST: check for holes in the PRB mask (RBGs not fully filled)
  rbgmask.resize(cell_params.nof_rbgs);
  rbgmask.reset();
  isrran::bounded_bitset<100, true> rev_alloc = ~dl_allocs;
  for (uint32_t i = 0; i < cell_params.nof_rbgs; ++i) {
    uint32_t lim = ISRRAN_MIN((i + 1) * cell_params.P, dl_allocs.size());
...
int test_sib_scheduling(const sf_output_res_t& sf_out, uint32_t enb_cc_idx)
{
  ...
  for (bc_elem* bc = bc_begin; bc != bc_end; ++bc) {
    ...
    CONDERROR(bc->dci.format != ISRRAN_DCI_FORMAT1A, "Invalid DCI format for SIB%d", bc->index);
    ...
    isrran::tti_point    win_start{sfn_start * 10 + sf};
    isrran::tti_interval window{win_start, win_start + cell_params.cfg.si_window_ms};
                          ...
                          const isrran::bounded_bitset<128, true>* expected_cce_mask)
{
  ...
  int         ret         = isrran_regs_pdcch_ncce(cell_params.regs.get(), dl_result.cfi);
  TESTASSERT(ret > 0);
  uint32_t                          ncce = ret;
  isrran::bounded_bitset<128, true> used_cce{ncce};

  // Helper Function: checks if there is any collision. If not, fills the PDCCH mask
  auto try_cce_fill = [&](const isrran_dci_location_t& dci_loc, const char* ch) {
...
int test_dci_content_common(const sf_output_res_t& sf_out, uint32_t enb_cc_idx)
{
  ...
  for (uint32_t i = 0; i < ul_result.pusch.size(); ++i) {
    ...
    if (pusch.dci.tb.rv == 0) {
      // newTx
      CONDERROR(pusch.dci.format != ISRRAN_DCI_FORMAT0, "Incorrect UL DCI format");
  ...
  auto test_ra_bc_coderate = [&dl_sf, &cell_params](uint32_t tbs, const isrran_dci_dl_t& dci) {
    isrran_pdsch_grant_t grant = {};
    isrran_ra_dl_grant_to_grant_prb_allocation(&dci, &grant, cell_params.cfg.cell.nof_prb);
    uint32_t       nof_re   = isrran_ra_dl_grant_nof_re(&cell_params.cfg.cell, &dl_sf, &grant);
    float          coderate = isrran_coderate(tbs * 8, nof_re);
    const uint32_t Qm       = 2;
    CONDERROR(
        coderate > 0.932f * Qm, "Max coderate was exceeded from %s DCI", dci.rnti == ISRRAN_SIRNTI ? "SIB" : "RAR");
    return ISRRAN_SUCCESS;
  };
  ...
  for (uint32_t i = 0; i < dl_result.bc.size(); ++i) {
    ...
    TESTASSERT(test_ra_bc_coderate(bc.tbs, bc.dci) == ISRRAN_SUCCESS);
  }
  ...
  for (uint32_t i = 0; i < dl_result.rar.size(); ++i) {
    ...
    TESTASSERT(test_ra_bc_coderate(rar.tbs, rar.dci) == ISRRAN_SUCCESS);
  }
...
int test_all_common(const sf_output_res_t& sf_out)
{
  for (uint32_t i = 0; i < sf_out.cc_params.size(); ++i) {
    TESTASSERT(test_pusch_collisions(sf_out, i, nullptr) == ISRRAN_SUCCESS);
    TESTASSERT(test_pdsch_collisions(sf_out, i, nullptr) == ISRRAN_SUCCESS);
    TESTASSERT(test_sib_scheduling(sf_out, i) == ISRRAN_SUCCESS);
    TESTASSERT(test_pdcch_collisions(sf_out, i, nullptr) == ISRRAN_SUCCESS);
    TESTASSERT(test_dci_content_common(sf_out, i) == ISRRAN_SUCCESS);
  }
  return ISRRAN_SUCCESS;
}
