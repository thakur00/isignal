...
#ifndef ISRENB_TEST_HELPERS_H
#define ISRENB_TEST_HELPERS_H
...
extern isrlog::basic_levels log_level;
  ...
  while ((opt = getopt(argc, argv, "i")) != -1) {
    switch (opt) {
      case 'i':
        ...
        break;
      case 'v':
        log_level = isrlog::basic_levels::debug;
        break;
      default:
        usage(argv[0]);
        exit(-1);
...
public:
  ...
  struct ho_req_ack {
    uint16_t                                      rnti;
    isrran::unique_byte_buffer_t                  ho_cmd_pdu;
...
public:
  struct last_sdu_t {
    uint16_t                     rnti;
    uint32_t                     lcid;
    isrran::unique_byte_buffer_t sdu;
  } last_sdu;
  struct lcid_cfg_t {
    bool                         enable_integrity  = false;
    bool                         enable_encryption = false;
    isrran::pdcp_lte_state_t     state{};
  ...
  bool set_bearer_state(uint16_t rnti, uint32_t lcid, const isrran::pdcp_lte_state_t& state) override
  {
...
public:
  ...
  void write_sdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu) override
  {
...
int parse_default_cfg(isrenb::all_args_t* args, rrc_cfg_t* rrc_cfg, phy_cfg_t* phy_cfg, rrc_nr_cfg_t* rrc_nr_cfg);
int parse_default_cfg(rrc_cfg_t* rrc_cfg, isrenb::all_args_t& args);
int parse_default_cfg(rrc_nr_cfg_t* rrc_nr_cfg);
int parse_default_cfg_phy(rrc_cfg_t* rrc_cfg, phy_cfg_t* phy_cfg, isrenb::all_args_t& args);

template <typename ASN1Type>
bool unpack_asn1(ASN1Type& asn1obj, isrran::const_byte_span pdu)
{
  asn1::cbit_ref bref{pdu.data(), (uint32_t)pdu.size()};
  if (asn1obj.unpack(bref) != asn1::ISRASN_SUCCESS) {
    isrlog::fetch_basic_logger("TEST").error("Failed to unpack ASN1 type");
    return false;
  }
  return true;
}

int bring_rrc_to_reconf_state(isrenb::rrc& rrc, isrran::timer_handler& timers, uint16_t rnti);
