...
struct mobility_tester {
  explicit mobility_tester(const test_event& args_) :
    args(args_), logger(isrlog::fetch_basic_logger("RRC")), rrc(&task_sched, bearers)
  {
  ...
  int         run_preamble()
  {
    ...
    logger.set_level(isrlog::basic_levels::none);
...
protected:
  ...
  {
    isrenb::all_args_t all_args;
    TESTASSERT(test_helpers::parse_default_cfg(&cfg, all_args) == ISRRAN_SUCCESS);
...
struct s1ap_mobility_tester : public mobility_tester {
  ...
  {
    TESTASSERT(generate_rrc_cfg_common() == ISRRAN_SUCCESS);
...
int test_s1ap_mobility(isrran::log_sink_spy& spy, test_event test_params)
{
  ...
  TESTASSERT(tester.generate_rrc_cfg() == ISRRAN_SUCCESS);
  TESTASSERT(tester.setup_rrc() == ISRRAN_SUCCESS);
  TESTASSERT(tester.run_preamble() == ISRRAN_SUCCESS);
  test_dummies::s1ap_mobility_dummy& s1ap = tester.s1ap;

  /* Receive correct measConfig */
  dl_dcch_msg_s dl_dcch_msg;
  TESTASSERT(test_helpers::unpack_asn1(dl_dcch_msg, isrran::make_span(tester.pdcp.last_sdu.sdu)));
  ...
  TESTASSERT(s1ap.last_ho_required.target_plmn.to_string() == "00101");
  {
    ...
    TESTASSERT(hoprep.unpack(bref) == asn1::ISRASN_SUCCESS);
  ...
  TESTASSERT(test_helpers::unpack_asn1(ho_cmd, isrran::make_span(tester.pdcp.last_sdu.sdu)));
...
int test_s1ap_tenb_mobility(test_event test_params)
{
  ...
  if (test_params == test_event::wrong_target_cell) {
    TESTASSERT_EQ(ISRRAN_INVALID_RNTI, rnti);
  ...
  if (test_params == test_event::unknown_qci) {
    TESTASSERT(rnti == ISRRAN_INVALID_RNTI);
    TESTASSERT(cause.type().value == asn1::s1ap::cause_c::types_opts::radio_network);
  ...
  isrran::as_security_config_t as_sec_cfg = sec_cfg.get_as_sec_cfg();
  ...
  TESTASSERT(ho_cmd.unpack(bref) == asn1::ISRASN_SUCCESS);
...
int test_intraenb_mobility(isrran::log_sink_spy& spy, test_event test_params)
{
  ...
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb1)].direction == isrenb::mac_lc_ch_cfg_t::DL);
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb2)].direction == isrenb::mac_lc_ch_cfg_t::DL);
  ...
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb0)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb1)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb2)].direction == isrenb::mac_lc_ch_cfg_t::IDLE);
  TESTASSERT(ue_cfg->ue_bearers[drb_to_lcid(lte_drb::drb1)].direction == isrenb::mac_lc_ch_cfg_t::IDLE);
  ...
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb0)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb1)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
  TESTASSERT(ue_cfg->ue_bearers[srb_to_lcid(lte_srb::srb2)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
  TESTASSERT(ue_cfg->ue_bearers[drb_to_lcid(lte_drb::drb1)].direction == isrenb::mac_lc_ch_cfg_t::BOTH);
...
int main(int argc, char** argv)
{
  ...
  auto& logger = isrlog::fetch_basic_logger("RRC", false);
  logger.set_level(isrlog::basic_levels::info);
  logger.set_hex_dump_max_size(1024);
