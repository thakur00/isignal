...
int test_correct_meascfg_calculation()
{
  ...
  {
    // TEST: creation of a meas_cfg using the isrenb::rrc_cfg_t
    rrc_cfg_t          cfg;
...
int test_minimize_meascfg_reordering()
{
  ...
  isrenb::all_args_t all_args;
  TESTASSERT(test_helpers::parse_default_cfg(&default_cfg, all_args) == ISRRAN_SUCCESS);
  cfg1                  = default_cfg;
  cfg1.enb_id           = 0x19B;
  cfg1.cell.nof_prb     = 6;
  ...
  TESTASSERT(isrran::get_carrier_freq(mcfg1.meas_obj_to_add_mod_list[0]) ==
             isrran::get_carrier_freq(mcfg2.meas_obj_to_add_mod_list[0]));
  TESTASSERT(isrran::get_carrier_freq(mcfg1.meas_obj_to_add_mod_list[1]) ==
             isrran::get_carrier_freq(mcfg2.meas_obj_to_add_mod_list[1]));
  TESTASSERT(std::is_sorted(
      mcfg1.meas_obj_to_add_mod_list.begin(), mcfg1.meas_obj_to_add_mod_list.end(), isrran::rrc_obj_id_cmp{}));
  TESTASSERT(std::is_sorted(
      mcfg2.meas_obj_to_add_mod_list.begin(), mcfg2.meas_obj_to_add_mod_list.end(), isrran::rrc_obj_id_cmp{}));
