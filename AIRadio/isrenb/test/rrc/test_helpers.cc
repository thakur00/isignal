...
int parse_default_cfg_phy(rrc_cfg_t* rrc_cfg, phy_cfg_t* phy_cfg, isrenb::all_args_t& args)
{
  *rrc_cfg                  = {};
  ...
  isrlog::fetch_basic_logger("TEST").debug("sib file path=%s", args.enb_files.sib_config.c_str());

  args.enb.enb_id = 0x19B;
  TESTASSERT(isrran::string_to_mcc("001", &args.stack.s1ap.mcc));
  TESTASSERT(isrran::string_to_mnc("01", &args.stack.s1ap.mnc));
...
int bring_rrc_to_reconf_state(isrenb::rrc& rrc, isrran::timer_handler& timers, uint16_t rnti)
{
  ...
  TESTASSERT(s1ap_pdu.unpack(bref) == asn1::ISRASN_SUCCESS);
