...
#ifndef ISRRAN_SCHED_NR_SCH_H
#define ISRRAN_SCHED_NR_SCH_H
...
using pdsch_alloc_result = isrran::expected<pdsch_t*, alloc_result>;
...
public:
  ...
  prb_bitmap occupied_prbs(uint32_t ss_id, isrran_dci_format_nr_t dci_fmt) const
  {
    if (dci_fmt == isrran_dci_format_nr_1_0) {
      const isrran_search_space_t* ss = bwp_cfg.get_ss(ss_id);
      if (ss != nullptr and ISRRAN_SEARCH_SPACE_IS_COMMON(ss->type)) {
   ...
   * @return pdsch_t* of allocated PDSCH in case of success. alloc_result error code in case of failure
   */
  pdsch_alloc_result alloc_si_pdsch(uint32_t ss_id, const prb_grant& grant, isrran_dci_dl_nr_t& dci);
  /// Similar to alloc_si_pdsch, but it doesn't verify if input parameters are valid
  pdsch_t& alloc_si_pdsch_unchecked(uint32_t ss_id, const prb_grant& grant, isrran_dci_dl_nr_t& dci);
   ...
   * @return pdsch_t* of allocated PDSCH in case of success. alloc_result error code in case of failure
   */
  pdsch_alloc_result alloc_rar_pdsch(const prb_grant& grant, isrran_dci_dl_nr_t& dci);
  /// Similar to alloc_rar_pdsch, but it doesn't verify if input parameters are valid
  pdsch_t& alloc_rar_pdsch_unchecked(const prb_grant& grant, isrran_dci_dl_nr_t& dci);
...
private:
  alloc_result is_grant_valid_common(isrran_search_space_type_t ss_type,
                                     isrran_dci_format_nr_t     dci_fmt,
                                     uint32_t                   coreset_id,
                                     const prb_grant&           grant) const;
...
using pusch_alloc_result = isrran::expected<pusch_t*, alloc_result>;
...
public:
  ...
  /// allocations
  alloc_result is_grant_valid(isrran_search_space_type_t ss_type, const prb_grant& grant, bool verbose = true) const;
   ...
   */
  pusch_alloc_result
  alloc_pusch(const isrran_search_space_type_t ss_type, const prb_grant& grant, isrran_dci_ul_nr_t& dci);
   ...
   */
  pusch_t& alloc_pusch_unchecked(const prb_grant& grant, isrran_dci_ul_nr_t& dci);
