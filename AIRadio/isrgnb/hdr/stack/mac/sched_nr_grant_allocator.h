...
#ifndef ISRRAN_SCHED_NR_GRANT_ALLOCATOR_H
#define ISRRAN_SCHED_NR_GRANT_ALLOCATOR_H

#include "lib/include/isrran/adt/circular_array.h"
...
struct harq_ack_t {
  const isrran::phy_cfg_nr_t* phy_cfg;
  isrran_harq_ack_resource_t  res;
};
using harq_ack_list_t = isrran::bounded_vector<harq_ack_t, MAX_GRANTS>;
...
struct bwp_slot_grid {
  ...
  isrran::unique_pool_ptr<tx_harq_softbuffer> rar_softbuffer;
...
private:
  ...
  isrran::bounded_vector<bwp_slot_grid, TTIMOD_SZ> slots;
};
...
public:
  ...
  alloc_result alloc_rar_and_msg3(uint16_t                                ra_rnti,
                                  uint32_t                                aggr_idx,
                                  prb_interval                            interv,
                                  isrran::const_span<dl_sched_rar_info_t> pending_rars);
  ...
  prb_bitmap occupied_dl_prbs(slot_point sl_tx, uint32_t ss_id, isrran_dci_format_nr_t dci_fmt) const
  {
...
#endif // ISRRAN_SCHED_NR_GRANT_ALLOCATOR_H
