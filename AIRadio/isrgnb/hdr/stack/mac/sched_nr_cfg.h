...
#ifndef ISRRAN_SCHED_NR_CFG_H
#define ISRRAN_SCHED_NR_CFG_H
...
using pdcch_dl_list_t    = isrran::bounded_vector<pdcch_dl_t, MAX_GRANTS>;
using pdcch_ul_list_t    = isrran::bounded_vector<pdcch_ul_t, MAX_GRANTS>;
using pdsch_list_t       = isrran::bounded_vector<pdsch_t, MAX_GRANTS>;
using pucch_list_t       = isrran::bounded_vector<pucch_t, MAX_GRANTS>;
using pusch_list_t       = isrran::bounded_vector<pusch_t, MAX_GRANTS>;
using nzp_csi_rs_list    = isrran::bounded_vector<isrran_csi_rs_nzp_resource_t, mac_interface_phy_nr::MAX_NZP_CSI_RS>;
using ssb_t              = mac_interface_phy_nr::ssb_t;
using ssb_list           = isrran::bounded_vector<ssb_t, mac_interface_phy_nr::MAX_SSB>;
...
using pdcch_cce_pos_list = isrran::bounded_vector<uint32_t, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR>;
using bwp_cce_pos_list   = std::array<std::array<pdcch_cce_pos_list, MAX_NOF_AGGR_LEVELS>, ISRRAN_NOF_SF_X_FRAME>;
...
struct bwp_params_t {
  ...
  isrran::bounded_vector<slot_cfg, ISRRAN_NOF_SF_X_FRAME> slots;
  ...
  isrran::optional_vector<bwp_cce_pos_list> common_cce_list;
  ...
  const isrran_search_space_t* get_ss(uint32_t ss_id) const
  {
...
private:
  ...
  isrran::optional_vector<coreset_cached_params> coresets;
};
...
struct cell_config_manager {
  ...
  isrran_mib_nr_t                                   mib;
  isrran::phy_cfg_nr_t::ssb_cfg_t                   ssb = {};
  ...
  const isrran::phy_cfg_nr_t                        default_ue_phy_cfg;
