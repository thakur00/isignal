...
#ifndef ISRRAN_UE_CFG_MANAGER_H
#define ISRRAN_UE_CFG_MANAGER_H
...
using ue_cc_cfg_list = isrran::bounded_vector<sched_nr_ue_cc_cfg_t, SCHED_NR_MAX_CARRIERS>;
...
public:
  ...
  uint32_t cc   = ISRRAN_MAX_CARRIERS;

  const ue_cfg_manager&       ue_cfg() const { return *cfg_; }
  const isrran::phy_cfg_nr_t& phy() const { return cfg_->phy_cfg; }
  ...
  isrran::const_span<uint32_t> cce_pos_list(uint32_t search_id, uint32_t slot_idx, uint32_t aggr_idx) const
  {
    if (cce_positions_list.size() > ss_id_to_cce_idx[search_id]) {
      auto& lst = cce_pos_list(search_id);
      return lst[slot_idx][aggr_idx];
    }
    return isrran::const_span<uint32_t>{};
  }
  ...
  {
    if (phy().duplex.mode == ISRRAN_DUPLEX_MODE_TDD) {
  ...
  const isrran_dci_cfg_nr_t& get_dci_cfg() const { return cached_dci_cfg; }

  int find_ss_id(isrran_dci_format_nr_t dci_fmt) const;
...
private:
  ...
  std::array<uint32_t, ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE> ss_id_to_cce_idx;
  isrran_dci_cfg_nr_t                                        cached_dci_cfg;
};

} // namespace sched_nr_impl
} // namespace isrenb

#endif // ISRRAN_UE_CFG_MANAGER_H
