...
#ifndef ISRRAN_SCHED_NR_HARQ_H
#define ISRRAN_SCHED_NR_HARQ_H
...
public:
  ...
  isrran::unique_byte_buffer_t* get_tx_pdu() { return &pdu; }
  ...
  bool new_retx(slot_point slot_tx, slot_point slot_ack, const prb_grant& grant, isrran_dci_dl_nr_t& dci);

private:
  void fill_dci(isrran_dci_dl_nr_t& dci);

  isrran::unique_pool_ptr<tx_harq_softbuffer> softbuffer;
...
public:
  ...
  bool new_tx(slot_point slot_tx, const prb_grant& grant, uint32_t mcs, uint32_t max_retx, isrran_dci_ul_nr_t& dci);

  bool new_retx(slot_point slot_tx, const prb_grant& grant, isrran_dci_ul_nr_t& dci);
...
private:
  void fill_dci(isrran_dci_ul_nr_t& dci);

  isrran::unique_pool_ptr<rx_harq_softbuffer> softbuffer;
};

class harq_entity
{
public:
  explicit harq_entity(uint16_t rnti, uint32_t nprb, uint32_t nof_harq_procs, isrlog::basic_logger& logger);
