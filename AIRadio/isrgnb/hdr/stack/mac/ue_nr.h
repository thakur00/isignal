...
#ifndef ISRENB_UE_NR_H
#define ISRENB_UE_NR_H
...
class ue_nr : public isrran::read_pdu_interface
{
public:
  ue_nr(uint16_t                rnti,
        uint32_t                enb_cc_idx,
  ...
  void     store_msg3(isrran::unique_byte_buffer_t pdu);

  int generate_pdu(isrran::byte_buffer_t* pdu, uint32_t grant_size, isrran::const_span<uint32_t> subpdu_lcids);
  ...
  void       metrics_dl_cqi(const isrran_uci_cfg_nr_t& cfg_, uint32_t dl_cqi);
...
private:
  ...
  isrran::mac_sch_pdu_nr                    mac_pdu_dl, mac_pdu_ul;
  std::vector<isrran::unique_byte_buffer_t> ue_tx_buffer;
  isrran::block_queue<isrran::unique_byte_buffer_t>
                               ue_rx_pdu_queue; ///< currently only DCH PDUs supported (add BCH, PCH, etc)
  isrran::unique_byte_buffer_t ue_rlc_buffer;

  isrran::unique_byte_buffer_t last_msg3; ///< holds UE ID received in Msg3 for ConRes CE
