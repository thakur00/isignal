...
#ifndef ISRRAN_SCHED_NR_INTERFACE_H
#define ISRRAN_SCHED_NR_INTERFACE_H

#include "isrenb/hdr/stack/mac/common/sched_config.h"
...
const static size_t   SCHED_NR_MAX_HARQ         = ISRRAN_DEFAULT_HARQ_PROC_DL_NR;
const static size_t   SCHED_NR_MAX_BWP_PER_CELL = 2;
const static size_t   SCHED_NR_MAX_LCID         = isrran::MAX_NR_NOF_BEARERS;
...
struct sched_nr_ue_cfg_t {
  ...
  isrran::bounded_vector<sched_nr_ue_cc_cfg_t, SCHED_NR_MAX_CARRIERS> carriers;
...
struct sched_nr_cell_cfg_t {
  ...
  isrran::optional<asn1::rrc_nr::tdd_ul_dl_cfg_common_s> tdd_ul_dl_cfg_common;
...
public:
  ...
  struct msg3_grant_t {
    rar_info_t         data;
    isrran_dci_ul_nr_t msg3_dci = {};
  };
  struct rar_t {
    isrran::bounded_vector<msg3_grant_t, MAX_GRANTS> grants;
  };

  ////// DL data signalling //////

  struct dl_pdu_t {
    isrran::bounded_vector<uint32_t, MAX_SUBPDUS> subpdus;
  };
  ...
  using sched_sib_list_t    = isrran::bounded_vector<uint32_t, MAX_GRANTS>; /// list of SI indexes
  using sched_rar_list_t    = isrran::bounded_vector<rar_t, MAX_GRANTS>;
  using sched_dl_pdu_list_t = isrran::bounded_vector<dl_pdu_t, MAX_GRANTS>;
  struct dl_res_t {
    dl_sched_t          phy;
  ...
  virtual int  config(const sched_args_t& sched_cfg, isrran::const_span<sched_nr_cell_cfg_t> ue_cfg) = 0;
...
#endif // ISRRAN_SCHED_NR_INTERFACE_H
