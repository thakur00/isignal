...
#ifndef ISRRAN_DUMMY_NR_CLASSES_H
#define ISRRAN_DUMMY_NR_CLASSES_H
...
{
public:
  int  read_pdu_bcch_bch(const uint32_t tti, isrran::byte_buffer_t& buffer) { return ISRRAN_SUCCESS; }
  int  read_pdu_bcch_dlsch(uint32_t sib_index, isrran::byte_buffer_t& buffer) { return ISRRAN_SUCCESS; }
  int  add_user(uint16_t rnti, uint32_t pcell_cc_idx) { return ISRRAN_SUCCESS; }
  int  update_user(uint16_t new_rnti, uint16_t old_rnti) { return ISRRAN_SUCCESS; }
...
{
public:
  int  read_pdu(uint16_t rnti, uint32_t lcid, uint8_t* payload, uint32_t nof_bytes) override { return ISRRAN_SUCCESS; }
...
public:
  ...
  int remove_ue(uint16_t rnti) override { return ISRRAN_SUCCESS; }

  std::vector<sched_nr_cell_cfg_t> nr_cells;
  uint16_t                         last_ue_cfg_rnti = ISRRAN_INVALID_RNTI;
...
{
public:
  int set_common_cfg(const phy_interface_rrc_nr::common_cfg_t& common_cfg_) override { return ISRRAN_SUCCESS; }
};

} // namespace isrenb

#endif // ISRRAN_DUMMY_NR_CLASSES_H
