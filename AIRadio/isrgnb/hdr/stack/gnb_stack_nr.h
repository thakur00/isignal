...
#ifndef ISRRAN_GNB_STACK_NR_H
#define ISRRAN_GNB_STACK_NR_H

#include "isrenb/hdr/stack/upper/pdcp.h"
#include "isrenb/hdr/stack/upper/rlc.h"
#include "isrgnb/hdr/stack/mac/mac_nr.h"
#include "isrgnb/hdr/stack/rrc/rrc_nr.h"
#include "isrgnb/hdr/stack/sdap/sdap.h"
...
public:
  ...
  bool        get_metrics(isrenb::stack_metrics_t* metrics) final;
  ...
  void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu) override;
  ...
  // X2 data interface
  void write_sdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t sdu, int pdcp_sn = -1) final
  {
    auto task = [this, rnti, lcid, pdcp_sn](isrran::unique_byte_buffer_t& sdu) {
      pdcp.write_sdu(rnti, lcid, std::move(sdu), pdcp_sn);
    };
    gtpu_task_queue.push(std::bind(task, std::move(sdu)));
  }
  std::map<uint32_t, isrran::unique_byte_buffer_t> get_buffered_pdus(uint16_t rnti, uint32_t lcid) final
  {
...
private:
  ...
  isrlog::basic_logger& ngap_logger;
  isrlog::basic_logger& gtpu_logger;
  isrlog::basic_logger& stack_logger;

  isrran::ngap_pcap ngap_pcap;

  // task scheduling
  static const int                      STACK_MAIN_THREAD_PRIO = 4;
  isrran::task_scheduler                task_sched;
  isrran::task_multiqueue::queue_handle sync_task_queue, gtpu_task_queue, metrics_task_queue, gnb_task_queue,
      x2_task_queue;
  ...
  isrenb::pdcp                  pdcp;
  isrenb::rrc_nr                rrc;
  std::unique_ptr<isrenb::ngap> ngap;
  std::unique_ptr<isrenb::gtpu> gtpu;
