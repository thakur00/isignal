...
#ifndef ISRRAN_RRC_NR_UE_H
#define ISRRAN_RRC_NR_UE_H
...
public:
  ...
  void send_dl_information_transfer(isrran::unique_byte_buffer_t sdu);

  /** TS 38.331 - 5.7.2 UL information transfer */
  void handle_ul_information_transfer(const asn1::rrc_nr::ul_info_transfer_s& msg);

  // NGAP interface
  void establish_eps_bearer(uint32_t pdu_session_id, isrran::const_byte_span nas_pdu, uint32_t lcid, uint32_t five_qi);

  /* TS 38.331 - 5.3.4 Initial AS security activation */
  void send_security_mode_command(isrran::unique_byte_buffer_t nas_pdu);
...
private:
  ...
  int send_dl_dcch(isrran::nr_srb srb, const asn1::rrc_nr::dl_dcch_msg_s& dl_dcch_msg);
  ...
  // logging helpers
  template <class M>
  void log_rrc_message(isrran::nr_srb          srb,
                       const direction_t       dir,
                       isrran::const_byte_span pdu,
                       const M&                msg,
                       const char*             msg_type);
  template <class M>
  void log_rrc_container(const direction_t dir, isrran::const_byte_span pdu, const M& msg, const char* msg_type);
  ...
  std::vector<isrran::unique_byte_buffer_t> nas_pdu_queue;
  ...
  // Security helper
  isrgnb::nr_security_context sec_ctx;
  ...
  bool     endc       = false;
  uint16_t eutra_rnti = ISRRAN_INVALID_RNTI;
};

} // namespace isrenb

#endif // ISRRAN_RRC_NR_UE_H
