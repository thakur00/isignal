...
#ifndef ISRENB_RRC_NR_H
#define ISRENB_RRC_NR_H
...
{
public:
  explicit rrc_nr(isrran::task_sched_handle task_sched_);
  ~rrc_nr();
  ...
  void get_metrics(isrenb::rrc_metrics_t& m);

  void    config_phy();
  void    config_mac();
  int32_t generate_sibs();
  int     read_pdu_bcch_bch(const uint32_t tti, isrran::byte_buffer_t& buffer) final;
  int     read_pdu_bcch_dlsch(uint32_t sib_index, isrran::byte_buffer_t& buffer) final;
  ...
  int  start_security_mode_procedure(uint16_t rnti, isrran::unique_byte_buffer_t nas_pdu) final;
  ...
  void write_dl_info(uint16_t rnti, isrran::unique_byte_buffer_t sdu) final;
...
private:
  ...
  struct cell_ctxt_t {
    ...
    std::vector<isrran::unique_byte_buffer_t>             sib_buffer;
    std::unique_ptr<const asn1::rrc_nr::cell_group_cfg_s> master_cell_group;
    isrran::phy_cfg_nr_t                                  default_phy_ue_cfg_nr;
  };
  ...
  /// Private Methods
  void handle_pdu(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu);
  void handle_ul_ccch(uint16_t rnti, isrran::const_byte_span pdu);
  void handle_ul_dcch(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu);
  ...
  template <class T>
  isrran::unique_byte_buffer_t pack_into_pdu(const T& msg, const char* context_name = nullptr)
  {
    ...
    if (msg.pack(bref) == asn1::ISRASN_ERROR_ENCODE_FAIL) {
