...
#include "isrenb/hdr/stack/upper/gtpu_pdcp_adapter.h"
...
gnb_stack_nr::gnb_stack_nr(isrlog::sink& log_sink) :
  task_sched{512, 128},
  thread("gNB"),
  mac_logger(isrlog::fetch_basic_logger("MAC-NR", log_sink)),
  rlc_logger(isrlog::fetch_basic_logger("RLC-NR", log_sink, false)),
  pdcp_logger(isrlog::fetch_basic_logger("PDCP-NR", log_sink, false)),
  rrc_logger(isrlog::fetch_basic_logger("RRC-NR", log_sink, false)),
  stack_logger(isrlog::fetch_basic_logger("STCK-NR", log_sink, false)),
  gtpu_logger(isrlog::fetch_basic_logger("GTPU", log_sink, false)),
  ngap_logger(isrlog::fetch_basic_logger("NGAP", log_sink, false)),
  mac(&task_sched),
  rrc(&task_sched),
  pdcp(&task_sched, pdcp_logger),
  bearer_manager(new isrenb::enb_bearer_manager()),
  rlc(rlc_logger)
{
                       ...
                       x2_interface*           x2_)
{
  ...
  ngap_logger.set_level(isrlog::str_to_basic_level(args.log.s1ap_level));
  gtpu_logger.set_level(isrlog::str_to_basic_level(args.log.gtpu_level));
  isrlog::fetch_basic_logger("COMN", false).set_level(isrlog::str_to_basic_level(args.log.stack_level));
  ...
  isrlog::fetch_basic_logger("COMN", false).set_hex_dump_max_size(args.log.stack_hex_limit);

  if (x2_ == nullptr) {
    // SA mode
    ngap.reset(new isrenb::ngap(&task_sched, ngap_logger, &isrran::get_rx_io_manager()));
    gtpu.reset(new isrenb::gtpu(&task_sched, gtpu_logger, &isrran::get_rx_io_manager()));
    gtpu_adapter.reset(new gtpu_pdcp_adapter(gtpu_logger, nullptr, &pdcp, gtpu.get(), *bearer_manager));
  }

  // Init all layers
  if (mac.init(args.mac, phy, nullptr, &rlc, &rrc) != ISRRAN_SUCCESS) {
  ...
  if (rrc.init(rrc_cfg_, phy, &mac, &rlc, &pdcp, ngap.get(), gtpu.get(), *bearer_manager, x2_) != ISRRAN_SUCCESS) {
...
void gnb_stack_nr::stop_impl()
{
  ...
  task_sched.stop();
  isrran::get_background_workers().stop();
...
bool gnb_stack_nr::get_metrics(isrenb::stack_metrics_t* metrics)
{
...
void gnb_stack_nr::write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)
{
  // not implemented
}
...
int gnb_stack_nr::slot_indication(const isrran_slot_cfg_t& slot_cfg)
{
  return mac.slot_indication(slot_cfg);
}
gnb_stack_nr::dl_sched_t* gnb_stack_nr::get_dl_sched(const isrran_slot_cfg_t& slot_cfg)
{
  return mac.get_dl_sched(slot_cfg);
}
gnb_stack_nr::ul_sched_t* gnb_stack_nr::get_ul_sched(const isrran_slot_cfg_t& slot_cfg)
{
  return mac.get_ul_sched(slot_cfg);
}
int gnb_stack_nr::pucch_info(const isrran_slot_cfg_t& slot_cfg, const mac_interface_phy_nr::pucch_info_t& pucch_info)
{
  return mac.pucch_info(slot_cfg, pucch_info);
}
int gnb_stack_nr::pusch_info(const isrran_slot_cfg_t& slot_cfg, mac_interface_phy_nr::pusch_info_t& pusch_info)
{
