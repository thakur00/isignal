...
dl_harq_proc::dl_harq_proc(uint32_t id_, uint32_t nprb) :
  harq_proc(id_), softbuffer(harq_softbuffer_pool::get_instance().get_tx(nprb)), pdu(isrran::make_byte_buffer())
{}

void dl_harq_proc::fill_dci(isrran_dci_dl_nr_t& dci)
{
...
bool dl_harq_proc::new_retx(slot_point slot_tx, slot_point slot_ack, const prb_grant& grant, isrran_dci_dl_nr_t& dci)
{
...
void ul_harq_proc::fill_dci(isrran_dci_ul_nr_t& dci)
{
  const static uint32_t rv_idx[4] = {0, 2, 3, 1};
...
bool ul_harq_proc::new_retx(slot_point slot_tx, const prb_grant& grant, isrran_dci_ul_nr_t& dci)
{
...
harq_entity::harq_entity(uint16_t rnti_, uint32_t nprb, uint32_t nof_harq_procs, isrlog::basic_logger& logger_) :
