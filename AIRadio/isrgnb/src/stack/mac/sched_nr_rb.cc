...
#include "isrgnb/hdr/stack/mac/sched_nr_rb.h"
...
{
  isrran_assert(bwp_nof_prb > 0 and bwp_nof_prb <= 275, "Invalid BWP size");
  if (bwp_nof_prb <= 36) {
...
uint32_t get_nof_rbgs(uint32_t bwp_nof_prb, uint32_t bwp_start, bool config1_or_2)
{
  ...
  return isrran::ceil_div(bwp_nof_prb + (bwp_start % P), P);
}
