...
void log_alloc_failure(isrlog::log_channel& log_ch, const char* cause_fmt, Args&&... args)
{
...
alloc_result pdsch_allocator::is_grant_valid_common(isrran_search_space_type_t ss_type,
                                                    ...
                                                    const prb_grant&           grant) const
{
  ...
  if (dci_fmt == isrran_dci_format_nr_1_0 and not grant.is_alloc_type1()) {
  ...
  if (dci_fmt == isrran_dci_format_nr_1_0 and ISRRAN_SEARCH_SPACE_IS_COMMON(ss_type)) {
...
alloc_result pdsch_allocator::is_si_grant_valid(uint32_t ss_id, const prb_grant& grant) const
{
  ...
  const isrran_search_space_t* ss = bwp_cfg.get_ss(ss_id);
  if (ss == nullptr) {
    // Couldn't find SearchSpace
    log_alloc_failure(bwp_cfg.logger.error, "SearchSpace has not been configured.");
    return alloc_result::invalid_grant_params;
  }
  return is_grant_valid_common(ss->type, isrran_dci_format_nr_1_0, ss->coreset_id, grant);
}

alloc_result pdsch_allocator::is_rar_grant_valid(const prb_grant& grant) const
{
  isrran_sanity_check(bwp_cfg.cfg.pdcch.ra_search_space_present,
...
alloc_result pdsch_allocator::is_ue_grant_valid(const ue_carrier_params_t& ue,
                                                ...
                                                const prb_grant&           grant) const
{
  const isrran_search_space_t* ss = ue.get_ss(ss_id);
  if (ss == nullptr) {
    // Couldn't find SearchSpace
  ...
  if (ue.phy().pdsch.alloc != isrran_resource_alloc_dynamic) {
    if ((ue.phy().pdsch.alloc == isrran_resource_alloc_type0) != grant.is_alloc_type0()) {
...
pdsch_alloc_result pdsch_allocator::alloc_si_pdsch(uint32_t ss_id, const prb_grant& grant, isrran_dci_dl_nr_t& dci)
{
...
pdsch_t& pdsch_allocator::alloc_si_pdsch_unchecked(uint32_t ss_id, const prb_grant& grant, isrran_dci_dl_nr_t& dci)
{
  // Verify SearchSpace validity
  const isrran_search_space_t* ss = bwp_cfg.get_ss(ss_id);
  isrran_sanity_check(ss != nullptr, "SearchSpace has not been configured");
  return alloc_pdsch_unchecked(ss->coreset_id, ss->type, isrran_dci_format_nr_1_0, grant, dci);
}

pdsch_alloc_result pdsch_allocator::alloc_rar_pdsch(const prb_grant& grant, isrran_dci_dl_nr_t& dci)
{
...
pdsch_t& pdsch_allocator::alloc_rar_pdsch_unchecked(const prb_grant& grant, isrran_dci_dl_nr_t& dci)
{
  // TS 38.213, 8.2 - "In response to a PRACH transmission, a UE attempts to detect a DCI format 1_0"
  const static isrran_dci_format_nr_t dci_fmt = isrran_dci_format_nr_1_0;
...
pdsch_t& pdsch_allocator::alloc_pdsch_unchecked(uint32_t                   coreset_id,
                                                isrran_search_space_type_t ss_type,
                                                isrran_dci_format_nr_t     dci_fmt,
                                                const prb_grant&           grant,
                                                isrran_dci_dl_nr_t&        out_dci)
{
  ...
  if (grant.is_alloc_type0()) {
    ...
    if (ISRRAN_SEARCH_SPACE_IS_COMMON(ss_type)) {
      ...
      if (dci_fmt == isrran_dci_format_nr_1_0) {
        isrran_sanity_check(rb_start >= lims.start(), "Invalid PRB grant");
        rb_start -= lims.start();
      }
      if (coreset_id == 0) {
        nof_prb = lims.length();
    ...
    isrran_sanity_check(rb_start + grant.prbs().length() <= nof_prb, "Invalid PRB grant");
    out_dci.freq_domain_assigment = isrran_ra_nr_type1_riv(nof_prb, rb_start, grant.prbs().length());
  }
...
{
  isrran_assert(not pdschs.empty(), "Trying to abort PDSCH allocation that does not exist");
  pdschs.pop_back();
...
template <typename... Args>
void log_pusch_alloc_failure(isrlog::log_channel& log_ch, const char* cause_fmt, Args&&... args)
{
  if (not log_ch.enabled()) {
    return;
  }
...
alloc_result
pusch_allocator::is_grant_valid(isrran_search_space_type_t ss_type, const prb_grant& grant, bool verbose) const
{
...
pusch_alloc_result
pusch_allocator::alloc_pusch(const isrran_search_space_type_t ss_type, const prb_grant& grant, isrran_dci_ul_nr_t& dci)
{
...
pusch_t& pusch_allocator::alloc_pusch_unchecked(const prb_grant& grant, isrran_dci_ul_nr_t& out_dci)
{
  ...
  if (grant.is_alloc_type0()) {
    ...
    out_dci.freq_domain_assigment = isrran_ra_nr_type1_riv(nof_prb, grant.prbs().start(), grant.prbs().length());
  }
...
{
  isrran_assert(not puschs.empty(), "Trying to abort PUSCH allocation that does not exist");
  puschs.pop_back();
}
