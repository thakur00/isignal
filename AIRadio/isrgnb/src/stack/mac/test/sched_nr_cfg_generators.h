...
#ifndef ISRRAN_SCHED_NR_CFG_GENERATORS_H
#define ISRRAN_SCHED_NR_CFG_GENERATORS_H
...
inline isrran_coreset_t get_default_coreset0(uint32_t nof_prb)
{
...
inline isrran_search_space_t get_default_search_space0()
{
  isrran_search_space_t ss{};
  ss.coreset_id        = 0;
  ss.nof_formats       = 1;
  ss.formats[0]        = isrran_dci_format_nr_1_0;
  ss.type              = isrran_search_space_type_common_0;
  ss.id                = 0;
...
inline sched_nr_cell_cfg_t get_default_cell_cfg(const isrran::phy_cfg_nr_t& phy_cfg = isrran::phy_cfg_nr_default_t{
                                                    isrran::phy_cfg_nr_default_t::reference_cfg_t{}})
{
  ...
  isrran::fill_rach_cfg_common(phy_cfg.prach, cell_cfg.ul_cfg_common.init_ul_bwp.rach_cfg_common.set_setup());
  ...
  if (phy_cfg.duplex.mode == ISRRAN_DUPLEX_MODE_TDD) {
    cell_cfg.tdd_ul_dl_cfg_common.emplace();
    isrran_assert(isrran::make_phy_tdd_cfg(
                      phy_cfg.duplex, isrran_subcarrier_spacing_15kHz, &cell_cfg.tdd_ul_dl_cfg_common.value()),
...
inline std::vector<sched_nr_cell_cfg_t> get_default_cells_cfg(
    ...
    const isrran::phy_cfg_nr_t& phy_cfg = isrran::phy_cfg_nr_default_t{isrran::phy_cfg_nr_default_t::reference_cfg_t{}})
{
...
inline sched_nr_interface::ue_cfg_t get_rach_ue_cfg(uint32_t                    cc,
                                                    const isrran::phy_cfg_nr_t& phy_cfg = isrran::phy_cfg_nr_default_t{
                                                        isrran::phy_cfg_nr_default_t::reference_cfg_t{}})
{
  ...
  uecfg.phy_cfg     = isrran::phy_cfg_nr_default_t{isrran::phy_cfg_nr_default_t::reference_cfg_t{}};
  uecfg.phy_cfg.csi = {};
  for (isrran_search_space_t& ss : view_active_search_spaces(uecfg.phy_cfg.pdcch)) {
    // disable UE-specific search spaces
    if (ss.type == isrran_search_space_type_ue) {
...
{
  isrran::phy_cfg_nr_default_t::reference_cfg_t ref;
  ref.duplex                                        = isrran::phy_cfg_nr_default_t::reference_cfg_t::R_DUPLEX_FDD;
  sched_nr_cell_cfg_t cell_cfg                      = get_default_cell_cfg(isrran::phy_cfg_nr_default_t{ref});
  ...
  cell_cfg.bwps[0].pdcch.search_space[1].type       = isrran_search_space_type_common_1;
  cell_cfg.bwps[0].pdcch.search_space[1].nof_candidates[2] = 1;
  cell_cfg.bwps[0].pdcch.search_space[1].nof_formats       = 2;
  cell_cfg.bwps[0].pdcch.search_space[1].formats[0]        = isrran_dci_format_nr_1_0;
  cell_cfg.bwps[0].pdcch.search_space[1].formats[1]        = isrran_dci_format_nr_0_0;
  cell_cfg.bwps[0].pdcch.ra_search_space                   = cell_cfg.bwps[0].pdcch.search_space[1];
  return cell_cfg;
}

// Generate default UE-dedicated CORESET config
inline isrran_coreset_t get_default_ue_specific_coreset(uint32_t id, uint32_t pci)
{
  isrran_coreset_t coreset = {};
  coreset.id               = id;
  coreset.mapping_type     = isrran_coreset_mapping_type_non_interleaved;
  coreset.duration         = 1;
...
inline isrran_search_space_t get_default_ue_specific_search_space(uint32_t id, uint32_t coreset_id)
{
  isrran_search_space_t ss = {};
  ss.id                    = id;
  ss.coreset_id            = coreset_id;
  ss.duration              = 1;
  ss.type                  = isrran_search_space_type_ue;
  ss.nof_formats           = 2;
  ss.formats[0]            = isrran_dci_format_nr_1_0;
  ss.formats[1]            = isrran_dci_format_nr_0_0;
...
#endif // ISRRAN_SCHED_NR_CFG_GENERATORS_H
