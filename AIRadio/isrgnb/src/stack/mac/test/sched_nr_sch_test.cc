...
#include "isrgnb/hdr/stack/mac/sched_nr_interface_utils.h"
#include "isrgnb/hdr/stack/mac/sched_nr_sch.h"
...
isrran_dci_ctx_t generate_dci_ctx(const isrran_pdcch_cfg_nr_t& pdcch,
                                  uint32_t                     ss_id,
                                  isrran_rnti_type_t           rnti_type,
                                  uint16_t                     rnti,
                                  isrran_dci_format_nr_t       dci_fmt = isrran_dci_format_nr_1_0)
{
  const isrran_search_space_t& ss = pdcch.search_space[ss_id];
  const isrran_coreset_t&      cs = pdcch.coreset[ss.coreset_id];

  isrran_dci_ctx_t ctx;
  ctx.location         = {2, 4};
  ctx.ss_type          = ss.type;
  ctx.coreset_id       = ss.coreset_id;
  ctx.coreset_start_rb = isrran_coreset_start_rb(&cs);
...
void test_dci_freq_assignment(const bwp_params_t& bwp_params, prb_interval grant, const pdcch_dl_t& pdcch)
{
  ...
  if (ISRRAN_SEARCH_SPACE_IS_COMMON(pdcch.dci.ctx.ss_type) and pdcch.dci.ctx.format == isrran_dci_format_nr_1_0) {
  ...
  uint32_t expected_freq_assignment =
      isrran_ra_nr_type1_riv(lims.length(), grant.start() - lims.start(), grant.length());
  TESTASSERT_EQ(expected_freq_assignment, pdcch.dci.freq_domain_assigment);

  uint32_t st, len;
  isrran_sliv_to_s_and_l(lims.length(), pdcch.dci.freq_domain_assigment, &st, &len);
...
void test_si()
{
  isrran::test_delimit_logger delimiter{"Test PDSCH SI Allocation"};
  ...
  pdcch_dl_t pdcch;
  pdcch.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, ss_id, isrran_rnti_type_si, ISRRAN_SIRNTI);
  ...
  for (uint32_t i = 0; i < grant_list.size(); ++i) {
    ...
    prb_bitmap   avail_prbs = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
...
void test_rar()
{
  isrran::test_delimit_logger delimiter{"Test PDSCH RAR Allocation"};
  ...
  pdcch_dl_t pdcch;
  pdcch.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, ss_id, isrran_rnti_type_ra, 0x2);
...
void test_ue_pdsch()
{
  isrran::test_delimit_logger delimiter{"Test PDSCH UE Allocation"};
  ...
  pdcch_common.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, 1, isrran_rnti_type_c, 0x4601);
  pdcch_ue.dci.ctx     = generate_dci_ctx(bwp_params.cfg.pdcch, 2, isrran_rnti_type_c, 0x4601);
  ...
  for (uint32_t i = 0; i < grant_list.size(); ++i) {
    ...
    alloc_result check_ret  = pdsch_sched.is_ue_grant_valid(ue_cc, ss_id, isrran_dci_format_nr_1_0, grant);
    prb_bitmap   avail_prbs = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
    ...
    printf("Attempt %d should be %ssuccessful\n", i, success_expected ? "" : "un");
    alloc_res = pdsch_sched.alloc_ue_pdsch(ss_id, isrran_dci_format_nr_1_0, grant, ue_cc, pdcch.dci);
...
{
  isrran::test_delimit_logger delimiter{"Test PDSCH Allocation Failure"};
  ...
  pdcch_si.dci.ctx     = generate_dci_ctx(bwp_params.cfg.pdcch, 0, isrran_rnti_type_si, ISRRAN_SIRNTI);
  pdcch_rar.dci.ctx    = generate_dci_ctx(bwp_params.cfg.pdcch, 1, isrran_rnti_type_ra, 0x2);
  pdcch_common.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, 1, isrran_rnti_type_c, 0x4601);
  pdcch_ue.dci.ctx     = generate_dci_ctx(bwp_params.cfg.pdcch, 2, isrran_rnti_type_c, 0x4601);
  ...
  TESTASSERT_EQ(alloc_result::invalid_grant_params,
                pdsch_sched.alloc_ue_pdsch(1, isrran_dci_format_nr_1_0, grant_type0, ue_cc, pdcch.dci).error());
  TESTASSERT_EQ(alloc_result::invalid_grant_params,
                pdsch_sched.alloc_ue_pdsch(2, isrran_dci_format_nr_1_0, grant_type0, ue_cc, pdcch.dci).error());

  // Resource Allocation type must be compatible with UE PDSCH configuration
  TESTASSERT_EQ(alloc_result::invalid_grant_params,
                pdsch_sched.alloc_ue_pdsch(2, isrran_dci_format_nr_1_1, grant_type0, ue_cc, pdcch.dci).error());
  ...
  TESTASSERT(pdsch_sched.alloc_ue_pdsch(1, isrran_dci_format_nr_1_0, grant_type1, ue_cc, pdcch.dci).is_error());
  grant_type1 = prb_interval{bwp_params.coreset_prb_range(0).start(), bwp_params.nof_prb};
  TESTASSERT(pdsch_sched.alloc_ue_pdsch(1, isrran_dci_format_nr_1_0, grant_type1, ue_cc, pdcch.dci).is_error());
  TESTASSERT(pdsch_sched.alloc_ue_pdsch(2, isrran_dci_format_nr_1_0, grant_type1, ue_cc, pdcch.dci).has_value());

  // PRB collisions are detected
  TESTASSERT(pdsch_sched.alloc_ue_pdsch(2, isrran_dci_format_nr_1_0, prb_interval{5, 6}, ue_cc, pdcch.dci).is_error());
}

void test_multi_pdsch()
{
  isrran::test_delimit_logger delimiter{"Test Multiple PDSCH Allocations"};
  ...
  pdcch_sib.dci.ctx    = generate_dci_ctx(bwp_params.cfg.pdcch, 0, isrran_rnti_type_si, ISRRAN_SIRNTI);
  pdcch_common.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, 1, isrran_rnti_type_c, 0x4601);
  pdcch_ue.dci.ctx     = generate_dci_ctx(bwp_params.cfg.pdcch, 2, isrran_rnti_type_c, 0x4602);

  // Allocate SIB1
  uint32_t    ss_id     = 0;
  pdcch_dl_t* pdcch     = &pdcch_sib;
  prb_bitmap  used_prbs = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  ...
  prb_bitmap used_prbs_sib1 = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  ...
  TESTASSERT_EQ(alloc_result::success, pdsch_sched.is_ue_grant_valid(ue_cc, ss_id, isrran_dci_format_nr_1_0, ue_grant));
  alloc_res = pdsch_sched.alloc_ue_pdsch(ss_id, isrran_dci_format_nr_1_0, ue_grant, ue_cc, pdcch->dci);
  TESTASSERT(alloc_res.has_value());
  test_dci_freq_assignment(bwp_params, ue_grant, *pdcch);
  prb_bitmap used_prbs_ue = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  TESTASSERT_EQ(used_prbs_ue.count(), used_prbs_sib1.count() + ue_grant.length());
  TESTASSERT_EQ(alloc_result::sch_collision,
                pdsch_sched.is_ue_grant_valid(ue_cc, ss_id, isrran_dci_format_nr_1_0, ue_grant));
  ...
  ss_id                  = 2;
  pdcch                  = &pdcch_ue;
  used_prbs              = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  ...
  TESTASSERT_EQ(alloc_result::success,
                pdsch_sched.is_ue_grant_valid(ue_cc, ss_id, isrran_dci_format_nr_1_0, ue_grant2));
  alloc_res = pdsch_sched.alloc_ue_pdsch(ss_id, isrran_dci_format_nr_1_0, ue_grant2, ue_cc, pdcch->dci);
  TESTASSERT(alloc_res.has_value());
  test_dci_freq_assignment(bwp_params, ue_grant2, *pdcch);
  prb_bitmap used_prbs_ue2 = pdsch_sched.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  ...
  pdsch_sched.reset();
  TESTASSERT_EQ(0, pdschs.size());
  TESTASSERT_EQ(0, pdsch_sched.occupied_prbs(2, isrran_dci_format_nr_1_0).count());
}

void test_multi_pusch()
{
  isrran::test_delimit_logger delimiter{"Test Multiple PUSCH Allocations"};
  ...
  pdcch_ue1.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, 1, isrran_rnti_type_c, 0x4601);
  pdcch_ue2.dci.ctx = generate_dci_ctx(bwp_params.cfg.pdcch, 2, isrran_rnti_type_c, 0x4602);
  ...
  TESTASSERT_EQ(alloc_result::success, pusch_sched.is_grant_valid(isrran_search_space_type_common_1, ue_grant));
  ...
  TESTASSERT_EQ(alloc_result::sch_collision,
                pusch_sched.is_grant_valid(isrran_search_space_type_common_1, ue_grant, false));
  ...
  TESTASSERT_EQ(alloc_result::success, pusch_sched.is_grant_valid(isrran_search_space_type_ue, ue2_grant));
  ...
  TESTASSERT_EQ(alloc_result::sch_collision, pusch_sched.is_grant_valid(isrran_search_space_type_ue, ue2_grant, false));
...
int main()
{
  auto& test_logger = isrlog::fetch_basic_logger("TEST");
  test_logger.set_level(isrlog::basic_levels::info);
  auto& mac_nr_logger = isrlog::fetch_basic_logger("MAC-NR");
  mac_nr_logger.set_level(isrlog::basic_levels::debug);
  auto& pool_logger = isrlog::fetch_basic_logger("POOL");
  pool_logger.set_level(isrlog::basic_levels::debug);
  ...
  isrenb::test_si();
  isrenb::test_rar();
  isrenb::test_ue_pdsch();
  isrenb::test_pdsch_fail();
  isrenb::test_multi_pdsch();
  isrenb::test_multi_pusch();
}