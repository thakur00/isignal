...
static const isrran::phy_cfg_nr_t default_phy_cfg =
    isrran::phy_cfg_nr_default_t{isrran::phy_cfg_nr_default_t::reference_cfg_t{}};
...
public:
  ...
  void process_slot_result(const sim_nr_enb_ctxt_t& slot_ctxt, isrran::const_span<cc_result_t> cc_list) override
  {
    ...
    for (auto& cc_out : cc_list) {
      ...
      bool is_dl_slot = isrran_duplex_nr_is_dl(&cell_params[cc_out.res.cc].duplex, 0, current_slot_tx.slot_idx());
  ...
  isrlog::basic_logger& test_logger = isrlog::fetch_basic_logger("TEST");
...
{
  isrran_assert(nof_workers > 0, "There must be at least one worker");
...
int main()
{
  ...
  test_logger.set_level(isrlog::basic_levels::warning);
  auto& mac_nr_logger = isrlog::fetch_basic_logger("MAC-NR");
  mac_nr_logger.set_level(isrlog::basic_levels::warning);
  ...
  isrenb::run_sched_nr_test(1);
  isrenb::run_sched_nr_test(2);
  isrenb::run_sched_nr_test(4);
}
