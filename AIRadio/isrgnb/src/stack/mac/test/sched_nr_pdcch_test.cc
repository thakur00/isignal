...
void test_coreset0_cfg()
{
  ...
  isrran::test_delimit_logger delimiter{"Test PDCCH Allocation in CORESET#0"};
  ...
  for (const isrran_coreset_t& cs : view_active_coresets(cell_cfg.bwps[0].cfg.pdcch)) {
  ...
  TESTASSERT_EQ(isrran_rnti_type_si, dl_pdcch->dci.ctx.rnti_type);
  ...
  TESTASSERT(pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 1, aggr_idx, ue_cc).error() == alloc_result::no_cch_space);
  TESTASSERT(pdcch_sched.alloc_ul_pdcch(1, aggr_idx, ue_cc).error() == alloc_result::no_cch_space);

  isrlog::fetch_basic_logger("TEST").info("%s", pdcch_sched.print_allocations());
  ...
  dl_pdcch = dl_pdcch_result.value();
  TESTASSERT_EQ(1, pdcch_sched.nof_allocations());
  TESTASSERT_EQ(isrran_rnti_type_ra, dl_pdcch->dci.ctx.rnti_type);
  TESTASSERT_EQ(0, dl_pdcch->dci.ctx.coreset_id);
  TESTASSERT_EQ(isrran_search_space_type_common_1, dl_pdcch->dci.ctx.ss_type);
  test_dci_ctx_consistency(bwp_params.cfg.pdcch, dl_pdcch->dci.ctx);
  ...
  dl_pdcch_result = pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 1, aggr_idx, ue_cc);
  TESTASSERT(dl_pdcch_result.has_value());
  dl_pdcch = dl_pdcch_result.value();
  TESTASSERT_EQ(1, pdcch_sched.nof_allocations());
  TESTASSERT_EQ(isrran_rnti_type_c, dl_pdcch->dci.ctx.rnti_type);
  ...
  TESTASSERT_EQ(1, pdcch_sched.nof_allocations());
  TESTASSERT_EQ(isrran_rnti_type_c, ul_pdcch->dci.ctx.rnti_type);
  TESTASSERT_EQ(0u, ul_pdcch->dci.ctx.coreset_id);
  TESTASSERT_EQ(isrran_search_space_type_common_1, ul_pdcch->dci.ctx.ss_type);
...
void test_coreset2_cfg()
{
  ...
  isrran::test_delimit_logger delimiter{"Test PDCCH Allocation in CORESET#0 and CORESET#2"};
  ...
  bwp_pdcch_allocator pdcch_sched(bwp_params, 0, dl_pdcchs, ul_pdcchs);
  for (const isrran_coreset_t& cs : view_active_coresets(cell_cfg.bwps[0].pdcch)) {
    // Verify nof CCEs is correctly computed
    TESTASSERT_EQ(coreset_nof_cces(cs), pdcch_sched.nof_cces(cs.id));
  }
  ...
  TESTASSERT_EQ(isrran_rnti_type_si, dl_pdcch->dci.ctx.rnti_type);
  TESTASSERT_EQ(0, dl_pdcch->dci.ctx.coreset_id);
  isrran_dci_location_t expected_loc{aggr_idx, 0};
  ...
  dl_pdcch = pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 2, aggr_idx, ue_cc).value();
  TESTASSERT(dl_pdcch != nullptr);
  TESTASSERT_EQ(2, pdcch_sched.nof_allocations());
  TESTASSERT_EQ(isrran_rnti_type_c, dl_pdcch->dci.ctx.rnti_type);
  TESTASSERT_EQ(2u, dl_pdcch->dci.ctx.coreset_id);
  TESTASSERT_EQ(isrran_search_space_type_ue, dl_pdcch->dci.ctx.ss_type);
  expected_loc = isrran_dci_location_t{aggr_idx, 0};
  TESTASSERT(dl_pdcch->dci.ctx.location == expected_loc);
  test_dci_ctx_consistency(bwp_params.cfg.pdcch, dl_pdcch->dci.ctx);
  ...
  TESTASSERT_EQ(isrran_rnti_type_c, ul_pdcch->dci.ctx.rnti_type);
  TESTASSERT_EQ(isrran_dci_format_nr_0_0, ul_pdcch->dci.ctx.format);
  TESTASSERT_EQ(2u, ul_pdcch->dci.ctx.coreset_id);
  TESTASSERT_EQ(isrran_search_space_type_ue, ul_pdcch->dci.ctx.ss_type);
  expected_loc = isrran_dci_location_t{aggr_idx, 4};
  TESTASSERT(ul_pdcch->dci.ctx.location == expected_loc);
  test_dci_ctx_consistency(bwp_params.cfg.pdcch, ul_pdcch->dci.ctx);

  // No space for 3rd PDCCH allocation in SS#2
  TESTASSERT(pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 2, aggr_idx, ue_cc).error() == alloc_result::no_cch_space);
...
void test_invalid_params()
{
  ...
  isrran::test_delimit_logger delimiter{"Test PDCCH Allocation with Invalid Arguments"};
  ...
  cell_cfg.bwps[0].pdcch.search_space[3].formats[0]  = isrran_dci_format_nr_1_0;
  sched_nr_interface::sched_args_t   sched_args;
  sched_nr_impl::cell_config_manager cellparams{0, cell_cfg, sched_args};
  bwp_params_t&                      bwp_params = cellparams.bwps[0];
  ...
  dl_res = pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 4, aggr_idx, ue_cc);
  TESTASSERT(dl_res.is_error() and dl_res.error() == alloc_result::invalid_grant_params);

  // TC-RNTI cannot be allocated in Common SearchSpace Type1
  dl_res = pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_tc, 2, aggr_idx, ue_cc);
  TESTASSERT(dl_res.is_error() and dl_res.error() == alloc_result::invalid_grant_params);

  // C-RNTI cannot be allocated in Common SearchSpace Type0
  dl_res = pdcch_sched.alloc_dl_pdcch(isrran_rnti_type_c, 0, aggr_idx, ue_cc);
  TESTASSERT(dl_res.is_error() and dl_res.error() == alloc_result::invalid_grant_params);
  ...
  TESTASSERT(ul_res.has_value() and ul_res.value()->dci.ctx.format == isrran_dci_format_nr_0_0);
...
int main()
{
  ...
  isrenb::test_coreset0_cfg();
  isrenb::test_coreset2_cfg();
  isrenb::test_invalid_params();
}