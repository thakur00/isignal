...
public:
  ...
  void process_slot_result(const sim_nr_enb_ctxt_t& enb_ctxt, isrran::const_span<cc_result_t> cc_out) override
  {
    for (auto& cc : cc_out) {
      for (auto& pdsch : cc.res.dl->phy.pdsch) {
        if (pdsch.sch.grant.rnti_type == isrran_rnti_type_c or pdsch.sch.grant.rnti_type == isrran_rnti_type_tc) {
      ...
      for (auto& pusch : cc.res.ul->pusch) {
        if (pusch.sch.grant.rnti_type == isrran_rnti_type_c or pusch.sch.grant.rnti_type == isrran_rnti_type_tc) {
...
int main(int argc, char** argv)
{
  ...
  // Parse args
  isrenb::sim_args_t args = isrenb::handle_args(argc, argv);

  // Initialize Loggers
  auto& test_logger = isrlog::fetch_basic_logger("TEST");
  test_logger.set_level(isrlog::str_to_basic_level(args.test_log_level));
  auto& mac_nr_logger = isrlog::fetch_basic_logger("MAC-NR");
  mac_nr_logger.set_level(isrlog::str_to_basic_level(args.mac_log_level));
  auto& pool_logger = isrlog::fetch_basic_logger("POOL");
  pool_logger.set_level(isrlog::basic_levels::debug);

  // Setup Random Generator
  isrenb::rand_gen = std::default_random_engine(args.rand_seed);
  fmt::print("TEST: Random Seed is {}", args.rand_seed);
  add_emergency_cleanup_handler(
      [](void* args_void) {
        auto* args = (isrenb::sim_args_t*)args_void;
        fmt::print("TEST: Random Seed was {}", args->rand_seed);
      },
      (void*)&args);

  isrenb::test_sched_nr_no_data(args);
  isrenb::test_sched_nr_data(args);
