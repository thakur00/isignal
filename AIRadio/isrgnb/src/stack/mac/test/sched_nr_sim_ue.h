...
#ifndef ISRRAN_SCHED_NR_SIM_UE_H
#define ISRRAN_SCHED_NR_SIM_UE_H

#include "isrenb/test/mac/sched_sim_ue.h"
...
struct ue_nr_cc_ctxt_t {
  ...
  isrran::circular_array<uint32_t, TTIMOD_SZ>      pending_acks;
};
...
struct ue_nr_slot_events {
  ...
  struct cc_data {
    ...
    isrran::bounded_vector<ack_t, MAX_GRANTS> dl_acks;
    isrran::bounded_vector<ack_t, MAX_GRANTS> ul_acks;
    int                                       cqi = -1;
  };
  slot_point           slot_rx;
...
struct sim_nr_enb_ctxt_t {
  isrran::span<const sched_nr_impl::cell_config_manager> cell_params;
...
public:
  ...
  isrran::const_span<sched_nr_impl::cell_config_manager> get_cell_params() const { return cell_params; }
   ...
   */
  virtual void process_slot_result(const sim_nr_enb_ctxt_t& enb_ctxt, isrran::const_span<cc_result_t> cc_out) {}
...
#endif // ISRRAN_SCHED_NR_SIM_UE_H
