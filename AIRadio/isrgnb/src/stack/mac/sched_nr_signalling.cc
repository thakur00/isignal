...
#include "isrgnb/hdr/stack/mac/sched_nr_signalling.h"
...
void sched_nzp_csi_rs(isrran::const_span<isrran_csi_rs_nzp_set_t> nzp_csi_rs_sets_cfg,
                      const isrran_slot_cfg_t&                    slot_cfg,
                      nzp_csi_rs_list&                            csi_rs_list)
{
  for (const isrran_csi_rs_nzp_set_t& set : nzp_csi_rs_sets_cfg) {
    ...
    for (uint32_t i = 0; i < set.count; ++i) {
      ...
      if (isrran_csi_rs_send(&nzp_csi_resource.periodicity, &slot_cfg)) {
        if (csi_rs_list.full()) {
          isrlog::fetch_basic_logger("MAC-NR").error("SCHED: Failed to allocate NZP-CSI RS");
          return;
        }
...
void sched_ssb_basic(const slot_point&      sl_point,
                     ...
                     const isrran_mib_nr_t& mib,
                     ssb_list&              ssb_list)
{
  if (ssb_list.full()) {
    isrlog::fetch_basic_logger("MAC-NR").error("SCHED: Failed to allocate SSB");
    return;
  }
  ...
  if (sl_point_mod == 0) {
    ssb_t           ssb_msg = {};
    isrran_mib_nr_t mib_msg = mib;
    mib_msg.sfn             = sl_point.sfn();
    mib_msg.hrf             = (sl_point.slot_idx() % ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz) >=
    ...
    int packing_ret_code = isrran_pbch_msg_nr_mib_pack(&mib_msg, &ssb_msg.pbch_msg);
    isrran_assert(packing_ret_code == ISRRAN_SUCCESS, "SSB packing returned en error");
...
si_sched::si_sched(const bwp_params_t& bwp_cfg_) :
  bwp_cfg(&bwp_cfg_), logger(isrlog::fetch_basic_logger(bwp_cfg_.sched_cfg.logger_name))
{
...
void si_sched::run_slot(bwp_slot_allocator& bwp_alloc)
{
  ...
  slot_point     sl_pdcch      = bwp_alloc.get_pdcch_tti();
  prb_bitmap     prbs          = bwp_alloc.res_grid()[sl_pdcch].pdschs.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
