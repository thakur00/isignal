...
#include "isrran/common/rwlock_guard.h"
#include "isrran/common/standard_streams.h"
#include "isrran/common/string_helpers.h"
#include "isrran/common/time_prof.h"
#include "isrran/mac/mac_rar_pdu_nr.h"
...
public:
  explicit mac_nr_rx(rlc_interface_mac*          rlc_,
                     ...
                     isrran::task_queue_handle&  stack_task_queue_,
  ...
  void handle_pdu(uint16_t rnti, isrran::unique_byte_buffer_t pdu)
  {
    task_queue.push(std::bind(
        [this, rnti](isrran::unique_byte_buffer_t& pdu) { handle_pdu_impl(rnti, std::move(pdu)); }, std::move(pdu)));
  }

private:
  int handle_pdu_impl(uint16_t rnti, isrran::unique_byte_buffer_t pdu)
  {
    pdu_ul.init_rx(true);
    if (pdu_ul.unpack(pdu->msg, pdu->N_bytes) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
    ...
    if (logger.info.enabled()) {
      ...
      logger.info("Rx PDU: rnti=0x%x, %s", rnti, isrran::to_c_str(str_buffer));
    }
    ...
    for (uint32_t n = pdu_ul.get_num_subpdus(); n > 0; --n) {
      isrran::mac_sch_subpdu_nr& subpdu = pdu_ul.get_subpdu(n - 1);
      if (subpdu.get_lcid() == isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CRNTI) {
        if (process_ce_subpdu(rnti, subpdu) != ISRRAN_SUCCESS) {
          return ISRRAN_ERROR;
        }
        crnti_ce_pos = n - 1;
      }
    ...
    for (uint32_t n = 0; n < pdu_ul.get_num_subpdus(); ++n) {
      isrran::mac_sch_subpdu_nr& subpdu = pdu_ul.get_subpdu(n);
      if (subpdu.is_sdu()) {
  ...
  int process_ce_subpdu(uint16_t& rnti, const isrran::mac_sch_subpdu_nr& subpdu)
  {
    // Handle MAC CEs
    switch (subpdu.get_lcid()) {
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CCCH_SIZE_48:
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CCCH_SIZE_64: {
        isrran::mac_sch_subpdu_nr& ccch_subpdu = const_cast<isrran::mac_sch_subpdu_nr&>(subpdu);
        ...
        mac.store_msg3(rnti,
                       isrran::make_byte_buffer(ccch_subpdu.get_sdu(), ccch_subpdu.get_sdu_length(), __FUNCTION__));
        sched->dl_mac_ce(rnti, isrran::mac_sch_subpdu_nr::CON_RES_ID);
      } break;
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CRNTI: {
        uint16_t ce_crnti  = subpdu.get_c_rnti();
        if (ce_crnti == ISRRAN_INVALID_RNTI) {
      ...
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::SHORT_BSR:
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::SHORT_TRUNC_BSR: {
        isrran::mac_sch_subpdu_nr::lcg_bsr_t sbsr = subpdu.get_sbsr();
        uint32_t buffer_size_bytes                = buff_size_field_to_bytes(sbsr.buffer_size, isrran::SHORT_BSR);
      ...
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::LONG_BSR:
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::LONG_TRUNC_BSR: {
        isrran::mac_sch_subpdu_nr::lbsr_t lbsr = subpdu.get_lbsr();
        for (auto& lb : lbsr.list) {
          sched->ul_bsr(rnti, lb.lcg_id, buff_size_field_to_bytes(lb.buffer_size, isrran::LONG_BSR));
        }
      } break;
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::SE_PHR:
        // SE_PHR not implemented
        break;
      case isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::PADDING:
        break;
      default:
   ...
   */
  static uint32_t buff_size_field_to_bytes(uint32_t buff_size_index, const isrran::bsr_format_nr_t& format)
  {
  ...
  isrran::task_queue_handle&  task_queue;

  isrran::mac_sch_pdu_nr pdu_ul;
};

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

mac_nr::mac_nr(isrran::task_sched_handle task_sched_) :
  logger(isrlog::fetch_basic_logger("MAC-NR")),
  task_sched(task_sched_),
  bcch_bch_payload(isrran::make_byte_buffer()),
  rar_pdu_buffer(isrran::make_byte_buffer()),
  sched(new sched_nr{})
{
                 ...
                 rrc_interface_mac_nr*   rrc_)
{
  ...
  if (args.pcap.enable) {
    pcap = std::unique_ptr<isrran::mac_pcap>(new isrran::mac_pcap());
...
void mac_nr::get_metrics(isrenb::mac_metrics_t& metrics)
{
...
void mac_nr::get_metrics_nolock(isrenb::mac_metrics_t& metrics)
{
  isrran::rwlock_read_guard lock(rwmutex);
...
int mac_nr::cell_cfg(const std::vector<isrenb::sched_nr_cell_cfg_t>& nr_cells)
{
  ...
  for (uint32_t i = 0; i < nr_cells[0].sibs.size(); i++) {
    ...
    sib.payload     = isrran::make_byte_buffer();
    if (sib.payload == nullptr) {
      logger.error("Couldn't allocate PDU in %s().", __FUNCTION__);
      return ISRRAN_ERROR;
    }
    if (rrc->read_pdu_bcch_dlsch(sib.index, *sib.payload) != ISRRAN_SUCCESS) {
...
{
  static isrran::mutexed_tprof<isrran::avg_time_stats> rach_tprof("rach_tprof", "MAC-NR", 1);
  ...
  stack_task_queue.push([this, rach_info, enb_cc_idx, rach_tprof_meas]() mutable {
    ...
    // Log this event.
    {
      isrran::rwlock_write_guard lock(rwmutex);
      ++detected_rachs[enb_cc_idx];
    }

    // Trigger scheduler RACH
    isrenb::sched_nr_interface::rar_info_t rar_info = {};
    ...
    logger.info("RACH:  slot=%d, cc=%d, preamble=%d, offset=%d, temp_crnti=0x%x",
                ...
                rach_info.time_adv,
                rnti);
    isrran::console("RACH:  slot=%d, cc=%d, preamble=%d, offset=%d, temp_crnti=0x%x\n",
                    rach_info.slot_index,
                    enb_cc_idx,
                    rach_info.preamble,
                    rach_info.time_adv,
...
uint16_t mac_nr::alloc_ue(uint32_t enb_cc_idx)
{
  ...
  do {
    ...
    {
      isrran::rwlock_read_guard read_lock(rwmutex);
    ...
    // Add UE to rnti map
    isrran::rwlock_write_guard rw_lock(rwmutex);
    if (not is_rnti_valid_nolock(rnti)) {
      continue;
    }
...
bool mac_nr::is_rnti_valid_nolock(uint16_t rnti)
{
  ...
  if (ue_db.full()) {
    logger.warning("Maximum number of connected UEs %zd connected to the eNB. Ignoring PRACH", ISRENB_MAX_UES);
    return false;
  }
...
int mac_nr::slot_indication(const isrran_slot_cfg_t& slot_cfg)
{
  return 0;
}

void mac_nr::store_msg3(uint16_t rnti, isrran::unique_byte_buffer_t pdu)
{
  isrran::rwlock_read_guard rw_lock(rwmutex);
  if (is_rnti_active_nolock(rnti)) {
...
mac_nr::dl_sched_t* mac_nr::get_dl_sched(const isrran_slot_cfg_t& slot_cfg)
{
  slot_point pdsch_slot = isrran::slot_point{NUMEROLOGY_IDX, slot_cfg.idx};
  ...
  for (pdsch_t& pdsch : dl_res->phy.pdsch) {
    ...
    } else if (pdsch.sch.grant.rnti_type == isrran_rnti_type_ra) {
    ...
    } else if (pdsch.sch.grant.rnti_type == isrran_rnti_type_si) {
...
mac_nr::ul_sched_t* mac_nr::get_ul_sched(const isrran_slot_cfg_t& slot_cfg)
{
  slot_point  pusch_slot = isrran::slot_point{NUMEROLOGY_IDX, slot_cfg.idx};
...
int mac_nr::pucch_info(const isrran_slot_cfg_t& slot_cfg, const mac_interface_phy_nr::pucch_info_t& pucch_info)
{
...
bool mac_nr::handle_uci_data(uint16_t rnti, const isrran_uci_cfg_nr_t& cfg_, const isrran_uci_value_nr_t& value)
{
  // Process HARQ-ACK
  for (uint32_t i = 0; i < cfg_.ack.count; i++) {
    const isrran_harq_ack_bit_t* ack_bit = &cfg_.ack.bits[i];
  ...
  for (uint32_t i = 0; i < cfg_.nof_csi; i++) {
    ...
    if (not value.valid or cfg_.csi[i].cfg.quantity != ISRRAN_CSI_REPORT_QUANTITY_CRI_RI_PMI_CQI or
        cfg_.csi[i].cfg.freq_cfg != ISRRAN_CSI_REPORT_FREQ_WIDEBAND or value.csi[i].wideband_cri_ri_pmi_cqi.cqi == 0) {
      continue;
    }
...
int mac_nr::pusch_info(const isrran_slot_cfg_t& slot_cfg, mac_interface_phy_nr::pusch_info_t& pusch_info)
{
...
isrran::byte_buffer_t* mac_nr::assemble_rar(isrran::const_span<sched_nr_interface::msg3_grant_t> grants)
{
  isrran::mac_rar_pdu_nr rar_pdu;

  uint32_t pdsch_tbs = 10; // TODO: how big is the PDSCH?
  rar_pdu.init_tx(rar_pdu_buffer.get(), pdsch_tbs);

  for (auto& rar_grant : grants) {
    isrran::mac_rar_subpdu_nr& rar_subpdu = rar_pdu.add_subpdu();
    ...
    if (isrran_dci_nr_ul_pack(&dci, &rar_grant.msg3_dci, &dci_msg) != ISRRAN_SUCCESS) {
    ...
    if (logger.info.enabled()) {
      ...
      isrran_dci_ul_nr_to_str(&dci, &rar_grant.msg3_dci, str.data(), str.size());
    ...
    std::copy(
        std::begin(dci_msg.payload), std::begin(dci_msg.payload) + ISRRAN_RAR_UL_GRANT_NBITS, packed_ul_grant.begin());
    rar_subpdu.set_ul_grant(packed_ul_grant);
  }

  if (rar_pdu.pack() != ISRRAN_SUCCESS) {
