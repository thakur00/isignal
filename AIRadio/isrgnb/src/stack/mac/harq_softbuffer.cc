...
{
  isrran_assert(nof_prb <= ISRRAN_MAX_PRB_NR, "Invalid nof prb=%d", nof_prb);
  ...
  tx_pool[idx].reset(new isrran::background_obj_pool<tx_harq_softbuffer>(
  ...
  rx_pool[idx].reset(new isrran::background_obj_pool<rx_harq_softbuffer>(
      batch_size, thres, init_size, init_rx_softbuffers, recycle_rx_softbuffers));
}

isrran::unique_pool_ptr<tx_harq_softbuffer> harq_softbuffer_pool::get_tx(uint32_t nof_prb)
{
  isrran_assert(nof_prb <= ISRRAN_MAX_PRB_NR, "Invalid Nprb=%d", nof_prb);
  size_t idx = nof_prb - 1;
  if (tx_pool[idx] == nullptr) {
    init_pool(nof_prb);
  }
...
isrran::unique_pool_ptr<rx_harq_softbuffer> harq_softbuffer_pool::get_rx(uint32_t nof_prb)
{
  isrran_assert(nof_prb <= ISRRAN_MAX_PRB_NR, "Invalid Nprb=%d", nof_prb);
  size_t idx = nof_prb - 1;
