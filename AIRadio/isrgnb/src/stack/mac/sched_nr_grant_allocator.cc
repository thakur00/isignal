...
#include "isrgnb/hdr/stack/mac/sched_nr_grant_allocator.h"
#include "isrgnb/hdr/stack/mac/sched_nr_bwp.h"
...
using candidate_ss_list_t =
    isrran::bounded_vector<const isrran_search_space_t*, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR>;

candidate_ss_list_t find_ss(const isrran_pdcch_cfg_nr_t&               pdcch,
                            uint32_t                                   aggr_idx,
                            isrran_rnti_type_t                         rnti_type,
                            isrran::const_span<isrran_dci_format_nr_t> prio_dcis)
{
  candidate_ss_list_t ret;
  auto                active_ss_lst = view_active_search_spaces(pdcch);

  auto contains_dci_fmt = [prio_dcis, aggr_idx](const isrran_search_space_t& ss) {
  ...
  auto is_common_ss_allowed = [rnti_type](isrran_search_space_type_t ss_type) {
    switch (rnti_type) {
      case isrran_rnti_type_c:
        return ss_type == isrran_search_space_type_common_1 or ss_type == isrran_search_space_type_common_3;
      case isrran_rnti_type_tc:
      case isrran_rnti_type_ra:
        // TODO: Fix UE config to not use common3
        return ss_type == isrran_search_space_type_common_1 or ss_type == isrran_search_space_type_common_3;
      case isrran_rnti_type_si:
        return ss_type == isrran_search_space_type_common_0;
      default:
  ...
  if (rnti_type == isrran_rnti_type_c) {
    // First search UE-specific
    for (const isrran_search_space_t& ss : active_ss_lst) {
      if (ss.type == isrran_search_space_type_ue and contains_dci_fmt(ss)) {
        ret.push_back(&ss);
      }
    }
  }
...
bwp_res_grid::bwp_res_grid(const bwp_params_t& bwp_cfg_) : cfg(&bwp_cfg_)
{
  for (uint32_t sl = 0; sl < slots.capacity(); ++sl) {
    slots.emplace_back(*cfg, sl % static_cast<uint32_t>(ISRRAN_NSLOTS_PER_FRAME_NR(bwp_cfg_.cell_cfg.carrier.scs)));
  }
}
                                          ...
                                          tx_harq_softbuffer& softbuffer)
{
  ...
  static const isrran_dci_format_nr_t dci_fmt = isrran_dci_format_nr_1_0;
  ...
  pdcch.dci_cfg.coreset0_bw = isrran_coreset_get_bw(&cfg.cfg.pdcch.coreset[0]);
  pdcch.dci.mcs             = 5;
  pdcch.dci.rv              = 0;
...
alloc_result bwp_slot_allocator::alloc_rar_and_msg3(uint16_t                                ra_rnti,
                                                    uint32_t                                aggr_idx,
                                                    prb_interval                            interv,
                                                    isrran::const_span<dl_sched_rar_info_t> pending_rachs)
{
  ...
  isrran_sanity_check(not bwp_pdcch_slot.dl.rar.full(), "The #RARs should be below #PDSCHs");
  ...
  int code     = isrran_ra_dl_dci_to_grant_nr(
      &cfg.cell_cfg.carrier, &slot_cfg, &cfg.cfg.pdsch, &pdcch.dci, &pdsch.sch, &pdsch.sch.grant);
  isrran_assert(code == ISRRAN_SUCCESS, "Error converting DCI to grant");
  ...
  for (const dl_sched_rar_info_t& grant : pending_rachs) {
    ...
    rar_grant.msg3_dci.ctx.rnti_type  = isrran_rnti_type_tc;
    rar_grant.msg3_dci.ctx.rnti       = ue->rnti;
    rar_grant.msg3_dci.ctx.ss_type    = isrran_search_space_type_rar;
    rar_grant.msg3_dci.ctx.format     = isrran_dci_format_nr_rar;
...
alloc_result bwp_slot_allocator::alloc_pdsch(slot_ue& ue, uint32_t ss_id, const prb_grant& dl_grant)
{
  ...
  static const isrran_dci_format_nr_t dci_fmt   = isrran_dci_format_nr_1_0;
  static const isrran_rnti_type_t     rnti_type = isrran_rnti_type_c;
  ...
  if (ue.h_dl->empty()) {
    if (mcs < 0) {
      mcs = isrran_ra_nr_cqi_to_mcs(/* cqi */ ue.dl_cqi(),
    ...
    if (ue.get_pending_bytes(isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CCCH) and mcs < min_MCS_ccch) {
      mcs = min_MCS_ccch;
    ...
    isrran_assert(success, "Failed to allocate DL HARQ");
  } else {
    bool success = ue.h_dl->new_retx(ue.pdsch_slot, ue.uci_slot, dl_grant, pdcch.dci);
    mcs          = ue.h_dl->mcs();
    isrran_assert(success, "Failed to allocate DL HARQ retx");
  }
  ...
  while (true) {
    ...
    isrran_assert(success, "Error converting DCI to grant");
    if (ue.h_dl->nof_retx() != 0) {
      isrran_assert(pdsch.sch.grant.tb[0].tbs == (int)ue.h_dl->tbs(), "The TBS did not remain constant in retx");
    }
    R_prime = pdsch.sch.grant.tb[0].R_prime;
    if (ue.h_dl->nof_retx() > 0 or R_prime < max_R or mcs <= 0 or
        (ue.get_pending_bytes(isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CCCH) and mcs <= min_MCS_ccch)) {
      break;
    }
  ...
  isrran_assert(success, "Error getting ack resource");
...
{
  static const uint32_t                              aggr_idx = 2;
  static const std::array<isrran_dci_format_nr_t, 2> dci_fmt_list{isrran_dci_format_nr_0_1, isrran_dci_format_nr_0_0};
  static const isrran_rnti_type_t                    rnti_type = isrran_rnti_type_c;
  ...
  const isrran_search_space_t& ss = *ss_candidates[0];
  ...
  if (ue.h_ul->empty()) {
    ...
    isrran_assert(success, "Failed to allocate UL HARQ");
  } else {
    bool success = ue.h_ul->new_retx(ue.pusch_slot, ul_grant, pdcch.dci);
    isrran_assert(success, "Failed to allocate UL HARQ retx");
  }
  ...
  if (ue.h_ul->nof_retx() == 0) {
    ...
    isrran_assert(pusch.sch.grant.tb[0].tbs == (int)ue.h_ul->tbs(), "The TBS did not remain constant in retx");
  }
...
{
  static const isrran_dci_format_nr_t dci_fmt = isrran_dci_format_nr_1_0; // TODO: Support more DCI formats
