...
cc_worker::cc_worker(const cell_config_manager& params) :
  cfg(params), logger(isrlog::fetch_basic_logger(params.sched_args.logger_name))
{
...
dl_sched_res_t* cc_worker::run_slot(slot_point tx_sl, ue_map_t& ue_db)
{
  ...
  prb_bitmap prbs_before = bwp_alloc.res_grid()[sl_pdcch].pdschs.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
  // Allocate cell DL signalling
  sched_dl_signalling(bwp_alloc);

  prb_bitmap prbs_after = bwp_alloc.res_grid()[sl_pdcch].pdschs.occupied_prbs(ss_id, isrran_dci_format_nr_1_0);
