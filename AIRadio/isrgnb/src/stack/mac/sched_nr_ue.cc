...
int ue_buffer_manager::get_dl_tx_total() const
{
  ...
  for (ue_buffer_manager::ce_t ce : pending_ces) {
    total_bytes += isrran::mac_sch_subpdu_nr::sizeof_ce(ce.lcid, false);
  }
  return total_bytes;
}
...
bool ue_buffer_manager::pdu_builder::alloc_subpdus(uint32_t rem_bytes, sched_nr_interface::dl_pdu_t& pdu)
{
  ...
  isrran::deque<ce_t> restore_ces;
  for (ce_t ce : parent->pending_ces) {
    if (ce.cc == cc) {
      // Note: This check also avoids thread collisions across UE carriers
      uint32_t size_ce = isrran::mac_sch_subpdu_nr::sizeof_ce(ce.lcid, false);
  ...
  for (uint32_t lcid = 0; rem_bytes > 0 and is_lcid_valid(lcid); ++lcid) {
    ...
    if (lcid == isrran::mac_sch_subpdu_nr::nr_lcid_sch_t::CCCH and pending_lcid_bytes > rem_bytes) {
...
ue_carrier::ue_carrier(uint16_t                              rnti_,
  ...
  rnti(rnti_),
  cc(cell_params_.cc),
  logger(isrlog::fetch_basic_logger(cell_params_.sched_args.logger_name)),
...
ue::ue(uint16_t rnti_, const sched_nr_ue_cfg_t& uecfg, const sched_params_t& sched_cfg_) :
  ...
  buffers(rnti_, isrlog::fetch_basic_logger(sched_cfg_.sched_cfg.logger_name)),
...
{
  isrran_assert(carriers[cc] != nullptr, "make_slot_ue() called for unknown rnti=0x%x,cc=%d", rnti, cc);
