...
ue_nr::ue_nr(uint16_t                rnti_,
  ...
  phy(phy_),
  logger(logger_),
  ue_rlc_buffer(isrran::make_byte_buffer())
{}
...
int ue_nr::generate_pdu(isrran::byte_buffer_t* pdu, uint32_t grant_size, isrran::const_span<uint32_t> subpdu_lcids)
{
  std::lock_guard<std::mutex> lock(mutex);

  if (mac_pdu_dl.init_tx(pdu, grant_size) != ISRRAN_SUCCESS) {
  ...
  for (const auto& lcid : subpdu_lcids) {
    ...
    if (lcid == isrran::mac_sch_subpdu_nr::CON_RES_ID) {
      if (last_msg3 != nullptr) {
        isrran::mac_sch_subpdu_nr::ue_con_res_id_t id;
        memcpy(id.data(), last_msg3->msg, id.size());
        if (mac_pdu_dl.add_ue_con_res_id_ce(id) != ISRRAN_SUCCESS) {
      ...
      while (remaining_len >= MIN_RLC_PDU_LEN) {
        ...
        remaining_len -= remaining_len >= isrran::mac_sch_subpdu_nr::MAC_SUBHEADER_LEN_THRESHOLD ? 3 : 2;
        ...
        if (pdu_len > remaining_len) {
          ...
          if (pdu_len > 0) {
            ...
            if (mac_pdu_dl.add_sdu(lcid, ue_rlc_buffer->msg, ue_rlc_buffer->N_bytes) != ISRRAN_SUCCESS) {
...
void ue_nr::metrics_dl_cqi(const isrran_uci_cfg_nr_t& cfg_, uint32_t dl_cqi)
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ...
  for (uint32_t i = 0; i < cfg_.nof_csi; i++) {
    ...
    if (cfg_.csi[i].cfg.quantity != ISRRAN_CSI_REPORT_QUANTITY_CRI_RI_PMI_CQI or
        cfg_.csi[i].cfg.freq_cfg != ISRRAN_CSI_REPORT_FREQ_WIDEBAND) {
      continue;
    }

    // Add statistics
    ue_metrics.dl_cqi = ISRRAN_VEC_SAFE_CMA(dl_cqi, ue_metrics.dl_cqi, dl_cqi_valid_counter);
...
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ue_metrics.dl_mcs = ISRRAN_VEC_CMA((float)mcs, ue_metrics.dl_mcs, ue_metrics.dl_mcs_samples);
...
{
  std::lock_guard<std::mutex> lock(metrics_mutex);
  ue_metrics.ul_mcs = ISRRAN_VEC_CMA((float)mcs, ue_metrics.ul_mcs, ue_metrics.ul_mcs_samples);
...
void ue_nr::metrics_pucch_sinr(float sinr)
{
  ...
  if (!std::isinf(sinr) && !std::isnan(sinr)) {
    ue_metrics.pucch_sinr = ISRRAN_VEC_SAFE_CMA((float)sinr, ue_metrics.pucch_sinr, pucch_sinr_counter);
    pucch_sinr_counter++;
  }
}
...
void ue_nr::metrics_pusch_sinr(float sinr)
{
  ...
  // discard nan or inf values for average SINR
  if (!std::isinf(sinr) && !std::isnan(sinr)) {
    ue_metrics.pusch_sinr = ISRRAN_VEC_SAFE_CMA((float)sinr, ue_metrics.pusch_sinr, pusch_sinr_counter);
    pusch_sinr_counter++;
  }
}

// Called from Stack thread when demuxing UL PDUs
void ue_nr::store_msg3(isrran::unique_byte_buffer_t pdu)
{
  std::lock_guard<std::mutex> lock(mutex);
