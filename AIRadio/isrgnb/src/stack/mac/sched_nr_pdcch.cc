...
void log_pdcch_alloc_failure(isrlog::log_channel& log_ch,
                             ...
                             Args&&... args)
{
  ...
  fmt::format_to(fmtbuf,
                 ...
                 isrran_rnti_type_str_short(rnti_type),
                 rnti,
                 ss_id);
  fmt::format_to(fmtbuf, cause_fmt, std::forward<Args>(args)...);
  log_ch("%s", isrran::to_c_str(fmtbuf));
}
  ...
  common_cce_list(bwp_cfg_.common_cce_list)
{
  ...
  nof_freq_res           = std::count(res_active, res_active + ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE, true);
  isrran_assert(get_td_symbols() <= ISRRAN_CORESET_DURATION_MAX,
                "Possible number of time-domain OFDM symbols in CORESET must be within {1,2,3}");
  isrran_assert(nof_freq_res <= bwp_cfg_.cell_cfg.nof_prb(),
...
bool coreset_region::alloc_pdcch(isrran_rnti_type_t         rnti_type,
                                 bool                       is_dl,
                                 uint32_t                   aggr_idx,
...
{
  isrran_assert(not dci_list.empty(), "%s called when no PDCCH have yet been allocated", __FUNCTION__);
...
bool coreset_region::alloc_dfs_node(const alloc_record& record, uint32_t start_dci_idx)
{
  ...
  node.rnti        = record.ue != nullptr ? record.ue->rnti : ISRRAN_INVALID_RNTI;
...
isrran::span<const uint32_t> coreset_region::get_cce_loc_table(const alloc_record& record) const
{
  switch (record.dci->rnti_type) {
    ...
    default:
      isrran_terminate("Invalid RNTI type=%s", isrran_rnti_type_str(record.dci->rnti_type));
      break;
  }
  return {};
}
...
void coreset_region::print_allocations(fmt::memory_buffer& fmtbuf) const
{
  ...
  for (const alloc_record& dci : dci_list) {
    fmt::format_to(fmtbuf,
                   "  {}-rnti=0x{:x}: ({}, {})\n",
                   isrran_rnti_type_str_short(dci.dci->rnti_type),
                   dci.dci->rnti,
...
{
  for (uint32_t cs_idx = 0; cs_idx < ISRRAN_UE_DL_NR_MAX_NOF_CORESET; ++cs_idx) {
...
void bwp_pdcch_allocator::fill_dci_ctx_common(isrran_dci_ctx_t&            dci,
                                              isrran_rnti_type_t           rnti_type,
                                              uint16_t                     rnti,
                                              ...
                                              const ue_carrier_params_t*   ue)
{
  ...
  dci.coreset_start_rb = isrran_coreset_start_rb(coreset);
...
{
  isrran_assert(bwp_cfg.cfg.pdcch.ra_search_space_present, "Allocating RAR PDCCH in BWP without RA SearchSpace");
  return alloc_dl_pdcch_common(
      isrran_rnti_type_ra, ra_rnti, bwp_cfg.cfg.pdcch.ra_search_space.id, aggr_idx, isrran_dci_format_nr_1_0, nullptr);
}

pdcch_dl_alloc_result bwp_pdcch_allocator::alloc_si_pdcch(uint32_t ss_id, uint32_t aggr_idx)
{
  return alloc_dl_pdcch_common(isrran_rnti_type_si, ISRRAN_SIRNTI, ss_id, aggr_idx, isrran_dci_format_nr_1_0, nullptr);
}

pdcch_dl_alloc_result bwp_pdcch_allocator::alloc_dl_pdcch(isrran_rnti_type_t         rnti_type,
                                                          uint32_t                   ss_id,
                                                          uint32_t                   aggr_idx,
                                                          const ue_carrier_params_t& user)
{
  static const isrran_dci_format_nr_t dci_fmt = isrran_dci_format_nr_1_0; // TODO: make it configurable
  isrran_assert(rnti_type == isrran_rnti_type_c or rnti_type == isrran_rnti_type_tc,
                "Invalid RNTI type=%s for UE-specific PDCCH",
                isrran_rnti_type_str_short(rnti_type));
  return alloc_dl_pdcch_common(rnti_type, user.rnti, ss_id, aggr_idx, dci_fmt, &user);
}

pdcch_dl_alloc_result bwp_pdcch_allocator::alloc_dl_pdcch_common(isrran_rnti_type_t         rnti_type,
                                                                 uint16_t                   rnti,
                                                                 uint32_t                   ss_id,
                                                                 uint32_t                   aggr_idx,
                                                                 isrran_dci_format_nr_t     dci_fmt,
                                                                 const ue_carrier_params_t* user)
{
  ...
  const isrran_search_space_t& ss =
      (user == nullptr)
          ? (rnti_type == isrran_rnti_type_ra ? bwp_cfg.cfg.pdcch.ra_search_space : *bwp_cfg.get_ss(ss_id))
          : *user->get_ss(ss_id);
  ...
  if (not success) {
    ...
    // Log PDCCH allocation failure
    isrlog::log_channel& ch = user == nullptr ? logger.warning : logger.debug;
...
{
  static const isrran_dci_format_nr_t dci_fmt = isrran_dci_format_nr_0_0; // TODO: make it configurable
  alloc_result r = check_args_valid(isrran_rnti_type_c, user.rnti, ss_id, aggr_idx, dci_fmt, &user, false);
  if (r != alloc_result::success) {
    return {r};
  }
  const isrran_search_space_t& ss = *user.get_ss(ss_id);

  // Add new UL PDCCH to sched result
  pdcch_ul_list.emplace_back();

  bool success = coresets[ss.coreset_id].alloc_pdcch(
      isrran_rnti_type_c, false, aggr_idx, ss_id, &user, pdcch_ul_list.back().dci.ctx);
  ...
  if (not success) {
    ...
    // Log PDCCH allocation failure
    log_pdcch_alloc_failure(logger.debug, isrran_rnti_type_c, ss_id, user.rnti, "No available PDCCH position");
  ...
  // Fill DCI context information
  fill_dci_ctx_common(pdcch.dci.ctx, isrran_rnti_type_c, user.rnti, ss, dci_fmt, &user);
...
{
  isrran_assert(pending_dci != nullptr, "Trying to abort PDCCH allocation that does not exist");
...
alloc_result bwp_pdcch_allocator::check_args_valid(isrran_rnti_type_t         rnti_type,
                                                   uint16_t                   rnti,
                                                   uint32_t                   ss_id,
                                                   uint32_t                   aggr_idx,
                                                   isrran_dci_format_nr_t     dci_fmt,
                                                   ...
                                                   bool                       is_dl) const
{
  isrran_assert(ss_id < ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE, "Invalid SearchSpace#%d", ss_id);
  isrran_assert(
      aggr_idx < ISRRAN_SEARCH_SPACE_NOF_AGGREGATION_LEVELS_NR, "Invalid aggregation level index=%d", aggr_idx);
  ...
  if (not is_rnti_type_valid_in_search_space(rnti_type, ss->type)) {
    ...
    log_pdcch_alloc_failure(logger.warning,
                            ...
                            isrran_ss_type_str(ss->type));
    return alloc_result::invalid_grant_params;
  }
  auto dci_fmt_equal = [dci_fmt](isrran_dci_format_nr_t f) { return f == dci_fmt; };
  if (std::none_of(&ss->formats[0], &ss->formats[ss->nof_formats], dci_fmt_equal)) {
    log_pdcch_alloc_failure(logger.warning,
                            ...
                            isrran_dci_format_nr_string(dci_fmt));
    return alloc_result::invalid_grant_params;
  }
  ...
  isrran_sanity_check(pdcch_dl_list.size() + pdcch_ul_list.size() == nof_allocations(), "Invalid PDCCH state");
