...
void get_dci_locs(const isrran_coreset_t&      coreset,
                  const isrran_search_space_t& search_space,
                  uint16_t                     rnti,
                  bwp_cce_pos_list&            cce_locs)
{
  for (uint32_t sl = 0; sl < ISRRAN_NOF_SF_X_FRAME; ++sl) {
    for (uint32_t agg_idx = 0; agg_idx < MAX_NOF_AGGR_LEVELS; ++agg_idx) {
      ...
      uint32_t n =
          isrran_pdcch_nr_locations_coreset(&coreset, &search_space, rnti, agg_idx, sl, cce_locs[sl][agg_idx].data());
...
{
  isrran_assert(cfg.pdcch.ra_search_space_present, "BWPs without RA search space not supported");
  ...
  for (const isrran_coreset_t& cs : view_active_coresets(cfg.pdcch)) {
    coresets.emplace(cs.id);
    uint32_t rb_start                         = isrran_coreset_start_rb(&cs);
    coresets[cs.id].prb_limits                = prb_interval{rb_start, rb_start + isrran_coreset_get_bw(&cs)};
  ...
  uint32_t nof_slots = ISRRAN_NSLOTS_PER_FRAME_NR(cfg.numerology_idx);
  for (size_t sl = 0; sl < nof_slots; ++sl) {
    slot_cfg sl_cfg{};
    sl_cfg.is_dl = isrran_duplex_nr_is_dl(&cell_cfg.duplex, cfg.numerology_idx, sl);
    sl_cfg.is_ul = isrran_duplex_nr_is_ul(&cell_cfg.duplex, cfg.numerology_idx, sl);
  ...
  for (uint32_t m = 0; m < cfg.pusch.nof_common_time_ra; ++m) {
    int ret =
        isrran_ra_ul_nr_time(&cfg.pusch, isrran_rnti_type_ra, isrran_search_space_type_rar, ra_coreset_id, m, &grant);
    isrran_assert(ret == ISRRAN_SUCCESS, "Failed to obtain RA config");
    pusch_ra_list[m].msg3_delay = grant.k;
    ret = isrran_ra_ul_nr_time(&cfg.pusch, isrran_rnti_type_c, isrran_search_space_type_ue, ra_coreset_id, m, &grant);
    pusch_ra_list[m].K = grant.k;
    pusch_ra_list[m].S = grant.S;
    pusch_ra_list[m].L = grant.L;
    isrran_assert(ret == ISRRAN_SUCCESS, "Failed to obtain RA config");
  }
  isrran_assert(not pusch_ra_list.empty(), "Time-Domain Resource Allocation not valid");

  for (uint32_t sl = 0; sl < ISRRAN_NOF_SF_X_FRAME; ++sl) {
    for (uint32_t agg_idx = 0; agg_idx < MAX_NOF_AGGR_LEVELS; ++agg_idx) {
      rar_cce_list[sl][agg_idx].resize(ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR);
      int n = isrran_pdcch_nr_locations_coreset(&cell_cfg.bwps[0].cfg.pdcch.coreset[ra_coreset_id],
                                                &cell_cfg.bwps[0].cfg.pdcch.ra_search_space,
                                                0,
                                                agg_idx,
                                                sl,
                                                rar_cce_list[sl][agg_idx].data());
      isrran_assert(n >= 0, "Failed to configure RAR DCI locations");
      rar_cce_list[sl][agg_idx].resize(n);
    }
  }

  for (uint32_t ss_id = 0; ss_id < ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE; ++ss_id) {
    ...
    for (uint32_t sl = 0; sl < ISRRAN_NOF_SF_X_FRAME; ++sl) {
      for (uint32_t agg_idx = 0; agg_idx < MAX_NOF_AGGR_LEVELS; ++agg_idx) {
        ss_cce_list[sl][agg_idx].resize(ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR);
        int n = isrran_pdcch_nr_locations_coreset(
            &coreset, &ss, ISRRAN_SIRNTI, agg_idx, sl, ss_cce_list[sl][agg_idx].data());
        isrran_assert(n >= 0, "Failed to configure DCI locations of search space id=%d", ss_id);
  ...
  cc(cc_), sched_args(sched_args_), default_ue_phy_cfg(get_common_ue_phy_cfg(cell)), sibs(cell.sibs)
{
  ...
  carrier.scs = (isrran_subcarrier_spacing_t)cell.dl_cfg_common.init_dl_bwp.generic_params.subcarrier_spacing.value;

  // TDD-UL-DL-ConfigCommon
  duplex.mode = ISRRAN_DUPLEX_MODE_FDD;
  if (cell.tdd_ul_dl_cfg_common.has_value()) {
    bool success = isrran::make_phy_tdd_cfg(*cell.tdd_ul_dl_cfg_common, &duplex);
    isrran_assert(success, "Failed to generate Cell TDD config");
  }
  ...
  isrran_assert(not bwps.empty(), "No BWPs were configured");
}

sched_params_t::sched_params_t(const sched_args_t& sched_cfg_) : sched_cfg(sched_cfg_)
{
  isrran_assert(sched_cfg.fixed_ul_mcs >= 0, "Dynamic DL MCS not supported");
}
