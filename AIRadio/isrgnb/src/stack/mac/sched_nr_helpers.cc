...
#include "isrgnb/hdr/stack/mac/sched_nr_harq.h"
#include "isrgnb/hdr/stack/mac/sched_nr_ue.h"
...
void log_sched_slot_ues(isrlog::basic_logger& logger, slot_point pdcch_slot, uint32_t cc, const slot_ue_map_t& slot_ues)
{
  ...
  logger.debug("%s]", isrran::to_c_str(fmtbuf));
}
                          ...
                          const slot_ue_map_t&  slot_ues)
{
  ...
  for (const pdcch_dl_t& pdcch : bwp_slot.dl.phy.pdcch_dl) {
    fmt::memory_buffer fmtbuf;
    if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_c) {
      ...
      fmt::format_to(fmtbuf,
                     ...
                     isrran_dci_format_nr_string(pdcch.dci.ctx.format),
                     ue.h_dl->prbs(),
                     ue.h_dl->nof_retx(),
                     pdcch.dci.dai,
                     ...
                     ue.uci_slot);
      data_count++;
    } else if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_ra) {
      const pdsch_t&           pdsch = bwp_slot.dl.phy.pdsch[std::distance(bwp_slot.dl.phy.pdcch_dl.data(), &pdcch)];
      isrran::const_span<bool> prbs{pdsch.sch.grant.prb_idx, pdsch.sch.grant.prb_idx + ISRRAN_MAX_PRB_NR};
      ...
      fmt::format_to(fmtbuf,
                     ...
                     res_grid.cfg->cc,
                     pdcch.dci.ctx.rnti,
                     isrran::interval<uint32_t>{start_idx, end_idx},
                     pdcch_slot,
                     pdcch_slot + res_grid.cfg->pusch_ra_list[0].msg3_delay,
                     bwp_slot.dl.rar[rar_count].grants.size());
      rar_count++;
    } else if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_si) {
    ...
    if (fmtbuf.size() > 0) {
      if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_si) {
        logger.debug("%s", isrran::to_c_str(fmtbuf));
      } else {
        logger.info("%s", isrran::to_c_str(fmtbuf));
      }
    }
  }
  for (const pdcch_ul_t& pdcch : bwp_slot.dl.phy.pdcch_ul) {
    ...
    if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_c) {
      ...
      fmt::format_to(fmtbuf,
                     ...
                     ue.ul_bytes,
                     ue.pusch_slot);
    } else if (pdcch.dci.ctx.rnti_type == isrran_rnti_type_tc) {
      const slot_ue& ue = slot_ues[pdcch.dci.ctx.rnti];
      fmt::format_to(fmtbuf,
