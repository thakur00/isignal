...
uint32_t coreset_nof_cces(const isrran_coreset_t& coreset)
{
  ...
  uint32_t    nof_freq_res = std::count(res_active, res_active + ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE, true);
...
{
  *mib            = {};
  mib->scs_common = (isrran_subcarrier_spacing_t)cfg.dl_cfg_common.init_dl_bwp.generic_params.subcarrier_spacing.value;
  mib->ssb_offset = cfg.ssb_offset;
  mib->dmrs_typeA_pos         = (isrran_dmrs_sch_typeA_pos_t)cfg.dmrs_type_a_position.value;
...
void make_ssb_cfg(const sched_nr_cell_cfg_t& cfg, isrran::phy_cfg_nr_t::ssb_cfg_t* ssb)
{
  ...
  ssb->scs     = (isrran_subcarrier_spacing_t)cfg.ssb_scs.value;
  ssb->pattern = ISRRAN_SSB_PATTERN_A;
  if (cfg.dl_cfg_common.freq_info_dl.freq_band_list.size() > 0 and
    ...
    ssb->pattern  = isrran::isrran_band_helper::get_ssb_pattern(band, ssb->scs);
  }
}

isrran::phy_cfg_nr_t get_common_ue_phy_cfg(const sched_nr_cell_cfg_t& cfg)
{
  isrran::phy_cfg_nr_t ue_phy_cfg;

  // TDD UL-DL config
  ue_phy_cfg.duplex.mode = ISRRAN_DUPLEX_MODE_FDD;
  if (cfg.tdd_ul_dl_cfg_common.has_value()) {
    bool success = isrran::make_phy_tdd_cfg(*cfg.tdd_ul_dl_cfg_common, &ue_phy_cfg.duplex);
    isrran_sanity_check(success, "Failed to convert Cell TDDConfig to UEPHYConfig");
  }
  ...
  isrran::make_phy_rach_cfg(cfg.ul_cfg_common.init_ul_bwp.rach_cfg_common.setup(),
                            cfg.tdd_ul_dl_cfg_common.has_value() ? ISRRAN_DUPLEX_MODE_TDD : ISRRAN_DUPLEX_MODE_FDD,
                            &ue_phy_cfg.prach);
  ...
  ue_phy_cfg.carrier.scs =
      (isrran_subcarrier_spacing_t)cfg.dl_cfg_common.init_dl_bwp.generic_params.subcarrier_spacing.value;
  ...
  for (uint32_t i = 0; i < ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE; ++i) {
    if (ue_phy_cfg.pdcch.search_space_present[i] and
        ue_phy_cfg.pdcch.search_space[i].type == isrran_search_space_type_ue) {
