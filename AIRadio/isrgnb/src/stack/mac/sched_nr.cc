...
#include "isrgnb/hdr/stack/mac/harq_softbuffer.h"
#include "isrgnb/hdr/stack/mac/sched_nr_bwp.h"
#include "isrgnb/hdr/stack/mac/sched_nr_worker.h"
...
public:
  ...
  struct logger {
    explicit logger(int cc_, isrlog::basic_logger& logger_) :
    ...
    ~logger()
    {
      if (log_enabled and event_fmtbuf.size() > 0) {
        if (cc < 0) {
          sched_logger.debug("SCHED: slot events: [%s]", isrran::to_c_str(event_fmtbuf));
        } else {
          sched_logger.debug("SCHED: slot events, cc=%d: [%s]", cc, isrran::to_c_str(event_fmtbuf));
        }
      }
    }
  ...
  explicit event_manager(sched_params_t& params) :
    sched_logger(isrlog::fetch_basic_logger(params.sched_cfg.logger_name)), carriers(params.cells.size())
  {
  }

  /// Enqueue an event that does not map into a ue method (e.g. rem_user, add_user)
  void enqueue_event(const char* event_name, isrran::move_callback<void(logger&)> ev)
  {
  ...
  void enqueue_ue_event(const char* event_name, uint16_t rnti, isrran::move_callback<void(ue&, logger&)> callback)
  {
    isrran_assert(rnti != ISRRAN_INVALID_RNTI, "Invalid rnti=0x%x passed to common event manager", rnti);
    std::lock_guard<std::mutex> lock(event_mutex);
  ...
  void enqueue_ue_cc_feedback(const char*                                       event_name,
                              uint16_t                                          rnti,
                              uint32_t                                          cc,
                              isrran::move_callback<void(ue_carrier&, logger&)> callback)
  {
    isrran_assert(rnti != ISRRAN_INVALID_RNTI, "Invalid rnti=0x%x passed to event manager", rnti);
    isrran_assert(cc < carriers.size(), "Invalid cc=%d passed to event manager", cc);
  ...
  void process_cc_events(ue_map_t& ues, uint32_t cc)
  {
    ...
    for (ue_event_t& ev : current_slot_ue_events) {
      if (ev.rnti == ISRRAN_INVALID_RNTI) {
...
private:
  struct event_t {
    ...
    isrran::move_callback<void(logger&)> callback;
    event_t(const char* event_name_, isrran::move_callback<void(logger&)> c) :
  ...
  struct ue_event_t {
    uint16_t                                  rnti;
    const char*                               event_name;
    isrran::move_callback<void(ue&, logger&)> callback;
    ue_event_t(uint16_t rnti_, const char* event_name_, isrran::move_callback<void(ue&, logger&)> c) :
  ...
  struct ue_cc_event_t {
    ...
    ue_cc_event_t(uint16_t                                          rnti_,
                  ...
                  isrran::move_callback<void(ue_carrier&, logger&)> c) :
  ...
  struct cc_events {
    ...
    isrran::deque<ue_cc_event_t> next_slot_ue_events, current_slot_ue_events;
  };
...
sched_nr::sched_nr() : logger(&isrlog::fetch_basic_logger("MAC-NR")), metrics_handler(new ue_metrics_manager{ue_db}) {}
...
int sched_nr::config(const sched_args_t& sched_cfg, isrran::const_span<sched_nr_cell_cfg_t> cell_list)
{
  cfg    = sched_params_t{sched_cfg};
  logger = &isrlog::fetch_basic_logger(sched_cfg.logger_name);

  // Initiate UE memory pool
  ue_pool.reset(new isrran::circular_stack_pool<ISRENB_MAX_UES>(8, sizeof(ue), 4));
...
{
  isrran_assert(assert_ue_cfg_valid(rnti, uecfg) == ISRRAN_SUCCESS, "Invalid UE configuration");
  pending_events->enqueue_event("ue_cfg", [this, rnti, uecfg](event_manager::logger& ev_logger) {
    if (ue_cfg_impl(rnti, uecfg) == ISRRAN_SUCCESS) {
...
int sched_nr::add_ue_impl(uint16_t rnti, sched_nr_impl::unique_ue_ptr u)
{
  ...
  return ue_db.insert(rnti, std::move(u)).has_value() ? ISRRAN_SUCCESS : ISRRAN_ERROR;
}
...
int sched_nr::ue_cfg_impl(uint16_t rnti, const ue_cfg_t& uecfg)
{
  if (not ue_db.contains(rnti)) {
    // create user object
    unique_ue_ptr u = isrran::make_pool_obj_with_fallback<ue>(*ue_pool, rnti, rnti, uecfg, cfg);
...
{
  isrran_assert(worker_count.load(std::memory_order_relaxed) == 0,
...
{
  isrran_assert(pdsch_tti == current_slot_tx, "Unexpected pdsch_tti slot received");
  ...
  isrran_assert(rem_workers >= 0, "invalid number of calls to get_dl_sched(slot, cc)");
  if (rem_workers == 0) {
...
int sched_nr::dl_rach_info(const rar_info_t& rar_info)
{
  ...
  unique_ue_ptr u =
      isrran::make_pool_obj_with_fallback<ue>(*ue_pool, rar_info.temp_crnti, rar_info.temp_crnti, rar_info.cc, cfg);
  ...
  auto add_ue = [this, rar_info, u = std::move(u)](event_manager::logger& ev_logger) mutable {
    ...
    if (add_ue_impl(rnti, std::move(u)) == ISRRAN_SUCCESS) {
...
#define VERIFY_INPUT(cond, msg, ...)                                                                                   \
  do {                                                                                                                 \
    if (not(cond)) {                                                                                                   \
      isrlog::fetch_basic_logger("MAC").warning(msg, ##__VA_ARGS__);                                                   \
      return ISRRAN_ERROR;                                                                                             \
    }                                                                                                                  \
  } while (0)
...
int assert_ue_cfg_valid(uint16_t rnti, const sched_nr_interface::ue_cfg_t& uecfg)
{
  VERIFY_INPUT(std::count(&uecfg.phy_cfg.pdcch.coreset_present[0],
                          &uecfg.phy_cfg.pdcch.coreset_present[ISRRAN_UE_DL_NR_MAX_NOF_CORESET],
                          true) > 0,
