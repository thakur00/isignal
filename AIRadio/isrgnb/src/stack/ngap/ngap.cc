...
#include "isrgnb/hdr/stack/ngap/ngap_ue.h"
...
isrran::proc_outcome_t ngap::ng_setup_proc_t::init()
{
  procInfo("Starting new AMF connection.");
  return start_amf_connection();
}

isrran::proc_outcome_t ngap::ng_setup_proc_t::start_amf_connection()
{
  ...
  if (not ngap_ptr->setup_ng()) {
    ...
    isrran::console("NG setup failed\n");
...
isrran::proc_outcome_t ngap::ng_setup_proc_t::react(const isrenb::ngap::ng_setup_proc_t::ngsetupresult& event)
{
  ...
  if (event.success) {
    ...
    isrran::console("NG connection successful\n");
    return isrran::proc_outcome_t::success;
  }
  procError("NGSetup failed.");
  isrran::console("NGsetup failed\n");
  return isrran::proc_outcome_t::error;
}

void ngap::ng_setup_proc_t::then(const isrran::proc_state_t& result) const
{
  if (result.is_error()) {
    procInfo("Failed to initiate NG connection. Attempting reconnection in %d seconds",
             ngap_ptr->amf_connect_timer.duration() / 1000);
    isrran::console("Failed to initiate NG connection. Attempting reconnection in %d seconds\n",
                    ngap_ptr->amf_connect_timer.duration() / 1000);
...
ngap::ngap(isrran::task_sched_handle   task_sched_,
...
int ngap::build_tai_cgi()
{
  ...
  // TAI
  isrran::s1ap_mccmnc_to_plmn(args.mcc, args.mnc, &plmn);
...
void ngap::write_pdu(uint16_t rnti, isrran::const_byte_span pdu)
{
...
{
  static isrlog::basic_logger& logger = isrlog::fetch_basic_logger("NGAP");
...
********************************************************************************/
bool ngap::handle_amf_rx_msg(isrran::unique_byte_buffer_t pdu,
...
bool ngap::handle_ngap_rx_pdu(isrran::byte_buffer_t* pdu)
{
  ...
  // Logging
  log_ngap_message(rx_pdu, Rx, isrran::make_span(*pdu));
...
bool ngap::handle_ng_setup_failure(const asn1::ngap::ng_setup_fail_s& msg)
{
  ...
  isrran::console("NG Setup Failure. Cause: %s\n", cause.c_str());
  return true;
}
...
bool ngap::connect_amf()
{
  ...
  rx_socket_handler->add_socket_handler(amf_socket.fd(),
                                        isrran::make_sctp_sdu_handler(logger, amf_task_queue, rx_callback));
...
{
  if (not amf_connected and rnti != ISRRAN_INVALID_RNTI) {
 ...
 * PCAP
 */
void ngap::start_pcap(isrran::ngap_pcap* pcap_)
{
  pcap = pcap_;
}

void ngap::log_ngap_message(const ngap_pdu_c& msg, const direction_t dir, isrran::const_byte_span pdu)
{
  std::string msg_type = {};
