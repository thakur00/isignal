...
#include "isrgnb/hdr/stack/ngap/ngap_ue_proc.h"
                                       ...
                                       uint32_t                             s_tmsi)
{
  ...
  if (has_tmsi) {
    ...
    isrran::uint32_to_uint8(s_tmsi, container->five_g_s_tmsi.value.five_g_tmsi.data());
...
bool ngap::ue::send_ul_nas_transport(isrran::const_byte_span pdu)
{
  if (not ngap_ptr->amf_connected) {
    logger.warning("AMF not connected");
    return false;
