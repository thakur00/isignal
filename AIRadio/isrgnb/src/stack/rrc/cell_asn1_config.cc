...
isrlog::basic_logger& get_logger(const rrc_nr_cfg_t& cfg)
{
  static isrlog::basic_logger& log_obj = isrlog::fetch_basic_logger(cfg.log_name);
  return log_obj;
}

void set_search_space_from_phy_cfg(const isrran_search_space_t& cfg, asn1::rrc_nr::search_space_s& out)
{
  ...
  isrran_assert(ret, "Failed to convert nof candidates=%d", cfg.nof_candidates[0]);
  ret = asn1::number_to_enum(out.nrof_candidates.aggregation_level2, cfg.nof_candidates[1]);
  isrran_assert(ret, "Failed to convert nof candidates=%d", cfg.nof_candidates[1]);
  ret = asn1::number_to_enum(out.nrof_candidates.aggregation_level4, cfg.nof_candidates[2]);
  isrran_assert(ret, "Failed to convert nof candidates=%d", cfg.nof_candidates[2]);
  ret = asn1::number_to_enum(out.nrof_candidates.aggregation_level8, cfg.nof_candidates[3]);
  isrran_assert(ret, "Failed to convert nof candidates=%d", cfg.nof_candidates[3]);
  ret = asn1::number_to_enum(out.nrof_candidates.aggregation_level16, cfg.nof_candidates[4]);
  isrran_assert(ret, "Failed to convert nof candidates=%d", cfg.nof_candidates[4]);

  out.search_space_type_present = true;
  if ((cfg.type == isrran_search_space_type_common_0) or (cfg.type == isrran_search_space_type_common_0A) or
      (cfg.type == isrran_search_space_type_common_1) or (cfg.type == isrran_search_space_type_common_2) or
      (cfg.type == isrran_search_space_type_common_3)) {
    out.search_space_type.set_common();

    out.search_space_type.common().dci_format0_minus0_and_format1_minus0_present = true;
  } else if (cfg.type == isrran_search_space_type_ue) {
    ...
    isrran_terminate("Config Error: Unsupported search space type.");
  }
}

void set_coreset_from_phy_cfg(const isrran_coreset_t& coreset_cfg, asn1::rrc_nr::ctrl_res_set_s& out)
{
  out.ctrl_res_set_id = coreset_cfg.id;

  std::bitset<ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE> freq_domain_res;
  for (uint32_t i = 0; i < ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE; i++) {
    freq_domain_res[ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE - 1 - i] = coreset_cfg.freq_resources[i];
  }
  out.freq_domain_res.from_number(freq_domain_res.to_ulong());

  out.dur = coreset_cfg.duration;

  if (coreset_cfg.mapping_type == isrran_coreset_mapping_type_non_interleaved) {
  ...
  if (coreset_cfg.precoder_granularity == isrran_coreset_precoder_granularity_reg_bundle) {
...
void set_rach_cfg_common(const isrran_prach_cfg_t& prach_cfg, asn1::rrc_nr::rach_cfg_common_s& out)
{
  // rach-ConfigGeneric
...
{
  isrran_assert(cfg.duplex_mode == ISRRAN_DUPLEX_MODE_TDD, "This function should only be called for TDD configs");
  // TDD UL-DL config
...
int fill_csi_report_from_enb_cfg(const rrc_nr_cfg_t& cfg, csi_meas_cfg_s& csi_meas_cfg)
{
  if (cfg.is_standalone) {
    ...
    if (cfg.cell_list[0].duplex_mode == ISRRAN_DUPLEX_MODE_FDD) {
...
{
  //  // Fill CSI Report
  //  if (fill_csi_report_from_enb_cfg(cfg, csi_meas_cfg) != ISRRAN_SUCCESS) {
...
void fill_pucch_cfg_from_enb_cfg(const rrc_nr_cfg_t& cfg, uint32_t cc, pucch_cfg_s& out)
{
  ...
  // DL data
  if (cfg.cell_list[cc].duplex_mode == ISRRAN_DUPLEX_MODE_FDD) {
...
int fill_freq_info_dl_from_enb_cfg(const rrc_nr_cfg_t& cfg, uint32_t cc, freq_info_dl_s& freq_info_dl)
{
  ...
  if (!asn1::number_to_enum(dl_carrier.subcarrier_spacing,
                            ISRRAN_SUBC_SPACING_NR(cell_cfg.phy_cell.carrier.scs) / 1000)) {
    get_logger(cfg).error("Config Error: Invalid subcarrier spacing (%d).\n",
                          ISRRAN_SUBC_SPACING_NR(cell_cfg.phy_cell.carrier.scs));
    return ISRRAN_ERROR;
  }
...
int fill_rach_cfg_common(const rrc_nr_cfg_t& cfg, uint32_t cc, rach_cfg_common_s& rach)
{
  ...
  if (cfg.cell_list[cc].duplex_mode == ISRRAN_DUPLEX_MODE_TDD) {
...
void fill_srb(const rrc_nr_cfg_t& cfg, isrran::nr_srb srb_id, asn1::rrc_nr::rlc_bearer_cfg_s& out)
{
  isrran_assert(srb_id > isrran::nr_srb::srb0 and srb_id < isrran::nr_srb::count, "Invalid srb_id argument");

  out.lc_ch_id                         = isrran::srb_to_lcid(srb_id);
  out.served_radio_bearer_present      = true;
  out.served_radio_bearer.set_srb_id() = (uint8_t)srb_id;

  if (srb_id == isrran::nr_srb::srb1) {
  ...
  } else if (srb_id == isrran::nr_srb::srb2) {
  ...
  out.mac_lc_ch_cfg.ul_specific_params.prio    = srb_id == isrran::nr_srb::srb1 ? 1 : 3;
...
int fill_master_cell_cfg_from_enb_cfg(const rrc_nr_cfg_t& cfg, uint32_t cc, asn1::rrc_nr::cell_group_cfg_s& out)
{
  ...
  fill_srb(cfg, isrran::nr_srb::srb1, out.rlc_bearer_to_add_mod_list[0]);
...
int fill_mib_from_enb_cfg(const rrc_cell_cfg_nr_t& cell_cfg, asn1::rrc_nr::mib_s& mib)
{
  ...
  switch (cell_cfg.phy_cell.carrier.scs) {
    ...
    case isrran_subcarrier_spacing_120kHz:
      ...
      break;
    default:
      isrran_terminate("Invalid carrier SCS=%d Hz", ISRRAN_SUBC_SPACING_NR(cell_cfg.phy_cell.carrier.scs));
  }
...
void fill_dl_cfg_common_sib(const rrc_nr_cfg_t& cfg, uint32_t cc, dl_cfg_common_sib_s& out)
{
  ...
  uint32_t scs_hz = ISRRAN_SUBC_SPACING_NR(cell_cfg.phy_cell.carrier.scs);
  uint32_t prb_bw = scs_hz * ISRRAN_NRE;
  ...
  double   ssb_freq_start                                     = cell_cfg.ssb_freq_hz - ISRRAN_SSB_BW_SUBC * scs_hz / 2;
...
int fill_serv_cell_cfg_common_sib(const rrc_nr_cfg_t& cfg, uint32_t cc, serving_cell_cfg_common_sib_s& out)
{
  ...
  // TDD UL-DL config
  if (cell_cfg.duplex_mode == ISRRAN_DUPLEX_MODE_TDD) {
...
int fill_sib1_from_enb_cfg(const rrc_nr_cfg_t& cfg, uint32_t cc, asn1::rrc_nr::sib1_s& sib1)
{
  ...
  isrran::to_asn1(&sib1.cell_access_related_info.plmn_id_list[0].plmn_id_list[0], plmn);
                                   ...
                                   radio_bearer_cfg_s&       diff)
{
  ...
  isrran::compute_cfg_diff(prev_bearers.drb_to_add_mod_list,
                                         ...
                                         asn1::rrc_nr::cell_group_cfg_s&         out)
{
  ...
  for (const srb_to_add_mod_s& srb : bearers.srb_to_add_mod_list) {
    ...
    fill_srb(cfg, (isrran::nr_srb)srb.srb_id, out.rlc_bearer_to_add_mod_list.back());
  }
  // Add DRBs
  for (const drb_to_add_mod_s& drb : bearers.drb_to_add_mod_list) {
    out.rlc_bearer_to_add_mod_list.push_back({});
    uint32_t                           lcid = drb.drb_id + (int)isrran::nr_srb::count - 1;
    ...
    if (rb.is_valid() and cfg.five_qi_cfg.find(rb.five_qi) != cfg.five_qi_cfg.end()) {
      fill_drb(cfg, rb, (isrran::nr_drb)drb.drb_id, out.rlc_bearer_to_add_mod_list.back());
    } else {
      return ISRRAN_ERROR;
    }
  }
