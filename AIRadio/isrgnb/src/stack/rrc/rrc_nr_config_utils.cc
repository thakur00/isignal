...
ctrl_res_set_s make_default_coreset(uint8_t coreset_id, uint32_t nof_prb)
{
  ...
  for (uint32_t i = 0; i < ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE; i++) {
    coreset.freq_domain_res.set(coreset.freq_domain_res.length() - i - 1, i < ISRRAN_FLOOR(nof_prb, 6));
  }
  coreset.dur = 1;
...
search_space_s make_default_common_search_space(uint8_t ss_id, const ctrl_res_set_s& cs)
{
  ...
  uint32_t nof_cand = ISRRAN_MIN(coreset_get_pdcch_nr_max_candidates(cs, 0), 2);
  asn1::number_to_enum(ss.nrof_candidates.aggregation_level1, nof_cand);
  nof_cand = ISRRAN_MIN(coreset_get_pdcch_nr_max_candidates(cs, 1), 2);
  asn1::number_to_enum(ss.nrof_candidates.aggregation_level2, nof_cand);
  nof_cand = ISRRAN_MIN(coreset_get_pdcch_nr_max_candidates(cs, 2), 2);
  asn1::number_to_enum(ss.nrof_candidates.aggregation_level4, nof_cand);
  nof_cand = ISRRAN_MIN(coreset_get_pdcch_nr_max_candidates(cs, 3), 2);
  asn1::number_to_enum(ss.nrof_candidates.aggregation_level8, nof_cand);
  nof_cand = ISRRAN_MIN(coreset_get_pdcch_nr_max_candidates(cs, 4), 2);
...
void generate_default_nr_phy_cell(phy_cell_cfg_nr_t& phy_cell)
{
  ...
  phy_cell.carrier.scs             = isrran_subcarrier_spacing_15kHz;
                      ...
                      rrc_cell_cfg_nr_t&          cell)
{
  ...
  ERROR_IF_NOT(pdcch_scs < isrran_subcarrier_spacing_invalid, "Invalid carrier SCS");
  ...
  cell.ssb_pattern = band_helper.get_ssb_pattern(band, isrran_subcarrier_spacing_15kHz);
  if (cell.ssb_pattern == ISRRAN_SSB_PATTERN_A) {
    // 15kHz SSB SCS
    cell.ssb_scs = isrran_subcarrier_spacing_15kHz;
  } else {
    // try to optain SSB pattern for same band with 30kHz SCS
    cell.ssb_pattern = band_helper.get_ssb_pattern(band, isrran_subcarrier_spacing_30kHz);
    if (cell.ssb_pattern == ISRRAN_SSB_PATTERN_B || cell.ssb_pattern == ISRRAN_SSB_PATTERN_C) {
      // SSB SCS is 30 kHz
      cell.ssb_scs = isrran_subcarrier_spacing_30kHz;
    } else {
      isrran_terminate("Can't derive SSB pattern from band %d", band);
    }
  }
  ...
  if (is_sa) {
    ...
    coreset0_rb_offset = isrran_coreset0_ssb_offset(coreset0_idx, cell.ssb_scs, pdcch_scs);
  ...
  double   pointA_abs_freq_Hz = dl_freq_hz - nof_prb * ISRRAN_NRE * ISRRAN_SUBC_SPACING_NR(pdcch_scs) / 2;
  uint32_t ssb_pointA_freq_offset_Hz =
      (cell.ssb_freq_hz > pointA_abs_freq_Hz) ? (uint32_t)(cell.ssb_freq_hz - pointA_abs_freq_Hz) : 0;

  cell.ssb_offset = (uint32_t)(ssb_pointA_freq_offset_Hz / ISRRAN_SUBC_SPACING_NR(pdcch_scs)) % ISRRAN_NRE;
  ...
  ERROR_IF_NOT(isrran_coreset_start_rb(&coreset0) + isrran_coreset_get_bw(&coreset0) <= cell.phy_cell.carrier.nof_prb,
...
int set_derived_nr_cell_params(bool is_sa, rrc_cell_cfg_nr_t& cell)
{
  ...
  if (is_sa) {
    ...
    //    cell.phy_cell.pdcch.search_space[1].type       = isrran_search_space_type_common_3;
  }
...
int check_nr_pdcch_cfg_valid(const isrran_pdcch_cfg_nr_t& pdcch)
{
  // Verify Search Spaces
  std::array<bool, ISRRAN_UE_DL_NR_MAX_NOF_CORESET> used_coresets{};
  ...
  // Verify CORESET id
  for (uint32_t cs_id = 0; cs_id < ISRRAN_UE_DL_NR_MAX_NOF_CORESET; ++cs_id) {
