...
rrc_nr::rrc_nr(isrran::task_sched_handle task_sched_) :
  logger(isrlog::fetch_basic_logger("RRC-NR")), task_sched(task_sched_)
{}
                 ...
                 rrc_eutra_interface_rrc_nr* rrc_eutra_)
{
  ...
  for (uint32_t i = 0; i < cfg.cell_list.size(); ++i) {
    ...
    logger.info("Cell idx=%d, pci=%d, nr_dl_arfcn=%d, nr_ul_arfcn=%d, band=%d, duplex=%s, n_rb_dl=%d, ssb_arfcn=%d",
                ...
                cell.ul_arfcn,
                cell.band,
                cell.duplex_mode == ISRRAN_DUPLEX_MODE_FDD ? "FDD" : "TDD",
  ...
  for (uint32_t i = 0; i < cfg.cell_list.size(); ++i) {
    ...
    isrran_assert(ret == ISRRAN_SUCCESS, "Failed to configure NR cell %d", i);
  }
  ...
  isrran_assert(ret == ISRRAN_SUCCESS, "Failed to configure MasterCellGroup");
...
{
  isrenb::phy_interface_rrc_nr::common_cfg_t common_cfg = {};
  common_cfg.carrier                                    = cfg.cell_list[0].phy_cell.carrier;
  fill_phy_pdcch_cfg_common(du_cfg->cell(0), &common_cfg.pdcch);
  bool ret = isrran::fill_phy_pdcch_cfg(
      cell_ctxt->master_cell_group->sp_cell_cfg.sp_cell_cfg_ded.init_dl_bwp.pdcch_cfg.setup(), &common_cfg.pdcch);
  isrran_assert(ret, "Failed to generate Dedicated PDCCH config");
  isrran::make_phy_rach_cfg(du_cfg->cell(0).serv_cell_cfg_common().ul_cfg_common.init_ul_bwp.rach_cfg_common.setup(),
                            cfg.cell_list[0].duplex_mode,
                            &common_cfg.prach);
  common_cfg.duplex_mode = cfg.cell_list[0].duplex_mode;
  ret                    = isrran::fill_phy_ssb_cfg(
      cfg.cell_list[0].phy_cell.carrier, du_cfg->cell(0).serv_cell_cfg_common(), &common_cfg.ssb);
  isrran_assert(ret, "Failed to generate PHY config");
  if (phy->set_common_cfg(common_cfg) < ISRRAN_SUCCESS) {
...
void rrc_nr::config_mac()
{
  ...
  isrran::phy_cfg_nr_default_t::reference_cfg_t ref_args{};
  ref_args.duplex = cfg.cell_list[cc].duplex_mode == ISRRAN_DUPLEX_MODE_TDD
                        ? isrran::phy_cfg_nr_default_t::reference_cfg_t::R_DUPLEX_TDD_CUSTOM_6_4
                        : isrran::phy_cfg_nr_default_t::reference_cfg_t::R_DUPLEX_FDD;
  std::vector<sched_nr_cell_cfg_t> sched_cells_cfg(1, get_default_cell_cfg(isrran::phy_cfg_nr_default_t{ref_args}));
  ...
  bool valid_cfg = isrran::make_pdsch_cfg_from_serv_cell(cell_ctxt->master_cell_group->sp_cell_cfg.sp_cell_cfg_ded,
                                                         &cell.bwps[0].pdsch);
  isrran_assert(valid_cfg, "Invalid NR cell configuration.");
...
int32_t rrc_nr::generate_sibs()
{
  ...
  for (uint32_t msg_index = 0; msg_index < nof_messages + 1; msg_index++) {
    isrran::unique_byte_buffer_t sib_pdu = pack_into_pdu(msg[msg_index]);
    if (sib_pdu == nullptr) {
    ...
    log_rrc_message("BCCH", Tx, *cell_ctxt->sib_buffer.back(), msg[msg_index], isrran::to_c_str(strbuf));
  }
...
int rrc_nr::read_pdu_bcch_bch(const uint32_t tti, isrran::byte_buffer_t& buffer)
{
...
int rrc_nr::read_pdu_bcch_dlsch(uint32_t sib_index, isrran::byte_buffer_t& buffer)
{
...
void rrc_nr::get_metrics(isrenb::rrc_metrics_t& m)
{
  if (running) {
    for (auto& ue : users) {
...
void rrc_nr::handle_pdu(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu)
{
  switch (static_cast<isrran::nr_srb>(lcid)) {
    case isrran::nr_srb::srb0:
      handle_ul_ccch(rnti, pdu);
      break;
    case isrran::nr_srb::srb1:
    case isrran::nr_srb::srb2:
    case isrran::nr_srb::srb3:
...
void rrc_nr::handle_ul_ccch(uint16_t rnti, isrran::const_byte_span pdu)
{
  // Parse UL-CCCH
  ...
  log_rrc_message(isrran::to_c_str(fmtbuf), Rx, pdu, ul_ccch_msg, isrran::to_c_str(fmtbuf2));
...
void rrc_nr::handle_ul_dcch(uint16_t rnti, uint32_t lcid, isrran::const_byte_span pdu)
{
  // Parse UL-DCCH
  ...
  // Log Rx message
  fmt::memory_buffer fmtbuf, fmtbuf2;
  fmt::format_to(fmtbuf, "rnti=0x{:x}, {}", rnti, isrran::get_srb_name(isrran::nr_lcid_to_srb(lcid)));
  fmt::format_to(fmtbuf2, "UL-DCCH.{}", ul_dcch_msg.msg.c1().type().to_string());
  log_rrc_message(isrran::to_c_str(fmtbuf), Rx, pdu, ul_dcch_msg, isrran::to_c_str(fmtbuf2));
...
/*******************************************************************************
  PDCP interface
*******************************************************************************/
void rrc_nr::write_pdu(uint16_t rnti, uint32_t lcid, isrran::unique_byte_buffer_t pdu)
{
  if (pdu == nullptr or pdu->N_bytes == 0) {
    logger.error("Rx %s PDU, rnti=0x%x - Discarding. Cause: PDU is empty", isrenb::get_rb_name(lcid), rnti);
    return;
  }
...
int rrc_nr::start_security_mode_procedure(uint16_t rnti, isrran::unique_byte_buffer_t nas_pdu)
{
  auto user_it = users.find(rnti);
  if (user_it == users.end()) {
...
void rrc_nr::write_dl_info(uint16_t rnti, isrran::unique_byte_buffer_t sdu)
{
  if (not users.contains(rnti)) {
...
void rrc_nr::sgnb_release_request(uint16_t nr_rnti)
{
  ...
  uint16_t eutra_rnti = it != users.end() ? it->second->get_eutra_rnti() : ISRRAN_INVALID_RNTI;
  rem_user(nr_rnti);
  if (eutra_rnti != ISRRAN_INVALID_RNTI) {
