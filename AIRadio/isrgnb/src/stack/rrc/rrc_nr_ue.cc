...
#include "isrgnb/hdr/stack/rrc/rrc_nr_ue.h"
...
std::string rrc_nr::ue::to_string(const activity_timeout_type_t& type)
{
  ...
  return isrran::enum_to_text(options, (uint32_t)activity_timeout_type_t::nulltype, (uint32_t)type);
}

int rrc_nr::ue::send_dl_ccch(const dl_ccch_msg_s& dl_ccch_msg)
{
  // Allocate a new PDU buffer, pack the message and send to PDCP
  isrran::unique_byte_buffer_t pdu = parent->pack_into_pdu(dl_ccch_msg, __FUNCTION__);
  if (pdu == nullptr) {
  ...
  log_rrc_message(isrran::nr_srb::srb0, Tx, *pdu.get(), dl_ccch_msg, isrran::to_c_str(fmtbuf));
  parent->rlc->write_sdu(rnti, isrran::srb_to_lcid(isrran::nr_srb::srb0), std::move(pdu));
  return ISRRAN_SUCCESS;
}

int rrc_nr::ue::send_dl_dcch(isrran::nr_srb srb, const asn1::rrc_nr::dl_dcch_msg_s& dl_dcch_msg)
{
  // Allocate a new PDU buffer, pack the message and send to PDCP
  isrran::unique_byte_buffer_t pdu = parent->pack_into_pdu(dl_dcch_msg, __FUNCTION__);
  if (pdu == nullptr) {
    return ISRRAN_ERROR;
  }
  fmt::memory_buffer fmtbuf;
  fmt::format_to(fmtbuf, "DL-DCCH.{}", dl_dcch_msg.msg.c1().type().to_string());
  log_rrc_message(srb, Tx, *pdu.get(), dl_dcch_msg, isrran::to_c_str(fmtbuf));
  parent->pdcp->write_sdu(rnti, isrran::srb_to_lcid(srb), std::move(pdu));
...
int rrc_nr::ue::pack_sp_cell_cfg_ded_ul_cfg_init_ul_bwp_pucch_cfg(asn1::rrc_nr::cell_group_cfg_s& cell_group_cfg_pack)
{
  ...
  // DL data
  if (parent->cfg.cell_list[0].duplex_mode == ISRRAN_DUPLEX_MODE_FDD) {
  ...
  if (not isrran::make_phy_res_config(resource_small, pucch_cfg.setup().res_to_add_mod_list[0], 0)) {
    logger.warning("Failed to create 1-2 bit NR PUCCH resource");
  }
  if (not isrran::make_phy_res_config(resource_big, pucch_cfg.setup().res_to_add_mod_list[1], 1)) {
    logger.warning("Failed to create >2 bit NR PUCCH resource");
  }
  if (not isrran::make_phy_res_config(resource_sr, pucch_cfg.setup().res_to_add_mod_list[2], 2)) {
...
int rrc_nr::ue::pack_sp_cell_cfg_ded(asn1::rrc_nr::cell_group_cfg_s& cell_group_cfg_pack)
{
  ...
  // spCellConfig
  if (fill_sp_cell_cfg_from_enb_cfg(parent->cfg, UE_PSCELL_CC_IDX, cell_group_cfg_pack.sp_cell_cfg) != ISRRAN_SUCCESS) {
...
int rrc_nr::ue::pack_recfg_with_sync_sp_cell_cfg_common(asn1::rrc_nr::cell_group_cfg_s& cell_group_cfg_pack)
{
  ...
  if (pscell_cfg.duplex_mode == ISRRAN_DUPLEX_MODE_TDD) {
...
int rrc_nr::ue::pack_secondary_cell_group_cfg(asn1::dyn_octstring& packed_secondary_cell_config)
{
  ...
  if (cell_group_cfg_pack.pack(bref_pack) != asn1::ISRASN_SUCCESS) {
...
int rrc_nr::ue::pack_rrc_reconfiguration(asn1::dyn_octstring& packed_rrc_reconfig)
{
  ...
  if (pack_secondary_cell_group_cfg(recfg_ies.secondary_cell_group) == ISRRAN_ERROR) {
  ...
  if (reconfig.pack(bref_pack) != asn1::ISRASN_SUCCESS) {
...
int rrc_nr::ue::pack_nr_radio_bearer_config(asn1::dyn_octstring& packed_nr_bearer_config)
{
  ...
  if (radio_bearer_cfg_pack.pack(bref_pack) != asn1::ISRASN_SUCCESS) {
...
int rrc_nr::ue::handle_sgnb_addition_request(uint16_t eutra_rnti_, const sgnb_addition_req_params_t& req_params)
{
  ...
  if (add_drb(req_params.five_qi) != ISRRAN_SUCCESS) {
  ...
  if (pack_rrc_reconfiguration(ack_params.nr_secondary_cell_group_cfg_r15) == ISRRAN_ERROR) {
    parent->logger.error("Failed to pack RRC Reconfiguration. Sending SgNB addition reject.");
    parent->rrc_eutra->sgnb_addition_reject(eutra_rnti_);
    return ISRRAN_ERROR;
  }

  if (pack_nr_radio_bearer_config(ack_params.nr_radio_bearer_cfg1_r15) == ISRRAN_ERROR) {
...
void rrc_nr::ue::crnti_ce_received()
{
  ...
  if (endc) {
    ...
    isrran::make_phy_ssb_cfg(parent->cfg.cell_list[0].phy_cell.carrier,
                             cell_group_cfg.sp_cell_cfg.recfg_with_sync.sp_cell_cfg_common,
                             &uecfg.phy_cfg.ssb);
    isrran::make_duplex_cfg_from_serv_cell(cell_group_cfg.sp_cell_cfg.recfg_with_sync.sp_cell_cfg_common,
                                           &uecfg.phy_cfg.duplex);

    isrran_assert(check_nr_pdcch_cfg_valid(uecfg.phy_cfg.pdcch) == ISRRAN_SUCCESS, "Invalid PhyCell Config");
 ...
 * @return int ISRRAN_SUCCESS on success
 */
int rrc_nr::ue::add_drb(uint32_t five_qi)
{
  ...
  // add RLC bearer
  isrran::rlc_config_t rlc_cfg;
  /// NOTE, we need to pass the radio-bearer to the rlc_config
  if (isrran::make_rlc_config_t(cell_group_cfg.rlc_bearer_to_add_mod_list[0].rlc_cfg,
                                rlc_bearer.served_radio_bearer.drb_id(),
                                &rlc_cfg) != ISRRAN_SUCCESS) {
  ...
  // Add DRB1 to PDCP
  isrran::pdcp_config_t pdcp_cnfg = isrran::make_drb_pdcp_config_t(drb_item.drb_id, false, drb_item.pdcp_cfg);
...
void rrc_nr::ue::handle_rrc_reestablishment_request(const asn1::rrc_nr::rrc_reest_request_s& msg)
{
  ...
  if (old_ue_it == parent->users.end()) {
    ...
    isrran::console("Fallback to connection establishment for rnti=0x%x. Cause: no context available\n", rnti);
  ...
  isrran::console("User 0x%x requesting RRC Reestablishment as 0x%x. Cause: %s\n",
                  rnti,
                  old_rnti,
                  msg.rrc_reest_request.reest_cause.to_string());
...
void rrc_nr::ue::send_connection_reest(uint8_t ncc)
{
  ...
  if (send_dl_dcch(isrran::nr_srb::srb1, msg) != ISRRAN_SUCCESS) {
    // TODO: Handle
  }
}
...
void rrc_nr::ue::send_rrc_reject(uint8_t reject_wait_time_secs)
{
  ...
  if (send_dl_ccch(msg) != ISRRAN_SUCCESS) {
    // TODO: Handle
  }
...
void rrc_nr::ue::send_rrc_setup()
{
  ...
  isrran::unique_byte_buffer_t pdu = parent->pack_into_pdu(next_cell_group_cfg, __FUNCTION__);
  if (pdu == nullptr) {
    send_rrc_reject(max_wait_time_secs);
    return;
  }
...
void rrc_nr::ue::send_security_mode_command(isrran::unique_byte_buffer_t nas_pdu)
{
  // apply selected security config and enable integrity on SRB1 before generating security mode command
  update_as_security(srb_to_lcid(isrran::nr_srb::srb1), true, false);
  ...
  if (send_dl_dcch(isrran::nr_srb::srb1, dl_dcch_msg) != ISRRAN_SUCCESS) {
 ...
 * PDCP config and returns ISRRAN_ERROR. In some cases, however,
 ...
 * @return int ISRRAN_SUCCESS if a valid AS security config was found and the security was configured
 */
int rrc_nr::ue::update_as_security(uint32_t lcid, bool enable_integrity = true, bool enable_ciphering = true)
{
  ...
  isrran::nr_as_security_config_t tmp_cnfg  = sec_ctx.get_as_sec_cfg();
  isrran::as_security_config_t    pdcp_cnfg = {};
  ...
  pdcp_cnfg.integ_algo                      = (isrran::INTEGRITY_ALGORITHM_ID_ENUM)tmp_cnfg.integ_algo;
  pdcp_cnfg.cipher_algo                     = (isrran::CIPHERING_ALGORITHM_ID_ENUM)tmp_cnfg.cipher_algo;
...
void rrc_nr::ue::handle_security_mode_complete(const asn1::rrc_nr::security_mode_complete_s& msg)
{
  ...
  update_as_security(srb_to_lcid(isrran::nr_srb::srb1), false, true);
...
void rrc_nr::ue::send_rrc_reconfiguration()
{
  ...
  if (ies.radio_bearer_cfg_present) {
    ...
    if (fill_cellgroup_with_radio_bearer_cfg(
            parent->cfg, rnti, *parent->bearer_mapper, ies.radio_bearer_cfg, master_cell_group) != ISRRAN_SUCCESS) {
    ...
    // Pack masterCellGroup into container
    isrran::unique_byte_buffer_t pdu = parent->pack_into_pdu(master_cell_group, __FUNCTION__);
    if (pdu == nullptr) {
      parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
      return;
    }
...
int rrc_nr::ue::send_ue_capability_enquiry()
{
  ...
  if (request_filter.pack(bref_pack) != asn1::ISRASN_SUCCESS) {
  ...
  send_dl_dcch(isrran::nr_srb::srb1, dl_dcch_msg);
...
void rrc_nr::ue::handle_rrc_reestablishment_complete(const asn1::rrc_nr::rrc_reest_complete_s& msg)
{
  ...
  for (const auto& drb : next_radio_bearer_cfg.drb_to_add_mod_list) {
    ...
    parent->bearer_mapper->add_eps_bearer(rnti, lcid - 3, isrran::isrran_rat_t::nr, lcid);
...
void rrc_nr::ue::send_dl_information_transfer(isrran::unique_byte_buffer_t sdu)
{
  dl_dcch_msg_s dl_dcch_msg;
                                      ...
                                      uint32_t                five_qi)
{
  ...
  parent->bearer_mapper->add_eps_bearer(
      rnti, lcid - 3, isrran::isrran_rat_t::nr, lcid); // TODO: configurable bearer id <-> lcid mapping
                                    ...
                                    const asn1::rrc_nr::cell_group_cfg_s&   cell_group_diff)
{
  ...
  for (const srb_to_add_mod_s& srb : radio_bearer_diff.srb_to_add_mod_list) {
    isrran::pdcp_config_t   pdcp_cnfg  = isrran::make_nr_srb_pdcp_config_t(srb.srb_id, false);
  ...
  for (const drb_to_add_mod_s& drb : radio_bearer_diff.drb_to_add_mod_list) {
    isrran::pdcp_config_t   pdcp_cnfg  = isrran::make_drb_pdcp_config_t(drb.drb_id, false, drb.pdcp_cfg);
    const rlc_bearer_cfg_s* rlc_bearer = nullptr;
    for (const rlc_bearer_cfg_s& item : cell_group_diff.rlc_bearer_to_add_mod_list) {
  ...
  for (const rlc_bearer_cfg_s& rb : cell_group_diff.rlc_bearer_to_add_mod_list) {
    ...
    if (rb.served_radio_bearer.type().value == rlc_bearer_cfg_s::served_radio_bearer_c_::types_opts::srb_id) {
      ...
      if (not rb.rlc_cfg_present) {
        rlc_cfg = isrran::rlc_config_t::default_rlc_am_nr_config();
      } else {
        if (isrran::make_rlc_config_t(rb.rlc_cfg, rb_id, &rlc_cfg) != ISRRAN_SUCCESS) {
...
int rrc_nr::ue::update_mac(const cell_group_cfg_s& cell_group_config, bool is_config_complete)
{
  if (not is_config_complete) {
    ...
    if (cell_group_config.sp_cell_cfg_present and cell_group_config.sp_cell_cfg.sp_cell_cfg_ded_present and
      ...
      isrran::fill_phy_pucch_cfg(pucch_cfg, &uecfg.phy_cfg.pucch);
    }
  } else {
    ...
    for (auto& ss : pdcch.search_spaces_to_add_mod_list) {
      ...
      isrran::make_phy_search_space_cfg(ss, &uecfg.phy_cfg.pdcch.search_space[ss.search_space_id]);
    }
    for (auto& cs : pdcch.ctrl_res_set_to_add_mod_list) {
      uecfg.phy_cfg.pdcch.coreset_present[cs.ctrl_res_set_id] = true;
      isrran::make_phy_coreset_cfg(cs, &uecfg.phy_cfg.pdcch.coreset[cs.ctrl_res_set_id]);
    }
  }
  ...
  uecfg.mac_cell_group_cfg.reset(new mac_cell_group_cfg_s{cell_group_cfg.mac_cell_group_cfg});
  uecfg.phy_cell_group_cfg.reset(new phys_cell_group_cfg_s{cell_group_cfg.phys_cell_group_cfg});
  isrran::make_csi_cfg_from_serv_cell(cell_group_config.sp_cell_cfg.sp_cell_cfg_ded, &uecfg.phy_cfg.csi);
  parent->mac->ue_cfg(rnti, uecfg);
...
void rrc_nr::ue::deactivate_bearers()
{
  ...
  // No need to check the returned value, as the function ue_cfg will return ISRRAN_SUCCESS (it asserts if it fails)
...
template <class M>
void rrc_nr::ue::log_rrc_message(isrran::nr_srb          srb,
                                 ...
                                 const char*             msg_type)
{
  ...
  fmt::format_to(strbuf, "rnti=0x{:x}, {}", rnti, isrran::get_srb_name(srb));
  parent->log_rrc_message(isrran::to_c_str(strbuf), Tx, pdu, msg, msg_type);
}
