                                          ...
                                          uint16_t                rnti)
{
  ...
  TESTASSERT_EQ(isrran::srb_to_lcid(isrran::nr_srb::srb0), rlc.last_sdu_lcid);
  ...
  TESTASSERT_EQ(isrran::srb_to_lcid(isrran::nr_srb::srb1), srb1.srb_id);
  ...
  for (uint32_t ss_id = 0; ss_id < ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE; ++ss_id) {
    if (mac.last_ue_cfg.phy_cfg.pdcch.search_space_present[ss_id]) {
      const isrran_search_space_t& ss = mac.last_ue_cfg.phy_cfg.pdcch.search_space[ss_id];
      // Ensure MAC ue_cfg does not have yet any UE-specific SS
      TESTASSERT_NEQ(isrran_search_space_type_ue, ss.type);
      for (uint32_t f = 0; f < ss.nof_formats; ++f) {
        found_common_ul_dci_format |= ss.formats[f] == isrran_dci_format_nr_0_0;
      }
    }
  }
                               ...
                               uint16_t                rnti)
{
  ...
  isrran::unique_byte_buffer_t nsa_sdu;
  nsa_sdu = isrran::make_byte_buffer();
  ...
  TESTASSERT_EQ(isrran::srb_to_lcid(isrran::nr_srb::srb1), pdcp.last_sdu_lcid);
