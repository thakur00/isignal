...
#include "isrenb/test/rrc/test_helpers.h"
...
namespace isrenb { // namespace isrenb
...
void parse_args(ngap_args_t* ngap_args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("name",          bpo::value<std::string>(&ngap_args->gnb_name)->default_value("isrgnb01"),          "gnb Name")
  ...
  if (!isrran::string_to_mcc(gnb_mcc, &ngap_args->mcc)) {
    std::cout << "Error parsing mcc:" << gnb_mcc << " - must be a 3-digit string." << std::endl;
  }
  if (!isrran::string_to_mnc(gnb_mnc, &ngap_args->mnc)) {
...
void test_rrc_sa_ngap_integration(ngap_args_t ngap_args)
{
  ...
  // NGAP Setup
  auto& ngap_logger = isrlog::fetch_basic_logger("NGAP");
  ngap_logger.set_level(isrlog::basic_levels::debug);
  ngap_logger.set_hex_dump_max_size(-1);
  auto& gtpu_logger = isrlog::fetch_basic_logger("GTPU");
  gtpu_logger.set_level(isrlog::basic_levels::debug);
  gtpu_logger.set_hex_dump_max_size(-1);

  isrran::socket_manager rx_sockets;
  isrenb::ngap           ngap_obj(&task_sched, ngap_logger, &rx_sockets);
  isrenb::gtpu           gtpu_obj(&task_sched, gtpu_logger, &rx_sockets);
  ...
  TESTASSERT(gtpu_obj.init(gtpu_args, &pdcp_obj) == ISRRAN_SUCCESS);
  TESTASSERT(ngap_obj.init(ngap_args, &rrc_obj, &gtpu_obj) == ISRRAN_SUCCESS);
  ...
  isrran::unique_byte_buffer_t auth_resp_pdu;
  auth_resp_pdu = isrran::make_byte_buffer();
  ...
  isrran::unique_byte_buffer_t sec_complete_pdu;
  sec_complete_pdu = isrran::make_byte_buffer();
...
{
  auto& logger = isrlog::fetch_basic_logger("ASN1");
  logger.set_level(isrlog::basic_levels::info);
  auto& rrc_logger = isrlog::fetch_basic_logger("RRC-NR");
  rrc_logger.set_level(isrlog::basic_levels::debug);

  isrlog::init();
  ngap_args_t ngap_args = {};
  isrenb::parse_args(&ngap_args, argc, argv);
  isrenb::test_rrc_sa_ngap_integration(ngap_args);
