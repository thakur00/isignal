...
print_use(){
  echo "Please call script with isrRAN build path as first argument and number of PRBs as second (number of component carrier is optional)"
...
kill_lte(){
  ...
  if [ -f ./isrRAN.backtrace.crash ]; then
    echo "Rename backtrace"
    mv ./isrRAN.backtrace.crash isrRAN.backtrace.log
  fi
...
check_ue()
{
  ...
  last_line=$(cat ./${nof_prb}prb_screenlog_isrue.log | tail -n1)
  if [[ "$last_line" != *"---  exiting  ---"* ]]; then
    echo "Error. isrUE didn't seem to have exited properly."
    exit 1
  fi
...
{
  # check that app exited correctly
  last_line=$(cat ./${nof_prb}prb_screenlog_isrenb.log | tail -n1)
  if [[ "$last_line" != *"---  exiting  ---"* ]]; then
    echo "Error. isrENB didn't seem to have exited properly."
    exit 1
  fi

  # Check PRACH results
  num_prach=$(cat ./${nof_prb}prb_screenlog_isrenb.log | grep RACH: | wc -l)
  if [ "$num_prach" != "1" ] 2>/dev/null; then
    echo "Error. Detected $num_prach PRACH(s). But should be only 1."
    exit 1
  fi
...
{
  # check that app exited correctly
  last_line=$(cat ./${nof_prb}prb_screenlog_isrepc.log | tail -n1)
  if [[ "$last_line" != *"---  exiting  ---"* ]]; then
    echo "Error. isrEPC didn't seem to have exited properly."
    exit 1
  fi
...
echo "Using $num_cc component carrier(s) in isrENB"
...
if [ ! -x "$build_path/isrenb/src/isrenb" ]; then
  echo "Error! isrENB binary not found or not executable!"
  exit 1
fi

if [ ! -x "$build_path/isrepc/src/isrepc" ]; then
  echo "Error! isrEPC binary not found or not executable!"
  exit 1
fi

if [ ! -x "$build_path/isrue/src/isrue" ]; then
  echo "Error! isrUE binary not found or not executable!"
  exit 1
fi
...
epc_args="$build_path/../isrepc/epc.conf.example \
          --hss.db_file=$build_path/../isrepc/user_db.csv.example \
          --log.filename=./${nof_prb}prb_epc.log"
enb_args="$build_path/../isrenb/enb.conf.example \
          --enb_files.sib_config=$build_path/../isrenb/sib.conf.example \
          --enb_files.rb_config=$build_path/../isrenb/rb.conf.example \
          --rf.device_name=zmq \
          --log.all_level=info \
          --enb.n_prb=$nof_prb \
          --log.filename=./${nof_prb}prb_enb.log"

ue_args="$build_path/../isrue/ue.conf.example \
         --rf.device_name=zmq \
         --rf.rx_gain 60 \
...
if ([ "$num_cc" == "2" ])
then
  enb_args="$enb_args --enb_files.rr_config=$build_path/../isrenb/rr_2ca.conf.example \
  ...
  enb_args="$enb_args --enb_files.rr_config=$build_path/../isrenb/rr.conf.example \
...
# Run isrEPC
echo "Starting isrEPC"
screen -S isrepc -dm -L -Logfile ./${nof_prb}prb_screenlog_isrepc.log $build_path/isrepc/src/isrepc $epc_args
sleep 2
epc_pid=$(pgrep isrepc)
if [ -z "$epc_pid" ]
then
...
echo "isrEPC's PID is $epc_pid"

# Run isrENB
echo "Starting isrENB"
screen -S isrenb -dm -L -Logfile ./${nof_prb}prb_screenlog_isrenb.log $build_path/isrenb/src/isrenb $enb_args
sleep 2
enb_pid=$(pgrep isrenb)
if [ -z "$enb_pid" ]
then
...
echo "isrENB's PID is $enb_pid"

# Run isrUE
echo "Starting isrUE"
screen -S isrue -dm -L -Logfile ${nof_prb}prb_screenlog_isrue.log $build_path/isrue/src/isrue $ue_args
sleep 2
ue_pid=$(pgrep isrue)
if [ -z "$ue_pid" ]
then
...
screen -S isrue -X stuff "t$(printf \\r)"
screen -S isrenb -X stuff "t$(printf \\r)"
