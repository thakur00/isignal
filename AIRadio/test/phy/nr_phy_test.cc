...
;
  ...
  phy_cfg = isrran::phy_cfg_nr_default_t(isrran::phy_cfg_nr_default_t::reference_cfg_t(reference_cfg_str));

  // Calculate the DL signal power from the number of PRBs
  dl_channel.awgn_signal_power_dBfs = isrran_gnb_dl_get_maximum_signal_power_dBfs(phy_cfg.carrier.nof_prb);

  // Reverses the Doppler shift for the UL
  ul_channel.hst_init_time_s = 0.5 * dl_channel.hst_period_s;

  // Calculate sampling rate in Hz
  srate_hz = (double)(isrran_min_symbol_sz_rb(phy_cfg.carrier.nof_prb) * ISRRAN_SUBC_SPACING_NR(phy_cfg.carrier.scs));
...
int main(int argc, char** argv)
{
  ...
  isrue::rrc_interface_phy_nr::cell_select_result_t cs_res =
      tb.run_cell_select(args.phy_cfg.carrier, args.phy_cfg.get_ssb_cfg());
  isrran_assert(cs_res.status == isrue::rrc_interface_phy_nr::cell_select_result_t::SUCCESSFUL,
  ...
  if (metrics.ue_stack.prach.size() > 0) {
    isrran::console("PRACH:\n");
    isrran::console(
        "   +------------+------------+------------+------------+------------+------------+------------+\n");
    isrran::console("   | %10s | %10s | %10s | %10s | %10s | %10s | %10s |\n",
                    "Preamble",
                    "Transmit'd",
                    "Received",
                    "Detection",
    ...
    for (const auto& p : metrics.ue_stack.prach) {
      ...
      if (metrics.gnb_stack.prach.count(p.first) > 0) {
        ...
        prach_ta  = ISRRAN_VEC_SAFE_CMA(gnb_prach.avg_ta, prach_ta, prach_rx_count);
      } else {
      ...
      isrran::console("   | %10d | %10d | %10d | %10.3f | %10.1f | %10.1f | %10.1f |\n",
                      p.first,
                      p.second.count,
                      gnb_prach.count,
                      detection,
  ...
  if (metrics.gnb_stack.pucch.count > 0) {
    isrran::console("PUCCH DMRS Receiver metrics:\n");
    isrran::console("   +------------+------------+------------+------------+\n");
    isrran::console("   | %10s | %10s | %10s | %10s |\n", "Measure", "Average", "Min", "Max");
    isrran::console("   +------------+------------+------------+------------+\n");
    isrran::console("   | %10s | %+10.2f | %+10.2f | %+10.2f |\n",
                    "EPRE (dB)",
  ...
  if (metrics.gnb_stack.mac.tx_pkts > 0) {
    ...
    isrran::console("PDSCH:\n");
    isrran::console("       Count: %d\n", metrics.gnb_stack.mac.tx_pkts);
    isrran::console("        BLER: %f\n", pdsch_bler);
    isrran::console("  Sched Rate: %f Mbps\n", pdsch_shed_rate);
    isrran::console("    Net Rate: %f Mbps\n", (1.0f - pdsch_bler) * pdsch_shed_rate);
    isrran::console("   Retx Rate: %f Mbps\n", pdsch_bler * pdsch_shed_rate);
    isrran::console("\n");
  }
  ...
  if (metrics.gnb_stack.pusch.count > 0) {
    isrran::console("PUSCH DMRS Receiver metrics:\n");
    isrran::console("   +------------+------------+------------+------------+\n");
    isrran::console("   | %10s | %10s | %10s | %10s |\n", "Measure", "Average", "Min", "Max");
    isrran::console("   +------------+------------+------------+------------+\n");
    isrran::console("   | %10s | %+10.2f | %+10.2f | %+10.2f |\n",
  ...
  if (metrics.ue_stack.sr_count > 0) {
    ...
    isrran::console("   | %10s | %10d | %10d | %10.5f | %10s |\n",
                    "SR",
  ...
  if (metrics.gnb_stack.cqi_count > 0) {
    ...
    isrran::console("   | %10s | %10d | %10d | %10.5f | %10.5f |\n",
                    "CQI",
  ...
  // Assert metrics
  isrran_assert(metrics.gnb_stack.mac.tx_pkts == 0 or pdsch_bler <= assert_pdsch_bler_max,
                "PDSCH BLER (%f) exceeds the assertion maximum (%f)",
                pdsch_bler,
                assert_pusch_bler_max);
  isrran_assert(metrics.gnb_stack.mac.rx_pkts == 0 or pusch_bler <= assert_pusch_bler_max,
                "PUSCH BLER (%f) exceeds the assertion maximum (%f)",
                pusch_bler,
                assert_pusch_bler_max);
  isrran_assert(metrics.ue_stack.sr_count == 0 or sr_detection >= assert_sr_detection_min,
                "SR detection probability (%f) did not reach the assertion minimum (%f)",
                sr_detection,
                assert_sr_detection_min);
  isrran_assert(metrics.gnb_stack.cqi_count == 0 or cqi_detection >= assert_cqi_detection_min,
                "CQI report detection probability (%f) did not reach the assertion minimum (%f)",
                cqi_detection,
                assert_sr_detection_min);
  isrran_assert(prach_tx_count == 0 or prach_detection >= assert_prach_detection_min,
                "PRACH detection probability (%f) did not reach the assertion minimum (%f)",
                prach_detection,
                assert_prach_detection_min);
  isrran_assert(prach_tx_count == 0 or (prach_ta >= assert_prach_ta_min and prach_ta <= assert_prach_ta_max),
                "PRACH TA average measurement %f is higher than minimum (%d) or above maximum (%f)",
                prach_ta,
                assert_prach_ta_min,
                assert_prach_ta_max);
  isrran_assert(metrics.gnb_stack.pucch.count == 0 or (metrics.gnb_stack.pucch.snr_db_min >= assert_pucch_snr_min),
