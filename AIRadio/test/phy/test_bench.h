...
#ifndef ISRRAN_TEST_BENCH_H
#define ISRRAN_TEST_BENCH_H

#include "dummy_phy_common.h"
#include "isrenb/hdr/phy/nr/worker_pool.h"
#include "isrran/radio/radio_dummy.h"
...
private:
  ...
  isrenb::nr::worker_pool gnb_phy;
  phy_common              gnb_phy_com;
  ue_dummy_stack          ue_stack;
  isrue::phy_nr_sa        ue_phy;
  isrran::radio_dummy     ue_radio;
  isrran::rf_timestamp_t  gnb_rx_time = {};

  bool                initialised = false;
  uint32_t            sf_sz       = 0;
  isrran::rf_buffer_t rf_buffer;
  // Channel simulator
  isrran::channel dl_channel;
  isrran::channel ul_channel;

public:
  struct args_t {
    double                          srate_hz     = (double)(768 * ISRRAN_SUBC_SPACING_NR(0));
    uint32_t                        nof_channels = 1;
    uint32_t                        buffer_sz_ms = 10;
    bool                            valid        = false;
    isrran::phy_cfg_nr_t            phy_cfg      = {};
    isrenb::phy_cell_cfg_list_nr_t  cell_list    = {};
    isrenb::nr::worker_pool::args_t gnb_phy;
    gnb_dummy_stack::args_t         gnb_stack;
    isrue::phy_args_nr_t            ue_phy;
    ...
    isrran::channel::args_t dl_channel;
    isrran::channel::args_t ul_channel;
  ...
  struct metrics_t {
    ...
    isrue::phy_metrics_t       ue_phy    = {};
  };

  test_bench(const args_t& args) :
    gnb_stack(args.gnb_stack),
    gnb_phy(gnb_phy_com, gnb_stack, isrlog::get_default_sink(), args.gnb_phy.nof_phy_threads),
    ue_stack(args.ue_stack, ue_phy),
    ue_phy("PHY"),
    ue_radio(),
    gnb_phy_com(phy_common::args_t(args.srate_hz, args.buffer_sz_ms, args.nof_channels),
                isrlog::fetch_basic_logger(GNB_PHY_COM_LOG_NAME, isrlog::get_default_sink(), false)),
    sf_sz((uint32_t)std::round(args.srate_hz * 1e-3)),
    duration_slots(args.durations_slots),
    dl_channel(args.dl_channel, 1, isrlog::fetch_basic_logger(CHANNEL_LOG_NAME, isrlog::get_default_sink(), false)),
    ul_channel(args.ul_channel, 1, isrlog::fetch_basic_logger(CHANNEL_LOG_NAME, isrlog::get_default_sink(), false)),
    rf_buffer(1)
  {
    isrlog::fetch_basic_logger(UE_PHY_COM_LOG_NAME).set_level(isrlog::str_to_basic_level(args.gnb_phy_com_log_level));
    isrlog::fetch_basic_logger(GNB_PHY_COM_LOG_NAME).set_level(isrlog::str_to_basic_level(args.gnb_phy_com_log_level));
    isrlog::fetch_basic_logger(CHANNEL_LOG_NAME).set_level(isrlog::basic_levels::error);
    ...
    if (gnb_phy.set_common_cfg(common_cfg) < ISRRAN_SUCCESS) {
      return;
    }

    // Initialise radio
    isrran::rf_args_t rf_args = {};
    rf_args.nof_antennas      = 1;
    rf_args.nof_carriers      = 1;
    rf_args.srate_hz          = args.srate_hz;
    rf_args.log_level         = args.ue_radio_log_level;
    if (ue_radio.init(rf_args, &ue_phy) != ISRRAN_SUCCESS) {
      return;
    }

    // Initialise UE PHY
    if (ue_phy.init(args.ue_phy, &ue_stack, &ue_radio) != ISRRAN_SUCCESS) {
      return;
    }
    ...
    if (args.phy_lib_log_level == "info") {
      ...
      set_isrran_verbose_level(ISRRAN_VERBOSE_NONE);
    }
  ...
  isrue::rrc_interface_phy_nr::cell_select_result_t run_cell_select(const isrran_carrier_nr_t& carrier,
                                                                    const isrran_ssb_cfg_t&    ssb_cfg)
  {
    // Prepare return value
    isrue::rrc_interface_phy_nr::cell_select_result_t ret = {};

    // Prepare cell selection arguments
    isrue::phy_interface_rrc_nr::cell_select_args_t cs_args = {};
  ...
  {
    // Get gNb worker
    isrenb::nr::slot_worker* gnb_worker = gnb_phy.wait_worker(slot_idx);
    ...
    // Set gNb TX time
    isrran::rf_timestamp_t gnb_time = gnb_rx_time;
    ...
    // Set gNb context
    isrran::phy_common_interface::worker_context_t gnb_context;
    ...
    slot_idx = (slot_idx + 1) % (1024 * ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz));
  ...
  metrics_t get_metrics()
  {
    ...
    ue_phy.get_metrics(isrran::isrran_rat_t::nr, &metrics.ue_phy); // get the metrics from the ue_phy
    return metrics;
  }
};

#endif // ISRRAN_TEST_BENCH_H
