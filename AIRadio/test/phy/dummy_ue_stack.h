...
#ifndef ISRRAN_DUMMY_UE_STACK_H
#define ISRRAN_DUMMY_UE_STACK_H
...
class ue_dummy_stack : public isrue::stack_interface_phy_nr
{
public:
  ...
  struct cell_search_metrics_t {
    ...
    isrue::stack_interface_phy_nr::cell_search_result_t last_result;
...
private:
  isrlog::basic_logger&          logger = isrlog::fetch_basic_logger("UE-STCK");
  std::mutex                     rnti_mutex;
  isrran_random_t                random_gen     = isrran_random_init(0x1323);
  isrran_rnti_type_t             dl_rnti_type   = isrran_rnti_type_c;
  uint16_t                       rnti           = 0;
  bool                           valid          = false;
  ...
  metrics_t                      metrics        = {};
  isrue::phy_interface_stack_nr& phy;
...
public:
  ...
  ue_dummy_stack(const args_t& args, isrue::phy_interface_stack_nr& phy_) :
  ...
  ~ue_dummy_stack() { isrran_random_free(random_gen); }
  ...
  void run_tti(const uint32_t tti, const uint32_t tti_jump) override
  {
    ...
    // Run PRACH
    if (prach_period != 0) {
      uint32_t slot_idx = tti % ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz);
      uint32_t sfn      = tti / ISRRAN_NSLOTS_PER_FRAME_NR(isrran_subcarrier_spacing_15kHz);
      if (not prach_pending and slot_idx == 0 and sfn % prach_period == 0) {
        prach_preamble = isrran_random_uniform_int_dist(random_gen, 0, 63);
  ...
  {
    std::unique_lock<std::mutex> lock(rnti_mutex);
    return {rnti, isrran_rnti_type_c};
  }
  ...
  {
    std::unique_lock<std::mutex> lock(rnti_mutex);
    dl_rnti_type = isrran_rnti_type_ra;
  ...
  void cell_search_found_cell(const cell_search_result_t& result) override
  {
    ...
    std::array<uint8_t, ISRRAN_PBCH_MSG_NR_SZ> bit_pack_pbch_msg = {};
    asn1::cbit_ref                             cbit(bit_pack_pbch_msg.data(), bit_pack_pbch_msg.size());
    isrran_bit_pack_vector((uint8_t*)result.pbch_msg.payload, bit_pack_pbch_msg.data(), ISRRAN_PBCH_MSG_NR_SZ);
    ...
    isrran_mib_nr_t mib_c = {};
    isrran_pbch_msg_nr_mib_unpack(&result.pbch_msg, &mib_c);

    // Convert MIB from C lib to info
    std::array<char, 512> mib_info = {};
    isrran_pbch_msg_nr_mib_info(&mib_c, mib_info.data(), (uint32_t)mib_info.size());

    // Convert CSI to string
    std::array<char, 512> csi_info = {};
    isrran_csi_meas_info_short(&result.measurements, csi_info.data(), (uint32_t)csi_info.size());
    ...
    m.epre_db_min            = ISRRAN_MIN(m.epre_db_min, result.measurements.epre_dB);
    m.epre_db_max            = ISRRAN_MAX(m.epre_db_max, result.measurements.epre_dB);
    m.epre_db_avg            = ISRRAN_VEC_SAFE_CMA(result.measurements.epre_dB, m.epre_db_avg, m.count);
    m.rsrp_db_min            = ISRRAN_MIN(m.rsrp_db_min, result.measurements.rsrp_dB);
    m.rsrp_db_max            = ISRRAN_MAX(m.rsrp_db_max, result.measurements.rsrp_dB);
    m.rsrp_db_avg            = ISRRAN_VEC_SAFE_CMA(result.measurements.rsrp_dB, m.rsrp_db_avg, m.count);
    m.snr_db_min             = ISRRAN_MIN(m.snr_db_min, result.measurements.snr_dB);
    m.snr_db_max             = ISRRAN_MAX(m.snr_db_max, result.measurements.snr_dB);
    m.snr_db_avg             = ISRRAN_VEC_SAFE_CMA(result.measurements.snr_dB, m.snr_db_avg, m.count);
    m.cfo_hz_min             = ISRRAN_MIN(m.cfo_hz_min, result.measurements.cfo_hz);
    m.cfo_hz_max             = ISRRAN_MAX(m.cfo_hz_max, result.measurements.cfo_hz);
    m.cfo_hz_avg             = ISRRAN_VEC_SAFE_CMA(result.measurements.cfo_hz, m.cfo_hz_avg, m.count);
    m.count++;
