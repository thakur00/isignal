...
void test_cached_deque_basic_operations()
{
  ...
  isrran::deque<int> my_deque2(my_deque);
...
{
  isrran::queue<C> my_queue;
  TESTASSERT(my_queue.empty());
  my_queue.push(C{});
  TESTASSERT(my_queue.size() == 1);

  isrran::queue<C> my_queue2(std::move(my_queue));
