...
int test_span_conversion()
{
...

  {
    ...
    isrran::span<int> v = values, v2 = values2;
    TESTASSERT(v == v2);
  ...
  {
    // TEST: const context
    const isrran::span<int> v = values, v2 = values2;
    TESTASSERT(v != v2);
    TESTASSERT(v[0] == 3);
    TESTASSERT(v2[0] == 2);
...

  {
    ...
    isrran::span<int> v = values, v2 = carray;
    TESTASSERT(v == v2);
...
{
  auto                  foo  = [](isrran::byte_span buffer) { return buffer.size() == 5 and buffer[4] == 4; };
  auto                  cfoo = [](const isrran::byte_span buffer) { return buffer.size() == 5 and buffer[4] == 4; };
  ...
  const isrran::byte_buffer_t& pdu2 = pdu;
  {
    const auto v = isrran::make_span(pdu2);
    TESTASSERT(v.size() == 5);
    TESTASSERT(v[0] == 0);
    TESTASSERT(v[2] == 2);
    TESTASSERT(v[4] == 4);
  ...
  TESTASSERT(foo(isrran::make_span(pdu)));
  TESTASSERT(cfoo(isrran::make_span(pdu)));
...
int main()
{
  TESTASSERT(test_span_access() == ISRRAN_SUCCESS);
  TESTASSERT(test_span_conversion() == ISRRAN_SUCCESS);
  TESTASSERT(test_byte_buffer_conversion() == ISRRAN_SUCCESS);
  printf("Success\n");
  return ISRRAN_SUCCESS;
}
