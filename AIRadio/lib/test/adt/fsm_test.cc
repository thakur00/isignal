...
void call_log_helper(State* state, isrlog::basic_logger& logger, const char* type)
{
  std::string callname = isrran::get_type_name<State>() + "::" + type;
  logger.info("%s custom called", callname.c_str());
  calls.push_back(callname);
}

class fsm1 : public isrran::fsm_t<fsm1>
{
public:
  ...
  explicit fsm1(isrlog::basic_logger& logger) : isrran::fsm_t<fsm1>(logger) {}
...
int test_hsm()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("HSM", false);
  ...
  TESTASSERT(calls[0] == isrran::get_type_name<fsm1::idle_st>() + "::enter"); // enter for init state called
  TESTASSERT(calls[1] == isrran::get_type_name<fsm1>() + "::action1");
  TESTASSERT(calls[2] == isrran::get_type_name<fsm1::state1>() + "::enter");
  TESTASSERT(calls[3] == isrran::get_type_name<fsm1>() + "::action2");
  TESTASSERT(calls[4] == isrran::get_type_name<fsm1::state1>() + "::exit");
  TESTASSERT(calls[5] == isrran::get_type_name<fsm1::fsm2>() + "::enter"); // entry is recursive
  TESTASSERT(calls[6] == isrran::get_type_name<fsm1::fsm2::state_inner>() + "::enter");
  TESTASSERT(calls[7] == isrran::get_type_name<fsm1::fsm2>() + "::inner_action1");
  TESTASSERT(calls[8] == isrran::get_type_name<fsm1::fsm2>() + "::inner_action2");
  TESTASSERT(calls[9] == isrran::get_type_name<fsm1::fsm2::state_inner2>() + "::enter");
  TESTASSERT(calls[10] == isrran::get_type_name<fsm1::fsm2::state_inner2>() + "::exit");
  TESTASSERT(calls[11] == isrran::get_type_name<fsm1::fsm2>() + "::exit");     // exit is recursive
  TESTASSERT(calls[12] == isrran::get_type_name<fsm1::state1>() + "::enter2"); // differentiates different entry funcs
...
struct proc1 : public isrran::proc_fsm_t<proc1, int> {
public:
  struct procstate1 {
    void enter(proc1* f, const isrran::proc_launch_ev<int>& ev);
  };

  explicit proc1(isrlog::basic_logger& logger) : base_t(logger) {}
...
struct proc_listener_fsm : public isrran::fsm_t<proc_listener_fsm> {
public:
  struct st1 {};
  struct st2 {};
  using proc1_st = isrran::proc_wait_st<proc1>;

  proc_listener_fsm(isrlog::basic_logger& logger, proc1* proc_ptr_) :
...
int test_fsm_proc()
{
  proc1 proc{isrlog::fetch_basic_logger("PROC", false)};
  proc.get_logger().set_level(isrlog::basic_levels::info);
  proc.set_fsm_event_log_level(isrlog::basic_levels::info);

  int v = 2;
  TESTASSERT(proc.current_state_name() == "idle_st");
  proc.trigger(isrran::proc_launch_ev<int>{v});
  TESTASSERT(proc.current_state_name() == "procstate1");
  proc.trigger(isrran::proc_launch_ev<int>{v});
  TESTASSERT(proc.current_state_name() == "procstate1");
  proc.trigger(isrran::proc_launch_ev<int>{5});
  TESTASSERT(proc.current_state_name() == "procstate1");
  ...
  {
    proc_listener_fsm outer_fsm{isrlog::fetch_basic_logger("TEST"), &proc};
...
class nas_fsm : public isrran::fsm_t<nas_fsm>
{
public:
  // states
  struct emm_null_st {};
  ...
  explicit nas_fsm(isrlog::basic_logger& logger) : fsm_t<nas_fsm>(logger) {}
...
int test_nas_fsm()
{
  auto& logger = isrlog::fetch_basic_logger("NAS", false);
  logger.set_level(isrlog::basic_levels::info);
...
struct fsm3 : public isrran::fsm_t<fsm3> {
  struct st1 {};
  struct st2 {
    int  counter = 0;
    void enter(fsm3* fsm)
  ...
  fsm3() : base_t(isrlog::fetch_basic_logger("TEST")) {}
...
int main()
{
  isrlog::basic_logger& logger = isrlog::fetch_basic_logger("TEST", false);
  logger.set_level(isrlog::basic_levels::info);
  isrlog::init();

  TESTASSERT(test_hsm() == ISRRAN_SUCCESS);
  logger.info("TEST \"hsm\" finished successfully\n");
  TESTASSERT(test_fsm_proc() == ISRRAN_SUCCESS);
  logger.info("TEST \"proc\" finished successfully\n");
  TESTASSERT(test_nas_fsm() == ISRRAN_SUCCESS);
  logger.info("TEST \"nas fsm\" finished successfully\n");
  TESTASSERT(test_fsm_self_trigger() == ISRRAN_SUCCESS);
