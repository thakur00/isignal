...
#include "isrran/adt/pool/fixed_size_pool.h"
#include "isrran/adt/pool/mem_pool.h"
...
struct BigObj {
  ...
  using pool_t = isrran::concurrent_fixed_memory_pool<512, true>;

  void* operator new(size_t sz)
  {
    isrran_assert(sz == sizeof(BigObj), "Allocated node size and object size do not match");
...
void test_fixedsize_pool()
{
  ...
  // TEST: one thread allocates, and the other deallocates
  {
    ...
    isrran::dyn_blocking_queue<std::unique_ptr<BigObj> > queue(pool_size / 2);
...
void test_background_pool()
{
  ...
  C::dtor_counter         = 0;
  {
    ...
    isrran::background_obj_pool<D> obj_pool(16, 4, 16, init_D_val);
    TESTASSERT(obj_pool.cache_size() == 16);
    std::vector<isrran::unique_pool_ptr<D> > objs;

    for (size_t i = 0; i < 16 - 4; ++i) {
      objs.push_back(obj_pool.make());
    }
    TESTASSERT(
        std::all_of(objs.begin(), objs.end(), [](const isrran::unique_pool_ptr<D>& d) { return d->val == 'c'; }));
