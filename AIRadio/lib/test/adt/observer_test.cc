...
static_assert(isrran::is_observer<isrran::observer<int> >::value, "is_observer<> meta-function failed");
static_assert(not isrran::is_observer<std::function<void(int)> >::value, "is_observer<> meta-function failed");
...
int observable_test()
{
  ...
  {
    M                     val;
    isrran::observable<M> subject;
    TESTASSERT(subject.nof_observers() == 0);

    isrran::observer_id id1 = subject.subscribe([&val](M v) { val = std::move(v); });

    lval_observer_tester observer{}, observer2{};
    isrran::observer_id  id2 = subject.subscribe(observer);
    isrran::observer_id  id3 = subject.subscribe(observer2, &lval_observer_tester::foo);
  ...
  {
    M                            val;
    isrran::observable<const M&> subject;
    TESTASSERT(subject.nof_observers() == 0);
  ...
  // Test r-value arguments
  {
    ...
    isrran::observer_id  id2 = subject.subscribe(observer);
    isrran::observer_id  id3 = subject.subscribe(observer2, &rref_observer_tester::foo);
...
{
  isrran::event_dispatcher<M> signaller;
...
{
  isrran::event_queue<M> signaller;
...
{
  {
    ...
    {
      isrran::unique_observer_id<M> obs{signaller, observer};
...
int main()
{
  TESTASSERT(observable_test() == ISRRAN_SUCCESS);
  TESTASSERT(event_dispatcher_test() == ISRRAN_SUCCESS);
  TESTASSERT(event_queue_test() == ISRRAN_SUCCESS);
  TESTASSERT(unique_subscribe_test() == ISRRAN_SUCCESS);
