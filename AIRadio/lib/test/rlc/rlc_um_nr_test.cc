...
public:
  rlc_um_nr_test_context1() :
    logger1(isrlog::fetch_basic_logger("RLC_UM_1", false)),
    logger2(isrlog::fetch_basic_logger("RLC_UM_2", false)),
    timers(16),
    rlc1(logger1, 3, &tester, &tester, &timers),
    rlc2(logger2, 3, &tester, &tester, &timers)
  {
    // setup logging
    logger1.set_level(isrlog::basic_levels::debug);
    logger1.set_hex_dump_max_size(-1);
    logger2.set_level(isrlog::basic_levels::debug);
  ...
  isrlog::basic_logger& logger1;
  isrlog::basic_logger& logger2;
...
int rlc_um_nr_test2(bool reverse_rx = false)
{
  ...
  while (ctxt.rlc1.get_buffer_state() != 0 && num_pdus < max_num_pdus) {
    pdu_bufs[num_pdus]          = isrran::make_byte_buffer();
