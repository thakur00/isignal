...
void mac_dummy::run_tti(isrue::rlc_interface_mac* tx_rlc, isrue::rlc_interface_mac* rx_rlc, bool is_dl)
{
...
void mac_dummy::run_tx_tti(isrue::rlc_interface_mac*                  tx_rlc,
...
void mac_dummy::run_rx_tti(isrue::rlc_interface_mac*                  tx_rlc,
                           isrue::rlc_interface_mac*                  rx_rlc,
                           bool                                       is_dl,
                           std::vector<isrran::unique_byte_buffer_t>& pdu_list)
{
  ...
  while (it != pdu_list.end()) {
    ...
    isrran::unique_byte_buffer_t& pdu = *it;
 ...
 ***********************/
// PDCP interface
void rlc_tester::write_pdu(uint32_t rx_lcid, isrran::unique_byte_buffer_t sdu)
{
  assert(rx_lcid == lcid);
  if (args.mode != "AM") {
...
{
  auto log_sink =
      (args.log_filename == "stdout") ? isrlog::create_stdout_sink() : isrlog::create_file_sink(args.log_filename);
  if (!log_sink) {
    return;
  }
  ...
  auto& log1 = isrlog::fetch_basic_logger("RLC_1", false);
  log1.set_level(static_cast<isrlog::basic_levels>(args.log_level));
  log1.set_hex_dump_max_size(args.log_hex_limit);
  auto& log2 = isrlog::fetch_basic_logger("RLC_2", false);
  log2.set_level(static_cast<isrlog::basic_levels>(args.log_level));
  log2.set_hex_dump_max_size(args.log_hex_limit);

  isrran::rlc_pcap pcap;
  uint32_t         lcid = 1;

  isrran::rlc_config_t cnfg_ = {};
  if (args.rat == "LTE") {
    if (args.mode == "AM") {
      // config RLC AM bearer
      cnfg_                    = isrran::rlc_config_t::default_rlc_am_config();
      cnfg_.am.max_retx_thresh = args.max_retx;
    } else if (args.mode == "UM") {
      // config UM bearer
      cnfg_ = isrran::rlc_config_t::default_rlc_um_config();
    ...
    if (args.mode == "UM6") {
      cnfg_ = isrran::rlc_config_t::default_rlc_um_nr_config(6);
    } else if (args.mode == "UM12") {
      cnfg_ = isrran::rlc_config_t::default_rlc_um_nr_config(12);
    } else if (args.mode == "AM12") {
      cnfg_                       = isrran::rlc_config_t::default_rlc_am_nr_config(12);
      cnfg_.am_nr.max_retx_thresh = args.max_retx;
    } else if (args.mode == "AM18") {
      cnfg_                       = isrran::rlc_config_t::default_rlc_am_nr_config(18);
      cnfg_.am_nr.max_retx_thresh = args.max_retx;
    } else {
      std::cout << "Unsupported RLC mode " << args.mode << ", exiting." << std::endl;
      exit(-1);
  ...
  isrran::rlc rlc1(log1.id().c_str());
  isrran::rlc rlc2(log2.id().c_str());
