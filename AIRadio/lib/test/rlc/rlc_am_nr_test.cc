...
int basic_test_tx(rlc_am* rlc, byte_buffer_t pdu_bufs[NBUFS], rlc_am_nr_sn_size_t sn_size)
{
  ...
  for (int i = 0; i < NBUFS; i++) {
    sdu_bufs[i]             = isrran::make_byte_buffer();
...
int window_checker_test(rlc_am_nr_sn_size_t sn_size)
{
  ...
  rlc_am              rlc1(isrran_rat_t::nr, isrlog::fetch_basic_logger("RLC_AM_1"), 1, &tester, &tester, &timers);
...
int retx_segmentation_required_checker_test(rlc_am_nr_sn_size_t sn_size)
{
  ...
  for (int i = 0; i < NBUFS; i++) {
    sdu_bufs[i]             = isrran::make_byte_buffer();
    pdu_bufs[i]             = isrran::make_byte_buffer();
    sdu_bufs[i]->msg[0]     = i; // Write the index into the buffer
...
int basic_test(rlc_am_nr_sn_size_t sn_size)
{
  ...
  rlc_am              rlc1(isrran_rat_t::nr, isrlog::fetch_basic_logger("RLC_AM_1"), 1, &tester, &tester, &timers);
  rlc_am              rlc2(isrran_rat_t::nr, isrlog::fetch_basic_logger("RLC_AM_2"), 1, &tester, &tester, &timers);
...
int handle_status_of_non_tx_last_segment(rlc_am_nr_sn_size_t sn_size)
{
  ...
  {
    unique_byte_buffer_t status_buf = isrran::make_byte_buffer();
...
int max_retx_lost_sdu_test(rlc_am_nr_sn_size_t sn_size)
{
  ...
  auto&  test_logger = isrlog::fetch_basic_logger("TESTER  ");
  rlc_am rlc1(isrran_rat_t::nr, isrlog::fetch_basic_logger("RLC_AM_1"), 1, &tester, &tester, &timers);
  isrlog::fetch_basic_logger("RLC_AM_1").set_hex_dump_max_size(100);
  isrlog::fetch_basic_logger("RLC").set_hex_dump_max_size(100);
...
int discard_test(rlc_am_nr_sn_size_t sn_size)
{
  ...
  isrlog::fetch_basic_logger("RLC_AM_1").set_hex_dump_max_size(100);
  isrlog::fetch_basic_logger("RLC_AM_2").set_hex_dump_max_size(100);
  isrlog::fetch_basic_logger("RLC").set_hex_dump_max_size(100);
  ...
  // Test discarding the single SDU from the queue
  {
    for (uint32_t i = 0; i < num_tx_sdus; ++i) {
      // Write SDU
      unique_byte_buffer_t sdu = isrran::make_byte_buffer();
  ...
  TESTASSERT(rlc1.has_data() == false);
  isrlog::fetch_basic_logger("TEST").info("Received %zd SDUs", tester.sdus.size());
...
int poll_retx(rlc_am_nr_sn_size_t sn_size)
{
  ...
  {
    unique_byte_buffer_t status_pdu = isrran::make_byte_buffer();
...
bool poll_retx_expiry(rlc_am_nr_sn_size_t sn_size)
{
  ...
  isrlog::fetch_basic_logger("TEST").info("t-PollRetransmssion successfully restarted.");
...
int lost_status_and_advanced_rx_window(rlc_am_nr_sn_size_t sn_size)
{
  ...
  for (uint32_t sn = 0; sn < 10; ++sn) {
    // Write SDU
    unique_byte_buffer_t sdu_buf = isrran::make_byte_buffer();
    ...
    // Read PDU
    unique_byte_buffer_t pdu_buf = isrran::make_byte_buffer();
...
int main()
{
  ...
  if (!isrlog::install_custom_sink(isrran::log_sink_message_spy::name(),
                                   std::unique_ptr<isrran::log_sink_message_spy>(
                                       new isrran::log_sink_message_spy(isrlog::get_default_log_formatter())))) {
    return ISRRAN_ERROR;
  }

  auto* spy = static_cast<isrran::log_sink_message_spy*>(isrlog::find_sink(isrran::log_sink_message_spy::name()));
  if (spy == nullptr) {
    return ISRRAN_ERROR;
  }
  isrlog::set_default_sink(*spy);

  auto& logger_rlc1 = isrlog::fetch_basic_logger("RLC_AM_1", *spy, false);
  auto& logger_rlc2 = isrlog::fetch_basic_logger("RLC_AM_2", *spy, false);
  logger_rlc1.set_hex_dump_max_size(100);
  logger_rlc2.set_hex_dump_max_size(100);
  logger_rlc1.set_level(isrlog::basic_levels::debug);
  logger_rlc2.set_level(isrlog::basic_levels::debug);
  ...
  for (auto sn_size : sn_sizes) {
    TESTASSERT(window_checker_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(retx_segmentation_required_checker_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(basic_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(lost_pdu_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(lost_pdu_duplicated_nack_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(lost_pdus_trimmed_nack_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(clean_retx_queue_of_acked_sdus_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(basic_segmentation_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(segment_retx_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(segment_retx_and_loose_segments_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(retx_segment_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(handle_status_of_non_tx_last_segment(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(max_retx_lost_sdu_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(max_retx_lost_segments_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(discard_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(poll_pdu(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(poll_byte(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(poll_retx(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(poll_retx_expiry(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(rx_nack_range_no_so_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(rx_nack_range_with_so_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(rx_nack_range_with_so_starting_with_full_sdu_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(rx_nack_range_with_so_ending_with_full_sdu_test(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(out_of_order_status(sn_size) == ISRRAN_SUCCESS);
    TESTASSERT(lost_status_and_advanced_rx_window(sn_size) == ISRRAN_SUCCESS);
  }
  TESTASSERT(full_rx_window_t_reassembly_expiry(rlc_am_nr_sn_size_t::size12bits) == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
