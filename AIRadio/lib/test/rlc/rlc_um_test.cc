...
int reassmble_test2()
{
  ...
  const int                    max_n_pdus = 100;
  int                          n_pdus     = 0;
  isrran::unique_byte_buffer_t pdu_bufs[max_n_pdus];
  for (int i = 0; i < max_n_pdus; i++) {
    pdu_bufs[i]          = isrran::make_byte_buffer();
...
int pdu_pack_no_space_test()
{
  ...
  {
    isrran::rlc_umd_pdu_header_t h;
    rlc_um_read_data_pdu_header(&pdu_bufs[0], isrran::rlc_umd_sn_size_t::size10bits, &h);
  ...
  {
    isrran::rlc_umd_pdu_header_t h;
    rlc_um_read_data_pdu_header(&pdu_bufs[1], isrran::rlc_umd_sn_size_t::size10bits, &h);
    TESTASSERT(h.fi == RLC_FI_FIELD_START_AND_END_ALIGNED);
  }
