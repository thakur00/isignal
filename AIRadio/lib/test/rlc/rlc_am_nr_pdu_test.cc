...
#include "isrran/rlc/rlc_am_nr_packing.h"
...
isrran::byte_buffer_t make_pdu_and_log(const std::array<uint8_t, N>& tv)
{
  isrran::byte_buffer_t pdu;
...
void corrupt_pdu_header(isrran::byte_buffer_t& pdu, const uint32_t header_len, const uint32_t payload_len)
{
  // clear header only
...
int rlc_am_nr_pdu_test1()
{
  ...
  isrran::byte_buffer_t                         pdu = make_pdu_and_log(tv);

  // unpack PDU
  rlc_am_nr_pdu_header_t header = {};
  TESTASSERT(rlc_am_nr_read_data_pdu_header(&pdu, isrran::rlc_am_nr_sn_size_t::size12bits, &header) != 0);
...
int rlc_am_nr_pdu_test4()
{
  ...
  // unpack PDU
  rlc_am_nr_pdu_header_t header = {};
  TESTASSERT(rlc_am_nr_read_data_pdu_header(&pdu, isrran::rlc_am_nr_sn_size_t::size18bits, &header) != 0);
  TESTASSERT(header.si == rlc_nr_si_field_t::full_sdu);
...
int rlc_am_nr_control_pdu_12bit_sn_test1()
{
  ...
  // unpack PDU
  rlc_am_nr_status_pdu_t status_pdu(isrran::rlc_am_nr_sn_size_t::size12bits);
  TESTASSERT(rlc_am_nr_read_status_pdu(&pdu, isrran::rlc_am_nr_sn_size_t::size12bits, &status_pdu) == ISRRAN_SUCCESS);
  ...
  // pack again
  TESTASSERT(rlc_am_nr_write_status_pdu(status_pdu, isrran::rlc_am_nr_sn_size_t::size12bits, &pdu) == ISRRAN_SUCCESS);
  TESTASSERT(pdu.N_bytes == tv.size());
...
int rlc_am_nr_control_pdu_12bit_sn_test5()
{
  ...
  // unpack PDU
  rlc_am_nr_status_pdu_t status_pdu(isrran::rlc_am_nr_sn_size_t::size12bits);
  TESTASSERT(rlc_am_nr_read_status_pdu(&pdu, isrran::rlc_am_nr_sn_size_t::size12bits, &status_pdu) == 0);
...
int rlc_am_nr_control_pdu_test_trimming(rlc_am_nr_sn_size_t sn_size)
{
  ...
  // status PDU with no NACKs
  {
    ...
    TESTASSERT_EQ(rlc_am_nr_write_status_pdu(status_pdu, sn_size, &pdu), ISRRAN_SUCCESS);
    ...
    TESTASSERT_EQ(rlc_am_nr_write_status_pdu(status_pdu_copy, sn_size, &pdu), ISRRAN_SUCCESS);
    TESTASSERT_EQ(pdu.N_bytes, min_size);
...
int rlc_am_nr_control_pdu_18bit_sn_test1()
{
  ...
  // unpack PDU
  rlc_am_nr_status_pdu_t status_pdu(isrran::rlc_am_nr_sn_size_t::size18bits);
  TESTASSERT(rlc_am_nr_read_status_pdu(&pdu, isrran::rlc_am_nr_sn_size_t::size18bits, &status_pdu) == ISRRAN_SUCCESS);
  ...
  // pack again
  TESTASSERT(rlc_am_nr_write_status_pdu(status_pdu, isrran::rlc_am_nr_sn_size_t::size18bits, &pdu) == ISRRAN_SUCCESS);
  TESTASSERT(pdu.N_bytes == tv.size());
...
int rlc_am_nr_control_pdu_18bit_sn_test5()
{
  ...
  // unpack PDU
  rlc_am_nr_status_pdu_t status_pdu(isrran::rlc_am_nr_sn_size_t::size18bits);
  TESTASSERT(rlc_am_nr_read_status_pdu(&pdu, isrran::rlc_am_nr_sn_size_t::size18bits, &status_pdu) == 0);
