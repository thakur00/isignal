...
inline int write_pdu_to_pcap(const std::unique_ptr<isrran::mac_pcap>& pcap_handle,
                             const bool                               is_dl,
                             const uint32_t                           lcid,
                             ...
                             const uint32_t                           len)
{
  ...
  if (pcap_handle) {
    isrran::byte_buffer_t  tx_buffer;
    isrran::mac_sch_pdu_nr tx_pdu;
...
void parse_args(stress_test_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
      ...
      ("loglevel",      bpo::value<uint32_t>(&args->log_level)->default_value((int)isrlog::basic_levels::debug), "Log level (1=Error,2=Warning,3=Info,4=Debug)")
  ...
  if (args->log_level > 4) {
    ...
    printf("Set log level to %d (%s)\n",
           args->log_level,
           isrlog::basic_level_to_string(static_cast<isrlog::basic_levels>(args->log_level)));
  }
...
class mac_dummy : public isrran::thread
{
public:
  mac_dummy(isrue::rlc_interface_mac* rlc1_,
            isrue::rlc_interface_mac* rlc2_,
            stress_test_args_t        args_,
            uint32_t                  lcid_,
            isrran::timer_handler*    timers_,
            isrran::rlc_pcap*         pcap_,
            uint32_t                  seed_) :
    run_enable(true),
    rlc1(rlc1_),
    rlc2(rlc2_),
    ...
    lcid(lcid_),
    timers(timers_),
    logger(isrlog::fetch_basic_logger("MAC", false)),
    thread("MAC_DUMMY"),
    real_dist(0.0, 1.0),
    mt19937(seed_)
  {
    logger.set_level(static_cast<isrlog::basic_levels>(args.log_level));
  ...
  void enqueue_task(isrran::move_task_t task) { pending_tasks.push(std::move(task)); }

private:
  void run_thread() final;

  void run_tti(isrue::rlc_interface_mac* tx_rlc, isrue::rlc_interface_mac* rx_rlc, bool is_dl);

  void run_tx_tti(isrue::rlc_interface_mac*                  tx_rlc,
                  isrue::rlc_interface_mac*                  rx_rlc,
                  std::vector<isrran::unique_byte_buffer_t>& pdu_list);

  void                      run_rx_tti(isrue::rlc_interface_mac*                  tx_rlc,
                                       isrue::rlc_interface_mac*                  rx_rlc,
                                       bool                                       is_dl,
                                       std::vector<isrran::unique_byte_buffer_t>& pdu_list);
  isrue::rlc_interface_mac* rlc1 = nullptr;
  isrue::rlc_interface_mac* rlc2 = nullptr;

  std::atomic<bool>      run_enable = {false};
  stress_test_args_t     args       = {};
  isrran::rlc_pcap*      pcap       = nullptr;
  uint32_t               lcid       = 0;
  isrlog::basic_logger&  logger;
  isrran::timer_handler* timers = nullptr;

  isrran::block_queue<isrran::move_task_t> pending_tasks;

  std::mt19937                          mt19937;
  std::uniform_real_distribution<float> real_dist;
};

class rlc_tester : public isrue::pdcp_interface_rlc, public isrue::rrc_interface_rlc, public isrran::thread
{
public:
  rlc_tester(isrue::rlc_interface_pdcp* rlc_pdcp_,
             std::string                name_,
             stress_test_args_t         args_,
             uint32_t                   lcid_,
             uint32_t                   seed_) :
    logger(isrlog::fetch_basic_logger(name_.c_str(), false)),
    rlc_pdcp(rlc_pdcp_),
    name(name_),
    args(args_),
    lcid(lcid_),
  ...
  // PDCP interface
  void write_pdu(uint32_t rx_lcid, isrran::unique_byte_buffer_t sdu) final;
  void write_pdu_bcch_bch(isrran::unique_byte_buffer_t sdu) final {}
  void write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t sdu) final {}
  void write_pdu_pcch(isrran::unique_byte_buffer_t sdu) final {}
  void write_pdu_mch(uint32_t lcid_, isrran::unique_byte_buffer_t sdu) final {}
  void notify_delivery(uint32_t lcid_, const isrran::pdcp_sn_vector_t& pdcp_sns) final {}
  void notify_failure(uint32_t lcid_, const isrran::pdcp_sn_vector_t& pdcp_sns) final {}
...
private:
  ...
  isrue::rlc_interface_pdcp* rlc_pdcp = nullptr; // used by run_thread to push PDCP SDUs to RLC
