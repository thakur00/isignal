...
int test_s1setup_request()
{
  ...
  s1ap_pdu_c pdu;
  TESTASSERT(pdu.unpack(bref) == ISRASN_SUCCESS);
  ...
  TESTASSERT(test_pack_unpack_consistency(pdu) == ISRASN_SUCCESS);
...
int test_proc_id_consistency(isrran::log_sink_spy& spy)
{
  s1ap_pdu_c pdu;
...
int unpack_test_served_gummeis_with_multiple_plmns()
{
  ...
  TESTASSERT(input_pdu.unpack(bref) == ISRASN_SUCCESS);
  TESTASSERT(asn1::test_pack_unpack_consistency(input_pdu) == ISRASN_SUCCESS);
...
int test_initial_ctxt_setup_response()
{
  ...
  uint8_t       buffer[1024];
  asn1::bit_ref bref(buffer, sizeof(buffer));
  TESTASSERT(tx_pdu.pack(bref) == ISRRAN_SUCCESS);

  isrlog::fetch_basic_logger("TEST").info(
...
int test_eci_pack()
{
  ...
  isrlog::fetch_basic_logger("TEST").info(buffer, bref.distance_bytes(), "Packed cell id:");
...
int main()
{
  ...
  auto& asn1_logger = isrlog::fetch_basic_logger("ASN1", *spy, false);
  asn1_logger.set_level(isrlog::basic_levels::debug);
  asn1_logger.set_hex_dump_max_size(-1);
  auto& test_logger = isrlog::fetch_basic_logger("TEST", false);
  test_logger.set_level(isrlog::basic_levels::debug);
