...
isrran::log_sink_spy* test_spy = nullptr;
...
int test_bit_ref()
{
  ...
  // pack bytes aligned
  {
    ...
    TESTASSERT(bref.pack_bytes(buf2, nof_bytes) == ISRASN_SUCCESS);
    ...
    TESTASSERT(bref2.unpack_bytes(&buf2[0], nof_bytes) == ISRASN_SUCCESS);
  ...
  // pack bytes unaligned
  {
    ...
    // this unaligns
    TESTASSERT(bref.pack(0, 1) == ISRASN_SUCCESS);
    TESTASSERT(bref.pack_bytes(buf2, nof_bytes) == ISRASN_SUCCESS);
    ...
    bit_ref  bref2(&buf[0], sizeof(buf));
    uint32_t val;
    TESTASSERT(bref2.unpack(val, 1) == ISRASN_SUCCESS);
    TESTASSERT(val == 0);
    TESTASSERT(bref2.unpack_bytes(&buf3[0], nof_bytes) == ISRASN_SUCCESS);
  ...
  {
    bit_ref bref(&buf[0], sizeof(buf));
    TESTASSERT(bref.advance_bits(4) == ISRASN_SUCCESS);
    TESTASSERT(bref.distance() == 4);
    TESTASSERT(bref.advance_bits(4) == ISRASN_SUCCESS);
    TESTASSERT(bref.distance() == 8);
    TESTASSERT(bref.advance_bits(3) == ISRASN_SUCCESS);
    TESTASSERT(bref.distance() == 11);
    TESTASSERT(bref.advance_bits(200) == ISRASN_SUCCESS);
    TESTASSERT(bref.distance() == 211);
    TESTASSERT(bref.advance_bits(5) == ISRASN_SUCCESS);
...
int test_bitstring()
{
  ...
  // fixed
  TESTASSERT(bstr1.pack(bref) == ISRASN_SUCCESS);
  fixed_bitstring<10> bstr2;
  TESTASSERT(bstr2.length() == 10);
  TESTASSERT(bstr2.unpack(bref2) == ISRASN_SUCCESS);
  TESTASSERT(bstr2 == bstr1);
...
int test_enum()
{
  ...
  bit_ref bref(&buffer[0], sizeof(buffer));
  bit_ref borig = bref;
  TESTASSERT(pack_enum(bref, e) == ISRASN_SUCCESS);
  TESTASSERT(bref.distance(borig) == (int)(floor(log2(e.nof_types)) + 1));

  cbit_ref bref2(&buffer[0], sizeof(buffer));
  TESTASSERT(unpack_enum(e2, bref2) == ISRASN_SUCCESS);
  ...
  bref2 = cbit_ref(&buffer[0], sizeof(buffer));
  e     = EnumTest::nulltype;
  TESTASSERT(pack_enum(bref, e) == ISRASN_ERROR_ENCODE_FAIL);
  buffer[0] = 255;
  TESTASSERT(unpack_enum(e, bref2) == ISRASN_ERROR_DECODE_FAIL);
...
void test_varlength_field_pack()
{
  uint8_t buffer[128];
  bit_ref bref(&buffer[0], sizeof(buffer));
  TESTASSERT_EQ(ISRRAN_SUCCESS, bref.pack(0, 1));
...
int main()
{
  ...
  test_spy = static_cast<isrran::log_sink_spy*>(isrlog::find_sink(isrran::log_sink_spy::name()));
  if (!test_spy) {
    return ISRRAN_ERROR;
  }

  auto& asn1_logger = isrlog::fetch_basic_logger("ASN1", *test_spy, false);
