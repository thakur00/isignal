...
#include "../../../isrue/hdr/stack/rrc/rrc.h" // for rrc_args_t
...
int rrc_nr_test_scg_fail_packing()
{
  ...
  TESTASSERT(ul_dcch_msg.pack(bref) == ISRRAN_SUCCESS);
  bref.align_bytes_zero();
  uint32_t cap_len = (uint32_t)bref.distance_bytes(buf);
  return ISRRAN_SUCCESS;
}

int rrc_ue_cap_info_test(isrran::mac_pcap* pcap)
{
...
int pack_fail_test()
{
  ...
  if (ul_dcch_msg.pack(bref) != asn1::ISRASN_SUCCESS) {
...
int main(int argc, char** argv)
{
  ...
  TESTASSERT(rrc_nr_test_scg_fail_packing() == ISRRAN_SUCCESS)
