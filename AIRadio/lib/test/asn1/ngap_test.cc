...
int test_amf_upd()
{
  ...
  ngap_pdu_c pdu;
  TESTASSERT_EQ(ISRASN_SUCCESS, pdu.unpack(bref));
  ...
  TESTASSERT_EQ("isrran", amf_name.value.to_string());

  TESTASSERT_EQ(sizeof(ngap_msg), ceil(bref.distance_bytes()));
  TESTASSERT_EQ(ISRASN_SUCCESS, test_pack_unpack_consistency(pdu));
...
int test_session_res_setup_request()
{
  ...
  TESTASSERT(req.unpack(bref2) == ISRASN_SUCCESS);
  ...
  TESTASSERT(test_pack_unpack_consistency(req) == ISRASN_SUCCESS);
