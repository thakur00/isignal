...
int meas_obj_test()
{
  ...
  isrran::plmn_id_t plmn = isrran::make_plmn_id_t(meas_list[0].cgi_info.cell_global_id.plmn_id);
...
int test_meas_config()
{
  ...
  TESTASSERT(recfg_msg_unpacked.unpack(bref) == ISRASN_SUCCESS);

  TESTASSERT(test_pack_unpack_consistency(recfg_msg_unpacked) == ISRASN_SUCCESS);
...
int main(int argc, char** argv)
{
  ...
  TESTASSERT(test_meas_config() == ISRRAN_SUCCESS);
