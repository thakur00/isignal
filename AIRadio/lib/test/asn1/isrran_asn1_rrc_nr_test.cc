...
#define HAVE_PCAP 0
#if HAVE_PCAP
#include "isrran/common/test_pcap.h"
#endif
...
int test_eutra_nr_capabilities()
{
  ...
  TESTASSERT(test_pack_unpack_consistency(mrdc_cap) == ISRASN_SUCCESS);

  isrlog::fetch_basic_logger("RRC").info(
...
int test_ue_mrdc_capabilities()
{
  ...
  TESTASSERT(mrdc_cap.unpack(bref) == ISRASN_SUCCESS);
...
int test_ue_rrc_reconfiguration()
{
  ...
  TESTASSERT(rrc_recfg.unpack(bref) == ISRASN_SUCCESS);
  TESTASSERT(rrc_recfg.rrc_transaction_id == 0);
#if JSON_OUTPUT
  json_writer jw;
  rrc_recfg.to_json(jw);
  isrlog::fetch_basic_logger("RRC").info("RRC Reconfig: \n %s", jw.to_string().c_str());
#endif
  ...
  TESTASSERT(cell_group_cfg.unpack(bref0) == ISRASN_SUCCESS);
#if JSON_OUTPUT
  json_writer jw1;
  cell_group_cfg.to_json(jw1);
  isrlog::fetch_basic_logger("RRC").info("RRC Secondary Cell Group: \n %s", jw1.to_string().c_str());
#endif
...
int test_radio_bearer_config()
{
  ...
  TESTASSERT(radio_bearer_cfg.unpack(bref) == ISRASN_SUCCESS);
#if JSON_OUTPUT
  json_writer jw;
  radio_bearer_cfg.to_json(jw);
  isrlog::fetch_basic_logger("RRC").info("RRC Bearer CFG Message: \n %s", jw.to_string().c_str());
#endif
  ...
  TESTASSERT(reconfig.pack(bref_pack) == asn1::ISRASN_SUCCESS);
  TESTASSERT(test_pack_unpack_consistency(reconfig) == ISRASN_SUCCESS);

#if JSON_OUTPUT
  reconfig.to_json(jw);
  isrlog::fetch_basic_logger("RRC").info("RRC Reconfig Message: \n %s", jw.to_string().c_str());
#endif
  ...
  TESTASSERT(radio_bearer_cfg_pack.pack(bref_pack2) == asn1::ISRASN_SUCCESS);

#if JSON_OUTPUT
  radio_bearer_cfg_pack.to_json(jw);
  isrlog::fetch_basic_logger("RRC").info("Radio bearer config Message: \n %s", jw.to_string().c_str());
#endif
...
int test_cell_group_config_tdd()
{
  ...
  asn1::ISRASN_CODE err;

  cbit_ref         bref(&cell_group_config_raw[0], sizeof(cell_group_config_raw));
  cell_group_cfg_s cell_group_cfg;

  TESTASSERT(cell_group_cfg.unpack(bref) == ISRASN_SUCCESS);

  TESTASSERT(test_pack_unpack_consistency(cell_group_cfg) == ISRASN_SUCCESS);
  ...
  isrlog::fetch_basic_logger("RRC").info("RRC Secondary Cell Group: Content: %s\n", json_writer.to_string().c_str());
#endif
  ...
  TESTASSERT(cell_group_cfg_pack.pack(bref_pack) == asn1::ISRASN_SUCCESS);
  TESTASSERT(test_pack_unpack_consistency(cell_group_cfg_pack) == ISRASN_SUCCESS);
  packed_cell_group.resize(bref_pack.distance_bytes());

#if JSON_OUTPUT
  asn1::json_writer json_writer2;
  cell_group_cfg_pack.to_json(json_writer2);
  isrlog::fetch_basic_logger("RRC").info(packed_cell_group.data(),
  ...
  TESTASSERT(dcch.pack(bref_dcch_pack) == asn1::ISRASN_SUCCESS);
  packed_dcch.resize(bref_dcch_pack.distance_bytes() + 10);

  asn1::json_writer json_writer3;
  dcch.to_json(json_writer3);
  isrlog::fetch_basic_logger("RRC").info(packed_dcch.data(),
                                         packed_dcch.size(),
                                         "Full DCCH repacked (%d B): \n %s",
                                         packed_dcch.size(),
                                         json_writer3.to_string().c_str());

  isrran::write_pdcp_sdu_nr(1, packed_dcch.data(), packed_dcch.size());
#endif
...
int main()
{
  ...
  pcap_handle->open("isrran_asn1_rrc_nr_test.pcap");
#endif

  test_rrc_setup_complete();
  TESTASSERT(test_eutra_nr_capabilities() == ISRRAN_SUCCESS);
  TESTASSERT(test_ue_mrdc_capabilities() == ISRRAN_SUCCESS);
  TESTASSERT(test_ue_rrc_reconfiguration() == ISRRAN_SUCCESS);
  TESTASSERT(test_radio_bearer_config() == ISRRAN_SUCCESS);
  TESTASSERT(test_cell_group_config_tdd() == ISRRAN_SUCCESS);
  TESTASSERT(test_cell_group_config_fdd() == ISRRAN_SUCCESS);
