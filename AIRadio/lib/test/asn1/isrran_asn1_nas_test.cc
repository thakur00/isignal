...
int nas_dedicated_eps_bearer_context_setup_request_test()
{
  ...
  isrran::unique_byte_buffer_t tst_msg, out_msg;
  tst_msg = isrran::make_byte_buffer();
  TESTASSERT(tst_msg != nullptr);
  out_msg = isrran::make_byte_buffer();
  ...
  uint16_t port;
  isrran::uint8_to_uint16(&ded_bearer_req.tft.packet_filter_list[0].filter[1], &port);
...
int main(int argc, char** argv)
{
  ...
  TESTASSERT(nas_dedicated_eps_bearer_context_setup_request_test() == ISRRAN_SUCCESS);
  TESTASSERT(downlink_generic_nas_transport_unpacking_test() == ISRRAN_SUCCESS);
  TESTASSERT(downlink_generic_nas_transport_packing_test() == ISRRAN_SUCCESS);
  TESTASSERT(downlink_generic_nas_transport_with_add_info_unpacking_test() == ISRRAN_SUCCESS);
  TESTASSERT(downlink_generic_nas_transport_with_add_info_packing_test() == ISRRAN_SUCCESS);
