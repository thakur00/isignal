...
public:
  ...
  isrran::pdcp_entity_nr&  pdcp_rx;
  gw_dummy&                gw_rx;
  isrue::stack_test_dummy& stack;
  isrlog::basic_logger&    logger;

  test_rx_helper(uint8_t pdcp_sn_len, isrlog::basic_logger& logger) :
    pdcp_hlp_rx({1,
...
int test_rx_all(isrlog::basic_logger& logger)
{
   ...
   */
  {
    ...
    isrran::test_delimit_logger delimiter("RX COUNT [4095,4096], 12 bit SN");
    test_rx_helper              rx_helper(isrran::PDCP_SN_LEN_12, logger);
    ...
    std::vector<pdcp_test_event_t> test1_pdus =
        gen_expected_pdus_vector(tst_sdu1, test1_counts, isrran::PDCP_SN_LEN_12, sec_cfg, logger);
  ...
  {
    auto&                       test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger delimiter("RX COUNT [4294967295,0], 12 bit SN");
    test_rx_helper              rx_helper(isrran::PDCP_SN_LEN_12, logger);
    ...
    std::vector<pdcp_test_event_t> test2_pdus =
        gen_expected_pdus_vector(tst_sdu1, test2_counts, isrran::PDCP_SN_LEN_12, sec_cfg, logger);
  ...
  {
    auto&                       test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger delimiter("RX COUNT [262144,262145], 12 bit SN");
    test_rx_helper              rx_helper(isrran::PDCP_SN_LEN_18, logger);
    ...
    std::vector<pdcp_test_event_t> test3_pdus =
        gen_expected_pdus_vector(tst_sdu1, test3_counts, isrran::PDCP_SN_LEN_18, sec_cfg, logger);
  ...
  {
    auto&                       test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger delimiter("RX COUNT [4294967295,4294967296], 18 bit SN");
    test_rx_helper              rx_helper(isrran::PDCP_SN_LEN_18, logger);
    ...
    std::vector<pdcp_test_event_t> test4_pdus =
        gen_expected_pdus_vector(tst_sdu1, test4_counts, isrran::PDCP_SN_LEN_18, sec_cfg, logger);
  ...
  {
    auto&                          test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger    delimiter("RX out-of-order COUNT [1,0], 12 bit SN");
    test_rx_helper                 rx_helper(isrran::PDCP_SN_LEN_12, logger);
  ...
  {
    auto&                          test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger    delimiter("RX out-of-order COUNT [1,0], 18 bit SN");
    test_rx_helper                 rx_helper(isrran::PDCP_SN_LEN_18, logger);
  ...
  {
    auto&                          test_logger = isrlog::fetch_basic_logger("TESTER  ");
    isrran::test_delimit_logger    delimiter("RX out-of-order COUNT [1,0] t_reordering expired, 12 bit SN");
    test_rx_helper                 rx_helper(isrran::PDCP_SN_LEN_12, logger);
   ...
   */
  {
    isrran::test_delimit_logger    delimiter("RX duplicate COUNTs [0,0], 12 bit SN");
    test_rx_helper                 rx_helper(isrran::PDCP_SN_LEN_12, logger);
...
{
  // Setup log
  auto& logger = isrlog::fetch_basic_logger("PDCP NR Test RX", false);
