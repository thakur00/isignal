...
#include "isrran/adt/choice_type.h"
...
{
  using isrran::choice_details::tagged_union_t;
...
int test_choice()
{
  ...
  TESTASSERT(D::counter == 0);
  {
    ...
    TESTASSERT(c2.is<int>() and isrran::get<int>(c2) == i and *isrran::get_if<int>(c2) == i);
    TESTASSERT(isrran::get<1>(c2) == isrran::get<int>(c2));
    TESTASSERT(c3.is<C>());
    TESTASSERT(C::counter == 2);

    // TEST: Invalid member access. get<>() should throw
    TESTASSERT(isrran::get_if<char>(c2) == nullptr);

    // TEST: simple emplace after construction
    c2 = 'c';
    TESTASSERT(c2.is<char>() and isrran::get<char>(c2) == 'c');

    // TEST: copy ctor test.
    choice_t<char, int, double, C> c5{c3};
    TESTASSERT(C::counter == 3);
    TESTASSERT(c5.is<C>());
    TESTASSERT(isrran::get_if<C>(c5) == &isrran::get<C>(c5));

    // TEST: copy assignment
    c = c5;
    TESTASSERT(C::counter == 4);
    TESTASSERT(c.is<C>() and isrran::get_if<C>(c) != isrran::get_if<C>(c5));
    c = c2;
    TESTASSERT(C::counter == 3);
    TESTASSERT(c2.is<char>() and isrran::get<char>(c) == 'c');
  }
  TESTASSERT(C::counter == 0);
  TESTASSERT(D::counter == 0);
  {
    ...
    TESTASSERT(c.is<char>());
    TESTASSERT(c2.is<double>() and isrran::get<double>(c2) == 5.0 and *isrran::get_if<double>(c2) == 5.0);
    TESTASSERT(c3.is<C>());
    TESTASSERT(c4.is<D>());
    TESTASSERT(C::counter == 1);
    ...
    TESTASSERT(isrran::get<double>(c) == isrran::get<double>(c2));
...
int test_visit()
{
  ...
  EVisitor v;
  isrran::visit(v, c);
  TESTASSERT(c.is<int>() and isrran::get<int>(c) == 5);
  TESTASSERT(v.pdu == nullptr);

  // TEST: visitor hits type E and steals pdu
  E e;
  e.pdu = isrran::make_byte_buffer();
  TESTASSERT(e.pdu != nullptr);
  c     = std::move(e);
  TESTASSERT(c.is<E>() and isrran::get<E>(c).pdu != nullptr);
  isrran::visit(v, c);
  TESTASSERT(v.pdu != nullptr);
  TESTASSERT(c.is<E>() and isrran::get<E>(c).pdu == nullptr);
...
int main()
{
  TESTASSERT(test_tagged_union() == ISRRAN_SUCCESS);
  TESTASSERT(test_choice() == ISRRAN_SUCCESS);
  TESTASSERT(test_visit() == ISRRAN_SUCCESS);
}
