...
public:
  ...
  void then(const isrran::proc_result_t<int>& result) const
  {
...
static_assert(std::is_same<typename isrran::proc_t<custom_proc, int>::result_type, int>::value,
              "Failed derivation of result type");
static_assert(std::is_same<typename isrran::proc_t<custom_proc, std::string>::result_type, std::string>::value,
              "Failed derivation of result type");
...
int test_local_1()
{
  ...
  new_test();
  printf("\n--- Test %s ---\n", __func__);
  isrran::proc_t<custom_proc, int> proc;
...
int test_callback_1()
{
  ...
  isrran::proc_future_t<int> proc_fut = proc.get_future();
  ...
  isrran::proc_future_t<int> proc_fut2 = proc.get_future();
...
int test_callback_2()
{
  ...
  isrran::proc_future_t<int> fut = proc.get_future();
...
int test_complete_callback_1()
{
  ...
  printf("\n--- Test %s ---\n", __func__);
  isrran::proc_manager_list_t                 callbacks;
  isrran::proc_t<custom_proc2_t, std::string> proc;

  std::string run_result;
  auto        continuation_task = [&run_result](const isrran::proc_result_t<std::string>& e) {
  ...
  for (uint32_t i = 0; i < 6; ++i) {
    ...
    isrran::proc_future_t<std::string> fut;
...
int test_event_handler_1()
{
   ...
   */
  printf("\n--- Test %s ---\n", __func__);
  isrran::proc_t<custom_proc2_t, std::string>      proc, proc2;
  isrran::event_handler_t<custom_proc2_t::event_t> ev_handler;
