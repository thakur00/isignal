...
{
  auto& logger = isrlog::fetch_basic_logger("S1AP", false);

  std::atomic<int> counter = {0};

  isrran::unique_socket  server_socket, client_socket, client_socket2;
  isrran::socket_manager sockhandler;
  ...
  auto pdu_handler =
      [&logger,
       &counter](isrran::unique_byte_buffer_t pdu, const sockaddr_in& from, const sctp_sndrcvinfo& sri, int flags) {
        if (pdu->N_bytes > 0) {
  ...
  sockhandler.add_socket_handler(server_socket.fd(),
                                 isrran::make_sctp_sdu_handler(logger, rx_tester.task_queue, pdu_handler));
  ...
  for (int32_t i = 0; i < nof_counts; ++i) {
    ...
    isrran::unique_socket* chosen = &client_socket;
    if (i % 2 == 1) {
...
{
  isrran::unique_socket sock;
  TESTASSERT(sock.open_socket(isrran::net_utils::addr_family::ipv4,
                              isrran::net_utils::socket_type::seqpacket,
                              isrran::net_utils::protocol_type::SCTP));
  TESTASSERT(not sock.bind_addr("1.1.1.1", 8000)); // Bogus IP address
                                                   // should not be able to bind

  isrran::unique_socket sock2;
  TESTASSERT(sock2.open_socket(isrran::net_utils::addr_family::ipv4,
                               isrran::net_utils::socket_type::seqpacket,
                               isrran::net_utils::protocol_type::SCTP));
