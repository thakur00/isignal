...
{
  auto& logger = isrlog::fetch_basic_logger("LOG", false);
  ...
  TESTASSERT(isrran::security_xor_f2345(k, rand, res_o, ck_o, ik_o, ak_o) == ISRRAN_SUCCESS);
  ...
  uint8_t mac_o[8];
  TESTASSERT(isrran::security_xor_f1(k, rand, sqn, amf, mac_o) == ISRRAN_SUCCESS);
...
int main(int argc, char* argv[])
{
  ...
  TESTASSERT(test_set_2() == ISRRAN_SUCCESS);
  TESTASSERT(test_set_xor_own_set_1() == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
