...
{
  isrran::task_scheduler task_sched{5, 0};
  ...
  state = task_result::null;
  isrran::get_background_workers().push_task([&task_sched, &state]() {
...
int main()
{
  TESTASSERT(test_task_scheduler_no_pool() == ISRRAN_SUCCESS);
  TESTASSERT(test_task_scheduler_with_pool() == ISRRAN_SUCCESS);
}
