...
#define ISRRAN_MAX_RADIOS 3

static std::array<std::string, ISRRAN_MAX_RADIOS> radios_args = {};
...
static cf_t*       buffers[ISRRAN_MAX_RADIOS][ISRRAN_MAX_PORTS];
...
static plot_real_t fft_plot[ISRRAN_MAX_RADIOS]          = {};
static cf_t*       fft_plot_buffer[ISRRAN_MAX_CHANNELS] = {};
static float*      fft_plot_temp                        = nullptr;
static uint32_t    fft_plot_buffer_size;
isrran_dft_plan_t  dft_spectrum = {};
#endif /* ENABLE_GUI */
...
void usage(char* prog)
{
  ...
  printf("\t-v Set isrran_verbose to info (v) or debug (vv) [Default none]\n");
...
static ISRRAN_AGC_CALLBACK(set_gain_callback)
{
  auto r = (radio*)h;
    ...
    if (fft_plot_buffer_size) {
      for (uint32_t r = 0; r < nof_radios; r++) {
        for (uint32_t p = 0; p < nof_ports; p++) {
          ...
          isrran_vec_abs_square_cf(fft_plot_buffer[plot_idx], fft_plot_temp, fft_plot_buffer_size);

          for (uint32_t j = 0; j < fft_plot_buffer_size; j++) {
            fft_plot_temp[j] = isrran_convert_power_to_dB(fft_plot_temp[j]);
          }
...
static int init_plots(uint32_t frame_size)
{
  ...
  for (uint32_t r = 0; r < nof_radios; r++) {
    for (uint32_t p = 0; p < nof_ports; p++) {
      uint32_t plot_idx         = r * nof_ports + p;
      fft_plot_buffer[plot_idx] = isrran_vec_cf_malloc(frame_size);
  ...
  fft_plot_temp = isrran_vec_f_malloc(frame_size);
  if (!fft_plot_temp) {
    ERROR("Error: Allocating buffer");
    return ISRRAN_ERROR;
  }

  if (isrran_dft_plan_c(&dft_spectrum, frame_size, ISRRAN_DFT_FORWARD)) {
    ERROR("Creating DFT spectrum plan");
    return ISRRAN_ERROR;
  }

  isrran_dft_plan_set_mirror(&dft_spectrum, true);
...
static int ret = ISRRAN_ERROR;

static void* radio_thread_run(void* arg)
{
  radio*                 radio_h[ISRRAN_MAX_RADIOS] = {nullptr};
  isrran::rf_timestamp_t ts_prev[ISRRAN_MAX_RADIOS], ts_rx[ISRRAN_MAX_RADIOS], ts_tx;
  uint32_t               nof_gaps                    = 0;
  char                   filename[256]               = {};
  isrran_filesink_t      filesink[ISRRAN_MAX_RADIOS] = {};
  isrran_dft_plan_t      dft_plan = {}, idft_plan = {};
  isrran_agc_t           agc[ISRRAN_MAX_RADIOS] = {};
  phy_dummy              phy;
  isrran::rf_metrics_t   rf_metrics = {};

  rf_buffer_t rf_buffers[ISRRAN_MAX_RADIOS] = {};

  float    delay_idx[ISRRAN_MAX_RADIOS] = {0};
  ...
  for (uint32_t r = 0; r < nof_radios; r++) {
    ...
    for (uint32_t p = 0; p < ISRRAN_MAX_PORTS; p++) {
      buffers[r][p] = NULL;
    }
  }
  ...
  for (uint32_t r = 0; r < nof_radios; r++) {
    for (uint32_t p = 0; p < nof_ports; p++) {
      buffers[r][p] = isrran_vec_cf_malloc(frame_size);
      if (!buffers[r][p]) {
...
#ifdef ENABLE_GUI
  if (fft_plot_enable) {
    if (init_plots(frame_size) != ISRRAN_SUCCESS) {
  ...
  for (uint32_t r = 0; r < nof_radios; r++) {
    ...
    if (radio_h[r]->init(radio_args, &phy) != ISRRAN_SUCCESS) {
    ...
    // enable and init agc
    if (agc_enable) {
      if (isrran_agc_init_uhd(&agc[r], ISRRAN_AGC_MODE_PEAK_AMPLITUDE, 0, set_gain_callback, radio_h[r])) {
  ...
  if (capture) {
    for (uint32_t r = 0; r < nof_radios; r++) {
      ...
      if (isrran_filesink_init(&filesink[r], filename, ISRRAN_COMPLEX_FLOAT_BIN)) {
  ...
  if (measure_delay) {
    if (nof_radios > 1) {
      if (isrran_dft_plan_c(&dft_plan, frame_size, ISRRAN_DFT_FORWARD)) {
        ERROR("Creating DFT plan");
        goto clean_exit;
      }
      if (isrran_dft_plan_c(&idft_plan, frame_size, ISRRAN_DFT_BACKWARD)) {
  ...
  for (int i = 0; i < ISRRAN_MAX_RADIOS; i++) {
  ...
  for (uint32_t i = 0; i < nof_frames; i++) {
    ...
    int gap    = 0;
    frame_size = ISRRAN_MIN(frame_size, nof_samples);
    ...
    if (agc_enable) {
      for (uint32_t r = 0; r < nof_radios; r++) {
        isrran_agc_process(&agc[r], buffers[r][0], frame_size);
      }
    }
    ...
    if (capture) {
      for (uint32_t r = 0; r < nof_radios; r++) {
        isrran_filesink_write_multi(&filesink[r], (void**)buffers[r], frame_size, nof_ports);
      }
    }
    ...
    if (fft_plot_enable) {
      if (frame_size != nof_samples) {
        for (uint32_t r = 0; r < nof_radios; r++) {
          for (uint32_t p = 0; p < nof_ports; p++) {
            uint32_t plot_idx = r * nof_ports + p;
            isrran_dft_run(&dft_spectrum, buffers[r][p], fft_plot_buffer[plot_idx]);
          }
        }
      } else {
    ...
    if (measure_delay && frame_size != nof_samples) {
      for (uint32_t r = 0; r < nof_radios; r++) {
        isrran_dft_run_c(&dft_plan, buffers[r][0], buffers[r][0]);
      }

      for (uint32_t r = 1; r < nof_radios; r++) {
        int relative_delay = 0;

        isrran_vec_prod_conj_ccc(buffers[0][0], buffers[r][0], buffers[r][0], frame_size);
        isrran_dft_run_c(&idft_plan, buffers[r][0], buffers[r][0]);
        relative_delay = isrran_vec_max_abs_ci(buffers[r][0], frame_size);
    ...
    if (i != 0) {
      for (uint32_t r = 0; r < nof_radios; r++) {
        isrran_timestamp_t ts_diff;

        isrran_timestamp_copy(&ts_diff, ts_rx[r].get_ptr(0));
        isrran_timestamp_sub(&ts_diff, ts_prev[r].get_ptr(0)->full_secs, ts_prev[r].get_ptr(0)->frac_secs);
        gap = (int32_t)round(isrran_timestamp_real(&ts_diff) * current_rate) - (int32_t)frame_size;
        ...
        if (gap != 0) {
          INFO("Timestamp gap (%d samples) detected! Frame %d/%d. ts=%.9f+%.9f=%.9f",
               ...
               i + 1,
               nof_frames,
               isrran_timestamp_real(ts_prev[r].get_ptr(0)),
               isrran_timestamp_real(&ts_diff),
               isrran_timestamp_real(ts_rx[r].get_ptr(0)));
          nof_gaps++;
        }
      }
    }
...
clean_exit:
  ...
  for (uint32_t r = 0; r < nof_radios; r++) {
    ...
    if (capture) {
      isrran_filesink_free(&filesink[r]);
    }
  }

  isrran_dft_plan_free(&dft_plan);
  isrran_dft_plan_free(&idft_plan);
  ...
  if (fft_plot_enable) {
    ...
    isrran_dft_plan_free(&dft_spectrum);
    for (uint32_t r = 0; r < nof_radios; r++) {
      for (uint32_t p = 0; p < nof_ports; p++) {
        uint32_t plot_idx = r * nof_ports + p;
        if (fft_plot_buffer[plot_idx]) {
