...
class phy_radio_listener : public isrran::phy_interface_radio
{
private:
...
{
private:
  isrlog::basic_logger& logger   = isrlog::fetch_basic_logger("SINK", false);
...
public:
  ...
  {
    if (isrran_filesink_init(&filesink, filename.c_str(), ISRRAN_COMPLEX_FLOAT_BIN) < ISRRAN_SUCCESS) {
  ...
  ~test_sink()
  {
    ...
    isrran_vec_fprint_f(stdout, meas_power_dB.data(), (int)meas_power_dB.size());
  }

  void write(std::vector<cf_t>& buffer, uint32_t nsamp)
  {
    // Measure average power
    float avg_pwr = isrran_vec_avg_power_cf(buffer.data(), nsamp);

    // Push measurement in dB
    meas_power_dB.push_back(isrran_convert_power_to_dB(avg_pwr));

    // Write signal in file
    isrran_filesink_write(&filesink, (void*)buffer.data(), (int)nsamp);
  }
};
...
public:
  ...
  {
    phase *= isrran_vec_gen_sine(phase, freq_norm, buffer.data(), (int)nsamp);
  }
};
...
int main(int argc, char** argv)
{
  ...
  // Initialise radio
  TESTASSERT(radio.init(rf_args, &radio_listener) == ISRRAN_SUCCESS);
