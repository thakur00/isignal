...
radio::radio()
{
  zeros.resize(ISRRAN_SF_LEN_MAX, 0);
  for (uint32_t i = 0; i < ISRRAN_MAX_CHANNELS; i++) {
    dummy_buffers[i].resize(ISRRAN_SF_LEN_MAX * ISRRAN_NOF_SF_X_FRAME, 0);
  }
}

radio::~radio()
{
  for (isrran_resampler_fft_t& q : interpolators) {
    isrran_resampler_fft_free(&q);
  }

  for (isrran_resampler_fft_t& q : decimators) {
    isrran_resampler_fft_free(&q);
  }
}
...
int radio::init(const rf_args_t& args, phy_interface_radio* phy_)
{
  ...
  if (args.nof_antennas > ISRRAN_MAX_PORTS) {
    logger.error("Maximum number of antennas exceeded (%d > %d)", args.nof_antennas, ISRRAN_MAX_PORTS);
    return ISRRAN_ERROR;
  }

  if (args.nof_carriers > ISRRAN_MAX_CARRIERS) {
    logger.error("Maximum number of carriers exceeded (%d > %d)", args.nof_carriers, ISRRAN_MAX_CARRIERS);
    return ISRRAN_ERROR;
  }

  if (!config_rf_channels(args)) {
    isrran::console("Error configuring RF channels\n");
    return ISRRAN_ERROR;
  }
  ...
  for (uint32_t i = 0; i < args.nof_carriers; i++) {
    if (args.tx_gain_ch[i] > 0) {
      for (uint32_t j = 0; j < nof_antennas; j++) {
        ...
        if (isrran_rf_set_tx_gain_ch(&rf_devices[rf_device_idx], rf_channel_idx, args.tx_gain_ch[i]) < 0) {
          logger.error(
  ...
  for (uint32_t i = 0; i < args.nof_carriers; i++) {
    if (args.rx_gain_ch[i] > 0) {
      for (uint32_t j = 0; j < nof_antennas; j++) {
        ...
        if (isrran_rf_set_rx_gain_ch(&rf_devices[rf_device_idx], rf_channel_idx, args.rx_gain_ch[i]) < 0) {
          logger.error(
...
void radio::stop()
{
  ...
  if (radio_is_streaming) {
    for (isrran_rf_t& rf_device : rf_devices) {
      isrran_rf_stop_rx_stream(&rf_device);
    }
  }
  if (is_initialized) {
    for (isrran_rf_t& rf_device : rf_devices) {
...
bool radio::start_agc(bool tx_gain_same_rx)
{
  ...
  for (isrran_rf_t& rf_device : rf_devices) {
    if (isrran_rf_start_gain_thread(&rf_device, tx_gain_same_rx)) {
...
bool radio::rx_now(rf_buffer_interface& buffer, rf_timestamp_interface& rxd_time)
{
  ...
  if (not radio_is_streaming) {
    ...
    if (rf_devices.size() > 1) {
      for (isrran_rf_t& rf_device : rf_devices) {
        isrran_rf_flush_buffer(&rf_device);
      }
    }
  }
  ...
  if (ratio > 1) {
    for (uint32_t ch = 0; ch < nof_channels; ch++) {
      if (buffer.get(ch) and buffer_rx.get(ch)) {
        isrran_resampler_fft_run(&decimators[ch], buffer_rx.get(ch), buffer.get(ch), buffer_rx.get_nof_samples());
      }
    }
  }
...
bool radio::rx_dev(const uint32_t& device_idx, const rf_buffer_interface& buffer, isrran_timestamp_t* rxd_time)
{
  if (!is_initialized) {
    return false;
  }
  ...
  void* radio_buffers[ISRRAN_MAX_CHANNELS] = {};
  ...
  if (nof_samples_offset < 0 and (uint32_t)(-nof_samples_offset) > nof_samples) {
    ...
    nof_samples = ISRRAN_MIN(nof_samples + nof_samples_offset, 2 * nof_samples);
  }
  ...
  int ret =
      isrran_rf_recv_with_time_multi(&rf_devices[device_idx], radio_buffers, nof_samples, true, full_secs, frac_secs);
  ...
  for (auto& b : radio_buffers) {
    if (b != nullptr) {
      cf_t* ptr = (cf_t*)b;
      isrran_vec_cf_zero(&ptr[nof_samples], nof_zeros);
    }
  }
...
bool radio::tx(rf_buffer_interface& buffer, const rf_timestamp_interface& tx_time)
{
  ...
  if (interpolators[0].ratio > 1) {
    for (uint32_t ch = 0; ch < nof_channels; ch++) {
      ...
      isrran_resampler_fft_run(&interpolators[ch], buffer.get(ch), tx_buffer[ch].data(), nof_samples);
...
{
  isrran_rf_t* rf_device = &rf_devices[device_idx];
  ...
  isrran::console("Opening %d channels in RF device=%s with args=%s\n",
                  nof_channels_x_dev,
                  dev_name ? dev_name : "default",
                  dev_args ? dev_args : "default");

  if (isrran_rf_open_devname(rf_device, dev_name, dev_args, nof_channels_x_dev)) {
    logger.error("Error opening RF device");
    return false;
  }

  // Suppress radio stdout
  isrran_rf_suppress_stdout(rf_device);

  // Register handler for processing O/U/L
  isrran_rf_register_error_handler(rf_device, rf_msg_callback, this);

  // Get device info
  rf_info[device_idx] = *isrran_rf_get_info(rf_device);
...
bool radio::open_dev(const uint32_t &device_idx, FILE** rx_files, FILE** tx_files, uint32_t nof_channels, uint32_t base_srate)
{
  ...
  isrran::console("Opening channels idx %d in RF device abstraction\n", device_idx);

  if (isrran_rf_open_file(rf_device, rx_files, tx_files, nof_channels, base_srate)) {
...
bool radio::tx_dev(const uint32_t& device_idx, rf_buffer_interface& buffer, const isrran_timestamp_t& tx_time_)
{
  uint32_t     nof_samples   = buffer.get_nof_samples();
  ...
  isrran_timestamp_t tx_time = tx_time_;
  if (!tx_adv_negative) {
    isrran_timestamp_sub(&tx_time, 0, tx_adv_sec);
  } else {
    isrran_timestamp_add(&tx_time, 0, tx_adv_sec);
  }

  // Calculates transmission time overlap with previous transmission
  isrran_timestamp_t ts_overlap = end_of_burst_time[device_idx];
  isrran_timestamp_sub(&ts_overlap, tx_time.full_secs, tx_time.frac_secs);

  // Calculates number of overlap samples with previous transmission
  int32_t past_nsamples = (int32_t)round(cur_tx_srate * isrran_timestamp_real(&ts_overlap));
  ...
  if (past_nsamples > 0) {
    ...
    logger.debug("Detected RF overlap of %.1f us. Discarding %d samples.",
                 isrran_timestamp_real(&ts_overlap) * 1.0e6,
                 past_nsamples);

  } else if (past_nsamples < 0 and not is_start_of_burst) {
    // if the gap is bigger than TX_MAX_GAP_ZEROS, stop burst
    if (fabs(isrran_timestamp_real(&ts_overlap)) > tx_max_gap_zeros) {
      logger.info("Detected RF gap of %.1f us. Sending end-of-burst.", isrran_timestamp_real(&ts_overlap) * 1.0e6);
      tx_end_nolock();
    } else {
      logger.debug("Detected RF gap of %.1f us. Tx'ing zeroes.", isrran_timestamp_real(&ts_overlap) * 1.0e6);
      // Otherwise, transmit zeros
      uint32_t gap_nsamples = abs(past_nsamples);
      while (gap_nsamples > 0) {
        // Transmission cannot exceed ISRRAN_SF_LEN_MAX (zeros buffer size limitation)
        uint32_t nzeros = ISRRAN_MIN(gap_nsamples, ISRRAN_SF_LEN_MAX);

        // Zeros transmission
        int ret = isrran_rf_send_timed2(rf_device,
                                        zeros.data(),
                                        nzeros,
        ...
        // Increase timestamp
        isrran_timestamp_add(&end_of_burst_time[device_idx], 0, (double)nzeros / cur_tx_srate);
      }
    }
  }

  // Save possible end of burst time
  isrran_timestamp_copy(&end_of_burst_time[device_idx], &tx_time);
  isrran_timestamp_add(&end_of_burst_time[device_idx], 0, (double)nof_samples / cur_tx_srate);
    ...
    if (cur_rx_freqs[device_mapping.carrier_idx] != freq) {
      if ((device_mapping.carrier_idx + 1) * nof_antennas <= nof_channels) {
        ...
        for (uint32_t i = 0; i < nof_antennas; i++) {
          ...
          isrran_rf_set_rx_freq(&rf_devices[dm.device_idx], dm.channel_idx, freq + freq_offset);
        }
      } else {
...
void radio::set_rx_gain(const float& gain)
{
  ...
  for (isrran_rf_t& rf_device : rf_devices) {
    isrran_rf_set_rx_gain(&rf_device, gain);
  }
}
...
void radio::set_rx_gain_th(const float& gain)
{
  ...
  for (isrran_rf_t& rf_device : rf_devices) {
    isrran_rf_set_rx_gain_th(&rf_device, gain);
  }
}
...
void radio::set_rx_srate(const double& srate)
{
  ...
  if (std::isnormal(fix_srate_hz)) {
    ...
    if (not std::isnormal(cur_rx_srate)) {
      for (isrran_rf_t& rf_device : rf_devices) {
        cur_rx_srate = isrran_rf_set_rx_srate(&rf_device, fix_srate_hz);
      }
    }

    // Assert ratio is integer
    isrran_assert(((uint32_t)cur_rx_srate % (uint32_t)srate) == 0,
    ...
    for (uint32_t ch = 0; ch < nof_channels; ch++) {
      isrran_resampler_fft_init(&decimators[ch], ISRRAN_RESAMPLER_MODE_DECIMATE, ratio);
    }

    decimator_busy = false;
  } else {
    for (isrran_rf_t& rf_device : rf_devices) {
      cur_rx_srate = isrran_rf_set_rx_srate(&rf_device, srate);
    }
  }
}
    ...
    if (cur_tx_freqs[device_mapping.carrier_idx] != freq) {
      if ((device_mapping.carrier_idx + 1) * nof_antennas <= nof_channels) {
        ...
        for (uint32_t i = 0; i < nof_antennas; i++) {
          ...
          isrran_rf_set_tx_freq(&rf_devices[device_mapping.device_idx], device_mapping.channel_idx, freq + freq_offset);
        }
      } else {
...
void radio::set_tx_gain(const float& gain)
{
  ...
  for (isrran_rf_t& rf_device : rf_devices) {
    isrran_rf_set_tx_gain(&rf_device, gain);
  }
}
...
float radio::get_rx_gain()
{
  ...
  return (float)isrran_rf_get_rx_gain(&rf_devices[0]);
}
...
double radio::get_dev_cal_tx_adv_sec(const std::string& device_name)
{
  ...
  if (tx_adv_auto) {
    /* This values have been calibrated using the prach_test_usrp tool in isrRAN */
    ...
    isrran::console("Setting manual TX/RX offset to %d samples\n", nsamples);
  }
...
void radio::set_tx_srate(const double& srate)
{
  ...
  if (std::isnormal(fix_srate_hz)) {
    ...
    if (not std::isnormal(cur_tx_srate)) {
      for (isrran_rf_t& rf_device : rf_devices) {
        cur_tx_srate = isrran_rf_set_tx_srate(&rf_device, fix_srate_hz);
      }
    }

    // Assert ratio is integer
    isrran_assert(((uint32_t)cur_tx_srate % (uint32_t)srate) == 0,
                  "The sampling rate ratio is not integer (%.2f MHz / %.2f MHz = %.3f)",
                  cur_rx_srate / 1e6,
                  srate / 1e6,
                  cur_rx_srate / srate);
    ...
    for (uint32_t ch = 0; ch < nof_channels; ch++) {
      isrran_resampler_fft_init(&interpolators[ch], ISRRAN_RESAMPLER_MODE_INTERPOLATE, ratio);
    }
  } else {
    for (isrran_rf_t& rf_device : rf_devices) {
      cur_tx_srate = isrran_rf_set_tx_srate(&rf_device, srate);
    }
  }

  // Get calibrated advanced
  tx_adv_sec = get_dev_cal_tx_adv_sec(std::string(isrran_rf_name(&rf_devices[0])));
...
isrran_rf_info_t* radio::get_info()
{
  if (!is_initialized) {
    return nullptr;
  }
  return isrran_rf_get_info(&rf_devices[0]);
}
...
void radio::handle_rf_msg(isrran_rf_error_t error)
{
  if (!is_initialized) {
    return;
  }
  if (error.type == isrran_rf_error_t::ISRRAN_RF_ERROR_OVERFLOW) {
    {
      std::lock_guard<std::mutex> lock(metrics_mutex);
      rf_metrics.rf_o++;
  ...
  } else if (error.type == isrran_rf_error_t::ISRRAN_RF_ERROR_UNDERFLOW) {
    logger.info("Underflow");
    std::lock_guard<std::mutex> lock(metrics_mutex);
    rf_metrics.rf_u++;
    rf_metrics.rf_error = true;
  } else if (error.type == isrran_rf_error_t::ISRRAN_RF_ERROR_LATE) {
    logger.info("Late (detected in %s)", error.opt ? "rx call" : "asynchronous thread");
    std::lock_guard<std::mutex> lock(metrics_mutex);
    rf_metrics.rf_l++;
    rf_metrics.rf_error = true;
  } else if (error.type == isrran_rf_error_t::ISRRAN_RF_ERROR_RX) {
    logger.error("Fatal radio error occured.");
    phy->radio_failure();
  } else if (error.type == isrran_rf_error_t::ISRRAN_RF_ERROR_OTHER) {
...
void radio::rf_msg_callback(void* arg, isrran_rf_error_t error)
{
  radio* h = (radio*)arg;
  if (arg != nullptr) {
                         ...
                         void*                      radio_buffers[ISRRAN_MAX_CHANNELS])
{
  ...
  // accepts up to ISRRAN_MAX_CHANNELS buffers
