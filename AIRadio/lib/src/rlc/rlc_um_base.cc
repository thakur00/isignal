...
#include "isrran/rlc/rlc_um_base.h"
#include "isrran/interfaces/ue_rrc_interfaces.h"
#include <sstream>

namespace isrran {

rlc_um_base::rlc_um_base(isrlog::basic_logger&      logger,
                         uint32_t                   lcid_,
...
void rlc_um_base::write_sdu(unique_byte_buffer_t sdu)
{
  ...
  if (tx->try_write_sdu(std::move(sdu)) == ISRRAN_SUCCESS) {
    std::lock_guard<std::mutex> lock(metrics_mutex);
    metrics.num_tx_sdus++;
...
std::string rlc_um_base::get_rb_name(isrue::rrc_interface_rlc* rrc, uint32_t lcid, bool is_mrb)
{
  if (is_mrb) {
    std::stringstream ss;
    ss << "MRB" << lcid;
