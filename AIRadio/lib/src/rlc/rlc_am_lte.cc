...
#include "isrran/rlc/rlc_am_lte_packing.h"
...
void rlc_am_lte_tx::check_sn_reached_max_retx(uint32_t sn)
{
  if (tx_window[sn].retx_count == cfg.max_retx_thresh) {
    ...
    isrran::pdcp_sn_vector_t pdcp_sns;
...
int rlc_am_lte_tx::build_segment(uint8_t* payload, uint32_t nof_bytes, rlc_amd_retx_lte_t retx)
{
  ...
  isrran_expect(head_len + (retx.so_end - retx.so_start) <= nof_bytes, "The provided buffer was overflown.");
...
int rlc_am_lte_tx::build_data_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (pdu == NULL) {
    ...
    isrran::console("Fatal Error: Could not allocate PDU in build_data_pdu()\n");
    isrran::console("tx_window size: %zd PDUs\n", tx_window.size());
    isrran::console("vt_a = %d, vt_ms = %d, vt_s = %d, poll_sn = %d\n", vt_a, vt_ms, vt_s, poll_sn);
    isrran::console("retx_queue size: %zd PDUs\n", retx_queue.size());
    std::map<uint32_t, rlc_amd_tx_pdu>::iterator txit;
    for (txit = tx_window.begin(); txit != tx_window.end(); txit++) {
      isrran::console("tx_window - SN=%d\n", txit->first);
    }
    exit(-1);
#else
  ...
  uint32_t to_move   = 0;
  uint32_t last_li   = 0;
  uint32_t pdu_space = ISRRAN_MIN(nof_bytes, pdu->get_tailroom());
  ...
  if (tx_sdu != nullptr) {
    ...
    if (pdu_space > to_move) {
      pdu_space -= ISRRAN_MIN(to_move, pdu->get_tailroom());
    } else {
      pdu_space = 0;
    }
      ...
      if (status.nacks[j].nack_sn == i) {
        ...
        if (tx_window.has_sn(i)) {
          ...
          if (not retx_queue.has_sn(i)) {
            ...
            rlc_amd_retx_lte_t& retx = retx_queue.push();
            isrran_expect(tx_window[i].rlc_sn == i, "Incorrect RLC SN=%d!=%d being accessed", tx_window[i].rlc_sn, i);
            retx.sn         = i;
            retx.is_segment = false;
            retx.so_start   = 0;
            retx.so_end     = pdu.buf->N_bytes;
...
void rlc_am_lte_rx::handle_data_pdu_full(uint8_t* payload, uint32_t nof_bytes, rlc_amd_pdu_header_t& header)
{
  ...
  pdu.buf             = isrran::make_byte_buffer();
  if (pdu.buf == NULL) {
#ifdef RLC_AM_BUFFER_DEBUG
    isrran::console("Fatal Error: Couldn't allocate PDU in handle_data_pdu().\n");
    exit(-1);
#else
...
void rlc_am_lte_rx::handle_data_pdu_segment(uint8_t* payload, uint32_t nof_bytes, rlc_amd_pdu_header_t& header)
{
  ...
  segment.buf = isrran::make_byte_buffer();
  if (segment.buf == NULL) {
#ifdef RLC_AM_BUFFER_DEBUG
    isrran::console("Fatal Error: Couldn't allocate PDU in handle_data_pdu_segment().\n");
    exit(-1);
#else
...
void rlc_am_lte_rx::reassemble_rx_sdus()
{
  uint32_t len = 0;
  if (rx_sdu == NULL) {
    rx_sdu = isrran::make_byte_buffer();
    if (rx_sdu == NULL) {
#ifdef RLC_AM_BUFFER_DEBUG
      isrran::console("Fatal Error: Could not allocate PDU in reassemble_rx_sdus() (1)\n");
      exit(-1);
#else
  ...
  while (rx_window.has_sn(vr_r)) {
    ...
    for (uint32_t i = 0; i < rx_window[vr_r].header.N_li; i++) {
      ...
      if (rx_sdu->get_tailroom() >= len) {
        if ((rx_window[vr_r].buf->msg - rx_window[vr_r].buf->buffer) + len < ISRRAN_MAX_BUFFER_SIZE_BYTES) {
          ...
          if (rx_sdu == nullptr) {
#ifdef RLC_AM_BUFFER_DEBUG
            isrran::console("Fatal Error: Could not allocate PDU in reassemble_rx_sdus() (2)\n");
            exit(-1);
#else
    ...
    if (rlc_am_end_aligned(rx_window[vr_r].header.fi)) {
      ...
      if (rx_sdu == NULL) {
#ifdef RLC_AM_BUFFER_DEBUG
        isrran::console("Fatal Error: Could not allocate PDU in reassemble_rx_sdus() (3)\n");
        exit(-1);
#else
...
bool rlc_am_lte_rx::add_segment_and_check(rlc_amd_rx_pdu_segments_t* pdu, rlc_amd_rx_pdu* segment)
{
  ...
  for (it = pdu->segments.begin(); it != pdu->segments.end(); /* Do not increment */) {
    ...
    if (it->header.so + it->buf->N_bytes <= so) {
      ...
      so = ISRRAN_MAX(so, it->header.so + it->buf->N_bytes);
  ...
  // Copy data
  unique_byte_buffer_t full_pdu = isrran::make_byte_buffer();
  if (full_pdu == NULL) {
#ifdef RLC_AM_BUFFER_DEBUG
    isrran::console("Fatal Error: Could not allocate PDU in add_segment_and_check()\n");
    exit(-1);
#else
