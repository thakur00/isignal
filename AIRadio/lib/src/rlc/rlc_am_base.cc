 ...
 *******************************************************/
rlc_am::rlc_am(isrran_rat_t               rat,
...
bool rlc_am::configure(const rlc_config_t& cfg_)
{
  ...
  if (cfg.rat == isrran_rat_t::lte) {
    RlcInfo("AM LTE configured - t_poll_retx=%d, poll_pdu=%d, poll_byte=%d, max_retx_thresh=%d, "
            ...
            cfg.tx_queue_length);
  } else if (cfg.rat == isrran_rat_t::nr) {
...
void rlc_am::write_sdu(unique_byte_buffer_t sdu)
{
  ...
  if (tx_base->write_sdu(std::move(sdu)) == ISRRAN_SUCCESS) {
...
int rlc_am::rlc_am_base_tx::write_sdu(unique_byte_buffer_t sdu)
{
  ...
  uint32_t                                 nof_bytes = sdu->N_bytes;
  isrran::error_type<unique_byte_buffer_t> ret       = tx_sdu_queue.try_write(std::move(sdu));
  if (ret) {
    RlcHexInfo(msg_ptr,
               nof_bytes,
