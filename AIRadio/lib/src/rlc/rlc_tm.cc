...
rlc_tm::rlc_tm(isrlog::basic_logger&      logger,
               uint32_t                   lcid_,
...
void rlc_tm::write_sdu(unique_byte_buffer_t sdu)
{
  ...
  if (sdu != nullptr) {
    ...
    isrran::error_type<unique_byte_buffer_t> ret       = ul_queue.try_write(std::move(sdu));
    if (ret) {
...
uint32_t rlc_tm::read_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (ul_queue.try_read(&buf)) {
    ...
    RlcHexInfo(payload,
               ...
               isrran::to_string(rlc_mode_t::tm),
               ul_queue.size(),
               ul_queue.size_bytes());
...
void rlc_tm::write_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (buf != nullptr) {
    ...
    if (isrran::srb_to_lcid(isrran::lte_srb::srb0) == lcid) {
