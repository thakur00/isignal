...
rlc_um_lte::rlc_um_lte(isrlog::basic_logger&      logger,
                       uint32_t                   lcid_,
...
bool rlc_um_lte::configure(const rlc_config_t& cnfg_)
{
  ...
  RlcInfo("configured in %s - t_reordering=%d ms, rx_sn_field_length=%u bits, tx_sn_field_length=%u bits",
          isrran::to_string(cnfg_.rlc_mode),
          cfg.um.t_reordering,
          isrran::to_number(cfg.um.rx_sn_field_length),
          isrran::to_number(cfg.um.tx_sn_field_length));
...
uint32_t rlc_um_lte::rlc_um_lte_tx::build_data_pdu(unique_byte_buffer_t pdu, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  int pdu_space = ISRRAN_MIN(nof_bytes, pdu->get_tailroom());
...
void rlc_um_lte::rlc_um_lte_rx::reassemble_rx_sdus()
{
  ...
  while (rx_window.end() != rx_window.find(vr_ur)) {
    ...
    if (rx_sdu->N_bytes < ISRRAN_MAX_BUFFER_SIZE_BYTES &&
        rx_window[vr_ur].buf->N_bytes < ISRRAN_MAX_BUFFER_SIZE_BYTES &&
        rx_window[vr_ur].buf->N_bytes + rx_sdu->N_bytes < ISRRAN_MAX_BUFFER_SIZE_BYTES) {
      RlcHexInfo(rx_window[vr_ur].buf->msg,
                 rx_window[vr_ur].buf->N_bytes,
