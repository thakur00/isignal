...
void rlc_am_read_status_pdu(uint8_t* payload, uint32_t nof_bytes, rlc_status_pdu_t* status)
{
  ...
  isrran_bit_unpack_vector(payload, tmp.msg, nof_bytes * 8);
  tmp.N_bits = nof_bytes * 8;

  rlc_dc_field_t dc = static_cast<rlc_dc_field_t>(isrran_bit_pack(&ptr, 1));

  if (RLC_DC_FIELD_CONTROL_PDU == dc) {
    uint8_t cpt = isrran_bit_pack(&ptr, 3); // 3-bit Control PDU Type (0 == status)
    if (0 == cpt) {
      status->ack_sn = isrran_bit_pack(&ptr, 10); // 10 bits ACK_SN
      ext1           = isrran_bit_pack(&ptr, 1);  // 1 bits E1
      status->N_nack = 0;
      while (ext1) {
        status->nacks[status->N_nack].nack_sn = isrran_bit_pack(&ptr, 10);
        ext1                                  = isrran_bit_pack(&ptr, 1); // 1 bits E1
        ext2                                  = isrran_bit_pack(&ptr, 1); // 1 bits E2
        if (ext2) {
          status->nacks[status->N_nack].has_so   = true;
          status->nacks[status->N_nack].so_start = isrran_bit_pack(&ptr, 15);
          status->nacks[status->N_nack].so_end   = isrran_bit_pack(&ptr, 15);
        }
        status->N_nack++;
      }
    }
...
int rlc_am_write_status_pdu(rlc_status_pdu_t* status, uint8_t* payload)
{
  ...
  isrran_bit_unpack(RLC_DC_FIELD_CONTROL_PDU, &ptr, 1); // D/C
  isrran_bit_unpack(0, &ptr, 3);                        // CPT (0 == STATUS)
  isrran_bit_unpack(status->ack_sn, &ptr, 10);          // 10 bit ACK_SN
  ext1 = (status->N_nack == 0) ? 0 : 1;
  isrran_bit_unpack(ext1, &ptr, 1); // E1
  for (i = 0; i < status->N_nack; i++) {
    isrran_bit_unpack(status->nacks[i].nack_sn, &ptr, 10); // 10 bit NACK_SN
    ext1 = ((status->N_nack - 1) == i) ? 0 : 1;
    isrran_bit_unpack(ext1, &ptr, 1); // E1
    if (status->nacks[i].has_so) {
      isrran_bit_unpack(1, &ptr, 1); // E2
      isrran_bit_unpack(status->nacks[i].so_start, &ptr, 15);
      isrran_bit_unpack(status->nacks[i].so_end, &ptr, 15);
    } else {
      isrran_bit_unpack(0, &ptr, 1); // E2
    }
  }
  ...
  isrran_bit_unpack(0, &ptr, n_pad);
  tmp.N_bits = ptr - tmp.msg;

  // Pack bits
  isrran_bit_pack_vector(tmp.msg, payload, tmp.N_bits);
  return tmp.N_bits / 8;
}
