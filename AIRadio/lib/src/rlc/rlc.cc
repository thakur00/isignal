...
#include "isrran/rlc/rlc_am_base.h"
#include "isrran/rlc/rlc_tm.h"
#include "isrran/rlc/rlc_um_lte.h"
#include "isrran/rlc/rlc_um_nr.h"

namespace isrran {

rlc::rlc(const char* logname) : logger(isrlog::fetch_basic_logger(logname)), pool(byte_buffer_pool::get_instance())
{
...
void rlc::init(isrue::pdcp_interface_rlc* pdcp_,
...
uint32_t rlc::read_pdu(uint32_t lcid, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  isrran_expect(ret <= nof_bytes, "Created too big RLC PDU (%d > %d)", ret, nof_bytes);
...
uint32_t rlc::read_pdu_mch(uint32_t lcid, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  isrran_expect(ret <= nof_bytes, "Created too big RLC PDU for MCH (%d > %d)", ret, nof_bytes);
...
void rlc::write_pdu_bcch_bch(isrran::unique_byte_buffer_t pdu)
{
...
// Pass directly to PDCP, no DL througput counting done
void rlc::write_pdu_pcch(isrran::unique_byte_buffer_t pdu)
{
