...
uint32_t rlc_am_nr_tx::read_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  if (do_status()) {
    unique_byte_buffer_t tx_pdu = isrran::make_byte_buffer();
    if (tx_pdu == nullptr) {
...
uint32_t rlc_am_nr_tx::build_new_pdu(uint8_t* payload, uint32_t nof_bytes)
{
  ...
  tx_pdu.sdu_buf            = isrran::make_byte_buffer();
...
uint32_t rlc_am_nr_tx::build_new_sdu_segment(rlc_amd_tx_pdu_nr& tx_pdu, uint8_t* payload, uint32_t nof_bytes)
{
  ...
  // Copy PDU to payload
  isrran_assert((hdr_len + segment_payload_len) <= nof_bytes, "Error calculating hdr_len and segment_payload_len");
...
{
  isrran_assert(tx_window->has_sn(retx.sn), "Called %s without checking retx SN", __FUNCTION__);
  isrran_assert(not is_retx_segmentation_required(retx, nof_bytes),
  ...
  isrran_assert(nof_bytes >= (expected_hdr_len + retx_payload_len),
  ...
  isrran_assert(pdu_bytes <= nof_bytes, "Error calculating hdr_len and pdu_payload_len");
...
uint32_t rlc_am_nr_tx::build_retx_pdu_with_segmentation(rlc_amd_retx_nr_t& retx, uint8_t* payload, uint32_t nof_bytes)
{
  // Get tx_pdu info from tx_window
  isrran_assert(tx_window->has_sn(retx.sn), "Called %s without checking retx SN", __FUNCTION__);
  isrran_assert(is_retx_segmentation_required(retx, nof_bytes),
  ...
  isrran_assert((hdr_len + retx_pdu_payload_size) <= nof_bytes, "Error calculating hdr_len and segment_payload_len");
  memcpy(&payload[hdr_len], &tx_pdu.sdu_buf->msg[retx.current_so], retx_pdu_payload_size);
...
int rlc_am_nr_rx::handle_full_data_sdu(const rlc_am_nr_pdu_header_t& header, const uint8_t* payload, uint32_t nof_bytes)
{
  ...
  rx_sdu.buf                  = isrran::make_byte_buffer();
                                          ...
                                          uint32_t                      nof_bytes)
{
  ...
  pdu_segment.buf               = isrran::make_byte_buffer();
    ...
    if ((rx_window->has_sn(i) && (*rx_window)[i].fully_received)) {
      ...
      if (not rx_window->has_sn(i)) {
          ...
          if (segm->header.so != last_so) {
            ...
            if (nack.so_start > nack.so_end) {
              ...
              RlcError("Error: SO_start=%d > SO_end=%d. NACK_SN=%d. SO_start=%d, SO_end=%d, seg.so=%d",
                       ...
                       nack.so_end,
                       segm->header.so);
              isrran_assert(nack.so_start <= nack.so_end,
        ...
        if (not last_segment_rx) {
          ...
          isrran_assert(nack.so_start <= nack.so_end, "Error: SO_start > SO_end. NACK_SN=%d", nack.nack_sn);
        }
      }
    }
  } // NACK loop
...
void rlc_am_nr_rx::timer_expired(uint32_t timeout_id)
{
  ...
  if (reassembly_timer.is_valid() && reassembly_timer.id() == timeout_id) {
    ...
    isrran_assert(valid_ack_sn(st.rx_highest_status), "Error: rx_highest_status assigned outside rx window");
