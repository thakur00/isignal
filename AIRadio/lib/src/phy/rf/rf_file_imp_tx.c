...
int rf_file_tx_open(rf_file_tx_t* q, rf_file_opts_t opts)
{
  ...
  if (q) {
    ...
    q->temp_buffer_convert = isrran_vec_malloc(FILE_MAX_BUFFER_SIZE);
    ...
    q->zeros = isrran_vec_malloc(FILE_MAX_BUFFER_SIZE);
    if (!q->zeros) {
...
static int _rf_file_tx_baseband(rf_file_tx_t* q, cf_t* buffer, uint32_t nsamples)
{
  ...
  if (q->sample_format == FILERF_TYPE_SC16) {
    ...
    isrran_vec_convert_fi((float*)buffer, INT16_MAX, (short*)q->temp_buffer_convert, 2 * nsamples);
  }
...
int rf_file_tx_baseband(rf_file_tx_t* q, cf_t* buffer, uint32_t nsamples)
{
  ...
  if (q->sample_offset > 0) {
    ...
    n = ISRRAN_MIN(-q->sample_offset, nsamples);
    buffer += n;
    nsamples -= n;
    q->sample_offset += n;
    if (nsamples == 0) {
