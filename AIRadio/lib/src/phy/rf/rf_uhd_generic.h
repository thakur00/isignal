...
#ifndef ISRRAN_RF_UHD_GENERIC_H
#define ISRRAN_RF_UHD_GENERIC_H
...
private:
  ...
  uhd_error usrp_make_internal(const uhd::device_addr_t& dev_addr) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp = uhd::usrp::multi_usrp::make(dev_addr);)
  }

  uhd_error set_tx_subdev(const std::string& string)
  {
    Info("Setting tx_subdev_spec to '" << string << "'");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_tx_subdev_spec(string);)
  }
  uhd_error set_rx_subdev(const std::string& string)
  {
    Info("Setting rx_subdev_spec to '" << string << "'");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_rx_subdev_spec(string);)
  }
...
public:
  ...
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(mboard_name = usrp->get_mboard_name();)
  }
  uhd_error get_mboard_sensor_names(std::vector<std::string>& sensors) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(sensors = usrp->get_mboard_sensor_names();)
  }
  uhd_error get_rx_sensor_names(std::vector<std::string>& sensors) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(sensors = usrp->get_rx_sensor_names();)
  }
  uhd_error get_sensor(const std::string& sensor_name, double& sensor_value) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(sensor_value = usrp->get_mboard_sensor(sensor_name).to_real();)
  }
  uhd_error get_sensor(const std::string& sensor_name, bool& sensor_value) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(sensor_value = usrp->get_mboard_sensor(sensor_name).to_bool();)
  }
  uhd_error get_rx_sensor(const std::string& sensor_name, bool& sensor_value) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(sensor_value = usrp->get_rx_sensor(sensor_name).to_bool();)
  }
  uhd_error set_time_unknown_pps(const uhd::time_spec_t& timespec) override
  {
    Debug("Setting Time at next PPS...");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_time_unknown_pps(timespec);)
  }
  uhd_error get_time_now(uhd::time_spec_t& timespec) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(timespec = usrp->get_time_now();)
  }
  uhd_error set_sync_source(const std::string& sync_source, const std::string& clock_source) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_clock_source(clock_source); usrp->set_time_source(sync_source);)
#else
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_sync_source(clock_source, sync_source);)
#endif
  }
  uhd_error get_gain_range(uhd::gain_range_t& tx_gain_range, uhd::gain_range_t& rx_gain_range) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(tx_gain_range = usrp->get_tx_gain_range(); rx_gain_range = usrp->get_rx_gain_range();)
  }
  uhd_error set_master_clock_rate(double rate) override
  {
    Debug("Setting master clock rate to " << rate / 1e6 << " MHz");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_master_clock_rate(rate);)
  }
  uhd_error set_rx_rate(double rate) override
  {
    Debug("Setting Rx Rate to " << rate / 1e6 << "MHz");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_rx_rate(rate);)
  }
  uhd_error set_tx_rate(double rate) override
  {
    Debug("Setting Tx Rate to " << rate / 1e6 << "MHz");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_tx_rate(rate);)
  }
  uhd_error set_command_time(const uhd::time_spec_t& timespec) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_command_time(timespec);)
  }
  uhd_error get_rx_stream(size_t& max_num_samps) override
  {
    Debug("Creating Rx stream");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(rx_stream = nullptr; rx_stream = usrp->get_rx_stream(stream_args);
  ...
  uhd_error get_tx_stream(size_t& max_num_samps) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(tx_stream = nullptr; tx_stream = usrp->get_tx_stream(stream_args);
  ...
  uhd_error set_tx_gain(size_t ch, double gain) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_tx_gain(gain, ch);)
  }
  uhd_error set_rx_gain(size_t ch, double gain) override
  {
    Debug("Setting channel " << ch << " Rx gain to " << gain << " dB");
    ISRRAN_UHD_SAFE_C_LOG_ERROR(usrp->set_rx_gain(gain, ch);)
  }
  uhd_error get_rx_gain(double& gain) override { ISRRAN_UHD_SAFE_C_LOG_ERROR(gain = usrp->get_rx_gain();) }
  uhd_error get_tx_gain(double& gain) override { ISRRAN_UHD_SAFE_C_LOG_ERROR(gain = usrp->get_tx_gain();) }
  uhd_error set_tx_freq(uint32_t ch, double target_freq, double& actual_freq) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(uhd::tune_result_t tune_result = usrp->set_tx_freq(tune_request, ch);
  ...
  uhd_error set_rx_freq(uint32_t ch, double target_freq, double& actual_freq) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(uhd::tune_result_t tune_result = usrp->set_rx_freq(tune_request, ch);
                                actual_freq                    = tune_result.target_rf_freq;)
  }
};

#endif // ISRRAN_RF_UHD_GENERIC_H
