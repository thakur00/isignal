...
#ifndef ISRRAN_RF_UHD_RFNOC_H
#define ISRRAN_RF_UHD_RFNOC_H
...
public:
  ...
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(if (device3->get_tree()->exists(TREE_MBOARD_SENSORS)) {
  ...
  uhd_error set_time_unknown_pps(const uhd::time_spec_t& timespec) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(for (auto& r : radio_ctrl) { r->set_time_next_pps(timespec); });
  }
  uhd_error get_time_now(uhd::time_spec_t& timespec) override
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(timespec = device3->get_tree()->access<uhd::time_spec_t>(TREE_TIME_NOW).get();
  ...
  uhd_error set_sync_source(const std::string& sync_source, const std::string& clock_source) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(for (size_t radio_idx = 0; radio_idx < nof_radios; radio_idx++) {
  ...
  {
    ISRRAN_UHD_SAFE_C_LOG_ERROR(for (size_t i = 0; i < nof_radios; i++) {
      for (size_t j = 0; j < nof_channels; j++) {
  ...
  uhd_error get_tx_gain(double& gain) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(gain = radio_ctrl[0]->get_tx_gain(0);)
  }
  uhd_error get_rx_gain(double& gain) override
  {
    ...
    ISRRAN_UHD_SAFE_C_LOG_ERROR(gain = radio_ctrl[0]->get_rx_gain(0);)
  }
