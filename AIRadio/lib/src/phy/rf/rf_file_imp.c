...
typedef struct {
  ...
  // FILEs
  rf_file_tx_t transmitter[ISRRAN_MAX_CHANNELS];
  rf_file_rx_t receiver[ISRRAN_MAX_CHANNELS];
  bool         close_files;
...
void rf_file_register_error_handler(void* h, isrran_rf_error_handler_t error_handler, void* arg)
{
  // do nothing
}
...
int rf_file_open_multi(char* args, void** h, uint32_t nof_channels)
{
  ...
  FILE* rx_files[ISRRAN_MAX_CHANNELS] = {NULL};
  FILE* tx_files[ISRRAN_MAX_CHANNELS] = {NULL};
    ...
    *h                        = handler;
    ...
    for (int i = 0; i < handler->nof_channels; i++) {
      if (rx_files != NULL && rx_files[i] != NULL) {
        ...
        if (rf_file_rx_open(&handler->receiver[i], rx_opts) != ISRRAN_SUCCESS) {
      ...
      if (tx_files != NULL && tx_files[i] != NULL) {
        ...
        if (rf_file_tx_open(&handler->transmitter[i], tx_opts) != ISRRAN_SUCCESS) {
    ...
    for (uint32_t i = 0; i < handler->nof_channels; i++) {
      handler->buffer_decimation[i] = isrran_vec_malloc(FILE_MAX_BUFFER_SIZE);
    ...
    handler->buffer_tx = isrran_vec_malloc(FILE_MAX_BUFFER_SIZE);
...
isrran_rf_info_t* rf_file_get_info(void* h)
{
                                 ...
                                 double*  frac_secs)
{
  ...
  if (h) {
    ...
    isrran_timestamp_init_uint64(&ts_tx, handler->transmitter[0].nsamples, handler->base_srate);
...
rf_dev_t isrran_rf_dev_file = {"file",
                               rf_file_devname,
                               ...
                               rf_file_open,
                               .isrran_rf_open_multi = rf_file_open_multi,
                               rf_file_close,
                               ...
                               rf_file_send_timed,
                               .isrran_rf_send_timed_multi = rf_file_send_timed_multi};
