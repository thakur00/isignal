...
void* ue_rx_thread_function(void* args)
{
  ...
  if (isrran_rf_open_devname(&ue_radio, "zmq", rf_args, NOF_RX_ANT)) {
...
void enb_tx_function(const char* tx_args, bool timed_tx)
{
  ...
  if (isrran_rf_open_devname(&enb_radio, "zmq", rf_args, NOF_RX_ANT)) {
    fprintf(stderr, "Error opening rf\n");
    exit(-1);
  }
...
int param_test(const char* args_param, const int num_channels)
{
  ...
  printf("opening tx device with args=%s\n", rf_args);
  if (isrran_rf_open_devname(&enb_radio, "zmq", rf_args, num_channels)) {
    fprintf(stderr, "Error opening rf\n");
    return ISRRAN_ERROR;
  }
