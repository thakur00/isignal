...
typedef struct {
  ...
  double   rx_gain;
  double   tx_gain;
  uint32_t tx_freq_mhz[ISRRAN_MAX_CHANNELS];
  uint32_t rx_freq_mhz[ISRRAN_MAX_CHANNELS];
  bool     tx_off;
  char     id[RF_PARAM_LEN];

  // Server
  void*       context;
  rf_zmq_tx_t transmitter[ISRRAN_MAX_CHANNELS];
  rf_zmq_rx_t receiver[ISRRAN_MAX_CHANNELS];

  // Various sample buffers
  cf_t* buffer_decimation[ISRRAN_MAX_CHANNELS];
  cf_t* buffer_tx;
...
static inline int update_ts(void* h, uint64_t* ts, int nsamples, const char* dir)
{
  ...
  if (h && nsamples > 0) {
    ...
    isrran_timestamp_t _ts = {};
    isrran_timestamp_init_uint64(&_ts, *ts, handler->base_srate);
    rf_zmq_info(
...
void rf_zmq_register_error_handler(void* h, isrran_rf_error_handler_t new_handler, void* arg)
{
  // do nothing
}
...
{
  int ret = ISRRAN_ERROR;
  if (h && nof_channels < ISRRAN_MAX_CHANNELS) {
    *h = NULL;
    ...
    *h                  = handler;
    ...
    if (args && strlen(args)) {
      ...
      if (parse_string(args, "rx_type", -1, tmp) == ISRRAN_SUCCESS) {
        if (!strcmp(tmp, "sub")) {
      ...
      if (parse_string(args, "rx_format", -1, tmp) == ISRRAN_SUCCESS) {
        if (!strcmp(tmp, "sc16")) {
      ...
      // tx_type
      if (parse_string(args, "tx_type", -1, tmp) == ISRRAN_SUCCESS) {
        if (!strcmp(tmp, "pub")) {
      ...
      if (parse_string(args, "tx_format", -1, tmp) == ISRRAN_SUCCESS) {
        if (!strcmp(tmp, "sc16")) {
    ...
    for (int i = 0; i < handler->nof_channels; i++) {
      ...
      if (strlen(tx_port) != 0) {
        if (rf_zmq_tx_open(&handler->transmitter[i], tx_opts, handler->context, tx_port) != ISRRAN_SUCCESS) {
      ...
      // initialize receiver
      if (strlen(rx_port) != 0) {
        if (rf_zmq_rx_open(&handler->receiver[i], rx_opts, handler->context, rx_port) != ISRRAN_SUCCESS) {
    ...
    for (uint32_t i = 0; i < handler->nof_channels; i++) {
      handler->buffer_decimation[i] = isrran_vec_malloc(ZMQ_MAX_BUFFER_SIZE);
    ...
    handler->buffer_tx = isrran_vec_malloc(ZMQ_MAX_BUFFER_SIZE);
...
isrran_rf_info_t* rf_zmq_get_info(void* h)
{
  isrran_rf_info_t* info = NULL;
  if (h) {
    rf_zmq_handler_t* handler = (rf_zmq_handler_t*)h;
    info                      = &handler->info;
  }
...
int rf_zmq_recv_with_time_multi(void* h, void** data, uint32_t nsamples, bool blocking, time_t* secs, double* frac_secs)
{
  ...
  if (h) {
    ...
    pthread_mutex_lock(&handler->rx_config_mutex);
    bool  mapped[ISRRAN_MAX_CHANNELS]  = {}; // Mapped mask, set to true when the physical channel is used
    cf_t* buffers[ISRRAN_MAX_CHANNELS] = {}; // Buffer pointers, NULL if unmatched
    ...
    for (uint32_t logical = 0; logical < handler->nof_channels; logical++) {
      ...
      if (unmatched) {
        isrran_vec_zero(data[logical], nsamples);
      }
    }
    pthread_mutex_unlock(&handler->rx_config_mutex);
    ...
    if (secs != NULL && frac_secs != NULL) {
      isrran_timestamp_t ts = {};
      isrran_timestamp_init_uint64(&ts, handler->next_rx_ts, handler->base_srate);
      *secs      = ts.full_secs;
    ...
    // receive samples
    isrran_timestamp_t ts_tx = {}, ts_rx = {};
    isrran_timestamp_init_uint64(&ts_tx, rf_zmq_tx_get_nsamples(&handler->transmitter[0]), handler->base_srate);
    isrran_timestamp_init_uint64(&ts_rx, handler->next_rx_ts, handler->base_srate);
    ...
    int32_t count[ISRRAN_MAX_CHANNELS] = {};
    while (!completed) {
      ...
      for (uint32_t i = 0; i < handler->nof_channels; i++) {
        ...
        if (count[i] < nsamples_baserate && rf_zmq_rx_is_running(&handler->receiver[i])) {
          ...
          if (n > ISRRAN_SUCCESS) {
            // No error
            count[i] += n;
          } else if (n == ISRRAN_ERROR_TIMEOUT) {
    ...
    rf_zmq_info(handler->id,
                ...
                NBYTES2NSAMPLES(isrran_ringbuffer_status(&handler->receiver[0].ringbuffer)));
    ...
    // Set gain
    pthread_mutex_lock(&handler->rx_gain_mutex);
    float scale = isrran_convert_dB_to_amplitude(handler->rx_gain);
    pthread_mutex_unlock(&handler->rx_gain_mutex);
    ...
    for (uint32_t c = 0; c < handler->nof_channels; c++) {
      if (buffers[c]) {
        isrran_vec_sc_prod_cfc(buffers[c], scale, buffers[c], nsamples);
      }
    }
                            ...
                            bool   is_end_of_burst)
{
  ...
  if (h && data && nsamples > 0) {
    ...
    pthread_mutex_lock(&handler->tx_config_mutex);
    bool  mapped[ISRRAN_MAX_CHANNELS]  = {}; // Mapped mask, set to true when the physical channel is used
    cf_t* buffers[ISRRAN_MAX_CHANNELS] = {}; // Buffer pointers, NULL if unmatched or zero transmission
    ...
    float tx_gain = isrran_convert_dB_to_amplitude(handler->tx_gain);
    ...
    if (has_time_spec) {
      ...
      isrran_timestamp_t ts = {};
      isrran_timestamp_init(&ts, secs, frac_secs);
      uint64_t tx_ts              = isrran_timestamp_uint64(&ts, handler->base_srate);
    ...
    for (int i = 0; i < handler->nof_channels; i++) {
      if (buffers[i] != NULL) {
        ...
        isrran_vec_sc_prod_cfc(buf, tx_gain, buf, nsamples_baseband);
...
rf_dev_t isrran_rf_dev_zmq = {"zmq",
                              rf_zmq_devname,
                              ...
                              rf_zmq_open,
                              .isrran_rf_open_multi = rf_zmq_open_multi,
                              rf_zmq_close,
                              rf_zmq_set_rx_srate,
                              rf_zmq_set_rx_gain,
                              ...
                              rf_zmq_send_timed,
                              .isrran_rf_send_timed_multi = rf_zmq_send_timed_multi};
