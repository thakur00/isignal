...
struct rf_uhd_handler_t {
  ...
  std::array<double, ISRRAN_MAX_CHANNELS> tx_freq             = {};
  std::array<double, ISRRAN_MAX_CHANNELS> rx_freq             = {};
  double                                  cur_rx_gain_ch0     = 0;

  std::mutex                                                 tx_gain_mutex;
  std::array<std::pair<double, double>, ISRRAN_MAX_CHANNELS> tx_gain_db = {};

  isrran_rf_error_handler_t                 uhd_error_handler     = nullptr;
...
static void log_overflow(rf_uhd_handler_t* h)
{
  ...
  if (h->uhd_error_handler != nullptr) {
    isrran_rf_error_t error;
    bzero(&error, sizeof(isrran_rf_error_t));
    error.type = isrran_rf_error_t::ISRRAN_RF_ERROR_OVERFLOW;
...
static void log_late(rf_uhd_handler_t* h, bool is_rx)
{
  ...
  if (h->uhd_error_handler != nullptr) {
    ...
    bzero(&error, sizeof(isrran_rf_error_t));
    error.opt  = is_rx ? 1 : 0;
    error.type = isrran_rf_error_t::ISRRAN_RF_ERROR_LATE;
    h->uhd_error_handler(h->uhd_error_handler_arg, error);
  }
}
...
static void log_underflow(rf_uhd_handler_t* h)
{
  ...
  if (h->uhd_error_handler != nullptr) {
    isrran_rf_error_t error;
    bzero(&error, sizeof(isrran_rf_error_t));
    error.type = isrran_rf_error_t::ISRRAN_RF_ERROR_UNDERFLOW;
    h->uhd_error_handler(h->uhd_error_handler_arg, error);
  }
}
#endif
...
static void log_rx_error(rf_uhd_handler_t* h)
{
  if (h->uhd_error_handler) {
    isrran_rf_error_t error;
    bzero(&error, sizeof(isrran_rf_error_t));
    error.type = isrran_rf_error_t::ISRRAN_RF_ERROR_RX;
    h->uhd_error_handler(h->uhd_error_handler_arg, error);
  }
}
...
void rf_uhd_register_error_handler(void* h, isrran_rf_error_handler_t new_handler, void* arg)
{
  rf_uhd_handler_t* handler      = (rf_uhd_handler_t*)h;
...
bool rf_uhd_rx_wait_lo_locked(void* h)
{
  ...
  if (not is_locked and error == ISRRAN_SUCCESS) {
...
int rf_uhd_stop_rx_stream(void* h)
{
  ...
  if (rf_uhd_stop_rx_stream_nolock(handler) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
...
void rf_uhd_flush_buffer(void* h)
{
  ...
  do {
    uint32_t nsamples = ISRRAN_MIN(handler->rx_nof_samples, (uint32_t)dummy_mem.size());
...
int rf_uhd_open_multi(char* args, void** h, uint32_t nof_channels)
{
  ...
  if (nof_channels > ISRRAN_MAX_CHANNELS) {
    ERROR("Error opening UHD: maximum number of channels exceeded (%d>%d)", nof_channels, ISRRAN_MAX_CHANNELS);
    return ISRRAN_ERROR;
  }
  ...
  *h = handler;
  ...
  if (uhd_init(handler, args, nof_channels) < ISRRAN_SUCCESS) {
...
static inline int rf_uhd_imp_end_burst(rf_uhd_handler_t* handler)
{
  ...
  void*              buffs_ptr[ISRRAN_MAX_CHANNELS] = {};
  size_t             txd_samples                    = 0;

  // Set buffer pointers
  for (int i = 0; i < ISRRAN_MAX_CHANNELS; i++) {
...
double rf_uhd_set_tx_srate(void* h, double freq)
{
  ...
  if (handler->uhd->is_tx_ready() and handler->tx_state != RF_UHD_IMP_TX_STATE_START_BURST) {
    if (rf_uhd_imp_end_burst(handler) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
...
int rf_uhd_set_tx_gain(void* h, double gain)
{
  ...
  for (size_t i = 0; i < handler->nof_tx_channels; i++) {
    if (rf_uhd_set_tx_gain_ch(h, i, gain) < ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  return ISRRAN_SUCCESS;
...
isrran_rf_info_t* rf_uhd_get_info(void* h)
{
  isrran_rf_info_t* info = nullptr;
                                ...
                                double*  frac_secs)
{
  ...
  if (not handler->rx_stream_enabled) {
    if (rf_uhd_start_rx_stream_nolock(handler) != ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
  ...
  while (rxd_samples_total < nsamples and trials < RF_UHD_IMP_MAX_RX_TRIALS) {
    ...
    size_t num_rx_samples = ISRRAN_MIN(handler->rx_nof_samples, num_samps_left);
    ...
    for (uint32_t i = 0; i < handler->nof_rx_channels; i++) {
      if (data[i] != nullptr) {
        ...
        num_rx_samples = ISRRAN_MIN(num_rx_samples, (uint32_t)dummy_mem.size());
      }
    }
                      ...
                      bool   is_end_of_burst)
{
  ...
  void* _data[ISRRAN_MAX_CHANNELS] = {};
                            ...
                            bool   is_end_of_burst)
{
  ...
  cf_t* data_c[ISRRAN_MAX_CHANNELS] = {};
...
rf_dev_t isrran_rf_dev_uhd = {"UHD",
                              rf_uhd_devname,
