...
static isrran_rf_error_handler_t blade_error_handler     = NULL;
...
void rf_blade_register_error_handler(UNUSED void* ptr, isrran_rf_error_handler_t new_handler, void* arg)
{
...
isrran_rf_info_t* rf_blade_get_info(void* h)
{
                            ...
                            double*     frac_secs)
{
  ...
  timestamp_to_secs(handler->rx_rate, meta.timestamp, secs, frac_secs);
  isrran_vec_convert_if(handler->rx_buffer, 2048, data, 2 * nsamples);
                        ...
                        bool        is_end_of_burst)
{
  ...
  isrran_vec_convert_fi(data, 2048, handler->tx_buffer, 2 * nsamples);

  memset(&meta, 0, sizeof(meta));
  if (is_start_of_burst) {
    if (has_time_spec) {
      // Convert time to ticks
      isrran_timestamp_t ts = {.full_secs = secs, .frac_secs = frac_secs};
      meta.timestamp        = isrran_timestamp_uint64(&ts, handler->tx_rate);
    } else {
...
rf_dev_t isrran_rf_dev_blade = {"bladeRF",
                                rf_blade_devname,
                                ...
                                rf_blade_open,
                                .isrran_rf_open_multi = rf_blade_open_multi,
                                rf_blade_close,
                                ...
                                rf_blade_send_timed,
                                .isrran_rf_send_timed_multi = rf_blade_send_timed_multi};
