...
typedef struct {
  ...
  isrran_rf_error_handler_t soapy_error_handler;
...
static void log_underflow(rf_soapy_handler_t* h)
{
  if (h->soapy_error_handler) {
    isrran_rf_error_t error;
    bzero(&error, sizeof(isrran_rf_error_t));
    error.type = ISRRAN_RF_ERROR_UNDERFLOW;
    h->soapy_error_handler(h->soapy_error_handler_arg, error);
  } else {
    h->num_underflows++;
  }
...
void rf_soapy_register_error_handler(void* h, isrran_rf_error_handler_t new_handler, void* arg)
{
  rf_soapy_handler_t* handler      = (rf_soapy_handler_t*)h;
...
int rf_soapy_open_multi(char* args, void** h, uint32_t num_requested_channels)
{
  ...
  dev_id = ISRRAN_MIN(dev_id, length - 1);
  ...
  if (devname != NULL && strstr(devname, "Lime") != NULL && isrran_symbol_size_is_standard() == false) {
    printf("\033[0;31mDetected LimeSDR. Consider using LTE rates for better RF performance.\nEither compile with "
           "\'-DUSE_LTE_RATES=True\' or start isrENB/isrUE with \'--expert.lte_sample_rates=true\'\033[0m\n");
  }
  ...
  *h                        = handler;
  ...
  if ((num_available_channels > 0) && (num_requested_channels > 0)) {
    handler->num_rx_channels = ISRRAN_MIN(num_available_channels, num_requested_channels);
  ...
  if ((num_available_channels > 0) && (num_requested_channels > 0)) {
    handler->num_tx_channels = ISRRAN_MIN(num_available_channels, num_requested_channels);
...
double rf_soapy_set_rx_srate(void* h, double rate)
{
  ...
  for (uint32_t i = 0; i < handler->num_rx_channels; i++) {
    ...
    for (int k = 0; k < bw_length; ++k) {
      double bw = rate * 0.75;
      bw        = ISRRAN_MIN(bw, bw_range[k].maximum);
      bw        = ISRRAN_MAX(bw, bw_range[k].minimum);
...
isrran_rf_info_t* rf_soapy_get_info(void* h)
{
                                  ...
                                  double*  frac_secs)
{
  ...
  do {
    size_t rx_samples = ISRRAN_MIN(nsamples - n, handler->rx_mtu);
#if PRINT_RX_STATS
    printf(" - rx_samples=%zd\n", rx_samples);
#endif

    void* buffs_ptr[ISRRAN_MAX_PORTS] = {};
    for (int i = 0; i < handler->num_rx_channels; i++) {
...
{
  void* data_multi[ISRRAN_MAX_PORTS] = {NULL};
  data_multi[0]                      = data;
...
{
  void* _data[ISRRAN_MAX_PORTS] = {data, zero_mem, zero_mem, zero_mem};
                              ...
                              bool   is_end_of_burst)
{
  ...
  do {
#if USE_TX_MTU
    size_t tx_samples = ISRRAN_MIN(nsamples - n, handler->tx_mtu);
#else
    ...
    const void* buffs_ptr[ISRRAN_MAX_PORTS] = {};
    for (int i = 0; i < handler->num_tx_channels; i++) {
...
rf_dev_t isrran_rf_dev_soapy = {"soapy",
                                rf_soapy_devname,
                                ...
                                rf_soapy_send_timed,
                                .isrran_rf_send_timed_multi = rf_soapy_send_timed_multi};
