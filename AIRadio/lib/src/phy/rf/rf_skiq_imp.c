 ...
 * @return ISRRAN_SUCCESS if it is possible to synchronize boards, ISRRAN_ERROR otherwise
 */
  ...
  do {
    ...
    for (int i = 0; i < h->nof_cards; i++) {
      ...
      if (rf_skiq_card_update_timestamp(
              &h->cards[i], do_1pps, h->cards->rx_ports->rb_tstamp_rem + h->current_srate_hz) != ISRRAN_SUCCESS) {
        return ISRRAN_ERROR;
      }
    }
...
void rf_skiq_register_error_handler(void* h_, isrran_rf_error_handler_t error_handler, void* arg)
{
...
isrran_rf_info_t* rf_skiq_get_info(void* h_)
{
  ...
  *ts_start = 0;
  while (nsamples > 0) {
    ...
    int32_t n = rf_skiq_card_receive(
        &h->cards[card], port, h->dummy_buffer, ISRRAN_MIN(nsamples, RF_SKIQ_DUMMY_BUFFER_SIZE), &ts);
...
static int rf_skiq_synch_rx_ports(rf_skiq_handler_t* h)
{
  ...
  for (uint32_t card = 0; card < h->nof_cards; card++) {
    ...
    for (uint32_t port = 0; port < h->nof_ports; port++) {
      ...
      // Is minimum?
      tstamp_min = ISRRAN_MIN(tstamp_min, ts);

      // Is maximum?
      tstamp_max = ISRRAN_MAX(tstamp_max, ts);
    }
  }
  ...
  for (uint32_t card = 0; card < h->nof_cards; card++) {
    ...
    for (uint32_t port = 0; port < h->nof_ports; port++) {
      ...
      ts = 0;
      if (rf_skiq_discard_rx_samples(h, card, port, nsamples, &ts) < ISRRAN_SUCCESS) {
                                 ...
                                 double*  frac_secs)
{
        ...
        if (pending > 0) {
          ...
          if (data[chan] == NULL) {
            ...
            n = rf_skiq_card_receive(
                &h->cards[card], port, h->dummy_buffer, ISRRAN_MIN(pending, RF_SKIQ_DUMMY_BUFFER_SIZE), &ts);
          } else {
            // ... read base-band
  ...
  // Convert u64 to isrran timestamp
...
rf_dev_t isrran_rf_dev_skiq = {.name                             = "Sidekiq",
                               .isrran_rf_devname                = rf_skiq_devname,
                               .isrran_rf_start_rx_stream        = rf_skiq_start_rx_stream,
                               .isrran_rf_stop_rx_stream         = rf_skiq_stop_rx_stream,
                               .isrran_rf_flush_buffer           = rf_skiq_flush_buffer,
                               .isrran_rf_has_rssi               = rf_skiq_has_rssi,
                               .isrran_rf_get_rssi               = rf_skiq_get_rssi,
                               .isrran_rf_suppress_stdout        = rf_skiq_suppress_stdout,
                               .isrran_rf_register_error_handler = rf_skiq_register_error_handler,
                               .isrran_rf_open                   = rf_skiq_open,
                               .isrran_rf_open_multi             = rf_skiq_open_multi,
                               .isrran_rf_close                  = rf_skiq_close,
                               .isrran_rf_set_rx_srate           = rf_skiq_set_rx_srate,
                               .isrran_rf_set_tx_srate           = rf_skiq_set_tx_srate,
                               .isrran_rf_set_rx_gain            = rf_skiq_set_rx_gain,
                               .isrran_rf_set_tx_gain            = rf_skiq_set_tx_gain,
                               .isrran_rf_set_tx_gain_ch         = rf_skiq_set_tx_gain_ch,
                               .isrran_rf_set_rx_gain_ch         = rf_skiq_set_rx_gain_ch,
                               .isrran_rf_get_rx_gain            = rf_skiq_get_rx_gain,
                               .isrran_rf_get_tx_gain            = rf_skiq_get_tx_gain,
                               .isrran_rf_get_info               = rf_skiq_get_info,
                               .isrran_rf_set_rx_freq            = rf_skiq_set_rx_freq,
                               .isrran_rf_set_tx_freq            = rf_skiq_set_tx_freq,
                               .isrran_rf_get_time               = rf_skiq_get_time,
                               .isrran_rf_recv_with_time         = rf_skiq_recv_with_time,
                               .isrran_rf_recv_with_time_multi   = rf_skiq_recv_with_time_multi,
                               .isrran_rf_send_timed             = rf_skiq_send_timed,
                               .isrran_rf_send_timed_multi       = rf_skiq_send_timed_multi};
