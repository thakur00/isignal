...
static isrran_rf_t ue_radio, enb_radio;
pthread_t          rx_thread;
...
void* ue_rx_thread_function(void* args)
{
  ...
  if (isrran_rf_open_devname(&ue_radio, "file", rf_args, NOF_RX_ANT)) {
  ...
  for (uint32_t i = 0; i < num_slots; ++i) {
    void* data_ptr[ISRRAN_MAX_PORTS] = {NULL};
    for (uint32_t c = 0; c < NOF_RX_ANT; c++) {
      data_ptr[c] = &ue_rx_buffer[c][i * num_samps_per_slot];
    }
    num_rxed_samps += isrran_rf_recv_with_time_multi(&ue_radio, data_ptr, num_samps_per_slot, true, NULL, NULL);
  }

  printf("received %d samples.\n", num_rxed_samps);

  printf("closing ue rx device\n");
  isrran_rf_close(&ue_radio);
...
void enb_tx_function(const char* tx_args, bool timed_tx)
{
  ...
  if (isrran_rf_open_devname(&enb_radio, "file", rf_args, NOF_RX_ANT)) {
    fprintf(stderr, "Error opening rf\n");
    exit(-1);
  }
  ...
  int ret = isrran_rf_send_multi(&enb_radio, (void**)data_ptr, SF_LEN, true, true, false);
  num_txed_samples += SF_LEN;

  // from here on, all transmissions are timed relative to the last rx time
  isrran_timestamp_t rx_time, tx_time;
  ...
  for (uint32_t i = 0; i < NUM_SF - ((timed_tx) ? TX_OFFSET_MS : 1); ++i) {
    ...
    isrran_rf_recv_with_time_multi(&enb_radio, data_ptr, SF_LEN, true, &rx_time.full_secs, &rx_time.frac_secs);
    ...
    if (timed_tx) {
      ...
      // subframes)
      isrran_timestamp_copy(&tx_time, &rx_time);
      isrran_timestamp_add(&tx_time, 0, TX_OFFSET_MS * 1e-3);
      ret = isrran_rf_send_timed_multi(
          &enb_radio, (void**)data_ptr, SF_LEN, tx_time.full_secs, tx_time.frac_secs, true, true, false);
    } else {
      // normal tx
      ret = isrran_rf_send_multi(&enb_radio, (void**)data_ptr, SF_LEN, true, true, false);
    }
...
int run_test(const char* rx_args, const char* tx_args, bool timed_tx)
{
  ...
  for (int c = 0; c < NOF_RX_ANT; c++) {
    ...
    for (uint32_t i = 0; i < NUM_SF - (timed_tx ? 3 : 0); ++i) {
      ...
      isrran_vec_fprint_c(stdout, &enb_tx_buffer[c][i * SF_LEN], 10);
      printf("ue_rx_buffer sf%d:\n", i);
      isrran_vec_fprint_c(stdout, &ue_rx_buffer[c][sf_offet + i * SF_LEN], 10);
#endif
      ...
      if (d) {
        ...
        printf("enb_tx_buffer sf%d:\n", i);
        isrran_vec_fprint_c(stdout, &enb_tx_buffer[i * SF_LEN + d], 10);
        printf("ue_rx_buffer sf%d:\n", i);
        isrran_vec_fprint_c(stdout, &ue_rx_buffer[sf_offet + i * SF_LEN + d], 10);
        goto exit;
      }
#else
      isrran_vec_sub_ccc(&ue_rx_buffer[c][sf_offet + i * SF_LEN],
                         &enb_tx_buffer[c][i * SF_LEN],
                         &ue_rx_buffer[c][sf_offet + i * SF_LEN],
                         SF_LEN);
      uint32_t max_ix = isrran_vec_max_abs_ci(&ue_rx_buffer[c][sf_offet + i * SF_LEN], SF_LEN);
...
int param_test(const char* args_param, const int num_channels)
{
  ...
  printf("opening tx device with args=%s\n", rf_args);
  if (isrran_rf_open_devname(&enb_radio, "file", rf_args, num_channels)) {
    fprintf(stderr, "Error opening rf\n");
    return ISRRAN_ERROR;
  }
...
int main()
{
  ...
  if (run_test("rx_file=tx_file0,base_srate=1.92e6", "tx_file=tx_file0,base_srate=1.92e6", false) != ISRRAN_SUCCESS) {
