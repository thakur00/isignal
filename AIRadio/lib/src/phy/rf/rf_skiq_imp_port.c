...
static void rf_skiq_rx_port_handle_overflow(rf_skiq_rx_port_t* q)
{
  ...
  error.type = ISRRAN_RF_ERROR_OVERFLOW;
  if (q->error_handler) {
...
static bool rf_skiq_tx_port_handle_late(rf_skiq_tx_port_t* q)
{
  ...
  if (q->error_handler) {
    isrran_rf_error_t error = {};
    error.type              = ISRRAN_RF_ERROR_LATE;
    error.opt               = new_late;
    q->error_handler(q->error_handler_arg, error);
  } else {
...
static void* writer_thread(void* arg)
{
  ...
  while (q->state != RF_SKIQ_PORT_STATE_STOP) {
    ...
    int n = isrran_ringbuffer_read_block(&q->rb, (void**)&p_tx_block, q->p_block_nbytes, 1000);
...
int rf_skiq_tx_port_init(rf_skiq_tx_port_t* q, uint8_t card, skiq_tx_hdl_t hdl, const rf_skiq_port_opts_t* opts)
{
  ...
  if (isrran_ringbuffer_init(&q->rb, (int)(opts->tx_rb_size * q->p_block_nbytes))) {
...
void rf_skiq_tx_port_free(rf_skiq_tx_port_t* q)
{
  ...
  // Unlock ringbuffer
  isrran_ringbuffer_write(&q->rb, (void*)q->p_tx_block, q->p_block_nbytes);
...
void rf_skiq_tx_port_end_of_burst(rf_skiq_tx_port_t* q)
{
  ...
  if (q->next_offset > 0) {
    ...
    isrran_vec_i16_zero(&q->p_tx_block->data[q->next_offset * 2], 2 * pending);

    // Write block into the ring-buffer
    isrran_ringbuffer_write_block(&q->rb, q->p_tx_block, q->p_block_nbytes);
...
int rf_skiq_tx_port_send(rf_skiq_tx_port_t* q, const cf_t* buffer, uint32_t nsamples, uint64_t ts)
{
  ...
  nsamples = ISRRAN_MIN(nsamples, q->block_size - q->next_offset);
  ...
  if (buffer == NULL) {
    // ... with zeros
    isrran_vec_i16_zero(p_tx_iq, 2 * nsamples);
  } else {
    // ... with samples, after conversion
    isrran_vec_convert_conj_cs(buffer, SKIQ_NORM, p_tx_iq, nsamples);
  }
  ...
  if (isrran_ringbuffer_space(&q->rb) < q->p_block_nbytes * 2) {
  ...
  int n = isrran_ringbuffer_write_timed_block(&q->rb, q->p_tx_block, q->p_block_nbytes, 5);
...
void rf_skiq_tx_port_set_error_handler(rf_skiq_tx_port_t* q, isrran_rf_error_handler_t error_handler, void* arg)
{
...
int rf_skiq_rx_port_init(rf_skiq_rx_port_t* q, uint8_t card, skiq_rx_hdl_t hdl, const rf_skiq_port_opts_t* opts)
{
  ...
  // initialise ring buffer
  if (isrran_ringbuffer_init(&q->rb, (int)(opts->rx_rb_size * rx_block_size))) {
    ERROR("Initialising ringbuffer");
    return ISRRAN_ERROR;
  }
...
{
  return isrran_ringbuffer_status(&q->rb);
}
...
int rf_skiq_rx_port_read(rf_skiq_rx_port_t* q, cf_t* dest, uint32_t nsamples, uint64_t* ts_start)
{
  ...
  if (q->rb_read_rem == 0) {
    ...
    if (q->rb_overflow) {
      // Reset ring buffer
      isrran_ringbuffer_reset(&q->rb);

      // Clear overflow flag
      q->rb_overflow = false;

      // Set samples to zero
      isrran_vec_cf_zero(dest, nsamples);
    ...
    if (isrran_ringbuffer_read(&q->rb, &header, sizeof(skiq_header_t)) != sizeof(skiq_header_t)) {
  ...
  nsamples = ISRRAN_MIN(q->rb_read_rem, nsamples);

  // Read any remainder of a packet from the ring buffer
  int n = isrran_ringbuffer_read_convert_conj(&q->rb, dest, SKIQ_NORM, nsamples);
...
int rf_skiq_rx_port_write(rf_skiq_rx_port_t* q, const skiq_rx_block_t* p_rx_block, uint32_t nsamples)
{
  ...
  if (isrran_ringbuffer_space(&q->rb) >= sizeof(skiq_header_t) + nsamples * 4) {
    // Write header
    if (isrran_ringbuffer_write_block(&q->rb, &header, sizeof(skiq_header_t)) != sizeof(skiq_header_t)) {
      ERROR("Writing header in Rx buffer %d:%d!\n", q->card, (int)q->hdl);
      return ISRRAN_ERROR;
    }

    // Write IQ samples
    if (isrran_ringbuffer_write_block(&q->rb, (uint8_t*)p_rx_block->data, (int)nsamples * 4) != nsamples * 4) {
...
void rf_skiq_rx_port_set_error_handler(rf_skiq_rx_port_t* q, isrran_rf_error_handler_t error_handler, void* arg)
{
  q->error_handler     = error_handler;
  q->error_handler_arg = arg;
}
