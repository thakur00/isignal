...
private:
  ...
  void* tx_routine()
  {
    ...
    while (tx_metadata.time_spec < config.duration_s + config.init_tx_time and running) {
      ...
      isrran_vec_gen_sine(v, config.get_freq_norm(), tx_buffer[0], packet_size_nsamples);
      isrran_vec_sc_prod_cfc(tx_buffer[0], -1, tx_buffer[1], packet_size_nsamples);
  ...
  void* rx_routine()
  {
    ...
    do {
      ...
      if (rx_metadata.error_code == uhd::rx_metadata_t::ERROR_CODE_TIMEOUT) {
        ...
        rxd_count++;
        float avg_power = isrran_vec_avg_power_cf(rx_buffer[0], rxd_nsamples);
...
public:
  ...
  int init()
  {
    ...
    packet_size_nsamples = ISRRAN_MIN(nof_tx_samples, nof_rx_samples);

    for (size_t j = 0; j < config.nof_channels; j++) {
      tx_buffer[j] = isrran_vec_cf_malloc(packet_size_nsamples);
      rx_buffer[j] = isrran_vec_cf_malloc(packet_size_nsamples);
    }
...
static int test(const rfnoc_custom_testbench_s::config_s& config)
{
  ...
  TESTASSERT(context.init() == ISRRAN_SUCCESS);
  TESTASSERT(context.run() == ISRRAN_SUCCESS);
...
int main(int argc, char* argv[])
{
  ...
  TESTASSERT(parse_args(argc, argv, config) == ISRRAN_SUCCESS);

  TESTASSERT(test(config) == ISRRAN_SUCCESS);
