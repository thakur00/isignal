...
const char* isrran_rnti_type_str(isrran_rnti_type_t rnti_type)
{
  switch (rnti_type) {
    case isrran_rnti_type_c:
      return "C-RNTI";
    case isrran_rnti_type_p:
      return "P-RNTI";
    ...
    case isrran_rnti_type_tc:
      return "TC-RNTI";
    case isrran_rnti_type_cs:
      return "CS-RNTI";
    case isrran_rnti_type_sp_csi:
      return "SP-CSI-RNTI";
    case isrran_rnti_type_mcs_c:
      return "MCS-C-RNTI";
    default:; // Do nothing
  }
  return "unknown";
}
const char* isrran_rnti_type_str_short(isrran_rnti_type_t rnti_type)
{
  switch (rnti_type) {
    case isrran_rnti_type_c:
      return "c";
...
const char* isrran_ss_type_str(isrran_search_space_type_t ss_type)
{
  switch (ss_type) {
    case isrran_search_space_type_common_0:
      return "common0";
    case isrran_search_space_type_common_0A:
      return "common0A";
    ...
    case isrran_search_space_type_ue:
      return "ue";
    case isrran_search_space_type_rar:
      return "rar";
    case isrran_search_space_type_cg:
      return "cg";
    default:; // Do nothing
      break;
  }
...
const char* isrran_dci_format_nr_string(isrran_dci_format_nr_t format)
{
  switch (format) {
    ...
    case isrran_dci_format_nr_1_1:
      return "1_1";
    case isrran_dci_format_nr_2_0:
      return "2_0";
    case isrran_dci_format_nr_2_1:
      return "2_1";
    case isrran_dci_format_nr_2_2:
      return "2_2";
    case isrran_dci_format_nr_2_3:
      return "2_3";
    case isrran_dci_format_nr_rar:
      return "RAR";
    case isrran_dci_format_nr_cg:
      return "CG";
    default:
    case ISRRAN_DCI_FORMAT_NR_COUNT:
      break;
  }
  return "unknown";
}

uint32_t isrran_coreset_get_bw(const isrran_coreset_t* coreset)
{
...
uint32_t isrran_coreset_get_sz(const isrran_coreset_t* coreset)
{
  // Returns the number of resource elements in time and frequency domains
  return isrran_coreset_get_bw(coreset) * ISRRAN_NRE * coreset->duration;
}

uint32_t isrran_coreset_start_rb(const isrran_coreset_t* coreset)
{
  ...
  for (uint32_t res = 0; res < ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE; res++) {
...
const char* isrran_sch_mapping_type_to_str(isrran_sch_mapping_type_t mapping_type)
{
  switch (mapping_type) {
...
const char* isrran_mcs_table_to_str(isrran_mcs_table_t mcs_table)
{
  switch (mcs_table) {
    case isrran_mcs_table_64qam:
      return "64qam";
    case isrran_mcs_table_256qam:
      return "256qam";
    case isrran_mcs_table_qam64LowSE:
      return "qam64LowSE";
    default:
      return "undefined";
  }
...
isrran_mcs_table_t isrran_mcs_table_from_str(const char* str)
{
  if (strcmp(str, "64qam") == 0) {
    return isrran_mcs_table_64qam;
  }
  if (strcmp(str, "256qam") == 0) {
    return isrran_mcs_table_256qam;
  }
  if (strcmp(str, "qam64LowSE") == 0) {
    return isrran_mcs_table_qam64LowSE;
  }
  return isrran_mcs_table_N;
}
...
uint32_t isrran_min_symbol_sz_rb(uint32_t nof_prb)
{
  uint32_t nof_re = nof_prb * ISRRAN_NRE;
  ...
  if (isrran_symbol_size_is_standard()) {
    ...
    if (nof_prb > 79 && nof_prb < 1536 / ISRRAN_NRE) {
      return 2048;
    }
  }
...
int isrran_symbol_sz_from_srate(double srate_hz, isrran_subcarrier_spacing_t scs)
{
  ...
  uint32_t scs_int_hz = ISRRAN_SUBC_SPACING_NR(scs);
...
float isrran_symbol_offset_s(uint32_t l, isrran_subcarrier_spacing_t scs)
{
  // Compute at what symbol there is a longer CP
  uint32_t cp_boundary = ISRRAN_EXT_CP_SYMBOL(scs);
  ...
  N += 16 * ISRRAN_CEIL(l, cp_boundary);

  // Compute time using reference sampling rate
  float TS = ISRRAN_LTE_TS;

  // Return symbol offset in seconds
  return (float)N * TS;
}

float isrran_symbol_distance_s(uint32_t l0, uint32_t l1, isrran_subcarrier_spacing_t scs)
{
  ...
  return isrran_symbol_offset_s(l1, scs) - isrran_symbol_offset_s(l0, scs);
}

static bool tdd_nr_is_dl(const isrran_tdd_config_nr_t* cfg, uint32_t numerology, uint32_t slot_idx)
{
  ...
  // Select pattern
  const isrran_tdd_pattern_t* pattern = &cfg->pattern1;
...
bool isrran_duplex_nr_is_dl(const isrran_duplex_config_nr_t* cfg, uint32_t numerology, uint32_t slot_idx)

{
  ...
  // In case of TDD
  if (cfg->mode == ISRRAN_DUPLEX_MODE_TDD) {
...
static bool tdd_nr_is_ul(const isrran_tdd_config_nr_t* cfg, uint32_t numerology, uint32_t slot_idx)
{
  // Prevent zero division
  if (cfg->pattern1.period_ms == 0 && cfg->pattern2.period_ms == 0) {
    return false;
...
bool isrran_duplex_nr_is_ul(const isrran_duplex_config_nr_t* cfg, uint32_t numerology, uint32_t slot_idx)
{
  // Protect NULL pointer access
  if (cfg == NULL) {
    return false;
...
int isrran_carrier_to_cell(const isrran_carrier_nr_t* carrier, isrran_cell_t* cell)
{
  ...
  ISRRAN_MEM_ZERO(cell, isrran_cell_t, 1);
...
uint32_t isrran_csi_meas_info(const isrran_csi_trs_measurements_t* meas, char* str, uint32_t str_len)
{
...
uint32_t isrran_csi_meas_info_short(const isrran_csi_trs_measurements_t* meas, char* str, uint32_t str_len)
{
  if (meas == NULL || str == NULL || str_len == 0) {
    return 0;
  }
...
isrran_subcarrier_spacing_t isrran_subcarrier_spacing_from_str(const char* str)
{
  if (str == NULL) {
...
const char* isrran_subcarrier_spacing_to_str(isrran_subcarrier_spacing_t scs)
{
  switch (scs) {
    case isrran_subcarrier_spacing_15kHz:
      return "15kHz";
    case isrran_subcarrier_spacing_30kHz:
      return "30kHz";
    case isrran_subcarrier_spacing_60kHz:
      return "60kHz";
    case isrran_subcarrier_spacing_120kHz:
      return "120kHz";
    case isrran_subcarrier_spacing_240kHz:
      return "240kHz";
    case isrran_subcarrier_spacing_invalid:
    default:
      return "invalid";
  }
}

void isrran_combine_csi_trs_measurements(const isrran_csi_trs_measurements_t* a,
                                         ...
                                         isrran_csi_trs_measurements_t*       dst)
{
  ...
  if (nof_re_sum == 0) {
    ISRRAN_MEM_ZERO(dst, isrran_csi_trs_measurements_t, 1);
    return;
  }

  // Perform proportional average
  dst->rsrp       = ISRRAN_VEC_PMA(a->rsrp, a->nof_re, b->rsrp, b->nof_re);
  dst->rsrp_dB    = ISRRAN_VEC_PMA(a->rsrp_dB, a->nof_re, b->rsrp_dB, b->nof_re);
  dst->epre       = ISRRAN_VEC_PMA(a->epre, a->nof_re, b->epre, b->nof_re);
  dst->epre_dB    = ISRRAN_VEC_PMA(a->epre_dB, a->nof_re, b->epre_dB, b->nof_re);
  dst->n0         = ISRRAN_VEC_PMA(a->n0, a->nof_re, b->n0, b->nof_re);
  dst->n0_dB      = ISRRAN_VEC_PMA(a->n0_dB, a->nof_re, b->n0_dB, b->nof_re);
  dst->snr_dB     = ISRRAN_VEC_PMA(a->snr_dB, a->nof_re, b->snr_dB, b->nof_re);
  dst->cfo_hz     = ISRRAN_VEC_PMA(a->cfo_hz, a->nof_re, b->cfo_hz, b->nof_re);
  dst->cfo_hz_max = ISRRAN_MAX(a->cfo_hz_max, b->cfo_hz_max);
  dst->delay_us   = ISRRAN_VEC_PMA(a->delay_us, a->nof_re, b->delay_us, b->nof_re);
...
{
  switch (x) {
    case isrran_coreset_bundle_size_n2:
      return 2;
    case isrran_coreset_bundle_size_n3:
      return 3;
    case isrran_coreset_bundle_size_n6:
      return 6;
  }
  return 0;
}
...
int isrran_coreset_to_str(isrran_coreset_t* coreset, char* str, uint32_t str_len)
{
  if (coreset == NULL || str == NULL || str_len == 0) {
    return 0;
  }

  char freq_res_str[ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE] = {};
  isrran_vec_sprint_bin(
      freq_res_str, sizeof(freq_res_str), (uint8_t*)coreset->freq_resources, ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE);
  ...
  return isrran_print_check(
      ...
      "  - offset_rb=%d\n",
      coreset->id,
      coreset->mapping_type == isrran_coreset_mapping_type_non_interleaved ? "non-interleaved" : "interleaved",
      coreset->duration,
      freq_res_str,
      coreset->dmrs_scrambling_id_present ? "true" : "false",
      coreset->dmrs_scrambling_id,
      coreset->precoder_granularity == isrran_coreset_precoder_granularity_contiguous ? "contiguous" : "reg_bundle",
...
int isrran_coreset_zero(uint32_t                    n_cell_id,
                        ...
                        isrran_coreset_t*           coreset)
{
  ...
  if (ssb_scs == isrran_subcarrier_spacing_15kHz && pdcch_scs == isrran_subcarrier_spacing_15kHz) {
  ...
  // bandwidth 5 MHz or 10 MHz
  if (ssb_scs == isrran_subcarrier_spacing_15kHz && pdcch_scs == isrran_subcarrier_spacing_30kHz) {
  ...
  // bandwidth 5 MHz or 10 MHz
  if (ssb_scs == isrran_subcarrier_spacing_30kHz && pdcch_scs == isrran_subcarrier_spacing_15kHz) {
  ...
  if (entry == NULL) {
    ERROR("Unhandled case ssb_scs=%s, pdcch_scs=%s",
          ...
          isrran_subcarrier_spacing_to_str(pdcch_scs));
    return ISRRAN_ERROR;
  }
  ...
  uint32_t ssb_half_bw_Hz = ISRRAN_SUBC_SPACING_NR(ssb_scs) * (ISRRAN_SSB_BW_SUBC / 2U);
  ...
  uint32_t ssb_pointA_freq_offset_rb =
      ISRRAN_FLOOR(ssb_pointA_freq_offset_Hz, ISRRAN_NRE * ISRRAN_SUBC_SPACING_NR(pdcch_scs));
  uint32_t offset_rb =
...
int isrran_coreset0_ssb_offset(uint32_t idx, isrran_subcarrier_spacing_t ssb_scs, isrran_subcarrier_spacing_t pdcch_scs)
{
  // Verify inputs
  if (idx >= 16) {
...
const char* isrran_ssb_pattern_to_str(isrran_ssb_pattern_t pattern)
{
  switch (pattern) {
...
isrran_ssb_pattern_t isrran_ssb_pattern_fom_str(const char* str)
{
  if (str == NULL) {
    return ISRRAN_SSB_PATTERN_INVALID;
  }

  if (strcasecmp(str, "A") == 0) {
    return ISRRAN_SSB_PATTERN_A;
  }

  if (strcasecmp(str, "B") == 0) {
    return ISRRAN_SSB_PATTERN_B;
  }

  if (strcasecmp(str, "C") == 0) {
    return ISRRAN_SSB_PATTERN_C;
  }

  if (strcasecmp(str, "D") == 0) {
    return ISRRAN_SSB_PATTERN_D;
  }

  if (strcasecmp(str, "E") == 0) {
    return ISRRAN_SSB_PATTERN_E;
  }

  return ISRRAN_SSB_PATTERN_INVALID;
}

bool isrran_carrier_nr_equal(const isrran_carrier_nr_t* a, const isrran_carrier_nr_t* b)
{
  if (a == NULL || b == NULL) {
    return false;
  }
