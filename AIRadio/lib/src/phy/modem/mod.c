...
int isrran_mod_modulate(const isrran_modem_table_t* q, uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
  uint32_t i, j, idx;
  uint8_t* b_ptr = (uint8_t*)bits;
  j              = 0;
  for (i = 0; i < nbits; i += q->nbits_x_symbol) {
    idx = isrran_bit_pack(&b_ptr, q->nbits_x_symbol);
    if (idx < q->nsymbols) {
...
static void mod_bpsk_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
...
static void mod_qpsk_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
...
static void mod_16qam_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
  for (int i = 0; i < nbits / 8; i++) {
...
static void mod_64qam_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
...
static void mod_256qam_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
  for (int i = 0; i < nbits / 8; i++) {
...
int isrran_mod_modulate_bytes(const isrran_modem_table_t* q, const uint8_t* bits, cf_t* symbols, uint32_t nbits)
{
  if (!q->byte_tables_init) {
    ERROR("Error need to initiated modem tables for packeted bits before calling isrran_mod_modulate_bytes()");
    return ISRRAN_ERROR;
  }
  ...
  switch (q->nbits_x_symbol) {
    ...
    case 8:
      ...
      break;
    default:
      ERROR("isrran_mod_modulate_bytes() accepts QPSK/16QAM/64QAM modulations only");
      return ISRRAN_ERROR;
  }
