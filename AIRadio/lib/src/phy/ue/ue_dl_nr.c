...
static int ue_dl_nr_alloc_prb(isrran_ue_dl_nr_t* q, uint32_t new_nof_prb)
{
  if (q->max_prb < new_nof_prb) {
    q->max_prb = new_nof_prb;

    isrran_chest_dl_res_free(&q->chest);
    if (isrran_chest_dl_res_init(&q->chest, q->max_prb) < ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
...
int isrran_ue_dl_nr_init(isrran_ue_dl_nr_t* q, cf_t* input[ISRRAN_MAX_PORTS], const isrran_ue_dl_nr_args_t* args)
{
  if (!q || !input || !args) {
  ...
  if (isrran_pdsch_nr_init_ue(&q->pdsch, &args->pdsch) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_pdcch_nr_init_rx(&q->pdcch, &args->pdcch)) {
    return ISRRAN_ERROR;
  }
  ...
  for (uint32_t i = 0; i < q->nof_rx_antennas; i++) {
    ...
    isrran_ofdm_rx_init_cfg(&q->fft[i], &fft_cfg);
  }

  if (isrran_dmrs_sch_init(&q->dmrs_pdsch, true) < ISRRAN_SUCCESS) {
    ERROR("Error DMRS");
    return ISRRAN_ERROR;
  }

  q->pdcch_ce = ISRRAN_MEM_ALLOC(isrran_dmrs_pdcch_ce_t, 1);
  if (q->pdcch_ce == NULL) {
    ERROR("Error alloc");
    return ISRRAN_ERROR;
  }
...
void isrran_ue_dl_nr_free(isrran_ue_dl_nr_t* q)
{
  if (q == NULL) {
    return;
  }
  ...
  isrran_dmrs_sch_free(&q->dmrs_pdsch);

  for (uint32_t i = 0; i < ISRRAN_UE_DL_NR_MAX_NOF_CORESET; i++) {
    isrran_dmrs_pdcch_estimator_free(&q->dmrs_pdcch[i]);
  }
  ...
  ISRRAN_MEM_ZERO(q, isrran_ue_dl_nr_t, 1);
}

int isrran_ue_dl_nr_set_carrier(isrran_ue_dl_nr_t* q, const isrran_carrier_nr_t* carrier)
{
  if (isrran_pdsch_nr_set_carrier(&q->pdsch, carrier) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_dmrs_sch_set_carrier(&q->dmrs_pdsch, carrier) < ISRRAN_SUCCESS) {
    ERROR("Error DMRS");
    return ISRRAN_ERROR;
  }
  ...
  if (carrier->nof_prb != q->carrier.nof_prb) {
    for (uint32_t i = 0; i < q->nof_rx_antennas; i++) {
      ...
      cfg.symbol_sz             = isrran_min_symbol_sz_rb(carrier->nof_prb);
      cfg.cp                    = ISRRAN_CP_NORM;
      cfg.keep_dc               = true;
      cfg.rx_window_offset      = UE_DL_NR_FFT_WINDOW_OFFSET;
      cfg.phase_compensation_hz = carrier->dl_center_frequency_hz;

      isrran_ofdm_rx_init_cfg(&q->fft[i], &cfg);
    }
  }
...
int isrran_ue_dl_nr_set_pdcch_config(isrran_ue_dl_nr_t*           q,
                                     ...
                                     const isrran_dci_cfg_nr_t*   dci_cfg)
{
  ...
  for (uint32_t i = 0; i < ISRRAN_UE_DL_NR_MAX_NOF_CORESET; i++) {
    ...
    if (isrran_dmrs_pdcch_estimator_init(&q->dmrs_pdcch[i], &q->carrier, &cfg->coreset[i]) < ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
  }
...
void isrran_ue_dl_nr_estimate_fft(isrran_ue_dl_nr_t* q, const isrran_slot_cfg_t* slot_cfg)
{
  ...
  // OFDM demodulation
  for (uint32_t i = 0; i < q->nof_rx_antennas; i++) {
    isrran_ofdm_rx_sf(&q->fft[i]);
  }

  // Estimate PDCCH channel for every configured CORESET
  for (uint32_t i = 0; i < ISRRAN_UE_DL_NR_MAX_NOF_CORESET; i++) {
    if (q->cfg.coreset_present[i]) {
      isrran_dmrs_pdcch_estimate(&q->dmrs_pdcch[i], slot_cfg, q->sf_symbols[0]);
    }
  }
}

static int ue_dl_nr_find_dci_ncce(isrran_ue_dl_nr_t*     q,
                                  isrran_dci_msg_nr_t*   dci_msg,
                                  isrran_pdcch_nr_res_t* pdcch_res,
                                  uint32_t               coreset_id)
{
  // Select debug information
  isrran_ue_dl_nr_pdcch_info_t* pdcch_info = NULL;
  if (q->pdcch_info_count < ISRRAN_MAX_NOF_CANDIDATES_SLOT_NR) {
    pdcch_info = &q->pdcch_info[q->pdcch_info_count];
    q->pdcch_info_count++;
  } else {
    ERROR("The UE does not expect more than %d candidates in this serving cell", ISRRAN_MAX_NOF_CANDIDATES_SLOT_NR);
    return ISRRAN_ERROR;
  }
  ISRRAN_MEM_ZERO(pdcch_info, isrran_ue_dl_nr_pdcch_info_t, 1);
  pdcch_info->dci_ctx            = dci_msg->ctx;
  pdcch_info->nof_bits           = dci_msg->nof_bits;
  isrran_dmrs_pdcch_measure_t* m = &pdcch_info->measure;

  // Measures the PDCCH transmission DMRS
  isrran_dci_location_t location = dci_msg->ctx.location;
  if (isrran_dmrs_pdcch_get_measure(&q->dmrs_pdcch[coreset_id], &location, m) < ISRRAN_SUCCESS) {
  ...
  if (isrran_dmrs_pdcch_get_ce(&q->dmrs_pdcch[coreset_id], &location, q->pdcch_ce) < ISRRAN_SUCCESS) {
    ERROR("Error extracting PDCCH DMRS");
    return ISRRAN_ERROR;
  }

  // Decode PDCCH
  if (isrran_pdcch_nr_decode(&q->pdcch, q->sf_symbols[0], q->pdcch_ce, dci_msg, pdcch_res) < ISRRAN_SUCCESS) {
  ...
  isrran_vec_save_file(tmpstr, q->pdcch.symbols, q->pdcch.M / 4 * sizeof(cf_t));
  // isrran_vec_fprint_c(stdout, q->pdcch.symbols, q->pdcch.M/4);
  num_pdcch++;
#endif
...
static bool find_dci_msg(isrran_dci_msg_nr_t* dci_msg, uint32_t nof_dci_msg, isrran_dci_msg_nr_t* match)
{
  bool     found    = false;
                                ...
                                isrran_rnti_type_t           rnti_type)
{
  ...
  for (uint32_t format_idx = 0; format_idx < ISRRAN_MIN(search_space->nof_formats, ISRRAN_DCI_FORMAT_NR_COUNT);
       format_idx++) {
    isrran_dci_format_nr_t dci_format = search_space->formats[format_idx];

    // Calculate number of DCI bits
    uint32_t dci_nof_bits = isrran_dci_nr_size(&q->dci, search_space->type, dci_format);
    if (dci_nof_bits == 0) {
    ...
    // Append size
    if (dci_sizes_count >= ISRRAN_DCI_NR_MAX_NOF_SIZES) {
    ...
    for (uint32_t L = 0;
         L < ISRRAN_SEARCH_SPACE_NOF_AGGREGATION_LEVELS_NR && q->dl_dci_msg_count < ISRRAN_MAX_DCI_MSG_NR;
         L++) {
      // Calculate possible PDCCH DCI candidates
      uint32_t candidates[ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR] = {};
      int      nof_candidates                                        = isrran_pdcch_nr_locations_coreset(
                                                      coreset, search_space, rnti, L, ISRRAN_SLOT_NR_MOD(q->carrier.scs, slot_cfg->idx), candidates);
      ...
      for (int ncce_idx = 0; ncce_idx < nof_candidates && q->dl_dci_msg_count < ISRRAN_MAX_DCI_MSG_NR; ncce_idx++) {
        // Build DCI context
        ...
        ctx.coreset_start_rb = isrran_coreset_start_rb(&q->cfg.coreset[search_space->coreset_id]);
        ...
        if (ue_dl_nr_find_dci_ncce(q, &dci_msg, &res, coreset_id) < ISRRAN_SUCCESS) {
          return ISRRAN_ERROR;
        }
        ...
        if (!isrran_dci_nr_valid_direction(&dci_msg)) {
          ...
          switch (dci_msg.ctx.format) {
            ...
            case isrran_dci_format_nr_0_1:
              dci_msg.ctx.format = isrran_dci_format_nr_1_1;
              break;
            case isrran_dci_format_nr_1_0:
              dci_msg.ctx.format = isrran_dci_format_nr_0_0;
              break;
            case isrran_dci_format_nr_1_1:
              dci_msg.ctx.format = isrran_dci_format_nr_0_1;
              break;
            default:
              continue;
          }
        ...
        if (dci_msg.ctx.format == isrran_dci_format_nr_0_0 || dci_msg.ctx.format == isrran_dci_format_nr_0_1) {
          // If the pending UL grant list is full or has the dci message, keep moving
          if (q->ul_dci_count >= ISRRAN_MAX_DCI_MSG_NR || find_dci_msg(q->ul_dci_msg, q->ul_dci_count, &dci_msg)) {
            continue;
          }
...
int isrran_ue_dl_nr_find_dl_dci(isrran_ue_dl_nr_t*       q,
                                const isrran_slot_cfg_t* slot_cfg,
                                uint16_t                 rnti,
                                isrran_rnti_type_t       rnti_type,
                                ...
                                uint32_t                 nof_dci_msg)
{
  ...
  nof_dci_msg = ISRRAN_MIN(nof_dci_msg, ISRRAN_MAX_DCI_MSG_NR);
  ...
  if (q->cfg.ra_search_space_present && rnti_type == isrran_rnti_type_ra) {
    ...
    for (uint32_t i = 0; i < ISRRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE && q->dl_dci_msg_count < nof_dci_msg; i++) {
  ...
  uint32_t dci_msg_count = ISRRAN_MIN(nof_dci_msg, q->dl_dci_msg_count);
  for (uint32_t i = 0; i < dci_msg_count; i++) {
    if (isrran_dci_nr_dl_unpack(&q->dci, &q->dl_dci_msg[i], &dci_dl_list[i]) < ISRRAN_SUCCESS) {
...
int isrran_ue_dl_nr_find_ul_dci(isrran_ue_dl_nr_t*       q,
                                const isrran_slot_cfg_t* slot_cfg,
                                uint16_t                 rnti,
                                isrran_rnti_type_t       rnti_type,
                                ...
                                uint32_t                 nof_dci_msg)
{
  ...
  for (uint32_t i = 0; i < q->ul_dci_count && count < nof_dci_msg; i++) {
    isrran_dci_msg_nr_t* dci_msg = &q->ul_dci_msg[i];

    if (dci_msg->ctx.rnti_type != rnti_type || dci_msg->ctx.rnti != rnti) {
      continue;
    }

    if (isrran_dci_nr_ul_unpack(&q->dci, dci_msg, &dci_ul_list[count]) < ISRRAN_SUCCESS) {
...
int isrran_ue_dl_nr_decode_pdsch(isrran_ue_dl_nr_t*         q,
                                 const isrran_slot_cfg_t*   slot,
                                 const isrran_sch_cfg_nr_t* cfg,
                                 isrran_pdsch_res_nr_t*     res)
{
  if (isrran_dmrs_sch_estimate(&q->dmrs_pdsch, slot, cfg, &cfg->grant, q->sf_symbols[0], &q->chest) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_pdsch_nr_decode(&q->pdsch, cfg, &cfg->grant, &q->chest, q->sf_symbols, res) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    char str[512];
    isrran_ue_dl_nr_pdsch_info(q, cfg, res, str, sizeof(str));
    INFO("PDSCH: %s", str);
  }
...
uint32_t isrran_ue_dl_nr_pdsch_info(const isrran_ue_dl_nr_t*    q,
                                    const isrran_sch_cfg_nr_t*  cfg,
                                    ...
                                    uint32_t                    str_len)
{
  ...
  // Append PDSCH info
  len += isrran_pdsch_nr_rx_info(&q->pdsch, cfg, &cfg->grant, res, &str[len], str_len - len);

  // Append channel estimator info
  len += isrran_csi_meas_info_short(&q->dmrs_pdsch.csi, &str[len], str_len - len);
                                    ...
                                    isrran_csi_trs_measurements_t* measurement)
{
  ...
  return isrran_csi_rs_nzp_measure_trs(&q->carrier, slot_cfg, csi_rs_nzp_set, q->sf_symbols[0], measurement);
}
                                        ...
                                        isrran_csi_channel_measurements_t* measurement)
{
  ...
  return isrran_csi_rs_nzp_measure_channel(&q->carrier, slot_cfg, csi_rs_nzp_set, q->sf_symbols[0], measurement);
}
