...
int fdd_tests()
{
  ...
  isrran_ue_ul_pucch_resource_selection(&cell, &pucch_cfg, &uci_cfg, &uci_value, b);
  TESTASSERT(pucch_cfg.format == ISRRAN_PUCCH_FORMAT_1);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    isrran_ue_ul_pucch_resource_selection(&cell, &pucch_cfg, &uci_cfg, &uci_value, b);
    TESTASSERT(pucch_cfg.format == ISRRAN_PUCCH_FORMAT_1A);
    if (i == 0) {
      TESTASSERT(pucch_cfg.n_pucch == pucch_cfg.n_pucch_sr);
    } else {
  ...
  pucch_cfg.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_NORMAL;
  for (int i = 0; i < 2; i++) {
    uci_value.scheduling_request = i == 0;
    ...
    uci_cfg.ack[0].ncce[0]  = 13;
    isrran_ue_ul_pucch_resource_selection(&cell, &pucch_cfg, &uci_cfg, &uci_value, b);
    TESTASSERT(pucch_cfg.format == ISRRAN_PUCCH_FORMAT_1B);
    if (i == 0) {
      TESTASSERT(pucch_cfg.n_pucch == pucch_cfg.n_pucch_sr);
    } else {
      TESTASSERT(pucch_cfg.n_pucch == uci_cfg.ack[0].ncce[0] + pucch_cfg.N_pucch_1);
  ...
  ZERO_OBJECT(uci_cfg);
  pucch_cfg.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_CS;
  uci_cfg.ack[0].nof_acks          = 1;
