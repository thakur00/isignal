...
#define CELL_SEARCH_BUFFER_MAX_SAMPLES (3 * ISRRAN_SF_LEN_MAX)

int isrran_ue_cellsearch_init(isrran_ue_cellsearch_t* q,
                              uint32_t                max_frames,
                              ...
                              void* stream_handler)
{
  ...
  if (q != NULL) {
    ...
    bzero(q, sizeof(isrran_ue_cellsearch_t));

    bzero(&cell, sizeof(isrran_cell_t));
    cell.id      = ISRRAN_CELL_ID_UNKNOWN;
    cell.nof_prb = ISRRAN_CS_NOF_PRB;

    if (isrran_ue_sync_init(&q->ue_sync, cell.nof_prb, true, recv_callback, stream_handler)) {
    ...
    for (int p = 0; p < ISRRAN_MAX_CHANNELS; p++) {
      q->sf_buffer[p] = NULL;
    }
    q->sf_buffer[0]    = isrran_vec_cf_malloc(CELL_SEARCH_BUFFER_MAX_SAMPLES);
    q->nof_rx_antennas = 1;

    q->candidates = calloc(sizeof(isrran_ue_cellsearch_result_t), max_frames);
    if (!q->candidates) {
      perror("malloc");
      goto clean_exit;
    }
...
clean_exit:
  if (ret == ISRRAN_ERROR) {
    isrran_ue_cellsearch_free(q);
  }
  return ret;
}

int isrran_ue_cellsearch_init_multi(
    isrran_ue_cellsearch_t* q,
    uint32_t                max_frames,
    ...
    void*    stream_handler)
{
  ...
  if (q != NULL && nof_rx_antennas < ISRRAN_MAX_CHANNELS) {
    ret = ISRRAN_ERROR;
    isrran_cell_t cell;
    ...
    if (isrran_ue_sync_init_multi(&q->ue_sync, cell.nof_prb, true, recv_callback, nof_rx_antennas, stream_handler)) {
    ...
    for (int i = 0; i < nof_rx_antennas; i++) {
      q->sf_buffer[i] = isrran_vec_cf_malloc(CELL_SEARCH_BUFFER_MAX_SAMPLES);
    }
...
void isrran_ue_cellsearch_free(isrran_ue_cellsearch_t* q)
{
...
int isrran_ue_cellsearch_set_nof_valid_frames(isrran_ue_cellsearch_t* q, uint32_t nof_frames)
{
...
void isrran_set_detect_cp(isrran_ue_cellsearch_t* q, bool enable)
{
  isrran_ue_sync_cp_en(&q->ue_sync, enable);
}

/* Decide the most likely cell based on the mode */
static void get_cell(isrran_ue_cellsearch_t* q, uint32_t nof_detected_frames, isrran_ue_cellsearch_result_t* found_cell)
{
  uint32_t i, j;
  ...
  for (i = 0; i < nof_detected_frames; i++) {
    if (q->candidates[i].cell_id == found_cell->cell_id) {
      if (ISRRAN_CP_ISNORM(q->candidates[i].cp)) {
        nof_normal++;
      }
      if (q->candidates[i].frame_type == ISRRAN_FDD) {
        nof_fdd++;
      }
    }
  ...
  if (nof_normal > q->mode_ntimes[mode_pos] / 2) {
    found_cell->cp = ISRRAN_CP_NORM;
  } else {
    found_cell->cp = ISRRAN_CP_EXT;
  }
  if (nof_fdd > q->mode_ntimes[mode_pos] / 2) {
    found_cell->frame_type = ISRRAN_FDD;
  } else {
    found_cell->frame_type = ISRRAN_TDD;
  }
 ...
 */
int isrran_ue_cellsearch_scan(isrran_ue_cellsearch_t*       q,
                              ...
                              uint32_t*                     max_N_id_2)
{
  ...
  for (uint32_t N_id_2 = 0; N_id_2 < 3; N_id_2++) {
    ...
    ret = isrran_ue_cellsearch_scan_N_id_2(q, N_id_2, &found_cells[N_id_2]);
    if (ret < 0) {
 ...
 */
int isrran_ue_cellsearch_scan_N_id_2(isrran_ue_cellsearch_t*        q,
                                     uint32_t                       N_id_2,
                                     isrran_ue_cellsearch_result_t* found_cell)
{
  int      ret                 = ISRRAN_ERROR_INVALID_INPUTS;
  ...
  if (q != NULL) {
    ...
    bzero(q->candidates, sizeof(isrran_ue_cellsearch_result_t) * q->max_frames);
    bzero(q->mode_ntimes, sizeof(uint32_t) * q->max_frames);
    bzero(q->mode_counted, sizeof(uint8_t) * q->max_frames);

    isrran_ue_sync_set_N_id_2(&q->ue_sync, N_id_2);
    isrran_ue_sync_reset(&q->ue_sync);
    isrran_ue_sync_cfo_reset(&q->ue_sync, 0.0f);
    isrran_ue_sync_set_nof_find_frames(&q->ue_sync, q->max_frames);

    do {
      ret = isrran_ue_sync_zerocopy(&q->ue_sync, q->sf_buffer, CELL_SEARCH_BUFFER_MAX_SAMPLES);
      if (ret < 0) {
        ERROR("Error calling isrran_ue_sync_work()");
        return -1;
      } else if (ret == 1) {
        /* This means a peak was found in find state */
        ret = isrran_sync_get_cell_id(&q->ue_sync.sfind);
        if (ret >= 0) {
          /* Save cell id, cp and peak */
          q->candidates[nof_detected_frames].cell_id    = (uint32_t)ret;
          q->candidates[nof_detected_frames].cp         = isrran_sync_get_cp(&q->ue_sync.sfind);
          q->candidates[nof_detected_frames].peak       = q->ue_sync.sfind.pss.peak_value;
          q->candidates[nof_detected_frames].psr        = isrran_sync_get_peak_value(&q->ue_sync.sfind);
          q->candidates[nof_detected_frames].cfo        = 15000 * isrran_sync_get_cfo(&q->ue_sync.sfind);
          q->candidates[nof_detected_frames].frame_type = isrran_ue_sync_get_frame_type(&q->ue_sync);
          INFO("CELL SEARCH: [%d/%d/%d]: Found peak PSR=%.3f, Cell_id: %d CP: %s, CFO=%.1f KHz",
               ...
               isrran_cp_string(q->candidates[nof_detected_frames].cp),
