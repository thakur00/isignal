...
#define PSS_OFFSET                                                                                                     \
  (q->sf_len / 2 + ((q->cell.frame_type == ISRRAN_FDD)                                                                 \
                        ? 0                                                                                            \
                        : ((ISRRAN_CP_NSYMB(q->cell.cp) - 3) * ISRRAN_SYMBOL_SZ(q->fft_size, q->cell.cp))))
...
static cf_t* dummy_offset_buffer[ISRRAN_MAX_CHANNELS] = {dummy_buffer0, dummy_buffer1, dummy_buffer1, dummy_buffer1};

int isrran_ue_sync_init_file(isrran_ue_sync_t* q, uint32_t nof_prb, char* file_name, int offset_time, float offset_freq)
{
  return isrran_ue_sync_init_file_multi(q, nof_prb, file_name, offset_time, offset_freq, 1);
}

void isrran_ue_sync_file_wrap(isrran_ue_sync_t* q, bool enable)
{
  q->file_wrap_enable = enable;
}

int isrran_ue_sync_init_file_multi(isrran_ue_sync_t* q,
                                   uint32_t          nof_prb,
                                   char*             file_name,
                                   int               offset_time,
                                   float             offset_freq,
                                   uint32_t          nof_rx_ant)
{
  ...
  if (q != NULL && file_name != NULL && isrran_nofprb_isvalid(nof_prb)) {
    ret = ISRRAN_ERROR;
    bzero(q, sizeof(isrran_ue_sync_t));
    q->file_mode        = true;
    q->file_wrap_enable = true;
    q->sf_len           = ISRRAN_SF_LEN(isrran_symbol_sz(nof_prb));
    q->file_cfo         = -offset_freq;
    q->fft_size         = isrran_symbol_sz(nof_prb);
    ...
    if (offset_time) {
      cf_t* file_offset_buffer = isrran_vec_cf_malloc(offset_time * nof_rx_ant);
      if (!file_offset_buffer) {
        perror("malloc");
        goto clean_exit;
      }
      isrran_filesource_read(&q->file_source, file_offset_buffer, offset_time * nof_rx_ant);
      free(file_offset_buffer);
    }

    isrran_ue_sync_cfo_reset(q, 0.0f);
    isrran_ue_sync_reset(q);

    ret = ISRRAN_SUCCESS;
  }
clean_exit:
  if (ret == ISRRAN_ERROR) {
    isrran_ue_sync_free(q);
  }
  return ret;
}

void isrran_ue_sync_cfo_reset(isrran_ue_sync_t* q, float init_cfo_hz)
{
  q->cfo_is_copied     = false;
  q->cfo_current_value = init_cfo_hz / 15e3f;
  isrran_sync_cfo_reset(&q->strack, init_cfo_hz);
  isrran_sync_cfo_reset(&q->sfind, init_cfo_hz);
}

void isrran_ue_sync_reset(isrran_ue_sync_t* q)
{
  if (!q->file_mode && q->mode == SYNC_MODE_PSS) {
    isrran_sync_reset(&q->sfind);
    isrran_sync_reset(&q->strack);
  } else {
    q->sf_idx = 9;
  }
...
int isrran_ue_sync_start_agc(isrran_ue_sync_t* q,
                             ISRRAN_AGC_CALLBACK(set_gain_callback),
                             float min_gain_db,
                             float max_gain_db,
                             float init_gain_value_db)
{
  int n     = isrran_agc_init_uhd(&q->agc, ISRRAN_AGC_MODE_PEAK_AMPLITUDE, 0, set_gain_callback, q->stream);
  q->do_agc = n == 0 ? true : false;
  if (q->do_agc) {
    isrran_agc_set_gain_range(&q->agc, min_gain_db, max_gain_db);
    isrran_agc_set_gain(&q->agc, init_gain_value_db);
    isrran_ue_sync_set_agc_period(q, 4);
  }
  return n;
}

int recv_callback_multi_to_single(void* h, cf_t* x[ISRRAN_MAX_CHANNELS], uint32_t nsamples, isrran_timestamp_t* t)
{
  isrran_ue_sync_t* q = (isrran_ue_sync_t*)h;
  return q->recv_callback_single(q->stream_single, (void*)x[0], nsamples, t);
}

int isrran_ue_sync_init(isrran_ue_sync_t* q,
                        uint32_t          max_prb,
                        bool              search_cell,
                        int(recv_callback)(void*, void*, uint32_t, isrran_timestamp_t*),
                        void* stream_handler)
{
  int ret = isrran_ue_sync_init_multi(q, max_prb, search_cell, recv_callback_multi_to_single, 1, (void*)q);
  q->recv_callback_single = recv_callback;
  q->stream_single        = stream_handler;
  return ret;
}

int isrran_ue_sync_init_multi(isrran_ue_sync_t* q,
                              uint32_t          max_prb,
                              bool              search_cell,
...
{
  return isrran_ue_sync_init_multi_decim(q, max_prb, search_cell, recv_callback, nof_rx_antennas, stream_handler, 1);
}

int isrran_ue_sync_init_multi_decim(
    isrran_ue_sync_t* q,
    uint32_t          max_prb,
    bool              search_cell,
    int(recv_callback)(void*, cf_t* [ISRRAN_MAX_CHANNELS], uint32_t, isrran_timestamp_t*),
    ...
    int      decimate)
{
  return isrran_ue_sync_init_multi_decim_mode(
      q, max_prb, search_cell, recv_callback, nof_rx_antennas, stream_handler, 1, SYNC_MODE_PSS);
}

int isrran_ue_sync_init_multi_decim_mode(
    isrran_ue_sync_t* q,
    uint32_t          max_prb,
    bool              search_cell,
    int(recv_callback)(void*, cf_t* [ISRRAN_MAX_CHANNELS], uint32_t, isrran_timestamp_t*),
    ...
    isrran_ue_sync_mode_t mode)
{
  ...
  if (q != NULL && stream_handler != NULL && nof_rx_antennas <= ISRRAN_MAX_CHANNELS && recv_callback != NULL) {
    ret = ISRRAN_ERROR;
    ...
    if (q->mode == SYNC_MODE_PSS) {
      if (isrran_sync_init_decim(&q->sfind, q->frame_len, q->frame_len, q->fft_size, q->decimate)) {
        ERROR("Error initiating sync find");
        goto clean_exit;
      }
      if (search_cell) {
        if (isrran_sync_init(&q->strack, q->frame_len, TRACK_FRAME_SIZE, q->fft_size)) {
          ERROR("Error initiating sync track");
          goto clean_exit;
        }
      } else {
        if (isrran_sync_init(&q->strack,
                             q->frame_len,
                             ISRRAN_MAX(TRACK_FRAME_SIZE, ISRRAN_CP_LEN_NORM(1, q->fft_size)),
                             q->fft_size)) {
          ERROR("Error initiating sync track");
          goto clean_exit;
        }
      ...
      isrran_sync_set_cfo_i_enable(&q->sfind, false);
      isrran_sync_set_cfo_pss_enable(&q->sfind, true);
      isrran_sync_set_pss_filt_enable(&q->sfind, true);
      isrran_sync_set_sss_eq_enable(&q->sfind, false);

      // During track, we do CFO correction outside the sync object
      isrran_sync_set_cfo_i_enable(&q->strack, false);
      isrran_sync_set_cfo_pss_enable(&q->strack, true);
      isrran_sync_set_pss_filt_enable(&q->strack, true);
      isrran_sync_set_sss_eq_enable(&q->strack, false);

      // TODO: CP detection not working very well. Not supporting Extended CP right now
      isrran_sync_cp_en(&q->strack, false);
      isrran_sync_cp_en(&q->sfind, false);

      // Enable SSS on find and disable in track
      isrran_sync_sss_en(&q->sfind, true);
      isrran_sync_sss_en(&q->strack, false);
    }
...
uint32_t isrran_ue_sync_sf_len(isrran_ue_sync_t* q)
{
  return q->frame_len;
}

void isrran_ue_sync_free(isrran_ue_sync_t* q)
{
  if (q->do_agc) {
...
int isrran_ue_sync_set_cell(isrran_ue_sync_t* q, isrran_cell_t cell)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;

  if (q != NULL && isrran_nofprb_isvalid(cell.nof_prb)) {
    ...
    q->cell     = cell;
    q->fft_size = isrran_symbol_sz(q->cell.nof_prb);
    q->sf_len   = ISRRAN_SF_LEN(q->fft_size);
    ...
    if (q->mode == SYNC_MODE_PSS) {
      ...
      if (isrran_sync_resize(&q->sfind, q->frame_len, q->frame_len, q->fft_size)) {
        ERROR("Error setting cell sync find");
        return ISRRAN_ERROR;
      }
      if (cell.id == 1000) {
        if (isrran_sync_resize(&q->strack, q->frame_len, TRACK_FRAME_SIZE, q->fft_size)) {
          ERROR("Error setting cell sync track");
          return ISRRAN_ERROR;
        }
      } else {
        if (isrran_sync_resize(&q->strack,
                               q->frame_len,
                               ISRRAN_MAX(TRACK_FRAME_SIZE, ISRRAN_CP_LEN_NORM(1, q->fft_size)),
                               q->fft_size)) {
          ERROR("Error setting cell sync track");
      ...
      isrran_sync_set_cp(&q->sfind, cell.cp);
      isrran_sync_set_cp(&q->strack, cell.cp);
      ...
      if (cell.id == 1000) {
        ...
        isrran_sync_set_cfo_ema_alpha(&q->sfind, 0.8);
        isrran_sync_set_cfo_ema_alpha(&q->strack, 0.1);

        isrran_sync_set_em_alpha(&q->sfind, 1);

        isrran_sync_set_threshold(&q->sfind, 2.0);
        isrran_sync_set_threshold(&q->strack, 1.2);

        isrran_sync_set_cfo_ema_alpha(&q->sfind, 0.1);
        isrran_sync_set_cfo_ema_alpha(&q->strack, 0.1);

      } else {
        isrran_sync_set_frame_type(&q->sfind, cell.frame_type);
        isrran_sync_set_frame_type(&q->strack, cell.frame_type);

        isrran_sync_set_N_id_2(&q->sfind, cell.id % 3);
        isrran_sync_set_N_id_2(&q->strack, cell.id % 3);

        isrran_sync_set_N_id_1(&q->sfind, cell.id / 3);
        // track does not correlate SSS so no need to generate sequences

        isrran_sync_set_cfo_ema_alpha(&q->sfind, 0.1);
        isrran_sync_set_cfo_ema_alpha(&q->strack, DEFAULT_CFO_EMA_TRACK);
        ...
        isrran_sync_set_em_alpha(&q->sfind, 1);
        isrran_sync_set_threshold(&q->sfind, 3.0);

        isrran_sync_set_em_alpha(&q->strack, 0.0);
        isrran_sync_set_threshold(&q->strack, 1.5);
      }

      // When cell is unknown, do CP CFO correction
      isrran_sync_set_cfo_cp_enable(&q->sfind, true, q->frame_len < 10000 ? 14 : 3);
      q->cfo_correct_enable_find = false;
    }
...
void isrran_ue_sync_set_nof_find_frames(isrran_ue_sync_t* q, uint32_t nof_frames)
{
  q->nof_avg_find_frames = nof_frames;
}

void isrran_ue_sync_set_frame_type(isrran_ue_sync_t* q, isrran_frame_type_t frame_type)
{
  isrran_sync_set_frame_type(&q->strack, frame_type);
  isrran_sync_set_frame_type(&q->sfind, frame_type);
}

isrran_frame_type_t isrran_ue_sync_get_frame_type(isrran_ue_sync_t* q)
{
  return q->sfind.frame_type;
}

void isrran_ue_sync_get_last_timestamp(isrran_ue_sync_t* q, isrran_timestamp_t* timestamp)
{
  *timestamp = q->last_timestamp;
}

void isrran_ue_sync_set_cfo_loop_bw(isrran_ue_sync_t* q,
                                    float             bw_pss,
                                    float             bw_ref,
                                    float             pss_tol,
                                    float             ref_tol,
...
void isrran_ue_sync_set_cfo_ema(isrran_ue_sync_t* q, float ema)
{
  isrran_sync_set_cfo_ema_alpha(&q->strack, ema);
}

void isrran_ue_sync_set_cfo_ref(isrran_ue_sync_t* q, float ref_cfo)
{
...
uint32_t isrran_ue_sync_get_sfn(isrran_ue_sync_t* q)
{
  return q->frame_number;
}

uint32_t isrran_ue_sync_get_sfidx(isrran_ue_sync_t* q)
{
  return q->sf_idx;
}

void isrran_ue_sync_set_cfo_i_enable(isrran_ue_sync_t* q, bool enable)
{
  printf("Warning: Setting integer CFO detection/correction. This is experimental!\n");
  isrran_sync_set_cfo_i_enable(&q->strack, enable);
  isrran_sync_set_cfo_i_enable(&q->sfind, enable);
}

float isrran_ue_sync_get_cfo(isrran_ue_sync_t* q)
{
  return 15000 * q->cfo_current_value;
}

void isrran_ue_sync_cp_en(isrran_ue_sync_t* q, bool enabled)
{
  isrran_sync_cp_en(&q->strack, enabled);
  isrran_sync_cp_en(&q->sfind, enabled);
}

void isrran_ue_sync_copy_cfo(isrran_ue_sync_t* q, isrran_ue_sync_t* src_obj)
{
  // Copy find object internal CFO averages
  isrran_sync_copy_cfo(&q->sfind, &src_obj->sfind);
  // Current CFO is tracking-phase CFO of previous object
  q->cfo_current_value = src_obj->cfo_current_value;
  q->cfo_is_copied     = true;
}

void isrran_ue_sync_set_cfo_tol(isrran_ue_sync_t* q, float cfo_tol)
{
  isrran_sync_set_cfo_tol(&q->strack, cfo_tol);
  isrran_sync_set_cfo_tol(&q->sfind, cfo_tol);
}

float isrran_ue_sync_get_sfo(isrran_ue_sync_t* q)
{
  return q->mean_sample_offset / 5e-3;
}

int isrran_ue_sync_get_last_sample_offset(isrran_ue_sync_t* q)
{
  return q->last_sample_offset;
}

void isrran_ue_sync_set_sfo_correct_period(isrran_ue_sync_t* q, uint32_t nof_subframes)
{
  q->sample_offset_correct_period = nof_subframes;
}

void isrran_ue_sync_set_sfo_ema(isrran_ue_sync_t* q, float ema_coefficient)
{
  q->sfo_ema = ema_coefficient;
}

void isrran_ue_sync_set_N_id_2(isrran_ue_sync_t* q, uint32_t N_id_2)
{
  if (!q->file_mode) {
    isrran_ue_sync_reset(q);
    isrran_sync_set_N_id_2(&q->strack, N_id_2);
    isrran_sync_set_N_id_2(&q->sfind, N_id_2);
  }
}

void isrran_ue_sync_set_agc_period(isrran_ue_sync_t* q, uint32_t period)
{
  q->agc_period = period;
}

static int find_peak_ok(isrran_ue_sync_t* q, cf_t* input_buffer[ISRRAN_MAX_CHANNELS])
{
  if (isrran_sync_sss_detected(&q->sfind)) {
    /* Get the subframe index (0 or 5) */
    q->sf_idx = (isrran_sync_get_sf_idx(&q->sfind) + q->nof_recv_sf) % 10;
  } else if (isrran_sync_sss_available(&q->sfind)) {
  ...
  DEBUG("Found peak %d at %d, value %.3f, Cell_id: %d CP: %s",
        q->frame_find_cnt,
        q->peak_idx,
        isrran_sync_get_peak_value(&q->sfind),
        q->cell.id,
        isrran_cp_string(q->cell.cp));
  ...
  if (q->frame_find_cnt >= q->nof_avg_find_frames || q->peak_idx < 2 * q->fft_size) {
    ...
    if (!q->cfo_is_copied) {
      q->cfo_current_value = isrran_sync_get_cfo(&q->sfind);
    }
    isrran_sync_cfo_reset(&q->strack, 0.0f);
  }
...
static int track_peak_ok(isrran_ue_sync_t* q, uint32_t track_idx)
{
  ...
  if (q->sample_offset_correct_period) {
    ...
    q->mean_sample_offset = ISRRAN_VEC_EMA((float)q->last_sample_offset, q->mean_sample_offset, q->sfo_ema);
  } else {
   ...
   */
  DEBUG("TRACK: cfo_current=%f, cfo_strack=%f", 15000 * q->cfo_current_value, 15000 * isrran_sync_get_cfo(&q->strack));
  if (15000 * fabsf(isrran_sync_get_cfo(&q->strack)) > q->cfo_pss_min) {
    q->cfo_current_value += isrran_sync_get_cfo(&q->strack) * q->cfo_loop_bw_pss;
    q->pss_stable_cnt = 0;
...
static int track_peak_no(isrran_ue_sync_t* q)
{
  ...
  if (q->frame_no_cnt >= TRACK_MAX_LOST) {
    ...
    INFO("Tracking peak not found. Peak %.3f, %d lost", isrran_sync_get_peak_value(&q->strack), (int)q->frame_no_cnt);
    return 1;
  }
}

static int receive_samples(isrran_ue_sync_t* q, cf_t* input_buffer[ISRRAN_MAX_CHANNELS], const uint32_t max_num_samples)
{
  ...
  cf_t* ptr[ISRRAN_MAX_CHANNELS] = {NULL};
...
int isrran_ue_sync_zerocopy(isrran_ue_sync_t* q,
                            ...
                            const uint32_t    max_num_samples)
{
  ...
  if (q != NULL && input_buffer != NULL) {
    if (q->file_mode) {
      int n = isrran_filesource_read_multi(&q->file_source, (void**)input_buffer, q->sf_len, q->nof_rx_antennas);
      if (n < 0) {
      ...
      if (n == 0) {
        if (q->file_wrap_enable) {
          ...
          q->sf_idx = 9;
          n = isrran_filesource_read_multi(&q->file_source, (void**)input_buffer, q->sf_len, q->nof_rx_antennas);
          if (n < 0) {
            ERROR("Error reading input file");
            return ISRRAN_ERROR;
          }
      ...
      switch (q->state) {
        case SF_FIND:
          ...
          if (q->mode == SYNC_MODE_PSS) {
            ret = isrran_ue_sync_run_find_pss_mode(q, input_buffer);
          } else if (q->mode == SYNC_MODE_GNSS) {
            ret = isrran_ue_sync_run_find_gnss_mode(q, input_buffer, max_num_samples);
          }
        ...
        case SF_TRACK:
          ...
          if (q->mode == SYNC_MODE_PSS) {
            isrran_ue_sync_run_track_pss_mode(q, input_buffer);
          } else {
            isrran_ue_sync_run_track_gnss_mode(q, input_buffer);
          }
          break;
        default:
...
int isrran_ue_sync_run_find_pss_mode(isrran_ue_sync_t* q, cf_t* input_buffer[ISRRAN_MAX_CHANNELS])
{
  int ret = ISRRAN_ERROR;
  int n   = isrran_sync_find(&q->sfind, input_buffer[0], 0, &q->peak_idx);
  ...
  INFO("SYNC FIND: sf_idx=%d, ret=%d, peak_pos=%d, peak_value=%.2f, mean_cp_cfo=%.2f, mean_pss_cfo=%.2f, "
       ...
       15 * isrran_sync_get_cfo(&q->sfind));

  return ret;
};

int isrran_ue_sync_run_track_pss_mode(isrran_ue_sync_t* q, cf_t* input_buffer[ISRRAN_MAX_CHANNELS])
{
  int      ret       = ISRRAN_ERROR;
  uint32_t track_idx = 0;
  /* Every SF idx 0 and 5, find peak around known position q->peak_idx */
  if ((q->sfind.frame_type == ISRRAN_FDD && (q->sf_idx == 0 || q->sf_idx == 5)) ||
      (q->sfind.frame_type == ISRRAN_TDD && (q->sf_idx == 1 || q->sf_idx == 6))) {
    ...
    int n = isrran_sync_find(&q->strack, input_buffer[0], pss_idx, &track_idx);
    switch (n) {
      case ISRRAN_SYNC_ERROR:
    ...
    INFO("SYNC TRACK: sf_idx=%d, ret=%d, peak_pos=%d, peak_value=%.2f, mean_cp_cfo=%.2f, mean_pss_cfo=%.2f, "
         ...
         15 * isrran_sync_get_cfo(&q->strack));
  } else {
...
int isrran_ue_sync_run_find_gnss_mode(isrran_ue_sync_t* q,
                                      cf_t*             input_buffer[ISRRAN_MAX_CHANNELS],
                                      const uint32_t    max_num_samples)
{
  ...
  isrran_timestamp_t ts_next_rx, ts_next_rx_tmp, ts_tmp;
  isrran_timestamp_copy(&ts_next_rx, &q->last_timestamp);
  ts_next_rx.full_secs++;
  ts_next_rx.frac_secs = 0.0;

  isrran_timestamp_copy(&ts_next_rx_tmp, &ts_next_rx);
  ...
  isrran_timestamp_sub(&ts_next_rx_tmp, q->last_timestamp.full_secs, q->last_timestamp.frac_secs);
  isrran_timestamp_sub(&ts_next_rx_tmp, 0, 0.001); ///< account for samples that have already been rx'ed

  uint64_t align_len = isrran_timestamp_uint64(&ts_next_rx_tmp, q->sf_len * 1000);
  ...
  while (align_len > q->sf_len) {
    uint32_t actual_rx_len = ISRRAN_MIN(align_len, q->sf_len);
    q->recv_callback(q->stream, dummy_offset_buffer, actual_rx_len, &q->last_timestamp);

    isrran_timestamp_copy(&ts_tmp, &ts_next_rx);
    isrran_timestamp_sub(&ts_tmp, q->last_timestamp.full_secs, q->last_timestamp.frac_secs);
    isrran_timestamp_sub(&ts_tmp, 0, 0.001); ///< account for samples that have already been rx'ed
    align_len = isrran_timestamp_uint64(&ts_tmp, q->sf_len * 1000);
  ...
  if (isrran_ue_sync_set_tti_from_timestamp(q, &q->last_timestamp)) {
...
int isrran_ue_sync_run_track_gnss_mode(isrran_ue_sync_t* q, cf_t* input_buffer[ISRRAN_MAX_CHANNELS])
{
  ...
  // update SF index
  q->sf_idx = ((int)round(q->last_timestamp.frac_secs * 1e3)) % ISRRAN_NOF_SF_X_FRAME;
 ...
 * @return ISRRAN_SUCCESS on success
 */
int isrran_ue_sync_set_tti_from_timestamp(isrran_ue_sync_t* q, isrran_timestamp_t* rx_timestamp)
{
