...
#include "isrran/phy/channel/fading.h"
...
const static float excess_tap_delay_ns[4][ISRRAN_CHANNEL_FADING_MAXTAPS] = {
...
const static float relative_power_db[4][ISRRAN_CHANNEL_FADING_MAXTAPS] = {
...
static inline int parse_model(isrran_channel_fading_t* q, const char* str)
{
  int      ret    = ISRRAN_SUCCESS;
  uint32_t offset = 3;

  if (strncmp("none", str, 4) == 0) {
    q->model = isrran_channel_fading_model_none;
    offset   = 4;
  } else if (strncmp("epa", str, 3) == 0) {
    q->model = isrran_channel_fading_model_epa;
  } else if (strncmp("eva", str, 3) == 0) {
    q->model = isrran_channel_fading_model_eva;
  } else if (strncmp("etu", str, 3) == 0) {
    q->model = isrran_channel_fading_model_etu;
  } else {
    ret = ISRRAN_ERROR;
  }
...
static inline cf_t
get_doppler_dispersion(isrran_channel_fading_t* q, float t, float F_d, float* alpha, float* a, float* b)
{
#ifdef LV_HAVE_SSE
  const float recN   = 1.0f / sqrtf(ISRRAN_CHANNEL_FADING_NTERMS);
  cf_t        ret    = 0;
  ...
  for (int i = 0; i < ISRRAN_CHANNEL_FADING_NTERMS; i += 4) {
  ...
  for (uint32_t i = 0; i < ISRRAN_CHANNEL_FADING_NTERMS; i++) {
...
{
  float amplitude = isrran_convert_dB_to_power(power_db);
  float O         = (delay_ns * 1e-9f * srate + path_delay) / (float)N;
  cf_t  a0        = amplitude / N;

  isrran_vec_gen_sine(a0, -O, buf, N);
}

static inline void generate_taps(isrran_channel_fading_t* q, float time)
{
  // Generate taps
  for (int i = 0; i < nof_taps[q->model]; i++) {
    ...
    if (i) {
      ...
      isrran_vec_sc_prod_ccc(q->h_tap[i], a, q->temp, q->N);

      // Add to frequency response, shifts FFT at same time
      isrran_vec_sum_ccc(q->h_freq, &q->temp[q->N / 2], q->h_freq, q->N / 2);
      isrran_vec_sum_ccc(&q->h_freq[q->N / 2], q->temp, &q->h_freq[q->N / 2], q->N / 2);
    } else {
      // Copy tap frequency response
      isrran_vec_sc_prod_ccc(&q->h_tap[i][q->N / 2], a, q->h_freq, q->N / 2);
      isrran_vec_sc_prod_ccc(&q->h_tap[i][0], a, &q->h_freq[q->N / 2], q->N / 2);
    }
  }
  // at this stage, q->h_freq should contain the frequency response
}

static inline void filter_segment(isrran_channel_fading_t* q, const cf_t* input, cf_t* output, uint32_t nsamples)
{
  // Fill Input vector
  isrran_vec_cf_copy(q->temp, input, nsamples);
  isrran_vec_cf_zero(&q->temp[nsamples], q->N - nsamples);

  // Do FFT
  isrran_dft_run_c_zerocopy(&q->fft, q->temp, q->y_freq);

  // Apply channel
  isrran_vec_prod_ccc(q->y_freq, q->h_freq, q->y_freq, q->N);

  // Do iFFT
  isrran_dft_run_c_zerocopy(&q->ifft, q->y_freq, q->temp);

  // Add state
  isrran_vec_sum_ccc(q->temp, q->state, q->temp, q->state_len);

  // Copy the first nsamples into the output
  isrran_vec_cf_copy(output, q->temp, nsamples);

  // Copy the rest of the samples into the state
  q->state_len = q->N - nsamples;
  isrran_vec_cf_copy(q->state, &q->temp[nsamples], q->state_len);
}

int isrran_channel_fading_init(isrran_channel_fading_t* q, double srate, const char* model, uint32_t seed)
{
  int ret = ISRRAN_ERROR;

  if (q) {
    // Parse model
    if (parse_model(q, model) != ISRRAN_SUCCESS) {
    ...
    q->N          = ISRRAN_MAX(1U << fft_min_pow, (uint32_t)(srate / (15e3f * 4.0f)));
    q->path_delay = q->N / 4;
    q->state_len  = 0;

    // Initialise random number
    isrran_random_t* random = isrran_random_init(seed);

    // Initialise values for each tap
    for (uint32_t i = 0; i < nof_taps[q->model]; i++) {
      // Random Jakes model Coeffients
      for (uint32_t j = 0; (float)j < ISRRAN_CHANNEL_FADING_NTERMS; j++) {
        q->coeff_a[i][j]     = isrran_random_uniform_real_dist(random, 0, 2.0f * (float)M_PI);
        q->coeff_b[i][j]     = isrran_random_uniform_real_dist(random, 0, 2.0f * (float)M_PI);
        q->coeff_alpha[i][j] = ((float)M_PI * ((float)i - (float)0.5f)) / (2.0f * nof_taps[q->model]);
      }

      // Allocate tap frequency response
      q->h_tap[i] = isrran_vec_cf_malloc(q->N);
    ...
    // Plan FFT
    if (isrran_dft_plan_c(&q->fft, q->N, ISRRAN_DFT_FORWARD) != ISRRAN_SUCCESS) {
      fprintf(stderr, "Error: planning fft\n");
      goto clean_exit;
    }

    // Plan iFFT
    if (isrran_dft_plan_c(&q->ifft, q->N, ISRRAN_DFT_BACKWARD) != ISRRAN_SUCCESS) {
      fprintf(stderr, "Error: planning ifft\n");
      goto clean_exit;
    }

    // Allocate memory
    q->temp = isrran_vec_cf_malloc(q->N);
    if (!q->temp) {
      fprintf(stderr, "Error: allocating h_freq\n");
      goto clean_exit;
    }

    q->h_freq = isrran_vec_cf_malloc(q->N);
    if (!q->h_freq) {
      fprintf(stderr, "Error: allocating h_freq\n");
      goto clean_exit;
    }

    q->y_freq = isrran_vec_cf_malloc(q->N);
    if (!q->y_freq) {
      fprintf(stderr, "Error: allocating y_freq\n");
      goto clean_exit;
    }

    q->state = isrran_vec_cf_malloc(q->N);
    if (!q->state) {
      fprintf(stderr, "Error: allocating y_freq\n");
      goto clean_exit;
    }
    isrran_vec_cf_zero(q->state, q->N);
  }
...
void isrran_channel_fading_free(isrran_channel_fading_t* q)
{
  if (q) {
...
double isrran_channel_fading_execute(isrran_channel_fading_t* q,
                                     const cf_t*              in,
                                     cf_t*                    out,
                                     uint32_t                 nsamples,
                                     double                   init_time)
{
  ...
  if (q) {
    while (counter < nsamples) {
      ...
      uint32_t n = ISRRAN_MIN(q->N / 2, nsamples - counter);
