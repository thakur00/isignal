...
#include <isrran/phy/channel/hst.h>
#include <isrran/phy/utils/debug.h>
#include <unistd.h>

static isrran_channel_hst_t hst = {};
...
int main(int argc, char** argv)
{
  ...
  isrran_timestamp_t ts            = {}; // Initialised to zero
  struct timeval     t[3]          = {};
  ...
  input_buffer  = isrran_vec_cf_malloc(size);
  output_buffer = isrran_vec_cf_malloc(size);
  ...
  if (ret == ISRRAN_SUCCESS) {
    isrran_vec_gen_sine(1.0f, 0.0f, input_buffer, size);
  }

  // Initialise delay channel
  if (ret == ISRRAN_SUCCESS) {
    ret = isrran_channel_hst_init(&hst, fd_hz, period_s, init_time_s);
    isrran_channel_hst_update_srate(&hst, srate_hz);
  }

  // Run actual test
  gettimeofday(&t[1], NULL);
  for (int i = 0; i < sim_time_periods && ret == ISRRAN_SUCCESS; i++) {
    for (int j = 0; j < 1000 * period_s; j++) {
      // Run delay channel
      isrran_channel_hst_execute(&hst, input_buffer, output_buffer, size, &ts);

      // Increment timestamp 1ms
      isrran_timestamp_add(&ts, 0, 0.001);

      float ideal_freq = hst.fs_hz;
      float meas_freq  = isrran_vec_estimate_frequency(output_buffer, size) * srate_hz;
