...
static isrran_channel_fading_t channel_fading;
...
int main(int argc, char** argv)
{
  ...
  isrran_dft_plan_c(&ifft, srate / 1000, ISRRAN_DFT_BACKWARD);
  ...
  if (enable_gui) {
    sdrgui_init();
    sdrgui_init_title("ISR Fading channel");
    ...
    isrran_dft_plan_c(&fft, srate / 1000, ISRRAN_DFT_FORWARD);

    fft_buffer = isrran_vec_cf_malloc(srate / 1000);
    if (!fft_buffer) {
      fprintf(stderr, "Error: malloc\n");
      goto clean_exit;
    }

    fft_mag = isrran_vec_f_malloc(srate / 1000);
    if (!fft_mag) {
      fprintf(stderr, "Error: malloc\n");
      goto clean_exit;
    }

    imp = isrran_vec_f_malloc(srate / 1000);
    if (!imp) {
      fprintf(stderr, "Error: malloc\n");
      goto clean_exit;
    }
  ...
  // Initialise channel
  if (isrran_channel_fading_init(&channel_fading, srate, model, 0x12345678)) {
    fprintf(stderr, "Error: initialising fading channel. model=%s, srate=%d\n", model, srate);
    goto clean_exit;
  }

  // Allocate buffers
  input_buffer = isrran_vec_cf_malloc(srate / 1000);
  if (!input_buffer) {
...
#if INPUT_TYPE == 0
  isrran_vec_cf_zero(input_buffer, srate / 1000);
  input_buffer[0] = 1;
#else
  for (int i = 0; i < srate / 1000; i++) {
    input_buffer[i] = cexpf(_Complex_I * ((double)rand() / (double)RAND_MAX) * M_PI / 2.0) / ((double)srate / 1000.0);
  }
  isrran_dft_run_c(&ifft, input_buffer, input_buffer);
#endif

  output_buffer = isrran_vec_cf_malloc(srate / 1000);
  if (!output_buffer) {
  ...
  for (int i = 0; i < duration_ms; i++) {
    ...
    isrran_channel_fading_execute(&channel_fading, input_buffer, output_buffer, srate / 1000, (double)i / 1000.0);
...
#ifdef ENABLE_GUI
    if (enable_gui) {
      isrran_dft_run_c_zerocopy(&fft, output_buffer, fft_buffer);
      isrran_vec_prod_conj_ccc(fft_buffer, fft_buffer, fft_buffer, srate / 1000);
      for (int j = 0; j < srate / 1000; j++) {
        fft_mag[j] = isrran_convert_power_to_dB(__real__ fft_buffer[j]);
      }
      plot_real_setNewData(&plot_fft, fft_mag, srate / 1000);

      for (int j = 0; j < channel_fading.N; j++) {
        fft_mag[j] = isrran_convert_amplitude_to_dB(cabsf(channel_fading.h_freq[j]));
      }
...
clean_exit:
  ...
  isrran_channel_fading_free(&channel_fading);
  return ret;
}
