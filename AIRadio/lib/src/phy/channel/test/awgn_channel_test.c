...
#define CDF(a) ((1 + erff((a)*SQRT1_2)) * .5)
  ...
  float mean = isrran_vec_acc_ff(x, length);
  mean /= length_f;

  isrran_vec_sc_sum_fff(x, -mean, y, length);
  float variance = isrran_vec_dot_prod_fff(y, y, length);
  variance /= length_f - 1;

  // standardize samples
  isrran_vec_sc_prod_fff(y, 1 / sqrtf(variance), y, length);
...
int main(int argc, char** argv)
{
  ...
  // Initialise buffers
  input_buffer  = isrran_vec_cf_malloc(nof_samples);
  output_buffer = isrran_vec_cf_malloc(nof_samples);
  help_buffer   = isrran_vec_f_malloc(2 * nof_samples);
  ...
  // Initialise input
  isrran_vec_cf_zero(input_buffer, nof_samples);

#ifdef ENABLE_GUI
  sdrgui_init();
  sdrgui_init_title("ISR AWGN Channel");
  ...
  fft_out               = isrran_vec_cf_malloc(nof_samples);
  isrran_dft_plan_t fft = {};
  if (isrran_dft_plan_c(&fft, nof_samples, ISRRAN_DFT_FORWARD)) {
  ...
  if (isrran_channel_awgn_init(&awgn, 0) < ISRRAN_SUCCESS) {
  ...
  while (!isnan(n0) && !isinf(n0) && n0 < n0_max) {
    ...
    if (isrran_channel_awgn_set_n0(&awgn, n0) < ISRRAN_SUCCESS) {
    ...
    isrran_channel_awgn_run_c(&awgn, input_buffer, output_buffer, nof_samples);
    gettimeofday(&t[2], NULL);
    get_time_interval(t);

    float power    = isrran_vec_avg_power_cf(output_buffer, nof_samples);
    float power_dB = isrran_convert_power_to_dB(power);
    ...
    isrran_dft_run_c(&fft, output_buffer, fft_out);

    float  min     = +INFINITY;
    float  max     = -INFINITY;
    float* fft_mag = (float*)fft_out;
    for (uint32_t i = 0; i < nof_samples; i++) {
      float mag = isrran_convert_amplitude_to_dB(cabsf(fft_out[i]));

      min = ISRRAN_MIN(min, mag);
      max = ISRRAN_MAX(max, mag);
  ...
  printf("Test n0_min=%.3f; n0_max=%.3f; n0_step=%.3f; nof_samples=%d; %s ... %.1f MSps\n",
         ...
         n0_step,
         nof_samples,
         (ret == ISRRAN_SUCCESS) ? "Passed" : "Failed",
         msps);
