...
static isrran_channel_delay_t delay = {};
...
int main(int argc, char** argv)
{
  ...
  isrran_random_uniform_complex_dist_vector(random_gen, input_buffer, size, -1.0f, +1.0f);

  // Initialise delay channel
  if (isrran_channel_delay_init(&delay, delay_min_us, delay_max_us, delay_period_s, delay_init_time_s, srate_hz) <
      ISRRAN_SUCCESS) {
  ...
  for (int i = 0; i < sim_time_periods; i++) {
    for (int j = 0; j < 1000 * delay_period_s; j++) {
      // Run delay channel
      isrran_channel_delay_execute(&delay, input_buffer, output_buffer, size, &ts);
...
clean_exit:
  ...
  isrran_channel_delay_free(&delay);
