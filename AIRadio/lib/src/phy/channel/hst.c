...
int isrran_channel_hst_init(isrran_channel_hst_t* q, float fd_hz, float period_d, float init_time_s)
{
...
void isrran_channel_hst_update_srate(isrran_channel_hst_t* q, uint32_t srate)
{
  if (q) {
    q->srate_hz = srate;
  }
...
void isrran_channel_hst_execute(isrran_channel_hst_t*     q,
                                cf_t*                     in,
                                cf_t*                     out,
                                uint32_t                  len,
                                const isrran_timestamp_t* ts)
{
  if (q && q->srate_hz) {
    ...
    isrran_vec_apply_cfo(in, -q->fs_hz / q->srate_hz, out, len);
  }
}

void isrran_channel_hst_free(isrran_channel_hst_t* q)
{
  if (q) {
    bzero(q, sizeof(isrran_channel_hst_t));
  }
}
