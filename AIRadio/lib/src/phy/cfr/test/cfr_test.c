...
static isrran_cfr_mode_t cfr_mode        = ISRRAN_CFR_THR_MANUAL;
static float             alpha           = 1.0f;
static bool              dc_empty        = true;
static float             thr_manual      = 1.5f;
static float             max_papr_db     = 8.0f;
static float             ema_alpha       = (float)1 / (float)ISRRAN_CP_NORM_NSYMB;
...
int main(int argc, char** argv)
{
  ...
  isrran_dft_plan_t ofdm_ifft = {};
  isrran_dft_plan_t ofdm_fft  = {};
  ...
  cfr_mode = isrran_cfr_str2mode(cfr_mode_str);
  if (cfr_mode == ISRRAN_CFR_THR_INVALID) {
  ...
  while (nof_prb <= max_prb) {
    const uint32_t symbol_sz      = (force_symbol_sz) ? force_symbol_sz : (uint32_t)isrran_symbol_sz(nof_prb);
    const uint32_t symbol_bw      = nof_prb * ISRRAN_NRE;
    const uint32_t nof_symb_slot  = ISRRAN_CP_NSYMB(cp);
    const uint32_t nof_symb_frame = nof_symb_slot * ISRRAN_NOF_SLOTS_PER_SF * ISRRAN_NOF_SF_X_FRAME;
    ...
    input     = isrran_vec_cf_malloc(total_nof_re);
    output    = isrran_vec_cf_malloc(total_nof_re);
    error     = isrran_vec_cf_malloc(total_nof_re);
    acpr_buff = isrran_vec_f_malloc(total_nof_symb);
    if (!input || !output || !error || !acpr_buff) {
      perror("malloc");
      goto clean_exit;
    }
    isrran_vec_cf_zero(input, total_nof_re);
    isrran_vec_cf_zero(output, total_nof_re);
    isrran_vec_cf_zero(error, total_nof_re);
    isrran_vec_f_zero(acpr_buff, total_nof_symb);

    // Set the parameters for the CFR.
    isrran_cfr_cfg_t cfr_tx_cfg = {};
    cfr_tx_cfg.cfr_enable       = true;
    cfr_tx_cfg.symbol_sz        = symbol_sz;
    cfr_tx_cfg.symbol_bw        = nof_prb * ISRRAN_NRE;
    ...
    if (!isrran_cfr_params_valid(&cfr_tx_cfg)) {
      ERROR("Invalid CFR configuration");
      goto clean_exit;
    }

    if (isrran_cfr_init(&cfr, &cfr_tx_cfg)) {
      ERROR("Error initializing CFR");
      goto clean_exit;
    }

    if (isrran_dft_plan_c(&ofdm_ifft, (int)symbol_sz, ISRRAN_DFT_BACKWARD)) {
      ERROR("Creating IFFT plan");
      goto clean_exit;
    }
    isrran_dft_plan_set_norm(&ofdm_ifft, true);
    if (isrran_dft_plan_c(&ofdm_fft, (int)symbol_sz, ISRRAN_DFT_FORWARD)) {
      ERROR("Creating FFT plan");
      goto clean_exit;
    }
    isrran_dft_plan_set_norm(&ofdm_fft, true);

    // Generate Random data
    cf_t* ofdm_symb = NULL;
    for (int i = 0; i < total_nof_symb; i++) {
      ofdm_symb = input + i * symbol_sz;
      isrran_random_uniform_complex_dist_vector(random_gen, ofdm_symb + dc_empty, symbol_bw / 2, -1.0f, +1.0f);
      isrran_random_uniform_complex_dist_vector(
          random_gen, ofdm_symb + symbol_sz - symbol_bw / 2, symbol_bw / 2, -1.0f, +1.0f);
      acpr_buff[i] = isrran_vec_acpr_c(ofdm_symb, symbol_bw / 2 + dc_empty, symbol_bw / 2, symbol_sz);
      isrran_dft_run_c(&ofdm_ifft, ofdm_symb, ofdm_symb);
    }
    // compute the average intput ACPR
    acpr_in_dB = isrran_vec_acc_ff(acpr_buff, total_nof_symb) / (float)total_nof_symb;
    acpr_in_dB = isrran_convert_power_to_dB(acpr_in_dB);
    ...
    // Compute metrics
    isrran_vec_sub_ccc(input, output, error, total_nof_re);

    float power_in  = isrran_vec_avg_power_cf(input, total_nof_re);
    float power_err = isrran_vec_avg_power_cf(error, total_nof_re);

    mse_dB  = isrran_convert_power_to_dB(power_err);
    nmse_dB = isrran_convert_power_to_dB(power_err / power_in);
    evm     = 100 * sqrtf(power_err / power_in);

    float snr_dB = isrran_convert_power_to_dB(power_in / power_err);

    float papr_in  = isrran_convert_power_to_dB(isrran_vec_papr_c(input, total_nof_re));
    float papr_out = isrran_convert_power_to_dB(isrran_vec_papr_c(output, total_nof_re));

    ofdm_symb = NULL;
    for (int i = 0; i < total_nof_symb; i++) {
      ofdm_symb = output + i * symbol_sz;
      isrran_dft_run_c(&ofdm_fft, ofdm_symb, ofdm_symb);
      acpr_buff[i] = isrran_vec_acpr_c(ofdm_symb, symbol_bw / 2 + dc_empty, symbol_bw / 2, symbol_sz);
    }

    // Compute the output average ACPR
    acpr_out_dB = isrran_vec_acc_ff(acpr_buff, total_nof_symb) / (float)total_nof_symb;
    acpr_out_dB = isrran_convert_power_to_dB(acpr_out_dB);
    ...
    isrran_dft_plan_free(&ofdm_ifft);
    isrran_dft_plan_free(&ofdm_fft);
    free(input);
    free(output);
    free(error);
    free(acpr_buff);
