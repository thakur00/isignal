...
int main(int argc, char** argv)
{
  ...
  cf_t *             d[ISRRAN_MAX_CODEWORDS], *x[ISRRAN_MAX_LAYERS], *dp[ISRRAN_MAX_CODEWORDS];
  isrran_tx_scheme_t type;
  int                nof_symb_cw[ISRRAN_MAX_CODEWORDS];
  int                n[2];
  ...
  for (i = 0; i < nof_cw; i++) {
    d[i] = isrran_vec_cf_malloc(nof_symb_cw[i]);
    if (!d[i]) {
      perror("malloc");
      exit(-1);
    }
    dp[i] = isrran_vec_cf_malloc(nof_symb_cw[i]);
    if (!dp[i]) {
      perror("malloc");
      exit(-1);
    }
  ...
  for (i = 0; i < nof_cw; i++) {
    for (j = 0; j < nof_symb_cw[i]; j++) {
      d[i][j] = isrran_random_uniform_complex_dist(random, -10, 10);
    }
  }

  /* layer mapping */
  if ((symbols_layer = isrran_layermap_type(d, x, nof_cw, nof_layers, nof_symb_cw, type)) < 0) {
    ERROR("Error layer mapper encoder");
    exit(-1);
  }

  /* layer de-mapping */
  if (isrran_layerdemap_type(x, dp, nof_layers, nof_cw, nof_symbols / nof_layers, nof_symb_cw, type) < 0) {
    ERROR("Error layer mapper encoder");
    exit(-1);
  }
