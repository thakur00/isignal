...
int main(int argc, char** argv)
{
  ...
  float    sinr_1l[ISRRAN_MAX_CODEBOOKS];
  float    sinr_2l[ISRRAN_MAX_CODEBOOKS];
  float    cn;
  uint32_t pmi[2];
  uint32_t nof_symbols = (uint32_t)ISRRAN_SF_LEN_RE(6, ISRRAN_CP_NORM);
  int      ret         = ISRRAN_ERROR;

  /* Allocate channels */
  for (int i = 0; i < ISRRAN_MAX_PORTS; i++) {
    for (int j = 0; j < ISRRAN_MAX_PORTS; j++) {
      h[i][j] = isrran_vec_cf_malloc(nof_symbols);
      if (!h[i][j]) {
        goto clean;
      }
      isrran_vec_cf_zero(h[i][j], nof_symbols);
    }
  }
  ...
  for (int c = 0; c < PMI_SELECT_TEST_NOF_CASES; c++) {
    ...
    ret = isrran_precoding_pmi_select(h, nof_symbols, noise_estimate, 1, &pmi[0], sinr_1l);
    if (ret < 0) {
    ...
    ret = isrran_precoding_pmi_select(h, nof_symbols, noise_estimate, 2, &pmi[1], sinr_2l);
    if (ret < 0) {
    ...
    /* Condition number */
    if (isrran_precoding_cn(h, 2, 2, nof_symbols, &cn)) {
