...
inline cf_t isrran_mat_cf_recip_gen(cf_t a)
{
  return conjf(a) / (crealf(a) * crealf(a) + cimagf(a) * cimagf(a));
}

/* Generic implementation for 2x2 determinant */
inline cf_t isrran_mat_2x2_det_gen(cf_t a00, cf_t a01, cf_t a10, cf_t a11)
{
  return a00 * a11 - a01 * a10;
}

/* 2x2 Matrix inversion, generic implementation */
inline void isrran_mat_2x2_inv_gen(cf_t a00, cf_t a01, cf_t a10, cf_t a11, cf_t* r00, cf_t* r01, cf_t* r10, cf_t* r11)
{
  cf_t div = isrran_mat_cf_recip_gen(isrran_mat_2x2_det_gen(a00, a01, a10, a11));
  *r00     = a11 * div;
  *r01     = -a01 * div;
  *r10     = -a10 * div;
  *r11     = a00 * div;
...
{
  cf_t _norm = isrran_mat_cf_recip_gen(isrran_mat_2x2_det_gen(h00, h01, h10, h11)) * norm;
  *x0        = (y0 * h11 - h01 * y1) * _norm;
  *x1        = (y1 * h00 - h10 * y0) * _norm;
}

/* Generic implementation for Minimum Mean Squared Error (MMSE) solver */
inline void isrran_mat_2x2_mmse_csi_gen(cf_t   y0,
                                        cf_t   y1,
                                        cf_t   h00,
                                        cf_t   h01,
                                        cf_t   h10,
                                        ...
                                        float  norm)
{
  ...
  cf_t a_det_rcp = isrran_mat_cf_recip_gen(isrran_mat_2x2_det_gen(a00, a01, a10, a11));
...
/* Generic implementation for Minimum Mean Squared Error (MMSE) solver */
void isrran_mat_2x2_mmse_gen(cf_t  y0,
                             cf_t  y1,
                             cf_t  h00,
                             cf_t  h01,
                             cf_t  h10,
...
{
  float csi0, csi1;
  isrran_mat_2x2_mmse_csi_gen(y0, y1, h00, h01, h10, h11, x0, x1, &csi0, &csi1, noise_estimate, norm);
}

int isrran_mat_2x2_cn(cf_t h00, cf_t h01, cf_t h10, cf_t h11, float* cn)
{
  // 1. A = H * H' (A = A')
  float a00 =
  ...
  xmin = ISRRAN_MAX(xmin, 1e-9f);
  xmax = ISRRAN_MIN(xmax, 1e+9f);
...
inline __m128 isrran_mat_cf_recip_sse(__m128 a)
{
...
inline __m128 isrran_mat_2x2_det_sse(__m128 a00, __m128 a01, __m128 a10, __m128 a11)
{
  return _mm_sub_ps(_MM_PROD_PS(a00, a11), _MM_PROD_PS(a01, a10));
}

/* SSE implementation for Zero Forcing (ZF) solver */
inline void isrran_mat_2x2_zf_sse(__m128  y0,
                                  __m128  y1,
                                  __m128  h00,
                                  __m128  h01,
                                  __m128  h10,
                                  ...
                                  float   norm)
{
  ...
  __m128 detrec = _mm_mul_ps(isrran_mat_cf_recip_sse(det), _mm_set1_ps(norm));
...
inline void isrran_mat_2x2_mmse_sse(__m128  y0,
                                    __m128  y1,
                                    __m128  h00,
                                    __m128  h01,
                                    __m128  h10,
                                    ...
                                    float   norm)
{
  ...
  __m128 b11 = a00;
  _norm      = _mm_mul_ps(_norm, isrran_mat_cf_recip_sse(isrran_mat_2x2_det_sse(a00, a01, a10, a11)));
...
inline __m256 isrran_mat_cf_recip_avx(__m256 a)
{
...
inline __m256 isrran_mat_2x2_det_avx(__m256 a00, __m256 a01, __m256 a10, __m256 a11)
{
#ifdef LV_HAVE_FMA
...
inline void isrran_mat_2x2_zf_avx(__m256  y0,
                                  __m256  y1,
                                  __m256  h00,
                                  __m256  h01,
                                  __m256  h10,
                                  ...
                                  float   norm)
{
  __m256 det    = isrran_mat_2x2_det_avx(h00, h01, h10, h11);
  __m256 detrec = _mm256_mul_ps(isrran_mat_cf_recip_avx(det), _mm256_set1_ps(norm));
...
inline void isrran_mat_2x2_mmse_avx(__m256  y0,
                                    __m256  y1,
                                    __m256  h00,
                                    __m256  h01,
                                    __m256  h10,
                                    ...
                                    float   norm)
{
  ...
  __m256 b11 = a00;
  _norm      = _mm256_mul_ps(_norm, isrran_mat_cf_recip_avx(isrran_mat_2x2_det_avx(a00, a01, a10, a11)));
...
int isrran_matrix_NxN_inv_init(isrran_matrix_NxN_inv_t* q, uint32_t N)
{
  int ret = ISRRAN_SUCCESS;

  if (q && N) {
    // Set all to zero
    bzero(q, sizeof(isrran_matrix_NxN_inv_t));

    q->N = N;

    q->row_buffer = isrran_vec_cf_malloc(N * 2);
    if (!q->row_buffer) {
      perror("malloc");
      ret = ISRRAN_ERROR;
    }
    ...
    if (!ret) {
      q->matrix = isrran_vec_cf_malloc(N * N * 2);
      if (!q->matrix) {
        perror("malloc");
        ret = ISRRAN_ERROR;
      }
...
static inline void isrran_vec_sc_prod_ccc_simd_inline(const cf_t* x, const cf_t h, cf_t* z, const int len)
{
  int i = 0;
...
static inline void isrran_vec_sub_fff_simd_inline(const float* x, const float* y, float* z, const int len)
{
  int i = 0;
...
static inline void isrran_vec_sub_ccc_simd_inline(const cf_t* x, const cf_t* y, cf_t* z, const int len)
{
  isrran_vec_sub_fff_simd_inline((float*)x, (float*)y, (float*)z, 2 * len);
}
...
void isrran_matrix_NxN_inv_run(isrran_matrix_NxN_inv_t* q, cf_t* in, cf_t* out)
{
  if (q && in && out) {
    int   N = q->N;
    cf_t* tmp_src_ptr;
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      isrran_vec_sc_prod_ccc_simd_inline(tmp_src_ptr, reciprocal(b), tmp_src_ptr, 2 * N);
      ...
      for (int j = 0; j < N - i - 1; j++) {
        ...
        if (a != 0.0f && b != 0.0f) {
          ...
          isrran_vec_sc_prod_ccc_simd_inline(tmp_dst_ptr, reciprocal(a), tmp_dst_ptr, 2 * N);
          isrran_vec_sub_ccc_simd_inline(tmp_dst_ptr, tmp_src_ptr, tmp_dst_ptr, 2 * N);
        }
      }
    }
    isrran_vec_sc_prod_ccc_simd_inline(q->matrix, reciprocal(q->matrix[0]), q->matrix, 2 * N);
    ...
    isrran_vec_sc_prod_ccc_simd_inline(&q->matrix[2 * N * (N - 1)],
...
void isrran_matrix_NxN_inv_free(isrran_matrix_NxN_inv_t* q)
{
  if (q) {
