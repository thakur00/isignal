...
void isrran_bit_interleaver_init(isrran_bit_interleaver_t* q, uint16_t* interleaver, uint32_t nof_bits)
{
  static const uint8_t mask[] = {0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1};

  bzero(q, sizeof(isrran_bit_interleaver_t));

  q->interleaver = isrran_vec_u16_malloc(nof_bits);
  q->byte_idx    = isrran_vec_u16_malloc(nof_bits);
  q->bit_mask    = isrran_vec_u8_malloc(nof_bits);
  q->nof_bits    = nof_bits;
...
void isrran_bit_interleaver_free(isrran_bit_interleaver_t* q)
{
  if (q->interleaver) {
    free(q->interleaver);
  }
...
void isrran_bit_interleaver_run(isrran_bit_interleaver_t* q, uint8_t* input, uint8_t* output, uint16_t w_offset)
{
  static const uint8_t mask[]     = {0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1};
...
void isrran_bit_interleave_i(uint8_t* input, uint8_t* output, uint32_t* interleaver, uint32_t nof_bits)
{
  isrran_bit_interleave_i_w_offset(input, output, interleaver, nof_bits, 0);
}

void isrran_bit_interleave_i_w_offset(uint8_t*  input,
                                      uint8_t*  output,
...
void isrran_bit_interleave(uint8_t* input, uint8_t* output, uint16_t* interleaver, uint32_t nof_bits)
{
  isrran_bit_interleave_w_offset(input, output, interleaver, nof_bits, 0);
}

void isrran_bit_interleave_w_offset(uint8_t*  input,
                                    uint8_t*  output,
 ...
 */
void isrran_bit_copy(uint8_t* dst, uint32_t dst_offset, uint8_t* src, uint32_t src_offset, uint32_t nof_bits)
{
...
void isrran_bit_unpack_vector(const uint8_t* packed, uint8_t* unpacked, int nof_bits)
{
  uint32_t i, nbytes;
  nbytes = nof_bits / 8;
  for (i = 0; i < nbytes; i++) {
    isrran_bit_unpack(packed[i], &unpacked, 8);
  }
  if (nof_bits % 8) {
    isrran_bit_unpack(packed[i] >> (8 - nof_bits % 8), &unpacked, nof_bits % 8);
  }
}

void isrran_bit_unpack_l(uint64_t value, uint8_t** bits, int nof_bits)
{
  int i;
 ...
 */
void isrran_bit_unpack(uint32_t value, uint8_t** bits, int nof_bits)
{
  int i;
 ...
 * @param[out] bits Points to buffer pointer. The buffer pointer will be advanced by nof_bits
 */
void isrran_bit_unpack_lsb(uint32_t value, uint8_t** bits, int nof_bits)
{
  int i;
  for (i = 0; i < nof_bits; i++) {
...
void isrran_bit_pack_vector(uint8_t* unpacked, uint8_t* packed, int nof_bits)
{
  uint32_t i, nbytes;
  nbytes = nof_bits / 8;
...
#else  /* LV_HAVE_SSE */
  for (i = 0; i < nbytes; i++) {
    packed[i] = isrran_bit_pack(&unpacked, 8);
  }
#endif /* LV_HAVE_SSE */

  if (nof_bits % 8) {
    packed[i] = isrran_bit_pack(&unpacked, nof_bits % 8);
    packed[i] <<= 8 - (nof_bits % 8);
  }
}

uint32_t isrran_bit_pack(uint8_t** bits, int nof_bits)
{
  int      i;
  uint32_t value = 0;
...
uint64_t isrran_bit_pack_l(uint8_t** bits, int nof_bits)
{
  int      i;
  uint64_t value = 0;
...
void isrran_bit_fprint(FILE* stream, uint8_t* bits, int nof_bits)
{
  int i;
...
uint32_t isrran_bit_diff(const uint8_t* x, const uint8_t* y, int nbits)
{
  uint32_t errors = 0;
...
uint32_t isrran_bit_count(uint32_t n)
{
  int c;
  for (c = 0; n; c++)
    n &= n - 1;
