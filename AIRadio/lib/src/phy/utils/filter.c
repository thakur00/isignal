...
#define ISRRAN_NUM_FILTERS 8
#define ISRRAN_MAX_FILTER_SIZE 11

// clang-format off
float isrran_filt_decim2[ISRRAN_NUM_FILTERS][ISRRAN_MAX_FILTER_SIZE] = 
{
...
float isrran_filt_decim3[ISRRAN_NUM_FILTERS][ISRRAN_MAX_FILTER_SIZE] = 
{
...
float isrran_filt_decim4[ISRRAN_NUM_FILTERS][ISRRAN_MAX_FILTER_SIZE]  = 
{
...
void isrran_filt_decim_cc_init(isrran_filt_cc_t* q, int factor, int order)
{
  ...
  q->num_taps     = order + 1;
  q->is_decimator = true;
  q->taps         = isrran_vec_f_malloc(q->num_taps);

  switch (q->factor) {
    case 2:
      for (int i = 0; i < (q->num_taps); i++)
        q->taps[i] = isrran_filt_decim2[(q->num_taps) - 4][i];
      break;
    case 3:
      for (int i = 0; i < (q->num_taps); i++)
        q->taps[i] = isrran_filt_decim3[(q->num_taps) - 4][i];
      break;
    case 4:
      for (int i = 0; i < (q->num_taps); i++)
        q->taps[i] = isrran_filt_decim4[(q->num_taps) - 4][i];
      break;
    default:
...
void isrran_filt_decim_cc_execute(isrran_filt_cc_t* q, cf_t* input, cf_t* downsampled_input, cf_t* output, int size)
{
  ...
  isrran_downsample_cc(input, downsampled_input + (q->num_taps - 1), q->factor, size);

  for (int i = 0; i < size / q->factor; i++) {
    output[i] = isrran_vec_dot_prod_cfc(&(downsampled_input[i]), q->taps, q->num_taps);
  }
}
