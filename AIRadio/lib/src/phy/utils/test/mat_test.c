...
#define RANDOM_F() isrran_random_uniform_real_dist(random_gen, -1.0f, +1.0f)
#define RANDOM_CF() isrran_random_uniform_complex_dist(random_gen, -1.0f, +1.0f)
...
static bool test_zf_solver_gen(void)
{
  ...
  isrran_mat_2x2_zf_gen(y0, y1, h00, h01, h10, h11, &x0, &x1, 1.0f);
...
static bool test_mmse_solver_gen(void)
{
  ...
  isrran_mat_2x2_mmse_gen(y0, y1, h00, h01, h10, h11, &x0, &x1, 0.0f, 1.0f);
...
static bool test_zf_solver_simd(void)
{
  ...
  simd_cf_t _y0 = isrran_simd_cf_set1(y0_1);
  simd_cf_t _y1 = isrran_simd_cf_set1(y1_1);

  simd_cf_t _h00 = isrran_simd_cf_set1(h00_1);
  simd_cf_t _h01 = isrran_simd_cf_set1(h01_1);
  simd_cf_t _h10 = isrran_simd_cf_set1(h10_1);
  simd_cf_t _h11 = isrran_simd_cf_set1(h11_1);

  simd_cf_t _x0, _x1;

  isrran_mat_2x2_zf_simd(_y0, _y1, _h00, _h01, _h10, _h11, &_x0, &_x1, 1.0f);

  isrran_simd_aligned cf_t x0[ISRRAN_SIMD_CF_SIZE];
  isrran_simd_aligned cf_t x1[ISRRAN_SIMD_CF_SIZE];

  isrran_simd_cfi_store(x0, _x0);
  isrran_simd_cfi_store(x1, _x1);
...
static bool test_mmse_solver_simd(void)
{
  ...
  cf_t x0_gold[ISRRAN_SIMD_CF_SIZE];
  cf_t x1_gold[ISRRAN_SIMD_CF_SIZE];
  cf_t h00[ISRRAN_SIMD_CF_SIZE];
  cf_t h01[ISRRAN_SIMD_CF_SIZE];
  cf_t h10[ISRRAN_SIMD_CF_SIZE];
  cf_t h11[ISRRAN_SIMD_CF_SIZE];
  cf_t y0[ISRRAN_SIMD_CF_SIZE];
  cf_t y1[ISRRAN_SIMD_CF_SIZE];
  ...
  simd_cf_t _y0 = isrran_simd_cfi_loadu(y0);
  simd_cf_t _y1 = isrran_simd_cfi_loadu(y1);

  simd_cf_t _h00 = isrran_simd_cfi_loadu(h00);
  simd_cf_t _h01 = isrran_simd_cfi_loadu(h01);
  simd_cf_t _h10 = isrran_simd_cfi_loadu(h10);
  simd_cf_t _h11 = isrran_simd_cfi_loadu(h11);

  simd_cf_t _x0, _x1;

  isrran_mat_2x2_mmse_simd(_y0, _y1, _h00, _h01, _h10, _h11, &_x0, &_x1, 0.0f, 1.0f);
...
static bool test_vec_dot_prod_ccc(void)
{
  ...
  res = isrran_vec_dot_prod_ccc(a, b, 14);
...
bool test_matrix_inv(void)
{
  ...
  isrran_matrix_NxN_inv_t matrix_nxn_inv = {};

  isrran_matrix_NxN_inv_init(&matrix_nxn_inv, N);

  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      x[i * N + j] = isrran_random_uniform_complex_dist(random_gen, -1.0f, +1.0f);
    }
  }

  isrran_matrix_NxN_inv_run(&matrix_nxn_inv, x, y);

  isrran_matrix_NxN_inv_free(&matrix_nxn_inv);
