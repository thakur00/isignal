...
int test_normal_read_write(isrran_ringbuffer_t* q, uint8_t* in, uint8_t* out, int len)
{
  isrran_ringbuffer_write(q, in, len / 2);
  isrran_ringbuffer_write(q, &in[len / 2], len / 2);

  for (int i = 0; i < 4; i++) {
    isrran_ringbuffer_read(q, &out[(len / 4) * i], len / 4);
  }
...
int test_overflow_write(isrran_ringbuffer_t* q, uint8_t* in, uint8_t* out, int len)
{
  int ret = isrran_ringbuffer_write(q, in, len / 2);
  ret     = isrran_ringbuffer_write(q, &in[len / 2], len / 2 + 2);
  if (ret != (len / 2 + 2)) {
    ret = -1;
  }
  return ret;
...
void* write_thread(void* args_)
{
  ...
  for (int i = 0; i < M; i++) {
    res = isrran_ringbuffer_write_block(args->buf, args->in, args->len);
  }
  if (res < 0) {
    args->res = res;
  }
...
void* read_thread(void* args_)
{
  ...
  struct thread_args_t* args = (struct thread_args_t*)args_;
  for (int i = 0; i < M; i++) {
    res = isrran_ringbuffer_read(args->buf, &args->out[args->len * i], args->len);
  }
  if (res < 0) {
    args->res = res;
  }
...
int main(int argc, char** argv)
{
  ...
  uint8_t*            in  = isrran_vec_u8_malloc(N * 2);
  uint8_t*            out = isrran_vec_u8_malloc(N * 10);
  isrran_ringbuffer_t ring_buf;
  isrran_ringbuffer_init(&ring_buf, N);
  ...
  bzero(out, N * 10);
  isrran_ringbuffer_reset(&ring_buf);
  ...
  isrran_ringbuffer_stop(&ring_buf);
  isrran_ringbuffer_free(&ring_buf);
  free(in);
  free(out);
  printf("Done\n");
  return ret;
