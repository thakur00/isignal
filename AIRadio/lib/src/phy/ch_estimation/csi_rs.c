...
#define RESOURCE_ERROR(R)                                                                                              \
  do {                                                                                                                 \
    ...
    isrran_csi_rs_resource_mapping_info(R, res_info_str, (uint32_t)sizeof(res_info_str));                              \
    ERROR("Unhandled configuration %s", res_info_str);                                                                 \
  } while (false)

static int csi_rs_location_f(const isrran_csi_rs_resource_mapping_t* resource, uint32_t i)
{
  uint32_t count           = 0;
  uint32_t nof_freq_domain = 0;
  uint32_t mul             = 1;
  switch (resource->row) {
    case isrran_csi_rs_resource_mapping_row_1:
      nof_freq_domain = ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_ROW1;
      break;
    case isrran_csi_rs_resource_mapping_row_2:
      nof_freq_domain = ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_ROW2;
      break;
    case isrran_csi_rs_resource_mapping_row_4:
      nof_freq_domain = ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_ROW4;
      mul             = 4;
      break;
    case isrran_csi_rs_resource_mapping_row_other:
      nof_freq_domain = ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_OTHER;
      mul             = 2;
      break;
  }
...
static int csi_rs_location_get_k_list(const isrran_csi_rs_resource_mapping_t* resource,
                                      uint32_t                                j,
                                      uint32_t                                k_list[CSI_RS_MAX_SUBC_PRB])
{
  ...
  if (k0 < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Row 1
  if (resource->row == isrran_csi_rs_resource_mapping_row_1 && resource->nof_ports == 1 &&
      resource->density == isrran_csi_rs_resource_mapping_density_three && resource->cdm == isrran_csi_rs_cdm_nocdm &&
      j == 0) {
    k_list[0] = k0;
    k_list[1] = k0 + 4;
    k_list[2] = k0 + 8;
  ...
  // Row 2
  if (resource->row == isrran_csi_rs_resource_mapping_row_2 && resource->cdm == isrran_csi_rs_cdm_nocdm) {
    if (resource->density == isrran_csi_rs_resource_mapping_density_one ||
        resource->density == isrran_csi_rs_resource_mapping_density_dot5_even ||
        resource->density == isrran_csi_rs_resource_mapping_density_dot5_odd) {
      k_list[0] = k0;
      return 1;
    }
  }
  ...
  // Row 4
  if (resource->row == isrran_csi_rs_resource_mapping_row_4 && resource->nof_ports == 4 &&
      resource->density == isrran_csi_rs_resource_mapping_density_one && resource->cdm == isrran_csi_rs_cdm_fd_cdm2) {
    if (j == 0) {
      k_list[0] = k0;
      k_list[1] = k0 + 1;
      return 2;
...
// Table 7.4.1.5.3-1: CSI-RS locations within a slot
static int csi_rs_location_get_l_list(const isrran_csi_rs_resource_mapping_t* resource,
                                      uint32_t                                j,
...
static uint32_t csi_rs_cinit(const isrran_carrier_nr_t*          carrier,
                             const isrran_slot_cfg_t*            slot_cfg,
                             const isrran_csi_rs_nzp_resource_t* resource,
                             uint32_t                            l)
{
  uint32_t n    = ISRRAN_SLOT_NR_MOD(carrier->scs, slot_cfg->idx);
  uint32_t n_id = resource->scrambling_id;

  return ISRRAN_SEQUENCE_MOD((((ISRRAN_NSYMB_PER_SLOT_NR * n + l + 1UL) * (2UL * n_id + 1UL)) << 10UL) + n_id);
}

bool isrran_csi_rs_send(const isrran_csi_rs_period_and_offset_t* periodicity, const isrran_slot_cfg_t* slot_cfg)
{
...
static int csi_rs_nof_cdm_groups(const isrran_csi_rs_resource_mapping_t* resource)
{
  // Row 1
  if (resource->row == isrran_csi_rs_resource_mapping_row_1 && resource->nof_ports == 1 &&
      resource->density == isrran_csi_rs_resource_mapping_density_three && resource->cdm == isrran_csi_rs_cdm_nocdm) {
...
bool isrran_csi_rs_resource_mapping_is_valid(const isrran_csi_rs_resource_mapping_t* res)
{
  if (res == NULL) {
    return false;
  }
  ...
  uint32_t l_list[CSI_RS_MAX_SYMBOLS_SLOT] = {};
  if (csi_rs_location_get_l_list(res, 0, l_list) < ISRRAN_SUCCESS) {
    return false;
  }

  uint32_t k_list[CSI_RS_MAX_SUBC_PRB] = {};
  if (csi_rs_location_get_k_list(res, 0, k_list) < ISRRAN_SUCCESS) {
    return false;
  }
...
uint32_t isrran_csi_rs_resource_mapping_info(const isrran_csi_rs_resource_mapping_t* res, char* str, uint32_t str_len)
{
  uint32_t len = 0;
  ...
  switch (res->cdm) {
    case isrran_csi_rs_cdm_nocdm:
      cdm_str = "nocdm";
      break;
    case isrran_csi_rs_cdm_fd_cdm2:
      cdm_str = "FD-CDM2";
      break;
    case isrran_csi_rs_cdm_cdm4_fd2_td2:
      cdm_str = "CDM4-FD2-TD2";
      break;
    case isrran_csi_rs_cdm_cdm8_fd2_td4:
  ...
  switch (res->density) {
    case isrran_csi_rs_resource_mapping_density_three:
      density_str = "3";
      break;
    case isrran_csi_rs_resource_mapping_density_dot5_even:
      density_str = ".5 (even)";
      break;
    case isrran_csi_rs_resource_mapping_density_dot5_odd:
      density_str = ".5 (odd)";
      break;
    case isrran_csi_rs_resource_mapping_density_one:
      density_str = "1";
      break;
    case isrran_csi_rs_resource_mapping_density_spare:
      density_str = "spare";
      break;
  }

  char frequency_domain_alloc[ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_MAX + 1];
  isrran_vec_sprint_bin(frequency_domain_alloc,
                        ISRRAN_CSI_RS_NOF_FREQ_DOMAIN_ALLOC_MAX + 1,
...
uint32_t csi_rs_count(isrran_csi_rs_density_t density, uint32_t nprb)
{
  switch (density) {
    case isrran_csi_rs_resource_mapping_density_three:
      return nprb * 3;
...
static uint32_t csi_rs_rb_begin(const isrran_carrier_nr_t* carrier, const isrran_csi_rs_resource_mapping_t* m)
{
  uint32_t ret = ISRRAN_MAX(carrier->start, m->freq_band.start_rb);

  if ((m->density == isrran_csi_rs_resource_mapping_density_dot5_even && ret % 2 == 1) ||
      (m->density == isrran_csi_rs_resource_mapping_density_dot5_odd && ret % 2 == 0)) {
    ret++;
  }
...
static uint32_t csi_rs_rb_end(const isrran_carrier_nr_t* carrier, const isrran_csi_rs_resource_mapping_t* m)
{
  return ISRRAN_MIN(carrier->start + carrier->nof_prb, m->freq_band.start_rb + m->freq_band.nof_rb);
}

static uint32_t csi_rs_rb_stride(const isrran_csi_rs_resource_mapping_t* m)
{
  uint32_t ret = 1;

  // Special .5 density cases
  if (m->density == isrran_csi_rs_resource_mapping_density_dot5_even ||
      m->density == isrran_csi_rs_resource_mapping_density_dot5_odd) {
    // Skip one RB
    ret = 2;
  }
...
int isrran_csi_rs_append_resource_to_pattern(const isrran_carrier_nr_t*              carrier,
                                             ...
                                             isrran_re_pattern_list_t*               re_pattern_list)
{
  ...
  if (nof_cdm_groups < ISRRAN_SUCCESS) {
  ...
  for (int j = 0; j < nof_cdm_groups; j++) {
    ...
    if (isrran_re_pattern_merge(re_pattern_list, &pattern) < ISRRAN_SUCCESS) {
...
int isrran_csi_rs_nzp_put_resource(const isrran_carrier_nr_t*          carrier,
                                   const isrran_slot_cfg_t*            slot_cfg,
                                   const isrran_csi_rs_nzp_resource_t* resource,
                                   cf_t*                               grid)
{
  ...
  float beta = isrran_convert_dB_to_amplitude((float)resource->power_control_offset);
  if (!isnormal(beta)) {
    beta = 1.0f;
  }
  ...
  for (int l_idx = 0; l_idx < nof_l; l_idx++) {
    ...
    // Skip unallocated RB
    isrran_sequence_state_advance(&sequence_state, 2 * csi_rs_count(resource->resource_mapping.density, rb_begin));
    ...
    for (uint32_t n = rb_begin; n < rb_end; n += rb_stride) {
      for (uint32_t k_idx = 0; k_idx < nof_k; k_idx++) {
        ...
        uint32_t k = ISRRAN_NRE * n + k_list[k_idx];

        // Do we need more r?
        if (r_idx >= 64) {
          // ... Generate a bunch of it!
          isrran_sequence_state_gen_f(&sequence_state, M_SQRT1_2 * beta, (float*)r, 64 * 2);
          r_idx = 0;
        }

        // Put CSI in grid
        grid[l * ISRRAN_NRE * carrier->nof_prb + k] = r[r_idx++];
      }
    }
  }
...
int isrran_csi_rs_nzp_put_set(const isrran_carrier_nr_t*     carrier,
                              const isrran_slot_cfg_t*       slot_cfg,
                              ...
                              cf_t*                          grid)
{
  ...
  for (uint32_t i = 0; i < set->count; i++) {
    // Skip resource
    if (!isrran_csi_rs_send(&set->data[i].periodicity, slot_cfg)) {
      continue;
    }

    // Put resource
    if (isrran_csi_rs_nzp_put_resource(carrier, slot_cfg, &set->data[i], grid) < ISRRAN_SUCCESS) {
...
static int csi_rs_nzp_measure_resource(const isrran_carrier_nr_t*          carrier,
                                       const isrran_slot_cfg_t*            slot_cfg,
                                       const isrran_csi_rs_nzp_resource_t* resource,
                                       const cf_t*                         grid,
                                       csi_rs_nzp_resource_measure_t*      measure)
{
  ...
  float avg_k_stride = (float)((k_list[0] + ISRRAN_NRE) - k_list[nof_k - 1]);
  ...
  ISRRAN_MEM_ZERO(measure, csi_rs_nzp_resource_measure_t, 1);
  ...
  for (int l_idx = 0; l_idx < nof_l; l_idx++) {
    ...
    cf_t     lse[CSI_RS_MAX_SUBC_PRB * ISRRAN_MAX_PRB_NR];
    uint32_t count_re = 0;
    ...
    for (uint32_t n = rb_begin; n < rb_end; n += rb_stride) {
      for (uint32_t k_idx = 0; k_idx < nof_k; k_idx++) {
        ...
        lse[count_re++] = grid[l * ISRRAN_NRE * carrier->nof_prb + k];
      }
    }
    ...
    // Compute LSE
    cf_t r[CSI_RS_MAX_SUBC_PRB * ISRRAN_MAX_PRB_NR];
    isrran_sequence_state_gen_f(&sequence_state, M_SQRT1_2, (float*)r, 2 * count_re);
    isrran_vec_prod_conj_ccc(lse, r, lse, count_re);

    // Compute average delay
    float delay = isrran_vec_estimate_frequency(lse, (int)count_re);
    delay_acc += delay;

    // Pre-compensate delay to avoid RSRP measurements get affected by average delay
    isrran_vec_apply_cfo(lse, delay, lse, (int)count_re);

    // Compute EPRE
    epre_acc += isrran_vec_avg_power_cf(lse, count_re);

    // Compute correlation
    corr_acc += isrran_vec_acc_cc(lse, count_re) / (float)count_re;
  }
  ...
  measure->delay_us = 1e6f * delay_acc / ((float)nof_l * ISRRAN_SUBC_SPACING_NR(carrier->scs));
  measure->nof_re   = nof_l * nof_re;

  return ISRRAN_SUCCESS;
}

static int csi_rs_nzp_measure_set(const isrran_carrier_nr_t*     carrier,
                                  const isrran_slot_cfg_t*       slot_cfg,
                                  const isrran_csi_rs_nzp_set_t* set,
                                  const cf_t*                    grid,
                                  csi_rs_nzp_resource_measure_t  measurements[ISRRAN_PHCH_CFG_MAX_NOF_CSI_RS_PER_SET])
{
  uint32_t count = 0;
  ...
  for (uint32_t i = 0; i < set->count; i++) {
    ...
    // Perform measurement
    if (csi_rs_nzp_measure_resource(carrier, slot_cfg, &set->data[i], grid, &measurements[count]) < ISRRAN_SUCCESS) {
...
int isrran_csi_rs_nzp_measure(const isrran_carrier_nr_t*          carrier,
                              const isrran_slot_cfg_t*            slot_cfg,
                              const isrran_csi_rs_nzp_resource_t* resource,
                              const cf_t*                         grid,
                              isrran_csi_trs_measurements_t*      measure)
{
  ...
  if (csi_rs_nzp_measure_resource(carrier, slot_cfg, resource, grid, &m) < ISRRAN_SUCCESS) {
    ERROR("Error measuring NZP-CSI-RS resource");
    return ISRRAN_ERROR;
  }
  ...
  // Copy measurements
  measure->epre     = m.epre;
  measure->rsrp     = ISRRAN_CSQABS(m.corr);
  ...
  measure->rsrp_dB = isrran_convert_power_to_dB(measure->rsrp);
  measure->epre_dB = isrran_convert_power_to_dB(measure->epre);
  measure->n0_dB   = isrran_convert_power_to_dB(measure->n0);
  measure->snr_dB  = measure->rsrp_dB - measure->n0_dB;

  return ISRRAN_SUCCESS;
}

int isrran_csi_rs_nzp_measure_trs(const isrran_carrier_nr_t*     carrier,
                                  const isrran_slot_cfg_t*       slot_cfg,
                                  const isrran_csi_rs_nzp_set_t* set,
                                  const cf_t*                    grid,
                                  isrran_csi_trs_measurements_t* measure)
{
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    rsrp_sum += ISRRAN_CSQABS(measurements[i].corr) / (float)count;
  ...
  float cfo_max = 0.0f;
  for (uint32_t i = 1; i < count; i++) {
    float time_diff    = isrran_symbol_distance_s(measurements[i - 1].l0, measurements[i].l0, carrier->scs);
    ...
    cfo_max = ISRRAN_MIN(cfo_max_temp, cfo_max);
  }
...
int isrran_csi_rs_nzp_measure_channel(const isrran_carrier_nr_t*         carrier,
                                      const isrran_slot_cfg_t*           slot_cfg,
                                      const isrran_csi_rs_nzp_set_t*     set,
                                      const cf_t*                        grid,
                                      isrran_csi_channel_measurements_t* measure)
{
  ...
  float n0_db = isrran_convert_power_to_dB(n0);

  // Set measurements
  measure->cri               = measurements[0].cri;
  measure->wideband_rsrp_dBm = isrran_convert_power_to_dB(rsrp_sum);
  measure->wideband_epre_dBm = isrran_convert_power_to_dB(epre_sum);
...
static int csi_rs_zp_measure_resource(const isrran_carrier_nr_t*         carrier,
                                      const isrran_slot_cfg_t*           slot_cfg,
                                      const isrran_csi_rs_zp_resource_t* resource,
                                      const cf_t*                        grid,
                                      csi_rs_zp_resource_measure_t*      measure)
{
  ...
  // Initialise measurement
  ISRRAN_MEM_ZERO(measure, csi_rs_zp_resource_measure_t, 1);
  ...
  for (int l_idx = 0; l_idx < nof_l; l_idx++) {
    ...
    // Temporal Least Square Estimates
    cf_t     temp[CSI_RS_MAX_SUBC_PRB * ISRRAN_MAX_PRB_NR];
    uint32_t count_re = 0;
    ...
    for (uint32_t n = rb_begin; n < rb_end; n += rb_stride) {
      for (uint32_t k_idx = 0; k_idx < nof_k; k_idx++) {
        ...
        temp[count_re++] = grid[l * ISRRAN_NRE * carrier->nof_prb + k];
      }
    }
    ...
    // Compute EPRE
    epre_acc += isrran_vec_avg_power_cf(temp, count_re);
  }
...
static int csi_rs_zp_measure_set(const isrran_carrier_nr_t*    carrier,
                                 const isrran_slot_cfg_t*      slot_cfg,
                                 const isrran_csi_rs_zp_set_t* set,
                                 const cf_t*                   grid,
                                 csi_rs_zp_resource_measure_t  measurements[ISRRAN_PHCH_CFG_MAX_NOF_CSI_RS_PER_SET])
{
  uint32_t count = 0;
  ...
  for (uint32_t i = 0; i < set->count; i++) {
    ...
    // Perform measurement
    if (csi_rs_zp_measure_resource(carrier, slot_cfg, &set->data[i], grid, &measurements[count]) < ISRRAN_SUCCESS) {
      ERROR("Error measuring NZP-CSI-RS resource");
      return ISRRAN_ERROR;
    }
    count++;
...
int isrran_csi_rs_zp_measure_channel(const isrran_carrier_nr_t*         carrier,
                                     const isrran_slot_cfg_t*           slot_cfg,
                                     const isrran_csi_rs_zp_set_t*      set,
                                     const cf_t*                        grid,
                                     isrran_csi_channel_measurements_t* measure)
...
uint32_t isrran_csi_rs_measure_info(const isrran_csi_trs_measurements_t* measure, char* str, uint32_t str_len)
{
  uint32_t len = 0;
  ...
  if (isnormal(measure->cfo_hz_max)) {
    len = isrran_print_check(str, str_len, len, "cfo_hz=%+.1f cfo_hz_max=%+.1f", measure->cfo_hz, measure->cfo_hz_max);
  }
