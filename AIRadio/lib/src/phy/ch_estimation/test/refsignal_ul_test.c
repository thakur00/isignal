...
isrran_cell_t cell = {
    ...
    ISRRAN_PHICH_NORM,
    ISRRAN_PHICH_R_1_6, // PHICH length
    ISRRAN_FDD,
...
int main(int argc, char** argv)
{
  ...
  if (isrran_refsignal_ul_set_cell(&refs, cell)) {
    ERROR("Error initializing UL reference signal");
    goto do_exit;
  }

  signal = isrran_vec_cf_malloc(2 * ISRRAN_NRE * cell.nof_prb);
  if (!signal) {
    perror("malloc");
    goto do_exit;
  }
  ...
  for (int n = 6; n < cell.nof_prb; n++) {
    for (int delta_ss = 29; delta_ss < ISRRAN_NOF_DELTA_SS; delta_ss++) {
      for (int cshift = 0; cshift < ISRRAN_NOF_CSHIFT; cshift++) {
        for (int h = 0; h < 3; h++) {
          for (int sf_idx = 0; sf_idx < 10; sf_idx++) {
            for (int cshift_dmrs = 0; cshift_dmrs < ISRRAN_NOF_CSHIFT; cshift_dmrs++) {
              uint32_t nof_prb         = n;
              ...
              isrran_refsignal_dmrs_pusch_gen(&refs, &pusch_cfg, nof_prb, sf_idx, cshift_dmrs, signal);
              ...
              gettimeofday(&t[1], NULL);
              isrran_refsignal_isr_gen(&refs, &isr_cfg, &pusch_cfg, sf_idx, signal);
              gettimeofday(&t[2], NULL);
              get_time_interval(t);
              printf("ISR ExecTime: %ld us\n", t[0].tv_usec);
            }
          }
        }
      }
