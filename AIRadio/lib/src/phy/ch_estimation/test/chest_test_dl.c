...
int main(int argc, char** argv)
{
  ...
  uint32_t num_re = 2 * cell.nof_prb * ISRRAN_NRE * ISRRAN_CP_NSYMB(cell.cp);
  ...
  if (isrran_chest_dl_init(&est, cell.nof_prb, 1)) {
    ERROR("Error initializing equalizer");
    goto do_exit;
  }
  while (cid <= max_cid) {
    cell.id = cid;
    if (isrran_chest_dl_set_cell(&est, cell)) {
      ERROR("Error initializing equalizer");
      goto do_exit;
    }
    ...
    for (uint32_t sf_idx = 0; sf_idx < 1; sf_idx++) {
      ...
      for (uint32_t n_port = 0; n_port < cell.nof_ports; n_port++) {
        ...
        isrran_refsignal_cs_put_sf(&est.csr_refs, &sf_cfg, n_port, input);

        for (i = 0; i < 2 * ISRRAN_CP_NSYMB(cell.cp); i++) {
          for (j = 0; j < cell.nof_prb * ISRRAN_NRE; j++) {
            float x = -1 + (float)i / ISRRAN_CP_NSYMB(cell.cp) + cosf(2 * M_PI * (float)j / cell.nof_prb / ISRRAN_NRE);
      ...
      cf_t* input_m[ISRRAN_MAX_PORTS];
      input_m[0] = input;

      struct timeval t[3];
      gettimeofday(&t[1], NULL);
      for (int k = 0; k < 100; k++) {
        isrran_chest_dl_estimate(&est, &sf_cfg, input_m, &res);
      }
      ...
      gettimeofday(&t[1], NULL);
      for (int k = 0; k < 100; k++) {
        isrran_predecoding_single(input, ce, output, NULL, num_re, 1.0f, 0);
      }
      gettimeofday(&t[2], NULL);
      get_time_interval(t);
      printf("CHEQ-ZF: %f us\n", (float)t[0].tv_usec / 100);
      ...
      gettimeofday(&t[1], NULL);
      for (int k = 0; k < 100; k++) {
        isrran_predecoding_single(input, ce, output, NULL, num_re, 1.0f, res.noise_estimate);
      }
      gettimeofday(&t[2], NULL);
      get_time_interval(t);
