 ...
 */
int isrran_nbiot_dci_rar_to_ul_grant(isrran_nbiot_dci_rar_grant_t* rar,
                                     isrran_ra_nbiot_ul_grant_t*   grant,
                                     uint32_t                      rx_tti)
{
  // create DCI from rar grant
  isrran_ra_nbiot_ul_dci_t dci;
  bzero(&dci, sizeof(isrran_ra_nbiot_ul_dci_t));
  dci.i_rep      = rar->n_rep;
  dci.i_sc       = rar->i_sc;
  dci.i_mcs      = rar->i_mcs;
  dci.i_delay    = rar->i_delay;
  dci.sc_spacing = (rar->sc_spacing == 1) ? ISRRAN_NPUSCH_SC_SPACING_15000 : ISRRAN_NPUSCH_SC_SPACING_3750;

  // use DCI to fill default UL grant values
  grant->format = ISRRAN_NPUSCH_FORMAT1; // UL-SCH is always format 1
  if (isrran_ra_nbiot_ul_rar_dci_to_grant(&dci, grant, rx_tti)) {
    fprintf(stderr, "Error converting RAR DCI to grant.\n");
    return ISRRAN_ERROR;
  }

  if (ISRRAN_VERBOSE_ISINFO()) {
    isrran_ra_nbiot_ul_grant_fprint(stdout, grant);
  }
  return ISRRAN_SUCCESS;
}

/* Unpack RAR UL grant as defined in Section 16.3.3 of 36.213 */
void isrran_nbiot_dci_rar_grant_unpack(isrran_nbiot_dci_rar_grant_t* rar,
                                       const uint8_t                 grant[ISRRAN_NBIOT_RAR_GRANT_LEN])
{
  uint8_t* grant_ptr = (uint8_t*)grant;
  rar->sc_spacing    = isrran_bit_pack(&grant_ptr, 1);
  rar->i_sc          = isrran_bit_pack(&grant_ptr, 6);
  rar->i_delay       = isrran_bit_pack(&grant_ptr, 2);
  rar->n_rep         = isrran_bit_pack(&grant_ptr, 3);
  rar->i_mcs         = isrran_bit_pack(&grant_ptr, 3);
}

// Creates the UL NPUSCH resource allocation grant from a DCI format N0 message
int isrran_nbiot_dci_msg_to_ul_grant(const isrran_dci_msg_t*          msg,
                                     ...
                                     const isrran_npusch_sc_spacing_t spacing)
{
  ...
  if (msg != NULL && ul_dci != NULL && grant != NULL) {
    ...
    bzero(ul_dci, sizeof(isrran_ra_nbiot_ul_dci_t));
    bzero(grant, sizeof(isrran_ra_nbiot_ul_grant_t));

    if (isrran_dci_msg_unpack_npusch(msg, ul_dci)) {
      return ret;
    }

    if (isrran_ra_nbiot_ul_dci_to_grant(ul_dci, grant, rx_tti, spacing)) {
      return ret;
    }

    if (ISRRAN_VERBOSE_ISINFO()) {
      isrran_ra_npusch_fprint(stdout, ul_dci);
      isrran_ra_nbiot_ul_grant_fprint(stdout, grant);
    }
 ...
 */
int isrran_nbiot_dci_msg_to_dl_grant(const isrran_dci_msg_t*     msg,
                                     ...
                                     const isrran_nbiot_mode_t   mode)
{
  ...
  if (msg != NULL && grant != NULL && dl_dci != NULL) {
    ...
    bzero(dl_dci, sizeof(isrran_ra_nbiot_dl_dci_t));
    bzero(grant, sizeof(isrran_ra_nbiot_dl_grant_t));

    bool crc_is_crnti = false;
    if (msg_rnti >= ISRRAN_CRNTI_START && msg_rnti <= ISRRAN_CRNTI_END) {
      crc_is_crnti = true;
    }
    isrran_dci_format_t tmp = msg->format;
    ret                     = isrran_dci_msg_unpack_npdsch(msg, dl_dci, crc_is_crnti);
    if (ret) {
      fprintf(stderr, "Can't unpack DCI message %s (%d)\n", isrran_dci_format_string(tmp), tmp);
      return ret;
    }
    ret = isrran_ra_nbiot_dl_dci_to_grant(dl_dci, grant, sfn, sf_idx, r_max, false, mode);
    if (ret) {
    ...
    if (ISRRAN_VERBOSE_ISINFO()) {
      isrran_nbiot_dl_dci_fprint(stdout, dl_dci);
      isrran_ra_nbiot_dl_grant_fprint(stdout, grant);
    }
...
// #2  1   0   1
// #3  2   1   0
bool isrran_nbiot_dci_location_isvalid(const isrran_dci_location_t* c)
{
 ...
 */
int dci_formatN0_pack(isrran_ra_nbiot_ul_dci_t* data, isrran_dci_msg_t* msg, uint32_t nof_prb)
{
  // pack bits
 ...
 */
int dci_formatN1_pack(const isrran_ra_nbiot_dl_dci_t* data, isrran_dci_msg_t* msg, bool crc_is_crnti)
{
  int      last_bits_val = 0;
  ...
  *y++ = data->alloc.is_ra;
  if (data->alloc.is_ra) {
    ...
    isrran_bit_unpack(data->alloc.i_delay, &y, 3);

    // Resource assignment – 3 bits
    isrran_bit_unpack(data->alloc.i_sf, &y, 3);

    // Modulation and coding scheme – 4 bits
    isrran_bit_unpack(data->mcs_idx, &y, 4);

    // Repetition number – 4 bits
    isrran_bit_unpack(data->alloc.i_rep, &y, 4);
    ...
    if (crc_is_crnti) {
      ...
      isrran_bit_unpack(data->alloc.harq_ack, &y, 4);
    }
  }
...
int dci_formatN0_unpack(const isrran_dci_msg_t* msg, isrran_ra_nbiot_ul_dci_t* data)
{
  uint8_t* y = (uint8_t*)msg->payload;
  ...
  data->format = isrran_bit_pack(&y, 1);

  // Subcarrier indication – 6 bits
  data->i_sc = isrran_bit_pack(&y, 6);

  // Resource assignment – 3 bits
  data->i_ru = isrran_bit_pack(&y, 3);

  // Scheduling delay – 2 bits
  data->i_delay = isrran_bit_pack(&y, 2);

  // Modulation and coding scheme – 4
  data->i_mcs = isrran_bit_pack(&y, 4);

  // Redundancy version – 1 bit
  data->i_rv = isrran_bit_pack(&y, 1);

  // Repetition number – 3 bits
  data->i_rep = isrran_bit_pack(&y, 3);

  // New data indicator – 1 bit
  data->ndi = isrran_bit_pack(&y, 1);

  // DCI subframe repetition number – 2 bits
  data->dci_sf_rep_num = isrran_bit_pack(&y, 2);

  // According to 16.5.1.1, SC spacing is determined by RAR grant
  // TODO: Add support for 3.75kHz
  data->sc_spacing = ISRRAN_NPUSCH_SC_SPACING_15000;

  return ISRRAN_SUCCESS;
}

int dci_formatN1_unpack(const isrran_dci_msg_t* msg, isrran_ra_nbiot_dl_dci_t* data, bool crc_is_crnti)
{
  uint8_t* y = (uint8_t*)msg->payload;
  ...
  data->alloc.is_ra = isrran_bit_pack(&y, 1);

  if (data->alloc.is_ra) {
    // This is a RA precedure, set field according to Section 6.4.3.2 in TS36.212
    data->alloc.nprach_start = isrran_bit_pack(&y, 2);
    data->alloc.nprach_sc    = isrran_bit_pack(&y, 6);
    ...
    // Scheduling delay – 3 bits
    data->alloc.i_delay = isrran_bit_pack(&y, 3);

    // Resource assignment – 3 bits
    data->alloc.i_sf = isrran_bit_pack(&y, 3);

    // Modulation and coding scheme – 4 bits
    data->mcs_idx = isrran_bit_pack(&y, 4);

    // Repetition number – 4 bits
    data->alloc.i_rep = isrran_bit_pack(&y, 4);
    ...
    // HARQ-ACK resource – 4 bits
    data->alloc.harq_ack = isrran_bit_pack(&y, 4);

    // DCI subframe repetition number – 2 bits
    data->alloc.dci_sf_rep_num = isrran_bit_pack(&y, 2);
  }
  return ISRRAN_SUCCESS;
}

int dci_formatN2_unpack(const isrran_dci_msg_t* msg, isrran_ra_nbiot_dl_dci_t* data)
{
  uint8_t* y = (uint8_t*)msg->payload;
  ...
  if (data->format == 0) {
    ...
    data->dir_indication_info = isrran_bit_pack(&y, 8);
  } else {
    // Paging
    ...
    data->alloc.dci_sf_rep_num = isrran_bit_pack(&y, 3);
  }
  return ISRRAN_SUCCESS;
}

uint32_t isrran_dci_nbiot_format_sizeof(const isrran_dci_format_t format)
{
  switch (format) {
...
int isrran_dci_msg_unpack_npdsch(const isrran_dci_msg_t* msg, isrran_ra_nbiot_dl_dci_t* data, const bool crc_is_crnti)
{
  switch (msg->format) {
    case ISRRAN_DCI_FORMATN1:
...
int isrran_dci_msg_unpack_npusch(const isrran_dci_msg_t* msg, isrran_ra_nbiot_ul_dci_t* data)
{
  switch (msg->format) {
    case ISRRAN_DCI_FORMATN0:
...
int isrran_dci_msg_pack_npdsch(const isrran_ra_nbiot_dl_dci_t* data,
