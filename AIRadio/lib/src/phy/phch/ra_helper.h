...
#ifndef ISRRAN_RA_HELPER_H
#define ISRRAN_RA_HELPER_H
...
static int ra_helper_freq_type0(const isrran_carrier_nr_t*    carrier,
...
static int ra_helper_freq_type1(uint32_t N_bwp_size, uint32_t start_rb, uint32_t riv, isrran_sch_grant_nr_t* grant)
{
  uint32_t start = 0;
  uint32_t len   = 0;
  isrran_sliv_to_s_and_l(N_bwp_size, riv, &start, &len);
  ...
  for (uint32_t i = start + len; i < ISRRAN_MAX_PRB_NR; i++) {
    grant->prb_idx[i] = false;
  }
  grant->nof_prb = len;
