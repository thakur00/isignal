...
#include "isrran/phy/fec/ldpc/ldpc_common.h"
...
static int sch_nr_cbsegm(isrran_basegraph_t bg, uint32_t tbs, isrran_cbsegm_t* cbsegm)
{
  if (bg == BG1) {
...
static int sch_nr_Nref(uint32_t N_rb, isrran_mcs_table_t mcs_table, uint32_t max_mimo_layers)
{
  uint32_t           N_re_lbrm = ISRRAN_MAX_NRE_NR * sch_nr_n_prb_lbrm(N_rb);
  double             TCR_lbrm  = 948.0 / 1024.0;
  uint32_t           Qm_lbrm   = (mcs_table == isrran_mcs_table_256qam) ? 8 : 6;
  uint32_t           TBS_LRBM  = isrran_ra_nr_tbs(N_re_lbrm, 1.0, TCR_lbrm, Qm_lbrm, ISRRAN_MIN(4, max_mimo_layers));
  double             R         = 2.0 / 3.0;
  isrran_basegraph_t bg        = isrran_sch_nr_select_basegraph(TBS_LRBM, R);
...
int isrran_sch_nr_fill_tb_info(const isrran_carrier_nr_t* carrier,
                               ...
                               isrran_sch_nr_tb_info_t*   cfg)
{
  ...
  isrran_basegraph_t bg = isrran_sch_nr_select_basegraph(tb->tbs, tb->R);

  // Compute code block segmentation
  isrran_cbsegm_t cbsegm = {};
  if (sch_nr_cbsegm(bg, tb->tbs, &cbsegm) < ISRRAN_SUCCESS) {
  ...
  cfg->bg   = bg;
  cfg->Qm   = isrran_mod_bits_x_symbol(tb->mod);
  cfg->A    = tb->tbs;
  ...
  if (sch_cfg->limited_buffer_rm) {
    ...
    if (Nref < ISRRAN_SUCCESS) {
      ERROR("Error computing N_ref");
    }
    cfg->Nref = (uint32_t)Nref;
  } else {
    cfg->Nref = ISRRAN_LDPC_MAX_LEN_ENCODED_CB;
  }
  ...
  for (uint32_t r = cbsegm.C; r < ISRRAN_SCH_NR_MAX_NOF_CB_LDPC; r++) {
    cfg->mask[r] = false;
  }
  cfg->C  = cbsegm.C;
  cfg->Cp = cbsegm.C;
...
static inline uint32_t sch_nr_get_E(const isrran_sch_nr_tb_info_t* cfg, uint32_t j)
{
  ...
  return cfg->Nl * cfg->Qm * ISRRAN_CEIL(cfg->G, cfg->Nl * cfg->Qm * cfg->Cp);
}

static inline int sch_nr_init_common(isrran_sch_nr_t* q)
{
  if (q == NULL) {
    return ISRRAN_ERROR_INVALID_INPUTS;
  }

  if (isrran_crc_init(&q->crc_tb_24, ISRRAN_LTE_CRC24A, 24) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_crc_init(&q->crc_cb, ISRRAN_LTE_CRC24B, 24) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_crc_init(&q->crc_tb_16, ISRRAN_LTE_CRC16, 16) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (!q->temp_cb) {
    q->temp_cb = isrran_vec_u8_malloc(ISRRAN_LDPC_MAX_LEN_CB * 8);
    if (!q->temp_cb) {
      return ISRRAN_ERROR;
    }
  }
...
int isrran_sch_nr_init_tx(isrran_sch_nr_t* q, const isrran_sch_nr_args_t* args)
{
  ...
  isrran_ldpc_encoder_type_t encoder_type = ISRRAN_LDPC_ENCODER_C;

#ifdef LV_HAVE_AVX512
  if (!args->disable_simd) {
    encoder_type = ISRRAN_LDPC_ENCODER_AVX512;
  }
#else // LV_HAVE_AVX512
#ifdef LV_HAVE_AVX2
  if (!args->disable_simd) {
    encoder_type = ISRRAN_LDPC_ENCODER_AVX2;
  }
#endif // LV_HAVE_AVX2
  ...
  for (uint16_t ls = 0; ls <= MAX_LIFTSIZE; ls++) {
    ...
    q->encoder_bg1[ls] = ISRRAN_MEM_ALLOC(isrran_ldpc_encoder_t, 1);
    if (!q->encoder_bg1[ls]) {
      ERROR("Error: calloc");
      return ISRRAN_ERROR;
    }
    ISRRAN_MEM_ZERO(q->encoder_bg1[ls], isrran_ldpc_encoder_t, 1);

    if (isrran_ldpc_encoder_init(q->encoder_bg1[ls], encoder_type, BG1, ls) < ISRRAN_SUCCESS) {
      ERROR("Error: initialising BG1 LDPC encoder for ls=%d", ls);
      return ISRRAN_ERROR;
    }

    q->encoder_bg2[ls] = ISRRAN_MEM_ALLOC(isrran_ldpc_encoder_t, 1);
    if (!q->encoder_bg2[ls]) {
      return ISRRAN_ERROR;
    }
    ISRRAN_MEM_ZERO(q->encoder_bg2[ls], isrran_ldpc_encoder_t, 1);

    if (isrran_ldpc_encoder_init(q->encoder_bg2[ls], encoder_type, BG2, ls) < ISRRAN_SUCCESS) {
      ERROR("Error: initialising BG2 LDPC encoder for ls=%d", ls);
      return ISRRAN_ERROR;
    }
  }

  if (isrran_ldpc_rm_tx_init(&q->tx_rm) < ISRRAN_SUCCESS) {
...
int isrran_sch_nr_init_rx(isrran_sch_nr_t* q, const isrran_sch_nr_args_t* args)
{
  int ret = sch_nr_init_common(q);
  if (ret < ISRRAN_SUCCESS) {
    return ret;
  ...
  isrran_ldpc_decoder_type_t decoder_type =
      args->decoder_use_flooded ? ISRRAN_LDPC_DECODER_C_FLOOD : ISRRAN_LDPC_DECODER_C;

#ifdef LV_HAVE_AVX512
  if (!args->disable_simd) {
    decoder_type = args->decoder_use_flooded ? ISRRAN_LDPC_DECODER_C_AVX512_FLOOD : ISRRAN_LDPC_DECODER_C_AVX512;
  }
#else // LV_HAVE_AVX512
#ifdef LV_HAVE_AVX2
  if (!args->disable_simd) {
    decoder_type = args->decoder_use_flooded ? ISRRAN_LDPC_DECODER_C_AVX2_FLOOD : ISRRAN_LDPC_DECODER_C_AVX2;
  }
#endif // LV_HAVE_AVX2
  ...
  for (uint16_t ls = 0; ls <= MAX_LIFTSIZE; ls++) {
    ...
    isrran_ldpc_decoder_args_t decoder_args = {};
    decoder_args.type                       = decoder_type;
    decoder_args.ls                         = ls;
    decoder_args.scaling_fctr               = scaling_factor;
    decoder_args.max_nof_iter               = args->max_nof_iter;

    q->decoder_bg1[ls] = ISRRAN_MEM_ALLOC(isrran_ldpc_decoder_t, 1);
    if (!q->decoder_bg1[ls]) {
      ERROR("Error: calloc");
      return ISRRAN_ERROR;
    }
    ISRRAN_MEM_ZERO(q->decoder_bg1[ls], isrran_ldpc_decoder_t, 1);

    decoder_args.bg = BG1;
    if (isrran_ldpc_decoder_init(q->decoder_bg1[ls], &decoder_args) < ISRRAN_SUCCESS) {
      ERROR("Error: initialising BG1 LDPC decoder for ls=%d", ls);
      return ISRRAN_ERROR;
    }

    q->decoder_bg2[ls] = ISRRAN_MEM_ALLOC(isrran_ldpc_decoder_t, 1);
    if (!q->decoder_bg2[ls]) {
      ERROR("Error: calloc");
      return ISRRAN_ERROR;
    }
    ISRRAN_MEM_ZERO(q->decoder_bg2[ls], isrran_ldpc_decoder_t, 1);

    decoder_args.bg = BG2;
    if (isrran_ldpc_decoder_init(q->decoder_bg2[ls], &decoder_args) < ISRRAN_SUCCESS) {
      ERROR("Error: initialising BG2 LDPC decoder for ls=%d", ls);
      return ISRRAN_ERROR;
    }
  }

  if (isrran_ldpc_rm_rx_init_c(&q->rx_rm) < ISRRAN_SUCCESS) {
...
int isrran_sch_nr_set_carrier(isrran_sch_nr_t* q, const isrran_carrier_nr_t* carrier)
{
  if (!q) {
    return ISRRAN_ERROR_INVALID_INPUTS;
  }
...
void isrran_sch_nr_free(isrran_sch_nr_t* q)
{
  // Protect pointer
  if (!q) {
    return;
  ...
  for (uint16_t ls = 0; ls <= MAX_LIFTSIZE; ls++) {
    if (q->encoder_bg1[ls]) {
      isrran_ldpc_encoder_free(q->encoder_bg1[ls]);
      free(q->encoder_bg1[ls]);
    }
    if (q->encoder_bg2[ls]) {
      isrran_ldpc_encoder_free(q->encoder_bg2[ls]);
      free(q->encoder_bg2[ls]);
    }
    if (q->decoder_bg1[ls]) {
      isrran_ldpc_decoder_free(q->decoder_bg1[ls]);
      free(q->decoder_bg1[ls]);
    }
    if (q->decoder_bg2[ls]) {
      isrran_ldpc_decoder_free(q->decoder_bg2[ls]);
      free(q->decoder_bg2[ls]);
    }
  }

  isrran_ldpc_rm_tx_free(&q->tx_rm);
  isrran_ldpc_rm_rx_free_c(&q->rx_rm);
}

static inline int sch_nr_encode(isrran_sch_nr_t*        q,
                                const isrran_sch_cfg_t* sch_cfg,
                                const isrran_sch_tb_t*  tb,
                                const uint8_t*          data,
                                uint8_t*                e_bits)
{
  ...
  isrran_sch_nr_tb_info_t cfg = {};
  if (isrran_sch_nr_fill_tb_info(&q->carrier, sch_cfg, tb, &cfg) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Select encoder and CRC
  isrran_ldpc_encoder_t* encoder = (cfg.bg == BG1) ? q->encoder_bg1[cfg.Z] : q->encoder_bg2[cfg.Z];
  isrran_crc_t*          crc_tb  = (cfg.L_tb == 24) ? &q->crc_tb_24 : &q->crc_tb_16;
  ...
  // Calculate TB CRC
  uint32_t checksum_tb = isrran_crc_checksum_byte(crc_tb, data, tb->tbs);
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
    DEBUG("tb=");
    isrran_vec_fprint_byte(stdout, data, tb->tbs / 8);
  }
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    if (data != NULL) {
      ...
      if (r == cfg.C - 1) {
        ...
        isrran_bit_unpack_vector(input_ptr, q->temp_cb, (int)cb_len);

        // Append TB CRC
        uint8_t* ptr = &q->temp_cb[cb_len];
        isrran_bit_unpack(checksum_tb, &ptr, cfg.L_tb);
      ...
      if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
        DEBUG("cb%d=", r);
        isrran_vec_fprint_byte(stdout, input_ptr, cb_len / 8);
      }
      ...
      if (cfg.L_cb) {
        isrran_crc_attach(&q->crc_cb, q->temp_cb, (int)(cfg.Kp - cfg.L_cb));
        SCH_INFO_TX("CB %d: CRC=%06x", r, (uint32_t)isrran_crc_checksum_get(&q->crc_cb));
      }
      ...
      // Encode code block
      isrran_ldpc_encoder_encode(encoder, q->temp_cb, rm_buffer, cfg.Kr);

      if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
        DEBUG("encoded=");
        isrran_vec_fprint_b(stdout, rm_buffer, encoder->liftN - 2 * encoder->ls);
      }
    }
    ...
    SCH_INFO_TX("RM CB %d: E=%d; F=%d; BG=%d; Z=%d; RV=%d; Qm=%d; Nref=%d;",
                ...
                cfg.Qm,
                cfg.Nref);
    isrran_ldpc_rm_tx(&q->tx_rm, rm_buffer, output_ptr, E, cfg.bg, cfg.Z, tb->rv, tb->mod, cfg.Nref);
    output_ptr += E;
  }
...
static int sch_nr_decode(isrran_sch_nr_t*        q,
                         const isrran_sch_cfg_t* sch_cfg,
                         const isrran_sch_tb_t*  tb,
                         int8_t*                 e_bits,
                         isrran_sch_tb_res_nr_t* res)
{
  ...
  // Select encoder and CRC
  isrran_ldpc_decoder_t* decoder = (cfg.bg == BG1) ? q->decoder_bg1[cfg.Z] : q->decoder_bg2[cfg.Z];
  isrran_crc_t*          crc_tb  = (cfg.L_tb == 24) ? &q->crc_tb_24 : &q->crc_tb_16;
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    SCH_INFO_RX("RM CB %d: E=%d; F=%d; BG=%d; Z=%d; RV=%d; Qm=%d; Nref=%d;",
                ...
                cfg.Nref);
    int n_llr =
        isrran_ldpc_rm_rx_c(&q->rx_rm, input_ptr, rm_buffer, E, cfg.F, cfg.bg, cfg.Z, tb->rv, tb->mod, cfg.Nref);
    if (n_llr < ISRRAN_SUCCESS) {
      ERROR("Error in LDPC rate mateching");
      return ISRRAN_ERROR;
    }

    // Select CB or TB early stop CRC
    isrran_crc_t* crc = (cfg.L_tb == 16) ? &q->crc_tb_16 : &q->crc_tb_24;
    if (cfg.L_cb) {
      crc = &q->crc_cb;
    }

    // Decode. if CRC=KO, then ret=0
    int ret = isrran_ldpc_decoder_decode_crc_c(decoder, rm_buffer, q->temp_cb, n_llr, crc);
    if (ret < ISRRAN_SUCCESS) {
    ...
    if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
      ...
      isrran_vec_fprint_hex(stdout, q->temp_cb, cb_len);
    }

    // Pack and count CRC OK only if CRC is match
    if (tb->softbuffer.rx->cb_crc[r]) {
      isrran_bit_pack_vector(q->temp_cb, tb->softbuffer.rx->data[r], cb_len);
      cb_ok++;
    }
  ...
  for (uint32_t r = 0; r < cfg.C; r++) {
    ...
    // Append CB
    isrran_vec_u8_copy(output_ptr, tb->softbuffer.rx->data[r], cb_len / 8);
    output_ptr += cb_len / 8;
    ...
    if (cfg.C > 1 && r == cfg.C - 1) {
      ...
      isrran_bit_unpack_vector(&tb->softbuffer.rx->data[r][cb_len / 8], tb_crc_unpacked, cfg.L_tb);
      checksum2 = isrran_bit_pack(&tb_crc_unpacked_ptr, cfg.L_tb);
    }
  }
  ...
  if (cfg.C == 1) {
    ...
    // More than one
    uint32_t checksum1 = isrran_crc_checksum_byte(crc_tb, res->payload, tb->tbs);
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
    DEBUG("Decode: ");
    isrran_vec_fprint_byte(stdout, res->payload, tb->tbs / 8);
  }

  return ISRRAN_SUCCESS;
}

int isrran_dlsch_nr_encode(isrran_sch_nr_t*        q,
                           const isrran_sch_cfg_t* pdsch_cfg,
                           const isrran_sch_tb_t*  tb,
                           const uint8_t*          data,
                           uint8_t*                e_bits)
{
...
int isrran_dlsch_nr_decode(isrran_sch_nr_t*        q,
                           const isrran_sch_cfg_t* sch_cfg,
                           const isrran_sch_tb_t*  tb,
                           int8_t*                 e_bits,
                           isrran_sch_tb_res_nr_t* res)
...
int isrran_ulsch_nr_encode(isrran_sch_nr_t*        q,
                           const isrran_sch_cfg_t* pdsch_cfg,
                           const isrran_sch_tb_t*  tb,
                           const uint8_t*          data,
                           uint8_t*                e_bits)
...
int isrran_ulsch_nr_decode(isrran_sch_nr_t*        q,
                           const isrran_sch_cfg_t* sch_cfg,
                           const isrran_sch_tb_t*  tb,
                           int8_t*                 e_bits,
                           isrran_sch_tb_res_nr_t* res)
...
int isrran_sch_nr_tb_info(const isrran_sch_tb_t* tb, const isrran_sch_tb_res_nr_t* res, char* str, uint32_t str_len)
{
  int len = 0;
  ...
  if (tb->enabled) {
    ...
    if (res != NULL) {
      len = isrran_print_check(str, str_len, len, "CRC=%s iter=%.1f ", res->crc ? "OK" : "KO", res->avg_iter);
    }
  }
