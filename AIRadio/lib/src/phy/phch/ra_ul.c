...
static int f_hop_sum(isrran_ra_ul_pusch_hopping_t* q, uint32_t i)
{
  uint32_t sum = 0;
...
static int f_hop(isrran_ra_ul_pusch_hopping_t* q, isrran_pusch_hopping_cfg_t* hopping, int i)
{
  if (i == -1) {
    return 0;
  } else {
...
static int f_m(isrran_ra_ul_pusch_hopping_t* q, isrran_pusch_hopping_cfg_t* hopping, uint32_t i, uint32_t current_tx_nb)
{
  if (hopping->n_sb == 1) {
    if (hopping->hop_mode == ISRRAN_PUSCH_HOP_MODE_INTER_SF) {
...
static void compute_freq_hopping(isrran_ra_ul_pusch_hopping_t* q,
                                 ...
                                 isrran_pusch_grant_t*         grant)
{
  ...
  for (uint32_t slot = 0; slot < 2; slot++) {
    ...
    if (grant->freq_hopping == 1) {
      if (hopping_cfg->hop_mode == ISRRAN_PUSCH_HOP_MODE_INTER_SF) {
...
static int ra_ul_grant_to_grant_prb_allocation(isrran_dci_ul_t*      dci,
                                               isrran_pusch_grant_t* grant,
                                               uint32_t              n_rb_ho,
                                               uint32_t              nof_prb)
{
  ...
  isrran_ra_type2_from_riv(dci->type2_alloc.riv, &grant->L_prb, &n_prb_1, nof_prb, nof_prb);
  if (n_rb_ho % 2) {
    n_rb_ho++;
  }

  if (dci->freq_hop_fl == ISRRAN_RA_PUSCH_HOP_DISABLED || dci->freq_hop_fl == ISRRAN_RA_PUSCH_HOP_TYPE2) {
    ...
    if (dci->freq_hop_fl == ISRRAN_RA_PUSCH_HOP_DISABLED) {
    ...
    switch (dci->freq_hop_fl) {
      case ISRRAN_RA_PUSCH_HOP_QUART:
        grant->n_prb[1] = (n_rb_pusch / 4 + n_prb_1_tilde) % n_rb_pusch;
        break;
      case ISRRAN_RA_PUSCH_HOP_QUART_NEG:
...
static void ul_fill_ra_mcs(isrran_ra_tb_t* tb, isrran_ra_tb_t* last_tb, uint32_t L_prb, bool cqi_request)
{
  // 8.6.2 First paragraph
  if (tb->mcs_idx <= 28) {
    /* Table 8.6.1-1 on 36.213 */
    if (tb->mcs_idx < 11) {
      tb->mod = ISRRAN_MOD_QPSK;
      tb->tbs = isrran_ra_tbs_from_idx(tb->mcs_idx, L_prb);
    } else if (tb->mcs_idx < 21) {
      tb->mod = ISRRAN_MOD_16QAM;
      tb->tbs = isrran_ra_tbs_from_idx(tb->mcs_idx - 1, L_prb);
    } else if (tb->mcs_idx < 29) {
      tb->mod = ISRRAN_MOD_64QAM;
      tb->tbs = isrran_ra_tbs_from_idx(tb->mcs_idx - 2, L_prb);
    } else {
...
void isrran_ra_ul_compute_nof_re(isrran_pusch_grant_t* grant, isrran_cp_t cp, uint32_t N_isr)
{
  grant->nof_symb    = 2 * (ISRRAN_CP_NSYMB(cp) - 1) - N_isr;
  grant->nof_re      = grant->nof_symb * grant->L_prb * ISRRAN_NRE;
  grant->tb.nof_bits = grant->nof_re * isrran_mod_bits_x_symbol(grant->tb.mod);
}
 ...
 * memory
 */
int isrran_ra_ul_pusch_hopping_init(isrran_ra_ul_pusch_hopping_t* q, isrran_cell_t cell)
{
  ...
  if (q) {
    if (cell.id != q->cell.id || !q->initialized) {
      ...
      if (isrran_sequence_LTE_pr(&q->seq_type2_fo, 210, q->cell.id)) {
...
void isrran_ra_ul_pusch_hopping_free(isrran_ra_ul_pusch_hopping_t* q)
{
  isrran_sequence_free(&q->seq_type2_fo);
}

void isrran_ra_ul_pusch_hopping(isrran_ra_ul_pusch_hopping_t* q,
                                isrran_ul_sf_cfg_t*           sf,
                                isrran_pusch_hopping_cfg_t*   hopping_cfg,
                                isrran_pusch_grant_t*         grant)
{
...
int isrran_ra_ul_dci_to_grant(isrran_cell_t*              cell,
                              isrran_ul_sf_cfg_t*         sf,
                              isrran_pusch_hopping_cfg_t* hopping_cfg,
                              isrran_dci_ul_t*            dci,
                              isrran_pusch_grant_t*       grant)
{
  ...
  if (!ra_ul_grant_to_grant_prb_allocation(dci, grant, hopping_cfg->n_rb_ho, cell->nof_prb)) {
    ...
    isrran_ra_ul_compute_nof_re(grant, cell->cp, 0);

    // TODO: Need to compute hopping here before determining if there is collision with ISR, but only MAC knows if it's
    // a
...
uint32_t isrran_ra_ul_info(const isrran_pusch_grant_t* grant, char* info_str, uint32_t len)
{
  return isrran_print_check(info_str,
                            len,
                            0,
                            ...
                            grant->nof_re,
                            grant->tb.tbs / 8,
                            isrran_mod_bits_x_symbol(grant->tb.mod),
                            grant->tb.rv);
}
