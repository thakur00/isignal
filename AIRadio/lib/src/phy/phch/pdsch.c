...
const static isrran_mod_t modulations[5] = {ISRRAN_MOD_BPSK,
                                            ISRRAN_MOD_QPSK,
                                            ISRRAN_MOD_16QAM,
                                            ISRRAN_MOD_64QAM,
                                            ISRRAN_MOD_256QAM};
...
typedef struct {
  ...
  bool started;
  bool quit;
} isrran_pdsch_coworker_t;

static void* isrran_pdsch_decode_thread(void* arg);

static inline bool pdsch_cp_skip_symbol(const isrran_cell_t*        cell,
                                        const isrran_pdsch_grant_t* grant,
                                        uint32_t                    sf_idx,
                                        uint32_t                    s,
                                        uint32_t                    l,
                                        uint32_t                    n)
...
static inline uint32_t pdsch_cp_crs_offset(const isrran_cell_t* cell, uint32_t l, bool has_crs)
{
  // No CRS, return 0
  if (!has_crs) {
    return 0;
...
static int isrran_pdsch_cp(const isrran_pdsch_t*       q,
                           cf_t*                       input,
                           cf_t*                       output,
                           const isrran_pdsch_grant_t* grant,
                           ...
                           bool                        put)
{
  ...
  // Iterate over slots
  for (uint32_t s = 0; s < ISRRAN_NOF_SLOTS_PER_SF; s++) {
    // Skip PDCCH symbols
    uint32_t lstart = (s == 0) ? lstart_grant : 0;

    // Iterate over symbols
    for (uint32_t l = lstart; l < grant->nof_symb_slot[s]; l++) {
      bool     has_crs    = ISRRAN_SYMBOL_HAS_REF(l, q->cell.cp, q->cell.nof_ports);
 ...
 */
int isrran_pdsch_put(isrran_pdsch_t*       q,
                     cf_t*                 symbols,
                     cf_t*                 sf_symbols,
                     ...
                     uint32_t              subframe)
{
  return isrran_pdsch_cp(q, symbols, sf_symbols, grant, lstart, subframe, true);
}
 ...
 * 36.211 10.3 section 6.3.5
 */
int isrran_pdsch_get(isrran_pdsch_t*       q,
                     cf_t*                 sf_symbols,
                     cf_t*                 symbols,
                     isrran_pdsch_grant_t* grant,
                     uint32_t              lstart,
                     uint32_t              subframe)
{
  return isrran_pdsch_cp(q, sf_symbols, symbols, grant, lstart, subframe, false);
}

/** Initializes the PDSCH transmitter and receiver */
static int pdsch_init(isrran_pdsch_t* q, uint32_t max_prb, bool is_ue, uint32_t nof_antennas)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;

  if (q != NULL) {
    bzero(q, sizeof(isrran_pdsch_t));
    ret = ISRRAN_ERROR;
    ...
    for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      ...
      q->e[i] = isrran_vec_i16_malloc(q->max_re * isrran_mod_bits_x_symbol(ISRRAN_MOD_256QAM));
      if (!q->e[i]) {
        goto clean;
      }

      q->d[i] = isrran_vec_cf_malloc(q->max_re);
      if (!q->d[i]) {
        goto clean;
      }
      ...
      if (is_ue) {
        q->evm_buffer[i] = isrran_evm_buffer_alloc(isrran_ra_tbs_from_idx(ISRRAN_RA_NOF_TBS_IDX - 1, 6));
        if (!q->evm_buffer[i]) {
    ...
    for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      if (!q->csi[i]) {
        q->csi[i] = isrran_vec_f_malloc(q->max_re * 2);
        if (!q->csi[i]) {
          return ISRRAN_ERROR;
        }
      }
...
int isrran_pdsch_init_ue(isrran_pdsch_t* q, uint32_t max_prb, uint32_t nof_antennas)
{
  return pdsch_init(q, max_prb, true, nof_antennas);
}

int isrran_pdsch_init_enb(isrran_pdsch_t* q, uint32_t max_prb)
{
  return pdsch_init(q, max_prb, false, 0);
}

static void isrran_pdsch_disable_coworker(isrran_pdsch_t* q)
{
  isrran_pdsch_coworker_t* h = (isrran_pdsch_coworker_t*)q->coworker_ptr;
  if (h) {
    /* Stop threads */
    h->quit = true;
    sem_post(&h->start);
...
int isrran_pdsch_enable_coworker(isrran_pdsch_t* q)
{
  int ret = ISRRAN_SUCCESS;

  if (!q->coworker_ptr) {
    isrran_pdsch_coworker_t* h = calloc(sizeof(isrran_pdsch_coworker_t), 1);
    ...
    pthread_create(&h->pthread, NULL, isrran_pdsch_decode_thread, (void*)h);
  }

clean:
  if (ret) {
    isrran_pdsch_disable_coworker(q);
  }
  return ret;
}

void isrran_pdsch_free(isrran_pdsch_t* q)
{
  isrran_pdsch_disable_coworker(q);
  ...
  for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    ...
    if (q->evm_buffer[i]) {
      isrran_evm_free(q->evm_buffer[i]);
    }
  }
...
int isrran_pdsch_set_cell(isrran_pdsch_t* q, isrran_cell_t cell)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;
  ...
  if (q != NULL && isrran_cell_isvalid(&cell)) {
    ...
    if (q->is_ue) {
      for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
        isrran_evm_buffer_resize(q->evm_buffer[i], isrran_ra_tbs_from_idx(ISRRAN_RA_NOF_TBS_IDX - 1, cell.nof_prb));
      }
    }
...
static float apply_power_allocation(isrran_pdsch_t* q, isrran_pdsch_cfg_t* cfg, cf_t* sf_symbols_m[ISRRAN_MAX_PORTS])
{
  ...
  float rho_a = isrran_convert_dB_to_amplitude(cfg->p_a) * ((q->cell.nof_ports == 1) ? 1.0f : M_SQRT2);
...
static void csi_correction(isrran_pdsch_t* q, isrran_pdsch_cfg_t* cfg, uint32_t codeword_idx, uint32_t tb_idx, void* e)
{
  ...
  const uint32_t csi_max_idx = isrran_vec_max_fi(q->csi[codeword_idx], cfg->grant.tb[tb_idx].nof_bits / qm);
  float          csi_max     = 1.0f;
...
static void pdsch_decode_debug(isrran_pdsch_t*     q,
                               isrran_pdsch_cfg_t* cfg,
                               ...
                               cf_t*               ce[ISRRAN_MAX_PORTS][ISRRAN_MAX_PORTS])
{
  if (ISRRAN_VERBOSE_ISDEBUG()) {
    ...
    for (int j = 0; j < q->nof_rx_antennas; j++) {
      ...
      isrran_vec_save_file(filename, sf_symbols[j], ISRRAN_NOF_RE(q->cell) * sizeof(cf_t));
      ...
      for (int i = 0; i < q->cell.nof_ports; i++) {
        ...
        isrran_vec_save_file(filename, ce[i][j], ISRRAN_NOF_RE(q->cell) * sizeof(cf_t));
      }
    }
    for (int i = 0; i < cfg->grant.nof_layers; i++) {
      ...
      isrran_vec_save_file(filename, q->d[i], cfg->grant.nof_re * sizeof(cf_t));
      ...
      isrran_vec_save_file(filename, q->e[i], cfg->grant.tb[0].nof_bits * sizeof(int16_t));
    }
  }
}

static int isrran_pdsch_codeword_decode(isrran_pdsch_t*     q,
                                        isrran_dl_sf_cfg_t* sf,
                                        isrran_pdsch_cfg_t* cfg,
                                        isrran_sch_t*       dl_sch,
                                        isrran_pdsch_res_t* data,
                                        ...
                                        bool*               ack)
{
  isrran_ra_tb_t*         mcs          = &cfg->grant.tb[tb_idx];
  uint32_t                rv           = mcs->rv;
  uint32_t                codeword_idx = mcs->cw_idx;
  uint32_t                nof_layers   = cfg->grant.nof_layers;
  isrran_softbuffer_rx_t* softbuffer   = cfg->softbuffers.rx[tb_idx];
  ...
  if (softbuffer && data && ack && cfg->grant.tb[tb_idx].nof_bits && cfg->grant.nof_re) {
     ...
     */
    if (q->llr_is_8bit) {
      isrran_demod_soft_demodulate_b(mcs->mod, q->d[codeword_idx], q->e[codeword_idx], cfg->grant.nof_re);
    } else {
      isrran_demod_soft_demodulate_s(mcs->mod, q->d[codeword_idx], q->e[codeword_idx], cfg->grant.nof_re);
    }
    if (cfg->meas_evm_en && q->evm_buffer[codeword_idx]) {
      if (q->llr_is_8bit) {
        data[tb_idx].evm = isrran_evm_run_b(q->evm_buffer[codeword_idx],
                                            &q->mod[mcs->mod],
                                            q->d[codeword_idx],
                                            q->e[codeword_idx],
                                            cfg->grant.tb[tb_idx].nof_bits);
      } else {
        data[tb_idx].evm = isrran_evm_run_s(q->evm_buffer[codeword_idx],
                                            &q->mod[mcs->mod],
                                            q->d[codeword_idx],
                                            q->e[codeword_idx],
                                            cfg->grant.tb[tb_idx].nof_bits);
    ...
    /* Bit scrambling */
    if (q->llr_is_8bit) {
      isrran_sequence_pdsch_apply_c(q->e[codeword_idx],
                                    q->e[codeword_idx],
                                    cfg->rnti,
                                    codeword_idx,
      ...
      isrran_sequence_pdsch_apply_s(q->e[codeword_idx],
                                    q->e[codeword_idx],
                                    cfg->rnti,
                                    codeword_idx,
                                    2 * (sf->tti % ISRRAN_NOF_SF_X_FRAME),
    ...
    /* Return  */
    ret = isrran_dlsch_decode2(dl_sch, cfg, q->e[codeword_idx], data[tb_idx].payload, tb_idx, nof_layers);

    if (ret == ISRRAN_SUCCESS) {
      *ack = true;
    } else if (ret == ISRRAN_ERROR) {
      *ack = false;
      ret  = ISRRAN_SUCCESS;
    } else if (ret == ISRRAN_ERROR_INVALID_INPUTS) {
      *ack = false;
      ret  = ISRRAN_ERROR;
    }
  } else {
...
static void* isrran_pdsch_decode_thread(void* arg)
{
  isrran_pdsch_coworker_t* q = (isrran_pdsch_coworker_t*)arg;

  INFO("[PDSCH Coworker] waiting for data");

  sem_wait(&q->start);
  while (!q->quit) {
    q->ret_status = isrran_pdsch_codeword_decode(q->pdsch_ptr, q->sf, q->cfg, &q->dl_sch, q->data, q->tb_idx, q->ack);
 ...
 */
int isrran_pdsch_decode(isrran_pdsch_t*        q,
                        isrran_dl_sf_cfg_t*    sf,
                        isrran_pdsch_cfg_t*    cfg,
                        ...
                        isrran_pdsch_res_t     data[ISRRAN_MAX_CODEWORDS])
{
  ...
  if (q != NULL && sf_symbols != NULL && data != NULL && cfg != NULL) {
    ...
    if (cfg->max_nof_iterations) {
      isrran_sch_set_max_noi(&q->dl_sch, cfg->max_nof_iterations);
    }

    float noise_estimate = cfg->decoder_type == ISRRAN_MIMO_DECODER_ZF ? 0 : channel->noise_estimate;
    ...
    INFO("Decoding PDSCH SF: %d, RNTI: 0x%x, NofSymbols: %d, C_prb=%d, mod=%s, nof_layers=%d, nof_tb=%d",
         ...
         cfg->grant.nof_re,
         cfg->grant.nof_prb,
         isrran_mod_string(cfg->grant.tb[0].mod),
    ...
    for (int j = 0; j < q->nof_rx_antennas; j++) {
      int n = isrran_pdsch_get(q, sf_symbols[j], q->symbols[j], &cfg->grant, lstart, sf->tti % 10);
      ...
      for (i = 0; i < q->cell.nof_ports; i++) {
        n = isrran_pdsch_get(q, channel->ce[i][j], q->ce[i][j], &cfg->grant, lstart, sf->tti % 10);
        if (n != cfg->grant.nof_re) {
          ERROR("Error expecting %d symbols but got %d", cfg->grant.nof_re, n);
          return ISRRAN_ERROR;
        }
    ...
    if (cfg->grant.nof_layers == 0 || cfg->grant.nof_layers > ISRRAN_MAX_LAYERS) {
    ...
    if (isrran_predecoding_type(q->symbols,
                                q->ce,
                                x,
                                q->csi,
                                q->nof_rx_antennas,
    ...
    if (cfg->grant.nof_layers != nof_tb) {
      isrran_layerdemap_type(x, q->d, cfg->grant.nof_layers, nof_tb, nof_symbols[0], nof_symbols, cfg->grant.tx_scheme);
    }
      ...
      if (cfg->grant.tb[tb_idx].enabled) {
        if (!data[tb_idx].crc) {
          ...
          if (cfg->grant.nof_tb > 1 && tb_idx == 0 && q->coworker_ptr) {
            ...
            ret = isrran_pdsch_codeword_decode(q, sf, cfg, &q->dl_sch, data, tb_idx, &data[tb_idx].crc);

            data[tb_idx].avg_iterations_block = isrran_sch_last_noi(&q->dl_sch);
          }
    ...
    if (q->coworker_ptr) {
      ...
      if (h->started) {
        ...
        data[h->tb_idx].avg_iterations_block = isrran_sch_last_noi(&q->dl_sch);
        h->started                           = false;
      }
    }
    ...
    if (cfg->meas_evm_en) {
      for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
        if (cfg->grant.tb[i].enabled && !isnan(data[i].evm)) {
          q->avg_evm = ISRRAN_VEC_EMA(data[i].evm, q->avg_evm, 0.1);
        }
      }
    }
...
static int isrran_pdsch_codeword_encode(isrran_pdsch_t*         q,
                                        isrran_dl_sf_cfg_t*     sf,
                                        isrran_pdsch_cfg_t*     cfg,
                                        ...
                                        uint32_t                nof_layers)
{
  ...
  if (cfg->grant.tb[tb_idx].enabled) {
    ...
    /* Channel coding */
    if (isrran_dlsch_encode2(&q->dl_sch, cfg, data, q->e[codeword_idx], tb_idx, nof_layers)) {
      ERROR("Error encoding (TB%d -> CW%d)", tb_idx, codeword_idx);
      return ISRRAN_ERROR;
    }

    /* Bit scrambling */
    isrran_sequence_pdsch_apply_pack((uint8_t*)q->e[codeword_idx],
...
int isrran_pdsch_encode(isrran_pdsch_t*     q,
                        isrran_dl_sf_cfg_t* sf,
                        isrran_pdsch_cfg_t* cfg,
                        ...
                        cf_t*               sf_symbols[ISRRAN_MAX_PORTS])
{
  ...
  if (q != NULL && cfg != NULL) {
    ...
    for (uint32_t tb_idx = 0; tb_idx < ISRRAN_MAX_TB; tb_idx++) {
      if (cfg->grant.tb[tb_idx].enabled) {
        ret |= isrran_pdsch_codeword_encode(
    ...
    if (cfg->rnti != ISRRAN_SIRNTI) {
      INFO("Encoding PDSCH SF: %d rho_a=%f, nof_ports=%d, nof_layers=%d, nof_tb=%d, pmi=%d, tx_scheme=%s",
           ...
           nof_tb,
           cfg->grant.pmi,
           isrran_mimotype2str(cfg->grant.tx_scheme));
    }
    ...
    if (q->cell.nof_ports > 1) {
      ...
      if (cfg->grant.nof_layers == nof_tb) {
        ...
        memset(&x[cfg->grant.nof_layers], 0, sizeof(cf_t*) * (ISRRAN_MAX_LAYERS - cfg->grant.nof_layers));

        nof_symbols = isrran_layermap_type(q->d,
                                           x,
                                           nof_tb,
                                           cfg->grant.nof_layers,
                                           (int[ISRRAN_MAX_CODEWORDS]){cfg->grant.nof_re, cfg->grant.nof_re},
                                           cfg->grant.tx_scheme);
      }

      /* Precode */
      uint32_t codebook_idx = nof_tb == 1 ? cfg->grant.pmi : (cfg->grant.pmi + 1);
      isrran_precoding_type(x,
                            q->symbols,
                            cfg->grant.nof_layers,
                            q->cell.nof_ports,
                            codebook_idx,
      ...
      if (scaling == 1.0f) {
        ...
        isrran_vec_sc_prod_cfc(q->d[0], scaling, q->symbols[0], cfg->grant.nof_re);
      }
    }
    ...
    uint32_t lstart = ISRRAN_NOF_CTRL_SYMBOLS(q->cell, sf->cfi);
    for (i = 0; i < q->cell.nof_ports; i++) {
      isrran_pdsch_put(q, q->symbols[i], sf_symbols[i], &cfg->grant, lstart, sf->tti % 10);
    }
...
int isrran_pdsch_select_pmi(isrran_pdsch_t*        q,
                            isrran_chest_dl_res_t* channel,
                            uint32_t               nof_layers,
                            uint32_t*              best_pmi,
                            float                  sinr[ISRRAN_MAX_CODEBOOKS])
{
  uint32_t nof_ce = ISRRAN_NOF_RE(q->cell);
  uint32_t pmi    = 0;

  if (isrran_precoding_pmi_select(channel->ce, nof_ce, channel->noise_estimate, nof_layers, &pmi, sinr) < 0) {
...
int isrran_pdsch_compute_cn(isrran_pdsch_t* q, isrran_chest_dl_res_t* channel, float* cn)
{
  return isrran_precoding_cn(channel->ce, q->cell.nof_ports, q->nof_rx_antennas, ISRRAN_NOF_RE(q->cell), cn);
}

uint32_t isrran_pdsch_grant_rx_info(isrran_pdsch_grant_t* grant,
                                    isrran_pdsch_res_t    res[ISRRAN_MAX_CODEWORDS],
                                    char*                 str,
                                    uint32_t              str_len)
{
  uint32_t len = isrran_ra_dl_info(grant, str, str_len);

  len = isrran_print_check(str, str_len, len, ", crc={");
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    if (grant->tb[i].enabled) {
      len = isrran_print_check(str, str_len, len, "%s", res[i].crc ? "OK" : "KO");
      if (i < ISRRAN_MAX_CODEWORDS - 1) {
...
uint32_t
isrran_pdsch_rx_info(isrran_pdsch_cfg_t* cfg, isrran_pdsch_res_t res[ISRRAN_MAX_CODEWORDS], char* str, uint32_t str_len)
{
  uint32_t len = isrran_print_check(str, str_len, 0, "rnti=0x%x", cfg->rnti);
  len += isrran_pdsch_grant_rx_info(&cfg->grant, res, &str[len], str_len - len);

  if (cfg->meas_evm_en) {
    len = isrran_print_check(str, str_len, len, ", evm={");
    for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      if (cfg->grant.tb[i].enabled && !isnan(res[i].evm)) {
        len = isrran_print_check(str, str_len, len, "%.2f", res[i].evm);
        if (i < ISRRAN_MAX_CODEWORDS - 1) {
          if (cfg->grant.tb[i + 1].enabled) {
            len = isrran_print_check(str, str_len, len, "/", 0);
          }
        }
      }
    }
...
uint32_t isrran_pdsch_tx_info(isrran_pdsch_cfg_t* cfg, char* str, uint32_t str_len)
{
  uint32_t len = isrran_print_check(str, str_len, 0, "rnti=0x%x", cfg->rnti);
  len += isrran_ra_dl_info(&cfg->grant, &str[len], str_len);
