...
bool isrran_ra_dl_nr_time_validate(isrran_sch_grant_nr_t* grant)
{
...
int isrran_ra_dl_nr_time_default_A(uint32_t m, isrran_dmrs_sch_typeA_pos_t dmrs_typeA_pos, isrran_sch_grant_nr_t* grant)
{
  if (grant == NULL) {
  ...
  static isrran_sch_mapping_type_t pdsch_mapping_lut[16] = {isrran_sch_mapping_type_A,
                                                            isrran_sch_mapping_type_A,
                                                            isrran_sch_mapping_type_A,
                                                            isrran_sch_mapping_type_A,
                                                            isrran_sch_mapping_type_A,
                                                            isrran_sch_mapping_type_B,
                                                            isrran_sch_mapping_type_B,
                                                            isrran_sch_mapping_type_B,
                                                            isrran_sch_mapping_type_B,
                                                            isrran_sch_mapping_type_B,
  ...
  static uint32_t S_pos2[ISRRAN_MAX_NOF_TIME_RA] = {2, 2, 2, 2, 2, 9, 4, 5, 5, 9, 12, 1, 1, 2, 4, 8};
  static uint32_t L_pos2[ISRRAN_MAX_NOF_TIME_RA] = {12, 10, 9, 7, 5, 4, 4, 7, 2, 2, 2, 13, 6, 4, 7, 4};
  static uint32_t S_pos3[ISRRAN_MAX_NOF_TIME_RA] = {3, 3, 3, 3, 3, 10, 6, 5, 5, 9, 12, 1, 1, 2, 4, 8};
  static uint32_t L_pos3[ISRRAN_MAX_NOF_TIME_RA] = {11, 9, 8, 6, 4, 4, 4, 7, 2, 2, 2, 13, 6, 4, 7, 4};
...
static void ra_dl_nr_time_hl(const isrran_sch_time_ra_t* hl_ra_cfg, isrran_sch_grant_nr_t* grant)
{
...
int isrran_ra_dl_nr_time(const isrran_sch_hl_cfg_nr_t*    cfg,
                         ...
                         isrran_sch_grant_nr_t*           grant)
{
  ...
  if (rnti_type == isrran_rnti_type_si && ss_type == isrran_search_space_type_common_0) {
    // Row 1
    // Note: Only Default A is supported, which corresponds SS/PBCH block and coreset mux pattern 1
    isrran_ra_dl_nr_time_default_A(m, cfg->typeA_pos, grant);
  } else if (rnti_type == isrran_rnti_type_si && ss_type == isrran_search_space_type_common_0A) {
    // Row 2
    ERROR("Row not implemented");
  } else if ((rnti_type == isrran_rnti_type_ra || rnti_type == isrran_rnti_type_tc) &&
             ss_type == isrran_search_space_type_common_1) {
    // Row 3
  ...
  } else if (rnti_type == isrran_rnti_type_p && ss_type == isrran_search_space_type_common_2) {
    // Row 4
    ERROR("Row not implemented");
  } else if ((rnti_type == isrran_rnti_type_c || rnti_type == isrran_rnti_type_mcs_c ||
              rnti_type == isrran_rnti_type_cs) &&
    ...
    ERROR("Unhandled case %s, ss_type=%s", isrran_rnti_type_str(rnti_type), isrran_ss_type_str(ss_type));
  }

  // Validate S and L parameters
  if (!isrran_ra_dl_nr_time_validate(grant)) {
...
int isrran_ra_dl_nr_nof_front_load_symbols(const isrran_sch_hl_cfg_nr_t* cfg,
                                           ...
                                           isrran_dmrs_sch_len_t*        dmrs_duration)
{
  ...
  if (cfg->dmrs_type == isrran_dmrs_sch_type_1 && cfg->dmrs_max_length == isrran_dmrs_sch_len_2) {
    ...
    if (dci->ports < 31) {
      *dmrs_duration = isrran_dmrs_sch_len_2;
      return ISRRAN_SUCCESS;
    }
  ...
  if (cfg->dmrs_type == isrran_dmrs_sch_type_2 && cfg->dmrs_max_length == isrran_dmrs_sch_len_2) {
    // Only one codeword supported!
    if (dci->ports < 3) {
      *dmrs_duration = isrran_dmrs_sch_len_1;
      return ISRRAN_SUCCESS;
...
int isrran_ra_dl_nr_nof_dmrs_cdm_groups_without_data(const isrran_sch_hl_cfg_nr_t* cfg,
                                                     const isrran_dci_dl_nr_t*     dci,
                                                     uint32_t                      L)
{
  ...
  switch (dci->ctx.format) {
    ...
    case isrran_dci_format_nr_1_1:
      ...
      // Table 7.3.1.2.2-1: Antenna port(s) (1000 + DMRS port), dmrs-Type=1, maxLength=1
      if (cfg->dmrs_type == isrran_dmrs_sch_type_1 && cfg->dmrs_max_length == isrran_dmrs_sch_len_1) {
        if (dci->ports < 3) {
          return 1;
        }
        if (dci->ports < 12) {
      ...
      return ISRRAN_ERROR;
    default:
      ERROR("Invalid DL DCI format %s", isrran_dci_format_nr_string(dci->ctx.format));
  }

  return ISRRAN_ERROR;
}

int isrran_ra_dl_nr_freq(const isrran_carrier_nr_t*    carrier,
                         ...
                         isrran_sch_grant_nr_t*        grant)
{
  ...
  uint32_t start_rb = 0;
  if (dci_dl->ctx.format == isrran_dci_format_nr_1_0 && ISRRAN_SEARCH_SPACE_IS_COMMON(dci_dl->ctx.ss_type)) {
  ...
  if (ISRRAN_SEARCH_SPACE_IS_COMMON(dci_dl->ctx.ss_type) && dci_dl->coreset0_bw != 0) {
  ...
  if (dci_dl->ctx.format == isrran_dci_format_nr_1_0) {
...
uint32_t isrran_ra_nr_type1_riv(uint32_t N_prb, uint32_t start_rb, uint32_t length_rb)
{
  return isrran_sliv_from_s_and_l(N_prb, start_rb, length_rb);
}
