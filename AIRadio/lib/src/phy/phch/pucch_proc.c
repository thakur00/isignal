...
static bool pucch_proc_tx_sr(const isrran_uci_cfg_t* uci_cfg, const isrran_uci_value_t* uci_value)
{
...
isrran_pucch_format_t isrran_pucch_proc_select_format(const isrran_cell_t*      cell,
                                                      const isrran_pucch_cfg_t* cfg,
                                                      const isrran_uci_cfg_t*   uci_cfg,
                                                      const isrran_uci_value_t* uci_value)
{
  isrran_pucch_format_t format    = ISRRAN_PUCCH_FORMAT_ERROR;
  uint32_t              total_ack = isrran_uci_cfg_total_ack(uci_cfg);

  // No CQI data
  if (!uci_cfg->cqi.data_enable && uci_cfg->cqi.ri_len == 0) {
    if (cfg->ack_nack_feedback_mode == ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_PUCCH3 &&
        total_ack > uci_cfg->ack[0].nof_acks) {
      format = ISRRAN_PUCCH_FORMAT_3;
    }
    // 1-bit ACK + optional SR
    else if (total_ack == 1) {
      format = ISRRAN_PUCCH_FORMAT_1A;
    }
    // 2-bit ACK + optional SR
    else if (total_ack >= 2 && total_ack <= 4) {
      format = ISRRAN_PUCCH_FORMAT_1B; // with channel selection if > 2
    }
    // If UCI value is provided, use SR signal only, otherwise SR request opportunity
    else if (pucch_proc_tx_sr(uci_cfg, uci_value)) {
      format = ISRRAN_PUCCH_FORMAT_1;
    } else {
  ...
  // CQI data
    ...
    // CQI and no ack
    if (total_ack == 0) {
      format = ISRRAN_PUCCH_FORMAT_2;
    }
    // CQI + 1-bit ACK
    else if (total_ack == 1 && ISRRAN_CP_ISNORM(cell->cp)) {
      format = ISRRAN_PUCCH_FORMAT_2A;
    }
    // CQI + 2-bit ACK
    else if (total_ack == 2) {
      format = ISRRAN_PUCCH_FORMAT_2B;
    }
    // CQI + 1-bit ACK + extended cyclic prefix
    else if (total_ack == 1 && ISRRAN_CP_ISEXT(cell->cp)) {
      format = ISRRAN_PUCCH_FORMAT_2B;
    }
  }
  if (format == ISRRAN_PUCCH_FORMAT_ERROR) {
...
static int pucch_cs_resources(const isrran_pucch_cfg_t* cfg,
                              const isrran_uci_cfg_t*   uci_cfg,
                              uint32_t                  n_pucch_i[ISRRAN_PUCCH_MAX_ALLOC])
{
  ...
  int k = 0;
  for (int i = 0; i < ISRRAN_PUCCH_CS_MAX_CARRIERS && k < ISRRAN_PUCCH_CS_MAX_ACK; i++) {
    if (uci_cfg->ack[i].grant_cc_idx == 0) {
      ...
      for (uint32_t j = 0; j < uci_cfg->ack[i].nof_acks && k < ISRRAN_PUCCH_CS_MAX_ACK; j++) {
      ...
      for (uint32_t j = 0; j < uci_cfg->ack[i].nof_acks && k < ISRRAN_PUCCH_CS_MAX_ACK; j++) {
        n_pucch_i[k++] = cfg->n1_pucch_an_cs[uci_cfg->ack[i].tpc_for_pucch % ISRRAN_PUCCH_SIZE_AN_CS][0] + j;
      }
    } else {
      ...
      for (uint32_t j = 0; j < uci_cfg->ack[i].nof_acks && k < ISRRAN_PUCCH_CS_MAX_ACK; j++) {
        n_pucch_i[k++] =
            cfg->n1_pucch_an_cs[uci_cfg->ack[i].tpc_for_pucch % ISRRAN_PUCCH_SIZE_AN_CS][j % ISRRAN_PUCCH_NOF_AN_CS];
      }
    }
  }
...
#define PUCCH_CS_SET_ACK(J, B0, B1, ...)                                                                               \
  do {                                                                                                                 \
    if (j == J && b[0] == B0 && b[1] == B1) {                                                                          \
      ...
      for (uint32_t i = 0; i < sizeof(pos) && pos[i] < ISRRAN_PUCCH_CS_MAX_ACK; i++) {                                 \
        uci_value[pos[i]] = 1;                                                                                         \
      }                                                                                                                \
      ret = ISRRAN_SUCCESS;                                                                                            \
    }                                                                                                                  \
...
static int puccch_cs_get_ack_a2(uint32_t j, const uint8_t b[2], uint8_t uci_value[ISRRAN_UCI_MAX_ACK_BITS])
{
  ...
  PUCCH_CS_SET_ACK(1, 0, 0, ISRRAN_PUCCH_CS_MAX_ACK);

  return ret;
}

static int puccch_cs_get_ack_a3(uint32_t j, const uint8_t b[2], uint8_t uci_value[ISRRAN_UCI_MAX_ACK_BITS])
{
  int ret = ISRRAN_ERROR;
...
static int puccch_cs_get_ack_a4(uint32_t j, const uint8_t b[2], uint8_t uci_value[ISRRAN_UCI_MAX_ACK_BITS])
{
  int ret = ISRRAN_ERROR;
  ...
  PUCCH_CS_SET_ACK(0, 1, 0, 0);
  PUCCH_CS_SET_ACK(0, 0, 1, 1);
  PUCCH_CS_SET_ACK(0, 0, 0, ISRRAN_PUCCH_CS_MAX_ACK);

  return ret;
}

int isrran_pucch_cs_get_ack(const isrran_pucch_cfg_t* cfg,
                            const isrran_uci_cfg_t*   uci_cfg,
                            uint32_t                  j,
                            const uint8_t             b[2],
                            isrran_uci_value_t*       uci_value)
{
  ...
  if (cfg && uci_cfg && uci_value) {
    ...
    memset(uci_value->ack.ack_value, 0, ISRRAN_UCI_MAX_ACK_BITS);

    uint32_t nof_ack = isrran_uci_cfg_total_ack(uci_cfg);
    switch (nof_ack) {
      case 2:
        // A = 2
...
static int pucch_f3_resources(const isrran_pucch_cfg_t* cfg, const isrran_uci_cfg_t* uci_cfg, uint32_t* n_pucch_3)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;

  if (cfg && uci_cfg && n_pucch_3) {
    n_pucch_3[0] = cfg->n3_pucch_an_list[uci_cfg->ack[0].tpc_for_pucch % ISRRAN_PUCCH_SIZE_AN_CS];
    ret          = 1;
  }
...
static uint32_t get_Np(uint32_t p, uint32_t nof_prb)
{
  if (p == 0) {
    ...
    return nof_prb * (ISRRAN_NRE * p - 4) / 36;
  }
}
...
static int pucch_tdd_resources(const isrran_cell_t*      cell,
                               const isrran_pucch_cfg_t* cfg,
                               const isrran_uci_cfg_t*   uci_cfg,
                               uint32_t                  n_pucch_tdd[ISRRAN_PUCCH_MAX_ALLOC])
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;
...
int isrran_pucch_proc_get_resources(const isrran_cell_t*      cell,
                                    const isrran_pucch_cfg_t* cfg,
                                    const isrran_uci_cfg_t*   uci_cfg,
                                    const isrran_uci_value_t* uci_value,
                                    uint32_t*                 n_pucch_i)
{
  ...
  uint32_t total_nof_ack = isrran_uci_cfg_total_ack(uci_cfg);

  // Available scheduling request and PUCCH format is not PUCCH3
  if (pucch_proc_tx_sr(uci_cfg, uci_value) && cfg->format != ISRRAN_PUCCH_FORMAT_3) {
  ...
  if (cfg->format < ISRRAN_PUCCH_FORMAT_2) {
    ...
    if (cfg->ack_nack_feedback_mode == ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_NORMAL ||
        (cfg->ack_nack_feedback_mode == ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_PUCCH3 &&
    ...
    ERROR("Unhandled PUCCH format mode %s", isrran_ack_nack_feedback_mode_string(cfg->ack_nack_feedback_mode));
    return ISRRAN_ERROR;
  }
...
static uint32_t get_npucch_cs(const isrran_pucch_cfg_t* cfg,
                              const isrran_uci_cfg_t*   uci_cfg,
                              const uint32_t            n_pucch_i[ISRRAN_PUCCH_MAX_ALLOC],
                              uint8_t                   b[ISRRAN_UCI_MAX_ACK_BITS])
{
  ...
  switch (nof_ack) {
    ...
    case 4:
      ...
      break;
    default:
      ERROR("Too many (%d) ACK for this CS mode", isrran_uci_cfg_total_ack(uci_cfg));
  }
...
static uint32_t
get_npucch_tdd(const uint32_t n_pucch[4], const isrran_uci_cfg_t* uci_cfg, uint8_t b[ISRRAN_UCI_MAX_ACK_BITS])
{
...
uint32_t isrran_pucch_proc_get_npucch(const isrran_cell_t*      cell,
                                      const isrran_pucch_cfg_t* cfg,
                                      const isrran_uci_cfg_t*   uci_cfg,
                                      const isrran_uci_value_t* uci_value,
                                      uint8_t                   b[ISRRAN_UCI_MAX_ACK_BITS])
{
  uint32_t n_pucch_i[ISRRAN_PUCCH_MAX_ALLOC] = {};
  int      nof_resources = isrran_pucch_proc_get_resources(cell, cfg, uci_cfg, uci_value, n_pucch_i);

  // Copy original bits in b
  memcpy(b, uci_value->ack.ack_value, ISRRAN_UCI_MAX_ACK_BITS);
