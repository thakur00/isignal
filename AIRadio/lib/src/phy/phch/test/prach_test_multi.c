...
void stagger_prach_powers(isrran_prach_t* prach,
                          cf_t*           preamble,
                          cf_t*           preamble_sum,
                          int             freq_offset,
                          int             n_seqs,
                          int*            offsets)
{
  for (int seq_index = 0; seq_index < n_seqs; seq_index++) {
    isrran_prach_gen(prach, seq_index, freq_offset, preamble);
    if (seq_index == 0) {
      isrran_vec_sc_prod_ccc(preamble, cexpf(_Complex_I * 0.5), preamble, prach->N_cp + prach->N_seq);
      isrran_vec_sc_prod_cfc(preamble, 1, preamble, prach->N_cp + prach->N_seq);
    }
    if (seq_index == 1) {
      isrran_vec_sc_prod_ccc(preamble, cexpf(_Complex_I * 1), preamble, prach->N_cp + prach->N_seq);
      isrran_vec_sc_prod_cfc(preamble, 0.8, preamble, prach->N_cp + prach->N_seq);
    }
    if (seq_index == 2) {
      isrran_vec_sc_prod_ccc(preamble, cexpf(_Complex_I * 0.1), preamble, prach->N_cp + prach->N_seq);
      isrran_vec_sc_prod_cfc(preamble, 0.05, preamble, prach->N_cp + prach->N_seq);
    }
    if (seq_index == 3) {
      isrran_vec_sc_prod_ccc(preamble, cexpf(_Complex_I * 0.9), preamble, prach->N_cp + prach->N_seq);
      isrran_vec_sc_prod_cfc(preamble, 0.7, preamble, prach->N_cp + prach->N_seq);
    }
    if (seq_index == 4) {
      isrran_vec_sc_prod_ccc(preamble, cexpf(_Complex_I * 0.3), preamble, prach->N_cp + prach->N_seq);
      isrran_vec_sc_prod_cfc(preamble, 0.15, preamble, prach->N_cp + prach->N_seq);
    }
    if (seq_index == 5) {
      isrran_vec_sc_prod_cfc(preamble, 0.15, preamble, prach->N_cp + prach->N_seq);
    }
...
int main(int argc, char** argv)
{
  ...
  if (test_offset_calculation || test_successive_cancellation || stagger_prach_power_and_phase) {
    ...
    if (test_offset_calculation) {
      for (int i = 0; i < 6; i++) {
        offsets[i] = isrran_random_uniform_int_dist(random_gen, 0, 49);
      }
    }
  }
  ...
  isrran_prach_set_detect_factor(&prach, 10);
  if (stagger_prach_power_and_phase) {
    ...
    for (seq_index = 0; seq_index < n_seqs; seq_index++) {
      isrran_prach_gen(&prach, seq_index, prach_cfg.freq_offset, preamble);
      int off = (offset == -1) ? offsets[seq_index] : offset;
  ...
  if (input_file_name) {
    isrran_filesource_read(&fsrc, &preamble_sum[prach.N_cp], prach.N_seq);
  }
  ...
  isrran_prach_detect_offset(&prach, 0, &preamble_sum[prach.N_cp], prach_len, indices, t_offsets, NULL, &n_indices);
