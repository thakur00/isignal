...
int main(int argc, char** argv)
{
  int  nof_frames;
  int  ret;
  bool acks[ISRRAN_MAX_TB];
  bzero(acks, sizeof(bool) * ISRRAN_MAX_TB);
  ...
  uint8_t* data[ISRRAN_MAX_CODEWORDS] = {malloc(100000)};
  if (!data[0]) {
    perror("malloc");
    exit(-1);
  }
  ...
  do {
    ...
    dl_sf.tti = sf_idx;
    ret       = isrran_ue_dl_find_and_decode(&ue_dl, &dl_sf, &ue_dl_cfg, &pdsch_cfg, data, acks);
    if (ret > 0) {
