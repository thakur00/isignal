...
void usage(char* prog)
{
  ...
  printf("  -v [set isrran_verbose to info, debug, default none]\n");
  printf("\n");
  ...
  while ((opt = getopt(argc, argv, "acdpfFgGrRstoPOvz")) != -1) {
    switch (opt) {
      ...
      case 'p':
        ...
        if (!isrran_nofprb_isvalid(nof_prb)) {
...
void rf_msg_callback(void* arg, isrran_rf_error_t error)
{
  switch (error.type) {
    case ISRRAN_RF_ERROR_LATE:
      printf("L");
      break;
    case ISRRAN_RF_ERROR_UNDERFLOW:
      printf("U");
      break;
    case ISRRAN_RF_ERROR_OVERFLOW:
      printf("O");
      break;
    case ISRRAN_RF_ERROR_RX:
      printf("R");
      break;
    case ISRRAN_RF_ERROR_OTHER:
      printf("X");
      break;
  }
}
...
int main(int argc, char** argv)
{
  ...
  // Allocate buffers
  cf_t* preamble = isrran_vec_cf_malloc(MAX_LEN);
  cf_t* zeros    = isrran_vec_cf_malloc(flen);
  cf_t* buffer   = isrran_vec_cf_malloc(flen * nof_frames);

  if (!preamble || !zeros || !buffer) {
    return ISRRAN_ERROR;
  }

  isrran_vec_cf_zero(preamble, MAX_LEN);
  isrran_vec_cf_zero(zeros, flen);
  isrran_vec_cf_zero(buffer, flen * nof_frames);
  ...
  if (isrran_prach_init(&prach, isrran_symbol_sz(nof_prb))) {
    return ISRRAN_ERROR;
  }

  if (isrran_prach_set_cfg(&prach, &prach_cfg, nof_prb)) {
    ERROR("Error initiating PRACH object");
    return ISRRAN_ERROR;
  }

  printf("Generating PRACH\n");
  isrran_prach_gen(&prach, seq_idx, frequency_offset, preamble);

  // Send through UHD
  isrran_rf_t rf;
  printf("Opening RF device...\n");
  if (isrran_rf_open_devname(&rf, device_name, uhd_args, 1)) {
  ...
  isrran_rf_set_rx_gain(&rf, uhd_rx_gain);
  isrran_rf_set_tx_gain(&rf, uhd_tx_gain);
  isrran_rf_set_rx_freq(&rf, 0, uhd_freq);
  isrran_rf_set_tx_freq(&rf, 0, uhd_freq);

  printf("Setting sampling rate %.2f MHz\n", (float)srate / 1000000);
  int srate_rf = (int)isrran_rf_set_rx_srate(&rf, (double)srate);
  if (srate_rf != srate) {
    ERROR("Could not set sampling rate");
    exit(-1);
  }
  isrran_rf_set_tx_srate(&rf, (double)srate);
  sleep(1);

  isrran_timestamp_t tstamp;

  // Register error handler
  isrran_rf_register_error_handler(&rf, rf_msg_callback, NULL);
  ...
  for (uint32_t rep = 0; rep < nof_repetitions; rep++) {
    ...
    for (uint32_t nframe = 0; nframe < nof_frames; nframe++) {
      ...
      isrran_rf_recv_with_time(&rf, &buffer[flen * nframe], flen, true, &tstamp.full_secs, &tstamp.frac_secs);

      isrran_timestamp_add(&tstamp, 0, tx_delay_ms * 1e-3 - timeadv * 1e-6);
      if (nframe == 10 - tx_delay_ms) {
        isrran_rf_send_timed2(&rf, preamble, flen, tstamp.full_secs, tstamp.frac_secs, false, !continous_tx);
        INFO("Rep %d. Transmitting PRACH", rep);
      } else if (nframe == 10 - tx_delay_ms - 1 || continous_tx) {
        isrran_rf_send_timed2(&rf, zeros, flen, tstamp.full_secs, tstamp.frac_secs, is_start_of_burst, false);
    ...
    if (isrran_prach_detect_offset(&prach,
                                   frequency_offset,
    ...
    for (int i = 0; i < nof_detected; i++) {
      ...
      // Update stats
      delay_us_min = ISRRAN_MIN(delay_us_min, offsets[i] * 1e6);
      delay_us_max = ISRRAN_MAX(delay_us_max, offsets[i] * 1e6);
      delay_us_avg = ISRRAN_VEC_CMA(offsets[i] * 1e6, delay_us_avg, count);

      peak_min = ISRRAN_MIN(peak_min, peak_to_avg[i]);
      peak_max = ISRRAN_MAX(peak_max, peak_to_avg[i]);
      peak_avg = ISRRAN_VEC_CMA(peak_to_avg[i], peak_avg, count);
  ...
  // End burst
  if (continous_tx) {
    isrran_rf_send_timed2(&rf, zeros, 0, tstamp.full_secs, tstamp.frac_secs, false, true);
  }
  ...
  if (output_filename) {
    ...
    isrran_vec_save_file("generated", preamble, (prach.N_seq + prach.N_cp) * sizeof(cf_t));

    // Save last received buffer
    isrran_vec_save_file(output_filename, buffer, 11 * flen * (uint32_t)sizeof(cf_t));
  }

  isrran_rf_close(&rf);
  isrran_prach_free(&prach);
