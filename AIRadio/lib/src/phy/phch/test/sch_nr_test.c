...
int main(int argc, char** argv)
{
  ...
  isrran_sch_nr_t sch_nr_tx = {};
  isrran_sch_nr_t sch_nr_rx = {};
  isrran_random_t rand_gen  = isrran_random_init(1234);

  uint8_t* data_tx = isrran_vec_u8_malloc(1024 * 1024);
  uint8_t* encoded = isrran_vec_u8_malloc(1024 * 1024 * 8);
  int8_t*  llr     = isrran_vec_i8_malloc(1024 * 1024 * 8);
  uint8_t* data_rx = isrran_vec_u8_malloc(1024 * 1024);
  ...
  isrran_sch_nr_args_t args   = {};
  args.disable_simd           = false;
  args.decoder_use_flooded    = false;
  args.decoder_scaling_factor = 0.8;
  args.max_nof_iter           = 20;
  if (isrran_sch_nr_init_tx(&sch_nr_tx, &args) < ISRRAN_SUCCESS) {
    ERROR("Error initiating SCH NR for Tx");
    goto clean_exit;
  }

  if (isrran_sch_nr_init_rx(&sch_nr_rx, &args) < ISRRAN_SUCCESS) {
    ERROR("Error initiating SCH NR for Rx");
    goto clean_exit;
  }

  if (isrran_sch_nr_set_carrier(&sch_nr_tx, &carrier)) {
    ERROR("Error setting SCH NR carrier");
    goto clean_exit;
  }

  if (isrran_sch_nr_set_carrier(&sch_nr_rx, &carrier)) {
    ERROR("Error setting SCH NR carrier");
    goto clean_exit;
  }
  ...
  for (n_prb = n_prb_start; n_prb < n_prb_end; n_prb++) {
    for (mcs = mcs_start; mcs < mcs_end; mcs++) {
      for (rv = rv_start; rv < rv_end; rv++) {
        ...
        isrran_sch_tb_t tb = {};
        tb.rv              = rv;
        if (isrran_ra_nr_fill_tb(&pdsch_cfg, &pdsch_cfg.grant, mcs, &tb) < ISRRAN_SUCCESS) {
          ERROR("Error filing tb");
          goto clean_exit;
        }

        for (uint32_t i = 0; i < tb.tbs; i++) {
          data_tx[i] = (uint8_t)isrran_random_uniform_int_dist(rand_gen, 0, UINT8_MAX);
        }

        tb.softbuffer.tx = &softbuffer_tx;

        if (isrran_dlsch_nr_encode(&sch_nr_tx, &pdsch_cfg.sch_cfg, &tb, data_tx, encoded) < ISRRAN_SUCCESS) {
        ...
        isrran_softbuffer_rx_reset(tb.softbuffer.rx);

        isrran_sch_tb_res_nr_t res = {};
        res.payload                = data_rx;
        if (isrran_dlsch_nr_decode(&sch_nr_rx, &pdsch_cfg.sch_cfg, &tb, llr, &res) < ISRRAN_SUCCESS) {
          ERROR("Error encoding");
          goto clean_exit;
        }
        ...
        if (rv == 0) {
          ...
          if (memcmp(data_tx, data_rx, tb.tbs / 8) != 0) {
            ...
            printf("Tx data: ");
            isrran_vec_fprint_byte(stdout, data_tx, tb.tbs / 8);
            printf("Rx data: ");
            isrran_vec_fprint_byte(stdout, data_rx, tb.tbs / 8);
            goto clean_exit;
          }
        }
...
clean_exit:
  ...
  isrran_sch_nr_free(&sch_nr_tx);
  isrran_sch_nr_free(&sch_nr_rx);
  if (data_tx) {
    free(data_tx);
  }
  if (data_rx) {
