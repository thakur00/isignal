...
int dl_timing_test()
{
  ...
  isrran_dci_msg_t dci_tmp           = {};
  dci_tmp.format   = ISRRAN_DCI_FORMATN1;
  dci_tmp.nof_bits = 23;
  isrran_bit_unpack_vector(dci_bits_packed, dci_tmp.payload, dci_tmp.nof_bits);
  ...
  int                        sfn    = 185;
  int                        sf_idx = 9;
  isrran_nbiot_dci_msg_to_dl_grant(&dci_tmp, 0x1234, &dl_dci, &dl_grant, sfn, sf_idx, 3, ISRRAN_NBIOT_MODE_STANDALONE);

  // make sure NPDSCH start is not on a SIB1 subframe
  isrran_nbiot_ue_dl_check_grant(&ue_dl, &dl_grant);

  // print final grant
  isrran_ra_nbiot_dl_grant_fprint(stdout, &dl_grant);
