...
#define ISRRAN_NBIOT_SFLEN ISRRAN_SF_LEN(isrran_symbol_sz(cell.base.nof_prb))
#define SFN 0
#define SF_IDX 1

#define ISRRAN_NBIOT_NOF_RE_X_PRB (ISRRAN_CP_NORM_SF_NSYMB * ISRRAN_NRE)
...
isrran_nbiot_mode_t mode            = ISRRAN_NBIOT_MODE_INBAND_SAME_PCI;
 ...
 */
int get_ref_res(isrran_nbiot_cell_t cell, int32_t* re_with_refs)
{
  bzero(re_with_refs, ISRRAN_NBIOT_MAX_NUM_RE_WITH_REFS * sizeof(uint32_t));
  uint32_t num_ref = 0;

  // add all RE that contain NRS
  for (int p = 0; p < cell.nof_ports; p++) {
    DEBUG("Adding NRS for port=%d n_id_ncell=%d", p, cell.n_id_ncell);
    uint32_t nof_syms = isrran_refsignal_dl_nbiot_nof_symbols(p);
    for (int l = 0; l < nof_syms; l++) {
      ...
      for (int m = 0; m < ISRRAN_NBIOT_NUM_NRS_X_SYM_X_PORT; m++) {
        uint32_t fidx   = isrran_refsignal_dl_nbiot_fidx(cell, l, p, m);
        uint32_t re_idx = ISRRAN_RE_IDX(cell.base.nof_prb, nsymbol, fidx);
  ...
  for (int p = 0; p < cell.base.nof_ports; p++) {
    ...
    uint32_t nof_syms = isrran_refsignal_cs_nof_symbols(NULL, NULL, p);
    for (int l = 0; l < nof_syms; l++) {
      uint32_t nsymbol = isrran_refsignal_cs_nsymbol(l, cell.base.cp, p);
      DEBUG("  - adding CRS for symbol=%d", nsymbol);

      // two references per symbol
      for (int m = 0; m < 2; m++) {
        uint32_t fidx   = ((isrran_refsignal_cs_v(p, l) + (cell.base.id % 6)) % 6) + m * 6;
        uint32_t re_idx = ISRRAN_RE_IDX(cell.base.nof_prb, nsymbol, fidx);
  ...
  isrran_vec_fprint_i(stdout, re_with_refs, num_ref);

  return num_ref;
}

int extract_re(isrran_nbiot_cell_t cell, uint32_t l_start, uint32_t expected_nof_re2)
{
  isrran_npdsch_t npdsch;
  bzero(&npdsch, sizeof(isrran_npdsch_t));
  npdsch.cell = cell;

  if (!isrran_nbiot_cell_isvalid(&npdsch.cell)) {
    printf("cell is not properly configured\n");
    return ISRRAN_ERROR;
  }

  // fill dummy values in RE
  cf_t sf_syms[ISRRAN_NBIOT_NOF_RE_X_PRB];
  for (int i = 0; i < ISRRAN_NBIOT_NOF_RE_X_PRB; i++) {
    sf_syms[i] = i;
  }

  isrran_ra_nbiot_dl_grant_t dl_grant;
  bzero(&dl_grant, sizeof(isrran_ra_nbiot_dl_grant_t));
  dl_grant.l_start = l_start;

  cf_t npdsch_syms[ISRRAN_NPDSCH_MAX_RE];
  int  nof_ext_syms = isrran_npdsch_cp(&npdsch, sf_syms, npdsch_syms, &dl_grant, false);

#if 0
  if (ISRRAN_VERBOSE_ISDEBUG()) {
    for (int i = 0; i < nof_ext_syms; i++) {
      printf("re%d:", i);
      isrran_vec_fprint_c(stdout, &npdsch_syms[i], 1);
    }
  }
#endif
  ...
  int32_t re_with_refs[ISRRAN_NBIOT_MAX_NUM_RE_WITH_REFS];
  ...
  for (int i = 0; i < num_re_with_refs; i++) {
    for (int l = 0; l < nof_ext_syms; l++) {
      if (npdsch_syms[l] == re_with_refs[i]) {
        ...
        isrran_vec_fprint_c(stdout, &npdsch_syms[l], 1);
        return ISRRAN_ERROR;
      }
    }
  }
...
int re_extract_test(int argc, char** argv)
{
  ...
  cell.mode           = mode;
  cell.base.nof_prb   = 1;
  cell.base.id        = (mode == ISRRAN_NBIOT_MODE_INBAND_SAME_PCI)
                     ? n_id_ncell
                     : isrran_random_uniform_int_dist(random_gen, 0, ISRRAN_NUM_PCI - 1);
  cell.n_id_ncell     = n_id_ncell;
  cell.nof_ports      = nof_ports_nbiot;
  cell.base.nof_ports = nof_ports_lte;
  uint32_t l_start    = 0;

  if (extract_re(cell, l_start, expected_nof_re) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
...
int coding_test(int argc, char** argv)
{
  ...
  cell.nof_ports           = 1;
  cell.mode                = ISRRAN_NBIOT_MODE_STANDALONE;
  cell.n_id_ncell          = n_id_ncell;
  ...
  bzero(ce, sizeof(cf_t*) * ISRRAN_MAX_PORTS);
  bzero(slot_symbols, sizeof(cf_t*) * ISRRAN_MAX_PORTS);

  isrran_ra_nbiot_dl_dci_t dci;
  bzero(&dci, sizeof(isrran_ra_nbiot_dl_dci_t));
  dci.mcs_idx = i_tbs_val;
  dci.rv_idx  = rv_idx;
  if (isrran_ra_nbiot_dl_dci_to_grant(&dci, &grant, SFN, SF_IDX, DUMMY_R_MAX, false, cell.mode)) {
  ...
  /* Configure NPDSCH */
  if (isrran_npdsch_cfg(&npdsch_cfg, cell, &grant, subframe)) {
  ...
  for (int i = 0; i < ISRRAN_MAX_PORTS; i++) {
    ce[i] = isrran_vec_malloc(sizeof(cf_t) * ISRRAN_SF_LEN_RE(cell.base.nof_prb, cell.base.cp));
    if (!ce[i]) {
    ...
    slot_symbols[i] = isrran_vec_malloc(sizeof(cf_t) * ISRRAN_SF_LEN_RE(cell.base.nof_prb, cell.base.cp));
    if (!slot_symbols[i]) {
      perror("isrran_vec_malloc");
      goto quit;
    }
    memset(slot_symbols[i], 0, sizeof(cf_t) * ISRRAN_SF_LEN_RE(cell.base.nof_prb, cell.base.cp));
  }

  sf_symbols = isrran_vec_malloc(sizeof(cf_t) * ISRRAN_NBIOT_SFLEN);
  memset(sf_symbols, 0, sizeof(cf_t) * ISRRAN_NBIOT_SFLEN);

  data = isrran_vec_malloc(sizeof(uint8_t) * grant.mcs[0].tbs / 8);
  if (!data) {
    perror("isrran_vec_malloc");
    goto quit;
  }

  rx_data = isrran_vec_malloc(sizeof(uint8_t) * grant.mcs[0].tbs / 8);
  if (!rx_data) {
    perror("isrran_vec_malloc");
    goto quit;
  }

  isrran_ofdm_tx_init(&ofdm_tx, cell.base.cp, slot_symbols[0], sf_symbols, cell.base.nof_prb);
  isrran_ofdm_rx_init(&ofdm_rx, cell.base.cp, sf_symbols, slot_symbols[0], cell.base.nof_prb);
  ...
  isrran_npdsch_set_rnti(&npdsch, rnti);

  if (input_file) {
    isrran_filesource_t fsrc;
    if (isrran_filesource_init(&fsrc, input_file, ISRRAN_COMPLEX_FLOAT_BIN)) {
      fprintf(stderr, "Error opening file %s\n", input_file);
      exit(-1);
    }
#ifdef DO_OFDM
    isrran_filesource_read(&fsrc, sf_symbols, ISRRAN_SF_LEN_PRB(cell.base.nof_prb));
#else
    isrran_filesource_read(&fsrc, slot_symbols[0], ISRRAN_SF_LEN_RE(cell.base.nof_prb, cell.base.cp));
#endif
    ...
    isrran_chest_dl_nbiot_estimate(&chest, slot_symbols[0], ce, subframe);
  ...
  for (int i = 0; i < grant.mcs[0].tbs / 8; i++) {
    data[i] = (uint8_t)isrran_random_uniform_int_dist(random_gen, 0, 255);
  }

  if (!input_file) {
    if (isrran_npdsch_encode(&npdsch, &npdsch_cfg, NULL, data, slot_symbols)) {
  ...
  if (ISRRAN_VERBOSE_ISDEBUG()) {
    ...
    isrran_vec_save_file("npdsch_tx_sf.bin", sf_symbols, ISRRAN_NBIOT_SFLEN * sizeof(cf_t));
  }
...
#endif
    r = isrran_npdsch_decode(&npdsch, &npdsch_cfg, NULL, slot_symbols[0], ce, 0, 0, rx_data);
  }
  ...
  printf("Tx payload: ");
  isrran_vec_fprint_b(stdout, data, grant.mcs[0].tbs / 8);
  printf("Rx payload: ");
  isrran_vec_fprint_b(stdout, rx_data, grant.mcs[0].tbs / 8);
...
int main(int argc, char** argv)
{
  ...
  if (re_extract_test(argc, argv) != ISRRAN_SUCCESS) {
    printf("Resource element extraction test failed!\n");
    return ret;
  }

  if (coding_test(argc, argv) != ISRRAN_SUCCESS) {
