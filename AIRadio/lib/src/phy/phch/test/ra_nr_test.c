...
int test_type0_multiple()
{
  ...
  dci_dl.format          = isrran_dci_format_nr_1_0;

  dci_dl.freq_domain_assigment = 0xC0; // RBG 0 and 1

  TESTASSERT(isrran_ra_dl_nr_freq(&carrier, &sch_cfg, &dci_dl, &grant) == ISRRAN_SUCCESS);
...
int test_type0_not_multiple()
{
  ...
  bzero(&grant, sizeof(isrran_sch_grant_nr_t));
...
{
  // TESTASSERT(test_type0_multiple() == ISRRAN_SUCCESS);
  TESTASSERT(test_type0_not_multiple() == ISRRAN_SUCCESS);
  return ISRRAN_SUCCESS;
}
