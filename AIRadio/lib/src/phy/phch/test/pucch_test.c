...
int test_uci_cqi_pucch(void)
{
  ...
  __attribute__((aligned(256))) uint8_t o_bits[ISRRAN_UCI_MAX_CQI_LEN_PUCCH] = {0};
  __attribute__((aligned(256))) uint8_t e_bits[ISRRAN_UCI_CQI_CODED_PUCCH_B] = {0};
  __attribute__((aligned(256))) int16_t e_symb[ISRRAN_CQI_MAX_BITS]          = {0};
  __attribute__((aligned(256))) uint8_t d_bits[ISRRAN_UCI_MAX_CQI_LEN_PUCCH] = {0};

  isrran_uci_cqi_pucch_t uci_cqi_pucch = {0};

  isrran_uci_cqi_pucch_init(&uci_cqi_pucch);

  for (uint32_t nof_bits = 1; nof_bits <= ISRRAN_UCI_MAX_CQI_LEN_PUCCH - 1; nof_bits++) {
    for (uint32_t cqi = 0; cqi < (1 << nof_bits); cqi++) {
      uint32_t recv;

      uint8_t* ptr = o_bits;
      isrran_bit_unpack(cqi, &ptr, nof_bits);

      isrran_uci_encode_cqi_pucch(o_bits, nof_bits, e_bits);
      // isrran_uci_encode_cqi_pucch_from_table(&uci_cqi_pucch, o_bits, nof_bits, e_bits);
      for (int i = 0; i < ISRRAN_UCI_CQI_CODED_PUCCH_B; i++) {
        e_symb[i] = 2 * e_bits[i] - 1;
      }

      isrran_uci_decode_cqi_pucch(&uci_cqi_pucch, e_symb, d_bits, nof_bits);

      ptr  = d_bits;
      recv = isrran_bit_pack(&ptr, nof_bits);

      if (recv != cqi) {
        printf("Error! cqi = %d (len: %d), %X!=%X \n", cqi, nof_bits, cqi, recv);
        if (get_isrran_verbose_level()) {
          printf("original: ");
          isrran_vec_fprint_b(stdout, o_bits, nof_bits);
          printf(" decoded: ");
          isrran_vec_fprint_b(stdout, d_bits, nof_bits);
        }
        ret = ISRRAN_ERROR;
      }
    }
  ...
  isrran_uci_cqi_pucch_free(&uci_cqi_pucch);
...
{
  isrran_pucch_t        pucch_ue   = {};
  isrran_pucch_t        pucch_enb  = {};
  isrran_pucch_cfg_t    pucch_cfg  = {};
  isrran_refsignal_ul_t dmrs       = {};
  cf_t*                 sf_symbols = NULL;
  cf_t                  pucch_dmrs[2 * ISRRAN_NRE * 3];
  int                   ret       = -1;
  ...
  if (isrran_pucch_init_ue(&pucch_ue)) {
    ERROR("Error creating PDSCH object");
    exit(-1);
  }
  if (isrran_pucch_set_cell(&pucch_ue, cell)) {
    ERROR("Error creating PDSCH object");
    exit(-1);
  }
  if (isrran_pucch_init_enb(&pucch_enb)) {
    ERROR("Error creating PDSCH object");
    exit(-1);
  }
  if (isrran_pucch_set_cell(&pucch_enb, cell)) {
    ERROR("Error creating PDSCH object");
    exit(-1);
  }
  if (isrran_refsignal_ul_set_cell(&dmrs, cell)) {
    ERROR("Error creating PDSCH object");
    exit(-1);
  }

  if (isrran_chest_ul_init(&chest, cell.nof_prb) < ISRRAN_SUCCESS) {
    ERROR("Error initiating channel estimator");
    goto quit;
  }

  if (isrran_chest_ul_res_init(&chest_res, cell.nof_prb) < ISRRAN_SUCCESS) {
    ERROR("Error initiating channel estimator result");
    goto quit;
  }

  if (isrran_chest_ul_set_cell(&chest, cell) < ISRRAN_SUCCESS) {
  ...
  sf_symbols = isrran_vec_cf_malloc(ISRRAN_NOF_RE(cell));
  if (!sf_symbols) {
    goto quit;
  }
  ...
  for (format = 0; format < ISRRAN_PUCCH_FORMAT_ERROR; format++) {
    for (uint32_t d = 1; d <= 3; d++) {
      for (uint32_t ncs = 0; ncs < 8; ncs += d) {
        for (uint32_t n_pucch = 1; n_pucch < 130; n_pucch += 50) {
          ...
          if (isrran_pucch_encode(&pucch_ue, &ul_sf, &pucch_cfg, &uci_data.value, sf_symbols)) {
            ERROR("Error encoding PUCCH");
            goto quit;
          }

          if (isrran_refsignal_dmrs_pucch_gen(&dmrs, &ul_sf, &pucch_cfg, pucch_dmrs)) {
            ERROR("Error encoding PUCCH");
            goto quit;
          }
          if (isrran_refsignal_dmrs_pucch_put(&dmrs, &pucch_cfg, pucch_dmrs, sf_symbols)) {
            ERROR("Error encoding PUCCH");
            goto quit;
          }
          ...
          // Run AWGN channel
          isrran_channel_awgn_run_c(&awgn, sf_symbols, sf_symbols, ISRRAN_NOF_RE(cell));

          // Decode PUCCH signals
          gettimeofday(&t[1], NULL);
          if (isrran_chest_ul_estimate_pucch(&chest, &ul_sf, &pucch_cfg, sf_symbols, &chest_res) < ISRRAN_SUCCESS) {
            ERROR("Error estimating PUCCH channel");
            goto quit;
          }

          isrran_pucch_res_t res = {};
          if (isrran_pucch_decode(&pucch_enb, &ul_sf, &pucch_cfg, &chest_res, sf_symbols, &res) < ISRRAN_SUCCESS) {
  ...
  ret = 0;
quit:
  isrran_pucch_free(&pucch_ue);
  isrran_pucch_free(&pucch_enb);
  isrran_chest_ul_free(&chest);
