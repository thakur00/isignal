...
#ifdef DO_OFDM
#define NOF_CE_SYMBOLS ISRRAN_SF_LEN_PRB(cell.nof_prb)
#else
...
uint32_t mcs_idx                      = 2;
uint32_t subframe                     = 1;
int      rv_idx[ISRRAN_MAX_CODEWORDS] = {0, 1};
uint16_t rnti                         = 1234;
...
int main(int argc, char** argv)
{
  ...
  static uint8_t*         data_tx[ISRRAN_MAX_CODEWORDS] = {NULL};
  static uint8_t*         data_rx[ISRRAN_MAX_CODEWORDS] = {NULL};
...
#ifdef DO_OFDM
  cf_t* tx_sf_symbols[ISRRAN_MAX_PORTS] = {};
  cf_t* rx_sf_symbols[ISRRAN_MAX_PORTS] = {};
#endif /* DO_OFDM */
  ...
  isrran_ofdm_t         ifft_mbsfn[ISRRAN_MAX_PORTS] = {};
  isrran_ofdm_t         fft_mbsfn[ISRRAN_MAX_PORTS]  = {};

  parse_args(argc, argv);
  /* Initialise to zeros */
  bzero(&pmch, sizeof(isrran_pmch_t));
  bzero(tx_slot_symbols, sizeof(cf_t*) * ISRRAN_MAX_PORTS);
  bzero(rx_slot_symbols, sizeof(cf_t*) * ISRRAN_MAX_PORTS);
  ...
  for (i = 0; i < cell.nof_ports; i++) {
    tx_sf_symbols[i] = isrran_vec_cf_malloc(ISRRAN_SF_LEN_PRB(cell.nof_prb));
    isrran_vec_cf_zero(tx_sf_symbols[i], ISRRAN_SF_LEN_PRB(cell.nof_prb));
    if (isrran_ofdm_tx_init_mbsfn(&ifft_mbsfn[i], ISRRAN_CP_EXT, tx_slot_symbols[i], tx_sf_symbols[i], cell.nof_prb)) {
      ERROR("Error creating iFFT object");
      exit(-1);
    }

    isrran_ofdm_set_non_mbsfn_region(&ifft_mbsfn[i], non_mbsfn_region);
    isrran_ofdm_set_normalize(&ifft_mbsfn[i], true);
  }

  for (i = 0; i < nof_rx_antennas; i++) {
    rx_sf_symbols[i] = isrran_vec_cf_malloc(ISRRAN_SF_LEN_PRB(cell.nof_prb));
    isrran_vec_cf_zero(rx_sf_symbols[i], ISRRAN_SF_LEN_PRB(cell.nof_prb));
    if (isrran_ofdm_rx_init_mbsfn(&fft_mbsfn[i], ISRRAN_CP_EXT, rx_sf_symbols[i], rx_slot_symbols[i], cell.nof_prb)) {
      ERROR("Error creating iFFT object");
      exit(-1);
    }

    isrran_ofdm_set_non_mbsfn_region(&fft_mbsfn[i], non_mbsfn_region);
    isrran_ofdm_set_normalize(&fft_mbsfn[i], true);
  }
  ...
  for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    if (pmch_cfg.pdsch_cfg.grant.tb[i].enabled) {
      data_tx[i] = isrran_vec_u8_malloc((uint32_t)pmch_cfg.pdsch_cfg.grant.tb[i].tbs);
      if (!data_tx[i]) {
        perror("isrran_vec_malloc");
        goto quit;
      }
      ...
      data_rx[i] = isrran_vec_u8_malloc((uint32_t)pmch_cfg.pdsch_cfg.grant.tb[i].tbs);
      if (!data_rx[i]) {
        perror("isrran_vec_malloc");
        goto quit;
      }
  ...
  INFO("         mcs.mod=%s", isrran_mod_string(pmch_cfg.pdsch_cfg.grant.tb[0].mod));
  ...
  for (int tb = 0; tb < ISRRAN_MAX_CODEWORDS; tb++) {
    if (pmch_cfg.pdsch_cfg.grant.tb[tb].enabled) {
      for (int byte = 0; byte < pmch_cfg.pdsch_cfg.grant.tb[tb].tbs / 8; byte++) {
        data_tx[tb][byte] = (uint8_t)isrran_random_uniform_int_dist(random, 0, 255);
      }
    }
  }

  if (isrran_pmch_encode(&pmch, &dl_sf, &pmch_cfg, data_tx[0], tx_slot_symbols)) {
  ...
  for (i = 0; i < cell.nof_ports; i++) {
    ...
    isrran_ofdm_tx_sf(&ifft_mbsfn[i]);
  }
  ...
  /* combine outputs */
  for (j = 0; j < nof_rx_antennas; j++) {
    for (k = 0; k < ISRRAN_NOF_RE(cell); k++) {
  ...
  for (i = 0; i < nof_rx_antennas; i++) {
    isrran_ofdm_rx_sf(&fft_mbsfn[i]);
  }
#endif
  for (i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    if (pmch_cfg.pdsch_cfg.grant.tb[i].enabled) {
      isrran_softbuffer_rx_reset_tbs(softbuffers_rx[i], (uint32_t)pmch_cfg.pdsch_cfg.grant.tb[i].tbs);
    }
  }

  pmch_cfg.pdsch_cfg.softbuffers.rx[0] = softbuffers_rx[0];

  isrran_pdsch_res_t pdsch_res[2];
  pdsch_res[0].payload = data_rx[0];

  r = isrran_pmch_decode(&pmch, &dl_sf, &pmch_cfg, &chest_dl_res, rx_slot_symbols, pdsch_res);
  gettimeofday(&t[2], NULL);
  get_time_interval(t);
...
quit:
  ...
  for (i = 0; i < nof_rx_antennas; i++) {
    isrran_ofdm_tx_free(&ifft_mbsfn[i]);
    isrran_ofdm_rx_free(&fft_mbsfn[i]);
    if (rx_sf_symbols[i]) {
