...
#define NOF_CE_SYMBOLS ISRRAN_NOF_RE(cell)
...
static isrran_cell_t cell = {
    ...
    ISRRAN_PHICH_R_1_6, // PHICH resources
    ISRRAN_FDD,

};

static isrran_tm_t tm                           = ISRRAN_TM1;
static uint32_t    cfi                          = 1;
static uint32_t    mcs[ISRRAN_MAX_CODEWORDS]    = {0, 0};
static uint32_t    subframe                     = 1;
static int         rv_idx[ISRRAN_MAX_CODEWORDS] = {0, 1};
  ...
  while ((opt = getopt(argc, argv, "fmMcsbrtRFpnqawvXxj")) != -1) {
    switch (opt) {
      ...
      case 'F':
        ...
        break;
      case 'x':
        tm = (isrran_tm_t)(strtol(argv[optind], NULL, 10) - 1);
        break;
      case 'p':
...
static int check_softbits(isrran_pdsch_t*     pdsch_enb,
                          isrran_pdsch_t*     pdsch_ue,
                          ...
                          int                 tb)
{
  ...
  if (!pdsch_ue->llr_is_8bit && !tb_cw_swap) {
    // Scramble
    isrran_sequence_pdsch_apply_s(pdsch_ue->e[tb],
                                  pdsch_ue->e[tb],
                                  rnti,
    ...
    if (memcmp(pdsch_ue->e[tb], pdsch_enb->e[tb], pdsch_cfg->grant.tb[tb].nof_bits / 8) != 0) {
      printf("tx=");
      isrran_vec_fprint_byte(stdout, pdsch_enb->e[tb], pdsch_cfg->grant.tb[tb].nof_bits / 8);
      printf("rx=");
      isrran_vec_fprint_byte(stdout, pdsch_ue->e[tb], pdsch_cfg->grant.tb[tb].nof_bits / 8);
      ret = ISRRAN_ERROR;
    }
  }
  return ret;
...
int main(int argc, char** argv)
{
  int                     ret  = -1;
  struct timeval          t[3] = {};
  isrran_softbuffer_tx_t* softbuffers_tx[ISRRAN_MAX_CODEWORDS];
  bool                    acks[ISRRAN_MAX_CODEWORDS] = {false};

  uint8_t*                data_tx[ISRRAN_MAX_CODEWORDS] = {NULL};
  uint8_t*                data_rx[ISRRAN_MAX_CODEWORDS] = {NULL};
  isrran_softbuffer_rx_t* softbuffers_rx[ISRRAN_MAX_CODEWORDS];
  isrran_pdsch_cfg_t      pdsch_cfg;
  isrran_dl_sf_cfg_t      dl_sf;
  cf_t*                   tx_slot_symbols[ISRRAN_MAX_PORTS];
  cf_t*                   rx_slot_symbols[ISRRAN_MAX_PORTS];
  isrran_pdsch_t          pdsch_tx, pdsch_rx;
  isrran_ofdm_t           ofdm_tx[ISRRAN_MAX_PORTS];
  isrran_ofdm_t           ofdm_rx[ISRRAN_MAX_PORTS];
  ...
  switch (tm) {
    case ISRRAN_TM1:
    case ISRRAN_TM2:
      dci.format = ISRRAN_DCI_FORMAT1A;
      break;
    case ISRRAN_TM3:
  ...
  pdsch_cfg.p_b         = (tm > ISRRAN_TM1) ? 1 : 0; // 0 dB

  /* Generate dci from DCI */
  if (isrran_ra_dl_dci_to_grant(&cell, &dl_sf, tm, enable_256qam, &dci, &pdsch_cfg.grant)) {
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_PORTS; i++) {
    rx_slot_symbols[i] = isrran_vec_cf_malloc(ISRRAN_NOF_RE(cell));
  ...
  for (uint32_t i = 0; i < cell.nof_ports; i++) {
    tx_slot_symbols[i] = calloc(ISRRAN_NOF_RE(cell), sizeof(cf_t));
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_TB; i++) {
    if (pdsch_cfg.grant.tb[i].enabled) {
      data_tx[i] = isrran_vec_u8_malloc(pdsch_cfg.grant.tb[i].tbs);
      if (!data_tx[i]) {
        perror("isrran_vec_malloc");
        goto quit;
      }
      ...
      data_rx[i] = isrran_vec_u8_malloc(pdsch_cfg.grant.tb[i].tbs);
      if (!data_rx[i]) {
        perror("isrran_vec_malloc");
        goto quit;
      }
  ...
  if (isrran_pdsch_init_ue(&pdsch_rx, cell.nof_prb, nof_rx_antennas)) {
    ERROR("Error creating PDSCH object");
    goto quit;
  }
  if (isrran_pdsch_set_cell(&pdsch_rx, cell)) {
    ERROR("Error creating PDSCH object");
    goto quit;
  }
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    softbuffers_rx[i] = calloc(sizeof(isrran_softbuffer_rx_t), 1);
    if (!softbuffers_rx[i]) {
      ERROR("Error allocating RX soft buffer");
      goto quit;
    }

    if (isrran_softbuffer_rx_init(softbuffers_rx[i], cell.nof_prb)) {
      ERROR("Error initiating RX soft buffer");
      goto quit;
    }

    isrran_softbuffer_rx_reset(softbuffers_rx[i]);
  }
  ...
  if (input_file) {
    ...
    isrran_chest_dl_estimate(&chest, &dl_sf, rx_slot_symbols, &chest_res);
    isrran_chest_dl_free(&chest);

    isrran_filesource_free(&fsrc);
  } else {
    if (isrran_pdsch_init_enb(&pdsch_tx, cell.nof_prb)) {
      ERROR("Error creating PDSCH object");
      goto quit;
    }
    if (isrran_pdsch_set_cell(&pdsch_tx, cell)) {
      ERROR("Error creating PDSCH object");
      goto quit;
    }

    for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      softbuffers_tx[i] = calloc(sizeof(isrran_softbuffer_tx_t), 1);
      if (!softbuffers_tx[i]) {
        ERROR("Error allocating TX soft buffer");
      }

      if (isrran_softbuffer_tx_init(softbuffers_tx[i], cell.nof_prb)) {
        ERROR("Error initiating TX soft buffer");
        goto quit;
      }
    }

    if (isrran_crc_init(&crc_tb, ISRRAN_LTE_CRC24A, 24) < ISRRAN_SUCCESS) {
      ERROR("Error initiating CRC24A");
      goto quit;
    }

    // Generate random data
    for (int tb = 0; tb < ISRRAN_MAX_CODEWORDS; tb++) {
      if (pdsch_cfg.grant.tb[tb].enabled) {
        for (int byte = 0; byte < pdsch_cfg.grant.tb[tb].tbs / 8; byte++) {
          data_tx[tb][byte] = (uint8_t)isrran_random_uniform_int_dist(random_gen, 0, 255);
        }
        // Attach CRC for making sure TB with 0 CRC are detected
        isrran_crc_attach_byte(&crc_tb, data_tx[tb], pdsch_cfg.grant.tb[tb].tbs - 24);
      }
    }

    /*uint8_t databit[100000];
    isrran_bit_unpack_vector(data, databit, dci.mcs.tbs);
    isrran_vec_save_file("data_in", databit, dci.mcs.tbs);*/
    ...
    if (rv_idx[0] != 0) {
      ...
      if (isrran_pdsch_encode(&pdsch_tx, &dl_sf, &pdsch_cfg, data_tx, tx_slot_symbols)) {
    ...
    for (uint32_t j = 0; j < nof_rx_antennas; j++) {
      for (uint32_t k = 0; k < ISRRAN_NOF_RE(cell); k++) {
  ...
  INFO("              id=%d", cell.id);
  INFO("              cp=%s", isrran_cp_string(cell.cp));
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    ...
    INFO("         mcs.mod=%s", isrran_mod_string(pdsch_cfg.grant.tb[i].mod));
  ...
  int r = 0;
  if (enable_coworker) {
    isrran_pdsch_enable_coworker(&pdsch_rx);
  }
  ...
  for (uint32_t k = 0; k < M; k++) {
    for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      if (pdsch_cfg.grant.tb[i].enabled) {
        isrran_softbuffer_rx_reset_tbs(softbuffers_rx[i], (uint32_t)pdsch_cfg.grant.tb[i].tbs);
      }
    }
    ...
    r = isrran_pdsch_decode(&pdsch_rx, &dl_sf, &pdsch_cfg, &chest_res, rx_slot_symbols, pdsch_res);
  }
  gettimeofday(&t[2], NULL);
  get_time_interval(t);
  ...
  for (int tb = 0; tb < ISRRAN_MAX_CODEWORDS; tb++) {
    if (pdsch_cfg.grant.tb[tb].enabled) {
      ret |= (acks[tb]) ? ISRRAN_SUCCESS : ISRRAN_ERROR;
    }
  }
...
quit:
  for (uint32_t i = 0; i < cell.nof_ports; i++) {
    isrran_ofdm_tx_free(&ofdm_tx[i]);
  }
  for (uint32_t i = 0; i < nof_rx_antennas; i++) {
    isrran_ofdm_rx_free(&ofdm_rx[i]);
  }
  isrran_chest_dl_res_free(&chest_res);
  isrran_chest_dl_free(&chest);
  isrran_pdsch_free(&pdsch_tx);
  isrran_pdsch_free(&pdsch_rx);
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    isrran_softbuffer_tx_free(softbuffers_tx[i]);
    if (softbuffers_tx[i]) {
      free(softbuffers_tx[i]);
    }

    isrran_softbuffer_rx_free(softbuffers_rx[i]);
    if (softbuffers_rx[i]) {
