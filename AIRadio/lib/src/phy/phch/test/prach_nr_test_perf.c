...
int main(int argc, char** argv)
{
  ...
  const int   fft_size         = isrran_symbol_sz(nof_prb);
  ...
  if (isrran_prach_init(&prach, fft_size)) {
  ...
  if (isrran_prach_gen(&prach, seq_index, 0, preamble) < ISRRAN_SUCCESS) {
  ...
  float prach_pwr_sqrt = sqrtf(isrran_vec_avg_power_cf(preamble, preamble_length));
  if (!isnormal(prach_pwr_sqrt)) {
    ERROR("PRACH preamble power is not a finite, nonzero value");
    isrran_prach_free(&prach);
    return ISRRAN_ERROR;
  }
  isrran_vec_sc_prod_cfc(preamble, 1.0F / prach_pwr_sqrt, preamble, preamble_length);
  ...
  float noise_var                  = isrran_convert_dB_to_power(-snr_dB);
  int   ok_detection               = 0;
  ...
  int step = ISRRAN_MAX(nof_runs / 100, 1);
  for (int i_run = 0; i_run < nof_runs; i_run++) {
    ...
    isrran_vec_cf_zero(noise_vec, vector_length);
    isrran_ch_awgn_c(noise_vec, noise_vec, noise_var, vector_length);
    if (is_verbose) {
      float prach_pwr = isrran_vec_avg_power_cf(preamble, preamble_length);
      float noise_pwr = isrran_vec_avg_power_cf(noise_vec, vector_length);
      printf("    Tx power: %.3f\n", prach_pwr);
      printf("    Noise power: %.3f\n", noise_pwr);
      printf("    Target/measured SNR: %.3f / %.3f dB\n", snr_dB, isrran_convert_power_to_dB(prach_pwr / noise_pwr));
    }
    ...
    for (int i = 0; i < nof_offset_steps; i++) {
      ...
      isrran_vec_cf_copy(symbols, noise_vec, vector_length);
      isrran_vec_sum_ccc(&symbols[offset_samples], preamble, &symbols[offset_samples], preamble_length);

      isrran_prach_detect_offset(&prach, 0, &symbols[prach.N_cp], slot_length, indices, offset_est, NULL, &n_indices);
    ...
    isrran_prach_detect_offset(&prach, 0, &noise_vec[prach.N_cp], slot_length, indices, offset_est, NULL, &n_indices);
