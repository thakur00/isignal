  ...
  while ((opt = getopt(argc, argv, "pmTLACv")) != -1) {
    switch (opt) {
      ...
      case 'm':
        ...
        break;
      case 'T':
        pusch_cfg.sch_cfg.mcs_table = isrran_mcs_table_from_str(argv[optind]);
        break;
      case 'L':
...
int main(int argc, char** argv)
{
  ...
  for (n_prb = n_prb_start; n_prb < n_prb_end; n_prb++) {
    for (mcs = mcs_start; mcs < mcs_end; mcs++) {
      ...
      if (isrran_pusch_nr_encode(&pusch_tx, &pusch_cfg, &pusch_cfg.grant, &data_tx, sf_symbols) < ISRRAN_SUCCESS) {
      ...
      if (isrran_pusch_nr_decode(&pusch_rx, &pusch_cfg, &pusch_cfg.grant, &chest, sf_symbols, &data_rx) <
          ISRRAN_SUCCESS) {
        ERROR("Error encoding");
        goto clean_exit;
      }
      ...
      uint32_t nof_re = isrran_ra_dl_nr_slot_nof_re(&pusch_cfg, &pusch_cfg.grant);
      if (nof_re * pusch_cfg.grant.nof_layers > 0) {
        ...
        if (mse > 0.001) {
          ...
          for (uint32_t i = 0; i < pusch_cfg.grant.nof_layers; i++) {
            printf("d_tx[%d]=", i);
            isrran_vec_fprint_c(stdout, pusch_tx.d[i], nof_re);
            printf("d_rx[%d]=", i);
            isrran_vec_fprint_c(stdout, pusch_rx.d[i], nof_re);
          }
          goto clean_exit;
        }
      }
      ...
      if (pusch_rx.G_ulsch > 0) {
        ...
        if (memcmp(pusch_tx.g_ulsch, pusch_rx.g_ulsch, pusch_tx.G_ulsch) != 0) {
          printf("g_ulsch_tx=");
          isrran_vec_fprint_byte(stdout, pusch_tx.g_ulsch, pusch_tx.G_ulsch);
          printf("g_ulsch_rx=");
          isrran_vec_fprint_byte(stdout, pusch_rx.g_ulsch, pusch_tx.G_ulsch);
          //            isrran_vec_fprint_bs(stdout, (int8_t*)pusch_rx.g_ulsch, pusch_rx.G_ulsch);
      ...
      if (memcmp(data_tx.payload[0], data_rx.tb[0].payload, pusch_cfg.grant.tb[0].tbs / 8) != 0) {
        ...
        printf("Tx data: ");
        isrran_vec_fprint_byte(stdout, data_tx.payload[0], pusch_cfg.grant.tb[0].tbs / 8);
        printf("Rx data: ");
        isrran_vec_fprint_byte(stdout, data_tx.payload[0], pusch_cfg.grant.tb[0].tbs / 8);
        goto clean_exit;
      }
      ...
      if (nof_ack_bits > 0) {
        if (memcmp(data_tx.uci.ack, data_rx.uci.ack, nof_ack_bits) != 0) {
          ...
          printf("Tx data: ");
          isrran_vec_fprint_byte(stdout, data_tx.uci.ack, nof_ack_bits);
          printf("Rx data: ");
          isrran_vec_fprint_byte(stdout, data_rx.uci.ack, nof_ack_bits);
          goto clean_exit;
        }
      }
      ...
      if (nof_csi_bits > 0) {
        if (memcmp(data_tx.uci.csi[0].none, data_rx.uci.csi[0].none, nof_csi_bits) != 0) {
          ...
          printf("Tx data: ");
          isrran_vec_fprint_byte(stdout, data_tx.uci.csi[0].none, nof_csi_bits);
          printf("Rx data: ");
          isrran_vec_fprint_byte(stdout, data_rx.uci.csi[0].none, nof_csi_bits);
          goto clean_exit;
        }
      }
