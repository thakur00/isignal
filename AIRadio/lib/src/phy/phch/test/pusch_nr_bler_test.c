...
static isrran_sch_cfg_nr_t pusch_cfg    = {};
...
void usage(char* prog)
{
  ...
  printf("\t-T Provide MCS table (64qam, 256qam, 64qamLowSE) [Default %s]\n",
         isrran_mcs_table_to_str(pusch_cfg.sch_cfg.mcs_table));
  ...
  while ((opt = getopt(argc, argv, "p:m:T:L:A:C:N:s:fv")) != -1) {
    switch (opt) {
      ...
      case 'm':
        ...
        break;
      case 'T':
        pusch_cfg.sch_cfg.mcs_table = isrran_mcs_table_from_str(optarg);
        break;
      case 'L':
...
int main(int argc, char** argv)
{
  ...
  isrran_pusch_nr_t     pusch_tx = {};
  isrran_pusch_nr_t     pusch_rx = {};
  isrran_chest_dl_res_t chest    = {};
  isrran_random_t       rand_gen = isrran_random_init(1234);

  isrran_pusch_data_nr_t data_tx                             = {};
  isrran_pusch_res_nr_t  data_rx                             = {};
  cf_t*                  sf_symbols_tx[ISRRAN_MAX_LAYERS_NR] = {};
  cf_t*                  sf_symbols_rx[ISRRAN_MAX_LAYERS_NR] = {};

  // Set default PUSCH configuration
  pusch_cfg.sch_cfg.mcs_table = isrran_mcs_table_64qam;

  if (parse_args(argc, argv) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }

  isrran_pusch_nr_args_t pusch_args = {};
  pusch_args.sch.disable_simd       = false;
  pusch_args.measure_evm            = true;

  if (isrran_pusch_nr_init_ue(&pusch_tx, &pusch_args) < ISRRAN_SUCCESS) {
    ERROR("Error initiating PUSCH for Tx");
    goto clean_exit;
  }

  if (isrran_pusch_nr_init_gnb(&pusch_rx, &pusch_args) < ISRRAN_SUCCESS) {
    ERROR("Error initiating SCH NR for Rx");
    goto clean_exit;
  }

  if (isrran_pusch_nr_set_carrier(&pusch_tx, &carrier)) {
    ERROR("Error setting SCH NR carrier");
    goto clean_exit;
  }

  if (isrran_pusch_nr_set_carrier(&pusch_rx, &carrier)) {
    ERROR("Error setting SCH NR carrier");
    goto clean_exit;
  }

  uint32_t slot_length = ISRRAN_SLOT_LEN_RE_NR(carrier.nof_prb);
  for (uint32_t i = 0; i < carrier.max_mimo_layers; i++) {
    sf_symbols_tx[i] = isrran_vec_cf_malloc(slot_length);
    sf_symbols_rx[i] = isrran_vec_cf_malloc(slot_length);
  ...
  for (uint32_t i = 0; i < pusch_tx.max_cw; i++) {
    data_tx.payload[i]    = isrran_vec_u8_malloc(ISRRAN_SLOT_MAX_NOF_BITS_NR);
    data_rx.tb[i].payload = isrran_vec_u8_malloc(ISRRAN_SLOT_MAX_NOF_BITS_NR);
  ...
  if (isrran_ra_ul_nr_pusch_time_resource_default_A(carrier.scs, 0, &pusch_cfg.grant) < ISRRAN_SUCCESS) {
  ...
  pusch_cfg.grant.dci_format                       = isrran_dci_format_nr_1_0;
  ...
  // Check input: MCS
  uint32_t mcs_end = pusch_cfg.sch_cfg.mcs_table == isrran_mcs_table_256qam ? 28 : 29;
  if (mcs > mcs_end) {
    ERROR("Invalid MCS");
    goto clean_exit;
  }

  isrran_sch_hl_cfg_nr_t sch_hl_cfg = {};
  ...
  pusch_cfg.grant.dci_format                       = isrran_dci_format_nr_0_0;
  pusch_cfg.grant.nof_dmrs_cdm_groups_without_data = 2;
  pusch_cfg.dmrs.type                              = isrran_dmrs_sch_type_1;
  pusch_cfg.dmrs.length                            = isrran_dmrs_sch_len_1;
  pusch_cfg.dmrs.additional_pos                    = isrran_dmrs_sch_add_pos_2;
  if (isrran_ra_nr_fill_tb(&pusch_cfg, &pusch_cfg.grant, mcs, &pusch_cfg.grant.tb[0]) < ISRRAN_SUCCESS) {
  ...
  for (; n_blocks < max_blocks && n_errors < 100; n_blocks++) {
    ...
    for (uint32_t tb = 0; tb < ISRRAN_MAX_TB; tb++) {
      ...
      for (uint32_t i = 0; i < pusch_cfg.grant.tb[tb].tbs / 8 + 1; i++) {
        data_tx.payload[tb][i] = (uint8_t)isrran_random_uniform_int_dist(rand_gen, 0, UINT8_MAX);
      }
    ...
    if (nof_ack_bits > 0) {
      ...
      for (uint32_t i = 0; i < nof_ack_bits; i++) {
        data_tx.uci.ack[i] = (uint8_t)isrran_random_uniform_int_dist(rand_gen, 0, 1);
      }
    }

    // Generate CSI report bits
    uint8_t csi_report_tx[ISRRAN_UCI_NR_MAX_CSI1_BITS] = {};
    uint8_t csi_report_rx[ISRRAN_UCI_NR_MAX_CSI1_BITS] = {};
    if (nof_csi_bits > 0) {
      pusch_cfg.uci.csi[0].cfg.quantity = ISRRAN_CSI_REPORT_QUANTITY_NONE;
      ...
      for (uint32_t i = 0; i < nof_csi_bits; i++) {
        csi_report_tx[i] = (uint8_t)isrran_random_uniform_int_dist(rand_gen, 0, 1);
      }

      data_rx.uci.csi[0].none = csi_report_rx;
    }

    if (isrran_ra_ul_set_grant_uci_nr(&carrier, &sch_hl_cfg, &pusch_cfg.uci, &pusch_cfg) < ISRRAN_SUCCESS) {
      ERROR("Setting UCI");
      goto clean_exit;
    }

    if (isrran_pusch_nr_encode(&pusch_tx, &pusch_cfg, &pusch_cfg.grant, &data_tx, sf_symbols_tx) < ISRRAN_SUCCESS) {
    ...
    for (uint32_t i = 0; i < carrier.max_mimo_layers; i++) {
      isrran_ch_awgn_c(sf_symbols_tx[i], sf_symbols_rx[i], noise_var, slot_length);
      // memcpy(sf_symbols_rx[i], sf_symbols_tx[i], slot_length * sizeof(cf_t));
    }

    if (get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO) {
      uint32_t nof_re_total = carrier.nof_prb * ISRRAN_NRE;
      uint32_t nof_re_used  = pusch_cfg.grant.nof_prb * ISRRAN_NRE;
      for (int i_layer = 0; i_layer < carrier.max_mimo_layers; i_layer++) {
        INFO("Layer %d", i_layer);
        float   tx_power  = 0;
        float   rx_power  = 0;
        uint8_t n_symbols = 0;
        for (int i = 0; i < ISRRAN_NSYMB_PER_SLOT_NR; i++) {
          if (!pusch_tx.dmrs_re_pattern.symbol[i]) {
            n_symbols++;
            tx_power += isrran_vec_avg_power_cf(sf_symbols_tx[0] + i * nof_re_total, nof_re_total);
            rx_power += isrran_vec_avg_power_cf(sf_symbols_rx[0] + i * nof_re_total, nof_re_total);
          }
        }
        ...
        INFO("    SNR: %.3f dB", isrran_convert_power_to_dB(tx_power / (rx_power - tx_power)));
      }
    }

    for (uint32_t tb = 0; tb < ISRRAN_MAX_TB; tb++) {
      pusch_cfg.grant.tb[tb].softbuffer.rx = &softbuffer_rx;
      isrran_softbuffer_rx_reset(pusch_cfg.grant.tb[tb].softbuffer.rx);
    }
    ...
    if (isrran_pusch_nr_decode(&pusch_rx, &pusch_cfg, &pusch_cfg.grant, &chest, sf_symbols_rx, &data_rx) <
        ISRRAN_SUCCESS) {
  ...
  char str[512];
  isrran_pusch_nr_rx_info(&pusch_rx, &pusch_cfg, &pusch_cfg.grant, &data_rx, str, (uint32_t)sizeof(str));

  char str_extra[2048];
  isrran_sch_cfg_nr_info(&pusch_cfg, str_extra, (uint32_t)sizeof(str_extra));
  printf("\nPUSCH: %s\n%s", str, str_extra);
...
clean_exit:
  ...
  isrran_pusch_nr_free(&pusch_tx);
  isrran_pusch_nr_free(&pusch_rx);
