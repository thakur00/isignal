...
ISRRAN_API int indices[100000];
ISRRAN_API int indices_ptr = 0;
#endif
...
void prb_cp_ref(cf_t** input, cf_t** output, int offset, int nof_refs, int nof_intervals, bool advance_output)
{
  ...
  int ref_interval = ((ISRRAN_NRE / nof_refs) - 1);
...
{
  memcpy(*output, *input, sizeof(cf_t) * ISRRAN_NRE * nof_prb);
  print_indexes(*input, ISRRAN_NRE);
  *input += nof_prb * ISRRAN_NRE;
  *output += nof_prb * ISRRAN_NRE;
}

void prb_cp_half(cf_t** input, cf_t** output, int nof_prb)
{
  memcpy(*output, *input, sizeof(cf_t) * ISRRAN_NRE * nof_prb / 2);
  print_indexes(*input, ISRRAN_NRE / 2);
  *input += nof_prb * ISRRAN_NRE / 2;
  *output += nof_prb * ISRRAN_NRE / 2;
}
