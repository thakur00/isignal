...
static int harq_ack_gen_ack_type2(const isrran_harq_ack_cfg_hl_t* cfg,
                                  ...
                                  isrran_uci_cfg_nr_t*            uci_cfg)
{
  ...
  // Initialise ACK bits
  ISRRAN_MEM_ZERO(uci_cfg->ack.bits, isrran_harq_ack_bit_t, ISRRAN_HARQ_ACK_MAX_NOF_BITS);

  // The following code follows the exact pseudo-code provided in TS 38.213 9.1.3.1 Type-2 HARQ-ACK codebook ...
  while (m < ISRRAN_UCI_NR_MAX_M) {
    ...
    while (c < N_DL_cells) {
      ...
      const isrran_harq_ack_m_t* ack = &ack_info->cc[c].m[m];
      ...
      // Get ACK values
      isrran_harq_ack_bit_t ack_tb0 = {};
      ack_tb0.tb0                   = true;
      ack_tb0.cc_idx                = c;
      ack_tb0.m_idx                 = m;
      ack_tb0.pid                   = ack->resource.pid;
      isrran_harq_ack_bit_t ack_tb1 = {};
      ack_tb1.tb1                   = true;
      ack_tb1.cc_idx                = c;
      ack_tb1.m_idx                 = m;
...
static int harq_ack_k1(const isrran_harq_ack_cfg_hl_t* cfg, const isrran_dci_dl_nr_t* dci_dl)
{
  ...
  if (dci_dl->harq_feedback >= ISRRAN_MAX_NOF_DL_DATA_TO_UL || dci_dl->harq_feedback >= cfg->nof_dl_data_to_ul_ack) {
...
int isrran_harq_ack_resource(const isrran_harq_ack_cfg_hl_t* cfg,
                             ...
                             isrran_harq_ack_resource_t*     pdsch_ack_resource)
{
  ...
  // Fill PDSCH resource
  pdsch_ack_resource->dci_format_1_1    = (dci_dl->ctx.format == isrran_dci_format_nr_1_1);
...
int isrran_harq_ack_gen_uci_cfg(const isrran_harq_ack_cfg_hl_t* cfg,
                                const isrran_pdsch_ack_nr_t*    ack_info,
                                isrran_uci_cfg_nr_t*            uci_cfg)
{
  // Check inputs
  ...
  if (cfg->harq_ack_codebook == isrran_pdsch_harq_ack_codebook_semi_static) {
  ...
  if (cfg->harq_ack_codebook == isrran_pdsch_harq_ack_codebook_dynamic) {
...
int isrran_harq_ack_pack(const isrran_harq_ack_cfg_hl_t* cfg,
                         const isrran_pdsch_ack_nr_t*    ack_info,
                         isrran_uci_data_nr_t*           uci_data)
{
  ...
  if (isrran_harq_ack_gen_uci_cfg(cfg, ack_info, &uci_data->cfg) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  // Actual packing
  for (uint32_t i = 0; i < uci_data->cfg.ack.count; i++) {
    isrran_harq_ack_bit_t* ack_bit = &uci_data->cfg.ack.bits[i];
...
int isrran_harq_ack_unpack(const isrran_harq_ack_cfg_hl_t* cfg,
                           ...
                           isrran_pdsch_ack_nr_t*          ack_info)
{
  ...
  // Actual packing
  for (uint32_t i = 0; i < uci_data->cfg.ack.count; i++) {
    const isrran_harq_ack_bit_t* ack_bit = &uci_data->cfg.ack.bits[i];
...
int isrran_harq_ack_insert_m(isrran_pdsch_ack_nr_t* ack_info, const isrran_harq_ack_m_t* m)
{
  // Check inputs
  ...
  if (m->resource.scell_idx >= ISRRAN_MAX_CARRIERS) {
    ERROR("Serving cell index (%d) exceeds maximum", m->resource.scell_idx);
    return ISRRAN_ERROR;
  }
  isrran_harq_ack_cc_t* cc = &ack_info->cc[m->resource.scell_idx];

  if (cc->M >= ISRRAN_UCI_NR_MAX_M) {
    ERROR("Accumulated M HARQ feedback exceeds maximum (%d)", ISRRAN_UCI_NR_MAX_M);
    return ISRRAN_ERROR;
  }
...
uint32_t isrran_harq_ack_info(const isrran_pdsch_ack_nr_t* ack_info, char* str, uint32_t str_len)
{
  uint32_t len = 0;
  ...
  for (uint32_t cc = 0; cc < ack_info->nof_cc; cc++) {
    len = isrran_print_check(str, str_len, len, "  CC %d: M=%d\n", cc, ack_info->cc[cc].M);
