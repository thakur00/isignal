...
uint32_t ra_re_x_prb(const isrran_cell_t* cell, isrran_dl_sf_cfg_t* sf, uint32_t slot, uint32_t prb_idx)
{
  uint32_t subframe         = sf->tti % 10;
  uint32_t nof_ctrl_symbols = ISRRAN_NOF_CTRL_SYMBOLS((*cell), sf->cfi);

  uint32_t    re;
  bool        skip_refs = true;
  isrran_cp_t cp_       = cell->cp;
  if (ISRRAN_SF_MBSFN == sf->sf_type) {
    cp_ = ISRRAN_CP_EXT;
  }

  uint32_t nof_symbols = ISRRAN_CP_NSYMB(cp_);
  if (cell->frame_type == ISRRAN_TDD && isrran_sfidx_tdd_type(sf->tdd_config, subframe) == ISRRAN_TDD_SF_S) {
    nof_symbols = isrran_sfidx_tdd_nof_dw_slot(sf->tdd_config, slot, cp_);
  }

  if (slot == 0) {
    re = (nof_symbols - nof_ctrl_symbols) * ISRRAN_NRE;
  } else {
    re = nof_symbols * ISRRAN_NRE;
  }
  ...
  if (cell->frame_type == ISRRAN_FDD) {
    if ((subframe == 0 || subframe == 5) &&
      ...
      if (subframe == 0) {
        if (slot == 0) {
          re = (nof_symbols - nof_ctrl_symbols - 2) * ISRRAN_NRE;
        } else {
          if (ISRRAN_CP_ISEXT(cp_)) {
            re        = (nof_symbols - 4) * ISRRAN_NRE;
            skip_refs = false;
          } else {
            re = (nof_symbols - 4) * ISRRAN_NRE + 2 * cell->nof_ports;
          }
        }
      } else if (subframe == 5) {
        if (slot == 0) {
      ...
      if ((cell->nof_prb % 2) && (prb_idx == cell->nof_prb / 2 - 3 || prb_idx == cell->nof_prb / 2 + 3)) {
        if (slot == 0) {
          ...
          re += 4 * ISRRAN_NRE / 2 - cell->nof_ports;
          if (ISRRAN_CP_ISEXT(cp_)) {
    ...
    if ((((subframe == 0 || subframe == 5) && slot == 1) || ((subframe == 1 || subframe == 6) && slot == 0)) &&
      ...
      if (subframe == 0) {
        if (ISRRAN_CP_ISEXT(cp_)) {
          re        = (nof_symbols - 5) * ISRRAN_NRE;
          skip_refs = false;
        } else {
          re = (nof_symbols - 5) * ISRRAN_NRE + 2 * cell->nof_ports;
        }
      } else if (subframe == 5) {
        re = (nof_symbols - 1) * ISRRAN_NRE;
      } else if (subframe == 1) {
        re = (nof_symbols - nof_ctrl_symbols - 1) * ISRRAN_NRE;
      } else if (subframe == 6) {
        re = (nof_symbols - nof_ctrl_symbols - 1) * ISRRAN_NRE;
      }
      if ((cell->nof_prb % 2) && (prb_idx == cell->nof_prb / 2 - 3 || prb_idx == cell->nof_prb / 2 + 3)) {
  ...
  if (skip_refs) {
    if (sf->sf_type == ISRRAN_SF_NORM) {
      switch (cell->nof_ports) {
        case 1:
        case 2:
          if ((cp_ == ISRRAN_CP_NORM && nof_symbols >= 5) || (cp_ == ISRRAN_CP_EXT && nof_symbols >= 4)) {
 ...
 */
/** Compute PRB allocation for Downlink as defined in 7.1.6 of 36.213 */
int isrran_ra_dl_grant_to_grant_prb_allocation(const isrran_dci_dl_t* dci,
                                               ...
                                               uint32_t               nof_prb)
{
  ...
  switch (dci->alloc_type) {
    case ISRRAN_RA_ALLOC_TYPE0:
      ...
      memcpy(&grant->prb_idx[1], &grant->prb_idx[0], ISRRAN_MAX_PRB * sizeof(bool));
      break;
    case ISRRAN_RA_ALLOC_TYPE1:
      ...
      n_rb_type1    = isrran_ra_type1_N_rb(nof_prb);
    ...
    case ISRRAN_RA_ALLOC_TYPE2:
      ...
      if (dci->type2_alloc.mode == ISRRAN_RA_TYPE2_LOC) {
        nof_vrb = nof_prb;
      } else {
        nof_vrb = isrran_ra_type2_n_vrb_dl(nof_prb, dci->type2_alloc.n_gap == ISRRAN_RA_TYPE2_NG1);
      }
      if (dci->format == ISRRAN_DCI_FORMAT1C) {
        n_step = isrran_ra_type2_n_rb_step(nof_prb);
        nof_vrb /= n_step;
        nof_prb_t2 = nof_vrb;
      } else {
        nof_prb_t2 = nof_prb;
      }
      isrran_ra_type2_from_riv(dci->type2_alloc.riv, &L_crb, &RB_start, nof_prb_t2, nof_vrb);
      ...
      if (dci->type2_alloc.mode == ISRRAN_RA_TYPE2_LOC) {
        ...
        if (dci->type2_alloc.n_gap == ISRRAN_RA_TYPE2_NG1) {
          N_tilde_vrb = isrran_ra_type2_n_vrb_dl(nof_prb, true);
          N_gap       = isrran_ra_type2_ngap(nof_prb, true);
        } else {
          N_tilde_vrb = 2 * isrran_ra_type2_n_vrb_dl(nof_prb, true);
          N_gap       = isrran_ra_type2_ngap(nof_prb, false);
        }
...
int isrran_dl_fill_ra_mcs(isrran_ra_tb_t* tb, int last_tbs, uint32_t nprb, bool pdsch_use_tbs_index_alt)
{
  // Get modulation
  tb->mod = isrran_ra_dl_mod_from_mcs(tb->mcs_idx, pdsch_use_tbs_index_alt);

  // Get Transport block size index
  int i_tbs = isrran_ra_tbs_idx_from_mcs(tb->mcs_idx, pdsch_use_tbs_index_alt, false);

  // If i_tbs = -1, TBS is determined from the latest PDCCH for this TB (7.1.7.2 36.213)
  int tbs = 0;
  if (i_tbs >= 0) {
    tbs     = isrran_ra_tbs_from_idx((uint32_t)i_tbs, nprb);
    tb->tbs = tbs;
  } else {
    tb->tbs = last_tbs;
  }
 ...
 * */
static int dl_dci_compute_tb(bool pdsch_use_tbs_index_alt, const isrran_dci_dl_t* dci, isrran_pdsch_grant_t* grant)
{
  uint32_t n_prb = 0;
  int      tbs   = -1;
  uint32_t i_tbs = 0;
  ...
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    ...
    if ((ISRRAN_DCI_IS_TB_EN(dci->tb[i]) && dci->format >= ISRRAN_DCI_FORMAT2) ||
        (dci->format < ISRRAN_DCI_FORMAT2 && i == 0)) {
  ...
  if (dci->format == ISRRAN_DCI_FORMAT1A || !ISRRAN_RNTI_ISUSER(dci->rnti)) {
    pdsch_use_tbs_index_alt = false;
  }

  if (!ISRRAN_RNTI_ISUSER(dci->rnti) && !ISRRAN_RNTI_ISMBSFN(dci->rnti)) {
    if (dci->format == ISRRAN_DCI_FORMAT1A) {
      n_prb = dci->type2_alloc.n_prb1a == ISRRAN_RA_TYPE2_NPRB1A_2 ? 2 : 3;
      i_tbs = dci->tb[0].mcs_idx;
      tbs   = isrran_ra_tbs_from_idx(i_tbs, n_prb);
      if (tbs < 0) {
        ERROR("Invalid TBS_index=%d or n_prb=%d", i_tbs, n_prb);
        return ISRRAN_ERROR;
      }
    } else if (dci->format == ISRRAN_DCI_FORMAT1C) {
    ...
    grant->tb[0].mod = ISRRAN_MOD_QPSK;
    if (tbs >= 0) {
    ...
    if (dci->is_dwpts) {
      n_prb = ISRRAN_MAX(1, 0.75 * grant->nof_prb);
    } else {
      n_prb = grant->nof_prb;
    }
    for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
      if (grant->tb[i].enabled) {
        grant->tb[i].tbs = isrran_dl_fill_ra_mcs(&grant->tb[i], grant->last_tbs[i], n_prb, pdsch_use_tbs_index_alt);
        if (grant->tb[i].tbs < 0) {
          char str[128];
          isrran_dci_dl_info(dci, str, sizeof(str));
...
void isrran_ra_dl_compute_nof_re(const isrran_cell_t* cell, isrran_dl_sf_cfg_t* sf, isrran_pdsch_grant_t* grant)
{
  // Compute number of RE
  grant->nof_re   = isrran_ra_dl_grant_nof_re(cell, sf, grant);
  isrran_cp_t cp_ = ISRRAN_SF_NORM == sf->sf_type ? cell->cp : ISRRAN_CP_EXT;
  if (cell->frame_type == ISRRAN_FDD) {
    grant->nof_symb_slot[0] = ISRRAN_CP_NSYMB(cp_);
    grant->nof_symb_slot[1] = ISRRAN_CP_NSYMB(cp_);
  } else {
    if (isrran_sfidx_tdd_type(sf->tdd_config, sf->tti % 10) == ISRRAN_TDD_SF_S) {
      grant->nof_symb_slot[0] = isrran_sfidx_tdd_nof_dw_slot(sf->tdd_config, 0, cp_);
      grant->nof_symb_slot[1] = isrran_sfidx_tdd_nof_dw_slot(sf->tdd_config, 1, cp_);
    } else {
      grant->nof_symb_slot[0] = ISRRAN_CP_NSYMB(cp_);
      grant->nof_symb_slot[1] = ISRRAN_CP_NSYMB(cp_);
    }
  ...
  for (int i = 0; i < ISRRAN_MAX_TB; i++) {
    ...
    if (grant->tb[i].enabled) {
      grant->tb[i].nof_bits = grant->nof_re * isrran_mod_bits_x_symbol(grant->tb[i].mod);
    }
  }
}
...
static int
config_mimo_type(const isrran_cell_t* cell, isrran_tm_t tm, const isrran_dci_dl_t* dci, isrran_pdsch_grant_t* grant)
{
  grant->tx_scheme  = ISRRAN_TXSCHEME_PORT0;
  ...
  switch (tm) {
    ...
    case ISRRAN_TM2:
      if (cell->nof_ports > 1) {
        grant->tx_scheme = ISRRAN_TXSCHEME_DIVERSITY;
      } else {
        grant->tx_scheme = ISRRAN_TXSCHEME_PORT0;
      }
      if (nof_tb != 1) {
        ERROR("Wrong number of transport blocks (%d) for %s.", nof_tb, isrran_mimotype2str(grant->tx_scheme));
        valid_config = false;
      }
      break;
    case ISRRAN_TM3:
      if (nof_tb == 1) {
        ...
        grant->tx_scheme = ISRRAN_TXSCHEME_CDD;
      } else {
    ...
    case ISRRAN_TM4:
      if (nof_tb == 1) {
        grant->tx_scheme = (dci->pinfo == 0) ? ISRRAN_TXSCHEME_DIVERSITY : ISRRAN_TXSCHEME_SPATIALMUX;
      } else if (nof_tb == 2) {
        grant->tx_scheme = ISRRAN_TXSCHEME_SPATIALMUX;
      } else {
  ...
  return valid_config ? ISRRAN_SUCCESS : ISRRAN_ERROR;
}

/* Translates Precoding Information (pinfo) to Precoding matrix Index (pmi) as 3GPP 36.212 Table 5.3.3.1.5-4 */
static int config_mimo_pmi(const isrran_cell_t* cell, const isrran_dci_dl_t* dci, isrran_pdsch_grant_t* grant)
{
  uint32_t nof_tb = grant->nof_tb;
  if (grant->tx_scheme == ISRRAN_TXSCHEME_SPATIALMUX) {
    if (nof_tb == 1) {
      ...
      if (dci->pinfo == 2) {
        ERROR("Not implemented codebook index (nof_tb=%d (%d/%d), pinfo=%d)",
              nof_tb,
              ISRRAN_DCI_IS_TB_EN(grant->tb[0]),
              ISRRAN_DCI_IS_TB_EN(grant->tb[1]),
              dci->pinfo);
        return -1;
      } else if (dci->pinfo > 2) {
...
static int config_mimo_layers(const isrran_cell_t* cell, const isrran_dci_dl_t* dci, isrran_pdsch_grant_t* grant)
{
  uint32_t nof_tb = grant->nof_tb;
...
static int
config_mimo(const isrran_cell_t* cell, isrran_tm_t tm, const isrran_dci_dl_t* dci, isrran_pdsch_grant_t* grant)
{
...
int isrran_ra_dl_dci_to_grant(const isrran_cell_t*   cell,
                              ...
                              isrran_pdsch_grant_t*  grant)
{
  bzero(grant, sizeof(isrran_pdsch_grant_t));

  // Compute PRB allocation
  int ret = isrran_ra_dl_grant_to_grant_prb_allocation(dci, grant, cell->nof_prb);
  if (ret == ISRRAN_SUCCESS) {
    ...
    if (ret == ISRRAN_SUCCESS) {
      ...
      isrran_ra_dl_compute_nof_re(cell, sf, grant);

      // Apply Section 7.1.7.3. If RA-RNTI and Format1C rv_idx=0
      if (dci->format == ISRRAN_DCI_FORMAT1C) {
        if ((ISRRAN_RNTI_ISRAR(dci->rnti)) || dci->rnti == ISRRAN_PRNTI) {
          for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
            grant->tb[i].rv = 0;
          }
        }
...
uint32_t isrran_ra_dl_approx_nof_re(const isrran_cell_t* cell, uint32_t nof_prb, uint32_t nof_ctrl_symbols)
{
  uint32_t nof_refs = 0;
  uint32_t nof_symb = 2 * ISRRAN_CP_NSYMB(cell->cp) - nof_ctrl_symbols;
  switch (cell->nof_ports) {
    case 1:
      nof_refs = 2 * 3;
      break;
  ...
  return nof_prb * (nof_symb * ISRRAN_NRE - nof_refs);
}

/* Computes the number of RE for each PRB in the prb_dist structure */
uint32_t isrran_ra_dl_grant_nof_re(const isrran_cell_t* cell, isrran_dl_sf_cfg_t* sf, isrran_pdsch_grant_t* grant)
{
  uint32_t j, s;
...
static uint32_t print_multi(char* info_str, uint32_t n, uint32_t len, isrran_pdsch_grant_t* grant, uint32_t value_id)
{
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    if (grant->tb[i].enabled) {
      switch (value_id) {
        case 0:
          n = isrran_print_check(info_str, len, n, "%d", grant->tb[i].rv);
          break;
        case 1:
          n = isrran_print_check(info_str, len, n, "%d", grant->tb[i].tbs / 8);
          break;
        case 2:
          n = isrran_print_check(info_str, len, n, "%d", isrran_mod_bits_x_symbol(grant->tb[i].mod));
          break;
      }
...
uint32_t isrran_ra_dl_info(isrran_pdsch_grant_t* grant, char* info_str, uint32_t len)
{
  int n = 0;

  n = isrran_print_check(info_str, len, n, ", nof_prb=%d, nof_re=%d", grant->nof_prb, grant->nof_re);

  n = isrran_print_check(info_str, len, n, ", tbs={");
  n = print_multi(info_str, n, len, grant, 1);
  n = isrran_print_check(info_str, len, n, "}");
  n = isrran_print_check(info_str, len, n, ", mod={");
  n = print_multi(info_str, n, len, grant, 2);
  n = isrran_print_check(info_str, len, n, "}");
  n = isrran_print_check(info_str, len, n, ", rv={");
  n = print_multi(info_str, n, len, grant, 0);
  n = isrran_print_check(info_str, len, n, "}");

  if (grant->tx_scheme != ISRRAN_TXSCHEME_PORT0) {
    n = isrran_print_check(info_str,
                           len,
                           n,
                           ", tx=%s, nof_tb=%d, nof_l=%d",
                           isrran_mimotype2str(grant->tx_scheme),
                           grant->nof_tb,
                           grant->nof_layers);
    if (grant->tx_scheme == ISRRAN_TXSCHEME_SPATIALMUX) {
      n = isrran_print_check(info_str, len, n, ", pmi=%d", grant->pmi);
    }
  }
  return n;
}
