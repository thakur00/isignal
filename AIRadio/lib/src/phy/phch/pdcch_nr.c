 ...
 */
static uint32_t isrran_pdcch_calculate_Y_p_n(uint32_t coreset_id, uint16_t rnti, uint32_t n)
{
 ...
 *
 */
static int isrran_pdcch_nr_get_ncce(const isrran_coreset_t*      coreset,
                                    ...
                                    uint32_t                     candidate)
{
  if (aggregation_level >= ISRRAN_SEARCH_SPACE_NOF_AGGREGATION_LEVELS_NR) {
  ...
  uint32_t Y_p_n = 0;
  if (search_space->type == isrran_search_space_type_ue) {
    Y_p_n = isrran_pdcch_calculate_Y_p_n(coreset->id, rnti, slot_idx);
  }
                                      ...
                                      uint32_t                     locations[ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR])
{
  ...
  nof_candidates = ISRRAN_MIN(nof_candidates, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR);

  for (uint32_t candidate = 0; candidate < nof_candidates; candidate++) {
    int ret = isrran_pdcch_nr_get_ncce(coreset, search_space, rnti, aggregation_level, slot_idx, candidate);
...
int isrran_pdcch_nr_max_candidates_coreset(const isrran_coreset_t* coreset, uint32_t aggregation_level)
{
  if (coreset == NULL) {
    return ISRRAN_ERROR;
  }
  ...
  return ISRRAN_MIN(nof_candidates, ISRRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR);
}

static int pdcch_nr_init_common(isrran_pdcch_nr_t* q, const isrran_pdcch_nr_args_t* args)
{
  if (q == NULL || args == NULL) {
    return ISRRAN_ERROR_INVALID_INPUTS;
  }
  ...
  q->c = isrran_vec_u8_malloc(ISRRAN_PDCCH_MAX_RE * 2);
  if (q->c == NULL) {
    return ISRRAN_ERROR;
  }

  q->d = isrran_vec_u8_malloc(ISRRAN_PDCCH_MAX_RE * 2);
  if (q->d == NULL) {
    return ISRRAN_ERROR;
  }

  q->f = isrran_vec_u8_malloc(ISRRAN_PDCCH_MAX_RE * 2);
  if (q->f == NULL) {
    return ISRRAN_ERROR;
  }

  q->symbols = isrran_vec_cf_malloc(ISRRAN_PDCCH_MAX_RE);
  if (q->symbols == NULL) {
    return ISRRAN_ERROR;
  }

  q->allocated = isrran_vec_u8_malloc(NMAX);
  if (q->allocated == NULL) {
    return ISRRAN_ERROR;
  }

  if (isrran_crc_init(&q->crc24c, ISRRAN_LTE_CRC24C, 24) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  isrran_modem_table_lte(&q->modem_table, ISRRAN_MOD_QPSK);
  if (args->measure_evm) {
    isrran_modem_table_bytes(&q->modem_table);
  }

  return ISRRAN_SUCCESS;
}

int isrran_pdcch_nr_init_tx(isrran_pdcch_nr_t* q, const isrran_pdcch_nr_args_t* args)
{
  if (pdcch_nr_init_common(q, args) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  q->is_tx = true;
  ...
  if (isrran_polar_encoder_init(&q->encoder, encoder_type, NMAX_LOG) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_polar_rm_tx_init(&q->rm) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
...
int isrran_pdcch_nr_init_rx(isrran_pdcch_nr_t* q, const isrran_pdcch_nr_args_t* args)
{
  if (pdcch_nr_init_common(q, args) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  if (isrran_polar_decoder_init(&q->decoder, decoder_type, NMAX_LOG) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (isrran_polar_rm_rx_init_c(&q->rm) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (args->measure_evm) {
    q->evm_buffer = isrran_evm_buffer_alloc(ISRRAN_PDCCH_MAX_RE * 2);
  }

  return ISRRAN_SUCCESS;
}

void isrran_pdcch_nr_free(isrran_pdcch_nr_t* q)
{
  if (q == NULL) {
    return;
  }
  ...
  if (q->is_tx) {
    ...
    isrran_polar_rm_tx_free(&q->rm);
  } else {
    isrran_polar_decoder_free(&q->decoder);
    isrran_polar_rm_rx_free_c(&q->rm);
  }
  ...
  isrran_modem_table_free(&q->modem_table);

  if (q->evm_buffer) {
    isrran_evm_free(q->evm_buffer);
  }

  ISRRAN_MEM_ZERO(q, isrran_pdcch_nr_t, 1);
}

int isrran_pdcch_nr_set_carrier(isrran_pdcch_nr_t*         q,
...
static int pdcch_nr_cce_to_reg_mapping_non_interleaved(const isrran_coreset_t*      coreset,
...
static int pdcch_nr_cce_to_reg_mapping_interleaved(const isrran_coreset_t*      coreset,
                                                   const isrran_dci_location_t* dci_location,
                                                   bool                         rb_mask[ISRRAN_MAX_PRB_NR])
{
  // Calculate CORESET constants
  uint32_t N_CORESET_REG = coreset->duration * isrran_coreset_get_bw(coreset);
...
int isrran_pdcch_nr_cce_to_reg_mapping(const isrran_coreset_t*      coreset,
                                       const isrran_dci_location_t* dci_location,
                                       bool                         rb_mask[ISRRAN_MAX_PRB_NR])
{
  ...
  if (coreset->mapping_type == isrran_coreset_mapping_type_non_interleaved) {
...
static uint32_t pdcch_nr_cp(const isrran_pdcch_nr_t*     q,
                            const isrran_dci_location_t* dci_location,
                            cf_t*                        slot_grid,
                            cf_t*                        symbols,
                            bool                         put)
{
  ...
  for (uint32_t l = 0; l < q->coreset.duration; l++) {
    ...
    uint32_t rb = 0;
    for (uint32_t r = 0; r < ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE; r++) {
      ...
      for (uint32_t i = r * 6; i < (r + 1) * 6; i++, rb++) {
        ...
        for (uint32_t k = i * ISRRAN_NRE; k < (i + 1) * ISRRAN_NRE; k++) {
          ...
          if (put) {
            slot_grid[q->carrier.nof_prb * ISRRAN_NRE * l + k + offset_k] = symbols[count++];
          } else {
            symbols[count++] = slot_grid[q->carrier.nof_prb * ISRRAN_NRE * l + k + offset_k];
          }
        }
      }
    }
...
static uint32_t pdcch_nr_c_init(const isrran_pdcch_nr_t* q, const isrran_dci_msg_nr_t* dci_msg)
{
  uint32_t n_id   = (dci_msg->ctx.ss_type == isrran_search_space_type_ue && q->coreset.dmrs_scrambling_id_present)
                        ? q->coreset.dmrs_scrambling_id
                        : q->carrier.pci;
  uint32_t n_rnti = (dci_msg->ctx.ss_type == isrran_search_space_type_ue && q->coreset.dmrs_scrambling_id_present)
                        ? dci_msg->ctx.rnti
                        : 0U;
  return ((n_rnti << 16U) + n_id) & 0x7fffffffU;
}

int isrran_pdcch_nr_encode(isrran_pdcch_nr_t* q, const isrran_dci_msg_nr_t* dci_msg, cf_t* slot_symbols)
{
  ...
  q->M           = (1U << dci_msg->ctx.location.L) * (ISRRAN_NRE - 3U) * 6U; // Number of RE
  q->E           = q->M * 2;                                                 // Number of Rate-Matched bits
  uint32_t cinit = pdcch_nr_c_init(q, dci_msg);                              // Pseudo-random sequence initiation

  // Get polar code
  if (isrran_polar_code_get(&q->code, q->K, q->E, 9U) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  uint8_t* c = q->c;
  isrran_bit_unpack(UINT32_MAX, &c, 24U);

  // Copy DCI message
  isrran_vec_u8_copy(c, dci_msg->payload, dci_msg->nof_bits);

  // Append CRC
  isrran_crc_attach(&q->crc24c, q->c, q->K);

  PDCCH_INFO_TX("Append CRC %06x", (uint32_t)isrran_crc_checksum_get(&q->crc24c));

  // Unpack RNTI
  uint8_t  unpacked_rnti[16] = {};
  uint8_t* ptr               = unpacked_rnti;
  isrran_bit_unpack(dci_msg->ctx.rnti, &ptr, 16);

  // Scramble CRC with RNTI
  isrran_vec_xor_bbb(unpacked_rnti, &c[q->K - 16], &c[q->K - 16], 16);

  // Interleave
  uint8_t c_prime[ISRRAN_POLAR_INTERLEAVER_K_MAX_IL];
  isrran_polar_interleaver_run_u8(c, c_prime, q->K, true);
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    PDCCH_INFO_TX("c=");
    isrran_vec_fprint_hex(stdout, c, q->K);
    PDCCH_INFO_TX("c_prime=");
    isrran_vec_fprint_hex(stdout, c_prime, q->K);
  }

  // Allocate channel
  isrran_polar_chanalloc_tx(c_prime, q->allocated, q->code.N, q->code.K, q->code.nPC, q->code.K_set, q->code.PC_set);
  ...
  // Rate matching
  isrran_polar_rm_tx(&q->rm, q->d, q->f, q->code.n, q->E, q->K, PDCCH_NR_POLAR_RM_IBIL);

  // Scrambling
  isrran_sequence_apply_bit(q->f, q->f, q->E, cinit);

  // Modulation
  isrran_mod_modulate(&q->modem_table, q->f, q->symbols, q->E);
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    char str[128] = {};
    isrran_pdcch_nr_info(q, NULL, str, sizeof(str));
    PDCCH_INFO_TX("%s", str);
  }
...
int isrran_pdcch_nr_decode(isrran_pdcch_nr_t*      q,
                           cf_t*                   slot_symbols,
                           ...
                           isrran_pdcch_nr_res_t*  res)
{
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
    PDCCH_DEBUG_RX("ce=");
    isrran_vec_fprint_c(stdout, ce->ce, q->M);
  }

  // Equalise
  isrran_predecoding_single(q->symbols, ce->ce, q->symbols, NULL, q->M, 1.0f, ce->noise_var);

  // Print symbols if enabled
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
    PDCCH_DEBUG_RX("symbols=");
    isrran_vec_fprint_c(stdout, q->symbols, q->M);
  }

  // Demodulation
  int8_t* llr = (int8_t*)q->f;
  isrran_demod_soft_demodulate_b(ISRRAN_MOD_QPSK, q->symbols, llr, q->M);

  // Measure EVM if configured
  if (q->evm_buffer != NULL) {
    res->evm = isrran_evm_run_b(q->evm_buffer, &q->modem_table, q->symbols, llr, q->E);
  } else {
    res->evm = NAN;
  }
  ...
  // Descrambling
  isrran_sequence_apply_c(llr, llr, q->E, pdcch_nr_c_init(q, dci_msg));

  // Un-rate matching
  int8_t* d = (int8_t*)q->d;
  if (isrran_polar_rm_rx_c(&q->rm, llr, d, q->E, q->code.n, q->K, PDCCH_NR_POLAR_RM_IBIL) < ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_DEBUG && !is_handler_registered()) {
    PDCCH_DEBUG_RX("d=");
    isrran_vec_fprint_bs(stdout, d, q->K);
  }
  ...
  // De-allocate channel
  uint8_t c_prime[ISRRAN_POLAR_INTERLEAVER_K_MAX_IL];
  isrran_polar_chanalloc_rx(q->allocated, c_prime, q->code.K, q->code.nPC, q->code.K_set, q->code.PC_set);
  ...
  // De-interleave
  isrran_polar_interleaver_run_u8(c_prime, c, q->K, false);
  ...
  // Check CRC
  ptr                = &c[q->K - 24];
  uint32_t checksum1 = isrran_crc_checksum(&q->crc24c, q->c, q->K);
  uint32_t checksum2 = isrran_bit_pack(&ptr, 24);
  res->crc           = checksum1 == checksum2;

  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    PDCCH_INFO_RX("CRC={%06x, %06x}; msg=", checksum1, checksum2);
    isrran_vec_fprint_hex(stdout, c, dci_msg->nof_bits);
  }

  // Copy DCI message
  isrran_vec_u8_copy(dci_msg->payload, c, dci_msg->nof_bits);
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    char str[128] = {};
    isrran_pdcch_nr_info(q, res, str, sizeof(str));
    PDCCH_INFO_RX("%s", str);
  }
...
uint32_t isrran_pdcch_nr_info(const isrran_pdcch_nr_t* q, const isrran_pdcch_nr_res_t* res, char* str, uint32_t str_len)
{
  int len = 0;
  ...
  len = isrran_print_check(str, str_len, len, "K=%d,E=%d", q->K, q->E);

  if (res != NULL) {
    len = isrran_print_check(str, str_len, len, ",crc=%s", res->crc ? "OK" : "KO");

    if (q->evm_buffer && res) {
      len = isrran_print_check(str, str_len, len, ",evm=%.2f", res->evm);
    }
  }
