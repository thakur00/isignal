...
static const ue_ra_time_resource_t ue_ul_default_A_lut[ISRRAN_MAX_NOF_TIME_RA] = {
    {isrran_sch_mapping_type_A, 0, 0, 14},
    {isrran_sch_mapping_type_A, 0, 0, 12},
    {isrran_sch_mapping_type_A, 0, 0, 10},
    {isrran_sch_mapping_type_B, 0, 2, 10},
    {isrran_sch_mapping_type_B, 0, 4, 10},
    {isrran_sch_mapping_type_B, 0, 4, 8},
    {isrran_sch_mapping_type_B, 0, 4, 6},
    {isrran_sch_mapping_type_A, 1, 0, 14},
    {isrran_sch_mapping_type_A, 1, 0, 12},
    {isrran_sch_mapping_type_A, 1, 0, 10},
    {isrran_sch_mapping_type_A, 2, 0, 14},
    {isrran_sch_mapping_type_A, 2, 0, 12},
    {isrran_sch_mapping_type_A, 2, 0, 10},
    {isrran_sch_mapping_type_B, 0, 8, 6},
    {isrran_sch_mapping_type_A, 3, 0, 14},
    {isrran_sch_mapping_type_A, 3, 0, 10}};

int isrran_ra_ul_nr_pusch_time_resource_default_A(uint32_t scs_cfg, uint32_t m, isrran_sch_grant_nr_t* grant)
{
...
static void ra_ul_nr_time_hl(const isrran_sch_time_ra_t* hl_ra_cfg, isrran_sch_grant_nr_t* grant)
{
  // Compute S and L from SLIV from higher layers
  isrran_sliv_to_s_and_l(ISRRAN_NSYMB_PER_SLOT_NR, hl_ra_cfg->sliv, &grant->S, &grant->L);
...
bool isrran_ra_ul_nr_time_validate(isrran_sch_grant_nr_t* grant)
{
...
int isrran_ra_ul_nr_time(const isrran_sch_hl_cfg_nr_t*    cfg,
                         ...
                         isrran_sch_grant_nr_t*           grant)
{
  ...
  if (ss_type == isrran_search_space_type_rar) {
    // Row 1
    if (cfg->nof_common_time_ra == 0) {
      isrran_ra_ul_nr_pusch_time_resource_default_A(cfg->scs_cfg, m, grant);
      ...
      ERROR("Time domain resource selection (m=%d) exceeds the maximum value (%d)",
            m,
            ISRRAN_MIN(cfg->nof_common_time_ra, ISRRAN_MAX_NOF_TIME_RA));
    }
  } else if ((rnti_type == isrran_rnti_type_c || rnti_type == isrran_rnti_type_mcs_c ||
              rnti_type == isrran_rnti_type_tc || rnti_type == isrran_rnti_type_cs) &&
             ISRRAN_SEARCH_SPACE_IS_COMMON(ss_type) && coreset_id == 0) {
    // Row 2
    if (cfg->nof_common_time_ra == 0) {
      isrran_ra_ul_nr_pusch_time_resource_default_A(cfg->scs_cfg, m, grant);
    } else {
      ra_ul_nr_time_hl(&cfg->common_time_ra[m], grant);
             ...
             ((ISRRAN_SEARCH_SPACE_IS_COMMON(ss_type) && coreset_id != 0) || ss_type == isrran_search_space_type_ue)) {
    // Row 3
  ...
  if (!isrran_ra_ul_nr_time_validate(grant)) {
...
int isrran_ra_ul_nr_nof_front_load_symbols(const isrran_sch_hl_cfg_nr_t* cfg,
                                           ...
                                           isrran_dmrs_sch_len_t*        dmrs_duration)
{
  ...
  if (cfg->dmrs_max_length == isrran_dmrs_sch_len_1) {
    *dmrs_duration = isrran_dmrs_sch_len_1;
    return ISRRAN_SUCCESS;
  }
...
int isrran_ra_ul_nr_nof_dmrs_cdm_groups_without_data(const isrran_sch_hl_cfg_nr_t* cfg,
                                                     const isrran_dci_ul_nr_t*     dci,
                                                     uint32_t                      L)
{
  ...
  switch (dci->ctx.format) {
    ...
    case isrran_dci_format_nr_rar:
      ...
      if (L <= 2 && !cfg->enable_transform_precoder) {
        return 1;
      } else if (L > 2 && cfg->dmrs_type == isrran_dmrs_sch_type_2 && dci->ctx.format == isrran_dci_format_nr_cg) {
        return 3;
      } else {
        return 2;
      }
    ...
    case isrran_dci_format_nr_0_1:
      ...
      // Table 7.3.1.1.2-6: Antenna port(s), transform precoder is enabled, dmrs-Type=1, maxLength=1
      if (cfg->enable_transform_precoder && cfg->dmrs_type == isrran_dmrs_sch_type_1 &&
          cfg->dmrs_max_length == isrran_dmrs_sch_len_1 && rank == 1) {
        return 2;
      }
      ...
      return ISRRAN_ERROR;
    default:
      ERROR("Invalid UL DCI format %s", isrran_dci_format_nr_string(dci->ctx.format));
  }
...
static double ra_ul_nr_pucch_code_rate_r(const isrran_pucch_nr_resource_t* resource)
{
...
// static int ra_ul_nr_pucch_Mrb(const isrran_pucch_nr_resource_t* resource)
//{
...
//      return ISRRAN_PUCCH_NR_FORMAT4_NPRB;
//    default:
//      ERROR("Invalid case");
//      break;
//  }
...
static int ra_ul_nr_pucch_nre(const isrran_pucch_nr_resource_t* resource)
{
  switch (resource->format) {
    case ISRRAN_PUCCH_NR_FORMAT_2:
      return ISRRAN_NRE - 4;
    case ISRRAN_PUCCH_NR_FORMAT_3:
      return ISRRAN_NRE;
    case ISRRAN_PUCCH_NR_FORMAT_4:
      return ISRRAN_NRE / resource->occ_lenth;
    default:
      ERROR("Invalid case");
      break;
  }
...
static int ra_ul_nr_pucch_nsymb(const isrran_pucch_nr_resource_t* resource)
{
  switch (resource->format) {
    case ISRRAN_PUCCH_NR_FORMAT_2:
    ...
    case ISRRAN_PUCCH_NR_FORMAT_4: {
      ...
      int nsymb_dmrs = isrran_dmrs_pucch_format_3_4_get_symbol_idx(resource, idx);
      if (nsymb_dmrs < ISRRAN_SUCCESS) {
        return ISRRAN_ERROR;
      }
...
// Calculate number of PUCCH symbols excluding the ones used exclusively for DMRS for formats 3 and 4
static int ra_ul_nr_pucch_qm(const isrran_pucch_nr_resource_t* resource)
{
  switch (resource->format) {
    case ISRRAN_PUCCH_NR_FORMAT_2:
      return 2;
...
int isrran_ra_ul_nr_pucch_format_2_3_min_prb(const isrran_pucch_nr_resource_t* resource,
                                             const isrran_uci_cfg_nr_t*        uci_cfg)
{
  ...
  uint32_t O_total = uci_cfg->ack.count + uci_cfg->o_sr + isrran_csi_part1_nof_bits(uci_cfg->csi, uci_cfg->nof_csi);

  // Add CRC bits if any
  O_total += isrran_uci_nr_crc_len(O_total);

  // Return the minimum
  return (int)ceil(O_total / nof_bits_rb);
}

int isrran_ra_ul_nr_freq(const isrran_carrier_nr_t*    carrier,
                         ...
                         isrran_sch_grant_nr_t*        grant)
{
  ...
  if (dci_ul->ctx.format == isrran_dci_format_nr_rar) {
  ...
  if (dci_ul->ctx.format == isrran_dci_format_nr_0_0) {
    return ra_helper_freq_type1(carrier->nof_prb, 0, dci_ul->freq_domain_assigment, grant);
  }
  ...
  if (cfg->alloc == isrran_resource_alloc_dynamic) {
  ...
  // resourceAllocation.
  if (cfg->alloc == isrran_resource_alloc_type1) {
    return ra_helper_freq_type1(carrier->nof_prb, 0, dci_ul->freq_domain_assigment, grant);
  }

  if (cfg->alloc == isrran_resource_alloc_type0) {
...
typedef struct {
  isrran_pucch_nr_format_t format;
...
static pucch_res_common_cfg_t pucch_res_common_cfg[PUCCH_RES_COMMON_CFG_IDX_MAX + 1] = {
    {ISRRAN_PUCCH_NR_FORMAT_0, 12, 0, 2},
    {ISRRAN_PUCCH_NR_FORMAT_0, 12, 0, 3},
    {ISRRAN_PUCCH_NR_FORMAT_0, 12, 3, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 10, 0, 2},
    {ISRRAN_PUCCH_NR_FORMAT_1, 10, 0, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 10, 2, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 10, 4, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 4, 0, 2},
    {ISRRAN_PUCCH_NR_FORMAT_1, 4, 0, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 4, 2, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 4, 4, 3},
    {ISRRAN_PUCCH_NR_FORMAT_1, 0, 0, 2},
    {ISRRAN_PUCCH_NR_FORMAT_1, 0, 0, 4},
    {ISRRAN_PUCCH_NR_FORMAT_1, 0, 2, 4},
    {ISRRAN_PUCCH_NR_FORMAT_1, 0, 4, 4},
    {ISRRAN_PUCCH_NR_FORMAT_1, 0, 0, 4}};
                                           ...
                                           isrran_pucch_nr_resource_t* resource)
{
  ...
  uint32_t csv_idx = 0;
  if (r_pucch / 8 == 0) {
    resource->starting_prb   = rb_offset_bwp + ISRRAN_FLOOR(r_pucch, N_cs);
    resource->second_hop_prb = N_size_bwp - 1 - resource->starting_prb;
    csv_idx                  = r_pucch % N_cs;
  } else {
    resource->second_hop_prb = rb_offset_bwp + ISRRAN_FLOOR(r_pucch - 8, N_cs);
...
static int ra_ul_nr_pucch_resource_hl(const isrran_pucch_nr_hl_cfg_t* cfg,
                                      const isrran_uci_cfg_nr_t*      uci_cfg,
                                      uint32_t                        pucch_resource_id,
                                      isrran_pucch_nr_resource_t*     resource)
{
  uint32_t O_uci = isrran_uci_nr_total_bits(uci_cfg);
  uint32_t N2    = cfg->sets[1].max_payload_size > 0 ? cfg->sets[1].max_payload_size : ISRRAN_UCI_NR_MAX_NOF_BITS;
  uint32_t N3    = cfg->sets[2].max_payload_size > 0 ? cfg->sets[2].max_payload_size : ISRRAN_UCI_NR_MAX_NOF_BITS;
  ...
  if (uci_cfg->nof_csi == 0 && uci_cfg->ack.count <= 2) {
    ...
    return ISRRAN_ERROR;
  } else if (O_uci > ISRRAN_UCI_NR_MAX_NOF_BITS) {
    ERROR("The number of UCI bits (%d), exceeds the maximum (%d)", O_uci, ISRRAN_UCI_NR_MAX_NOF_BITS);
    return ISRRAN_ERROR;
  }

  // Select resource from the set
  if (pucch_resource_id >= ISRRAN_PUCCH_NR_MAX_NOF_RESOURCES_PER_SET ||
...
int isrran_ra_ul_nr_pucch_resource(const isrran_pucch_nr_hl_cfg_t* pucch_cfg,
                                   const isrran_uci_cfg_nr_t*      uci_cfg,
                                   uint32_t                        N_bwp_sz,
                                   isrran_pucch_nr_resource_t*     resource)
{
  ...
  if (uci_cfg->sr_positive_present > 0 && uci_cfg->ack.count == 0 && uci_cfg->nof_csi == 0) {
    ...
    if (sr_resource_id >= ISRRAN_PUCCH_MAX_NOF_SR_RESOURCES) {
      ERROR("SR resource ID (%d) exceeds the maximum ID (%d)", sr_resource_id, ISRRAN_PUCCH_MAX_NOF_SR_RESOURCES);
      return ISRRAN_ERROR;
    }
  ...
  // - up to 2 HARQ-ACK
  // - No CSI report
  if (uci_cfg->sr_positive_present && uci_cfg->ack.count <= ISRRAN_PUCCH_NR_FORMAT1_MAX_NOF_BITS &&
      uci_cfg->nof_csi == 0) {
    uint32_t sr_resource_id = uci_cfg->pucch.sr_resource_id;
    if (sr_resource_id >= ISRRAN_PUCCH_MAX_NOF_SR_RESOURCES) {
      ERROR("SR resource ID (%d) exceeds the maximum ID (%d)", sr_resource_id, ISRRAN_PUCCH_MAX_NOF_SR_RESOURCES);
    ...
    isrran_pucch_nr_resource_t resource_sr = pucch_cfg->sr_resources[sr_resource_id].resource;

    // Select PUCCH resource for HARQ-ACK
    isrran_pucch_nr_resource_t resource_harq = {};
    if (ra_ul_nr_pucch_resource_hl(pucch_cfg, uci_cfg, uci_cfg->pucch.resource_id, &resource_harq) < ISRRAN_SUCCESS) {
    ...
    if (resource_sr.format == ISRRAN_PUCCH_NR_FORMAT_0 && resource_harq.format == ISRRAN_PUCCH_NR_FORMAT_1) {
    ...
    if (resource_sr.format == ISRRAN_PUCCH_NR_FORMAT_1 && resource_harq.format == ISRRAN_PUCCH_NR_FORMAT_1) {
  ...
  // - No CSI report
  if (uci_cfg->o_sr > 0 && uci_cfg->ack.count > ISRRAN_PUCCH_NR_FORMAT1_MAX_NOF_BITS && uci_cfg->nof_csi == 0) {
  ...
  if (uci_cfg->ack.count == 0 && uci_cfg->nof_csi == 1 && uci_cfg->csi[0].cfg.type == ISRRAN_CSI_REPORT_TYPE_PERIODIC) {
  ...
  if (!pucch_cfg->enabled) {
    uint32_t N_cce   = ISRRAN_FLOOR(N_bwp_sz, 6);
...
uint32_t isrran_ra_ul_nr_nof_sr_bits(uint32_t K)
{
  if (K > 0) {
