...
// objects
#define PRACH_N_ZC_LONG_LUT_SIZE (2 * ISRRAN_PRACH_N_ZC_LONG)
static cf_t cexp_table_long[PRACH_N_ZC_LONG_LUT_SIZE] = {};
#define PRACH_N_ZC_SHORT_LUT_SIZE (2 * ISRRAN_PRACH_N_ZC_SHORT)
...
static void prach_cexp(uint32_t N_zc, uint32_t u, cf_t* root)
{
  ...
  if (N_zc == ISRRAN_PRACH_N_ZC_LONG) {
    for (int j = 0; j < ISRRAN_PRACH_N_ZC_LONG; j++) {
  ...
  if (N_zc == ISRRAN_PRACH_N_ZC_SHORT) {
    for (int j = 0; j < ISRRAN_PRACH_N_ZC_SHORT; j++) {
      uint32_t phase_idx = u * j * (j + 1);
...
int isrran_prach_set_cell_(isrran_prach_t*      p,
                           uint32_t             N_ifft_ul,
                           isrran_prach_cfg_t*  cfg,
                           isrran_tdd_config_t* tdd_config);

uint32_t isrran_prach_get_preamble_format(uint32_t config_idx)
{
  return config_idx / 16;
}

isrran_prach_sfn_t isrran_prach_get_sfn(uint32_t config_idx)
{
  if ((config_idx % 16) < 3 || (config_idx % 16) == 15) {
    return ISRRAN_PRACH_SFN_EVEN;
  } else {
    return ISRRAN_PRACH_SFN_ANY;
  }
}
 ...
 */
bool isrran_prach_tti_opportunity(isrran_prach_t* p, uint32_t current_tti, int allowed_subframe)
{
  if (p == NULL) {
    return false;
  }
  ...
  if (p->tdd_config.configured) {
    if (p->is_nr) {
      return isrran_prach_nr_tti_opportunity_fr1_unpaired(p->config_idx, current_tti);
    } else {
      return isrran_prach_tti_opportunity_config_tdd(
          config_idx, p->tdd_config.sf_config, current_tti, &p->current_prach_idx);
    }
  } else {
    if (p->is_nr) {
      return isrran_prach_nr_tti_opportunity_fr1_paired(p->config_idx, current_tti);
    } else {
      return isrran_prach_tti_opportunity_config_fdd(config_idx, current_tti, allowed_subframe);
    }
  }
}

bool isrran_prach_tti_opportunity_config_fdd(uint32_t config_idx, uint32_t current_tti, int allowed_subframe)
{
  // Get SFN and sf_idx from the PRACH configuration index
  isrran_prach_sfn_t prach_sfn = isrran_prach_get_sfn(config_idx);
  ...
  if ((prach_sfn == ISRRAN_PRACH_SFN_EVEN && ((current_tti / 10) % 2) == 0) || prach_sfn == ISRRAN_PRACH_SFN_ANY) {
    isrran_prach_sf_config_t sf_config;
    isrran_prach_sf_config(config_idx, &sf_config);
...
bool isrran_prach_in_window_config_fdd(uint32_t config_idx, uint32_t current_tti, int allowed_subframe)
{
  if (isrran_prach_tti_opportunity_config_fdd(config_idx, current_tti, allowed_subframe)) {
    return true;
  }

  uint32_t preamble_format = isrran_prach_get_preamble_format(config_idx);
  float    T_tot           = (prach_Tseq[preamble_format] + prach_Tcp[preamble_format]) * ISRRAN_LTE_TS;
  uint32_t tti_dur         = (uint32_t)ceilf(T_tot * 1000);
  for (uint32_t i = 1; i < tti_dur; ++i) {
    if (isrran_prach_tti_opportunity_config_fdd(config_idx, current_tti - i, allowed_subframe)) {
      return true;
    }
  }
  return false;
...
uint32_t isrran_prach_nof_f_idx_tdd(uint32_t config_idx, uint32_t tdd_ul_dl_config)
{
...
uint32_t isrran_prach_f_id_tdd(uint32_t config_idx, uint32_t tdd_ul_dl_config, uint32_t prach_idx)
{
  if (config_idx < 64 && tdd_ul_dl_config < 7) {
...
uint32_t isrran_prach_f_ra_tdd(uint32_t config_idx,
...
bool isrran_prach_tti_opportunity_config_tdd(uint32_t  config_idx,
                                             uint32_t  tdd_ul_dl_config,
                                             uint32_t  current_tti,
                                             uint32_t* prach_idx)
{
    ...
    if (((sfn % 2) && t0 == 2) || (!(sfn % 2) && t0 == 1) || (t0 == 0)) {
      if ((sf_idx < 5 && t1 == 0) || (sf_idx >= 5 && t1 == 1)) {
        if (config_idx < 48) { // format 0 to 3
          ...
          // Only UpTs subframes
          isrran_tdd_config_t c = {tdd_ul_dl_config, 0, true};
          if (isrran_sfidx_tdd_type(c, sf_idx) == ISRRAN_TDD_SF_S) {
            if (prach_idx) {
              *prach_idx = i;
            }
            return true;
...
void isrran_prach_sf_config(uint32_t config_idx, isrran_prach_sf_config_t* sf_config)
{
  memcpy(sf_config, &prach_sf_config[config_idx % 16], sizeof(isrran_prach_sf_config_t));
}

const prach_nr_config_t* isrran_prach_nr_get_cfg_fr1_paired(uint32_t config_idx)
{
...
bool isrran_prach_nr_tti_opportunity_fr1_paired(uint32_t config_idx, uint32_t current_tti)
{
  uint32_t sfn    = current_tti / ISRRAN_NOF_SF_X_FRAME;
  uint32_t sf_idx = current_tti % ISRRAN_NOF_SF_X_FRAME;

  // Get configuration
  const prach_nr_config_t* cfg = isrran_prach_nr_get_cfg_fr1_paired(config_idx);
  if (cfg == NULL) {
    return false;
  }
...
uint32_t isrran_prach_nr_start_symbol_fr1_paired(uint32_t config_idx)
{
  // Get configuration
  const prach_nr_config_t* cfg = isrran_prach_nr_get_cfg_fr1_paired(config_idx);
  if (cfg == NULL) {
...
const prach_nr_config_t* isrran_prach_nr_get_cfg_fr1_unpaired(uint32_t config_idx)
{
...
bool isrran_prach_nr_tti_opportunity_fr1_unpaired(uint32_t config_idx, uint32_t current_tti)
{
  uint32_t sfn    = current_tti / ISRRAN_NOF_SF_X_FRAME;
  uint32_t sf_idx = current_tti % ISRRAN_NOF_SF_X_FRAME;

  // Get configuration
  const prach_nr_config_t* cfg = isrran_prach_nr_get_cfg_fr1_unpaired(config_idx);
  if (cfg == NULL) {
    return false;
  }
...
uint32_t isrran_prach_nr_start_symbol(uint32_t config_idx, isrran_duplex_mode_t duplex_mode)
{
  const prach_nr_config_t* cfg;
  if (duplex_mode == ISRRAN_DUPLEX_MODE_TDD) {
    // Get configuration
    cfg = isrran_prach_nr_get_cfg_fr1_unpaired(config_idx);
    if (cfg == NULL) {
      return 0;
    }
  } else {
    // Get configuration
    cfg = isrran_prach_nr_get_cfg_fr1_paired(config_idx);
    if (cfg == NULL) {
      return 0;
    }
  }
...
static cf_t* get_precoded_dft(isrran_prach_t* p, uint32_t idx)
{
  ...
  if (!(p->dft_gen_bitmap & gen_mask)) {
    isrran_dft_run(&p->zc_fft, p->seqs[idx], p->dft_seqs[idx]);
...
int isrran_prach_gen_seqs(isrran_prach_t* p)
{
  uint32_t u           = 0;
  uint32_t v           = 1;
  int      v_max       = 0;
  ...
  for (int i = 0; i < N_SEQS; i++) {
    ...
    // }
    isrran_vec_cf_copy(p->seqs[i], &root[C_v], p->N_zc - C_v);
    isrran_vec_cf_copy(&p->seqs[i][p->N_zc - C_v], root, C_v);
...
int isrran_prach_set_cfg(isrran_prach_t* p, isrran_prach_cfg_t* cfg, uint32_t nof_prb)
{
  return isrran_prach_set_cell_(p, isrran_symbol_sz(nof_prb), cfg, &cfg->tdd_config);
}

int isrran_prach_init(isrran_prach_t* p, uint32_t max_N_ifft_ul)
{
  int ret = ISRRAN_ERROR;
  if (p != NULL && max_N_ifft_ul < 2049) {
    bzero(p, sizeof(isrran_prach_t));

    p->max_N_ifft_ul = max_N_ifft_ul;

    // Set up containers
    p->prach_bins = isrran_vec_cf_malloc(ISRRAN_PRACH_N_ZC_LONG);
    p->corr_spec  = isrran_vec_cf_malloc(ISRRAN_PRACH_N_ZC_LONG);
    p->corr       = isrran_vec_f_malloc(ISRRAN_PRACH_N_ZC_LONG);
    p->cross      = isrran_vec_cf_malloc(ISRRAN_PRACH_N_ZC_LONG);
    p->corr_freq  = isrran_vec_cf_malloc(ISRRAN_PRACH_N_ZC_LONG);

    // Set up ZC FFTS
    if (isrran_dft_plan(&p->zc_fft, ISRRAN_PRACH_N_ZC_LONG, ISRRAN_DFT_FORWARD, ISRRAN_DFT_COMPLEX)) {
      return ISRRAN_ERROR;
    }
    isrran_dft_plan_set_mirror(&p->zc_fft, false);
    isrran_dft_plan_set_norm(&p->zc_fft, true);

    if (isrran_dft_plan(&p->zc_ifft, ISRRAN_PRACH_N_ZC_LONG, ISRRAN_DFT_BACKWARD, ISRRAN_DFT_COMPLEX)) {
      return ISRRAN_ERROR;
    }
    isrran_dft_plan_set_mirror(&p->zc_ifft, false);
    isrran_dft_plan_set_norm(&p->zc_ifft, false);

    uint32_t fft_size_alloc = max_N_ifft_ul * DELTA_F / DELTA_F_RA;

    p->ifft_in  = isrran_vec_cf_malloc(fft_size_alloc);
    p->ifft_out = isrran_vec_cf_malloc(fft_size_alloc);
    if (isrran_dft_plan(&p->ifft, fft_size_alloc, ISRRAN_DFT_BACKWARD, ISRRAN_DFT_COMPLEX)) {
      ERROR("Error creating DFT plan");
      return -1;
    }
    isrran_dft_plan_set_mirror(&p->ifft, true);
    isrran_dft_plan_set_norm(&p->ifft, true);

    if (isrran_dft_plan(&p->fft, fft_size_alloc, ISRRAN_DFT_FORWARD, ISRRAN_DFT_COMPLEX)) {
      ERROR("Error creating DFT plan");
      return -1;
    }

    p->signal_fft = isrran_vec_cf_malloc(fft_size_alloc);
    if (!p->signal_fft) {
      ERROR("Error allocating memory");
      return -1;
    }

    isrran_dft_plan_set_mirror(&p->fft, true);
    isrran_dft_plan_set_norm(&p->fft, true);
                           ...
                           isrran_tdd_config_t* tdd_config)
{
  ...
  if (p != NULL && N_ifft_ul < 2049 && cfg->config_idx < 64 && cfg->root_seq_idx < MAX_ROOTS) {
    ...
    uint32_t preamble_format   = isrran_prach_get_preamble_format(cfg->config_idx);
    p->is_nr                   = cfg->is_nr;
    ...
    if (4 == preamble_format) {
      if (p->zczc < 7) {
        p->N_zc = ISRRAN_PRACH_N_ZC_SHORT;
    ...
    // Set up ZC FFTS
    if (p->N_zc != ISRRAN_PRACH_N_ZC_LONG) {
      if (isrran_dft_replan(&p->zc_fft, p->N_zc)) {
        return ISRRAN_ERROR;
      }
      if (isrran_dft_replan(&p->zc_ifft, p->N_zc)) {
        return ISRRAN_ERROR;
      }
    }
    ...
    if (isrran_dft_replan(&p->ifft, p->N_ifft_prach)) {
      ERROR("Error creating DFT plan");
      return -1;
    }
    if (isrran_dft_replan(&p->fft, p->N_ifft_prach)) {
      ERROR("Error creating DFT plan");
      return -1;
    }
    ...
    p->T_seq = prach_Tseq[p->f] * ISRRAN_LTE_TS;
    p->T_tot = (prach_Tseq[p->f] + prach_Tcp[p->f]) * ISRRAN_LTE_TS;

    if (p->successive_cancellation) {
      for (int i = 0; i < 64; i++) {
        if (!p->td_signals[i]) {
          p->td_signals[i] = isrran_vec_malloc(sizeof(cf_t) * (p->N_seq + p->N_cp));
        }
      }
    }
    ret = ISRRAN_SUCCESS;
...
int isrran_prach_gen(isrran_prach_t* p, uint32_t seq_index, uint32_t freq_offset, cf_t* signal)
{
  int ret = ISRRAN_ERROR;
  if (p != NULL && seq_index < N_SEQS && signal != NULL) {
    // Calculate parameters
    uint32_t N_rb_ul = isrran_nof_prb(p->N_ifft_ul);
    ...
    isrran_vec_cf_zero(p->ifft_in, begin);

    // Map dft-precoded sequence to ifft bins
    isrran_vec_cf_copy(&p->ifft_in[begin], get_precoded_dft(p, seq_index), p->N_zc);

    // Fill top guard frequency domain with zeros
    isrran_vec_cf_zero(&p->ifft_in[begin + p->N_zc], p->N_ifft_prach - begin - p->N_zc);

    // Generate frequency domain signal
    isrran_dft_run(&p->ifft, p->ifft_in, p->ifft_out);
...
void isrran_prach_set_detect_factor(isrran_prach_t* p, float ratio)
{
  p->detect_factor = ratio;
}

int isrran_prach_detect(isrran_prach_t* p,
                        ...
                        uint32_t*       n_indices)
{
  return isrran_prach_detect_offset(p, freq_offset, signal, sig_len, indices, NULL, NULL, n_indices);
}
...
void isrran_prach_cancellation(isrran_prach_t* p)
{
  isrran_vec_cf_zero(p->sub, p->N_zc * 2);
  isrran_vec_cf_copy(p->sub, get_precoded_dft(p, p->root_seqs_idx[p->prach_cancel.idx]), p->N_zc);

  isrran_vec_prod_ccc(p->sub, p->prach_cancel.phase_array, p->sub, p->N_zc);
#ifdef PRACH_CANCELLATION_HARD
  isrran_vec_prod_conj_ccc(p->prach_bins, p->sub, p->corr_spec, p->N_zc);
  isrran_dft_run(&p->zc_ifft, p->corr_spec, p->corr_spec);
  isrran_vec_abs_square_cf(p->corr_spec, p->corr, p->N_zc);
  prach_cancel->factor = sqrt(p->corr[0] / (p->N_zc * p->N_zc));
#endif
  isrran_vec_sc_prod_cfc(p->sub, p->prach_cancel.factor, p->sub, p->N_zc);
  isrran_vec_sub_ccc(p->prach_bins, p->sub, p->prach_bins, p->N_zc);
}
...
bool isrran_prach_have_stored(int current_idx, uint32_t* indices, uint32_t n_indices)
{
...
float isrran_prach_get_offset_secs(isrran_prach_t* p, int n_win)
{
...
// approach
float isrran_prach_calculate_time_offset_secs(isrran_prach_t* p, cf_t* cross)
{
  // calculate the phase of the cross correlation
  float freq_domain_phase = cargf(isrran_vec_acc_cc(cross, p->N_zc));
  float ratio             = (float)(p->N_ifft_ul * DELTA_F) / (float)(ISRRAN_PRACH_N_ZC_LONG * DELTA_F_RA);
...
void isrran_prach_calculate_correction_array(isrran_prach_t* p, cf_t* corr_freq)
{
  isrran_vec_arg_deg_cf(corr_freq, 0, p->phase, p->N_zc);
...
int isrran_prach_process(isrran_prach_t* p,
                         cf_t*           signal,
                         uint32_t*       indices,
                         float*          t_offsets,
                         float*          peak_to_avg,
                         ...
                         uint32_t        sig_len)
{
  ...
  cancellation_idx    = -1;
  int max_idx         = 0;
  isrran_vec_cf_zero(p->cross, p->N_zc);
  isrran_vec_cf_zero(p->corr_freq, p->N_zc);
  for (int i = 0; i < p->num_ra_preambles; i++) {
    cf_t* root_spec = get_precoded_dft(p, p->root_seqs_idx[i]);

    isrran_vec_prod_conj_ccc(p->prach_bins, root_spec, p->corr_spec, p->N_zc);

    isrran_vec_prod_conj_ccc(p->corr_spec, &p->corr_spec[1], p->cross, p->N_zc - 1);
    if (p->successive_cancellation) {
      isrran_vec_cf_copy(p->corr_freq, p->corr_spec, p->N_zc);
    }
    isrran_dft_run(&p->zc_ifft, p->corr_spec, p->corr_spec);

    isrran_vec_abs_square_cf(p->corr_spec, p->corr, p->N_zc);

    float corr_ave = isrran_vec_acc_ff(p->corr, p->N_zc) / p->N_zc;
        ...
        if (p->peak_values[j] > p->detect_factor * corr_ave) {
          if (indices) {
            if (p->successive_cancellation) {
              if (max_peak > max_to_cancel) {
                ...
                isrran_prach_calculate_correction_array(p, p->corr_freq);
              }
              if (isrran_prach_have_stored(((i * n_wins) + j), indices, *n_indices)) {
                break;
              }
            }
          ...
          if (t_offsets) {
            ...
            t_offsets[*n_indices] = (p->freq_domain_offset_calc)
                                        ? (isrran_prach_calculate_time_offset_secs(p, p->cross))
                                        : (isrran_prach_get_offset_secs(p, j));
          }
          (*n_indices)++;
        }
      }
...
int isrran_prach_detect_offset(isrran_prach_t* p,
                               uint32_t        freq_offset,
                               cf_t*           signal,
                               uint32_t        sig_len,
                               uint32_t*       indices,
                               ...
                               uint32_t*       n_indices)
{
  ...
  if (p != NULL && signal != NULL && sig_len > 0 && indices != NULL) {
    if (sig_len < p->N_ifft_prach) {
      ERROR("isrran_prach_detect: Signal length is %d and should be %d", sig_len, p->N_ifft_prach);
      return ISRRAN_ERROR_INVALID_INPUTS;
    }
    int cancellation_idx = -2;
    bzero(&p->prach_cancel, sizeof(isrran_prach_cancellation_t));

    // FFT incoming signal
    isrran_dft_run(&p->fft, signal, p->signal_fft);
...
int isrran_prach_free(isrran_prach_t* p)
{
  free(p->prach_bins);
  free(p->corr_spec);
  free(p->corr);
  isrran_dft_plan_free(&p->ifft);
  free(p->ifft_in);
  free(p->ifft_out);
  free(p->cross);
  free(p->corr_freq);
  isrran_dft_plan_free(&p->fft);
  isrran_dft_plan_free(&p->zc_fft);
  isrran_dft_plan_free(&p->zc_ifft);
...
int isrran_prach_print_seqs(isrran_prach_t* p)
{
  for (int i = 0; i < N_SEQS; i++) {
    FILE* f;
    char  str[32];
