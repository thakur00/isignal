...
#define REG_IDX(r, i, n) r->k[i] + r->l* n* ISRRAN_NRE

isrran_regs_reg_t* regs_find_reg(isrran_regs_t* h, uint32_t k, uint32_t l);
int                regs_put_reg(isrran_regs_reg_t* reg, cf_t* reg_data, cf_t* slot_symbols, uint32_t nof_prb);

int regs_add_reg(isrran_regs_reg_t* reg, cf_t* reg_data, cf_t* slot_symbols, uint32_t nof_prb);

int regs_get_reg(isrran_regs_reg_t* reg, cf_t* slot_symbols, cf_t* reg_data, uint32_t nof_prb);

int regs_reset_reg(isrran_regs_reg_t* reg, cf_t* slot_symbols, uint32_t nof_prb);
...
void regs_pdcch_free(isrran_regs_t* h)
{
  int i;
  for (i = 0; i < 3; i++) {
    if (h->pdcch[i].regs) {
 ...
 */
int regs_pdcch_init(isrran_regs_t* h)
{
  int                 i, m, cfi, nof_ctrl_symbols;
  int                 ret = ISRRAN_ERROR;
  int                 nrows, ndummy, j;
  uint32_t            k, kp;
  isrran_regs_reg_t** tmp = NULL;

  bzero(&h->pdcch, sizeof(isrran_regs_ch_t));
  ...
  for (cfi = 0; cfi < 3; cfi++) {
    ...
    tmp = malloc(sizeof(isrran_regs_reg_t*) * h->nof_regs);
    if (!tmp) {
      perror("malloc");
      goto clean_and_exit;
    }
    ...
    h->pdcch[cfi].regs = malloc(sizeof(isrran_regs_reg_t*) * h->pdcch[cfi].nof_regs);
    if (!h->pdcch[cfi].regs) {
      perror("malloc");
      goto clean_and_exit;
    }
...
int isrran_regs_pdcch_nregs(isrran_regs_t* h, uint32_t cfi)
{
  if (cfi < 1 || cfi > 3) {
...
int isrran_regs_pdcch_ncce(isrran_regs_t* h, uint32_t cfi)
{
  int nregs = isrran_regs_pdcch_nregs(h, cfi);
  if (nregs > 0) {
...
int isrran_regs_pdcch_put_offset(isrran_regs_t* h,
                                 uint32_t       cfi,
                                 cf_t*          d,
                                 cf_t*          slot_symbols,
                                 uint32_t       start_reg,
...
int isrran_regs_pdcch_put(isrran_regs_t* h, uint32_t cfi, cf_t* d, cf_t* slot_symbols)
{
  if (cfi < 1 || cfi > 3) {
    ERROR("Invalid CFI=%d", cfi);
    return ISRRAN_ERROR;
  }
  return isrran_regs_pdcch_put_offset(h, cfi, d, slot_symbols, 0, h->pdcch[cfi - 1].nof_regs);
}

int isrran_regs_pdcch_get_offset(isrran_regs_t* h,
                                 uint32_t       cfi,
                                 cf_t*          slot_symbols,
                                 cf_t*          d,
                                 uint32_t       start_reg,
...
int isrran_regs_pdcch_get(isrran_regs_t* h, uint32_t cfi, cf_t* slot_symbols, cf_t* d)
{
  if (cfi < 1 || cfi > 3) {
    ERROR("Invalid CFI=%d", cfi);
    return ISRRAN_ERROR;
  }
  return isrran_regs_pdcch_get_offset(h, cfi, slot_symbols, d, 0, h->pdcch[cfi - 1].nof_regs);
}
 ...
 */
int regs_phich_init(isrran_regs_t* h, uint32_t phich_mi, bool mbsfn_or_sf1_6_tdd)
{
  float               ng;
  uint32_t            i, ni, li, n[3], nreg, mi;
  isrran_regs_reg_t** regs_phich[3];
  int                 ret = ISRRAN_ERROR;
  ...
  h->phich            = malloc(sizeof(isrran_regs_ch_t) * h->ngroups_phich);
  if (!h->phich) {
    perror("malloc");
    return -1;
  }
  INFO("Creating %d PHICH mapping units. %s length, Ng=%.2f",
       h->ngroups_phich,
       h->phich_len == ISRRAN_PHICH_EXT ? "Extended" : "Normal",
       ng);
  for (i = 0; i < h->ngroups_phich; i++) {
    h->phich[i].nof_regs = REGS_PHICH_REGS_X_GROUP;
    h->phich[i].regs     = malloc(sizeof(isrran_regs_reg_t*) * REGS_PHICH_REGS_X_GROUP);
    if (!h->phich[i].regs) {
      perror("malloc");
      goto clean_and_exit;
    }
  ...
  bzero(regs_phich, sizeof(isrran_regs_reg_t*) * 3);
  for (i = 0; i < 3; i++) {
    regs_phich[i] = malloc(n[i] * sizeof(isrran_regs_reg_t*));
    if (!regs_phich[i]) {
      perror("malloc");
      goto clean_and_exit;
    }
  ...
  for (mi = 0; mi < h->ngroups_phich; mi++) { // here ngroups is the number of mapping units
    for (i = 0; i < 3; i++) {
      // Step 7
      if (h->phich_len == ISRRAN_PHICH_NORM) {
        li = 0;
      } else if (h->phich_len == ISRRAN_PHICH_EXT && mbsfn_or_sf1_6_tdd) {
        li = (mi / 2 + i + 1) % 2;
      } else {
        li = i;
      }
      // Step 8
      if (h->phich_len == ISRRAN_PHICH_EXT && mbsfn_or_sf1_6_tdd) {
...
void regs_phich_free(isrran_regs_t* h)
{
  uint32_t i;
  if (h->phich) {
    if (ISRRAN_CP_ISEXT(h->cell.cp)) {
...
uint32_t isrran_regs_phich_nregs(isrran_regs_t* h)
{
  uint32_t i;
  uint32_t n;
  n = 0;
...
uint32_t isrran_regs_phich_ngroups(isrran_regs_t* h)
{
  return h->ngroups_phich;
}

uint32_t isrran_regs_phich_ngroups_m1(isrran_regs_t* h)
{
 ...
 *
 * Each subframe, the user shall call the isrran_v function before adding PHICH symbols.
 *
 * Returns the number of written symbols, or -1 on error
 */
int isrran_regs_phich_add(isrran_regs_t* h, cf_t symbols[REGS_PHICH_NSYM], uint32_t ngroup, cf_t* slot_symbols)
{
  uint32_t i;
  ...
  isrran_regs_ch_t* rch = &h->phich[ngroup];
 ...
 * Returns the number of written symbols, or -1 on error
 */
int isrran_regs_phich_reset(isrran_regs_t* h, cf_t* slot_symbols)
{
  uint32_t i;
  uint32_t ngroup, ng;
  for (ngroup = 0; ngroup < h->ngroups_phich; ISRRAN_CP_ISEXT(h->cell.cp) ? ngroup += 2 : ngroup++) {
    if (ISRRAN_CP_ISEXT(h->cell.cp)) {
      ng = ngroup / 2;
    } else {
      ng = ngroup;
    }
    isrran_regs_ch_t* rch = &h->phich[ng];
    for (i = 0; i < rch->nof_regs && i * REGS_RE_X_REG < REGS_PHICH_NSYM; i++) {
 ...
 * Returns the number of written symbols, or -1 on error
 */
int isrran_regs_phich_get(isrran_regs_t* h, cf_t* slot_symbols, cf_t symbols[REGS_PHICH_NSYM], uint32_t ngroup)
{
  uint32_t i;
  if (ngroup >= h->ngroups_phich) {
    ERROR("Error invalid ngroup %d", ngroup);
 ...
 */
int regs_pcfich_init(isrran_regs_t* h)
{
  uint32_t          i;
  uint32_t          k_hat, k;
  isrran_regs_ch_t* ch = &h->pcfich;

  ch->regs = malloc(sizeof(isrran_regs_reg_t*) * REGS_PCFICH_NREGS);
  if (!ch->regs) {
    perror("malloc");
    return ISRRAN_ERROR;
  }
  ...
  k_hat = (ISRRAN_NRE / 2) * (h->cell.id % (2 * h->cell.nof_prb));
  for (i = 0; i < REGS_PCFICH_NREGS; i++) {
    k           = (k_hat + (i * h->cell.nof_prb / 2) * (ISRRAN_NRE / 2)) % (h->cell.nof_prb * ISRRAN_NRE);
...
void regs_pcfich_free(isrran_regs_t* h)
{
  if (h->pcfich.regs) {
    free(h->pcfich.regs);
    h->pcfich.regs = NULL;
...
uint32_t isrran_regs_pcfich_nregs(isrran_regs_t* h)
{
 ...
 * Returns the number of written symbols, or -1 on error
 */
int isrran_regs_pcfich_put(isrran_regs_t* h, cf_t symbols[REGS_PCFICH_NSYM], cf_t* slot_symbols)
{
  isrran_regs_ch_t* rch = &h->pcfich;
 ...
 * Returns the number of written symbols, or -1 on error
 */
int isrran_regs_pcfich_get(isrran_regs_t* h, cf_t* slot_symbols, cf_t ch_data[REGS_PCFICH_NSYM])
{
  isrran_regs_ch_t* rch = &h->pcfich;
  uint32_t          i;
  for (i = 0; i < rch->nof_regs && i * REGS_RE_X_REG < REGS_PCFICH_NSYM; i++) {
 ...
 */
int regs_num_x_symbol(uint32_t symbol, uint32_t nof_port, isrran_cp_t cp)
{
  switch (symbol) {
    case 0:
      return 2;
 ...
 * 36.211 Section 6.2.4
 */
int regs_reg_init(isrran_regs_reg_t* reg, uint32_t symbol, uint32_t nreg, uint32_t k0, uint32_t maxreg, uint32_t vo)
{
  uint32_t i, j, z;
...
void isrran_regs_free(isrran_regs_t* h)
{
  if (h->regs) {
    free(h->regs);
  }
  ...
  bzero(h, sizeof(isrran_regs_t));
}

int isrran_regs_init(isrran_regs_t* h, isrran_cell_t cell)
{
  return isrran_regs_init_opts(h, cell, 1, false);
}
 ...
 */
int isrran_regs_init_opts(isrran_regs_t* h, isrran_cell_t cell, uint32_t phich_mi, bool mbsfn_or_sf1_6_tdd)
{
  ...
  if (h != NULL && isrran_cell_isvalid(&cell)) {
    bzero(h, sizeof(isrran_regs_t));
    ret = ISRRAN_ERROR;
    ...
    INFO("Indexing %d REGs. CellId: %d, %d PRB, CP: %s",
         ...
         h->cell.id,
         h->cell.nof_prb,
         ISRRAN_CP_ISNORM(h->cell.cp) ? "Normal" : "Extended");
    h->regs = malloc(sizeof(isrran_regs_reg_t) * h->nof_regs);
    if (!h->regs) {
      perror("malloc");
      goto clean_and_exit;
    }
    ...
    while (k < h->nof_regs) {
      if (n[i] == 3 || (n[i] == 2 && jmax != 1)) {
        if (regs_reg_init(&h->regs[k], i, j[i], prb * ISRRAN_NRE, n[i], vo)) {
