...
int isrran_pucch_init_(isrran_pucch_t* q, bool is_ue)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;
  if (q != NULL) {
    ret = ISRRAN_ERROR;
    bzero(q, sizeof(isrran_pucch_t));
    ...
    if (isrran_sequence_init(&q->seq_f2, 20)) {
      goto clean_exit;
    }

    isrran_uci_cqi_pucch_init(&q->cqi);

    q->z     = isrran_vec_cf_malloc(ISRRAN_PUCCH_MAX_SYMBOLS);
    q->z_tmp = isrran_vec_cf_malloc(ISRRAN_PUCCH_MAX_SYMBOLS);

    if (!q->is_ue) {
      q->ce = isrran_vec_cf_malloc(ISRRAN_PUCCH_MAX_SYMBOLS);
    }
...
int isrran_pucch_init_ue(isrran_pucch_t* q)
{
  return isrran_pucch_init_(q, true);
}

int isrran_pucch_init_enb(isrran_pucch_t* q)
{
  return isrran_pucch_init_(q, false);
}

void isrran_pucch_free(isrran_pucch_t* q)
{
  isrran_sequence_free(&q->seq_f2);

  isrran_uci_cqi_pucch_free(&q->cqi);
  if (q->z) {
    free(q->z);
  }
  if (q->z_tmp) {
...
int isrran_pucch_set_cell(isrran_pucch_t* q, isrran_cell_t cell)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;
...
static int
uci_mod_bits(isrran_pucch_t* q, isrran_ul_sf_cfg_t* sf, isrran_pucch_cfg_t* cfg, uint8_t bits[ISRRAN_PUCCH_MAX_BITS])
{
  uint8_t tmp[2];
  switch (cfg->format) {
    ...
    case ISRRAN_PUCCH_FORMAT_2B:
      if (isrran_sequence_pucch(&q->seq_f2, cfg->rnti, 2 * (sf->tti % 10), q->cell.id)) {
        ERROR("Error computing PUCCH Format 2 scrambling sequence\n");
        return ISRRAN_ERROR;
      }
      isrran_vec_u8_copy(q->bits_scram, bits, ISRRAN_PUCCH2_NOF_BITS);
      isrran_scrambling_b_offset(&q->seq_f2, q->bits_scram, 0, ISRRAN_PUCCH2_NOF_BITS);
      isrran_mod_modulate(&q->mod, q->bits_scram, q->d, ISRRAN_PUCCH2_NOF_BITS);
      break;
    case ISRRAN_PUCCH_FORMAT_3:
      ...
      isrran_vec_u8_copy(q->bits_scram, bits, ISRRAN_PUCCH3_NOF_BITS);
      isrran_scrambling_b_offset(&q->seq_f2, q->bits_scram, 0, ISRRAN_PUCCH3_NOF_BITS);
      isrran_mod_modulate(&q->mod, q->bits_scram, q->d, ISRRAN_PUCCH3_NOF_BITS);
      break;
    default:
      ERROR("PUCCH format %s not supported", isrran_pucch_format_text(cfg->format));
      return ISRRAN_ERROR;
  }
...
static uint32_t get_N_sf(isrran_pucch_format_t format, uint32_t slot_idx, bool shortened)
{
  switch (format) {
    case ISRRAN_PUCCH_FORMAT_1:
    case ISRRAN_PUCCH_FORMAT_1A:
...
static uint32_t get_pucch_symbol(uint32_t m, isrran_pucch_format_t format, isrran_cp_t cp)
{
  switch (format) {
    case ISRRAN_PUCCH_FORMAT_1:
    case ISRRAN_PUCCH_FORMAT_1A:
...
static int pucch_cp(isrran_pucch_t*     q,
        ...
        for (uint32_t i = 0; i < N_sf; i++) {
          ...
          if (!source_is_grid) {
            memcpy(&dest[ISRRAN_RE_IDX(q->cell.nof_prb, l + ns * nsymbols, n_prb * ISRRAN_NRE)],
                   &source[i * ISRRAN_NRE + ns * N_sf_0 * ISRRAN_NRE],
                   ISRRAN_NRE * sizeof(cf_t));
          } else {
            memcpy(&dest[i * ISRRAN_NRE + ns * N_sf_0 * ISRRAN_NRE],
...
static int pucch_put(isrran_pucch_t* q, isrran_ul_sf_cfg_t* sf, isrran_pucch_cfg_t* cfg, cf_t* z, cf_t* output)
{
  return pucch_cp(q, sf, cfg, z, output, false);
}

static int pucch_get(isrran_pucch_t* q, isrran_ul_sf_cfg_t* sf, isrran_pucch_cfg_t* cfg, cf_t* input, cf_t* z)
{
  return pucch_cp(q, sf, cfg, input, z, true);
}

static int encode_signal_format12(isrran_pucch_t*     q,
                                  isrran_ul_sf_cfg_t* sf,
                                  isrran_pucch_cfg_t* cfg,
                                  uint8_t             bits[ISRRAN_PUCCH_MAX_BITS],
                                  cf_t                z[ISRRAN_PUCCH_MAX_SYMBOLS],
                                  bool                signal_only)
{
  if (!signal_only) {
    ...
    // Set all ones
    for (uint32_t i = 0; i < ISRRAN_PUCCH_MAX_BITS / 2; i++) {
      q->d[i] = 1.0f;
    }
  }
  uint32_t N_sf_0 = get_N_sf(cfg->format, 0, sf->shortened);
  uint32_t sf_idx = sf->tti % ISRRAN_NOF_SF_X_FRAME;
  for (uint32_t ns = ISRRAN_NOF_SLOTS_PER_SF * sf_idx; ns < ISRRAN_NOF_SLOTS_PER_SF * (sf_idx + 1); ns++) {
    uint32_t N_sf = get_N_sf(cfg->format, ns % 2, sf->shortened);
    ...
    for (uint32_t m = 0; m < N_sf; m++) {
      ...
      cf_t* z_m = &z[(ns % 2) * N_sf_0 * ISRRAN_PUCCH_N_SEQ + m * ISRRAN_PUCCH_N_SEQ];

      // Get symbol index
      uint32_t l   = get_pucch_symbol(m, cfg->format, q->cell.cp);
      cf_t     w_m = 1.0;
      if (cfg->format >= ISRRAN_PUCCH_FORMAT_2) {
        float alpha = isrran_pucch_alpha_format2(q->n_cs_cell, cfg, ns, l);
        isrran_zc_sequence_generate_lte(u, 0, alpha, ISRRAN_PUCCH_N_SEQ / ISRRAN_NRE, z_m);
        w_m = q->d[(ns % 2) * N_sf + m];
      } else {
        uint32_t n_prime_ns = 0;
        uint32_t n_oc       = 0;
        float    alpha = isrran_pucch_alpha_format1(q->n_cs_cell, cfg, q->cell.cp, true, ns, l, &n_oc, &n_prime_ns);
        float    S_ns  = 0;
        if (n_prime_ns % 2) {
          S_ns = M_PI / 2;
        }
      ...
      // Apply w_m
      isrran_vec_sc_prod_ccc(z_m, w_m, z_m, ISRRAN_PUCCH_N_SEQ);
    }
  }
  return ISRRAN_SUCCESS;
}

static int encode_signal_format3(isrran_pucch_t*     q,
                                 isrran_ul_sf_cfg_t* sf,
                                 isrran_pucch_cfg_t* cfg,
                                 uint8_t             bits[ISRRAN_PUCCH_MAX_BITS],
                                 cf_t                z[ISRRAN_PUCCH_MAX_SYMBOLS],
                                 bool                signal_only)
{
  if (!signal_only) {
    ...
    for (int i = 0; i < ISRRAN_PUCCH_MAX_BITS / 2; i++) {
      q->d[i] = 1.0;
    }
  }
  ...
  for (uint32_t n = 0; n < N_sf_0 + N_sf_1; n++) {
    ...
    uint32_t n_cs_cell = q->n_cs_cell[(2 * (sf->tti % 10) + ((n < N_sf_0) ? 0 : 1)) % ISRRAN_NSLOTS_X_FRAME][l];

    cf_t y_n[ISRRAN_NRE];
    isrran_vec_cf_zero(y_n, ISRRAN_NRE);

    cf_t h;
    if (n < N_sf_0) {
      h = w_n_oc_0[n] * cexpf(I * M_PI * floorf(n_cs_cell / 64.0f) / 2);
      for (uint32_t i = 0; i < ISRRAN_NRE; i++) {
        y_n[i] = h * q->d[(i + n_cs_cell) % ISRRAN_NRE];
      }
    } else {
      h = w_n_oc_1[n - N_sf_0] * cexpf(I * M_PI * floorf(n_cs_cell / 64.0f) / 2);
      for (uint32_t i = 0; i < ISRRAN_NRE; i++) {
        y_n[i] = h * q->d[((i + n_cs_cell) % ISRRAN_NRE) + ISRRAN_NRE];
      }
    }

    for (int k = 0; k < ISRRAN_NRE; k++) {
      cf_t acc = 0.0f;
      for (int i = 0; i < ISRRAN_NRE; i++) {
        acc += y_n[i] * cexpf(-I * 2.0 * M_PI * i * k / (float)ISRRAN_NRE);
      }
      z[n * ISRRAN_NRE + k] = acc / sqrtf(ISRRAN_NRE);
    }
  }
...
static int decode_signal_format3(isrran_pucch_t*     q,
                                 isrran_ul_sf_cfg_t* sf,
                                 isrran_pucch_cfg_t* cfg,
                                 uint8_t             bits[ISRRAN_PUCCH_MAX_BITS],
                                 cf_t                z[ISRRAN_PUCCH_MAX_SYMBOLS])
{
  ...
  memset(q->d, 0, sizeof(cf_t) * 2 * ISRRAN_NRE);
  ...
  for (uint32_t n = 0; n < N_sf_0 + N_sf_1; n++) {
    ...
    for (int k = 0; k < ISRRAN_NRE; k++) {
      cf_t acc = 0.0f;
      for (int i = 0; i < ISRRAN_NRE; i++) {
        acc += z[n * ISRRAN_NRE + i] * cexpf(I * 2.0 * M_PI * i * k / (float)ISRRAN_NRE);
      }
      y_n[k] = acc / sqrtf(ISRRAN_NRE);
    }

    if (n < N_sf_0) {
      cf_t h = w_n_oc_0[n] * cexpf(-I * M_PI * floorf(n_cs_cell / 64.0f) / 2);
      for (uint32_t i = 0; i < ISRRAN_NRE; i++) {
        q->d[(i + n_cs_cell) % ISRRAN_NRE] += h * y_n[i];
      }
    } else {
      cf_t h = w_n_oc_1[n - N_sf_0] * cexpf(-I * M_PI * floorf(n_cs_cell / 64.0f) / 2);
      for (uint32_t i = 0; i < ISRRAN_NRE; i++) {
        q->d[((i + n_cs_cell) % ISRRAN_NRE) + ISRRAN_NRE] += h * y_n[i];
      }
    }
  }

  isrran_vec_sc_prod_cfc(q->d, 2.0f / (N_sf_0 + N_sf_1), q->d, ISRRAN_NRE * 2);

  isrran_demod_soft_demodulate_s(ISRRAN_MOD_QPSK, q->d, q->llr, ISRRAN_PUCCH3_NOF_BITS);

  if (isrran_sequence_pucch(&q->seq_f2, cfg->rnti, 2 * (sf->tti % 10), q->cell.id)) {
    ERROR("Error computing PUCCH Format 2 scrambling sequence\n");
    return ISRRAN_ERROR;
  }
  isrran_scrambling_s_offset(&q->seq_f2, q->llr, 0, ISRRAN_PUCCH3_NOF_BITS);

  return (int)isrran_block_decode_i16(q->llr, ISRRAN_PUCCH3_NOF_BITS, bits, ISRRAN_UCI_MAX_ACK_SR_BITS);
}

static int encode_signal(isrran_pucch_t*     q,
                         isrran_ul_sf_cfg_t* sf,
                         isrran_pucch_cfg_t* cfg,
                         uint8_t             bits[ISRRAN_PUCCH_MAX_BITS],
                         cf_t                z[ISRRAN_PUCCH_MAX_SYMBOLS])
{
  if (cfg->format == ISRRAN_PUCCH_FORMAT_3) {
...
static int encode_bits(isrran_pucch_cfg_t*   cfg,
                       isrran_uci_value_t*   uci_data,
                       isrran_pucch_format_t format,
                       uint8_t               pucch_bits[ISRRAN_PUCCH_MAX_BITS],
                       uint8_t               pucch2_bits[ISRRAN_PUCCH_MAX_BITS])
{
  if (format < ISRRAN_PUCCH_FORMAT_2) {
    isrran_vec_u8_copy(pucch_bits, uci_data->ack.ack_value, isrran_uci_cfg_total_ack(&cfg->uci_cfg));
  } else if (format >= ISRRAN_PUCCH_FORMAT_2 && format < ISRRAN_PUCCH_FORMAT_3) {
    /* Put RI (goes alone) */
    if (cfg->uci_cfg.cqi.ri_len) {
      uint8_t temp[2] = {uci_data->ri, 0};
      isrran_uci_encode_cqi_pucch(temp, cfg->uci_cfg.cqi.ri_len, pucch_bits);
    } else {
      /* Put CQI Report*/
      uint8_t buff[ISRRAN_CQI_MAX_BITS];
      int     uci_cqi_len = isrran_cqi_value_pack(&cfg->uci_cfg.cqi, &uci_data->cqi, buff);
      if (uci_cqi_len < 0) {
        ERROR("Error encoding CQI");
        return ISRRAN_ERROR;
      }
      isrran_uci_encode_cqi_pucch(buff, (uint32_t)uci_cqi_len, pucch_bits);
    }
    if (format > ISRRAN_PUCCH_FORMAT_2) {
      pucch2_bits[0] = uci_data->ack.ack_value[0];
      pucch2_bits[1] = uci_data->ack.ack_value[1]; // this will be ignored in format 2a
    }
  } else if (format == ISRRAN_PUCCH_FORMAT_3) {
    uint8_t  temp[ISRRAN_UCI_MAX_ACK_BITS + 1];
    uint32_t k = 0;
    for (; k < isrran_uci_cfg_total_ack(&cfg->uci_cfg); k++) {
    ...
    isrran_block_encode(temp, k, pucch_bits, ISRRAN_PUCCH3_NOF_BITS);
  }
  return ISRRAN_SUCCESS;
}

static bool decode_signal(isrran_pucch_t*     q,
                          isrran_ul_sf_cfg_t* sf,
                          isrran_pucch_cfg_t* cfg,
                          uint8_t             pucch_bits[ISRRAN_CQI_MAX_BITS],
                          uint32_t            nof_re,
                          uint32_t            nof_uci_bits,
                          float*              correlation)
{
  int16_t llr_pucch2[ISRRAN_CQI_MAX_BITS];
  bool    detected = false;
  float   corr = 0, corr_max = -1e9;
  uint8_t b_max = 0, b2_max = 0; // default bit value, eg. HI is NACK

  cf_t ref[ISRRAN_PUCCH_MAX_SYMBOLS];

  switch (cfg->format) {
    case ISRRAN_PUCCH_FORMAT_1:
      encode_signal(q, sf, cfg, pucch_bits, q->z_tmp);
      corr = isrran_vec_corr_ccc(q->z, q->z_tmp, nof_re);
    ...
    case ISRRAN_PUCCH_FORMAT_2B:
      ...
      isrran_vec_prod_conj_ccc(q->z, ref, q->z_tmp, ISRRAN_PUCCH_MAX_SYMBOLS);
      for (int i = 0; i < (ISRRAN_PUCCH2_N_SF * ISRRAN_NOF_SLOTS_PER_SF); i++) {
        q->z[i] = isrran_vec_acc_cc(&q->z_tmp[i * ISRRAN_NRE], ISRRAN_NRE) / ISRRAN_NRE;
      }
      isrran_demod_soft_demodulate_s(ISRRAN_MOD_QPSK, q->z, llr_pucch2, ISRRAN_PUCCH2_NOF_BITS / 2);
      isrran_scrambling_s_offset(&q->seq_f2, llr_pucch2, 0, ISRRAN_PUCCH2_NOF_BITS);

      // Calculate the LLR RMS for normalising
      float llr_pow = isrran_vec_avg_power_sf(llr_pucch2, ISRRAN_PUCCH2_NOF_BITS);

      if (isnormal(llr_pow)) {
        float llr_rms = sqrtf(llr_pow) * ISRRAN_PUCCH2_NOF_BITS;
        corr          = ((float)isrran_uci_decode_cqi_pucch(&q->cqi, llr_pucch2, pucch_bits, nof_uci_bits)) / (llr_rms);
      } else {
        corr = 0;
      }
      detected = true;
...
static void decode_bits(isrran_pucch_cfg_t* cfg,
                        bool                pucch_found,
                        uint8_t             pucch_bits[ISRRAN_PUCCH_MAX_BITS],
                        uint8_t             pucch2_bits[ISRRAN_PUCCH_MAX_BITS],
                        isrran_uci_value_t* uci_data)
{
  if (cfg->format == ISRRAN_PUCCH_FORMAT_3) {
    uint32_t nof_ack = isrran_uci_cfg_total_ack(&cfg->uci_cfg);
    ...
    // Save ACK bits
    for (uint32_t a = 0; a < isrran_pucch_nof_ack_format(cfg->format); a++) {
    ...
    if (cfg->uci_cfg.cqi.data_enable) {
      isrran_cqi_value_unpack(&cfg->uci_cfg.cqi, pucch_bits, &uci_data->cqi);
    }
...
int isrran_pucch_encode(isrran_pucch_t*     q,
                        isrran_ul_sf_cfg_t* sf,
                        isrran_pucch_cfg_t* cfg,
                        isrran_uci_value_t* uci_data,
                        cf_t*               sf_symbols)
{
  uint8_t pucch_bits[ISRRAN_PUCCH_MAX_BITS];
  isrran_vec_u8_zero(pucch_bits, ISRRAN_PUCCH_MAX_BITS);
...
int isrran_pucch_decode(isrran_pucch_t*        q,
                        isrran_ul_sf_cfg_t*    sf,
                        isrran_pucch_cfg_t*    cfg,
...
{
  uint8_t pucch_bits[ISRRAN_CQI_MAX_BITS];
  bzero(pucch_bits, ISRRAN_CQI_MAX_BITS * sizeof(uint8_t));

  int ret = ISRRAN_ERROR_INVALID_INPUTS;

  if (q != NULL && cfg != NULL && channel != NULL && data != NULL) {
    uint32_t nof_cqi_bits = isrran_cqi_size(&cfg->uci_cfg.cqi);
    ...
    // Equalization
    isrran_predecoding_single(q->z_tmp, q->ce, q->z, NULL, nof_re, 1.0f, channel->noise_estimate);

    // Perform DMRS Detection, if enabled
    if (isnormal(cfg->threshold_dmrs_detection)) {
      cf_t  _dmrs_corr       = isrran_vec_acc_cc(q->ce, ISRRAN_NRE) / ISRRAN_NRE;
      float rms              = __real__(conjf(_dmrs_corr) * _dmrs_corr);
      float power            = isrran_vec_avg_power_cf(q->ce, ISRRAN_NRE);
...
char* isrran_pucch_format_text(isrran_pucch_format_t format)
{
  char* ret = NULL;
  ...
  switch (format) {
    ...
    case ISRRAN_PUCCH_FORMAT_3:
      ret = "Format 3";
      break;
    case ISRRAN_PUCCH_FORMAT_ERROR:
    default:
      ret = "Format Error";
  }
...
char* isrran_pucch_format_text_short(isrran_pucch_format_t format)
{
  char* ret = NULL;
...
uint32_t isrran_pucch_nof_ack_format(isrran_pucch_format_t format)
{
  uint32_t ret = 0;
...
bool isrran_pucch_cfg_isvalid(isrran_pucch_cfg_t* cfg, uint32_t nof_prb)
{
...
uint32_t isrran_pucch_n_prb(const isrran_cell_t* cell, const isrran_pucch_cfg_t* cfg, uint32_t ns)
{
  uint32_t m = isrran_pucch_m(cfg, cell->cp);
  // Determine n_prb
  uint32_t n_prb = m / 2;
  if ((m + ns) % 2) {
...
uint32_t isrran_pucch_m(const isrran_pucch_cfg_t* cfg, isrran_cp_t cp)
{
  uint32_t m = 0;
  switch (cfg->format) {
    case ISRRAN_PUCCH_FORMAT_1:
    ...
    case ISRRAN_PUCCH_FORMAT_1B:
      ...
      uint32_t c = ISRRAN_CP_ISNORM(cp) ? 3 : 2;
      if (cfg->n_pucch >= c * cfg->N_cs / cfg->delta_pucch_shift) {
        m = (cfg->n_pucch - c * cfg->N_cs / cfg->delta_pucch_shift) / (c * ISRRAN_NRE / cfg->delta_pucch_shift) +
...
int isrran_pucch_n_cs_cell(isrran_cell_t cell, uint32_t n_cs_cell[ISRRAN_NSLOTS_X_FRAME][ISRRAN_CP_NORM_NSYMB])
{
  isrran_sequence_t seq;
  bzero(&seq, sizeof(isrran_sequence_t));

  isrran_sequence_LTE_pr(&seq, 8 * ISRRAN_CP_NSYMB(cell.cp) * ISRRAN_NSLOTS_X_FRAME, cell.id);

  for (uint32_t ns = 0; ns < ISRRAN_NSLOTS_X_FRAME; ns++) {
    for (uint32_t l = 0; l < ISRRAN_CP_NSYMB(cell.cp); l++) {
      n_cs_cell[ns][l] = 0;
      for (uint32_t i = 0; i < 8; i++) {
        n_cs_cell[ns][l] += seq.c[8 * ISRRAN_CP_NSYMB(cell.cp) * ns + 8 * l + i] << i;
      }
    }
  }
...
int isrran_pucch_collision(const isrran_cell_t* cell, const isrran_pucch_cfg_t* cfg1, const isrran_pucch_cfg_t* cfg2)
{
  ...
  uint32_t m1 = isrran_pucch_m(cfg1, cell->cp);
  uint32_t m2 = isrran_pucch_m(cfg2, cell->cp);
  ...
  isrran_pucch_n_cs_cell(*cell, n_cs_cell);
  ...
  switch (cfg1->format) {
    ...
    case ISRRAN_PUCCH_FORMAT_1A:
    case ISRRAN_PUCCH_FORMAT_1B:
      isrran_pucch_alpha_format1(n_cs_cell, cfg1, cell->cp, false, 0, 0, &n_oc1, &n_prime1);
      isrran_pucch_alpha_format1(n_cs_cell, cfg2, cell->cp, false, 0, 0, &n_oc2, &n_prime2);
      return ((n_oc1 == n_oc2) && (n_prime1 % 2 == n_prime2 % 2)) ? ISRRAN_ERROR : ISRRAN_SUCCESS;

    case ISRRAN_PUCCH_FORMAT_2:
    case ISRRAN_PUCCH_FORMAT_2A:
    case ISRRAN_PUCCH_FORMAT_2B:
      alpha1 = isrran_pucch_alpha_format2(n_cs_cell, cfg1, 0, 0);
      alpha2 = isrran_pucch_alpha_format2(n_cs_cell, cfg2, 0, 0);
      return (alpha1 == alpha2) ? ISRRAN_ERROR : ISRRAN_SUCCESS;

    case ISRRAN_PUCCH_FORMAT_3:
      return (cfg1->n_pucch % 5 == cfg2->n_pucch % 5) ? ISRRAN_ERROR : ISRRAN_SUCCESS;
...
int isrran_pucch_cfg_assert(const isrran_cell_t* cell, const isrran_pucch_cfg_t* cfg)
{
  ...
  isrran_pucch_cfg_t cfg1 = *cfg;
  isrran_pucch_cfg_t cfg2 = *cfg;

  // Set Format 1b
  cfg1.format = ISRRAN_PUCCH_FORMAT_1B;
  cfg2.format = ISRRAN_PUCCH_FORMAT_1B;

  // Check collision with N_pucch_1 Vs n_pucch_sr
  cfg1.n_pucch = cfg->N_pucch_1;
  cfg2.n_pucch = cfg->n_pucch_sr;
  if (isrran_pucch_collision(cell, &cfg1, &cfg2) != ISRRAN_SUCCESS) {
    return ISRRAN_ERROR;
  }

  if (cfg->ack_nack_feedback_mode == ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_CS) {
    // Checks channel selection resources do not collide with N_pucch_1
    for (uint32_t i = 0; i < ISRRAN_PUCCH_SIZE_AN_CS; i++) {
      for (uint32_t j = 0; j < ISRRAN_PUCCH_NOF_AN_CS; j++) {
        ...
        cfg1.n_pucch = cfg2.n1_pucch_an_cs[i][(j + 1) % ISRRAN_PUCCH_NOF_AN_CS];
        if (isrran_pucch_collision(cell, &cfg1, &cfg2) != ISRRAN_SUCCESS) {
          return ISRRAN_ERROR;
        }
      }
...
float isrran_pucch_alpha_format1(const uint32_t            n_cs_cell[ISRRAN_NSLOTS_X_FRAME][ISRRAN_CP_NORM_NSYMB],
...
{
  uint32_t c       = ISRRAN_CP_ISNORM(cp) ? 3 : 2;
  uint32_t N_prime = (cfg->n_pucch < c * cfg->N_cs / cfg->delta_pucch_shift) ? cfg->N_cs : ISRRAN_NRE;

  uint32_t n_prime = cfg->n_pucch;
  if (cfg->n_pucch >= c * cfg->N_cs / cfg->delta_pucch_shift) {
    n_prime = (cfg->n_pucch - c * cfg->N_cs / cfg->delta_pucch_shift) % (c * ISRRAN_NRE / cfg->delta_pucch_shift);
  }
  if (ns % 2) {
    if (cfg->n_pucch >= c * cfg->N_cs / cfg->delta_pucch_shift) {
      n_prime = (c * (n_prime + 1)) % (c * ISRRAN_NRE / cfg->delta_pucch_shift + 1) - 1;
    } else {
      uint32_t d = ISRRAN_CP_ISNORM(cp) ? 2 : 0;
  ...
  uint32_t n_oc_div = (!is_dmrs && ISRRAN_CP_ISEXT(cp)) ? 2 : 1;

  uint32_t n_oc = (n_prime * cfg->delta_pucch_shift) / N_prime;
  if (!is_dmrs && ISRRAN_CP_ISEXT(cp)) {
    n_oc *= 2;
  }
  if (n_oc_ptr) {
    *n_oc_ptr = n_oc;
  ...
  if (ISRRAN_CP_ISNORM(cp)) {
    ...
    n_cs = (n_cs_cell[ns][l] + (n_prime * cfg->delta_pucch_shift + n_oc / n_oc_div) % N_prime) % ISRRAN_NRE;
  }
  ...
  return 2 * M_PI * (n_cs) / ISRRAN_NRE;
}

/* Calculates alpha for format 2/a/b according to 5.4.2 of 36.211 */
float isrran_pucch_alpha_format2(const uint32_t            n_cs_cell[ISRRAN_NSLOTS_X_FRAME][ISRRAN_CP_NORM_NSYMB],
                                 const isrran_pucch_cfg_t* cfg,
                                 uint32_t                  ns,
                                 uint32_t                  l)
{
  uint32_t n_prime = cfg->n_pucch % ISRRAN_NRE;
  if (cfg->n_pucch >= ISRRAN_NRE * cfg->n_rb_2) {
    n_prime = (cfg->n_pucch + cfg->N_cs + 1) % ISRRAN_NRE;
  }
  if (ns % 2) {
    n_prime = (ISRRAN_NRE * (n_prime + 1)) % (ISRRAN_NRE + 1) - 1;
    if (cfg->n_pucch >= ISRRAN_NRE * cfg->n_rb_2) {
      int x = (ISRRAN_NRE - 2 - (int)cfg->n_pucch) % ISRRAN_NRE;
      if (x >= 0) {
        n_prime = (uint32_t)x;
      } else {
        n_prime = ISRRAN_NRE + x;
      }
    }
  }
  uint32_t n_cs  = (n_cs_cell[ns][l] + n_prime) % ISRRAN_NRE;
  float    alpha = 2 * M_PI * (n_cs) / ISRRAN_NRE;
...
int isrran_pucch_format2ab_mod_bits(isrran_pucch_format_t format, uint8_t bits[2], cf_t* d_10)
{
  if (d_10) {
    if (format == ISRRAN_PUCCH_FORMAT_2A) {
      *d_10 = bits[0] ? -1.0 : 1.0;
      return ISRRAN_SUCCESS;
    } else if (format == ISRRAN_PUCCH_FORMAT_2B) {
      if (bits[0] == 0) {
        if (bits[1] == 0) {
          *d_10 = 1.0;
        } else {
...
void isrran_pucch_tx_info(isrran_pucch_cfg_t* cfg, isrran_uci_value_t* uci_data, char* str, uint32_t str_len)
{
  uint32_t n = isrran_print_check(str,
                                  str_len,
                                  0,
                                  "rnti=0x%x, f=%s, n_pucch=%d",
                                  cfg->rnti,
                                  isrran_pucch_format_text_short(cfg->format),
                                  cfg->n_pucch);

  if (uci_data) {
    isrran_uci_data_info(&cfg->uci_cfg, uci_data, &str[n], str_len - n);
  }
}

void isrran_pucch_rx_info(isrran_pucch_cfg_t* cfg, isrran_pucch_res_t* pucch_res, char* str, uint32_t str_len)
{
  uint32_t n = isrran_print_check(str,
                                  str_len,
                                  0,
  ...
  if (pucch_res) {
    if (isnormal(cfg->threshold_dmrs_detection)) {
      n = isrran_print_check(
          str, str_len, n, ", dmrs_corr=%.3f, snr=%.1f dB", pucch_res->dmrs_correlation, pucch_res->snr_db);
    }

    n = isrran_print_check(str, str_len, n, ", corr=%.3f", pucch_res->correlation);

    n += isrran_uci_data_info(&cfg->uci_cfg, &pucch_res->uci_data, &str[n], str_len - n);

    if (pucch_res->ta_valid) {
      n = isrran_print_check(str, str_len, n, ", ta=%.1f us", pucch_res->ta_us);
    }
  }
}
