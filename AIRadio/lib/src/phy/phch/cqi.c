 ...
 *            PACKING FUNCTIONS                        *
 *******************************************************/
static int cqi_hl_subband_pack(isrran_cqi_cfg_t* cfg, isrran_cqi_hl_subband_t* msg, uint8_t* buff)
{
  uint8_t* body_ptr  = buff;
  uint32_t bit_count = 0;

  /* Unpack codeword 0, common for 3GPP 36.212 Tables 5.2.2.6.2-1 and 5.2.2.6.2-2 */
  isrran_bit_unpack(msg->wideband_cqi_cw0, &body_ptr, 4);
  isrran_bit_unpack(msg->subband_diff_cqi_cw0, &body_ptr, 2 * cfg->N);
  bit_count += 4 + 2 * cfg->N;

  /* Unpack codeword 1, 3GPP 36.212 Table 5.2.2.6.2-2 */
  if (cfg->rank_is_not_one) {
    isrran_bit_unpack(msg->wideband_cqi_cw1, &body_ptr, 4);
    isrran_bit_unpack(msg->subband_diff_cqi_cw1, &body_ptr, 2 * cfg->N);
    bit_count += 4 + 2 * cfg->N;
  }
  ...
  if (cfg->pmi_present) {
    if (cfg->four_antenna_ports) {
      isrran_bit_unpack(msg->pmi, &body_ptr, 4);
      bit_count += 4;
    } else {
      if (cfg->rank_is_not_one) {
        isrran_bit_unpack(msg->pmi, &body_ptr, 1);
        bit_count += 1;
      } else {
        isrran_bit_unpack(msg->pmi, &body_ptr, 2);
        bit_count += 2;
      }
    }
  }
...
static int cqi_ue_subband_pack(isrran_cqi_cfg_t* cfg, isrran_cqi_ue_diff_subband_t* msg, uint8_t* buff)
{
  uint8_t* body_ptr = buff;
  isrran_bit_unpack(msg->wideband_cqi, &body_ptr, 4);
  isrran_bit_unpack(msg->subband_diff_cqi, &body_ptr, 2);
  isrran_bit_unpack(msg->subband_diff_cqi, &body_ptr, cfg->L);

  return 4 + 2 + cfg->L;
}

/* Pack CQI following 3GPP TS 36.212 Tables 5.2.3.3.1-1 and 5.2.3.3.1-2 */
static int cqi_format2_wideband_pack(isrran_cqi_cfg_t* cfg, isrran_cqi_format2_wideband_t* msg, uint8_t* buff)
{
  uint8_t* body_ptr = buff;
  ...
  if (cfg->pmi_present) {
    if (cfg->four_antenna_ports) {
      if (cfg->rank_is_not_one) {
        isrran_bit_unpack(msg->spatial_diff_cqi, &body_ptr, 3);
      }
      isrran_bit_unpack(msg->pmi, &body_ptr, 4);
    } else {
      if (cfg->rank_is_not_one) {
...
static int cqi_format2_subband_pack(isrran_cqi_cfg_t* cfg, isrran_cqi_ue_subband_t* msg, uint8_t* buff)
{
  uint8_t* body_ptr = buff;
  isrran_bit_unpack(msg->subband_cqi, &body_ptr, 4);
  isrran_bit_unpack(msg->subband_label, &body_ptr, cfg->subband_label_2_bits ? 2 : 1);
  return 4 + ((cfg->subband_label_2_bits) ? 2 : 1);
}

int isrran_cqi_value_pack(isrran_cqi_cfg_t* cfg, isrran_cqi_value_t* value, uint8_t buff[ISRRAN_CQI_MAX_BITS])
{
  switch (cfg->type) {
    case ISRRAN_CQI_TYPE_WIDEBAND:
      return cqi_format2_wideband_pack(cfg, &value->wideband, buff);
    case ISRRAN_CQI_TYPE_SUBBAND_UE:
      return cqi_format2_subband_pack(cfg, &value->subband_ue, buff);
    case ISRRAN_CQI_TYPE_SUBBAND_UE_DIFF:
      return cqi_ue_subband_pack(cfg, &value->subband_ue_diff, buff);
    case ISRRAN_CQI_TYPE_SUBBAND_HL:
...
int cqi_hl_subband_unpack(isrran_cqi_cfg_t* cfg, uint8_t* buff, isrran_cqi_hl_subband_t* msg)
{
  uint8_t* body_ptr  = buff;
  uint32_t bit_count = 0;

  msg->wideband_cqi_cw0     = (uint8_t)isrran_bit_pack(&body_ptr, 4);
  msg->subband_diff_cqi_cw0 = isrran_bit_pack(&body_ptr, 2 * cfg->N);
  bit_count += 4 + 2 * cfg->N;

  /* Unpack codeword 1, 3GPP 36.212 Table 5.2.2.6.2-2 */
  if (cfg->rank_is_not_one) {
    msg->wideband_cqi_cw1     = (uint8_t)isrran_bit_pack(&body_ptr, 4);
    msg->subband_diff_cqi_cw1 = isrran_bit_pack(&body_ptr, 2 * cfg->N);
    bit_count += 4 + 2 * cfg->N;
  }
  ...
  if (cfg->pmi_present) {
    if (cfg->four_antenna_ports) {
      msg->pmi = (uint8_t)isrran_bit_pack(&body_ptr, 4);
      bit_count += 4;
    } else {
      if (cfg->rank_is_not_one) {
        msg->pmi = (uint8_t)isrran_bit_pack(&body_ptr, 1);
        bit_count += 1;
      } else {
        msg->pmi = (uint8_t)isrran_bit_pack(&body_ptr, 2);
        bit_count += 2;
      }
    }
  }
...
int cqi_ue_subband_unpack(isrran_cqi_cfg_t* cfg, uint8_t* buff, isrran_cqi_ue_diff_subband_t* msg)
{
  uint8_t* body_ptr     = buff;
  msg->wideband_cqi     = isrran_bit_pack(&body_ptr, 4);
  msg->subband_diff_cqi = isrran_bit_pack(&body_ptr, 2);
  msg->subband_diff_cqi = isrran_bit_pack(&body_ptr, cfg->L);

  return 4 + 2 + cfg->L;
}

/* Unpack CQI following 3GPP TS 36.212 Tables 5.2.3.3.1-1 and 5.2.3.3.1-2 */
static int cqi_format2_wideband_unpack(isrran_cqi_cfg_t* cfg, uint8_t* buff, isrran_cqi_format2_wideband_t* msg)
{
  uint8_t* body_ptr = buff;
  msg->wideband_cqi = (uint8_t)isrran_bit_pack(&body_ptr, 4);

  if (cfg->pmi_present) {
    if (cfg->four_antenna_ports) {
      if (cfg->rank_is_not_one) {
        msg->spatial_diff_cqi = (uint8_t)isrran_bit_pack(&body_ptr, 3);
      }
      msg->pmi = (uint8_t)isrran_bit_pack(&body_ptr, 4);
    } else {
      if (cfg->rank_is_not_one) {
...
static int cqi_format2_subband_unpack(isrran_cqi_cfg_t* cfg, uint8_t* buff, isrran_cqi_ue_subband_t* msg)
{
  uint8_t* body_ptr  = buff;
  msg->subband_cqi   = isrran_bit_pack(&body_ptr, 4);
  msg->subband_label = isrran_bit_pack(&body_ptr, cfg->subband_label_2_bits ? 2 : 1);
  return 4 + ((cfg->subband_label_2_bits) ? 2 : 1);
}

int isrran_cqi_value_unpack(isrran_cqi_cfg_t* cfg, uint8_t buff[ISRRAN_CQI_MAX_BITS], isrran_cqi_value_t* value)
{
  switch (cfg->type) {
    case ISRRAN_CQI_TYPE_WIDEBAND:
...
static int
cqi_format2_wideband_tostring(isrran_cqi_cfg_t* cfg, isrran_cqi_format2_wideband_t* msg, char* buff, uint32_t buff_len)
{
  int n = 0;
...
static int
cqi_format2_subband_tostring(isrran_cqi_cfg_t* cfg, isrran_cqi_ue_subband_t* msg, char* buff, uint32_t buff_len)
{
  int n = 0;
...
static int
cqi_ue_subband_tostring(isrran_cqi_cfg_t* cfg, isrran_cqi_ue_diff_subband_t* msg, char* buff, uint32_t buff_len)
{
  int n = 0;
...
static int cqi_hl_subband_tostring(isrran_cqi_cfg_t* cfg, isrran_cqi_hl_subband_t* msg, char* buff, uint32_t buff_len)
{
  int n = 0;
...
int isrran_cqi_value_tostring(isrran_cqi_cfg_t* cfg, isrran_cqi_value_t* value, char* buff, uint32_t buff_len)
{
  int ret = -1;
...
int isrran_cqi_size(isrran_cqi_cfg_t* cfg)
{
  int size = 0;
...
uint32_t isrran_cqi_get_sb_idx(uint32_t                       tti,
                               uint32_t                       subband_label,
                               const isrran_cqi_report_cfg_t* cqi_report_cfg,
                               const isrran_cell_t*           cell)
{
  uint32_t bw_part_idx = isrran_cqi_periodic_sb_bw_part_idx(cqi_report_cfg, tti, cell->nof_prb, cell->frame_type);
  return subband_label + bw_part_idx * cqi_sb_get_Nj(bw_part_idx, cell->nof_prb);
}

int isrran_cqi_hl_get_L(int nof_prb)
{
  ...
  return (int)ISRRAN_CEIL_LOG2((double)nof_prb / (subband_size * bwp_J));
}

bool isrran_cqi_periodic_ri_send(const isrran_cqi_report_cfg_t* cfg, uint32_t tti, isrran_frame_type_t frame_type)
{
  return cfg->periodic_configured && cfg->ri_idx_present &&
         ri_send(cfg->pmi_idx, cfg->ri_idx, tti, frame_type == ISRRAN_FDD);
}

bool isrran_cqi_periodic_send(const isrran_cqi_report_cfg_t* cfg, uint32_t tti, isrran_frame_type_t frame_type)
{
  return cfg->periodic_configured && cqi_send(cfg->pmi_idx, tti, frame_type == ISRRAN_FDD, 1);
}

uint32_t cqi_sb_get_H(const isrran_cqi_report_cfg_t* cfg, uint32_t nof_prb)
{
...
bool isrran_cqi_periodic_is_subband(const isrran_cqi_report_cfg_t* cfg,
                                    uint32_t                       tti,
                                    uint32_t                       nof_prb,
                                    isrran_frame_type_t            frame_type)
{
  ...
  return isrran_cqi_periodic_send(cfg, tti, frame_type) && !cqi_send(cfg->pmi_idx, tti, frame_type == ISRRAN_FDD, H);
}

uint32_t isrran_cqi_periodic_sb_bw_part_idx(const isrran_cqi_report_cfg_t* cfg,
                                            uint32_t                       tti,
                                            uint32_t                       nof_prb,
                                            isrran_frame_type_t            frame_type)
{
...
float isrran_cqi_to_coderate(uint32_t cqi, bool use_alt_table)
{
  if (cqi < 16) {
...
uint8_t isrran_cqi_from_snr(float snr)
{
 ...
 */
int isrran_cqi_hl_get_no_subbands(int nof_prb)
{
...
void isrran_cqi_to_str(const uint8_t* cqi_value, int cqi_len, char* str, int str_len)
{
  int i = 0;
