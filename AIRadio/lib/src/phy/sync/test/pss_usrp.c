...
int main(int argc, char** argv)
{
  ...
  while (frame_cnt < nof_frames || nof_frames == -1) {
    n = isrran_rf_recv(&rf, buffer, flen - peak_offset, 1);
    if (n < 0) {
      ...
      if (peak_idx >= fft_size) {
        ...
        if (!tdd_mode) {
          ...
          sss_idx = peak_idx - 4 * fft_size -
                    3 * (ISRRAN_CP_ISNORM(cp) ? ISRRAN_CP_LEN(fft_size, ISRRAN_CP_NORM_LEN)
                                              : ISRRAN_CP_LEN(fft_size, ISRRAN_CP_EXT_LEN));
        }
        if (sss_idx >= 0 && sss_idx < flen - fft_size) {
          // Filter SSS
          isrran_pss_filter(&pss, &buffer[sss_idx], &buffer[sss_idx]);

          INFO("Full N_id_1: %d", isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value));
          isrran_sss_m0m1_partial(&sss, &buffer[sss_idx], 1, ce, &m0, &m0_value, &m1, &m1_value);
          ...
          isrran_sss_m0m1_diff_coh(&sss, &buffer[sss_idx], ce, &m0, &m0_value, &m1, &m1_value);
          if (isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value) != N_id_1) {
            sss_error3++;
          }
    ...
    if (frame_cnt > 100) {
      ...
      sfo = ISRRAN_VEC_CMA((peak_idx - last_peak) / 5e-3, sfo, frame_cnt);
