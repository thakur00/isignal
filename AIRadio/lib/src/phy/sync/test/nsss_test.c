...
int      sfn        = 0;
uint32_t n_id_ncell = ISRRAN_CELL_ID_UNKNOWN;
...
#define SFLEN (1 * ISRRAN_SF_LEN(ISRRAN_NBIOT_FFT_SIZE))
...
int main(int argc, char** argv)
{
  ...
  buffer = isrran_vec_cf_malloc(SFLEN * max_num_sf);
  if (!buffer) {
    perror("malloc");
    return ret;
  }
  ...
  fft_buffer = isrran_vec_cf_malloc(SFLEN * max_num_sf);
  if (!fft_buffer) {
    perror("malloc");
    goto exit;
  }
  ...
  if (isrran_ofdm_tx_init(&ifft, ISRRAN_CP_NORM, buffer, fft_buffer, NOF_PRB)) {
  ...
  if (input_file_name != NULL) {
    ...
    // offset file
    int file_offset = 0;
    isrran_filesource_read(&fsrc, buffer, file_offset);

    // now read
    while (num_sf < max_num_sf) {
      int n = isrran_filesource_read(&fsrc, &fft_buffer[num_sf * SFLEN], SFLEN);
      if (n < 0) {
  ...
  if (isrran_nsss_synch_init(&syncobj, num_sf * SFLEN, fft_size)) {
  ...
  if (!input_file_name) {
    ...
    cf_t nsss_signals[ISRRAN_NSSS_TOT_LEN] = {};
    isrran_nsss_generate(nsss_signals, n_id_ncell == ISRRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell);

#if DUMP_SIGNALS
    isrran_vec_save_file("nsss_signal_freq.bin", nsss_signals, ISRRAN_NSSS_LEN * sizeof(cf_t));
#endif

    isrran_nsss_put_subframe(
        &syncobj, nsss_signals, buffer, sfn, ISRRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL, ISRRAN_NBIOT_DEFAULT_PRB_OFFSET);
  ...
  uint32_t n_id_ncell_detected = ISRRAN_CELL_ID_UNKNOWN;
  uint32_t sfn_partial         = 0;
  isrran_nsss_sync_find(&syncobj, fft_buffer, &peak_value, &n_id_ncell_detected, &sfn_partial);
  printf("NSSS with peak=%f, n_id_ncell: %d, partial SFN: %x\n", peak_value, n_id_ncell_detected, sfn_partial);

  if (n_id_ncell_detected == (n_id_ncell == ISRRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell)) {
    printf("Ok\n");
    ret = ISRRAN_SUCCESS;
  } else {
    printf("Failed\n");
...
#define MAX_FNAME_LEN 40
  ...
  isrran_vec_save_file(fname, fft_buffer, num_sf * SFLEN * sizeof(cf_t));
  isrran_vec_save_file("nsss_corr_seq_time.bin",
                       syncobj.nsss_signal_time[n_id_ncell == ISRRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell],
                       ISRRAN_NSSS_CORR_FILTER_LEN * sizeof(cf_t));
  if (n_id_ncell_detected != ISRRAN_CELL_ID_UNKNOWN) {
    // run correlation again with found cell to populate conv_output_abs
    isrran_nsss_sync_find(&syncobj, fft_buffer, &peak_value, &n_id_ncell_detected, &sfn_partial);
  }
  isrran_vec_save_file("nsss_find_conv_output_abs.bin",
                       syncobj.conv_output_abs,
                       (ISRRAN_NSSS_CORR_FILTER_LEN + num_sf * SFLEN) * sizeof(float));
  ...
  // cleanup
exit:
  isrran_nsss_synch_free(&syncobj);
...
void write_to_file()
{
  ...
  fprintf(debug_fsink.f, "corr_isrran = read_real('nsss_find_conv_output_abs.bin', max_len);\n");
  fprintf(debug_fsink.f, "t=1:length(corr_isrran);\n");
  fprintf(debug_fsink.f, "figure;\n");
  fprintf(debug_fsink.f, "plot(t,corr_isrran);\n");
  fprintf(debug_fsink.f, "xlabel('sample index');\n");
