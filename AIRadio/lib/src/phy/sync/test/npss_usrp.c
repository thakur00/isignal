...
isrran_nbiot_cell_t cell = {
    .base       = {.nof_prb = 1, .cp = ISRRAN_CP_NORM, .nof_ports = 1, .id = 0},
    .nbiot_prb  = 0,
    .n_id_ncell = 0,
};
...
int main(int argc, char** argv)
{
  ...
  if (isrran_cfo_init(&cfocorr, flen)) {
    fprintf(stderr, "Error initiating CFO\n");
    exit(-1);
  }
  isrran_cfo_set_tol(&cfocorr, 50.0 / (15000.0 * fft_size));
  ...
  isrran_vec_cf_zero(buffer, flen * 2);

  if (isrran_npss_synch_init(&npss, flen, fft_size)) {
  ...
  while ((frame_cnt < nof_frames || nof_frames == -1) && !go_exit) {
    ...
    if (save_frame_to_file && save_and_exit) {
      ...
      isrran_vec_save_file(filename, buffer, flen * sizeof(cf_t));
      go_exit = true;
    }

    // perform CFO correction
    if (has_cfo_corr) {
      isrran_cfo_correct(&cfocorr, buffer, buffer, -cfo_fixed / (15000 * fft_size));
    }

    peak_idx = isrran_npss_sync_find(&npss, buffer, &peak_value);
    if (peak_idx < 0) {
    ...
    if (peak_value >= threshold) {
      ...
      if (save_frame_to_file && !save_and_exit) {
        ...
        int num_drop = peak_idx - ISRRAN_NPSS_CORR_OFFSET + flen / 2;
        ...
        isrran_rf_recv(&rf, dummy, num_drop, 1);
        save_and_exit = true;
      }
    } else {
      nof_nodet++;
    ...
    printf("[%5d]: Pos: %5d, PSR: %4.1f (~%4.1f) Pdet: %4.2f, "
           "FA: %4.2f\r",
           frame_cnt,
           (peak_value > threshold) ? (peak_idx - ISRRAN_NPSS_CORR_OFFSET - flen / 10 / 2) : 0,
           peak_value,
           mean_peak,
...
#ifndef DISABLE_GRAPHICS
    if (!disable_plots) {
      int len = ISRRAN_NPSS_CORR_FILTER_LEN + npss.frame_size - 1;
