...
int main(int argc, char** argv)
{
  ...
  input = isrran_vec_cf_malloc(num_samples);
  if (!input) {
    perror("malloc");
    exit(-1);
  }
  output = isrran_vec_cf_malloc(num_samples);
  if (!output) {
    perror("malloc");
    exit(-1);
  }
  ...
  if (isrran_cfo_init(&cfocorr, num_samples)) {
    ERROR("Error initiating CFO");
    return -1;
  }

  isrran_cfo_correct(&cfocorr, output, output, freq);
  isrran_cfo_correct(&cfocorr, output, output, -freq);
