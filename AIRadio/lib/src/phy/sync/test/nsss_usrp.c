...
int main(int argc, char** argv)
{
  ...
  buffer = isrran_vec_cf_malloc(input_len * 2);
  if (!buffer) {
    perror("malloc");
    exit(-1);
  }

  if (isrran_nsss_synch_init(&nsss, input_len, fft_size)) {
  ...
  if (has_cfo_corr) {
    ...
    if (isrran_cfo_init(&cfocorr, input_len)) {
  ...
  uint32_t cell_id = ISRRAN_CELL_ID_UNKNOWN;
  uint32_t sfn_partial;
  isrran_nsss_sync_find(&nsss, buffer, &nsss_peak_value, &cell_id, &sfn_partial);
