...
static void run_channel()
{
  ...
  // CFO
  isrran_vec_apply_cfo(buffer, cfo_hz / srate_hz, buffer, sf_len);

  // AWGN
  isrran_channel_awgn_run_c(&awgn, buffer, buffer, sf_len);
}

static int assert_measure(const isrran_csi_trs_measurements_t* meas)
{
...
static int test_case_1(isrran_ssb_t* ssb)
{
  ...
  for (uint32_t pci = 0; pci < ISRRAN_NOF_NID_NR; pci++) {
    struct timeval t[3] = {};
    ...
    TESTASSERT(isrran_ssb_add(ssb, pci, &pbch_msg, buffer, buffer) == ISRRAN_SUCCESS);
    ...
    isrran_csi_trs_measurements_t meas_search = {};
    TESTASSERT(isrran_ssb_csi_search(ssb, buffer, sf_len, &N_id_found, &meas_search) == ISRRAN_SUCCESS);
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
    ...
    // Print measurement
    char str[512] = {};
    isrran_csi_meas_info(&meas_search, str, sizeof(str));
    ...
    TESTASSERT(isrran_ssb_csi_measure(ssb, pci, 0, buffer, &meas) == ISRRAN_SUCCESS);
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
    ...
    // Assert measurements
    TESTASSERT(assert_measure(&meas) == ISRRAN_SUCCESS);
  }

  INFO("test_case_1 - %.1f usec/encode; %.1f usec/search; Max srate %.1f MSps; %.1f usec/measurement",
       (double)t_add_usec / (double)ISRRAN_NOF_NID_NR,
       (double)t_find_usec / (double)ISRRAN_NOF_NID_NR,
       (double)sf_len * (double)ISRRAN_NOF_NID_NR / (double)t_find_usec,
       (double)t_meas_usec / (double)ISRRAN_NOF_NID_NR);
...
int main(int argc, char** argv)
{
  ...
  if (test_case_1(&ssb) != ISRRAN_SUCCESS) {
