...
isrran_nbiot_cell_t cell = {
    .base           = {.nof_prb = 1, .cp = ISRRAN_CP_NORM, .id = 0},
    .base.nof_ports = 1,
    .nbiot_prb      = 0,
};
...
int main(int argc, char** argv)
{
  ...
  while (frame_cnt < nof_frames || nof_frames == -1) {
    ...
    if (peak_value >= threshold) {
      ...
      if (save_frame_to_file && !save_and_exit) {
        ...
        isrran_filesource_read(&fsrc, dummy, num_drop);
        save_and_exit = true;
      }
  ...
  isrran_vec_save_file(
      "npss_find_conv_output_abs.bin", npss.conv_output_abs, (FLEN + ISRRAN_NPSS_CORR_FILTER_LEN) * sizeof(float));
  isrran_vec_save_file("npss_corr_seq_time.bin", npss.npss_signal_time, ISRRAN_NPSS_CORR_FILTER_LEN * sizeof(cf_t));
