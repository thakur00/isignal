...
int main(int argc, char** argv)
{
  ...
  int32_t symbol_sz = isrran_symbol_sz(nof_prb);
  printf("Symbol SZ: %i\n", symbol_sz);
  ...
  cf_t* input_buffer_temp = isrran_vec_cf_malloc(sf_n_samples);

  uint32_t output_buffer_len = 0;
  cf_t*    output_buffer     = isrran_vec_cf_malloc(sf_n_samples);

  isrran_ofdm_t ifft;
  if (isrran_ofdm_tx_init(&ifft, cp, sf_buffer, output_buffer, nof_prb)) {
  ...
  isrran_psss_t psss;
  isrran_psss_init(&psss, nof_prb, cp);

  isrran_ssss_t ssss;
  isrran_ssss_init(&ssss, nof_prb, cp, tm);
  ...
  if (input_file_name) {
    ...
    isrran_psss_put_sf_buffer(psss.psss_signal[((N_sl_id < 168) ? 0 : 1)], sf_buffer, nof_prb, cp);
    isrran_ssss_put_sf_buffer(ssss.ssss_signal[N_sl_id], sf_buffer, nof_prb, cp);
    ...
    isrran_vec_cf_zero(&sf_buffer[ISRRAN_NRE * nof_prb * (ISRRAN_CP_NSYMB(cp) * 2 - 1)], ISRRAN_NRE * nof_prb);
    ...
    if (snr < 50) {
      ...
      isrran_ch_awgn_c(output_buffer, output_buffer, var, output_buffer_len);
    }
  ...
  int search_window_size = sf_n_samples - (2 * (symbol_sz + ISRRAN_CP_SZ(symbol_sz, cp)));
  ...
  while (sync == false) {
    ...
    if (offset < samples_to_read) {
      isrran_vec_cf_zero(input_buffer, offset);
      samples_to_read = sf_n_samples - offset_pos - offset;

      if (input_file_name) {
        // Read and normalize samples from file
        samples_read = isrran_filesource_read(&fsrc, &input_buffer[offset + offset_pos], samples_to_read);
      ...
      isrran_vec_cf_zero(input_buffer, samples_read);
    }
    ...
    if (isrran_psss_find(&psss, input_buffer, nof_prb, cp) == ISRRAN_SUCCESS) {
      ...
      if (psss.corr_peak_pos - sf_n_samples == 0) {
        // Find SSSS
        if (isrran_ssss_find(&ssss, input_buffer, nof_prb, psss.N_id_2, cp) == ISRRAN_SUCCESS) {
