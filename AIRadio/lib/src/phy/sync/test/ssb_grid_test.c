...
static uint32_t        grid_bw_sc = 52 * ISRRAN_NRE; // Resource grid bandwidth in subcarriers
...
static void gen_pbch_msg(isrran_pbch_msg_nr_t* pbch_msg)
{
  // Default all to zero
...
static int test_case(isrran_ssb_t* ssb)
{
  // SSB configuration
  ...
  TESTASSERT(isrran_ssb_put_grid(ssb, pci, &pbch_msg_tx, grid, grid_bw_sc) == ISRRAN_SUCCESS);

  // Decode
  isrran_pbch_msg_nr_t pbch_msg_rx = {};
  TESTASSERT(isrran_ssb_decode_grid(ssb, pci, pbch_msg_tx.hrf, pbch_msg_tx.ssb_idx, grid, grid_bw_sc, &pbch_msg_rx) ==
             ISRRAN_SUCCESS);
...
int main(int argc, char** argv)
{
  ...
  srate_hz   = (double)ISRRAN_SUBC_SPACING_NR(carrier_scs) * isrran_min_symbol_sz_rb(grid_bw_sc / ISRRAN_NRE);
  grid       = isrran_vec_cf_malloc(grid_bw_sc * ISRRAN_NSYMB_PER_SLOT_NR);
  ...
  if (test_case(&ssb) != ISRRAN_SUCCESS) {
