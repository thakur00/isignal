...
#define SFLEN (10 * ISRRAN_SF_LEN(128))
...
int main(int argc, char** argv)
{
  ...
  fft_buffer = isrran_vec_cf_malloc(SFLEN * 2);
  if (!fft_buffer) {
    perror("malloc");
    goto exit;
  }
  ...
  if (isrran_cfo_init(&cfocorr, SFLEN)) {
    fprintf(stderr, "Error initiating CFO\n");
    goto exit;
  }
  // Set a CFO tolerance of approx 100 Hz
  isrran_cfo_set_tol(&cfocorr, 100.0 / (15000.0 * fft_size));

  // init synch object for a maximum SFLEN samples
  if (isrran_sync_nbiot_init(&syncobj, SFLEN, SFLEN, fft_size)) {
    fprintf(stderr, "Error initiating NPSS/NSSS\n");
    goto exit;
  }
  isrran_sync_nbiot_set_cfo_enable(&syncobj, true);

  if (input_file_name == NULL) {
    // generating test sequence
    if (isrran_ofdm_tx_init(
            &ifft, ISRRAN_CP_NORM, buffer, &fft_buffer[offset], ISRRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL)) {
    ...
    isrran_npss_put_subframe(
        &syncobj.npss, npss_signal, buffer, ISRRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL, ISRRAN_NBIOT_DEFAULT_PRB_OFFSET);
    ...
    int n = isrran_filesource_read(&fsrc, fft_buffer, SFLEN);
    if (n < 0) {
  ...
  isrran_vec_save_file(
      "npss_corr_seq_time.bin", syncobj.npss.npss_signal_time, ISRRAN_NPSS_CORR_FILTER_LEN * sizeof(cf_t));
  isrran_vec_save_file("npss_sf_time.bin", fft_buffer, SFLEN * sizeof(cf_t));
#endif

  if (cfo > 0.0) {
    float delta_freq = cfo / 15000 / ISRRAN_NBIOT_FFT_SIZE;
    printf("Adding CFO with target: %.4f\n", delta_freq);
    printf("WARNING: not working at the moment!\n");
    isrran_cfo_correct(&cfocorr, fft_buffer, fft_buffer, delta_freq);
  }
  ...
  if (snr != -1.0) {
    ...
    isrran_ch_awgn_c(fft_buffer, fft_buffer, var, SFLEN);
  }

  // look for NPSS signal
  if (isrran_sync_nbiot_find(&syncobj, fft_buffer, 0, &find_idx) < 0) {
    fprintf(stderr, "Error running isrran_sync_nbiot_find()\n");
    exit(-1);
  }
...
#ifdef DUMP_SIGNALS
  isrran_vec_save_file("npss_find_conv_output_abs.bin",
                       syncobj.npss.conv_output_abs,
                       (SFLEN + ISRRAN_NPSS_CORR_FILTER_LEN - 1) * sizeof(float));
#endif
...
exit:
  ...
  isrran_sync_nbiot_free(&syncobj);
  isrran_cfo_free(&cfocorr);
...
void write_to_file()
{
  ...
  fprintf(debug_fsink.f, "num_samples = %u;\n", ISRRAN_NPSS_CORR_FILTER_LEN + SFLEN - 1);
