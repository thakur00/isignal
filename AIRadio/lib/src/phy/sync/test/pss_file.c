...
#ifdef ENABLE_GUI
void init_plots();
void do_plots(float* corr, float energy, uint32_t size, cf_t ce[ISRRAN_PSS_LEN]);
...
int main(int argc, char** argv)
{
  ...
  int                 frame_cnt, n;
  isrran_pss_t        pss;
  isrran_cfo_t        cfocorr, cfocorr64;
  isrran_sss_t        sss;
  int32_t             flen;
  ...
  buffer = isrran_vec_cf_malloc(flen * 2);
  if (!buffer) {
    perror("malloc");
    exit(-1);
  }

  if (isrran_pss_init_fft(&pss, flen, fft_size)) {
    ERROR("Error initiating PSS");
    exit(-1);
  }

  if (isrran_pss_set_N_id_2(&pss, N_id_2_sync)) {
    ERROR("Error setting N_id_2=%d", N_id_2_sync);
    exit(-1);
  }

  isrran_cfo_init(&cfocorr, flen);
  isrran_cfo_init(&cfocorr64, flen);

  if (isrran_sss_init(&sss, fft_size)) {
    ERROR("Error initializing SSS object");
    return ISRRAN_ERROR;
  }

  isrran_sss_set_N_id_2(&sss, N_id_2);
  ...
  isrran_sync_t ssync;
  bzero(&ssync, sizeof(isrran_sync_t));
  ssync.fft_size = fft_size;

  n = isrran_filesource_read(&fsrc, buffer, file_offset);

  while (frame_cnt < nof_frames || nof_frames == -1) {
    n = isrran_filesource_read(&fsrc, buffer, flen - peak_offset);
    if (n < 0) {
    ...
    peak_idx = isrran_pss_find_pss(&pss, buffer, &peak_value);
    if (peak_idx < 0) {
      ERROR("Error finding PSS peak");
      exit(-1);
    }

    mean_peak = ISRRAN_VEC_CMA(peak_value, mean_peak, frame_cnt);
    ...
    if (peak_value >= threshold) {
      ...
      if (peak_idx >= fft_size) {
        // Estimate CFO
        cfo      = isrran_pss_cfo_compute(&pss, &buffer[peak_idx - fft_size]);
        mean_cfo = ISRRAN_VEC_CMA(cfo, mean_cfo, frame_cnt);

        // Correct CFO
        isrran_cfo_correct(&cfocorr, buffer, buffer, -mean_cfo / fft_size);

        // Estimate channel
        if (isrran_pss_chest(&pss, &buffer[peak_idx - fft_size], ce)) {
        ...
        int sss_idx = peak_idx - 2 * fft_size -
                      (ISRRAN_CP_ISNORM(cp) ? ISRRAN_CP_LEN(fft_size, ISRRAN_CP_NORM_LEN)
                                            : ISRRAN_CP_LEN(fft_size, ISRRAN_CP_EXT_LEN));
        if (sss_idx >= 0 && sss_idx < flen - fft_size) {
          isrran_sss_m0m1_partial(&sss, &buffer[sss_idx], 3, NULL, &m0, &m0_value, &m1, &m1_value);
          if (isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value) != N_id_1) {
            sss_error2++;
          }
          INFO("sf_idx = %d", isrran_sss_subframe(m0, m1));
          INFO("Partial N_id_1: %d", isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value));
          isrran_sss_m0m1_diff(&sss, &buffer[sss_idx], &m0, &m0_value, &m1, &m1_value);
          if (isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value) != N_id_1) {
            sss_error3++;
          }
          INFO("Diff N_id_1: %d", isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value));
          isrran_sss_m0m1_partial(&sss, &buffer[sss_idx], 1, NULL, &m0, &m0_value, &m1, &m1_value);
          if (isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value) != N_id_1) {
            sss_error1++;
          }
          INFO("Full N_id_1: %d", isrran_sss_N_id_1(&sss, m0, m1, m1_value + m0_value));
        }

        // Estimate CP
        if (peak_idx > 2 * (fft_size + ISRRAN_CP_LEN_EXT(fft_size))) {
          isrran_cp_t cp = isrran_sync_detect_cp(&ssync, buffer, peak_idx);
      ...
      if (isrran_sss_subframe(m0, m1) == 0) {
#ifdef ENABLE_GUI
        if (!disable_plots)
...
void do_plots(float* corr, float energy, uint32_t size, cf_t ce[ISRRAN_PSS_LEN])
{
  isrran_vec_sc_prod_fff(corr, 1. / energy, tmp, size);
  plot_real_setNewData(&pssout, tmp, size);

  //  float norm = isrran_vec_avg_power_cf(ce, ISRRAN_PSS_LEN);
  // isrran_vec_sc_prod_cfc(ce, 1.0/sqrt(norm), tmpce, ISRRAN_PSS_LEN);

  // plot_complex_setNewData(&pce, tmpce, ISRRAN_PSS_LEN);
}

void do_plots_sss(float* corr_m0, float* corr_m1)
{
  if (m0_value > 0)
    isrran_vec_sc_prod_fff(corr_m0, 1. / m0_value, corr_m0, ISRRAN_SSS_N);
  plot_real_setNewData(&psss1, corr_m0, ISRRAN_SSS_N);

  //  if (m1_value > 0)
  //    isrran_vec_sc_prod_fff(corr_m1,1./m1_value,corr_m1, ISRRAN_SSS_N);
  //  plot_real_setNewData(&psss2, corr_m1, ISRRAN_SSS_N);
}
