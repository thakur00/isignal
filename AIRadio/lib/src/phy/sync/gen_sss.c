...
void generate_zsc_tilde(int* z_tilde, int* s_tilde, int* c_tilde)
{
  ...
  int i;
  int x[ISRRAN_SSS_N];
  bzero(x, sizeof(int) * ISRRAN_SSS_N);
  x[4] = 1;
...
{
  uint32_t q_prime = N_id_1 / (ISRRAN_SSS_N - 1);
  uint32_t q       = (N_id_1 + (q_prime * (q_prime + 1) / 2)) / (ISRRAN_SSS_N - 1);
  uint32_t m_prime = N_id_1 + (q * (q + 1) / 2);
  *m0              = m_prime % ISRRAN_SSS_N;
  *m1              = (*m0 + m_prime / ISRRAN_SSS_N + 1) % ISRRAN_SSS_N;
}
...
void generate_s(int* s, int* s_tilde, uint32_t m0_m1)
{
  uint32_t i;
  for (i = 0; i < ISRRAN_SSS_N; i++) {
    s[i] = s_tilde[(i + m0_m1) % ISRRAN_SSS_N];
  }
}

void generate_s_all(int s[ISRRAN_SSS_N][ISRRAN_SSS_N], int* s_tilde)
{
  uint32_t i;
  for (i = 0; i < ISRRAN_SSS_N; i++) {
...
void generate_c(int* c, int* c_tilde, uint32_t N_id_2, bool is_c0)
{
  uint32_t i;
  for (i = 0; i < ISRRAN_SSS_N; i++) {
    c[i] = c_tilde[(i + N_id_2 + (is_c0 ? 3 : 0)) % ISRRAN_SSS_N];
  }
}
...
void generate_z(int* z, int* z_tilde, uint32_t m0_m1)
{
  uint32_t i;
  for (i = 0; i < ISRRAN_SSS_N; i++) {
    z[i] = z_tilde[(i + (m0_m1 % 8)) % ISRRAN_SSS_N];
  }
}

void generate_z_all(int z[ISRRAN_SSS_N][ISRRAN_SSS_N], int* z_tilde)
{
  uint32_t i;
  for (i = 0; i < ISRRAN_SSS_N; i++) {
...
{
  uint32_t i;
  int      s_t[ISRRAN_SSS_N], c_t[ISRRAN_SSS_N], z_t[ISRRAN_SSS_N];
...
void isrran_sss_generate(float* signal0, float* signal5, uint32_t cell_id)
{
  ...
  uint32_t m1;
  int      s_t[ISRRAN_SSS_N], c_t[ISRRAN_SSS_N], z_t[ISRRAN_SSS_N];
  int s0[ISRRAN_SSS_N], s1[ISRRAN_SSS_N], c0[ISRRAN_SSS_N], c1[ISRRAN_SSS_N], z1_0[ISRRAN_SSS_N], z1_1[ISRRAN_SSS_N];
