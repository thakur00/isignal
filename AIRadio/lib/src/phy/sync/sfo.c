 ...
 */
float isrran_sfo_estimate(int* t0, int len, float period)
{
  int   i;
  float sfo = 0.0;
  for (i = 1; i < len; i++) {
...
/* Same as isrran_sfo_estimate but period is non-uniform.
 * Vector t is the sampling time times period for each t0
 */
float isrran_sfo_estimate_period(int* t0, int* t, int len, float period)
{
  int   i;
  float sfo = 0.0;
  for (i = 1; i < len; i++) {
