 ...
 *     isrue/test/phy/scell_search_test --duration=5 --cell.nof_prb=6 --active_cell_list=all
 ...
 *     isrue/test/phy/scell_search_test --duration=30 --cell.nof_prb=6 --active_cell_list=2,3,4,5,6
 *       --simulation_cell_list=1,2,3,4,5,6 --channel_period_s=30 --channel.hst.fd=750 --channel.delay_max=10000
 *
 * - 6 PRB. 6 cell PSS, SSS overlapped. Brut-forced search.
 *     isrue/test/phy/scell_search_test --duration=30 --cell.nof_prb=6 --active_cell_list=all
 ...
 *     isrue/test/phy/scell_search_test --duration=30 --cell.nof_prb=100 --active_cell_list=2,3,4,5,6
 ...
 * Local Helpers
 */
static inline void refsignal_set_results_not_found(isrran_refsignal_dl_sync_t* q)
{
  q->found      = false;
  q->rsrp_dBfs  = NAN;
  q->rssi_dBfs  = NAN;
...
static inline void refsignal_sf_prepare_correlation(isrran_refsignal_dl_sync_t* q)
{
  uint32_t sf_len   = q->ifft.sf_sz;
  cf_t*    ptr_filt = q->conv_fft_cc.filter_fft;

  // Put first subframe in buffer
  isrran_vec_cf_copy(ptr_filt, q->sequences[0], sf_len);

  // Zero the rest of the buffer
  isrran_vec_cf_zero(&ptr_filt[sf_len], q->conv_fft_cc.output_len - sf_len);

  // Make correlation sequence in frequency domain
  isrran_dft_run_c(&q->conv_fft_cc.filter_plan, ptr_filt, ptr_filt);
}

static inline void
refsignal_sf_correlate(isrran_refsignal_dl_sync_t* q, cf_t* ptr_in, float* peak_value, uint32_t* peak_idx, float* rms)
{
  // Correlate
  isrran_corr_fft_cc_run_opt(&q->conv_fft_cc, ptr_in, q->conv_fft_cc.filter_fft, q->correlation);

  // Find maximum, calculate RMS and peak
  uint32_t imax = isrran_vec_max_abs_ci(q->correlation, q->ifft.sf_sz);
  ...
  if (rms) {
    *rms = sqrtf(isrran_vec_avg_power_cf(q->correlation, q->ifft.sf_sz));
  }
}

static inline void refsignal_dl_pss_sss_strength(isrran_refsignal_dl_sync_t* q,
                                                 cf_t*                       buffer,
                                                 uint32_t                    sf_idx,
                                                 float*                      pss_strength,
                                                 float*                      sss_strength,
                                                 float*                      sss_strength_false)
{
  ...
  uint32_t cp_len0   = ISRRAN_CP_LEN_NORM(0, symbol_sz);
  uint32_t cp_len1   = ISRRAN_CP_LEN_NORM(1, symbol_sz);

  uint32_t pss_n = cp_len0 + cp_len1 * 6 + symbol_sz * 6;
  uint32_t sss_n = cp_len0 + cp_len1 * 5 + symbol_sz * 5;

  sf_idx %= ISRRAN_NOF_SF_X_FRAME;

  isrran_dl_sf_cfg_t dl_sf_cfg = {};
  dl_sf_cfg.tti                = sf_idx;

  float k = (float)(isrran_refsignal_cs_nof_re(&q->refsignal, &dl_sf_cfg, 0)) / (float)ISRRAN_PSS_LEN;

  if (pss_strength) {
    cf_t corr     = isrran_vec_dot_prod_conj_ccc(&buffer[pss_n], &q->sequences[sf_idx][pss_n], symbol_sz);
    *pss_strength = k * __real__(corr * conjf(corr));
  }

  if (sss_strength) {
    cf_t corr     = isrran_vec_dot_prod_conj_ccc(&buffer[sss_n], &q->sequences[sf_idx][sss_n], symbol_sz);
    *sss_strength = k * __real__(corr * conjf(corr));
  }

  if (sss_strength_false) {
    uint32_t sf_idx2    = (sf_idx + ISRRAN_NOF_SF_X_FRAME / 2) % ISRRAN_NOF_SF_X_FRAME;
    cf_t     corr       = isrran_vec_dot_prod_conj_ccc(&buffer[sss_n], &q->sequences[sf_idx2][sss_n], symbol_sz);
    *sss_strength_false = k * __real__(corr * conjf(corr));
  }
}

int isrran_refsignal_dl_sync_init(isrran_refsignal_dl_sync_t* q, isrran_cp_t cp)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;

  if (q) {
    // Initialise internals
    memset(q, 0, sizeof(isrran_refsignal_dl_sync_t));

    // Initialise Reference signals
    ret = isrran_refsignal_cs_init(&q->refsignal, ISRRAN_MAX_PRB);

    // Allocate time buffers
    for (int i = 0; i < ISRRAN_NOF_SF_X_FRAME; i++) {
      q->sequences[i] = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX);
      if (!q->sequences[i]) {
    ...
    q->ifft_buffer_in = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX);
    if (!q->ifft_buffer_in) {
      perror("Allocating ifft_buffer_in\n");
    }

    q->ifft_buffer_out = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX);
    if (!q->ifft_buffer_out) {
      perror("Allocating ifft_buffer_out\n");
    }

    // Allocate correlation
    q->correlation = isrran_vec_cf_malloc(ISRRAN_SF_LEN_MAX * 2);
    if (!q->correlation) {
    ...
    if (!ret) {
      ret = isrran_ofdm_tx_init(&q->ifft, cp, q->ifft_buffer_in, q->ifft_buffer_out, ISRRAN_MAX_PRB);
    }

    // Set PRB
    if (!ret) {
      ret = isrran_ofdm_tx_set_prb(&q->ifft, cp, ISRRAN_MAX_PRB);
    }

    // Initiate FFT Convolution
    if (!ret) {
      ret = isrran_conv_fft_cc_init(&q->conv_fft_cc, q->ifft.sf_sz, q->ifft.sf_sz);
    }
...
int isrran_refsignal_dl_sync_set_cell(isrran_refsignal_dl_sync_t* q, isrran_cell_t cell)
{
  int ret = ISRRAN_ERROR_INVALID_INPUTS;
  ...
  if (q) {
    ...
    isrran_pss_generate(pss_signal, cell.id % 3);
    isrran_sss_generate(sss_signal0, sss_signal5, cell.id);

    // Set cell for Ref signals
    ret = isrran_refsignal_cs_set_cell(&q->refsignal, cell);

    // Resize OFDM
    if (!ret && q->ifft.nof_re != cell.nof_prb * ISRRAN_NRE) {
      ret = isrran_ofdm_tx_set_prb(&q->ifft, cell.cp, cell.nof_prb);
    }

    // Replan convolution
    if (q->conv_fft_cc.filter_len != q->ifft.sf_sz) {
      isrran_conv_fft_cc_replan(&q->conv_fft_cc, q->ifft.sf_sz, q->ifft.sf_sz);
    }

    // Generate frame with references
    if (!ret) {
      for (int i = 0; i < ISRRAN_NOF_SF_X_FRAME && ret == ISRRAN_SUCCESS; i++) {
        uint32_t nof_re = 0;
        ...
        // Reset OFDM buffer
        isrran_vec_cf_zero(q->ifft_buffer_in, q->ifft.sf_sz);

        // Put Synchronization signals
        if (i == 0 || i == 5) {
          isrran_pss_put_slot(pss_signal, q->ifft_buffer_in, cell.nof_prb, cell.cp);
          isrran_sss_put_slot(i ? sss_signal5 : sss_signal0, q->ifft_buffer_in, cell.nof_prb, cell.cp);
        }

        // Put Reference signals
        for (int p = 0; p < cell.nof_ports; p++) {
          ret = isrran_refsignal_cs_put_sf(&q->refsignal, &dl_sf_cfg, p, q->ifft_buffer_in);

          // Increment number of resource elements
          if (p == 0) {
            nof_re += isrran_refsignal_cs_nof_re(&q->refsignal, &dl_sf_cfg, p);
          }
        }
        ...
        isrran_vec_sc_prod_cfc(q->ifft_buffer_out, scale, q->sequences[i], q->ifft.sf_sz);
      }
    }
  }
...
void isrran_refsignal_dl_sync_free(isrran_refsignal_dl_sync_t* q)
{
  if (q) {
    isrran_refsignal_free(&q->refsignal);
    ...
    isrran_conv_fft_cc_free(&q->conv_fft_cc);
  }
}

int refsignal_dl_sync_find_peak(isrran_refsignal_dl_sync_t* q, cf_t* buffer, uint32_t nsamples)
{
  ...
  if (ret >= 0) {
    ...
    isrran_refsignal_dl_sync_measure_sf(q, &buffer[peak_idx], 0, &rsrp_lin, NULL, NULL);
    isrran_refsignal_dl_sync_measure_sf(q, &buffer[peak_idx], 5, &rsrp_lin_false, NULL, NULL);

    // Change base sub-frame
    if (sss_strength_false > sss_strength && rsrp_lin_false > rsrp_lin) {
      ret += (q->ifft.sf_sz * ISRRAN_NOF_SF_X_FRAME) / 2;
    }
  }
...
int isrran_refsignal_dl_sync_run(isrran_refsignal_dl_sync_t* q, cf_t* buffer, uint32_t nsamples)
{
  ...
  if (peak_idx >= 0) {
    ...
    uint32_t sf_idx_init = ISRRAN_NOF_SF_X_FRAME - (peak_idx / sf_len) % ISRRAN_NOF_SF_X_FRAME;
    uint32_t n_init      = peak_idx % sf_len;

    for (uint32_t sf_idx = sf_idx_init, n = n_init; n < (nsamples - sf_len + 1);
         sf_idx = (sf_idx + 1) % ISRRAN_NOF_SF_X_FRAME, n += sf_len) {
      cf_t* buf = &buffer[n];

      // Measure subframe rsrp, rssi and accumulate
      float rsrp = 0.0f, rssi = 0.0f, cfo = 0.0f;
      isrran_refsignal_dl_sync_measure_sf(q, buf, sf_idx, &rsrp, &rssi, &cfo);

      // Update measurements
      rsrp_lin += rsrp;
      rsrp_lin_min = ISRRAN_MIN(rsrp_lin_min, rsrp);
      rsrp_lin_max = ISRRAN_MAX(rsrp_lin_max, rsrp);

      rssi_lin += rssi;

      cfo_acc += cfo;
      cfo_min = ISRRAN_MIN(cfo_min, cfo);
      cfo_max = ISRRAN_MAX(cfo_max, cfo);

      // Compute PSS/SSS strength
      if (sf_idx % (ISRRAN_NOF_SF_X_FRAME / 2) == 0) {
        float sss_strength       = 0.0f;
        float sss_strength_false = 0.0f;
        refsignal_dl_pss_sss_strength(q, buf, sf_idx, NULL, &sss_strength, &sss_strength_false);

        float rsrp_false = 0.0f;
        isrran_refsignal_dl_sync_measure_sf(q, buf, sf_idx + 1, &rsrp_false, NULL, NULL);
    ...
    if (sf_count) {
      ...
      rssi_lin /= sf_count;
      cfo_acc /= sf_count;
      sss_strength_avg /= (2.0f * sf_count / ISRRAN_NOF_SF_X_FRAME);
      sss_strength_false_avg /= (2.0f * sf_count / ISRRAN_NOF_SF_X_FRAME);
      rsrp_false_avg /= (2.0f * sf_count / ISRRAN_NOF_SF_X_FRAME);
    }

    // RSRP conversion to dB
    float rsrp_dB_min   = isrran_convert_power_to_dBm(rsrp_lin_min);
    float rsrp_dB_max   = isrran_convert_power_to_dBm(rsrp_lin_max);
    float rsrp_dB       = isrran_convert_power_to_dBm(rsrp_lin);
    float rsrp_false_dB = isrran_convert_power_to_dBm(rsrp_false_avg);
    ...
    if (!false_alarm) {
      ...
      q->rssi_dBfs = isrran_convert_power_to_dBm(rssi_lin);

      // Calculate RSRQ
      q->rsrq_dB = isrran_convert_power_to_dB(q->refsignal.cell.nof_prb) + q->rsrp_dBfs - q->rssi_dBfs;
...
void isrran_refsignal_dl_sync_measure_sf(isrran_refsignal_dl_sync_t* q,
                                         cf_t*                       buffer,
                                         uint32_t                    sf_idx,
                                         float*                      rsrp,
                                         float*                      rssi,
                                         float*                      cfo)
{
  ...
  if (q) {
    cf_t* sf_sequence = q->sequences[sf_idx % ISRRAN_NOF_SF_X_FRAME];

    uint32_t symbol_sz = q->ifft.cfg.symbol_sz;
    uint32_t nsymbols  = isrran_refsignal_cs_nof_symbols(&q->refsignal, &dl_sf_cfg, 0);
    uint32_t cp_len0   = ISRRAN_CP_LEN_NORM(0, symbol_sz);
    uint32_t cp_len1   = ISRRAN_CP_LEN_NORM(1, symbol_sz);

    for (uint32_t l = 0; l < nsymbols; l++) {
      // Calculate FFT window offset for reference signals symbols
      uint32_t symbidx = isrran_refsignal_cs_nsymbol(l, q->refsignal.cell.cp, 0);
      ...
      // Complex correlation
      corr[l] = isrran_vec_dot_prod_conj_ccc(&buffer[offset], &sf_sequence[offset], symbol_sz);
      ...
      // Calculate RSSI
      if (rssi) {
        rssi_lin += isrran_vec_dot_prod_conj_ccc(&buffer[offset], &buffer[offset], symbol_sz);
      }
    }
