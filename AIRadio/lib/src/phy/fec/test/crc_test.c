...
int main(int argc, char** argv)
{
  ...
  data = isrran_vec_u8_malloc(num_bits + crc_length * 2);
  if (!data) {
    perror("malloc");
    exit(-1);
  }
  ...
  if (ISRRAN_DEBUG_ENABLED && get_isrran_verbose_level() >= ISRRAN_VERBOSE_INFO && !is_handler_registered()) {
    INFO("data=");
    isrran_vec_fprint_b(stdout, data, num_bits);
  }

  // Initialize CRC params and tables
  if (isrran_crc_init(&crc_p, crc_poly, crc_length)) {
    exit(-1);
  }

  // generate CRC word
  crc_word = isrran_crc_checksum(&crc_p, data, num_bits);
