...
static uint32_t        E               = ISRRAN_FEC_BLOCK_SIZE;
...
{
  struct timeval t[3]                               = {};
  uint8_t        tx[ISRRAN_FEC_BLOCK_MAX_NOF_BITS]  = {};
  uint8_t        rx[ISRRAN_FEC_BLOCK_MAX_NOF_BITS]  = {};
  uint8_t        encoded[4 * ISRRAN_FEC_BLOCK_SIZE] = {};
  int16_t        llr_i16[4 * ISRRAN_FEC_BLOCK_SIZE] = {};
  int8_t         llr_i8[4 * ISRRAN_FEC_BLOCK_SIZE]  = {};

  // Generate random data
  for (uint32_t i = 0; i < block_size; i++) {
    tx[i] = (uint8_t)isrran_random_uniform_int_dist(random_gen, 0, 1);
  }

  gettimeofday(&t[1], NULL);
  for (uint32_t r = 0; r < nof_repetitions; r++) {
    isrran_block_encode(tx, block_size, encoded, E);
  }
  ...
  for (uint32_t r = 0; r < nof_repetitions; r++) {
    corr_i16 = isrran_block_decode_i16(llr_i16, E, rx, block_size);
  }
  gettimeofday(&t[2], NULL);
  get_time_interval(t);
  ...
  int32_t corr_i8 = 0;
  for (uint32_t r = 0; r < nof_repetitions; r++) {
    corr_i8 = isrran_block_decode_i8(llr_i8, E, rx, block_size);
  }
  gettimeofday(&t[2], NULL);
  get_time_interval(t);
...
int main(int argc, char** argv)
{
  ...
  random_gen = isrran_random_init(seed);

  for (uint32_t block_size = 3; block_size <= ISRRAN_FEC_BLOCK_MAX_NOF_BITS; block_size++) {
    if (test(block_size) < ISRRAN_SUCCESS) {
      break;
    }
  }
