...
static const uint64_t M_basis_seq_b[ISRRAN_FEC_BLOCK_SIZE] = {
...
{
  // Apply mask
  uint64_t d = w & M_basis_seq_b[bit_idx % ISRRAN_FEC_BLOCK_SIZE];
...
static uint8_t block_unpacked_lut[1U << ISRRAN_FEC_BLOCK_MAX_NOF_BITS][ISRRAN_FEC_BLOCK_SIZE];

// LLR signed table
static block_llr_t block_llr_lut[1U << ISRRAN_FEC_BLOCK_MAX_NOF_BITS][ISRRAN_FEC_BLOCK_SIZE];

// Initialization function, as the table is read-only after initialization, it can be initialised from constructor
__attribute__((constructor)) static void isrran_block_init()
{
  for (uint32_t word = 0; word < (1U << ISRRAN_FEC_BLOCK_MAX_NOF_BITS); word++) {
    for (uint32_t i = 0; i < ISRRAN_FEC_BLOCK_SIZE; i++) {
...
void isrran_block_encode(const uint8_t* input, uint32_t input_len, uint8_t* output, uint32_t output_len)
{
  if (!input || !output) {
  ...
  input_len = ISRRAN_MIN(input_len, ISRRAN_FEC_BLOCK_MAX_NOF_BITS);
...
#if USE_LUT
  uint32_t i = 0;
  for (; i < output_len / ISRRAN_FEC_BLOCK_SIZE; i++) {
    isrran_vec_u8_copy(&output[i * ISRRAN_FEC_BLOCK_SIZE], block_unpacked_lut[w], ISRRAN_FEC_BLOCK_SIZE);
  }
  isrran_vec_u8_copy(&output[i * ISRRAN_FEC_BLOCK_SIZE], block_unpacked_lut[w], output_len % ISRRAN_FEC_BLOCK_SIZE);
#else  // USE_LUT
  for (uint32_t i = 0; i < ISRRAN_MIN(output_len, ISRRAN_FEC_BLOCK_SIZE); i++) {
    output[i] = encode_M_basis_seq_u16(w, i);
  }

  // Avoid repeating operation by copying repeated sequence
  for (uint32_t i = ISRRAN_FEC_BLOCK_SIZE; i < output_len; i++) {
    output[i] = output[i % ISRRAN_FEC_BLOCK_SIZE];
  }
#endif // USE_LUT
}
...
static int32_t block_decode(const block_llr_t* llr, uint8_t* data, uint32_t data_len)
{
  ...
  data_len = ISRRAN_MIN(data_len, ISRRAN_FEC_BLOCK_MAX_NOF_BITS);
...
int32_t isrran_block_decode_i8(const int8_t* llr, uint32_t nof_llr, uint8_t* data, uint32_t data_len)
{
  block_llr_t llr_[ISRRAN_FEC_BLOCK_SIZE] = {};
  ...
  for (uint32_t i = 0; i < nof_llr; i++) {
    llr_[i % ISRRAN_FEC_BLOCK_SIZE] += (block_llr_t)llr[i];
  }

  return block_decode(llr_, data, data_len);
}

int32_t isrran_block_decode_i16(const int16_t* llr, uint32_t nof_llr, uint8_t* data, uint32_t data_len)
{
  block_llr_t llr_[ISRRAN_FEC_BLOCK_SIZE] = {};
  ...
  // Accumulate all copies of the 32-length sequence
  uint32_t i = 0;
  for (; i < nof_llr / ISRRAN_FEC_BLOCK_SIZE; i++) {
    isrran_vec_sum_sss(&llr[i * ISRRAN_FEC_BLOCK_SIZE], llr_, llr_, ISRRAN_FEC_BLOCK_SIZE);
  }
  isrran_vec_sum_sss(&llr[i * ISRRAN_FEC_BLOCK_SIZE], llr_, llr_, nof_llr % ISRRAN_FEC_BLOCK_SIZE);
