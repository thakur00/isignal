...
int init37(isrran_viterbi_t* q, int poly[3], uint32_t framebits, bool tail_biting)
{
  q->K            = 7;
  q->R            = 3;
  ...
  q->free         = free37;
  q->decode_f     = NULL;
  q->symbols_uc   = isrran_vec_u8_malloc(3 * (q->framebits + q->K - 1));
  if (!q->symbols_uc) {
    perror("malloc");
    return -1;
  }
  if (q->tail_biting) {
    q->tmp = isrran_vec_u8_malloc(TB_ITER * 3 * (q->framebits + q->K - 1));
...
#ifdef LV_HAVE_SSE
int init37_sse(isrran_viterbi_t* q, int poly[3], uint32_t framebits, bool tail_biting)
{
  q->K            = 7;
  q->R            = 3;
  q->framebits    = framebits;
...
#ifdef VITERBI_16
  q->symbols_us = isrran_vec_u16_malloc(3 * (q->framebits + q->K - 1));
  if (!q->symbols_us) {
    perror("malloc");
    return -1;
  }
...
#ifdef HAVE_NEON
int init37_neon(isrran_viterbi_t* q, int poly[3], uint32_t framebits, bool tail_biting)
{
  q->K            = 7;
  q->R            = 3;
  q->framebits    = framebits;
...
#ifdef LV_HAVE_AVX2
int init37_avx2(isrran_viterbi_t* q, int poly[3], uint32_t framebits, bool tail_biting)
{
  q->K            = 7;
  q->R            = 3;
  q->framebits    = framebits;
...
int init37_avx2_16bit(isrran_viterbi_t* q, int poly[3], uint32_t framebits, bool tail_biting)
{
  q->K            = 7;
  q->R            = 3;
  q->framebits    = framebits;
  ...
  if (q->tail_biting) {
    q->tmp   = isrran_vec_u8_malloc(TB_ITER * 3 * (q->framebits + q->K - 1));
    q->tmp_s = isrran_vec_u16_malloc(TB_ITER * 3 * (q->framebits + q->K - 1));
    if (!q->tmp) {
      perror("malloc");
      free37(q);
      return -1;
...
void isrran_viterbi_set_gain_quant(isrran_viterbi_t* q, float gain_quant)
{
  q->gain_quant = gain_quant;
}

void isrran_viterbi_set_gain_quant_s(isrran_viterbi_t* q, int16_t gain_quant)
{
  q->gain_quant_s = gain_quant;
}

int isrran_viterbi_init(isrran_viterbi_t*     q,
                        isrran_viterbi_type_t type,
                        int                   poly[3],
                        uint32_t              max_frame_length,
                        bool                  tail_bitting)
{
  bzero(q, sizeof(isrran_viterbi_t));
  switch (type) {
    case ISRRAN_VITERBI_37:
#ifdef LV_HAVE_SSE
...
#ifdef LV_HAVE_SSE
int isrran_viterbi_init_sse(isrran_viterbi_t*     q,
                            isrran_viterbi_type_t type,
                            int                   poly[3],
                            uint32_t              max_frame_length,
                            bool                  tail_bitting)
...
#ifdef LV_HAVE_AVX2
int isrran_viterbi_init_avx2(isrran_viterbi_t*     q,
                             isrran_viterbi_type_t type,
                             int                   poly[3],
                             uint32_t              max_frame_length,
                             bool                  tail_bitting)
...
void isrran_viterbi_free(isrran_viterbi_t* q)
{
  if (q->free) {
    q->free(q);
  }
...
int isrran_viterbi_decode_f(isrran_viterbi_t* q, float* symbols, uint8_t* data, uint32_t frame_length)
{
  uint32_t len;
  if (frame_length > q->framebits) {
    ERROR("Initialized decoder for max frame length %d bits", q->framebits);
  ...
  if (!q->decode_f) {
    float    max   = 1e-9;
    uint32_t max_i = isrran_vec_max_abs_fi(symbols, len);
    if (max_i < len && isnormal(symbols[max_i])) {
      max = fabsf(symbols[max_i]);
    }
#ifdef VITERBI_16
    isrran_vec_quant_fus(symbols, q->symbols_us, q->gain_quant / max, 32767.5, 65535, len);
    return isrran_viterbi_decode_us(q, q->symbols_us, data, frame_length);
#else
    isrran_vec_quant_fuc(symbols, q->symbols_uc, q->gain_quant / max, 127.5, 255, len);
    return isrran_viterbi_decode_uc(q, q->symbols_uc, data, frame_length);
#endif
  } else {
...
/* symbols are int16 */
int isrran_viterbi_decode_s(isrran_viterbi_t* q, int16_t* symbols, uint8_t* data, uint32_t frame_length)
{
  uint32_t len;
  if (frame_length > q->framebits) {
    ERROR("Initialized decoder for max frame length %d bits", q->framebits);
...
#ifdef VITERBI_16
  isrran_vec_quant_sus(symbols, q->symbols_us, 1, (float)INT16_MAX, UINT16_MAX, len);
  return isrran_viterbi_decode_us(q, q->symbols_us, data, frame_length);
#else
  isrran_vec_quant_suc(symbols, q->symbols_uc, (float)q->gain_quant / max, 127, 255, len);
  return isrran_viterbi_decode_uc(q, q->symbols_uc, data, frame_length);
#endif
}

int isrran_viterbi_decode_us(isrran_viterbi_t* q, uint16_t* symbols, uint8_t* data, uint32_t frame_length)
{
...
int isrran_viterbi_decode_uc(isrran_viterbi_t* q, uint8_t* symbols, uint8_t* data, uint32_t frame_length)
{
  int ret = ISRRAN_ERROR;
