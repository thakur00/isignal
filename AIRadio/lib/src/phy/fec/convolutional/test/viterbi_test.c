...
#define VITERBI_TEST(FUNC, DEC, LLR, NOF_ERRORS)                                                                       \
  do {                                                                                                                 \
    ...
    if (NOF_ERRORS >= 0) {                                                                                             \
      NOF_ERRORS += isrran_bit_diff(data_tx, data_rx, frame_length);                                                   \
    }                                                                                                                  \
  } while (0)
...
int main(int argc, char** argv)
{
  ...
  int       errors_sse = 0;
#ifdef TEST_SSE
  isrran_viterbi_t dec_sse;
#endif
  isrran_viterbi_t   dec;
  isrran_convcoder_t cod;
  int                coded_length;
  ...
  isrran_viterbi_init(&dec, ISRRAN_VITERBI_37, cod.poly, frame_length, cod.tail_biting);
  printf("Convolutional Code 1/3 K=%d Tail bitting: %s\n", cod.K, cod.tail_biting ? "yes" : "no");

#ifdef TEST_SSE
  isrran_viterbi_init_sse(&dec_sse, ISRRAN_VITERBI_37, cod.poly, frame_length, cod.tail_biting);
#endif
  ...
  llr_s = isrran_vec_i16_malloc(2 * coded_length);
  if (!llr_s) {
    perror("malloc");
    exit(-1);
  }
  llr_us = isrran_vec_u16_malloc(2 * coded_length);
  if (!llr_us) {
    perror("malloc");
    exit(-1);
  }
  llr_c = isrran_vec_u8_malloc(2 * coded_length);
  if (!llr_c) {
    perror("malloc");
    exit(-1);
  }
  ...
  if (ebno_db == 100.0) {
    ...
    for (uint32_t i = 0; i < snr_points; i++) {
      ...
      varunc[i] = isrran_convert_dB_to_power(-ebno_db);
    }
  } else {
    esno_db    = ebno_db + isrran_convert_power_to_dB(1.0f / 3.0f);
    var[0]     = isrran_convert_dB_to_power(-esno_db);
    varunc[0]  = isrran_convert_dB_to_power(-ebno_db);
    snr_points = 1;
  }
  ...
  for (uint32_t i = 0; i < snr_points; i++) {
    ...
    while (frame_cnt < nof_frames) {
      ...
      isrran_ch_awgn_f(llr, llr, varunc[i], frame_length);

      /* coded BER */
      isrran_convcoder_encode(&cod, data_tx, symbols, frame_length);
      ...
      // isrran_vec_fprint_f(stdout, llr, 100);

      isrran_vec_convert_fi(llr, 1000, llr_s, coded_length);
      isrran_vec_quant_fuc(llr, llr_c, 32, INT8_MAX, UINT8_MAX, coded_length);
      isrran_vec_quant_fus(llr, llr_us, 8192, INT16_MAX, UINT16_MAX, coded_length);

      VITERBI_TEST(isrran_viterbi_decode_s, dec, llr_s, errors_s);
      VITERBI_TEST(isrran_viterbi_decode_us, dec, llr_us, errors_us);
      VITERBI_TEST(isrran_viterbi_decode_uc, dec, llr_c, errors_c);
      VITERBI_TEST(isrran_viterbi_decode_f, dec, llr, errors_f);
#ifdef TEST_SSE
      VITERBI_TEST(isrran_viterbi_decode_uc, dec_sse, llr_c, errors_sse);
#endif
      frame_cnt++;
  ...
  isrran_viterbi_free(&dec);
#ifdef TEST_SSE
  isrran_viterbi_free(&dec_sse);
#endif
