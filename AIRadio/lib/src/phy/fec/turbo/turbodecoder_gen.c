...
#define debug_state                                                                                                    \
  ...
  isrran_vec_fprint_s(stdout, alpha, 8);                                                                               \
  printf(", beta=");                                                                                                   \
  isrran_vec_fprint_s(stdout, &beta[8 * (k)], 8);                                                                      \
  printf("\n");
#else
#define debug_state
#endif
...
static void map_gen_beta(tdec_gen_t* s, int16_t* input, int16_t* app, int16_t* parity, uint32_t long_cb)
{
  ...
  int16_t  x, y, xy;
  int      k;
  uint32_t end  = long_cb + ISRRAN_TCOD_RATE;
  int16_t* beta = s->beta;
  uint32_t i;
...
int tdec_gen_init(void** hh, uint32_t max_long_cb)
{
  *hh = calloc(1, sizeof(tdec_gen_t));
  ...
  h->beta = isrran_vec_i16_malloc((max_long_cb + ISRRAN_TCOD_TOTALTAIL + 1) * NUMSTATES);
  if (!h->beta) {
                            ...
                            uint32_t long_cb)
{
  ...
  for (uint32_t i = 0; i < long_cb; i++) {
    syst[i]    = input[ISRRAN_TCOD_RATE * i];
    parity0[i] = input[ISRRAN_TCOD_RATE * i + 1];
    parity1[i] = input[ISRRAN_TCOD_RATE * i + 2];
  }
  for (uint32_t i = long_cb; i < long_cb + ISRRAN_TCOD_RATE; i++) {
    syst[i]    = input[ISRRAN_TCOD_RATE * long_cb + NINPUTS * (i - long_cb)];
    parity0[i] = input[ISRRAN_TCOD_RATE * long_cb + NINPUTS * (i - long_cb) + 1];

    app2[i]    = input[ISRRAN_TCOD_RATE * long_cb + NINPUTS * ISRRAN_TCOD_RATE + NINPUTS * (i - long_cb)];
    parity1[i] = input[ISRRAN_TCOD_RATE * long_cb + NINPUTS * ISRRAN_TCOD_RATE + NINPUTS * (i - long_cb) + 1];
  }
}
