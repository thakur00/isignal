...
const uint32_t f1_list[ISRRAN_NOF_TC_CB_SIZES] = {
...
const uint32_t f2_list[ISRRAN_NOF_TC_CB_SIZES] = {
...
int isrran_tc_interl_LTE_gen(isrran_tc_interl_t* h, uint32_t long_cb)
{
  return isrran_tc_interl_LTE_gen_interl(h, long_cb, 1);
}

#define deinter(x, win) ((x % (long_cb / win)) * (win) + x / (long_cb / win))
#define inter(x, win) ((x % win) * (long_cb / win) + x / win)

int isrran_tc_interl_LTE_gen_interl(isrran_tc_interl_t* h, uint32_t long_cb, uint32_t interl_win)
{
  ...
  cb_table_idx = isrran_cbsegm_cbindex(long_cb);
  if (cb_table_idx == -1) {
  ...
  if (interl_win != 1) {
    uint16_t* f = isrran_vec_u16_malloc(long_cb);
    uint16_t* r = isrran_vec_u16_malloc(long_cb);
