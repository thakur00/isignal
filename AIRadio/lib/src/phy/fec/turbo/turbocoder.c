...
static isrran_bit_interleaver_t tcod_interleavers[188];

static bool table_initiated = false;

int isrran_tcod_init(isrran_tcod_t* h, uint32_t max_long_cb)
{
  h->max_long_cb = max_long_cb;
  h->temp        = isrran_vec_malloc(max_long_cb / 8);

  if (!table_initiated) {
    table_initiated = true;
    isrran_tcod_gentable();
  }
  return 0;
}

void isrran_tcod_free(isrran_tcod_t* h)
{
  h->max_long_cb = 0;
  if (h->temp) {
    free(h->temp);
  ...
  if (table_initiated) {
    for (int i = 0; i < 188; i++) {
      isrran_bit_interleaver_free(&tcod_interleavers[i]);
    }
...
int isrran_tcod_encode(isrran_tcod_t* h, uint8_t* input, uint8_t* output, uint32_t long_cb)
{
  ...
  int longcb_idx = isrran_cbsegm_cbindex(long_cb);
  if (longcb_idx < 0) {
...
int isrran_tcod_encode_lut(isrran_tcod_t* h,
                           isrran_crc_t*  crc_tb,
                           isrran_crc_t*  crc_cb,
                           uint8_t*       input,
                           uint8_t*       parity,
...
{
  if (cblen_idx < 188) {
    uint32_t long_cb = (uint32_t)isrran_cbsegm_cbsize(cblen_idx);
    ...
    if (crc_cb) {
      ...
      for (int i = 0; i < block_size_nocrc; i++) {
        ...
        isrran_crc_checksum_put_byte(crc_tb, in);

        /* Put byte in CB CRC and save latest checksum */
        isrran_crc_checksum_put_byte(crc_cb, in);
      ...
      if (last_cb) {
        uint32_t checksum = (uint32_t)isrran_crc_checksum_get(crc_tb);
      ...
      uint32_t checksum = (uint32_t)isrran_crc_checksum_get(crc_cb);
    ...
    /* Interleave input */
    isrran_bit_interleaver_run(&tcod_interleavers[cblen_idx], input, h->temp, 0);
    // isrran_bit_interleave(input, h->temp, tcod_per_fw[cblen_idx], long_cb);
    ...
    uint8_t* x         = tailv[0];
    input[long_cb / 8] = (isrran_bit_pack(&x, 4) << 4);
    x                  = tailv[1];
    parity[long_cb / 8] |= (isrran_bit_pack(&x, 4) << 4);
    x = tailv[2];
    parity[2 * long_cb / 8] |= (isrran_bit_pack(&x, 4) & 0xf);
...
void isrran_tcod_gentable()
{
  isrran_tc_interl_t interl;

  if (isrran_tc_interl_init(&interl, 6144)) {
    ERROR("Error initiating interleave");
    return;
  }

  for (uint32_t len = 0; len < 188; len++) {
    uint32_t long_cb = isrran_cbsegm_cbsize(len);
    if (isrran_tc_interl_LTE_gen(&interl, long_cb)) {
    ...
    isrran_bit_interleaver_init(&tcod_interleavers[len], tcod_per_fw[len], long_cb);
