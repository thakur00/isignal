...
isrran_tdec_16bit_impl_t gen_impl = {tdec_gen_init,
                                     tdec_gen_free,
                                     tdec_gen_dec,
...
#ifdef LV_HAVE_SSE
#include "isrran/phy/fec/turbo/turbodecoder_sse.h"
isrran_tdec_16bit_impl_t sse_impl = {tdec_sse_init,
                                     tdec_sse_free,
                                     tdec_sse_dec,
...
#include "isrran/phy/fec/turbo/turbodecoder_win.h"
#undef WINIMP_IS_SSE16

isrran_tdec_16bit_impl_t sse16_win_impl = {tdec_winsse16_init,
                                           tdec_winsse16_free,
                                           tdec_winsse16_dec,
...
isrran_tdec_16bit_impl_t avx16_win_impl = {tdec_winavx16_init,
                                           tdec_winavx16_free,
                                           tdec_winavx16_dec,
...
isrran_tdec_8bit_impl_t sse8_win_impl = {tdec_winsse8_init,
                                         tdec_winsse8_free,
                                         tdec_winsse8_dec,
...
isrran_tdec_8bit_impl_t avx8_win_impl = {tdec_winavx8_init,
                                         tdec_winavx8_free,
                                         tdec_winavx8_dec,
...
isrran_tdec_16bit_impl_t arm16_win_impl = {tdec_winarm16_init,
                                           tdec_winarm16_free,
                                           tdec_winarm16_dec,
...
#define LLR_IS_8BIT
#include "isrran/phy/fec/turbo/turbodecoder_iter.h"
#undef LLR_IS_8BIT
...
int isrran_tdec_init(isrran_tdec_t* h, uint32_t max_long_cb)
{
  return isrran_tdec_init_manual(h, max_long_cb, ISRRAN_TDEC_AUTO);
}
...
int isrran_tdec_init_manual(isrran_tdec_t* h, uint32_t max_long_cb, isrran_tdec_impl_type_t dec_type)
{
  int ret = -1;
  bzero(h, sizeof(isrran_tdec_t));
  uint32_t len = max_long_cb + ISRRAN_TCOD_TOTALTAIL;

  h->dec_type = dec_type;

  // Set manual
  switch (dec_type) {
    case ISRRAN_TDEC_AUTO:
      break;
#ifdef LV_HAVE_SSE
    case ISRRAN_TDEC_SSE:
      h->dec16[0]         = &sse_impl;
      h->current_llr_type = ISRRAN_TDEC_16;
      break;
    case ISRRAN_TDEC_SSE_WINDOW:
      h->dec16[0]         = &sse16_win_impl;
      h->current_llr_type = ISRRAN_TDEC_16;
      break;
    case ISRRAN_TDEC_SSE8_WINDOW:
      h->dec8[0]          = &sse8_win_impl;
      h->current_llr_type = ISRRAN_TDEC_8;
      break;
#endif /* LV_HAVE_SSE */
#ifdef HAVE_NEON
    case ISRRAN_TDEC_NEON_WINDOW:
      h->dec16[0]         = &arm16_win_impl;
      h->current_llr_type = ISRRAN_TDEC_16;
      break;
#else  /* HAVE_NEON */
    case ISRRAN_TDEC_GENERIC:
      h->dec16[0]         = &gen_impl;
      h->current_llr_type = ISRRAN_TDEC_16;
      break;
#endif /* HAVE_NEON */
#ifdef LV_HAVE_AVX2
    case ISRRAN_TDEC_AVX_WINDOW:
      h->dec16[0]         = &avx16_win_impl;
      h->current_llr_type = ISRRAN_TDEC_16;
      break;
    case ISRRAN_TDEC_AVX8_WINDOW:
  ...
  h->app1 = isrran_vec_i16_malloc(len);
  if (!h->app1) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->app2 = isrran_vec_i16_malloc(len);
  if (!h->app2) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->ext1 = isrran_vec_i16_malloc(len);
  if (!h->ext1) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->ext2 = isrran_vec_i16_malloc(len);
  if (!h->ext2) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->syst0 = isrran_vec_i16_malloc(len);
  if (!h->syst0) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->parity0 = isrran_vec_i16_malloc(len);
  if (!h->parity0) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->parity1 = isrran_vec_i16_malloc(len);
  if (!h->parity1) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }
  h->input_conv = isrran_vec_i16_malloc(len * 3 + 32 * 3);
  if (!h->input_conv) {
    perror("isrran_vec_malloc");
    goto clean_and_exit;
  }

  if (dec_type == ISRRAN_TDEC_AUTO) {
#ifdef HAVE_NEON
    ...
    for (int td = 0; td < ISRRAN_TDEC_NOF_AUTO_MODES_16; td++) {
      if (h->dec16[td]) {
    ...
    for (int td = 0; td < ISRRAN_TDEC_NOF_AUTO_MODES_8; td++) {
      if (h->dec8[td]) {
    ...
    for (int s = 0; s < 4; s++) {
      for (int i = 0; i < ISRRAN_NOF_TC_CB_SIZES; i++) {
        if (isrran_tc_interl_init(&h->interleaver[s][i], isrran_cbsegm_cbsize(i)) < 0) {
          goto clean_and_exit;
        }
        isrran_tc_interl_LTE_gen_interl(&h->interleaver[s][i], isrran_cbsegm_cbsize(i), s ? (8 << (s - 1)) : 1);
      }
    }
  } else {
    uint32_t nof_subblocks;
    if (dec_type < ISRRAN_TDEC_SSE8_WINDOW) {
    ...
    for (int i = 0; i < ISRRAN_NOF_TC_CB_SIZES; i++) {
      if (isrran_tc_interl_init(&h->interleaver[interleaver_idx(nof_subblocks)][i], isrran_cbsegm_cbsize(i)) < 0) {
        goto clean_and_exit;
      }
      isrran_tc_interl_LTE_gen_interl(
          &h->interleaver[interleaver_idx(nof_subblocks)][i], isrran_cbsegm_cbsize(i), nof_subblocks);
    }
  }
...
void isrran_tdec_free(isrran_tdec_t* h)
{
  if (h->app1) {
    free(h->app1);
  }
  ...
  for (int s = 0; s < 4; s++) {
    for (int i = 0; i < ISRRAN_NOF_TC_CB_SIZES; i++) {
      isrran_tc_interl_free(&h->interleaver[s][i]);
    }
  }
...
void isrran_tdec_force_not_sb(isrran_tdec_t* h)
{
  h->force_not_sb = true;
}

static void tdec_decision_byte(isrran_tdec_t* h, uint8_t* output)
{
  if (h->current_llr_type == ISRRAN_TDEC_16) {
...
uint32_t isrran_tdec_autoimp_get_subblocks(uint32_t long_cb)
{
#ifdef LV_HAVE_AVX2
...
{
  uint32_t nof_sb = isrran_tdec_autoimp_get_subblocks(long_cb);
  switch (nof_sb) {
    case 16:
...
uint32_t isrran_tdec_autoimp_get_subblocks_8bit(uint32_t long_cb)
{
#ifdef LV_HAVE_AVX2
...
{
  uint32_t nof_sb = isrran_tdec_autoimp_get_subblocks_8bit(long_cb);
  switch (nof_sb) {
    case 32:
...
static void tdec_iteration_8(isrran_tdec_t* h, int8_t* input)
{
  // Select decoder if in auto mode
  if (h->dec_type == ISRRAN_TDEC_AUTO) {
    h->current_llr_type  = ISRRAN_TDEC_8;
    h->current_dec       = tdec_sb_idx_8(h->current_long_cb);
    h->current_inter_idx = interleaver_idx(h->nof_blocks8[h->current_dec % ISRRAN_TDEC_NOF_AUTO_MODES_8]);
...
static void tdec_iteration_16(isrran_tdec_t* h, int16_t* input)
{
  // Select decoder if in auto mode
  if (h->dec_type == ISRRAN_TDEC_AUTO) {
    h->current_llr_type = ISRRAN_TDEC_16;
  ...
  if (h->current_llr_type == ISRRAN_TDEC_8) {
...
int isrran_tdec_new_cb(isrran_tdec_t* h, uint32_t long_cb)
{
  ...
  h->current_cbidx   = isrran_cbsegm_cbindex(long_cb);
...
void isrran_tdec_iteration(isrran_tdec_t* h, int16_t* input, uint8_t* output)
{
  if (h->current_cbidx >= 0) {
...
int isrran_tdec_run_all(isrran_tdec_t* h, int16_t* input, uint8_t* output, uint32_t nof_iterations, uint32_t long_cb)
{
  if (isrran_tdec_new_cb(h, long_cb)) {
    return ISRRAN_ERROR;
  }
...
void isrran_tdec_iteration_8bit(isrran_tdec_t* h, int8_t* input, uint8_t* output)
{
  if (h->current_cbidx >= 0) {
...
/* Runs nof_iterations iterations and decides the output bits */
int isrran_tdec_run_all_8bit(isrran_tdec_t* h,
                             int8_t*        input,
                             uint8_t*       output,
...
int isrran_tdec_get_nof_iterations(isrran_tdec_t* h)
{
  return h->n_iter;
}
