...
int main(int argc, char** argv)
{
  ...
  rm_bits_s = isrran_vec_i16_malloc(nof_e_bits);
  if (!rm_bits_s) {
    perror("malloc");
    exit(-1);
  }
  rm_bits_f = isrran_vec_f_malloc(nof_e_bits);
  if (!rm_bits_f) {
    perror("malloc");
    exit(-1);
  }
  rm_bits = isrran_vec_u8_malloc(nof_e_bits);
  if (!rm_bits) {
    perror("malloc");
    exit(-1);
  }
  rm_bits2 = isrran_vec_u8_malloc(nof_e_bits);
  if (!rm_bits2) {
    perror("malloc");
    exit(-1);
  }
  rm_bits2_bytes = isrran_vec_u8_malloc(nof_e_bits / 8 + 1);
  if (!rm_bits2_bytes) {
    perror("malloc");
    exit(-1);
  }
  ...
  for (cb_idx = st; cb_idx < end; cb_idx++) {
    for (rv_idx = rv_st; rv_idx < rv_end; rv_idx++) {
      uint32_t long_cb_enc = 3 * isrran_cbsegm_cbsize(cb_idx) + 12;
      ...
      isrran_rm_turbo_tx(buff_b, BUFFSZ, bits, long_cb_enc, rm_bits, nof_e_bits, 0);

      if (rv_idx > 0) {
        isrran_rm_turbo_tx(buff_b, BUFFSZ, bits, long_cb_enc, rm_bits, nof_e_bits, rv_idx);
      }
      ...
      isrran_bit_pack_vector(systematic, systematic_bytes, long_cb_enc / 3);
      isrran_bit_pack_vector(parity, parity_bytes, 2 * long_cb_enc / 3);

      bzero(buff_b, BUFFSZ * sizeof(uint8_t));

      bzero(rm_bits2_bytes, nof_e_bits / 8);
      isrran_rm_turbo_tx_lut(buff_b, systematic_bytes, parity_bytes, rm_bits2_bytes, cb_idx, nof_e_bits, 0, 0);
      if (rv_idx > 0) {
        bzero(rm_bits2_bytes, nof_e_bits / 8);
        isrran_rm_turbo_tx_lut(buff_b, systematic_bytes, parity_bytes, rm_bits2_bytes, cb_idx, nof_e_bits, 0, rv_idx);
      }

      isrran_bit_unpack_vector(rm_bits2_bytes, rm_bits2, nof_e_bits);
      ...
      isrran_vec_f_zero(buff_f, BUFFSZ);
      isrran_rm_turbo_rx(buff_f, BUFFSZ, rm_bits_f, nof_e_bits, bits_f, long_cb_enc, rv_idx, 0);

      bzero(bits2_s, long_cb_enc * sizeof(short));
      isrran_rm_turbo_rx_lut_(rm_bits_s, bits2_s, nof_e_bits, cb_idx, rv_idx, false);
