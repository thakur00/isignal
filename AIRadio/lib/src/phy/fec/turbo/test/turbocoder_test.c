...
int main(int argc, char** argv)
{
  ...
  isrran_tcod_t tcod;
  isrran_tcod_init(&tcod, 6144);

  uint32_t st = 0, end = 187;
  if (long_cb) {
    int n = isrran_cbsegm_cbindex(long_cb);
    if (n < ISRRAN_SUCCESS) {
      return ISRRAN_ERROR;
    }
    st  = (uint32_t)n;
  ...
  for (uint32_t len = st; len <= end; len++) {
    long_cb = isrran_cbsegm_cbsize(len);
    printf("Checking long_cb=%d\n", long_cb);
    for (int i = 0; i < long_cb / 8; i++) {
      input_bytes[i] = isrran_random_uniform_int_dist(random_gen, 0, 256);
    }

    isrran_bit_unpack_vector(input_bytes, input_bits, long_cb);

    if (ISRRAN_VERBOSE_ISINFO()) {
      printf("Input bits:\n");
      for (int i = 0; i < long_cb / 8; i++) {
        isrran_vec_fprint_b(stdout, &input_bits[i * 8], 8);
      }
    }
    ...
    if (isrran_crc_init(&crc_tb, ISRRAN_LTE_CRC24A, 24)) {
      printf("error initialising CRC\n");
      exit(-1);
    }

    isrran_tcod_encode(&tcod, input_bits, output_bits, long_cb);
    isrran_tcod_encode_lut(&tcod, &crc_tb, NULL, input_bytes, parity, len, false);

    isrran_bit_unpack_vector(parity, parity_bits, 2 * (long_cb + 4));
    ...
    if (ISRRAN_VERBOSE_ISINFO()) {
      isrran_vec_fprint_b(stdout, output_bits2, 3 * long_cb);
      isrran_vec_fprint_b(stdout, output_bits, 3 * long_cb);
      printf("\n");
    }
