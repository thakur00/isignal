...
int main(int argc, char** argv)
{
  ...
  bits = isrran_vec_u8_malloc(nof_tx_bits);
  if (!bits) {
    perror("malloc");
    exit(-1);
  }
  rm_bits = isrran_vec_u8_malloc(nof_rx_bits);
  if (!rm_bits) {
    perror("malloc");
    exit(-1);
  }
  rm_symbols = isrran_vec_f_malloc(nof_rx_bits);
  if (!rm_symbols) {
    perror("malloc");
    exit(-1);
  }
  unrm_symbols = isrran_vec_f_malloc(nof_tx_bits);
  if (!unrm_symbols) {
    perror("malloc");
    exit(-1);
  }
  ...
  if (isrran_rm_conv_tx(bits, nof_tx_bits, rm_bits, nof_rx_bits)) {
    exit(-1);
  }
  ...
  if (isrran_rm_conv_rx(rm_symbols, nof_rx_bits, unrm_symbols, nof_tx_bits)) {
    exit(-1);
  }
