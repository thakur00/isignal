...
#ifdef LV_HAVE_SSE
#include <x86intrin.h>
int isrran_rm_turbo_rx_lut_sse(int16_t*  input,
                               int16_t*  output,
                               uint16_t* deinter,
                               uint32_t  in_len,
                               uint32_t  cb_idx,
                               uint32_t  rv_idx);
int isrran_rm_turbo_rx_lut_sse_8bit(int8_t*   input,
                                    int8_t*   output,
                                    uint16_t* deinter,
                                    uint32_t  in_len,
                                    uint32_t  cb_idx,
...
#ifdef LV_HAVE_AVX
#include <x86intrin.h>
int isrran_rm_turbo_rx_lut_avx(int16_t*  input,
                               int16_t*  output,
                               uint16_t* deinter,
                               uint32_t  in_len,
                               uint32_t  cb_idx,
                               uint32_t  rv_idx);
int isrran_rm_turbo_rx_lut_avx_8bit(int8_t*   input,
                                    int8_t*   output,
                                    uint16_t* deinter,
                                    uint32_t  in_len,
                                    uint32_t  cb_idx,
...
static isrran_bit_interleaver_t bit_interleavers_systematic_bits[192];
static uint16_t                 interleaver_parity_bits[192][2 * 6160];
static isrran_bit_interleaver_t bit_interleavers_parity_bits[192];
static uint16_t                 deinterleaver[192][4][18448];
static int                      k0_vec[ISRRAN_NOF_TC_CB_SIZES][4][2];
static bool                     rm_turbo_tables_generated = false;

// Store deinterleaver version for sub-block turbo decoder
#if ISRRAN_TDEC_EXPECT_INPUT_SB == 1
...
static void isrran_rm_turbo_gentable_systematic(uint16_t* table_bits, int k0_vec_[4][2], uint32_t nrows, int ndummy)
{
...
static void
isrran_rm_turbo_gentable_parity(uint16_t* table_parity, int k0_vec_[4][2], int offset, uint16_t nrows, int ndummy)
{
  bool last_is_null = true;
...
static void isrran_rm_turbo_gentable_receive(uint16_t* table, uint32_t cb_len, uint32_t rv_idx)
{
...
{
  int long_cb = isrran_cbsegm_cbsize(cb_idx);
...
void isrran_rm_turbo_gentables()
{
  if (!rm_turbo_tables_generated) {
    rm_turbo_tables_generated = true;
    for (int cb_idx = 0; cb_idx < ISRRAN_NOF_TC_CB_SIZES; cb_idx++) {
      int cb_len = isrran_cbsegm_cbsize(cb_idx);
      ...
      isrran_rm_turbo_gentable_systematic(interleaver_systematic_bits[cb_idx], k0_vec[cb_idx], nrows, ndummy);
      isrran_bit_interleaver_init(&bit_interleavers_systematic_bits[cb_idx],
                                  interleaver_systematic_bits[cb_idx],
                                  (uint32_t)isrran_cbsegm_cbsize(cb_idx) + 4);

      isrran_rm_turbo_gentable_parity(interleaver_parity_bits[cb_idx], k0_vec[cb_idx], in_len / 3, nrows, ndummy);
      isrran_bit_interleaver_init(&bit_interleavers_parity_bits[cb_idx],
                                  interleaver_parity_bits[cb_idx],
                                  (uint32_t)(isrran_cbsegm_cbsize(cb_idx) + 4) * 2);

      for (int i = 0; i < 4; i++) {
        isrran_rm_turbo_gentable_receive(deinterleaver[cb_idx][i], in_len, i);
...
void isrran_rm_turbo_free_tables()
{
  if (rm_turbo_tables_generated) {
    for (int i = 0; i < ISRRAN_NOF_TC_CB_SIZES; i++) {
      isrran_bit_interleaver_free(&bit_interleavers_systematic_bits[i]);
      isrran_bit_interleaver_free(&bit_interleavers_parity_bits[i]);
    }
 ...
 * @return Error code
 */
int isrran_rm_turbo_tx_lut(uint8_t* w_buff,
                           uint8_t* systematic,
                           uint8_t* parity,
                           uint8_t* output,
                           uint32_t cb_idx,
                           ...
                           uint32_t rv_idx)
{
  if (rv_idx < 4 && cb_idx < ISRRAN_NOF_TC_CB_SIZES) {
    int in_len = 3 * isrran_cbsegm_cbsize(cb_idx) + 12;

    /* Sub-block interleaver (5.1.4.1.1) and bit collection */
    if (rv_idx == 0) {
      // Systematic bits
      // isrran_bit_interleave(systematic, w_buff, interleaver_systematic_bits[cb_idx], in_len/3);
      isrran_bit_interleaver_run(&bit_interleavers_systematic_bits[cb_idx], systematic, w_buff, 0);

      // Parity bits
      // isrran_bit_interleave_w_offset(parity, &w_buff[in_len/24], interleaver_parity_bits[cb_idx], 2*in_len/3, 4);
      isrran_bit_interleaver_run(&bit_interleavers_parity_bits[cb_idx], parity, &w_buff[in_len / 24], 4);
    }
    ...
    while (w_len < out_len) {
      ...
      isrran_bit_copy(output, w_len + w_offset, w_buff, r_ptr, cp_len);
      r_ptr += cp_len;
      if (r_ptr >= in_len) {
        r_ptr -= in_len;
      }
...
int isrran_rm_turbo_rx_lut(int16_t* input, int16_t* output, uint32_t in_len, uint32_t cb_idx, uint32_t rv_idx)
{
  return isrran_rm_turbo_rx_lut_(input, output, in_len, cb_idx, rv_idx, true);
}
/**
 ...
 * @param[out] output Output buffer of size 3*isrran_cbsegm_cbsize(cb_idx)+12
 * @param[in] cb_idx Code block table index
 * @param[in] rv_idx Redundancy Version from DCI control message
 * @return Error code
 */
int isrran_rm_turbo_rx_lut_(int16_t* input,
                            int16_t* output,
                            uint32_t in_len,
                            uint32_t cb_idx,
                            uint32_t rv_idx,
                            bool     enable_input_tdec)
{
  if (rv_idx < 4 && cb_idx < ISRRAN_NOF_TC_CB_SIZES) {
#if ISRRAN_TDEC_EXPECT_INPUT_SB == 1
    int       cb_len  = isrran_cbsegm_cbsize(cb_idx);
    int       idx     = deinter_table_idx_from_sb_len(isrran_tdec_autoimp_get_subblocks(cb_len));
    ...
    if (idx < 0 || !enable_input_tdec) {
      ...
      ERROR("Sub-block size index %d not supported in isrran_rm_turbo_rx_lut()", idx);
      return -1;
    }
#else
...
#ifdef LV_HAVE_AVX
    return isrran_rm_turbo_rx_lut_avx(input, output, deinter, in_len, cb_idx, rv_idx);
#else
#ifdef LV_HAVE_SSE
    return isrran_rm_turbo_rx_lut_sse(input, output, deinter, in_len, cb_idx, rv_idx);
#else
    uint32_t out_len = 3 * isrran_cbsegm_cbsize(cb_idx) + 12;
...
int isrran_rm_turbo_rx_lut_8bit(int8_t* input, int8_t* output, uint32_t in_len, uint32_t cb_idx, uint32_t rv_idx)
{
  if (rv_idx < 4 && cb_idx < ISRRAN_NOF_TC_CB_SIZES) {
#if ISRRAN_TDEC_EXPECT_INPUT_SB == 1
    int       cb_len  = isrran_cbsegm_cbsize(cb_idx);
    int       idx     = deinter_table_idx_from_sb_len(isrran_tdec_autoimp_get_subblocks_8bit(cb_len));
    uint16_t* deinter = NULL;
    if (idx < 0) {
      deinter = deinterleaver[cb_idx][rv_idx];
    } else if (idx < NOF_DEINTER_TABLE_SB_IDX) {
...
#ifdef LV_HAVE_SSE
    return isrran_rm_turbo_rx_lut_sse_8bit(input, output, deinter, in_len, cb_idx, rv_idx);
#else
    uint32_t  out_len = 3 * isrran_cbsegm_cbsize(cb_idx) + 12;
 ...
 */
int isrran_rm_turbo_tx(uint8_t* w_buff,
                       uint32_t w_buff_len,
                       uint8_t* input,
                       uint32_t in_len,
                       uint8_t* output,
                       ...
                       uint32_t rv_idx)
{
  ...
  if (rv_idx == 0) {
    ...
    for (k = 0; k < K_p; k++) {
      ...
      if ((kidx - ndummy) < 0) {
        w_buff[K_p + 2 * k + 1] = ISRRAN_TX_NULL;
      } else {
  ...
  while (k < out_len) {
    if (w_buff[(k0 + j) % N_cb] != ISRRAN_TX_NULL) {
 ...
 */
int isrran_rm_turbo_rx(float*   w_buff,
                       uint32_t w_buff_len,
                       float*   input,
                       uint32_t in_len,
                       float*   output,
                       ...
                       uint32_t nof_filler_bits)
{
  ...
  while (k < in_len) {
    ...
    if (!isdummy) {
      if (w_buff[jp] == ISRRAN_RX_NULL) {
        w_buff[jp] = input[k];
      } else if (input[k] != ISRRAN_RX_NULL) {
  ...
  // printf("wbuff:\n");
  // isrran_vec_fprint_f(stdout, w_buff, out_len);
