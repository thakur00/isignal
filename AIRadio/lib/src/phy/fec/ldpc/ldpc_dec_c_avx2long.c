...
void* create_ldpc_dec_c_avx2long(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = ISRRAN_MEM_ALLOC(struct ldpc_regs_c_avx2long, 1)) == NULL) {
    return NULL;
  }
  ISRRAN_MEM_ZERO(vp, struct ldpc_regs_c_avx2long, 1);
  ...
  if ((vp->check_to_var = ISRRAN_MEM_ALLOC(__m256i, (hrr + 1) * bgM * n_subnodes)) == NULL) {
    delete_ldpc_dec_c_avx2long(vp);
    return NULL;
  }

  if ((vp->var_to_check_to_free = ISRRAN_MEM_ALLOC(__m256i, (hrr + 1) * n_subnodes + 2)) == NULL) {
    delete_ldpc_dec_c_avx2long(vp);
    return NULL;
  }
  ...
  if ((vp->rotated_v2c = ISRRAN_MEM_ALLOC(__m256i, (hrr + 1) * n_subnodes)) == NULL) {
    delete_ldpc_dec_c_avx2long(vp);
    return NULL;
  }
...
int init_ldpc_dec_c_avx2long(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  for (i = 2; i < vp->bgN; i++) {
    ...
    isrran_vec_i8_zero(&(vp->soft_bits[i * vp->n_subnodes + j - 1].c[k]), ISRRAN_AVX2_B_SIZE - k);
  }

  ISRRAN_MEM_ZERO(vp->check_to_var, __m256i, (vp->hrr + 1) * vp->bgM * vp->n_subnodes);
  ISRRAN_MEM_ZERO(vp->var_to_check, __m256i, (vp->hrr + 1) * vp->n_subnodes);
  ISRRAN_MEM_ZERO(vp->this_c2v_epi8_to_free, __m256i, vp->n_subnodes + 2);
  ISRRAN_MEM_ZERO(vp->min_ix_epi8, __m256i, vp->n_subnodes);
  ISRRAN_MEM_ZERO(vp->var_to_check_to_free, __m256i, (vp->hrr + 1) * vp->n_subnodes + 2);
  return 0;
}
