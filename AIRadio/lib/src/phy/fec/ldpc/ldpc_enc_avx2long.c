...
typedef union bg_node_t {
  uint8_t c[ISRRAN_AVX2_B_SIZE]; /*!< Each base node may contain up to \ref ISRRAN_AVX2_B_SIZE lifted nodes. */
...
struct ldpc_enc_avx2long {
  ...
  bg_node_t* codeword_to_free;   /*!< \brief Auxiliary pointer with a free memory of size ISRRAN_AVX2_B_SIZE previous to
                                    codeword */
  ...
  __m256i* rotated_node_to_free; /*!< \brief Auxiliary pointer to store rotated versions of the nodes with extra free
                                    memory of size ISRRAN_AVX2_B_SIZE previous to rotated_node */
...
void* create_ldpc_enc_avx2long(isrran_ldpc_encoder_t* q)
{
  ...
  int left_out   = q->ls % ISRRAN_AVX2_B_SIZE;
  vp->n_subnodes = q->ls / ISRRAN_AVX2_B_SIZE + (left_out > 0);

  if ((vp->codeword_to_free = isrran_vec_malloc((q->bgN * vp->n_subnodes + 1) * sizeof(bg_node_t))) == NULL) {
    free(vp);
    return NULL;
  }
  vp->codeword = &vp->codeword_to_free[1];

  if ((vp->aux = isrran_vec_malloc(q->bgM * vp->n_subnodes * sizeof(__m256i))) == NULL) {
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
  }

  if ((vp->rotated_node_to_free = isrran_vec_malloc((vp->n_subnodes + 2) * sizeof(__m256i))) == NULL) {
    free(vp->aux);
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
  ...
  vp->node_size = ISRRAN_AVX2_B_SIZE * vp->n_subnodes;
  return vp;
}
...
int load_avx2long(void* p, const uint8_t* input, const uint8_t msg_len, const uint8_t cdwd_len, const uint16_t ls)
{
  ...
  ISRRAN_MEM_ZERO(vp->codeword + msg_len * vp->n_subnodes, bg_node_t, (cdwd_len - msg_len) * (uint32_t)vp->n_subnodes);
...
void preprocess_systematic_bits_avx2long(isrran_ldpc_encoder_t* q)
{
  ...
  ISRRAN_MEM_ZERO(vp->aux, __m256i, M * vp->n_subnodes);
