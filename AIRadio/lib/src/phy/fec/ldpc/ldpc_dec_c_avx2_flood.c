...
typedef union bg_node_t {
  int8_t*  c; /*!< Each base node may contain up to \ref ISRRAN_AVX2_B_SIZE lifted nodes. */
...
void* create_ldpc_dec_c_avx2_flood(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = ISRRAN_MEM_ALLOC(struct ldpc_regs_c_avx2_flood, 1)) == NULL) {
    return NULL;
  }
  ISRRAN_MEM_ZERO(vp, struct ldpc_regs_c_avx2_flood, 1);

  if ((vp->llrs = ISRRAN_MEM_ALLOC(__m256i, bgN)) == NULL) {
    delete_ldpc_dec_c_avx2_flood(vp);
    return NULL;
  }

  if ((vp->soft_bits.v = ISRRAN_MEM_ALLOC(__m256i, bgN)) == NULL) {
    delete_ldpc_dec_c_avx2_flood(vp);
    return NULL;
  }

  uint32_t sz = (uint32_t)(hrr + 1) * (uint32_t)bgM;
  if ((vp->check_to_var = ISRRAN_MEM_ALLOC(__m256i, sz)) == NULL) {
    delete_ldpc_dec_c_avx2_flood(vp);
    return NULL;
  }

  if ((vp->var_to_check = ISRRAN_MEM_ALLOC(__m256i, sz)) == NULL) {
    delete_ldpc_dec_c_avx2_flood(vp);
    return NULL;
  }

  if ((vp->rotated_v2c = ISRRAN_MEM_ALLOC(__m256i, hrr + 1)) == NULL) {
    delete_ldpc_dec_c_avx2_flood(vp);
    return NULL;
  }
...
int init_ldpc_dec_c_avx2_flood(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  for (i = 2; i < vp->bgN; i++) {
    for (j = 0; j < ls; j++) {
      vp->soft_bits.c[i * ISRRAN_AVX2_B_SIZE + j] = llrs[(i - 2) * ls + j];
    }
    isrran_vec_i8_zero(&(vp->soft_bits.c[i * ISRRAN_AVX2_B_SIZE + ls]), ISRRAN_AVX2_B_SIZE - ls);
    vp->llrs[i] = vp->soft_bits.v[i];
  }

  ISRRAN_MEM_ZERO(vp->check_to_var, __m256i, (vp->hrr + 1) * (uint32_t)vp->bgM);
  ISRRAN_MEM_ZERO(vp->var_to_check, __m256i, (vp->hrr + 1) * (uint32_t)vp->bgM);
  return 0;
}
...
int extract_ldpc_message_c_avx2_flood(void* p, uint8_t* message, uint16_t liftK)
{
  ...
  for (int i = 0; i < liftK / vp->ls; i++) {
    for (j = 0; j < vp->ls; j++) {
      message[i * vp->ls + j] = (vp->soft_bits.c[i * ISRRAN_AVX2_B_SIZE + j] < 0);
    }
  }
