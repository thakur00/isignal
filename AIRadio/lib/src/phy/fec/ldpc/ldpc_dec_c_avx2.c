...
void* create_ldpc_dec_c_avx2(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = ISRRAN_MEM_ALLOC(struct ldpc_regs_c_avx2, 1)) == NULL) {
    return NULL;
  }
  ISRRAN_MEM_ZERO(vp, struct ldpc_regs_c_avx2, 1);
  ...
  if ((vp->check_to_var = ISRRAN_MEM_ALLOC(__m256i, (hrr + 1) * (uint32_t)bgM)) == NULL) {
    delete_ldpc_dec_c_avx2(vp);
    return NULL;
  }

  if ((vp->var_to_check = ISRRAN_MEM_ALLOC(__m256i, hrr + 1)) == NULL) {
    delete_ldpc_dec_c_avx2(vp);
    return NULL;
  }
...
int init_ldpc_dec_c_avx2(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  for (i = 2; i < vp->bgN; i++) {
    ...
    ISRRAN_MEM_ZERO(&(vp->soft_bits.c[i * ISRRAN_AVX2_B_SIZE + ls]), int8_t, ISRRAN_AVX2_B_SIZE - ls);
  }

  ISRRAN_MEM_ZERO(vp->check_to_var, __m256i, (vp->hrr + 1) * (uint32_t)vp->bgM);
  ISRRAN_MEM_ZERO(vp->var_to_check, __m256i, vp->hrr + 1);
  return 0;
}
