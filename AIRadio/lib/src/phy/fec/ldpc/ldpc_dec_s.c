...
void* create_ldpc_dec_s(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp->soft_bits = isrran_vec_i16_malloc(liftN)) == NULL) {
    free(vp);
    return NULL;
  }

  if ((vp->check_to_var = isrran_vec_i16_malloc((hrrN + ls) * bgM)) == NULL) {
    free(vp->soft_bits);
    free(vp);
    return NULL;
  }

  if ((vp->var_to_check = isrran_vec_i16_malloc(hrrN + ls)) == NULL) {
...
int init_ldpc_dec_s(void* p, const int16_t* llrs, uint16_t ls)
{
  ...
  isrran_vec_i16_zero(vp->soft_bits, skip);
  for (i = skip; i < vp->liftN; i++) {
    vp->soft_bits[i] = llrs[i - skip];
  }

  isrran_vec_i16_zero(vp->check_to_var, (vp->hrrN + vp->ls) * (uint32_t)vp->bgM);
  isrran_vec_i16_zero(vp->var_to_check, vp->hrrN + vp->ls);
  return 0;
}
