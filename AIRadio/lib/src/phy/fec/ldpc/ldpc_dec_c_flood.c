...
void* create_ldpc_dec_c_flood(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp->llrs = isrran_vec_i8_malloc(liftN)) == NULL) {
    free(vp);
    return NULL;
  }

  if ((vp->soft_bits = isrran_vec_i8_malloc(liftN)) == NULL) {
    free(vp->llrs);
    free(vp);
    return NULL;
  }

  if ((vp->check_to_var = isrran_vec_i8_malloc((hrrN + ls) * bgM)) == NULL) {
    free(vp->soft_bits);
    free(vp->llrs);
    free(vp);
    return NULL;
  ...
  if ((vp->var_to_check = isrran_vec_i8_malloc((hrrN + ls) * bgM)) == NULL) {
  ...
  if ((vp->min_v_index = isrran_vec_i32_malloc(ls)) == NULL) {
    free(vp->min_v2c);
    free(vp->var_to_check);
    free(vp->check_to_var);
    free(vp->soft_bits);
  ...
  if ((vp->prod_v2c = isrran_vec_i32_malloc(ls)) == NULL) {
    free(vp->min_v_index);
    free(vp->min_v2c);
    free(vp->var_to_check);
    free(vp->check_to_var);
...
int init_ldpc_dec_c_flood(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  isrran_vec_i8_zero(vp->llrs, skip);
  isrran_vec_i8_zero(vp->soft_bits, skip);
  for (i = skip; i < vp->liftN; i++) {
    vp->llrs[i]      = llrs[i - skip];
    vp->soft_bits[i] = llrs[i - skip];
  }

  isrran_vec_i8_zero(vp->check_to_var, (vp->hrrN + vp->ls) * (uint32_t)vp->bgM);
  isrran_vec_i8_zero(vp->var_to_check, (vp->hrrN + vp->ls) * (uint32_t)vp->bgM);
  return 0;
}
