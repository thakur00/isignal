...
int main(int argc, char** argv)
{
  ...
  symbols_rm                = isrran_vec_f_malloc((rm_length + F) * batch_size);
  ...
  while (((n_error_words_f < req_errors) || (n_error_words_s < req_errors) || (n_error_words_c < req_errors)) &&
    ...
    // Apply AWGN
    isrran_ch_awgn_f(symbols_rm, symbols_rm, noise_var, batch_size * (rm_length + F));
    ...
    gettimeofday(&t[1], NULL);
    for (j = 0; j < batch_size; j++) {
      isrran_ldpc_decoder_decode_f(&decoder_f, symbols + j * finalN, messages_sim_f + j * finalK, n_useful_symbols);
    }
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
    ...
    isrran_vec_quant_fs(symbols, symbols_s, gain_s, 0, inf15, batch_size * finalN);

    // Recover messages
    gettimeofday(&t[1], NULL);
    for (j = 0; j < batch_size; j++) {
      isrran_ldpc_decoder_decode_s(&decoder_s, symbols_s + j * finalN, messages_sim_s + j * finalK, n_useful_symbols);
    }
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
    ...
    isrran_vec_quant_fc(symbols, symbols_c, gain_c, 0, inf7, batch_size * finalN);

    // Recover messages
    gettimeofday(&t[1], NULL);
    for (j = 0; j < batch_size; j++) {
      isrran_ldpc_decoder_decode_c(&decoder_c, symbols_c + j * finalN, messages_sim_c + j * finalK, n_useful_symbols);
    }
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
