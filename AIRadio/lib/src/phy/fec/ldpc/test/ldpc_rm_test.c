...
static isrran_basegraph_t base_graph = BG2; /*!< \brief Base Graph (BG1 or BG2). */
...
static isrran_mod_t       mod_type = ISRRAN_MOD_QPSK; /*!< \brief Modulation type: BPSK, QPSK, QAM16, QAM64, QAM256. */
...
int main(int argc, char** argv)
{
  ...
  codeblocks     = isrran_vec_u8_malloc(C * K);
  codewords      = isrran_vec_u8_malloc(C * N);
  rm_codewords   = isrran_vec_u8_malloc(C * E);
  rm_symbols     = isrran_vec_f_malloc(C * E);
  rm_symbols_s   = isrran_vec_i16_malloc(C * E);
  rm_symbols_c   = isrran_vec_i8_malloc(C * E);
  unrm_symbols   = isrran_vec_f_malloc(C * N);
  unrm_symbols_s = isrran_vec_i16_malloc(C * N);
  unrm_symbols_c = isrran_vec_i8_malloc(C * N);
  ...
  for (r = 0; r < C; r++) {
    for (i = 0; i < K - F; i++) {
      codeblocks[r * K + i] = isrran_random_uniform_int_dist(random_gen, 0, 1);
    }
  ...
  for (r = 0; r < C; r++) {
    if (isrran_ldpc_encoder_encode(&encoder, codeblocks + r * K, codewords + r * N, K)) {
      exit(-1);
    }

    // LDPC rate matching
    if (isrran_ldpc_rm_tx(
