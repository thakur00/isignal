...
isrran_basegraph_t base_graph = BG1; /*!< \brief Base Graph (BG1 or BG2). */
...
int main(int argc, char** argv)
{
  ...
  isrran_ldpc_decoder_type_t dectype =
      (scheduling == 0) ? ISRRAN_LDPC_DECODER_C_AVX2 : ISRRAN_LDPC_DECODER_C_AVX2_FLOOD;
  ...
  isrran_ldpc_decoder_t decoder;
  if (isrran_ldpc_decoder_init(&decoder, &decoder_args) != 0) {
    perror("decoder init");
    exit(-1);
  }
  ...
  messages_true = isrran_vec_u8_malloc(finalK * NOF_MESSAGES);
  messages_sim  = isrran_vec_u8_malloc(finalK * NOF_MESSAGES);
  codewords     = isrran_vec_u8_malloc(finalN * NOF_MESSAGES);
  symbols       = isrran_vec_i8_malloc(finalN * NOF_MESSAGES);
  ...
  for (j = 0; j < NOF_MESSAGES; j++) {
    ...
    for (l = 0; l < nof_reps; l++) {
      isrran_ldpc_decoder_decode_c(&decoder, symbols + j * finalN, messages_sim + j * finalK, finalN);
    }
  ...
  free(messages_sim);
  free(messages_true);
  isrran_ldpc_decoder_free(&decoder);
}
