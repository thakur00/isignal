...
int main(int argc, char** argv)
{
  ...
  messages       = isrran_vec_u8_malloc(finalK * NOF_MESSAGES);
  codewords_true = isrran_vec_u8_malloc(finalN * NOF_MESSAGES);
  codewords_sim  = isrran_vec_u8_malloc(finalN * NOF_MESSAGES);
  ...
  for (j = 0; j < NOF_MESSAGES; j++) {
    ...
    for (l = 0; l < nof_reps; l++) {
      isrran_ldpc_encoder_encode_rm(&encoder, messages + j * finalK, codewords_sim + j * finalN, finalK, finalN);
    }
