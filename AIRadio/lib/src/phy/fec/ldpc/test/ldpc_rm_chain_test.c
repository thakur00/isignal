...
static isrran_basegraph_t base_graph = BG1;     /*!< \brief Base Graph (BG1 or BG2). */
...
static isrran_mod_t mod_type = ISRRAN_MOD_BPSK; /*!< \brief Modulation type: BPSK, QPSK, QAM16, QAM64, QAM256 = 4 */
  ...
  while ((opt = getopt(argc, argv, "b:l:e:f:r:m:w:M:s:B:N:E:")) != -1) {
    switch (opt) {
      ...
      case 'r':
        ...
        break;
      case 'm':
        mod_type = (isrran_mod_t)strtol(optarg, NULL, 10);
        break;
      case 'M':
...
int main(int argc, char** argv)
{
  ...
  // LDPC encoder
  isrran_ldpc_encoder_t encoder = {};

  // LDPC decoder (8 bit)
  isrran_ldpc_decoder_t decoder_c = {};
  // LDPC decoder (8 bit, flooded)
  isrran_ldpc_decoder_t decoder_c_flood = {};
  // LDPC decoder (16 bit)
  isrran_ldpc_decoder_t decoder_s = {};
  // LDPC decoder (float)
  isrran_ldpc_decoder_t decoder_f = {};

#ifdef LV_HAVE_AVX2
  // LDPC decoder (8 bit, AVX2 version)
  isrran_ldpc_decoder_t decoder_avx = {};
  // LDPC decoder (8 bit, flooded scheduling, AVX2 version)
  isrran_ldpc_decoder_t decoder_avx_flood = {};

#endif

#ifdef LV_HAVE_AVX512
  // LDPC decoder (8 bit, AVX512 version)
  isrran_ldpc_decoder_t decoder_avx512 = {};
  // LDPC decoder (8 bit, flooded scheduling, AVX512 version)
  isrran_ldpc_decoder_t decoder_avx512_flood = {};
#endif

  // LDPC rate Matcher
  isrran_ldpc_rm_t rm_tx = {};
  // LDPC rate DeMatcher
  isrran_ldpc_rm_t rm_rx = {};
  // LDPC rate DeMatcher (int16_t)
  isrran_ldpc_rm_t rm_rx_s = {};
  // LDPC rate DeMatcher (int8_t)
  isrran_ldpc_rm_t rm_rx_c = {};

  // Create a random generator
  isrran_random_t random_gen = NULL;
...
#ifdef LV_HAVE_AVX512
  if (isrran_ldpc_encoder_init(&encoder, ISRRAN_LDPC_ENCODER_AVX512, base_graph, lift_size) != 0) {
    perror("encoder init");
    goto clean_exit;
  }
#else // no AVX512
#ifdef LV_HAVE_AVX2
  if (isrran_ldpc_encoder_init(&encoder, ISRRAN_LDPC_ENCODER_AVX2, base_graph, lift_size) != 0) {
    perror("encoder init");
    goto clean_exit;
  }
#else  // no AVX2
  if (isrran_ldpc_encoder_init(&encoder, ISRRAN_LDPC_ENCODER_C, base_graph, lift_size) != 0) {
    perror("encoder init");
    goto clean_exit;
  }
#endif // LV_HAVE_AVX2
  ...
  decoder_args.type = ISRRAN_LDPC_DECODER_F;
  if (isrran_ldpc_decoder_init(&decoder_f, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }
  // Init the LDPC decoder (16 bit)
  decoder_args.type = ISRRAN_LDPC_DECODER_S;
  if (isrran_ldpc_decoder_init(&decoder_s, &decoder_args) != 0) {
    perror("decoder init (int16_t)");
    goto clean_exit;
  }
  // Init the LDPC decoder (8 bit)
  decoder_args.type = ISRRAN_LDPC_DECODER_C;
  if (isrran_ldpc_decoder_init(&decoder_c, &decoder_args) != 0) {
    perror("decoder init (int8_t)");
    goto clean_exit;
  }
  // Init the LDPC decoder (8 bit, flooded)
  decoder_args.type = ISRRAN_LDPC_DECODER_C_FLOOD;
  if (isrran_ldpc_decoder_init(&decoder_c_flood, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }
#ifdef LV_HAVE_AVX2
  // Init the LDPC decoder (8 bit, AVX2 version)
  decoder_args.type = ISRRAN_LDPC_DECODER_C_AVX2;
  if (isrran_ldpc_decoder_init(&decoder_avx, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }

  // Init the LDPC decoder (8 bit, flooded scheduling, AVX2 version)
  decoder_args.type = ISRRAN_LDPC_DECODER_C_AVX2_FLOOD;
  if (isrran_ldpc_decoder_init(&decoder_avx_flood, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }
#endif // LV_HAVE_AVX2
  ...
  decoder_args.type = ISRRAN_LDPC_DECODER_C_AVX512;
  if (isrran_ldpc_decoder_init(&decoder_avx512, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }

  // Init LDPC decoder (8 bit, flooded scheduling, AVX512 version)
  decoder_args.type = ISRRAN_LDPC_DECODER_C_AVX512_FLOOD;
  if (isrran_ldpc_decoder_init(&decoder_avx512_flood, &decoder_args) != 0) {
    perror("decoder init");
    goto clean_exit;
  }
#endif // LV_HAVE_AVX512
  ...
  messages_true             = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_f            = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_s            = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_c            = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_c_flood      = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_avx          = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_avx_flood    = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_avx512       = isrran_vec_u8_malloc(finalK * batch_size);
  messages_sim_avx512_flood = isrran_vec_u8_malloc(finalK * batch_size);
  codewords                 = isrran_vec_u8_malloc(finalN * batch_size);
  rm_codewords              = isrran_vec_u8_malloc(rm_length * batch_size);
  rm_symbols                = isrran_vec_f_malloc(rm_length * batch_size);
  rm_symbols_s              = isrran_vec_i16_malloc(rm_length * batch_size);
  rm_symbols_c              = isrran_vec_i8_malloc(rm_length * batch_size);

  symbols   = isrran_vec_f_malloc(finalN * batch_size);
  symbols_s = isrran_vec_i16_malloc(finalN * batch_size);
  symbols_c = isrran_vec_i8_malloc(finalN * batch_size);
  ...
  float noise_var     = isrran_convert_dB_to_power(-snr);
  float noise_std_dev = isrran_convert_dB_to_amplitude(-snr);
  ...
  while (((n_error_words_f < req_errors) || (n_error_words_s < req_errors) || (n_error_words_c < req_errors)) &&
    ...
    for (i = 0; i < batch_size; i++) {
      for (j = 0; j < finalK - F; j++) {
        messages_true[i * finalK + j] = isrran_random_uniform_int_dist(random_gen, 0, 1);
      }
      for (; j < finalK; j++) {
    ...
    // Apply AWGN
    isrran_ch_awgn_f(rm_symbols, rm_symbols, noise_var, batch_size * rm_length);
    ...
    gettimeofday(&t[1], NULL);
    for (j = 0; j < batch_size; j++) {
      isrran_ldpc_decoder_decode_f(&decoder_f, symbols + j * finalN, messages_sim_f + j * finalK, n_useful_symbols_dec);
    }
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
    ...
    isrran_vec_quant_fs(rm_symbols, rm_symbols_s, gain_s, 0, inf15, batch_size * rm_length);

    // Rate dematcher
    for (i = 0; i < batch_size; i++) {
      if (isrran_ldpc_rm_rx_s(&rm_rx_s,
    ...
    isrran_vec_quant_fc(rm_symbols, rm_symbols_c, gain_c, 0, inf7, batch_size * rm_length);

    // Rate dematcher
    for (i = 0; i < batch_size; i++) {
      if (isrran_ldpc_rm_rx_c(&rm_rx_c,
    ...
    gettimeofday(&t[1], NULL);
    for (j = 0; j < batch_size; j++) {
      isrran_ldpc_decoder_decode_c(&decoder_avx512, symbols_c + j * finalN, messages_sim_avx512 + j * finalK, finalN);
    }
    gettimeofday(&t[2], NULL);
    get_time_interval(t);
  ...
  if (print_decoder("FLOATING POINT", i_batch, n_error_words_f, elapsed_time_dec_f) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
  if (print_decoder("FIXED POINT (16 bits)", i_batch, n_error_words_s, elapsed_time_dec_s) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
  if (print_decoder("FIXED POINT (8 bits)", i_batch, n_error_words_c, elapsed_time_dec_c) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
  if (print_decoder(
...
#ifdef LV_HAVE_AVX2
  if (print_decoder("FIXED POINT (8 bits - AVX2)", i_batch, n_error_words_avx, elapsed_time_dec_avx) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
  if (print_decoder("FIXED POINT (8 bits, flooded scheduling - AVX2)",
                    i_batch,
                    n_error_words_avx_flood,
                    elapsed_time_dec_avx_flood) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
#endif // LV_HAVE_AVX2
  ...
  if (print_decoder("FIXED POINT (8 bits, flooded scheduling - AVX512)",
                    i_batch,
                    n_error_words_avx512_flood,
                    elapsed_time_dec_avx512_flood) < ISRRAN_SUCCESS) {
    goto clean_exit;
  }
#endif // LV_HAVE_AVX512
...
#ifdef LV_HAVE_AVX2
  isrran_ldpc_decoder_free(&decoder_avx);
  isrran_ldpc_decoder_free(&decoder_avx_flood);
#endif // LV_HAVE_AVX2
#ifdef LV_HAVE_AVX512
  isrran_ldpc_decoder_free(&decoder_avx512);
  isrran_ldpc_decoder_free(&decoder_avx512_flood);
#endif // LV_HAVE_AVX512
  isrran_ldpc_decoder_free(&decoder_c_flood);
  isrran_ldpc_decoder_free(&decoder_c);
  isrran_ldpc_decoder_free(&decoder_s);
  isrran_ldpc_decoder_free(&decoder_f);
  isrran_ldpc_encoder_free(&encoder);
  isrran_ldpc_rm_tx_free(&rm_tx);
  isrran_ldpc_rm_rx_free_f(&rm_rx);
  isrran_ldpc_rm_rx_free_s(&rm_rx_s);
  isrran_ldpc_rm_rx_free_c(&rm_rx_c);
