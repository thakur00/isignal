...
typedef union bg_node_avx512_t {
  uint8_t c[ISRRAN_AVX512_B_SIZE]; /*!< Each base node may contain up to \ref ISRRAN_AVX512_B_SIZE lifted nodes. */
...
struct ldpc_enc_avx512long {
  ...
  bg_node_avx512_t* codeword_to_free; /*!< \brief Auxiliary pointer with a free memory of size ISRRAN_AVX512_B_SIZE
  ...
  __m512i* rotated_node_to_free; /*!< \brief Auxiliary pointer to store rotated versions of the nodes with extra free
                                   memory of size ISRRAN_AVX512_B_SIZE previous to rotated_node */
...
void* create_ldpc_enc_avx512long(isrran_ldpc_encoder_t* q)
{
  ...
  int left_out   = q->ls % ISRRAN_AVX512_B_SIZE;
  vp->n_subnodes = q->ls / ISRRAN_AVX512_B_SIZE + (left_out > 0);

  if ((vp->codeword_to_free = isrran_vec_malloc((q->bgN * vp->n_subnodes + 1) * sizeof(bg_node_avx512_t))) == NULL) {
    free(vp);
    return NULL;
  }
  vp->codeword = &vp->codeword_to_free[1];

  if ((vp->aux = isrran_vec_malloc(q->bgM * vp->n_subnodes * sizeof(__m512i))) == NULL) {
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
  }

  if ((vp->rotated_node_to_free = isrran_vec_malloc((vp->n_subnodes + 2) * sizeof(__m512i))) == NULL) {
    free(vp->aux);
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
...
static void rotate_node_right(const uint8_t* mem_addr, __m512i* out, uint16_t this_shift2, uint16_t ls)
{
  ...
  for (j = this_shift2; j <= ls - ISRRAN_AVX512_B_SIZE; j = j + ISRRAN_AVX512_B_SIZE) {
  ...
  for (j = shift; j < this_shift2; j = j + ISRRAN_AVX512_B_SIZE) {
