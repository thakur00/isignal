...
static int init_c(isrran_ldpc_encoder_t* q)
{
  ...
  q->ptr = isrran_vec_u8_malloc(q->bgM * q->ls);
  if (!q->ptr) {
    perror("malloc");
    free_enc_c(q);
    return -1;
...
static int init_avx2(isrran_ldpc_encoder_t* q)
{
  int ls_index = get_ls_index(q->ls);
...
/*! Initializes an optimized encoder. */
static int init_avx2long(isrran_ldpc_encoder_t* q)
{
  int ls_index = get_ls_index(q->ls);
...
/*! Initializes an optimized encoder. */
static int init_avx512(isrran_ldpc_encoder_t* q)
{
  int ls_index = get_ls_index(q->ls);
...
/*! Initializes an optimized encoder. */
static int init_avx512long(isrran_ldpc_encoder_t* q)
{
  int ls_index = get_ls_index(q->ls);
...
int isrran_ldpc_encoder_init(isrran_ldpc_encoder_t*     q,
                             isrran_ldpc_encoder_type_t type,
                             isrran_basegraph_t         bg,
                             uint16_t                   ls)
{
  switch (bg) {
    case BG1:
  ...
  switch (type) {
    case ISRRAN_LDPC_ENCODER_C:
      return init_c(q);
#ifdef LV_HAVE_AVX2
    case ISRRAN_LDPC_ENCODER_AVX2:
...
#endif // LV_HAVE_AVX2
#ifdef LV_HAVE_AVX512
    case ISRRAN_LDPC_ENCODER_AVX512:
...
void isrran_ldpc_encoder_free(isrran_ldpc_encoder_t* q)
{
  if (q->free) {
    q->free(q);
  }
  bzero(q, sizeof(isrran_ldpc_encoder_t));
}

int isrran_ldpc_encoder_encode(isrran_ldpc_encoder_t* q, const uint8_t* input, uint8_t* output, uint32_t input_length)
{
  return q->encode(q, input, output, input_length, q->liftN - 2 * q->ls);
}

int isrran_ldpc_encoder_encode_rm(isrran_ldpc_encoder_t* q,
                                  const uint8_t*         input,
                                  uint8_t*               output,
