...
void* create_ldpc_dec_f(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp->soft_bits = isrran_vec_f_malloc(liftN)) == NULL) {
    free(vp);
    return NULL;
  }

  if ((vp->check_to_var = isrran_vec_f_malloc((hrrN + ls) * bgM)) == NULL) {
    free(vp->soft_bits);
    free(vp);
    return NULL;
  }

  if ((vp->var_to_check = isrran_vec_f_malloc((hrrN + ls))) == NULL) {
...
int init_ldpc_dec_f(void* p, const float* llrs, uint16_t ls)
{
  ...
  isrran_vec_f_zero(vp->check_to_var, (vp->hrrN + vp->ls) * (uint32_t)vp->bgM);
  isrran_vec_f_zero(vp->var_to_check, vp->hrrN + vp->ls);
  return 0;
}
...
int update_ldpc_var_to_check_f(void* p, int i_layer)
{
  ...
  isrran_vec_sub_fff(vp->soft_bits, this_check_to_var, vp->var_to_check, vp->hrrN);

  if (i_layer >= 4) {
    // Update the extension region.
    isrran_vec_sub_fff(vp->soft_bits + vp->hrrN + (i_layer - 4) * vp->ls,
