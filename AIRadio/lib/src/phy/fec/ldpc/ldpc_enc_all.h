...
#ifndef ISRRAN_LDPCENC_ALL_H
#define ISRRAN_LDPCENC_ALL_H
 ...
 */
void preprocess_systematic_bits(isrran_ldpc_encoder_t* q, const uint8_t* input);
 ...
 *                       layers are needed).
 */
void encode_ext_region(isrran_ldpc_encoder_t* q, uint8_t* output, uint8_t n_layers);

/*!
 * Creates the inner registers required by the optimized LDPC encoder (LS <= \ref ISRRAN_AVX512_B_SIZE).
 * \param[in,out] q A pointer to an encoder.
 * \return A pointer to the newly created structure of registers.
 */
void* create_ldpc_enc_avx2(isrran_ldpc_encoder_t* q);

/*!
 * Deletes the inner registers of an optimized LDPC encoder (LS <= \ref ISRRAN_AVX2_B_SIZE).
 * \param[in] p A pointer to the register structure.
 */
void delete_ldpc_enc_avx2(void* p);

/*!
 * Loads the message in the opimized encoder registers (LS <= \ref ISRRAN_AVX2_B_SIZE).
...
/*! Extracts the final codeword from the optimized encoder registers (LS <= \ref ISRRAN_AVX2_B_SIZE).
 * \param[in]  p        The register structure.
 ...
 * systematic bits (SIMD-optimized version, LS <= \ref ISRRAN_AVX2_B_SIZE).
 * \param[in,out] q     A pointer to an encoder.
 */
void preprocess_systematic_bits_avx2(isrran_ldpc_encoder_t* q);

/*! Computes the high-rate parity bits for BG1 and ls_index in {0, 1, 2, 3, 4, 5, 7}
 * (SIMD-optimized version, LS <= \ref ISRRAN_AVX2_B_SIZE).
 ...
 * ISRRAN_AVX2_B_SIZE). \param[in,out]  o  A pointer to an encoder.
 */
...
/*! Computes the extended-region parity bits (SIMD-optimized version, LS <= \ref ISRRAN_AVX2_B_SIZE).
 * \param[in,out]  q      A pointer to an encoder.
 * \param[in]  n_layers The number of layers to process (when doing rate matching not all
 *                       layers are needed).
 */
void encode_ext_region_avx2(isrran_ldpc_encoder_t* q, uint8_t n_layers);

/*!
 * Creates the inner registers required by the optimized LDPC encoder (for LS > \ref ISRRAN_AVX2_B_SIZE).
 * \param[in,out] q A pointer to an encoder.
 * \return A pointer to the newly created structure of registers.
 */
void* create_ldpc_enc_avx2long(isrran_ldpc_encoder_t* q);
...
/*!
 * Loads the message in the optimized encoder registers (LS > \ref ISRRAN_AVX2_B_SIZE).
 * \param[in] p        The register structure.
 * \param[in] input    The message to encode.
 * \param[in] msg_len  Number of variable nodes in one message.
 * \param[in] cdwd_len Number of variable nodes in one message.
 ...
 * \param[in,out] q     A pointer to an encoder.
 */
void preprocess_systematic_bits_avx2long(isrran_ldpc_encoder_t* q);
 ...
 *                       layers are needed).
 */
void encode_ext_region_avx2long(isrran_ldpc_encoder_t* q, uint8_t n_layers);

/*!
 * Creates the inner registers required by the optimized LDPC encoder (LS <= \ref ISRRAN_AVX2_B_SIZE).
 * \param[in,out] q A pointer to an encoder.
 * \return A pointer to the newly created structure of registers.
 */
void* create_ldpc_enc_avx2(isrran_ldpc_encoder_t* q);
...
/*!
 * Creates the inner registers required by the optimized LDPC encoder (for LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in,out] q A pointer to an encoder.
 * \return A pointer to the newly created structure of registers.
 */
void* create_ldpc_enc_avx512long(isrran_ldpc_encoder_t* q);

/*!
 * Deletes the inner registers of an optimized LDPC encoder (LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in] p A pointer to the register structure.
 */
void delete_ldpc_enc_avx512long(void* p);

/*!
 * Loads the message in the optimized encoder registers (LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in] p        The register structure.
 * \param[in] input    The message to encode.
 * \param[in] msg_len  Number of variable nodes in one message.
 * \param[in] cdwd_len Number of variable nodes in one message.
...
/*! Extracts the final codeword from the optimized encoder registers (LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in]  p        The register structure.
 * \param[out] output   The output codeword.
 * \param[in]  cdwd_len The number of variable nodes (after rate-matching, if enabled).
 * \param[in]  ls       The lifting size.
...
/*! Computes the product between the first (K - 2) columns of the PCM and the
 * systematic bits (SIMD-optimized version, LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in,out] q     A pointer to an encoder.
 */
void preprocess_systematic_bits_avx512long(isrran_ldpc_encoder_t* q);

/*! Computes the high-rate parity bits for BG1 and ls_index in {0, 1, 2, 3, 4, 5, 7}
 * (SIMD-optimized version, LS > \ref ISRRAN_avx512_B_SIZE).
 * \param[in,out]  o  A pointer to an encoder.
 */
void encode_high_rate_case1_avx512long(void* o);

/*! Computes the high-rate parity bits for BG1 and ls_index in {6} (SIMD-optimized version, LS > \ref
 * ISRRAN_avx512_B_SIZE).
 * \param[in,out]  o  A pointer to an encoder.
 */
...
/*! Computes the extended-region parity bits (SIMD-optimized version, LS > \ref ISRRAN_AVX512_B_SIZE).
 * \param[in,out]  q      A pointer to an encoder.
 * \param[in]  n_layers The number of layers to process (when doing rate matching not all
 *                       layers are needed).
 */
void encode_ext_region_avx512long(isrran_ldpc_encoder_t* q, uint8_t n_layers);
 ...
 * \return A pointer to the newly created structure of registers.
 */
void* create_ldpc_enc_avx512(isrran_ldpc_encoder_t* q);
 ...
 * \param[in,out] q     A pointer to an encoder.
 */
void preprocess_systematic_bits_avx512(isrran_ldpc_encoder_t* q);
 ...
 *                       layers are needed).
 */
void encode_ext_region_avx512(isrran_ldpc_encoder_t* q, uint8_t n_layers);

#endif // ISRRAN_LDPCENC_ALL_H
