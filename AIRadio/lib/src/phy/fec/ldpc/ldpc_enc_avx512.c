...
void* create_ldpc_enc_avx512(isrran_ldpc_encoder_t* q)
{
  ...
  if ((vp->codeword_to_free = isrran_vec_malloc((q->bgN + 1) * sizeof(bg_node_avx512_t))) == NULL) {
    free(vp);
    return NULL;
  }
  vp->codeword = &vp->codeword_to_free[1];

  if ((vp->aux = isrran_vec_malloc(q->bgM * sizeof(__m512i))) == NULL) {
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
  }

  if ((vp->rotated_node_to_free = isrran_vec_malloc((1 + 2) * sizeof(__m512i))) == NULL) {
    free(vp->aux);
    free(vp->codeword_to_free);
    free(vp);
    return NULL;
...
int load_avx512(void* p, const uint8_t* input, const uint8_t msg_len, const uint8_t cdwd_len, const uint16_t ls)
{
  ...
  for (; i < msg_len; i++) {
    ...
    bzero(&(vp->codeword[i].c[k]), (ISRRAN_AVX512_B_SIZE - k) * sizeof(uint8_t));
  }
