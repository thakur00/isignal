...
#define LDPC_DECODER_TEMPLATE(LLR_TYPE, SUFFIX)                                                                        \
  static int decode_##SUFFIX(                                                                                          \
      void* o, const LLR_TYPE* llrs, uint8_t* message, uint32_t cdwd_rm_length, isrran_crc_t* crc)                     \
  {                                                                                                                    \
    isrran_ldpc_decoder_t* q = o;                                                                                      \
                                                                                                                       \
    ...
    for (int i_iteration = 0; i_iteration < q->max_nof_iter; i_iteration++) {                                          \
      ...
      if (crc != NULL) {                                                                                               \
        extract_ldpc_message_##SUFFIX(q->ptr, message, q->liftK);                                                      \
                                                                                                                       \
        if (isrran_crc_match(crc, message, q->liftK - crc->order)) {                                                   \
...
static int init_f(isrran_ldpc_decoder_t* q)
{
  q->free = free_dec_f;
...
static int init_s(isrran_ldpc_decoder_t* q)
{
  q->free = free_dec_s;
...
static int init_c(isrran_ldpc_decoder_t* q)
{
  q->free = free_dec_c;
...
/*! Initializes the decoder to work with 8-bit integer-valued LLRs. */
static int init_c_flood(isrran_ldpc_decoder_t* q)
{
...
static int init_c_avx2(isrran_ldpc_decoder_t* q)
{
...
static int init_c_avx2long(isrran_ldpc_decoder_t* q)
{
...
static int init_c_avx2_flood(isrran_ldpc_decoder_t* q)
{
...
/*! Initializes the decoder to work with 8-bit integer-valued LLRs
 * (flooded scheduling, AVX2 implementation, large lifting size). */
static int init_c_avx2long_flood(isrran_ldpc_decoder_t* q)
{
...
static int init_c_avx512(isrran_ldpc_decoder_t* q)
{
...
static int init_c_avx512long(isrran_ldpc_decoder_t* q)
{
...
/*! Initializes the decoder to work with 8-bit integer-valued LLRs
 * (flooded scheduling, AVX512 implementation, large lifting size). */
static int init_c_avx512long_flood(isrran_ldpc_decoder_t* q)
{
...
int isrran_ldpc_decoder_init(isrran_ldpc_decoder_t* q, const isrran_ldpc_decoder_args_t* args)
{
  if (q == NULL || args == NULL) {
    return -1;
  }
  ...
  uint16_t                   ls           = args->ls;
  isrran_basegraph_t         bg           = args->bg;
  float                      scaling_fctr = args->scaling_fctr;
  isrran_ldpc_decoder_type_t type         = args->type;
  ...
  q->pcm = isrran_vec_u16_malloc(q->bgM * q->bgN);
  if (!q->pcm) {
    perror("malloc");
    return -1;
  }

  q->var_indices = isrran_vec_malloc(q->bgM * sizeof(int8_t[MAX_CNCT]));
  if (!q->var_indices) {
    free(q->pcm);
    perror("malloc");
    return -1;
  ...
  switch (type) {
    case ISRRAN_LDPC_DECODER_F:
      return init_f(q);
    case ISRRAN_LDPC_DECODER_S:
      return init_s(q);
    case ISRRAN_LDPC_DECODER_C:
      return init_c(q);
    case ISRRAN_LDPC_DECODER_C_FLOOD:
      return init_c_flood(q);
#ifdef LV_HAVE_AVX2
    case ISRRAN_LDPC_DECODER_C_AVX2:
      if (ls <= ISRRAN_AVX2_B_SIZE) {
        return init_c_avx2(q);
      } else {
        return init_c_avx2long(q);
      }
    case ISRRAN_LDPC_DECODER_C_AVX2_FLOOD:
      if (ls <= ISRRAN_AVX2_B_SIZE) {
...
#endif // LV_HAVE_AVX2
#ifdef LV_HAVE_AVX512
    case ISRRAN_LDPC_DECODER_C_AVX512:
      if (ls <= ISRRAN_AVX512_B_SIZE) {
        return init_c_avx512(q);
      } else {
        return init_c_avx512long(q);
      }
    case ISRRAN_LDPC_DECODER_C_AVX512_FLOOD:
...
void isrran_ldpc_decoder_free(isrran_ldpc_decoder_t* q)
{
  if (q->free) {
    q->free(q);
  }
  bzero(q, sizeof(isrran_ldpc_decoder_t));
}

int isrran_ldpc_decoder_decode_f(isrran_ldpc_decoder_t* q, const float* llrs, uint8_t* message, uint32_t cdwd_rm_length)
{
  return q->decode_f(q, llrs, message, cdwd_rm_length, NULL);
}

int isrran_ldpc_decoder_decode_s(isrran_ldpc_decoder_t* q,
                                 const int16_t*         llrs,
                                 uint8_t*               message,
...
int isrran_ldpc_decoder_decode_c(isrran_ldpc_decoder_t* q,
                                 const int8_t*          llrs,
                                 uint8_t*               message,
                                 uint32_t               cdwd_rm_length)
{
...
int isrran_ldpc_decoder_decode_crc_c(isrran_ldpc_decoder_t* q,
                                     const int8_t*          llrs,
                                     uint8_t*               message,
                                     uint32_t               cdwd_rm_length,
                                     isrran_crc_t*          crc)
