...
typedef union bg_node_t {
  uint8_t* c; /*!< Each base node may contain up to \ref ISRRAN_AVX2_B_SIZE lifted nodes. */
...
void* create_ldpc_enc_avx2(isrran_ldpc_encoder_t* q)
{
  ...
  if ((vp = ISRRAN_MEM_ALLOC(struct ldpc_enc_avx2, 1)) == NULL) {
    return NULL;
  }

  if ((vp->codeword.v = ISRRAN_MEM_ALLOC(__m256i, q->bgN)) == NULL) {
    delete_ldpc_enc_avx2(vp);
    return NULL;
  }

  if ((vp->aux = ISRRAN_MEM_ALLOC(__m256i, q->bgM)) == NULL) {
    delete_ldpc_enc_avx2(vp);
    return NULL;
  }
...
int load_avx2(void* p, const uint8_t* input, const uint8_t msg_len, const uint8_t cdwd_len, const uint16_t ls)
{
  ...
  int ini       = 0;
  int node_size = ISRRAN_AVX2_B_SIZE;
  for (int i = 0; i < msg_len * ls; i = i + ls) {
    for (int k = 0; k < ls; k++) {
      vp->codeword.c[ini + k] = input[i + k];
    }
    // this zero padding can be removed
    isrran_vec_u8_zero(&vp->codeword.c[ini + ls], node_size - ls);
    ini = ini + node_size;
  }

  ISRRAN_MEM_ZERO(vp->codeword.v + msg_len, __m256i, cdwd_len - msg_len);
...
int return_codeword_avx2(void* p, uint8_t* output, const uint8_t cdwd_len, const uint16_t ls)
{
  ...
  int ini = ISRRAN_AVX2_B_SIZE + ISRRAN_AVX2_B_SIZE;
  for (int i = 0; i < (cdwd_len - 2) * ls; i = i + ls) {
    for (int k = 0; k < ls; k++) {
      output[i + k] = vp->codeword.c[ini + k];
    }
    ini = ini + ISRRAN_AVX2_B_SIZE;
  }
  return 0;
}
