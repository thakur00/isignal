...
typedef union bg_node_avx512_t {
  int8_t  c[ISRRAN_AVX512_B_SIZE]; /*!< Each base node may contain up to \ref ISRRAN_AVX512_B_SIZE lifted nodes. */
...
void* create_ldpc_dec_c_avx512long_flood(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = isrran_vec_malloc(sizeof(struct ldpc_regs_c_avx512long_flood))) == NULL) {
    return NULL;
  }

  // compute number of subnodes
  int left_out   = ls % ISRRAN_AVX512_B_SIZE;
  int n_subnodes = ls / ISRRAN_AVX512_B_SIZE + (left_out > 0);

  if ((vp->llrs = isrran_vec_malloc(bgN * n_subnodes * sizeof(__m512i))) == NULL) {
    free(vp);
    return NULL;
  }

  if ((vp->soft_bits = isrran_vec_malloc(bgN * n_subnodes * sizeof(bg_node_avx512_t))) == NULL) {
    free(vp->llrs);
    free(vp);
    return NULL;
  }

  if ((vp->check_to_var = isrran_vec_malloc((hrr + 1) * bgM * n_subnodes * sizeof(__m512i))) == NULL) {
    free(vp->soft_bits);
    free(vp->llrs);
    free(vp);
    return NULL;
  ...
  if ((vp->var_to_check_to_free = isrran_vec_malloc(((hrr + 1) * bgM * n_subnodes + 2) * sizeof(__m512i))) == NULL) {
  ...
  if ((vp->minp_v2c_epi8 = isrran_vec_malloc(n_subnodes * sizeof(__m512i))) == NULL) {
  ...
  if ((vp->mins_v2c_epi8 = isrran_vec_malloc(n_subnodes * sizeof(__m512i))) == NULL) {
  ...
  if ((vp->prod_v2c_epi8 = isrran_vec_malloc(n_subnodes * sizeof(__m512i))) == NULL) {
  ...
  if ((vp->min_ix_epi8 = isrran_vec_malloc(n_subnodes * sizeof(__m512i))) == NULL) {
  ...
  if ((vp->rotated_v2c = isrran_vec_malloc((hrr + 1) * n_subnodes * sizeof(__m512i))) == NULL) {
    free(vp->min_ix_epi8);
    free(vp->prod_v2c_epi8);
    free(vp->mins_v2c_epi8);
    free(vp->minp_v2c_epi8);
  ...
  if ((vp->this_c2v_epi8_to_free = isrran_vec_malloc((n_subnodes + 2) * sizeof(__m512i))) == NULL) {
    free(vp->rotated_v2c);
    free(vp->min_ix_epi8);
    free(vp->prod_v2c_epi8);
    free(vp->mins_v2c_epi8);
...
int init_ldpc_dec_c_avx512long_flood(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  for (i = 2; i < vp->bgN; i++) {
    for (j = 0; j < vp->n_subnodes; j++) {
      for (k = 0; (k < ISRRAN_AVX512_B_SIZE) && (j * ISRRAN_AVX512_B_SIZE + k < ls); k++) {
        vp->soft_bits[i * vp->n_subnodes + j].c[k] = llrs[(i - 2) * ls + j * ISRRAN_AVX512_B_SIZE + k];
      }
      vp->llrs[i * vp->n_subnodes + j] = vp->soft_bits[i * vp->n_subnodes + j].v;
    }
    bzero(&(vp->soft_bits[i * vp->n_subnodes + j - 1].c[k]), (ISRRAN_AVX512_B_SIZE - k) * sizeof(int8_t));
    bzero((int8_t*)(vp->llrs + i * vp->n_subnodes + j - 1) + k, (ISRRAN_AVX512_B_SIZE - k) * sizeof(int8_t));
  }

  ISRRAN_MEM_ZERO(vp->check_to_var, __m512i, (vp->hrr + 1) * (uint32_t)vp->bgM * (uint32_t)vp->n_subnodes);
  ISRRAN_MEM_ZERO(vp->var_to_check, __m512i, (vp->hrr + 1) * (uint32_t)vp->bgM * (uint32_t)vp->n_subnodes);
  return 0;
}
...
int extract_ldpc_message_c_avx512long_flood(void* p, uint8_t* message, uint16_t liftK)
{
  ...
  for (int i = 0; i < liftK / vp->ls; i++) {
    for (j = 0; j < vp->n_subnodes; j++) {
      for (k = 0; (k < ISRRAN_AVX512_B_SIZE) && (j * ISRRAN_AVX512_B_SIZE + k < vp->ls); k++) {
        message[i * vp->ls + j * ISRRAN_AVX512_B_SIZE + k] = (vp->soft_bits[i * vp->n_subnodes + j].c[k] < 0);
      }
    }
  }
...
static void rotate_node_right(const uint8_t* mem_addr, __m512i* out, uint16_t this_shift, uint16_t ls)
{
  ...
  for (j = this_shift; j <= ls - ISRRAN_AVX512_B_SIZE; j = j + ISRRAN_AVX512_B_SIZE) {
  ...
  if (ls > j) {
    _shift = ls - j;
    shift  = ISRRAN_AVX512_B_SIZE - _shift;
  ...
  for (j = shift; j < this_shift; j = j + ISRRAN_AVX512_B_SIZE) {
