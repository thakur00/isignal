...
typedef union bg_node_avx512_t {
  int8_t*  c; /*!< Each base node may contain up to \ref ISRRAN_AVX512_B_SIZE lifted nodes. */
...
void* create_ldpc_dec_c_avx512(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = ISRRAN_MEM_ALLOC(struct ldpc_regs_c_avx512, 1)) == NULL) {
    return NULL;
  }
  ISRRAN_MEM_ZERO(vp, struct ldpc_regs_c_avx512, 1);

  if ((vp->soft_bits.v = ISRRAN_MEM_ALLOC(__m512i, bgN)) == NULL) {
    delete_ldpc_dec_c_avx512(vp);
    return NULL;
  }

  if ((vp->check_to_var = ISRRAN_MEM_ALLOC(__m512i, (hrr + 1) * bgM)) == NULL) {
    delete_ldpc_dec_c_avx512(vp);
    return NULL;
  }

  if ((vp->var_to_check_to_free = ISRRAN_MEM_ALLOC(__m512i, (hrr + 1) + 2)) == NULL) {
    delete_ldpc_dec_c_avx512(vp);
    return NULL;
  }
  vp->var_to_check = &vp->var_to_check_to_free[1];

  if ((vp->rotated_v2c = ISRRAN_MEM_ALLOC(__m512i, hrr + 1)) == NULL) {
    delete_ldpc_dec_c_avx512(vp);
    return NULL;
  }

  if ((vp->this_c2v_epi8_to_free = ISRRAN_MEM_ALLOC(__m512i, 1 + 2)) == NULL) {
    delete_ldpc_dec_c_avx512(vp);
    return NULL;
  }
  vp->this_c2v_epi8 =
...
int init_ldpc_dec_c_avx512(void* p, const int8_t* llrs, uint16_t ls)
{
  ...
  int ini = ISRRAN_AVX512_B_SIZE + ISRRAN_AVX512_B_SIZE;
  isrran_vec_i8_zero(vp->soft_bits.c, ini);
  ...
  for (i = 0; i < vp->finalN; i = i + ls) {
    ...
    isrran_vec_i8_zero(&vp->soft_bits.c[ini + ls], ISRRAN_AVX512_B_SIZE - ls);
    ini = ini + ISRRAN_AVX512_B_SIZE;
  }

  ISRRAN_MEM_ZERO(vp->check_to_var, __m512i, (vp->hrr + 1) * vp->bgM);
  ISRRAN_MEM_ZERO(vp->var_to_check, __m512i, vp->hrr + 1);
