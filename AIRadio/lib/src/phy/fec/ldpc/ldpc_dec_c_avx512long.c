...
void* create_ldpc_dec_c_avx512long(uint8_t bgN, uint8_t bgM, uint16_t ls, float scaling_fctr)
{
  ...
  if ((vp = isrran_vec_malloc(sizeof(struct ldpc_regs_c_avx512long))) == NULL) {
    return NULL;
  }
  ...
  if ((vp->var_to_check_to_free = isrran_vec_malloc(((hrr + 1) * n_subnodes + 2) * sizeof(__m512i))) == NULL) {
  ...
  vp->node_size  = ISRRAN_AVX512_B_SIZE * vp->n_subnodes;
  vp->finalN     = (bgN - 2) * ls;
