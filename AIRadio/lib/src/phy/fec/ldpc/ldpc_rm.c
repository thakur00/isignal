...
#include "isrran/phy/fec/ldpc/ldpc_common.h" //FILLER_BIT definition
 ...
 */
static int init_rm(isrran_ldpc_rm_t*  p,
                   const uint32_t     E,
                   const uint32_t     F,
                   const uint32_t     bg,
                   const uint32_t     ls,
                   ...
                   const uint32_t     Nref)
{
  ...
  uint32_t mod_order = isrran_mod_bits_x_symbol(mod_type);
...
int isrran_ldpc_rm_tx_init(isrran_ldpc_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  if ((pp->tmp_rm_codeword = isrran_vec_u8_malloc(MAXE)) == NULL) {
    free(pp);
    return -1;
  }
...
int isrran_ldpc_rm_rx_init_f(isrran_ldpc_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  if ((pp->tmp_rm_symbol = isrran_vec_f_malloc(MAXE)) == NULL) {
    free(pp);
    return -1;
  }

  if ((pp->indices = isrran_vec_u32_malloc(MAXE)) == NULL) {
...
int isrran_ldpc_rm_rx_init_s(isrran_ldpc_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  // allocate memory to the temporal buffer
  if ((pp->tmp_rm_symbol = isrran_vec_i16_malloc(MAXE)) == NULL) {
    free(pp);
    return -1;
  }
...
int isrran_ldpc_rm_rx_init_c(isrran_ldpc_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  // allocate memory to the temporal buffer
  if ((pp->tmp_rm_symbol = isrran_vec_i8_malloc(MAXE)) == NULL) {
    free(pp);
    return -1;
  }
...
void isrran_ldpc_rm_tx_free(isrran_ldpc_rm_t* q)
{
  if (q != NULL) {
...
void isrran_ldpc_rm_rx_free_f(isrran_ldpc_rm_t* q)
{
  if (q != NULL) {
...
void isrran_ldpc_rm_rx_free_s(isrran_ldpc_rm_t* q)
{
  if (q != NULL) {
...
void isrran_ldpc_rm_rx_free_c(isrran_ldpc_rm_t* q)
{
  if (q != NULL) {
...
int isrran_ldpc_rm_tx(isrran_ldpc_rm_t*        q,
                      const uint8_t*           input,
                      uint8_t*                 output,
                      const uint32_t           E,
...
int isrran_ldpc_rm_rx_f(isrran_ldpc_rm_t*        q,
                        const float*             input,
                        float*                   output,
                        const uint32_t           E,
                        const uint32_t           F,
...
int isrran_ldpc_rm_rx_s(isrran_ldpc_rm_t*        q,
                        const int16_t*           input,
                        int16_t*                 output,
                        const uint32_t           E,
                        const uint32_t           F,
...
int isrran_ldpc_rm_rx_c(isrran_ldpc_rm_t*        q,
                        const int8_t*            input,
                        int8_t*                  output,
                        const uint32_t           E,
                        const uint32_t           F,
                        ...
                        const uint32_t           Nref)
{
  ...
  return (int)ISRRAN_MIN(q->k0 + q->E, q->Ncb);
}
