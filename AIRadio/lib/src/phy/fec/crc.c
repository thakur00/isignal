...
static void gen_crc_table(isrran_crc_t* h)
{
...
uint64_t reversecrcbit(uint32_t crc, int nbits, isrran_crc_t* h)
{
...
int isrran_crc_set_init(isrran_crc_t* crc_par, uint64_t crc_init_value)
{
...
int isrran_crc_init(isrran_crc_t* h, uint32_t crc_poly, int crc_order)
{
  ...
  if (isrran_crc_set_init(h, h->crcinit)) {
...
uint32_t isrran_crc_checksum(isrran_crc_t* h, uint8_t* data, int len)
{
  int      i, k, len8, res8, a = 0;
  uint32_t crc = 0;
  uint8_t* pter;

  isrran_crc_set_init(h, 0);
...
#else  /* LV_HAVE_SSE */
      byte = (uint8_t)(isrran_bit_pack(&pter, 8) & 0xFF);
#endif /* LV_HAVE_SSE */
    }
    isrran_crc_checksum_put_byte(h, byte);
  }
  crc = (uint32_t)isrran_crc_checksum_get(h);
...
uint32_t isrran_crc_checksum_byte(isrran_crc_t* h, const uint8_t* data, int len)
{
  int      i;
  uint32_t crc = 0;
  ...
  // Calculate CRC
  for (i = 0; i < len / 8; i++) {
    isrran_crc_checksum_put_byte(h, data[i]);
  }
  crc = (uint32_t)isrran_crc_checksum_get(h);
...
uint32_t isrran_crc_attach_byte(isrran_crc_t* h, uint8_t* data, int len)
{
  uint32_t checksum = isrran_crc_checksum_byte(h, data, len);
 ...
 */
uint32_t isrran_crc_attach(isrran_crc_t* h, uint8_t* data, int len)
{
  uint32_t checksum = isrran_crc_checksum(h, data, len);

  // Add CRC
  uint8_t* ptr = &data[len];
  isrran_bit_unpack(checksum, &ptr, h->order);
  return checksum;
}

bool isrran_crc_match(isrran_crc_t* h, uint8_t* data, int len)
{
  return isrran_crc_checksum(h, data, len + h->order) == 0;
}

bool isrran_crc_match_byte(isrran_crc_t* h, uint8_t* data, int len)
{
  return isrran_crc_checksum_byte(h, data, len + h->order) == 0;
}
