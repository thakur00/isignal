...
int isrran_polar_rm_tx_init(isrran_polar_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  if ((pp->y_e = isrran_vec_u8_malloc(EMAX)) == NULL) {
    free(pp);
    return -1;
  }
  return 0;
...
int isrran_polar_rm_rx_init_f(isrran_polar_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  if ((pp->y_e = isrran_vec_f_malloc(EMAX + NMAX)) == NULL) {
    free(pp);
    return -1;
  }
  pp->e = pp->y_e + NMAX;
...
int isrran_polar_rm_rx_init_s(isrran_polar_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  // allocate memory to the temporal buffer of chDeInterleaved llrs
  if ((pp->y_e = isrran_vec_i16_malloc(EMAX + NMAX)) == NULL) {
    free(pp);
    return -1;
  }
  pp->e = pp->y_e + NMAX;
...
int isrran_polar_rm_rx_init_c(isrran_polar_rm_t* p)
{
  if (p == NULL) {
    return -1;
  }
  ...
  // allocate memory to the temporal buffer of chDeInterleaved llrs
  if ((pp->y_e = isrran_vec_i8_malloc(EMAX + NMAX)) == NULL) {
    free(pp);
    return -1;
  }
  pp->e = pp->y_e + NMAX;
...
void isrran_polar_rm_tx_free(isrran_polar_rm_t* q)
{
  if (q == NULL) {
    return;
  }
...
void isrran_polar_rm_rx_free_f(isrran_polar_rm_t* q)
{
  if (q == NULL) {
    return;
  }
...
void isrran_polar_rm_rx_free_s(isrran_polar_rm_t* q)
{
  if (q == NULL) {
    return;
  }
...
void isrran_polar_rm_rx_free_c(isrran_polar_rm_t* q)
{
  if (q == NULL) {
    return;
  }
...
int isrran_polar_rm_tx(isrran_polar_rm_t* q,
                       const uint8_t*     input,
                       uint8_t*           output,
                       const uint8_t      n,
                       const uint32_t     E,
...
int isrran_polar_rm_rx_f(isrran_polar_rm_t* q,
                         const float*       input,
                         float*             output,
                         const uint32_t     E,
                         const uint8_t      n,
...
int isrran_polar_rm_rx_s(isrran_polar_rm_t* q,
                         const int16_t*     input,
                         int16_t*           output,
                         const uint32_t     E,
                         const uint8_t      n,
...
int isrran_polar_rm_rx_c(isrran_polar_rm_t* q,
                         const int8_t*      input,
                         int8_t*            output,
                         const uint32_t     E,
                         const uint8_t      n,
