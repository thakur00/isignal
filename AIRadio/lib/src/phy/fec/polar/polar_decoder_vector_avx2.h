 ...
 * the output must have size larger than \ref ISRRAN_AVX2_B_SIZE.
 ...
 */
ISRRAN_API void isrran_vec_sign_to_bit_c_avx2(uint8_t* x, uint16_t len);
 ...
 * \param[in] len Length of vectors x, y and z.
 */
ISRRAN_API void isrran_vec_function_f_ccc_avx2(const int8_t* x, const int8_t* y, int8_t* z, uint16_t len);
 ...
 */
ISRRAN_API void
isrran_vec_function_g_bccc_avx2(const uint8_t* b, const int8_t* x, const int8_t* y, int8_t* z, uint16_t len);
 ...
 * \param[in] len Length of vectors x, y and z.
 */
ISRRAN_API void isrran_vec_xor_bbb_avx2(const uint8_t* x, const uint8_t* y, uint8_t* z, uint16_t len);

/*!
 * Returns 1 if \f$ (x < 0) \f$ and 0 if \f$ (x >= 0) \f$ with AVX2 instructions,
 * the output must have size larger that \ref ISRRAN_AVX2_B_SIZE.
 * \param[in] x A pointer to a vector of int8_t.
 * \param[out] z A pointer to a vector of uint8_t with 0's and 1's.
 * \param[in] len Length of vectors x and z.
 */
ISRRAN_API void isrran_vec_hard_bit_cc_avx2(const int8_t* x, uint8_t* z, uint16_t len);
