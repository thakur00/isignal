...
//  pp->f        = isrran_vec_function_f_sss;
//  pp->g        = isrran_vec_function_g_bsss;
//  pp->xor      = isrran_vec_xor_bbb;
//  pp->hard_bit = isrran_vec_hard_bit_sc;
...
void* create_polar_decoder_ssc_s(const uint8_t nMax)
{
  ...
  pp->llr0[0] = isrran_vec_i16_malloc(llr_all_stages); // 32*8=256
  // allocate memory to the polar decoder instance
  if (pp->llr0[0] == NULL) {
    free(pp->est_bit);
    free(pp->state);
