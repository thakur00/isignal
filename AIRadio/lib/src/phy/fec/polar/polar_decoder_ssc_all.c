...
void* create_tmp_node_type(const uint8_t nMax)
{
  ...
  ISRRAN_MEM_ZERO(tmp, struct Tmp_node_type, 1);

  uint16_t max_code_size      = (1U << nMax);
  uint8_t  nMax_1             = nMax - 1;
  uint16_t max_code_half_size = (1U << nMax_1);

  tmp->is_not_rate_0 = isrran_vec_u8_malloc(2 * max_code_size);
  ...
  tmp->i_odd = isrran_vec_u16_malloc(max_code_half_size);
  if (!tmp->i_odd) {
  ...
  tmp->i_even = isrran_vec_u16_malloc(max_code_half_size);
  if (!tmp->i_even) {
    free(tmp->is_not_rate_0);
    free(tmp->i_odd);
    free(tmp);
...
int init_node_type(const uint16_t* frozen_set, struct Params* param)
{
  ...
  is_not_rate_0 = isrran_vec_u8_malloc(2 * code_size);
  if (!is_not_rate_0) {
    perror("malloc");
    return -1;
  }
  ...
  i_odd = isrran_vec_u16_malloc(code_half_size);
  if (!i_odd) {
    free(is_not_rate_0);
    perror("malloc");
    return -1;
  ...
  i_even = isrran_vec_u16_malloc(code_half_size);
  if (!i_even) {
    free(is_not_rate_0);
    free(i_odd);
    perror("malloc");
