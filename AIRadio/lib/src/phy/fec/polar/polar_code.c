 ...
 */
int get_code_params(isrran_polar_code_t* c, const uint16_t K, const uint16_t E, const uint8_t nMax)
{
...
void isrran_polar_code_free(isrran_polar_code_t* c)
{
  if (c != NULL) {
    free(c->F_set);
...
int isrran_polar_code_init(isrran_polar_code_t* c)
{
  c->tmp_K_set = isrran_vec_u16_malloc(NMAX + 1); // +1 to mark the end with 1024
  if (!c->tmp_K_set) {
    perror("malloc");
    exit(-1);
  }

  c->F_set = isrran_vec_u16_malloc(NMAX);
  if (!c->F_set) {
    free(c->tmp_K_set);
    perror("malloc");
    exit(-1);
...
int isrran_polar_code_get(isrran_polar_code_t* c, uint16_t K, uint16_t E, uint8_t nMax)
{
  if (c == NULL) {
    return -1;
  }
