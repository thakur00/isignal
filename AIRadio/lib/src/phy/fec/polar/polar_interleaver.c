...
static const uint16_t polar_interleaver_pattern[ISRRAN_POLAR_INTERLEAVER_K_MAX_IL] = {
...
void isrran_polar_interleaver_run(const void* in, void* out, uint32_t S, uint32_t K, bool dir)
{
  if (in == NULL || out == NULL) {
    return;
  }
  ...
  uint32_t k = 0;
  for (uint32_t m = 0; m < ISRRAN_POLAR_INTERLEAVER_K_MAX_IL; m++) {
    if (polar_interleaver_pattern[m] >= ISRRAN_POLAR_INTERLEAVER_K_MAX_IL - K) {
      uint32_t pi_k = polar_interleaver_pattern[m] - (ISRRAN_POLAR_INTERLEAVER_K_MAX_IL - K);
      if (dir) {
