...
struct pSSC_c_avx2 {
  ...
  isrran_polar_encoder_t* enc;                /*!< \brief Pointer to a isrran_polar_encoder_t. */
 ...
 * At stage \f$ s \f$, this function runs function isrran_vec_function_f_fff() and isrran_vec_function_g_bfff()
 * with vector size \f$2^{ s - 1}\f$ and updates \a llr0 and \a llr1 memory space for stage \f$(s - 1)\f$.
 * This function also runs isrran_vec_xor_bbb() with vector size \f$2^{s-1}\f$ and
...
void delete_polar_decoder_ssc_c_avx2(void* p)
{
  ...
  if (p != NULL) {
    ...
    if (pp->enc) {
      isrran_polar_encoder_free(pp->enc);
      free(pp->enc);
    }
    if (pp->tmp_node_type) {
...
void* create_polar_decoder_ssc_c_avx2(const uint8_t nMax)
{
  ...
  // set functions
  pp->f        = isrran_vec_function_f_ccc_avx2;
  pp->g        = isrran_vec_function_g_bccc_avx2;
  pp->xor      = isrran_vec_xor_bbb_avx2;
  pp->hard_bit = isrran_vec_hard_bit_cc_avx2;

  // encoder of maximum size
  if ((pp->enc = malloc(sizeof(isrran_polar_encoder_t))) == NULL) {
    free(pp);
    return NULL;
  }

  isrran_polar_encoder_init(pp->enc, ISRRAN_POLAR_ENCODER_AVX2, nMax);
  ...
  if ((pp->param->code_stage_size = isrran_vec_u16_malloc(nMax + 1)) == NULL) {
    free(pp->param);
    free(pp->enc);
    free(pp);
    return NULL;
  ...
  // allocates extra ISRRAN_AVX2_B_SIZE bytes to allow store the output of 256-bit instructions
  int est_bit_size = pp->param->code_stage_size[nMax] + ISRRAN_AVX2_B_SIZE;

  pp->est_bit = isrran_vec_u8_malloc(est_bit_size); // every 32 chars are aligned

  // LLR MEMORY NOT ALIGNED FOR LLR_BUFFERS_SIZE < ISRRAN_SIMB_LLR_ALIGNED
  ...
  // Reserve at least ISRRAN_AVX2_B_SIZE bytes for each stage, so that there is space for the output
  ...
  if (nMax >= 5) {
    llr_all_stages_avx2 += ISRRAN_AVX2_B_SIZE * 5;
  } else {
    llr_all_stages_avx2 += (nMax + 1) * ISRRAN_AVX2_B_SIZE;
  }

  // add extra ISRRAN_AVX2_B_SIZE llrs positions for hard_bit functions on the last bits have
  // access to allocated memory
  llr_all_stages_avx2 += ISRRAN_AVX2_B_SIZE;

  pp->llr0[0] = isrran_vec_i8_malloc(llr_all_stages_avx2); // 32*8=256
                                  ...
                                  const uint16_t  frozen_set_size)
{
  ...
  int est_bit_size = pp->param->code_stage_size[code_size_log] + ISRRAN_AVX2_B_SIZE;
...
int polar_decoder_ssc_c_avx2(void* p, uint8_t* data_decoded)
{
  ...
  isrran_polar_encoder_encode(pp->enc, pp->est_bit, data_decoded, pp->param->code_size_log);

  // transform {0,-128} into {0, 1}
  isrran_vec_sign_to_bit_c_avx2(data_decoded, 1U << pp->param->code_size_log);
  return 0;
}
