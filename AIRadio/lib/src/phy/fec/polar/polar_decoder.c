...
static int init_ssc_f(isrran_polar_decoder_t* q)
{
...
static int init_ssc_s(isrran_polar_decoder_t* q)
{
...
static int init_ssc_c(isrran_polar_decoder_t* q)
{
 ...
 * instructions. */
static int init_ssc_c_avx2(isrran_polar_decoder_t* q)
{
...
int isrran_polar_decoder_init(isrran_polar_decoder_t* q, isrran_polar_decoder_type_t type, const uint8_t nMax)
{
  q->nMax = nMax;
  switch (type) {
    case ISRRAN_POLAR_DECODER_SSC_F:
      return init_ssc_f(q);
    case ISRRAN_POLAR_DECODER_SSC_S:
      return init_ssc_s(q);
    case ISRRAN_POLAR_DECODER_SSC_C:
      return init_ssc_c(q);
#ifdef LV_HAVE_AVX2
    case ISRRAN_POLAR_DECODER_SSC_C_AVX2:
...
void isrran_polar_decoder_free(isrran_polar_decoder_t* q)
{
  if (q->free) {
    q->free(q);
  }
  memset(q, 0, sizeof(isrran_polar_decoder_t));
}

int isrran_polar_decoder_decode_f(isrran_polar_decoder_t* q,
                                  const float*            llr,
...
int isrran_polar_decoder_decode_s(isrran_polar_decoder_t* q,
                                  const int16_t*          llr,
                                  uint8_t*                data_decoded,
                                  const uint8_t           n,
                                  const uint16_t*         frozen_set,
...
int isrran_polar_decoder_decode_c(isrran_polar_decoder_t* q,
                                  const int8_t*           llr,
                                  uint8_t*                data_decoded,
                                  const uint8_t           n,
                                  const uint16_t*         frozen_set,
