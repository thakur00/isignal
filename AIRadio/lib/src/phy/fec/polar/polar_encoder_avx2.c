...
void* create_polar_encoder_avx2(const uint8_t code_size_log)
{
  ...
  if (code_size_log > ISRRAN_AVX2_B_SIZE_LOG) {
    q->tmp = isrran_vec_u8_malloc(code_size);
  } else {
    q->tmp = isrran_vec_u8_malloc(ISRRAN_AVX2_B_SIZE);
  }
  if (!q->tmp) {
    free(q);
    perror("malloc");
 ...
 */
static inline void isrran_vec_polar_encoder_32_avx2(const uint8_t* x, uint8_t* z, uint8_t stage)
{
 ...
 */
static inline void isrran_vec_xor_bbb_avx2(const uint8_t* x, const uint8_t* y, uint8_t* z, uint16_t len)
{
...
int polar_encoder_encode_avx2(void* p, const uint8_t* input, uint8_t* output, const uint8_t code_size_log)
{
  ...
  for (; s > ISRRAN_AVX2_B_SIZE_LOG; s--) {
    ...
    for (uint32_t b = 0; b < num_blocks; b++) {
      ...
      z = x;
      isrran_vec_xor_bbb_avx2(x, y, z, code_half_size_stage);
    }
  }

  uint32_t num_simd_size_blocks = 1;
  if (code_size_log > ISRRAN_AVX2_B_SIZE_LOG) {
    num_simd_size_blocks = 1U << (code_size_log - ISRRAN_AVX2_B_SIZE_LOG);
  }

  for (uint32_t b = 0; b < num_simd_size_blocks; b++) {
    x = &tmp[b * ISRRAN_AVX2_B_SIZE];
    z = x;
    isrran_vec_polar_encoder_32_avx2(x, z, s);
  }
