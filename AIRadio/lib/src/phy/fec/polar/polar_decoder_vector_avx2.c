...
void isrran_vec_function_f_ccc_avx2(const int8_t* x, const int8_t* y, int8_t* z, const uint16_t len)
{

  for (int i = 0; i < len; i += ISRRAN_AVX2_B_SIZE) {
...
void isrran_vec_function_g_bccc_avx2(const uint8_t* b, const int8_t* x, const int8_t* y, int8_t* z, const uint16_t len)
{
...
void isrran_vec_xor_bbb_avx2(const uint8_t* x, const uint8_t* y, uint8_t* z, uint16_t len)
{
...
void isrran_vec_hard_bit_cc_avx2(const int8_t* x, uint8_t* z, const uint16_t len)
{
  ...
  // restore, by setting to 0, the memory positions between z + len and z + len + ISRRAN_AVX2_B_SIZE
  memset(z + len, 0, ISRRAN_AVX2_B_SIZE);
}

void isrran_vec_sign_to_bit_c_avx2(uint8_t* x, uint16_t len)
{
  const __m256i M_NEG1 = _mm256_set1_epi8(-1);

  int i = 0;
  for (; i < len - ISRRAN_AVX2_B_SIZE + 1; i += ISRRAN_AVX2_B_SIZE) {
    __m256i m_x = _mm256_loadu_si256((__m256i*)&x[i]);
