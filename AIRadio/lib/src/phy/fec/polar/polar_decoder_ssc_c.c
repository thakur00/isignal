...
//  pp->f        = isrran_vec_function_f_ccc;
//  pp->g        = isrran_vec_function_g_bccc;
//  pp->xor      = isrran_vec_xor_bbb;
//  pp->hard_bit = isrran_vec_hard_bit_cc;
...
void* create_polar_decoder_ssc_c(const uint8_t nMax)
{
  ...
  ISRRAN_MEM_ZERO(pp, struct pSSC_c, 1);
  ...
  if ((pp->enc = ISRRAN_MEM_ALLOC(isrran_polar_encoder_t, 1)) == NULL) {
    delete_polar_decoder_ssc_c(pp);
    return NULL;
  }
  isrran_polar_encoder_init(pp->enc, ISRRAN_POLAR_ENCODER_PIPELINED, nMax);

  // algorithm constants/parameters
  if ((pp->param = ISRRAN_MEM_ALLOC(struct Params, 1)) == NULL) {
    delete_polar_decoder_ssc_c(pp);
    return NULL;
  }
  ...
  if ((pp->state->active_node_per_stage = isrran_vec_u16_malloc(nMax + 1)) == NULL) {
    free(pp->state);
    free(pp->param->code_stage_size);
    free(pp->param);
    free(pp->enc);
  ...
  pp->est_bit = isrran_vec_u8_malloc(est_bits_size); // every 32 chars are aligned
  ...
  // Only the stages starting at multiples of ISRRAN_AVX2_B_SIZE are aligned.
  ...
  pp->llr0[0] = isrran_vec_i8_malloc(llr_all_stages); // 32*8=256
  // allocate memory to the polar decoder instance
  if (pp->llr0[0] == NULL) {
    free(pp->est_bit);
    free(pp->state);
...
static void rate_1_node(void* p, uint8_t* message)
{
  ...
  if (stage != 0) {
    isrran_polar_encoder_encode(pp->enc, codeword, message + bit_pos, stage);
  } else {
