...
void isrran_polar_code_sets_free(isrran_polar_sets_t* c)
{
  if (c != NULL) {
    free(c->frozen_set);
    free(c->info_set);
...
int isrran_polar_code_sets_read(isrran_polar_sets_t* c,
                                ...
                                const uint8_t        nWmPC)
{
  ...
  c->frozen_set = isrran_vec_u16_malloc(c->frozen_set_size);
  if (!c->frozen_set) {
    perror("malloc");
    exit(-1);
  }

  c->info_set = isrran_vec_u16_malloc(c->info_set_size);
  if (!c->info_set) {
    perror("malloc");
    exit(-1);
  }

  c->message_set = isrran_vec_u16_malloc(c->message_set_size);
  if (!c->message_set) {
    perror("malloc");
    exit(-1);
  }

  c->parity_set = isrran_vec_u16_malloc(parity_set_size);
  if (!c->parity_set) {
    perror("malloc");
    exit(-1);
  }
