...
int main(int argc, char** argv)
{
  ...
  isrran_polar_encoder_t enc;
  isrran_polar_decoder_t dec;
  isrran_polar_decoder_t dec_s; // 16-bit
  isrran_polar_decoder_t dec_c; // 8-bit
  isrran_polar_rm_t      rm_tx;
  isrran_polar_rm_t      rm_rx_f;
  isrran_polar_rm_t      rm_rx_s;
  isrran_polar_rm_t      rm_rx_c;

#ifdef LV_HAVE_AVX2
  isrran_polar_encoder_t enc_avx2;
  isrran_polar_decoder_t dec_c_avx2; // 8-bit
#endif                               // LV_HAVE_AVX2

  parse_args(argc, argv);

  // uinitialize polar code
  isrran_polar_code_init(&code);

  // initialize encoder pipeline
  isrran_polar_encoder_init(&enc, ISRRAN_POLAR_ENCODER_PIPELINED, nMax);

  // initialize rate-matcher
  isrran_polar_rm_tx_init(&rm_tx);

  // initialize rate-matcher
  isrran_polar_rm_rx_init_f(&rm_rx_f);

  // initialize rate-matcher
  isrran_polar_rm_rx_init_s(&rm_rx_s);

  // initialize rate-matcher
  isrran_polar_rm_rx_init_c(&rm_rx_c);

  // initialize a POLAR decoder (float)
  isrran_polar_decoder_init(&dec, ISRRAN_POLAR_DECODER_SSC_F, nMax);

  // initialize a POLAR decoder (16 bit)
  isrran_polar_decoder_init(&dec_s, ISRRAN_POLAR_DECODER_SSC_S, nMax);

  // initialize a POLAR decoder (8 bit)
  isrran_polar_decoder_init(&dec_c, ISRRAN_POLAR_DECODER_SSC_C, nMax);

#ifdef LV_HAVE_AVX2

  // initialize encoder  avx2
  isrran_polar_encoder_init(&enc_avx2, ISRRAN_POLAR_ENCODER_AVX2, nMax);

  // initialize a POLAR decoder (8 bit, avx2)
  isrran_polar_decoder_init(&dec_c_avx2, ISRRAN_POLAR_DECODER_SSC_C_AVX2, nMax);
#endif // LV_HAVE_AVX2
  ...
  data_tx        = isrran_vec_u8_malloc(K * BATCH_SIZE);
  data_rx        = isrran_vec_u8_malloc(K * BATCH_SIZE);
  data_rx_s      = isrran_vec_u8_malloc(K * BATCH_SIZE);
  data_rx_c      = isrran_vec_u8_malloc(K * BATCH_SIZE);
  data_rx_c_avx2 = isrran_vec_u8_malloc(K * BATCH_SIZE);

  input_enc       = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  output_enc      = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  output_enc_avx2 = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);

  rm_codeword = isrran_vec_u8_malloc(E * BATCH_SIZE);

  rm_llr        = isrran_vec_f_malloc(E * BATCH_SIZE);
  rm_llr_s      = isrran_vec_i16_malloc(E * BATCH_SIZE);
  rm_llr_c      = isrran_vec_i8_malloc(E * BATCH_SIZE);
  rm_llr_c_avx2 = isrran_vec_i8_malloc(E * BATCH_SIZE);

  llr        = isrran_vec_f_malloc(NMAX * BATCH_SIZE);
  llr_s      = isrran_vec_i16_malloc(NMAX * BATCH_SIZE);
  llr_c      = isrran_vec_i8_malloc(NMAX * BATCH_SIZE);
  llr_c_avx2 = isrran_vec_i8_malloc(NMAX * BATCH_SIZE);

  output_dec        = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  output_dec_s      = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  output_dec_c      = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  output_dec_c_avx2 = isrran_vec_u8_malloc(NMAX * BATCH_SIZE);
  ...
  if (snr_db == 100.0) {
    ...
    for (int i = 0; i < snr_points; i++) {
      ...
      snr_db_vec[i] = snr_db;
      var[i]        = isrran_convert_dB_to_power(-snr_db);
    }
    snr_db_vec[snr_points] = 101; // include the no noise case
    snr_points++;
  } else {
    snr_db_vec[0] = snr_db;
    var[0]        = isrran_convert_dB_to_power(-snr_db);
    snr_points    = 1;
  }
...
// generate data_tx
      ...
      for (int i = 0; i < BATCH_SIZE; i++) {
        for (j = 0; j < K; j++) {
          data_tx[i * K + j] = isrran_random_uniform_int_dist(random_gen, 0, 1);
        }
      }
#endif

      // get polar code, compute frozen_set (F_set), message_set (K_set) and parity bit set (PC_set)
      if (isrran_polar_code_get(&code, K, E, nMax) == -1) {
        return -1;
      }
      ...
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_encoder_encode(&enc, input_enc + j * code.N, output_enc + j * code.N, code.n);
      }
      ...
      // rate matcher
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_rm_tx(&rm_tx, output_enc + j * code.N, rm_codeword + j * E, code.n, E, K, bil);
      }
      ...
      gettimeofday(&t[1], NULL);
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_encoder_encode(&enc_avx2, input_enc + j * code.N, output_enc_avx2 + j * code.N, code.n);
      }
      gettimeofday(&t[2], NULL);
      get_time_interval(t);
      ...
      for (int i = 0; i < BATCH_SIZE; i++) {
        if (isrran_bit_diff(output_enc + i * code.N, output_enc_avx2 + i * code.N, code.N) != 0) {
      ...
      // add noise
      if (snr_db_vec[i_snr] != 101) {
        isrran_ch_awgn_f(rm_llr, rm_llr, var[i_snr], BATCH_SIZE * E);
      ...
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_rm_rx_f(&rm_rx_f, rm_llr + j * E, llr + j * code.N, E, code.n, K, bil);
      }

      // decoding float point
      gettimeofday(&t[1], NULL);
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_decoder_decode_f(
      ...
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_chanalloc_rx(output_dec + j * code.N, data_rx + j * K, code.K, code.nPC, code.K_set, code.PC_set);
      }
...
// check errors - float decpder
...
#endif
      for (int i = 0; i < BATCH_SIZE; i++) {
        errors_symb = isrran_bit_diff(data_tx + i * K, data_rx + i * K, K);
      ...
      // 16-quantization
      if (snr_db_vec[i_snr] == 101) {
        isrran_vec_quant_fs(rm_llr, rm_llr_s, 8192, 0, 32767, BATCH_SIZE * E);
      } else {
        gain_s = inf16 * var[i_snr] / 20 / (1 / var[i_snr] + 2);
        // printf("gain_s: %f, inf16:%d\n", gain_s, inf16);
        isrran_vec_quant_fs(rm_llr, rm_llr_s, gain_s, 0, inf16, BATCH_SIZE * E);
      }

      // Rate dematcher
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_rm_rx_s(&rm_rx_s, rm_llr_s + j * E, llr_s + j * code.N, E, code.n, K, bil);
      }

      // decoding 16-bit
      gettimeofday(&t[1], NULL);
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_decoder_decode_s(
      ...
      // 8-bit quantization
      if (snr_db_vec[i_snr] == 101) {
        isrran_vec_quant_fc(rm_llr, rm_llr_c, 32, 0, 127, BATCH_SIZE * E);
      } else {
        gain_c = inf8 * var[i_snr] / 20 / (1 / var[i_snr] + 2);
        isrran_vec_quant_fc(rm_llr, rm_llr_c, gain_c, 0, inf8, BATCH_SIZE * E);
      }

      // Rate dematcher
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_rm_rx_c(&rm_rx_c, rm_llr_c + j * E, llr_c + j * code.N, E, code.n, K, bil);
      }

      // Decoding
      gettimeofday(&t[1], NULL);
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_decoder_decode_c(
      ...
      for (int i = 0; i < BATCH_SIZE; i++) {
        errors_symb_c = isrran_bit_diff(data_tx + i * K, data_rx_c + i * K, K);
      ...
      // 8-bit quantization
      if (snr_db_vec[i_snr] == 101) {
        isrran_vec_quant_fc(rm_llr, rm_llr_c_avx2, 32, 0, 127, BATCH_SIZE * E);
      } else {
        gain_c_avx2 = inf8 * var[i_snr] / 20 / (1 / var[i_snr] + 2);
        isrran_vec_quant_fc(rm_llr, rm_llr_c_avx2, gain_c_avx2, 0, inf8, BATCH_SIZE * E);
      }

      // Rate dematcher
      for (j = 0; j < BATCH_SIZE; j++) {
        isrran_polar_rm_rx_c(&rm_rx_c, rm_llr_c_avx2 + j * E, llr_c_avx2 + j * code.N, E, code.n, K, bil);
      }
      ...
      // check errors 8-bits decoder
      for (int i = 0; i < BATCH_SIZE; i++) {
        errors_symb_c_avx2 = isrran_bit_diff(data_tx + i * K, data_rx_c_avx2 + i * K, K);
...
#endif
  // free code
  isrran_polar_code_free(&code);
  isrran_polar_encoder_free(&enc);
  isrran_polar_decoder_free(&dec);
  isrran_polar_decoder_free(&dec_s);
  isrran_polar_decoder_free(&dec_c);
  isrran_polar_rm_rx_free_f(&rm_rx_f);
  isrran_polar_rm_rx_free_s(&rm_rx_s);
  isrran_polar_rm_rx_free_c(&rm_rx_c);
  isrran_polar_rm_tx_free(&rm_tx);
#ifdef LV_HAVE_AVX2
  isrran_polar_encoder_free(&enc_avx2);
  isrran_polar_decoder_free(&dec_c_avx2);
#endif // LV_HAVE_AVX2
