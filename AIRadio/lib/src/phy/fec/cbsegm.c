 ...
 */
const uint32_t tc_cb_sizes[ISRRAN_NOF_TC_CB_SIZES] = {
...
static void cbsegm_cb_size(uint32_t B, uint32_t Z, uint32_t* C, uint32_t* B_prime)
{
  if (B <= Z) {
    ...
    *C       = ISRRAN_CEIL(B, (Z - 24U));
    *B_prime = B + 24U * (*C);
  }
}

int isrran_cbsegm(isrran_cbsegm_t* s, uint32_t tbs)
{
  uint32_t Bp, B, idx1;
  int      ret;

  if (tbs == 0) {
    bzero(s, sizeof(isrran_cbsegm_t));
    ret = ISRRAN_SUCCESS;
  } else {
    B      = tbs + 24;
    s->tbs = tbs;
    ...
    // Calculate CB sizes
    cbsegm_cb_size(B, ISRRAN_TCOD_MAX_LEN_CB, &s->C, &Bp);

    ret = isrran_cbsegm_cbindex((Bp - 1) / s->C + 1);
    if (ret != ISRRAN_ERROR) {
      idx1 = (uint32_t)ret;
      ret  = isrran_cbsegm_cbsize(idx1);
      if (ret != ISRRAN_ERROR) {
        s->K1     = (uint32_t)ret;
        s->K1_idx = idx1;
        if (idx1 > 0) {
          ret = isrran_cbsegm_cbsize(idx1 - 1);
        }
        if (ret != ISRRAN_ERROR) {
          if (s->C == 1) {
            s->K2     = 0;
...
int isrran_cbsegm_cbindex(uint32_t long_cb)
{
  int j = 0;
  while (j < ISRRAN_NOF_TC_CB_SIZES && tc_cb_sizes[j] < long_cb) {
    j++;
  }

  if (j == ISRRAN_NOF_TC_CB_SIZES) {
    return ISRRAN_ERROR;
  } else {
    return j;
  }
...
int isrran_cbsegm_cbsize(uint32_t index)
{
  if (index < ISRRAN_NOF_TC_CB_SIZES) {
...
bool isrran_cbsegm_cbsize_isvalid(uint32_t size)
{
...
static int cbsegm_ldpc_select_ls(uint32_t Kp, uint32_t K_b, uint32_t* Z_c, uint8_t* i_ls)
{
  ...
  if (ISRRAN_CEIL(Kp, K_b) > MAX_LIFTSIZE) {
    return ISRRAN_ERROR;
  }

  // Iterate from the minimum required lift size to the maximum value
  for (uint16_t Z = ISRRAN_CEIL(Kp, K_b); Z <= MAX_LIFTSIZE; Z++) {
 ...
 */
static uint32_t isrran_cbsegm_ldpc_L(uint32_t tbs)
{
  if (tbs <= 3824) {
    return 16;
  }
...
static int isrran_cbsegm_ldpc(isrran_cbsegm_t* s, isrran_basegraph_t bg, uint32_t tbs)
{
  // Check input pointer
  if (s == NULL) {
  ...
  uint32_t L = isrran_cbsegm_ldpc_L(tbs);

  // Select maximum code block size
  uint32_t K_cb = (bg == BG1) ? ISRRAN_LDPC_BG1_MAX_LEN_CB : ISRRAN_LDPC_BG2_MAX_LEN_CB;
...
int isrran_cbsegm_ldpc_bg1(isrran_cbsegm_t* s, uint32_t tbs)
{
  return isrran_cbsegm_ldpc(s, BG1, tbs);
}

int isrran_cbsegm_ldpc_bg2(isrran_cbsegm_t* s, uint32_t tbs)
{
  return isrran_cbsegm_ldpc(s, BG2, tbs);
}