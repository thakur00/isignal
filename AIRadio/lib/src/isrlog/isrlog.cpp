...
{
  return isrlog_instance::get().get_channel_repo().emplace(
...
log_channel* isrlog::find_log_channel(const std::string& id)
{
  return isrlog_instance::get().get_channel_repo().find(id);
}

log_channel& isrlog::fetch_log_channel(const std::string& id)
{
  ...
  isrlog_instance& instance = isrlog_instance::get();
  return fetch_log_channel_helper(clean_id, instance.get_default_sink(), instance.get_backend());
}

log_channel& isrlog::fetch_log_channel(const std::string& id, sink& s, log_channel_config config)
{
  assert(!id.empty() && "Empty id string");
...
void isrlog::set_default_log_formatter(std::unique_ptr<log_formatter> f)
{
  isrlog_instance::get().set_default_formatter(std::move(f));
}

std::unique_ptr<log_formatter> isrlog::get_default_log_formatter()
{
  return isrlog_instance::get().get_default_formatter();
}

std::unique_ptr<log_formatter> isrlog::create_text_formatter()
{
  return std::unique_ptr<log_formatter>(new text_formatter);
}

std::unique_ptr<log_formatter> isrlog::create_json_formatter()
{
...
void isrlog::set_default_sink(sink& s)
{
  isrlog_instance::get().set_default_sink(s);
}

sink& isrlog::get_default_sink()
{
  return isrlog_instance::get().get_default_sink();
}

sink* isrlog::find_sink(const std::string& id)
{
  auto ptr = isrlog_instance::get().get_sink_repo().find(id);
  return (ptr) ? ptr->get() : nullptr;
}

sink& isrlog::fetch_stdout_sink(const std::string& id, std::unique_ptr<log_formatter> f)
{
  assert(!id.empty() && "Empty id string");
  ...
  auto& s = isrlog_instance::get().get_sink_repo().emplace(
...
sink& isrlog::fetch_stderr_sink(const std::string& id, std::unique_ptr<log_formatter> f)
{
  assert(!id.empty() && "Empty id string");
...
sink& isrlog::fetch_file_sink(const std::string&             path,
                              size_t                         max_size,
                              bool                           force_flush,
...
sink& isrlog::fetch_syslog_sink(const std::string&             preamble_,
...
bool isrlog::install_custom_sink(const std::string& id, std::unique_ptr<sink> s)
{
  assert(!id.empty() && "Empty path string");

  sink* input_sink    = s.get();
  sink* returned_sink = isrlog_instance::get().get_sink_repo().emplace(id, std::move(s)).get();
...
void isrlog::init(backend_priority priority)
{
  isrlog_instance::get().get_backend().start(priority);
}
...
void isrlog::set_error_handler(error_handler handler)
{
  isrlog_instance::get().set_error_handler(std::move(handler));
}
...
detail::any* isrlog::detail::find_logger(const std::string& id)
{
  return isrlog_instance::get().get_logger_repo().find(id);
}

detail::any* isrlog::detail::fetch_logger(const std::string& id, detail::any&& logger)
{
  assert(!id.empty() && "Empty id string");
  return &isrlog_instance::get().get_logger_repo().emplace(id, std::move(logger));
}
...
basic_logger& isrlog::fetch_basic_logger(const std::string& id, bool should_print_context)
{
  assert(!id.empty() && "Empty id string");
  ...
  return fetch_basic_logger_helper(id, isrlog_instance::get().get_default_sink(), should_print_context);
}

basic_logger& isrlog::fetch_basic_logger(const std::string& id, sink& s, bool should_print_context)
{
  assert(!id.empty() && "Empty id string");
...
log_channel* isrlog::create_log_channel(const std::string& id, sink& s)
{
...
sink* isrlog::create_stdout_sink(const std::string& name)
{
  return isrlog_instance::get().get_sink_repo().find("stdout")->get();
}

sink* isrlog::create_stderr_sink(const std::string& name)
{
  return isrlog_instance::get().get_sink_repo().find("stderr")->get();
}

sink* isrlog::create_file_sink(const std::string& path, size_t max_size)
{
  //: TODO: GCC5 or lower versions emits an error if we use the new() expression
  // directly, use redundant piecewise_construct instead.
  return isrlog_instance::get()
      .get_sink_repo()
...
basic_logger* isrlog::create_basic_logger(const std::string& id, sink& s, bool should_print_context)
{
  assert(!id.empty() && "Empty id string");

  static constexpr char basic_logger_chan_tags[] = {'E', 'W', 'I', 'D'};

  auto& logger_repo = isrlog_instance::get().get_logger_repo();
