...
/// Tracer sink name.
static constexpr char sink_name[] = "isrlog_trace_sink";

void isrlog::event_trace_init()
{
...
void isrlog::event_trace_init(log_channel& c)
{
...
bool isrlog::event_trace_init(const std::string& filename, std::size_t capacity)
{
  // Nothing to do if the user previously set a custom channel or this is not
  // the first time this function is called.
  if (tracer) {
...
isrlog::detail::scoped_complete_event::~scoped_complete_event()
{
  if (!tracer) {
    return;
  }
