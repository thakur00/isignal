  ...
  if (metadata.fmtstring) {
    if (metadata.store) {
      ...
      try {
        ...
        fmt::print(stderr, "isrLog error - Invalid format string: \"{}\"\n", metadata.fmtstring);
        fmt::format_to(buffer, " -> isrLog error - Invalid format string: \"{}\"", metadata.fmtstring);
                                        ...
                                        fmt::memory_buffer&               buffer)
{
  ...
  if (md.store) {
    ...
    try {
      ...
      fmt::print(stderr, "isrLog error - Invalid format string: \"{}\"\n", md.fmtstring);
      fmt::format_to(buffer, " -> isrLog error - Invalid format string: \"{}\"", md.fmtstring);
#ifdef STOP_ON_WARNING
      std::abort();
#endif
    }
