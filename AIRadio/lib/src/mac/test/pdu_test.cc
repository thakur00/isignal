...
static std::unique_ptr<isrran::mac_pcap> pcap_handle = nullptr;
...
int mac_rar_pdu_unpack_test3()
{
  ...
  TESTASSERT(rar_pdu_msg.parse_packet(rar_pdu) != ISRRAN_SUCCESS);
...
class rlc_dummy : public isrran::read_pdu_interface
{
public:
...
int mac_sch_pdu_pack_test1()
{
  ...
  isrran::sch_pdu pdu(10, mac_logger);
...
int mac_sch_pdu_pack_test5()
{
  ...
  const uint32_t  pdu_size = ISRRAN_MAX_TBSIZE_BITS / 8; // Max. DL allocation for a single TB using 256 QAM
  isrran::sch_pdu pdu(10, mac_logger);
...
int mac_sch_pdu_pack_test6()
{
  ...
  TESTASSERT(pdu.new_subh());
  TESTASSERT(pdu.get()->set_bsr(buff_size_tx, isrran::ul_sch_lcid::LONG_BSR));
  ...
  while (pdu.next()) {
    if (!pdu.get()->is_sdu() && pdu.get()->ul_sch_ce_type() == isrran::ul_sch_lcid::LONG_BSR) {
  ...
  TESTASSERT(pdu.new_subh());
  TESTASSERT(pdu.get()->set_bsr(buff_size_tx_low_edge, isrran::ul_sch_lcid::LONG_BSR));
  ...
  TESTASSERT(pdu.new_subh());
  TESTASSERT(pdu.get()->set_bsr(buff_size_max_idx, isrran::ul_sch_lcid::LONG_BSR));
...
int mac_sch_pdu_pack_test7()
{
  ...
  // adding SDU fails
  TESTASSERT(pdu.get()->set_sdu(2, 8, &rlc) == ISRRAN_ERROR);
...
int mac_sch_pdu_pack_test8()
{
  ...
  tv[1] &= ((1u << (uint32_t)ISRRAN_MAX_CARRIERS) - 1u);
  ...
  TESTASSERT(pdu.new_subh());
  std::array<bool, ISRRAN_MAX_CARRIERS> cc_activ_list = {};
  for (uint8_t i = 1; i < ISRRAN_MAX_CARRIERS; ++i) {
...
int mac_sch_pdu_pack_test9()
{
  ...
  TESTASSERT(pdu.new_subh());
  TESTASSERT(pdu.get()->set_bsr(buff_size, isrran::ul_sch_lcid::SHORT_BSR));
...
int mac_sch_pdu_pack_test11()
{
  ...
  TESTASSERT(pdu.new_subh());
  TESTASSERT(pdu.get()->set_bsr(buff_size, isrran::ul_sch_lcid::LONG_BSR) == false);
...
int mac_sch_pdu_pack_error_test()
{
  ...
  // adding SDU fails
  TESTASSERT(pdu.get()->set_sdu(1, 8, &rlc) == ISRRAN_ERROR);
...
int mac_mch_pdu_pack_test1()
{
  ...
  isrran::mch_pdu mch_pdu(10, mac_logger);
  byte_buffer_t   buffer;
...
int mac_sch_pdu_unpack_test1()
{
  ...
  isrran::sch_pdu pdu(10, isrlog::fetch_basic_logger("MAC"));
...
int mac_sch_pdu_unpack_test2()
{
  ...
  isrran::sch_pdu pdu(20, isrlog::fetch_basic_logger("MAC"));
  pdu.init_rx(sizeof(tv), false);
  pdu.parse_packet(tv);
...
int mac_sch_pdu_unpack_test3()
{
  ...
  TESTASSERT(pdu.nof_subh() == 2);
  while (pdu.next()) {
    if (!pdu.get()->is_sdu() && pdu.get()->dl_sch_ce_type() == isrran::dl_sch_lcid::TA_CMD) {
...
int mac_sch_pdu_unpack_test4()
{
  ...
  isrran::sch_pdu pdu(5, isrlog::fetch_basic_logger("MAC"));
  pdu.init_rx(sizeof(tv), true);
  pdu.parse_packet(tv);
  ...
  TESTASSERT(pdu.get()->ul_sch_ce_type() == isrran::ul_sch_lcid::PADDING);

  // PHR
  TESTASSERT(pdu.next());
  TESTASSERT(pdu.get()->is_sdu() == false);
  TESTASSERT(pdu.get()->ul_sch_ce_type() == isrran::ul_sch_lcid::PHR_REPORT);
...
{
#if HAVE_PCAP
  pcap_handle = std::unique_ptr<isrran::mac_pcap>(new isrran::mac_pcap());
  pcap_handle->open("mac_pdu_test.pcap");
#endif
  auto& mac_logger = isrlog::fetch_basic_logger("MAC", false);
  mac_logger.set_level(isrlog::basic_levels::debug);
  mac_logger.set_hex_dump_max_size(-1);
  auto& rlc_logger = isrlog::fetch_basic_logger("RLC", false);
  rlc_logger.set_level(isrlog::basic_levels::debug);
  rlc_logger.set_hex_dump_max_size(-1);

  isrlog::init();

  TESTASSERT(mac_rar_pdu_unpack_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_rar_pdu_unpack_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_rar_pdu_unpack_test3() == ISRRAN_SUCCESS);
  TESTASSERT(mac_rar_pdu_pack_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_rar_pdu_pack_test2() == ISRRAN_SUCCESS);

  TESTASSERT(mac_sch_pdu_pack_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test3() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test4() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test5() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test6() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test7() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test8() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test9() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test10() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_pack_test11() == ISRRAN_SUCCESS);

  TESTASSERT(mac_sch_pdu_pack_error_test() == ISRRAN_SUCCESS);

  TESTASSERT(mac_mch_pdu_pack_test1() == ISRRAN_SUCCESS);

  TESTASSERT(mac_sch_pdu_unpack_test1() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_unpack_test2() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_unpack_test3() == ISRRAN_SUCCESS);
  TESTASSERT(mac_sch_pdu_unpack_test4() == ISRRAN_SUCCESS);

  TESTASSERT(mac_slsch_pdu_unpack_test1() == ISRRAN_SUCCESS);
