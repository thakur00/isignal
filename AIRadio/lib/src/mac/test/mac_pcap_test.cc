...
void write_pcap_eutra_thread_function(isrran::mac_pcap*               pcap_handle,
...
void write_pcap_nr_thread_function(isrran::mac_pcap* pcap_handle, const std::array<uint8_t, 11>& pdu, uint32_t num_pdus)
{
  for (uint32_t i = 0; i < num_pdus; i++) {
...
{
  auto& pcap_logger = isrlog::fetch_basic_logger("PCAP");
  ...
  std::unique_ptr<isrran::mac_pcap> pcap_handle = std::unique_ptr<isrran::mac_pcap>(new isrran::mac_pcap());
  TESTASSERT(pcap_handle->open("mac_pcap_test.pcap") == ISRRAN_SUCCESS);
  TESTASSERT(pcap_handle->open("mac_pcap_test.pcap") != ISRRAN_SUCCESS); // open again will fail
  ...
  TESTASSERT(pcap_handle->close() == ISRRAN_SUCCESS);
...
int mac_pcap_nr_test()
{
  ...
  std::unique_ptr<isrran::mac_pcap> pcap_handle = std::unique_ptr<isrran::mac_pcap>(new isrran::mac_pcap());
  TESTASSERT(pcap_handle->open("mac_pcap_nr_test.pcap") == ISRRAN_SUCCESS);
  TESTASSERT(pcap_handle->open("mac_pcap_nr_test.pcap") != ISRRAN_SUCCESS); // open again will fail
...
int main()
{
  ...
  auto& pcap_logger = isrlog::fetch_basic_logger("PCAP", false);
  pcap_logger.set_level(isrlog::basic_levels::debug);
  pcap_logger.set_hex_dump_max_size(-1);

  pcap_logger.info("Start mac_pcap_eutra_test");
  TESTASSERT(mac_pcap_eutra_test() == ISRRAN_SUCCESS);
  pcap_logger.info("Start mac_pcap_nr_test");
  TESTASSERT(mac_pcap_nr_test() == ISRRAN_SUCCESS);
