...
int mac_dl_sch_pdu_pack_test5()
{
  ...
  auto& mac_logger = isrlog::fetch_basic_logger("MAC");
  mac_logger.info(
      tx_buffer.msg, tx_buffer.N_bytes, "Generated MAC PDU (%d B): %s", tx_buffer.N_bytes, isrran::to_c_str(buff));
...
int mac_dl_sch_pdu_unpack_pack_test7()
{
  ...
  // add ConRes CE
  isrran::mac_sch_subpdu_nr::ue_con_res_id_t id = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
  TESTASSERT(tx_pdu.add_ue_con_res_id_ce(id) == ISRRAN_SUCCESS);
...
int mac_rar_pdu_test7()
{
  ...
  auto& mac_logger = isrlog::fetch_basic_logger("MAC");
  mac_logger.info("Rx PDU: %s", isrran::to_c_str(buff));
...
int mac_ul_sch_pdu_unpack_test1()
{
  ...
  isrran::mac_sch_pdu_nr pdu(true);
...
int mac_dl_sch_pdu_unpack_and_pack_test6()
{
  ...
  // Unpack TV
  {
    isrran::mac_sch_pdu_nr pdu_rx(true);
...
int mac_ul_sch_pdu_unpack_test6()
{
  ...
  isrran::mac_sch_pdu_nr pdu(true);
  TESTASSERT(pdu.unpack(tv, sizeof(tv)) == ISRRAN_ERROR);
