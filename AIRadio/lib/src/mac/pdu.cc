...
const char* lcid_t::to_string() const
{
  switch (type) {
    case ch_type::dl_sch:
      return isrran::to_string(dl_sch);
    case ch_type::ul_sch:
      return isrran::to_string(ul_sch);
    case ch_type::mch:
      return isrran::to_string(mch);
    default:
...
bool lcid_t::is_sdu() const
{
  switch (type) {
    case ch_type::dl_sch:
      return isrran::is_sdu(dl_sch);
    case ch_type::ul_sch:
      return isrran::is_sdu(ul_sch);
    case ch_type::mch:
      return isrran::is_sdu(mch);
    default:
      return false;
  }
}
...
{
  return write_packet(isrlog::fetch_basic_logger("MAC"));
}

/* Writes the MAC PDU in the packet, including the MAC headers and CE payload. Section 6.1.2 */
uint8_t* sch_pdu::write_packet(isrlog::basic_logger& log)
{
  ...
  if (buffer_tx->N_bytes != pdu_len) {
    isrran::console("------------------------------\n");
    isrran::console("Wrote PDU: pdu_len=%d, expected_pdu_len=%d, header_and_ce=%d (%d+%d), nof_subh=%d, last_sdu=%d, "
                    "onepad=%d, multi=%d\n",
                    buffer_tx->N_bytes,
                    pdu_len,
                    header_sz + ce_payload_sz,
...
void sch_subh::init()
{
  isrran_vec_u8_zero(w_payload_ce, 64);
...
bool sch_subh::set_scell_activation_cmd(const std::array<bool, ISRRAN_MAX_CARRIERS>& active_scell_idxs)
{
...
rar_pdu::rar_pdu(uint32_t max_rars_, isrlog::basic_logger& logger) : pdu(max_rars_, logger)
{
  backoff_indicator     = 0;
...
void rar_subh::to_string(fmt::memory_buffer& buffer)
{
  ...
  char tmp[16];
  isrran_vec_sprint_hex(tmp, sizeof(tmp), grant, RAR_GRANT_LEN);
...
void rar_subh::write_payload(uint8_t** ptr)
{
  ...
  *(*ptr + 1) = (uint8_t)((ta & 0xf) << 4) | (grant[0] << 3) | (grant[1] << 2) | (grant[2] << 1) | grant[3];
  uint8_t* x  = &grant[4];
  *(*ptr + 2) = (uint8_t)isrran_bit_pack(&x, 8);
  *(*ptr + 3) = (uint8_t)isrran_bit_pack(&x, 8);
...
void rar_subh::read_payload(uint8_t** ptr)
{
  if (type == RAPID) {
    ...
    uint8_t* x = &grant[4];
    isrran_bit_unpack(*(*ptr + 2), &x, 8);
    isrran_bit_unpack(*(*ptr + 3), &x, 8);
