...
mac_rar_subpdu_nr::mac_rar_subpdu_nr(mac_rar_pdu_nr* parent_) :
  parent(parent_), logger(isrlog::fetch_basic_logger("MAC-NR"))
{}
  ...
  if (type == RAPID) {
    ...
    if (parent->has_si_rapid() == false) {
      if (parent->get_remaining_len() >= MAC_RAR_NBYTES) {
        ...
        isrran_bit_unpack(*(rar + 2), &x, 8);
        isrran_bit_unpack(*(rar + 3), &x, 8);
        isrran_bit_unpack(*(rar + 4), &x, 8);
    ...
    *ptr = (uint8_t)((E_bit ? 1 : 0) << 7) | ((int)rar_subh_type_t::RAPID << 6) | ((uint8_t)rapid & 0x3f);
    ...
    if (parent->has_si_rapid() == false) {
      ...
      *ptr = (uint8_t)((ta & 0x1f) << 3) | (ul_grant.at(0) << 2) | (ul_grant.at(1) << 1) | (ul_grant.at(2));
      ...
      uint8_t* x = &ul_grant.at(3);
      *(ptr + 0) = (uint8_t)isrran_bit_pack(&x, 8);
      *(ptr + 1) = (uint8_t)isrran_bit_pack(&x, 8);
...
void mac_rar_subpdu_nr::to_string(fmt::memory_buffer& buffer)
{
  ...
  if (has_rapid()) {
    char tmp[16] = {};
    isrran_vec_sprint_hex(tmp, sizeof(tmp), ul_grant.data(), UL_GRANT_NBITS);
...
mac_rar_pdu_nr::mac_rar_pdu_nr() : logger(isrlog::fetch_basic_logger("MAC-NR")) {}
