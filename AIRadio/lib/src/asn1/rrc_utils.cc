...
isrran::rlc_config_t make_rlc_config_t(const asn1::rrc::rlc_cfg_c& asn1_type)
{
...
isrran::rlc_config_t make_rlc_config_t(const asn1::rrc::srb_to_add_mod_s& asn1_type)
{
...
void to_asn1(asn1::rrc::rlc_cfg_c* asn1_type, const isrran::rlc_config_t& cfg)
{
...
isrran::pdcp_config_t make_srb_pdcp_config_t(const uint8_t bearer_id, bool is_ue)
{
...
isrran::pdcp_config_t make_drb_pdcp_config_t(const uint8_t bearer_id, bool is_ue)
{
  pdcp_config_t cfg(bearer_id,
                    PDCP_RB_IS_DRB,
                    is_ue ? SECURITY_DIRECTION_UPLINK : SECURITY_DIRECTION_DOWNLINK,
...
uint8_t get_pdcp_drb_sn_len(const pdcp_cfg_s& pdcp_cfg)
{
  ...
  if (pdcp_cfg.rlc_um_present) {
    if (pdcp_cfg.rlc_um.pdcp_sn_size.value == pdcp_cfg_s::rlc_um_s_::pdcp_sn_size_e_::len7bits) {
      sn_len = isrran::PDCP_SN_LEN_7;
    }
  }
  if (pdcp_cfg.ext) {
    if (pdcp_cfg.pdcp_sn_size_v1130_present) {
      sn_len = isrran::PDCP_SN_LEN_15;
    }
...
isrran::pdcp_config_t make_drb_pdcp_config_t(const uint8_t bearer_id, bool is_ue, const asn1::rrc::pdcp_cfg_s& pdcp_cfg)
{
...
isrenb::sched_interface::ant_info_ded_t make_ant_info_ded(const asn1::rrc::ant_info_ded_s& asn1_type)
{
  isrenb::sched_interface::ant_info_ded_t ant_ded = {};
  ant_ded.tx_mode = static_cast<isrenb::sched_interface::ant_info_ded_t::tx_mode_t>(asn1_type.tx_mode.value);
  ant_ded.cookbook_subset_type = isrenb::sched_interface::ant_info_ded_t::codebook_t::none;
  if (asn1_type.codebook_subset_restrict_present) {
    ...
    ant_ded.cookbook_subset_type =
        static_cast<isrenb::sched_interface::ant_info_ded_t::codebook_t>(asn1code.type().value);
  ...
  ant_ded.ue_tx_ant_sel = isrenb::sched_interface::ant_info_ded_t::ue_tx_ant_sel_t::release;
  if (asn1_type.ue_tx_ant_sel.type().value == setup_opts::setup) {
    ant_ded.ue_tx_ant_sel =
        (asn1_type.ue_tx_ant_sel.setup().value == ant_info_ded_s::ue_tx_ant_sel_c_::setup_opts::closed_loop)
            ? isrenb::sched_interface::ant_info_ded_t::ue_tx_ant_sel_t::closed_loop
            : isrenb::sched_interface::ant_info_ded_t::ue_tx_ant_sel_t::open_loop;
  }
  return ant_ded;
}
...
void set_phy_cfg_t_dedicated_cfg(phy_cfg_t* cfg, const asn1::rrc::phys_cfg_ded_s& asn1_type)
{
  ...
  if (asn1_type.pucch_cfg_ded_v1020.is_present()) {
    ...
    if (pucch_cfg_ded->pucch_format_r10_present) {
      ...
      if (pucch_format_r10->type() == pucch_format_r10_t::types::format3_r10) {
        ...
        cfg->ul_cfg.pucch.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_PUCCH3;

        auto* format3_r13 = &pucch_format_r10->format3_r10();
        for (uint32_t n = 0; n < ISRRAN_MIN(format3_r13->n3_pucch_an_list_r13.size(), ISRRAN_PUCCH_SIZE_AN_CS); n++) {
        ...
        if (ch_sel_r10->n1_pucch_an_cs_r10_present) {
          ...
          if (n1_pucch_an_cs_r10->type() == asn1::rrc::setup_e::setup) {
            // Select feedback mode
            cfg->ul_cfg.pucch.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_CS;
            ...
            for (uint32_t i = 0; i < ISRRAN_MIN(n1_pucch_an_cs_list.size(), ISRRAN_PUCCH_NOF_AN_CS); i++) {
              asn1::rrc::n1_pucch_an_cs_r10_l n1_pucch_an_cs = n1_pucch_an_cs_list[i];
              for (uint32_t j = 0; j < ISRRAN_PUCCH_SIZE_AN_CS; j++) {
                cfg->ul_cfg.pucch.n1_pucch_an_cs[j][i] = n1_pucch_an_cs[j];
              }
            }
          } else {
            cfg->ul_cfg.pucch.ack_nack_feedback_mode = ISRRAN_PUCCH_ACK_NACK_FEEDBACK_MODE_NORMAL;
          }
        }
      } else {
        // Do nothing
  ...
  if (asn1_type.ul_pwr_ctrl_ded_present) {
    ...
    cfg->ul_cfg.power_ctrl.p_isr_offset = asn1_type.ul_pwr_ctrl_ded.pisr_offset;
  }
  ...
  if (asn1_type.isr_ul_cfg_ded_present) {
    cfg->ul_cfg.isr.dedicated_enabled = asn1_type.isr_ul_cfg_ded.type() == asn1::rrc::setup_e::setup;
    if (cfg->ul_cfg.isr.dedicated_enabled) {
      cfg->ul_cfg.isr.configured = cfg->ul_cfg.isr.dedicated_enabled and cfg->ul_cfg.isr.common_enabled;
      cfg->ul_cfg.isr.I_isr      = asn1_type.isr_ul_cfg_ded.setup().isr_cfg_idx;
      cfg->ul_cfg.isr.B          = asn1_type.isr_ul_cfg_ded.setup().isr_bw;
      cfg->ul_cfg.isr.b_hop      = asn1_type.isr_ul_cfg_ded.setup().isr_hop_bw;
      cfg->ul_cfg.isr.n_rrc      = asn1_type.isr_ul_cfg_ded.setup().freq_domain_position;
      cfg->ul_cfg.isr.k_tc       = asn1_type.isr_ul_cfg_ded.setup().tx_comb;
      cfg->ul_cfg.isr.n_isr      = asn1_type.isr_ul_cfg_ded.setup().cyclic_shift;
    }
  }
  ...
  if (asn1_type.ant_info_r10.is_present() &&
    ...
    if ((isrran_tm_t)tx_mode < ISRRAN_TMINV) {
      cfg->dl_cfg.tm = (isrran_tm_t)tx_mode;
    } else {
      fprintf(stderr,
...
void set_phy_cfg_t_common_pusch(phy_cfg_t* cfg, const asn1::rrc::pusch_cfg_common_s& asn1_type)
{
  ...
  bzero(&cfg->ul_cfg.dmrs, sizeof(isrran_refsignal_dmrs_pusch_cfg_t));
  ...
  bzero(&cfg->ul_cfg.hopping, sizeof(isrran_pusch_hopping_cfg_t));
  ...
  cfg->ul_cfg.hopping.hop_mode =
      asn1_type.pusch_cfg_basic.hop_mode.value ==
          ...
          ? cfg->ul_cfg.hopping.ISRRAN_PUSCH_HOP_MODE_INTRA_SF
          : cfg->ul_cfg.hopping.ISRRAN_PUSCH_HOP_MODE_INTER_SF;
}
...
void set_phy_cfg_t_common_isr(phy_cfg_t* cfg, const asn1::rrc::isr_ul_cfg_common_c& asn1_type)
{
  cfg->ul_cfg.isr.common_enabled = asn1_type.type() == asn1::rrc::setup_e::setup;
  if (cfg->ul_cfg.isr.common_enabled) {
    cfg->ul_cfg.isr.simul_ack       = asn1_type.setup().ack_nack_isr_simul_tx;
    cfg->ul_cfg.isr.bw_cfg          = asn1_type.setup().isr_bw_cfg.to_number();
    cfg->ul_cfg.isr.subframe_config = asn1_type.setup().isr_sf_cfg.to_number();
  }
}
...
void set_phy_cfg_t_scell_config(phy_cfg_t* cfg, const asn1::rrc::scell_to_add_mod_r10_s& asn1_type)
{
  if (asn1_type.rr_cfg_common_scell_r10_present) {
    ...
    if (rr_cfg_common_scell_r10->ul_cfg_r10_present) {
      ...
      bzero(&cfg->ul_cfg.power_ctrl, sizeof(isrran_ue_ul_powerctrl_t));
      ...
      // Parse ISR
      cfg->ul_cfg.isr.common_enabled = ul_cfg_r10->isr_ul_cfg_common_r10.type() == asn1::rrc::setup_e::setup;
      if (cfg->ul_cfg.isr.common_enabled) {
        auto* isr_ul_cfg_common         = &ul_cfg_r10->isr_ul_cfg_common_r10.setup();
        cfg->ul_cfg.isr.simul_ack       = isr_ul_cfg_common->ack_nack_isr_simul_tx;
        cfg->ul_cfg.isr.bw_cfg          = isr_ul_cfg_common->isr_bw_cfg.to_number();
        cfg->ul_cfg.isr.subframe_config = isr_ul_cfg_common->isr_sf_cfg.to_number();
      }
    ...
    if (rr_cfg_ded_scell_r10->phys_cfg_ded_scell_r10_present) {
      ...
      if (phys_cfg_ded_scell_r10->non_ul_cfg_r10_present) {
        ...
        if (non_ul_cfg->ant_info_r10_present) {
          if (non_ul_cfg->ant_info_r10.tx_mode_r10.to_number() < (uint8_t)ISRRAN_TMINV) {
            cfg->dl_cfg.tm = (isrran_tm_t)non_ul_cfg->ant_info_r10.tx_mode_r10.to_number();
          } else {
            fprintf(stderr,
                    "Transmission mode (R10) %s is not supported\n",
      ...
      if (phys_cfg_ded_scell_r10->ul_cfg_r10_present) {
        ...
        if (ul_cfg_r10->isr_ul_cfg_ded_r10_present) {
          cfg->ul_cfg.isr.dedicated_enabled = ul_cfg_r10->isr_ul_cfg_ded_r10.type() == asn1::rrc::setup_e::setup;
          if (cfg->ul_cfg.isr.dedicated_enabled) {
            auto* isr_ul_cfg_ded_r10   = &ul_cfg_r10->isr_ul_cfg_ded_r10.setup();
            cfg->ul_cfg.isr.configured = cfg->ul_cfg.isr.dedicated_enabled and cfg->ul_cfg.isr.common_enabled;
            cfg->ul_cfg.isr.I_isr      = isr_ul_cfg_ded_r10->isr_cfg_idx;
            cfg->ul_cfg.isr.B          = isr_ul_cfg_ded_r10->isr_bw;
            cfg->ul_cfg.isr.b_hop      = isr_ul_cfg_ded_r10->isr_hop_bw;
            cfg->ul_cfg.isr.n_rrc      = isr_ul_cfg_ded_r10->freq_domain_position;
            cfg->ul_cfg.isr.k_tc       = isr_ul_cfg_ded_r10->tx_comb;
            cfg->ul_cfg.isr.n_isr      = isr_ul_cfg_ded_r10->cyclic_shift;
          }
        }
      }
...
static_assert(ASN1_RRC_MAX_SESSION_PER_PMCH == pmch_info_t::max_session_per_pmch, "ASN1 to isrRAN interface mismatch");
