...
{
  if (isrran_unlikely(val != choice_id)) {
...
void log_error_code(ISRASN_CODE code, const char* filename, int line)
{
  switch (code) {
    case ISRASN_ERROR_ENCODE_FAIL:
      log_error("[%s][%d] Encoding failure.", filename, line);
      break;
    case ISRASN_ERROR_DECODE_FAIL:
      log_error("[%s][%d] Decoding failure.", filename, line);
      break;
    default:
      log_warning("[%s][%d] ISRASN_CODE=%d not recognized.", filename, line, (int)code);
  }
}
...
ISRASN_CODE bit_ref::pack(uint64_t val, uint32_t n_bits)
{
  if (n_bits >= 64) {
...
ISRASN_CODE unpack_bits(T& val, Ptr& ptr, uint8_t& offset, const uint8_t* max_ptr, uint32_t n_bits)
{
...
template ISRASN_CODE unpack_bits<uint8_t, const uint8_t*>(uint8_t&        val,
                                                          const uint8_t*& ptr,
                                                          uint8_t&        offset,
                                                          const uint8_t*  max_ptr,
                                                          uint32_t        n_bits);
...
template ISRASN_CODE unpack_bits<uint16_t, const uint8_t*>(uint16_t&       val,
                                                           const uint8_t*& ptr,
                                                           uint8_t&        offset,
                                                           const uint8_t*  max_ptr,
                                                           uint32_t        n_bits);
...
template ISRASN_CODE unpack_bits<uint32_t, const uint8_t*>(uint32_t&       val,
                                                           const uint8_t*& ptr,
                                                           uint8_t&        offset,
                                                           const uint8_t*  max_ptr,
                                                           uint32_t        n_bits);
...
template ISRASN_CODE unpack_bits<uint64_t, const uint8_t*>(uint64_t&       val,
                                                           const uint8_t*& ptr,
                                                           uint8_t&        offset,
                                                           const uint8_t*  max_ptr,
                                                           uint32_t        n_bits);
...
template <typename Ptr>
ISRASN_CODE bit_ref_impl<Ptr>::unpack_bytes(uint8_t* buf, uint32_t n_bytes)
{
  if (n_bytes == 0) {
    return ISRASN_SUCCESS;
  }
...
template <typename Ptr>
ISRASN_CODE bit_ref_impl<Ptr>::align_bytes()
{
  if (offset == 0)
    return ISRASN_SUCCESS;
  if (ptr >= max_ptr) {
...
template <typename Ptr>
ISRASN_CODE bit_ref_impl<Ptr>::advance_bits(uint32_t n_bits)
{
...
ISRASN_CODE bit_ref::pack_bytes(const uint8_t* buf, uint32_t n_bytes)
{
  if (n_bytes == 0) {
    return ISRASN_SUCCESS;
  }
...
ISRASN_CODE bit_ref::align_bytes_zero()
{
  if (offset == 0)
    return ISRASN_SUCCESS;
  if (ptr >= max_ptr) {
...
ISRASN_CODE pack_unsupported_ext_flag(bit_ref& bref, bool ext)
{
...
ISRASN_CODE unpack_unsupported_ext_flag(bool& ext, bit_ref& bref)
{
  ISRASN_CODE ret = bref.unpack(ext, 1);
  if (ext) {
    log_error("ASN extensions not currently supported");
    return ISRASN_ERROR_DECODE_FAIL;
  }
...
ISRASN_CODE pack_enum(bit_ref& bref, uint32_t enum_val, uint32_t nbits)
{
  return bref.pack(enum_val, nbits);
}

// Pack with extension
ISRASN_CODE pack_enum(bit_ref& bref, uint32_t enum_val, uint32_t nbits, uint32_t nof_noext)
{
...
ISRASN_CODE pack_enum(bit_ref& bref, uint32_t e, uint32_t nof_types, uint32_t nof_exts, bool has_ext)
{
  if (e >= nof_types) {
    log_error(
...
ValOrError unpack_enum(uint32_t nof_types, uint32_t nof_exts, bool has_ext, cbit_ref& bref)
{
  ...
  if (ret.val >= nof_types) {
    log_error("The provided enum is not within the range of possible values (%u>=%u)",
              (unsigned)ret.val,
              (unsigned)nof_types);
    ret.code = ISRASN_ERROR_DECODE_FAIL;
  }
  return ret;
}
...
ISRASN_CODE pack_constrained_whole_number(bit_ref& bref, IntType n, IntType lb, IntType ub, bool aligned)
{
...
template ISRASN_CODE pack_constrained_whole_number<int8_t>(bit_ref& bref, int8_t n, int8_t lb, int8_t ub, bool aligned);
template ISRASN_CODE
 ...
 */
template <class IntType>
ISRASN_CODE unpack_constrained_whole_number(IntType& n, cbit_ref& bref, IntType lb, IntType ub, bool aligned)
{
  if (ub < lb) {
...
ISRASN_CODE pack_norm_small_non_neg_whole_number(bit_ref& bref, UintType n)
{
  if (n <= 63) {
...
template <typename UintType>
ISRASN_CODE unpack_norm_small_non_neg_whole_number(UintType& n, cbit_ref& bref)
{
  bool        ext;
  ISRASN_CODE ret = bref.unpack(ext, 1);
  HANDLE_CODE(ret);
...
template ISRASN_CODE pack_norm_small_non_neg_whole_number<uint8_t>(bit_ref& bref, uint8_t n);
template ISRASN_CODE pack_norm_small_non_neg_whole_number<uint16_t>(bit_ref& bref, uint16_t n);
template ISRASN_CODE pack_norm_small_non_neg_whole_number<uint32_t>(bit_ref& bref, uint32_t n);
template ISRASN_CODE pack_norm_small_non_neg_whole_number<uint64_t>(bit_ref& bref, uint64_t n);
template ISRASN_CODE unpack_norm_small_non_neg_whole_number<uint8_t>(uint8_t& n, cbit_ref& bref);
template ISRASN_CODE unpack_norm_small_non_neg_whole_number<uint16_t>(uint16_t& n, cbit_ref& bref);
template ISRASN_CODE unpack_norm_small_non_neg_whole_number<uint32_t>(uint32_t& n, cbit_ref& bref);
template ISRASN_CODE unpack_norm_small_non_neg_whole_number<uint64_t>(uint64_t& n, cbit_ref& bref);
 ...
 */
template <typename IntType>
ISRASN_CODE pack_unconstrained_whole_number(bit_ref& bref, IntType n, bool aligned)
{
...
template <typename IntType>
ISRASN_CODE unpack_unconstrained_whole_number(IntType& n, cbit_ref& bref, uint32_t len, bool aligned)
{
  if (aligned) {
    HANDLE_CODE(bref.align_bytes());
  }
...
template ISRASN_CODE pack_unconstrained_whole_number<int8_t>(bit_ref& bref, int8_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<int16_t>(bit_ref& bref, int16_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<int32_t>(bit_ref& bref, int32_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<int64_t>(bit_ref& bref, int64_t n, bool aligned);
template ISRASN_CODE unpack_unconstrained_whole_number<int8_t>(int8_t& n, cbit_ref& bref, uint32_t len, bool aligned);
template ISRASN_CODE unpack_unconstrained_whole_number<int16_t>(int16_t& n, cbit_ref& bref, uint32_t len, bool aligned);
template ISRASN_CODE unpack_unconstrained_whole_number<int32_t>(int32_t& n, cbit_ref& bref, uint32_t len, bool aligned);
template ISRASN_CODE unpack_unconstrained_whole_number<int64_t>(int64_t& n, cbit_ref& bref, uint32_t len, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<uint8_t>(bit_ref& bref, uint8_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<uint16_t>(bit_ref& bref, uint16_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<uint32_t>(bit_ref& bref, uint32_t n, bool aligned);
template ISRASN_CODE pack_unconstrained_whole_number<uint64_t>(bit_ref& bref, uint64_t n, bool aligned);
template ISRASN_CODE unpack_unconstrained_whole_number<uint8_t>(uint8_t& n, cbit_ref& bref, uint32_t len, bool aligned);
template ISRASN_CODE
...
template <typename IntType>
ISRASN_CODE pack_length(bit_ref& bref, IntType n, IntType lb, IntType ub, bool aligned)
{
  if (ub >= ASN_64K) {
...
template ISRASN_CODE pack_length<uint8_t>(bit_ref& bref, uint8_t n, uint8_t lb, uint8_t ub, bool aligned);
template ISRASN_CODE pack_length<uint16_t>(bit_ref& bref, uint16_t n, uint16_t lb, uint16_t ub, bool aligned);
template ISRASN_CODE pack_length<uint32_t>(bit_ref& bref, uint32_t n, uint32_t lb, uint32_t ub, bool aligned);
template ISRASN_CODE pack_length<uint64_t>(bit_ref& bref, uint64_t n, uint64_t lb, uint64_t ub, bool aligned);
template ISRASN_CODE pack_length<int8_t>(bit_ref& bref, int8_t n, int8_t lb, int8_t ub, bool aligned);
template ISRASN_CODE pack_length<int16_t>(bit_ref& bref, int16_t n, int16_t lb, int16_t ub, bool aligned);
template ISRASN_CODE pack_length<int32_t>(bit_ref& bref, int32_t n, int32_t lb, int32_t ub, bool aligned);
template ISRASN_CODE pack_length<int64_t>(bit_ref& bref, int64_t n, int64_t lb, int64_t ub, bool aligned);

template <typename IntType>
ISRASN_CODE unpack_length(IntType& n, cbit_ref& bref, IntType lb, IntType ub, bool aligned)
{
  if (ub >= ASN_64K) {
    uint32_t    len;
    ISRASN_CODE ret = unpack_length(len, bref, aligned);
    n               = len;
    return ret;
  }
  return unpack_constrained_whole_number(n, bref, lb, ub, aligned);
}
template ISRASN_CODE unpack_length<uint8_t>(uint8_t& n, cbit_ref& bref, uint8_t lb, uint8_t ub, bool aligned);
template ISRASN_CODE unpack_length<uint16_t>(uint16_t& n, cbit_ref& bref, uint16_t lb, uint16_t ub, bool aligned);
template ISRASN_CODE unpack_length<uint32_t>(uint32_t& n, cbit_ref& bref, uint32_t lb, uint32_t ub, bool aligned);
template ISRASN_CODE unpack_length<uint64_t>(uint64_t& n, cbit_ref& bref, uint64_t lb, uint64_t ub, bool aligned);
template ISRASN_CODE unpack_length<int8_t>(int8_t& n, cbit_ref& bref, int8_t lb, int8_t ub, bool aligned);
template ISRASN_CODE unpack_length<int16_t>(int16_t& n, cbit_ref& bref, int16_t lb, int16_t ub, bool aligned);
template ISRASN_CODE unpack_length<int32_t>(int32_t& n, cbit_ref& bref, int32_t lb, int32_t ub, bool aligned);
template ISRASN_CODE unpack_length<int64_t>(int64_t& n, cbit_ref& bref, int64_t lb, int64_t ub, bool aligned);
 ...
 * @return
 */
ISRASN_CODE pack_length(bit_ref& bref, uint32_t val, bool aligned)
{
  if (not aligned) {
    if (val < 128) {
...
ISRASN_CODE unpack_length(uint32_t& val, cbit_ref& bref, bool aligned)
{
  bool ext;
  if (not aligned) {
    ISRASN_CODE ret = bref.unpack(ext, 1);
...
template <typename IntType>
ISRASN_CODE pack_integer(bit_ref& bref, IntType n, IntType lb, IntType ub, bool has_ext, bool aligned)
{
  // pack ext marker
...
template ISRASN_CODE pack_integer<int8_t>(bit_ref& bref, int8_t n, int8_t lb, int8_t ub, bool has_ext, bool aligned);
template ISRASN_CODE
...
template <typename IntType>
ISRASN_CODE pack_unconstrained_integer(bit_ref& bref, IntType n, bool has_ext, bool aligned)
{
  return pack_integer(
      bref, n, std::numeric_limits<IntType>::min(), std::numeric_limits<IntType>::max(), has_ext, aligned);
}
template ISRASN_CODE pack_unconstrained_integer<int8_t>(bit_ref& bref, int8_t n, bool has_ext, bool aligned);
template ISRASN_CODE pack_unconstrained_integer<int16_t>(bit_ref& bref, int16_t n, bool has_ext, bool aligned);
template ISRASN_CODE pack_unconstrained_integer<int32_t>(bit_ref& bref, int32_t n, bool has_ext, bool aligned);
template ISRASN_CODE pack_unconstrained_integer<int64_t>(bit_ref& bref, int64_t n, bool has_ext, bool aligned);

template <typename IntType>
ISRASN_CODE unpack_integer(IntType& n, cbit_ref& bref, IntType lb, IntType ub, bool has_ext, bool aligned)
{
...
// unconstrained specialization case
template <typename IntType>
ISRASN_CODE unpack_unconstrained_integer(IntType& n, cbit_ref& bref, bool has_ext, bool aligned)
{
  return unpack_integer(
      n, bref, std::numeric_limits<IntType>::min(), std::numeric_limits<IntType>::max(), has_ext, aligned);
}
template ISRASN_CODE unpack_unconstrained_integer<int8_t>(int8_t& n, cbit_ref& bref, bool has_ext, bool aligned);
template ISRASN_CODE unpack_unconstrained_integer<int16_t>(int16_t& n, cbit_ref& bref, bool has_ext, bool aligned);
template ISRASN_CODE unpack_unconstrained_integer<int32_t>(int32_t& n, cbit_ref& bref, bool has_ext, bool aligned);
template ISRASN_CODE unpack_unconstrained_integer<int64_t>(int64_t& n, cbit_ref& bref, bool has_ext, bool aligned);
...
template <class IntType>
ISRASN_CODE integer_packer<IntType>::pack(bit_ref& bref, IntType n)
{
  return pack_integer(bref, n, lb, ub, has_ext, aligned);
}
template <class IntType>
ISRASN_CODE integer_packer<IntType>::unpack(IntType& n, cbit_ref& bref)
{
...
template <bool Al>
ISRASN_CODE unbounded_octstring<Al>::pack(bit_ref& bref) const
{
...
template <bool Al>
ISRASN_CODE unbounded_octstring<Al>::unpack(cbit_ref& bref)
{
  uint32_t len;
  HANDLE_CODE(unpack_length(len, bref, aligned));
  resize(len);
 ...
 */
ISRASN_CODE pack_length_prefix(bit_ref& bref,
                               uint32_t len,
                               uint32_t lb         = 0,
...
ISRASN_CODE pack_bitfield(bit_ref& bref, const uint8_t* buf, uint32_t nbits, uint32_t lb, uint32_t ub, bool is_aligned)
{
  if (nbits == 0) {
 ...
 */
ISRASN_CODE unpack_length_prefix(uint32_t& len, cbit_ref& bref, uint32_t lb, uint32_t ub, bool has_ext, bool is_aligned)
{
  bool ext = false;
  if (has_ext) {
...
ISRASN_CODE unpack_bitfield(uint8_t* buf, cbit_ref& bref, uint32_t n, uint32_t lb, uint32_t ub, bool is_aligned)
{
  if (n > ASN_64K) {
...
ISRASN_CODE unpack(std::string& s, cbit_ref& bref, size_t lb, size_t ub, size_t alb, size_t aub, bool ext, bool aligned)
{
  size_t b              = asn_string_utils::get_nof_bits_per_char(lb, ub, aligned);
  bool   octet_aligned  = asn_string_utils::is_octet_aligned(b, alb, aub, aligned);
  bool   length_encoded = asn_string_utils::is_length_encoded(alb, aub, aligned);
...
ISRASN_CODE ext_groups_packer_guard::pack(asn1::bit_ref& bref) const
{
...
ISRASN_CODE ext_groups_unpacker_guard::unpack(cbit_ref& bref)
{
  bref_tracker = &bref;
...
varlength_field_pack_guard::varlength_field_pack_guard(bit_ref& bref, bool align_) :
  buffer_ptr(isrran::make_buffer_pool_obj<byte_array_t>())
{
...
ISRASN_CODE detail::empty_obj_set_item_c::pack(bit_ref& bref) const
{
  varlength_field_pack_guard varlen_scope(bref, true);
  return ISRASN_SUCCESS;
}
ISRASN_CODE detail::empty_obj_set_item_c::unpack(cbit_ref& bref)
{
