...
#include "isrran/common/s3g.h"
#include "isrran/common/ssl.h"
                                 ...
                                 uint8_t*       k_asme)
{
  ...
  uint8_t output[32];
  if (kdf_common(FC_EPS_K_ASME_DERIVATION, key, sn_id, ak_xor_sqn, output) != ISRRAN_SUCCESS) {
                                 ...
                                 uint8_t*       k_ausf)
{
  ...
  uint8_t output[32];
  if (kdf_common(FC_5G_KAUSF_DERIVATION, key, ssn, ak_xor_sqn, output) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
...
uint8_t security_generate_k_seaf(const uint8_t* k_ausf, const char* serving_network_name, uint8_t* k_seaf)
{
  ...
  if (kdf_common(FC_5G_KSEAF_DERIVATION, key, ssn, k_seaf) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
  return ISRRAN_SUCCESS;
                                ...
                                uint8_t*       k_amf)
{
  ...
  if (kdf_common(FC_5G_KAMF_DERIVATION, key, supi, abba, k_amf) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
  return ISRRAN_SUCCESS;
...
uint8_t security_generate_k_enb(const uint8_t* k_asme, const uint32_t nas_count_, uint8_t* k_enb)
{
  ...
  if (kdf_common(FC_EPS_K_ENB_DERIVATION, key, nas_count, k_enb) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
  return ISRRAN_SUCCESS;
                                           ...
                                           uint8_t*       k_enb_star)
{
  ...
  if (kdf_common(fc, key, pci, earfcn, k_enb_star) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
  return ISRRAN_SUCCESS;
...
uint8_t security_generate_nh(const uint8_t* k_asme, const uint8_t* sync_, uint8_t* nh)
{
  ...
  if (kdf_common(FC_EPS_NH_DERIVATION, key, sync, nh) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
  return ISRRAN_SUCCESS;
...
uint8_t security_generate_sk_gnb(const uint8_t* k_enb, const uint16_t scg_count_, uint8_t* sk_gnb)
{
  ...
  // Derive sk_gnb
  uint8_t output[32];
  if (kdf_common(0x1C, key, scg_count, output) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
                                   ...
                                   uint8_t*       res_star)
{
  ...
  uint8_t output[32];
  if (kdf_common(FC_5G_RES_STAR_DERIVATION, key, ssn, rand, res, output) != ISRRAN_SUCCESS) {
    log_error("Failed to run kdf_common");
    return ISRRAN_ERROR;
  }
