...
#include "isrran/common/phy_cfg_nr_default.h"
...
phy_cfg_nr_default_t::reference_cfg_t::reference_cfg_t(const std::string& args)
{
  ...
  for (std::string& e : param_list) {
    ...
    if (param.size() != 2) {
      isrran_assertion_failure("Invalid reference argument '%s'", e.c_str());
    }
    ...
    if (param.front() == "carrier") {
      ...
      isrran_assert(carrier != R_CARRIER_COUNT, "Invalid carrier reference configuration '%s'", param.back().c_str());
      ...
      isrran_assert(duplex != R_DUPLEX_COUNT, "Invalid duplex reference configuration '%s'", param.back().c_str());
      ...
      isrran_assert(pdsch != R_PDSCH_COUNT, "Invalid PDSCH reference configuration '%s'", param.back().c_str());
    } else {
      isrran_assertion_failure("Invalid %s reference component", param.front().c_str());
    }
  }
}

void phy_cfg_nr_default_t::make_carrier_custom_10MHz(isrran_carrier_nr_t& carrier)
{
  carrier.nof_prb                = 52;
  ...
  carrier.scs                    = isrran_subcarrier_spacing_15kHz;
}

void phy_cfg_nr_default_t::make_carrier_custom_20MHz(isrran_carrier_nr_t& carrier)
{
  carrier.nof_prb                = 106;
  carrier.max_mimo_layers        = 1;
  carrier.pci                    = 500;
...
void phy_cfg_nr_default_t::make_tdd_custom_6_4(isrran_duplex_config_nr_t& conf)
{
  // Set the duplex mode to TDD
  conf.mode = ISRRAN_DUPLEX_MODE_TDD;

  // Select TDD config
  isrran_tdd_config_nr_t& tdd = conf.tdd;
...
void phy_cfg_nr_default_t::make_tdd_fr1_15_1(isrran_duplex_config_nr_t& conf)
{ // Set the duplex mode to TDD
  conf.mode = ISRRAN_DUPLEX_MODE_TDD;
...
void phy_cfg_nr_default_t::make_pdcch_custom_common_ss(isrran_pdcch_cfg_nr_t& pdcch, const isrran_carrier_nr_t& carrier)
{
  // Configure CORESET ID 1
  pdcch.coreset_present[1]              = true;
  pdcch.coreset[1].id                   = 1;
  pdcch.coreset[1].duration             = 1;
  pdcch.coreset[1].mapping_type         = isrran_coreset_mapping_type_non_interleaved;
  pdcch.coreset[1].precoder_granularity = isrran_coreset_precoder_granularity_reg_bundle;

  // Generate frequency resources for the full BW
  for (uint32_t i = 0; i < ISRRAN_CORESET_FREQ_DOMAIN_RES_SIZE; i++) {
    pdcch.coreset[1].freq_resources[i] = i < ISRRAN_FLOOR(carrier.nof_prb, 6);
  }
  ...
  pdcch.search_space[1].formats[0]  = isrran_dci_format_nr_0_0; // DCI format for PUSCH
  pdcch.search_space[1].formats[1]  = isrran_dci_format_nr_1_0; // DCI format for PDSCH
  pdcch.search_space[1].nof_formats = 2;
  pdcch.search_space[1].type        = isrran_search_space_type_common_3;
  ...
  for (uint32_t L = 0; L < ISRRAN_SEARCH_SPACE_NOF_AGGREGATION_LEVELS_NR; L++) {
    pdcch.search_space[1].nof_candidates[L] =
        ISRRAN_MIN(2, isrran_pdcch_nr_max_candidates_coreset(&pdcch.coreset[1], L));
  }
...
void phy_cfg_nr_default_t::make_pdsch_default(isrran_sch_hl_cfg_nr_t& pdsch)
{
  // Select PDSCH time resource allocation
  pdsch.common_time_ra[0].k            = 0;
  pdsch.common_time_ra[0].mapping_type = isrran_sch_mapping_type_A;
  pdsch.common_time_ra[0].sliv = isrran_ra_type2_to_riv(ISRRAN_NSYMB_PER_SLOT_NR - 1, 1, ISRRAN_NSYMB_PER_SLOT_NR);
  pdsch.nof_common_time_ra     = 1;

  // Set contiguous PRBs as default
  pdsch.alloc = isrran_resource_alloc_type1;

  // Setup PDSCH DMRS type A position
  pdsch.typeA_pos = isrran_dmrs_sch_typeA_pos_2;
}

void make_nzp_csi_rs_ts38101_table_5_2_1(const isrran_carrier_nr_t& carrier, isrran_csi_rs_nzp_set_t& trs)
{
  // Set defaults
  trs = {};
  ...
  isrran_csi_rs_nzp_resource_t& res1 = trs.data[0];
  isrran_csi_rs_nzp_resource_t& res2 = trs.data[1];
  isrran_csi_rs_nzp_resource_t& res3 = trs.data[2];
  isrran_csi_rs_nzp_resource_t& res4 = trs.data[3];
  ...
  res1.resource_mapping.cdm = isrran_csi_rs_cdm_nocdm;
  res2.resource_mapping.cdm = isrran_csi_rs_cdm_nocdm;
  res3.resource_mapping.cdm = isrran_csi_rs_cdm_nocdm;
  res4.resource_mapping.cdm = isrran_csi_rs_cdm_nocdm;

  res1.resource_mapping.density = isrran_csi_rs_resource_mapping_density_three;
  res2.resource_mapping.density = isrran_csi_rs_resource_mapping_density_three;
  res3.resource_mapping.density = isrran_csi_rs_resource_mapping_density_three;
  res4.resource_mapping.density = isrran_csi_rs_resource_mapping_density_three;

  if (carrier.scs == isrran_subcarrier_spacing_15kHz) {
  ...
  } else if (carrier.scs == isrran_subcarrier_spacing_30kHz) {
    ...
    isrran_assertion_failure("Invalid subcarrier spacing %d kHz", 15U << (uint32_t)carrier.scs);
  }
...
void phy_cfg_nr_default_t::make_pdsch_2_1_1_tdd(const isrran_carrier_nr_t& carrier, isrran_sch_hl_cfg_nr_t& pdsch)
{
  // Select PDSCH time resource allocation
  pdsch.common_time_ra[0].mapping_type = isrran_sch_mapping_type_A;
  pdsch.common_time_ra[0].k            = 0;
  pdsch.common_time_ra[0].sliv = isrran_ra_type2_to_riv(ISRRAN_NSYMB_PER_SLOT_NR - 2, 2, ISRRAN_NSYMB_PER_SLOT_NR);
  pdsch.nof_common_time_ra     = 1;

  // Setup PDSCH DMRS
  pdsch.typeA_pos                 = isrran_dmrs_sch_typeA_pos_2;
  pdsch.dmrs_typeA.present        = true;
  pdsch.dmrs_typeA.additional_pos = isrran_dmrs_sch_add_pos_2;
...
void phy_cfg_nr_default_t::make_pusch_default(isrran_sch_hl_cfg_nr_t& pusch)
{
  // Select PUSCH time resource allocation
  pusch.common_time_ra[0].k            = 4;
  pusch.common_time_ra[0].mapping_type = isrran_sch_mapping_type_A;
  pusch.common_time_ra[0].sliv         = isrran_ra_type2_to_riv(ISRRAN_NSYMB_PER_SLOT_NR, 0, ISRRAN_NSYMB_PER_SLOT_NR);
  pusch.nof_common_time_ra             = 1;

  // Setup PUSCH DMRS type A position
  pusch.typeA_pos = isrran_dmrs_sch_typeA_pos_2;
...
void phy_cfg_nr_default_t::make_pucch_custom_one(isrran_pucch_nr_hl_cfg_t& pucch)
{
  // PUCCH Resource for format 1
  isrran_pucch_nr_resource_t resource_small = {};
  resource_small.starting_prb               = 0;
  resource_small.format                     = ISRRAN_PUCCH_NR_FORMAT_1;
  ...
  isrran_pucch_nr_resource_t resource_big = {};
  resource_big.starting_prb               = 51;
  resource_big.format                     = ISRRAN_PUCCH_NR_FORMAT_2;
  resource_big.nof_prb                    = 1;
  resource_big.nof_symbols                = 2;
  resource_big.start_symbol_idx           = 12;

  // Resource for SR
  isrran_pucch_nr_resource_t resource_sr = {};
  resource_sr.starting_prb               = 51;
  resource_sr.format                     = ISRRAN_PUCCH_NR_FORMAT_1;
...
void phy_cfg_nr_default_t::make_harq_auto(isrran_harq_ack_cfg_hl_t&        harq,
                                          const isrran_carrier_nr_t&       carrier,
                                          const isrran_duplex_config_nr_t& duplex_cfg)
{
  if (duplex_cfg.mode == ISRRAN_DUPLEX_MODE_TDD) {
    const isrran_tdd_config_nr_t& tdd_cfg = duplex_cfg.tdd;

    // Generate as many entries as DL slots
    harq.nof_dl_data_to_ul_ack = ISRRAN_MIN(tdd_cfg.pattern1.nof_dl_slots, ISRRAN_MAX_NOF_DL_DATA_TO_UL);
    ...
    for (uint32_t n = 0; n < harq.nof_dl_data_to_ul_ack; n++) {
      ...
      if (isrran_duplex_nr_is_ul(&duplex_cfg, carrier.scs, n + 4)) {
  ...
  // Zero the rest
  for (uint32_t i = harq.nof_dl_data_to_ul_ack; i < ISRRAN_MAX_NOF_DL_DATA_TO_UL; i++) {
    harq.dl_data_to_ul_ack[i] = 0;
  }

  // Select dynamic HARQ-ACK codebook
  harq.harq_ack_codebook = isrran_pdsch_harq_ack_codebook_dynamic;
}

void phy_cfg_nr_default_t::make_prach_default_lte(isrran_prach_cfg_t& prach)
{
  prach.is_nr            = true;
  prach.config_idx       = 8;
...
phy_cfg_nr_default_t::phy_cfg_nr_default_t(const reference_cfg_t& reference_cfg)
{
  switch (reference_cfg.carrier) {
    ...
    case reference_cfg_t::R_CARRIER_COUNT:
      isrran_assertion_failure("Invalid carrier reference");
  }

  switch (reference_cfg.duplex) {
    case reference_cfg_t::R_DUPLEX_FDD:
      duplex.mode = ISRRAN_DUPLEX_MODE_FDD;
      break;
    ...
    case reference_cfg_t::R_DUPLEX_COUNT:
      isrran_assertion_failure("Invalid TDD reference");
  }

  if (duplex.mode == ISRRAN_DUPLEX_MODE_TDD) {
  ...
  switch (reference_cfg.pdsch) {
    ...
    case reference_cfg_t::R_PDSCH_COUNT:
      isrran_assertion_failure("Invalid PDSCH reference configuration");
  }
  ...
  prach.tdd_config.configured = (duplex.mode == ISRRAN_DUPLEX_MODE_TDD);

  // Make default CSI report configuration always
  csi.reports[0].channel_meas_id                    = 0;
  csi.reports[0].type                               = ISRRAN_CSI_REPORT_TYPE_PERIODIC;
  csi.reports[0].periodic.period                    = 20;
  csi.reports[0].periodic.offset                    = 9;
  csi.reports[0].periodic.resource.format           = ISRRAN_PUCCH_NR_FORMAT_2;
  ...
  csi.reports[0].quantity                           = ISRRAN_CSI_REPORT_QUANTITY_CRI_RI_PMI_CQI;
  csi.reports[0].cqi_table                          = ISRRAN_CSI_CQI_TABLE_1;
  csi.reports[0].freq_cfg                           = ISRRAN_CSI_REPORT_FREQ_WIDEBAND;
  csi.csi_resources[0].type = isrran_csi_hl_resource_cfg_t::ISRRAN_CSI_HL_RESOURCE_CFG_TYPE_NZP_CSI_RS_SSB;
