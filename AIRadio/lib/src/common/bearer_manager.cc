...
const ue_bearer_manager_impl::radio_bearer_t ue_bearer_manager_impl::invalid_rb{isrran::isrran_rat_t::nulltype, 0, 0};

bool ue_bearer_manager_impl::add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid)
{
...
ue_bearer_manager::ue_bearer_manager() : logger(isrlog::fetch_basic_logger("STCK", false))
{
...
void ue_bearer_manager::add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid)
{
...
enb_bearer_manager::enb_bearer_manager() : logger(isrlog::fetch_basic_logger("STCK", false)) {}

enb_bearer_manager::~enb_bearer_manager() {}

void enb_bearer_manager::add_eps_bearer(uint16_t rnti, uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid)
{
  auto user_it = users_map.find(rnti);
  if (user_it == users_map.end()) {
    // add empty bearer map
    // users_map.emplace( )   returns pair<iterator,bool>
    auto p = users_map.emplace(rnti, isrran::detail::ue_bearer_manager_impl{});
    if (!p.second) {
...
enb_bearer_manager::radio_bearer_t enb_bearer_manager::get_lcid_bearer(uint16_t rnti, uint32_t lcid) const
{
  auto user_it = users_map.find(rnti);
  if (user_it == users_map.end()) {
    return isrran::detail::ue_bearer_manager_impl::invalid_rb;
  }
