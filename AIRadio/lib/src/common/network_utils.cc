...
int open_socket(net_utils::addr_family ip_type, net_utils::socket_type socket_type, net_utils::protocol_type protocol)
{
  ...
  if (fd == -1) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Failed to open %s socket.", net_utils::protocol_to_string(protocol));
    perror("Could not create socket\n");
    return -1;
  }
  isrlog::fetch_basic_logger(LOGSERVICE).debug("Opened %s socket=%d", net_utils::protocol_to_string(protocol), fd);
...
{
  if (fd < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Trying to bind to a closed socket");
    return false;
  }

  if (bind(fd, (struct sockaddr*)&addr_in, sizeof(addr_in)) != 0) {
    isrlog::fetch_basic_logger(LOGSERVICE)
...
{
  if (fd < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("tried to connect to remote address with an invalid socket.");
    return false;
  }
  ...
  if (connect(fd, (const struct sockaddr*)&sockaddr_tmp, sizeof(sockaddr_tmp)) == -1) {
    isrlog::fetch_basic_logger(LOGSERVICE).info("Failed to establish socket connection to %s", dest_addr_str);
    perror("connect()");
    return false;
  }
  return true;
...
{
  if (fd < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Tried to listen for connections with an invalid socket.");
    return false;
  }

  // Listen for connections
  if (listen(fd, SOMAXCONN) != 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Failed to listen to incoming SCTP connections");
    perror("listen()");
    return false;
  }
  return true;
...
{
  if (fd < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Trying reuse_addr a closed socket. Socket=%d", fd);
    return false;
  }

  int enable = 1;
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Failed to set SO_REUSEADDR. Socket=%d", fd);
    return false;
  }
  isrlog::fetch_basic_logger(LOGSERVICE).debug("Successfully set SO_REUSEADDR. Socket=%d", fd);
  return true;
}
...
{
  if (fd < 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Trying subscribe to SCTP events on a closed socket. Socket=%d", fd);
    return false;
  }
  ...
  if (setsockopt(fd, IPPROTO_SCTP, SCTP_EVENTS, &evnts, sizeof(evnts)) != 0) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Failed to subscribe to SCTP_SHUTDOWN event: %s", strerror(errno));
...
{
  if (is_open()) {
    isrlog::fetch_basic_logger(LOGSERVICE).error("Socket is already open.");
    return false;
  }
...
void unique_socket::close()
{
  if (sockfd >= 0) {
    if (::close(sockfd) == -1) {
      isrlog::fetch_basic_logger(LOGSERVICE).error("Socket=%d could not be closed.", sockfd);
    } else {
      isrlog::fetch_basic_logger(LOGSERVICE).debug("Socket=%d was closed.", sockfd);
    }
    sockfd = -1;
    addr   = {};
  } else {
    isrlog::fetch_basic_logger(LOGSERVICE).debug("Socket=%d could not be closed.", sockfd);
  }
}
...
socket_manager::socket_manager() : thread("RXsockets"), socket_manager_itf(isrlog::fetch_basic_logger("COMN"))
{
  // register control pipe fd
  int fd = pipe(pipefd);
  isrran_assert(fd != -1, "Failed to open control pipe");
  start(thread_prio);
}
...
public:
  ...
  explicit sctp_recvmsg_pdu_task(isrlog::basic_logger& logger, isrran::task_queue_handle& queue_, callback_t func_) :
  ...
  bool operator()(int fd)
  {
    ...
    queue.push(std::bind(
        [this, from, sri, flags](isrran::unique_byte_buffer_t& sdu) { func(std::move(sdu), from, sri, flags); },
        std::move(pdu)));
    return ret;
  }
...
private:
  ...
  isrran::task_queue_handle& queue;
  callback_t                 func;
};

socket_manager_itf::recv_callback_t
make_sctp_sdu_handler(isrlog::basic_logger& logger, isrran::task_queue_handle& queue, sctp_recv_callback_t rx_callback)
{
...
public:
  ...
  explicit recvfrom_pdu_task(isrlog::basic_logger& logger, isrran::task_queue_handle& queue_, callback_t func_) :
    logger(logger), queue(queue_), func(std::move(func_))
  {}
  ...
  bool operator()(int fd)
  {
    ...
    // Defer handling of received packet to provided queue
    queue.push(
        std::bind([this, from](isrran::unique_byte_buffer_t& sdu) { func(std::move(sdu), from); }, std::move(pdu)));
...
socket_manager_itf::recv_callback_t
make_sdu_handler(isrlog::basic_logger& logger, isrran::task_queue_handle& queue, recvfrom_callback_t rx_callback)
{
