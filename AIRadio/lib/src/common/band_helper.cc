...
constexpr std::array<isrran_band_helper::nr_band, isrran_band_helper::nof_nr_bands_fr1>
          isrran_band_helper::nr_band_table_fr1;
constexpr std::array<isrran_band_helper::nr_raster_params, 3> isrran_band_helper::nr_fr_params;
constexpr std::array<isrran_band_helper::nr_operating_band, isrran_band_helper::nof_nr_operating_band_fr1>
          isrran_band_helper::nr_operating_bands_fr1;
constexpr std::array<isrran_band_helper::nr_band_ss_raster, isrran_band_helper::nof_nr_band_ss_raster>
          isrran_band_helper::nr_band_ss_raster_table;

// Formula in 5.4.2.1
double isrran_band_helper::nr_arfcn_to_freq(uint32_t nr_arfcn)
{
...
uint32_t isrran_band_helper::freq_to_nr_arfcn(double freq)
{
...
std::vector<uint32_t> isrran_band_helper::get_bands_nr(uint32_t                             nr_arfcn,
                                                       isrran_band_helper::delta_f_raster_t delta_f_raster)
{
...
uint16_t isrran_band_helper::get_band_from_dl_freq_Hz(double freq) const
{
...
uint16_t isrran_band_helper::get_band_from_dl_arfcn(uint32_t arfcn) const
{
...
uint32_t isrran_band_helper::get_ul_arfcn_from_dl_arfcn(uint32_t dl_arfcn) const
{
  // return same ARFCN for TDD bands
  if (get_duplex_mode(get_band_from_dl_arfcn(dl_arfcn)) == ISRRAN_DUPLEX_MODE_TDD) {
    return dl_arfcn;
  }
...
double isrran_band_helper::get_center_freq_from_abs_freq_point_a(uint32_t nof_prb, uint32_t freq_point_a_arfcn)
{
  ...
  return abs_freq_point_a_freq +
         (nof_prb / 2 * ISRRAN_SUBC_SPACING_NR(isrran_subcarrier_spacing_t::isrran_subcarrier_spacing_15kHz) *
          ISRRAN_NRE);
}

uint32_t isrran_band_helper::get_abs_freq_point_a_arfcn(uint32_t nof_prb, uint32_t arfcn)
{
  return freq_to_nr_arfcn(get_abs_freq_point_a_from_center_freq(nof_prb, nr_arfcn_to_freq(arfcn)));
}

double isrran_band_helper::get_abs_freq_point_a_from_center_freq(uint32_t nof_prb, double center_freq)
{
  // for FR1 unit of resources blocks for freq calc is always 180kHz regardless for actual SCS of carrier
  // TODO: add offset_to_carrier
  return center_freq -
...
uint32_t isrran_band_helper::find_lower_bound_abs_freq_ssb(uint16_t                    band,
                                                           ...
                                                           uint32_t                    min_center_freq_hz)
{
  ...
  double ssb_bw_hz = ISRRAN_SSB_BW_SUBC * ISRRAN_SUBC_SPACING_NR(scs);
  while (!sync_raster.end()) {
    ...
    if ((abs_freq_ssb_hz > min_center_freq_hz + ssb_bw_hz / 2) and
        ((uint32_t)std::round(abs_freq_ssb_hz - min_center_freq_hz) % ISRRAN_SUBC_SPACING_NR(scs) == 0)) {
...
uint32_t isrran_band_helper::get_abs_freq_ssb_arfcn(uint16_t                    band,
                                                    isrran_subcarrier_spacing_t scs,
                                                    uint32_t                    freq_point_a_arfcn,
                                                    uint32_t                    coreset0_offset_rb)
{
  double freq_point_a_hz    = nr_arfcn_to_freq(freq_point_a_arfcn);
  double coreset0_offset_hz = coreset0_offset_rb * ISRRAN_NRE * ISRRAN_SUBC_SPACING_NR(scs);
  return find_lower_bound_abs_freq_ssb(band, scs, freq_point_a_hz + coreset0_offset_hz);
}

isrran_ssb_pattern_t isrran_band_helper::get_ssb_pattern(uint16_t band, isrran_subcarrier_spacing_t scs)
{
...
isrran_subcarrier_spacing_t isrran_band_helper::get_ssb_scs(uint16_t band) const
{
  // Look for the given band and SCS
  for (const nr_band_ss_raster& ss_raster : nr_band_ss_raster_table) {
    // Check if band and SCS match!
...
isrran_duplex_mode_t isrran_band_helper::get_duplex_mode(uint16_t band) const
{
  ...
  for (const nr_operating_band& b : nr_operating_bands_fr1) {
    ...
    // As bands are in ascending order, do not waste more time if the current band is bigger
    if (b.band > band) {
      return ISRRAN_DUPLEX_MODE_INVALID;
    }
  }
...
struct sync_raster_impl : public isrran_band_helper::sync_raster_t {
public:
...
double isrran_band_helper::sync_raster_t::get_frequency() const
{
...
uint32_t isrran_band_helper::sync_raster_t::get_gscn() const
{
  if (gscn_last <= 7498) {
...
isrran_band_helper::sync_raster_t isrran_band_helper::get_sync_raster(uint16_t                    band,
                                                                      isrran_subcarrier_spacing_t scs) const
{
  // Look for the given band and SCS
  for (const nr_band_ss_raster& ss_raster : nr_band_ss_raster_table) {
    // Check if band and SCS match!
...
isrran_band_helper::nr_raster_params isrran_band_helper::get_raster_params(uint32_t nr_arfcn)
{
...
isrran_band_helper::nr_raster_params isrran_band_helper::get_raster_params(double freq)
{
  for (auto& fr : nr_fr_params) {
...
bool isrran_band_helper::is_valid_raster_param(const isrran_band_helper::nr_raster_params& raster)
{
  for (auto& fr : nr_fr_params) {
    if (fr == raster) {
      return true;
