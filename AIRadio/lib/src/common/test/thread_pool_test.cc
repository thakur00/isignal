...
public:
  explicit dummy_radio(isrlog::basic_logger& logger) : logger(logger) { logger.info("Dummy radio created"); }
...
class dummy_worker : public isrran::thread_pool::worker
{
private:
  ...
  isrlog::basic_logger&            logger;
  isrran::tti_semaphore<uint32_t>* tti_semaphore = nullptr;
...
public:
  dummy_worker(uint32_t                         id,
               isrran::tti_semaphore<uint32_t>* tti_semaphore_,
               isrlog::basic_logger&            logger,
               dummy_radio*                     radio_) :
    logger(logger)
  {
    ...
    radio         = radio_;
    random_gen    = isrran_random_init(id);
    logger.info("Dummy worker created");
  }

  ~dummy_worker() { isrran_random_free(random_gen); }
...
protected:
  void work_imp() override
  {
    ...
    int sleep_time_us = isrran_random_uniform_int_dist(random_gen, sleep_time_min_us, sleep_time_max_us);
...
int main(int argc, char** argv)
{
  ...
  isrran_random_t random_gen         = isrran_random_init(1234);

  // Pools and workers
  isrran::thread_pool                         pool(nof_workers);
  std::vector<std::unique_ptr<dummy_worker> > workers;
  isrran::tti_semaphore<uint32_t>             tti_semaphore;

  // Loggers.
  auto& radio_logger = isrlog::fetch_basic_logger("radio", false);
  radio_logger.set_level(isrlog::basic_levels::none);
  ...
  for (uint32_t i = 0; i < nof_workers; i++) {
    ...
    auto& logger = isrlog::fetch_basic_logger(log_name, false);
  ...
  for (uint32_t tti = 0; tti < nof_tti && !radio.is_late(); tti++) {
    if (enable_probability > isrran_random_uniform_real_dist(random_gen, 0.0f, 1.0f)) {
      // Wait worker
