...
int bands_test_nr()
{
  ...
  // n3
  TESTASSERT(bands.get_duplex_mode(3) == ISRRAN_DUPLEX_MODE_FDD);
  ...
  TESTASSERT(bands.get_abs_freq_ssb_arfcn(3, isrran_subcarrier_spacing_15kHz, 367564) > 367924);
  TESTASSERT_EQ(368410, bands.get_abs_freq_ssb_arfcn(3, isrran_subcarrier_spacing_15kHz, 367564, 12));
  // n5
  TESTASSERT(bands.get_duplex_mode(5) == ISRRAN_DUPLEX_MODE_FDD);
  ...
  TESTASSERT(bands.get_abs_freq_ssb_arfcn(5, isrran_subcarrier_spacing_15kHz, 175364) > 175724);
  // n7 n38
  TESTASSERT(bands.get_duplex_mode(7) == ISRRAN_DUPLEX_MODE_FDD);
  ...
  TESTASSERT(bands.get_abs_freq_ssb_arfcn(7, isrran_subcarrier_spacing_15kHz, 528984) > 529344);

  // n78
  TESTASSERT(bands.get_duplex_mode(78) == ISRRAN_DUPLEX_MODE_TDD);
  TESTASSERT(bands.nr_arfcn_to_freq(634240) == 3513.6e6);
  TESTASSERT(bands.get_abs_freq_point_a_arfcn(52, 634240) == 633928);
  TESTASSERT(bands.get_abs_freq_ssb_arfcn(78, isrran_subcarrier_spacing_30kHz, 633928) > 634168);
  ...
  band_vector = bands.get_bands_nr(620001, isrran::isrran_band_helper::KHZ_30);
...
{
  TESTASSERT(bands_test_nr() == ISRRAN_SUCCESS);
