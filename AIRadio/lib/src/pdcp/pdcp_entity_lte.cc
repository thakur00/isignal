...
pdcp_entity_lte::pdcp_entity_lte(isrue::rlc_interface_pdcp* rlc_,
                                 isrue::rrc_interface_pdcp* rrc_,
                                 isrue::gw_interface_pdcp*  gw_,
...
bool pdcp_entity_lte::configure(const pdcp_config_t& cnfg_)
{
  ...
  if (!check_valid_config()) {
    isrran::console("Warning: Invalid PDCP config.\n");
    return false;
  }
  active = true;
  return true;
...
void pdcp_entity_lte::write_sdu(unique_byte_buffer_t sdu, int upper_sn)
{
  ...
  logger.info(sdu->msg,
              ...
              rb_name.c_str(),
              used_sn,
              isrran_direction_text[integrity_direction],
              isrran_direction_text[encryption_direction]);
 ...
 ***************************************************************************/
// SRBs (5.1.2.2)
void pdcp_entity_lte::handle_srb_pdu(isrran::unique_byte_buffer_t pdu)
{
  // Read SN from header
  uint32_t sn = read_data_header(pdu);
...
void pdcp_entity_lte::handle_um_drb_pdu(isrran::unique_byte_buffer_t pdu)
{
  uint32_t sn = read_data_header(pdu);
  discard_data_header(pdu);
...
void pdcp_entity_lte::handle_am_drb_pdu(isrran::unique_byte_buffer_t pdu)
{
  uint32_t sn = read_data_header(pdu);
  discard_data_header(pdu);
...
std::map<uint32_t, isrran::unique_byte_buffer_t> pdcp_entity_lte::get_buffered_pdus()
{
  if (undelivered_sdus == nullptr) {
    logger.error("Buffered PDUs being requested for non-AM DRB");
    return std::map<uint32_t, isrran::unique_byte_buffer_t>{};
  }
 ...
 ***************************************************************************/
undelivered_sdus_queue::undelivered_sdus_queue(isrran::task_sched_handle task_sched, uint32_t sn_mod) : sn_mod(sn_mod)
{
  for (auto& e : sdus) {
...
bool undelivered_sdus_queue::add_sdu(uint32_t                              sn,
                                     const isrran::unique_byte_buffer_t&   sdu,
                                     uint32_t                              discard_timeout,
                                     isrran::move_callback<void(uint32_t)> callback)
{
  ...
  isrran::unique_byte_buffer_t tmp = make_byte_buffer();
  if (tmp == nullptr) {
    return false;
  }
...
std::map<uint32_t, isrran::unique_byte_buffer_t> undelivered_sdus_queue::get_buffered_sdus()
{
  std::map<uint32_t, isrran::unique_byte_buffer_t> fwd_sdus;
  for (auto& sdu : sdus) {
    if (sdu.sdu != nullptr) {
      // TODO: Find ways to avoid deep copy
      isrran::unique_byte_buffer_t fwd_sdu = make_byte_buffer();
      if (fwd_sdu != nullptr) {
        *fwd_sdu = *sdu.sdu;
        fwd_sdus.emplace(sdu.sdu->md.pdcp_sn, std::move(fwd_sdu));
      } else {
        isrlog::fetch_basic_logger("PDCP").warning("Can't allocate buffer to forward buffered SDUs.");
      }
    }
  }
  return fwd_sdus;
