...
pdcp_entity_base::pdcp_entity_base(task_sched_handle task_sched_, isrlog::basic_logger& logger) :
...
bool pdcp_entity_base::integrity_verify(uint8_t* msg, uint32_t msg_len, uint32_t count, uint8_t* mac)
{
  ...
  if (sec_cfg.integ_algo != INTEGRITY_ALGORITHM_ID_EIA0) {
    ...
    isrlog::log_channel& channel = is_valid ? logger.debug : logger.warning;
...
uint32_t pdcp_entity_base::read_data_header(const unique_byte_buffer_t& pdu)
{
  ...
  switch (cfg.sn_len) {
    ...
    case PDCP_SN_LEN_7:
      ...
      break;
    case PDCP_SN_LEN_12:
      isrran::uint8_to_uint16(pdu->msg, &rcvd_sn_16);
      rcvd_sn_32 = SN(rcvd_sn_16);
      break;
    case PDCP_SN_LEN_18:
      isrran::uint8_to_uint24(pdu->msg, &rcvd_sn_32);
...
void pdcp_entity_base::write_data_header(const isrran::unique_byte_buffer_t& sdu, uint32_t count)
{
  // Add room for header
  ...
  switch (cfg.sn_len) {
    ...
    case PDCP_SN_LEN_7:
      ...
      break;
    case PDCP_SN_LEN_12:
      isrran::uint16_to_uint8(SN(count), sdu->msg);
      if (is_drb()) {
        sdu->msg[0] |= 0x80; // On Data PDUs for DRBs we must set the D flag.
      }
      break;
    case PDCP_SN_LEN_18:
      isrran::uint24_to_uint8(SN(count), sdu->msg);
