...
#include "isrran/upper/pdcp_entity_nr.h"

namespace isrran {

pdcp::pdcp(isrran::task_sched_handle task_sched_, const char* logname) :
  task_sched(task_sched_), logger(isrlog::fetch_basic_logger(logname))
{}
...
void pdcp::init(isrue::rlc_interface_pdcp* rlc_, isrue::rrc_interface_pdcp* rrc_, isrue::gw_interface_pdcp* gw_)
{
  rlc = rlc_;
  rrc = rrc_;
  gw  = gw_;
...
void pdcp::enable_integrity(uint32_t lcid, isrran_direction_t direction)
{
  if (valid_lcid(lcid)) {
...
void pdcp::enable_encryption(uint32_t lcid, isrran_direction_t direction)
{
  if (valid_lcid(lcid)) {
...
void pdcp::enable_security_timed(uint32_t lcid, isrran_direction_t direction, uint32_t sn)
{
  if (valid_lcid(lcid)) {
...
bool pdcp::get_bearer_state(uint32_t lcid, isrran::pdcp_lte_state_t* state)
{
...
bool pdcp::set_bearer_state(uint32_t lcid, const isrran::pdcp_lte_state_t& state)
{
  if (not valid_lcid(lcid)) {
    return false;
  }
...
std::map<uint32_t, isrran::unique_byte_buffer_t> pdcp::get_buffered_pdus(uint32_t lcid)
{
  if (not valid_lcid(lcid)) {
    return {};
  }
...
void pdcp::notify_failure(uint32_t lcid, const isrran::pdcp_sn_vector_t& pdcp_sns)
{
  if (valid_lcid(lcid)) {
...
{
  if (lcid >= ISRRAN_N_RADIO_BEARERS) {
    logger.error("Radio bearer id must be in [0:%d] - %d", ISRRAN_N_RADIO_BEARERS, lcid);
    return false;
  }
