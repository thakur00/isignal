...
static isrran_cell_sl_t cell_sl = {.nof_prb = 50, .tm = ISRRAN_SIDELINK_TM4, .cp = ISRRAN_CP_NORM, .N_sl_id = 0};
...
static isrran_pscch_t pscch = {}; // Defined global for plotting thread
...
int main(int argc, char** argv)
{
  ...
  isrran_use_standard_symbol_size(prog_args.use_standard_lte_rates);

  isrran_sl_comm_resource_pool_t sl_comm_resource_pool;
  if (isrran_sl_comm_resource_pool_get_default_config(&sl_comm_resource_pool, cell_sl) != ISRRAN_SUCCESS) {
    ERROR("Error initializing sl_comm_resource_pool");
    return ISRRAN_ERROR;
  }

  if (prog_args.input_file_name) {
    if (isrran_filesource_init(&fsrc, prog_args.input_file_name, ISRRAN_COMPLEX_FLOAT_BIN)) {
  ...
  if (!prog_args.input_file_name) {
    ...
    if (isrran_rf_open_devname(&radio, prog_args.rf_dev, prog_args.rf_args, prog_args.nof_rx_antennas)) {
      ERROR("Error opening rf");
      exit(-1);
    }

    isrran_rf_set_rx_gain(&radio, prog_args.rf_gain);

    printf("Set RX freq: %.6f MHz\n",
           isrran_rf_set_rx_freq(&radio, prog_args.nof_rx_antennas, prog_args.rf_freq) / 1e6);
    printf("Set RX gain: %.1f dB\n", prog_args.rf_gain);
    int srate = isrran_sampling_freq_hz(cell_sl.nof_prb);

    if (srate != -1) {
      printf("Setting sampling rate %.2f MHz\n", (float)srate / 1000000);
      float srate_rf = isrran_rf_set_rx_srate(&radio, (double)srate);
      if (srate_rf != srate) {
  ...
  uint32_t sf_len = ISRRAN_SF_LEN_PRB(cell_sl.nof_prb);
  printf("Using a SF len of %d samples\n", sf_len);

  cf_t* rx_buffer[ISRRAN_MAX_CHANNELS] = {};     //< For radio to receive samples
  cf_t* sf_buffer[ISRRAN_MAX_PORTS]    = {NULL}; ///< For OFDM object to store subframe after FFT

  for (int i = 0; i < prog_args.nof_rx_antennas; i++) {
    rx_buffer[i] = isrran_vec_cf_malloc(sf_len);
    if (!rx_buffer[i]) {
      perror("malloc");
      exit(-1);
    }
    sf_buffer[i] = isrran_vec_cf_malloc(sf_len);
    if (!sf_buffer[i]) {
      perror("malloc");
      exit(-1);
    }
  ...
  uint32_t sf_n_re             = ISRRAN_CP_NSYMB(ISRRAN_CP_NORM) * ISRRAN_NRE * 2 * cell_sl.nof_prb;
  cf_t*    equalized_sf_buffer = isrran_vec_malloc(sizeof(cf_t) * sf_n_re);
  ...
  for (int i = 0; i < prog_args.nof_rx_antennas; i++) {
    ...
    if (isrran_ofdm_rx_init_cfg(&fft[i], &ofdm_cfg)) {
  ...
  // SCI
  isrran_sci_t sci;
  isrran_sci_init(&sci, &cell_sl, &sl_comm_resource_pool);
  ...
  if (isrran_pscch_set_cell(&pscch, cell_sl) != ISRRAN_SUCCESS) {
    ERROR("Error in PSCCH set cell");
    return ISRRAN_ERROR;
  }

  // PSCCH Channel estimation
  isrran_chest_sl_cfg_t pscch_chest_sl_cfg = {};
  isrran_chest_sl_t     pscch_chest        = {};
  if (isrran_chest_sl_init(&pscch_chest, ISRRAN_SIDELINK_PSCCH, cell_sl, &sl_comm_resource_pool) != ISRRAN_SUCCESS) {
    ERROR("Error in chest PSCCH init");
    return ISRRAN_ERROR;
  }

  if (isrran_pssch_init(&pssch, &cell_sl, &sl_comm_resource_pool) != ISRRAN_SUCCESS) {
    ERROR("Error initializing PSSCH");
    return ISRRAN_ERROR;
  }

  isrran_chest_sl_cfg_t pssch_chest_sl_cfg = {};
  isrran_chest_sl_t     pssch_chest        = {};
  if (isrran_chest_sl_init(&pssch_chest, ISRRAN_SIDELINK_PSSCH, cell_sl, &sl_comm_resource_pool) != ISRRAN_SUCCESS) {
    ERROR("Error in chest PSSCH init");
    return ISRRAN_ERROR;
  }

  uint8_t tb[ISRRAN_SL_SCH_MAX_TB_LEN]            = {};
  uint8_t packed_tb[ISRRAN_SL_SCH_MAX_TB_LEN / 8] = {};
  ...
  if (!prog_args.input_file_name) {
    ...
    if (isrran_ue_sync_init_multi_decim_mode(&ue_sync,
                                             cell.nof_prb,
                                             false,
    ...
    isrran_rf_start_rx_stream(&radio, false);
  }
#endif
  ...
  while (keep_running) {
    if (prog_args.input_file_name) {
      ...
      int nread = isrran_filesource_read(&fsrc, rx_buffer[0], sf_len);
      if (nread < 0) {
      ...
      int ret = isrran_ue_sync_zerocopy(&ue_sync, rx_buffer, sf_len);
      if (ret < 0) {
        ERROR("Error calling isrran_ue_sync_work()");
      }

      // update SF index
      current_sf_idx = isrran_ue_sync_get_sfidx(&ue_sync);
#endif // DISABLE_RF
    }

    // do FFT (on first port)
    isrran_ofdm_rx_sf(&fft[0]);
    ...
    for (int sub_channel_idx = 0; sub_channel_idx < sl_comm_resource_pool.num_sub_channel; sub_channel_idx++) {
      ...
      for (uint32_t cyclic_shift = 0; cyclic_shift <= 9; cyclic_shift += 3) {
        ...
        isrran_chest_sl_ls_estimate_equalize(&pscch_chest, sf_buffer[0], equalized_sf_buffer);
        ...
        if (isrran_pscch_decode(&pscch, equalized_sf_buffer, sci_rx, pscch_prb_start_idx) == ISRRAN_SUCCESS) {
          if (isrran_sci_format1_unpack(&sci, sci_rx) == ISRRAN_SUCCESS) {
            ...
            for (int j = 0; j < ISRRAN_SCI_CRC_LEN; j++) {
              N_x_id += pscch.sci_crc[j] * exp2(ISRRAN_SCI_CRC_LEN - 1 - j);
            }
            ...
            isrran_chest_sl_ls_estimate_equalize(&pssch_chest, sf_buffer[0], equalized_sf_buffer);
            ...
            if (isrran_pssch_set_cfg(&pssch, pssch_cfg) == ISRRAN_SUCCESS) {
              if (isrran_pssch_decode(&pssch, equalized_sf_buffer, tb, ISRRAN_SL_SCH_MAX_TB_LEN) == ISRRAN_SUCCESS) {
                ...
                isrran_bit_pack_vector(tb, packed_tb, pssch.sl_sch_tb_len);
...
#ifndef DISABLE_RF
  isrran_rf_stop_rx_stream(&radio);
...
void* plot_thread_run(void* arg)
{
  ...
  while (keep_running) {
    sem_wait(&plot_sem);
    plot_scatter_setNewData(&pscatequal_pscch, pscch.mod_symbols, pscch.E / ISRRAN_PSCCH_QM);
