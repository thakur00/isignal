...
static isrran_nbiot_si_params_t sib2_params;
...
void args_default(prog_args_t* args)
{
  ...
  args->n_id_ncell                         = ISRRAN_CELL_ID_UNKNOWN;
  args->is_r14                             = true;
...
static uint8_t rx_tb[ISRRAN_MAX_DL_BITS_CAT_NB1]; // Byte buffer for rx'ed transport blocks
                         ...
                         uint8_t  rnti_type)
{
  MAC_Context_Info_t context = {.radioType      = FDD_RADIO,
                                ...
                                .isRetx         = reTX,
                                .crcStatusOK    = crc_ok,
                                .sysFrameNumber = (uint16_t)(tti / ISRRAN_NOF_SF_X_FRAME),
                                .subFrameNumber = (uint16_t)(tti % ISRRAN_NOF_SF_X_FRAME),
                                .nbiotMode      = 1};
  if (pdu) {
...
int isrran_rf_recv_wrapper(void* h, void* data, uint32_t nsamples, isrran_timestamp_t* t)
{
  ...
  return isrran_rf_recv_with_time(h, data, nsamples, true, &t->full_secs, &t->frac_secs);
}

void isrran_rf_set_rx_gain_th_wrapper_(void* h, float f)
{
  isrran_rf_set_rx_gain_th((isrran_rf_t*)h, f);
}
...
int main(int argc, char** argv)
{
  ...
  cell.base.nof_prb = ISRRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL;
  cell.nbiot_prb    = ISRRAN_NBIOT_DEFAULT_PRB_OFFSET;
  ...
  if (!prog_args.input_file_name) {
    ...
    if (isrran_rf_open_devname(&rf, prog_args.rf_dev, prog_args.rf_args, 1)) {
    ...
    printf("Set RX freq: %.6f MHz\n", isrran_rf_set_rx_freq(&rf, 0, prog_args.rf_freq) / 1000000);

    // set sampling frequency
    int srate = isrran_sampling_freq_hz(cell.base.nof_prb);
    if (srate != -1) {
      printf("Setting sampling rate %.2f MHz\n", (float)srate / 1e6);
      double srate_rf = isrran_rf_set_rx_srate(&rf, srate);
    ...
    if (prog_args.n_id_ncell == ISRRAN_CELL_ID_UNKNOWN) {
      ...
      cf_t* buffer = isrran_vec_cf_malloc(input_len * 2);
      if (!buffer) {
        perror("malloc");
        exit(-1);
      }

      if (isrran_nsss_synch_init(&nsss, input_len, srate / 15000)) {
        fprintf(stderr, "Error initializing NSSS object\n");
        exit(-1);
      }

      isrran_rf_start_rx_stream(&rf, false);
      n = isrran_rf_recv(&rf, buffer, input_len, 1);
      if (n != input_len) {
      ...
      isrran_nsss_sync_find(&nsss, buffer, &nsss_peak_value, (uint32_t*)&cell.n_id_ncell, &sfn_partial);
      printf("done!");
      isrran_nsss_synch_free(&nsss);
      free(buffer);
    } else {
      cell.n_id_ncell = prog_args.n_id_ncell;
    }
...
#else
    if (cell.n_id_ncell == ISRRAN_CELL_ID_UNKNOWN) {
      uint32_t ntrial = 0;
      do {
      ...
      } while (ret != ISRRAN_SUCCESS && !go_exit);
    }
#endif
  ...
  if (prog_args.input_file_name) {
    ...
    if (isrran_ue_sync_nbiot_init_file(
...
#ifndef DISABLE_RF
    if (isrran_ue_sync_nbiot_init(&ue_sync, cell, isrran_rf_recv_wrapper, (void*)&rf)) {
      fprintf(stderr, "Error initiating ue_sync\n");
      exit(-1);
    }
    // reduce AGC period to every 10th frame
    isrran_ue_sync_nbiot_set_agc_period(&ue_sync, 10);
#endif
  }

  // Allocate memory to fit a full frame (needed for time re-alignment)
  cf_t* buff_ptrs[ISRRAN_MAX_PORTS] = {NULL};
  buff_ptrs[0]                      = isrran_vec_cf_malloc(ISRRAN_SF_LEN_PRB_NBIOT * 10);

  if (isrran_ue_mib_nbiot_init(&ue_mib, buff_ptrs, ISRRAN_NBIOT_MAX_PRB)) {
    fprintf(stderr, "Error initaiting UE MIB decoder\n");
    exit(-1);
  }
  if (isrran_ue_mib_nbiot_set_cell(&ue_mib, cell) != ISRRAN_SUCCESS) {
...
#ifndef DISABLE_RF
  if (prog_args.rf_gain < 0) {
    isrran_ue_sync_nbiot_start_agc(&ue_sync, isrran_rf_set_rx_gain_th_wrapper_, cell_detect_config.init_agc);
  }
#endif
#ifdef PRINT_CHANGE_SCHEDULIGN
  isrran_ra_nbiot_dl_dci_t old_dl_dci;
  bzero(&old_dl_dci, sizeof(isrran_ra_nbiot_dl_dci_t));
#endif

  ue_sync.correct_cfo = !prog_args.disable_cfo;

  // Set initial CFO for ue_sync
  isrran_ue_sync_nbiot_set_cfo(&ue_sync, cfo);

  isrran_npbch_decode_reset(&ue_mib.npbch);

  INFO("\nEntering main loop...");
  while (!go_exit && (sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    ret = isrran_ue_sync_nbiot_zerocopy_multi(&ue_sync, buff_ptrs);
    if (ret < 0) {
      fprintf(stderr, "Error calling isrran_nbiot_ue_sync_zerocopy_multi()\n");
      break;
    }
    ...
    // isrran_ue_sync_nbiot_zerocopy_multi() returns 1 if successfully read 1 aligned subframe
    if (ret == 1) {
      switch (state) {
        case DECODE_MIB:
          if (isrran_ue_sync_nbiot_get_sfidx(&ue_sync) == 0) {
            n = isrran_ue_mib_nbiot_decode(&ue_mib, buff_ptrs[0], bch_payload, &cell.nof_ports, &sfn_offset);
            if (n < 0) {
              fprintf(stderr, "Error decoding UE MIB\n");
              exit(-1);
            } else if (n == ISRRAN_UE_MIB_NBIOT_FOUND) {
              printf("MIB received (CFO: %+6.2f kHz)\n", isrran_ue_sync_nbiot_get_cfo(&ue_sync) / 1000);
              isrran_mib_nb_t mib;
              isrran_npbch_mib_unpack(bch_payload, &mib);
              ...
              if (cell.mode == ISRRAN_NBIOT_MODE_INBAND_SAME_PCI) {
              ...
              isrran_nbiot_ue_dl_set_rnti(&ue_dl, prog_args.rnti);

              // Pretty-print MIB
              isrran_bit_pack_vector(bch_payload, rx_tb, ISRRAN_MIB_NB_CRC_LEN);
#ifdef ENABLE_GUI
              if (bcch_bch_to_pretty_string(
                      rx_tb, ISRRAN_MIB_NB_CRC_LEN, mib_buffer_decode, sizeof(mib_buffer_decode))) {
              ...
              pcap_pack_and_write(pcap_file,
                                  rx_tb,
                                  ISRRAN_MIB_NB_CRC_LEN,
                                  0,
                                  true,
                                  system_frame_number * ISRRAN_NOF_SF_X_FRAME,
                                  0,
                                  DIRECTION_DOWNLINK,
                                  NO_RNTI);
#endif
              // activate SIB1 decoding
              isrran_nbiot_ue_dl_decode_sib1(&ue_dl, system_frame_number);
              state = DECODE_SIB;
            }
          }
          break;
        ...
        case DECODE_SIB:
          if (!have_sib1) {
            int dec_ret = isrran_nbiot_ue_dl_decode_npdsch(&ue_dl,
                                                           &buff_ptrs[0][prog_args.time_offset],
                                                           rx_tb,
                                                           system_frame_number,
                                                           isrran_ue_sync_nbiot_get_sfidx(&ue_sync),
                                                           ISRRAN_SIRNTI);
            if (dec_ret == ISRRAN_SUCCESS) {
              printf("SIB1 received\n");
              isrran_sys_info_block_type_1_nb_t sib = {};
              isrran_npdsch_sib1_unpack(rx_tb, &sib);
              ...
              isrran_nbiot_ue_dl_decode_sib(
                  &ue_dl, hyper_frame_number, system_frame_number, ISRRAN_NBIOT_SI_TYPE_SIB2, sib2_params);
#if HAVE_PCAP
              pcap_pack_and_write(pcap_file,
                                  rx_tb,
                                  ...
                                  0,
                                  true,
                                  system_frame_number * 10 + isrran_ue_sync_nbiot_get_sfidx(&ue_sync),
                                  ISRRAN_SIRNTI,
                                  DIRECTION_DOWNLINK,
                                  SI_RNTI);
#endif
              ...
              break;
            } else if (dec_ret == ISRRAN_ERROR) {
              // reactivate SIB1 grant
              if (isrran_nbiot_ue_dl_has_grant(&ue_dl) == false) {
                isrran_nbiot_ue_dl_decode_sib1(&ue_dl, system_frame_number);
              }
            }
          } else if (!have_sib2 && !isrran_nbiot_ue_dl_is_sib1_sf(
                                       &ue_dl, system_frame_number, isrran_ue_sync_nbiot_get_sfidx(&ue_sync))) {
        ...
        case DECODE_NPDSCH:
          if (prog_args.rnti != ISRRAN_SIRNTI) {
            if (isrran_nbiot_ue_dl_has_grant(&ue_dl)) {
              ...
              n = isrran_nbiot_ue_dl_decode_npdsch(&ue_dl,
                                                   &buff_ptrs[0][prog_args.time_offset],
                                                   rx_tb,
                                                   system_frame_number,
                                                   isrran_ue_sync_nbiot_get_sfidx(&ue_sync),
              ...
              n = isrran_nbiot_ue_dl_decode_npdcch(&ue_dl,
                                                   &buff_ptrs[0][prog_args.time_offset],
                                                   system_frame_number,
                                                   isrran_ue_sync_nbiot_get_sfidx(&ue_sync),
                                                   prog_args.rnti,
              ...
              if (n == ISRRAN_NBIOT_UE_DL_FOUND_DCI) {
                ...
                if (isrran_nbiot_dci_msg_to_dl_grant(&dci_msg,
                                                     prog_args.rnti,
                                                     &dci_unpacked,
                                                     &grant,
                                                     system_frame_number,
          ...
          rsrq  = ISRRAN_VEC_EMA(isrran_chest_dl_nbiot_get_rsrq(&ue_dl.chest), rsrq, 0.1);
          rsrp  = ISRRAN_VEC_EMA(isrran_chest_dl_nbiot_get_rsrp(&ue_dl.chest), rsrp, 0.05);
          noise = ISRRAN_VEC_EMA(isrran_chest_dl_nbiot_get_noise_estimate(&ue_dl.chest), noise, 0.05);
          nframes++;
          if (isnan(rsrq)) {
            rsrq = 0;
          }
          ...
          // Plot and Printf
          if (isrran_ue_sync_nbiot_get_sfidx(&ue_sync) == 5) {
            float gain = prog_args.rf_gain;
            if (gain < 0) {
              gain = 10 * log10(isrran_agc_get_gain(&ue_sync.agc));
            }
            printf(
                ...
                ue_dl.pkt_errors,
                ue_dl.pkts_total,
                (ue_dl.bits_total / ((system_frame_number * 10 + isrran_ue_sync_nbiot_get_sfidx(&ue_sync)) / 1000.0)) /
                    1000.0);
          }
          break;
      }
      if (isrran_ue_sync_nbiot_get_sfidx(&ue_sync) == 9) {
        system_frame_number++;
      ...
      if (!prog_args.disable_plots) {
        if ((system_frame_number % 4) == 0) {
          plot_sf_idx = isrran_ue_sync_nbiot_get_sfidx(&ue_sync);
          plot_track  = true;
          sem_post(&plot_sem);
        }
      }
      ...
      printf("Finding PSS... Peak: %8.1f, FrameCnt: %d, State: %d\r",
             isrran_sync_nbiot_get_peak_value(&ue_sync.sfind),
...
#ifndef DISABLE_RF
  if (!prog_args.input_file_name) {
    isrran_ue_mib_nbiot_free(&ue_mib);
    isrran_rf_close(&rf);
...
{
  uint32_t nof_re = ISRRAN_SF_LEN_RE(ue_dl.cell.base.nof_prb, ue_dl.cell.base.cp);
#if HAVE_RSRP_PLOT
  float rsrp_lin = 0;
#endif
  ...
  while (!go_exit) {
    ...
    if (!prog_args.disable_plots_except_constellation) {
      ...
      int numpoints = ISRRAN_NRE * 2;
      bzero(tmp_plot2, sizeof(float) * numpoints);
      int g = (numpoints - ISRRAN_NRE) / 2;
      ...
      if (!prog_args.input_file_name) {
        if (plot_track) {
          isrran_npss_synch_t* pss_obj = &ue_sync.strack.npss;
          ...
          int len = ISRRAN_NPSS_CORR_FILTER_LEN + ue_sync.sfind.npss.frame_size - 1;
          int max = isrran_vec_max_fi(ue_sync.sfind.npss.conv_output_avg, len);
          isrran_vec_sc_prod_fff(
      ...
      rsrp_lin                       = ISRRAN_VEC_EMA(isrran_chest_dl_nbiot_get_rsrp(&ue_dl.chest), rsrp_lin, 0.05);
      ...
      key_value_setValueText(&mode_label, isrran_nbiot_mode_string(cell.mode));
