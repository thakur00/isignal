...
/* Example function to initialize the Radio frontend. In this case, we use isrRAN RF API to open a device,
...
{
  // Uses isrRAN RF API to open a device, could use other code here
  printf("Opening RF device...\n");
  if (isrran_rf_open_multi(&radio, rf_args, nof_rx_antennas)) {
    ERROR("Error opening rf");
    return -1;
  }
  isrran_rf_set_rx_gain(&radio, rf_gain);
  isrran_rf_set_rx_freq(&radio, nof_rx_antennas, rf_freq);

  printf("Set RX freq: %.2f MHz\n", rf_freq / 1000000);
  printf("Set RX gain: %.2f dB\n", rf_gain);
  float srate = isrran_rf_set_rx_srate(&radio, rf_rate);
  if (srate != rf_rate) {
...
/* Example implementation to receive from Radio frontend. In this case we use isrRAN
 */
static int rx_radio(void** buffer, uint32_t buf_len)
{
  return isrran_rf_recv_with_time_multi(&radio, buffer, buf_len, true, NULL, NULL);
}
