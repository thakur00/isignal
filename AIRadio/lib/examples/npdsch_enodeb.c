...
static isrran_nbiot_cell_t cell = {
    .base       = {.nof_ports = 1, .nof_prb = ISRRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL, .cp = ISRRAN_CP_NORM, .id = 0},
    .nbiot_prb  = ISRRAN_NBIOT_DEFAULT_PRB_OFFSET,
    .n_id_ncell = 0,
    .nof_ports  = 1,
...
static isrran_random_t*         random_gen;
static isrran_filesink_t        fsink;
static isrran_ofdm_t            ifft;
static isrran_npss_synch_t      npss_sync;
static isrran_nsss_synch_t      nsss_sync;
static isrran_npbch_t           npbch;
static isrran_npdcch_t          npdcch;
static isrran_npdsch_t          npdsch;
static isrran_npdsch_cfg_t      sib1_npdsch_cfg;
static isrran_npdsch_cfg_t      npdsch_cfg;
static isrran_nbiot_ue_dl_t     ue_dl;
static isrran_softbuffer_tx_t   softbuffer;
static isrran_ra_nbiot_dl_dci_t ra_dl;
static isrran_ra_nbiot_dl_dci_t ra_dl_sib1;
static isrran_chest_dl_nbiot_t  ch_est;
static isrran_mib_nb_t          mib_nb;
...
void base_init()
{
  ...
  output_buffer = isrran_vec_cf_malloc(sf_n_samples);
  if (!output_buffer) {
    perror("malloc");
    exit(-1);
  }
  ...
  if (output_file_name) {
    ...
    if (isrran_rf_open(&radio, rf_args)) {
  ...
  if (isrran_ofdm_tx_init(&ifft, ISRRAN_CP_NORM, sf_buffer, output_buffer, cell.base.nof_prb)) {
  ...
  if (isrran_npss_synch_init(&npss_sync, sf_n_samples, isrran_symbol_sz(cell.base.nof_prb))) {
    fprintf(stderr, "Error initializing NPSS object\n");
    exit(-1);
  }

  if (isrran_nsss_synch_init(&nsss_sync, sf_n_samples, isrran_symbol_sz(cell.base.nof_prb))) {
  ...
  if (isrran_npdsch_init(&npdsch)) {
    fprintf(stderr, "Error creating NPDSCH object\n");
    exit(-1);
  }

  if (isrran_npdsch_set_cell(&npdsch, cell)) {
    fprintf(stderr, "Configuring cell in NPDSCH\n");
    exit(-1);
  }
  isrran_npdsch_set_rnti(&npdsch, UE_CRNTI);

  if (isrran_softbuffer_tx_init(&softbuffer, cell.base.nof_prb)) {
    fprintf(stderr, "Error initiating soft buffer\n");
    exit(-1);
  }

  random_gen = isrran_random_init(time(NULL));
}

void base_free()
{
  isrran_random_free(random_gen);
  isrran_softbuffer_tx_free(&softbuffer);
  isrran_npdsch_free(&npdsch);
  isrran_npdcch_free(&npdcch);
  isrran_npbch_free(&npbch);
  isrran_chest_dl_nbiot_free(&ch_est);
  isrran_npss_synch_free(&npss_sync);
  isrran_nsss_synch_free(&nsss_sync);
...
static int update_radl(void)
{
  ...
  bzero(&ra_dl, sizeof(isrran_ra_nbiot_dl_dci_t));
  ...
  isrran_ra_nbiot_dl_grant_t dummy_grant;
  isrran_ra_nbits_t          dummy_nbits;

#define DUMMY_SFIDX 1
#define DUMMY_SFN 0
  isrran_ra_nbiot_dl_dci_to_grant(&ra_dl, &dummy_grant, DUMMY_SFN, DUMMY_SFIDX, DUMMY_R_MAX, false, cell.mode);
  isrran_ra_nbiot_dl_grant_to_nbits(&dummy_grant, cell, 0, &dummy_nbits);
  isrran_ra_nbiot_dl_grant_fprint(stdout, &dummy_grant);
...
{
  int  nf = 0, sf_idx = 0;
  cf_t npss_signal[ISRRAN_NPSS_TOT_LEN];
  cf_t nsss_signal[ISRRAN_NSSS_LEN * ISRRAN_NSSS_NUM_SEQ]; // for subframe 9

  uint8_t bch_payload[ISRRAN_MIB_NB_LEN];
  uint8_t sib1_nb_payload[ISRRAN_NPDSCH_MAX_TBS];
  ...
  sf_n_re      = 2 * ISRRAN_CP_NORM_NSYMB * cell.base.nof_prb * ISRRAN_NRE;
  sf_n_samples = 2 * ISRRAN_SLOT_LEN(isrran_symbol_sz(cell.base.nof_prb));

  /* this *must* be called after setting slot_len_* */
  base_init();

  // buffer for outputting signal in RE representation (using sf_n_re)
  cf_t* sf_re_symbols[ISRRAN_MAX_PORTS] = {NULL, NULL, NULL, NULL};
  sf_re_symbols[0]                      = sf_buffer;

  // buffer for outputting actual IQ samples (using sf_n_samples)
  cf_t* sf_symbols[ISRRAN_MAX_PORTS] = {NULL, NULL, NULL, NULL};
  ...
  // Initialize UE DL
  if (isrran_nbiot_ue_dl_init(&ue_dl, sf_symbols, ISRRAN_NBIOT_MAX_PRB, ISRRAN_NBIOT_NUM_RX_ANTENNAS)) {
  ...
  isrran_nbiot_ue_dl_set_mib(&ue_dl, mib_nb);

  /* Generate NPSS/NSSS signals */
  isrran_npss_generate(npss_signal);
  isrran_nsss_generate(nsss_signal, cell.n_id_ncell);

#ifdef NPSS_DUMP
  isrran_filesink_t debug_fsink;
  char              fname[] = "npss.bin";
  if (isrran_filesink_init(&debug_fsink, fname, ISRRAN_COMPLEX_FLOAT_BIN)) {
    fprintf(stderr, "Error opening file %s\n", fname);
    exit(-1);
  }
  isrran_filesink_write(&debug_fsink, npss_signal, ISRRAN_NPSS_LEN * 11);
  isrran_filesink_free(&debug_fsink);
#endif

  /* Generate CRS+NRS signals */
  if (isrran_chest_dl_nbiot_init(&ch_est, ISRRAN_NBIOT_MAX_PRB)) {
    fprintf(stderr, "Error initializing equalizer\n");
    exit(-1);
  }
  if (isrran_chest_dl_nbiot_set_cell(&ch_est, cell) != ISRRAN_SUCCESS) {
  ...
  if (!output_file_name) {
    int srate = isrran_sampling_freq_hz(cell.base.nof_prb);
    if (srate != -1) {
    ...
    printf("Set TX freq: %.2f MHz\n", isrran_rf_set_tx_freq(&radio, 0, rf_freq) / 1000000);
  }
#endif
  ...
  isrran_softbuffer_tx_reset(&softbuffer);
  bzero(&sib1_npdsch_cfg, sizeof(isrran_npdsch_cfg_t));
  bzero(&npdsch_cfg, sizeof(isrran_npdsch_cfg_t));
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    for (sf_idx = 0; sf_idx < ISRRAN_NOF_SF_X_FRAME && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      isrran_vec_cf_zero(sf_buffer, sf_n_re);

      // Transmit NPBCH in subframe 0
      if (sf_idx == 0) {
        if ((sfn % ISRRAN_NPBCH_NUM_FRAMES) == 0) {
          isrran_npbch_mib_pack(hfn, sfn, mib_nb, bch_payload);
        }
        isrran_npbch_put_subframe(&npbch, bch_payload, sf_re_symbols, sfn);
      ...
      if (sf_idx == 5) {
        // NPSS at subframe 5
        isrran_npss_put_subframe(&npss_sync, npss_signal, sf_buffer, cell.base.nof_prb, cell.nbiot_prb);
      } else if ((sfn % 2 == 0) && sf_idx == 9) {
        // NSSS in every even numbered frame at subframe 9
        isrran_nsss_put_subframe(&nsss_sync, nsss_signal, sf_buffer, sfn, cell.base.nof_prb, cell.nbiot_prb);
      } else {
        // NRS in all other subframes (using CSR signal intentionally)
        // DEBUG("%d.%d: Putting %d NRS pilots", sfn, sf_idx, ISRRAN_REFSIGNAL_NUM_SF(1, cell.nof_ports));
        isrran_refsignal_nrs_put_sf(cell, 0, ch_est.nrs_signal.pilots[0][sf_idx], sf_buffer);
      }
      ...
      if (sf_idx != 0 && sf_idx != 5 && (!(sf_idx == 9 && sfn % 2 == 0)) &&
          !isrran_nbiot_ue_dl_is_sib1_sf(&ue_dl, sfn, sf_idx)) {
        send_data = true;
      } else {
        send_data = false;
      }
      ...
      if (hfn % 4 == 0 && sfn == 0 && sf_idx == 0) {
        ...
        isrran_bit_unpack_vector(sib1_nb_payload, unpacked_hfn, 4 * 8);
        uint8_t* tmp = unpacked_hfn;
        tmp += 12;
        isrran_bit_unpack(hfn >> 2, &tmp, 8);
        uint8_t packed_hfn[4];
        isrran_bit_pack_vector(unpacked_hfn, packed_hfn, 32);
        memcpy(sib1_nb_payload, packed_hfn, sizeof(packed_hfn));

        if (ISRRAN_VERBOSE_ISDEBUG()) {
          printf("SIB1-NB payload: ");
          isrran_vec_fprint_byte(stdout, sib1_nb_payload, sizeof(dummy_sib1_payload));
        }
      }

      if (isrran_nbiot_ue_dl_is_sib1_sf(&ue_dl, sfn, sf_idx)) {
        ...
        if (sib1_npdsch_cfg.sf_idx == 0) {
          ...
          isrran_ra_nbiot_dl_dci_to_grant(&ra_dl_sib1, &grant, sfn, sf_idx, DUMMY_R_MAX, true, cell.mode);
          if (isrran_npdsch_cfg(&sib1_npdsch_cfg, cell, &grant, sf_idx)) {
        ...
        // Encode SIB1 content
        if (isrran_npdsch_encode_rnti(
                &npdsch, &sib1_npdsch_cfg, &softbuffer, sib1_nb_payload, ISRRAN_SIRNTI, sf_re_symbols)) {
      ...
      if (send_data) {
        ...
        if (sf_idx == NPDCCH_SF_IDX && !npdsch_active) {
          ...
          isrran_dci_msg_pack_npdsch(&ra_dl, ISRRAN_DCI_FORMATN1, &dci_msg, false);
          if (isrran_npdcch_encode(&npdcch, &dci_msg, locations[sf_idx][0], UE_CRNTI, sf_re_symbols, sf_idx)) {
          ...
          isrran_ra_nbiot_dl_grant_t grant;
          isrran_ra_nbiot_dl_dci_to_grant(&ra_dl, &grant, sfn, sf_idx, DUMMY_R_MAX, false, cell.mode);
          if (isrran_npdsch_cfg(&npdsch_cfg, cell, &grant, sf_idx)) {
            fprintf(stderr, "Error configuring NPDSCH\n");
            exit(-1);
          }
        }
        ...
        if (!npdsch_active && (sf_idx == npdsch_cfg.grant.start_sfidx && sfn == npdsch_cfg.grant.start_sfn)) {
          ...
          for (int i = 0; i < npdsch_cfg.grant.mcs[0].tbs / 8; i++) {
            data[i] = isrran_random_uniform_int_dist(random_gen, 0, 255);
          }
          if (ISRRAN_VERBOSE_ISDEBUG()) {
            printf("Tx payload: ");
            isrran_vec_fprint_b(stdout, data, npdsch_cfg.grant.mcs[0].tbs / 8);
          }
          npdsch_active = true;
        }
        ...
        if (npdsch_active) {
          ...
          // Encode NPDSCH
          if (isrran_npdsch_encode(&npdsch, &npdsch_cfg, &softbuffer, data, sf_re_symbols)) {
            fprintf(stderr, "Error encoding NPDSCH\n");
            exit(-1);
          }
      ...
      if (output_file_name && file_snr != -100.0) {
        ...
        float abs_avg = isrran_vec_avg_power_cf(output_buffer, sf_n_samples);

        // find the noise spectral density
        float snr_lin = isrran_convert_dB_to_power(file_snr);
        float n0      = abs_avg / snr_lin;

        // add some noise to the signal
        isrran_ch_awgn_c(output_buffer, output_buffer, n0, sf_n_samples);
      }
      ...
      if (output_file_name) {
        // write to file
        if (!null_file_sink) {
          isrran_filesink_write(&fsink, output_buffer, sf_n_samples);
        }
        usleep(1000);
      } else {
#ifndef DISABLE_RF
        // TODO: output scaling needed?
        isrran_rf_send2(&radio, output_buffer, sf_n_samples, true, start_of_burst, false);
