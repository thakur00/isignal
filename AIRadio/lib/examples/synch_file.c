...
int main(int argc, char** argv)
{
  ...
  isrran_filesink_t   fsink;
  isrran_pss_t        pss[3]; // One for each N_id_2
  isrran_sss_t        sss[3]; // One for each N_id_2
  ...
  input = isrran_vec_cf_malloc(frame_length);
  if (!input) {
    perror("malloc");
    exit(-1);
  }
  cfo = isrran_vec_f_malloc(nof_frames);
  if (!cfo) {
    perror("malloc");
    exit(-1);
  }
  exec_time = isrran_vec_i32_malloc(nof_frames);
  if (!exec_time) {
    perror("malloc");
    exit(-1);
  }

  if (isrran_cfo_init(&cfocorr, frame_length)) {
  ...
  for (N_id_2 = 0; N_id_2 < 3; N_id_2++) {
    if (isrran_pss_init_fft(&pss[N_id_2], frame_length, symbol_sz)) {
      ERROR("Error initializing PSS object");
      exit(-1);
    }
    if (isrran_pss_set_N_id_2(&pss[N_id_2], N_id_2)) {
      ERROR("Error initializing N_id_2");
      exit(-1);
    }
    if (isrran_sss_init(&sss[N_id_2], symbol_sz)) {
      ERROR("Error initializing SSS object");
      exit(-1);
    }
    if (isrran_sss_set_N_id_2(&sss[N_id_2], N_id_2)) {
      ERROR("Error initializing N_id_2");
      exit(-1);
    }
  }
  ...
  frame_cnt = 0;
  while (frame_length == isrran_filesource_read(&fsrc, input, frame_length) && frame_cnt < nof_frames) {
    gettimeofday(&tdata[1], NULL);
    if (force_cfo != CFO_AUTO) {
      isrran_cfo_correct(&cfocorr, input, input, force_cfo / 128);
    }

    if (force_N_id_2 != -1) {
      N_id_2           = force_N_id_2;
      peak_pos[N_id_2] = isrran_pss_find_pss(&pss[N_id_2], input, &peak_value[N_id_2]);
    } else {
      for (N_id_2 = 0; N_id_2 < 3; N_id_2++) {
        peak_pos[N_id_2] = isrran_pss_find_pss(&pss[N_id_2], input, &peak_value[N_id_2]);
      }
    ...
    if (peak_value[N_id_2] > corr_peak_threshold) {
      sss_idx = peak_pos[N_id_2] - 2 * (symbol_sz + ISRRAN_CP_LEN(symbol_sz, ISRRAN_CP_NORM_LEN));
      if (sss_idx >= 0) {
        isrran_sss_m0m1_diff(&sss[N_id_2], &input[sss_idx], &m0, &m0_value, &m1, &m1_value);

        cfo[frame_cnt] = isrran_pss_cfo_compute(&pss[N_id_2], &input[peak_pos[N_id_2] - 128]);
        printf("\t%d\t%d\t%d\t%d\t%.3f\t\t%3d\t%d\t%d\t%.3f\n",
               frame_cnt,
               N_id_2,
               isrran_sss_N_id_1(&sss[N_id_2], m0, m1, m1_value + m0_value),
               isrran_sss_subframe(m0, m1),
               peak_value[N_id_2],
               peak_pos[N_id_2],
               m0,
               m1,
  ...
  for (N_id_2 = 0; N_id_2 < 3; N_id_2++) {
    isrran_pss_free(&pss[N_id_2]);
    isrran_sss_free(&sss[N_id_2]);
  }
