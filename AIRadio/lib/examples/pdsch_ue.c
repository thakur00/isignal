  ...
  while ((opt = getopt(argc, argv, "adAogliIpPcOCtdDFRqnvrfuUsSZyWMNBTGQ")) != -1) {
    switch (opt) {
      ...
      case 'v':
        ...
        args->verbose = get_isrran_verbose_level();
        break;
      case 'Z':
...
cf_t* sf_buffer[ISRRAN_MAX_PORTS] = {NULL};

#ifndef DISABLE_RF

int isrran_rf_recv_wrapper(void* h, cf_t* data_[ISRRAN_MAX_PORTS], uint32_t nsamples, isrran_timestamp_t* t)
{
...
static ISRRAN_AGC_CALLBACK(isrran_rf_set_rx_gain_th_wrapper_)
{
  isrran_rf_set_rx_gain_th((isrran_rf_t*)h, gain_db);
}
...
isrran_netsink_t net_sink, net_sink_signal;
...
int main(int argc, char** argv)
{
  ...
  isrran_use_standard_symbol_size(prog_args.use_standard_lte_rate);
  ...
  for (int i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    data[i] = isrran_vec_u8_malloc(2000 * 8);
    if (!data[i]) {
  ...
  if (prog_args.net_port > 0) {
    if (isrran_netsink_init(&net_sink, prog_args.net_address, prog_args.net_port, ISRRAN_NETSINK_UDP)) {
      ERROR("Error initiating UDP socket to %s:%d", prog_args.net_address, prog_args.net_port);
      exit(-1);
    }
    isrran_netsink_set_nonblocking(&net_sink);
  }
  if (prog_args.net_port_signal > 0) {
    if (isrran_netsink_init(
            &net_sink_signal, prog_args.net_address_signal, prog_args.net_port_signal, ISRRAN_NETSINK_UDP)) {
      ERROR("Error initiating UDP socket to %s:%d", prog_args.net_address_signal, prog_args.net_port_signal);
      exit(-1);
    }
    isrran_netsink_set_nonblocking(&net_sink_signal);
  }
  ...
  if (!prog_args.input_file_name) {
    ...
    if (isrran_rf_open_devname(&rf, prog_args.rf_dev, prog_args.rf_args, prog_args.rf_nof_rx_ant)) {
      fprintf(stderr, "Error opening rf\n");
      exit(-1);
    }
    /* Set receiver gain */
    if (prog_args.rf_gain > 0) {
      isrran_rf_set_rx_gain(&rf, prog_args.rf_gain);
    } else {
      ...
      isrran_rf_set_rx_gain(&rf, isrran_rf_get_rx_gain(&rf));
      cell_detect_config.init_agc = isrran_rf_get_rx_gain(&rf);
    }
    ...
    isrran_rf_set_rx_freq(&rf, prog_args.rf_nof_rx_ant, prog_args.rf_freq + prog_args.file_offset_freq);
  ...
  if (prog_args.input_file_name) {
    ...
    if (isrran_ue_sync_init_file_multi(&ue_sync,
    ...
    if (isrran_ue_sync_init_multi_decim(&ue_sync,
                                        cell.nof_prb,
                                        cell.id == 1000,
                                        isrran_rf_recv_wrapper,
                                        prog_args.rf_nof_rx_ant,
                                        (void*)&rf,
                                        decimate)) {
      ERROR("Error initiating ue_sync");
  ...
  uint32_t max_num_samples = 3 * ISRRAN_SF_LEN_PRB(cell.nof_prb); /// Length in complex samples
  for (int i = 0; i < prog_args.rf_nof_rx_ant; i++) {
    sf_buffer[i] = isrran_vec_cf_malloc(max_num_samples);
  }
  isrran_ue_mib_t ue_mib;
  if (isrran_ue_mib_init(&ue_mib, sf_buffer[0], cell.nof_prb)) {
    ERROR("Error initaiting UE MIB decoder");
    exit(-1);
  }
  if (isrran_ue_mib_set_cell(&ue_mib, cell)) {
    ERROR("Error initaiting UE MIB decoder");
    exit(-1);
  }

  if (isrran_ue_dl_init(&ue_dl, sf_buffer, cell.nof_prb, prog_args.rf_nof_rx_ant)) {
  ...
  isrran_sync_set_cfo_cp_enable(&ue_sync.sfind, false, 0);
  ...
  if (cell.frame_type == ISRRAN_TDD && prog_args.tdd_special_sf >= 0 && prog_args.sf_config >= 0) {
  ...
  isrran_chest_dl_cfg_t chest_pdsch_cfg = {};
  chest_pdsch_cfg.cfo_estimate_enable   = prog_args.enable_cfo_ref;
  chest_pdsch_cfg.cfo_estimate_sf_mask  = 1023;
  chest_pdsch_cfg.estimator_alg         = isrran_chest_dl_str2estimator_alg(prog_args.estimator_alg);
  ...
  chest_mbsfn_cfg.filter_type           = ISRRAN_CHEST_FILTER_TRIANGLE;
  chest_mbsfn_cfg.filter_coef[0]        = 0.1;
  chest_mbsfn_cfg.estimator_alg         = ISRRAN_ESTIMATOR_ALG_INTERPOLATE;
  chest_mbsfn_cfg.noise_alg             = ISRRAN_NOISE_ALG_PSS;

  // Allocate softbuffer buffers
  isrran_softbuffer_rx_t rx_softbuffers[ISRRAN_MAX_CODEWORDS];
  for (uint32_t i = 0; i < ISRRAN_MAX_CODEWORDS; i++) {
    pdsch_cfg.softbuffers.rx[i] = &rx_softbuffers[i];
    isrran_softbuffer_rx_init(pdsch_cfg.softbuffers.rx[i], cell.nof_prb);
  }
  ...
  if (prog_args.mbsfn_area_id > -1) {
    isrran_ue_dl_set_mbsfn_area_id(&ue_dl, prog_args.mbsfn_area_id);
    isrran_ue_dl_set_non_mbsfn_region(&ue_dl, prog_args.non_mbsfn_region);
  }
  ...
  if (prog_args.rf_gain < 0 && !prog_args.input_file_name) {
    ...
    isrran_ue_sync_start_agc(&ue_sync,
                             isrran_rf_set_rx_gain_th_wrapper_,
                             rf_info->min_rx_gain,
                             rf_info->max_rx_gain,
  ...
  isrran_ra_dl_grant_t old_dl_dci;
  bzero(&old_dl_dci, sizeof(isrran_ra_dl_grant_t));
#endif

  ue_sync.cfo_correct_enable_track = !prog_args.disable_cfo;

  isrran_pbch_decode_reset(&ue_mib.pbch);
  ...
  float    rsrp0 = 0.0, rsrp1 = 0.0, rsrq = 0.0, snr = 0.0, enodebrate = 0.0, uerate = 0.0, procrate = 0.0,
        sinr[ISRRAN_MAX_LAYERS][ISRRAN_MAX_CODEBOOKS] = {}, sync_err[ISRRAN_MAX_PORTS][ISRRAN_MAX_PORTS] = {};
  bool decode_pdsch = false;

  for (int i = 0; i < ISRRAN_MAX_LAYERS; i++) {
    isrran_vec_f_zero(sinr[i], ISRRAN_MAX_CODEBOOKS);
  }
  ...
  while (!go_exit && (sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    ...
    set_isrran_verbose_level(prog_args.verbose);
    ...
    cf_t* buffers[ISRRAN_MAX_CHANNELS] = {};
    for (int p = 0; p < ISRRAN_MAX_PORTS; p++) {
      buffers[p] = sf_buffer[p];
    }
    ret = isrran_ue_sync_zerocopy(&ue_sync, buffers, max_num_samples);
    if (ret < 0) {
    ...
    float sample_offset =
        (float)isrran_ue_sync_get_last_sample_offset(&ue_sync) + isrran_ue_sync_get_sfo(&ue_sync) / 1000;
    isrran_ue_dl_set_sample_offset(&ue_dl, sample_offset);
#endif

    /* isrran_ue_sync_get_buffer returns 1 if successfully read 1 aligned subframe */
    if (ret == 1) {
      bool           acks[ISRRAN_MAX_CODEWORDS] = {false};
      struct timeval t[3];

      uint32_t sf_idx = isrran_ue_sync_get_sfidx(&ue_sync);
      ...
      switch (state) {
        case DECODE_MIB:
          if (sf_idx == 0) {
            ...
            int     sfn_offset;
            n = isrran_ue_mib_decode(&ue_mib, bch_payload, NULL, &sfn_offset);
            if (n < 0) {
              ERROR("Error decoding UE MIB");
              exit(-1);
            } else if (n == ISRRAN_UE_MIB_FOUND) {
              isrran_pbch_mib_unpack(bch_payload, &cell, &sfn);
              isrran_cell_fprint(stdout, &cell, sfn);
        ...
        case DECODE_PDSCH:
          ...
          if (prog_args.rnti != ISRRAN_SIRNTI) {
            decode_pdsch = true;
            if (isrran_sfidx_tdd_type(dl_sf.tdd_config, sf_idx) == ISRRAN_TDD_SF_U) {
              decode_pdsch = false;
            }
          } else {
          ...
          if (decode_pdsch) {
            ...
            for (uint32_t tm = 0; tm < 4 && !n; tm++) {
              ...
              ue_dl_cfg.cfg.tm                      = (isrran_tm_t)tm;
              ue_dl_cfg.cfg.pdsch.use_tbs_index_alt = prog_args.enable_256qam;

              if ((ue_dl_cfg.cfg.tm == ISRRAN_TM1 && cell.nof_ports == 1) ||
                  (ue_dl_cfg.cfg.tm > ISRRAN_TM1 && cell.nof_ports > 1)) {
                n = isrran_ue_dl_find_and_decode(&ue_dl, &dl_sf, &ue_dl_cfg, &pdsch_cfg, data, acks);
                if (n > 0) {
                  nof_detected++;
                  last_decoded_tm = tm;
            ...
            if (sf_idx == 5 && prog_args.enable_cfo_ref) {
              isrran_ue_sync_set_cfo_ref(&ue_sync, ue_dl.chest_res.cfo);
            }
            ...
            if (n > 0) {
              ...
              if (prog_args.net_port > 0) {
                if (sf_idx == 1) {
                  isrran_netsink_write(&net_sink, data[0], 1 + (n - 1) / 8);
                } else {
                  // TODO: UDP Data transmission does not work
                  for (uint32_t tb = 0; tb < ISRRAN_MAX_CODEWORDS; tb++) {
                    if (pdsch_cfg.grant.tb[tb].enabled) {
                      isrran_netsink_write(&net_sink, data[tb], 1 + (pdsch_cfg.grant.tb[tb].tbs - 1) / 8);
                    }
                  }
                }
              }
              ...
              if (pdsch_cfg.dci.cw[0].mcs_idx != old_dl_dci.cw[0].mcs_idx ||
                  memcmp(&pdsch_cfg.dci.type0_alloc, &old_dl_dci.type0_alloc, sizeof(isrran_ra_type0_t)) ||
                  memcmp(&pdsch_cfg.dci.type1_alloc, &old_dl_dci.type1_alloc, sizeof(isrran_ra_type1_t)) ||
                  memcmp(&pdsch_cfg.dci.type2_alloc, &old_dl_dci.type2_alloc, sizeof(isrran_ra_type2_t))) {
                old_dl_dci = pdsch_cfg.dci;
                fflush(stdout);
                printf("DCI %s\n", isrran_dci_format_string(pdsch_cfg.dci.dci_format));
                isrran_ra_pdsch_fprint(stdout, &old_dl_dci, cell.nof_prb);
              }
#endif
            }
            ...
            rsrq              = ISRRAN_VEC_EMA(ue_dl.chest_res.rsrp_dbm, rsrq, 0.1f);
            rsrp0             = ISRRAN_VEC_EMA(ue_dl.chest_res.rsrp_port_dbm[0], rsrp0, 0.05f);
            rsrp1             = ISRRAN_VEC_EMA(ue_dl.chest_res.rsrp_port_dbm[1], rsrp1, 0.05f);
            snr               = ISRRAN_VEC_EMA(ue_dl.chest_res.snr_db, snr, 0.05f);
            enodebrate        = ISRRAN_VEC_EMA(enb_bits / 1000.0f, enodebrate, 0.05f);
            uerate            = ISRRAN_VEC_EMA(ue_bits / 1000.0f, uerate, 0.001f);
            if (chest_pdsch_cfg.sync_error_enable) {
              for (uint32_t i = 0; i < cell.nof_ports; i++) {
                for (uint32_t j = 0; j < prog_args.rf_nof_rx_ant; j++) {
                  sync_err[i][j] = ISRRAN_VEC_EMA(ue_dl.chest.sync_err[i][j], sync_err[i][j], 0.001f);
            ...
            if (elapsed != 0.0f) {
              procrate = ISRRAN_VEC_EMA(ue_bits / elapsed, procrate, 0.01f);
            }
          ...
          if (sf_idx == 5) {
            ...
            if (gain < 0) {
              gain = isrran_convert_power_to_dB(isrran_agc_get_gain(&ue_sync.agc));
            }

            /* Print transmission scheme */

            /* Print basic Parameters */
            PRINT_LINE("          CFO: %+7.2f Hz", isrran_ue_sync_get_cfo(&ue_sync));
            ...
            if (cell.nof_ports > 1 && ue_dl.pdsch.nof_rx_antennas > 1) {
              ...
              if (isrran_ue_dl_select_ri(&ue_dl, &ri, &cn)) {
      ...
      printf("Finding PSS... Peak: %8.1f, FrameCnt: %d, State: %d\r",
             isrran_sync_get_peak_value(&ue_sync.sfind),
             ue_sync.frame_total_cnt,
             ue_sync.state);
#ifdef ENABLE_GUI
      if (!prog_args.disable_plots) {
        plot_sf_idx = isrran_ue_sync_get_sfidx(&ue_sync);
        plot_track  = false;
        sem_post(&plot_sem);
      }
#endif /* ENABLE_GUI */
...
{
  int      i;
  uint32_t nof_re = ISRRAN_SF_LEN_RE(ue_dl.cell.nof_prb, ue_dl.cell.cp);
  ...
  while (1) {
    ...
    if (!prog_args.disable_plots_except_constellation) {
      for (i = 0; i < nof_re; i++) {
        tmp_plot[i] = isrran_convert_amplitude_to_dB(cabsf(ue_dl.sf_symbols[0][i]));
        if (isinf(tmp_plot[i])) {
          tmp_plot[i] = -80;
        }
      }
      int sz = isrran_symbol_sz(ue_dl.cell.nof_prb);
      if (sz > 0) {
        isrran_vec_f_zero(tmp_plot2, sz);
      }
      int g = (sz - 12 * ue_dl.cell.nof_prb) / 2;
      for (i = 0; i < 12 * ue_dl.cell.nof_prb; i++) {
        tmp_plot2[g + i] = isrran_convert_amplitude_to_dB(cabsf(ue_dl.chest_res.ce[0][0][i]));
      ...
      if (!prog_args.input_file_name) {
        if (plot_track) {
          isrran_pss_t* pss_obj = isrran_sync_get_cur_pss_obj(&ue_sync.strack);
          int           max = isrran_vec_max_fi(pss_obj->conv_output_avg, pss_obj->frame_size + pss_obj->fft_size - 1);
          isrran_vec_sc_prod_fff(pss_obj->conv_output_avg,
          ...
          int max = isrran_vec_max_fi(ue_sync.sfind.pss.conv_output_avg,
                                      ue_sync.sfind.pss.frame_size + ue_sync.sfind.pss.fft_size - 1);
          isrran_vec_sc_prod_fff(ue_sync.sfind.pss.conv_output_avg,
    ...
    if (plot_sf_idx == 1) {
      if (prog_args.net_port_signal > 0) {
        isrran_netsink_write(
            &net_sink_signal, &sf_buffer[isrran_ue_sync_sf_len(&ue_sync) / 7], isrran_ue_sync_sf_len(&ue_sync));
      }
    }
  }
  return NULL;
