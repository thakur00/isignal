...
int main(int argc, char** argv)
{
  ...
  while ((subframe_count < nof_subframes || nof_subframes == -1) && !stop_capture) {
    int n = isrran_ue_sync_nbiot_zerocopy_multi(&ue_sync, buff_ptrs);
    if (n < 0) {
    ...
    if (n == 1) {
      if (!start_capture) {
        ...
        isrran_filesink_write(&sink, buff_ptrs[0], ISRRAN_SF_LEN_PRB(nof_prb));
        subframe_count++;
      }
    }
    if (!keep_running) {
      if (!start_capture || (start_capture && isrran_ue_sync_nbiot_get_sfidx(&ue_sync) == 9)) {
