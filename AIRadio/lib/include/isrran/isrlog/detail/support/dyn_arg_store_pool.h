...
#ifndef ISRLOG_DETAIL_SUPPORT_DYN_ARG_STORE_POOL_H
#define ISRLOG_DETAIL_SUPPORT_DYN_ARG_STORE_POOL_H

#include "isrran/isrlog/bundled/fmt/printf.h"
...
public:
  dyn_arg_store_pool()
  {
    pool.resize(ISRLOG_QUEUE_CAPACITY);
    for (auto& elem : pool) {
      // Reserve for 10 normal and 2 named arguments.
      elem.reserve(10, 2);
    }
    free_list.reserve(ISRLOG_QUEUE_CAPACITY);
    for (auto& elem : pool) {
...
#endif // ISRLOG_DETAIL_SUPPORT_DYN_ARG_STORE_POOL_H
