...
#ifndef ISRASN_COMMON_UTILS_H
#define ISRASN_COMMON_UTILS_H
...
{
  isrlog::fetch_basic_logger("ASN1").error(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_warning(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("ASN1").warning(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_info(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("ASN1").info(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_debug(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("ASN1").debug(format, std::forward<Args>(args)...);
}
...
{
  if (isrran_unlikely(current_type.value != access_type)) {
...
enum ISRASN_CODE { ISRASN_SUCCESS, ISRASN_ERROR_ENCODE_FAIL, ISRASN_ERROR_DECODE_FAIL };

void log_error_code(ISRASN_CODE code, const char* filename, int line);

#define HANDLE_CODE(ret)                                                                                               \
  do {                                                                                                                 \
    ISRASN_CODE macrocode = ((ret));                                                                                   \
    if (macrocode != ISRASN_SUCCESS) {                                                                                 \
...
struct ValOrError {
  uint32_t    val;
  ISRASN_CODE code;
  ValOrError() : val(0), code(ISRASN_SUCCESS) {}
  ValOrError(uint32_t val_, ISRASN_CODE code_) : val(val_), code(code_) {}
};
...
public:
  ...
  template <class T>
  ISRASN_CODE unpack(T& val, uint32_t n_bits)
  {
    return unpack_bits(val, ptr, offset, max_ptr, n_bits);
  }
  ISRASN_CODE unpack_bytes(uint8_t* buf, uint32_t n_bytes);
  ISRASN_CODE align_bytes();
  ISRASN_CODE advance_bits(uint32_t n_bits);
...
public:
  ...
  ISRASN_CODE pack(uint64_t val, uint32_t n_bits);
  ISRASN_CODE pack_bytes(const uint8_t* buf, uint32_t n_bytes);
  ISRASN_CODE align_bytes_zero();
};
...
public:
  ...
  void resize(uint32_t new_size, uint32_t new_cap = 0)
  {
    ...
    if (cap_ > 0) {
      data_ = new T[cap_];
      if (old_data != NULL) {
        isrran_assert(cap_ > size_, "Old size larger than new capacity in dyn_array\n");
...
struct asn1_null_t {
  ISRASN_CODE pack(bit_ref& bref) const { return ISRASN_SUCCESS; }
  ISRASN_CODE unpack(cbit_ref& bref) const { return ISRASN_SUCCESS; }
};
...
ISRASN_CODE pack_enum(bit_ref& bref, EnumType e)
{
  return pack_enum(bref, e, EnumType::nof_types, EnumType::nof_exts, EnumType::has_ext);
}
template <typename EnumType>
ISRASN_CODE unpack_enum(EnumType& e, cbit_ref& bref)
{
...
struct EnumPacker {
  ...
  ISRASN_CODE pack(bit_ref& bref, EnumType e)
  {
    return pack_enum(bref, e);
  }
  template <class EnumType>
  ISRASN_CODE unpack(EnumType& e, cbit_ref& bref)
  {
...
public:
  ...
  ISRASN_CODE pack(bit_ref& bref) const { return pack_enum(bref, *this); }
  ISRASN_CODE unpack(cbit_ref& bref) { return unpack_enum(*this, bref); }
...
template <typename IntType>
ISRASN_CODE unpack_unconstrained_whole_number(IntType& n, cbit_ref& bref, bool aligned);
...
template <typename IntType>
ISRASN_CODE pack_length(bit_ref& bref, IntType n, IntType lb, IntType ub, bool aligned = false);
template <typename IntType>
ISRASN_CODE unpack_length(IntType& n, cbit_ref& bref, IntType lb, IntType ub, bool aligned = false);

// Pack as a small non-negative whole number
ISRASN_CODE pack_length(bit_ref& ref, uint32_t val, bool aligned = false);
ISRASN_CODE unpack_length(uint32_t& val, cbit_ref& ref, bool aligned = false);
...
template <typename IntType>
ISRASN_CODE pack_integer(bit_ref& bref,
                         IntType  n,
                         ...
                         bool     aligned = false);
template <typename IntType>
ISRASN_CODE unpack_integer(IntType&  n,
                           cbit_ref& bref,
                           IntType   lb      = std::numeric_limits<IntType>::min(),
                           IntType   ub      = std::numeric_limits<IntType>::max(),
                           bool      has_ext = false,
...
// unconstrained case
template <typename IntType>
ISRASN_CODE pack_unconstrained_integer(bit_ref& bref, IntType n, bool has_ext = false, bool aligned = false);
template <typename IntType>
ISRASN_CODE unpack_unconstrained_integer(IntType& n, cbit_ref& bref, bool has_ext = false, bool aligned = false);

template <class IntType>
struct integer_packer {
  integer_packer(IntType lb_, IntType ub_, bool has_ext_ = false, bool aligned_ = false);
  ISRASN_CODE pack(bit_ref& bref, IntType n);
  ISRASN_CODE unpack(IntType& n, cbit_ref& bref);
  IntType     lb;
  IntType     ub;
  bool        has_ext;
  bool        aligned;
...
public:
  ...
  ISRASN_CODE pack(bit_ref& bref) const { return pack_integer(bref, value, lb, ub, has_ext, is_aligned); }
  ISRASN_CODE unpack(cbit_ref& bref) { return unpack_integer(value, bref, lb, ub, has_ext, is_aligned); }
};
...
struct BitPacker {
  ...
  template <typename T>
  ISRASN_CODE pack(bit_ref& bref, const T& topack)
  {
    bref.pack(topack, nof_bits);
    return ISRASN_SUCCESS;
  }
  template <typename T>
  ISRASN_CODE unpack(T& tounpack, cbit_ref& bref)
  {
...
ISRASN_CODE fixed_octstring<N, aligned>::pack(bit_ref& bref) const
{
  if (aligned and N > 2) {
...
template <uint32_t N, bool aligned>
ISRASN_CODE fixed_octstring<N, aligned>::unpack(cbit_ref& bref)
{
  if (aligned and N > 2) {
    bref.align_bytes();
  }
...
public:
  ...
  ISRASN_CODE              pack(bit_ref& ie_ref) const;
  ISRASN_CODE              unpack(cbit_ref& ie_ref);
...
ISRASN_CODE pack_fixed_seq_of(bit_ref& bref, const T* item_array, uint32_t nof_items, ItemPacker packer)
{
...
template <class T>
ISRASN_CODE pack_fixed_seq_of(bit_ref& bref, const T* item_array, uint32_t nof_items)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
ISRASN_CODE unpack_fixed_seq_of(T* item_array, cbit_ref& bref, uint32_t nof_items, ItemUnpacker unpacker)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
template <class T>
ISRASN_CODE unpack_fixed_seq_of(T* item_array, cbit_ref& bref, uint32_t nof_items)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
struct FixedSeqOfPacker {
  ...
  template <typename T>
  ISRASN_CODE unpack(T* tounpack, bit_ref& bref)
  {
...
ISRASN_CODE pack_dyn_seq_of(bit_ref&         bref,
...
ISRASN_CODE pack_dyn_seq_of(bit_ref& bref, const ArrayType& seqof, uint32_t lb, uint32_t ub, bool aligned = false)
{
  HANDLE_CODE(pack_length(bref, seqof.size(), lb, ub, aligned));
  for (uint32_t i = 0; i < seqof.size(); ++i) {
...
ISRASN_CODE unpack_dyn_seq_of(ArrayType&   seqof,
                              cbit_ref&    bref,
                              uint32_t     lb,
                              uint32_t     ub,
...
template <class ArrayType>
ISRASN_CODE unpack_dyn_seq_of(ArrayType& seqof, cbit_ref& bref, uint32_t lb, uint32_t ub, bool aligned = false)
{
  uint32_t nof_items;
  HANDLE_CODE(unpack_length(nof_items, bref, lb, ub, aligned));
  seqof.resize(nof_items);
...
struct SeqOfPacker {
  ...
  template <typename T>
  ISRASN_CODE pack(bit_ref& bref, const T& topack) const
  {
...
struct dyn_seq_of : public dyn_array<ItemType> {
  ...
  ISRASN_CODE pack(bit_ref& bref) const { return pack_dyn_seq_of(bref, *this, lb, ub, aligned); }
  ISRASN_CODE unpack(cbit_ref& bref) { return unpack_dyn_seq_of(*this, bref, lb, ub, aligned); }
};
...
{
public:
  ISRASN_CODE pack(bit_ref& bref) const { return asn_string_utils::pack(bref, str, LB, UB, ALB, AUB, ext, aligned); }
  ISRASN_CODE unpack(cbit_ref& bref) { return asn_string_utils::unpack(str, bref, LB, UB, ALB, AUB, ext, aligned); }
...
private:
  using byte_array_t   = std::array<uint8_t, isrran::byte_buffer_pool::BLOCK_SIZE>;
  using byte_array_ptr = isrran::any_pool_ptr<byte_array_t>;
...
int test_pack_unpack_consistency(const Msg& msg)
{
  ...
  if (msg.pack(bref) != asn1::ISRASN_SUCCESS) {
    log_error_code(ISRASN_ERROR_ENCODE_FAIL, __FILE__, __LINE__);
    return -1;
  }
  if (msg2.unpack(bref2) != asn1::ISRASN_SUCCESS) {
    log_error_code(ISRASN_ERROR_DECODE_FAIL, __FILE__, __LINE__);
    return -1;
  }
  if (msg2.pack(bref3) != asn1::ISRASN_SUCCESS) {
    log_error_code(ISRASN_ERROR_ENCODE_FAIL, __FILE__, __LINE__);
    return -1;
  }
