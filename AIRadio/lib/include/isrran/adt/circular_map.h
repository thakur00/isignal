...
#ifndef ISRRAN_ID_MAP_H
#define ISRRAN_ID_MAP_H
...
public:
  ...
  public:
    ...
    obj_t& operator*()
    {
      isrran_assert(idx < ptr->capacity(), "Iterator out-of-bounds (%zd >= %zd)", idx, ptr->capacity());
  ...
  isrran::expected<iterator, T> insert(K id, T&& obj)
  {
    size_t idx = id % N;
    if (present[idx]) {
      return isrran::expected<iterator, T>(std::move(obj));
    }
  ...
  {
    isrran_assert(it.idx < N and it.ptr == this, "Iterator out-of-bounds (%zd >= %zd)", it.idx, N);
    iterator next = it;
    ++next;
  ...
  T& operator[](K id)
  {
    isrran_assert(contains(id), "Accessing non-existent ID=%zd", (size_t)id);
