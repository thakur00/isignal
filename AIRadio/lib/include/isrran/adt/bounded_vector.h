...
#ifndef ISRRAN_BOUNDED_VECTOR_H
#define ISRRAN_BOUNDED_VECTOR_H

#include "isrran/adt/detail/type_storage.h"
...
public:
  ...
  {
    isrran_assert(i < size_, "Array index is out of bounds.");
    return buffer[i].get();
  }
  ...
  T& back()
  {
    isrran_assert(size_ > 0, "Trying to get back of empty array.");
  ...
  {
    isrran_assert(pos >= this->begin(), "Iterator to erase is out of bounds.");
    isrran_assert(pos < this->end(), "Erasing at past-the-end iterator.");
    iterator ret = pos;
  ...
  {
    isrran_assert(it_start >= begin(), "Range to erase is out of bounds.");
    isrran_assert(it_start <= it_end, "Trying to erase invalid range.");
    isrran_assert(it_end <= end(), "Trying to erase past the end.");
  ...
  void push_back(const T& value)
  {
    ...
    size_++;
    isrran_assert(size_ <= MAX_N, "bounded vector maximum size=%zd was exceeded", MAX_N);
    new (&back()) T(value);
  }
  ...
  void pop_back()
  {
    isrran_assert(size_ > 0, "Trying to erase element from empty vector.");
    back().~T();
    size_--;
  }
...
private:
  ...
  void append(const_iterator it_begin, const_iterator it_end)
  {
    ...
    isrran_assert(N + size_ <= MAX_N, "bounded vector maximum size=%zd was exceeded", MAX_N);
