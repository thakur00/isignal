...
#ifndef ISRRAN_OPTIONAL_ARRAY_H
#define ISRRAN_OPTIONAL_ARRAY_H
...
public:
  ...
  {
    isrran_assert(idx < this->vec.size(), "Out-of-bounds access to array: %zd>=%zd", idx, this->vec.size());
...
template <typename T>
class optional_span : public detail::base_optional_span<isrran::span<optional<T> > >
{
  using base_t = detail::base_optional_span<isrran::span<optional<T> > >;
...
public:
  ...
  {
    isrran_assert(contains(idx), "Access to inexistent element of index=%zd", idx);
    return ptr[idx];
  }
  T& at(size_t idx)
  {
    isrran_assert(this->contains(idx), "Access to inexistent element of index=%zd", idx);
    return this->ptr[idx];
  }
...
public:
  ...
  void insert(size_t idx, U&& u)
  {
    isrran_assert(idx < this->len, "Out-of-bounds access to array: %zd>=%zd", idx, this->len);
