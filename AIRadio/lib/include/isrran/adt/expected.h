...
#ifndef ISRRAN_EXPECTED_H
#define ISRRAN_EXPECTED_H
...
public:
  ...
  const T& value() const
  {
    isrran_assert(has_value(), "Bad expected<T> value access");
    return val;
  }
  T& value()
  {
  ...
  const E& error() const
  {
    isrran_assert(not has_value(), "Bad expected<T> error access");
    return unexpected;
  }
  E& error()
  {
