...
#ifndef ISRRAN_DYN_BITSET_H
#define ISRRAN_DYN_BITSET_H
...
public:
  ...
  {
    isrran_assert(new_size <= max_size(), "ERROR: new size=%zd exceeds bitset capacity=%zd", new_size, max_size());
  ...
  {
    isrran_assert(nof_words_() == 1, "ERROR: cannot convert bitset of size=%zd to uint64_t", size());
    return get_word_(0);
  }
...
private:
  ...
  {
    isrran_assert(pos < size() or (not strict and pos == size()),
  ...
  {
    isrran_assert(startpos <= endpos and endpos <= size(),
...
template <size_t N, bool reversed>
struct formatter<isrran::bounded_bitset<N, reversed> > {
  ...
  auto format(const isrran::bounded_bitset<N, reversed>& s, FormatContext& ctx)
