...
#ifndef ISRRAN_CIRCULAR_BUFFER_H
#define ISRRAN_CIRCULAR_BUFFER_H

#include "isrran/adt/detail/type_storage.h"
#include "isrran/adt/expected.h"
#include "isrran/adt/pool/pool_utils.h"
...
class base_circular_buffer
{
  ...
  private:
    ...
    {
      isrran_assert(idx + (idx >= parent->rpos ? 0 : parent->max_size()) < parent->rpos + parent->count,
...
public:
  ...
  {
    isrran_assert(not full(), "Circular buffer is full.");
  ...
  void pop()
  {
    isrran_assert(not empty(), "Cannot call pop() in empty circular buffer");
    buffer[rpos].destroy();
  ...
  T& top()
  {
    isrran_assert(not empty(), "Cannot call top() in empty circular buffer");
  ...
  T& operator[](size_t i)
  {
    isrran_assert(i < count, "Out-of-bounds access to circular buffer (%zd >= %zd)", i, count);
...
public:
  ...
  isrran::error_type<T> try_push(T&& t) { return push_(std::move(t), false); }
  bool                  push_blocking(const T& t) { return push_(t, true); }
  isrran::error_type<T> push_blocking(T&& t) { return push_(std::move(t), true); }
...
protected:
  ...
  isrran::error_type<T> push_(T&& t, bool block_mode)
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (not active) {
      return std::move(t);
...
public:
  ...
  {
    isrran_assert(base_t::empty() or sz == base_t::size(),
...
#endif // ISRRAN_CIRCULAR_BUFFER_H
