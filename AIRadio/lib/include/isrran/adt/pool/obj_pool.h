...
#ifndef ISRRAN_OBJ_POOL_H
#define ISRRAN_OBJ_POOL_H
...
public:
  using init_mem_oper_t = isrran::move_callback<void(void*)>;
  using recycle_oper_t  = isrran::move_callback<void(T&)>;
  ...
  void clear()
  {
    if (not allocated.empty()) {
      isrran_assert(allocated.size() * objs_per_batch == cache_size(),
...
public:
  ...
  {
    isrran_assert(thres_ > 1, "The provided threshold=%zd is not valid", thres_);
  }
  ~background_obj_pool()
  {
