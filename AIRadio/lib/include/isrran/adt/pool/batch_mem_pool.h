...
#ifndef ISRRAN_BATCH_MEM_POOL_H
#define ISRRAN_BATCH_MEM_POOL_H
...
public:
  ...
  {
    isrran_assert(cache_size() == size(), "Not all nodes have been deallocated yet (%zd < %zd)", cache_size(), size());
  }
...
public:
  ...
  {
    isrran_assert(batch_threshold > 1, "Invalid arguments for background memory pool");
  }
  ~background_mem_pool()
  {
  ...
  {
    isrran_assert(sz <= grow_pool.get_node_max_size(),
