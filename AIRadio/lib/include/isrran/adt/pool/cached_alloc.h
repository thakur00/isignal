...
#ifndef ISRRAN_CACHED_ALLOC_H
#define ISRRAN_CACHED_ALLOC_H
...
bool operator==(const isrran::cached_alloc<T1>& lhs, const isrran::cached_alloc<T2>& rhs) noexcept
{
  return &lhs == &rhs;
}
...
template <typename T>
using queue = std::queue<T, isrran::deque<T> >;

} // namespace isrran

#endif // ISRRAN_CACHED_ALLOC_H
