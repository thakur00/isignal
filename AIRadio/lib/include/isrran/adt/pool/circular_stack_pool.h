...
#ifndef ISRRAN_CIRCULAR_MAP_STACK_POOL_H
#define ISRRAN_CIRCULAR_MAP_STACK_POOL_H
...
public:
  ...
  ~circular_stack_pool()
  {
    for (mem_block_elem_t& elem : pools) {
      ...
      isrran_expect(elem.count == 0, "There are missing deallocations for stack id=%zd", elem.key);
...
private:
  isrran::circular_array<mem_block_elem_t, NofStacks> pools;
  isrran::background_mem_pool                         central_cache;
...
#endif // ISRRAN_CIRCULAR_MAP_STACK_POOL_H
