...
#ifndef ISRRAN_MEMBLOCK_CACHE_H
#define ISRRAN_MEMBLOCK_CACHE_H
...
public:
  ...
  {
    isrran_assert(is_aligned(block, min_memblock_align()), "The provided memory block is not aligned");
  ...
  void* pop() noexcept
  {
    isrran_assert(not empty(), "pop() called on empty list");
    node* last_head = head;
    head            = head->next;
    last_head->~node();
    count--;
...
public:
  ...
  {
    isrran_assert(node_size_ > 0 and is_valid_alignment(node_alignment_),
  ...
  {
    isrran_assert(is_aligned(payload_addr, memblock_alignment), "Provided address is not valid");
  ...
  {
    isrran_assert(not other.empty(), "Trying to steal from empty memblock list");
