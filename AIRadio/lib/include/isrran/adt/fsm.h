...
#include "isrran/adt/detail/type_utils.h"
#include "isrran/adt/move_callback.h"
...
struct state_traits {
  ...
  using state_t   = State;
  using is_subfsm = std::integral_constant<bool, ::isrran::is_composite_fsm<State>::value>;
...
public:
  ...
  struct state_list : public std::tuple<States...> {
    ...
    {
      return isrran::type_list_contains<State, States...>();
    }
  ...
  std::string current_state_name() const
  {
    ...
    isrran::visit(visitor, derived()->states);
    return visitor.name;
  }
...
public:
  ...
  explicit fsm_t(isrlog::basic_logger& logger) : logger(logger) {}
  ...
  void set_fsm_event_log_level(isrlog::basic_levels lvl) { log_level = lvl; }

  isrlog::basic_logger& get_logger() const { return logger; }
...
protected:
  ...
  isrlog::basic_levels                       log_level      = isrlog::basic_levels::info;
  bool                                       trigger_locked = false;
  std::deque<isrran::move_callback<void()> > pending_events;
};
...
public:
  ...
  isrlog::basic_logger& get_logger() const { return parent_fsm()->get_logger(); }
...
public:
  ...
  using launch_ev   = isrran::proc_launch_ev<Arg>;
  using complete_ev = isrran::proc_complete_ev<Result>;
  ...
  explicit proc_fsm_t(isrlog::basic_logger& logger) : fsm_t<Derived>(logger) {}

  bool is_running() const { return not base_t::template is_in_state<idle_st>(); }

  const Result& get_result() const
  {
    isrran_assert(launch_counter > 0 and base_t::template is_in_state<idle_st>(), "in proc_fsm_t::get_result");
    return last_result;
  }
...
public:
  ...
  void enter(FSM* f, const Ev& ev)
  {
    ...
    proc_ptr->trigger(isrran::proc_launch_ev<Ev>{ev});
    proc_ptr->await(f);
  }
...
struct event_callback {
  ...
  isrran::move_task_t to_move_task(const Event& ev)
  {
...
template <typename Event>
isrran::move_task_t make_move_task(const event_callback<Event>& callback, const Event& ev)
{
  auto& copied_callback = callback;
  return [copied_callback, ev]() { copied_callback(ev); };
}
...
template <typename Event>
isrran::move_task_t make_move_task(std::vector<event_callback<Event> >&& callbacks, const Event& ev)
{
  return std::bind(
