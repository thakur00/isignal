...
{
public:
  pdu(uint32_t max_subheaders_, isrlog::basic_logger& logger) :
...
public:
  ...
  bool set_scell_activation_cmd(const std::array<bool, ISRRAN_MAX_CARRIERS>& active_scell_idxs);
...
{
public:
  sch_pdu(uint32_t max_subh, isrlog::basic_logger& logger) : pdu(max_subh, logger) {}

  void     parse_packet(uint8_t* ptr);
  uint8_t* write_packet();
  uint8_t* write_packet(isrlog::basic_logger& log);
...
{
public:
  rar_pdu(uint32_t max_rars = 16, isrlog::basic_logger& logger = isrlog::fetch_basic_logger("MAC"));
...
{
public:
  mch_pdu(uint32_t max_subh, isrlog::basic_logger& logger) : sch_pdu(max_subh, logger) {}
