...
#ifndef ISRRAN_RADIO_H
#define ISRRAN_RADIO_H
 ...
 * ISRRAN_MAX_CHANNELS). Note that all carriers must have the same number of antennas.
 *
 ...
 */
class radio : public radio_interface_phy, public isrran::radio_base
{
public:
  radio();
  ~radio();
  ...
  void        handle_rf_msg(isrran_rf_error_t error);
  static void rf_msg_callback(void* arg, isrran_rf_error_t error);

private:
  std::vector<isrran_rf_t>                                rf_devices  = {};
  std::vector<isrran_rf_info_t>                           rf_info     = {};
  ...
  std::array<std::vector<cf_t>, ISRRAN_MAX_CHANNELS>      dummy_buffers;
  std::mutex                                              tx_mutex;
  std::mutex                                              rx_mutex;
  std::array<std::vector<cf_t>, ISRRAN_MAX_CHANNELS>      tx_buffer;
  std::array<std::vector<cf_t>, ISRRAN_MAX_CHANNELS>      rx_buffer;
  std::array<isrran_resampler_fft_t, ISRRAN_MAX_CHANNELS> interpolators = {};
  std::array<isrran_resampler_fft_t, ISRRAN_MAX_CHANNELS> decimators    = {};
   ...
   */
  bool tx_dev(const uint32_t& device_idx, rf_buffer_interface& buffer, const isrran_timestamp_t& tx_time_);
   ...
   */
  bool rx_dev(const uint32_t& device_idx, const rf_buffer_interface& buffer, isrran_timestamp_t* rxd_time);
