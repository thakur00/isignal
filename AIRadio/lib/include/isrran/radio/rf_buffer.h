...
#ifndef ISRRAN_RF_BUFFER_H
#define ISRRAN_RF_BUFFER_H
...
public:
  ...
  explicit rf_buffer_t(uint32_t nof_subframes_)
  {
    if (nof_subframes_ > 0) {
      ...
      for (uint32_t i = 0; i < ISRRAN_MAX_CHANNELS; i++) {
        sample_buffer[i] = isrran_vec_cf_malloc(nof_subframes_ * ISRRAN_SF_LEN_MAX);
        isrran_vec_cf_zero(sample_buffer[i], ISRRAN_SF_LEN_MAX);
      }
      allocated     = true;
   ...
   * contain up to ISRRAN_MAX_CHANNELS pointers
   * @param data Flat array to use as initializer for the internal buffer pointers
   */
  explicit rf_buffer_t(cf_t* data[ISRRAN_MAX_CHANNELS], uint32_t nof_samples_)
  {
    for (uint32_t i = 0; i < ISRRAN_MAX_CHANNELS; i++) {
  ...
  void set_combine(const uint32_t& channel_idx, cf_t* ptr)
  {
    if (sample_buffer.at(channel_idx) == nullptr) {
      ...
      isrran_vec_sum_ccc(ptr, sample_buffer.at(channel_idx), sample_buffer.at(channel_idx), nof_samples);
    }
  }
