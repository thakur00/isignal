...
#ifndef ISRRAN_SECURITY_H
#define ISRRAN_SECURITY_H
...
{
  isrlog::fetch_basic_logger("SEC").error(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_warning(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("SEC").warning(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_info(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("SEC").info(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_debug(const char* format, Args&&... args)
{
  isrlog::fetch_basic_logger("SEC").debug(format, std::forward<Args>(args)...);
}
