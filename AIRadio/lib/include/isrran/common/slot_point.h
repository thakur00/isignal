...
#ifndef ISRRAN_SLOT_POINT_H
#define ISRRAN_SLOT_POINT_H
...
public:
  ...
  {
    isrran_assert(numerology < NOF_NUMEROLOGIES, "Invalid numerology idx=%d passed", (int)numerology);
    isrran_assert(count < nof_slots_per_hf(), "Invalid slot count=%d passed", (int)count);
  }
  ...
  {
    isrran_assert(numerology < NOF_NUMEROLOGIES, "Invalid numerology idx=%d passed", (int)numerology);
    isrran_assert(sfn_val < NOF_SFNS, "Invalid SFN=%d provided", (int)sfn_val);
    isrran_assert(slot < nof_slots_per_frame(),
  ...
  {
    isrran_assert(numerology_idx() == other.numerology_idx(), "Comparing slots of different numerologies");
...
using slot_interval = isrran::interval<slot_point>;

} // namespace isrran

namespace fmt {
template <>
struct formatter<isrran::slot_point> {
  ...
  auto format(isrran::slot_point slot, FormatContext& ctx) -> decltype(std::declval<FormatContext>().out())
  {
...
using slot_point    = isrran::slot_point;
using slot_interval = isrran::slot_interval;

} // namespace isrenb

#endif // ISRRAN_SLOT_POINT_H
