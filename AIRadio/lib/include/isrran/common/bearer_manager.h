...
#ifndef ISRRAN_BEARER_MANAGER_H
#define ISRRAN_BEARER_MANAGER_H
...
/**
 * @brief Implementation of UE bearer manager internal functionality that is common to both isrue and
 * isrenb applications
 */
...
public:
  struct radio_bearer_t {
    isrran::isrran_rat_t rat;
    uint32_t             lcid;
    uint32_t             eps_bearer_id;
    uint32_t             five_qi = 0;
    bool                 is_valid() const { return rat != isrran_rat_t::nulltype; }
  };
  static const radio_bearer_t invalid_rb;

  /// Registers EPS bearer with PDCP RAT type and LCID
  bool add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid);
...
{
public:
  using radio_bearer_t = isrran::detail::ue_bearer_manager_impl::radio_bearer_t;
  ...
  // RRC interface
  /// Registers EPS bearer with PDCP RAT type and LCID
  void add_eps_bearer(uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid);
  ...
  bool has_active_radio_bearer(uint32_t eps_bearer_id)
  {
    isrran::rwlock_read_guard rw_lock(rwlock);
...
private:
  ...
  isrran::detail::ue_bearer_manager_impl impl;
};
...
public:
  ...
  void           add_eps_bearer(uint16_t rnti, uint8_t eps_bearer_id, isrran::isrran_rat_t rat, uint32_t lcid);
...
private:
  ...
  std::unordered_map<uint16_t, isrran::detail::ue_bearer_manager_impl> users_map;
};

} // namespace isrenb

#endif // ISRRAN_BEARER_MANAGER_H
