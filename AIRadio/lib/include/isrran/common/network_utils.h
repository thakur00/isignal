...
#ifndef ISRRAN_RX_SOCKET_HANDLER_H
#define ISRRAN_RX_SOCKET_HANDLER_H
...
public:
  ...
  using recv_callback_t = isrran::move_callback<bool(int)>;

  explicit socket_manager_itf(isrlog::basic_logger& logger_) : logger(logger_) {}
...
using sctp_recv_callback_t =
    isrran::move_callback<void(isrran::unique_byte_buffer_t, const sockaddr_in&, const sctp_sndrcvinfo&, int)>;

/// Function signature for SDU byte buffers received from any sockaddr_in-based socket
using recvfrom_callback_t = isrran::move_callback<void(isrran::unique_byte_buffer_t, const sockaddr_in&)>;
...
#endif // ISRRAN_RX_SOCKET_HANDLER_H
