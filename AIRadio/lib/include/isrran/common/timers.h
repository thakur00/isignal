...
#ifndef ISRRAN_TIMERS_H
#define ISRRAN_TIMERS_H
...
class timer_handler
{
  ...
  struct timer_impl : public intrusive_double_linked_list_element<>, public intrusive_forward_list_element<> {
    ...
    {
      isrran_assert(duration_ <= MAX_TIMER_DURATION,
    ...
    void set(uint32_t duration_, isrran::move_callback<void(uint32_t)> callback_)
    {
      isrran_assert(duration_ <= MAX_TIMER_DURATION,
                    "Invalid timer duration=%" PRIu32 ">%" PRIu32,
                    duration_,
...
public:
  ...
  public:
    ...
    {
      isrran_assert(is_valid(), "Trying to setup empty timer handle");
    ...
    void run()
    {
      isrran_assert(is_valid(), "Starting invalid timer");
      handle->run();
    }
  ...
  void defer_callback(uint32_t duration, const F& func)
  {
    ...
    isrran::move_callback<void(uint32_t)> c     = [func, &timer](uint32_t tid) {
      func();
      // auto-deletes timer
      timer.deallocate();
    };
...
private:
  timer_impl& alloc_timer()
  {
    ...
    if (not free_list.empty()) {
      t = &free_list.front();
      isrran_assert(not t->allocated, "Invalid timer id=%d state", t->id);
      free_list.pop_front();
      nof_free_timers--;
    } else {
  ...
  void dealloc_timer_(timer_impl& timer)
  {
    ...
    timer.callback = isrran::move_callback<void(uint32_t)>();
  ...
  isrran::intrusive_forward_list<timer_impl>                     free_list;
  std::vector<isrran::intrusive_double_linked_list<timer_impl> > time_wheel;
