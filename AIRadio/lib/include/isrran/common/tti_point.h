...
#ifndef ISRRAN_TTI_POINT_H
#define ISRRAN_TTI_POINT_H
...
struct tti_point {
  ...
  explicit tti_point(uint32_t tti_val_) : tti_val(tti_val_ % 10240U)
  {
    if (tti_val_ > std::numeric_limits<uint32_t>::max() / 2) {
      ...
      isrran_expect(diff < 10240, "Invalid TTI point assigned");
...
using tti_interval = isrran::interval<isrran::tti_point>;

} // namespace isrran

namespace fmt {
template <>
struct formatter<isrran::tti_point> {
  ...
  auto format(isrran::tti_point tti, FormatContext& ctx) -> decltype(std::declval<FormatContext>().out())
  {
...
using tti_point    = isrran::tti_point;
using tti_interval = isrran::tti_interval;
