...
#ifndef ISRRAN_BUFFER_POOL_H
#define ISRRAN_BUFFER_POOL_H
...
#ifdef ISRRAN_BUFFER_POOL_LOG_ENABLED
      if (debug_name) {
        strncpy(b->debug_name, debug_name, ISRRAN_BUFFER_POOL_LOG_NAME_LEN);
        b->debug_name[ISRRAN_BUFFER_POOL_LOG_NAME_LEN - 1] = 0;
      }
#endif
    } else if (blocking) {
      // blocking allocation
...
inline unique_byte_buffer_t make_byte_buffer(const char* debug_ctxt) noexcept
{
  ...
  if (buffer == nullptr) {
    isrlog::fetch_basic_logger("POOL").error("Failed to allocate byte buffer in %s", debug_ctxt);
  }
  return buffer;
}
...
inline unique_byte_buffer_t make_byte_buffer(const uint8_t* payload, uint32_t len, const char* debug_ctxt) noexcept
{
  ...
  if (buffer == nullptr) {
    ...
    if (buffer->get_tailroom() >= len) {
      ...
      isrlog::fetch_basic_logger("POOL").error(
