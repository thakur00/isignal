...
#ifndef ISRRAN_COMMON_H
#define ISRRAN_COMMON_H
...
#define ISRRAN_UE_CATEGORY 4

#define ISRRAN_N_SRB 3
#define ISRRAN_N_DRB 8
#define ISRRAN_N_RADIO_BEARERS 11

#define ISRRAN_N_MCH_LCIDS 32
...
#define ISRRAN_MAX_TBSIZE_BITS 97896
#define ISRRAN_BUFFER_HEADER_OFFSET 1020
#define ISRRAN_MAX_BUFFER_SIZE_BITS (ISRRAN_MAX_TBSIZE_BITS + ISRRAN_BUFFER_HEADER_OFFSET)
#define ISRRAN_MAX_BUFFER_SIZE_BYTES (ISRRAN_MAX_TBSIZE_BITS / 8 + ISRRAN_BUFFER_HEADER_OFFSET)
...
enum class isrran_rat_t { lte, nr, nulltype };
inline std::string to_string(const isrran_rat_t& type)
{
  constexpr static const char* options[] = {"LTE", "NR"};
  return enum_to_text(options, (uint32_t)isrran_rat_t::nulltype, (uint32_t)type);
}

} // namespace isrran

#endif // ISRRAN_COMMON_H
