...
#ifndef ISRRAN_TASK_SCHEDULER_H
#define ISRRAN_TASK_SCHEDULER_H
...
public:
  ...
  isrran::unique_timer get_unique_timer() { return timers.get_unique_timer(); }

  //! Creates new queue for tasks coming from external thread
  isrran::task_queue_handle make_task_queue() { return external_tasks.add_queue(); }
  isrran::task_queue_handle make_task_queue(uint32_t qsize) { return external_tasks.add_queue(qsize); }
  ...
  void defer_task(isrran::move_task_t func)
  {
    if (not internal_tasks.try_push(std::move(func))) {
      isrlog::fetch_basic_logger("COMN", false).warning("Couldn't add new internal task");
    }
  }

  //! Defer the handling of the result of a background task to next tic
  void notify_background_task_result(isrran::move_task_t task)
  {
  ...
  isrran::timer_handler* get_timer_handler() { return &timers; }
...
private:
  ...
  isrran::task_multiqueue   external_tasks;
  isrran::task_queue_handle background_queue; ///< Queue for handling the outcomes of tasks run in the background
  isrran::timer_handler     timers;
  isrran::dyn_blocking_queue<isrran::move_task_t>
...
public:
  ...
  isrran::unique_timer get_unique_timer() { return sched->get_unique_timer(); }
  void                 notify_background_task_result(isrran::move_task_t task)
  {
  ...
  void                      defer_task(isrran::move_task_t func) { sched->defer_task(std::move(func)); }
  isrran::task_queue_handle make_task_queue() { return sched->make_task_queue(); }
