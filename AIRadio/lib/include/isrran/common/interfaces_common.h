...
#ifndef ISRRAN_INTERFACES_COMMON_H
#define ISRRAN_INTERFACES_COMMON_H
...
struct rf_args_t {
  ...
  float       freq_offset;
  float       rx_gain;
  float       rx_gain_ch[ISRRAN_MAX_CARRIERS];
  float       tx_gain;
  float       tx_gain_ch[ISRRAN_MAX_CARRIERS];
  float       tx_max_power;
  ...
  std::array<rf_args_band_t, ISRRAN_MAX_CARRIERS> ch_rx_bands;
  std::array<rf_args_band_t, ISRRAN_MAX_CARRIERS> ch_tx_bands;
...
class isrran_gw_config_t
{
public:
  explicit isrran_gw_config_t(uint32_t lcid_ = 0) : lcid(lcid_) {}
...
#endif // ISRRAN_INTERFACES_COMMON_H
