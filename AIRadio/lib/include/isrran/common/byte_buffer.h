...
#ifndef ISRRAN_BYTE_BUFFER_H
#define ISRRAN_BYTE_BUFFER_H
...
//#define ISRRAN_BUFFER_POOL_LOG_ENABLED
#define ISRRAN_BUFFER_POOL_LOG_NAME_LEN 128
...
public:
  ...
  uint32_t N_bytes = 0;
  uint8_t  buffer[ISRRAN_MAX_BUFFER_SIZE_BYTES];
  uint8_t* msg = nullptr;
  ...
  byte_buffer_t() : msg(&buffer[ISRRAN_BUFFER_HEADER_OFFSET])
  {
#ifdef ISRRAN_BUFFER_POOL_LOG_ENABLED
    bzero(debug_name, ISRRAN_BUFFER_POOL_LOG_NAME_LEN);
#endif
  }
  explicit byte_buffer_t(uint32_t size) : msg(&buffer[ISRRAN_BUFFER_HEADER_OFFSET]), N_bytes(size)
  {
#ifdef ISRRAN_BUFFER_POOL_LOG_ENABLED
    bzero(debug_name, ISRRAN_BUFFER_POOL_LOG_NAME_LEN);
#endif
  ...
  byte_buffer_t(const byte_buffer_t& buf) : msg(&buffer[ISRRAN_BUFFER_HEADER_OFFSET]), md(buf.md), N_bytes(buf.N_bytes)
  {
  ...
  void clear()
  {
    msg     = &buffer[ISRRAN_BUFFER_HEADER_OFFSET];
    N_bytes = 0;
    md      = {};
  }
...
struct bit_buffer_t {
  uint32_t N_bits = 0;
  uint8_t  buffer[ISRRAN_MAX_BUFFER_SIZE_BITS];
  uint8_t* msg = nullptr;
#ifdef ISRRAN_BUFFER_POOL_LOG_ENABLED
  char debug_name[128];
#endif

  bit_buffer_t() : msg(&buffer[ISRRAN_BUFFER_HEADER_OFFSET]) {}
  bit_buffer_t(const bit_buffer_t& buf) : msg(&buffer[ISRRAN_BUFFER_HEADER_OFFSET]), N_bits(buf.N_bits)
  {
