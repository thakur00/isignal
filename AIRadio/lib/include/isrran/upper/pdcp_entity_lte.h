...
#ifndef ISRRAN_PDCP_ENTITY_LTE_H
#define ISRRAN_PDCP_ENTITY_LTE_H
...
#include "isrran/upper/pdcp_entity_base.h"
...
{
public:
  explicit undelivered_sdus_queue(isrran::task_sched_handle task_sched, uint32_t sn_mod);
  ...
  std::map<uint32_t, isrran::unique_byte_buffer_t> get_buffered_sdus();
...
private:
  ...
  struct sdu_data {
    ...
    isrran::unique_timer         discard_timer;
  };
  ...
  uint32_t                                   lms   = 0;
  isrran::circular_array<sdu_data, capacity> sdus;
};
...
{
public:
  pdcp_entity_lte(isrue::rlc_interface_pdcp* rlc_,
  ...
  std::map<uint32_t, isrran::unique_byte_buffer_t> get_buffered_pdus() override;

  // Status report helper(s)
  void send_status_report() override;
  void handle_status_report_pdu(isrran::unique_byte_buffer_t pdu);
...
private:
  ...
  // PDU handlers
  void handle_control_pdu(isrran::unique_byte_buffer_t pdu);
  void handle_srb_pdu(isrran::unique_byte_buffer_t pdu);
  void handle_um_drb_pdu(isrran::unique_byte_buffer_t pdu);
  void handle_am_drb_pdu(isrran::unique_byte_buffer_t pdu);
...
#endif // ISRRAN_PDCP_ENTITY_LTE_H
