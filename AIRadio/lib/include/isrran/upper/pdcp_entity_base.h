...
#ifndef ISRRAN_PDCP_ENTITY_BASE_H
#define ISRRAN_PDCP_ENTITY_BASE_H
...
{
public:
  pdcp_entity_base(task_sched_handle task_sched_, isrlog::basic_logger& logger);
  ...
  // RRC interface
  void enable_integrity(isrran_direction_t direction = DIRECTION_TXRX)
  {
    ...
    logger.debug("Enabled integrity. LCID=%d, integrity=%s", lcid, isrran_direction_text[integrity_direction]);
  }

  void enable_encryption(isrran_direction_t direction = DIRECTION_TXRX)
  {
    // if either DL or UL is already enabled, both are enabled
    ...
    logger.debug("Enabled encryption. LCID=%d, encryption=%s", lcid, isrran_direction_text[integrity_direction]);
  }

  void enable_security_timed(isrran_direction_t direction, uint32_t sn)
  {
    switch (direction) {
      case DIRECTION_TX:
      ...
      case DIRECTION_RX:
        ...
        break;
      default:
        logger.error("Timed security activation for direction %s not supported.", isrran_direction_text[direction]);
        break;
    }
  }
  ...
  virtual std::map<uint32_t, isrran::unique_byte_buffer_t> get_buffered_pdus() = 0;
...
protected:
  ...
  bool               active               = false;
  uint32_t           lcid                 = 0;
  isrran_direction_t integrity_direction  = DIRECTION_NONE;
  isrran_direction_t encryption_direction = DIRECTION_NONE;
  ...
  void            write_data_header(const isrran::unique_byte_buffer_t& sdu, uint32_t count);
  ...
  isrran::rolling_average<double> tx_pdu_ack_latency_ms;
};
...
#endif // ISRRAN_PDCP_ENTITY_BASE_H
