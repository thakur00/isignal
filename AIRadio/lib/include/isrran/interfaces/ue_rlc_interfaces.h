...
#ifndef ISRRAN_UE_RLC_INTERFACES_H
#define ISRRAN_UE_RLC_INTERFACES_H
...
public:
  ...
  virtual int  add_bearer(uint32_t lcid, const isrran::rlc_config_t& cnfg) = 0;
  ...
  virtual void write_sdu(uint32_t lcid, isrran::unique_byte_buffer_t sdu)  = 0;
};
...
class rlc_interface_mac : public isrran::read_pdu_interface
{
public:
  ...
  virtual void write_pdu_bcch_bch(isrran::unique_byte_buffer_t payload)           = 0;
  virtual void write_pdu_bcch_dlsch(uint8_t* payload, uint32_t nof_bytes)         = 0;
  virtual void write_pdu_pcch(isrran::unique_byte_buffer_t payload)               = 0;
  virtual void write_pdu_mch(uint32_t lcid, uint8_t* payload, uint32_t nof_bytes) = 0;
};

} // namespace isrue

#endif // ISRRAN_UE_RLC_INTERFACES_H
