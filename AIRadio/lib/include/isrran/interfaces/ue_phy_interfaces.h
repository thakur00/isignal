...
#ifndef ISRRAN_UE_PHY_INTERFACES_H
#define ISRRAN_UE_PHY_INTERFACES_H
...
public:
  ...
  virtual void set_rar_grant(uint8_t grant_payload[ISRRAN_RAR_GRANT_LEN], uint16_t rnti) = 0;
...
{
public:
  virtual bool set_config(const isrran::phy_cfg_t& config, uint32_t cc_idx = 0)           = 0;
  virtual bool set_scell(isrran_cell_t cell_info, uint32_t cc_idx, uint32_t earfcn)       = 0;
  virtual void set_config_tdd(isrran_tdd_config_t& tdd_config)                            = 0;
  virtual void set_config_mbsfn_sib2(isrran::mbsfn_sf_cfg_t* cfg_list, uint32_t nof_cfgs) = 0;
  virtual void set_config_mbsfn_sib13(const isrran::sib13_t& sib13)                       = 0;
  virtual void set_config_mbsfn_mcch(const isrran::mcch_msg_t& mcch)                      = 0;
...
#endif // ISRRAN_UE_PHY_INTERFACES_H
