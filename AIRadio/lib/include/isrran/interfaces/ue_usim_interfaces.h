...
#ifndef ISRRAN_UE_USIM_INTERFACES_H
#define ISRRAN_UE_USIM_INTERFACES_H
...
public:
  ...
  virtual bool        get_home_plmn_id(isrran::plmn_id_t* home_plmn_id) = 0;
  ...
  virtual void generate_nas_keys(uint8_t*                            k_asme,
                                 ...
                                 isrran::INTEGRITY_ALGORITHM_ID_ENUM integ_algo) = 0;
...
{
public:
  virtual void generate_as_keys(uint8_t* k_asme, uint32_t count_ul, isrran::as_security_config_t* sec_cfg)        = 0;
  virtual void generate_as_keys_ho(uint32_t pci, uint32_t earfcn, int ncc, isrran::as_security_config_t* sec_cfg) = 0;
  virtual void store_keys_before_ho(const isrran::as_security_config_t& as_cfg)                                   = 0;
  virtual void restore_keys_from_failed_ho(isrran::as_security_config_t* as_cfg)                                  = 0;
};
...
public:
  virtual void
  generate_nr_as_keys(isrran::as_key_t& k_amf, uint32_t count_ul, isrran::as_security_config_t* sec_cfg) = 0;
  virtual bool generate_nr_context(uint16_t sk_counter, isrran::as_security_config_t* sec_cfg)           = 0;
  virtual bool update_nr_context(isrran::as_security_config_t* sec_cfg)                                  = 0;
};

} // namespace isrue

#endif // ISRRAN_UE_USIM_INTERFACES_H
