...
#ifndef ISRRAN_RADIO_INTERFACES_H
#define ISRRAN_RADIO_INTERFACES_H
...
{
public:
  virtual const isrran_timestamp_t& get(uint32_t idx) const = 0;
  virtual isrran_timestamp_t*       get_ptr(uint32_t idx)   = 0;
  ...
  isrran_timestamp_t& operator[](uint32_t idx) { return *this->get_ptr(idx); }
};
 ...
 * number of carriers <= ISRRAN_MAX_CARRIERS
 * number of ports <= ISRRAN_MAX_PORTS
 *
...
public:
  ...
  virtual isrran_rf_info_t* get_info()              = 0;
};
...
#endif // ISRRAN_RADIO_INTERFACES_H
