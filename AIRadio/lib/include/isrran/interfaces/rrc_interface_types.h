...
#ifndef ISRRAN_RRC_INTERFACE_TYPES_H
#define ISRRAN_RRC_INTERFACE_TYPES_H
...
struct plmn_id_t {
  ...
  {
    isrran::mcc_to_bytes(mcc_num, &mcc[0]);
    if (not isrran::mnc_to_bytes(mnc_num, &mnc[0], &nof_mnc_digits)) {
      reset();
      return ISRRAN_ERROR;
    }
  ...
  std::pair<uint16_t, uint16_t> to_number()
  {
    ...
    isrran::bytes_to_mcc(&mcc[0], &mcc_num);
    isrran::bytes_to_mnc(&mnc[0], &mnc_num, nof_mnc_digits);
  ...
  uint32_t to_s1ap_plmn()
  {
    ...
    uint32_t s1ap_plmn;
    isrran::s1ap_mccmnc_to_plmn(mcc_mnc_pair.first, mcc_mnc_pair.second, &s1ap_plmn);
    return s1ap_plmn;
  }
...
struct phy_cfg_t {
  ...
  /**
   *  @brief Sets default PUCCH and ISR configuration (release)
   *
   *  @remark Implemented as specified by TS 36.331 V8.21.0 5.3.13 UE actions upon PUCCH/ ISR release request
   ...
   * @see phy_controller::set_phy_to_default_pucch_isr
   */
  void set_defaults_pucch_sr()
  {
    ...
    ul_cfg.isr.dedicated_enabled = false;
    ul_cfg.isr.configured        = false;
  ...
  void set_defaults_dedicated()
  {
    ...
    ul_cfg.power_ctrl.p_isr_offset     = 7;
...
#endif // ISRRAN_RRC_INTERFACE_TYPES_H
