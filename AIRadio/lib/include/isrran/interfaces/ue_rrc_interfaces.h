...
#ifndef ISRRAN_UE_RRC_INTERFACES_H
#define ISRRAN_UE_RRC_INTERFACES_H
...
public:
  ...
  virtual void release_pucch_isr() = 0;
};
...
public:
  ...
  virtual void nr_scg_failure_information(const isrran::scg_failure_cause_t cause) = 0;
};
...
public:
  ...
  virtual void        write_sdu(isrran::unique_byte_buffer_t sdu)                       = 0;
  virtual uint16_t    get_mcc()                                                         = 0;
  virtual uint16_t    get_mnc()                                                         = 0;
  virtual void        enable_capabilities()                                             = 0;
  virtual bool        plmn_search()                                                     = 0;
  virtual void        plmn_select(isrran::plmn_id_t plmn_id)                            = 0;
  virtual bool        connection_request(isrran::establishment_cause_t cause,
                                         isrran::unique_byte_buffer_t  dedicatedInfoNAS) = 0;
  virtual void        set_ue_identity(isrran::s_tmsi_t s_tmsi)                          = 0;
...
public:
  ...
  virtual void        write_pdu_bcch_bch(isrran::unique_byte_buffer_t pdu)           = 0;
  virtual void        write_pdu_bcch_dlsch(isrran::unique_byte_buffer_t pdu)         = 0;
  virtual void        write_pdu_pcch(isrran::unique_byte_buffer_t pdu)               = 0;
  virtual void        write_pdu_mch(uint32_t lcid, isrran::unique_byte_buffer_t pdu) = 0;
...
{
public:
  virtual int  get_eutra_nr_capabilities(isrran::byte_buffer_t* eutra_nr_caps)                                    = 0;
  virtual int  get_nr_capabilities(isrran::byte_buffer_t* nr_cap)                                                 = 0;
...
public:
  ...
  virtual int      write_sdu(isrran::unique_byte_buffer_t sdu) = 0;
  virtual bool     is_connected()                              = 0;
  virtual int      connection_request(isrran::nr_establishment_cause_t cause, isrran::unique_byte_buffer_t sdu) = 0;
  virtual uint16_t get_mcc()                                                                                    = 0;
  virtual uint16_t get_mnc()                                                                                    = 0;
};
} // namespace isrue

#endif // ISRRAN_UE_RRC_INTERFACES_H
