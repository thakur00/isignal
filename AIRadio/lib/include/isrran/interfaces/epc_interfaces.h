...
#ifndef ISRRAN_EPC_INTERFACES_H
#define ISRRAN_EPC_INTERFACES_H
...
public:
  ...
  virtual bool send_modify_bearer_request(uint64_t imsi, uint16_t erab_to_modify, isrran::gtp_fteid_t* enb_fteid) = 0;
  ...
  virtual bool send_downlink_data_notification_failure_indication(uint64_t                      imsi,
                                                                  enum isrran::gtpc_cause_value cause)            = 0;
};
...
public:
  ...
  virtual bool modify_gtpu_tunnel(in_addr_t ue_ipv4, isrran::gtpc_f_teid_ie dw_user_fteid, uint32_t up_ctrl_teid) = 0;
  ...
  virtual void send_all_queued_packets(isrran::gtp_fteid_t                       dw_user_fteid,
                                       std::queue<isrran::unique_byte_buffer_t>& pkt_queue)                       = 0;
};

class gtpc_interface_gtpu // GTP-U -> GTP-C
{
public:
  virtual bool queue_downlink_packet(uint32_t spgw_ctr_teid, isrran::unique_byte_buffer_t msg) = 0;
  virtual bool send_downlink_data_notification(uint32_t spgw_ctr_teid)                         = 0;
};

} // namespace isrepc
#endif // ISRRAN_EPC_INTERFACES_H
