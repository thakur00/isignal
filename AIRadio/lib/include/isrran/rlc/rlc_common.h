...
#ifndef ISRRAN_RLC_COMMON_H
#define ISRRAN_RLC_COMMON_H
...
{
public:
  explicit rlc_common(isrlog::basic_logger& logger_) : logger(logger_) {}
...
private:
  ...
  {
    unique_byte_buffer_t rx_pdu = isrran::make_byte_buffer();
    if (rx_pdu == nullptr) {
      isrlog::fetch_basic_logger("RLC").warning("Couldn't allocate PDU in %s().", __FUNCTION__);
      return;
    }

    if (rx_pdu->get_tailroom() < nof_bytes) {
      isrlog::fetch_basic_logger("RLC").warning("Not enough space to store PDU.");
      return;
    }
    ...
    if (!rx_pdu_resume_queue.try_push(std::move(rx_pdu))) {
      isrlog::fetch_basic_logger("RLC").warning("Dropping SDUs while bearer suspended.");
      return;
    }
  }
