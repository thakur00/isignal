...
#ifndef ISRRAN_RLC_AM_DATA_STRUCTS_H
#define ISRRAN_RLC_AM_DATA_STRUCTS_H

#include "isrran/adt/circular_buffer.h"
#include "isrran/adt/circular_map.h"
#include "isrran/adt/intrusive_list.h"
...
struct rlc_ringbuffer_t : public rlc_ringbuffer_base<T> {
  ...
  {
    isrran_expect(not has_sn(sn), "The same SN=%zd should not be added twice", sn);
    window.overwrite(sn, T(sn));
    return window[sn];
  }
  void remove_pdu(size_t sn) override
  {
    isrran_expect(has_sn(sn), "The removed SN=%zd is not in the window", sn);
    window.erase(sn);
  }
...
private:
  isrran::static_circular_map<uint32_t, T, WINDOW_SIZE> window;
};
...
public:
  ...
  {
    isrran_expect(sn <= max_pdcp_sn or sn == status_report_sn, "Invalid PDCP SN=%d", sn);
    isrran_assert(not has_pdcp_sn(sn), "Cannot re-add same PDCP SN twice");
  ...
  {
    isrran_expect(has_pdcp_sn(sn), "Invalid access to non-existent PDCP SN=%d", sn);
    return get_pdu_(sn);
  }

  bool has_pdcp_sn(uint32_t pdcp_sn) const
  {
    isrran_expect(pdcp_sn <= max_pdcp_sn or pdcp_sn == status_report_sn, "Invalid PDCP SN=%d", pdcp_sn);
...
public:
  ...
  {
    isrran_assert(idx < size(), "Out-of-bounds access to element idx=%zd", idx);
...
#endif // ISRRAN_RLC_AM_DATA_STRUCTS_H
