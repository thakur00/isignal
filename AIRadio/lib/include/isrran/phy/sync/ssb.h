...
typedef struct ISRRAN_API {
  ...
  isrran_ssb_pattern_t        pattern;        ///< SSB pattern as defined in TS 38.313 section 4.1 Cell search
  isrran_duplex_mode_t        duplex_mode;    ///< Set to true if the spectrum is paired (FDD)
 ...
 */
typedef struct ISRRAN_API {
  isrran_ssb_args_t args; ///< Stores initialization arguments
  isrran_ssb_cfg_t  cfg;  ///< Stores last configuration
  ...
  /// Other parameters
  uint32_t l_first[ISRRAN_SSB_NOF_CANDIDATES]; ///< Start symbol for each SSB candidate in half radio frame
  uint32_t Lmax;                               ///< Number of SSB candidates

  /// Internal Objects
  isrran_dft_plan_t ifft;      ///< IFFT object for modulating the SSB
  isrran_dft_plan_t fft;       ///< FFT object for demodulate the SSB.
  isrran_dft_plan_t fft_corr;  ///< FFT for correlation
  isrran_dft_plan_t ifft_corr; ///< IFFT for correlation
  isrran_pbch_nr_t  pbch;      ///< PBCH encoder and decoder
  ...
  cf_t* pss_seq[ISRRAN_NOF_NID_2_NR]; ///< Possible frequency domain PSS for find
} isrran_ssb_t;
...
typedef struct {
  ...
  isrran_pbch_msg_nr_t          pbch_msg;     ///< Physical broadcast channel message of the most suitable SSB candidate
  isrran_csi_trs_measurements_t measurements; ///< Measurements
} isrran_ssb_search_res_t;
 ...
 */
ISRRAN_API int isrran_ssb_init(isrran_ssb_t* q, const isrran_ssb_args_t* args);

/**
 * @brief Frees NR SSB object
 * @param q SSB object
 */
ISRRAN_API void isrran_ssb_free(isrran_ssb_t* q);
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_set_cfg(isrran_ssb_t* q, const isrran_ssb_cfg_t* cfg);
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_search(isrran_ssb_t* q, const cf_t* in, uint32_t nof_samples, isrran_ssb_search_res_t* res);
 ...
 */
ISRRAN_API bool isrran_ssb_send(isrran_ssb_t* q, uint32_t sf_idx);
 ...
 * @return ISRRAN_SUCCES if inputs and data are correct, otherwise ISRRAN_ERROR code
 */
ISRRAN_API int isrran_ssb_put_grid(isrran_ssb_t*               q,
                                   uint32_t                    N_id,
 ...
 */
ISRRAN_API int
isrran_ssb_add(isrran_ssb_t* q, uint32_t N_id, const isrran_pbch_msg_nr_t* msg, const cf_t* in, cf_t* out);
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_decode_pbch(isrran_ssb_t*         q,
                                      uint32_t              N_id,
                                      uint32_t              n_hf,
                                      uint32_t              ssb_idx,
                                      const cf_t*           grid,
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_decode_grid(isrran_ssb_t*         q,
                                      uint32_t              N_id,
                                      uint32_t              n_hf,
                                      uint32_t              ssb_idx,
                                      const cf_t*           in,
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_csi_search(isrran_ssb_t*                  q,
                                     const cf_t*                    in,
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_csi_measure(isrran_ssb_t*                  q,
                                      uint32_t                       N_id,
                                      uint32_t                       ssb_idx,
                                      const cf_t*                    in,
                                      isrran_csi_trs_measurements_t* meas);
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_find(isrran_ssb_t*                  q,
 ...
 * @return ISRRAN_SUCCESS if the parameters are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_ssb_track(isrran_ssb_t*                  q,
                                const cf_t*                    sf_buffer,
                                uint32_t                       N_id,
                                uint32_t                       ssb_idx,
                                uint32_t                       n_hf,
 ...
 */
ISRRAN_API uint32_t isrran_ssb_candidate_sf_idx(const isrran_ssb_t* q, uint32_t ssb_idx, bool half_frame);
 ...
 */
ISRRAN_API uint32_t isrran_ssb_candidate_sf_offset(const isrran_ssb_t* q, uint32_t ssb_idx);

ISRRAN_API uint32_t isrran_ssb_cfg_to_str(const isrran_ssb_cfg_t* cfg, char* str, uint32_t str_len);
