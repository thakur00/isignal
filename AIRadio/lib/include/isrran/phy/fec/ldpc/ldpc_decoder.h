...
#ifndef ISRRAN_LDPCDECODER_H
#define ISRRAN_LDPCDECODER_H
 ...
 */
typedef enum {
  ISRRAN_LDPC_DECODER_F = 0,        /*!< \brief %Decoder working with real-valued LLRs. */
  ISRRAN_LDPC_DECODER_S,            /*!< \brief %Decoder working with 16-bit integer-valued LLRs. */
  ISRRAN_LDPC_DECODER_C,            /*!< \brief %Decoder working with 8-bit integer-valued LLRs. */
  ISRRAN_LDPC_DECODER_C_FLOOD,      /*!< \brief %Decoder working with 8-bit integer-valued LLRs, flooded scheduling. */
  ISRRAN_LDPC_DECODER_C_AVX2,       /*!< \brief %Decoder working with 8-bit integer-valued LLRs (AVX2 version). */
  ISRRAN_LDPC_DECODER_C_AVX2_FLOOD, /*!< \brief %Decoder working with 8-bit integer-valued LLRs, flooded scheduling
                                   (AVX2 version). */
  ISRRAN_LDPC_DECODER_C_AVX512,     /*!< \brief %Decoder working with 8-bit integer-valued LLRs (AVX512 version). */
  ISRRAN_LDPC_DECODER_C_AVX512_FLOOD, /*!< \brief %Decoder working with 8-bit integer-valued LLRs, flooded scheduling
                                   (AVX512 version). */
} isrran_ldpc_decoder_type_t;

/*!
 * \brief Describes the LDPC decoder configuration arguments.
 */
typedef struct {
  isrran_ldpc_decoder_type_t type;         /*!< \brief Type of LDPC decoder. */
  isrran_basegraph_t         bg;           /*!< \brief The desired base graph (BG1 or BG2). */
...
typedef struct ISRRAN_API {
  ...
  int (*decode_f)(void*,
                  ...
                  uint8_t*,
                  uint32_t,
                  isrran_crc_t*); /*!< \brief Pointer to the decoding function (float version). */
  int (*decode_s)(void*,
                  const int16_t*,
                  uint8_t*,
                  uint32_t,
                  isrran_crc_t*); /*!< \brief Pointer to the decoding function (16-bit version). */
  int (*decode_c)(void*,
                  const int8_t*,
                  uint8_t*,
                  uint32_t,
                  isrran_crc_t*); /*!< \brief Pointer to the decoding function (16-bit version). */
} isrran_ldpc_decoder_t;

/*!
 * Initializes all the LDPC decoder variables according to the given base graph
 * and lifting size.
 * \param[out] q            A pointer to a isrran_ldpc_decoder_t structure.
 * \param[in]  args         LDPC configuration arguments.
 * \return An integer: 0 if the function executes correctly, -1 otherwise.
 */
ISRRAN_API int isrran_ldpc_decoder_init(isrran_ldpc_decoder_t* q, const isrran_ldpc_decoder_args_t* args);
 ...
 */
ISRRAN_API void isrran_ldpc_decoder_free(isrran_ldpc_decoder_t* q);

/*!
 * Carries out the actual decoding with real-valued LLRs.
 * \param[in] q A pointer to the LDPC decoder (a isrran_ldpc_decoder_t structure
 ...
 */
ISRRAN_API int
isrran_ldpc_decoder_decode_f(isrran_ldpc_decoder_t* q, const float* llrs, uint8_t* message, uint32_t cdwd_rm_length);
 ...
 */
ISRRAN_API int
isrran_ldpc_decoder_decode_s(isrran_ldpc_decoder_t* q, const int16_t* llrs, uint8_t* message, uint32_t cdwd_rm_length);
 ...
 */
ISRRAN_API int
isrran_ldpc_decoder_decode_c(isrran_ldpc_decoder_t* q, const int8_t* llrs, uint8_t* message, uint32_t cdwd_rm_length);
 ...
 */
ISRRAN_API int isrran_ldpc_decoder_decode_crc_c(isrran_ldpc_decoder_t* q,
                                                const int8_t*          llrs,
                                                uint8_t*               message,
                                                uint32_t               cdwd_rm_length,
                                                isrran_crc_t*          crc);

#endif // ISRRAN_LDPCDECODER_H
