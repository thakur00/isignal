...
#ifndef ISRRAN_SUB_CHANNEL_ALLOC_H
#define ISRRAN_SUB_CHANNEL_ALLOC_H
 ...
 */
typedef struct ISRRAN_API isrran_subchn_alloc_t {
...
} isrran_subchn_alloc_t;

/*!
 * Initializes a subchannel allocation instance.
 * \param[out] c  A pointer to the  isrran_subchn_alloc_t structure
 ...
 * data and CRC bits.
 */
void isrran_subchannel_allocation_init(isrran_subchn_alloc_t* c,
 ...
 * are set to 0.
 * \param[in] c A pointer to the isrran_subchn_alloc_t structure containing
 ...
 */
void isrran_subchannel_allocation(const isrran_subchn_alloc_t* c, const uint8_t* message, uint8_t* input_encoder);

/*!
 * Extracts message bits (data + CRC) from the decoder output vector
 * according to the positions specified in  \a c->message_set.
 * \param[in] c A pointer to the isrran_subchn_alloc_t structure containing the
 ...
 */
void isrran_subchannel_deallocation(const isrran_subchn_alloc_t* c, const uint8_t* output_decoder, uint8_t* message);

#endif // ISRRAN_SUB_CHANNEL_ALLOC_H
