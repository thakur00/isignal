...
#define PRINT CONCAT2(isrran_vec_fprint, print_suffix)
...
void MAKE_CALL(run_tdec_iteration)(isrran_tdec_t* h, llr_t* input)
{
  ...
  if (h->current_cbidx >= 0) {
    ...
    if (ISRRAN_TDEC_EXPECT_INPUT_SB && !h->force_not_sb && input_is_interleaved) {
      syst = input;
    ...
    if ((n_iter % 2) == 0) {
      ...
      if (n_iter) {
        MAKE_VEC(isrran_vec_sub)(app1, ext1, app1, long_cb);
      }
    ...
    if (n_iter % 2) {
      ...
      if (n_iter > 1) {
        MAKE_VEC(isrran_vec_sub)(ext1, app1, ext1, long_cb);
      }

      MAKE_VEC(isrran_vec_lut)(ext1, deinter, app2, long_cb);
      ...
      MAKE_VEC(isrran_vec_lut)(ext2, inter, app1, long_cb);
    }
    ...
    ERROR("Error CB index not set (call isrran_tdec_new_cb() first");
  }
}
