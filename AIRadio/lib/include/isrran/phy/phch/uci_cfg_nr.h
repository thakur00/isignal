...
#ifndef ISRRAN_UCI_CFG_NR_H
#define ISRRAN_UCI_CFG_NR_H
 ...
 */
#define ISRRAN_UCI_NR_MAX_NOF_BITS 1706U
 ...
 */
#define ISRRAN_UCI_NR_MAX_CSI1_BITS 360
...
typedef struct {
  ...
  uint32_t     M_pusch_sc[ISRRAN_NSYMB_PER_SLOT_NR]; ///< Number of potential RE for PUSCH transmission
  uint32_t     M_uci_sc[ISRRAN_NSYMB_PER_SLOT_NR];   ///< Number of potential RE for UCI transmission
  uint32_t     K_sum;                                ///< Sum of UL-SCH code block sizes, set to zero if no UL-SCH
  isrran_mod_t modulation;                           ///< Modulation for the PUSCH
...
typedef struct ISRRAN_API {
  /// Common Parameters
  isrran_harq_ack_cfg_t   ack;                                 ///< HARQ-ACK configuration
  uint32_t                o_sr;                                ///< Number of SR bits
  bool                    sr_positive_present;                 ///< Set to true if there is at least one positive SR
  isrran_csi_report_cfg_t csi[ISRRAN_CSI_SLOT_MAX_NOF_REPORT]; ///< CSI report configuration
  uint32_t                nof_csi;                             ///< Number of CSI reports
  union {
    isrran_uci_nr_pucch_cfg_t pucch; ///< Configuration for transmission in PUCCH
    isrran_uci_nr_pusch_cfg_t pusch; ///< Configuration for transmission in PUSCH
  };
} isrran_uci_cfg_nr_t;
 ...
 */
typedef struct ISRRAN_API {
  uint8_t                   ack[ISRRAN_HARQ_ACK_MAX_NOF_BITS];   ///< HARQ ACK feedback bits
  uint32_t                  sr;                                  ///< Number of positive SR
  isrran_csi_report_value_t csi[ISRRAN_CSI_SLOT_MAX_NOF_REPORT]; ///< Packed CSI report values
  bool valid; ///< Indicates whether the message has been decoded successfully, ignored in the transmitter
} isrran_uci_value_nr_t;

/**
 * @brief Uplink Control Information (UCI) data (configuration + values)
 */
typedef struct ISRRAN_API {
  isrran_uci_cfg_nr_t   cfg;
  isrran_uci_value_nr_t value;
} isrran_uci_data_nr_t;

#endif // ISRRAN_UCI_CFG_NR_H
