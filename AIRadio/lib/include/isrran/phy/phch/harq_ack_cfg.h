...
#ifndef ISRRAN_HARQ_ACK_CFG_H
#define ISRRAN_HARQ_ACK_CFG_H
 ...
 */
#define ISRRAN_HARQ_ACK_MAX_NOF_BITS 360
...
typedef struct {
  ...
  uint32_t n_cce;
} isrran_harq_ack_resource_t;

/**
 * @brief Describes a single PDSCH transmission HARQ ACK feedback
 */
typedef struct {
  isrran_harq_ack_resource_t resource;
...
typedef struct ISRRAN_API {
  ...
  isrran_harq_ack_codebook_t harq_ack_codebook; ///< pdsch-HARQ-ACK-Codebook configuration
  bool max_cw_sched_dci_is_2; ///< Param maxNrofCodeWordsScheduledByDCI, set to true if present and equal to 2

  uint32_t dl_data_to_ul_ack[ISRRAN_MAX_NOF_DL_DATA_TO_UL];
  uint32_t nof_dl_data_to_ul_ack;
} isrran_harq_ack_cfg_hl_t;
...
typedef struct ISRRAN_API {
  ...
  uint32_t pid;    ///< HARQ process identifier
} isrran_harq_ack_bit_t;

/**
 * @brief Describes HARQ ACK feedback configuration for UCI encoding/decoding
 */
typedef struct ISRRAN_API {
  isrran_harq_ack_bit_t bits[ISRRAN_HARQ_ACK_MAX_NOF_BITS]; ///< HARQ-ACK feedback bits
  uint32_t              count;                              ///< Number of ACK bits
} isrran_harq_ack_cfg_t;

#endif // ISRRAN_HARQ_ACK_CFG_H
