...
#ifndef ISRRAN_DCI_NR_H
#define ISRRAN_DCI_NR_H
 ...
 */
#define ISRRAN_DCI_NR_MAX_NOF_SIZES 4
...
typedef struct ISRRAN_API {
  ...
  isrran_harq_ack_codebook_t harq_ack_codebok;       ///< PDSCH HARQ-ACK codebook mode
  ...
  uint32_t nof_isr;             ///< Number of configured ISR resources
  ...
  isrran_resource_alloc_t pusch_alloc_type; ///< PUSCH resource allocation type
  isrran_dmrs_sch_type_t  pusch_dmrs_type;  ///< PUSCH DMRS type
  isrran_dmrs_sch_len_t   pusch_dmrs_max_len; ///< PUSCH DMRS maximum length
  ...
  isrran_resource_alloc_t pdsch_alloc_type;   ///< PDSCH resource allocation type, set to 0 for default
  isrran_dmrs_sch_type_t  pdsch_dmrs_type;    ///< PDSCH DMRS type
  isrran_dmrs_sch_len_t   pdsch_dmrs_max_len; ///< PDSCH DMRS maximum length
} isrran_dci_cfg_nr_t;
 ...
 */
typedef struct ISRRAN_API {
  isrran_dci_location_t      location;         ///< DCI location
  isrran_search_space_type_t ss_type;          ///< Search space type
  uint32_t                   coreset_id;       ///< CORESET identifier
  uint32_t                   coreset_start_rb; ///< CORESET lowest RB index in the resource grid
  isrran_rnti_type_t         rnti_type;        ///< RNTI type
  isrran_dci_format_nr_t     format;           ///< DCI format
 ...
 */
typedef struct ISRRAN_API {
  isrran_dci_ctx_t ctx; ///< DCI context
  uint8_t          payload[50];
  uint32_t         nof_bits;
} isrran_dci_msg_nr_t;
 ...
 */
typedef struct ISRRAN_API {
  isrran_dci_ctx_t ctx; ///< DCI search context
  ...
  uint32_t isr_request;  ///< ISR request
...
typedef struct ISRRAN_API {
  // Context information
  isrran_dci_ctx_t ctx; ///< Context information
  ...
  uint32_t isr_id;      ///< ISR resource indicator
  uint32_t ports;       ///< Antenna ports
  uint32_t isr_request; ///< ISR request
  uint32_t cbg_info;    ///< CBG transmission information (CBGTI)
 ...
 * @return ISRRAN_SUCCESS if the configuration is valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_dci_nr_set_cfg(isrran_dci_nr_t* dci, const isrran_dci_cfg_nr_t* cfg);
 ...
 */
ISRRAN_API uint32_t isrran_dci_nr_size(const isrran_dci_nr_t*     q,
                                       isrran_search_space_type_t ss_type,
                                       isrran_dci_format_nr_t     format);
 ...
 */
ISRRAN_API bool isrran_dci_nr_valid_direction(const isrran_dci_msg_nr_t* dci);
 ...
 */
ISRRAN_API int isrran_dci_nr_dl_pack(const isrran_dci_nr_t* q, const isrran_dci_dl_nr_t* dci, isrran_dci_msg_nr_t* msg);
 ...
 * @return ISRRAN_SUCCESS if provided arguments are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_dci_nr_dl_unpack(const isrran_dci_nr_t* q, isrran_dci_msg_nr_t* msg, isrran_dci_dl_nr_t* dci);
 ...
 * @return ISRRAN_SUCCESS if provided arguments are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_dci_nr_ul_pack(const isrran_dci_nr_t* q, const isrran_dci_ul_nr_t* dci, isrran_dci_msg_nr_t* msg);
 ...
 * @return ISRRAN_SUCCESS if provided arguments are valid, ISRRAN_ERROR code otherwise
 */
ISRRAN_API int isrran_dci_nr_ul_unpack(const isrran_dci_nr_t* q, isrran_dci_msg_nr_t* msg, isrran_dci_ul_nr_t* dci);
 ...
 */
ISRRAN_API uint32_t isrran_dci_ctx_to_str(const isrran_dci_ctx_t* ctx, char* str, uint32_t str_len);
 ...
 * @return The number of written characters
 */
ISRRAN_API uint32_t isrran_dci_dl_nr_to_str(const isrran_dci_nr_t*    q,
 ...
 * @return The number of written characters
 */
ISRRAN_API uint32_t isrran_dci_ul_nr_to_str(const isrran_dci_nr_t*    q,
                                            const isrran_dci_ul_nr_t* dci,
                                            char*                     str,
                                            uint32_t                  str_len);

#endif // ISRRAN_DCI_NR_H
